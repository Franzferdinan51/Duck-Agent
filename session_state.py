...
class _FieldSpec:
    ...
    def __init__(self, scope: str, name: str, default: Callable[[], Any], is_present: Callable[[Any], bool]) -> None:
        self.scope = scope
        self.name = name
...
class SessionFieldView(MutableMapping):
    ...
    def __delitem__(self, key: str) -> None:
        ...
        if state is None or not self._spec.is_present(self._value(state)):
            raise KeyError(key)
        setattr(getattr(state, self._spec.scope), self._spec.name, self._spec.default())
...
class TurnLeaseTokenView(MutableMapping):
    ...
    def __getitem__(self, key: Any) -> Any:
        ...
        if state is None or state.turn.lease_token is None or state.turn.lease_generation != generation:
            raise KeyError(key)
    ...
    def __ne__(self, other: object) -> bool:
        result = self.__eq__(other)
        return NotImplemented if result is NotImplemented else not result
LEGACY_FIELD_SPECS: Dict[str, _FieldSpec] = {'_running_agents': _FieldSpec('turn', 'agent', lambda: None, _present_not_none), '_running_agents_ts': _FieldSpec('turn', 'started_ts', float, _present_nonzero), '_active_session_leases': _FieldSpec('turn', 'lease', lambda: None, _present_not_none), '_busy_ack_ts': _FieldSpec('turn', 'busy_ack_ts', float, _present_nonzero), '_session_model_overrides': _FieldSpec('conversation', 'model_override', lambda: None, _present_not_none), '_pending_one_turn_model_restores': _FieldSpec('conversation', 'one_turn_restore', lambda: None, _present_not_none), '_session_reasoning_overrides': _FieldSpec('conversation', 'reasoning_override', lambda: None, _present_not_none), '_session_service_tier_overrides': _FieldSpec('conversation', 'service_tier_override', lambda: _UNSET_TIER, _present_not_unset), '_last_resolved_model': _FieldSpec('conversation', 'last_resolved_model', str, _present_nonzero), '_queued_events': _FieldSpec('conversation', 'queued_events', list, _present_nonzero), '_pending_turn_sidecar_notes': _FieldSpec('conversation', 'sidecar_notes', list, _present_nonzero), '_session_ephemeral_pin': _FieldSpec('conversation', 'ephemeral_pin', lambda: None, _present_not_none), '_session_vc_last': _FieldSpec('conversation', 'vc_last', lambda: None, _present_not_none), '_pending_approvals': _FieldSpec('persistent', 'approvals', lambda: None, _present_not_none), '_update_prompt_pending': _FieldSpec('persistent', 'update_prompt_pending', bool, _present_nonzero), '_pending_native_image_paths_by_session': _FieldSpec('persistent', 'native_image_paths', list, _present_nonzero), '_pending_messages': _FieldSpec('persistent', 'pending_command_text', lambda: None, _present_not_none), '_session_run_generation': _FieldSpec('persistent', 'run_generation', int, _present_nonzero)}
...
def legacy_dict_property(attr_name: str) -> property:
    ...
    return property(fget, fset, fdel, doc=f'Legacy dict view over SessionState.{spec.scope}.{spec.name} (kept for tests that access the pre-SessionState attribute).')
...
def legacy_lease_token_property() -> property:
    ...
    def fdel(self: Any) -> None:
        ...
        for key in list(view):
            del view[key]
    return property(fget, fset, fdel, doc='Legacy (session_key, generation)-keyed turn-lease token view.')