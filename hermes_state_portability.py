...
from hermes_state_common import SCHEMA_SQL, _PREVIEW_RAW_SELECT, _shape_preview, _sql_session_last_active
...
class SessionPortabilityMixin:
    ...
    def _compact_session_cols(cls) -> str:
        ...
        if cls._session_compact_cols_sql is None:
            ...
            cls._session_compact_cols_sql = ', '.join((f's.{name}' for name in declared if name not in cls._SESSION_COMPACT_EXCLUDED))
    ...
    def distinct_session_cwds(self, include_archived: bool=False) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            rows = self._conn.execute(f'SELECT cwd AS cwd, COUNT(*) AS sessions, MAX(COALESCE(ended_at, started_at, 0)) AS last_active FROM sessions WHERE {where} GROUP BY cwd').fetchall()
        return [{'cwd': r['cwd'], 'sessions': int(r['sessions'] or 0), 'last_active': float(r['last_active'] or 0)} for r in rows]
    ...
    def get_first_assistant_text(self, session_id: str) -> str:
        ...
        """
        with self._lock:
            row = self._conn.execute("SELECT content FROM messages WHERE session_id = ? AND role = 'assistant' AND content IS NOT NULL ORDER BY timestamp, id LIMIT 1", (session_id,)).fetchone()
        if not row:
            return ''
    ...
    def import_sessions(self, sessions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_messages = 0
        total_bytes = 0
        session_text_fields = ('source', 'user_id', 'model', 'system_prompt', 'end_reason', 'cwd', 'git_branch', 'git_repo_root', 'billing_provider', 'billing_base_url', 'billing_mode', 'cost_status', 'cost_source', 'pricing_version', 'title')
        message_text_fields = ('role', 'tool_call_id', 'tool_name', 'effect_disposition', 'finish_reason', 'reasoning', 'reasoning_content', 'platform_message_id', 'message_id')
        for index, raw in enumerate(sessions):
            ...
            try:
                ...
                clean_session['parent_session_id'] = self._import_text_or_none(clean_session.get('parent_session_id'), 'parent_session_id')
            ...
            normalized.append({'index': index, 'session': clean_session, 'messages': clean_messages})
        if errors:
        ...
        def _do(conn):
            ...
            def _would_create_cycle(session_id: str, parent_id: str) -> bool:
                ...
                while current:
                    ...
                    if current in parent_by_child:
                        ...
                        continue
                    row = conn.execute('SELECT parent_session_id FROM sessions WHERE id = ? LIMIT 1', (current,)).fetchone()
                    if row is None:
                        return False
            ...
            for session_id, parent_id in parent_updates:
                ...
                if parent_exists and (not _would_create_cycle(session_id, parent_id)):
                    ...
                    detached += 1
            return {'ok': True, 'imported': len(imported_ids), 'skipped': len(skipped_ids), 'detached': detached, 'imported_ids': imported_ids, 'skipped_ids': skipped_ids, 'errors': []}
