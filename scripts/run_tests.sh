#!/usr/bin/env bash
# Canonical test runner for duck-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.duck-agent/...)
#
# Usage:
...
# at ~/.duck-agent/duck-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.duck-agent/duck-agent/venv"; do
...
if [ -n "$VENV" ]; then
  ...
  # venv (no pytest) when inherited from a wrapped `duck-agent` binary rather
...
if [ -f "$HOME/.duck-agent/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.duck-agent"
