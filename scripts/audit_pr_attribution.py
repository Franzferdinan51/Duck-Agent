...
ID_NOREPLY_RE = re.compile('\\d+\\+.+@users\\.noreply\\.github\\.com$')
BARE_NOREPLY_RE = re.compile('^([A-Za-z0-9](?:[A-Za-z0-9]|-(?=[A-Za-z0-9])){0,38})@users\\.noreply\\.github\\.com$')

def run(*args: str, check: bool=True) -> str:
    result = subprocess.run(list(args), capture_output=True, text=True, encoding='utf-8', errors='replace', cwd=str(REPO_ROOT))
...
def main() -> int:
    ...
    parser.add_argument('--fix', action='store_true', help='auto-create contributors/emails/ mapping files')
    ...
    for email in unmapped:
        ...
        if resolved:
            ...
            if BARE_NOREPLY_RE.match(email):
                print(f'  ⚠ local part is user-controlled — confirm @{login} really is the contributor (git name: {author!r}) before pushing.')
        else:
