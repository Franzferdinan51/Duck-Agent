...
batch (tracker: [#77056](https://github.com/NousResearch/duck-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Duck Agent home, same model, same tasks, same reps.
...
1. Create a dedicated Duck Agent home with credentials for the models under test:
   ...
   DUCK_AGENT_HOME=$ABEVAL_HOME duck-agent plugins enable observability/nemo_relay
   ```
