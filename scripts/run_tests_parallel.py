...
def _run_one_file(file: Path, pytest_args: List[str], repo_root: Path, file_timeout: float, retries: int=0) -> Tuple[Path, int, str, dict[str, int], float]:
    ...
    """
    file, rc, output, summary, subproc_wall = _run_one_file_once(file, pytest_args, repo_root, file_timeout)
    attempt = 0
    while rc != 0 and attempt < retries:
        attempt += 1
        first_output = output
        file, rc, output, summary, subproc_wall2 = _run_one_file_once(file, pytest_args, repo_root, file_timeout)
        subproc_wall += subproc_wall2
        if rc == 0:
            output = f'⚠ FLAKY: failed on attempt 1, passed on retry (attempt {attempt + 1}). Fix the flake — do not ignore this.\n--- first-attempt output ---\n{first_output}\n--- retry output ---\n{output}'
            with _flaky_lock:
...
def _run_one_file_once(file: Path, pytest_args: List[str], repo_root: Path, file_timeout: float) -> Tuple[Path, int, str, dict[str, int], float]:
    """Single attempt of a per-file pytest subprocess (see _run_one_file)."""
    cmd = [sys.executable, '-m', 'pytest', str(file), *pytest_args]
    subproc_start = time.monotonic()
    proc = subprocess.Popen(cmd, cwd=repo_root, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', env=os.environ, start_new_session=True)
    pgid: int | None = None
    if sys.platform != 'win32':
        try:
...
def _parse_pytest_summary(output: str) -> dict[str, int]:
    ...
    for line in reversed(output.splitlines()):
        ...
        if not line:
            continue
        for m in re.finditer('(\\d+)\\s+(passed|failed|skipped|errors|xfailed|xpassed)', line):
            result[m.group(2)] = int(m.group(1))
        for m in re.finditer('(\\d+)\\s+error\\b', line):
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/duck-agent/duck-agent/tests/acp/test_auth.py``.
...
def _print_progress(tests_done: int, approx_total_tests: int, file: Path, rc: int, dur: float, repo_root: Path, tests_passed: int, tests_failed: int, test_counts: dict[Path, int], file_summary: dict[str, int] | None=None, subproc_wall: float | None=None) -> None:
    ...
    if subproc_wall is not None:
        ...
        time_str = f'{dur:.1f}s'
    msg = f'[{pct:5.1f}% | {tests_done:>5}/~{approx_total_tests} | ✓{tests_passed:>{fw}} | ✗{tests_failed:>{fw}}] {status} {_format_file(file, repo_root)} ({test_str}{time_str})'
    try:
...
def _print_inline_failure(file: Path, output: str, repo_root: Path, pytest_passthrough: List[str]) -> None:
...
def _save_durations(file_times: List[Tuple[Path, float]], repo_root: Path) -> None:
...
def _compute_lpt_slices(files: List[Path], slice_count: int, durations: dict[str, float], repo_root: Path) -> List[List[Path]]:
...
def _slice_files(files: List[Path], slice_index: int, slice_count: int, durations: dict[str, float], repo_root: Path) -> List[Path]:
    ...
    if not 1 <= slice_index <= slice_count:
        print(f'error: --slice index must be 1..{slice_count}, got {slice_index}', file=sys.stderr)
        sys.exit(2)
    ...
    total_dur = sum((durations.get(_format_file(f, repo_root), 2.0) for bucket in bucket_files for f in bucket))
    print(f'Slice {slice_index}/{slice_count}: {len(target)} files (~{target_dur:.0f}s estimated of {total_dur:.0f}s total)', flush=True)
    return target
...
def main() -> int:
    ...
    parser.add_argument('-j', '--jobs', type=int, default=int(os.environ.get('HERMES_TEST_WORKERS') or (os.cpu_count() or 4) * 2), help='Parallel worker count (default: $HERMES_TEST_WORKERS or cpu_count*2)')
    parser.add_argument('--paths', default=os.environ.get('HERMES_TEST_PATHS', ':'.join(_DEFAULT_ROOTS)), help="Colon-separated discovery roots (default: 'tests')")
    parser.add_argument('--include-integration', action='store_true', help="Don't skip integration/ e2e/ during discovery")
    parser.add_argument('--file-timeout', type=float, default=float(os.environ.get('HERMES_TEST_FILE_TIMEOUT', _DEFAULT_FILE_TIMEOUT_SECONDS)), help=f"Per-file wall-clock cap in seconds. On timeout, the pytest subprocess and its full process tree are SIGKILL'd. Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS / 60)} min), env: HERMES_TEST_FILE_TIMEOUT.")
    parser.add_argument('--file-retries', type=int, default=int(os.environ.get('HERMES_TEST_FILE_RETRIES', _DEFAULT_FILE_RETRIES)), help=f'Re-run a failing test FILE this many times in a fresh subprocess before declaring it failed. A pass-on-retry counts as passed but is reported as FLAKY in the summary. 0 disables. Default: {_DEFAULT_FILE_RETRIES}, env: HERMES_TEST_FILE_RETRIES.')
    parser.add_argument('--slice', metavar='I/N', help='Run only slice I of N (e.g. --slice 1/4). Files are distributed across slices using cached durations so each slice takes roughly equal wall time. Without a duration cache, files are distributed by count. Env: HERMES_TEST_SLICE (format: I/N).')
    parser.add_argument('--generate-slices', metavar='N', type=int, help='Discover test files, distribute them across N slices using LPT on cached durations, and print a JSON matrix to stdout then exit (no tests run). The JSON has the shape \'{"slices": [{"index": 1, "files": ["tests/foo.py", ...]}, ...]}\' so the CI generate job can feed it directly into a matrix.')
    parser.add_argument('--files', metavar='LIST', help='Explicit colon-separated list of test files to run. Bypasses discovery entirely — used by CI matrix jobs that receive their file list from the generate job.')
    parser.add_argument('paths_positional', nargs='*', metavar='PATH', help="Restrict discovery to these paths (directories or .py files). Mutually exclusive with --paths. Anything after a literal '--' separator is passed through to each per-file pytest invocation.")
    OUR_FLAGS = {'-j', '--jobs', '--paths', '--include-integration', '--file-timeout', '--file-retries', '--slice', '--generate-slices', '--files'}
    ...
    if args.paths_positional:
        ...
        if node_id_selectors:
            ...
            for raw, leaf in node_id_selectors:
                print(f"note: '{raw}' is a pytest node id; this runner is file-granular. Running the file with -k {leaf!r}.", file=sys.stderr)
            if not any((t == '-k' or t.startswith('-k=') or (t.startswith('-k') and len(t) > 2) for t in bare_passthrough + explicit_passthrough)):
    ...
    if args.generate_slices is not None:
        ...
        matrix = {'slice': [{'index': i + 1, 'files': ':'.join((_format_file(f, repo_root) for f in bucket))} for i, bucket in enumerate(slices)]}
    ...
    if roots:
        ...
        print(f'Discovered {len(files)} test files (~{approx_total_tests} tests) under {roots_str}; running with -j {args.jobs}', flush=True)
    else:
        print(f'Running {len(files)} test files (~{approx_total_tests} tests) with -j {args.jobs}', flush=True)
    ...
    def _on_done(file: Path, started_at: float, fut: 'Future[Tuple[Path, int, str, Dict[str, int], float]]') -> None:
        ...
        try:
            ...
            with lock:
                ...
                _print_progress(tests_done, approx_total_tests, file, 1, time.monotonic() - started_at, repo_root, tests_passed, tests_failed, test_counts, subproc_wall=0.0)
            return
        with lock:
            files_done += 1
            tests_done += n_tests
            ...
            tests_collected += sum((summary.get(k, 0) for k in ('passed', 'failed', 'skipped', 'errors', 'xfailed', 'xpassed')))
            ...
            _print_progress(tests_done, approx_total_tests, fpath, rc, time.monotonic() - started_at, repo_root, tests_passed, tests_failed, test_counts, file_summary=summary, subproc_wall=subproc_wall)
            if rc != 0:
    ...
    with ThreadPoolExecutor(max_workers=args.jobs) as pool:
        ...
        for file in files:
            t0 = time.monotonic()
            fut = pool.submit(_run_one_file, file, pytest_passthrough, repo_root, args.file_timeout, args.file_retries)
    ...
    if no_tests_ran_at_all:
        print()
        print(f"=== ✗ NO TESTS RAN — 0 collected across {len(files)} file{('s' if len(files) != 1 else '')}. This is NOT a pass. ===")
        print('  Common causes: the selected venv has no pytest; a -k/-m filter matched nothing; or collection errored in every file.')
    ...
    if failures:
        ...
        all_passed_but_nonzero = [(f, s) for f, _o, s in failures if s.get('failed', 0) == 0 and s.get('passed', 0) > 0]
        no_tests_ran = [(f, s) for f, _o, s in failures if s.get('failed', 0) == 0 and s.get('passed', 0) == 0]
        if test_fail_files:
