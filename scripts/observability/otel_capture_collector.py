"""Tiny local OTLP/HTTP capture collector for Duck Agent gateway health smoke tests.
...
class CaptureHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        record = {'ts': time.time(), 'path': self.path, 'content_type': self.headers.get('content-type'), 'content_length': length, 'body_prefix_hex': body[:24].hex()}
