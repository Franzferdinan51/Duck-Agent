...
def main() -> None:
    ...
    hermes_home = Path(tempfile.mkdtemp(prefix='duck-agent-otel-smoke-'))
    ...
    write_runtime_status(platform='slack', platform_state='fatal', error_code='auth_failed', error_message='Bearer *** rejected for smoke@example.com')
    ...
    print(json.dumps({'duck_agent_home': str(hermes_home), 'requests': len(rows), 'paths': sorted(paths)}, indent=2))
