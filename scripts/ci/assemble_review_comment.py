...
MARKER = '<!-- duck-agent-ci-review-bot -->'
_SEVERITY_ORDER = ['error', 'action_required', 'warning', 'info', 'debug']
_BLOCKING_SEVERITIES = ('error', 'action_required', 'warning')
_SEVERITY_GROUP_HEADER = {'error': '## ❌ Job failures', 'action_required': '## ⚠️ Action required', 'warning': '## ⚠️ Warnings', 'info': '## ℹ️ Details'}
    ...
    for entry in data:
        ...
        for r in entry.get('results', []):
            ...
            if kind not in _SEVERITY_ORDER:
                kind = 'info'
            items.append(ReviewItem(severity=kind, title=r.get('title', 'Unknown'), summary=r.get('summary', ''), detail=r.get('detail', ''), link=r.get('link', ''), link_label=r.get('link_label', 'View logs'), how_to_fix=r.get('how_to_fix', ''), source=source))
    return (items, sources)

def collect_failed_jobs(needs_json: str, run_url: str, exclude_sources: set[str] | None=None, job_urls: dict[str, str] | None=None) -> list[ReviewItem]:
    ...
    try:
        ...
        return []
    norm_sources = {src.lower().replace('-', ' ') for src in exclude_sources or set()}
    items: list[ReviewItem] = []
    for name, result in sorted(needs.items()):
        ...
        items.append(ReviewItem(severity='error', title=name, summary=f'Job **{name}** failed.', job_url=job_url))
    return items
...
def assemble(needs_json: str='', run_url: str='', job_urls: dict[str, str] | None=None, review_statuses_json: str='', pending_jobs: list[str] | None=None, commit_info: str='') -> str:
...
def main() -> int:
    ...
    parser.add_argument('--needs-json', default='', help='JSON string of {job_name: result} from the all-checks-pass job.')
    parser.add_argument('--run-url', default='', help='URL to the CI run summary page (for failed job links).')
    parser.add_argument('--review-statuses-json', default='', help='JSON array of {source, results: [...]} objects from workflow_call jobs.')
    parser.add_argument('--pending-jobs', default='', help='Comma-separated list of job names still running (shown in a dimmed footer).')
    parser.add_argument('--output', type=Path, required=True, help='Output file for the assembled comment body.')
    args = parser.parse_args()
    pending = [j.strip() for j in args.pending_jobs.split(',') if j.strip()] if args.pending_jobs else None
    body = assemble(needs_json=args.needs_json, run_url=args.run_url, review_statuses_json=args.review_statuses_json, pending_jobs=pending)
