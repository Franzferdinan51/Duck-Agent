...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``duck-agent serve`` backend, the Docker image)
...
def classify(files: list[str]) -> dict[str, bool]:
    ...
    ret = {'python': any((not _py_irrelevant(f) for f in files)), 'python_prod': any((not _py_irrelevant(f) and (not _py_test_only(f)) for f in files)), 'docker_meta': any((f.startswith(_DOCKER_META) for f in files)), 'frontend': any((f.startswith(_FRONTEND) or f in _ROOT_NPM for f in files)), 'site': any((f.startswith(_SITE) for f in files)), 'scan': any((_is_scan(f) for f in files)), 'deps': any((f == 'pyproject.toml' for f in files)), 'npm_lock': any((f.split('/')[-1] == 'package-lock.json' for f in files)), 'installer': any((_is_installer(f) for f in files)), 'mcp_catalog': any((_is_mcp_catalog(f) for f in files)), 'ci_review': any((_is_ci_review(f) for f in files))}
...
def main() -> int:
    ...
    out = '\n'.join([*(f'{key}={str(value).lower()}' for key, value in lanes.items()), f'ci_review_files={json.dumps(ci_review_files(files))}'])
