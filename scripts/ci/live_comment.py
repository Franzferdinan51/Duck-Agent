...
The comment is identified by the ``<!-- duck-agent-ci-review-bot -->`` marker
...
_INFRA_JOBS = frozenset({'detect', 'all-checks-pass', 'comment-pending', 'comment-results', 'comment-live', 'CI review comment (pending)', 'CI review comment (results)', 'CI review comment (live)', 'All required checks pass', 'Detect affected areas'})
_CONCLUSION_MAP = {'success': 'success', 'failure': 'failure', 'skipped': 'skipped', 'cancelled': 'skipped', 'neutral': 'skipped', 'timed_out': 'failure', 'action_required': 'skipped'}
...
def _api_request(url: str, token: str) -> dict:
    ...
    req = urllib.request.Request(url, headers={'Authorization': f'Bearer {token}', 'Accept': 'application/vnd.github+json', 'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'ci-live-comment'})
...
def collect_run_jobs(token: str, repo: str, run_id: str) -> list[dict]:
    ...
    orch_jobs = _api_get_paginated(f'{API_BASE}/repos/{owner}/{repo_name}/actions/runs/{run_id}/jobs', token, list_key='jobs')
    sub_runs = _api_get_paginated(f'{API_BASE}/repos/{owner}/{repo_name}/actions/runs?head_sha={head_sha}&event=workflow_call&per_page=100', token, list_key='workflow_runs')
    ...
    for sr in sub_runs:
        sr_id = sr['id']
        sr_name = sr.get('name', '')
        sr_jobs = _api_get_paginated(f'{API_BASE}/repos/{owner}/{repo_name}/actions/runs/{sr_id}/jobs', token, list_key='jobs')
        for j in sr_jobs:
...
def find_comment_id(token: str, repo: str, pr_number: str) -> int | None:
    ...
    owner, repo_name = repo.split('/')
    comments = _api_get_paginated(f'{API_BASE}/repos/{owner}/{repo_name}/issues/{pr_number}/comments', token)
    for c in comments:
        body = c.get('body', '') if isinstance(c, dict) else ''
        if body.startswith('<!-- duck-agent-ci-review-bot -->'):
            return c.get('id') if isinstance(c, dict) else None
    return None

def upsert_comment(token: str, repo: str, pr_number: str, body: str, comment_id: int | None=None) -> int | None:
    ...
    req = urllib.request.Request(url, data=data, method=method, headers={'Authorization': f'Bearer {token}', 'Accept': 'application/vnd.github+json', 'X-GitHub-Api-Version': '2022-11-28', 'Content-Type': 'application/json', 'User-Agent': 'ci-live-comment'})
    try:
        with urllib.request.urlopen(req) as resp:
...
def _list_artifacts(token: str, repo: str, run_id: str) -> list[dict]:
    ...
    owner, repo_name = repo.split('/')
    return _api_get_paginated(f'{API_BASE}/repos/{owner}/{repo_name}/actions/runs/{run_id}/artifacts', token, list_key='artifacts')
...
def _download_artifact(token: str, repo: str, artifact: dict, dest_dir: Path) -> Path | None:
    ...
    location = ''
    try:
        opener.open(urllib.request.Request(archive_download_url, headers={'Authorization': f'Bearer {token}', 'Accept': 'application/vnd.github+json', 'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'ci-live-comment'}), timeout=30)
    except urllib.error.HTTPError as e:
    ...
    try:
        with urllib.request.urlopen(urllib.request.Request(location, headers={'User-Agent': 'ci-live-comment'}), timeout=60) as resp:
...
def fetch_all_review_statuses(token: str, repo: str, run_id: str) -> list[dict]:
    ...
    try:
        ...
        return all_statuses
    rs_artifacts = [a for a in artifacts if a.get('name', '').startswith(_REVIEW_STATUS_ARTIFACT_PREFIX)]
    if not rs_artifacts:
        return all_statuses
...
def build_comment_body(asm_mod, completed: dict[str, str], pending: list[str], run_url: str, job_urls: dict[str, str], review_statuses_json: str, commit_info: str='') -> str:
    """Assemble the comment body from current job states + static inputs."""
    needs_json = json.dumps(completed) if completed else ''
    return asm_mod.assemble(needs_json=needs_json, run_url=run_url, job_urls=job_urls, review_statuses_json=review_statuses_json, pending_jobs=pending if pending else None, commit_info=commit_info)
...
def run(token: str, repo: str, run_id: str, pr_number: str, run_url: str, commit_info: str='', interval: int=15, timeout: int=1800, dry_run: bool=False) -> int:
    ...
    while True:
        ...
        print(f'  [{elapsed:.0f}s] fetched {len(jobs)} jobs from API ({infra_count} infra filtered) → {len(completed)} completed, {len(pending)} pending ({total} review jobs)')
        ...
        if artifact_count_changed:
            print(f'  Found {len(artifact_statuses)} review status entries from artifacts (was {prev_artifact_count} last poll)')
        ...
        body = build_comment_body(asm, completed, pending, run_url, job_urls, merged_json, current_commit_info)
        if body != last_body:
            change_reasons = []
            if new_completed:
...
def main() -> int:
    ...
    parser.add_argument('--timeout', type=int, default=1800, help='Max seconds to poll before giving up (default: 1800).')
    parser.add_argument('--dry-run', action='store_true', help='Print comment body instead of posting to PR.')
    ...
    return run(token=token, repo=repo, run_id=run_id, pr_number=pr_number, run_url=run_url, commit_info=commit_info, interval=args.interval, timeout=args.timeout, dry_run=args.dry_run)
