...
def _is_explicit_screenshot(path: Path) -> bool:
    ...
    return not (path.name.startswith(('test-finished-', 'test-failed-')) or path.name.endswith(('-actual.png', '-expected.png', '-diff.png')))
...
def select_evidence(results_dir: Path, base_manifest: Path | None=None) -> dict:
    ...
    screenshots = [] if base_names is None else [path for path in _files(results_dir, '*.png') if _is_explicit_screenshot(path) and path.name not in base_names]
...
def stage_evidence(results_dir: Path, evidence_dir: Path, selection: dict) -> dict:
    ...
    for screenshot in selection['screenshots']:
        manifest['screenshots'].append({'name': screenshot.name, 'file': stage('screenshot', screenshot)})
    ...
    (evidence_dir / 'e2e-evidence.json').write_text(json.dumps(manifest, sort_keys=True) + '\n', encoding='utf-8')
    return manifest
...
def build_status(selection: dict, artifact_url: str='') -> list[dict]:
    ...
    summary_parts = []
    if screenshots:
        summary_parts.append(f"{len(screenshots)} new screenshot{('s' if len(screenshots) != 1 else '')} vs main")
    if diffs:
        summary_parts.append(f"{len(diffs)} visual diff{('s' if len(diffs) != 1 else '')}")
    result: dict[str, str] = {'kind': 'info', 'title': 'Desktop E2E visual evidence', 'summary': '; '.join(summary_parts) + '.', 'detail': '\n'.join((EVIDENCE_START, '<sub>inline evidence is publishing...</sub>', EVIDENCE_END))}
    if artifact_url:
...
def main() -> int:
    ...
    args.manifest_output.write_text(json.dumps(build_manifest(args.results_dir), sort_keys=True) + '\n', encoding='utf-8')
    selection = select_evidence(args.results_dir, args.base_manifest)
    stage_evidence(args.results_dir, args.evidence_dir, selection)
    args.output.write_text(json.dumps(build_status(selection, args.artifact_url)) + '\n', encoding='utf-8')
    return 0
