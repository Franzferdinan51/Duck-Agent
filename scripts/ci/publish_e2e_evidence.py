...
EVIDENCE_START = '<!-- duck-agent-e2e-evidence:start -->'
EVIDENCE_END = '<!-- duck-agent-e2e-evidence:end -->'
...
_SAFE_FILE = re.compile('^[A-Za-z0-9][A-Za-z0-9._-]*\\.png$')
...
def _api_request(url: str, token: str, method: str='GET', payload: dict[str, Any] | None=None) -> dict[str, Any]:
    """Send one authenticated GitHub API request and return its JSON object."""
    data = json.dumps(payload).encode('utf-8') if payload is not None else None
    request = urllib.request.Request(url, data=data, method=method, headers={'Authorization': f'Bearer {token}', 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json', 'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'duck-agent-e2e-evidence-publisher'})
...
def render_evidence(files: list[EvidenceFile], attachment_urls: dict[str, str]) -> str:
    ...
    for item in files:
        ...
        blocks.extend(('<details>', f'<summary>{item.label}</summary>', '', f'![{item.label}]({url})', '', '</details>'))
...
def render_upload_failure(error: Exception) -> str:
    ...
    return '\n'.join((EVIDENCE_START, '<sub>inline evidence upload failed.</sub>', '', f'<pre>{html.escape(str(error))}</pre>', EVIDENCE_END))
...
def _find_review_comment(comments: object) -> dict[str, Any] | None:
    ...
    for item in comments:
        ...
        if body.startswith('<!-- duck-agent-ci-review-bot -->') and EVIDENCE_START in body and (EVIDENCE_END in body):
            return item
    return None
...
def _wait_for_review_comment(token: str, source_repo: str, pr_number: str) -> dict[str, Any]:
    ...
    request = urllib.request.Request(f'{API_BASE}/repos/{source_repo}/issues/{pr_number}/comments?per_page=100', headers={'Authorization': f'Bearer {token}', 'Accept': 'application/vnd.github+json', 'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'duck-agent-e2e-evidence-publisher'})
...
def upload_evidence(files: list[EvidenceFile], evidence_dir: Path, source_repo: str, session_token: str) -> dict[str, str]:
    ...
    for item in files:
        try:
            result = subprocess.run(['gh', 'image', '--repo', source_repo, str(evidence_dir / item.filename)], check=True, capture_output=True, text=True, encoding='utf-8', errors='replace', env=environment)
        except subprocess.CalledProcessError as exc:
            output = '; '.join((value.strip() for value in (exc.stdout, exc.stderr) if value and value.strip()))
...
def publish(token: str, source_repo: str, evidence_dir: Path, pr_number: str, session_token: str) -> bool:
    ...
    try:
        ...
        body = replace_evidence_marker(str(comment.get('body', '')), render_upload_failure(exc))
        _api_request(f"{API_BASE}/repos/{source_repo}/issues/comments/{comment['id']}", token, method='PATCH', payload={'body': body})
        raise
