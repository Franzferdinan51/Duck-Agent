...
from tool_search_livetest_ue import load_epic_tools, _SANITIZE
N_REPS = int(os.environ.get('TS_BENCH_REPS', '2'))
WORLD = {'/Game/Meshes/SM_Rock': 'StaticMesh', '/Game/Chars/SK_Guard': 'SkeletalMesh', '/Game/Data/CT_Damage': 'CurveTable', '/Game/Data/DT_Loot': 'DataTable', '/Game/FX/NS_Sparks': 'NiagaraSystem', 'Torch_3': 'Actor', 'Crate_2': 'Actor'}
...
def make_mock(sanitized_name: str):
    ...
    def _h(*a, **kw):
        ...
        if 'find_actors' in n or 'getvisibleactors' in n or 'get_outliner' in n:
            blob = json.dumps(kw)
            actors = [{'label': 'Torch_3', 'path': '/Game/Map:PersistentLevel.Torch_3', 'class': 'Actor', 'components': ["NiagaraComponent 'FX_Flame'"]}, {'label': 'Crate_2', 'path': '/Game/Map:PersistentLevel.Crate_2', 'class': 'StaticMeshActor'}]
            if 'Torch' in blob:
                actors = actors[:1]
            elif 'Crate' in blob:
                actors = actors[1:]
        ...
        if 'get_components' in n:
            blob = json.dumps(kw)
            if 'Torch' in blob:
                return json.dumps({'result': [{'name': 'FX_Flame', 'class': 'NiagaraComponent'}, {'name': 'PointLight0', 'class': 'PointLightComponent'}]})
...
def run_one(scenario, mode, rep, out_dir: Path):
    ...
    ue_calls = [c for c in call_log if c['name'].lower() not in ('tool_search', 'tool_describe', 'tool_call') and (not c['name'].startswith(('skills_list', 'skill_', 'todo', 'memory')))]
    ...
    rec = {'scenario_id': scenario['id'], 'mode': mode, 'rep': rep, 'n_tools_registered': n_registered, 'elapsed_seconds': round(elapsed, 2), 'api_calls': len(usage_log), 'prompt_tokens_total': sum((u['prompt_tokens'] or 0 for u in usage_log)), 'first_tool': first_ue.split('_')[-2:] if first_ue else None, 'first_correct': first_correct, 'final_correct': final_correct, 'wrong_calls': wrong_calls, 'success': bool(success), 'ue_calls': [c['name'][-70:] for c in ue_calls][:20], 'bridge_calls': [(b['name'], (b.get('args') or {}).get('query') or (b.get('args') or {}).get('name')) for b in bridge_call_log][:20], 'error': error, 'final_response': base._redact_secrets(final_response)[:300]}
    ...
    for scenario in SCENARIOS:
        for mode in modes:
            for rep in range(1, N_REPS + 1):
                ...
                print(f"{scenario['id']:22} {mode:8} rep{rep}: 1st={('Y' if rec['first_correct'] else 'n')} final={('Y' if rec['final_correct'] else 'n')} wrong={rec['wrong_calls']} ok={rec['success']} api={rec['api_calls']} in={rec['prompt_tokens_total']:>9,} t={rec['elapsed_seconds']:>5}s", flush=True)
                rows.append(rec)
