"""Drive the Duck Agent TUI under HERMES_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.duck-agent/perf.log on exit.
...
Environment overrides:
  HERMES_PERF_LOG     (default ~/.duck-agent/perf.log)
...
try:
    ...
    def get_hermes_home() -> Path:
        val = (os.environ.get('DUCK_AGENT_HOME') or '').strip()
        return Path(val) if val else Path.home() / '.duck-agent'
DEFAULT_TUI_DIR = Path(os.environ.get('HERMES_TUI_DIR') or str(Path(__file__).resolve().parent.parent / 'ui-tui'))
DEFAULT_LOG = Path(os.environ.get('HERMES_PERF_LOG', str(get_hermes_home() / 'perf.log')))
DEFAULT_STATE_DB = get_hermes_home() / 'state.db'
KEYS = {'page_up': b'\x1b[5~', 'page_down': b'\x1b[6~', 'wheel_up': b'\x1b[M`!!', 'shift_up': b'\x1b[1;2A', 'shift_down': b'\x1b[1;2B'}

def pick_longest_session(db: Path) -> str:
    conn = sqlite3.connect(db)
    row = conn.execute('SELECT id FROM sessions s ORDER BY (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) DESC LIMIT 1').fetchone()
    if not row:
...
def summarize(log: Path, since_ts_ms: int) -> dict[str, Any]:
    ...
    return {'react': react_events, 'frame': frame_events}
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        ...
        for pid, ms in sorted(by_id.items(), key=lambda kv: -pct(kv[1], 0.99)):
            out.append(f'  {pid:<14} {len(ms):>6} {pct(ms, 0.5):>8.2f} {pct(ms, 0.95):>8.2f} {pct(ms, 0.99):>8.2f} {max(ms):>8.2f}')
    out.append('')
    ...
    if not frames:
        ...
        out.append(f'  durationMs  p50={pct(dur, 0.5):.2f}  p95={pct(dur, 0.95):.2f}  p99={pct(dur, 0.99):.2f}  max={max(dur):.2f}')
        ...
        if phases_present:
            ...
            for field in fields:
                ...
                if vals:
                    out.append(f'  {field:<10} {pct(vals, 0.5):>8.2f} {pct(vals, 0.95):>8.2f} {pct(vals, 0.99):>8.2f} {max(vals):>8.2f}')
            sum_ps = [sum((f['phases'][k] for k in fields)) for f in frames if f.get('phases')]
            if sum_ps:
                dur_match = [f['durationMs'] for f in frames if f.get('phases')]
                deltas = [d - s for d, s in zip(dur_match, sum_ps)]
                out.append(f"  {'dur-Σphases':<10} {pct(deltas, 0.5):>8.2f} {pct(deltas, 0.95):>8.2f} {pct(deltas, 0.99):>8.2f} {max(deltas):>8.2f}   (unaccounted-for time)")
            ...
            for name, vals in (('visited', visited), ('measured', measured), ('cacheHits', cache_hits), ('live', live)):
                if vals:
            ...
            if patches:
                out.append(f'  patches     p50={pct(patches, 0.5):.0f}  p99={pct(patches, 0.99):.0f}  max={max(patches)}  total={sum(patches)}')
            optimized = [f['phases'].get('optimizedPatches', 0) for f in frames if f.get('phases')]
            if any(optimized):
                out.append(f'  optimized   p50={pct(optimized, 0.5):.0f}  p99={pct(optimized, 0.99):.0f}  max={max(optimized)}  total={sum(optimized)}  (ratio: {sum(optimized) / max(1, sum(patches)):.2f})')
            bytes_written = [f['phases'].get('writeBytes', 0) for f in frames if f.get('phases')]
            if any(bytes_written):
                total_b = sum(bytes_written)
                kb = total_b / 1024
                out.append(f'  writeBytes  p50={pct(bytes_written, 0.5):.0f}B  p99={pct(bytes_written, 0.99):.0f}B  max={max(bytes_written)}B  total={kb:.1f}KB')
            drains = [f['phases'].get('prevFrameDrainMs', 0) for f in frames if f.get('phases')]
            if any((d > 0 for d in drains)):
                nonzero = [d for d in drains if d > 0]
                out.append(f'  drainMs     p50={pct(nonzero, 0.5):.2f}  p95={pct(nonzero, 0.95):.2f}  p99={pct(nonzero, 0.99):.2f}  max={max(nonzero):.2f}   (terminal flush latency)')
            backpressure = sum((1 for f in frames if f.get('phases', {}).get('backpressure')))
            if backpressure:
                out.append(f'  backpressure: {backpressure}/{len(frames)} frames ({100 * backpressure / len(frames):.0f}%)   (Node stdout buffer full — terminal slow)')
...
def key_metrics(data: dict[str, Any]) -> dict[str, float]:
    ...
    if frames:
        ...
        if len(ts) >= 2:
            ...
            if gaps:
                metrics['gap_p50_ms'] = pct(gaps, 0.5)
        ...
        bytes_list = [f['phases'].get('writeBytes', 0) for f in frames if f.get('phases')]
        if any(bytes_list):
...
def format_diff(before: dict[str, float], after: dict[str, float]) -> str:
    ...
    for k in keys:
        ...
        lower_is_better = any((token in k for token in ('p50', 'p95', 'p99', '_max', '_total', 'gaps_over', 'backpressure', 'drain')))
...
def main() -> int:
    ...
    p.add_argument('--save', metavar='LABEL', help='save the final metrics as /tmp/perf-<LABEL>.json for later --compare')
    p.add_argument('--compare', metavar='LABEL', help='diff against /tmp/perf-<LABEL>.json after running')
    p.add_argument('--loop', action='store_true', help='watch for source changes, rebuild, rerun, and diff vs previous run')
    p.add_argument('--extra-flag', dest='extra_flags', action='append', default=[], help='pass through to node dist/entry.js (repeatable)')
    args = p.parse_args()
    if args.loop:
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / 'packages' / 'duck-agent-ink' / 'src'
    ...
    try:
        while True:
            ...
            if iteration > 1:
                print('• rebuilding…')
                result = subprocess.run(['npm', 'run', 'build'], cwd=tui_dir, capture_output=True, text=True, encoding='utf-8', errors='replace')
