...
def _normalize_ruff(entries: list[dict]) -> list[dict]:
    ...
    for e in entries:
        ...
        out.append({'tool': 'ruff', 'rule': code, 'path': filename, 'line': line, 'message': e.get('message', '')})
    return out
...
def _normalize_ty(entries: list[dict]) -> list[dict]:
    ...
    for e in entries:
        ...
        out.append({'tool': 'ty', 'rule': e.get('check_name', 'unknown'), 'path': loc.get('path', ''), 'line': begin.get('line', 0), 'message': e.get('description', '')})
    return out
...
def _diff(base: list[dict], head: list[dict]) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    return ([head_map[k] for k in new_keys], [base_map[k] for k in fixed_keys], [head_map[k] for k in unchanged_keys])
...
def _tool_report(tool_name: str, base: list[dict], head: list[dict], base_available: bool) -> str:
    ...
    if not base_available:
        lines.append('_Base report unavailable (likely main has no config for this tool yet); treating all head diagnostics as new._\n')
    lines.append(f'**Total:** {len(head)} on HEAD, {len(base)} on base ({emoji} {delta_str})\n')
...
def main() -> int:
    ...
    ap.add_argument('--output', type=Path, help='Write summary to this file instead of stdout')
    args = ap.parse_args()
    ...
    buf.append('_Diagnostics are surfaced as warnings — this check never fails the build._\n')
    summary = '\n'.join(buf)
    if args.output:
