"""Summarize micro-compaction telemetry from Duck Agent logs.
...
With no LOGFILE, reads ``$DUCK_AGENT_HOME/logs/agent.log`` (default ~/.duck-agent).
...
def default_log() -> Path:
    home = os.environ.get('DUCK_AGENT_HOME') or str(Path.home() / '.duck-agent')
...
def report(micro: list[dict], batch: list[dict], per_session: bool) -> int:
    ...
    if per_session:
        ...
        for sid, evs in sorted(by_session.items(), key=lambda kv: -len(kv[1])):
            ...
            print(f'{sid[:26]:<26} {len(evs):>6} {occ_s:>18} {batch_by_session.get(sid, 0):>6} {s:>+10,}')
        print()
    print('-- headroom ----------------------------------')
    ...
    if durations:
        ...
        print(f'pass duration           median {ordered[len(ordered) // 2]:,} ms  max {ordered[-1]:,} ms')
    print()
    print('-- tokens (least interesting) ----------------')
    print(f'net tokens saved        {saved:+,}')
    if absorbed:
        sizes = [e.get('exchange_tokens') or 0 for e in absorbed]
        print(f'exchanges absorbed      {len(absorbed):,}  (mean {sum(sizes) // len(absorbed):,} tokens each)')
