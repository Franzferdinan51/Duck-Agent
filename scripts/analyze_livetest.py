...
def main():
    ...
    for sid in scenarios:
        ...
        for label, rec in [('ENABLED ', en), ('DISABLED', di)]:
            ...
            if mark == '✗':
                fails += 1
            print(f"│  {label} {mark}  bridges={len(rec['bridge_calls']):2}  underlying={len(rec['underlying_tool_calls']):2}  iters={rec['n_iterations']:2}  elapsed={rec['elapsed_seconds']:5.1f}s  err={bool(rec['error'])}")
