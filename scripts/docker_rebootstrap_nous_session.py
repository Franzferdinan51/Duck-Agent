...
Background
----------
A Nous bootstrap session (client_id ``duck-agent-cli-vps``) can take a terminal
...
BOOTSTRAP_CLIENT_ID = 'duck-agent-cli-vps'
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    ...
    if nous.get('client_id') != BOOTSTRAP_CLIENT_ID:
        return None
    if not (isinstance(nous.get('access_token'), str) and nous['access_token'].strip() and isinstance(nous.get('refresh_token'), str) and nous['refresh_token'].strip()):
        return None
    return nous
...
def _seed_is_newer(local_nous: Any, seed_nous: dict) -> bool:
    ...
    return bool(local_obtained is not None and seed_obtained is not None and (seed_obtained > local_obtained))
...
def main() -> int:
    ...
    try:
        ...
        return 0
    if result == 'reseeded':
        print(f'[rebootstrap] Nous bootstrap session was terminal; re-seeded auth.json from {REBOOTSTRAP_ENV}')
    elif result == 'reseeded_newer':
        print(f'[rebootstrap] Applied newer orchestrator-issued Nous bootstrap session from {REBOOTSTRAP_ENV}')
    else:
