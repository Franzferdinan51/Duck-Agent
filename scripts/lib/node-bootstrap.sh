...
#   2. ~/.duck-agent/node/ from a prior Duck Agent-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.duck-agent/node/ (always works, zero shell rc edits)
#
# Usage:
...
# Redirect a Duck Agent-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $DUCK_AGENT_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    [ -x "$DUCK_AGENT_HOME/node/bin/npm" ] || return 0
    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$DUCK_AGENT_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$DUCK_AGENT_HOME/node/etc/npmrc"
}
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$DUCK_AGENT_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$DUCK_AGENT_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $DUCK_AGENT_HOME/node/..."
    ...
    rm -rf "$DUCK_AGENT_HOME/node"
    mv "$extracted" "$DUCK_AGENT_HOME/node"
    rm -rf "$tmp"
    ...
    if [ "${HERMES_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$DUCK_AGENT_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$DUCK_AGENT_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$DUCK_AGENT_HOME/node/bin/npx"  "$_link_dir/npx"
    fi

    _nb_configure_npm_prefix

    export PATH="$DUCK_AGENT_HOME/node/bin:$PATH"

    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $DUCK_AGENT_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken Duck Agent-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$DUCK_AGENT_HOME/node/bin/$tool" \
        "$DUCK_AGENT_HOME/node/${tool}.exe" \
        "$DUCK_AGENT_HOME/node/$tool"; do
...
_nb_managed_node_outdated() {
    ...
    for probe in "$DUCK_AGENT_HOME/node/bin/node" "$DUCK_AGENT_HOME/node/node"; do
        [ -x "$probe" ] || continue
...
heal_managed_node() {
    [ -d "$DUCK_AGENT_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Duck Agent-managed Node is broken — redownloading to $DUCK_AGENT_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if [ -x "$DUCK_AGENT_HOME/node/bin/node" ]; then
        export PATH="$DUCK_AGENT_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Duck Agent-managed)"
            HERMES_NODE_AVAILABLE=true
