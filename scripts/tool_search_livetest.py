"""Live test harness for Duck Agent's Tool Search feature.
...
def setup_isolated_home(enabled: bool, listing: str='off', listing_max_tokens: int=4000, model: str='anthropic/claude-haiku-4.5') -> Path:
    ...
    hermes_home = home_dir / '.duck-agent'
    hermes_home.mkdir(parents=True)
    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, hermes_home / 'auth.json')
    real_env_file = Path.home() / '.duck-agent' / '.env'
    if real_env_file.exists():
        shutil.copy(real_env_file, hermes_home / '.env')
        from hermes_cli.env_loader import load_hermes_dotenv
        load_hermes_dotenv(hermes_home=str(Path.home() / '.duck-agent'))
    cfg = {'model': {'provider': 'openrouter', 'model': model}, 'tools': {'tool_search': {'enabled': 'on' if enabled else 'off', 'threshold_pct': 10, 'search_default_limit': 5, 'max_search_limit': 20, 'listing': listing, 'listing_max_tokens': listing_max_tokens}}, 'logging': {'level': 'WARNING'}}
...
def register_fake_tools() -> int:
    ...
    for tdef in FAKE_MCP_TOOLS:
        ...
        registry.register(name=tdef['name'], toolset='mcp-fake', schema={'name': tdef['name'], 'description': tdef['description'], 'parameters': {'type': 'object', 'properties': properties, 'required': required}}, handler=make_handler(tdef))
        count += 1
    return count

def reset_module_state():
    """Drop cached modules so the new DUCK_AGENT_HOME takes effect."""
    keys = [k for k in sys.modules.keys() if k.startswith(('tools.', 'model_tools', 'toolsets', 'hermes_cli', 'agent.', 'run_agent'))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ['DUCK_AGENT_HOME'] = str(home)
    ...
    try:
        ...
        agent = AIAgent(provider='openrouter', model='anthropic/claude-haiku-4.5', enabled_toolsets=None, quiet_mode=True, save_trajectories=False, skip_context_files=True, skip_memory=True, platform='cli', max_iterations=15)
        result = agent.run_conversation(user_message=scenario['prompt'], system_message="You are a test agent. Complete the user's task using available tools. Be concise; don't add commentary beyond what's needed.")
    ...
    record = {'scenario_id': scenario['id'], 'scenario_description': scenario['description'], 'tool_search_enabled': enabled, 'model': 'anthropic/claude-haiku-4.5 (via openrouter)', 'prompt': scenario['prompt'], 'expected_underlying_tools': scenario.get('expected_underlying_tools', []), 'n_fake_tools_registered': n_registered, 'elapsed_seconds': round(elapsed, 2), 'bridge_calls': bridge_call_log, 'underlying_tool_calls': tool_call_log, 'final_response': _redact_secrets(final_response), 'n_iterations': _count_assistant_turns(messages_out), 'error': _redact_secrets(error) if error else error}
...
def _redact_secrets(text: str) -> str:
    ...
    out = re.sub('sk-[A-Za-z0-9_\\-]{12,}', '[REDACTED]', out)
    out = re.sub('(?i)(authorization|bearer)\\s*[:=]\\s*\\S+', '\\1: [REDACTED]', out)
    return out
...
def main():
    ...
    for scenario in SCENARIOS:
        for enabled in (True, False):
            ...
            err = record['error']
            print(f"  bridge calls: {n_bridge}, underlying tool calls: {n_under}, elapsed: {record['elapsed_seconds']}s, error: {bool(err)}")
            if err:
                print(f'  ERROR: {err[:300]}')
            summary.append({'scenario': scenario['id'], 'enabled': enabled, 'n_bridge': n_bridge, 'n_underlying': n_under, 'elapsed': record['elapsed_seconds'], 'error': bool(err), 'underlying_tools_called': [c['name'] for c in record['underlying_tool_calls']], 'expected': scenario.get('expected_underlying_tools', [])})
    summary_path = out_dir / '_summary.json'
    summary_path.write_text(json.dumps(summary, indent=2), encoding='utf-8')
    print(f'\nSummary saved to: {summary_path}')
    if ORIGINAL_HOME is not None:
        os.environ['DUCK_AGENT_HOME'] = ORIGINAL_HOME
    else:
        os.environ.pop('DUCK_AGENT_HOME', None)
