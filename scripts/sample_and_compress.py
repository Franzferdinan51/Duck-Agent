...
load_dotenv()
DEFAULT_DATASETS = ['NousResearch/swe-terminus-agent-glm-kimi-minimax', 'NousResearch/duck-agent-megascience-sft1', 'NousResearch/Duck Agent-Agent-Thinking-GLM-4.7-SFT2', 'NousResearch/Duck Agent-Agent-Thinking-GLM-4.7-SFT1', 'NousResearch/terminal-tasks-glm-duck-agent']
...
def sample_from_datasets(datasets: List[str], total_samples: int, min_tokens: int=16000, tokenizer_name: str='moonshotai/Kimi-K2-Thinking', seed: int=42, num_proc: int=8) -> List[Dict[str, Any]]:
    """
    ...
    filtered_entries = []
    token_counts = []
    with Pool(processes=num_proc, initializer=_init_tokenizer_worker, initargs=(tokenizer_name,)) as pool:
        chunk_size = 1000
        processed = 0
...
def save_samples_for_compression(samples: List[Dict[str, Any]], output_dir: Path, batch_size: int=100):
    """
...
def main(total_samples: int=2500, output_name: str='compressed_agentic', datasets: str=None, config: str='configs/trajectory_compression.yaml', seed: int=42, batch_size: int=100, min_tokens: int=16000, num_proc: int=8, skip_download: bool=False):
    """
    ...
    if not skip_download:
        samples = sample_from_datasets(dataset_list, total_samples, min_tokens=min_tokens, seed=seed, num_proc=num_proc)
        if not samples:
