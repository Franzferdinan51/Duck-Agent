...
probe date 2026-07-02) into the Duck Agent tool registry with mock handlers,
...
EDITOR_TOOLSETS = ('EditorToolset.EditorAppToolset', 'editor_toolset.toolsets.scene.SceneTools', 'editor_toolset.toolsets.primitive.PrimitiveTools', 'editor_toolset.toolsets.actor.ActorTools')
_SANITIZE = re.compile('[^A-Za-z0-9_]')
...
def _mock_result(tool_name: str) -> str:
    ...
    if any((v in tool_name.lower() for v in ('get', 'list', 'find', 'search', 'query', 'is_', 'can_', 'checked'))):
        return json.dumps({'result': [{'name': 'Cube_1', 'path': '/Game/Level:PersistentLevel.Cube_1', 'class': 'StaticMeshActor', 'location': [0, 0, 100]}]})
...
def load_epic_tools(scale: str) -> List[Dict[str, Any]]:
    ...
    for ts_name, ts in raw['toolsets'].items():
        ...
        for t in ts['tools']:
            ...
            if not name:
                continue
            out.append({'name': name, 'description': t.get('description', '') or '', 'parameters': t.get('inputSchema') or {'type': 'object', 'properties': {}}})
    return out
...
def run_one(scenario, mode, scale, rep, out_dir: Path):
    ...
    hermes_home = base.setup_isolated_home(enabled, listing='auto' if mode == 'listing' else 'off', listing_max_tokens=lmax, model=model)
    ...
    try:
        ...
        def _norm_spy(raw, **kw):
            ...
            try:
                usage_log.append({'prompt_tokens': cu.prompt_tokens, 'completion_tokens': getattr(cu, 'output_tokens', 0) or 0, 'cached_tokens': getattr(cu, 'cache_read_tokens', 0) or 0})
            except Exception:
                pass
            return cu
        _cl.normalize_usage = _norm_spy
        result = agent.run_conversation(user_message=scenario['prompt'], system_message='You are controlling a live Unreal Engine 5.8 editor. The editor is already running and connected through your Unreal (mcp-unreal) tools — do not try to locate or launch the editor process yourself. Complete the task with the available tools. Be concise.')
    ...
    if not exp1:
        success = error is None and ('CANNOT' in (final_response or '').upper() or "can't" in (final_response or '').lower() or 'cannot' in (final_response or '').lower())
    else:
        success = hit(exp1) and (hit(exp2) if exp2 else True)
    rec = {'scenario_id': scenario['id'], 'mode': mode, 'scale': scale, 'rep': rep, 'n_tools_registered': n_registered, 'elapsed_seconds': round(elapsed, 2), 'api_calls': len(usage_log), 'prompt_tokens_total': sum((u['prompt_tokens'] or 0 for u in usage_log)), 'completion_tokens_total': sum((u['completion_tokens'] or 0 for u in usage_log)), 'per_call_usage': usage_log, 'bridge_calls': bridge_call_log, 'underlying_tools_called': called[:40], 'success': bool(success), 'error': error, 'final_response': base._redact_secrets(final_response)[:400]}
    ...
    for scenario in SCENARIOS:
        ...
        for mode in modes:
            for rep in range(1, N_REPS + 1):
                ...
                print(f"{scenario['id']:18} {mode:8} {scale:6} rep{rep}: api={rec['api_calls']} in={rec['prompt_tokens_total']:>8,} t={rec['elapsed_seconds']:>6}s ok={rec['success']} err={bool(rec['error'])}", flush=True)
                rows.append(rec)
    name = os.environ.get('TS_UE_SUMMARY', f'_ue_bench_{scale}.json')
    (out_dir / name).write_text(json.dumps([{k: v for k, v in r.items() if k not in ('per_call_usage', 'bridge_calls', 'final_response')} for r in rows], indent=1), encoding='utf-8')
