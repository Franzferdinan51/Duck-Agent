...
IGNORED_PATTERNS = [re.compile('^Claude', re.IGNORECASE), re.compile('^Copilot$', re.IGNORECASE), re.compile('^Cursor(\\s+Agent)?$', re.IGNORECASE), re.compile('^Codex$', re.IGNORECASE), re.compile('^OpenAI Codex$', re.IGNORECASE), re.compile('^CommandCode', re.IGNORECASE), re.compile('^github-advanced-security(\\[bot\\])?$', re.IGNORECASE), re.compile('^GitHub\\s*Actions?$', re.IGNORECASE), re.compile('^github-actions(\\[bot\\])?$', re.IGNORECASE), re.compile('^dependabot', re.IGNORECASE), re.compile('^renovate', re.IGNORECASE), re.compile('^Duck Agent\\s+(Agent|Audit)$', re.IGNORECASE), re.compile('^nousbot(-eng)?$', re.IGNORECASE), re.compile('^Ubuntu$', re.IGNORECASE), re.compile('^Blut-?Agent$', re.IGNORECASE), re.compile('.*\\[bot\\]$', re.IGNORECASE), re.compile('^TRON$', re.IGNORECASE), re.compile('^Happy$', re.IGNORECASE), re.compile('^Orca$', re.IGNORECASE)]
IGNORED_EMAILS = {'noreply@anthropic.com', 'noreply@github.com', 'noreply@nousresearch.com', 'cursoragent@cursor.com', 'duck-agent@nousresearch.com', 'duck-agent-audit@example.com', 'nousbot@nousresearch.com', 'duck-agent@habibilabs.dev', 'omx@oh-my-codex.dev', 'codex@openai.com', 'noreply@commandcode.ai', 'tron-agent@agentmail.to', 'yesreply@happy.engineering', 'help@stably.ai'}
...
def git(*args, cwd=None):
    ...
    result = subprocess.run(['git'] + list(args), capture_output=True, text=True, encoding='utf-8', errors='replace', cwd=cwd or str(REPO_ROOT))
...
def gh_pr_list():
    ...
    """
    try:
        result = subprocess.run(['gh', 'pr', 'list', '--repo', 'NousResearch/duck-agent', '--state', 'merged', '--json', 'number,title,body,author,mergedAt', '--limit', '300'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=60)
        if result.returncode != 0:
        ...
        return []
SALVAGE_PATTERNS = [re.compile('[Ss]alvaged\\s+from\\s+@(\\w[\\w-]*)'), re.compile('[Ss]alvaged\\s+from\\s+#(\\d+)'), re.compile('[Cc]herry[- ]?picked\\s+from\\s+@(\\w[\\w-]*)'), re.compile('[Bb]ased\\s+on\\s+work\\s+by\\s+@(\\w[\\w-]*)'), re.compile('[Oo]riginal\\s+PR\\s+by\\s+@(\\w[\\w-]*)'), re.compile('[Cc]o[- ]?authored\\s+with\\s+@(\\w[\\w-]*)')]
CO_AUTHORED_RE = re.compile('Co-authored-by:\\s*(.+?)\\s*<([^>]+)>', re.IGNORECASE)
...
def collect_commit_authors(since_tag, until='HEAD'):
    ...
    log = git('log', range_spec, '--format=%H|%an|%ae|%s', '--no-merges')
...
def collect_co_authors(since_tag, until='HEAD'):
    ...
    """
    range_spec = f'{since_tag}..{until}'
    log = git('log', range_spec, '--format=__COMMIT__%H%n%b', '--no-merges')
    contributors = defaultdict(set)
    unknown_emails = {}
    if not log:
        return (contributors, unknown_emails)
...
def main():
    parser = argparse.ArgumentParser(description='Audit contributors across git history, co-author trailers, and salvaged PRs.')
    parser.add_argument('--since-tag', required=True, help='Git tag to start from (e.g., v2026.4.8)')
    parser.add_argument('--until', default='HEAD', help='Git ref to end at (default: HEAD)')
    parser.add_argument('--release-file', default=None, help='Path to a release notes file to check for missing contributors')
    parser.add_argument('--strict', action='store_true', help='Exit with code 1 if new unmapped emails are found (for CI)')
    parser.add_argument('--diff-base', default=None, help='Git ref to diff against (only flag emails from commits after this ref)')
    ...
    if args.strict and all_unknowns:
        new_unknowns = {}
        if args.diff_base:
            new_commits_output = git('log', f'{args.diff_base}..HEAD', '--format=%ae', '--no-merges')
