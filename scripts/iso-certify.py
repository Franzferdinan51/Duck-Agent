...
What it does
------------
1. Spawns a SCRATCH dashboard (``duck-agent dashboard``) bound to loopback on a
   free port, with an ISOLATED ``DUCK_AGENT_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.duck-agent.dashboard / live
...
the scratch DUCK_AGENT_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile('HERMES_(?:DASHBOARD|BACKEND)_READY port=(\\d+)')
_STALL_LOG_RE = re.compile('event loop stalled|ws write slow \\(loop stalled')
...
def summarize(values: list[float]) -> dict[str, float]:
    return {'count': len(values), 'p50_ms': percentile(values, 50), 'p95_ms': percentile(values, 95), 'p99_ms': percentile(values, 99), 'max_ms': max(values) if values else 0.0}
...
def seed_scratch_home(home: Path, *, isolation: str, heartbeat_secs: int, respawn_max: int) -> None:
    ...
    cfg = {'provider': 'synthetic', 'model': 'synthetic-heavy', 'dashboard': {'turn_isolation': isolation == 'on', 'compute_host_heartbeat_secs': heartbeat_secs, 'compute_host_respawn_max': respawn_max}, 'memory': {'enabled': False}}
    import yaml
...
class ScratchDashboard:
    ...
    def __init__(self, *, home: Path, port: int, isolation: str, env_extra: dict[str, str]) -> None:
        self.home = home
        self.port = port
    ...
    def __enter__(self) -> 'ScratchDashboard':
        ...
        env['DUCK_AGENT_HOME'] = str(self.home)
        env['HOME'] = str(self.home.parent) if str(self.home.parent) else env.get('HOME', '')
        env['DUCK_AGENT_HOME'] = str(self.home)
        env['PYTHONPATH'] = str(REPO_ROOT) + os.pathsep + env.get('PYTHONPATH', '')
        env['HERMES_ISO_CERTIFY_SYNTH_TURN'] = '1'
        cmd = [python, '-m', 'hermes_cli.main', 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', str(self.port)]
        self.proc = subprocess.Popen(cmd, cwd=str(REPO_ROOT), env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', bufsize=1)
...
class WSClient:
    ...
    def __init__(self, port: int, token: str) -> None:
        ...
        self.ws = ws_connect(self.url, open_timeout=15, max_size=None, additional_headers={'Origin': f'http://127.0.0.1:{port}'})
        self._id = 0
...
def drive_heavy_turn(port: int, token: str, turn_spec: dict, stop_at: float, results: list[dict]) -> None:
    ...
    try:
        ...
        while time.monotonic() < stop_at:
            ...
            while time.monotonic() < deadline:
                try:
                    o = cli._recv_until(lambda o: o.get('method') == 'event' and (o.get('params') or {}).get('type') in {'message.start', 'message.delta', 'message.complete', 'error'}, timeout=max(0.5, deadline - time.monotonic()))
                except TimeoutError:
                    break
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    home = parent_tmp / 'duck-agent-home'
    seed_scratch_home(home, isolation=args.isolation, heartbeat_secs=args.heartbeat_secs, respawn_max=args.respawn_max)
    ...
    turn_spec = {'duration_s': turn_duration, 'delta_interval_s': args.delta_interval_s, 'tokens_per_delta': args.tokens_per_delta, 'chunk': args.chunk}
    result: dict[str, Any] = {'mode': 'dry-run' if args.dry_run else 'heavy', 'isolation': args.isolation, 'concurrency': concurrency, 'run_duration_s': duration_s, 'turn_spec': turn_spec, 'threshold_ms': threshold_ms, 'scratch_home': str(home), 'port': port}
    try:
        with ScratchDashboard(home=home, port=port, isolation=args.isolation, env_extra={'HERMES_DASHBOARD_SESSION_TOKEN': token}) as dash:
            ...
            probe_thread = threading.Thread(target=probe_loop, args=(actual_port, token, stop_at, args.cadence_s, ws_samples, rest_samples), name='probe', daemon=True)
            probe_thread.start()
            ...
            load_valid = lanes_with_turn >= concurrency and total_turns >= concurrency and (min_turn_dur >= 0.7 * expected_turn_s) and (worst_min_deltas >= 5)
            result.update({'ws_probe': ws_stat, 'rest_probe': rest_stat, 'ws_probes_over_threshold': ws_over, 'rest_probes_over_threshold': rest_over, 'serving_stalls': serving_stalls, 'event_loop_stall_log_lines': log_stalls, 'heavy_turns_completed': total_turns, 'lanes_with_turn': lanes_with_turn, 'median_turn_duration_s': round(median_turn_dur, 2), 'min_turn_duration_s': round(min_turn_dur, 2), 'worst_lane_min_deltas': worst_min_deltas, 'load_valid': load_valid, 'lane_errors': lane_errors[:20]})
            ...
            if args.dry_run:
                ...
                serving_ok = serving_p99 < threshold_ms and serving_stalls == 0 and (log_stalls == 0) and probe_thread_samples_ok(ws_samples, rest_samples)
                if not load_valid:
                    result['verdict'] = 'INCONCLUSIVE'
                    result.setdefault('notes', []).append('load invalid: lanes/turn-duration/deltas below the sustained-heavy-load floor — not the AC-4 incident regime, verdict cannot certify')
                else:
...
def main(argv: list[str] | None=None) -> int:
    ...
    p.add_argument('--isolation', choices=['on', 'off'], default='on', help='set dashboard.turn_isolation in the scratch DUCK_AGENT_HOME')
    p.add_argument('--dry-run', action='store_true', help='1 short light turn plumbing smoke — NOT an AC-4 verdict')
    p.add_argument('--concurrency', type=int, default=6, help='concurrent heavy-turn lanes (AC-4: 6)')
    p.add_argument('--duration-s', type=float, default=600.0, dest='duration_s', help='sustained run window seconds (AC-4: ~600 = 10 min)')
    p.add_argument('--turn-duration-s', type=float, default=12.0, dest='turn_duration_s', help='wall seconds of GIL-holding compute per heavy turn')
    p.add_argument('--delta-interval-s', type=float, default=0.05, dest='delta_interval_s', help='streamed-delta cadence per heavy turn')
    ...
    p.add_argument('--cadence-s', type=float, default=0.5, dest='cadence_s', help='serving-path probe cadence (AC-4: 500ms)')
    p.add_argument('--threshold-ms', type=float, default=1000.0, dest='threshold_ms', help='serving p99 threshold (AC-4: <1s)')
    ...
    p.add_argument('--keep-home', action='store_true', help='do not delete the scratch DUCK_AGENT_HOME on exit')
