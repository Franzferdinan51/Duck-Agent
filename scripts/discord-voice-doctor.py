...
HERMES_HOME = Path(os.getenv('DUCK_AGENT_HOME', Path.home() / '.duck-agent'))
    ...
    if _discord_available:
        try:
            ...
            if not opus_loaded:
                ...
                if not opus_path:
                    candidates = ['/opt/homebrew/lib/libopus.dylib', '/usr/local/lib/libopus.dylib', '/usr/lib/x86_64-linux-gnu/libopus.so.0', '/usr/lib/aarch64-linux-gnu/libopus.so.0', '/usr/lib/libopus.so', '/usr/lib64/libopus.so']
                    for p in candidates:
                        if os.path.isfile(p):
                            opus_path = p
                            break
...
def check_env_vars():
    ...
    try:
        ...
        load_hermes_dotenv(hermes_home=ENV_FILE.parent, project_env=PROJECT_ROOT / '.env')
    except ImportError:
        pass
    ok = True
    ...
    if allowed:
        ...
        for uid in users:
            ...
            if token and uid.isdigit():
                try:
                    import requests
                    r = requests.get(f'https://discord.com/api/v10/users/{uid}', headers={'Authorization': f'Bot {token}'}, timeout=3)
...
def check_bot_permissions(token):
    ...
    try:
        ...
        return True
    VOICE_PERMS = {'Priority Speaker': 8, 'Stream': 9, 'View Channel': 10, 'Send Messages': 11, 'Embed Links': 14, 'Attach Files': 15, 'Read Message History': 16, 'Connect': 20, 'Speak': 21, 'Mute Members': 22, 'Deafen Members': 23, 'Move Members': 24, 'Use VAD': 25, 'Send Voice Messages': 46}
...
def main():
    print()
    print('\x1b[1m' + '=' * 50 + '\x1b[0m')
    print('\x1b[1m  Discord Voice Doctor\x1b[0m')
    print('\x1b[1m' + '=' * 50 + '\x1b[0m')
    all_ok = True
    ...
    print('\x1b[1m' + '-' * 50 + '\x1b[0m')
    if all_ok:
        print(f'  {OK} \x1b[92mAll checks passed — voice mode ready!\x1b[0m')
    else:
        print(f'  {FAIL} \x1b[91mSome checks failed — fix issues above.\x1b[0m')
    print()
