...
from hermes_cli.config import check_config_version, get_config_path, get_env_path, migrate_config
...
def main() -> int:
    ...
    if current_ver < SUPPORT_FLOOR_VERSION:
        print(f'[config-migrate] WARNING: {support_floor_message()}', file=sys.stderr)
        return 0
    ...
    print(f'[config-migrate] Migrating config schema {current_ver} -> {latest_ver}; backups: {backup_text}')
    try:
        migrate_config(interactive=False, quiet=False)
    except Exception:
        restored = _restore_backups(backups)
        if restored:
            print('[config-migrate] Migration failed; restored ' + ', '.join((str(path) for path in restored)))
        raise
    ...
    if post_ver < latest_ver:
        ...
        raise RuntimeError(f'migration did not advance config version to {latest_ver} (still {post_ver}); restored: {restored_text}')
    return 0
