...
_EMAIL_RE = re.compile('^[^/\\\\\\s]+@[^/\\\\\\s]+$')
_LOGIN_RE = re.compile('^[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?$')
...
def _legacy_login(email: str) -> str | None:
    ...
    try:
        ...
        from release import LEGACY_AUTHOR_MAP
...
def add_contributor(email: str, login: str, comment: str='') -> int:
    ...
    if existing is not None:
        if existing == login:
            print('present')
            return 0
        print(f'error: {email} already maps to {existing!r} (asked for {login!r}) — resolve manually', file=sys.stderr)
        return 1
