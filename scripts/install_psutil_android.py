...
from hermes_cli.psutil_android import PSUTIL_URL, PsutilAndroidInstallError, prepare_patched_psutil_sdist
...
def main() -> int:
    ...
    parser.add_argument('--pip', help="Explicit installer command (e.g. '/usr/bin/uv pip' or 'python -m pip')")
    parser.add_argument('--uv', action='store_true', help='Force using uv (errors out if uv is not on PATH)')
    args = parser.parse_args()
    install_cmd_prefix = _resolve_install_cmd(args.pip, args.uv)
    print('→ Termux/Android: prebuilding psutil with Linux source path compatibility shim (see psutil#2762)...')
