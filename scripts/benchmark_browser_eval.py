...
def _start_chrome(port: int):
    profile = tempfile.mkdtemp(prefix='duck-agent-bench-eval-')
    proc = subprocess.Popen([_find_chrome(), f'--remote-debugging-port={port}', f'--user-data-dir={profile}', '--no-first-run', '--no-default-browser-check', '--headless=new', '--disable-gpu'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def main():
    ...
    try:
        ...
        def fmt(name: str, ts: list[float]) -> str:
            ...
            mn, mx = (min(ts), max(ts))
            return f'  {name:<40} mean={mean:>7.2f}ms  median={median:>7.2f}ms  min={mn:>7.2f}ms  max={mx:>7.2f}ms'
        print()
