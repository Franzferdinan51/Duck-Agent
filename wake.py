...
* Stateless request/response adapters (the API server,
  ...
  ``X-Duck Agent-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Duck Agent-Session-Id`` header — the
...
async def deliver_wake(adapter: Any, *, text: str, session_id: str='', source: Any=None) -> None:
    ...
    ``session_id`` is the RAW session id (the ``X-Duck Agent-Session-Id`` value /
    ...
    if adapter_supports_push(adapter):
        ...
        return
    if not session_id:
        raise ValueError('deliver_wake: non-push adapter (supports_async_delivery=False) requires the raw session id to self-post the wake turn')
...
async def _self_post_chat_completion(adapter: Any, *, text: str, session_id: str) -> None:
    ...
    Session continuation via ``X-Duck Agent-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError('wake self-post requires API_SERVER_KEY: session continuation via X-Duck Agent-Session-Id is rejected (403) on an unauthenticated API server, so the wake cannot reach the target session')
    if ':' in host and (not host.startswith('[')):
        host = f'[{host}]'
    url = f'http://{host}:{port}/v1/chat/completions'
    headers = {'Authorization': f'Bearer {api_key}', 'X-Duck Agent-Session-Id': session_id}
    payload = {'model': str(getattr(adapter, '_model_name', '') or 'duck-agent'), 'messages': [{'role': 'user', 'content': text}], 'stream': False}
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as http:
                async with http.post(url, json=payload, headers=headers) as resp:
                    if resp.status == 429:
                        last_err = RuntimeError(f'wake self-post got HTTP 429 (concurrency cap) for session {session_id}')
                        logger.warning('%s; attempt %d/%d', last_err, attempt + 1, attempts)
                        continue
                    if resp.status >= 400:
                        body = (await resp.text())[:300]
                        raise RuntimeError(f'wake self-post failed for session {session_id}: HTTP {resp.status}: {body}')
                    await resp.read()
                    logger.info('wake self-post delivered for session %s (attempt %d)', session_id, attempt + 1)
                    return
        except (aiohttp.ClientError, asyncio.TimeoutError, OSError) as exc:
            last_err = exc
            logger.warning('wake self-post transient failure for session %s (attempt %d/%d): %s', session_id, attempt + 1, attempts, exc)
            continue
    raise RuntimeError(f'wake self-post gave up for session {session_id} after {attempts} attempts: {last_err}') from last_err