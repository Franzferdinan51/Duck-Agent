...
class TurnLeaseTimeoutError(TimeoutError):
    ...
    def __init__(self, session_id: str, *, owner_key: str, generation: int, wait_seconds: float) -> None:
        ...
        super().__init__(f'turn lease wait timed out after {wait_seconds:.0f}s on session {session_id} for routing key {owner_key} (gen {generation})')
...
class TurnLeaseToken:
    ...
    def __repr__(self) -> str:
        return f'TurnLeaseToken(session_id={self.session_id!r}, owner_key={self.owner_key!r}, generation={self.generation}, released={self.released})'
...
class SessionTurnLeaseRegistry:
    ...
    def _evict_idle(self) -> None:
        ...
        if overflow <= 0:
            return
        idle_ids = sorted((sid for sid, lease in self._leases.items() if lease.idle), key=lambda sid: self._leases[sid].last_used)
    ...
    async def acquire(self, session_id: str, *, owner_key: str, generation: int, timeout: Optional[float]=None) -> Optional[TurnLeaseToken]:
        ...
        if lease.lock.locked():
            holder = lease.holder
            logger.warning("turn lease contention on session %s: routing key %s (gen %s) waiting behind in-flight turn held by routing key %s (gen %s, held %.0fs) — two routing keys are mapped to one session_id (#64934); serializing this turn behind the previous turn's flush", session_id, owner_key, generation, holder.owner_key if holder else '?', holder.generation if holder else '?', time.time() - lease.acquired_at if lease.acquired_at else -1.0)
        lease.pending_acquires += 1
        try:
            await asyncio.wait_for(lease.lock.acquire(), timeout=wait)
        except asyncio.TimeoutError:
            holder = lease.holder
            logger.error('turn lease wait timed out after %.0fs on session %s (waiter: routing key %s gen %s; holder: routing key %s gen %s) — failing closed: refusing to run this turn UNSERIALIZED against the still-held lease', wait, session_id, owner_key, generation, holder.owner_key if holder else '?', holder.generation if holder else '?')
            raise TurnLeaseTimeoutError(session_id, owner_key=owner_key, generation=generation, wait_seconds=wait) from None
        finally:
            lease.pending_acquires -= 1
        lease.holder = token
    ...
    def rebind(self, token: Optional[TurnLeaseToken], new_session_id: str) -> bool:
        ...
        """
        if token is None or token.released or (not new_session_id) or (new_session_id == token.session_id):
            return False
    ...
    def release(self, token: Optional[TurnLeaseToken]) -> bool:
        ...
        if lease.holder is not token:
            logger.debug('turn lease release skipped on session %s: token (key %s gen %s) is not the current holder', token.session_id, token.owner_key, token.generation)
            return False
        lease.holder = None
