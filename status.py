...
The PID file lives at ``{DUCK_AGENT_HOME}/gateway.pid``.  DUCK_AGENT_HOME defaults to
``~/.duck-agent`` but can be overridden via the environment variable.  This means
separate DUCK_AGENT_HOME directories naturally get separate PID files — a property
...
if sys.platform == 'win32':
    ...
    import fcntl
_GATEWAY_KIND = 'duck-agent-gateway'
...
def _get_process_hermes_home() -> Path:
    """Return the process-level DUCK_AGENT_HOME, skipping context-local overrides.
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Return a stable absolute DUCK_AGENT_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)

def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare DUCK_AGENT_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_hermes_home(left))) == os.path.normcase(str(_canonical_hermes_home(right)))

def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting DUCK_AGENT_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    return state_home / 'duck-agent' / _LOCKS_DIRNAME

def _utc_now_iso() -> str:
    return datetime.now(timezone.utc).isoformat()
_EPOCH_MIN_PLAUSIBLE = 946684800.0
...
def terminate_pid(pid: int, *, force: bool=False) -> None:
    ...
    if force and _IS_WINDOWS:
        ...
        try:
            result = subprocess.run(['taskkill', '/PID', str(pid), '/T', '/F'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, creationflags=windows_hide_flags())
...
def _read_process_cmdline(pid: int) -> Optional[str]:
    ...
    if not _IS_WINDOWS:
        try:
            result = subprocess.run(['ps', '-p', str(pid), '-o', 'command='], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Duck Agent gateway lifecycle subcommand from a command line.
    ...
    (``"C:\\Program Files\\...\\duck-agent-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Duck Agent'ss
    ...
    for token in tokens:
        ...
        if basename in ('duck-agent-gateway', 'duck-agent-gateway.exe'):
            return 'run'
    joined = ' '.join(tokens)
    has_gateway_entry = 'hermes_cli.main' in joined or 'hermes_cli/main.py' in joined or any((t.rsplit('/', 1)[-1] in ('duck-agent', 'duck-agent.exe') for t in tokens))
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Duck Agent-owned runtime records
    or no-supervisor cleanup scans.
    """
    return _gateway_command_subcommand(command) in {'run', 'restart'}

def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Duck Agent gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a DUCK_AGENT_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.duck-agent`` or ``$DUCK_AGENT_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``DUCK_AGENT_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != 'default':
        ...
        return f'--profile {profile_lc}' in command_lc or f'-p {profile_lc}' in command_lc or f'duck_agent_home={home_lc}' in command_lc
    if '--profile ' in command_lc or ' -p ' in command_lc:
        return False
    if 'duck_agent_home=' in command_lc and f'duck_agent_home={home_lc}' not in command_lc:
        return False
    return True

def _record_matches_live_gateway_pid(record: dict[str, Any], pid: int, *, expected_home: Optional[Path]=None) -> bool:
    ...
    if live_cmdline:
        if not looks_like_gateway_runtime_command_line(live_cmdline):
            return False
        if expected_home is not None and (not _command_line_belongs_to_profile(live_cmdline, expected_home)):
            return False
        return True
    return _record_looks_like_gateway(record)

def _build_pid_record() -> dict:
    return {'pid': os.getpid(), 'kind': _GATEWAY_KIND, 'argv': list(sys.argv), 'start_time': _get_process_start_time(os.getpid()), 'duck_agent_home': str(_canonical_hermes_home(_get_process_hermes_home()))}

def _build_runtime_status_record() -> dict[str, Any]:
    payload = _build_pid_record()
    payload.update({'gateway_state': 'starting', 'exit_reason': None, 'restart_requested': False, 'active_agents': 0, 'platforms': {}, 'updated_at': _utc_now_iso()})
    return payload
...
def _running_pid_cache_signature(pid_path: Path, *, include_runtime_status: bool) -> tuple[Any, ...]:
    parts: list[Any] = [_file_cache_signature(pid_path), _file_cache_signature(_get_gateway_lock_path(pid_path))]
...
def _pid_exists(pid: int) -> bool:
    ...
    if _IS_WINDOWS:
        ...
        try:
            ...
            try:
                r = subprocess.run(['ps', '-o', 'state=', '-p', str(int(pid))], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
def read_runtime_status(path: Optional[Path]=None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``DUCK_AGENT_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(*, profile_dir: Optional[Path]=None, runtime: Any=_UNSET, health_probe: Optional[Callable[[], tuple[bool, Optional[dict[str, Any]]]]]=None, use_cache: bool=True, pid_probe: Optional[Callable[..., Optional[int]]]=None, runtime_reader: Optional[Callable[..., Optional[dict[str, Any]]]]=None, runtime_pid_probe: Optional[Callable[..., Optional[int]]]=None) -> GatewayLiveness:
    ...
    if runtime is _UNSET:
        try:
            runtime = _runtime_reader(path=profile_dir / 'gateway_state.json') if profile_dir is not None else _runtime_reader()
        except Exception:
            runtime = None
            probe_error = True
    try:
        runtime_pid = _runtime_pid_probe(runtime, expected_home=profile_dir) if profile_dir is not None else _runtime_pid_probe(runtime)
    except Exception:
        runtime_pid = None
        probe_error = True
...
def get_runtime_status_running_pid(runtime: Optional[dict[str, Any]]=None, *, expected_home: Optional[Path]=None) -> Optional[int]:
    ...
    DUCK_AGENT_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]]=None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different DUCK_AGENT_HOME dirs).
    """
    lock_path = _get_scope_lock_path(scope, identity)
    lock_path.parent.mkdir(parents=True, exist_ok=True)
    record = {**_build_pid_record(), 'scope': scope, 'identity_hash': _scope_hash(identity), 'metadata': metadata or {}, 'updated_at': _utc_now_iso()}
        ...
        if not stale:
            if not _pid_exists(existing_pid):
                ...
                if existing.get('start_time') is not None and current_start is not None and (current_start != existing.get('start_time')):
                    stale = True
                if not stale and (existing.get('start_time') is None or current_start is None) and (not _looks_like_gateway_process(existing_pid)):
                    live_cmdline = _read_process_cmdline(existing_pid)
                    if live_cmdline is not None or not _record_looks_like_gateway(existing):
                        stale = True
                if not stale and existing.get('start_time') is not None and (current_start is not None) and (not _looks_like_gateway_process(existing_pid)):
                    live_cmdline = _read_process_cmdline(existing_pid)
...
def _get_takeover_marker_path(hermes_home: Optional[Path]=None) -> Path:
    ...
    ``duck_agent_home`` is supplied only for a verified cross-home handoff.  The
...
def _consume_pid_marker_for_self(path: Path, *, pid_field: str, start_time_field: str, ttl_s: int) -> bool:
    record = _read_json_file(path)
    if not record:
        return False
    try:
...
def write_takeover_marker(target_pid: int, *, target_home: Optional[Path]=None, target_start_time: Any=_UNSET) -> bool:
    ...
    gateway's DUCK_AGENT_HOME rather than the replacer's.  Same-home callers omit
    ...
    try:
        ...
        record = {'target_pid': target_pid, 'target_start_time': target_start_time, 'target_hermes_home': str(marker_home), 'replacer_pid': os.getpid(), 'replacer_hermes_home': str(_canonical_hermes_home(_get_process_hermes_home())), 'written_at': _utc_now_iso()}
...
def consume_takeover_marker_for_self() -> bool:
    ...
    """
    return _consume_pid_marker_for_self(_get_takeover_marker_path(), pid_field='target_pid', start_time_field='target_start_time', ttl_s=_TAKEOVER_MARKER_TTL_S)
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    the lock record, the target DUCK_AGENT_HOME's gateway PID record, and the live
    ...
    if not isinstance(owner_start_time, int) or isinstance(owner_start_time, bool):
        return None
    raw_home = record.get('duck_agent_home')
    ...
    if pid_record_pid != owner_pid or pid_record.get('start_time') != owner_start_time:
        return None
    pid_record_home = pid_record.get('duck_agent_home')
...
def _wait_for_scoped_lock_owner_exit(owner_pid: int, owner_start_time: int, *, attempts: int, delay: float) -> tuple[bool, bool]:
    ...
    try:
        ...
        for child in children:
            try:
                ...
                if child.ppid() == parent_pid:
                    logger.debug('Skipping child PID %d of old gateway %d: parent still appears alive', child.pid, parent_pid)
                    continue
                child.terminate()
                live.append(child)
                ...
                logger.debug('Could not terminate child PID %s of old gateway %d', getattr(child, 'pid', '?'), parent_pid, exc_info=True)
        if not live:
            return 0
        ...
        for child in alive:
            try:
                ...
                logger.debug('Could not force-kill child PID %s of old gateway %d', getattr(child, 'pid', '?'), parent_pid, exc_info=True)
        if reaped:
...
def take_over_scoped_lock_holder(record: dict[str, Any], *, graceful_attempts: int=20, force_attempts: int=20) -> Optional[int]:
    ...
    replaced = _terminate_scoped_lock_owner_once(owner_pid, owner_start_time, target_home, graceful_attempts=graceful_attempts, force_attempts=force_attempts)
    if replaced is not None:
        reap_gateway_children(owner_children, parent_pid=owner_pid)
    return replaced

def _terminate_scoped_lock_owner_once(owner_pid: int, owner_start_time: int, target_home: Path, *, graceful_attempts: int=20, force_attempts: int=20) -> Optional[int]:
    ...
    try:
        ...
        try:
            ...
            return None
        exited, safe_to_force = _wait_for_scoped_lock_owner_exit(owner_pid, owner_start_time, attempts=graceful_attempts, delay=0.5)
        if exited:
            return owner_pid
        ...
        try:
            ...
            return None
        exited, _ = _wait_for_scoped_lock_owner_exit(owner_pid, owner_start_time, attempts=force_attempts, delay=0.25)
...
def write_planned_stop_marker(target_pid: int) -> bool:
    ...
    try:
        target_start_time = _get_process_start_time(target_pid)
        record = {'target_pid': target_pid, 'target_start_time': target_start_time, 'stopper_pid': os.getpid(), 'written_at': _utc_now_iso()}
...
def consume_planned_stop_marker_for_self() -> bool:
    ...
    return _consume_pid_marker_for_self(_get_planned_stop_marker_path(), pid_field='target_pid', start_time_field='target_start_time', ttl_s=_PLANNED_STOP_MARKER_TTL_S)
