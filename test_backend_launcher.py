...
class TestLauncherScript(unittest.TestCase):
    ...
    def test_launcher_is_executable(self):
        ...
        self.assertTrue(
            ...
            f"Launcher at {LAUNCHER} is not executable",
        )
    ...
    def test_launcher_shows_banner(self):
        ...
        # Either returns 0 or shows backend info in stderr
        output = result.stdout + result.stderr
        self.assertTrue(
            "Duck Agent" in output or "backend" in output.lower(),
            f"Expected banner or backend info, got: {output[:200]}",
        )
    ...
    def test_launcher_handles_invalid_backend(self):
        ...
        # Should either report error or fall back
        output = result.stdout + result.stderr
        self.assertTrue(
            len(output) > 0,
            "Expected some output for invalid backend",
        )
...
class TestEnvFile(unittest.TestCase):
    ...
    def test_env_example_lists_all_backends(self):
        ...
        self.assertIn("hermes-compatible", content)
...
class TestBackendSwitching(unittest.TestCase):
    ...
    def test_backend_via_env_var(self):
        ...
        for backend in ["grok-build", "hermes-compatible", "prime-agent"]:
            env = os.environ.copy()
            env["DUCK_AGENT_BACKEND"] = backend
            # Just verify the launcher runs without crashing
            result = subprocess.run(
                [str(LAUNCHER), "--help"],
                capture_output=True,
                text=True,
