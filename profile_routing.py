...
Allows a single Duck Agent instance to route specific Discord guilds/channels/threads
...
def parse_profile_routes(raw: Optional[List[Dict[str, Any]]]) -> List[ProfileRoute]:
    ...
    for entry in raw:
        ...
        try:
            ...
            continue
        routes.append(ProfileRoute(name=name, platform=platform, profile=profile, guild_id=entry.get('guild_id'), chat_id=entry.get('chat_id'), thread_id=entry.get('thread_id'), enabled=entry.get('enabled', True)))
