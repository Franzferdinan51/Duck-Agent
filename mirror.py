...
def mirror_to_session(platform: str, chat_id: str, message_text: str, source_label: str='cli', thread_id: Optional[str]=None, user_id: Optional[str]=None, role: str='assistant') -> bool:
    ...
    try:
        ...
        if not session_id:
            logger.debug('Mirror: no session found for %s:%s:%s:%s', platform, chat_id, thread_id, user_id)
            return False
        mirror_msg = {'role': role, 'content': message_text, 'timestamp': datetime.now().isoformat(), 'mirror': True, 'mirror_source': source_label}
...
def _find_session_id(platform: str, chat_id: str, thread_id: Optional[str]=None, user_id: Optional[str]=None) -> Optional[str]:
    ...
    if not candidates:
        return None
    if user_id:
        exact_user_matches = [entry for entry in candidates if str((entry.get('origin') or {}).get('user_id') or '') == str(user_id)]
        if exact_user_matches:
            candidates = exact_user_matches
        elif len(candidates) > 1:
            return None
    elif len(candidates) > 1:
        distinct_user_ids = {str((entry.get('origin') or {}).get('user_id') or '').strip() for entry in candidates if str((entry.get('origin') or {}).get('user_id') or '').strip()}
