"""Shared utility functions for duck-agent."""
import errno
import json
import logging
import os
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.duck-agent/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
    ...
    try:
        ...
        if exc.errno not in (errno.EXDEV, errno.EBUSY):
            raise
        logger.debug('atomic_replace: %s -> %s failed with %s; falling back to copy', tmp_str, real_path, errno.errorcode.get(exc.errno, exc.errno))
...
def atomic_write_text(path: Union[str, Path], content: str, *, encoding: str='utf-8', tmp_prefix: str='.tmp_', preserve_mode: bool=False, create_mode: 'int | None'=None) -> None:
...
def warn_if_credential_file_broadly_readable(path: Union[str, Path], *, label: str='', log: logging.Logger | None=None) -> bool:
    ...
    Secret-bearing files that users create by hand (or that older Duck Agent
    ...
    if not file_mode & (stat.S_IRGRP | stat.S_IROTH):
        return False
    _log.warning('%s%s is group/world-readable (mode 0%o) and contains secrets. Run: chmod 600 %s', f'{label} ' if label else '', p.name, stat.S_IMODE(file_mode), p)
    return True
...
def atomic_yaml_write(path: Union[str, Path], data: Any, *, default_flow_style: bool=False, sort_keys: bool=False, extra_content: str | None=None, create_mode: 'int | None'=None) -> None:
...
def model_forces_max_completion_tokens(model: str) -> bool:
    ...
    if '/' in m:
        m = m.rsplit('/', 1)[-1]
    return m.startswith('gpt-4o') or m.startswith('gpt-4.1') or m.startswith('gpt-5') or m.startswith('o1') or m.startswith('o3') or m.startswith('o4')
