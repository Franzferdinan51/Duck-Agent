...
The store is a small JSON file under the active profile's DUCK_AGENT_HOME so each
...
class DeadTargetRegistry:
    ...
    def mark_dead(self, platform: str, chat_id: Optional[str], reason: str='') -> bool:
        ...
        with self._lock:
            ...
            self._dead[key] = {'platform': str(platform).strip().lower(), 'chat_id': str(chat_id), 'reason': str(reason)[:200], 'marked_at': time.time()}
            self._flush_locked()
        if not existed:
            logger.info('dead_targets: marked %s as unreachable (%s) — future deliveries to this target will be skipped until a send succeeds', key, reason or 'no reason given')
        return not existed
    ...
    def all_dead(self) -> Dict[str, Dict[str, object]]:
        """Snapshot of the current dead set (for diagnostics / `duck-agent` CLI)."""
        with self._lock:
