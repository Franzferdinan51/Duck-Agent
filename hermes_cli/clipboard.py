...
def _powershell_write_script(b64: str) -> str:
    return f"Set-Clipboard -Value ([System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('{b64}')))"
...
def write_clipboard_text(text: str) -> bool:
    ...
    for argv, use_stdin in _write_clipboard_commands():
        try:
            if use_stdin:
                proc = subprocess.run(argv, input=text.encode('utf-8'), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10)
            else:
                b64 = base64.b64encode(text.encode('utf-8')).decode('ascii')
                proc = subprocess.run(argv + ['-Command', _powershell_write_script(b64)], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10)
...
def _macos_has_image() -> bool:
    ...
    try:
        info = subprocess.run(['osascript', '-e', 'clipboard info'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3)
...
def _macos_osascript(dest: Path) -> bool:
    ...
    try:
        r = subprocess.run(['osascript', '-e', script], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
_PS_CHECK_IMAGE_GET_CLIPBOARD = "try { $img = Get-Clipboard -Format Image -ErrorAction Stop;if ($null -ne $img) { 'True' } else { 'False' }} catch { 'False' }"
...
_PS_CHECK_FILEDROP_IMAGE = f"try {{ $files = Get-Clipboard -Format FileDropList -ErrorAction Stop;$exts = @({_FILEDROP_IMAGE_EXTS});$hit = $files | Where-Object {{ $exts -contains ([System.IO.Path]::GetExtension($_).ToLowerInvariant()) }} | Select-Object -First 1;if ($null -ne $hit) {{ 'True' }} else {{ 'False' }}}} catch {{ 'False' }}"
_PS_EXTRACT_FILEDROP_IMAGE = f'try {{ $files = Get-Clipboard -Format FileDropList -ErrorAction Stop;$exts = @({_FILEDROP_IMAGE_EXTS});$hit = $files | Where-Object {{ $exts -contains ([System.IO.Path]::GetExtension($_).ToLowerInvariant()) }} | Select-Object -First 1;if ($null -eq $hit) {{ exit 1 }}[System.Convert]::ToBase64String([System.IO.File]::ReadAllBytes($hit))}} catch {{ exit 1 }}'
_POWERSHELL_HAS_IMAGE_SCRIPTS = (_PS_CHECK_IMAGE, _PS_CHECK_IMAGE_GET_CLIPBOARD, _PS_CHECK_FILEDROP_IMAGE)
_POWERSHELL_EXTRACT_IMAGE_SCRIPTS = (_PS_EXTRACT_IMAGE, _PS_EXTRACT_IMAGE_GET_CLIPBOARD, _PS_EXTRACT_FILEDROP_IMAGE)

def _run_powershell(exe: str, script: str, timeout: int) -> subprocess.CompletedProcess:
    return subprocess.run([exe, '-NoProfile', '-NonInteractive', '-Command', script], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout)
...
def _find_powershell() -> str | None:
    ...
    for name in ('powershell', 'pwsh'):
        try:
            r = subprocess.run([name, '-NoProfile', '-NonInteractive', '-Command', 'echo ok'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
def _wayland_has_image() -> bool:
    ...
    try:
        r = subprocess.run(['wl-paste', '--list-types'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3)
...
def _wayland_save(dest: Path) -> bool:
    ...
    try:
        types_r = subprocess.run(['wl-paste', '--list-types'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3)
        ...
        with open(dest, 'wb') as f:
            subprocess.run(['wl-paste', '--type', mime], stdout=f, stderr=subprocess.DEVNULL, timeout=5, check=True)
...
def _convert_to_png(path: Path) -> bool:
    ...
    try:
        path.rename(tmp)
        r = subprocess.run(['convert', str(tmp), 'png:' + str(path)], capture_output=True, timeout=5)
...
def _xclip_has_image() -> bool:
    ...
    try:
        r = subprocess.run(['xclip', '-selection', 'clipboard', '-t', 'TARGETS', '-o'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3)
...
def _xclip_save(dest: Path) -> bool:
    ...
    try:
        targets = subprocess.run(['xclip', '-selection', 'clipboard', '-t', 'TARGETS', '-o'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3)
    ...
    try:
        with open(dest, 'wb') as f:
            subprocess.run(['xclip', '-selection', 'clipboard', '-t', 'image/png', '-o'], stdout=f, stderr=subprocess.DEVNULL, timeout=5, check=True)
