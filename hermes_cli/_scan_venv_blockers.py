...
_SENSITIVE_LONG_FLAGS: list[str] = ['--token', '--api-key', '--password', '--secret', '--authorization', '--access-key', '--private-key', '--session-key']
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    ``duck-agent-setup``, so the CLI updater's own
    ``_pause_windows_gateways_for_update()`` — which exists precisely to
    stop these processes (and is always active: ``duck-agent-setup`` invokes
    ``duck-agent update --yes --gateway``) — never gets the chance to run.
...
def main() -> None:
    ...
    try:
        from hermes_cli.main import _detect_venv_python_processes
        matches = _detect_venv_python_processes()
    except Exception as exc:
        _emit_probe_fail(f'scan aborted: {exc}')
    processes = [{'pid': pid, 'name': name, 'cmdline': _redact_sensitive_cmdline(cmdline)} for pid, name, cmdline in matches if not _is_pausable_gateway(cmdline)]
    exempted = sum((1 for _pid, _name, cmdline in matches if _is_pausable_gateway(cmdline)))
    data = {'ok': True, 'blocked': bool(processes), 'processes': processes, 'pausable_gateways': exempted}
