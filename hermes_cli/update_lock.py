"""Cross-process mutual exclusion for in-flight Duck Agent updates.

Three different surfaces can start an update of the same install tree:

* ``duck-agent update`` from a terminal,
* the dashboard's Update button (``POST /api/duck-agent/update`` →
  ``_spawn_hermes_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``duck-agent-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``duck-agent update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <DUCK_AGENT_HOME>/.duck-agent-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``duck-agent update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Duck Agent is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``duck-agent-setup`` binary under ``~/.duck-agent`` is only refreshed by a full
  ...
  ancestry check those users get exit 2 ("Duck Agent is still running") on every
  GUI update forever, with no Duck Agent process actually running.
"""
...
MARKER_NAME = '.duck-agent-update-in-progress'
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Duck Agent home (never the context-local profile override):
    the Rust updater resolves ``$DUCK_AGENT_HOME`` or the platform default, and the
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``duck-agent update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``duck-agent-setup`` binaries
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `duck-agent update` as a stage): we run under ITS claim
