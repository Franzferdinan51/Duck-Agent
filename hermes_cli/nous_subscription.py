...
from tools.tool_backend_helpers import fal_key_is_configured, has_direct_modal_credentials, managed_nous_tools_enabled, normalize_browser_cloud_provider, normalize_modal_mode, resolve_modal_backend_state, resolve_openai_audio_api_key
_DEFAULT_PLATFORM_TOOLSETS = {'cli': 'duck-agent-cli'}
MANAGED_FEATURE_COVERAGE_CATEGORY: Dict[str, str] = {'web': 'firecrawl', 'image_gen': 'fal', 'video_gen': 'fal-video', 'tts': 'openai-audio', 'stt': 'openai-audio', 'browser': 'browser-use', 'modal': 'modal'}
...
def _browser_label(current_provider: str) -> str:
    mapping = {'browserbase': 'Browserbase', 'browser-use': 'Browser Use', 'firecrawl': 'Firecrawl', 'camofox': 'Camofox', 'local': 'Local browser'}
    return mapping.get(current_provider or 'local', current_provider or 'Local browser')

def _tts_label(current_provider: str) -> str:
    mapping = {'openai': 'OpenAI TTS', 'elevenlabs': 'ElevenLabs', 'edge': 'Edge TTS', 'xai': 'xAI TTS', 'mistral': 'Mistral Voxtral TTS', 'neutts': 'NeuTTS'}
    return mapping.get(current_provider or 'edge', current_provider or 'Edge TTS')

def _stt_label(current_provider: str) -> str:
    mapping = {'openai': 'OpenAI Whisper', 'groq': 'Groq Whisper', 'mistral': 'Mistral Voxtral Transcribe', 'local': 'Local faster-whisper'}
...
def _resolve_browser_feature_state(*, browser_tool_enabled: bool, browser_provider: str, browser_provider_explicit: bool, browser_local_available: bool, browser_local_runnable: bool, direct_camofox: bool, direct_browserbase: bool, direct_browser_use: bool, direct_firecrawl: bool, managed_browser_available: bool) -> tuple[str, bool, bool, bool]:
    ...
    if browser_provider_explicit:
        ...
        if current_provider == 'browser-use':
            ...
            managed = bool(browser_tool_enabled and browser_local_available and managed_browser_available and (not direct_browser_use))
            active = bool(browser_tool_enabled and available)
...
def get_nous_subscription_features(config: Optional[Dict[str, object]]=None, *, force_fresh: bool=False) -> NousSubscriptionFeatures:
    ...
    browser_provider = normalize_browser_cloud_provider(browser_cfg.get('cloud_provider') if browser_provider_explicit else None)
    ...
    try:
        from tools.transcription_tools import _HAS_FASTER_WHISPER
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(get_env_value('HERMES_LOCAL_STT_COMMAND'))
    except Exception:
    ...
    managed_web_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready('firecrawl') and _entitled_for('firecrawl')
    managed_image_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready('fal-queue') and _entitled_for('fal')
    managed_video_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready('fal-queue') and _entitled_for('fal-video')
    managed_tts_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready('openai-audio') and _entitled_for('openai-audio')
    managed_stt_available = managed_tts_available
    managed_browser_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready('browser-use') and _entitled_for('browser-use')
    managed_modal_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready('modal') and _entitled_for('modal')
    modal_state = resolve_modal_backend_state(modal_mode, has_direct=direct_modal, managed_ready=managed_modal_available, managed_enabled=managed_tools_flag)
    web_managed = web_backend == 'firecrawl' and managed_web_available and (not direct_firecrawl)
    web_active = bool(web_tool_enabled and (web_managed or (web_backend == 'exa' and direct_exa) or (web_backend == 'firecrawl' and direct_firecrawl) or (web_backend == 'parallel' and direct_parallel) or (web_backend == 'tavily' and direct_tavily) or (web_backend == 'searxng' and direct_searxng) or (web_search_backend == 'searxng' and direct_searxng) or (web_search_backend == 'exa' and direct_exa) or (web_search_backend == 'firecrawl' and direct_firecrawl) or (web_search_backend == 'parallel' and direct_parallel) or (web_search_backend == 'tavily' and direct_tavily)))
    web_available = bool(managed_web_available or direct_exa or direct_firecrawl or direct_parallel or direct_tavily or direct_searxng)
    ...
    tts_managed = tts_tool_enabled and tts_current_provider == 'openai' and managed_tts_available and (not direct_openai_tts)
    tts_available = bool(tts_current_provider in {'edge', 'neutts'} or (tts_current_provider == 'openai' and (managed_tts_available or direct_openai_tts)) or (tts_current_provider == 'elevenlabs' and direct_elevenlabs) or (tts_current_provider == 'mistral' and bool(get_env_value('MISTRAL_API_KEY'))))
    tts_active = bool(tts_tool_enabled and tts_available)
    stt_current_provider = stt_provider or 'local'
    stt_managed = stt_current_provider == 'openai' and managed_stt_available and (not direct_openai_stt)
    stt_available = bool(stt_current_provider == 'local' and local_stt_available or (stt_current_provider == 'openai' and (managed_stt_available or direct_openai_stt)) or (stt_current_provider == 'groq' and direct_groq_stt) or (stt_current_provider == 'mistral' and direct_mistral_stt))
    stt_active = stt_available
    browser_local_available = _has_agent_browser()
    browser_local_runnable = _local_browser_runnable()
    browser_current_provider, browser_available, browser_active, browser_managed = _resolve_browser_feature_state(browser_tool_enabled=browser_tool_enabled, browser_provider=browser_provider, browser_provider_explicit=browser_provider_explicit, browser_local_available=browser_local_available, browser_local_runnable=browser_local_runnable, direct_camofox=direct_camofox, direct_browserbase=direct_browserbase, direct_browser_use=direct_browser_use, direct_firecrawl=direct_firecrawl, managed_browser_available=managed_browser_available)
...
def apply_nous_managed_defaults(config: Dict[str, object], *, enabled_toolsets: Optional[Iterable[str]]=None, force_fresh: bool=False) -> set[str]:
    ...
    if not (features.account_info and features.account_info.logged_in and features.account_info.tool_gateway_entitled):
        return set()
    ...
    if 'web' in selected_toolsets and (not features.web.explicit_configured) and (not (get_env_value('PARALLEL_API_KEY') or get_env_value('TAVILY_API_KEY') or get_env_value('FIRECRAWL_API_KEY') or get_env_value('FIRECRAWL_API_URL'))):
        web_cfg['backend'] = 'firecrawl'
        changed.add('web')
    if 'tts' in selected_toolsets and (not features.tts.explicit_configured) and (not (resolve_openai_audio_api_key() or get_env_value('ELEVENLABS_API_KEY'))):
        tts_cfg['provider'] = 'openai'
        changed.add('tts')
    if not features.stt.explicit_configured and (not _local_stt_backend_available()) and (not (resolve_openai_audio_api_key() or get_env_value('GROQ_API_KEY') or get_env_value('MISTRAL_API_KEY'))) and (features.account_info is not None) and features.account_info.tool_gateway_entitled_for('openai-audio'):
        stt_cfg['provider'] = 'openai'
        changed.add('stt')
    if 'browser' in selected_toolsets and (not features.browser.explicit_configured) and (not (get_env_value('BROWSER_USE_API_KEY') or get_env_value('BROWSERBASE_API_KEY'))):
    ...
    if 'video_gen' in selected_toolsets and (not fal_key_is_configured()) and features.account_info.tool_gateway_entitled_for('fal-video'):
    ...
    return changed
_GATEWAY_TOOL_LABELS = {'web': 'Web search & extract (Firecrawl)', 'image_gen': 'Image generation (FAL)', 'video_gen': 'Video generation (FAL)', 'tts': 'Text-to-speech (OpenAI TTS)', 'stt': 'Speech-to-text (OpenAI Whisper)', 'browser': 'Browser automation (Browser Use)'}
...
def _get_gateway_direct_credentials() -> Dict[str, bool]:
    ...
    return {'web': bool(get_env_value('FIRECRAWL_API_KEY') or get_env_value('FIRECRAWL_API_URL') or get_env_value('PARALLEL_API_KEY') or get_env_value('TAVILY_API_KEY') or get_env_value('EXA_API_KEY')), 'image_gen': fal_direct, 'video_gen': fal_direct, 'tts': bool(resolve_openai_audio_api_key() or get_env_value('ELEVENLABS_API_KEY')), 'stt': bool(resolve_openai_audio_api_key() or get_env_value('GROQ_API_KEY') or get_env_value('MISTRAL_API_KEY')), 'browser': bool(get_env_value('BROWSER_USE_API_KEY') or (get_env_value('BROWSERBASE_API_KEY') and get_env_value('BROWSERBASE_PROJECT_ID')))}
_GATEWAY_DIRECT_LABELS = {'web': 'Firecrawl/Exa/Parallel/Tavily key', 'image_gen': 'FAL key', 'video_gen': 'FAL key', 'tts': 'OpenAI/ElevenLabs key', 'stt': 'OpenAI/Groq/Mistral key', 'browser': 'Browser Use/Browserbase key'}
...
def get_gateway_eligible_tools(config: Optional[Dict[str, object]]=None, *, force_fresh: bool=False) -> tuple[list[str], list[str], list[str]]:
    ...
    opted_in = {'web': _uses_gateway(config.get('web')), 'image_gen': _uses_gateway(config.get('image_gen')), 'video_gen': _uses_gateway(config.get('video_gen')), 'tts': _uses_gateway(config.get('tts')), 'stt': _uses_gateway(config.get('stt')), 'browser': _uses_gateway(config.get('browser'))}
    unconfigured: list[str] = []
    has_direct: list[str] = []
    already_managed: list[str] = []
    for key in _ALL_GATEWAY_KEYS:
        if not account_info.tool_gateway_entitled_for(MANAGED_FEATURE_COVERAGE_CATEGORY[key]):
            continue
        if opted_in.get(key):
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool=True) -> set[str]:
    ...
    """
    unconfigured, has_direct, already_managed = get_gateway_eligible_tools(config, force_fresh=force_fresh)
    ...
    try:
        ...
        account_info = None
    pool_only = bool(account_info and account_info.paid_service_access is not True and (account_info.tool_access is not None) and account_info.tool_access.enabled)
    ...
    labels += [f'{_GATEWAY_TOOL_LABELS[k]} — keep using your {_GATEWAY_DIRECT_LABELS[k]}' for k in has_direct]
    pre_selected = list(range(len(unconfigured)))
    if pool_only:
        title = 'Your free Nous tool pool — pick the tools to enable:'
    else:
        title = 'Your Nous subscription includes the Tool Gateway — pick the tools to enable:'
    try:
...
def ensure_nous_portal_access(*, capability: str='the Nous Tool Gateway', coverage_category: Optional[str]=None) -> bool:
    ...
    Used by ``duck-agent tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``duck-agent model``'s Nous
    login, this:
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    ...
    """
    try:
        from hermes_cli.auth import _auth_store_lock, _load_auth_store, _nous_device_code_login, _read_shared_nous_state, _save_auth_store, _save_provider_state, _sync_nous_pool_from_auth_store, _try_import_shared_nous_state, _write_shared_nous_state
