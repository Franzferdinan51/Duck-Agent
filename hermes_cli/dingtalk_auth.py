...
def begin_registration() -> dict:
    ...
    return {'device_code': device_code, 'verification_uri_complete': verification_uri_complete, 'expires_in': int(begin_data.get('expires_in', 7200)), 'interval': max(int(begin_data.get('interval', 3)), 2)}
...
def poll_registration(device_code: str) -> dict:
    ...
    if status_raw not in {'WAITING', 'SUCCESS', 'FAIL', 'EXPIRED'}:
        status_raw = 'UNKNOWN'
    return {'status': status_raw, 'client_id': str(data.get('client_id', '')).strip() or None, 'client_secret': str(data.get('client_secret', '')).strip() or None, 'fail_reason': str(data.get('fail_reason', '')).strip() or None}
...
def dingtalk_qr_auth() -> Optional[Tuple[str, str]]:
    ...
    def _on_waiting():
        ...
        if dot_count % 10 == 0:
            ...
            sys.stdout.flush()
    try:
        client_id, client_secret = wait_for_registration_success(device_code=reg['device_code'], interval=reg['interval'], expires_in=reg['expires_in'], on_waiting=_on_waiting)
    except RegistrationError as exc:
        print()
