...
``duck-agent gateway start/stop/restart`` when running inside a container.
"""
...
_VALID_PROFILE_RE = re.compile('^[a-z0-9][a-z0-9_-]*$')
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``duck-agent gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged duck-agent
    ...
    detection always fails. Since every Duck Agent runtime call inside the
    container drops to duck-agent via ``s6-setuidgid``, that silent failure
...
class _RegistrationUnsupportedMixin:
    ...
    def register_profile_gateway(self, profile: str, *, extra_env: dict[str, str] | None=None, start_now: bool=True) -> None:
        raise NotImplementedError(f'{type(self).__name__} does not support runtime profile gateway registration (container-only feature)')

    def unregister_profile_gateway(self, profile: str) -> None:
        raise NotImplementedError(f'{type(self).__name__} does not support runtime profile gateway unregistration (container-only feature)')
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    ...
    def install(self, *, force: bool=False, start_now: bool | None=None, start_on_login: bool | None=None, elevated_handoff: bool=False) -> None:
        ...
        gateway_windows.install(force=force, start_now=start_now, start_on_login=start_on_login, elevated_handoff=elevated_handoff)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's DUCK_AGENT_HOME blindly;
    derive the shared profile root from the current DUCK_AGENT_HOME and map the
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the duck-agent user.
    ...
    (the ``duck-agent`` user — UID 10000 — running every Duck Agent runtime
    ...
    is dispatched as the duck-agent user.
    ...
    with duck-agent ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           duck-agent:duck-agent, 0755 (parent must already exist)
    ``svc_dir/event/``                     duck-agent:duck-agent, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 duck-agent:duck-agent, 0755
    ``svc_dir/supervise/event/``           duck-agent:duck-agent, 03730
    ``svc_dir/supervise/control``          duck-agent:duck-agent, 0660    (FIFO)
    ...
    duck-agent user reads them fine.
    ...
    slot's supervise/ was duck-agent-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            pass
    _mkdir_owned(svc_dir / 'event', 2008)
    supervise = svc_dir / 'supervise'
    _mkdir_owned(supervise, 493)
    _mkdir_owned(supervise / 'event', 2008)
    control = supervise / 'control'
    if not control.exists():
        os.mkfifo(control, 432)
        control.chmod(432)
        try:
    ...
    log_dir = svc_dir / 'log'
    if log_dir.is_dir():
        _mkdir_owned(log_dir / 'event', 2008)
        log_supervise = log_dir / 'supervise'
        _mkdir_owned(log_supervise, 493)
        _mkdir_owned(log_supervise / 'event', 2008)
        log_control = log_supervise / 'control'
        if not log_control.exists():
            os.mkfifo(log_control, 432)
            log_control.chmod(432)
            try:
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``duck-agent -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(f'no such gateway {profile!r}: register it with `duck-agent profile create {profile}` first, or pass an existing profile name via `-p <name>`', service=f'gateway-{profile}')
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-duck-agent, dashboard)
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        The script:
          1. Sources DUCK_AGENT_HOME (and any extra env) via with-contenv —
             so e.g. ``-e DUCK_AGENT_HOME=/data/duck-agent`` is honored at run
          ...
          4. Drops to the duck-agent user and exec's
             ``duck-agent -p <profile> gateway run`` (or just ``duck-agent
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``duck-agent gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root DUCK_AGENT_HOME profile" (the implicit profile that exists at
        the top of $DUCK_AGENT_HOME, not under profiles/). It must be
        ...
        would instead look up ``$DUCK_AGENT_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``DUCK_AGENT_HOME``,
        ...
        """
        import shlex
        lines = ['#!/command/with-contenv sh', '# shellcheck shell=sh', 'set -e', 'export HOME=/opt/data', 'cd /opt/data', '. /opt/duck-agent/.venv/bin/activate']
        for k, v in sorted(extra_env.items()):
            lines.append(f'export {k}={shlex.quote(v)}')
        lines.append('export HERMES_S6_SUPERVISED_CHILD=1')
        if profile == 'default':
            gateway_cmd = 'duck-agent gateway run --replace'
        else:
            gateway_cmd = f'duck-agent -p {shlex.quote(profile)} gateway run --replace'
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f'exec s6-setuidgid duck-agent {gateway_cmd}')
        return '\n'.join(lines) + '\n'
    ...
    def _render_finish_script() -> str:
        ...
        return f'''#!/command/with-contenv sh\n# shellcheck shell=sh\n# $1 = exit code from the run script.\n# Exit {code} (EX_CONFIG) = fatal config error — don't restart.\nif [ "$1" = "{code}" ]; then\n  exit 125\nfi\nexit 0\n'''

    @staticmethod
    def _render_log_run(profile: str) -> str:
        """Generate the log/run script for a profile-gateway service.

        OQ8-C: persist to ``${DUCK_AGENT_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the DUCK_AGENT_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e DUCK_AGENT_HOME=/data/duck-agent``
        gets its logs under /data/duck-agent/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``duck-agent logs`` reads and what
        ...
        return f'#!/command/with-contenv sh\n# shellcheck shell=sh\n: "${{DUCK_AGENT_HOME:=/opt/data}}"\nlog_dir="$DUCK_AGENT_HOME/logs/gateways/{prof}"\nif [ "$(id -u)" = 0 ]; then\n  s6-setuidgid duck-agent mkdir -p "$log_dir"\n  s6-setuidgid duck-agent rm -f "$log_dir/lock"\nelse\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\nexec s6-setuidgid duck-agent s6-log 1 n10 s1000000 T "$log_dir"\n'
    ...
    def _run_svc(self, action_flag: str, action_label: str, name: str) -> None:
        ...
        * ``S6CommandError`` — anything else (EACCES on the
        ...
        try:
            subprocess.run([f'{_S6_BIN_DIR}/s6-svc', action_flag, str(service_dir)], check=True, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
        except subprocess.CalledProcessError as exc:
            raise S6CommandError(service=name, action=action_label, returncode=exc.returncode, stderr=exc.stderr or '') from exc
    ...
    def _supervised_pid(self, name: str) -> int | None:
        ...
        import subprocess
        try:
            result = subprocess.run([f'{_S6_BIN_DIR}/s6-svstat', str(self.scandir / name)], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
        except (OSError, subprocess.SubprocessError):
            return None
        if result.returncode != 0:
            return None
        m = re.search('\\(pid (\\d+)\\)', result.stdout)
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``duck-agent gateway
    ...
    def register_profile_gateway(self, profile: str, *, extra_env: dict[str, str] | None=None, start_now: bool=True) -> None:
        ...
        user explicitly runs ``duck-agent -p <profile> gateway start``.
        ...
        try:
            ...
            shutil.rmtree(tmp_dir, ignore_errors=True)
            raise
        result = subprocess.run([f'{_S6_BIN_DIR}/s6-svscanctl', '-a', str(self.scandir)], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
        if result.returncode != 0:
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        if not svc_dir.exists():
            return
        subprocess.run([f'{_S6_BIN_DIR}/s6-svc', '-d', str(svc_dir)], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5, check=False)
        subprocess.run([f'{_S6_BIN_DIR}/s6-svwait', '-D', '-t', '10000', str(svc_dir)], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=15, check=False)
        subprocess.run([f'{_S6_BIN_DIR}/s6-svscanctl', '-an', str(self.scandir)], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5, check=False)
        time.sleep(0.2)
        shutil.rmtree(svc_dir, ignore_errors=True)
