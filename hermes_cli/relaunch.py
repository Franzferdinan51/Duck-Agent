"""
Unified self-relaunch for Duck Agent CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``duck-agent sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``duck-agent`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
import os
import shutil
import sys
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched duck-agent."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the duck-agent entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("duck-agent")`` on PATH.
    ...
    ``duck-agent.exe`` on PATH or the ``sys.executable -m hermes_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    if not argv0.startswith('-') and os.path.isfile(argv0):
        ...
        if os.access(abs_path, os.X_OK):
            if not (_is_windows and _is_python_script(abs_path)):
                return abs_path
    path_bin = shutil.which('duck-agent')
    if path_bin:
        return path_bin
    return None
...
def build_relaunch_argv(extra_args: Sequence[str], *, preserve_inherited: bool=True, original_argv: Optional[Sequence[str]]=None) -> list[str]:
    """Construct an argv list for replacing the current process with duck-agent.
...
def relaunch(extra_args: Sequence[str], *, preserve_inherited: bool=True, original_argv: Optional[Sequence[str]]=None) -> None:
    """Replace the current process with a fresh duck-agent invocation.
    ...
    relaunch contract wants: "run duck-agent again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``duck-agent.exe`` (a Python console-script shim that wraps
    ...
    That's functionally equivalent — the user sees "duck-agent exited, then
    new duck-agent started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == 'win32':
        ...
        try:
            ...
            print(f"\nDuck Agent relaunch failed: {exc}\nCommand: {' '.join(new_argv)}\nFix: open a new terminal so PATH picks up, then re-run duck-agent.", file=sys.stderr)
            sys.exit(1)
    else:
