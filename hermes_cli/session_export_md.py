"""Markdown/QMD export helpers for Duck Agent sessions.
...
EXPORTER_VERSION = 'duck-agent sessions export (md/qmd) v1'
_SHA_LINE_RE = re.compile('- SHA256 of exported body: `([0-9a-f]{64})`')
...
def _export_body_without_hash(session: dict[str, Any], *, fmt: str, exported_at: float) -> str:
    ...
    frontmatter.extend([_frontmatter_line('format', fmt), _frontmatter_line('exported_at', exported_iso), _frontmatter_line('exporter', EXPORTER_VERSION), '---', ''])
...
def safe_session_filename(session: dict[str, Any], *, fmt: str='md') -> str:
    ...
    slug = re.sub('[^A-Za-z0-9._-]+', '-', title).strip('.-_').lower()
    if not slug:
        slug = 'session'
    slug = slug[:60]
...
def append_manifest_entry(output_dir: Path | str, session: dict[str, Any], path: Path | str, *, fmt: str) -> Path:
    ...
    entry = {'session_id': _session_id(session), 'lineage_session_ids': session.get('lineage_session_ids') or [_session_id(session)], 'path': str(export_path), 'format': fmt, 'message_count': _message_count(session), 'sha256': file_sha256(export_path), 'exported_at': time.time()}
