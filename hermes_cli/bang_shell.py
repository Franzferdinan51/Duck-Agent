...
def bang_shell_enabled() -> bool:
    ...
    try:
        ...
        def env_var_enabled(name, default=''):
...
def _bang_env() -> dict:
    """Environment for a bang command, with Duck Agent-managed secrets filtered.
...
def run_bang_command(command: str, *, cwd: Optional[str]=None, timeout: int=DEFAULT_TIMEOUT, writer=None) -> int:
    ...
    try:
        ...
        creationflags = 0
    try:
        proc = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', cwd=run_cwd, env=_bang_env(), creationflags=creationflags)
