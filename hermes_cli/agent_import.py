"""duck-agent import-agent — import Claude Code / Codex CLI setups into Duck Agent.

Usage:
    duck-agent import-agent                       # auto-detect ~/.claude or ~/.codex
    duck-agent import-agent claude-code           # import from ~/.claude
    duck-agent import-agent codex                 # import from ~/.codex
    duck-agent import-agent claude-code --dry-run # preview only, no changes
    duck-agent import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``duck-agent claw migrate`` /
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in DUCK_AGENT_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → DUCK_AGENT_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in DUCK_AGENT_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in DUCK_AGENT_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → DUCK_AGENT_HOME/skills/codex-imports/<name>/
...
deliberately via ``duck-agent setup`` or config.yaml.
"""
...
_SECRET_KEY_RE = re.compile('(?:^|_)(?:API[_-]?KEY|APIKEY|TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIALS?|AUTH|PRIVATE[_-]?KEY|ACCESS[_-]?KEY)(?:_|$)|KEY$', re.IGNORECASE)
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(f'Refusing to overwrite {path}: the existing file cannot be read ({exc}). Fix the file permissions or move it aside first.') from exc
    try:
        data = yaml.safe_load(raw)
    except yaml.YAMLError as exc:
        raise ConfigReadError(f'Refusing to overwrite {path}: the existing file is not valid YAML ({exc}). Fix it with `duck-agent config edit` (or move it aside), then re-run the import.') from exc
    if data is None:
        return {}
    if not isinstance(data, dict):
        raise ConfigReadError(f'Refusing to overwrite {path}: expected the existing file to hold a YAML mapping but found {type(data).__name__}. Fix it with `duck-agent config edit` (or move it aside), then re-run the import.')
    return data
...
def extract_markdown_entries(text: str) -> List[str]:
    ...
    def context_prefix() -> str:
        filtered = [h for h in headings if h and (not re.search('\\b(MEMORY|USER|SOUL|AGENTS|TOOLS|IDENTITY|CLAUDE)\\.md\\b', h, re.I))]
    ...
    for raw_line in (text or '').splitlines():
        ...
        if in_code_block:
            continue
        heading_match = re.match('^(#{1,6})\\s+(.*\\S)\\s*$', stripped)
        if heading_match:
            flush_paragraph()
            ...
            continue
        bullet_match = re.match('^\\s*(?:[-*]|\\d+\\.)\\s+(.*\\S)\\s*$', line)
        if bullet_match:
            flush_paragraph()
...
def merge_entries(existing: Sequence[str], incoming: Sequence[str], limit: int) -> Tuple[List[str], Dict[str, int]]:
    ...
    for entry in incoming:
        ...
        stats['added'] += 1
    return (merged, stats)
_BASH_RULE_RE = re.compile('^Bash\\((?P<inner>.*)\\)$')

def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Duck Agent glob.
...
class AgentImporter:
    ...
    def __init__(self, agent: str, source_root: Path, target_root: Path, execute: bool=False, overwrite: bool=False) -> None:
    ...
    def record(self, kind: str, source, destination, status: str, reason: str='', **details) -> None:
        item: Dict[str, Any] = {'kind': kind, 'source': str(source) if source else None, 'destination': str(destination) if destination else None, 'status': status, 'reason': reason}
        if details:
            item.update(details)
        self.items.append(item)

    def load_target_config(self, kind: str, source, destination: Path) -> Optional[Dict[str, Any]]:
    ...
    def build_report(self) -> Dict[str, Any]:
        ...
        report: Dict[str, Any] = {'agent': self.agent, 'source': str(self.source_root), 'target': str(self.target_root), 'dry_run': not self.execute, 'items': self.items, 'summary': summary}
    ...
    def run(self) -> Dict[str, Any]:
        if not self.source_root.is_dir():
            self.record('source', self.source_root, None, 'error', 'Source directory does not exist')
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob('*.md')):
            self.record('slash-commands', commands_dir, None, 'skipped', 'Claude slash commands have no direct Duck Agent equivalent — consider converting them into skills')
    ...
    def _load_claude_settings(self) -> Dict[str, Any]:
        ...
        if not path.exists():
            self.record('settings', None, None, 'skipped', 'No settings.json found')
            return {}
        try:
            data = json.loads(read_text(path))
        except (json.JSONDecodeError, OSError) as exc:
            self.record('settings', path, None, 'error', f'Could not parse settings.json: {exc}')
            return {}
        if not isinstance(data, dict):
            self.record('settings', path, None, 'error', 'settings.json is not a JSON object')
            return {}
        return data
    ...
    def _claude_mcp_servers(self, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if claude_json.exists():
            try:
                ...
                self.record('mcp-servers', claude_json, None, 'error', f'Could not parse .claude.json: {exc}')
    ...
    def _load_codex_config(self) -> Dict[str, Any]:
        ...
        if not path.exists():
            self.record('config', None, None, 'skipped', 'No config.toml found')
            return {}
        try:
            import tomllib
            data = tomllib.loads(read_text(path))
        except Exception as exc:
            self.record('config', path, None, 'error', f'Could not parse config.toml: {exc}')
            return {}
    ...
    def import_context_file(self, source: Path, kind: str) -> None:
        ...
        if not source.exists():
            self.record(kind, None, destination, 'skipped', f'No {source.name} found')
            return
        try:
            incoming = extract_markdown_entries(read_text(source))
        except OSError as exc:
            self.record(kind, source, destination, 'error', f'Could not read file: {exc}')
            return
        if not incoming:
    ...
    def import_memories_dir(self, memories_dir: Path) -> None:
        ...
        if not memories_dir.is_dir():
            self.record('memories', None, destination, 'skipped', 'No memories directory found')
            return
        incoming: List[str] = []
        for md_file in sorted(memories_dir.glob('*.md')):
            try:
                ...
                self.record('memories', md_file, destination, 'error', f'Could not read file: {exc}')
        if not incoming:
            self.record('memories', memories_dir, destination, 'skipped', 'No importable entries found')
            return
        self._merge_memory_entries('memories', memories_dir, destination, incoming)

    def _merge_memory_entries(self, kind: str, source: Path, destination: Path, incoming: List[str]) -> None:
        existing = parse_existing_memory_entries(destination)
        merged, stats = merge_entries(existing, incoming, MEMORY_CHAR_LIMIT)
        details = {'existing_entries': stats['existing'], 'added_entries': stats['added'], 'duplicate_entries': stats['duplicates'], 'overflowed_entries': stats['overflowed']}
        if stats['added'] == 0:
        ...
        if self.execute:
            ...
            try:
                ...
                self.record(kind, source, destination, 'error', f'Could not back up existing memory file: {exc}', **details)
                return
            ...
            try:
                ...
                self.record(kind, source, destination, 'error', f'Could not write merged memory file: {exc}', **details)
                return
            self.record(kind, source, destination, 'imported', **details)
        else:
            self.record(kind, source, destination, 'imported', 'Would merge entries', **details)
    ...
    def import_permission_allowlist(self, settings: Dict[str, Any]) -> None:
        ...
        if not isinstance(allow, list) or not allow:
            self.record('command-allowlist', None, destination, 'skipped', 'No permissions.allow rules found')
            return
        patterns: List[str] = []
        ...
        if not patterns:
            self.record('command-allowlist', None, destination, 'skipped', 'No Bash(...) allow rules to import', unmapped_rules=skipped_rules)
            return
        config = self.load_target_config('command-allowlist', 'settings.json permissions.allow', destination)
        if config is None:
            return
        ...
        if not added:
            self.record('command-allowlist', 'settings.json permissions.allow', destination, 'skipped', 'All patterns already present')
            return
        ...
        if self.execute:
            ...
            self.record('command-allowlist', 'settings.json permissions.allow', destination, 'imported', **details)
        else:
            self.record('command-allowlist', 'settings.json permissions.allow', destination, 'imported', 'Would merge patterns', **details)
    ...
    def import_permission_denylist(self, settings: Dict[str, Any]) -> None:
        ...
        if not isinstance(deny, list) or not deny:
            self.record('command-denylist', None, destination, 'skipped', 'No permissions.deny rules found')
            return
        patterns: List[str] = []
        for rule in deny:
            if not isinstance(rule, str):
        ...
        patterns = sorted(dict.fromkeys(patterns))
        if not patterns:
            self.record('command-denylist', None, destination, 'skipped', 'No Bash(...) deny rules to import')
            return
        config = self.load_target_config('command-denylist', 'settings.json permissions.deny', destination)
        if config is None:
            return
        ...
        added = [p for p in merged if p not in current]
        if not added:
            self.record('command-denylist', 'settings.json permissions.deny', destination, 'skipped', 'All patterns already present')
            return
        if self.execute:
            approvals['deny'] = merged
            config['approvals'] = approvals
            dump_yaml_file(destination, config)
            self.record('command-denylist', 'settings.json permissions.deny', destination, 'imported', added_patterns=added)
        else:
            self.record('command-denylist', 'settings.json permissions.deny', destination, 'imported', 'Would merge patterns', added_patterns=added)
    ...
    def import_mcp_servers(self, servers: Dict[str, Any], kind: str) -> None:
        ...
        for name, srv in servers.items():
            ...
            if name in existing and (not self.overwrite):
                self.record(kind, name, f'mcp_servers.{name}', 'conflict', 'MCP server already exists in Duck Agent config')
                continue
            ...
            if srv.get('command'):
                ...
                if env_stripped:
                    self.stripped_secrets.extend((f'mcp_servers.{name}.env.{k}' for k in env_stripped))
                if srv.get('cwd'):
            ...
            if srv.get('url'):
                ...
                if isinstance(headers, dict):
                    kept_headers = {k: v for k, v in headers.items() if not is_secret_key(str(k)) and 'authorization' not in str(k).lower()}
                    if kept_headers:
                        hermes_srv['headers'] = kept_headers
                    for k in headers:
                        if k not in kept_headers:
                            self.stripped_secrets.append(f'mcp_servers.{name}.headers.{k}')
            if not hermes_srv:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → DUCK_AGENT_HOME/skills/<category>/<name>."""
        ...
        for skill_dir in skill_dirs:
            ...
            if self.execute:
                ...
                self.record('skill', skill_dir, destination, 'imported', 'Would copy skill directory')

def import_agent_command(args) -> None:
    """Handle ``duck-agent import-agent`` (invoked from hermes_cli.main)."""
    from hermes_cli.config import get_config_path, load_config, save_config
    from hermes_constants import get_hermes_home
    from hermes_cli.setup import Colors, color, print_header, print_info, print_success, print_error, prompt_yes_no
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info('Specify one explicitly: duck-agent import-agent claude-code --source /path')
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info('Multiple agent setups detected: ' + ', '.join(detected))
            print_info('Pick one: duck-agent import-agent claude-code   or   duck-agent import-agent codex')
            return
        agent = detected[0]
    source_dir = Path(explicit_source) if explicit_source else default_source_dir(agent)
    print()
    print(color('┌─────────────────────────────────────────────────────────┐', Colors.MAGENTA))
    print(color('│          ⚕ Duck Agent — Import From Another Agent          │', Colors.MAGENTA))
    print(color('└─────────────────────────────────────────────────────────┘', Colors.MAGENTA))
    if not source_dir.is_dir():
        print()
        print_error(f'Agent directory not found: {source_dir}')
        print_info(f'Specify a custom path: duck-agent import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}')
        return
    ...
    print_info("Secrets:     never imported — run 'duck-agent setup' for credentials")
    config_path = get_config_path()
    if not config_path.exists():
        save_config(load_config())
    try:
        preview = AgentImporter(agent=agent, source_root=source_dir.resolve(), target_root=hermes_home.resolve(), execute=False, overwrite=overwrite).run()
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f'To execute, re-run with: duck-agent import-agent {agent} --yes')
            return
        if not prompt_yes_no('Proceed with import?', default=True):
            print_info('Import cancelled.')
            return
    try:
        report = AgentImporter(agent=agent, source_root=source_dir.resolve(), target_root=hermes_home.resolve(), execute=True, overwrite=overwrite).run()
    except Exception as e:
        print()
    ...
    print_info("API keys and credentials were NOT imported — run 'duck-agent setup' to configure providers, or add them to ~/.duck-agent/.env.")
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    groups = (('imported', Colors.GREEN, '✓ Would import' if dry_run else '✓ Imported'), ('conflict', Colors.YELLOW, '⚠ Conflicts (skipped — use --overwrite to force)'), ('skipped', Colors.DIM, '─ Skipped'), ('error', Colors.RED, '✗ Errors'))
    ...
    if stripped:
        ...
        for name in stripped:
            print(f'      {name}')
        print_info("Re-add credentials deliberately via 'duck-agent setup' or ~/.duck-agent/.env.")
        print()
    parts = []
