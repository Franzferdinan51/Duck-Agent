...
_HEX32 = re.compile('[0-9a-f]{32}\\Z')
_HEX16 = re.compile('[0-9a-f]{16}\\Z')
_HEX64 = re.compile('[0-9a-f]{64}\\Z')
_MAX_JSON = 1024 * 1024
_MAX_LOG = 512 * 1024
_OPEN_REPARSE_POINT = 2097152
_DELETE_ON_CLOSE = 67108864
_MOVE_REPLACE_EXISTING = 1
_MOVE_WRITE_THROUGH = 8
...
def _allowed_sids():
    ...
    return {win32security.ConvertSidToStringSid(_current_sid()), win32security.ConvertSidToStringSid(_system_sid())}
...
def _verify_security(handle) -> None:
    ...
    allow_types = {win32security.ACCESS_ALLOWED_ACE_TYPE, win32security.ACCESS_ALLOWED_OBJECT_ACE_TYPE, getattr(win32security, 'ACCESS_ALLOWED_CALLBACK_ACE_TYPE', 9), getattr(win32security, 'ACCESS_ALLOWED_CALLBACK_OBJECT_ACE_TYPE', 11)}
...
def _ensure_directory(path: Path) -> None:
    ...
    if not path.exists():
        try:
            ...
            if exc.winerror != 183:
                raise
    handle = _open(path, win32con.GENERIC_READ | win32con.READ_CONTROL, win32con.OPEN_EXISTING, win32con.FILE_FLAG_BACKUP_SEMANTICS | _OPEN_REPARSE_POINT, win32con.FILE_SHARE_READ | win32con.FILE_SHARE_WRITE | win32con.FILE_SHARE_DELETE)
    win32file.CloseHandle(handle)
...
def upload_token(ownership_id: str, spawn_nonce: str, token: bytes) -> dict[str, Any]:
    ...
    try:
        handle = _open(path, win32con.GENERIC_WRITE | win32con.READ_CONTROL, win32con.CREATE_NEW, win32con.FILE_ATTRIBUTE_NORMAL | _OPEN_REPARSE_POINT)
        try:
...
def read_token(path_value: str) -> str:
    ...
    if len(relative.parts) != 2 or not _HEX32.fullmatch(relative.parts[0]) or (not re.fullmatch('[0-9a-f]{16}\\.token', relative.parts[1])):
    ...
    try:
        handle = _open(path, win32con.GENERIC_READ | win32con.READ_CONTROL | win32con.DELETE, win32con.OPEN_EXISTING, flags)
...
def read_lock(ownership_id: str) -> dict[str, Any] | None:
    ...
    try:
        handle = _open(path, win32con.GENERIC_READ | win32con.READ_CONTROL, win32con.OPEN_EXISTING, win32con.FILE_ATTRIBUTE_NORMAL | _OPEN_REPARSE_POINT, win32con.FILE_SHARE_READ)
    except pywintypes.error as exc:
...
def write_lock(ownership_id: str, payload: dict[str, Any]) -> None:
    ...
    handle = _open(temporary, win32con.GENERIC_WRITE | win32con.READ_CONTROL, win32con.CREATE_NEW, win32con.FILE_ATTRIBUTE_NORMAL | _OPEN_REPARSE_POINT)
    try:
        win32file.WriteFile(handle, data)
        win32file.FlushFileBuffers(handle)
    finally:
        win32file.CloseHandle(handle)
    win32file.MoveFileEx(str(temporary), str(_lock_path(ownership_id)), _MOVE_REPLACE_EXISTING | _MOVE_WRITE_THROUGH)

def remove_artifact(path: Path) -> bool:
    _, pywintypes, _, win32con, win32file, _, _ = _win32()
    try:
        handle = _open(path, win32con.DELETE | win32con.READ_CONTROL, win32con.OPEN_EXISTING, win32con.FILE_ATTRIBUTE_NORMAL | _OPEN_REPARSE_POINT | _DELETE_ON_CLOSE)
    except pywintypes.error as exc:
        if exc.winerror in (2, 3):
            return False
        raise
...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    try:
        actual_creation = int(process.create_time() * 1000000000)
    ...
    if actual_creation != creation_time_ns:
        return {'alive': False, 'owned': False, 'indeterminate': False, 'reason': 'creation-time', 'actualCreationTimeNs': str(actual_creation), 'expectedCreationTimeNs': str(creation_time_ns)}
    if not argv:
    ...
    launches_module = argv[1:3] == ['-m', 'hermes_cli.main'] or (len(argv) > 2 and argv[1] == '-c' and ('hermes_cli.main' in argv[2]))
    ...
    try:
        ...
        owned = False
    return {'alive': process.is_running(), 'owned': owned, 'indeterminate': False, 'creationTimeNs': str(actual_creation), 'reason': 'owned' if owned else 'argv', 'argv': argv[:20], 'expectedExecutable': expected}
...
def terminate_owned(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> bool:
    ...
    import psutil
    process = psutil.Process(pid)
    if int(process.create_time() * 1000000000) != creation_time_ns:
        return False
    process.terminate()
    try:
        process.wait(5)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    query = "import sys,json,os,importlib.util as u;s=u.find_spec('hermes_cli');root=os.path.dirname(os.path.dirname(s.origin)) if s and s.origin else '';print(json.dumps({'base':getattr(sys,'_base_executable','') or sys.executable,'path':[p for p in sys.path if p],'root':root}))"
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not os.path.isabs(configured_path):
        raise ValueError('Duck Agent path must be absolute')
    ...
    if not os.path.isfile(python_entry):
        raise ValueError('Duck Agent Python runtime was not found')
    base_python, sys_path = _resolve_direct_interpreter(python_entry)
    bootstrap = f"import sys,runpy;sys.path[:0]={sys_path!r};runpy.run_module('hermes_cli.main',run_name='__main__',alter_sys=True)"
    args = [base_python, '-c', bootstrap]
    if profile:
        args.extend(['--profile', profile])
    args.extend(['serve', '--isolated', '--host', '127.0.0.1', '--port', '0', '--ssh-session-token-file', token_path, '--ssh-owner-nonce', spawn_nonce])
    env = dict(os.environ)
    env['VIRTUAL_ENV'] = os.path.dirname(venv_dir)
    env.pop('PYTHONPATH', None)
    _ensure_scope(ownership_id)
    creationflags = 8 | 512 | 16777216
    _, _, _, win32con, win32file, _, _ = _win32()
    log_handle = _open(log_path, win32con.GENERIC_WRITE | win32con.READ_CONTROL, win32con.CREATE_NEW, win32con.FILE_ATTRIBUTE_NORMAL | _OPEN_REPARSE_POINT, win32con.FILE_SHARE_READ | win32con.FILE_SHARE_WRITE)
    import msvcrt
    log_fd = msvcrt.open_osfhandle(int(log_handle), os.O_WRONLY)
    with os.fdopen(log_fd, 'wb', buffering=0) as log_stream:
        process = subprocess.Popen(args, stdin=subprocess.DEVNULL, stdout=log_stream, stderr=log_stream, close_fds=True, creationflags=creationflags, env=env)
    creation_time_ns = int(__import__('psutil').Process(process.pid).create_time() * 1000000000)
    return {'pid': process.pid, 'creationTimeNs': str(creation_time_ns), 'logPath': str(log_path), 'tokenPath': token_path}
...
def inspect_hermes(hermes_path: str) -> dict[str, Any]:
    ...
    if not os.path.isabs(hermes_path) or not os.path.isfile(path):
        raise ValueError('Duck Agent path is not an executable file')
    ...
    return {'path': path, 'version': (version.stdout + version.stderr).splitlines()[0] if version.returncode == 0 else '', 'supported': '--ssh-session-token-file' in help_text and '--ssh-owner-nonce' in help_text}
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == 'write-lock' and len(argv) == 2:
        write_lock(argv[1], _read_json_stdin())
        return {'ok': True}
    ...
    if operation == 'read-log' and len(argv) == 3:
        ...
        _, pywintypes, _, win32con, win32file, _, _ = _win32()
        try:
            handle = _open(path, win32con.GENERIC_READ | win32con.READ_CONTROL, win32con.OPEN_EXISTING, win32con.FILE_ATTRIBUTE_NORMAL | _OPEN_REPARSE_POINT, win32con.FILE_SHARE_READ | win32con.FILE_SHARE_WRITE)
        except pywintypes.error as exc:
            if exc.winerror in (2, 3):
                return {'content': ''}
            raise
