...
The Duck Agent docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.duck-agent/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = 'https://duck-agent.nousresearch.com/docs/api/model-catalog.json'
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = ('https://raw.githubusercontent.com/NousResearch/duck-agent/main/website/static/api/model-catalog.json',)
DEFAULT_TTL_HOURS = 1
...
def _load_catalog_config() -> dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raw = {}
    return {'enabled': bool(raw.get('enabled', True)), 'url': str(raw.get('url') or DEFAULT_CATALOG_URL), 'ttl_hours': float(raw.get('ttl_hours') or DEFAULT_TTL_HOURS), 'providers': raw.get('providers') if isinstance(raw.get('providers'), dict) else {}}
...
def _fetch_manifest_with_fallback(primary_url: str, timeout: float, fallback_urls: tuple[str, ...]=DEFAULT_CATALOG_FALLBACK_URLS) -> dict[str, Any] | None:
...
def get_catalog(*, force_refresh: bool=False) -> dict[str, Any]:
    ...
    if not force_refresh and _catalog_cache is not None and (disk_data is not None) and (disk_mtime == _catalog_cache_source_mtime) and disk_fresh:
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Duck Agent silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`duck-agent update` paths;
...
def seed_cache_from_checkout(project_root: 'Path | str') -> bool:
    ...
    ``duck-agent update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.duck-agent/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``duck-agent model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
