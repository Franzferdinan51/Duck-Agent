...
def summarize_holders(entries: list[dict[str, Any]]) -> str:
    ...
    held = ', '.join((f'{surface} x{n}' if n > 1 else surface for surface, n in sorted(counts.items(), key=lambda kv: (-kv[1], kv[0]))))
...
def active_session_limit_message(active_count: int, max_sessions: int, entries: Optional[list[dict[str, Any]]]=None) -> str:
    held = summarize_holders(entries or [])
    detail = f' Held by: {held}.' if held else ''
    return f'Duck Agent is at the active session limit ({active_count}/{max_sessions}).{detail} Try again when another session finishes.'
...
def _prune_dead(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [entry for entry in entries if _pid_alive(entry.get('pid'), entry.get('process_start_time'))]
...
def try_acquire_active_session(*, session_id: str, surface: str, config: Any, metadata: Optional[dict[str, Any]]=None) -> tuple[Optional[ActiveSessionLease], Optional[str]]:
    ...
    if max_sessions is None:
        return (ActiveSessionLease(lease_id=lease_id, session_id=session_id, surface=surface, enabled=False), None)
    now = time.time()
    entry = {'lease_id': lease_id, 'session_id': str(session_id), 'surface': str(surface), 'pid': os.getpid(), 'process_start_time': _process_start_time(os.getpid()), 'started_at': now, 'updated_at': now}
    if metadata:
        entry['metadata'] = {str(k): v for k, v in metadata.items() if isinstance(k, str)}
    ...
    with _FileLock(_lock_path()):
        ...
        if active_count >= max_sessions:
            ...
            logger.info('Active session limit reached: active=%d max=%d surface=%s', active_count, max_sessions, surface)
            return (None, active_session_limit_message(active_count, max_sessions, entries))
        entries.append(entry)
        _write_entries(state_path, entries)
    return (ActiveSessionLease(lease_id=lease_id, session_id=str(session_id), surface=str(surface)), None)
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    try:
        with _FileLock(_lock_path()):
            ...
            kept = [entry for entry in entries if str(entry.get('lease_id') or '') != lease.lease_id]
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``duck-agent dashboard`` / ``serve``) never trips that
    ...
    with _FileLock(_lock_path()):
        entries = _prune_dead(_read_entries(state_path))
        kept = [entry for entry in entries if entry.get('pid') != pid or str(entry.get('lease_id') or '') in live_lease_ids]
