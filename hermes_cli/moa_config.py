...
DEFAULT_MOA_REFERENCE_MODELS: list[dict[str, str]] = [{'provider': 'openai-codex', 'model': 'gpt-5.5'}, {'provider': 'openrouter', 'model': 'deepseek/deepseek-v4-pro'}]
DEFAULT_MOA_AGGREGATOR: dict[str, str] = {'provider': 'openrouter', 'model': 'anthropic/claude-opus-4.8'}
...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model Duck Agent agent
...
def _default_preset() -> dict[str, Any]:
    return {'reference_models': _default_reference_models(), 'aggregator': deepcopy(DEFAULT_MOA_AGGREGATOR), 'reference_temperature': None, 'aggregator_temperature': None, 'reference_timeout': DEFAULT_MOA_REFERENCE_TIMEOUT, 'degraded_reference_policy': 'loud', 'max_tokens': 4096, 'reference_max_tokens': None, 'fanout': 'user_turn', 'enabled': True}
...
def _normalize_preset(raw: Any) -> dict[str, Any]:
    ...
    return {'enabled': _coerce_bool(raw.get('enabled'), True), 'reference_models': refs, 'aggregator': aggregator, 'reference_temperature': _coerce_float_or_none(raw.get('reference_temperature')), 'aggregator_temperature': _coerce_float_or_none(raw.get('aggregator_temperature')), 'reference_timeout': _coerce_reference_timeout(raw.get('reference_timeout')), 'degraded_reference_policy': _coerce_degraded_reference_policy(raw.get('degraded_reference_policy')), 'max_tokens': _coerce_int(raw.get('max_tokens'), 4096), 'reference_max_tokens': _coerce_int_or_none(raw.get('reference_max_tokens')), 'fanout': _coerce_fanout(raw.get('fanout'))}
...
def normalize_moa_config(raw: Any) -> dict[str, Any]:
    ...
    return {'default_preset': default_name, 'active_preset': active_name, 'presets': presets, 'reference_models': deepcopy(active['reference_models']), 'aggregator': deepcopy(active['aggregator']), 'reference_temperature': active['reference_temperature'], 'aggregator_temperature': active['aggregator_temperature'], 'reference_timeout': active['reference_timeout'], 'degraded_reference_policy': active['degraded_reference_policy'], 'max_tokens': active['max_tokens'], 'reference_max_tokens': active.get('reference_max_tokens'), 'fanout': active.get('fanout', 'user_turn'), 'enabled': active['enabled'], 'privacy_filter': coerce_privacy_filter(raw.get('privacy_filter'))}
...
def resolve_moa_preset(config: Any, name: str | None=None) -> dict[str, Any]:
    ...
    if preset is None:
        ...
        raise MoAPresetNotFoundError(f"MoA preset '{preset_name}' was not found. Available presets: {available}. Run `duck-agent moa list`.")
...
def encode_moa_turn(prompt: str, config: Any=None, preset: str | None=None) -> str:
    ...
    payload = {'prompt': str(prompt or ''), 'config': resolve_moa_preset(config or {}, preset)}
    encoded = base64.urlsafe_b64encode(json.dumps(payload, separators=(',', ':'), ensure_ascii=False).encode('utf-8')).decode('ascii')
