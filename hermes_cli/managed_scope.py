...
A system-level directory (default ``/etc/duck-agent``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.duck-agent/config.yaml`` and ``~/.duck-agent/.env`` on a per-leaf-key basis.
...
_DEFAULT_MANAGED_DIR = Path('/etc/duck-agent')
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/duck-agent`` during tests so a real
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      ...
      2. ``/etc/duck-agent`` — POSIX default, when it exists. Ignored under pytest so
...
def _cached_read(path: Path, cache: Dict[str, tuple], parse):
    ...
    try:
        ...
        logger.warning('managed scope: failed to parse %s: %s — IGNORING this managed file. Admin policy from this file is NOT being applied. Fix and restart.', path, exc)
        return None
    with _CACHE_LOCK:
...
def load_managed_config() -> dict:
    ...
    if managed_dir is None:
        return {}
    parsed = _cached_read(managed_dir / 'config.yaml', _CONFIG_CACHE, lambda f: yaml.safe_load(f) or {})
