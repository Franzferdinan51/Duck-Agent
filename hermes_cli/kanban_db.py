...
``<root>`` is the **shared Duck Agent root** (the parent of any active
...
``duck-agent -p <profile>`` joins the same board as the dispatcher that
...
the profile's Duck Agent home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``duck-agent kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.duck-agent``. In Docker / custom
deployments where ``DUCK_AGENT_HOME`` points outside ``~/.duck-agent`` (e.g.
``/opt/duck-agent``), ``<root>`` is ``DUCK_AGENT_HOME``. Legacy env-var
...
coding workloads.  See ``docs/duck-agent-kanban-v1-spec.pdf`` for the full
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active DUCK_AGENT_HOME so dispatcher- and
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar('hermes_kanban_current_board_override', default=None)
...
_BOARD_SLUG_RE = re.compile('^[a-z0-9][a-z0-9\\-_]{0,63}$')
...
def _normalize_board_slug(slug: Optional[str]) -> Optional[str]:
    ...
    if not _BOARD_SLUG_RE.match(s):
        raise ValueError(f"invalid board slug {slug!r}: must be 1-64 chars, lowercase alphanumerics / hyphens / underscores, not starting with '-' or '_'")
    return s

def kanban_home() -> Path:
    """Return the shared Duck Agent root that anchors the kanban board.
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``DUCK_AGENT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``DUCK_AGENT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``DUCK_AGENT_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``duck-agent kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``duck-agent kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``duck-agent kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str]=None) -> Path:
    ...
    board — makes ``duck-agent kanban log`` unambiguous even when multiple
...
def read_board_metadata(board: Optional[str]=None) -> dict:
    ...
    """
    slug = _normalize_board_slug(board) or DEFAULT_BOARD
    meta: dict[str, Any] = {'slug': slug, 'name': _default_board_display_name(slug), 'description': '', 'icon': '', 'color': '', 'default_workdir': None, 'project_id': None, 'created_at': None, 'archived': False}
    try:
...
def write_board_metadata(board: Optional[str], *, name: Optional[str]=None, description: Optional[str]=None, icon: Optional[str]=None, color: Optional[str]=None, archived: Optional[bool]=None, default_workdir: Optional[str]=None, project_id: Optional[str]=None) -> dict:
...
def create_board(slug: str, *, name: Optional[str]=None, description: Optional[str]=None, icon: Optional[str]=None, color: Optional[str]=None, default_workdir: Optional[str]=None, project_id: Optional[str]=None) -> dict:
    ...
    if not normed:
        raise ValueError('board slug is required')
    meta = write_board_metadata(normed, name=name, description=description, icon=icon, color=color, default_workdir=default_workdir, project_id=project_id)
    ...
    def from_row(cls, row: sqlite3.Row) -> 'Task':
        ...
        if 'skills' in keys and row['skills']:
            try:
                ...
                skills_value = None
        return cls(id=row['id'], title=row['title'], body=row['body'], assignee=row['assignee'], status=row['status'], priority=row['priority'], created_by=row['created_by'], created_at=row['created_at'], started_at=row['started_at'], completed_at=row['completed_at'], workspace_kind=row['workspace_kind'], workspace_path=row['workspace_path'], branch_name=row['branch_name'] if 'branch_name' in keys else None, project_id=row['project_id'] if 'project_id' in keys else None, claim_lock=row['claim_lock'], claim_expires=row['claim_expires'], tenant=row['tenant'] if 'tenant' in keys else None, result=row['result'] if 'result' in keys else None, idempotency_key=row['idempotency_key'] if 'idempotency_key' in keys else None, consecutive_failures=row['consecutive_failures'] if 'consecutive_failures' in keys else row['spawn_failures'] if 'spawn_failures' in keys else 0, worker_pid=row['worker_pid'] if 'worker_pid' in keys else None, last_failure_error=row['last_failure_error'] if 'last_failure_error' in keys else row['last_spawn_error'] if 'last_spawn_error' in keys else None, max_runtime_seconds=row['max_runtime_seconds'] if 'max_runtime_seconds' in keys else None, last_heartbeat_at=row['last_heartbeat_at'] if 'last_heartbeat_at' in keys else None, current_run_id=row['current_run_id'] if 'current_run_id' in keys else None, workflow_template_id=row['workflow_template_id'] if 'workflow_template_id' in keys else None, current_step_key=row['current_step_key'] if 'current_step_key' in keys else None, skills=skills_value, model_override=row['model_override'] if 'model_override' in keys and row['model_override'] else None, provider_override=row['provider_override'] if 'provider_override' in keys and row['provider_override'] else None, reasoning_effort=row['reasoning_effort'] if 'reasoning_effort' in keys and row['reasoning_effort'] else None, max_retries=row['max_retries'] if 'max_retries' in keys else None, goal_mode=bool(row['goal_mode']) if 'goal_mode' in keys and row['goal_mode'] else False, goal_max_turns=row['goal_max_turns'] if 'goal_max_turns' in keys and row['goal_max_turns'] else None, session_id=row['session_id'] if 'session_id' in keys else None, block_kind=row['block_kind'] if 'block_kind' in keys and row['block_kind'] else None, block_recurrences=int(row['block_recurrences']) if 'block_recurrences' in keys and row['block_recurrences'] is not None else 0)
...
class Run:
    ...
    def from_row(cls, row: sqlite3.Row) -> 'Run':
        try:
            ...
            meta = None
        return cls(id=int(row['id']), task_id=row['task_id'], profile=row['profile'], step_key=row['step_key'], status=row['status'], claim_lock=row['claim_lock'], claim_expires=row['claim_expires'], worker_pid=row['worker_pid'], max_runtime_seconds=row['max_runtime_seconds'], last_heartbeat_at=row['last_heartbeat_at'], started_at=int(row['started_at']), ended_at=int(row['ended_at']) if row['ended_at'] is not None else None, outcome=row['outcome'], summary=row['summary'], metadata=meta, error=row['error'])
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    conn = connect_tracked(path, connect_fn=sqlite3.connect, isolation_level=None, timeout=busy_timeout_ms / 1000.0)
        ...
        if _IS_WINDOWS:
            ...
            while True:
                try:
                    ...
                    time.sleep(_INIT_LOCK_POLL_SECONDS)
        if not acquired:
            _log.warning('kanban init lock for %s not acquired within %.0fs — proceeding without the cross-process lock (in-process lock + idempotent init are the correctness backstop). A stuck holder is no longer able to block this connect indefinitely (#36644).', lock_path, _INIT_LOCK_TIMEOUT_SECONDS)
        yield
    finally:
        try:
            if acquired:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``duck-agent gateway run --replace`` /
...
def _maybe_checkpoint_wal(conn: sqlite3.Connection, db_path: Path) -> None:
    ...
    try:
        ...
        _log.debug('kanban WAL checkpoint (TRUNCATE) on %s -> %s (busy, wal_frames, checkpointed_frames)', key, tuple(row) if row is not None else None)
...
def _looks_like_tls_record_at(data: bytes, offset: int) -> bool:
    ...
    return content_type in {20, 21, 22, 23} and major == 3 and (minor in {0, 1, 2, 3, 4}) and (0 < length <= 18432)
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    raise sqlite3.DatabaseError(f"file is not a database: invalid SQLite header for {path}{signature}; first_32={head[:32].hex(' ')}")
...
class KanbanDbCorruptError(RuntimeError):
    ...
    def __init__(self, db_path: Path, backup_path: Optional[Path], reason: str):
        ...
        super().__init__(f'Refusing to open corrupt kanban DB at {db_path}: {reason}. Original preserved; backup at {backup_str}.')
...
def _prune_corrupt_backups(parent: Path, base_name: str, keep: Optional[Path]=None) -> None:
    ...
    """
    try:
        backups = [candidate for candidate in parent.glob(f'{base_name}.corrupt.*.bak') if candidate.is_file() and candidate != keep]
    except OSError:
        return
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    if has_live_connection(resolved):
        _log.error("refusing to quarantine %s: a connection to it is still open in this process, and fingerprinting the file would cancel that connection's POSIX locks. Close all connections first.", resolved)
        return None
    ...
    for suffix in ('-wal', '-shm'):
        ...
        try:
            ...
            pass
    return candidate
_REPAIRABLE_INDEX_ERROR_PATTERNS = (re.compile('^wrong # of entries in index (?P<index>.+)$'), re.compile('^row \\d+ missing from index (?P<index>.+)$'))
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Duck Agent treats as user-controlled-but-trusted on the
    ...
    try:
        ...
        if not _integrity_messages_ok(messages):
            reason = f"integrity_check returned {(messages[0] if messages else '<no row>')!r}"
    ...
    if index_names:
        _log.warning('kanban DB %s failed integrity_check with index-only errors (%s); pre-repair backup at %s — attempting REINDEX auto-repair.', resolved, ', '.join(index_names), backup if backup is not None else '<backup failed>')
        repaired, post = _attempt_index_reindex_repair(resolved, index_names)
        if repaired:
            _log.warning('kanban DB %s auto-repaired via REINDEX (%s); integrity_check now clean. Pre-repair copy kept at %s.', resolved, ', '.join(index_names), backup if backup is not None else '<backup failed>')
            return
        reason = f"{reason}; REINDEX auto-repair attempted but integrity_check still returned {(post[0] if post else '<no row>')!r}"
...
def repair_db(db_path: Optional[Path]=None, *, board: Optional[str]=None) -> RepairResult:
    ...
    :class:`RepairResult` so ``duck-agent kanban repair`` can report and choose
    its own exit code.
    ...
    with _cross_process_init_lock(resolved):
        ...
        try:
            ...
            return RepairResult(status='corrupt', db_path=resolved, messages=[f'sqlite refused to open file: {exc}'], backup_path=_backup_corrupt_db(resolved))
        ...
        return RepairResult(status='repaired' if repaired else 'corrupt', db_path=resolved, messages=messages, post_repair_messages=post, backup_path=backup, reindexed=index_names)
...
def init_db(db_path: Optional[Path]=None, *, board: Optional[str]=None) -> Path:
    ...
    Kept as a public entry point so CLI ``duck-agent kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if 'consecutive_failures' not in cols:
        added = _add_column_if_missing(conn, 'tasks', 'consecutive_failures', 'consecutive_failures INTEGER NOT NULL DEFAULT 0')
    ...
    if 'last_failure_error' not in cols:
        added = _add_column_if_missing(conn, 'tasks', 'last_failure_error', 'last_failure_error TEXT')
    ...
    if 'max_runtime_seconds' not in cols:
        _add_column_if_missing(conn, 'tasks', 'max_runtime_seconds', 'max_runtime_seconds INTEGER')
    if 'last_heartbeat_at' not in cols:
        _add_column_if_missing(conn, 'tasks', 'last_heartbeat_at', 'last_heartbeat_at INTEGER')
    ...
    if 'workflow_template_id' not in cols:
        _add_column_if_missing(conn, 'tasks', 'workflow_template_id', 'workflow_template_id TEXT')
    if 'current_step_key' not in cols:
        _add_column_if_missing(conn, 'tasks', 'current_step_key', 'current_step_key TEXT')
    ...
    if 'reasoning_effort' not in cols:
        _add_column_if_missing(conn, 'tasks', 'reasoning_effort', 'reasoning_effort TEXT')
    if 'goal_mode' not in cols:
        _add_column_if_missing(conn, 'tasks', 'goal_mode', 'goal_mode INTEGER NOT NULL DEFAULT 0')
    if 'goal_max_turns' not in cols:
        _add_column_if_missing(conn, 'tasks', 'goal_max_turns', 'goal_max_turns INTEGER')
    ...
    if 'block_recurrences' not in cols:
        _add_column_if_missing(conn, 'tasks', 'block_recurrences', 'block_recurrences INTEGER NOT NULL DEFAULT 0')
    ...
    notify_table_exists = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='kanban_notify_subs'").fetchone() is not None
    if notify_table_exists:
        notify_cols = {row['name'] for row in conn.execute('PRAGMA table_info(kanban_notify_subs)')}
        if 'notifier_profile' not in notify_cols:
            _add_column_if_missing(conn, 'kanban_notify_subs', 'notifier_profile', 'notifier_profile TEXT')
        if 'chat_type' not in notify_cols:
            _add_column_if_missing(conn, 'kanban_notify_subs', 'chat_type', 'chat_type TEXT')
        if 'delivery_metadata' not in notify_cols:
            _add_column_if_missing(conn, 'kanban_notify_subs', 'delivery_metadata', 'delivery_metadata TEXT')
    runs_exist = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='task_runs'").fetchone() is not None
    if runs_exist:
        with write_txn(conn):
            inflight = conn.execute("SELECT id, assignee, claim_lock, claim_expires, worker_pid,        max_runtime_seconds, last_heartbeat_at, started_at FROM tasks WHERE status = 'running' AND current_run_id IS NULL").fetchall()
            for row in inflight:
                started = row['started_at'] or int(time.time())
                cur = conn.execute("\n                    INSERT INTO task_runs (\n                        task_id, profile, status,\n                        claim_lock, claim_expires, worker_pid,\n                        max_runtime_seconds, last_heartbeat_at,\n                        started_at\n                    ) VALUES (?, ?, 'running', ?, ?, ?, ?, ?, ?)\n                    ", (row['id'], row['assignee'], row['claim_lock'], row['claim_expires'], row['worker_pid'], row['max_runtime_seconds'], row['last_heartbeat_at'], started))
                upd = conn.execute('UPDATE tasks SET current_run_id = ? WHERE id = ? AND current_run_id IS NULL', (cur.lastrowid, row['id']))
                if upd.rowcount != 1:
                    conn.execute("UPDATE task_runs SET status = 'reclaimed',     outcome = 'reclaimed', ended_at = ? WHERE id = ?", (int(time.time()), cur.lastrowid))
    _EVENT_RENAMES = (('ready', 'promoted'), ('priority', 'reprioritized'), ('spawn_auto_blocked', 'gave_up'))
    for old, new in _EVENT_RENAMES:
        conn.execute('UPDATE task_events SET kind = ? WHERE kind = ?', (new, old))
    _rebuild_drifted_tables(conn)
_REBUILD_SPECS = {'task_events': ('CREATE TABLE task_events ( id INTEGER PRIMARY KEY AUTOINCREMENT, task_id TEXT NOT NULL, run_id INTEGER, kind TEXT NOT NULL, payload TEXT, created_at INTEGER NOT NULL)', ('CREATE INDEX idx_events_task ON task_events(task_id, created_at)', 'CREATE INDEX idx_events_run ON task_events(run_id, id)')), 'task_comments': ('CREATE TABLE task_comments ( id INTEGER PRIMARY KEY AUTOINCREMENT, task_id TEXT NOT NULL, author TEXT NOT NULL, body TEXT NOT NULL, created_at INTEGER NOT NULL)', ('CREATE INDEX idx_comments_task ON task_comments(task_id, created_at)',)), 'task_runs': ('CREATE TABLE task_runs ( id INTEGER PRIMARY KEY AUTOINCREMENT, task_id TEXT NOT NULL, profile TEXT, step_key TEXT, status TEXT NOT NULL, claim_lock TEXT, claim_expires INTEGER, worker_pid INTEGER, max_runtime_seconds INTEGER, last_heartbeat_at INTEGER, started_at INTEGER NOT NULL, ended_at INTEGER, outcome TEXT, summary TEXT, metadata TEXT, error TEXT)', ('CREATE INDEX idx_runs_task ON task_runs(task_id, started_at)', 'CREATE INDEX idx_runs_status ON task_runs(status)')), 'kanban_notify_subs': ("CREATE TABLE kanban_notify_subs ( task_id TEXT NOT NULL, platform TEXT NOT NULL, chat_id TEXT NOT NULL, chat_type TEXT, thread_id TEXT NOT NULL DEFAULT '', user_id TEXT, notifier_profile TEXT, delivery_metadata TEXT, created_at INTEGER NOT NULL, last_event_id INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (task_id, platform, chat_id, thread_id))", ('CREATE INDEX idx_notify_task ON kanban_notify_subs(task_id)',))}
    ...
    try:
        for table in drifted:
            ...
            if table == 'kanban_notify_subs':
                ...
                conn.execute(f'INSERT INTO {table} ({cols_csv}, last_event_id) SELECT {cols_csv}, COALESCE(CAST(last_event_id AS INTEGER), 0) FROM {table}_legacy')
            else:
                shared = [c for c in old_cols if c in new_cols and c != 'id']
                cols_csv = ', '.join(shared)
                conn.execute(f'INSERT INTO {table} ({cols_csv}) SELECT {cols_csv} FROM {table}_legacy')
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    if ok is False:
        raise sqlite3.DatabaseError('torn-extend detected: the database file is shorter than its header page count claims')
_BUSY_MAX_RETRIES = 5
_BUSY_RETRY_MIN_S = 0.02
_BUSY_RETRY_MAX_S = 0.15

def _is_busy_error(exc: BaseException) -> bool:
    return isinstance(exc, sqlite3.OperationalError) and ('database is locked' in str(exc).lower() or 'database is busy' in str(exc).lower())
...
def create_task(conn: sqlite3.Connection, *, title: str, body: Optional[str]=None, assignee: Optional[str]=None, created_by: Optional[str]=None, workspace_kind: str='scratch', workspace_path: Optional[str]=None, branch_name: Optional[str]=None, tenant: Optional[str]=None, priority: int=0, parents: Iterable[str]=(), triage: bool=False, idempotency_key: Optional[str]=None, max_runtime_seconds: Optional[int]=None, skills: Optional[Iterable[str]]=None, max_retries: Optional[int]=None, model_override: Optional[str]=None, provider_override: Optional[str]=None, reasoning_effort: Optional[str]=None, goal_mode: bool=False, goal_max_turns: Optional[int]=None, initial_status: str='running', session_id: Optional[str]=None, board: Optional[str]=None, project_id: Optional[str]=None, project_source_task_id: Optional[str]=None) -> str:
    ...
    each name to ``duck-agent --skills ...``. Use this to pin a task to a
    ...
    if workspace_kind not in VALID_WORKSPACE_KINDS:
        raise ValueError(f'workspace_kind must be one of {sorted(VALID_WORKSPACE_KINDS)}, got {workspace_kind!r}')
    ...
    if project_id:
        ...
        if project_obj is None and project_source_task_id:
            ...
            if source_task is not None and source_task.project_id == project_id and (source_task.workspace_kind == 'worktree') and source_task.workspace_path:
                source_path = Path(source_task.workspace_path)
                if source_path.is_absolute() and source_path.name == source_task.id and (source_path.parent.name == '.worktrees'):
                    project_slug = None
                    ...
                    if project_slug:
                        ...
                        project_obj = _pdb.Project(id=project_id, slug=project_slug, name=project_slug, created_at=0, primary_path=project_repo)
    ...
    if skills is not None:
        ...
        for s in skills:
            ...
            if not name:
                continue
            if ',' in name:
                raise ValueError(f'skill name cannot contain comma: {name!r} (pass a list of separate names instead of a comma-joined string)')
        ...
        if toolset_typos:
            ...
            raise ValueError(f"{quoted} {noun}, not skill name(s). Put toolsets in the assignee profile's `toolsets:` config instead of per-task skills. Skills are named skill bundles (e.g. `blogwatcher`, `github-code-review`); toolsets are runtime capabilities (e.g. `web`, `browser`, `terminal`).")
        skills_list = cleaned
    if idempotency_key:
        row = conn.execute("SELECT id FROM tasks WHERE idempotency_key = ? AND status != 'archived' ORDER BY created_at DESC LIMIT 1", (idempotency_key,)).fetchone()
        if row:
            return row['id']
    now = int(time.time())
            ...
            with write_txn(conn):
                if initial_status == 'blocked':
                    ...
                    if parents:
                        ...
                        if missing:
                            raise ValueError(f"unknown parent task(s): {', '.join(missing)}")
                        rows = conn.execute('SELECT status FROM tasks WHERE id IN (' + ','.join('?' * len(parents)) + ')', parents).fetchall()
                ...
                if project_obj is not None and workspace_kind == 'worktree':
                    ...
                    if not branch_name:
                        try:
                            branch_name = _pdb.branch_name_for(project_obj, task_id, title=title or '')
                        except Exception:
                            branch_name = None
                conn.execute('\n                    INSERT INTO tasks (\n                        id, title, body, assignee, status, priority,\n                        created_by, created_at, workspace_kind, workspace_path,\n                        branch_name, project_id, tenant, idempotency_key,\n                        max_runtime_seconds,\n                        skills, max_retries, model_override, provider_override,\n                        reasoning_effort,\n                        goal_mode, goal_max_turns, session_id\n                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n                    ', (task_id, title.strip(), body, assignee, task_status, priority, created_by, now, workspace_kind, workspace_path, branch_name, project_id, tenant, idempotency_key, int(max_runtime_seconds) if max_runtime_seconds is not None else None, json.dumps(skills_list) if skills_list is not None else None, int(max_retries) if max_retries is not None else None, model_override, provider_override, reasoning_effort, 1 if goal_mode else 0, int(goal_max_turns) if goal_max_turns is not None else None, session_id))
                for pid in parents:
                    conn.execute('INSERT OR IGNORE INTO task_links (parent_id, child_id) VALUES (?, ?)', (pid, task_id))
                _append_event(conn, task_id, 'created', {'assignee': assignee, 'status': task_status, 'parents': list(parents), 'tenant': tenant, 'workspace_kind': workspace_kind, 'workspace_path': workspace_path, 'branch_name': branch_name, 'project_id': project_id, 'skills': list(skills_list) if skills_list else None, 'goal_mode': bool(goal_mode) or None, 'model_override': model_override, 'provider_override': provider_override})
...
def _find_missing_parents(conn: sqlite3.Connection, parents: Iterable[str]) -> list[str]:
    ...
    rows = conn.execute(f'SELECT id FROM tasks WHERE id IN ({placeholders})', parents).fetchall()
    present = {r['id'] for r in rows}
    return [p for p in parents if p not in present]

def _inherit_notify_subs(conn: sqlite3.Connection, child_id: str, parents: Iterable[str], *, created_at: Optional[int]=None) -> None:
    ...
    if not parent_ids:
        return
    row = conn.execute('SELECT COALESCE(MAX(id), 0) AS cursor FROM task_events WHERE task_id = ?', (child_id,)).fetchone()
...
def assign_task(conn: sqlite3.Connection, task_id: str, profile: Optional[str]) -> bool:
    ...
    with write_txn(conn):
        row = conn.execute('SELECT status, claim_lock, assignee FROM tasks WHERE id = ?', (task_id,)).fetchone()
        if not row:
            return False
        if row['claim_lock'] is not None and row['status'] == 'running':
            raise RuntimeError(f'cannot reassign {task_id}: currently running (claimed). Wait for completion or reclaim the stale lock first.')
        if row['assignee'] != profile:
            conn.execute('UPDATE tasks SET assignee = ?, consecutive_failures = 0, last_failure_error = NULL WHERE id = ?', (profile, task_id))
        else:
            conn.execute('UPDATE tasks SET assignee = ? WHERE id = ?', (profile, task_id))
        _append_event(conn, task_id, 'assigned', {'assignee': profile})
        return True

def set_model_override(conn: sqlite3.Connection, task_id: str, model: Optional[str], provider: Optional[str]=None) -> bool:
    ...
    with write_txn(conn):
        ...
        conn.execute('UPDATE tasks SET model_override = ?, provider_override = ? WHERE id = ?', (model, provider, task_id))
        _append_event(conn, task_id, 'model_override_set', {'model': model, 'provider': provider})
        return True
...
def link_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> None:
    ...
    with write_txn(conn):
        ...
        conn.execute('INSERT OR IGNORE INTO task_links (parent_id, child_id) VALUES (?, ?)', (parent_id, child_id))
        parent_status = conn.execute('SELECT status FROM tasks WHERE id = ?', (parent_id,)).fetchone()['status']
...
def _would_cycle(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    while stack:
        ...
        if node in seen:
            continue
        seen.add(node)
        rows = conn.execute('SELECT child_id FROM task_links WHERE parent_id = ?', (node,)).fetchall()
        stack.extend((r['child_id'] for r in rows))
    return False

def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    with write_txn(conn):
        cur = conn.execute('DELETE FROM task_links WHERE parent_id = ? AND child_id = ?', (parent_id, child_id))
        if cur.rowcount:
...
def add_comment(conn: sqlite3.Connection, task_id: str, author: str, body: str) -> int:
    ...
    with write_txn(conn):
        ...
        cur = conn.execute('INSERT INTO task_comments (task_id, author, body, created_at) VALUES (?, ?, ?, ?)', (task_id, author.strip(), body.strip(), now))
...
def list_comments(conn: sqlite3.Connection, task_id: str) -> list[Comment]:
    ...
    return [Comment(id=r['id'], task_id=r['task_id'], author=r['author'], body=r['body'], created_at=r['created_at']) for r in rows]

def list_comments_after(conn: sqlite3.Connection, task_id: str, *, after_id: int=0) -> list[Comment]:
    ...
    """
    rows = conn.execute('SELECT id, task_id, author, body, created_at FROM task_comments WHERE task_id = ? AND id > ? ORDER BY id ASC', (task_id, int(after_id))).fetchall()
    return [Comment(id=r['id'], task_id=r['task_id'], author=r['author'], body=r['body'], created_at=r['created_at']) for r in rows]
...
def store_attachment_bytes(conn: sqlite3.Connection, task_id: str, filename: str, data: bytes, *, content_type: Optional[str]=None, uploaded_by: Optional[str]=None, board: Optional[str]=None, max_bytes: Optional[int]=None) -> int:
    ...
    (``duck-agent kanban attach``) so name-sanitisation, the size cap, and the
    ...
    try:
        return add_attachment(conn, task_id, filename=dest_path.name, stored_path=str(dest_path.resolve()), content_type=content_type, size=len(data), uploaded_by=uploaded_by)
    except Exception:
        try:
...
def add_attachment(conn: sqlite3.Connection, task_id: str, *, filename: str, stored_path: str, content_type: Optional[str]=None, size: int=0, uploaded_by: Optional[str]=None) -> int:
    ...
    with write_txn(conn):
        if not conn.execute('SELECT 1 FROM tasks WHERE id = ?', (task_id,)).fetchone():
            raise ValueError(f'unknown task {task_id}')
        cur = conn.execute('INSERT INTO task_attachments (task_id, filename, stored_path, content_type, size, uploaded_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', (task_id, filename.strip(), stored_path, content_type, int(size), uploaded_by, now))
        _append_event(conn, task_id, 'attached', {'filename': filename.strip(), 'size': int(size), 'by': uploaded_by})
        return int(cur.lastrowid or 0)

def list_attachments(conn: sqlite3.Connection, task_id: str) -> list[Attachment]:
    rows = conn.execute('SELECT * FROM task_attachments WHERE task_id = ? ORDER BY created_at ASC, id ASC', (task_id,)).fetchall()
    return [Attachment(id=r['id'], task_id=r['task_id'], filename=r['filename'], stored_path=r['stored_path'], content_type=r['content_type'], size=r['size'] or 0, uploaded_by=r['uploaded_by'], created_at=r['created_at']) for r in rows]
...
def get_attachment(conn: sqlite3.Connection, attachment_id: int) -> Optional[Attachment]:
    ...
    if r is None:
        return None
    return Attachment(id=r['id'], task_id=r['task_id'], filename=r['filename'], stored_path=r['stored_path'], content_type=r['content_type'], size=r['size'] or 0, uploaded_by=r['uploaded_by'], created_at=r['created_at'])
...
def list_events(conn: sqlite3.Connection, task_id: str) -> list[Event]:
    rows = conn.execute('SELECT * FROM task_events WHERE task_id = ? ORDER BY created_at ASC, id ASC', (task_id,)).fetchall()
    out = []
    for r in rows:
        try:
            ...
            payload = None
        out.append(Event(id=r['id'], task_id=r['task_id'], kind=r['kind'], payload=payload, created_at=r['created_at'], run_id=int(r['run_id']) if 'run_id' in r.keys() and r['run_id'] is not None else None))
    return out

def _append_event(conn: sqlite3.Connection, task_id: str, kind: str, payload: Optional[dict]=None, *, run_id: Optional[int]=None) -> None:
    ...
    conn.execute('INSERT INTO task_events (task_id, run_id, kind, payload, created_at) VALUES (?, ?, ?, ?, ?)', (task_id, run_id, kind, pl, now))

def _end_run(conn: sqlite3.Connection, task_id: str, *, outcome: str, summary: Optional[str]=None, error: Optional[str]=None, metadata: Optional[dict]=None, status: Optional[str]=None) -> Optional[int]:
    ...
    existed (e.g. a CLI user calling ``duck-agent kanban complete`` on a
    ...
    conn.execute('\n        UPDATE task_runs\n           SET status        = ?,\n               outcome       = ?,\n               summary       = ?,\n               error         = ?,\n               metadata      = ?,\n               ended_at      = ?,\n               claim_lock    = NULL,\n               claim_expires = NULL,\n               worker_pid    = NULL\n         WHERE id = ?\n           AND ended_at IS NULL\n        ', (status or outcome, outcome, summary, error, json.dumps(metadata, ensure_ascii=False) if metadata else None, now, run_id))
...
def _synthesize_ended_run(conn: sqlite3.Connection, task_id: str, *, outcome: str, summary: Optional[str]=None, error: Optional[str]=None, metadata: Optional[dict]=None) -> int:
    """Insert a zero-duration, already-closed run row.

    Used when a terminal transition happens on a task that was never
    claimed (CLI user calling ``duck-agent kanban complete <ready-task>
    ...
    """
    now = int(time.time())
    trow = conn.execute('SELECT assignee, current_step_key FROM tasks WHERE id = ?', (task_id,)).fetchone()
    profile = trow['assignee'] if trow else None
    step_key = trow['current_step_key'] if trow else None
    cur = conn.execute('\n        INSERT INTO task_runs (\n            task_id, profile, step_key,\n            status, outcome,\n            summary, error, metadata,\n            started_at, ended_at\n        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n        ', (task_id, profile, step_key, outcome, outcome, summary, error, json.dumps(metadata, ensure_ascii=False) if metadata else None, now, now))
    return int(cur.lastrowid or 0)
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``duck-agent kanban block <id>``).  This is a deliberate handoff that
    ...
    * **Circuit-breaker** — ``_record_task_failure`` tripped after
    ...
    for that path.
    """
    row = conn.execute("SELECT kind FROM task_events WHERE task_id = ? AND kind IN ('blocked', 'unblocked') ORDER BY id DESC LIMIT 1", (task_id,)).fetchone()
...
def recompute_ready(conn: sqlite3.Connection, failure_limit: int=None) -> int:
    ...
    promoted = 0
    with write_txn(conn):
        todo_rows = conn.execute("SELECT id, status, consecutive_failures, max_retries FROM tasks WHERE status IN ('todo', 'blocked')").fetchall()
        for row in todo_rows:
            task_id = row['id']
            cur_status = row['status']
            if cur_status == 'blocked' and _has_sticky_block(conn, task_id):
                continue
            parents = conn.execute('SELECT t.status FROM tasks t JOIN task_links l ON l.parent_id = t.id WHERE l.child_id = ?', (task_id,)).fetchall()
...
def claim_task(conn: sqlite3.Connection, task_id: str, *, ttl_seconds: Optional[int]=None, claimer: Optional[str]=None) -> Optional[Task]:
    ...
    with write_txn(conn):
        undone = conn.execute("SELECT 1 FROM task_links l JOIN tasks p ON p.id = l.parent_id WHERE l.child_id = ? AND p.status NOT IN ('done', 'archived') LIMIT 1", (task_id,)).fetchone()
        if undone:
            ...
            return None
        stale = conn.execute("SELECT current_run_id FROM tasks WHERE id = ? AND status = 'ready'", (task_id,)).fetchone()
        if stale and stale['current_run_id']:
            conn.execute("\n                UPDATE task_runs\n                   SET status = 'reclaimed', outcome = 'reclaimed',\n                       summary = COALESCE(summary, 'invariant recovery on re-claim'),\n                       ended_at = ?,\n                       claim_lock = NULL, claim_expires = NULL, worker_pid = NULL\n                 WHERE id = ? AND ended_at IS NULL\n                ", (now, int(stale['current_run_id'])))
        cur = conn.execute("\n            UPDATE tasks\n               SET status        = 'running',\n                   claim_lock    = ?,\n                   claim_expires = ?,\n                   started_at    = COALESCE(started_at, ?)\n             WHERE id = ?\n               AND status = 'ready'\n               AND claim_lock IS NULL\n            ", (lock, expires, now, task_id))
        if cur.rowcount != 1:
            return None
        trow = conn.execute('SELECT assignee, max_runtime_seconds, current_step_key FROM tasks WHERE id = ?', (task_id,)).fetchone()
        run_cur = conn.execute("\n            INSERT INTO task_runs (\n                task_id, profile, step_key, status,\n                claim_lock, claim_expires, max_runtime_seconds,\n                started_at\n            ) VALUES (?, ?, ?, 'running', ?, ?, ?, ?)\n            ", (task_id, trow['assignee'] if trow else None, trow['current_step_key'] if trow else None, lock, expires, trow['max_runtime_seconds'] if trow else None, now))
        run_id = run_cur.lastrowid
        conn.execute('UPDATE tasks SET current_run_id = ? WHERE id = ?', (run_id, task_id))
        _append_event(conn, task_id, 'claimed', {'lock': lock, 'expires': expires, 'run_id': run_id}, run_id=run_id)
        claimed = get_task(conn, task_id)
    _fire_kanban_lifecycle_hook('kanban_task_claimed', task_id, board=get_current_board(), assignee=claimed.assignee if claimed else None, run_id=run_id)
    return claimed
...
def claim_review_task(conn: sqlite3.Connection, task_id: str, *, ttl_seconds: Optional[int]=None, claimer: Optional[str]=None) -> Optional[Task]:
    ...
    with write_txn(conn):
        ...
        run_id = run_cur.lastrowid
        conn.execute('UPDATE tasks SET current_run_id = ? WHERE id = ?', (run_id, task_id))
        _append_event(conn, task_id, 'claimed', {'lock': lock, 'expires': expires, 'run_id': run_id, 'source_status': 'review'}, run_id=run_id)
...
def heartbeat_claim(conn: sqlite3.Connection, task_id: str, *, ttl_seconds: Optional[int]=None, claimer: Optional[str]=None) -> bool:
    ...
    lock = claimer or _claimer_id()
    with write_txn(conn):
        cur = conn.execute("UPDATE tasks SET claim_expires = ? WHERE id = ? AND status = 'running' AND claim_lock = ?", (expires, task_id, lock))
        if cur.rowcount == 1:
...
def release_stale_claims(conn: sqlite3.Connection, *, signal_fn=None) -> int:
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    stale = conn.execute("SELECT id, claim_lock, worker_pid, claim_expires, last_heartbeat_at FROM tasks WHERE status = 'running' AND claim_expires IS NOT NULL   AND claim_expires < ?", (now,)).fetchall()
    for row in stale:
        ...
        if host_local and row['worker_pid'] and _pid_alive(row['worker_pid']) and (not heartbeat_stale):
            ...
            with write_txn(conn):
                cur = conn.execute("UPDATE tasks SET claim_expires = ? WHERE id = ? AND status = 'running'   AND claim_lock IS ?   AND claim_expires IS NOT NULL   AND claim_expires < ?", (new_expires, row['id'], row['claim_lock'], now))
                if cur.rowcount != 1:
                    continue
                run_id = _current_run_id(conn, row['id'])
                if run_id is not None:
                    conn.execute('UPDATE task_runs SET claim_expires = ? WHERE id = ?', (new_expires, run_id))
                _append_event(conn, row['id'], 'claim_extended', {'reason': 'pid_alive', 'worker_pid': int(row['worker_pid']), 'claim_lock': row['claim_lock'], 'claim_expires_was': int(row['claim_expires']), 'claim_expires_now': new_expires, 'last_heartbeat_at': int(row['last_heartbeat_at']) if row['last_heartbeat_at'] is not None else None}, run_id=run_id)
            continue
        termination = _terminate_reclaimed_worker(row['worker_pid'], row['claim_lock'], signal_fn=signal_fn)
        if _worker_survived_termination(termination):
            _defer_reclaim_for_live_worker(conn, row['id'], row['claim_lock'], now, termination, reason='ttl_expired_worker_alive')
            continue
        with write_txn(conn):
            cur = conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL WHERE id = ? AND status = 'running' AND claim_lock IS ? AND claim_expires IS NOT NULL AND claim_expires < ?", (row['id'], row['claim_lock'], now))
            if cur.rowcount != 1:
                continue
            run_id = _end_run(conn, row['id'], outcome='reclaimed', status='reclaimed', error=f"stale_lock={row['claim_lock']}", metadata=termination)
            payload = {'stale_lock': row['claim_lock'], 'worker_pid': int(row['worker_pid']) if row['worker_pid'] is not None else None, 'claim_expires': int(row['claim_expires']), 'last_heartbeat_at': int(row['last_heartbeat_at']) if row['last_heartbeat_at'] is not None else None, 'now': now, 'host_local': host_local, 'heartbeat_stale': bool(heartbeat_stale)}
            payload.update(termination)
            _append_event(conn, row['id'], 'reclaimed', payload, run_id=run_id)
            reclaimed += 1
    return reclaimed
...
def reclaim_task(conn: sqlite3.Connection, task_id: str, *, reason: Optional[str]=None, signal_fn=None) -> bool:
    ...
    with write_txn(conn):
        cur = conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL WHERE id = ? AND status IN ('running', 'ready', 'blocked') AND claim_lock IS ?", (task_id, prev_lock))
        if cur.rowcount != 1:
            return False
        run_id = _end_run(conn, task_id, outcome='reclaimed', status='reclaimed', error=f'manual_reclaim: {reason}' if reason else f'manual_reclaim lock={prev_lock}', metadata=termination)
...
def _verify_created_cards(conn: sqlite3.Connection, completing_task_id: str, claimed_ids: Iterable[str]) -> tuple[list[str], list[str]]:
    ...
    * The card is linked as a ``task_links.child`` of the completing
    ...
    rows = conn.execute(f'SELECT id, created_by FROM tasks WHERE id IN ({placeholders})', tuple(ordered)).fetchall()
...
_TASK_ID_PROSE_RE = re.compile('\\bt_[a-f0-9]{8,}\\b')
...
def _scan_prose_for_phantom_ids(conn: sqlite3.Connection, text: str) -> list[str]:
    ...
    rows = conn.execute(f'SELECT id FROM tasks WHERE id IN ({placeholders})', tuple(unique)).fetchall()
...
class HallucinatedCardsError(ValueError):
    ...
    def __init__(self, phantom: list[str], completing_task_id: str):
        ...
        super().__init__(f"completion blocked: claimed created_cards that do not exist or were not created by this worker: {', '.join(phantom)}")
...
def complete_task(conn: sqlite3.Connection, task_id: str, *, result: Optional[str]=None, summary: Optional[str]=None, metadata: Optional[dict]=None, created_cards: Optional[Iterable[str]]=None, expected_run_id: Optional[int]=None) -> bool:
    ...
    completion (``duck-agent kanban complete <id>``) works without requiring
    ...
    if created_cards:
        ...
        if phantom_cards:
            with write_txn(conn):
                _append_event(conn, task_id, 'completion_blocked_hallucination', {'phantom_cards': phantom_cards, 'verified_cards': verified_cards, 'summary_preview': (summary or result or '').strip().splitlines()[0][:200] if summary or result else None})
    ...
    with write_txn(conn):
        ...
        run_id = _end_run(conn, task_id, outcome='completed', status='done', summary=summary if summary is not None else result, metadata=metadata)
        if run_id is None and (summary or metadata or result):
            run_id = _synthesize_ended_run(conn, task_id, outcome='completed', summary=summary if summary is not None else result, metadata=metadata)
        ev_summary = (summary if summary is not None else result) or ''
        ev_summary = ev_summary.strip().splitlines()[0][:400] if ev_summary else ''
        completed_payload: dict = {'result_len': len(result) if result else 0, 'summary': ev_summary or None}
        if verified_cards:
...
def _merge_completion_prose_artifacts(conn: sqlite3.Connection, task_id: str, metadata: Optional[dict], *, summary: Optional[str], result: Optional[str]) -> Optional[dict]:
    ...
    discovered: list[str] = []
    for match in re.finditer(prefix + '(?:[/\\\\][^\\s`\\"\'<>]+)', text):
...
def _persist_scratch_completion_artifacts(conn: sqlite3.Connection, task_id: str, metadata: dict) -> None:
    ...
    for item in raw_artifacts:
        ...
        if not src.is_file():
            _discard_copies()
            raise ArtifactPreservationError(f'declared scratch artifact is unavailable or not a regular file: {artifact}')
        size = resolved_src.stat().st_size
        if size > KANBAN_ATTACHMENT_MAX_BYTES:
            _discard_copies()
            raise ArtifactPreservationError(f'declared scratch artifact exceeds the {KANBAN_ATTACHMENT_MAX_BYTES}-byte limit: {artifact}')
        ...
        try:
            ...
            with resolved_src.open('rb') as source_file, dest.open('xb') as destination_file:
                ...
                while (chunk := source_file.read(1024 * 1024)):
                    ...
                    if copied > KANBAN_ATTACHMENT_MAX_BYTES:
                        raise ArtifactPreservationError(f'declared scratch artifact grew beyond the size limit: {artifact}')
            ...
            if isinstance(exc, ArtifactPreservationError):
                raise
            raise ArtifactPreservationError(f'could not preserve declared scratch artifact {artifact}: {exc}') from exc
        used_destinations.add(dest)
        persisted.append(str(dest.resolve()))
        changed = True
    if changed:
        metadata['artifacts'] = persisted
        metadata['_staged_artifacts'] = [path for path in persisted if path.startswith(str(attachment_dir.resolve()))]

def _insert_completion_attachment(conn: sqlite3.Connection, task_id: str, *, filename: str, stored_path: str, size: int, created_at: int) -> None:
    """Record a worker-produced artifact in the existing attachment table."""
    conn.execute("INSERT INTO task_attachments (task_id, filename, stored_path, content_type, size, uploaded_by, created_at) VALUES (?, ?, ?, NULL, ?, 'kanban_complete', ?)", (task_id, filename, stored_path, size, created_at))
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Duck Agent' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Duck Agent-managed storage. A board ``default_workdir`` pointing at a
...
def _cleanup_workspace(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        if kind != 'scratch' or not path:
            ...
            return
        _active_children = conn.execute("SELECT 1 FROM task_links l JOIN tasks t ON t.id = l.child_id WHERE l.parent_id = ? AND t.status NOT IN ('done', 'archived', 'failed', 'cancelled') LIMIT 1", (task_id,)).fetchone()
        if _active_children:
            _log.debug('Deferring scratch workspace cleanup for task %s: active children still need workspace at %s', task_id, path)
            return
        import shutil
        wp = Path(path)
        if wp.is_dir():
            if _is_managed_scratch_path(wp):
                ...
                _log.warning("Refusing to remove out-of-scratch workspace for task %s: %s (workspace_kind='scratch' but path is outside any kanban-managed workspaces root)", task_id, wp)
...
def _try_cleanup_parent_workspaces(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        for parent_id, in parents:
            row = conn.execute('SELECT workspace_kind, workspace_path FROM tasks WHERE id = ?', (parent_id,)).fetchone()
            if not row or row['workspace_kind'] != 'scratch' or (not row['workspace_path']):
                continue
            active = conn.execute("SELECT 1 FROM task_links l JOIN tasks t ON t.id = l.child_id WHERE l.parent_id = ? AND t.status NOT IN ('done', 'archived', 'failed', 'cancelled') LIMIT 1", (parent_id,)).fetchone()
            if active:
                continue
            import shutil
...
def _cleanup_worker_tmux(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        out = subprocess.run(['tmux', 'list-panes', '-t', session, '-F', '#{pane_dead}'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
_SCRATCH_TIP_MESSAGE = "scratch workspaces are ephemeral — they're deleted when the task completes. Use --workspace worktree: (git worktree) or --workspace dir:/abs/path (existing dir) to preserve worker output."
...
def _maybe_emit_scratch_tip(conn: sqlite3.Connection, task_id: str, workspace_kind: Optional[str]) -> None:
    ...
    try:
        ...
        with write_txn(conn):
            _append_event(conn, task_id, 'tip_scratch_workspace', {'message': _SCRATCH_TIP_MESSAGE})
    except Exception:
        pass
    finally:
...
def edit_completed_task_result(conn: sqlite3.Connection, task_id: str, *, result: str, summary: Optional[str]=None, metadata: Optional[dict]=None) -> bool:
    ...
    with write_txn(conn):
        ...
        if run_id is None:
            run_id = _synthesize_ended_run(conn, task_id, outcome='completed', summary=handoff_summary, metadata=metadata)
        else:
        ...
        ev_summary = handoff_summary.strip().splitlines()[0][:400] if handoff_summary else ''
        _append_event(conn, task_id, 'edited', {'fields': ['result', 'summary'] + (['metadata'] if metadata is not None else []), 'result_len': len(result) if result else 0, 'summary': ev_summary or None}, run_id=run_id)
    return True
...
def block_task(conn: sqlite3.Connection, task_id: str, *, reason: Optional[str]=None, kind: Optional[str]=None, expected_run_id: Optional[int]=None) -> bool:
    ...
    * ``transient`` — treated like a generic block for routing, but a worker
    ...
    recurrences = 0
    with write_txn(conn):
        cur_row = conn.execute('SELECT status, block_kind, block_recurrences FROM tasks WHERE id = ?', (task_id,)).fetchone()
        if cur_row is None:
            return False
        prev_kind = cur_row['block_kind'] if 'block_kind' in cur_row.keys() else None
        prev_recurrences = int(cur_row['block_recurrences']) if 'block_recurrences' in cur_row.keys() and cur_row['block_recurrences'] is not None else 0
        if kind == 'dependency':
            cur = conn.execute("\n                UPDATE tasks\n                   SET status        = 'todo',\n                       claim_lock    = NULL,\n                       claim_expires = NULL,\n                       worker_pid    = NULL,\n                       block_kind    = ?\n                 WHERE id = ?\n                   AND status IN ('running', 'ready')\n                " + ('' if expected_run_id is None else ' AND current_run_id = ?'), (kind, task_id) if expected_run_id is None else (kind, task_id, int(expected_run_id)))
            if cur.rowcount != 1:
                return False
            ...
            _append_event(conn, task_id, 'dependency_wait', {'reason': reason, 'kind': kind}, run_id=run_id)
            _blocked_task = get_task(conn, task_id)
            _fire_kanban_lifecycle_hook('kanban_task_blocked', task_id, board=get_current_board(), assignee=_blocked_task.assignee if _blocked_task else None, run_id=run_id, reason=reason)
            return True
        same_cause = prev_kind == kind
        recurrences = prev_recurrences + 1 if same_cause else 1
        if recurrences >= BLOCK_RECURRENCE_LIMIT:
            cur = conn.execute("\n                UPDATE tasks\n                   SET status        = 'triage',\n                       claim_lock    = NULL,\n                       claim_expires = NULL,\n                       worker_pid    = NULL,\n                       block_kind    = ?,\n                       block_recurrences = ?\n                 WHERE id = ?\n                   AND status IN ('running', 'ready')\n                " + ('' if expected_run_id is None else ' AND current_run_id = ?'), (kind, recurrences, task_id) if expected_run_id is None else (kind, recurrences, task_id, int(expected_run_id)))
            if cur.rowcount != 1:
                return False
            run_id = _end_run(conn, task_id, outcome='blocked', status='blocked', summary=reason)
            if run_id is None and reason:
                run_id = _synthesize_ended_run(conn, task_id, outcome='blocked', summary=reason)
            _append_event(conn, task_id, 'block_loop_detected', {'reason': reason, 'kind': kind, 'recurrences': recurrences, 'limit': BLOCK_RECURRENCE_LIMIT}, run_id=run_id)
        else:
            if expected_run_id is None:
            ...
            if run_id is None and reason:
                run_id = _synthesize_ended_run(conn, task_id, outcome='blocked', summary=reason)
            _append_event(conn, task_id, 'blocked', {'reason': reason, 'kind': kind, 'recurrences': recurrences}, run_id=run_id)
        _blocked_task = get_task(conn, task_id)
    _fire_kanban_lifecycle_hook('kanban_task_blocked', task_id, board=get_current_board(), assignee=_blocked_task.assignee if _blocked_task else None, run_id=run_id, reason=reason)
    return True
...
def promote_task(conn: sqlite3.Connection, task_id: str, *, actor: str, reason: Optional[str]=None, force: bool=False, dry_run: bool=False) -> tuple[bool, Optional[str]]:
    ...
    if cur_status not in ('todo', 'blocked'):
        return (False, f"task {task_id} is {cur_status!r}; promote only applies to 'todo' or 'blocked'")
    if not force:
        parents = conn.execute('SELECT t.id, t.status FROM tasks t JOIN task_links l ON l.parent_id = t.id WHERE l.child_id = ?', (task_id,)).fetchall()
        unsatisfied = [p['id'] for p in parents if p['status'] not in ('done', 'archived')]
        if unsatisfied:
            return (False, f"unsatisfied parent dependencies: {', '.join(unsatisfied)} (use --force to override)")
    if dry_run:
        return (True, None)
    with write_txn(conn):
        upd = conn.execute("UPDATE tasks SET status = 'ready' WHERE id = ? AND status IN ('todo', 'blocked')", (task_id,))
        if upd.rowcount != 1:
...
def unblock_task(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    now = int(time.time())
    with write_txn(conn):
        stale = conn.execute("SELECT current_run_id FROM tasks WHERE id = ? AND status IN ('blocked', 'scheduled')", (task_id,)).fetchone()
        if stale and stale['current_run_id']:
            conn.execute("\n                UPDATE task_runs\n                   SET status = 'reclaimed', outcome = 'reclaimed',\n                       summary = COALESCE(summary, 'invariant recovery on unblock'),\n                       ended_at = ?,\n                       claim_lock = NULL, claim_expires = NULL, worker_pid = NULL\n                 WHERE id = ? AND ended_at IS NULL\n                ", (now, int(stale['current_run_id'])))
        undone_parents = conn.execute("SELECT 1 FROM task_links l JOIN tasks p ON p.id = l.parent_id WHERE l.child_id = ? AND p.status != 'done' LIMIT 1", (task_id,)).fetchone()
        new_status = 'todo' if undone_parents else 'ready'
        cur = conn.execute("UPDATE tasks SET status = ?, current_run_id = NULL, consecutive_failures = 0, last_failure_error = NULL WHERE id = ? AND status IN ('blocked', 'scheduled')", (new_status, task_id))
        if cur.rowcount != 1:
            return False
        _append_event(conn, task_id, 'unblocked', {'status': new_status} if new_status != 'ready' else None)
        return True
...
def specify_triage_task(conn: sqlite3.Connection, task_id: str, *, title: Optional[str]=None, body: Optional[str]=None, assignee: Optional[str]=None, author: Optional[str]=None) -> bool:
    ...
    assignee = _canonical_assignee(assignee)
    with write_txn(conn):
        existing = conn.execute("SELECT title, body, assignee FROM tasks WHERE id = ? AND status = 'triage'", (task_id,)).fetchone()
        if existing is None:
            return False
        ...
        cur = conn.execute(f"UPDATE tasks SET {', '.join(sets)} WHERE id = ? AND status = 'triage'", tuple(params))
        if cur.rowcount != 1:
            return False
        if changed_fields and author and author.strip():
            conn.execute('INSERT INTO task_comments (task_id, author, body, created_at) VALUES (?, ?, ?, ?)', (task_id, author.strip(), 'Specified — updated ' + ', '.join(changed_fields) + ' and promoted to todo.', int(time.time())))
        _append_event(conn, task_id, 'specified', {'changed_fields': changed_fields} if changed_fields else None)
    recompute_ready(conn)
    return True
...
def decompose_triage_task(conn: sqlite3.Connection, task_id: str, *, root_assignee: Optional[str], children: list[dict], author: Optional[str]=None, auto_promote: bool=True) -> Optional[list[str]]:
    ...
    child_ids: list[str] = []
    with write_txn(conn):
        root_row = conn.execute('SELECT id, status, tenant, workspace_kind, workspace_path FROM tasks WHERE id = ?', (task_id,)).fetchone()
        if root_row is None:
            return None
        ...
        for idx, child in enumerate(children):
            ...
            if child.get('workspace_path'):
                ...
                child_ws_path = None
            conn.execute("INSERT INTO tasks (id, title, body, assignee, status, workspace_kind,  workspace_path, tenant, created_at, created_by) VALUES (?, ?, ?, ?, 'todo', ?, ?, ?, ?, ?)", (new_id, title, body if isinstance(body, str) else None, assignee, child_ws_kind, child_ws_path, tenant, now, author or 'decomposer'))
            _append_event(conn, new_id, 'created', {'by': author or 'decomposer', 'from_decompose_of': task_id})
        ...
        for cid in child_ids:
            conn.execute('INSERT OR IGNORE INTO task_links (parent_id, child_id) VALUES (?, ?)', (cid, task_id))
        sets = ["status = 'todo'"]
        params: list[Any] = []
        if root_assignee is not None:
            sets.append('assignee = ?')
        ...
        if author and author.strip():
            conn.execute('INSERT INTO task_comments (task_id, author, body, created_at) VALUES (?, ?, ?, ?)', (task_id, author.strip(), 'Decomposed into ' + ', '.join(child_ids) + '. Root will wake when all children complete.', now))
        _append_event(conn, task_id, 'decomposed', {'child_ids': child_ids, 'root_assignee': root_assignee})
    if auto_promote:
        recompute_ready(conn)
    return child_ids
...
def archive_task(conn: sqlite3.Connection, task_id: str) -> bool:
    with write_txn(conn):
        cur = conn.execute("UPDATE tasks SET status = 'archived',     claim_lock = NULL, claim_expires = NULL, worker_pid = NULL WHERE id = ? AND status != 'archived'", (task_id,))
        if cur.rowcount != 1:
            return False
        run_id = _end_run(conn, task_id, outcome='reclaimed', status='reclaimed', summary='task archived with run still active')
...
def _git_toplevel(path: Path) -> Optional[Path]:
    ...
    try:
        result = subprocess.run(['git', '-C', str(path), 'rev-parse', '--show-toplevel'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, check=False)
    except Exception:
        return None
...
def _git_branch_exists(repo_root: Path, branch_name: str) -> bool:
    try:
        result = subprocess.run(['git', '-C', str(repo_root), 'show-ref', '--verify', f'refs/heads/{branch_name}'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, check=False)
    except Exception:
        return False
...
def _git_common_dir(path: Path) -> Optional[Path]:
    try:
        result = subprocess.run(['git', '-C', str(path), 'rev-parse', '--path-format=absolute', '--git-common-dir'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, check=False)
    except Exception:
        return None
    if result.returncode != 0:
        return None
...
def _git_dir(path: Path) -> Optional[Path]:
    try:
        result = subprocess.run(['git', '-C', str(path), 'rev-parse', '--path-format=absolute', '--git-dir'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, check=False)
    except Exception:
        return None
    if result.returncode != 0:
        return None
...
def _git_current_branch(path: Path) -> Optional[str]:
    try:
        result = subprocess.run(['git', '-C', str(path), 'branch', '--show-current'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, check=False)
    except Exception:
        return None
    if result.returncode != 0:
        return None
...
def _resolve_worktree_workspace(task: Task, *, board: Optional[str]=None) -> tuple[Path, str]:
    ...
    launched from, e.g. the Duck Agent checkout). If no anchor is configured
    ...
    if not task.workspace_path:
        ...
        if not board_default:
            raise ValueError(f'task {task.id} has workspace_kind=worktree but no workspace_path, and board {board_slug!r} has no default_workdir set. Set a board default workdir (a git repo) or create the task with --workspace worktree:<absolute-repo-path>.')
        anchor = Path(board_default).expanduser()
        if not anchor.is_absolute():
            raise ValueError(f'board {board_slug!r} default_workdir {board_default!r} is not absolute; use an absolute path to a git repo')
        repo_root = _git_toplevel(anchor)
        if repo_root is None:
            raise ValueError(f'task {task.id} has workspace_kind=worktree but board {board_slug!r} default_workdir {board_default!r} is not inside a git repo')
        target = repo_root / '.worktrees' / task.id
        _ensure_git_worktree(repo_root, target, branch_name)
        return (target, branch_name)
    requested = Path(task.workspace_path).expanduser()
    if not requested.is_absolute():
        raise ValueError(f'task {task.id} has non-absolute worktree path {task.workspace_path!r}; use an absolute path')
    ...
    if repo_root is None:
        raise ValueError(f'task {task.id} worktree path {task.workspace_path!r} is not inside a git repo and does not point at a git repo root')
...
def resolve_workspace(task: Task, *, board: Optional[str]=None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Duck Agent treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Duck Agent creates/reuses that linked worktree. With
      no ``workspace_path``, Duck Agent anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Duck Agent uses
      ``wt/<task-id>``.
    ...
    if kind == 'scratch':
        if task.workspace_path:
            ...
            if not p.is_absolute():
                raise ValueError(f'task {task.id} has non-absolute workspace_path {task.workspace_path!r}; workspace paths must be absolute')
        else:
    ...
    if kind == 'dir':
        ...
        p = Path(task.workspace_path).expanduser()
        if not p.is_absolute():
            raise ValueError(f"task {task.id} has non-absolute workspace_path {task.workspace_path!r}; use an absolute path (relative paths are ambiguous against the dispatcher's CWD)")
        p.mkdir(parents=True, exist_ok=True)
        return p
    if kind == 'worktree':
...
def set_branch_name(conn: sqlite3.Connection, task_id: str, branch_name: str) -> None:
    with write_txn(conn):
        conn.execute('UPDATE tasks SET branch_name = ? WHERE id = ?', (str(branch_name), task_id))
...
_RESPAWN_BLOCKER_RE = re.compile('\\b(quota|rate[\\s_\\-]?limit|429|403|auth\\w*|unauthorized|forbidden|billing|subscription|access[\\s_]denied|permission[\\s_]denied|invalid[\\s_]api[\\s_]key)\\b', re.IGNORECASE)
_RESPAWN_GUARD_SUCCESS_WINDOW = 3600
DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS = 300
_RESPAWN_GUARD_PR_WINDOW = 86400
_RESPAWN_GUARD_PR_URL_RE = re.compile('https?://github\\.com/[^/\\s]+/[^/\\s]+/pull/\\d+', re.IGNORECASE)
...
def _pid_alive(pid: Optional[int]) -> bool:
    ...
    **Zombie handling:** the existence check succeeds against zombie
    ...
    if sys.platform == 'linux':
        ...
        try:
            proc = subprocess.run(['ps', '-o', 'stat=', '-p', str(int(pid))], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True, encoding='utf-8', errors='replace', timeout=1, check=False)
...
def _terminate_reclaimed_worker(pid: Optional[int], claim_lock: Optional[str], *, signal_fn=None) -> dict[str, Any]:
    """Best-effort host-local worker termination for reclaim paths."""
    import signal
    info: dict[str, Any] = {'prev_pid': int(pid) if pid else None, 'host_local': False, 'termination_attempted': False, 'terminated': False, 'sigkill': False}
...
def _worker_survived_termination(termination: dict) -> bool:
    ...
    """
    return bool(termination.get('termination_attempted') and termination.get('host_local') and (not termination.get('terminated')))

def _defer_reclaim_for_live_worker(conn: sqlite3.Connection, task_id: str, claim_lock: Optional[str], now: int, termination: dict, *, reason: str) -> None:
    ...
    event so the hold is visible in ``duck-agent kanban tail``. The next dispatch
    ...
    with write_txn(conn):
        cur = conn.execute("UPDATE tasks SET claim_expires = ? WHERE id = ? AND status = 'running' AND claim_lock IS ?", (grace, task_id, claim_lock))
        if cur.rowcount != 1:
            return
        run_id = _current_run_id(conn, task_id)
        if run_id is not None:
...
def heartbeat_worker(conn: sqlite3.Connection, task_id: str, *, note: Optional[str]=None, expected_run_id: Optional[int]=None) -> bool:
    ...
    with write_txn(conn):
        if expected_run_id is None:
            cur = conn.execute("UPDATE tasks SET last_heartbeat_at = ? WHERE id = ? AND status = 'running'", (now, task_id))
        else:
            cur = conn.execute("UPDATE tasks SET last_heartbeat_at = ? WHERE id = ? AND status = 'running' AND current_run_id = ?", (now, task_id, int(expected_run_id)))
        if cur.rowcount != 1:
            return False
        run_id = int(expected_run_id) if expected_run_id is not None else _current_run_id(conn, task_id)
        if run_id is not None:
            conn.execute('UPDATE task_runs SET last_heartbeat_at = ? WHERE id = ?', (now, run_id))
        _append_event(conn, task_id, 'heartbeat', {'note': note} if note else None, run_id=run_id)
    return True
...
def enforce_max_runtime(conn: sqlite3.Connection, *, signal_fn=None) -> list[str]:
    ...
    now = int(time.time())
    host_prefix = f"{_claimer_id().split(':', 1)[0]}:"
    rows = conn.execute("SELECT t.id, t.worker_pid,        COALESCE(r.started_at, t.started_at) AS active_started_at,        t.max_runtime_seconds, t.claim_lock FROM tasks t LEFT JOIN task_runs r ON r.id = t.current_run_id WHERE t.status = 'running' AND t.max_runtime_seconds IS NOT NULL   AND COALESCE(r.started_at, t.started_at) IS NOT NULL   AND t.worker_pid IS NOT NULL").fetchall()
    for row in rows:
        lock = row['claim_lock'] or ''
        ...
        if kill is not None:
            ...
            if _pid_alive(pid):
                try:
                    ...
                    pass
        with write_txn(conn):
            cur = conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL, last_heartbeat_at = NULL WHERE id = ? AND status = 'running'   AND worker_pid = ? AND claim_lock IS ?", (tid, pid, row['claim_lock']))
            if cur.rowcount == 1:
                payload = {'pid': pid, 'elapsed_seconds': int(elapsed), 'limit_seconds': int(row['max_runtime_seconds']), 'sigkill': killed}
                run_id = _end_run(conn, tid, outcome='timed_out', status='timed_out', error=f"elapsed {int(elapsed)}s > limit {int(row['max_runtime_seconds'])}s", metadata=payload)
                _append_event(conn, tid, 'timed_out', payload, run_id=run_id)
                timed_out.append(tid)
        if cur.rowcount == 1:
            _record_task_failure(conn, tid, error=f"elapsed {int(elapsed)}s > limit {int(row['max_runtime_seconds'])}s", outcome='timed_out', release_claim=False, end_run=False, event_payload_extra={'pid': pid, 'sigkill': killed})
    return timed_out
...
def detect_stale_running(conn: sqlite3.Connection, *, stale_timeout_seconds: int=0, signal_fn=None) -> list[str]:
    ...
    now = int(time.time())
    reclaimed: list[str] = []
    rows = conn.execute("SELECT t.id, t.worker_pid, t.last_heartbeat_at, t.claim_lock,        COALESCE(r.started_at, t.started_at) AS active_started_at FROM tasks t LEFT JOIN task_runs r ON r.id = t.current_run_id WHERE t.status = 'running'").fetchall()
    for row in rows:
        ...
        if _worker_survived_termination(termination):
            _defer_reclaim_for_live_worker(conn, tid, lock, now, termination, reason='heartbeat_stale_worker_alive')
            continue
        with write_txn(conn):
            cur = conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL, last_heartbeat_at = NULL WHERE id = ? AND status = 'running'   AND claim_lock IS ?", (tid, row['claim_lock']))
            if cur.rowcount != 1:
                continue
            payload = {'elapsed_seconds': int(elapsed), 'last_heartbeat_at': int(last_hb) if last_hb is not None else None, 'heartbeat_age_seconds': int(hb_age) if hb_age is not None else None, 'timeout_seconds': stale_timeout_seconds, 'pid': int(pid) if pid else None}
            payload.update(termination)
            run_id = _end_run(conn, tid, outcome='stale', status='stale', error=(f'no heartbeat for {int(hb_age)}s ' if hb_age is not None else 'no heartbeat ever') + f' after {int(elapsed)}s running', metadata=payload)
            _append_event(conn, tid, 'stale', payload, run_id=run_id)
...
def reconcile_orphaned_running(conn: sqlite3.Connection) -> list[str]:
    ...
    now = int(time.time())
    reconciled: list[str] = []
    rows = conn.execute("SELECT id, claim_lock, claim_expires, worker_pid FROM tasks WHERE status = 'running'   AND (claim_lock IS NULL OR claim_expires IS NULL)").fetchall()
    for row in rows:
        tid = row['id']
        pid = row['worker_pid']
        if pid and _pid_alive(pid):
            _log.debug('kanban reconcile: task %s has broken claim bookkeeping but pid %s is alive on this host — deferring', tid, pid)
            continue
        with write_txn(conn):
            cur = conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL, last_heartbeat_at = NULL WHERE id = ? AND status = 'running'   AND claim_lock IS ? AND claim_expires IS ?", (tid, row['claim_lock'], row['claim_expires']))
            if cur.rowcount != 1:
                continue
            payload = {'reason': 'orphaned_running', 'claim_lock': row['claim_lock'], 'claim_expires': int(row['claim_expires']) if row['claim_expires'] is not None else None, 'worker_pid': int(pid) if pid else None, 'now': now}
            run_id = _end_run(conn, tid, outcome='reclaimed', status='reclaimed', error='orphaned running card (broken claim bookkeeping)', metadata=payload)
            conn.execute('INSERT INTO task_comments (task_id, author, body, created_at) VALUES (?, ?, ?, ?)', (tid, 'dispatcher', "reconciliation: card was 'running' with no valid claim (dead/gone worker) — requeued to ready", now))
            _append_event(conn, tid, 'reconciled', payload, run_id=run_id)
            reconciled.append(tid)
        _log.info('kanban reconcile: requeued orphaned running task %s (claim_lock=%r, worker_pid=%r)', tid, row['claim_lock'], pid)
    return reconciled
...
def _error_fingerprint(error_text: str) -> str:
    ...
    """
    fp = re.sub('\\bpid \\d+\\b', 'pid N', error_text[:80])
    fp = re.sub('\\b\\d{10,}\\b', '<TS>', fp)
...
def _protocol_violation_streak(conn: sqlite3.Connection, task_id: str) -> int:
    ...
    * Any other closed run (completed, plain crash, timeout, spawn failure,
    ...
    """
    streak = 0
    rows = conn.execute('SELECT outcome, error, metadata FROM task_runs WHERE task_id = ? AND ended_at IS NOT NULL ORDER BY id DESC LIMIT ?', (task_id, _PROTOCOL_VIOLATION_SCAN_LIMIT)).fetchall()
    for row in rows:
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    with write_txn(conn):
        rows = conn.execute("SELECT id, worker_pid, claim_lock, started_at FROM tasks WHERE status = 'running' AND worker_pid IS NOT NULL").fetchall()
        host_prefix = f"{_claimer_id().split(':', 1)[0]}:"
        for row in rows:
            lock = row['claim_lock'] or ''
            if not lock.startswith(host_prefix):
            ...
            if kind == 'clean_exit':
                ...
                error_text = 'worker exited cleanly (rc=0) without calling kanban_complete or kanban_block — protocol violation. If the prior run already did the work, verify it and report the result via kanban_complete; a run that ends without a terminal kanban call counts as failed no matter what it did.'
            ...
            cur = conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL WHERE id = ? AND status = 'running'   AND worker_pid = ? AND claim_lock IS ?", (row['id'], pid, row['claim_lock']))
            if cur.rowcount == 1:
                _run_outcome = 'rate_limited' if rate_limited_exit else 'crashed'
                run_id = _end_run(conn, row['id'], outcome=_run_outcome, status=_run_outcome, error=error_text, metadata=dict(event_payload))
                _append_event(conn, row['id'], event_kind, event_payload, run_id=run_id)
                if rate_limited_exit:
                    conn.execute('UPDATE tasks SET last_failure_error = ? WHERE id = ?', (error_text[:500], row['id']))
                    rate_limited.append(row['id'])
                else:
                    if protocol_violation:
                        conn.execute('UPDATE tasks SET last_failure_error = ? WHERE id = ?', (error_text[:500], row['id']))
                    crashed.append(row['id'])
                    crash_details.append((row['id'], pid, row['claim_lock'], protocol_violation, error_text))
    ...
    if crash_details:
        ...
        for tid, pid, claimer, protocol_violation, error_text in crash_details:
            if protocol_violation:
                ...
                if trow is None:
                    continue
                task_override = trow['max_retries'] if 'max_retries' in trow.keys() else None
                violation_limit = int(task_override) if task_override is not None else _PROTOCOL_VIOLATION_FAILURE_LIMIT
                if streak < violation_limit:
                    continue
                tripped = _record_task_failure(conn, tid, error=error_text, outcome='crashed', failure_limit=violation_limit, force_trip=True, release_claim=False, end_run=False, event_payload_extra={'pid': pid, 'claimer': claimer, 'protocol_violations': streak, 'protocol_violation_limit': violation_limit})
                if tripped:
                    auto_blocked.append(tid)
                continue
            fp = _error_fingerprint(error_text)
            is_systemic = _fp_counts.get(fp, 0) >= 3
            tripped = _record_task_failure(conn, tid, error=error_text, outcome='crashed', failure_limit=1 if is_systemic else None, release_claim=False, end_run=False, event_payload_extra={'pid': pid, 'claimer': claimer})
            if tripped:
                auto_blocked.append(tid)
...
def _record_task_failure(conn: sqlite3.Connection, task_id: str, error: str, *, outcome: str, failure_limit: int=None, force_trip: bool=False, release_claim: bool=False, end_run: bool=False, event_payload_extra: Optional[dict]=None) -> bool:
    ...
    * ``release_claim=False, end_run=False`` — timeout/crash path.
    ...
    blocked = False
    with write_txn(conn):
        row = conn.execute('SELECT consecutive_failures, status, max_retries FROM tasks WHERE id = ?', (task_id,)).fetchone()
        if row is None:
            return False
        ...
        if force_trip or failures >= effective_limit:
            if release_claim:
                conn.execute("UPDATE tasks SET status = 'blocked', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL, consecutive_failures = ?, last_failure_error = ? WHERE id = ? AND status IN ('running', 'ready')", (failures, error[:500], task_id))
            else:
                conn.execute("UPDATE tasks SET status = 'blocked', consecutive_failures = ?, last_failure_error = ? WHERE id = ? AND status IN ('ready', 'running')", (failures, error[:500], task_id))
            run_id = None
            if end_run:
                run_id = _end_run(conn, task_id, outcome='gave_up', status='gave_up', error=error[:500], metadata={'failures': failures, 'trigger_outcome': outcome, 'effective_limit': effective_limit, 'limit_source': limit_source})
            payload = {'failures': failures, 'effective_limit': effective_limit, 'limit_source': limit_source, 'error': error[:500], 'trigger_outcome': outcome}
            ...
            if release_claim:
                conn.execute("UPDATE tasks SET status = 'ready', claim_lock = NULL, claim_expires = NULL, worker_pid = NULL, consecutive_failures = ?, last_failure_error = ? WHERE id = ? AND status = 'running'", (failures, error[:500], task_id))
            else:
                conn.execute('UPDATE tasks SET consecutive_failures = ?, last_failure_error = ? WHERE id = ?', (failures, error[:500], task_id))
            if end_run:
                run_id = _end_run(conn, task_id, outcome=outcome, status=outcome, error=error[:500], metadata={'failures': failures})
                _append_event(conn, task_id, outcome, {'error': error[:500], 'failures': failures}, run_id=run_id)
    return blocked

def _record_spawn_failure(conn: sqlite3.Connection, task_id: str, error: str, *, failure_limit: int=None) -> bool:
    return _record_task_failure(conn, task_id, error, outcome='spawn_failed', failure_limit=failure_limit, release_claim=True, end_run=True)

def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    """Record the spawned child's pid + emit a ``spawned`` event.

    The event's payload carries the pid so a human reading ``duck-agent kanban
...
def _clear_failure_counter(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    """
    with write_txn(conn):
        conn.execute('UPDATE tasks SET consecutive_failures = 0, last_failure_error = NULL WHERE id = ?', (task_id,))
...
def check_respawn_guard(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    """
    row = conn.execute('SELECT last_failure_error FROM tasks WHERE id = ?', (task_id,)).fetchone()
    if row is None:
        return None
    now = int(time.time())
    rl_cooldown = _resolve_rate_limit_cooldown_seconds()
    latest_run = conn.execute('SELECT outcome, ended_at FROM task_runs WHERE task_id = ? AND ended_at IS NOT NULL ORDER BY ended_at DESC LIMIT 1', (task_id,)).fetchone()
    ...
    recent_completed = conn.execute("SELECT ended_at FROM task_runs WHERE task_id = ? AND outcome = 'completed' AND ended_at >= ? ORDER BY ended_at DESC LIMIT 1", (task_id, cutoff)).fetchone()
    if recent_completed:
        completed_at = int(recent_completed['ended_at'] or 0)
        requeued_after = conn.execute("SELECT 1 FROM task_events WHERE task_id = ? AND created_at >= ? AND kind IN ('status', 'promoted', 'unblocked', 'reclaimed') LIMIT 1", (task_id, completed_at)).fetchone()
        if not requeued_after:
            return 'recent_success'
    pr_cutoff = now - _RESPAWN_GUARD_PR_WINDOW
    for c in conn.execute('SELECT body FROM task_comments WHERE task_id = ? AND created_at >= ?', (task_id, pr_cutoff)).fetchall():
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Duck Agent profile.
    ...
    """
    rows = conn.execute("SELECT DISTINCT assignee FROM tasks WHERE status = 'ready' AND assignee IS NOT NULL     AND claim_lock IS NULL").fetchall()
    if not rows:
        return False
    try:
...
def has_spawnable_review(conn: sqlite3.Connection) -> bool:
    ...
    """
    rows = conn.execute("SELECT DISTINCT assignee FROM tasks WHERE status = 'review' AND assignee IS NOT NULL     AND claim_lock IS NULL").fetchall()
    if not rows:
        return False
    try:
        from hermes_cli.profiles import profile_exists
...
def dispatch_once(conn: sqlite3.Connection, *, spawn_fn=None, ttl_seconds: Optional[int]=None, dry_run: bool=False, max_spawn: Optional[int]=None, max_in_progress: Optional[int]=None, failure_limit: int=DEFAULT_SPAWN_FAILURE_LIMIT, stale_timeout_seconds: int=0, board: Optional[str]=None, default_assignee: Optional[str]=None, max_in_progress_per_profile: Optional[int]=None, reconcile_orphans: bool=True) -> DispatchResult:
    ...
    with _dispatch_tick_lock(db_path) as held:
        ...
        result = _dispatch_once_locked(conn, spawn_fn=spawn_fn, ttl_seconds=ttl_seconds, dry_run=dry_run, max_spawn=max_spawn, max_in_progress=max_in_progress, failure_limit=failure_limit, stale_timeout_seconds=stale_timeout_seconds, board=board, default_assignee=default_assignee, max_in_progress_per_profile=max_in_progress_per_profile, reconcile_orphans=reconcile_orphans)
...
def _dispatch_once_locked(conn: sqlite3.Connection, *, spawn_fn=None, ttl_seconds: Optional[int]=None, dry_run: bool=False, max_spawn: Optional[int]=None, max_in_progress: Optional[int]=None, failure_limit: int=DEFAULT_SPAWN_FAILURE_LIMIT, stale_timeout_seconds: int=0, board: Optional[str]=None, default_assignee: Optional[str]=None, max_in_progress_per_profile: Optional[int]=None, reconcile_orphans: bool=True) -> DispatchResult:
    ...
    if max_spawn is not None:
        running_count = int(conn.execute("SELECT COUNT(*) FROM tasks WHERE status = 'running'").fetchone()[0])
    ready_rows = conn.execute("SELECT id, assignee FROM tasks WHERE status = 'ready' AND claim_lock IS NULL ORDER BY priority DESC, created_at ASC").fetchall()
    if max_in_progress is not None and ready_rows:
        in_progress = conn.execute("SELECT COUNT(*) FROM tasks WHERE status = 'running'").fetchone()[0]
        ...
        if max_spawn is None or max_spawn > remaining:
            max_spawn = remaining
    spawned = 0
    _per_profile_cap = max_in_progress_per_profile if isinstance(max_in_progress_per_profile, int) and max_in_progress_per_profile > 0 else None
    _per_profile_running: dict[str, int] = {}
    if _per_profile_cap is not None:
        for prow in conn.execute("SELECT assignee, COUNT(*) AS n FROM tasks WHERE status = 'running' AND assignee IS NOT NULL GROUP BY assignee"):
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            if _default_assignee and _default_assignee_resolved:
                if not dry_run:
                    try:
                        with write_txn(conn):
                            conn.execute("UPDATE tasks SET assignee = ? WHERE id = ? AND (assignee IS NULL OR assignee = '')", (_default_assignee, row['id']))
                            _append_event(conn, row['id'], 'assigned', {'assignee': _default_assignee, 'source': 'kanban.default_assignee'})
                    except Exception:
                        _log.debug('kanban dispatch: failed to apply default_assignee=%r to task %s', _default_assignee, row['id'], exc_info=True)
        ...
        if _per_profile_cap is not None:
            ...
            if current >= _per_profile_cap:
                result.skipped_per_profile_capped.append((row['id'], row_assignee, current))
                continue
        ...
        if dry_run:
            ...
            if _per_profile_cap is not None and row_assignee:
                _per_profile_running[row_assignee] = _per_profile_running.get(row_assignee, 0) + 1
            continue
        ...
        try:
            ...
            auto = _record_spawn_failure(conn, claimed.id, f'workspace: {exc}', failure_limit=failure_limit)
            if auto:
        ...
        try:
            ...
            if _per_profile_cap is not None and claimed.assignee:
                _per_profile_running[claimed.assignee] = _per_profile_running.get(claimed.assignee, 0) + 1
        except Exception as exc:
            auto = _record_spawn_failure(conn, claimed.id, str(exc), failure_limit=failure_limit)
            if auto:
                result.auto_blocked.append(claimed.id)
    review_rows = conn.execute("SELECT id, assignee FROM tasks WHERE status = 'review' AND claim_lock IS NULL ORDER BY priority DESC, created_at ASC").fetchall()
...
def worker_log_rotation_config(kanban_cfg: Optional[dict]=None) -> tuple[int, int]:
    ...
    if kanban_cfg is None:
        try:
            ...
            kanban_cfg = {}
    max_bytes = _positive_int((kanban_cfg or {}).get('worker_log_rotate_bytes'), DEFAULT_LOG_ROTATE_BYTES, minimum=1)
    backup_count = _positive_int((kanban_cfg or {}).get('worker_log_backup_count'), DEFAULT_LOG_BACKUP_COUNT, minimum=0)
    return (max_bytes, backup_count)

def _rotated_log_path(log_path: Path, generation: int) -> Path:
    return log_path.with_suffix(log_path.suffix + f'.{generation}')

def _rotate_worker_log(log_path: Path, max_bytes: int, backup_count: int=DEFAULT_LOG_BACKUP_COUNT) -> None:
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Duck Agent CLI invocation."""
    return [sys.executable, '-m', 'hermes_cli.main']

def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Duck Agent shim."""
...
def _looks_like_path(value: str) -> bool:
    ...
    return expanded.startswith('~') or os.path.isabs(expanded) or bool(os.path.dirname(expanded)) or ('\\' in expanded) or bool(re.match('^[A-Za-z]:', expanded))
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Duck Agent executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``duck-agent`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("duck-agent")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\duck-agent.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Duck Agent is launched from a venv and the ``duck-agent`` shim is not on
    ...
    if env_bin:
        ...
        return _module_hermes_argv()
    hermes_bin = _safe_which_no_cwd('duck-agent') if _IS_WINDOWS else shutil.which('duck-agent')
    if hermes_bin:
        return _hermes_path_argv(hermes_bin)
    return _module_hermes_argv()

def _worker_terminal_timeout_env(max_runtime_seconds: Optional[int], current_timeout: Optional[str]) -> Optional[str]:
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        _log.debug('kanban worker: could not resolve CLI toolsets for DUCK_AGENT_HOME=%r (%s)', hermes_home, exc)
        return None
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str]=None) -> Optional[int]:
    """Fire-and-forget ``duck-agent -p <profile> chat -q ...`` subprocess.
    ...
    try:
        env['DUCK_AGENT_HOME'] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        pass
    if task.tenant:
    ...
    foreground_timeout = _worker_terminal_timeout_env(task.max_runtime_seconds, env.get('TERMINAL_MAX_FOREGROUND_TIMEOUT'))
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get('DUCK_AGENT_HOME'))
    if worker_toolsets:
    ...
    try:
        proc = subprocess.Popen(cmd, cwd=workspace if os.path.isdir(workspace) else None, stdin=subprocess.DEVNULL, stdout=log_f, stderr=subprocess.STDOUT, env=env, start_new_session=True, creationflags=subprocess.CREATE_NO_WINDOW if _IS_WINDOWS else 0)
    except FileNotFoundError:
        log_f.close()
        raise RuntimeError('`duck-agent` executable not found on PATH. Install Duck Agent or activate its venv before running the kanban dispatcher.')
    return proc.pid
...
def run_daemon(*, interval: float=60.0, max_spawn: Optional[int]=None, failure_limit: int=DEFAULT_SPAWN_FAILURE_LIMIT, stop_event=None, on_tick=None) -> None:
    ...
    on SIGINT / SIGTERM so ``duck-agent kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if task.max_runtime_seconds is not None:
        terminal_timeout = _worker_terminal_timeout_env(task.max_runtime_seconds, os.environ.get('TERMINAL_TIMEOUT'))
    ...
    if attachments:
        ...
        lines.append('Files attached to this task. Read them with the file/terminal tools at the absolute paths below:')
    ...
    if shown:
        ...
        if omitted:
            lines.append(f"_({omitted} earlier attempt{('s' if omitted != 1 else '')} omitted; showing most recent {len(shown)})_")
        for offset, run in enumerate(shown):
            ...
            if run.metadata:
                try:
                    ...
                    pass
            lines.append('')
    parent_rows = conn.execute('SELECT parent_id FROM task_links WHERE child_id = ? ORDER BY parent_id', (task_id,)).fetchall()
    ...
    if parent_ids:
        ...
        for pid in parent_ids:
            ...
            if not wrote_header:
                ...
                lines.append("_Handoffs from upstream tasks, captured when each parent completed (see age below). These are point-in-time snapshots, not live state — if a result drives your current work and it's not recent, re-verify against the source before acting on it as current._")
                wrote_header = True
            done_ts = None
            ...
            lines.append('')
    if task.assignee:
        role_rows = conn.execute("SELECT t.id, t.title, r.summary, r.ended_at FROM task_runs r JOIN tasks t ON r.task_id = t.id WHERE r.profile = ? AND r.task_id != ?   AND r.outcome = 'completed' ORDER BY r.ended_at DESC LIMIT 5", (task.assignee, task_id)).fetchall()
        if role_rows:
            lines.append(f'## Recent work by @{task.assignee}')
            for row in role_rows:
                ts = time.strftime('%Y-%m-%d %H:%M', time.localtime(int(row['ended_at'])))
    ...
    if shown_c:
        ...
        if omitted_c:
            lines.append(f"_({omitted_c} earlier comment{('s' if omitted_c != 1 else '')} omitted; showing most recent {len(shown_c)})_")
        for c in shown_c:
...
def board_stats(conn: sqlite3.Connection) -> dict:
    ...
    for row in conn.execute("SELECT status, COUNT(*) AS n FROM tasks WHERE status != 'archived' GROUP BY status"):
        by_status[row['status']] = int(row['n'])
    by_assignee: dict[str, dict[str, int]] = {}
    for row in conn.execute("SELECT assignee, status, COUNT(*) AS n FROM tasks WHERE status != 'archived' AND assignee IS NOT NULL GROUP BY assignee, status"):
        by_assignee.setdefault(row['assignee'], {})[row['status']] = int(row['n'])
    oldest_row = conn.execute("SELECT MIN(created_at) AS ts FROM tasks WHERE status = 'ready'").fetchone()
    now = int(time.time())
    oldest_ready_age = now - int(oldest_row['ts']) if oldest_row and oldest_row['ts'] is not None else None
    return {'by_status': by_status, 'by_assignee': by_assignee, 'oldest_ready_age_seconds': oldest_ready_age, 'now': now}
...
def task_age(task: Task) -> dict:
    ...
    return {'created_age_seconds': age_since_created, 'started_age_seconds': age_since_started, 'time_to_complete_seconds': time_to_complete}
...
def add_notify_sub(conn: sqlite3.Connection, *, task_id: str, platform: str, chat_id: str, chat_type: Optional[str]=None, thread_id: Optional[str]=None, user_id: Optional[str]=None, notifier_profile: Optional[str]=None, delivery_metadata: Optional[Mapping[str, Any]]=None) -> None:
...
def _notify_profile_filter(notifier_profiles: Optional[Iterable[str]], *, include_unowned: bool) -> tuple[str, list[str]]:
    """Build an optional SQL predicate for notification profile ownership."""
    if notifier_profiles is None:
        return ('', [])
    profiles = sorted({str(profile).strip() for profile in notifier_profiles if str(profile).strip()})
    clauses: list[str] = []
    params: list[str] = []
    if profiles:
...
def list_notify_subs(conn: sqlite3.Connection, task_id: Optional[str]=None, *, notifier_profiles: Optional[Iterable[str]]=None, include_unowned: bool=False) -> list[dict]:
    ...
    """
    owner_where, owner_params = _notify_profile_filter(notifier_profiles, include_unowned=include_unowned)
    where: list[str] = []
    params: list[Any] = []
...
def count_notify_subs(db_path: Optional[Path]=None, *, board: Optional[str]=None, notifier_profiles: Optional[Iterable[str]]=None, include_unowned: bool=False, platform: Optional[str]=None, chat_id: Optional[str]=None, thread_id: Optional[str]=None) -> int:
...
def remove_notify_sub(conn: sqlite3.Connection, *, task_id: str, platform: str, chat_id: str, thread_id: Optional[str]=None) -> bool:
    with write_txn(conn):
        cur = conn.execute('DELETE FROM kanban_notify_subs WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?', (task_id, platform, chat_id, thread_id or ''))
...
def unseen_events_for_sub(conn: sqlite3.Connection, *, task_id: str, platform: str, chat_id: str, thread_id: Optional[str]=None, kinds: Optional[Iterable[str]]=None) -> tuple[int, list[Event]]:
    ...
    """
    row = conn.execute('SELECT last_event_id FROM kanban_notify_subs WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?', (task_id, platform, chat_id, thread_id or '')).fetchone()
    if row is None:
        return (0, [])
    cursor = int(row['last_event_id'])
    kind_list = list(kinds) if kinds else None
    q = 'SELECT * FROM task_events WHERE task_id = ? AND id > ? ' + ('AND kind IN (' + ','.join('?' * len(kind_list)) + ') ' if kind_list else '') + 'ORDER BY id ASC'
...
def claim_unseen_events_for_sub(conn: sqlite3.Connection, *, task_id: str, platform: str, chat_id: str, thread_id: Optional[str]=None, kinds: Optional[Iterable[str]]=None) -> tuple[int, int, list[Event]]:
    ...
    with write_txn(conn):
        ...
        new_cursor, events = unseen_events_for_sub(conn, task_id=task_id, platform=platform, chat_id=chat_id, thread_id=thread_id, kinds=kinds)
        if not events:
            return (old_cursor, old_cursor, [])
        conn.execute('UPDATE kanban_notify_subs SET last_event_id = ? WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ? AND last_event_id = ?', (int(new_cursor), task_id, platform, chat_id, thread_id or '', int(old_cursor)))
...
def advance_notify_cursor(conn: sqlite3.Connection, *, task_id: str, platform: str, chat_id: str, thread_id: Optional[str]=None, new_cursor: int) -> None:
    with write_txn(conn):
        conn.execute('UPDATE kanban_notify_subs SET last_event_id = ? WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?', (int(new_cursor), task_id, platform, chat_id, thread_id or ''))
...
def rewind_notify_cursor(conn: sqlite3.Connection, *, task_id: str, platform: str, chat_id: str, thread_id: Optional[str]=None, claimed_cursor: int, old_cursor: int) -> bool:
    ...
    """
    with write_txn(conn):
        cur = conn.execute('UPDATE kanban_notify_subs SET last_event_id = ? WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ? AND last_event_id = ?', (int(old_cursor), task_id, platform, chat_id, thread_id or '', int(claimed_cursor)))
    return cur.rowcount > 0
...
def gc_events(conn: sqlite3.Connection, *, older_than_seconds: int=30 * 24 * 3600) -> int:
    ...
    with write_txn(conn):
        cur = conn.execute("DELETE FROM task_events WHERE created_at < ? AND task_id IN (SELECT id FROM tasks WHERE status IN ('done', 'archived'))", (cutoff,))
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Duck Agent root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``duck-agent kanban assignees`` for the terminal.
    ...
    return [{'name': name, 'on_disk': name in on_disk, 'counts': counts.get(name, {})} for name in names]
...
def latest_run(conn: sqlite3.Connection, task_id: str) -> Optional[Run]:
    ...
    row = conn.execute('SELECT * FROM task_runs WHERE task_id = ? ORDER BY started_at DESC, id DESC LIMIT 1', (task_id,)).fetchone()
    return Run.from_row(row) if row else None
...
def latest_summary(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    """
    row = conn.execute("SELECT summary FROM task_runs WHERE task_id = ? AND summary IS NOT NULL AND summary != '' ORDER BY COALESCE(ended_at, started_at) DESC, id DESC LIMIT 1", (task_id,)).fetchone()
