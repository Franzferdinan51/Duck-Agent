...
2. The persistence shape from the June 2026 ``duck-agent-0day`` campaign: a shell
   ...
   whose payload appended an attacker SSH key to ``authorized_keys``; Duck Agent
...
3. A hardcoded indicator-of-compromise (IOC) blocklist for that campaign — the
   attacker's ``duck-agent-0day`` SSH public key and source IPs. Any entry whose
...
_SHELL_INTERPRETERS = frozenset({'bash', 'sh', 'zsh', 'dash', 'fish', 'cmd', 'cmd.exe', 'powershell', 'powershell.exe', 'pwsh', 'pwsh.exe'})
_EGRESS_PATTERN = re.compile('(?<![\\w.-])(?:curl|wget|nc|ncat|socat)(?![\\w.-])|/dev/tcp/|\\bInvoke-WebRequest\\b|\\bInvoke-RestMethod\\b|\\bSystem\\.Net\\.WebClient\\b', re.IGNORECASE)
_EXFIL_HINT_PATTERN = re.compile('\\.env\\b|--data-binary|--data-raw|\\b-X\\s+POST\\b|\\bPOST\\b|<\\s*[^\\s]+', re.IGNORECASE)
_PERSISTENCE_PATTERN = re.compile('authorized_keys|\\.ssh/|/etc/ssh\\b|/etc/pam\\.d\\b|pam_[\\w-]+\\.so|/etc/sudoers|/etc/cron|crontab\\b|/etc/rc\\.local|/etc/systemd|\\.bashrc\\b|\\.bash_profile\\b|\\.profile\\b|\\.zshrc\\b', re.IGNORECASE)
_IOC_SUBSTRINGS = ('AAAAC3NzaC1lZDI1NTE5AAAAICBoh1oDC4DnsO1m5mJ4yfEKrQebaFh', 'duck-agent-0day', '60.165.167.', '118.182.244.156', '61.178.123.196')
...
def validate_mcp_server_entry(name: str, entry: dict[str, Any]) -> list[str]:
    ...
    * a known duck-agent-0day IOC anywhere in command/args/env (hardcoded blocklist);
    ...
    * a shell interpreter whose inline script writes to an OS persistence
      surface (June 2026 duck-agent-0day SSH/PAM/sudoers/cron shape).
    """
    ...
    for ioc in _IOC_SUBSTRINGS:
        if ioc in flat:
            issues.append(f"MCP server '{name}' contains a known duck-agent-0day indicator-of-compromise ('{ioc}')")
            return issues
    ...
    if _PERSISTENCE_PATTERN.search(script):
        issues.append(f"MCP server '{name}' uses shell interpreter '{command}' to write to an OS persistence surface (SSH keys / PAM / sudoers / cron / shell rc) — this is the duck-agent-0day backdoor shape, not a real MCP server")
    return issues
