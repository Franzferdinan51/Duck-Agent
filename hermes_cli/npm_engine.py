...
Scope of the repair is deliberately narrow. Duck Agent only upgrades an npm that
lives inside its **own** managed Node tree (``$DUCK_AGENT_HOME/node``), installing
...
and their other projects; Duck Agent never modifies those. When the failing npm is
one of those foreign installs, Duck Agent instead provisions its own managed Node
...
from hermes_constants import bootstrap_hermes_managed_node, get_hermes_home, with_hermes_node_path
__all__ = ['is_ebadengine', 'required_npm_range', 'managed_npm_prefix', 'upgrade_managed_npm', 'maybe_repair_npm_engine']
_REQUIRED_RE = re.compile('Required:\\s*(\\{.*?\\})')
_ACTUAL_RE = re.compile('Actual:\\s*(\\{.*?\\})')
...
def required_npm_range(output: str) -> str | None:
    ...
    if not is_ebadengine(output):
        return None
    ranges = [str(block['npm']).strip() for block in _iter_required_blocks(output) if block.get('npm')]
    if not ranges:
        return None
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Duck Agent-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$DUCK_AGENT_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool=False) -> bool:
    ...
    ``prefix=~/.local`` into ``$DUCK_AGENT_HOME/node/etc/npmrc`` so that global
    ...
    """
    if not quiet:
        print(f'→ Upgrading Duck Agent-managed npm to satisfy {npm_range}…', flush=True)
    try:
        with tempfile.TemporaryDirectory(prefix='duck-agent-npm-upgrade-') as tmp:
            result = subprocess.run([npm, 'install', '--global', '--prefix', str(prefix), f'npm@{npm_range}', '--no-fund', '--no-audit', '--progress=false'], cwd=tmp, env=_upgrade_env(), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=_UPGRADE_TIMEOUT, check=False)
...
def _probe_version(npm: str) -> str | None:
    try:
        result = subprocess.run([npm, '--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, env=with_hermes_node_path(), check=False)
    except (OSError, subprocess.SubprocessError):
        return None
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(f'\n✗ {have}does not satisfy the range this project requires: {npm_range}\n  Resolved npm: {npm}\n  Duck Agent could not provision its own Node.js runtime and never\n  modifies a system/nvm/brew/Nix npm. Upgrade yours yourself with:\n      npm install -g npm@"{npm_range}"', file=sys.stderr)

def _provision_managed_npm(npm_range: str | None, *, quiet: bool=False) -> str | None:
    """Provision a Duck Agent-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$DUCK_AGENT_HOME/node`` (reusing a healthy
    ...
    """
    if not quiet:
        print('→ Provisioning a Duck Agent-managed Node.js runtime (the resolved npm belongs to your system and is left alone)…', flush=True)
...
def maybe_repair_npm_engine(npm: str | None, output: str, *, quiet: bool=False) -> str | None:
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Duck Agent-managed install, or
