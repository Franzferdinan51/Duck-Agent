"""``duck-agent gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.duck-agent/auth.json``) — the same path ``duck-agent dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.duck-agent/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _default_gateway_id() -> str:
    ...
    try:
        ...
        host = ''
    return f"gw-{host or 'duck-agent'}"
...
def _post_enroll(*, connector_base_url: str, access_token: str, enrollment_token: str, gateway_id: str, timeout: float=15.0) -> dict:
    ...
    try:
        ...
        try:
            ...
            pass
        if exc.code == 401:
            raise RuntimeError('Connector rejected the caller identity (401). Your Nous Portal token could not be verified — try `duck-agent auth add nous` and retry.') from exc
        if exc.code == 403:
            raise RuntimeError(detail or 'Enrollment token invalid, expired, already used, or tenant mismatch (403).') from exc
        raise RuntimeError(f'Connector returned HTTP {exc.code}' + (f': {detail}' if detail else '')) from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f'Could not reach the connector at {connector_base_url}: {exc.reason}') from exc
...
def cmd_gateway_enroll(args) -> None:
    ...
    try:
        ...
        sys.exit(1)
    try:
        result = _post_enroll(connector_base_url=connector_base_url, access_token=access_token, enrollment_token=enrollment_token, gateway_id=gateway_id)
    ...
    to_write = {'GATEWAY_RELAY_ID': resolved_gateway_id, 'GATEWAY_RELAY_SECRET': secret, 'GATEWAY_RELAY_DELIVERY_KEY': delivery_key}
