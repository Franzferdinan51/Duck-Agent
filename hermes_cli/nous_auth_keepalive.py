...
from hermes_cli.auth import ACCESS_TOKEN_REFRESH_SKEW_SECONDS, NOUS_INVOKE_JWT_MIN_TTL_SECONDS, AuthError, _agent_key_is_usable, _is_expiring, get_provider_auth_state, resolve_nous_runtime_credentials
...
def _entry_state(entry: object) -> dict:
    return {'agent_key': getattr(entry, 'agent_key', None), 'agent_key_expires_at': getattr(entry, 'agent_key_expires_at', None), 'scope': getattr(entry, 'scope', None)}

def _refresh_selected_pool_entry(*, min_key_ttl_seconds: int) -> Optional[bool]:
    ...
    if entry is None:
        return False
    access_expiring = _is_expiring(getattr(entry, 'expires_at', None), ACCESS_TOKEN_REFRESH_SKEW_SECONDS)
...
def refresh_nous_auth_keepalive_once(*, min_key_ttl_seconds: int=NOUS_INVOKE_JWT_MIN_TTL_SECONDS, timeout_seconds: Optional[float]=None) -> bool:
    ...
    pool_result = _refresh_selected_pool_entry(min_key_ttl_seconds=min_key_ttl_seconds)
...
def _keepalive_loop(stop_event: threading.Event, *, interval_seconds: int, initial_delay_seconds: int, min_key_ttl_seconds: int, timeout_seconds: Optional[float]) -> None:
    if initial_delay_seconds > 0 and stop_event.wait(initial_delay_seconds):
        return
    while not stop_event.is_set():
        refresh_nous_auth_keepalive_once(min_key_ttl_seconds=min_key_ttl_seconds, timeout_seconds=timeout_seconds)
...
def start_nous_auth_keepalive(*, interval_seconds: int=NOUS_AUTH_KEEPALIVE_INTERVAL_SECONDS, initial_delay_seconds: int=NOUS_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS, min_key_ttl_seconds: int=NOUS_INVOKE_JWT_MIN_TTL_SECONDS, timeout_seconds: Optional[float]=None) -> Optional[threading.Thread]:
    ...
    with _keepalive_lock:
        ...
        _keepalive_thread = threading.Thread(target=_keepalive_loop, args=(_keepalive_stop,), kwargs={'interval_seconds': int(interval_seconds), 'initial_delay_seconds': max(0, int(initial_delay_seconds)), 'min_key_ttl_seconds': max(60, int(min_key_ttl_seconds)), 'timeout_seconds': timeout_seconds}, daemon=True, name='nous-auth-keepalive')
