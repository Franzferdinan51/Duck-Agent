...
class ExpensiveModelWarning:
    """Confirmation payload for models above Duck Agent' cost guardrail."""
    model: str
    provider: str
...
def _pricing_from_model_info(model_info: Optional[ModelInfo]) -> tuple[Optional[Decimal], Optional[Decimal], str]:
...
def expensive_model_warning(model_name: str, *, provider: Optional[str]=None, base_url: Optional[str]=None, api_key: Optional[str]=None, model_info: Optional[ModelInfo]=None) -> Optional[ExpensiveModelWarning]:
    ...
    if not over_input and (not over_output):
        return None
    lines = ['!!! EXPENSIVE MODEL WARNING !!!', '', f"{model} has known pricing above Duck Agent' safety threshold.", f'Input tokens: {_format_money(input_cost)}', f'Output tokens: {_format_money(output_cost)}', 'Threshold: more than $20/M input tokens or more than $100/M output tokens.']
    if source:
    ...
    return ExpensiveModelWarning(model=model, provider=(provider or '').strip(), input_cost_per_million=input_cost, output_cost_per_million=output_cost, source=source or 'unknown', message='\n'.join(lines))