"""``duck-agent skin`` — list, switch, and tweak skins from the CLI.
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    if path.exists():
        ...
        data = {'name': target, 'description': f'{name} + custom {key}', 'colors': dict(resolved.colors), 'branding': dict(resolved.branding), 'tool_prefix': resolved.tool_prefix}
