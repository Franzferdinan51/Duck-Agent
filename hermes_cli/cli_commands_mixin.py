...
from hermes_cli.browser_connect import DEFAULT_BROWSER_CDP_URL, discover_local_cdp_url, find_free_debug_port, is_browser_debug_ready, launch_chrome_debug, local_port_in_use, manual_chrome_debug_command
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print('  Enable with: duck-agent --checkpoints')
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Duck Agent changed (checkpoint baseline)
        ...
        if len(diff_lines) > 400:
            ...
            print(f'\n  ... ({len(diff_lines) - 400} more lines — run /diff --stat for a summary)')
        else:
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get('empty') or (not stat and (not diff)):
            print("  No changes — Duck Agent hasn't edited any files here yet.")
            return
        if stat:
        ...
        if len(diff_lines) > 400:
            self._print_diff_text('\n'.join(diff_lines[:400]))
            print(f'\n  ... ({len(diff_lines) - 400} more lines — run /diff session --stat for a summary)')
        else:
            self._print_diff_text(diff)
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Duck Agent config/state.
        ...
        """
        from hermes_cli.backup import create_quick_snapshot, list_quick_snapshots, restore_quick_snapshot, prune_quick_snapshots
    ...
    def _handle_export_command(self, command: str):
        ...
        try:
            ...
            print('  Share it: the other user runs /import or `duck-agent profile import <archive>`.')
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f'  Use it: duck-agent -p {imported}')
    ...
    def _handle_agents_command(self):
        ...
        if delegations:
            ...
            for d in delegations:
                ...
                status = d.get('status', '?')
                line = f"    {d.get('delegation_id', '?')} · {status} · {goal}"
                if status == 'stalling':
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `duck-agent journey`).
    ...
    def _handle_paste_command(self):
        ...
        if _is_termux_environment():
            _cprint(f'  {_DIM}Clipboard image paste is not available on Termux — use /image <path> or paste a local image path like {_termux_example_image_path()}{_RST}')
            return
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            ...
            if is_remote_shell_session():
                ...
                _cprint(f'  Copied assistant response #{idx + 1} via OSC 52 (terminal support required)')
                return
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f'  {_DIM}Tip: type your next message, or run duck-agent chat -q --image {_termux_example_image_path(image_path.name)} "What do you see?"{_RST}')
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint('  Timed out waiting for the gateway. Is `duck-agent gateway` running?')
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if len(target) >= 2 and (target[0] == '<' and target[-1] == '>' or (target[0] == '[' and target[-1] == ']') or (target[0] == '"' and target[-1] == '"') or (target[0] == "'" and target[-1] == "'")):
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason='resume'):
                ...
                return
            _cprint('  Tip:   Use /history or `duck-agent sessions list` to find sessions.')
            return
        ...
        if not session_meta:
            ...
            _cprint('  Use /history or `duck-agent sessions list` to see available sessions.')
            return
        try:
        ...
        if resolved_id and resolved_id != target_id:
            _cprint(f'  Session {target_id} was compressed into {resolved_id}; resuming the descendant with your transcript.')
        ...
        if self.agent:
            try:
                self.agent._flush_messages_to_session_db(self.conversation_history, conversation_history=self.conversation_history)
            except Exception:
                pass
        try:
        ...
        if self.agent:
            ...
            try:
                ...
                if _mm is not None:
                    _mm.on_session_switch(target_id, parent_session_id=old_session_id or '', reset=False, reason='resume')
            except Exception:
                pass
        ...
        if self.conversation_history:
            _cprint(f"  ↻ Resumed session {target_id}{title_part} ({msg_count} user message{('s' if msg_count != 1 else '')}, {len(self.conversation_history)} total)")
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            ...
            pass
        try:
            self._session_db.create_session(session_id=new_session_id, source=os.environ.get('HERMES_SESSION_SOURCE', 'cli'), model=self.model, model_config={'max_iterations': self.max_turns, 'reasoning_config': self.reasoning_config, '_branched_from': parent_session_id}, parent_session_id=parent_session_id)
        except Exception as e:
            _cprint(f'  Failed to create branch session: {e}')
            return
        try:
            self._session_db.append_messages_batch(new_session_id, [{'role': msg.get('role', 'user'), 'content': msg.get('content'), 'tool_name': msg.get('tool_name') or msg.get('name'), 'tool_calls': msg.get('tool_calls'), 'tool_call_id': msg.get('tool_call_id'), 'reasoning': msg.get('reasoning'), 'api_content': extract_api_content_sidecar(msg), 'timestamp': msg.get('timestamp')} for msg in self.conversation_history], chunk_rows=500)
        except Exception:
            pass
        try:
        ...
        if self.agent:
            ...
            try:
                _mm = getattr(self.agent, '_memory_manager', None)
                if _mm is not None:
                    _mm.on_session_switch(new_session_id, parent_session_id=parent_session_id or '', reset=False, reason='branch')
            except Exception:
                pass
        msg_count = len([m for m in self.conversation_history if m.get('role') == 'user'])
        _cprint(f'''  ⑂ Branched session "{branch_title}" ({msg_count} user message{('s' if msg_count != 1 else '')})''')
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        def _progress(event: str, detail: str) -> None:
            if event == 'row':
                ...
                print('  ┊ saving…')
        try:
            result = orchestrate.hatch_pet(base_image=drafts[0], slug=slug, display_name=display_name, concept=concept, on_progress=_progress)
        except GenerationError as exc:
    ...
    def _handle_cron_command(self, cmd: str):
        ...
        def _parse_flags(tokens):
            opts = {'name': None, 'deliver': None, 'repeat': None, 'skills': [], 'add_skills': [], 'remove_skills': [], 'clear_skills': False, 'all': False, 'prompt': None, 'schedule': None, 'positionals': []}
            i = 0
            while i < len(tokens):
                token = tokens[i]
        ...
        if subcommand in {'add', 'create'}:
            ...
            if not prompt and (not skills):
                ...
                return
            result = _cron_api(action='create', schedule=schedule, prompt=prompt or None, name=opts['name'], deliver=opts['deliver'], repeat=opts['repeat'], skills=skills or None)
        ...
        if subcommand == 'edit':
            ...
            result = _cron_api(action='update', job_id=job_id, schedule=opts['schedule'], prompt=opts['prompt'], name=opts['name'], deliver=opts['deliver'], repeat=opts['repeat'], skills=final_skills)
            if result.get('success'):
                job = result['job']
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hermes_cli.curator so the CLI and the `duck-agent curator`
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        args = parts[1:] if len(parts) > 1 else []
        if args and args[0].lower() in {'pending', 'approve', 'apply', 'reject', 'deny', 'drop', 'diff', 'approval', 'mode'}:
            from hermes_cli.write_approval_commands import handle_pending_subcommand
            from tools import write_approval as wa
            out = handle_pending_subcommand(wa.SKILLS, args, set_mode_fn=lambda enabled: self._save_write_approval('skills', enabled))
            if out is not None:
                print(out)
                return
    ...
    def _handle_memory_command(self, cmd: str):
        ...
        out = handle_pending_subcommand(wa.MEMORY, args, memory_store=store, set_mode_fn=lambda enabled: self._save_write_approval('memory', enabled))
        if out is None:
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        label = _skin.get_branding('response_label', '⚕ Duck Agent')
                    ...
                    _chat_console.print(Panel(_render_final_assistant_content(response, mode=self.final_response_markdown), title=f'[{_resp_color} bold]{label} (background #{task_num})[/]', title_align='left', border_style=_resp_color, style=_resp_text, box=rich_box.HORIZONTALS, padding=(1, 4), width=self._scrollback_box_width()))
                else:
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``duck-agent bundles list`` but renders inside the running
        ...
        if 'error' in reply.data:
            _cprint(f"\x1b[1;31mBundle subsystem unavailable: {reply.data['error']}{_RST}")
            return
        bundles = reply.data['bundles']
        if not bundles:
            _cprint('  No skill bundles installed.')
            _cprint(f'  {_DIM}Create one with: duck-agent bundles create <name> --skill <s1> --skill <s2>{_RST}')
        ...
        for info in bundles:
            ...
            ChatConsole().print(f"    [bold {_accent_hex()}]/{info['slug']:<20}[/] [dim]-[/] {_escape(desc)} [dim]({skill_count} skills)[/]")
            for s in info.get('skills', []):
                ChatConsole().print(f'        [dim]· {_escape(s)}[/]')
        _cprint(f'\n  {_DIM}Invoke a bundle with /<slug>. Manage with `duck-agent bundles`.{_RST}')
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub.startswith('connect'):
            ...
            if parsed_cdp.scheme not in {'http', 'https', 'ws', 'wss'}:
                print()
                print(f"   ⚠ Unsupported browser url scheme: {parsed_cdp.scheme or '(missing)'} (expected one of: http, https, ws, wss)")
                print()
                return
            try:
            ...
            if current:
                ...
                print()
                if hasattr(self, '_pending_input'):
                    self._pending_input.put('[System note: The user has disconnected the browser tools from their live Chromium-family browser. Browser tools are back to default mode (headless local browser or cloud provider).]')
            else:
                print()
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `duck-agent cron`.
        """
        ...
        _cprint(f'  {_DIM}Fires as a normal turn whenever the session is idle and the interval has elapsed. /heartbeat pause | resume | clear to manage; lives only while this Duck Agent process runs — use `duck-agent cron` for durable schedules.{_RST}')
    ...
    def _handle_refine_command(self, cmd: str) -> None:
        ...
        try:
            agent._spawn_background_review(messages_snapshot=snapshot, review_memory=True, review_skills=review_skills, focus=focus or None)
        except Exception as exc:
            _cprint(f'  /refine failed to start: {exc}')
            return
        tail = f' (focus: {focus})' if focus else ''
        _cprint(f'  ⚗ Reviewing this conversation in the background{tail} — any memory/skill updates will be reported when done.')
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        if lower == 'resume':
            ...
            if state is None:
                ...
                _cprint(f"  {_DIM}Send any message (or press Enter on an empty prompt is a no-op; type 'continue' to kick it off).{_RST}")
            return
        ...
        if lower == 'gate' or lower.startswith('gate '):
            ...
            if gate_lower.startswith('add '):
                ...
                try:
                    ...
                    return
                _cprint(f'  ⚿ Gate added: $ {gate.command} ({gate.max_retries} retries, {gate.timeout_seconds}s timeout). It must pass before the goal can complete.')
                return
        ...
        if state.has_contract():
            ...
            for line in state.contract.render_block().splitlines():
                _cprint(f'    {line}')
        _cprint(f"  {_DIM}After each turn, a judge model checks if the goal is done{(' against the contract above' if state.has_contract() else '')}. Duck Agent keeps working until it is, you pause/clear it, or the budget is exhausted. Use /goal status, /goal show, /goal pause, /goal resume, /goal clear.{_RST}")
        try:
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        if state.has_contract():
            ...
            for line in state.contract.render_block().splitlines():
                _cprint(f'    {line}')
            _cprint(f'  {_DIM}Tighten any field by re-setting the goal with inline lines (e.g. verify: <command>), then /goal resume. Use /goal show to review.{_RST}')
        else:
            _cprint(f"  {_DIM}Couldn't draft a contract (aux model unavailable) — running as a free-form goal. The per-turn judge still applies.{_RST}")
        try:
            self._pending_input.put(state.goal)
        except Exception:
            pass
    ...
    def _compose_in_editor(self, initial_text: str='') -> str:
        ...
        header = "#! Compose your prompt below. Lines starting with '#!' are ignored.\n#! Save and quit to send; leave empty to cancel.\n\n"
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from hermes_cli.focus_view import FOCUS_CONFIG_KEY, FOCUS_TOOL_PROGRESS_MODE, format_focus_status, format_focus_toggle_message, normalize_tool_progress_mode, resolve_focus_arg
        arg = ''
        try:
            parts = (cmd_original or '').strip().split(None, 1)
            if len(parts) > 1:
        ...
        if action == 'usage':
            ...
            return
        restore_mode = normalize_tool_progress_mode(getattr(self, '_focus_saved_tool_progress', None) if current else getattr(self, 'tool_progress_mode', 'all'))
        if action == 'status':
            ...
            _cprint(f'  {_Colors.BOLD}{label}:{_Colors.RESET}{state_color}{rest}{_Colors.RESET}' + (f'\n{_Colors.DIM}  {tail.strip()}{_Colors.RESET}' if tail else ''))
            return
        if target == current:
        ...
        state = f'{_Colors.GREEN}enabled{_Colors.RESET}' if target else f'{_Colors.DIM}disabled{_Colors.RESET}'
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        if arg in {'status', '?'}:
            ...
            _cprint(f"  {_Colors.BOLD}Runtime footer:{_Colors.RESET} {state}\n  Fields: {', '.join(fields)}")
            return
        ...
        if save_config_value('display.runtime_footer.enabled', new_state):
            state = f'{_Colors.GREEN}ON{_Colors.RESET}' if new_state else f'{_Colors.DIM}OFF{_Colors.RESET}'
    ...
    def _handle_reasoning_command(self, cmd: str):
        ...
        if explicit_global or '--session' in arg_tokens:
            arg = ' '.join((token for token in arg_tokens if token not in ('--global', '--session')))
        if arg in {'show', 'on'}:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Duck Agent is working.
        ...
        if save_config_value('display.busy_input_mode', arg):
            if arg == 'queue':
                behavior = 'Enter will queue follow-up input while Duck Agent is busy.'
            elif arg == 'steer':
                behavior = 'Enter will steer your message into the current run (after the next tool call).'
            else:
                behavior = 'Enter will redirect the current run while Duck Agent is busy; /stop still cancels it.'
    ...
    def _handle_debug_command(self, cmd_original: str=''):
        ...
        args = SimpleNamespace(lines=200, expire=7, local=local, nous=nous, yes=True)
        run_debug_share(args)

    def _handle_update_command(self) -> bool:
        """Handle /update — update Duck Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``duck-agent update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Duck Agent')}")
            return False
        choices = [('once', 'Update Now', 'exit the current session and update Duck Agent'), ('cancel', 'Cancel', 'keep the current session')]
        raw = self._prompt_text_input_modal(title='⚕  Update Duck Agent', detail='This will exit the current session and run `duck-agent update`.', choices=choices)
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Duck Agent' hotword listener.
    ...
    def _persist_wake_word_enabled(self, enabled: bool):
        ...
        if save_config_value('wake_word.enabled', enabled):
            _cprint(f"{_DIM}Wake word {('enabled' if enabled else 'disabled')} in config (wake_word.enabled: {str(enabled).lower()}).{_RST}")