...
- **Auth** = the OAuth bearer JWT Duck Agent already holds for inference
...
class BillingError(Exception):
    ...
    def __init__(self, message: str, *, status: Optional[int]=None, error: Optional[str]=None, portal_url: Optional[str]=None, retry_after: Optional[int]=None, payload: Optional[dict[str, Any]]=None, actor: Optional[str]=None, code: Optional[str]=None, recovery: Optional[str]=None) -> None:
...
_token_cache: tuple[float, str, str] | None = None
...
def _billing_not_logged_in(exc: Optional[BaseException]=None) -> 'BillingAuthError':
    ...
    err = BillingAuthError('Not logged into Nous Portal — run `duck-agent portal` to log in.', status=401, error='invalid_token')
    if exc is not None:
        err.__cause__ = exc
    return err
...
def _raise_for_error(status: int, payload: dict[str, Any], headers: Any=None) -> None:
    ...
    portal_url = _absolutize_portal_url(payload.get('portalUrl') if isinstance(payload, dict) else None)
    retry_after = _retry_after_seconds(headers)
    common = {'status': status, 'error': error, 'portal_url': portal_url, 'retry_after': retry_after, 'payload': payload if isinstance(payload, dict) else None, 'actor': actor, 'code': code, 'recovery': recovery}
    if error == 'stripe_unavailable':
        raise BillingStripeUnavailable(message or 'Stripe is temporarily unavailable — try again shortly.', **common)
    if error == 'upgrade_cap_exceeded':
        raise BillingUpgradeCapExceeded(message or 'Daily plan-change limit reached — try again tomorrow.', **common)
    if status == 401:
        if error == 'session_revoked':
            raise BillingSessionRevoked(message or 'Your session was logged out — log in again.', **common)
        raise BillingAuthError(message or 'Authentication required.', **common)
    if status == 403:
        if error == 'remote_spending_revoked':
            raise BillingRemoteSpendingRevoked(message or 'Remote spending was stopped for this terminal.', **common)
        if error == 'insufficient_scope':
            raise BillingScopeRequired(message or 'This action needs the billing:manage scope.', **common)
        raise BillingError(message or error or 'Billing request denied.', **common)
    if status in (429, 503):
        raise BillingRateLimited(message or 'Rate limited — try again shortly.', **common)
    raise BillingError(message or error or f'Billing request failed ({status}).', **common)

def _request(method: str, path: str, *, body: Optional[dict[str, Any]]=None, extra_headers: Optional[dict[str, str]]=None, timeout: float=DEFAULT_TIMEOUT, _retried_auth: bool=False) -> dict[str, Any]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            ...
            try:
                ...
                raise BillingError('Billing endpoint returned a non-JSON response (it may not be available on this deployment).', error='endpoint_unavailable', status=getattr(resp, 'status', None)) from exc
...
def patch_auto_top_up(*, enabled: bool, threshold: float | str, top_up_amount: float | str, timeout: float=DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    """
    return _request('PATCH', '/api/billing/auto-top-up', body={'enabled': bool(enabled), 'threshold': float(threshold), 'topUpAmount': float(top_up_amount)}, timeout=timeout)
...
def post_charge(*, amount_usd: float | str, idempotency_key: str, timeout: float=DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    if not (isinstance(idempotency_key, str) and idempotency_key.strip()):
        raise BillingError('Idempotency-Key is required for a charge.', error='idempotency_key_required')
    return _request('POST', '/api/billing/charge', body={'amountUsd': float(amount_usd)}, extra_headers={'Idempotency-Key': idempotency_key.strip()}, timeout=timeout)
...
def put_subscription_pending_change(*, subscription_type_id: str | None=None, cancel: bool=False, timeout: float=DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    if cancel:
        ...
        if not (isinstance(subscription_type_id, str) and subscription_type_id.strip()):
            raise BillingError('A subscription tier is required to schedule a plan change.', error='invalid_subscription_type')
        body = {'type': 'tier_change', 'subscriptionTypeId': subscription_type_id.strip()}
...
def post_subscription_upgrade(*, subscription_type_id: str, idempotency_key: str, timeout: float=DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    """
    if not (isinstance(idempotency_key, str) and idempotency_key.strip()):
        raise BillingError('Idempotency-Key is required for an upgrade.', error='idempotency_key_required')
    return _request('POST', '/api/billing/subscription/upgrade', body={'subscriptionTypeId': subscription_type_id}, extra_headers={'Idempotency-Key': idempotency_key.strip()}, timeout=timeout)