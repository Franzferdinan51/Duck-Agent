"""CLI handlers for ``duck-agent secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so Duck Agent expects
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    setup = sub.add_parser('setup', help='Verify the op CLI, set account / token env var, and enable')
    setup.add_argument('--account', help='1Password account shorthand or sign-in address (op --account)')
    setup.add_argument('--token-env', help=f'Env var holding a service-account token (default {_DEFAULT_TOKEN_ENV})')
    setup.add_argument('--token', help='Service-account token to store in .env non-interactively')
    setup.add_argument('--binary-path', help='Absolute path to the op binary (skips PATH lookup)')
    setup.set_defaults(func=cmd_setup)
    status = sub.add_parser('status', help='Show config + op binary + references')
    status.set_defaults(func=cmd_status)
    token = sub.add_parser('token', help='Rotate the service-account token: validate and store it in .env')
    token.add_argument('--token', help='Provide the new token non-interactively (default: masked prompt)')
    token.add_argument('--no-verify', action='store_true', help='Store without probing 1Password first (not recommended)')
    ...
    sync.add_argument('--apply', action='store_true', help='Actually export resolved values into the current shell (default: dry-run)')
...
def cmd_setup(args: argparse.Namespace) -> int:
    console = Console()
    console.print(Panel.fit(f"[bold]1Password secret source setup[/bold]\n\nDuck Agent resolves [cyan]op://vault/item/field[/cyan] references through your\nalready-installed, already-authenticated 1Password CLI (`op`).\n\nDon't have it yet? Install + sign in: [cyan]{_DOCS_URL}[/cyan]", border_style='cyan'))
    cfg = load_config()
    ...
    if token:
        ...
        if who:
            ...
            console.print(f'  [yellow]No service-account token and no active op session detected.[/yellow]\n  Either run [cyan]op signin[/cyan] (desktop/interactive) or set a service-account token in {token_env}, then re-run status.')
    op_cfg['enabled'] = True
    ...
    console.print('  Map credentials:  [cyan]duck-agent secrets onepassword set OPENAI_API_KEY "op://Private/OpenAI/api key"[/cyan]\n  Preview:          [cyan]duck-agent secrets onepassword sync[/cyan]\n  Status:           [cyan]duck-agent secrets onepassword status[/cyan]')
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print('\n  Run [cyan]duck-agent secrets onepassword setup[/cyan] to enable.')
        return 0
    if binary and (not token_set):
        ...
        if who:
            ...
            console.print(f'\n  [yellow]No active op session and {token_env} is unset — Duck Agent will warn and skip 1Password on next startup.[/yellow]')
    if not references:
        console.print('\n  [yellow]No references mapped yet.[/yellow]  Add one: [cyan]duck-agent secrets onepassword set ENV_VAR "op://…"[/cyan]')
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get('enabled'):
        console.print('  [yellow]Note: the integration is disabled — run [cyan]duck-agent secrets onepassword setup[/cyan] to turn it on.[/yellow]')
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not token:
        if not sys.stdin.isatty():
            ...
            return 1
        console.print('Create a new service-account token at https://my.1password.com → Developer → Service Accounts.\n')
    ...
    if not args.no_verify:
        binary = op_src.find_op(binary_path)
        if binary is None:
            console.print(f'[red]op CLI not found — install it ({_DOCS_URL}) or re-run with --no-verify to store anyway.[/red]')
            return 1
    ...
    if not op_cfg.get('enabled'):
        console.print('[yellow]Note: the 1Password integration is currently disabled — run `duck-agent secrets onepassword setup` to turn it on.[/yellow]')
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    op_cfg = (cfg.get('secrets') or {}).get('onepassword') or {}
    if not op_cfg.get('enabled'):
        console.print('[yellow]1Password integration is disabled.  Run `duck-agent secrets onepassword setup` first.[/yellow]')
        return 1
    references = op_cfg.get('env') if isinstance(op_cfg.get('env'), dict) else {}
    if not references:
        console.print('[yellow]No op:// references configured.  Add one with `duck-agent secrets onepassword set ENV_VAR "op://…"`.[/yellow]')
        return 0
    account = str(op_cfg.get('account', '') or '').strip()
    token_env = op_cfg.get('service_account_token_env', _DEFAULT_TOKEN_ENV)
    binary_path = str(op_cfg.get('binary_path', '') or '').strip()
    if args.apply:
        result = op_src.apply_onepassword_secrets(enabled=True, env=references, account=account, service_account_token_env=token_env, binary_path=binary_path, override_existing=bool(op_cfg.get('override_existing', True)), cache_ttl_seconds=0)
        if result.error:
        ...
        return 0
    try:
        secrets, warnings = op_src.fetch_onepassword_secrets(references=references, account=account, token_env=token_env, binary_path=binary_path, use_cache=False)
    ...
    for name in sorted(references):
        if name == token_env:
            ...
            table.add_row(name, '[green]would export[/green]' + (' (overrides)' if already else ''))
    console.print(table)
    for w in warnings:
        console.print(f'[yellow]warning:[/yellow] {w}')
    console.print('\n  This was a dry-run — references resolve automatically on the next [cyan]duck-agent[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export into the current shell instead.')
    return 0
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    save_config(cfg)
    console.print('[green]Disabled.[/green]  1Password references will NOT be resolved on the next Duck Agent invocation.\n  Your reference mappings are left in config.yaml — remove them with [cyan]duck-agent secrets onepassword remove ENV_VAR[/cyan] if you no longer need them.')
    return 0
...
def _op_whoami(binary: Path, account: str, *, token_value: str='') -> Optional[str]:
    ...
    try:
        res = subprocess.run(cmd, env=env, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10)
    except (OSError, subprocess.TimeoutExpired):
        return None
    if res.returncode != 0:
        return None
