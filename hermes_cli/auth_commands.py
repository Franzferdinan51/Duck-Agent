...
def _classify_exhausted_status(entry) -> tuple[str, bool]:
    ...
    if code == 429 or any((token in reason for token in ('rate_limit', 'usage_limit', 'quota', 'exhausted'))) or any((token in message for token in ('rate limit', 'usage limit', 'quota', 'too many requests'))):
        return ('rate-limited', True)
    if code in {401, 403} or any((token in reason for token in ('invalid_token', 'invalid_grant', 'unauthorized', 'forbidden', 'auth'))) or any((token in message for token in ('unauthorized', 'forbidden', 'expired', 'revoked', 'invalid token', 'authentication'))):
...
def auth_add_command(args) -> None:
    ...
    if requested_type == AUTH_TYPE_API_KEY:
        ...
        if not label:
            if sys.stdin.isatty():
                ...
                label = default_label
        entry = PooledCredential(provider=provider, id=uuid.uuid4().hex[:6], label=label, auth_type=AUTH_TYPE_API_KEY, priority=0, source=SOURCE_MANUAL, access_token=token, base_url=_provider_base_url(provider))
    ...
    if provider == 'anthropic':
        ...
        label = (getattr(args, 'label', None) or '').strip() or label_from_token(creds['access_token'], _oauth_default_label(provider, len(pool.entries()) + 1))
        entry = PooledCredential(provider=provider, id=uuid.uuid4().hex[:6], label=label, auth_type=AUTH_TYPE_OAUTH, priority=0, source=f'{SOURCE_MANUAL}:hermes_pkce', access_token=creds['access_token'], refresh_token=creds.get('refresh_token'), expires_at_ms=creds.get('expires_at_ms'), base_url=_provider_base_url(provider))
        pool.add_entry(entry)
    ...
    if provider == 'nous':
        ...
        if shared:
            ...
            if do_import in {'', 'y', 'yes'}:
                ...
                rehydrated = auth_mod._try_import_shared_nous_state(timeout_seconds=getattr(args, 'timeout', None) or 15.0)
                if rehydrated is not None:
                    custom_label = (getattr(args, 'label', None) or '').strip() or None
                    entry = auth_mod.persist_nous_credentials(rehydrated, label=custom_label)
                    shown_label = entry.label if entry is not None else label_from_token(rehydrated.get('access_token', ''), _oauth_default_label(provider, 1))
                    print(f'Imported {provider} OAuth credentials: "{shown_label}"')
                    return
                print('Could not refresh shared credentials — falling back to device-code login.')
        creds = auth_mod._nous_device_code_login(portal_base_url=getattr(args, 'portal_url', None), inference_base_url=getattr(args, 'inference_url', None), client_id=getattr(args, 'client_id', None), scope=getattr(args, 'scope', None), open_browser=not getattr(args, 'no_browser', False), timeout_seconds=getattr(args, 'timeout', None) or 15.0, insecure=bool(getattr(args, 'insecure', False)), ca_bundle=getattr(args, 'ca_bundle', None))
        custom_label = (getattr(args, 'label', None) or '').strip() or None
        entry = auth_mod.persist_nous_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(creds.get('access_token', ''), _oauth_default_label(provider, 1))
        print(f'Saved {provider} OAuth device-code credentials: "{shown_label}"')
        return
    if provider == 'openai-codex':
        creds = auth_mod._codex_device_code_login()
        label = (getattr(args, 'label', None) or '').strip() or label_from_token(creds['tokens']['access_token'], _oauth_default_label(provider, len(pool.entries()) + 1))
        entry = PooledCredential(provider=provider, id=uuid.uuid4().hex[:6], label=label, auth_type=AUTH_TYPE_OAUTH, priority=0, source=SOURCE_MANUAL_DEVICE_CODE, access_token=creds['tokens']['access_token'], refresh_token=creds['tokens'].get('refresh_token'), base_url=creds.get('base_url'), last_refresh=creds.get('last_refresh'))
    ...
    if provider == 'qwen-oauth':
        ...
        label = (getattr(args, 'label', None) or '').strip() or label_from_token(creds['api_key'], _oauth_default_label(provider, len(pool.entries()) + 1))
        entry = PooledCredential(provider=provider, id=uuid.uuid4().hex[:6], label=label, auth_type=AUTH_TYPE_OAUTH, priority=0, source=f'{SOURCE_MANUAL}:qwen_cli', access_token=creds['api_key'], base_url=creds.get('base_url'))
        pool.add_entry(entry)
        print(f'Added {provider} OAuth credential #{len(pool.entries())}: "{entry.label}"')
        return
    if provider == 'minimax-oauth':
        creds = auth_mod._minimax_oauth_login(open_browser=not getattr(args, 'no_browser', False), timeout_seconds=getattr(args, 'timeout', None) or 15.0)
        label = (getattr(args, 'label', None) or '').strip() or label_from_token(creds['access_token'], _oauth_default_label(provider, len(pool.entries()) + 1))
        entry = PooledCredential(provider=provider, id=uuid.uuid4().hex[:6], label=label, auth_type=AUTH_TYPE_OAUTH, priority=0, source=f'{SOURCE_MANUAL}:minimax_oauth', access_token=creds['access_token'], refresh_token=creds.get('refresh_token'), base_url=creds.get('inference_base_url'))
        pool.add_entry(entry)
        print(f'Added {provider} OAuth credential #{len(pool.entries())}: "{entry.label}"')
        return
    raise SystemExit(f'`duck-agent auth add {provider}` is not implemented for auth type {requested_type} yet.')
...
def auth_list_command(args) -> None:
    ...
    if provider_filter:
        ...
        providers = sorted({*PROVIDER_REGISTRY.keys(), 'openrouter', *list_custom_pool_providers()})
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit('Provider is required. Example: `duck-agent auth status spotify`.')
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `duck-agent auth` is called bare."""
    ...
    try:
        ...
        if isinstance(_model_cfg, dict):
            ...
            if _cfg_provider == 'azure-foundry' and _cfg_auth_mode == 'entra_id':
                ...
                if not isinstance(_entra, dict):
                    _entra = {}
                _scope = str(_entra.get('scope') or '').strip() or SCOPE_AI_AZURE_DEFAULT
                ...
                if not has_azure_identity_installed():
                    ...
                    _entra_cfg = EntraIdentityConfig(scope=_scope)
        ...
        pass
    print()
    choices = ['Add a credential', 'Remove a credential', 'Reset cooldowns for a provider', 'Set rotation strategy for a provider', 'Exit']
...
def _interactive_add() -> None:
    ...
    if typed_label:
        label = typed_label
    auth_add_command(SimpleNamespace(provider=provider, auth_type=auth_type, label=label, api_key=None, portal_url=None, inference_url=None, client_id=None, scope=None, no_browser=False, timeout=None, insecure=False, ca_bundle=None))
...
def _interactive_strategy() -> None:
    ...
    print()
    descriptions = {STRATEGY_FILL_FIRST: 'Use first key until exhausted, then next', STRATEGY_ROUND_ROBIN: 'Cycle through keys evenly', STRATEGY_LEAST_USED: 'Always pick the least-used key', STRATEGY_RANDOM: 'Random selection'}
