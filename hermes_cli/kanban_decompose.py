...
Invoked by ``duck-agent kanban decompose [task_id | --all]`` and the
...
def _build_roster() -> tuple[list[dict], set[str]]:
    ...
    for p in all_profiles:
        ...
        roster.append({'name': p.name, 'description': desc or f'(no description; profile named {p.name!r})', 'has_description': bool(desc)})
        valid.add(p.name)
    return (roster, valid)
...
def decompose_task(task_id: str, *, author: Optional[str]=None, timeout: Optional[int]=None) -> DecomposeOutcome:
    ...
    if task.status != 'triage':
        return DecomposeOutcome(task_id, False, f'task is not in triage (status={task.status!r})')
    cfg = _load_config()
    ...
    user_msg = _USER_TEMPLATE.format(task_id=task.id, title=_truncate(task.title or '', 400), body=_truncate(task.body or '(no body)', 4000), roster=_format_roster(roster), default_assignee=default_assignee)
    try:
        resp = call_llm(task='kanban_decomposer', messages=[{'role': 'system', 'content': _SYSTEM_PROMPT}, {'role': 'user', 'content': user_msg}], temperature=0.3, max_tokens=4000, timeout=timeout or 180)
    except Exception as exc:
    ...
    if not fanout:
        ...
        if not task.assignee:
            assignee_val = _normalize_assignee_choice(parsed.get('assignee'), default_assignee=default_assignee, valid_names=valid_names)
        if title_val is None and body_val is None:
            return DecomposeOutcome(task_id, False, 'decomposer returned fanout=false with no title/body')
        with kb.connect_closing() as conn:
            ok = kb.specify_triage_task(conn, task_id, title=title_val, body=body_val, assignee=assignee_val, author=audit_author)
        if not ok:
            return DecomposeOutcome(task_id, False, 'task moved out of triage before promotion')
        return DecomposeOutcome(task_id, True, 'single task (no fanout)', fanout=False, new_title=title_val)
    ...
    for idx, entry in enumerate(raw_tasks):
        if not isinstance(entry, dict):
            return DecomposeOutcome(task_id, False, f'tasks[{idx}] is not an object')
        title = entry.get('title')
        if not isinstance(title, str) or not title.strip():
            return DecomposeOutcome(task_id, False, f'tasks[{idx}].title is missing or empty')
        body = entry.get('body')
        if not isinstance(body, str):
            body = ''
        assignee = entry.get('assignee')
        chosen = _normalize_assignee_choice(assignee, default_assignee=default_assignee, valid_names=valid_names)
        if isinstance(assignee, str) and assignee.strip() and (assignee.strip() not in valid_names):
            logger.info('decompose: task %s child %d picked unknown assignee %r — routing to default_assignee %r', task_id, idx, assignee, default_assignee)
        parents = entry.get('parents') or []
        if not isinstance(parents, list):
            parents = []
        clean_parents = [p for p in parents if isinstance(p, int) and 0 <= p < len(raw_tasks) and (p != idx)]
        children.append({'title': title.strip()[:200], 'body': body.strip(), 'assignee': chosen, 'parents': clean_parents})
    try:
        with kb.connect_closing() as conn:
            child_ids = kb.decompose_triage_task(conn, task_id, root_assignee=orchestrator, children=children, author=audit_author, auto_promote=auto_promote)
    ...
    return DecomposeOutcome(task_id, True, f'decomposed into {len(child_ids)} children', fanout=True, child_ids=child_ids)
