...
_AZURE_OPENAI_PROBE_API_VERSIONS = ('2025-04-01-preview', '2024-10-21')
...
def _resolve_credential(api_key: Any, token_provider: Optional[Callable[[], str]]=None) -> tuple[Optional[str], str]:
...
def _apply_auth_headers(req: urllib_request.Request, token: Optional[str], mode: str) -> None:
...
def _http_get_json(url: str, api_key: Any, timeout: float=6.0, *, token_provider: Optional[Callable[[], str]]=None) -> tuple[int, Optional[dict]]:
    ...
    req.add_header('User-Agent', 'duck-agent/azure-detect')
    try:
...
def _probe_openai_models(base_url: str, api_key: Any, *, token_provider: Optional[Callable[[], str]]=None) -> tuple[bool, list[str]]:
...
def _probe_anthropic_messages(base_url: str, api_key: Any, *, token_provider: Optional[Callable[[], str]]=None) -> bool:
...
def detect(base_url: str, api_key: Any='', *, token_provider: Optional[Callable[[], str]]=None) -> DetectionResult:
    ...
    if ok:
        ...
        result.reason = f'GET /models returned {len(models)} model(s) — OpenAI-style endpoint' if models else 'GET /models returned an OpenAI-shaped empty list — OpenAI-style endpoint'
        return result
    if _probe_anthropic_messages(base_url, api_key, token_provider=token_provider):
        ...
        return result
    result.reason = 'Could not probe endpoint (private network, missing model list, or non-standard path) — falling back to manual API-mode selection'
    return result
