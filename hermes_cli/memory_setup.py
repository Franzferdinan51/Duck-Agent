"""duck-agent memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``duck-agent memory setup`` — if the update-time refresh only reinstalled
...
def _curses_select(title: str, items: list[tuple[str, str]], default: int=0, *, cancel_returns: int | None=None) -> int:
...
def _install_dependencies(provider_name: str, *, force: bool=False) -> None:
    ...
    ranges. This is how ``duck-agent update`` heals the active memory provider
    ...
    if not pip_deps:
        return
    _IMPORT_NAMES = {'honcho-ai': 'honcho', 'mem0ai': 'mem0', 'hindsight-client': 'hindsight_client', 'hindsight-all': 'hindsight'}
    missing = []
    for dep in pip_deps:
        if force:
            missing.append(dep)
            continue
        dep_name = re.match('^[A-Za-z0-9_][A-Za-z0-9_.\\-]*', dep)
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'duck-agent memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print('  Install a plugin to ~/.duck-agent/plugins/ and try again.\n')
        return
    items = []
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.duck-agent/plugins/")
    if providers:
        print('\n  Installed plugins:')
