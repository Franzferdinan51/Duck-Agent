...
def clarify_callback(cli, question, choices, multi_select=False):
    ...
    cli._clarify_state = {'question': question, 'choices': choices if not is_open_ended else [], 'selected': 0, 'multi_select': effective_multi, 'selected_indices': set() if effective_multi else None, 'response_queue': response_queue}
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.duck-agent/.env and never exposed to the model.
    """
    if not getattr(cli, '_app', None):
        ...
        if not value:
            ...
            return {'success': True, 'reason': 'cancelled', 'stored_as': var_name, 'validated': False, 'skipped': True, 'message': 'Secret setup was skipped.'}
        stored = save_env_value_secure(var_name, value)
        _dhh = display_hermes_home()
        cprint(f'\n{_DIM}  ✓ Stored secret in {_dhh}/.env as {var_name}{_RST}')
        return {**stored, 'skipped': False, 'message': 'Secret stored securely. The secret value was not exposed to the model.'}
    timeout = 120
    response_queue = queue.Queue()
    cli._secret_state = {'var_name': var_name, 'prompt': prompt, 'metadata': metadata or {}, 'response_queue': response_queue}
    ...
    return {'success': True, 'reason': 'timeout', 'stored_as': var_name, 'validated': False, 'skipped': True, 'message': 'Secret setup timed out and was skipped.'}
...
def approval_callback(cli, command: str, description: str) -> str:
    ...
    with lock:
        ...
        if len(command) > 70:
            choices.append('view')
        cli._approval_state = {'command': command, 'description': description, 'choices': choices, 'selected': 0, 'response_queue': response_queue}
