"""``duck-agent monitoring`` subcommand parser.
...
def build_monitoring_parser(subparsers, *, cmd_monitoring: Callable) -> None:
    ...
    p = subparsers.add_parser('monitoring', help='Inspect gateway monitoring (health & diagnostics export)', description='Gateway monitoring: service health metrics plus redacted diagnostics, exported over OTLP to an operator-configured endpoint. Content-free by construction — no prompts, messages, tool args/results, or usage analytics. Configure under monitoring.* in config.yaml.')
