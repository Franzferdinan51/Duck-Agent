"""``duck-agent webhook`` subcommand parser.
...
def build_webhook_parser(subparsers, *, cmd_webhook: Callable) -> None:
    ...
    webhook_parser = subparsers.add_parser('webhook', help='Manage dynamic webhook subscriptions', description='Create, list, and remove webhook subscriptions for event-driven agent activation')
    ...
    wh_sub.add_argument('--deliver-only', action='store_true', help="Skip the agent — deliver the rendered prompt directly as the message. Zero LLM cost. Requires --deliver to be a real target (not 'log').")
    wh_sub.add_argument('--script', default='', help='Filter/transform script under ~/.duck-agent/scripts/. The route payload is passed as JSON on stdin; empty stdout, [SILENT], or a nonzero exit code ignores the webhook.')
