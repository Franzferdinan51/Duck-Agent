"""``duck-agent skin`` subcommand parser."""
from __future__ import annotations
from typing import Callable

def build_skin_parser(subparsers, *, cmd_skin: Callable) -> None:
    """Attach the ``skin`` subcommand to ``subparsers``."""
    skin_parser = subparsers.add_parser('skin', help='List, switch, and tweak skins', description='Manage Duck Agent skins. `set` tweaks one color of the active skin in place.')
    ...
    skin_set = skin_subparsers.add_parser('set', help="Set one color of the active skin (e.g. `skin set ui_tool '#00FFFF'`)")
