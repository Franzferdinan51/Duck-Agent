"""``duck-agent config`` subcommand parser.
...
def build_config_parser(subparsers, *, cmd_config: Callable) -> None:
    ...
    config_parser = subparsers.add_parser('config', help='View and edit configuration', description='Manage Duck Agent configuration')
    ...
    config_set.add_argument('--force', action='store_true', help="Skip the unknown-key notice printed after writing a key the running version doesn't recognize (the value is saved either way).")
