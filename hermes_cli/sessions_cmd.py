"""``duck-agent sessions`` command — extracted from ``hermes_cli/main.py``.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == 'repair':
        ...
        if report.get('repaired'):
            ...
            try:
                ...
                n = SessionDB()._conn.execute('SELECT COUNT(*) FROM sessions').fetchone()[0]
            ...
            print(f'    duck-agent sessions recover --source {source_hint} \\')
            print('        --inspect-only')
    ...
    if action == 'recover':
        ...
        from hermes_cli.session_recovery import SessionRecoveryError, inspect_session_database, recover_session_database, write_recovery_report
        source = args.source
        ...
        try:
            if inspect_only:
                report = inspect_session_database(source, work_dir=getattr(args, 'work_dir', None))
            else:
                ...
                report = recover_session_database(source, output, work_dir=getattr(args, 'work_dir', None), chunk_size=getattr(args, 'chunk_size', 1000), progress_cb=_recovery_progress, allow_partial=allow_partial)
                if last_progress['table'] is not None:
                    print()
        ...
        if allow_partial and report.get('verified'):
            ...
            print(f"  Recovered {int(counts.get('sessions') or 0):,} sessions and {int(counts.get('messages') or 0):,} messages.")
            print('  The active session database was not changed.')
            print('  This output is incomplete. Review every skipped range and orphan count in the JSON report before installing it.')
            return 0
    ...
    if action == 'list':
        ...
        _filter_arg_names = ('older_than', 'newer_than', 'before', 'after', 'source', 'title', 'end_reason', 'cwd', 'min_messages', 'max_messages', 'model', 'provider', 'user', 'chat_id', 'chat_type', 'branch', 'min_tokens', 'max_tokens', 'min_cost', 'max_cost', 'min_tool_calls', 'max_tool_calls')
        _any_filters = any((getattr(args, a, None) is not None for a in _filter_arg_names))
        filters = None
        if _any_filters:
            try:
        ...
        if args.format == 'trace':
            ...
            if getattr(args, 'upload', False):
                ...
                db.close()
                status = upload_session_trace(resolved, cwd='', redact=redact_trace, private=not getattr(args, 'public', False))
                print(status)
                return
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = Path(args.output).expanduser() if args.output and args.output != '-' else get_hermes_home() / 'session-exports'
        ...
        if args.format == 'jsonl':
            ...
            if args.session_id:
                ...
                if filters:
                    ...
                    if args.dry_run:
                        ...
                        if len(candidates) > 100:
                            print(f'  ... {len(candidates) - 100} more')
                        return
                    sessions = [s for s in (db.export_session(row['id']) for row in candidates) if s]
                else:
                    if args.dry_run:
                        print('--dry-run requires at least one filter.')
                        return
                ...
                if args.output == '-':
                    for s in sessions:
                        sys.stdout.write(_json.dumps(_redact(s), ensure_ascii=False) + '\n')
                else:
                    with open(args.output, 'w', encoding='utf-8') as f:
                        for s in sessions:
            ...
            return
        from hermes_cli.session_export_md import append_manifest_entry, verify_export_file, write_session_markdown
        if args.output == '-':
        ...
        if args.session_id:
            ...
            for target_id in delete_target_ids:
                try:
                    data, exported_path = _export_one(target_id, include_lineage=target_id == resolved_session_id and lineage_is_logical)
            ...
            if args.delete_after_verified:
                ...
                if db.delete_session(resolved_session_id, sessions_dir=sessions_dir, expected_delete_ids=delete_target_ids):
                    delegate_count = len(delete_target_ids) - 1
                    delegate_suffix = '' if not delegate_count else f" and {delegate_count} delegate session{('' if delegate_count == 1 else 's')}"
            ...
            return
        if not filters:
            print('Refusing bulk export without a filter. Pass --session-id or at least one filter (e.g. --older-than 90, --source telegram).')
            db.close()
            return
        candidates = db.list_prune_candidates(**filters)
        if args.dry_run:
        ...
        _non_time_filters = any((getattr(args, a, None) is not None for a in ('source', 'title', 'end_reason', 'cwd', 'min_messages', 'max_messages', 'model', 'provider', 'user', 'chat_id', 'chat_type', 'branch', 'min_tokens', 'max_tokens', 'min_cost', 'max_cost', 'min_tool_calls', 'max_tool_calls')))
        if action == 'prune' and args.older_than is None and (args.newer_than is None) and (args.before is None) and (args.after is None) and (not _non_time_filters):
            args.older_than = '90'
        try:
            filters = build_prune_filters(args)
        except ValueError as e:
        ...
        if action == 'archive' and (not any((v for k, v in filters.items() if k != 'older_than_days'))):
            print('Refusing to archive every ended session: pass at least one filter (e.g. --newer-than 5h, --source cli, --title codex).')
            return
        if action == 'prune':
            filters['archived'] = None if getattr(args, 'include_archived', False) else False
        else:
        ...
        _span = f'oldest activity {format_epoch(_oldest)}, newest activity {format_epoch(_newest)}'
        if args.dry_run or not args.yes:
            ...
            for s in shown:
                ...
                print(f"  {s['id']}  {format_epoch(s.get('last_active')):<17} {s['source']:<10} {model:<24} {s['message_count']:>4} msgs  {title}")
            ...
            if args.dry_run:
                ...
                return
        if not args.yes:
            if not _confirm_prompt(f'{verb} these {len(candidates)} session(s) ({_span})? [y/N] '):
                print('Cancelled.')
                return
        if action == 'prune':
            sessions_dir = get_hermes_home() / 'sessions'
            ...
            print(f"Archived {count} session(s). They're hidden from listings but fully recoverable (nothing was deleted).")
    elif action == 'rename':
        resolved_session_id = db.resolve_session_id(args.session_id)
        if not resolved_session_id:
            print(f"Session '{args.session_id}' not found.")
        ...
        if not candidates:
            ...
            return
        print(f"{len(candidates)} session(s) opened with a /skill{('' if apply_changes else ' (dry run — pass --apply to write)')}:")
        changed = 0
        for row in candidates:
            session_id = row['id']
        ...
        db_path = db.db_path
        before_mb = os.path.getsize(db_path) / (1024 * 1024) if db_path.exists() else 0.0
        ...
        print(f'Database size: {before_mb:.1f} MB -> {after_mb:.1f} MB ({_size_delta_label(saved)})')
        ...
        if free_bytes is not None:
            print(f"  Free disk: {free_bytes / (1024 * 1024):.0f} MB (need ~{need_bytes / (1024 * 1024):.0f} MB to complete{(' incl. VACUUM' if do_vacuum else '')})")
            if free_bytes < need_bytes:
                print()
                print("⚠ Not enough free disk to complete safely. Free up space, or run with --no-vacuum (rebuilds the index but doesn't reclaim space until a later VACUUM).")
                db.close()
                return
        if before_mb > 500:
            print('  This may take a while on a large database. It runs in the foreground with progress below; safe to Ctrl-C and re-run (it resumes).')
        ...
        def _progress(info):
            ...
            if phase == 'backfill':
                print(f"\r  Rebuilding index: {pct:3d}% ({info.get('indexed', 0):,}/{info.get('total', 0):,})", end='', flush=True)
            elif phase != _last['phase']:
                label = {'teardown': 'Reclaiming old index', 'vacuum': 'Compacting database (VACUUM)', 'done': 'Done'}.get(phase, phase)
                print(f'\n  {label}…', flush=True)
            _last['phase'] = phase
        ...
        if result.get('vacuumed') is False:
            print('  (VACUUM was skipped or failed — run `duck-agent sessions optimize` later to reclaim freed space.)')
    elif action == 'stats':
