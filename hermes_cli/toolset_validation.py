...
the tool registry or launching Duck Agent (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``duck-agent-cli`` to the non-existent ``duck-agent``. ``resolve_toolset('duck-agent')``
...
def validate_platform_toolsets(platform_toolsets: object, is_valid_toolset: Callable[[str], bool]) -> List[str]:
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``duck-agent-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``duck-agent`` instead of ``duck-agent-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f'duck-agent-{platform}'
            hint = f" — did you mean '{suggestion}'?" if is_valid_toolset(suggestion) else ''
            warnings.append(f"platform '{platform}' references unknown toolset '{name}'{hint}")
    if valid_count == 0:
        warnings.append('platform_toolsets resolves to zero valid toolsets — the agent will have no tools. Run `duck-agent tools` to reconfigure.')
    return warnings