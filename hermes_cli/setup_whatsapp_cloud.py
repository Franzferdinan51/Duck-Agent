...
Entry point: ``duck-agent whatsapp-cloud`` (dispatched from
...
Duck Agent gateway and the cloudflared tunnel both run in separate
...
def _validate_phone_number_id(value: str) -> tuple[bool, Optional[str]]:
    ...
    if 10 <= len(s) <= 12:
        return (False, "That looks like a phone number — but this field needs the Phone Number ID (Meta's internal ID, 15-17 digits, e.g. '7794189252778687'). Look just BELOW the 'From' dropdown in API Setup → it's labelled 'Phone number ID'.")
    if len(s) < 13:
...
def _validate_app_secret(value: str) -> tuple[bool, Optional[str]]:
    ...
    s = value.strip()
    if not re.fullmatch('[0-9a-f]+', s.lower()):
        return (False, "App Secret should be a hex string (only digits 0-9 and letters a-f). Make sure you copied the 'App secret' from Settings → Basic, not some other token.")
    if len(s) != 32:
...
def _validate_access_token(value: str) -> tuple[bool, Optional[str]]:
    ...
    if not s.startswith('EAA'):
        if s.startswith('sk-'):
            return (False, "That's an OpenAI key (starts with 'sk-'), not a Meta WhatsApp access token. Meta tokens start with 'EAA'.")
        if s.startswith('xoxb-') or s.startswith('xoxp-'):
            return (False, "That's a Slack token, not a Meta WhatsApp access token. Meta tokens start with 'EAA'.")
        if s.startswith('ghp_') or s.startswith('gho_'):
            return (False, "That's a GitHub token, not a Meta WhatsApp access token. Meta tokens start with 'EAA'.")
        return (False, "Meta WhatsApp access tokens start with 'EAA'. Check that you're copying from the right place (API Setup → 'Generate access token', or Business Settings → System Users → 'Generate token' for a permanent one).")
    if len(s) < 100:
...
def _prompt_validated(message: str, validator, *, current: Optional[str]=None, help_text: Optional[str]=None, secret: bool=False) -> Optional[str]:
...
def run_whatsapp_cloud_setup() -> int:
    ...
    print('=' * 50)
    print()
    print("This wizard configures Duck Agent to talk to WhatsApp via Meta's")
    ...
    app_id = _prompt_validated('App ID (optional, press Enter to skip)', lambda v: (True, None) if not v else _validate_app_id(v), current=current_app_id, help_text="Found in: App Dashboard → Settings → Basic → 'App ID' at the\ntop of the page. Numeric, ~15-16 digits.\nNot required for messaging — useful only for analytics later.")
    if app_id:
    ...
    waba_id = _prompt_validated('WABA ID (optional, press Enter to skip)', lambda v: (True, None) if not v else _validate_waba_id(v), current=current_waba_id, help_text="WhatsApp Business Account ID. Found in: App Dashboard →\nWhatsApp → API Setup, near the top — 'WhatsApp Business\nAccount ID'. Numeric, ~15+ digits.\nNot required for messaging — useful for analytics.")
    if waba_id:
    ...
    try:
        allowed = input(f"  → Allowed users{(' [' + allow_default + ']' if allow_default else '')}: ").strip() or (allow_default or '')
    except (EOFError, KeyboardInterrupt):
        allowed = ''
    if allowed:
        allowed = ','.join((re.sub('[\\s\\-+]', '', part) for part in allowed.split(',') if part.strip()))
    ...
    print('─' * 50)
    print()
    print('  Duck Agent needs a public HTTPS URL to receive WhatsApp messages.')
    ...
    print()
    print('    3. Start the Duck Agent gateway in another terminal:')
    print('         duck-agent gateway')
    print()
    ...
    if effective_waba:
        ...
        print(f'        https://business.facebook.com/wa/manage/phone-numbers/?waba_id={effective_waba}')
    else:
        print('    • Display name + profile picture:')
    ...
    print()
    print('  Docs: https://duck-agent.nousresearch.com/docs/user-guide/')
