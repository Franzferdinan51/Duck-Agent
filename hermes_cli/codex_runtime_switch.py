...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Duck Agent'
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {'off', 'default', 'disable', 'duck-agent'}:
        return ('auto', [])
...
def apply(config: dict, new_value: Optional[str], *, persist_callback=None) -> CodexRuntimeStatus:
    ...
    if new_value is None:
        ...
        msg = f"openai_runtime: {current}\ncodex CLI: {('OK ' + ver if ok else 'not available — ' + (ver or 'install with `npm i -g @openai/codex`'))}"
        return CodexRuntimeStatus(success=True, new_value=current, old_value=current, message=msg, codex_binary_ok=ok, codex_version=ver if ok else None)
    reapplying_enable = new_value == current == 'codex_app_server'
    if new_value == current and (not reapplying_enable):
        return CodexRuntimeStatus(success=True, new_value=current, old_value=current, message=f'openai_runtime already set to {current}')
    if new_value == 'codex_app_server':
        ok, ver_or_msg = _check_binary_cached()
        if not ok:
            return CodexRuntimeStatus(success=False, new_value=None, old_value=current, message=f"Cannot enable codex_app_server runtime: {ver_or_msg or 'codex CLI not available'}\nInstall with: npm i -g @openai/codex", codex_binary_ok=False, codex_version=None)
    if not reapplying_enable:
        ...
        if persist_callback is not None:
            try:
                ...
                return CodexRuntimeStatus(success=False, new_value=new_value, old_value=current, message=f'updated config in memory but persist failed: {exc}')
    ...
    if new_value == 'codex_app_server':
        ...
        try:
            ...
            user_servers = [s for s in mig_report.migrated if s != 'duck-agent-tools']
            if user_servers:
                msg_lines.append(f"Migrated {len(user_servers)} MCP server(s): {', '.join(user_servers)}")
            if mig_report.migrated_plugins:
                msg_lines.append(f"Migrated {len(mig_report.migrated_plugins)} native Codex plugin(s): {', '.join(mig_report.migrated_plugins)}")
            ...
            if mig_report.wrote_permissions_default:
                msg_lines.append(f'Default sandbox: {mig_report.wrote_permissions_default} (no approval prompt on every write)')
            if 'duck-agent-tools' in mig_report.migrated:
                msg_lines.append('Duck Agent tool callback registered: codex can now use web_search, web_extract, browser_*, vision_analyze, image_generate, skill_view, skills_list, text_to_speech, kanban_* (worker + orchestrator) via MCP.')
                msg_lines.append('  (delegate_task, memory, session_search, todo run only on the default Duck Agent runtime — they need the agent loop context.)')
        ...
        msg_lines.append('OpenAI/Codex turns now run through `codex app-server` (terminal/file ops/patching inside Codex; Duck Agent tools available via MCP callback).')
        msg_lines.append('Effective on next session — current cached agent keeps the prior runtime to preserve prompt cache.')
    else:
        msg_lines.append('OpenAI/Codex turns will use the default Duck Agent runtime.')
        msg_lines.append('Effective on next session.')
    return CodexRuntimeStatus(success=True, new_value=new_value, old_value=current, message='\n'.join(msg_lines), requires_new_session=True)