...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None=None) -> list[tuple[int, str]]:
    ...
    ``duck-agent dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``duck-agent update`` replaces files on disk, the running
    ...
    ``duck-agent-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Duck Agent Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``duck-agent serve`` as
    ...
    """
    patterns = ['duck-agent dashboard', 'hermes_cli.main dashboard', 'hermes_cli/main.py dashboard', 'duck-agent serve', 'hermes_cli.main serve', 'hermes_cli/main.py serve']
    self_pid = os.getpid()
    dashboard_processes: list[tuple[int, str]] = []
    try:
        if sys.platform == 'win32':
            from hermes_cli._subprocess_compat import windows_hide_flags
            result = subprocess.run(['wmic', 'process', 'get', 'ProcessId,CommandLine', '/FORMAT:LIST'], capture_output=True, text=True, timeout=10, encoding='utf-8', errors='ignore', creationflags=windows_hide_flags())
            ...
            result = subprocess.run(['ps', '-A', '-o', 'pid=,command='], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10)
        ...
        return []
    if exclude_pids:
        dashboard_processes = [proc for proc in dashboard_processes if proc[0] not in exclude_pids]
    return dashboard_processes

def _kill_stale_dashboard_processes(reason: str='the running backend no longer matches the updated frontend', *, restart_managed: bool=False) -> dict[str, list]:
    """Kill running ``duck-agent dashboard`` / ``duck-agent serve`` processes.

    Called at the end of ``duck-agent update`` (default ``reason``) and also
    from ``duck-agent dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``duck-agent update`` path), a detected
    ``duck-agent-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``duck-agent-serve.service``) has its owning unit
    ...
    if sys.platform == 'win32':
        for pid in pids:
            try:
                result = subprocess.run(['taskkill', '/PID', str(pid), '/F'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10)
                if result.returncode == 0:
                    killed.append(pid)
                else:
...
def _detect_concurrent_hermes_instances(scripts_dir: Path, *, exclude_pid: int | None=None) -> list[tuple[int, str]]:
    ...
    The Duck Agent Desktop Electron app spawns ``duck-agent.EXE`` as a backend child,
    so during ``duck-agent update`` the user-invoked process and the desktop's
    ...
    shims (``duck-agent.exe`` / ``duck-agent-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``duck-agent update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``duck-agent.exe``) is a separate process from the Python
