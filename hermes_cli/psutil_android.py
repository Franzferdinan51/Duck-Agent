...
PSUTIL_URL = 'https://files.pythonhosted.org/packages/aa/c6/d1ddf4abb55e93cebc4f2ed8b5d6dbad109ecb8d63748dd2b20ab5e57ebe/psutil-7.2.2.tar.gz'
...
def prepare_patched_psutil_sdist(archive: Path, destination: Path) -> Path:
    ...
    src_roots = sorted((path for path in destination.iterdir() if path.is_dir() and path.name.startswith('psutil-')), key=lambda path: path.name)
    if not src_roots:
    ...
    if not common_py.is_file():
        raise PsutilAndroidInstallError(f'psutil sdist did not contain {common_py.relative_to(src_root)!s}')
    try:
        content = common_py.read_text(encoding='utf-8')
    except OSError as exc:
        raise PsutilAndroidInstallError(f'Failed to read {common_py.relative_to(src_root)!s}') from exc
    ...
    try:
        ...
        raise PsutilAndroidInstallError(f'Failed to write {common_py.relative_to(src_root)!s}') from exc
    return src_root