...
configured for "cli" in `duck-agent tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `duck-agent chat`:
...
def _validate_explicit_toolsets(toolsets: object=None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return (None, f'duck-agent -z: failed to validate --toolsets: {exc}\n')
    ...
    if any((name in {'all', '*'} for name in built_in)):
        ...
        if ignored:
            sys.stderr.write(f"duck-agent -z: --toolsets all enables every toolset; ignoring additional entries: {', '.join(ignored)}\n")
        return (None, None)
    ...
    if unknown:
        sys.stderr.write(f"duck-agent -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(f"duck-agent -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): {', '.join(disabled)}\n")
    if not valid:
        return (None, 'duck-agent -z: --toolsets did not contain any valid toolsets.\n')
    return (valid, None)
...
def run_oneshot(prompt: str, model: Optional[str]=None, provider: Optional[str]=None, toolsets: object=None, usage_file: Optional[str]=None) -> int:
    ...
    if provider and (not ((model or '').strip() or env_model_early)):
        sys.stderr.write('duck-agent -z: --provider requires --model (or HERMES_INFERENCE_MODEL). Pass both explicitly, or neither to use your configured defaults.\n')
        return 2
    ...
    try:
        with redirect_stdout(devnull), redirect_stderr(devnull):
            try:
                response, result = _run_agent(prompt, model=model, provider=provider, toolsets=explicit_toolsets, use_config_toolsets=use_config_toolsets)
    ...
    if failure is not None:
        ...
        real_stderr.write(f'duck-agent -z: agent failed: {failure}\n')
        real_stderr.flush()
        return 1
    ...
    if not (response or '').strip():
        real_stderr.write('duck-agent -z: no final response was produced; treating the run as failed.\n')
        real_stderr.flush()
        return 1
    return 0

def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``duck-agent -z`` / oneshot mode.
...
def _run_agent(prompt: str, model: Optional[str]=None, provider: Optional[str]=None, toolsets: object=None, use_config_toolsets: bool=True) -> tuple[str, dict]:
    ...
    runtime = resolve_runtime_provider(requested=effective_provider, target_model=effective_model or None, explicit_base_url=explicit_base_url_from_alias)
...
def _oneshot_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    ...
    if choices:
        if multi_select:
            return f'[oneshot mode: no user available. Pick the best subset from {choices} using your own judgment and continue.]'
        return f'[oneshot mode: no user available. Pick the best option from {choices} using your own judgment and continue.]'
