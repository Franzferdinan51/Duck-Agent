...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return (False, 'Classic Personal Access Tokens (ghp_*) are not supported by the Copilot API. Use one of:\n  → `copilot login` or `duck-agent model` to authenticate via OAuth\n  → A fine-grained PAT (github_pat_*) with Copilot Requests permission\n  → `gh auth login` with the default device code flow (produces gho_* tokens)')
    return (True, 'OK')
...
def resolve_copilot_token() -> tuple[str, str]:
    ...
    for env_var in COPILOT_ENV_VARS:
        ...
        if val:
            ...
            return (val, env_var)
    if any_env_var_set:
        logger.debug('Copilot env var(s) set but none held a supported token; skipping `gh auth token` fallback to honor explicit env-var intent (and avoid the subprocess cost on cold start, #60800).')
        return ('', '')
...
def _gh_cli_candidates() -> list[str]:
    ...
    for candidate in ('/opt/homebrew/bin/gh', '/usr/local/bin/gh', str(Path.home() / '.local' / 'bin' / 'gh')):
...
def _try_gh_cli_token() -> Optional[str]:
    ...
    for gh_path in _gh_cli_candidates():
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5, env=clean_env, **_popen_kwargs)
...
def copilot_device_code_login(*, host: str='github.com', timeout_seconds: float=300) -> Optional[str]:
    ...
    req = urllib.request.Request(device_code_url, data=data, headers={'Accept': 'application/json', 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'HermesAgent/1.0'})
    try:
    ...
    while time.monotonic() < deadline:
        ...
        poll_data = urllib.parse.urlencode({'client_id': COPILOT_OAUTH_CLIENT_ID, 'device_code': device_code, 'grant_type': 'urn:ietf:params:oauth:grant-type:device_code'}).encode()
        poll_req = urllib.request.Request(access_token_url, data=poll_data, headers={'Accept': 'application/json', 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'HermesAgent/1.0'})
        try:
...
def _save_jwt_to_disk(fp: str, api_token: str, expires_at: float, base_url: Optional[str]) -> None:
    ...
    try:
        ...
        now = time.time()
        store = {k: v for k, v in store.items() if isinstance(v, dict) and float(v.get('expires_at', 0) or 0) > now}
...
def exchange_copilot_token(raw_token: str, *, timeout: float=10.0) -> tuple[str, float, Optional[str]]:
    ...
    if time.time() < _fail_until:
        raise ValueError(f'Copilot token exchange recently failed; skipping re-attempt for another {int(_fail_until - time.time())}s')
    req = urllib.request.Request(_TOKEN_EXCHANGE_URL, method='GET', headers={'Authorization': f'token {raw_token}', 'User-Agent': _EXCHANGE_USER_AGENT, 'Accept': 'application/json', 'Editor-Version': _EDITOR_VERSION})
    data = None
    ...
    for attempt in range(_EXCHANGE_MAX_ATTEMPTS):
        try:
            ...
            if attempt < _EXCHANGE_MAX_ATTEMPTS - 1:
                ...
                logger.debug('Copilot token exchange attempt %d/%d failed (%s); retrying in %.1fs', attempt + 1, _EXCHANGE_MAX_ATTEMPTS, exc, sleep_s)
                time.sleep(sleep_s)
    if data is None:
        ttl = _EXCHANGE_FAILURE_TTL_PERMANENT_SECONDS if permanent_failure else _EXCHANGE_FAILURE_TTL_TRANSIENT_SECONDS
        _exchange_failure_cache[fp] = time.time() + ttl
        raise ValueError(f'Copilot token exchange failed after {_EXCHANGE_MAX_ATTEMPTS} attempts: {last_exc}') from last_exc
    _exchange_failure_cache.pop(fp, None)
...
def _derive_base_url_from_proxy_ep(token: str) -> Optional[str]:
    ...
    """
    import re
    m = re.search('(?:^|;)\\s*proxy-ep=([^;\\s]+)', token)
    if not m:
        return None
    proxy_ep = m.group(1)
...
def copilot_request_headers(*, is_agent_turn: bool=True, is_vision: bool=False) -> dict[str, str]:
    ...
    """
    headers: dict[str, str] = {'Editor-Version': 'vscode/1.104.1', 'User-Agent': 'HermesAgent/1.0', 'Copilot-Integration-Id': 'vscode-chat', 'Openai-Intent': 'conversation-edits', 'x-initiator': 'agent' if is_agent_turn else 'user'}
    if is_vision:
