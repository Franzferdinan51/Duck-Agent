"""duck-agent claw — OpenClaw migration commands.

Usage:
    duck-agent claw migrate              # Preview then migrate (always shows preview first)
    duck-agent claw migrate --dry-run    # Preview only, no changes
    duck-agent claw migrate --yes        # Skip confirmation prompt
    duck-agent claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    duck-agent claw migrate --no-backup  # Skip pre-migration snapshot
    duck-agent claw cleanup              # Archive leftover OpenClaw directories
    duck-agent claw cleanup --dry-run    # Preview what would be archived
"""
...
def _detect_openclaw_processes() -> list[str]:
    ...
    if sys.platform != 'win32':
        try:
            result = subprocess.run(['systemctl', '--user', 'is-active', 'openclaw-gateway.service'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
    ...
    if sys.platform == 'win32':
        try:
            for exe in ('openclaw.exe', 'clawd.exe'):
                result = subprocess.run(['tasklist', '/FI', f'IMAGENAME eq {exe}'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
                if exe in result.stdout.lower():
                    found.append(f'process: {exe}')
            ps_cmd = 'Get-CimInstance Win32_Process -Filter "Name = \'node.exe\'" | Where-Object { $_.CommandLine -match "openclaw|clawd" } | Select-Object -First 1 ProcessId'
            result = subprocess.run(['powershell', '-NoProfile', '-Command', ps_cmd], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
        ...
        try:
            result = subprocess.run(['pgrep', '-f', 'openclaw'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3)
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    for detail in running:
        print_info(f'  * {detail}')
    print_info('Messaging platforms (Telegram, Discord, Slack) only allow one active session per bot token. If you continue, both OpenClaw and Duck Agent may try to use the same token, causing disconnects.')
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Duck Agent gateway is running with connected platforms.
    ...
    connected = [name for name, info in platforms.items() if isinstance(info, dict) and info.get('state') == 'connected']
    if not connected:
        return
    print()
    print_error('Duck Agent gateway is running with active connections: ' + ', '.join(connected))
    print_info('Migrating bot tokens while the gateway is active will cause conflicts (Telegram, Discord, and Slack only allow one active session per token).')
    print_info("Recommendation: stop the gateway first with 'duck-agent gateway stop'.")
    print()
...
def claw_command(args):
    """Route duck-agent claw subcommands."""
    ...
    if action == 'migrate':
        ...
        print('Usage: duck-agent claw <command> [options]')
        print()
        print('Commands:')
        print('  migrate          Migrate settings from OpenClaw to Duck Agent')
        print('  cleanup          Archive leftover OpenClaw directories after migration')
        print()
        print("Run 'duck-agent claw <command> --help' for options.")

def _cmd_migrate(args):
    """Run the OpenClaw → Duck Agent migration."""
    ...
    print(color('│          ⚕ Duck Agent — OpenClaw Migration                 │', Colors.MAGENTA))
    print(color('└─────────────────────────────────────────────────────────┘', Colors.MAGENTA))
    if not source_dir.is_dir():
        print()
        ...
        print_info('You can specify a custom path: duck-agent claw migrate --source /path/to/.openclaw')
        return
    ...
    try:
        preview = mod.Migrator(source_root=source_dir.resolve(), target_root=hermes_home.resolve(), execute=False, workspace_target=ws_target, overwrite=overwrite, migrate_secrets=migrate_secrets, output_dir=None, selected_options=selected, preset_name=preset, skill_conflict_mode=skill_conflict)
    ...
    if preview_count > 0:
        ...
        print_header(f'Migration Preview — {preview_conflicts} conflict(s), nothing would be imported')
    ...
    if preview_conflicts > 0 and (not overwrite):
        ...
        print_info('Each conflict is an item whose target already exists in ~/.duck-agent/. Re-run with --overwrite to replace conflicting targets (item-level backups are written to the migration report directory).')
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info('To execute, re-run with: duck-agent claw migrate --yes')
            return
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f'Restore with: duck-agent import {backup_archive.name}')
        except Exception as e:
            print()
            print_error(f'Could not create pre-migration backup: {e}')
            print_info('Re-run with --no-backup to skip, or free up disk space under the Duck Agent home.')
            logger.debug('Pre-migration backup error', exc_info=True)
            return
    try:
        migrator = mod.Migrator(source_root=source_dir.resolve(), target_root=hermes_home.resolve(), execute=True, workspace_target=ws_target, overwrite=overwrite, migrate_secrets=migrate_secrets, output_dir=None, selected_options=selected, preset_name=preset, skill_conflict_mode=skill_conflict)
...
def _cmd_cleanup(args):
    ...
    print()
    print(color('┌─────────────────────────────────────────────────────────┐', Colors.MAGENTA))
    print(color('│          ⚕ Duck Agent — OpenClaw Cleanup                   │', Colors.MAGENTA))
    print(color('└─────────────────────────────────────────────────────────┘', Colors.MAGENTA))
    if explicit_source:
    ...
    if running:
        ...
        for detail in running:
            print_info(f'  * {detail}')
        print_info('Archiving .openclaw/ while the service is active may cause it to immediately recreate an empty skeleton directory, destroying your config.')
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no('Proceed anyway?', default=False):
                print_info('Aborted. Stop OpenClaw first, then re-run: duck-agent claw cleanup')
                return
    total_archived = 0
    for source_dir in dirs_to_check:
        ...
        try:
            workspace_dirs = [d for d in source_dir.iterdir() if d.is_dir() and (not d.name.startswith('.')) and any(((d / name).exists() for name in ('todo.json', 'SOUL.md', 'MEMORY.md', 'USER.md')))]
        except OSError:
            workspace_dirs = []
        if workspace_dirs:
        ...
        if dry_run:
            ...
            print_info('To execute, re-run with: duck-agent claw cleanup --yes')
    ...
    if dry_run:
        ...
        print_info(f"Dry run complete. {_n_dirs} {('directory' if _n_dirs == 1 else 'directories')} would be archived.")
        print_info('Run without --dry-run to archive them.')
    elif total_archived:
        print_success(f"Cleaned up {total_archived} OpenClaw {('directory' if total_archived == 1 else 'directories')}.")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  duck-agent claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        print_success('Migration complete!')
        skipped_keys = [i for i in report.get('items', []) if i.get('kind') == 'provider-keys' and i.get('status') == 'skipped']
        if skipped_keys:
            print()
            ...
            print_info('  duck-agent claw migrate --migrate-secrets')
            print()
            print_info('Or add your key manually:')
            print_info('  duck-agent config set OPENROUTER_API_KEY sk-or-v1-...')