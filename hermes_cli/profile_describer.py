...
Used by ``duck-agent profile describe <name> --auto`` and the dashboard's
...
def describe_profile(profile_name: str, *, overwrite: bool=False, timeout: Optional[int]=None) -> DescribeOutcome:
    ...
    if existing.get('description') and (not existing.get('description_auto')) and (not overwrite):
        return DescribeOutcome(canon, False, 'profile already has a user-authored description (use --overwrite to replace)')
    skill_names = _collect_skills(profile_dir)
    skill_list = '\n'.join((f'  - {n}' for n in skill_names)) or '  (no skills installed)'
    skill_count = sum((1 for _ in (profile_dir / 'skills').rglob('SKILL.md') if not is_excluded_skill_path(_))) if (profile_dir / 'skills').is_dir() else 0
    try:
    ...
    user_msg = _USER_TEMPLATE.format(name=canon, model=model or '(unset)', provider=provider or '(unset)', skill_count=skill_count, skill_cap=MAX_SKILLS_FOR_PROMPT, skill_list=skill_list)
    try:
        resp = call_llm(task='profile_describer', messages=[{'role': 'system', 'content': _SYSTEM_PROMPT}, {'role': 'user', 'content': user_msg}], temperature=0.3, max_tokens=400, timeout=timeout or 60)
    except Exception as exc:
