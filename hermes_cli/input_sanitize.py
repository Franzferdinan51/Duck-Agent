...
import re
_BRACKETED_PASTE_BOUNDARY_START = re.compile('(^|[\\s\\n>:\\]\\)])\\[200~')
_BRACKETED_PASTE_BOUNDARY_END = re.compile('\\[201~(?=$|[\\s\\n<\\[\\(\\):;.,!?])')
_BRACKETED_PASTE_DEGRADED_START = re.compile('(^|[\\s\\n>:\\]\\)])00~')
_BRACKETED_PASTE_DEGRADED_END = re.compile('01~(?=$|[\\s\\n<\\[\\(\\):;.,!?])')
...
def strip_leaked_bracketed_paste_wrappers(text: str) -> str:
    ...
    if not text:
        return text
    text = text.replace('\x1b[200~', '').replace('\x1b[201~', '').replace('^[[200~', '').replace('^[[201~', '')
    text = _BRACKETED_PASTE_BOUNDARY_START.sub('\\1', text)
    text = _BRACKETED_PASTE_BOUNDARY_END.sub('', text)
    text = _BRACKETED_PASTE_DEGRADED_START.sub('\\1', text)
