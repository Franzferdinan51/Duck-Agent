"""``duck-agent doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``duck-agent mcp test``.
    """
...
def _probe_firecrawl(timeout: float) -> ProbeResult:
    ...
    resp = _http_get(FIRECRAWL_HEALTH_URL, headers={'Authorization': f'Bearer {key}'}, timeout=timeout)
...
def _probe_fal(timeout: float) -> ProbeResult:
    ...
    resp = _http_get(FAL_MODELS_URL, headers={'Authorization': f'Key {key}'}, timeout=timeout)
...
def _audio_provider_probe(kind: str, provider: str, timeout: float) -> ProbeResult:
    ...
    probes = {'openai': (OPENAI_MODELS_URL, 'OPENAI_API_KEY', 'Bearer'), 'groq': (GROQ_MODELS_URL, 'GROQ_API_KEY', 'Bearer'), 'elevenlabs': (ELEVENLABS_VOICES_URL, 'ELEVENLABS_API_KEY', 'xi')}
...
def run_live_checks(issues: List[str]) -> List[ProbeResult]:
    ...
    results.append(_run_one('Firecrawl', lambda: _probe_firecrawl(timeout), issues))
    results.append(_run_one('FAL', lambda: _probe_fal(timeout), issues))
    ...
    if isinstance(servers, dict) and servers:
        for name in sorted(servers):
            ...
            def _probe(n=name, e=entry) -> ProbeResult:
                ...
                return ProbeResult(f'MCP: {n}', 'pass', f'({len(tools)} tool(s))')
    ...
    results.append(_run_one('TTS', lambda: _probe_tts(config, timeout), issues))
    results.append(_run_one('STT', lambda: _probe_stt(config, timeout), issues))
    return results
