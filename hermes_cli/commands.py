"""Slash command definitions and autocomplete for the Duck Agent CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [CommandDef('start', 'Acknowledge platform start pings without a reply', 'Session', gateway_only=True, busy_policy='dispatch', busy_handler='start'), CommandDef('new', 'Start a new session (fresh session ID + history)', 'Session', aliases=('reset',), args_hint='[name]', busy_policy='interrupt_then_dispatch', busy_handler='new'), CommandDef('topic', 'Enable or inspect Telegram DM topic sessions', 'Session', gateway_only=True, args_hint='[off|help|session-id]'), CommandDef('clear', 'Clear screen and start a new session', 'Session', cli_only=True), CommandDef('redraw', 'Force a full UI repaint (recovers from terminal drift)', 'Session', cli_only=True), CommandDef('history', 'Show conversation history', 'Session', cli_only=True), CommandDef('save', 'Save the current conversation', 'Session', cli_only=True), CommandDef('retry', 'Retry the last message (resend to agent)', 'Session'), CommandDef('prompt', 'Compose your next prompt in $EDITOR (markdown), then send it', 'Session', cli_only=True, args_hint='[initial text]', aliases=('compose',)), CommandDef('undo', 'Back up N user turns and re-prompt (default 1)', 'Session', args_hint='[N]'), CommandDef('title', 'Set a title for the current session', 'Session', args_hint='[name]'), CommandDef('handoff', 'Hand off this session to a messaging platform (Telegram, Discord, etc.)', 'Session', args_hint='<platform>', cli_only=True), CommandDef('branch', 'Branch the current session (explore a different path)', 'Session', aliases=('fork',), args_hint='[name]'), CommandDef('compress', "Compress conversation context (add 'here [N]' to keep recent N turns; --preview shows what would happen)", 'Session', aliases=('compact',), args_hint='[here [N] | focus topic | --preview|--dry-run]'), CommandDef('rollback', 'List or restore filesystem checkpoints', 'Session', args_hint='[number]'), CommandDef('snapshot', 'Create or restore state snapshots of Duck Agent config/state', 'Session', cli_only=True, aliases=('snap',), args_hint='[create|restore <id>|prune]'), CommandDef('export', 'Export a profile (config, skills, theme) to a shareable archive', 'Configuration', cli_only=True, args_hint='[profile] [-o output.tar.gz]'), CommandDef('import', 'Import a shared profile archive as a new profile', 'Configuration', cli_only=True, args_hint='<archive.tar.gz> [--name <name>]'), CommandDef('stop', 'Kill all running background processes', 'Session', busy_policy='interrupt_then_dispatch', busy_handler='stop'), CommandDef('approve', 'Approve a pending dangerous command', 'Session', gateway_only=True, args_hint='[session|always]', busy_policy='dispatch'), CommandDef('deny', 'Deny a pending dangerous command (optionally with a reason)', 'Session', gateway_only=True, args_hint='[all] [reason]', busy_policy='dispatch'), CommandDef('background', 'Run a prompt in the background', 'Session', aliases=('bg', 'btw'), args_hint='<prompt>', busy_policy='dispatch'), CommandDef('agents', 'Show active agents and running tasks', 'Session', aliases=('tasks',), busy_policy='dispatch'), CommandDef('journey', 'Open the learning journey timeline', 'Session', aliases=('learning', 'memory-graph'), cli_only=True, args_hint='[list|delete <id>|edit <id>]', subcommands=('list', 'delete', 'edit')), CommandDef('queue', "Queue a prompt for the next turn (doesn't interrupt)", 'Session', aliases=('q',), args_hint='<prompt>', busy_policy='dispatch', busy_handler='queue'), CommandDef('steer', 'Inject a message after the next tool call without interrupting', 'Session', args_hint='<prompt>', busy_policy='dispatch', busy_handler='steer'), CommandDef('goal', 'Set a standing goal Duck Agent works on across turns until achieved', 'Session', args_hint='[text | draft <text> | show | gate add <cmd> | pause | resume | clear | status | wait <pid> | unwait]', busy_policy='dispatch', busy_handler='goal'), CommandDef('heartbeat', 'Set a recurring prompt that re-enters this session when idle', 'Session', aliases=('hb',), args_hint='[every <interval> <prompt> | status | pause | resume | clear]', subcommands=('status', 'pause', 'resume', 'clear'), busy_policy='dispatch'), CommandDef('refine', 'Review this conversation now and save lessons to memory/skills', 'Session', args_hint='[focus instructions]'), CommandDef('moa', 'Run one prompt through the default Mixture of Agents preset, then restore your model', 'Session', args_hint='<prompt>', busy_policy='reject', busy_handler='moa'), CommandDef('subgoal', 'Add or manage extra criteria on the active goal', 'Session', args_hint='[text | remove N | clear]', busy_policy='dispatch'), CommandDef('status', 'Show session, model, token, and context info', 'Session', busy_policy='dispatch'), CommandDef('egress', 'Show Docker egress proxy status', 'Session', args_hint='[status]', subcommands=('status',), busy_policy='dispatch', busy_handler='egress', execute='egress'), CommandDef('context', 'Show detailed context window view with usage gauge, category breakdown, compression stats, and throughput', 'Session', aliases=('ctx',), args_hint='[all]', subcommands=('all',), busy_policy='dispatch'), CommandDef('whoami', 'Show your slash command access (admin / user)', 'Info'), CommandDef('profile', 'Show active profile name and home directory', 'Info', busy_policy='dispatch', execute='profile'), CommandDef('sethome', 'Set this chat as the home channel', 'Session', gateway_only=True, aliases=('set-home',)), CommandDef('resume', 'Resume a previously-named session', 'Session', args_hint='[name]'), CommandDef('sessions', 'Browse and resume previous sessions', 'Session'), CommandDef('config', 'Show current configuration', 'Configuration', cli_only=True), CommandDef('model', 'Switch model (session-scoped; --global to persist)', 'Configuration', args_hint='[model] [--provider name] [--global|--session] [--refresh]', busy_policy='reject', busy_handler='model'), CommandDef('codex-runtime', 'Toggle codex app-server runtime for OpenAI/Codex models', 'Configuration', aliases=('codex_runtime',), args_hint='[auto|codex_app_server]', busy_policy='reject', busy_handler='codex-runtime'), CommandDef('personality', 'Set a predefined personality', 'Configuration', args_hint='[name]'), CommandDef('statusbar', 'Toggle the context/model status bar', 'Configuration', cli_only=True, aliases=('sb',)), CommandDef('battery', 'Toggle a color-coded battery indicator in the status bar', 'Configuration', cli_only=True, args_hint='[on|off|status]', subcommands=('on', 'off', 'status')), CommandDef('timestamps', 'Toggle [HH:MM] timestamps on messages and /history', 'Configuration', cli_only=True, args_hint='[on|off|status]', subcommands=('on', 'off', 'status'), aliases=('ts',)), CommandDef('diff', 'Show git changes in the working directory', 'Info', args_hint='[staged|all|session] [--stat] [path...]', subcommands=('staged', 'all', 'session')), CommandDef('verbose', 'Cycle tool progress display: off -> new -> all -> verbose -> log', 'Configuration', cli_only=True, gateway_config_gate='display.tool_progress_command', busy_policy='dispatch'), CommandDef('focus', 'Toggle focus view — show only your prompt and the final response', 'Configuration', cli_only=True, args_hint='[on|off|status]', subcommands=('on', 'off', 'status')), CommandDef('footer', 'Toggle gateway runtime-metadata footer on final replies', 'Configuration', args_hint='[on|off|status]', subcommands=('on', 'off', 'status'), busy_policy='dispatch'), CommandDef('yolo', 'Toggle YOLO mode (skip all dangerous command approvals)', 'Configuration', busy_policy='dispatch'), CommandDef('approvals', 'Show or set the persistent dangerous-command approval mode', 'Configuration', args_hint='[manual|smart|off]', subcommands=('manual', 'smart', 'off')), CommandDef('reasoning', 'Manage reasoning effort and display', 'Configuration', args_hint='[level|show|hide|full|clamp] [--global]', subcommands=('none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max', 'ultra', 'show', 'hide', 'on', 'off', 'full', 'clamp', '--global')), CommandDef('fast', 'Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode (Normal/Fast)', 'Configuration', args_hint='[normal|fast|status] [--global]', subcommands=('normal', 'fast', 'status', 'on', 'off', '--global')), CommandDef('skin', 'Show or change the display skin/theme', 'Configuration', cli_only=True, args_hint='[name]'), CommandDef('indicator', 'Pick the TUI busy-indicator style', 'Configuration', cli_only=True, args_hint=f"[{'|'.join(INDICATOR_STYLES)}]", subcommands=INDICATOR_STYLES), CommandDef('voice', 'Toggle voice mode', 'Configuration', args_hint='[on|off|tts|status]', subcommands=('on', 'off', 'tts', 'status')), CommandDef('wake', "Toggle the 'Hey Duck Agent' wake word listener", 'Configuration', cli_only=True, args_hint='[on|off|status]', subcommands=('on', 'off', 'status')), CommandDef('busy', 'Control what Enter does while Duck Agent is working', 'Configuration', cli_only=True, args_hint='[queue|steer|interrupt|status]', subcommands=('queue', 'steer', 'interrupt', 'status')), CommandDef('tools', 'Manage tools: /tools [list|disable|enable] [name...]', 'Tools & Skills', args_hint='[list|disable|enable] [name...]', cli_only=True), CommandDef('toolsets', 'List available toolsets', 'Tools & Skills', cli_only=True), CommandDef('skills', 'Search, install, inspect, or manage skills', 'Tools & Skills', cli_only=True, gateway_config_gate='skills.write_approval', subcommands=('search', 'browse', 'inspect', 'install', 'audit', 'pending', 'approve', 'reject', 'diff', 'approval')), CommandDef('memory', 'Review pending memory writes / toggle the approval gate', 'Tools & Skills', args_hint='[pending|approve|reject|approval] [id|on|off]', subcommands=('pending', 'approve', 'reject', 'approval')), CommandDef('bundles', 'List skill bundles (aliases /<name> for multiple skills)', 'Tools & Skills', execute='bundles'), CommandDef('pet', 'Toggle or adopt a petdex mascot (/pet, /pet list, /pet <slug>)', 'Tools & Skills', cli_only=True, args_hint='[toggle|list|scale <n>|<slug>]', subcommands=('toggle', 'list', 'scale', 'off')), CommandDef('hatch', 'Generate a new petdex pet from a description', 'Tools & Skills', cli_only=True, aliases=('generate-pet',), args_hint='[description]'), CommandDef('learn', 'Learn a reusable skill from anything you describe (dirs, URLs, this chat, notes)', 'Tools & Skills', args_hint='<what to learn from>'), CommandDef('init', 'Generate or update AGENTS.md project instructions from a repo scan', 'Tools & Skills', args_hint='[notes]'), CommandDef('cron', 'Manage scheduled tasks', 'Tools & Skills', cli_only=True, args_hint='[subcommand]', subcommands=('list', 'add', 'create', 'edit', 'pause', 'resume', 'run', 'remove')), CommandDef('suggestions', 'Review suggested automations (accept/dismiss)', 'Tools & Skills', aliases=('suggest',), args_hint='[accept|dismiss N | catalog]', subcommands=('accept', 'dismiss', 'catalog', 'clear')), CommandDef('blueprint', 'Set up an automation from a blueprint template', 'Tools & Skills', aliases=('bp',), args_hint='[name] [slot=value ...]'), CommandDef('curator', 'Background skill maintenance (status, run, pin, archive, list-archived)', 'Tools & Skills', args_hint='[subcommand]', subcommands=('status', 'run', 'pause', 'resume', 'pin', 'unpin', 'restore', 'list-archived')), CommandDef('kanban', 'Multi-profile collaboration board (tasks, links, comments)', 'Tools & Skills', args_hint='[subcommand]', subcommands=('init', 'boards', 'create', 'list', 'ls', 'show', 'assign', 'reclaim', 'reassign', 'diagnostics', 'diag', 'link', 'unlink', 'claim', 'comment', 'complete', 'edit', 'block', 'unblock', 'archive', 'tail', 'dispatch', 'stats', 'notify-subscribe', 'notify-list', 'notify-unsubscribe', 'log', 'runs', 'heartbeat', 'assignees', 'context', 'specify', 'gc'), busy_policy='dispatch'), CommandDef('reload', 'Reload .env variables into the running session', 'Tools & Skills', cli_only=True), CommandDef('reload-mcp', 'Reload MCP servers from config', 'Tools & Skills', aliases=('reload_mcp',)), CommandDef('reload-skills', 'Re-scan ~/.duck-agent/skills/ for newly installed or removed skills', 'Tools & Skills', aliases=('reload_skills',)), CommandDef('browser', 'Connect browser tools to your live Chromium-family browser via CDP', 'Tools & Skills', cli_only=True, args_hint='[connect|disconnect|status]', subcommands=('connect', 'disconnect', 'status')), CommandDef('plugins', 'List installed plugins and their status', 'Tools & Skills', cli_only=True), CommandDef('commands', 'Browse all commands and skills (paginated)', 'Info', gateway_only=True, args_hint='[page]', busy_policy='dispatch', execute='gateway_commands'), CommandDef('help', 'Show available commands', 'Info', busy_policy='dispatch', execute='gateway_help'), CommandDef('restart', 'Gracefully restart the gateway after draining active runs', 'Session', gateway_only=True, busy_policy='dispatch'), CommandDef('usage', 'Show token usage and rate limits; `reset` redeems a banked Codex limit reset', 'Info', args_hint='[reset [--force]]'), CommandDef('subscription', 'View your Nous plan and change it in the browser', 'Info', cli_only=True, aliases=('upgrade',)), CommandDef('topup', 'Show your Nous balance and manage billing on the portal', 'Info'), CommandDef('insights', 'Show usage insights and analytics', 'Info', args_hint='[days]'), CommandDef('platforms', 'Show gateway/messaging platform status', 'Info', cli_only=True, aliases=('gateway',)), CommandDef('platform', 'Pause, resume, or list a failing gateway platform', 'Info', gateway_only=True, args_hint='<pause|resume|list> [name]'), CommandDef('copy', 'Copy the last assistant response to clipboard', 'Info', cli_only=True, args_hint='[number]'), CommandDef('paste', 'Attach clipboard image from your clipboard', 'Info', cli_only=True), CommandDef('image', 'Attach a local image file for your next prompt', 'Info', cli_only=True, args_hint='<path>'), CommandDef('update', 'Update Duck Agent to the latest version', 'Info', busy_policy='dispatch'), CommandDef('version', 'Show Duck Agent version', 'Info', aliases=('v',), busy_policy='dispatch', execute='version'), CommandDef('debug', 'Upload debug report (system info + logs) and get shareable links', 'Info', args_hint='[nous|local]'), CommandDef('quit', 'Exit the CLI (use --delete to also remove session history)', 'Exit', cli_only=True, aliases=('exit',), args_hint='[--delete]')]
...
_PIPE_SUBS_RE = re.compile('[a-z]+(?:\\|[a-z]+)+')
for _cmd in COMMAND_REGISTRY:
    key = f'/{_cmd.name}'
...
def is_gateway_known_command(name: str | None) -> bool:
    ...
    for plugin_name, _description, _args_hint in _iter_plugin_command_entries():
        if plugin_name == name:
            return True
    return False
ACTIVE_SESSION_BYPASS_COMMANDS: frozenset[str] = frozenset((cmd.name for cmd in COMMAND_REGISTRY if cmd.busy_policy != 'reject'))
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    Telegram command menu, in Slack's ``/duck-agent`` subcommand mapping, and
...
_TELEGRAM_MENU_PRIORITY = ('help', 'new', 'stop', 'status', 'egress', 'resume', 'sessions', 'model', 'debug', 'restart', 'update', 'verbose', 'commands', 'approve', 'deny', 'queue', 'steer', 'background', 'reasoning', 'usage', 'platforms', 'platform', 'profile', 'whoami')
"Built-in commands that should stay visible in Telegram's capped menu.\n\nTelegram only displays a small BotCommand menu in practice.  The full Duck Agent\nregistry is still dispatchable when typed manually, but operational commands\nneed to survive the visible menu cap ahead of lower-priority built-ins.\n"
...
def _telegram_command_menu_config() -> dict[str, Any]:
    ...
    if isinstance(raw_priority, list):
        ...
        priority = []
    return {'max_commands': max_commands, 'priority_mode': priority_mode, 'priority': priority}
...
def _prioritize_telegram_menu_commands(commands: list[tuple[str, str]]) -> list[tuple[str, str]]:
    priority = {name: index for index, name in enumerate(_telegram_effective_priority())}
    return [command for _index, command in sorted(enumerate(commands), key=lambda item: (0, priority[item[1][0]], item[0]) if item[1][0] in priority else (1, item[0]))]
_CMD_NAME_LIMIT = 32
'Max command name length shared by Telegram and Discord.'
_TG_NAME_LIMIT = _CMD_NAME_LIMIT
_TG_INVALID_CHARS = re.compile('[^a-z0-9_]')
_TG_MULTI_UNDERSCORE = re.compile('_{2,}')
...
def _clamp_command_names(entries: list[tuple[str, ...]], reserved: set[str]) -> list[tuple[str, ...]]:
...
def _collect_gateway_skill_entries(platform: str, max_slots: int, reserved_names: set[str], desc_limit: int=100, sanitize_name: 'Callable[[str], str] | None'=None) -> tuple[list[tuple[str, str, str]], int]:
...
def telegram_menu_commands(max_commands: int=100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``duck-agent skills
    ...
    entries, hidden_count = _collect_gateway_skill_entries(platform='telegram', max_slots=remaining_slots, reserved_names=reserved_names, desc_limit=40, sanitize_name=_sanitize_telegram_name)
...
def discord_skill_commands(max_slots: int, reserved_names: set[str]) -> tuple[list[tuple[str, str, str]], int]:
    ...
    """
    return _collect_gateway_skill_entries(platform='discord', max_slots=max_slots, reserved_names=set(reserved_names), desc_limit=100)
...
def discord_skill_commands_by_category(reserved_names: set[str]) -> tuple[dict[str, list[tuple[str, str, str]]], list[tuple[str, str, str]], int]:
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``duck-agent skills list`` and
    ...
    try:
        ...
        for cmd_key in sorted(skill_cmds):
            ...
            if discord_name in _names_used:
                ...
                if prior == '<reserved>':
                    logger.warning("Discord /skill: %r (from %r) collides on its 32-char clamp with a reserved gateway command name %r — the skill will not appear in the /skill autocomplete. Rename the skill's frontmatter ``name:`` to differ in its first 32 chars.", discord_name, cmd_key, discord_name)
                else:
                    logger.warning("Discord /skill: %r and %r both clamp to %r on Discord's 32-char command-name limit — only %r will appear in the /skill autocomplete. Rename one skill's frontmatter ``name:`` to differ in its first 32 chars.", prior, cmd_key, discord_name, prior)
                hidden += 1
                continue
...
_SLACK_INVALID_CHARS = re.compile('[^a-z0-9_\\-]')
_SLACK_RESERVED_COMMANDS = frozenset({'me', 'status', 'away', 'dnd', 'shrug', 'remind', 'msg', 'feed', 'who', 'collapse', 'expand', 'leave', 'join', 'open', 'search', 'topic', 'mute', 'pro', 'shortcuts'})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/duck-agent <verb>`` subcommand.
    ...
    can still reach them via ``/duck-agent <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/duck-agent`` is always reserved as the first entry so the
    legacy ``/duck-agent <subcommand>`` form keeps working for anything that
    gets dropped by the clamp or for free-form questions.
    """
    overrides = _resolve_config_gates()
    entries: list[tuple[str, str, str]] = []
    seen: set[str] = set()
    entries.append(('duck-agent', 'Talk to Duck Agent or run a subcommand', '[subcommand] [args]'))
    seen.add('duck-agent')
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        seen.add(slack_name)
    _alias_to_cmd = {alias: cmd for cmd in COMMAND_REGISTRY if _is_gateway_available(cmd, overrides) for alias in cmd.aliases}
...
def slack_app_manifest(request_url: str='https://duck-agent.local/slack/commands') -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /duck-agent handler.

    Maps both canonical names and aliases so /duck-agent bg do stuff works
    the same as /duck-agent background do stuff.

    Plugin-registered slash commands are included so ``/duck-agent <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def __init__(self, skill_commands_provider: Callable[[], Mapping[str, dict[str, Any]]] | None=None, command_filter: Callable[[str], bool] | None=None, skill_bundles_provider: Callable[[], Mapping[str, dict[str, Any]]] | None=None) -> None:
    ...
    def _stacked_skill_completions(self, text: str):
        ...
        for cmd, info in self._iter_skill_commands().items():
            ...
            yield Completion(replacement, start_position=-len(current_word), display=cmd, display_meta=f'⚡ {short_desc}')
    ...
    def _path_completions(word: str, limit: int=30):
        ...
        for entry in sorted(entries):
            ...
            yield Completion(display_path, start_position=-len(word), display=entry + suffix, display_meta=meta)
            count += 1
    ...
    def _context_completions(self, word: str, limit: int=30):
        ...
        """
        lowered = word.lower()
        _STATIC_REFS = (('@diff', 'Git working tree diff'), ('@staged', 'Git staged diff'), ('@file:', 'Attach a file'), ('@folder:', 'Attach a folder'), ('@git:', 'Git log with diffs (e.g. @git:5)'), ('@url:', 'Fetch web content'))
        ...
        for prefix in ('@file:', '@folder:'):
            ...
            if word == bare or word.startswith(prefix):
                ...
                for entry in sorted(entries):
                    ...
                    yield Completion(completion, start_position=-len(word), display=entry + suffix, display_meta=meta)
                    count += 1
                return
        query = word[1:]
    ...
    def _get_project_files(self) -> list[str]:
        ...
        files: list[str] = []
        for cmd in [['rg', '--files', '--sortr=modified', cwd], ['rg', '--files', cwd], ['fd', '--type', 'f', '--base-directory', cwd]]:
            tool = cmd[0]
            if not shutil.which(tool):
                continue
            try:
                proc = subprocess.run(cmd, capture_output=True, text=True, timeout=2, cwd=cwd, encoding='utf-8', errors='replace')
    ...
    def _fuzzy_file_completions(self, word: str, query: str, limit: int=20):
        ...
        if not query:
            for fp in files[:limit]:
                ...
                yield Completion(f'@{kind}:{fp}', start_position=-len(word), display=filename, display_meta=meta)
            return
        scored = []
        for fp in files:
        ...
        for _, fp in scored[:limit]:
            ...
            kind = 'folder' if is_dir else 'file'
            meta = 'dir' if is_dir else _file_size_label(os.path.join(os.getcwd(), fp))
            yield Completion(f'@{kind}:{fp}', start_position=-len(word), display=filename, display_meta=f'{fp}  {meta}' if meta else fp)
        ...
        try:
            ...
            for s in list_skins():
                ...
                if name.startswith(sub_lower) and name != sub_lower:
                    yield Completion(name, start_position=-len(sub_text), display=name, display_meta=s.get('description', '') or s.get('source', ''))
        except Exception:
            pass
        ...
        try:
            ...
            for ts_key, label, _desc in CONFIGURABLE_TOOLSETS:
                ...
                if subcommand == 'disable' and (not is_on):
                    continue
                yield Completion(ts_key, start_position=-len(partial), display=ts_key, display_meta=label)
            for ts_key in sorted(_get_plugin_toolset_keys()):
                ...
                if subcommand == 'disable' and (not is_on):
                    continue
                yield Completion(ts_key, start_position=-len(partial), display=ts_key, display_meta='plugin toolset')
            ...
            if isinstance(mcp_servers, dict):
                for server in sorted(mcp_servers):
                    ...
                    if prefix in already or not prefix.startswith(partial_lower):
                        continue
                    yield Completion(prefix, start_position=-len(partial), display=prefix, display_meta=f"MCP server '{server}'")
        except Exception:
            return
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        ...
        try:
            ...
            if 'none'.startswith(sub_lower) and 'none' != sub_lower:
                yield Completion('none', start_position=-len(sub_text), display='none', display_meta='clear personality overlay')
            for name, prompt in personalities.items():
                if name.startswith(sub_lower) and name != sub_lower:
                    if isinstance(prompt, dict):
                        ...
                        meta = str(prompt)[:50]
                    yield Completion(name, start_position=-len(sub_text), display=name, display_meta=meta)
        except Exception:
            pass
    ...
    def get_completions(self, document, complete_event):
        ...
        for cmd, info in self._iter_skill_bundles().items():
            ...
            if cmd_name.startswith(word):
                ...
                yield Completion(self._completion_text(cmd_name, word), start_position=-len(word), display=cmd, display_meta=f'▣ {short_desc} ({skill_count} skills)')
        for cmd, info in self._iter_skill_commands().items():
            cmd_name = cmd[1:]
            if cmd_name.startswith(word):
                description = str(info.get('description', 'Skill command'))
                short_desc = description[:50] + ('...' if len(description) > 50 else '')
                yield Completion(self._completion_text(cmd_name, word), start_position=-len(word), display=cmd, display_meta=f'⚡ {short_desc}')
        try:
            from hermes_cli.plugins import get_plugin_commands
            for cmd_name, cmd_info in get_plugin_commands().items():
                if cmd_name.startswith(word):
                    ...
                    yield Completion(self._completion_text(cmd_name, word), start_position=-len(word), display=f'/{cmd_name}', display_meta=f'🔌 {short_desc}')
        except Exception:
            pass
...
class SlashCommandAutoSuggest(AutoSuggest):
    ...
    def __init__(self, history_suggest: AutoSuggest | None=None, completer: SlashCommandCompleter | None=None) -> None:
    ...
    def get_suggestion(self, buffer, document):
        ...
        if self._completer is not None and self._completer._is_skill_command(base_cmd):
            for completion in self._completer._stacked_skill_completions(text):
                remainder = completion.text[-completion.start_position:] if completion.start_position else completion.text
                if remainder.strip():
