...
3. Running inside a container with no persistent volume mount over the
   DUCK_AGENT_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger('duck-agent.security_audit')
_AUDIT_RAN = False
...
def _running_as_root() -> Optional[str]:
    if not _is_root():
        return None
    return "Running as ROOT. The agent's terminal/file tools execute with full root privileges — a single prompt-injection or exposed endpoint is a full host compromise. Run Duck Agent as an unprivileged user (or in a sandboxed terminal backend / container with a non-root user)."
...
def _ssh_password_auth_enabled() -> Optional[str]:
    ...
    saw_directive = False
    for line in lines:
        m = re.match('(?i)^PasswordAuthentication\\s+(\\w+)', line)
        if m:
    ...
    return f"SSH password authentication is ENABLED{qualifier}. Password auth is brute-forceable and dangerous on an internet-facing box. Set 'PasswordAuthentication no' in sshd_config and use key-based auth."
...
def _container_no_volume_mount(hermes_home: Optional[Path]) -> Optional[str]:
    ...
    try:
        ...
        return None
    return f'Running in a container but the data dir ({home}) is NOT on a persistent volume mount — sessions, memory, skills, and API keys are ephemeral and lost on container restart. Mount a host volume over the DUCK_AGENT_HOME data directory.'
    ...
    try:
        ...
        if isinstance(api, dict) and api.get('enabled'):
            ...
            if is_network_accessible(str(host)) and (not str(key).strip()):
                findings.append(f'OpenAI-compatible API server is network-accessible ({host}) with NO API_SERVER_KEY. It dispatches terminal-capable agent work — an unauthenticated network endpoint is remote code execution. Set a strong API_SERVER_KEY.')
    except Exception:
        pass
    return findings
...
def run_security_audit(*, hermes_home: Optional[Path]=None, config: Optional[dict]=None) -> list[str]:
    ...
    """
    findings: list[str] = []
    for check in (_running_as_root, _ssh_password_auth_enabled):
        try:
            r = check()
            if r:
                findings.append(r)
