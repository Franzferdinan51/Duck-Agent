"""
Skills Hub CLI — Unified interface for the Duck Agent Skills Hub.

Powers both:
  - `duck-agent skills <subcommand>` (CLI argparse entry point)
...
_VALID_NAME_RE = re.compile('^[a-z][a-z0-9_-]*$')
_VALID_CATEGORY_RE = re.compile('^[a-z][a-z0-9_/-]*$')
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.duck-agent/skills/`` that look
    ...
    try:
        return sorted((name for name in set(_category_skill_dirs(SKILLS_DIR)) if not (SKILLS_DIR / name / 'SKILL.md').exists()))
...
def _prompt_for_skill_name(c: Console, url: str, default: str='') -> Optional[str]:
    ...
    c.print(f'[bold]Enter a skill name{default_hint}:[/] [dim](lowercase letters, digits, hyphens, underscores; starts with a letter)[/]')
    try:
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    c.print()
    if existing:
        c.print('[bold]Pick a category[/] [dim](reuse an existing bucket, type a new one, or press Enter to install flat)[/]')
        c.print(f"[dim]Existing: {', '.join(existing)}[/]")
    else:
        c.print('[bold]Category[/] [dim](optional — press Enter to install flat at ~/.duck-agent/skills/<name>/)[/]')
    try:
...
def do_search(query: str, source: str='all', limit: int=10, console: Optional[Console]=None, as_json: bool=False) -> None:
    ...
    if as_json:
        ...
        payload = [{'name': r.name, 'identifier': r.identifier, 'source': r.source, 'trust_level': r.trust_level, 'description': r.description} for r in results]
    ...
    for r in results:
        trust_style = {'builtin': 'bright_cyan', 'trusted': 'green', 'community': 'yellow'}.get(r.trust_level, 'dim')
        trust_label = 'official' if r.source == 'official' else r.trust_level
        table.add_row(r.name, r.description[:60] + ('...' if len(r.description) > 60 else ''), _display_source(r), f'[{trust_style}]{trust_label}[/]', r.identifier)
    c.print(table)
    c.print('[dim]Use: duck-agent skills inspect <identifier> to preview, duck-agent skills install <identifier> to install (--json for scripting)[/]\n')
...
def do_browse(page: int=1, page_size: int=20, source: str='all', console: Optional[Console]=None) -> None:
    ...
    _PER_SOURCE_LIMIT = {'duck-agent-index': 1000000, 'official': 200, 'skills-sh': 200, 'well-known': 50, 'github': 200, 'clawhub': 500, 'lobehub': 500, 'browse-sh': 500}
    with c.status('[bold]Fetching skills from registries...') as status:
        ...
        def _on_source_done(sid: str, count: int) -> None:
            ...
            status.update(f"[bold]Fetching skills from registries...[/]  [dim]done: {', '.join(_done)}[/]")
        all_results, source_counts, timed_out = parallel_search_sources(sources, query='', per_source_limits=_PER_SOURCE_LIMIT, source_filter=source, overall_timeout=30, on_source_done=_on_source_done)
    if not all_results:
    ...
    for i, r in enumerate(page_items, start=start + 1):
        ...
        if len(r.description) > 44:
            desc += '...'
        table.add_row(str(i), r.name, desc, _display_source(r), f'[{trust_style}]{trust_label}[/]', r.identifier)
    c.print(table)
    nav_parts = []
    if page > 1:
    ...
    if timed_out:
        c.print(f"  [yellow]⚡ Slow sources skipped: {', '.join(timed_out)} — run again for cached results[/]")
    c.print("[dim]Tip: 'duck-agent skills inspect <identifier>' to preview, 'duck-agent skills install <identifier>' to install, 'duck-agent skills search <query>' to search deeper[/]\n")

def do_install(identifier: str, category: str='', force: bool=False, console: Optional[Console]=None, skip_confirm: bool=False, invalidate_cache: bool=True, name_override: str='', source_id: Optional[str]=None) -> None:
    ...
    """
    from tools.skills_hub import GitHubAuth, create_source_router, ensure_hub_dirs, quarantine_bundle, install_from_quarantine, HubLockFile, _source_matches
    ...
    if source_id:
        ...
        if pinned:
            ...
            c.print(f"[bold red]Error:[/] no source adapter for '{source_id}'. Refusing to resolve '{identifier}' against other registries (that would change the skill's provenance).\n")
            return
    ...
    if not bundle:
        rate_limited = any((getattr(src, 'is_rate_limited', False) or getattr(getattr(src, 'github', None), 'is_rate_limited', False) for src in sources))
        c.print(f"[bold red]Error:[/] Could not fetch '{identifier}' from any source.")
        if rate_limited:
            c.print('[yellow]Hint:[/] GitHub API rate limit exhausted (unauthenticated: 60 requests/hour).\nSet [bold]GITHUB_TOKEN[/] in your .env or install the [bold]gh[/] CLI and run [bold]gh auth login[/] to raise the limit to 5,000/hr.\n')
        else:
            c.print()
        return
    ...
    result, scan_provenance = scan_skill_cached(q_path, source=scan_source, source_url=source_url_for_bundle(bundle), cache_dir=HUB_DIR / 'scan-cache')
    c.print(format_scan_report(result))
    freshness = 'fresh' if scan_provenance['fresh'] else 'cached'
    c.print(f"[dim]Scan provenance: {freshness}; scanner {scan_provenance['scanner_version']}; hash {scan_provenance['bundle_hash']}[/]")
    rules = ', '.join(scan_provenance['rules']) or 'none'
    c.print(f"[dim]Source: {scan_provenance['source_url']}; scanned {scan_provenance['scanned_at']}; rules: {rules}[/]")
    ...
    if not force and (not skip_confirm):
        c.print()
        if bundle.source == 'official':
            c.print(Panel(f"[bold bright_cyan]This is an official optional skill maintained by Nous Research.[/]\n\nIt ships with duck-agent but is not activated by default.\nInstalling will copy it to your skills directory where the agent can use it.\n\nFiles will be at: [cyan]{display_hermes_home()}/skills/{(category + '/' if category else '')}{bundle.name}/[/]", title='Official Skill', border_style='bright_cyan'))
        else:
            c.print(Panel(f"[bold yellow]You are installing a third-party skill at your own risk.[/]\n\nExternal skills can contain instructions that influence agent behavior,\nshell commands, and scripts. Even after automated scanning, you should\nreview the installed files before use.\n\nFiles will be at: [cyan]{display_hermes_home()}/skills/{(category + '/' if category else '')}{bundle.name}/[/]", title='Disclaimer', border_style='yellow'))
    ...
    try:
        ...
        if spec is not None:
            ...
            if registered is not None:
                c.print(f"[bold cyan]Blueprint:[/] '{bundle.name}' is an automation (schedule [bold]{spec.schedule}[/]).")
                c.print('[dim]Added to your suggestions — run[/] [bold]/suggestions[/] [dim]to schedule or dismiss it.[/]\n')
            else:
                c.print(f"[bold cyan]Blueprint:[/] '{bundle.name}' is an automation (schedule [bold]{spec.schedule}[/]), but it wasn't added to your suggestions (already offered/dismissed, or the pending list is full — run [bold]/suggestions[/] to review).")
                c.print('[dim]You can still schedule it any time by asking the agent or via[/] [bold]duck-agent cron add[/][dim].[/]\n')
    except Exception:
        pass
    if invalidate_cache:
        try:
...
def do_inspect(identifier: str, console: Optional[Console]=None) -> None:
    ...
    info_lines = [f'[bold]Name:[/] {meta.name}', f'[bold]Description:[/] {meta.description}', f'[bold]Source:[/] {meta.source}', f'[bold]Trust:[/] [{trust_style}]{trust_label}[/]', f'[bold]Identifier:[/] {meta.identifier}']
    if meta.tags:
    ...
    if bundle and 'SKILL.md' in bundle.files:
        ...
        c.print(Panel(preview, title='SKILL.md Preview', subtitle='duck-agent skills install <id> to install'))
    c.print()
...
def browse_skills(page: int=1, page_size: int=20, source: str='all') -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {'builtin': 3, 'trusted': 2, 'community': 1}
    _PER_SOURCE_LIMIT = {'duck-agent-index': 5000, 'official': 100, 'skills-sh': 100, 'well-known': 25, 'github': 100, 'clawhub': 50, 'lobehub': 50, 'browse-sh': 500}
    auth = GitHubAuth()
    sources = create_source_router(auth)
    all_results, _counts, _timed_out = parallel_search_sources(sources, query='', per_source_limits=_PER_SOURCE_LIMIT, source_filter=source, overall_timeout=30)
    if not all_results:
    ...
    return {'items': [{'name': r.name, 'description': r.description, 'source': r.source, 'trust': r.trust_level, 'identifier': r.identifier} for r in page_items], 'page': page, 'total_pages': total_pages, 'total': total}
...
def inspect_skill(identifier: str) -> Optional[dict]:
    ...
    if not meta:
        return None
    out: dict = {'name': meta.name, 'description': meta.description, 'source': meta.source, 'identifier': meta.identifier, 'tags': list(meta.tags) if meta.tags else []}
    if bundle and 'SKILL.md' in bundle.files:
        content = bundle.files['SKILL.md']
        if isinstance(content, bytes):
            content = content.decode('utf-8', errors='replace')
...
def do_list(source_filter: str='all', enabled_only: bool=False, console: Optional[Console]=None) -> None:
    ...
    config — ``duck-agent -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``DUCK_AGENT_HOME`` at process
...
def do_update(name: Optional[str]=None, console: Optional[Console]=None) -> None:
    ...
    for entry in updates:
        ...
        do_install(entry['identifier'], category=category, force=True, console=c, source_id=entry.get('source', '') or None)
...
def do_list_modified(console: Optional[Console]=None, as_json: bool=False) -> None:
    """List bundled skills the user has edited (which `duck-agent update` keeps)."""
    ...
    if not modified:
        ...
        return
    c.print(f'\n[bold]{len(modified)} user-modified bundled skill(s)[/] [dim](kept as-is by `duck-agent update`):[/]')
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print('[dim]See changes:   duck-agent skills diff <name>[/]')
    c.print('[dim]Resume updates: duck-agent skills reset <name>          (keep your copy, re-baseline)[/]')
    c.print('[dim]Revert to stock: duck-agent skills reset <name> --restore[/]\n')
...
def do_diff(name: str, console: Optional[Console]=None) -> None:
    ...
    c.print()
    c.print(f'[dim]Revert with: duck-agent skills reset {name} --restore[/]\n')
...
def do_opt_out(remove: bool=False, console: Optional[Console]=None, skip_confirm: bool=False, invalidate_cache: bool=True) -> None:
    ...
    if not remove:
        c.print('[dim]Existing skills on disk were left in place. Re-run with --remove to also delete unmodified bundled skills.[/]\n')
        return
    preview = remove_pristine_bundled_skills(dry_run=True)
    candidates = preview['removed']
    kept = preview['skipped']
    if not candidates:
        c.print('[dim]No pristine bundled skills to remove (nothing tracked, or all are user-modified/local).[/]\n')
        return
    ...
    if not skip_confirm:
        c.print('[dim]This deletes the on-disk copies. User-edited and hub/local skills are NOT touched.[/]')
        try:
            answer = input('Confirm [y/N]: ').strip().lower()
        except (EOFError, KeyboardInterrupt):
            answer = 'n'
...
def do_tap(action: str, repo: str='', console: Optional[Console]=None) -> None:
    ...
    if action == 'list':
        ...
        if not repo:
            c.print('[bold red]Error:[/] Repo required. Usage: duck-agent skills tap add owner/repo\n')
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print('[bold red]Error:[/] Repo required. Usage: duck-agent skills tap remove owner/repo\n')
            return
        if mgr.remove(repo):
...
def _github_publish(skill_path: Path, skill_name: str, target_repo: str, auth) -> tuple:
    ...
    try:
        resp = httpx.post(f'https://api.github.com/repos/{target_repo}/forks', headers=headers, timeout=30)
    ...
    try:
        resp = httpx.get(f'https://api.github.com/repos/{target_repo}', headers=headers, timeout=15)
        default_branch = resp.json().get('default_branch', 'main')
    except Exception:
        default_branch = 'main'
    try:
        resp = httpx.get(f'https://api.github.com/repos/{fork_repo}/git/refs/heads/{default_branch}', headers=headers, timeout=15)
        base_sha = resp.json()['object']['sha']
    except Exception as e:
        return (False, f'Failed to get base branch: {e}')
    branch_name = f'add-skill-{skill_name}'
    try:
        httpx.post(f'https://api.github.com/repos/{fork_repo}/git/refs', headers=headers, timeout=15, json={'ref': f'refs/heads/{branch_name}', 'sha': base_sha})
    except Exception as e:
    ...
    for f in skill_path.rglob('*'):
        ...
        try:
            ...
            httpx.put(f'https://api.github.com/repos/{fork_repo}/contents/{upload_path}', headers=headers, timeout=15, json={'message': f'Add {skill_name} skill: {rel}', 'content': content_b64, 'branch': branch_name})
        except Exception as e:
            return (False, f'Failed to upload {rel}: {e}')
    try:
        resp = httpx.post(f'https://api.github.com/repos/{target_repo}/pulls', headers=headers, timeout=15, json={'title': f'Add skill: {skill_name}', 'body': f'Submitting the `{skill_name}` skill via Duck Agent Skills Hub.\n\nThis skill was scanned by the Duck Agent Skills Guard before submission.', 'head': f"{fork_repo.split('/')[0]}:{branch_name}", 'base': default_branch})
...
def do_snapshot_export(output_path: str, console: Optional[Console]=None) -> None:
    ...
    snapshot = {'hermes_version': '0.1.0', 'exported_at': __import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(), 'skills': [{'name': entry['name'], 'source': entry.get('source', ''), 'identifier': entry.get('identifier', ''), 'category': str(Path(entry.get('install_path', '')).parent) if '/' in entry.get('install_path', '') else ''} for entry in installed], 'taps': tap_list}
...
def skills_command(args) -> None:
    """Router for `duck-agent skills <subcommand>` — called from hermes_cli/main.py."""
    action = getattr(args, 'skills_action', None)
    if action == 'browse':
        do_browse(page=args.page, page_size=args.size, source=args.source)
    elif action == 'search':
        do_search(args.query, source=args.source, limit=args.limit, as_json=getattr(args, 'json', False))
    elif action == 'install':
        ...
        do_list(source_filter=args.source, enabled_only=getattr(args, 'enabled_only', False))
    elif action == 'check':
        ...
        do_reset(args.name, restore=getattr(args, 'restore', False), skip_confirm=getattr(args, 'yes', False))
    elif action == 'list-modified':
        do_list_modified(as_json=getattr(args, 'json', False))
    elif action == 'diff':
        do_diff(args.name)
    elif action == 'opt-out':
        do_opt_out(remove=getattr(args, 'remove', False), skip_confirm=getattr(args, 'yes', False))
    elif action == 'opt-in':
        do_opt_in(sync=getattr(args, 'sync', False))
    elif action == 'repair-official':
        do_repair_official(args.name, restore=getattr(args, 'restore', False), skip_confirm=getattr(args, 'yes', False))
    elif action == 'publish':
        ...
        if snap_action == 'export':
            ...
            _console.print('Usage: duck-agent skills snapshot [export|import]\n')
    elif action == 'tap':
        tap_action = getattr(args, 'tap_action', None)
        repo = getattr(args, 'repo', '') or getattr(args, 'name', '')
        if not tap_action:
            _console.print('Usage: duck-agent skills tap [list|add|remove]\n')
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print('Usage: duck-agent skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n')
        _console.print("Run 'duck-agent skills <command> --help' for details.\n")
...
def handle_skills_slash(cmd: str, console: Optional[Console]=None) -> None:
    ...
    if action == 'browse':
        ...
        do_install(identifier, category=category, force=force, skip_confirm=skip_confirm, invalidate_cache=invalidate_cache, name_override=name_override, console=c)
    elif action == 'inspect':
        if not args:
        ...
        skip_confirm = True
        invalidate_cache = '--now' in args
        do_uninstall(args[0], console=c, skip_confirm=skip_confirm, invalidate_cache=invalidate_cache)
    elif action == 'reset':
        if not args:
        ...
        invalidate_cache = '--now' in args
        do_reset(name, restore=restore, console=c, skip_confirm=True, invalidate_cache=invalidate_cache)
