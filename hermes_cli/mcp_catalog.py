...
entries via ``duck-agent mcp catalog`` or the interactive ``duck-agent mcp picker``,
and install them with ``duck-agent mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into duck-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``duck-agent mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.duck-agent/.env`` (the
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Duck Agent install."""
...
def _parse_env_spec(raw: Any) -> EnvVarSpec:
    ...
    if not name or not re.match('^[A-Za-z_][A-Za-z0-9_]*$', name):
        raise CatalogError(f'invalid env var name: {name!r}')
    return EnvVarSpec(name=name, prompt=raw.get('prompt') or name, required=bool(raw.get('required', True)), secret=bool(raw.get('secret', True)), default=str(raw.get('default') or ''))
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(f'{path}: manifest_version {mv!r} unsupported (this Duck Agent understands version {_MANIFEST_VERSION})')
    ...
    transport = TransportSpec(type=t_type, command=transport_raw.get('command'), args=[str(a) for a in args], url=transport_raw.get('url'), version=transport_raw.get('version'), env=dict(env_raw))
    ...
    auth = AuthSpec(type=a_type, env=env_list, provider=auth_raw.get('provider'), scopes=list(auth_raw.get('scopes') or []), env_var=auth_raw.get('env_var'))
    if t_type == 'http' and a_type == 'api_key':
        ...
        if not any((spec.name == _required_key for spec in env_list)):
            raise CatalogError(f"{path}: http + api_key auth requires auth.env to declare '{_required_key}' (the key the Authorization header references)")
    tools_raw = data.get('tools') or {}
    if not isinstance(tools_raw, dict):
        raise CatalogError(f"{path}: 'tools' must be a mapping")
    default_enabled = tools_raw.get('default_enabled')
    if default_enabled is not None:
        if not isinstance(default_enabled, list) or not all((isinstance(t, str) for t in default_enabled)):
    ...
    return CatalogEntry(name=name, description=description, source=source, transport=transport, auth=auth, tools=tools_spec, install=install, post_install=str(data.get('post_install') or ''), manifest_path=path)
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Duck Agent is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Duck Agent
        understands. Update Duck Agent to install this entry.
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.duck-agent/mcp-installs/<name>`` and run
    ...
    is_sha_ref = bool(re.fullmatch('[0-9a-f]{7,40}', install.ref))
    _git_env = noninteractive_git_env()
    if not is_sha_ref:
        proc = subprocess.run([git, 'clone', '--depth', '1', '--branch', install.ref, install.url, str(dest)], stdin=subprocess.DEVNULL, env=_git_env)
        if proc.returncode == 0:
            pass
        else:
            if dest.exists():
            ...
            is_sha_ref = True
    if is_sha_ref:
        proc = subprocess.run([git, 'clone', install.url, str(dest)], stdin=subprocess.DEVNULL, env=_git_env)
        if proc.returncode != 0:
            raise CatalogError(f'git clone failed for {install.url}')
        proc = subprocess.run([git, '-C', str(dest), 'checkout', install.ref], stdin=subprocess.DEVNULL, env=_git_env)
        if proc.returncode != 0:
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.duck-agent/.env via save_env_value()."""
...
def _apply_tool_selection(entry: CatalogEntry, *, prior_selection: Optional[List[str]]) -> None:
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``duck-agent mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(f"  Couldn't probe server. Applied manifest default ({len(manifest_default)} tools). Run `duck-agent mcp configure {entry.name}` after the server is reachable to refine.", Colors.YELLOW))
        else:
            _write_tools_include(entry.name, None)
            print(color(f"  Couldn't probe server; installed with no tool filter (all tools enabled when reachable). Run `duck-agent mcp configure {entry.name}` after first connect to prune.", Colors.YELLOW))
        return
    if not probed:
        _write_tools_include(entry.name, None)
    ...
    if not _sys.stdin.isatty():
        if prior_selection is not None:
            ...
            _write_tools_include(entry.name, None)
        return
    print(color(f'  Found {len(probed)} tool(s). Pre-checked: {len(pre_indices)}.', Colors.GREEN))
    from hermes_cli.curses_ui import curses_checklist
    labels = [f"{n}  —  {(d[:60] + '...' if len(d) > 60 else d)}" for n, d in probed]
    chosen_indices = curses_checklist(f"Select tools for '{entry.name}' (SPACE toggle, ENTER confirm)", labels, pre_indices)
    if not chosen_indices:
        _write_tools_include(entry.name, [])
        print(color(f'  No tools selected. Run `duck-agent mcp configure {entry.name}` to change.', Colors.YELLOW))
        return
    if len(chosen_indices) == len(probed):
        _write_tools_include(entry.name, None)
        print(color(f'  ✓ All {len(probed)} tools enabled (no filter — new tools the server adds later will be auto-enabled).', Colors.GREEN))
        return
    chosen_names = [tool_names[i] for i in sorted(chosen_indices)]
    _write_tools_include(entry.name, chosen_names)
    print(color(f'  ✓ {len(chosen_names)}/{len(probed)} tools enabled.', Colors.GREEN))
...
def install_entry(entry: CatalogEntry, *, enable: bool=True) -> None:
    ...
    if entry.auth.type == 'api_key':
        ...
        if entry.auth.provider:
            print(color(f'  This MCP uses {entry.auth.provider} OAuth. Run `duck-agent auth {entry.auth.provider}` if you have not already authenticated.', Colors.YELLOW))
        else:
            print(color('  This MCP uses native OAuth 2.1; tokens will be acquired on first connection (browser flow).', Colors.DIM))
    ...
    print()
    print(color(f"  ✓ Installed '{entry.name}' ({('enabled' if enable else 'disabled')}). Start a new Duck Agent session to load its tools.", Colors.GREEN))
