"""
Status command for duck-agent CLI.

Shows the status of all Duck Agent components.
"""
import os
import sys
import time
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``duck-agent config``'s
...
def _estop_status_line():
    """One-line pause banner for `duck-agent status`, or None when not paused.

    Cheap: a single stat on $DUCK_AGENT_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f'⏸️  PAUSED (global emergency stop{suffix}; `duck-agent resume` to lift)'

def show_status(args):
    """Show status of all Duck Agent components."""
    deep = getattr(args, 'deep', False)
    print()
    print(color('┌─────────────────────────────────────────────────────────┐', Colors.CYAN))
    print(color('│                 ⚕ Duck Agent Status                  │', Colors.CYAN))
    print(color('└─────────────────────────────────────────────────────────┘', Colors.CYAN))
    ...
    keys: dict[str, str | tuple[str, ...]] = {'OpenRouter': 'OPENROUTER_API_KEY', 'OpenAI': 'OPENAI_API_KEY', 'Anthropic': ('ANTHROPIC_API_KEY', 'ANTHROPIC_TOKEN'), 'Google / Gemini': ('GOOGLE_API_KEY', 'GEMINI_API_KEY'), 'DeepSeek': 'DEEPSEEK_API_KEY', 'xAI / Grok': 'XAI_API_KEY', 'NVIDIA NIM': 'NVIDIA_API_KEY', 'Z.AI / GLM': 'GLM_API_KEY', 'Kimi': 'KIMI_API_KEY', 'StepFun Step Plan': 'STEPFUN_API_KEY', 'MiniMax': 'MINIMAX_API_KEY', 'MiniMax-CN': 'MINIMAX_CN_API_KEY', 'DeepInfra': 'DEEPINFRA_API_KEY', 'Firecrawl': 'FIRECRAWL_API_KEY', 'Tavily': 'TAVILY_API_KEY', 'Browser Use': 'BROWSER_USE_API_KEY', 'Browserbase': 'BROWSERBASE_API_KEY', 'FAL': 'FAL_KEY', 'ElevenLabs': 'ELEVENLABS_API_KEY', 'GitHub': 'GITHUB_TOKEN'}
    ...
    try:
        from hermes_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_qwen_auth_status, get_minimax_oauth_auth_status
    ...
    if nous_status.get('logged_in') or nous_status.get('access_token') or nous_status.get('portal_base_url') or nous_status.get('inference_credential_present') or nous_status.get('error_code'):
        try:
    ...
    nous_inference_present = bool(nous_status.get('inference_credential_present') or (nous_account_info and nous_account_info.inference_credential_present))
    ...
    if nous_logged_in:
        ...
        nous_label = 'not logged in (run: duck-agent portal)'
    print(f"  {'Nous Portal':<12}  {check_mark(nous_logged_in)} {nous_label}")
    portal_url = nous_status.get('portal_base_url') or '(unknown)'
    inference_url = nous_status.get('inference_base_url') or (nous_account_info.inference_base_url if nous_account_info else None)
    ...
    print(f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} {('logged in' if codex_logged_in else 'not logged in (run: duck-agent model)')}")
    ...
    print(f"  {'Qwen OAuth':<12}  {check_mark(qwen_logged_in)} {('logged in' if qwen_logged_in else 'not logged in (run: qwen auth qwen-oauth)')}")
    ...
    print(f"  {'MiniMax OAuth':<12}  {check_mark(minimax_logged_in)} {('logged in' if minimax_logged_in else 'not logged in (run: duck-agent auth add minimax-oauth)')}")
    ...
    print(f"  {'xAI OAuth':<12}  {check_mark(xai_oauth_logged_in)} {('logged in' if xai_oauth_logged_in else 'not logged in (run: duck-agent auth add xai-oauth)')}")
    ...
    if managed_nous_tools_enabled():
        ...
        print()
        print(color('◆ Nous Tool Gateway', Colors.CYAN, Colors.BOLD))
        message = format_nous_portal_entitlement_message(nous_account_info, capability='managed web, image, TTS, STT, browser, and Modal tools')
        if message:
            for line in message.splitlines():
                print(f'  {line}')
    print()
    print(color('◆ API-Key Providers', Colors.CYAN, Colors.BOLD))
    apikey_providers = {'Z.AI / GLM': ('GLM_API_KEY', 'ZAI_API_KEY', 'Z_AI_API_KEY'), 'Kimi / Moonshot': ('KIMI_API_KEY',), 'StepFun Step Plan': ('STEPFUN_API_KEY',), 'MiniMax': ('MINIMAX_API_KEY',), 'MiniMax (China)': ('MINIMAX_CN_API_KEY',), 'DeepInfra': ('DEEPINFRA_API_KEY',)}
    for pname, env_vars in apikey_providers.items():
        ...
        label = 'configured' if configured else 'not configured (run: duck-agent model)'
    ...
    if terminal_env == 'ssh':
        ...
        sdk_label = 'installed' if sdk_ok else "missing (install: pip install 'duck-agent[vercel]')"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print('  Start with:   duck-agent gateway')
        ...
        if sessions_file.exists():
            ...
            try:
                with open(sessions_file, encoding='utf-8') as f:
                    data = json.load(f)
                    _entries = {k: v for k, v in data.items() if not str(k).startswith('_')} if isinstance(data, dict) else {}
    ...
    try:
        from hermes_cli.active_sessions import active_session_registry_snapshot, format_age, resolve_max_concurrent_sessions
    ...
    if _cap:
        ...
        for _entry in sorted(_held, key=lambda e: e.get('started_at') or 0):
            ...
            print(f"                {_entry.get('surface') or 'unknown':<17} {_entry.get('session_id') or '?':<24} {_age}")
    if deep:
        print()
    ...
    print(color("  Run 'duck-agent doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'duck-agent setup' to configure", Colors.DIM))
    print()