"""
MCP Server Management CLI — ``duck-agent mcp`` subcommand.

Implements ``duck-agent mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.duck-agent/config.yaml under the ``mcp_servers`` key.
"""
import asyncio
import logging
import os
...
from hermes_cli.config import cfg_get, load_config, save_config, get_env_value, save_env_value, get_hermes_home
...
_MCP_PRESETS: Dict[str, Dict[str, Any]] = {'codex': {'command': 'codex', 'args': ['mcp-server']}}
...
def _env_key_for_server(name: str) -> str:
    ...
    suffix = re.sub('[^A-Za-z0-9_]', '_', name.upper()).strip('_')
...
def _apply_mcp_preset(name: str, *, preset_name: Optional[str], url: Optional[str], command: Optional[str], cmd_args: List[str], server_config: Dict[str, Any]) -> tuple[Optional[str], Optional[str], List[str], bool]:
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.duck-agent/.env`` into ``os.environ`` and recursively interpolate any
...
def _probe_single_server(name: str, config: dict, connect_timeout: Optional[float]=None, *, details: Optional[dict]=None) -> List[Tuple[str, str]]:
    ...
    from tools.mcp_tool import _ensure_mcp_loop, _run_on_mcp_loop, _connect_server, _stop_mcp_loop_if_idle, _parse_boolish
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``duck-agent mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    try:
        ...
        url, command, cmd_args, _preset_applied = _apply_mcp_preset(name, preset_name=preset_name, url=url, command=command, cmd_args=list(cmd_args), server_config=server_config)
    ...
    if not url and (not command):
        ...
        _info('Examples:')
        _info('  duck-agent mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  duck-agent mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  duck-agent mcp add myserver --preset mypreset')
        return
    ...
    if url and auth_type == 'oauth':
        ...
        try:
            ...
            oauth_auth = get_manager().get_or_build_provider(name, url, server_config.get('oauth'))
            if oauth_auth:
    ...
    try:
        ...
        if _confirm('Save config anyway (you can test later)?', default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info('Fix the issue, then: duck-agent mcp test ' + name)
        return
    if not tools:
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info('Add one with:')
        _info('  duck-agent mcp add <name> --url <endpoint>')
        _info('  duck-agent mcp add <name> --command <cmd> --args <args...>')
        print()
        return
    print()
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``duck-agent mcp login`` and
    ``duck-agent mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get('auth') != 'oauth':
        ...
        _info('Use `duck-agent mcp remove` + `duck-agent mcp add` to reconfigure auth.')
        return False
    try:
        from tools.mcp_oauth_manager import get_manager
        get_manager().remove(name)
    ...
    try:
        ...
        with force_interactive_oauth():
            tools = _probe_single_server(name, server_config, connect_timeout=_login_connect_timeout)
        if not _oauth_tokens_present(name):
            ...
            print()
            _info('Some providers (e.g. Google Drive, Atlassian) do not support automatic client registration. For those you must create an OAuth client yourself and add its credentials to config.yaml:')
            print()
            ...
            print()
            _info('Then re-run `duck-agent mcp login ' + name + '`.')
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``duck-agent mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``duck-agent mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info('Usage: duck-agent mcp reauth <name>   |   duck-agent mcp reauth --all')
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'duck-agent mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
    if name not in servers:
...
def mcp_command(args):
    """Main dispatcher for ``duck-agent mcp`` subcommands."""
    ...
    if action == 'install':
        ...
        if rc:
            _sys.exit(rc)
        return
    handlers = {'add': cmd_mcp_add, 'remove': cmd_mcp_remove, 'rm': cmd_mcp_remove, 'list': cmd_mcp_list, 'ls': cmd_mcp_list, 'test': cmd_mcp_test, 'configure': cmd_mcp_configure, 'config': cmd_mcp_configure, 'login': cmd_mcp_login, 'reauth': cmd_mcp_reauth}
    ...
    if handler:
        ...
        _info('duck-agent mcp                                    Open the catalog picker (default)')
        _info('duck-agent mcp catalog                            List Nous-approved MCPs')
        _info('duck-agent mcp install <name>                     Install a catalog MCP')
        _info('duck-agent mcp serve                              Run as MCP server')
        _info('duck-agent mcp add <name> --url <endpoint>        Add a custom MCP server')
        _info('duck-agent mcp add <name> --command <cmd>         Add a stdio server')
        _info('duck-agent mcp add <name> --preset <preset>       Add from a known preset')
        _info('duck-agent mcp remove <name>                      Remove a server')
        _info('duck-agent mcp list                               List configured servers')
        _info('duck-agent mcp test <name>                        Test connection')
        _info('duck-agent mcp configure <name>                   Toggle tools')
        _info('duck-agent mcp login <name>                       Re-authenticate OAuth')
        _info('duck-agent mcp reauth <name> | --all              Re-auth one or all OAuth servers')
        print()