...
def _resolve_origin(explicit: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if platform and chat_id:
            return {'platform': platform, 'chat_id': chat_id, 'chat_name': get_session_env('HERMES_SESSION_CHAT_NAME') or None, 'thread_id': get_session_env('HERMES_SESSION_THREAD_ID') or None}
    except Exception:
        pass
    return None
...
def match_blueprint(query: str) -> Tuple[Optional[Any], List[Any]]:
    ...
    if exact is not None:
        return (exact, [])
    prefix = [r for r in CATALOG if r.key.lower().startswith(q) or any((w.lower().startswith(q) for w in r.title.split()))]
    if len(prefix) == 1:
        return (prefix[0], [])
    if len(prefix) > 1:
        return (None, prefix)
    substr = [r for r in CATALOG if q in r.key.lower() or q in r.title.lower() or q in r.description.lower()]
    if len(substr) == 1:
        return (substr[0], [])
    if len(substr) > 1:
        return (None, substr)
...
def build_blueprint_seed(blueprint) -> str:
    ...
    lines: List[str] = []
    lines.append(f"Set up the '{blueprint.title}' automation for me (automation blueprint '{blueprint.key}'). {blueprint.description}")
    lines.append('')
    lines.append("Ask me for each of these, one at a time, offering the default in brackets if I don't have a preference:")
    ...
    lines.append('')
    lines.append(f'''Once you have my answers, create the job by calling the cronjob tool with action='create'. Build the schedule as a cron expression from this template: `{blueprint.schedule_template}` (fill {{minute}}/{{hour}} from the chosen time, {{dow}} from the weekday choice using {dict(WEEKDAY_PRESETS)}, {{interval_min}} from any interval). Use this exact prompt for the job (substituting my answers into any {{slot}} placeholders): "{blueprint.prompt_template}". Confirm the schedule and what it will do before you create it.''')
    return '\n'.join(lines)
...
def handle_blueprint_command(args: str, *, origin: Optional[Dict[str, Any]]=None, surface: str='cli') -> BlueprintCommandResult:
    ...
    try:
        ...
        return BlueprintCommandResult(f"Can't set up '{blueprint.title}': {e}\nOr just run /blueprint {blueprint.key} and I'll ask you for the values.")
    try:
    ...
    return BlueprintCommandResult(f"Scheduled '{blueprint.title}'" + (f' ({sched})' if sched else '') + f", delivering to {spec.get('deliver', 'origin')}. {_manage_hint(surface)}")