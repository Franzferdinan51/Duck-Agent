...
from hermes_cli.providers import ProviderDef, custom_provider_aliases, custom_provider_slug, determine_api_mode, get_label, host_mandated_api_mode, is_aggregator, resolve_provider_full
from hermes_cli.model_normalize import normalize_model_for_provider
from agent.models_dev import ModelCapabilities, ModelInfo, get_model_capabilities, get_model_info, list_provider_models
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``duck-agent model`` wizard — not a
    ...
    while ``duck-agent model`` still live-probed the full ``/v1/models`` list.
...
def _bare_custom_provider_def(current_base_url: str) -> Optional[ProviderDef]:
    ...
    if not base_url:
        return None
    return ProviderDef(id='custom', name='Custom endpoint', transport='openai_chat', api_key_env_vars=(), base_url=base_url, is_aggregator=False, auth_type='api_key', source='model-config')
_HERMES_MODEL_WARNING = 'Nous Research Duck Agent 3 & 4 models are NOT agentic and are not designed for use with Duck Agent. They lack the tool-calling capabilities required for agent workflows. Consider using an agentic model instead (Claude, GPT, Gemini, DeepSeek, etc.).'
_NOUS_HERMES_NON_AGENTIC_RE = re.compile('(?:^|[/:])duck-agent[-_ ]?[34](?:[-_.:]|$)', re.IGNORECASE)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Duck Agent 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Duck Agent 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``duck-agent config set model.aliases.xxx``)
    ...
    try:
        ...
        if isinstance(user_aliases, dict):
            for name, entry in user_aliases.items():
                ...
                if model:
                    merged[name.strip().lower()] = DirectAlias(model=model, provider=provider, base_url=base_url)
...
def parse_model_flags_detailed(raw_args: str) -> ModelFlagParseResult:
    ...
    *effective* persistence decision is resolved by
    ...
    is_once = False
    import re as _re
    raw_args = _re.sub('[\\u2012\\u2013\\u2014\\u2015](provider|global|session|refresh|once)', '--\\1', raw_args)
    ...
    return ModelFlagParseResult(model_input=model_input, explicit_provider=explicit_provider, is_global=is_global, force_refresh=force_refresh, is_session=is_session, is_once=is_once)
...
def parse_model_flags(raw_args: str) -> tuple[str, str, bool, bool, bool]:
    ...
    return (parsed.model_input, parsed.explicit_provider, parsed.is_global, parsed.force_refresh, parsed.is_session)
...
MODEL_SWITCH_ERROR_TEXT = {MODEL_SWITCH_ERR_ONCE_WITH_GLOBAL: '/model --once cannot be combined with --global', MODEL_SWITCH_ERR_ONCE_REQUIRES_TARGET: '/model --once requires a model or provider.'}
...
class ModelSwitchRequest:
    ...
    def flags(self) -> 'ModelFlagParseResult':
        return ModelFlagParseResult(model_input=self.target, explicit_provider=self.explicit_provider, is_global=self.is_global, force_refresh=self.force_refresh, is_session=self.is_session, is_once=self.is_once)
...
def parse_model_switch_args(raw: str) -> ModelSwitchRequest:
    ...
    * ``--once`` with no model and no ``--provider``
    ...
    if parsed.is_once:
        ...
        scope = 'default'
    return ModelSwitchRequest(raw=raw, target=parsed.model_input, explicit_provider=parsed.explicit_provider, is_global=parsed.is_global, is_session=parsed.is_session, is_once=parsed.is_once, force_refresh=parsed.force_refresh, scope=scope, errors=tuple(errors))
...
def resolve_effective_model(session_overrides: Any=None, channel_config: Any=None, global_config: Any='') -> str:
...
def get_authenticated_provider_slugs(current_provider: str='', user_providers: dict=None, custom_providers: list | None=None) -> list[str]:
    ...
    """
    try:
        providers = list_authenticated_providers(current_provider=current_provider, user_providers=user_providers, custom_providers=custom_providers, max_models=0)
...
def resolve_display_context_length(model: str, provider: str, base_url: str='', api_key: str='', model_info: Optional[ModelInfo]=None, custom_providers: list | None=None, config_context_length: int | None=None, configured_model: str | None=None, configured_provider: str | None=None, configured_base_url: str | None=None) -> Optional[int]:
    ...
    try:
        ...
        ctx = get_model_context_length(model, base_url=base_url or '', api_key=api_key or '', provider=provider or None, custom_providers=custom_providers, config_context_length=config_context_length)
        if ctx:
            return int(ctx)
    except Exception:
        pass
...
async def resolve_display_context_length_async(model: str, provider: str, base_url: str='', api_key: str='', model_info: Optional[ModelInfo]=None, custom_providers: list | None=None, config_context_length: int | None=None, configured_model: str | None=None, configured_provider: str | None=None, configured_base_url: str | None=None) -> Optional[int]:
    ...
    """
    import asyncio
    return await asyncio.to_thread(resolve_display_context_length, model, provider, base_url=base_url, api_key=api_key, model_info=model_info, custom_providers=custom_providers, config_context_length=config_context_length, configured_model=configured_model, configured_provider=configured_provider, configured_base_url=configured_base_url)

def _configured_provider_matches(model_name: str, user_providers: Optional[dict], custom_providers: Optional[list]) -> dict[str, str]:
...
def switch_model(raw_input: str, current_provider: str, current_model: str, current_base_url: str='', current_api_key: str='', is_global: bool=False, explicit_provider: str='', user_providers: dict=None, custom_providers: list | None=None) -> ModelSwitchResult:
    ...
    """
    from hermes_cli.models import copilot_model_api_mode, detect_provider_for_model, validate_requested_model, opencode_model_api_mode
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = f"Unknown provider '{explicit_provider}'. Check 'duck-agent model' for available providers, or define it in config.yaml under 'providers:'."
            try:
                from hermes_cli.config import validate_config_structure
                _cfg_issues = validate_config_structure()
                if _cfg_issues:
                    _switch_err += "\n\nRun 'duck-agent doctor' — config issues detected:"
                    for _ci in _cfg_issues[:3]:
                        _switch_err += f'\n  • {_ci.message}'
            except Exception:
                pass
            return ModelSwitchResult(success=False, is_global=is_global, error_message=_switch_err)
        ...
        if _alias_target and _alias_target == target_provider and (target_provider != _explicit_norm) and (target_provider in _AGG_PROVIDERS):
            _authed = get_authenticated_provider_slugs(current_provider=current_provider, user_providers=user_providers, custom_providers=custom_providers)
            if target_provider not in _authed:
                _suggestions = [s for s in _authed if s.startswith(_explicit_norm) and s != _explicit_norm]
                _hint = f" Did you mean: {', '.join(_suggestions)}?" if _suggestions else ''
                return ModelSwitchResult(success=False, target_provider=target_provider, provider_label=pdef.name, is_global=is_global, error_message=f"Provider '{_explicit_norm}' is an alias that routes through {get_label(target_provider)}, which has no credentials configured.{_hint}")
        if not new_model:
            if pdef.base_url:
                ...
                if detected:
                    ...
                    return ModelSwitchResult(success=False, target_provider=target_provider, provider_label=pdef.name, is_global=is_global, error_message=f'No model detected on {pdef.name} ({pdef.base_url}). Specify the model explicitly: /model <model-name> --provider {explicit_provider}')
            else:
                return ModelSwitchResult(success=False, target_provider=target_provider, provider_label=pdef.name, is_global=is_global, error_message=f"Provider '{pdef.name}' has no base URL configured. Specify a model: /model <model-name> --provider {explicit_provider}")
        ...
        if resolved_moa_preset:
            ...
            if key in MODEL_ALIASES:
                authed = get_authenticated_provider_slugs(current_provider=current_provider, user_providers=user_providers, custom_providers=custom_providers)
                fallback_result = _resolve_alias_fallback(raw_input, authed)
                if fallback_result is not None:
                    target_provider, new_model, resolved_alias = fallback_result
                    logger.debug("Alias '%s' resolved via fallback to %s on %s", resolved_alias, new_model, target_provider)
                else:
                    identity = MODEL_ALIASES[key]
                    return ModelSwitchResult(success=False, is_global=is_global, error_message=f"Alias '{key}' maps to {identity.vendor}/{identity.family} but no matching model was found in any provider catalog. Try specifying the full model name.")
            ...
            if cfg_matches:
                if current_provider in cfg_matches:
                    ...
                    if len(match_slugs) > 1:
                        return ModelSwitchResult(success=False, is_global=is_global, error_message=f"'{new_model}' is declared by multiple configured providers ({', '.join(match_slugs)}). Re-run with --provider <slug> to choose which one to use.")
        ...
        is_custom = current_provider in {'custom', 'local'} or current_provider.startswith('custom:') or ('localhost' in _base or '127.0.0.1' in _base)
        if target_provider == current_provider and (not is_custom) and (not resolved_alias) and (not resolved_in_current_catalog) and (not config_routed):
    ...
    if provider_changed or explicit_provider:
        ...
        if _user_pdef is not None and _user_pdef.base_url:
            _ucfg = (user_providers or {}).get(explicit_provider.strip().lower()) or (user_providers or {}).get(target_provider) or {}
            ...
            try:
                runtime = resolve_runtime_provider(requested=target_provider, explicit_api_key=_ukey or None, explicit_base_url=_user_pdef.base_url, target_model=new_model)
            ...
            try:
                runtime = resolve_runtime_provider(requested=target_provider, target_model=new_model)
                api_key = runtime.get('api_key', '')
                base_url = runtime.get('base_url', '')
                api_mode = runtime.get('api_mode', '')
            except Exception as e:
                return ModelSwitchResult(success=False, target_provider=target_provider, provider_label=provider_label, is_global=is_global, error_message=f"Could not resolve credentials for provider '{provider_label}': {e}")
    else:
        try:
            runtime = resolve_runtime_provider(requested=current_provider, target_model=new_model)
            api_key = runtime.get('api_key', '')
            base_url = runtime.get('base_url', '')
            api_mode = runtime.get('api_mode', '')
        except Exception:
    ...
    try:
        validation = validate_requested_model(new_model, target_provider, api_key=api_key, base_url=base_url, api_mode=api_mode or None)
    except Exception as e:
        validation = {'accepted': False, 'persist': False, 'recognized': False, 'message': f'Could not validate `{new_model}`: {e}'}
    if not validation.get('accepted'):
        ...
        if not override and custom_providers and isinstance(custom_providers, list):
            for entry in custom_providers:
                ...
                entry_aliases = custom_provider_aliases(str(entry_name or ''), str(entry.get('provider_key') or ''))
        ...
        if override:
            ...
            return ModelSwitchResult(success=False, new_model=new_model, target_provider=target_provider, provider_label=provider_label, is_global=is_global, error_message=msg)
    ...
    return ModelSwitchResult(success=True, new_model=new_model, target_provider=target_provider, provider_changed=provider_changed, api_key=api_key, base_url=base_url, api_mode=api_mode, warning_message=' | '.join(warnings) if warnings else '', provider_label=provider_label, resolved_via_alias=resolved_alias, capabilities=capabilities, model_info=model_info, is_global=is_global)
...
def prewarm_picker_cache_async() -> Optional['_threading.Thread']:
    ...
    def _warm() -> None:
        try:
            ...
            list_authenticated_providers(current_provider=ctx.current_provider, current_base_url=ctx.current_base_url, current_model=ctx.current_model, user_providers=ctx.user_providers, custom_providers=ctx.custom_providers, excluded_providers=ctx.excluded_providers or [])
        except Exception:
...
def list_authenticated_providers(current_provider: str='', current_base_url: str='', user_providers: dict=None, custom_providers: list | None=None, *, force_fresh_nous_tier: bool=False, max_models: int | None=None, current_model: str='', refresh: bool=False, probe_custom_providers: bool=True, probe_current_custom_provider: bool=False, for_picker: bool=False, excluded_providers: list | None=None) -> List[dict]:
    ...
    """
    import os
    from agent.models_dev import PROVIDER_TO_MODELS_DEV, fetch_models_dev, get_provider_info as _mdev_pinfo
    from hermes_cli.auth import PROVIDER_REGISTRY
    from hermes_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids, clear_provider_models_cache, get_curated_nous_model_ids
    if refresh:
        try:
    ...
    def _has_fast_aws_sdk_signal() -> bool:
        ...
        if os.environ.get('AWS_BEARER_TOKEN_BEDROCK', '').strip():
            return True
        if os.environ.get('AWS_ACCESS_KEY_ID', '').strip() and os.environ.get('AWS_SECRET_ACCESS_KEY', '').strip():
            return True
        return any((os.environ.get(name, '').strip() for name in ('AWS_PROFILE', 'AWS_CONTAINER_CREDENTIALS_RELATIVE_URI', 'AWS_CONTAINER_CREDENTIALS_FULL_URI', 'AWS_WEB_IDENTITY_TOKEN_FILE')))
    ...
    if 'lmstudio' not in curated and (os.environ.get('LM_API_KEY') or os.environ.get('LM_BASE_URL') or current_provider.strip().lower() == 'lmstudio'):
        from hermes_cli.models import fetch_lmstudio_models
        from hermes_cli.auth import AuthError
        is_current_lmstudio = current_provider.strip().lower() == 'lmstudio'
        lm_base = os.environ.get('LM_BASE_URL') or (current_base_url if is_current_lmstudio and current_base_url else None) or 'http://127.0.0.1:1234/v1'
        try:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if _alias_target and _alias_target != hermes_id and (_alias_target in _AGG_PROVIDERS):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(store and store.get('credential_pool', {}).get(hermes_id))
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(hermes_id, raw_pool_present=True)
            except Exception:
                pass
        if not has_creds:
            continue
        ...
        results.append({'slug': slug, 'name': display_name, 'is_current': slug == current_provider or hermes_id == current_provider or mdev_id == current_provider, 'is_user_defined': False, 'models': top, 'total_models': total, 'source': 'built-in'})
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if hermes_slug in {'openai-codex', 'copilot', 'copilot-acp'}:
            ...
            try:
                from hermes_cli.models import get_pricing_for_provider as _nous_pricing, check_nous_free_tier as _nous_free, union_with_portal_free_recommendations as _union_free, union_with_portal_paid_recommendations as _union_paid
        ...
        if hermes_slug in _UNCAPPED_PICKER_PROVIDERS:
            ...
            top = model_ids[:max_models] if max_models is not None else model_ids
        results.append({'slug': hermes_slug, 'name': get_label(hermes_slug), 'is_current': hermes_slug == current_provider or pid == current_provider, 'is_user_defined': False, 'models': top, 'total_models': total, 'source': 'duck-agent'})
    ...
    for _cp in _canon_provs:
        ...
        results.append({'slug': _cp.slug, 'name': _cp.label, 'is_current': _cp.slug == current_provider, 'is_user_defined': False, 'models': _cp_top, 'total_models': _cp_total, 'source': 'canonical'})
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            api_mode = str(ep_cfg.get('api_mode') or ep_cfg.get('transport') or '').strip().lower()
            credential_identity = inline_api_key if inline_api_key else f'env:{key_env}' if key_env else ''
            ...
            if group_key not in ep_groups:
                ...
                grp_slug = ep_name
                ep_groups[group_key] = {'slug': grp_slug, 'name': grp_display or display_name, 'api_url': api_url, 'models': [], 'has_explicit_models': False, 'ep_cfg': ep_cfg, 'raw_names': [], 'aliases': set()}
            ...
            ep_groups[group_key]['aliases'].update(custom_provider_aliases(display_name, str(ep_name)))
        for grp in ep_groups.values():
            ...
            _ep_aliases = {str(alias).lower() for alias in grp.get('aliases', set())}
            _ep_is_current = _ep_slug_norm == _current_provider_norm or _current_provider_norm in _ep_aliases or (_current_provider_norm == 'custom' and bool(_current_base_url_norm) and (_ep_url_norm == _current_base_url_norm))
            should_probe = _can_probe_custom_provider(row_is_current=_ep_is_current) and bool(api_url) and discover and (bool(api_key) or not has_explicit_models)
            if should_probe:
                try:
                    from hermes_cli.models import cached_fetch_api_models
                    live_models = cached_fetch_api_models(api_key, api_url, timeout=1.5 if for_picker else 5.0, headers=_extra_headers_from_config(ep_cfg) or None)
                    if live_models:
                        models_list = live_models
                except Exception:
                    pass
            results.append({'slug': ep_name, 'name': display_name, 'is_current': _ep_is_current, 'is_user_defined': True, 'models': models_list, 'total_models': len(models_list) if models_list else 0, 'source': 'user-config', 'api_url': api_url})
            ...
            for _raw_name in grp.get('raw_names') or [display_name]:
                _pair = (str(_raw_name).strip().lower(), _url_norm_for_pair)
                if _pair[0] and _pair[1]:
                    _section3_emitted_pairs.add(_pair)
                    seen_slugs.add(custom_provider_slug(_raw_name).lower())
            _pair = (str(display_name).strip().lower(), _url_norm_for_pair)
            if _pair[0] and _pair[1]:
                _section3_emitted_pairs.add(_pair)
    if _current_provider_norm == 'custom' and current_base_url and ('custom' not in seen_slugs) and (not any((isinstance(_cp, dict) and str(_cp.get('base_url', '') or _cp.get('url', '') or _cp.get('api', '')).strip().rstrip('/').lower() == str(current_base_url).strip().rstrip('/').lower() for _cp in custom_providers or []))):
        _models = [current_model] if current_model else []
        if refresh or probe_current_custom_provider:
            try:
                from hermes_cli.models import cached_fetch_api_models
                _live_models = cached_fetch_api_models('', str(current_base_url).strip().rstrip('/'), timeout=1.5 if for_picker else 5.0)
                if _live_models:
                    _models = _live_models
            except Exception:
                pass
        results.append({'slug': 'custom', 'name': 'Custom endpoint', 'is_current': True, 'is_user_defined': True, 'models': _models[:max_models] if max_models is not None else _models, 'total_models': len(_models), 'source': 'model-config', 'api_url': str(current_base_url).strip().rstrip('/')})
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            api_url = (entry.get('base_url', '') or entry.get('url', '') or entry.get('api', '') or '').strip().rstrip('/')
            if not raw_name or not api_url:
                continue
            inline_api_key = (entry.get('api_key') or '').strip()
            key_env = (entry.get('key_env') or '').strip()
            api_key = inline_api_key or (os.environ.get(key_env, '').strip() if key_env else '')
            ...
            if group_key not in groups:
                ...
                groups[group_key] = {'slug': slug, 'name': display_name, 'api_url': api_url, 'api_key': api_key, 'models': [], 'has_explicit_models': False, 'discover_models': discover, 'extra_headers': entry_extra_headers, 'aliases': set()}
            else:
                if api_key and (not groups[group_key].get('api_key')):
                    groups[group_key]['api_key'] = api_key
                if not discover:
                    groups[group_key]['discover_models'] = False
            groups[group_key]['aliases'].update(custom_provider_aliases(raw_name, str(entry.get('provider_key') or '')))
        ...
        _current_base_url_group_count = sum((1 for _grp in groups.values() if _current_base_url_norm and str(_grp['api_url']).strip().rstrip('/').lower() == _current_base_url_norm))
        for grp in groups.values():
            ...
            if slug.lower() in _section4_emitted_slugs:
                ...
                slug = f'{base_slug}-{n}'
                grp['slug'] = slug
            _pair_key = (str(grp['name']).strip().lower(), str(grp['api_url']).strip().rstrip('/').lower())
            if _pair_key[0] and _pair_key[1] and (_pair_key in _section3_emitted_pairs):
                continue
            _grp_url_norm = _pair_key[1]
            if _grp_url_norm and _grp_url_norm in _builtin_endpoints:
                continue
            _grp_is_current = (slug.lower() == _current_provider_norm or _current_provider_norm in {str(alias).lower() for alias in grp.get('aliases', set())}) or (_current_provider_norm == 'custom' and bool(_current_base_url_norm) and (_grp_url_norm == _current_base_url_norm) and (_current_base_url_group_count == 1))
            should_probe = _can_probe_custom_provider(row_is_current=_grp_is_current) and bool(api_url) and (bool(api_key) or not grp.get('has_explicit_models')) and grp.get('discover_models', True)
            if should_probe:
                try:
                    from hermes_cli.models import cached_fetch_api_models
                    live_models = cached_fetch_api_models(api_key, api_url, timeout=1.5 if for_picker else 5.0, headers=grp.get('extra_headers') or None)
                    if live_models:
                    ...
                    pass
            results.append({'slug': slug, 'name': grp['name'], 'is_current': _grp_is_current, 'is_user_defined': True, 'models': grp['models'], 'total_models': len(grp['models']), 'source': 'user-config', 'api_url': grp['api_url']})
            seen_slugs.add(slug.lower())
            _section4_emitted_slugs.add(slug.lower())
    try:
        from hermes_cli.config import is_provider_enabled
        if isinstance(user_providers, dict):
            _disabled_slugs = {str(name).strip().lower() for name, cfg in user_providers.items() if isinstance(cfg, dict) and (not is_provider_enabled(cfg))}
            if _disabled_slugs:
                results = [r for r in results if str(r.get('provider_id', '')).strip().lower() not in _disabled_slugs and str(r.get('slug', '')).strip().lower() not in _disabled_slugs]
    except Exception:
        pass
    if current_model:
        for _row in results:
...
def list_picker_providers(current_provider: str='', current_base_url: str='', user_providers: dict=None, custom_providers: list | None=None, max_models: int | None=None, current_model: str='', include_moa: bool=False, excluded_providers: list | None=None) -> List[dict]:
    ...
    providers = list_authenticated_providers(current_provider=current_provider, current_base_url=current_base_url, user_providers=user_providers, custom_providers=custom_providers, max_models=max_models, current_model=current_model, for_picker=True, excluded_providers=excluded_providers)
    if include_moa:
