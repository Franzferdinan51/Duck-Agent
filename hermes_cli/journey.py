"""``duck-agent journey`` — what Duck Agent has learned, on a timeline.
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if not payload.get('nodes'):
        console.print('[grey62]No learning yet — use Duck Agent a while and your learned skills and memories will start mapping out here.[/grey62]')
        return 0
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    parent.add_argument('--reveal', type=float, default=1.0, metavar='0..1', help='Render the timeline built up to this point (0=oldest, 1=now).')
...
if __name__ == '__main__':
    _p = argparse.ArgumentParser(prog='duck-agent journey')
    register_cli(_p)
    _a = _p.parse_args()
    sys.exit(_a.func(_a))