"""``duck-agent plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.duck-agent/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Duck Agent subprocess resolution: :func:`shutil.which` first,
    ...
    if os.name == 'nt':
        ...
        candidates = [os.path.join(prog, 'Git', 'cmd', 'git.exe'), os.path.join(prog, 'Git', 'bin', 'git.exe'), os.path.join(prog_x86, 'Git', 'cmd', 'git.exe'), os.path.join(prog_x86, 'Git', 'bin', 'git.exe')]
        if local:
            candidates.extend((os.path.join(local, 'Programs', 'Git', 'cmd', 'git.exe'), os.path.join(local, 'Programs', 'Git', 'bin', 'git.exe')))
    else:
...
def _sanitize_plugin_name(name: str, plugins_dir: Path, *, allow_subdir: bool=False) -> Path:
    ...
    return target
_GITHUB_BROWSER_SEGMENTS = {'actions', 'blob', 'commit', 'commits', 'issues', 'pull', 'pulls', 'releases', 'tree', 'wiki'}
...
def _resolve_git_url(identifier: str) -> tuple[str, Optional[str]]:
    ...
    if len(parts) >= 2:
        ...
        return (git_url, subdir or None)
    raise ValueError(f"Invalid plugin identifier: '{identifier}'. Use a Git URL or 'owner/repo' shorthand (optionally with a subdirectory: 'owner/repo/path/to/plugin').")
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.duck-agent/.env``."""
...
def _display_after_install(plugin_dir: Path, identifier: str) -> None:
    ...
    if after_install.exists():
        ...
        console.print()
        console.print(Panel(f'[green bold]Plugin installed:[/] {identifier}\n[dim]Location:[/] {plugin_dir}', border_style='green', title='✓ Installed', expand=False))
        console.print()
...
def _require_installed_plugin(name: str, plugins_dir: Path, console) -> Path:
    ...
    if not target.exists():
        ...
        console.print(f"[red]Error:[/red] Plugin '{name}' not found in {plugins_dir}.\nInstalled plugins: {installed}")
        sys.exit(1)
    return target

def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.duck-agent/plugins``.
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            result = subprocess.run([git_exe, 'clone', '--depth', '1', git_url, str(tmp_clone)], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=60, stdin=subprocess.DEVNULL, env=noninteractive_git_env())
        ...
        has_native_manifest = (tmp_target / 'plugin.yaml').exists() or (tmp_target / 'plugin.yml').exists()
        has_portable_manifest = (tmp_target / 'plugin.json').exists() or (tmp_target / 'plugin.json').is_symlink()
        ...
        plugin_name = manifest.get('name') or (subdir.rstrip('/').rsplit('/', 1)[-1] if subdir else _repo_name_from_url(git_url))
        try:
        ...
        if mv is not None:
            try:
                ...
                raise PluginOperationError(f"Plugin '{plugin_name}' has invalid manifest_version '{mv}' (expected an integer).") from None
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                from hermes_cli.config import recommended_update_command
                raise PluginOperationError(f"Plugin '{plugin_name}' requires manifest_version {mv}, but this installer only supports up to {_SUPPORTED_MANIFEST_VERSION}. Run {recommended_update_command()} to update Duck Agent.") from None
        if target.exists():
            if not force:
                raise PluginOperationError(f"Plugin '{plugin_name}' already exists. Use force reinstall or run `duck-agent plugins update {plugin_name}`.")
...
def cmd_install(identifier: str, force: bool=False, enable: Optional[bool]=None) -> None:
    ...
    if git_url.startswith(('http://', 'file://')):
        console.print('[yellow]Warning:[/yellow] Using insecure/local URL scheme. Consider using https:// or git@ for production installs.')
    if _subdir:
    ...
    try:
        ...
        console.print(f'[red]Error:[/red] {e}')
        sys.exit(1)
    if not (target / 'plugin.yaml').exists() and (not (target / 'plugin.yml').exists()) and (not (target / 'plugin.json').exists()) and (not (target / '__init__.py').exists()):
        console.print(f"[yellow]Warning:[/yellow] {installed_name} doesn't contain plugin.yaml, plugin.json, or __init__.py. It may not be a valid Duck Agent plugin.")
    ...
    if should_enable is None:
        if sys.stdin.isatty() and sys.stdout.isatty():
            try:
                answer = input(f"  Enable '{installed_name}' now? [y/N]: ").strip().lower()
    ...
    if should_enable:
        ...
        console.print(f'[dim]Plugin installed but not enabled. Run `duck-agent plugins enable {installed_name}` to activate.[/dim]')
    console.print('[dim]Restart the gateway for the plugin to take effect:[/dim]')
    console.print('[dim]  duck-agent gateway restart[/dim]')
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if not (target / '.git').exists():
        console.print(f"[red]Error:[/red] Plugin '{name}' was not installed from git (no .git directory). Cannot update.")
        sys.exit(1)
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``duck-agent setup`` / ``duck-agent plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``duck-agent plugins enable`` /
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        console.print(f'[green]✓[/green] Granted [bold]{key}[/bold] permission to override built-in tools ([dim]plugins.entries.{plugin_id}.allow_tool_override: true[/dim]).')
    else:
        console.print(f'[dim]{key} may not override built-in tools. Re-run `duck-agent plugins enable {key} --allow-tool-override` to grant this later.[/dim]')
...
def cmd_disable(name: str) -> None:
    ...
    _save_enabled_set(enabled)
    _save_disabled_set(disabled)
    console.print(f'[yellow]⊘[/yellow] Plugin [bold]{key}[/bold] disabled. Takes effect on next session.')
...
def _discover_all_plugins() -> list:
    ...
    for base, source, skip in ((repo_plugins, 'bundled', {'memory', 'context_engine'}), (_plugins_dir(), 'user', set())):
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    plugin directory under ``~/.duck-agent/plugins``. Include package metadata here
    so ``duck-agent plugins list`` can show and enable them.
    """
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``duck-agent plugins list`` CLI filters."""
    filtered = entries
    if getattr(args, 'no_bundled', False) or getattr(args, 'user', False):
        filtered = [entry for entry in filtered if entry[3] != 'bundled']
    if getattr(args, 'enabled', False):
        filtered = [entry for entry in filtered if _plugin_status(entry[0], enabled, disabled, key=entry[5]) == 'enabled']
    return filtered
...
def cmd_list(args: Any | None=None) -> None:
    ...
    if not entries:
        ...
        console.print('[dim]Install with:[/dim] duck-agent plugins install owner/repo')
        return
    enabled = _get_enabled_set()
    disabled = _get_disabled_set()
    entries = _filter_plugin_entries(entries, args, enabled, disabled)
    if getattr(args, 'json', False):
        payload = [{'name': name, 'status': _plugin_status(name, enabled, disabled, key=key), 'version': str(version), 'description': description, 'source': source} for name, version, description, source, _dir, key in entries]
    ...
    console.print(table)
    console.print()
    console.print('[dim]Compact view:[/dim] duck-agent plugins list --plain --no-bundled')
    console.print('[dim]Interactive toggle:[/dim] duck-agent plugins')
    console.print('[dim]Enable/disable:[/dim] duck-agent plugins enable/disable <name>')
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Duck Agent
...
def _configure_memory_provider() -> bool:
    ...
    if current and current not in names:
        ...
        selected = len(items) - 1
    choice = curses_radiolist(title='Memory Provider (select one)', items=items, selected=selected)
...
def _configure_context_engine() -> bool:
    ...
    if current != 'compressor' and current not in names:
        ...
        items.append(f'{current} (not found)')
        selected = len(items) - 1
    choice = curses_radiolist(title='Context Engine (select one)', items=items, selected=selected)
...
def cmd_toggle() -> None:
    ...
    for i, (name, _version, description, source, _d, key) in enumerate(entries):
        ...
        is_on = (key in enabled_set or name in enabled_set) and key not in disabled_set and (name not in disabled_set)
        if is_on:
            plugin_selected.add(i)
    current_memory = _get_current_memory_provider() or 'built-in'
    current_context = _get_current_context_engine()
    categories = [('Memory Provider', current_memory, _configure_memory_provider), ('Context Engine', current_context, _configure_context_engine)]
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    total_items = n_plugins + n_categories
    ...
    def _draw(stdscr):
        ...
        while True:
            ...
            try:
                ...
                stdscr.addnstr(1, 0, '  ↑↓/j/k navigate  PgUp/PgDn page  SPACE toggle  ENTER configure/confirm  ESC done', max_x - 1, curses.A_DIM)
            except curses.error:
                pass
                ...
                if cursor < n_plugins:
                    ...
                    if 0 <= ci < n_categories:
                        ...
                        if changed:
                            ...
                            categories[ci] = (_cat_name, _get_current_memory_provider() or 'built-in' if ci == 0 else _get_current_context_engine(), cat_fn)
    ...
    if enabled_changed or disabled_changed:
        ...
        console.print(f'\n[green]✓[/green] General plugins: {len(new_enabled)} enabled, {len(plugin_keys) - len(new_enabled)} disabled.')
    elif n_plugins > 0:
    ...
    if result_holder['providers_changed']:
        ...
        console.print(f'[green]✓[/green] Memory provider: [bold]{new_memory}[/bold]  Context engine: [bold]{new_context}[/bold]')
...
def dashboard_install_plugin(identifier: str, *, force: bool, enable: bool) -> dict[str, Any]:
    ...
    if ap.exists():
        hint = str(ap)
    return {'ok': True, 'plugin_name': installed_name, 'warnings': warnings, 'missing_env': missing_env, 'after_install_path': hint, 'enabled': enable}
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.duck-agent/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.duck-agent/plugins/<name>``."""
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    try:
        result = subprocess.run([git_exe, 'pull', '--ff-only'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=60, cwd=str(target), stdin=subprocess.DEVNULL, env=noninteractive_git_env())
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.duck-agent/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch duck-agent plugins subcommands."""
    ...
    if action == 'install':
        ...
        if getattr(args, 'enable', False):
            ...
            enable_arg = False
        cmd_install(args.identifier, force=getattr(args, 'force', False), enable=enable_arg)
    elif action == 'update':
