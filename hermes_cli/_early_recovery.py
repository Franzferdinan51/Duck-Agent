...
The ``duck-agent`` console entry point is ``hermes_cli.main:main``.  Importing
...
LAZY_REFRESH_IMPORT_PROBES: tuple[tuple[str, str], ...] = (('yaml', 'SafeDumper'), ('dotenv', 'load_dotenv'), ('click', 'Command'), ('certifi', 'contents'), ('rich', 'print'), ('cryptography', '__version__'), ('jwt', 'encode'))
LAZY_REFRESH_REPAIR_PACKAGES: dict[str, str] = {'yaml': 'PyYAML', 'dotenv': 'python-dotenv', 'click': 'click', 'certifi': 'certifi', 'rich': 'rich', 'cryptography': 'cryptography', 'jwt': 'PyJWT'}
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``duck-agent acp`` speaks JSON-RPC on stdout);
    ...
    try:
        ...
        pass
    try:
        result = subprocess.run([sys.executable, '-m', 'pip', 'install', '--force-reinstall', *specs], cwd=project_root, capture_output=True, text=True, encoding='utf-8', errors='replace')
...
def recover_if_needed(project_root: Path | None=None, argv: list[str] | None=None) -> None:
    ...
    recovery marker from a prior ``duck-agent update`` exists AND an import probe
    ...
    try:
        ...
        try:
            ...
            print(f"⚠ Core package(s) broken by an interrupted update — repairing before launch: {', '.join(broken)}", file=sys.stderr)
            if _run_repair_install(specs, root) and (not _probe_broken_packages()):
                ...
                print(f'    {sys.executable} -m pip install --force-reinstall ' + ' '.join(specs), file=sys.stderr)
        finally:
            try:
                lock_path.unlink()
            except OSError:
