...
Differences from Claude Code:
    ...
    - Tailored to duck-agent's tool vocabulary (``terminal``, ``patch``,
...
_MAX_FILES_LISTED = 5
_FILE_EDIT_TOOLS: Mapping[str, str] = {'write_file': 'path', 'patch': 'path', 'read_file': 'path', 'skill_manage': 'file_path', 'skill_view': 'file_path'}
...
def _latest_user_prompt(messages: Sequence[Mapping[str, Any]]) -> Optional[str]:
...
def _latest_assistant_text(messages: Sequence[Mapping[str, Any]]) -> Optional[str]:
    for msg in reversed(messages):
        if not isinstance(msg, Mapping):
            continue
        if msg.get('role') != 'assistant':
...
def _recent_window(messages: Sequence[Mapping[str, Any]], window: int=_RECENT_TURN_WINDOW) -> List[Mapping[str, Any]]:
...
def _summarise_tool_activity(tool_calls: Sequence[Tuple[str, Mapping[str, Any]]]) -> Tuple[List[Tuple[str, int]], List[str]]:
...
def build_recap(messages: Sequence[Mapping[str, Any]], *, session_title: Optional[str]=None, session_id: Optional[str]=None, platform: Optional[str]=None) -> str:
    ...
    scope = f"{win_users} user turn{('s' if win_users != 1 else '')} / {win_assistants} assistant repl{('ies' if win_assistants != 1 else 'y')}"
