"""
Doctor command for duck-agent CLI.

Diagnoses issues with Duck Agent setup.
"""
import os
import sys
import subprocess
...
from hermes_cli.config import detect_install_method, get_env_path, get_hermes_home, get_project_root, recommended_update_command_for_method
...
_PROVIDER_ENV_HINTS = ('DEEPINFRA_API_KEY', 'OPENROUTER_API_KEY', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'ANTHROPIC_TOKEN', 'OPENAI_BASE_URL', 'NOUS_API_KEY', 'GLM_API_KEY', 'ZAI_API_KEY', 'Z_AI_API_KEY', 'KIMI_API_KEY', 'KIMI_CN_API_KEY', 'GMI_API_KEY', 'FIREWORKS_API_KEY', 'ACTUAL_API_KEY', 'ACTUAL_BASE_URL', 'MINIMAX_API_KEY', 'MINIMAX_CN_API_KEY', 'KILOCODE_API_KEY', 'DEEPSEEK_API_KEY', 'DASHSCOPE_API_KEY', 'HF_TOKEN', 'AI_GATEWAY_API_KEY', 'OPENCODE_ZEN_API_KEY', 'OPENCODE_GO_API_KEY', 'XIAOMI_API_KEY', 'TOKENHUB_API_KEY')
...
def _sqlite_upgrade_hint(install_method: str | None=None) -> str:
    ...
    if method == 'docker':
        ...
        action = f'run `{command}`, then recreate all Duck Agent containers'
    elif method in {'nix', 'nixos'}:
        action = recommended_update_command_for_method(method)
    else:
        action = 'run `duck-agent update`'
...
def _fail_and_issue(text: str, detail: str, fix: str, issues: list[str]) -> None:
    ...
    issues.append(fix)
_DEPRECATED_CONFIG_KEYS: tuple[tuple[str, str, str], ...] = (('display', 'tool_progress_overrides', 'display.platforms'), ('delegation', 'max_async_children', 'delegation.max_concurrent_children'))
_DEPRECATED_COMPRESSION_SUMMARY_KEYS: tuple[str, ...] = ('summary_model', 'summary_provider', 'summary_base_url')
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (('HERMES_TOOL_PROGRESS', 'display.tool_progress in config.yaml — ignored/unsupported since config floor v12'), ('HERMES_TOOL_PROGRESS_MODE', 'display.tool_progress in config.yaml'), ('TERMINAL_CWD', 'terminal.cwd in config.yaml'), ('MESSAGING_CWD', 'terminal.cwd in config.yaml'), ('QQ_HOME_CHANNEL', 'QQBOT_HOME_CHANNEL'), ('QQ_HOME_CHANNEL_NAME', 'QQBOT_HOME_CHANNEL_NAME'))
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``duck-agent --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue('Version mismatch between source files', f'(pyproject.toml {pyproject_version} != hermes_cli/__init__.py {init_version})', "Re-sync version files (e.g. run 'duck-agent update', or set hermes_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-duck-agent and dashboard static services are up
    ...
    for static in ('main-duck-agent', 'dashboard'):
    ...
    if not profiles:
        check_info('No per-profile gateways registered yet — create one with `duck-agent profile create <name>`')
        return
    up_count = sum((1 for p in profiles if mgr.is_running(f'gateway-{p}')))
    check_ok(f'Per-profile gateways: {up_count}/{len(profiles)} supervised up' + (f" ({', '.join(sorted(profiles))})" if len(profiles) <= 8 else ''))
...
def check_certificates(should_fix: bool=False, issues: 'list | None'=None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(f'Repair the CA bundle: run `duck-agent doctor --fix`, or `{sys.executable} -m pip install --force-reinstall certifi`')
        return
    check_fail('SSL CA certificate bundle is broken', first_error)
    print('    → Repairing: force-reinstalling certifi...')
    try:
        result = subprocess.run([sys.executable, '-m', 'pip', 'install', '--force-reinstall', 'certifi'], capture_output=True, text=True, timeout=300)
    except Exception as exc:
        check_fail('certifi repair could not run pip', str(exc))
        if issues is not None:
            issues.append(f'Reinstall certifi manually: {sys.executable} -m pip install --force-reinstall certifi')
        return
    ...
    try:
        ...
        if issues is not None:
            issues.append('certifi reinstall did not restore the CA bundle — check for a custom CA env var (SSL_CERT_FILE/REQUESTS_CA_BUNDLE) pointing at a missing file, or recreate the venv.')
    ...
    try:
        ...
        for _pp in list_providers():
            ...
            if _candidates & _dedicated_canonical:
                continue
            _key_vars = tuple((v for v in _pp.env_vars if not v.endswith('_BASE_URL') and (not v.endswith('_URL'))))
            _base_var = next((v for v in _pp.env_vars if v.endswith('_BASE_URL') or v.endswith('_URL')), None)
            if not _key_vars:
                continue
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(f'  ✗ Failed to persist ack for {ack_target}. Check ~/.duck-agent/config.yaml is writable.', Colors.RED))
            sys.exit(1)
        return
    issues = []
    manual_issues = []
    ...
    print(color('│                 🩺 Duck Agent Doctor                        │', Colors.CYAN))
    print(color('└─────────────────────────────────────────────────────────┘', Colors.CYAN))
            ...
            for hit in fresh_hits:
                ...
                for line in full_remediation_text(hit):
                    if line:
                        ...
                        print()
                manual_issues.append(f'Resolve security advisory {hit.advisory.id}: uninstall {hit.package}=={hit.installed_version} and rotate credentials, then run `duck-agent doctor --ack {hit.advisory.id}`.')
    ...
    try:
        ...
        if isinstance(servers, dict):
            for name, entry in sorted(servers.items()):
                ...
                manual_issues.append(f'Review/remove mcp_servers.{name} in config.yaml; rotate any credentials that may have been exposed.')
        if suspicious == 0:
    ...
    if py_version >= (3, 11):
        ...
        _fail_and_issue(f'Python {py_version.major}.{py_version.minor}.{py_version.micro}', '(3.10+ required)', 'Upgrade Python to 3.10+', issues)
    try:
        import sqlite3
        ...
        _sqlite_src_short = _sqlite_src[:48] + '…' if len(_sqlite_src) > 48 else _sqlite_src
        if is_sqlite_wal_reset_vulnerable():
            check_warn(f'SQLite {_sqlite_ver} (WAL-reset bug)', _sqlite_upgrade_hint())
        else:
    ...
    required_packages = [('openai', 'OpenAI SDK'), ('rich', 'Rich (terminal UI)'), ('dotenv', 'python-dotenv'), ('yaml', 'PyYAML'), ('httpx', 'HTTPX')]
    optional_packages = [('croniter', 'Croniter (cron expressions)'), ('telegram', 'python-telegram-bot'), ('discord', 'discord.py')]
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'duck-agent setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                env_path.touch()
                try:
                    os.chmod(str(env_path), 384)
                except OSError:
                    pass
                check_ok(f'Created empty {_DHH}/.env')
                check_info("Run 'duck-agent setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'duck-agent setup' to create one")
                issues.append("Run 'duck-agent setup' to create .env")
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                from hermes_cli.auth import PROVIDER_REGISTRY, resolve_provider as _resolve_auth_provider
            ...
            try:
                ...
                from hermes_cli.providers import custom_provider_aliases as _custom_provider_aliases, normalize_provider as _normalize_catalog_provider, resolve_provider_full as _resolve_provider_full
            except Exception:
            ...
            if isinstance(user_providers, dict):
                ...
                known_providers.update((str(name).strip().lower() for name, prov_cfg in user_providers.items() if str(name).strip() and is_provider_enabled(prov_cfg)))
            for entry in custom_providers:
                ...
                if name and _custom_provider_aliases is not None:
                    known_providers.update(_custom_provider_aliases(name, provider_key))
            ...
            if provider and _resolve_auth_provider is not None and (provider not in {'auto', 'custom'}):
                try:
            ...
            if provider and provider != 'auto':
                if catalog_provider is None or (known_providers and (not provider_ids_to_accept & valid_provider_ids)):
                    known_list = ', '.join(sorted(known_providers)) if known_providers else '(unavailable)'
                    _fail_and_issue(f"model.provider '{provider_raw}' is not a recognised provider", f'(known: {known_list})', f"model.provider '{provider_raw}' is unknown. Valid providers: {known_list}. Fix: run 'duck-agent config set model.provider <valid_provider>'", issues)
            provider_for_policy = runtime_provider or catalog_provider
            provider_policy_id = str(provider_for_policy or '').strip().lower()
            providers_accepting_vendor_slugs = {'openrouter', 'auto', 'ai-gateway', 'kilocode', 'opencode-zen', 'huggingface', 'lmstudio', 'nous', 'nvidia', 'fireworks', 'deepinfra'}
            provider_accepts_vendor_slug = provider_policy_id in providers_accepting_vendor_slugs or provider_policy_id == 'custom' or provider_policy_id.startswith('custom:')
            if default_model and '/' in default_model and provider_policy_id and (not provider_accepts_vendor_slug):
                check_warn(f"model.default '{default_model}' uses a vendor/model slug but provider is '{provider_raw}'", '(vendor-prefixed slugs belong to aggregators like openrouter)')
                issues.append(f"model.default '{default_model}' is vendor-prefixed but model.provider is '{provider_raw}'. Either set model.provider to 'openrouter', or drop the vendor prefix.")
            if runtime_provider and runtime_provider not in ('auto', 'custom'):
                try:
                    if runtime_provider == 'openrouter':
                        from hermes_cli.config import get_env_value
                        configured = bool(str(get_env_value('OPENROUTER_API_KEY') or '').strip() or str(get_env_value('OPENAI_API_KEY') or '').strip())
                    else:
                    ...
                    if not configured:
                        _fail_and_issue(f"model.provider '{runtime_provider}' is set but no API key is configured", "(check ~/.duck-agent/.env or run 'duck-agent setup')", f"No credentials found for provider '{runtime_provider}'. Run 'duck-agent setup' or set the provider's API key in {_DHH}/.env, or switch providers with 'duck-agent config set model.provider <name>'", issues)
                except Exception:
                    pass
        except Exception as e:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'duck-agent setup' to migrate config")
                else:
                    issues.append("Run 'duck-agent doctor --fix' or 'duck-agent setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'duck-agent doctor --fix'")
        except Exception:
            pass
        try:
            ...
            drift = cfg_max_turns is not None and env_ghost is not None and (str(cfg_max_turns).strip() != str(env_ghost).strip())
            if drift:
                check_warn(f'HERMES_MAX_ITERATIONS={env_ghost} in .env shadows agent.max_turns={cfg_max_turns} in config.yaml', '(stale ghost from an earlier `duck-agent setup` run)')
                if should_fix:
                    if remove_env_value('HERMES_MAX_ITERATIONS'):
                        ...
                        manual_issues.append(f'Manually delete the HERMES_MAX_ITERATIONS line from {_DHH}/.env — config.yaml agent.max_turns is authoritative.')
                else:
                    issues.append("Stale HERMES_MAX_ITERATIONS in .env shadows config.yaml — run 'duck-agent doctor --fix'")
        except Exception:
            pass
        try:
    ...
    try:
        ...
        if not retired_refs:
            ...
            manual_issues.append(f'Update {len(retired_refs)} retired xAI model reference(s) in config.yaml — see {MIGRATION_GUIDE_URL}')
    ...
    if soul_path.exists():
        ...
        check_warn(f'{_DHH}/SOUL.md not found', '(create it to give Duck Agent a custom personality)')
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text('# Duck Agent Persona\n\n<!-- Edit this file to customize how Duck Agent communicates. -->\n\nYou are Duck Agent, a helpful AI assistant.\n', encoding='utf-8')
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    if report.get('repaired'):
                        ...
                        check_ok('Repaired state.db FTS write health', f"(strategy: {report.get('strategy')}; backup: {backup_name})")
                        fixed_count += 1
                    else:
                        check_warn('state.db FTS write-health repair did not recover automatically', f"({report.get('error')}; backup: {report.get('backup_path')})")
                        issues.append('state.db FTS write corruption and auto-repair failed — restore from the backup copy beside state.db')
                else:
                    issues.append("state.db FTS write corruption — run 'duck-agent doctor --fix' (or 'duck-agent sessions repair') to rebuild the FTS index")
        except Exception as e:
                ...
                if should_fix:
                    ...
                    if report.get('repaired'):
                        try:
                            ...
                            count = '?'
                        backup_name = Path(report['backup_path']).name if report.get('backup_path') else 'n/a'
                        check_ok(f'Repaired state.db schema ({count} sessions recovered)', f"(strategy: {report.get('strategy')}; backup: {backup_name})")
                        fixed_count += 1
                    else:
                        check_warn('state.db schema repair did not recover automatically', f"({report.get('error')}; backup: {report.get('backup_path')})")
                        issues.append('state.db schema malformed and auto-repair failed — restore from the backup copy beside state.db')
                else:
                    issues.append("state.db schema malformed — run 'duck-agent doctor --fix' (or 'duck-agent sessions repair') to recover hidden sessions")
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'duck-agent doctor --fix' to checkpoint")
    ...
    if sys.platform != 'win32':
        ...
        for _venv_name in ('venv', '.venv'):
            _candidate = PROJECT_ROOT / _venv_name / 'bin' / 'duck-agent'
        ...
        _cmd_link = _cmd_link_dir / 'duck-agent'
        if _venv_bin is None:
            check_warn('Venv entry point not found', "(duck-agent not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
            manual_issues.append(f"Reinstall entry point: cd {PROJECT_ROOT} && source venv/bin/activate && pip install -e '.[all]'")
        else:
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f'{_cmd_link_display}/duck-agent → correct target')
                else:
                    check_warn(f'{_cmd_link_display}/duck-agent points to wrong target', f'(→ {_target}, expected → {_expected})')
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f'Fixed symlink: {_cmd_link_display}/duck-agent → {_venv_bin}')
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/duck-agent — run 'duck-agent doctor --fix'")
            elif _cmd_link.exists():
                check_ok(f'{_cmd_link_display}/duck-agent exists (non-symlink)')
            else:
                check_fail(f'{_cmd_link_display}/duck-agent not found', '(duck-agent command may not work outside the venv)')
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f'Created symlink: {_cmd_link_display}/duck-agent → {_venv_bin}')
                    fixed_count += 1
                    _path_dirs = os.environ.get('PATH', '').split(os.pathsep)
                    if str(_cmd_link_dir) not in _path_dirs:
                        check_warn(f'{_cmd_link_display} is not on your PATH', '(add it to your shell config: export PATH="$HOME/.local/bin:$PATH")')
                        manual_issues.append(f'Add {_cmd_link_display} to your PATH')
                else:
                    issues.append(f"Missing {_cmd_link_display}/duck-agent symlink — run 'duck-agent doctor --fix'")
    ...
    if terminal_env == 'vercel_sandbox':
        ...
        if disk in {'', '0', '51200'}:
            ...
            _fail_and_issue('Vercel custom disk unsupported', '(reset terminal.container_disk to 51200)', 'Vercel Sandbox does not support custom container_disk; use the shared default 51200', issues)
        if importlib.util.find_spec('vercel') is not None:
            check_ok('vercel SDK', '(installed)')
        else:
            _fail_and_issue('vercel SDK not installed', "(pip install 'duck-agent[vercel]')", "Install the Vercel optional dependency: pip install 'duck-agent[vercel]'", issues)
    ...
    if _safe_which('node'):
        ...
        def _which_in(directory) -> str | None:
            try:
                ...
                return None
        _managed_ab = _which_in(HERMES_HOME / 'node' / 'bin') or _which_in(HERMES_HOME / 'node')
        ...
        if agent_browser_path.exists():
            ...
            check_warn('agent-browser found but not runnable', f'(broken symlink at {_which_ab}? run: npm install)')
        elif _is_termux():
        ...
        if agent_browser_ok and (not _is_termux()):
            try:
                from tools.browser_tool import _chromium_installed, _is_camofox_mode, _get_cloud_provider, _get_cdp_override_raw, _using_lightpanda_engine
            except Exception:
                pass
            else:
                skip_chromium_check = _is_camofox_mode() or bool(_get_cdp_override_raw()) or _get_cloud_provider() is not None or _using_lightpanda_engine()
                if not skip_chromium_check:
                    if _chromium_installed():
                        ...
                        if sys.platform == 'win32':
                            check_info(f'Install with: cd {PROJECT_ROOT} && npx playwright install chromium')
                        else:
                            check_info(f'Install with: cd {PROJECT_ROOT} && npx playwright install --with-deps chromium')
    elif _is_termux():
    ...
    if _npm_bin:
        ...
        npm_audit_targets = [(PROJECT_ROOT, 'Browser tools (agent-browser)', ['--workspaces=false']), (PROJECT_ROOT, 'web workspace', ['--workspace', 'web']), (PROJECT_ROOT, 'ui-tui workspace', ['--workspace', 'ui-tui']), (_whatsapp_bridge_dir, 'WhatsApp bridge', [])]
        for npm_dir, label, audit_extra in npm_audit_targets:
            ...
            if not (check_dir / 'node_modules').exists():
                continue
            try:
                audit_result = subprocess.run([_npm_bin, 'audit', '--json', *audit_extra], cwd=str(npm_dir), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30)
                ...
                if total == 0:
                    ...
                    if fix_cmd:
                        ...
                        vuln_detail = f'{critical} critical, {high} high, {moderate} moderate — build-tool advisory; clears via lockfile bump'
                    check_warn(f'{label} deps', f'({vuln_detail})')
                    if audit_extra and audit_extra[0] == '--workspace':
                        check_info("  ^ build-time tooling (not runtime); if manual npm remediation errors with an arborist crash it's a known npm bug — clears via a lockfile bump")
                    issues.append(f"{label} has {total} npm {('vulnerability' if total == 1 else 'vulnerabilities')}")
                else:
                    check_ok(f'{label} deps', f"({moderate} moderate {('vulnerability' if moderate == 1 else 'vulnerabilities')})")
            except Exception:
                pass
    if _is_termux():
    ...
    _ConnectivityResult = _namedtuple('_ConnectivityResult', ['label', 'lines', 'issues'])
    _probes: list = []

    def _probe_openrouter() -> _ConnectivityResult:
        key = os.getenv('OPENROUTER_API_KEY')
        if not key:
            return _ConnectivityResult('OpenRouter API', [(color('⚠', Colors.YELLOW), 'OpenRouter API', color('(not configured)', Colors.DIM))], [])
        try:
            import httpx
            r = httpx.get(OPENROUTER_MODELS_URL, headers={'Authorization': f'Bearer {key}'}, timeout=10)
            if r.status_code == 200:
                return _ConnectivityResult('OpenRouter API', [(color('✓', Colors.GREEN), 'OpenRouter API', '')], [])
            if r.status_code == 401:
                return _ConnectivityResult('OpenRouter API', [(color('✗', Colors.RED), 'OpenRouter API', color('(invalid API key)', Colors.DIM))], ['Check OPENROUTER_API_KEY in .env'])
            if r.status_code == 402:
                return _ConnectivityResult('OpenRouter API', [(color('✗', Colors.RED), 'OpenRouter API', color('(out of credits — payment required)', Colors.DIM))], ["OpenRouter account has insufficient credits. Fix: run 'duck-agent config set model.provider <provider>' to switch providers, or fund your OpenRouter account at https://openrouter.ai/settings/credits"])
            if r.status_code == 429:
                return _ConnectivityResult('OpenRouter API', [(color('✗', Colors.RED), 'OpenRouter API', color('(rate limited)', Colors.DIM))], ['OpenRouter rate limit hit — consider switching to a different provider or waiting'])
            return _ConnectivityResult('OpenRouter API', [(color('✗', Colors.RED), 'OpenRouter API', color(f'(HTTP {r.status_code})', Colors.DIM))], [])
        except Exception as e:
            return _ConnectivityResult('OpenRouter API', [(color('✗', Colors.RED), 'OpenRouter API', color(f'({e})', Colors.DIM))], ['Check network connectivity'])
    ...
    def _probe_anthropic() -> _ConnectivityResult:
        ...
        try:
            ...
            if is_oauth and r.status_code == 400 and ('long context beta' in r.text.lower()) and ('not yet available' in r.text.lower()):
                ...
                r = httpx.get('https://api.anthropic.com/v1/models', headers=headers, timeout=10)
            if r.status_code == 200:
                return _ConnectivityResult('Anthropic API', [(color('✓', Colors.GREEN), 'Anthropic API', '')], [])
            if r.status_code == 401:
                return _ConnectivityResult('Anthropic API', [(color('✗', Colors.RED), 'Anthropic API', color('(invalid API key)', Colors.DIM))], [])
            return _ConnectivityResult('Anthropic API', [(color('⚠', Colors.YELLOW), 'Anthropic API', color("(couldn't verify)", Colors.DIM))], [])
        except Exception as e:
            return _ConnectivityResult('Anthropic API', [(color('⚠', Colors.YELLOW), 'Anthropic API', color(f'({e})', Colors.DIM))], [])

    def _probe_apikey_provider(pname, env_vars, default_url, base_env, supports_health_check) -> _ConnectivityResult:
        key = ''
        for ev in env_vars:
            key = os.getenv(ev, '')
            if key:
        ...
        if not supports_health_check:
            return _ConnectivityResult(pname, [(color('✓', Colors.GREEN), label, color('(key configured)', Colors.DIM))], [])
        try:
            import httpx
            ...
            return _ConnectivityResult(pname, [(color('⚠', Colors.YELLOW), label, color(f'({e})', Colors.DIM))], [])
    ...
    def _probe_bedrock() -> _ConnectivityResult:
        ...
        try:
            ...
            return _ConnectivityResult('AWS Bedrock', [(color('✓', Colors.GREEN), label, color(f'({auth_var}, {region}, {n} models)', Colors.DIM))], [])
        except ImportError:
            return _ConnectivityResult('AWS Bedrock', [(color('⚠', Colors.YELLOW), label, color(f'(boto3 not installed — {sys.executable} -m pip install boto3)', Colors.DIM))], [f'Install boto3 for Bedrock: {sys.executable} -m pip install boto3'])
        except Exception as e:
            err_name = type(e).__name__
            return _ConnectivityResult('AWS Bedrock', [(color('⚠', Colors.YELLOW), label, color(f'({err_name}: {e})', Colors.DIM))], [f'AWS Bedrock: {err_name} — check IAM permissions for bedrock:ListFoundationModels'])
    ...
    def _probe_azure_entra() -> _ConnectivityResult:
        ...
        try:
            ...
            return _ConnectivityResult('Azure Foundry (Entra ID)', [], [])
        try:
            from agent.azure_identity_adapter import EntraIdentityConfig, SCOPE_AI_AZURE_DEFAULT, describe_active_credential, has_azure_identity_installed
        except Exception as exc:
            return _ConnectivityResult('Azure Foundry (Entra ID)', [(color('⚠', Colors.YELLOW), label, color(f'(adapter import failed: {exc})', Colors.DIM))], [f'Azure Foundry adapter import failed: {exc}'])
        if not has_azure_identity_installed():
            return _ConnectivityResult('Azure Foundry (Entra ID)', [(color('⚠', Colors.YELLOW), label, color('(azure-identity not installed)', Colors.DIM))], [f'Install azure-identity: {sys.executable} -m pip install azure-identity'])
        ...
        if info.get('ok'):
            ...
            return _ConnectivityResult('Azure Foundry (Entra ID)', [(color('✓', Colors.GREEN), label, color(f'({tag}, scope={scope})', Colors.DIM))], [])
        ...
        return _ConnectivityResult('Azure Foundry (Entra ID)', [(color('⚠', Colors.YELLOW), label, color(f'({err})', Colors.DIM))], [f'Azure Foundry Entra: {err}. {hint}'])
    ...
    for _entry in _APIKEY_PROVIDERS_CACHE:
        ...
        _probes.append((_pname, lambda p=_pname, e=_env_vars, u=_default_url, b=_base_env, s=_supports: _probe_apikey_provider(p, e, u, b, s)))
    _probes.append(('AWS Bedrock', _probe_bedrock))
    _probes.append(('Azure Foundry (Entra ID)', _probe_azure_entra))
    print(f"  {color(f'Running {len(_probes)} connectivity checks in parallel…', Colors.DIM)}", end='', flush=True)
    _imds_prev = os.environ.get('AWS_EC2_METADATA_DISABLED')
    os.environ['AWS_EC2_METADATA_DISABLED'] = 'true'
    try:
        with _futures.ThreadPoolExecutor(max_workers=8, thread_name_prefix='doctor-probe') as _ex:
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'duck-agent setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn('Skills Hub directory not initialized', '(run: duck-agent skills list)')
    from hermes_cli.config import get_env_value
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                if hcfg.api_key or hcfg.base_url:
                    check_ok('Honcho configured via environment variables', f'config file {_honcho_cfg_path} not found, using HONCHO_API_KEY env var')
                else:
                    check_warn('Honcho config not found', 'run: duck-agent memory setup')
            elif not hcfg.enabled:
                check_info(f'Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)')
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue('Honcho API key or base URL not set', 'run: duck-agent memory setup', "No Honcho API key — run 'duck-agent memory setup'", issues)
            else:
            ...
            _fail_and_issue('honcho-ai not installed', 'pip install honcho-ai', 'Honcho is set as memory provider but honcho-ai is not installed', issues)
        except Exception as _e:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue('Mem0 API key not set', '(set MEM0_API_KEY in .env or run duck-agent memory setup)', 'Mem0 is set as memory provider but API key is missing', issues)
        except ImportError:
            _fail_and_issue('Mem0 plugin not loadable', 'pip install mem0ai', 'Mem0 is set as memory provider but mem0ai is not installed', issues)
        except Exception as _e:
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f'{_active_memory_provider} configured but not available', 'run: duck-agent memory status')
            else:
                check_warn(f'{_active_memory_provider} plugin not found', 'run: duck-agent memory setup')
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if 'duck-agent -p' in content:
                            _m = _re.search('duck-agent -p (\\S+)', content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'duck-agent doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color('─' * 60, Colors.GREEN))
