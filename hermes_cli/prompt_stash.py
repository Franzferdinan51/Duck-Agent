...
through ``get_hermes_home()`` rather than hardcoding ``~/.duck-agent``.
"""
...
class PromptStash:
    ...
    def stash(self, text: str, images: Optional[Sequence[Any]]=None) -> bool:
        ...
        if not (text or '').strip() and (not has_images):
            return False
        entry = StashEntry(text=text or '', images=list(images or []), stashed_at=self._clock(), preview=build_preview(text or '') or '(images only)')
...
def resolve_ctrl_s(stash: PromptStash, buffer_text: str, images: Optional[Sequence[Any]]=None) -> Tuple[str, Optional[Tuple[str, List[Any]]]]:
