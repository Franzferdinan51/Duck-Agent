"""``duck-agent portal`` — the human-readable entry point for Nous Portal.

Running ``duck-agent portal`` with no subcommand performs the one-shot Portal
...
``duck-agent auth add nous --type oauth`` (which still works), is identical to
``duck-agent setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``duck-agent auth`` or ``duck-agent tools``. It's the onboarding + discovery
...
DOCS_URL = 'https://duck-agent.nousresearch.com/docs/user-guide/features/tool-gateway'
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print('  Login:   duck-agent portal')
    model_cfg = config.get('model') if isinstance(config.get('model'), dict) else {}
    provider = str(model_cfg.get('provider') or '').strip().lower()
    if provider == 'nous':
        print(f"  Model:   {color('✓ using Nous as inference provider', Colors.GREEN)}")
    elif provider:
        print(f'  Model:   currently {provider} (switch with `duck-agent model`)')
    print()
...
def _cmd_tools(args) -> int:
    ...
    try:
        ...
        return 1
    catalog = [('web', 'Web search & extract', 'Firecrawl'), ('image_gen', 'Image generation', 'FAL'), ('tts', 'Text-to-speech', 'OpenAI TTS'), ('browser', 'Browser automation', 'Browser Use'), ('modal', 'Cloud terminal', 'Modal')]
    print()
    print(color('  Tool Gateway catalog', Colors.MAGENTA))
    print(color('  ────────────────────', Colors.MAGENTA))
    if not features.nous_auth_present:
        print(color('  Not logged into Nous Portal — sign in with `duck-agent portal`.', Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `duck-agent auth add nous --type
    oauth`. It reuses the exact wiring behind `duck-agent setup --portal` (which in
...
def portal_command(args) -> int:
    """Top-level dispatch for `duck-agent portal <subcommand>`."""
    ...
    print('Run `duck-agent portal -h` for usage.', file=sys.stderr)
    return 1

def add_parser(subparsers) -> None:
    """Register `duck-agent portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser('portal', help='Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`', description='Run `duck-agent portal` with no subcommand to log in to Nous Portal and set it up — pick a model, set Nous as your provider, and offer the Tool Gateway (the human-readable alias for `duck-agent auth add nous --type oauth`, identical to `duck-agent setup --portal`). Subcommands: login (default), info, open, tools.')
    portal_sub = portal_parser.add_subparsers(dest='portal_command')
    portal_sub.add_parser('login', help='Log in to Nous Portal + set it up (default; one-shot onboarding)')
    portal_sub.add_parser('info', help='Show Portal auth + Tool Gateway routing summary')
    portal_sub.add_parser('status')
    portal_sub.add_parser('open', help='Open the Portal subscription page in your default browser')
    portal_sub.add_parser('tools', help='List Tool Gateway tools and which are routed via Nous')
