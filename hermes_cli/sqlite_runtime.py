...
update code must be able to use it before Duck Agent' third-party dependencies are
healthy.
"""
...
def probe_sqlite_runtime(python: str | Path, *, timeout: float=30.0) -> SQLiteRuntimeInfo | None:
    ...
    for key in ('CONDA_DEFAULT_ENV', 'CONDA_PREFIX', 'PYTHONHOME', 'PYTHONPATH', 'UV_PROJECT_ENVIRONMENT', 'UV_PYTHON', 'VIRTUAL_ENV'):
        env.pop(key, None)
    try:
        result = subprocess.run([str(executable), '-I', '-c', _PROBE_SCRIPT], capture_output=True, text=True, timeout=timeout, check=False, env=env)
    ...
    try:
        ...
        return SQLiteRuntimeInfo(executable=Path(str(payload['executable'])), base_prefix=Path(str(payload['base_prefix'])), python_version=_version_tuple(payload['python_version']), sqlite_version=_version_tuple(payload['sqlite_version']), sqlite_version_string=str(payload['sqlite_version_string']), sqlite_source_id=str(payload.get('sqlite_source_id', '')))
