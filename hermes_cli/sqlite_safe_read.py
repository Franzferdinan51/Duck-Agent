...
Duck Agent is exactly the topology this hits: gateway, dispatcher, dashboard,
...
def connect_tracked(path: Path | str, *, tracking_path: Path | str | None=None, connect_fn=None, **kwargs) -> sqlite3.Connection:
...
def _retrofit_tracking(conn: sqlite3.Connection, resolved: str) -> sqlite3.Connection:
    ...
    try:
        ...
        raise UntrackableConnectionError(f'connection to {resolved} uses factory {cls.__name__}, which cannot release its tracking entry on close; byte-probe safety for this database would be silently lost') from exc
...
def read_header_bytes_preopen(path: Path | str, *, length: int=100, force: bool=False) -> Optional[bytes]:
    ...
    with _live_lock:
        if not force and _key(path) in _live_connections:
            logger.debug('refusing byte-level read of %s: a live connection exists in this process and close() would cancel its POSIX locks', path)
            return None
        try:
...
def offline_file_access(path: Path | str, *, what: str='read'):
    ...
    with _live_lock:
        if _key(path) in _live_connections:
            raise LiveConnectionError(f"Refusing to {what} {path}: a connection to it is still open in this process, and raw file access would cancel that connection's POSIX advisory locks. Close all database handles (stop the gateway/dashboard) and retry.")
        yield