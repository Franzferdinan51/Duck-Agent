"""Safe Duck Agent Console command engine.

This module backs ``duck-agent console`` and is intentionally narrower than the
full Duck Agent CLI. It exposes a curated set of native adapters that can later be
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'duck-agent doctor'") and last.startswith("Run 'duck-agent setup'")):
        return text.rstrip()
    lines = lines[:-2]
    while lines and (not _strip_ansi(lines[-1]).strip()):
        lines.pop()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog='duck-agent', add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ''
    if summary.startswith('Run `duck-agent '):
        return ''
    return summary
    ...
    def walk(current: argparse.ArgumentParser, path: tuple[str, ...]) -> None:
        for action in _subparser_actions(current):
            for name, child in action.choices.items():
                ...
                summary = _clean_summary(_choice_help(action, name)) or _clean_summary(child.description)
                if summary:
...
def _extracted_summaries(module_name: str, builder_name: str, main_handler_name: str) -> dict[tuple[str, ...], str]:
    try:
...
@functools.lru_cache(maxsize=None)
def _registered_summaries(root: str, module_name: str, register_name: str) -> dict[tuple[str, ...], str]:
    try:
        parser, subparsers = _parser_root()
        module = importlib.import_module(module_name)
...
@functools.lru_cache(maxsize=None)
def _builder_summaries(module_name: str, builder_name: str) -> dict[tuple[str, ...], str]:
    try:
        parser, subparsers = _parser_root()
        module = importlib.import_module(module_name)
...
def _dispatch_extracted_subcommand(*, root: str, fixed: Sequence[str], args: Sequence[str], module_name: str, builder_name: str, main_handler_name: str, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> str:
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
...
def _dispatch_registered_subcommand(*, root: str, fixed: Sequence[str], args: Sequence[str], module_name: str, register_name: str, handler_name: str | None=None, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> str:
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    top_parser = subparsers.add_parser(root)
    register = getattr(module, register_name)
...
def _dispatch_builder_subcommand(*, root: str, fixed: Sequence[str], args: Sequence[str], module_name: str, builder_name: str, main_handler_name: str, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> str:
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module('hermes_cli.main')
...
def _dispatch_adder_subcommand(*, root: str, fixed: Sequence[str], args: Sequence[str], module_name: str, add_name: str, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> str:
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    getattr(module, add_name)(subparsers)
    namespace = parser.parse_args([root, *fixed, *args])
...
def _extracted_handler(root: str, fixed: Sequence[str], module_name: str, builder_name: str, main_handler_name: str, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> Callable[['HermesConsoleEngine', list[str]], str]:

    def handler(_engine: HermesConsoleEngine, args: list[str]) -> str:
        return _dispatch_extracted_subcommand(root=root, fixed=fixed, args=args, module_name=module_name, builder_name=builder_name, main_handler_name=main_handler_name, namespace_update=namespace_update)
    return handler

def _registered_handler(root: str, fixed: Sequence[str], module_name: str, register_name: str, handler_name: str | None=None, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> Callable[['HermesConsoleEngine', list[str]], str]:

    def handler(_engine: HermesConsoleEngine, args: list[str]) -> str:
        return _dispatch_registered_subcommand(root=root, fixed=fixed, args=args, module_name=module_name, register_name=register_name, handler_name=handler_name, namespace_update=namespace_update)
    return handler

def _builder_handler(root: str, fixed: Sequence[str], module_name: str, builder_name: str, main_handler_name: str, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> Callable[['HermesConsoleEngine', list[str]], str]:

    def handler(_engine: HermesConsoleEngine, args: list[str]) -> str:
        return _dispatch_builder_subcommand(root=root, fixed=fixed, args=args, module_name=module_name, builder_name=builder_name, main_handler_name=main_handler_name, namespace_update=namespace_update)
    return handler

def _adder_handler(root: str, fixed: Sequence[str], module_name: str, add_name: str, namespace_update: Callable[[argparse.Namespace], None] | None=None) -> Callable[['HermesConsoleEngine', list[str]], str]:

    def handler(_engine: HermesConsoleEngine, args: list[str]) -> str:
        return _dispatch_adder_subcommand(root=root, fixed=fixed, args=args, module_name=module_name, add_name=add_name, namespace_update=namespace_update)
    return handler

def _register_command_family(engine: 'HermesConsoleEngine', *, root: str, paths: Iterable[Sequence[str]], handler_factory: Callable[[Sequence[str]], Callable[['HermesConsoleEngine', list[str]], str]], mutating: Iterable[Sequence[str]]=(), summary: str='', summaries: dict[tuple[str, ...], str] | None=None, confirmation: str='') -> None:
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f'Run `duck-agent {usage}`.'
        engine.register(full_path, usage, command_summary, handler_factory(tuple(child_path)), mutating=child_key in mutating_paths, confirmation=confirmation or f'Run `duck-agent {usage}`?')

class HermesConsoleEngine:
    """Curated line-command executor for Duck Agent Console."""
    ...
    def execute(self, line: str, *, confirmed: bool=False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == 'duck-agent':
                tokens = tokens[1:]
            if not tokens:
                return self._help_result()
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError('Duck Agent Console does not run shell syntax. Use one supported Duck Agent command at a time.')
            ...
            if command.mutating and (not confirmed):
                return ConsoleResult('confirm_required', command=raw_line, confirmation_message=command.confirmation or f'Run `{command.usage}`?')
    ...
    def help_text(self, subject: str | None=None) -> str:
        ...
        lines = ['Duck Agent Console', '', 'Supported commands:']
        ...
        lines.extend(['', '* requires confirmation', 'Built-ins: help, help <command>, history, clear, exit, quit'])
        return '\n'.join(lines)

    def _register_defaults(self) -> None:
        self.register(('status',), 'status', 'Show Duck Agent component status.', _status)
        self.register(('doctor',), 'doctor', 'Run diagnostics without auto-fix.', _doctor)
        self.register(('logs',), 'logs [name] [-n N]', 'Show recent Duck Agent logs.', _logs)
        ...
        self.register(('config', 'set'), 'config set <key> <value>', 'Set a configuration value.', _config_set, mutating=True, confirmation='Update Duck Agent configuration?')
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        for root, (module, builder, main_handler, paths, mutating) in extracted.items():
            ...
            _register_command_family(self, root=root, paths=paths, mutating=mutating, summaries=summaries, handler_factory=lambda fixed, root=root, module=module, builder=builder, main_handler=main_handler: _extracted_handler(root, fixed, module, builder, main_handler, namespace_update=_apply_confirmed_defaults))
        self.register(('config', 'migrate'), 'config migrate', 'Update config with new options.', _config_migrate, mutating=True, confirmation='Update Duck Agent configuration with missing defaults?')
        self.register(('sessions', 'export'), 'sessions export <output> [--source SOURCE] [--session-id ID]', 'Export sessions to JSONL.', _sessions_export, mutating=True, confirmation='Export session data?')
        self.register(('sessions', 'rename'), 'sessions rename <session> <title>', 'Rename a session.', _sessions_rename, mutating=True, confirmation='Rename this session?')
        self.register(('sessions', 'optimize'), 'sessions optimize', 'Optimize the session store.', _sessions_optimize, mutating=True, confirmation='Optimize the session database?')
        self.register(('sessions', 'repair'), 'sessions repair [--check-only] [--no-backup]', 'Repair a malformed session database schema.', _sessions_repair, mutating=True, confirmation='Repair the session database?')
        self.register(('profile',), 'profile', 'Show active profile status.', _profile_status)
        self.register(('send',), 'send --to <target> <message>', 'Send a message to a configured platform.', _adder_handler('send', (), 'hermes_cli.send_cmd', 'register_send_subparser'), mutating=True, confirmation='Send this message?')
        portal_paths = [('info',), ('tools',)]
        _register_command_family(self, root='portal', paths=portal_paths, summaries=_adder_summaries('hermes_cli.portal_cli', 'add_parser'), handler_factory=lambda fixed: _adder_handler('portal', fixed, 'hermes_cli.portal_cli', 'add_parser'))
        _register_command_family(self, root='project', paths=[('list',), ('show',), ('create',), ('add-folder',), ('remove-folder',), ('rename',), ('set-primary',), ('use',), ('archive',), ('restore',), ('bind-board',)], summaries=_builder_summaries('hermes_cli.projects_cmd', 'build_parser'), mutating=[('create',), ('add-folder',), ('remove-folder',), ('rename',), ('set-primary',), ('use',), ('archive',), ('restore',), ('bind-board',)], handler_factory=lambda fixed: _builder_handler('project', fixed, 'hermes_cli.projects_cmd', 'build_parser', 'cmd_project'))
        _register_command_family(self, root='kanban', paths=[('init',), ('boards', 'list'), ('boards', 'create'), ('boards', 'rm'), ('boards', 'switch'), ('boards', 'current'), ('boards', 'rename'), ('boards', 'set-workdir'), ('create',), ('list',), ('show',), ('assign',), ('reclaim',), ('reassign',), ('diagnose',), ('link',), ('unlink',), ('claim',), ('comment',), ('complete',), ('edit',), ('block',), ('schedule',), ('unblock',), ('promote',), ('archive',), ('stats',), ('runs',), ('heartbeat',), ('assignments',), ('context',)], summaries=_builder_summaries('hermes_cli.kanban', 'build_parser'), mutating=[('init',), ('boards', 'create'), ('boards', 'rm'), ('boards', 'switch'), ('boards', 'rename'), ('boards', 'set-workdir'), ('create',), ('assign',), ('reclaim',), ('reassign',), ('link',), ('unlink',), ('claim',), ('comment',), ('complete',), ('edit',), ('block',), ('schedule',), ('unblock',), ('promote',), ('archive',)], handler_factory=lambda fixed: _builder_handler('kanban', fixed, 'hermes_cli.kanban', 'build_parser', 'cmd_kanban'))
        registered = {'bundles': ('hermes_cli.bundles', 'register_cli', 'bundles_command', [('list',), ('show',), ('create',), ('delete',), ('reload',)], {('create',), ('delete',), ('reload',)}), 'checkpoints': ('hermes_cli.checkpoints', 'register_cli', None, [('status',), ('list',), ('prune',), ('clear',), ('clear-legacy',)], {('prune',), ('clear',), ('clear-legacy',)}), 'curator': ('hermes_cli.curator', 'register_cli', None, [('status',), ('run',), ('pause',), ('resume',), ('pin',), ('unpin',), ('restore',), ('list-archived',), ('archive',), ('prune',), ('backup',), ('rollback',)], {('run',), ('pause',), ('resume',), ('pin',), ('unpin',), ('restore',), ('archive',), ('prune',), ('backup',), ('rollback',)}), 'pets': ('hermes_cli.pets', 'register_cli', None, [('list',), ('install',), ('select',), ('show',), ('off',), ('scale',), ('remove',), ('doctor',)], {('install',), ('select',), ('off',), ('scale',), ('remove',)})}
        for root, (module, register, handler_name, paths, mutating) in registered.items():
            ...
            _register_command_family(self, root=root, paths=paths, mutating=mutating, summaries=summaries, handler_factory=lambda fixed, root=root, module=module, register=register, handler_name=handler_name: _registered_handler(root, fixed, module, register, handler_name=handler_name, namespace_update=_apply_confirmed_defaults))
    ...
    def register(self, path: Iterable[str], usage: str, summary: str, handler: Callable[['HermesConsoleEngine', list[str]], str], *, mutating: bool=False, confirmation: str='') -> None:
        key = tuple(path)
        self.commands[key] = ConsoleCommand(path=key, usage=usage, summary=summary, handler=handler, mutating=mutating, confirmation=confirmation)
    ...
    def _execute_builtin(self, tokens: list[str]) -> ConsoleResult | None:
        ...
        if head == 'clear':
            return ConsoleResult('clear', output='\x1b[2J\x1b[H')
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f'Unsupported Duck Agent Console command: {probe}.{suffix}')

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith('-'):
            return f'{first} is not available in Duck Agent Console.'
        blocked_top = {'acp', 'chat', 'claw', 'completion', 'dashboard', 'desktop', 'fallback', 'gateway', 'gui', 'login', 'logout', 'model', 'moa', 'oneshot', 'proxy', 'serve', 'setup', 'uninstall', 'update', 'whatsapp', 'whatsapp-cloud'}
        if first in blocked_top:
            return f'`duck-agent {first}` is not available in Duck Agent Console.'
        blocked_pairs = {('config', 'edit'): '`config edit` opens an editor and is not available in Duck Agent Console.', ('mcp', 'serve'): '`mcp serve` starts a server and is not available in Duck Agent Console.', ('profile', 'alias'): '`profile alias` creates shell wrappers and is not available in Duck Agent Console.', ('skills', 'config'): '`skills config` is interactive and is not available in Duck Agent Console.', ('skills', 'publish'): '`skills publish` is not available in Duck Agent Console.', ('portal', 'login'): '`portal login` is interactive and is not available in Duck Agent Console.', ('portal', 'open'): '`portal open` opens a browser and is not available in Duck Agent Console.', ('kanban', 'tail'): '`kanban tail` streams output and is not available in Duck Agent Console.', ('kanban', 'watch'): '`kanban watch` streams output and is not available in Duck Agent Console.', ('kanban', 'daemon'): '`kanban daemon` starts a service and is not available in Duck Agent Console.', ('kanban', 'dispatcher'): '`kanban dispatcher` starts a worker and is not available in Duck Agent Console.', ('kanban', 'swarm'): '`kanban swarm` starts agent work and is not available in Duck Agent Console.', ('kanban', 'decompose'): '`kanban decompose` starts agent work and is not available in Duck Agent Console.', ('kanban', 'specify'): '`kanban specify` starts agent work and is not available in Duck Agent Console.', ('kanban', 'gc'): '`kanban gc` is not available in Duck Agent Console.'}
        if len(tokens) >= 2:
            pair = (tokens[0], tokens[1])
            if pair in blocked_pairs:
                return blocked_pairs[pair]
        if tuple(tokens[:2]) in {('sessions', 'delete'), ('sessions', 'prune')}:
            return '`sessions delete` and `sessions prune` are not available in Duck Agent Console.'
        return ''
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, 'auth_action', None) == 'add':
        ...
        if auth_type in {'api-key', 'api_key'} and (not getattr(args, 'api_key', None)):
            raise ConsoleCommandError('auth add --type api-key requires --api-key in Duck Agent Console.')
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if '-f' in args or '--follow' in args:
        raise ConsoleCommandError('`logs -f` is not available in Duck Agent Console.')
    ...
    return _capture_output(lambda: tail_log(ns.log_name, num_lines=ns.lines, follow=False, level=ns.level, session=ns.session, since=ns.since, component=ns.component))
...
def _sessions_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    db = SessionDB()
    try:
        sessions = db.list_sessions_rich(exclude_sources=['kanban', 'tool'], limit=ns.limit, order_by_last_active=True)
    finally:
        db.close()
...
def _sessions_stats(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        ...
        lines = [f'Total sessions: {total}', f'Listable sessions: {listable}', f'Total messages: {messages}']
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(root='profile', fixed=(), args=(), module_name='hermes_cli.subcommands.profile', builder_name='build_profile_parser', main_handler_name='cmd_profile')
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason='paused from duck-agent console')
...
def run_console_repl(*, stdin=None, stdout=None, stderr=None, interactive: bool | None=None) -> int:
    """Run the local ``duck-agent console`` REPL."""
    ...
    if interactive:
        print('Duck Agent Console. Type `help` for commands, `exit` to quit.', file=stdout)
    while True:
        if interactive:
            print('duck-agent> ', end='', file=stdout, flush=True)
        line = stdin.readline()
        if line == '':
            if interactive:
                print(file=stdout)
