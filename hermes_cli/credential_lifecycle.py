"""Unified provider-credential lifecycle across every store Duck Agent reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.duck-agent/.env``                     — the canonical secret store
    2. ``~/.duck-agent/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.duck-agent/config.yaml``              — inline mirrors written by the
...
__all__ = ['save_provider_env_credential', 'remove_provider_env_credential', 'purge_env_credential_references']
    ...
    with _auth_store_lock():
        ...
        for provider in list(pool.keys()):
            ...
            if not isinstance(entries, list):
                continue
            kept = [entry for entry in entries if not (isinstance(entry, dict) and entry.get('source') == source)]
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``duck-agent auth add``.
    """
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    return {'ok': True, 'key': env_var, 'removed': removed_from_env, 'pool_pruned': refs['pool_pruned'], 'providers': refs['providers'], 'config_scrubbed': config_scrubbed, 'found': bool(removed_from_env or refs['pool_pruned'] or config_scrubbed)}