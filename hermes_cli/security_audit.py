"""On-demand supply-chain audit for Duck Agent installs.

Scans three surfaces a Duck Agent user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Duck Agent venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.duck-agent/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.duck-agent/plugins``.
    ...
    for plugin_dir in sorted(plugins_dir.iterdir()):
        ...
        if pyproject.is_file():
            ...
            for name, version in pins:
                out.append(Component(name=name, version=version, ecosystem='PyPI', source=source))
    return out
_NPX_PKG = re.compile('^(@[A-Za-z0-9._-]+/[A-Za-z0-9._-]+|[A-Za-z0-9._-]+)@([A-Za-z0-9._+-]+)$')
_UVX_PKG = re.compile('^([A-Za-z0-9][A-Za-z0-9._-]*)==([A-Za-z0-9._+!-]+)$')
...
def _extract_mcp_component(server_name: str, command: str, args: list[str]) -> Optional[Component]:
    ...
    if cmd.endswith('npx') or cmd == 'npx':
        for token in args:
            ...
            if m:
                return Component(name=m.group(1), version=m.group(2), ecosystem='npm', source=f'mcp:{server_name}')
            return None
    if cmd.endswith('uvx') or cmd == 'uvx':
        for token in args:
            ...
            if m:
                return Component(name=m.group(1), version=m.group(2), ecosystem='PyPI', source=f'mcp:{server_name}')
            return None
    return None
...
def _osv_fetch_details(vuln_ids: Iterable[str]) -> dict[str, Vulnerability]:
    ...
    def _fetch_one(vid: str) -> Vulnerability:
        ...
        return Vulnerability(osv_id=vid, severity=_osv_severity_from_record(rec), summary=(rec.get('summary') or '').strip(), fixed_versions=_osv_fixed_versions(rec))
...
def _discover_components(*, skip_venv: bool=False, skip_plugins: bool=False, skip_mcp: bool=False, hermes_home: Optional[Path]=None) -> list[Component]:
...
def run_audit(*, skip_venv: bool=False, skip_plugins: bool=False, skip_mcp: bool=False, hermes_home: Optional[Path]=None, components: Optional[list[Component]]=None) -> list[Finding]:
    ...
    if components is None:
        components = _discover_components(skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, hermes_home=hermes_home)
    if not components:
        return []
    ...
    findings.sort(key=lambda f: (-SEVERITY_ORDER.get(f.vuln.severity, 0), f.component.source, f.component.name.lower(), f.vuln.osv_id))
    return findings
...
def _render_human(findings: list[Finding], total_components: int) -> str:
    ...
    lines: list[str] = []
    lines.append(f'Found {len(findings)} known vulnerability finding(s) across {total_components} component(s):')
    lines.append('')
    last_source = None
    for f in findings:
...
def _render_json(findings: list[Finding], total_components: int) -> str:
    payload = {'total_components_scanned': total_components, 'finding_count': len(findings), 'findings': [{'package': f.component.name, 'version': f.component.version, 'ecosystem': f.component.ecosystem, 'source': f.component.source, 'vuln_id': f.vuln.osv_id, 'severity': f.vuln.severity, 'summary': f.vuln.summary, 'fixed_versions': f.vuln.fixed_versions} for f in findings]}
    return json.dumps(payload, indent=2)

def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `duck-agent security audit`."""
    ...
    if fail_on not in SEVERITY_ORDER:
        ...
        return 2
    components = _discover_components(skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, hermes_home=home)
    ...
    if total == 0:
        ...
        return 0
    try:
        findings = run_audit(skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, hermes_home=home, components=components)
