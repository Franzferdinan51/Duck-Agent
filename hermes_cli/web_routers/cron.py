...
from hermes_cli.web_models import CronJobCreate, CronJobUpdate, AutomationBlueprintInstantiate
...
async def cron_fire_webhook(request: Request):
    ...
    cfg = load_config()
    claims = get_fire_verifier()(token=token, expected_audience=cfg_get(cfg, 'cron', 'chronos', 'expected_audience', default=''), jwks_or_key=cfg_get(cfg, 'cron', 'chronos', 'nas_jwks_url', default='') or None, issuer=cfg_get(cfg, 'cron', 'chronos', 'portal_url', default='') or None)
    if claims is None:
