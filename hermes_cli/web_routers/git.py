...
from hermes_cli.web_models import GitPathBody, GitFileBody, GitCommitBody, GitWorktreeAddBody, GitWorktreeRemoveBody, GitBranchSwitchBody
router = APIRouter()
...
async def git_review_diff_route(path: str, file: str, scope: str='uncommitted', base: Optional[str]=None, staged: bool=False):
...
async def git_worktree_add_route(body: GitWorktreeAddBody):
    options = {key: value for key, value in {'name': body.name, 'branch': body.branch, 'base': body.base, 'existingBranch': body.existingBranch}.items() if value}
...
async def git_worktree_remove_route(body: GitWorktreeRemoveBody):
    return await _git_op(_web_git.worktree_remove, _git_path(body.path), _git_path(body.worktreePath), body.force)
