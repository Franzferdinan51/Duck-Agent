...
from hermes_cli.web_models import SkillContentUpdate, SkillCreate, SkillInstallRequest, SkillToggle, SkillUninstallRequest, SkillsUpdateRequest
...
async def install_skill_hub(body: SkillInstallRequest, profile: Optional[str]=None):
    ...
    try:
        proc = _spawn_hermes_action(_profile_cli_args(body.profile or profile) + ['skills', 'install', identifier, '--yes'], name)
...
async def uninstall_skill_hub(body: SkillUninstallRequest, profile: Optional[str]=None):
    ...
    try:
        proc = _spawn_hermes_action(_profile_cli_args(body.profile or profile) + ['skills', 'uninstall', name, '--yes'], action)
    except HTTPException:
        raise
    except Exception as exc:
...
async def update_skills_hub(body: Optional[SkillsUpdateRequest]=None, profile: Optional[str]=None):
    try:
        effective = (body.profile if body else None) or profile
        proc = _spawn_hermes_action(_profile_cli_args(effective) + ['skills', 'update'], 'skills-update')
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_skills_hub_sources(profile: Optional[str]=None):
    ...
    def _run():
        ...
        return {'sources': out, 'index_available': index_available, 'featured': featured, 'installed': _installed_hub_identifiers(profile)}
    try:
...
async def search_skills_hub(q: str='', source: str='all', limit: int=20, profile: Optional[str]=None):
    ...
    def _run():
        ...
        all_results, source_counts, timed_out = parallel_search_sources(sources, query=query, source_filter=source or 'all', overall_timeout=30)
        ...
        return {'results': [_skill_meta_to_payload(m) for m in deduped], 'source_counts': source_counts, 'timed_out': timed_out, 'installed': _installed_hub_identifiers(profile)}
    try:
        return await asyncio.to_thread(_run)
    except HTTPException:
        raise
...
async def preview_skill_hub(identifier: str='', profile: Optional[str]=None):
    ...
    def _run():
        ...
        m = meta or bundle
        return {'name': getattr(m, 'name', ident), 'description': getattr(m, 'description', '') or '', 'source': getattr(m, 'source', '') or '', 'identifier': getattr(m, 'identifier', ident) or ident, 'trust_level': getattr(m, 'trust_level', 'community') or 'community', 'repo': getattr(m, 'repo', None), 'tags': list(getattr(m, 'tags', None) or []), 'skill_md': skill_md, 'files': sorted(files.keys())}
    try:
...
async def scan_skill_hub(identifier: str='', profile: Optional[str]=None):
    ...
    def _run():
        ...
        if allowed is True:
            ...
            policy = 'block'
        findings = [{'severity': f.severity, 'category': f.category, 'file': f.file, 'line': f.line, 'description': f.description} for f in result.findings]
        counts = {'critical': 0, 'high': 0, 'medium': 0, 'low': 0}
        for f in result.findings:
            if f.severity in counts:
                counts[f.severity] += 1
        return {'name': result.skill_name, 'identifier': ident, 'source': result.source, 'trust_level': result.trust_level, 'verdict': result.verdict, 'summary': result.summary, 'policy': policy, 'policy_reason': reason, 'findings': findings, 'severity_counts': counts}
    try:
        result = await asyncio.to_thread(_run)
    except Exception as exc:
...
async def get_skills(profile: Optional[str]=None):
    ...
    for s in skills:
        ...
        s['provenance'] = 'hub' if s['name'] in hub_names else 'bundled' if s['name'] in bundled_names else 'agent'
    return skills
