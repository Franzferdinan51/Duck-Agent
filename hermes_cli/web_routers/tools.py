...
from hermes_cli.web_models import TerminalBackendSelect, ToolsetEnvUpdate, ToolsetModelSelect, ToolsetPostSetup, ToolsetProviderSelect, ToolsetToggle
...
async def get_toolsets(profile: Optional[str]=None):
    from hermes_cli.tools_config import _CONFIG_ONLY_TOOLSETS, _get_effective_configurable_toolsets, _get_platform_tools, _toolset_configuration_platform, _toolset_has_keys, get_nous_subscription_features, gui_toolset_label
    ...
    with _profile_scope(profile):
        ...
        enabled_by_platform = {platform: _get_platform_tools(config, platform, include_default_mcp_servers=False) for platform in target_platforms}
    ...
    for name, label, desc in toolset_rows:
        ...
        result.append({'name': name, 'label': gui_toolset_label(label), 'description': desc, 'platform': target_platform, 'platform_label': gui_toolset_label(platform_label(target_platform, target_platform)), 'enabled': is_enabled, 'available': is_enabled, 'configured': _toolset_has_keys(name, config, features=features), 'tools': tools})
    return result
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str]=None):
    ...
    """
    from hermes_cli.tools_config import _CONFIG_ONLY_TOOLSETS, _get_effective_configurable_toolsets, _get_platform_tools, _save_platform_tools, _toolset_configuration_platform
...
async def get_toolset_config(name: str, profile: Optional[str]=None):
    ...
    Surfaces the same provider rows the CLI ``duck-agent tools`` picker shows
    ...
    """
    from hermes_cli.tools_config import TOOL_CATEGORIES, _get_effective_configurable_toolsets, _is_provider_active, _visible_providers, provider_readiness_status, web_provider_capabilities
    ...
    with _profile_scope(profile):
        ...
        if cat:
            ...
            for prov in _visible_providers(cat, config, force_fresh=True):
                env_vars = [{'key': e['key'], 'prompt': e.get('prompt', e['key']), 'url': e.get('url'), 'default': e.get('default'), 'is_set': bool(get_env_value(e['key']))} for e in prov.get('env_vars', [])]
                ...
                row = {'name': prov['name'], 'badge': prov.get('badge', ''), 'tag': prov.get('tag', ''), 'env_vars': env_vars, 'post_setup': prov.get('post_setup'), 'requires_nous_auth': bool(prov.get('requires_nous_auth')), 'is_active': is_active, 'status': provider_readiness_status(prov, config, features=features, is_active=is_active)}
        ...
        if name == 'web':
            try:
                ...
                active_search_backend = None
                active_extract_backend = None
    payload = {'name': name, 'has_category': cat is not None, 'providers': providers, 'active_provider': active_provider}
    if name == 'web':
...
async def get_toolset_models(name: str, provider: Optional[str]=None, profile: Optional[str]=None):
    """Return the model catalog for a toolset backend (image/video gen).

    The GUI counterpart of the model picker `duck-agent tools` runs after a
    ...
    models = [{'id': model_id, 'display': meta.get('display', model_id), 'speed': meta.get('speed', ''), 'strengths': meta.get('strengths', ''), 'price': meta.get('price', '')} for model_id, meta in catalog.items()]
    return {'name': name, 'has_models': bool(models), 'provider': row.get('name') if row else None, 'plugin': plugin, 'models': models, 'current': current, 'default': default_model}

@router.put('/api/tools/toolsets/{name}/model')
async def select_toolset_model(name: str, body: ToolsetModelSelect, profile: Optional[str]=None):
    ...
    with _profile_scope(body.profile or profile):
        ...
        plugin = _resolve_toolset_model_plugin(name, row) if row else None
        if not plugin:
            raise HTTPException(status_code=400, detail=f'No model-capable backend is active for {name}')
        catalog, _default = _toolset_model_catalog(name, plugin)
        if model_id not in catalog:
            raise HTTPException(status_code=400, detail=f'Unknown model {model_id!r} for backend {plugin!r}')
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect, profile: Optional[str]=None):
    ...
    core extracted from the CLI configurator — so the GUI and ``duck-agent tools``
    ...
    and refetch.
    """
    from hermes_cli.tools_config import TOOL_CATEGORIES, apply_provider_selection, web_provider_capabilities, _get_effective_configurable_toolsets, _visible_providers
    from hermes_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, get_nous_subscription_features
    valid = {ts_key for ts_key, _, _ in _get_effective_configurable_toolsets()}
    if name not in valid:
        raise HTTPException(status_code=400, detail=f'Unknown toolset: {name}')
    if body.capability is not None:
        if name != 'web':
            raise HTTPException(status_code=400, detail='capability selection is only supported for the web toolset')
        if body.capability not in ('search', 'extract'):
            raise HTTPException(status_code=400, detail=f"Unknown capability: {body.capability!r} (expected 'search' or 'extract')")
    with _profile_scope(body.profile or profile):
        config = load_config()
        if body.capability is not None:
            cat = TOOL_CATEGORIES.get(name)
            ...
            if prov is None:
                raise HTTPException(status_code=400, detail=f'Unknown provider {body.provider!r} for toolset {name!r}')
            backend = prov.get('web_backend')
            if not backend:
                raise HTTPException(status_code=400, detail=f'Provider {body.provider!r} has no web backend key')
            if body.capability not in web_provider_capabilities(backend):
                raise HTTPException(status_code=400, detail=f'{body.provider} does not support {body.capability}')
        ...
        row = None
        if cat:
            row = next((p for p in _visible_providers(cat, config, force_fresh=True) if p.get('name') == body.provider), None)
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str]=None):
    ...
    Writes each ``key: value`` to ``~/.duck-agent/.env`` via ``save_env_value`` —
    the same store ``duck-agent tools`` writes when it prompts for keys. Keys are
    ...
    with _profile_scope(body.profile or profile):
        ...
        if unknown:
            raise HTTPException(status_code=400, detail=f"Unknown env var(s) for toolset {name}: {', '.join(sorted(unknown))}")
        saved: List[str] = []
        skipped: List[str] = []
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup, profile: Optional[str]=None):
    ...
    ``duck-agent tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``duck-agent -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same DUCK_AGENT_HOME the rest of the
    ...
    try:
        proc = _spawn_hermes_action(_profile_cli_args(body.profile or profile) + ['tools', 'post-setup', body.key], 'tools-post-setup')
...
async def get_terminal_backends(profile: Optional[str]=None):
    ...
    with _profile_scope(profile):
        ...
        for row in _TERMINAL_BACKENDS:
            ...
            backends.append({'name': row['name'], 'label': row['label'], 'description': row['description'], 'active': row['name'] == active, 'status': status, 'detail': detail})
    return {'active': active, 'backends': backends}

@router.put('/api/tools/terminal/backend')
async def select_terminal_backend(body: TerminalBackendSelect, profile: Optional[str]=None):
    ...
    if backend not in _TERMINAL_BACKEND_NAMES:
        raise HTTPException(status_code=400, detail=f"Unknown terminal backend: {body.backend!r}. Use one of: {', '.join(sorted(_TERMINAL_BACKEND_NAMES))}")
    with _profile_scope(body.profile or profile):
        config = load_config()
...
async def grant_computer_use_permissions(profile: Optional[str]=None):
    """Spawn ``duck-agent computer-use permissions grant`` as a background action.
    ...
    if sys.platform != 'darwin':
        raise HTTPException(status_code=400, detail='Computer Use permission grants are a macOS concept.')
    try:
        proc = _spawn_hermes_action(_profile_cli_args(profile) + ['computer-use', 'permissions', 'grant'], 'computer-use-grant')
    except HTTPException:
        raise
    except Exception as exc:
        _log.exception('Failed to spawn computer-use permissions grant')
        raise HTTPException(status_code=500, detail=f'Failed to request permissions: {exc}')
