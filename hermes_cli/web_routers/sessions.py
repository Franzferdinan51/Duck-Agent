...
from hermes_cli.web_models import BulkDeleteSessions, SessionImport, SessionPrune, SessionRename
...
def get_sessions(limit: int=Query(20, ge=0, le=100), offset: int=Query(0, ge=0), min_messages: int=0, archived: str='exclude', order: str='created', source: str=None, sources: str=None, exclude_sources: str=None, cwd_prefix: str=None, full: bool=False, profile: Optional[str]=None):
    """List sessions.
    ...
    try:
        ...
        try:
            ...
            sessions = db.list_sessions_rich(source=source or None, sources=source_list or None, exclude_sources=exclude_list or None, cwd_prefix=cwd_prefix or None, limit=limit, offset=offset, min_message_count=min_message_count, include_archived=include_archived, archived_only=archived_only, order_by_last_active=order == 'recent', compact_rows=not full, include_pinned=True)
            total = db.session_count(source=source or None, sources=source_list or None, cwd_prefix=cwd_prefix or None, exclude_sources=exclude_list or None, min_message_count=min_message_count, include_archived=include_archived, archived_only=archived_only, exclude_children=True)
            now = time.time()
...
async def search_sessions(q: str='', limit: int=20, profile: Optional[str]=None, source: str=None, sources: str=None, exclude_sources: str=None):
        ...
        try:
            ...
            def compression_root(session_id: str) -> str:
                ...
                while cur and cur not in visited:
                    ...
                    is_compression_edge = parent_session.get('end_reason') == 'compression' and parent_ended_at is not None and (started_at is not None) and (started_at >= parent_ended_at)
            ...
            def add_lineage_result(raw_sid: str, payload: dict) -> None:
                ...
                try:
                    ...
                    row = None
                if row:
                    payload.update({'id': row.get('id') or sid, 'source': row.get('source'), 'model': row.get('model'), 'title': row.get('title'), 'started_at': row.get('started_at'), 'ended_at': row.get('ended_at'), 'last_active': row.get('last_active') or row.get('started_at'), 'is_active': row.get('ended_at') is None and now - (row.get('last_active') or row.get('started_at') or 0) < 300, 'message_count': row.get('message_count') or 0, 'tool_call_count': row.get('tool_call_count') or 0, 'input_tokens': row.get('input_tokens') or 0, 'output_tokens': row.get('output_tokens') or 0, 'preview': row.get('preview'), 'parent_session_id': row.get('parent_session_id'), 'archived': bool(row.get('archived'))})
                else:
                    payload['id'] = sid
                seen[root] = payload
            for row in db.search_sessions_by_id(q, limit=safe_limit, include_archived=True, source=source_filter, sources=source_list or None, exclude_sources=exclude_list or None):
                sid = row.get('id')
                preview = (row.get('preview') or '').strip()
                snippet = preview or f'Session ID: {sid}'
                add_lineage_result(sid, {'snippet': snippet, 'role': None, 'source': row.get('source'), 'model': row.get('model'), 'session_started': row.get('started_at')})
            import re
            terms = []
            for token in re.findall('"[^"]*"|\\S+', q.strip()):
            ...
            matches = db.search_messages(query=prefix_query, source_filter=include_sources, exclude_sources=exclude_list or None, limit=fetch_limit, fields=('session_id', 'role', 'snippet', 'source', 'model', 'session_started'))
            for m in matches:
                if len(seen) >= safe_limit:
                    break
                add_lineage_result(m['session_id'], {'snippet': m.get('snippet', ''), 'role': m.get('role'), 'source': m.get('source'), 'model': m.get('model'), 'session_started': m.get('session_started')})
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Duck Agent backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str]=None):
    """Session-store statistics for the Sessions page (mirrors `duck-agent sessions stats`).
    ...
    try:
        ...
        try:
            by_source = db.session_count_by_source(include_archived=True, exclude_children=True)
        except Exception:
            pass
        return {'total': total, 'active_store': active_store, 'archived': archived, 'messages': messages, 'by_source': by_source}
    finally:
        db.close()
...
async def get_session_detail(session_id: str, profile: Optional[str]=None):
    ...
    try:
        ...
        session['profile'] = _cron_profile_home(profile)[0] if profile else _cron_default_profile()
...
async def get_session_latest_descendant(session_id: str, profile: Optional[str]=None):
    ...
    if not latest:
        raise HTTPException(status_code=404, detail='Session not found')
    return {'requested_session_id': path[0] if path else session_id, 'session_id': latest, 'path': path, 'changed': bool(path and latest != path[0])}

@manage_router.get('/api/sessions/{session_id}/messages')
async def get_session_messages(session_id: str, profile: Optional[str]=None, limit: Optional[int]=Query(None, ge=0), offset: int=Query(0, ge=0)):
    ...
    return {'session_id': sid, 'messages': messages, 'pagination': {'limit': _limit, 'offset': offset, 'returned': len(messages)}}
...
async def rename_session_endpoint(session_id: str, body: SessionRename):
    ...
    try:
        ...
        if body.title is None and body.archived is None and (body.pinned is None):
            raise HTTPException(status_code=400, detail="Nothing to update; provide 'title', 'archived', and/or 'pinned'.")
