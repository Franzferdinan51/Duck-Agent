...
(``~/.duck-agent/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        runtime = None
        try:
            runtime = resolve_runtime_provider(requested=self.requested_provider, explicit_api_key=self._explicit_api_key, explicit_base_url=self._explicit_base_url)
            ...
            if isinstance(_primary_exc, AuthError):
                ...
                for _fb in _fb_chain:
                    ...
                    try:
                        ...
                        logger.warning('Primary provider auth failed (%s). Falling through to fallback: %s/%s', _primary_exc, _fb_provider, _fb_model)
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                logger.debug('No API key for custom endpoint %s (source=%s), using placeholder — local servers typically ignore auth', base_url, _source)
            else:
                ...
                print("   Run 'duck-agent model' to choose a provider, or 'duck-agent setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print('\n⚠️  Provider resolver returned an empty base URL. Check your provider config or run: duck-agent setup')
            return False
        credentials_changed = api_key != self.api_key or base_url != self.base_url
        routing_changed = resolved_provider != self.provider or resolved_api_mode != self.api_mode or resolved_acp_command != self.acp_command or (resolved_acp_args != self.acp_args)
        ...
        if runtime_model and isinstance(runtime_model, str):
            should_use_runtime_model = not self.model or self.model == self.provider or self.model == runtime.get('name')
        ...
        if not self.model and resolved_provider:
            try:
                ...
                if _default:
                    self.model = _default
                    logger.info('No model configured — defaulting to %s for provider %s', _default, resolved_provider)
            except Exception:
                pass
    ...
    def _runtime_credentials_ready(self) -> bool:
        ...
        if isinstance(api_key, str) and api_key:
            return bool(base_url)
        return bool(isinstance(base_url, str) and base_url and ('openrouter.ai' not in base_url))
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``duck-agent model`` (which fronts Quick Setup / Nous
        ...
        _cprint("  You'll pick a provider (Nous Portal OAuth is the fastest; no API key needed) and a model.")
        try:
            ...
            answer = 'n'
        if answer in {'n', 'no'}:
            _cprint("  Skipped. Run 'duck-agent model' or 'duck-agent setup' any time.")
            return False
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'duck-agent model' any time.")
            return False
        except Exception as exc:
            logger.debug('first-run provider setup failed: %s', exc)
            _cprint(f'  ⚠️  Provider setup failed: {exc}')
            _cprint("  Run 'duck-agent model' to try again.")
            return False
        try:
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'duck-agent model' to retry.")
        return False
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        runtime = {'api_key': self.api_key, 'base_url': self.base_url, 'provider': self.provider, 'requested_provider': getattr(self, 'requested_provider', self.provider), 'api_mode': self.api_mode, 'command': self.acp_command, 'args': list(self.acp_args or []), 'credential_pool': getattr(self, '_credential_pool', None)}
        route = {'model': self.model, 'runtime': runtime, 'signature': (self.model, runtime['provider'], runtime['requested_provider'], runtime['base_url'], runtime['api_mode'], runtime['command'], tuple(runtime['args']))}
    ...
    def _init_agent(self, *, model_override: str=None, runtime_override: dict=None, request_overrides: dict | None=None) -> bool:
        ...
        ensure_mcp_discovery_before_agent_build(logger=logger, single_query=getattr(self, '_single_query_mode', False))
        ...
        if self._resumed and self._session_db and (not self.conversation_history):
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print('Use a session ID from a previous CLI run (duck-agent sessions list).', file=sys.stderr)
                else:
                    _cprint(f'\x1b[1;31mSession not found: {self.session_id}{_RST}')
                    _cprint(f'{_DIM}Use a session ID from a previous CLI run (duck-agent sessions list).{_RST}')
                return False
            try:
            ...
            if resolved_id and resolved_id != self.session_id:
                ChatConsole().print(f'[dim]Session {_escape(self.session_id)} was compressed into {_escape(resolved_id)}; resuming the descendant with your transcript.[/dim]')
            ...
            if restored:
                ...
                if _quiet_mode:
                    print(f"↻ Resumed session {self.session_id}{title_part} ({msg_count} user message{('s' if msg_count != 1 else '')}, {len(restored)} total messages)", file=sys.stderr)
                else:
        ...
        try:
            ...
            try:
                ...
                pass
            self._active_agent_route_signature = (effective_model, runtime.get('provider'), runtime.get('requested_provider'), runtime.get('base_url'), runtime.get('api_mode'), runtime.get('command'), tuple(runtime.get('args') or ()))
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print('[dim]Use a session ID from a previous CLI run (duck-agent sessions list).[/]')
            return False
        try:
            resolved_id = self._session_db.resolve_resume_session_id(self.session_id)
        except Exception:
            resolved_id = self.session_id
        if resolved_id and resolved_id != self.session_id:
            self._console_print(f'[dim]Session {self.session_id} was compressed into {resolved_id}; resuming the descendant with your transcript.[/]')
            self.session_id = resolved_id
            resolved_meta = self._session_db.get_session(self.session_id)
            if resolved_meta:
                session_meta = resolved_meta
        ...
        if restored:
            ...
            self._console_print(f"[{accent_color}]↻ Resumed session [bold]{self.session_id}[/bold]{title_part} ({msg_count} user message{('s' if msg_count != 1 else '')}, {len(restored)} total messages)[/]")
            self._restore_session_cwd(session_meta)
            self._restore_session_yolo(session_meta)
        else:
            accent_color = _accent_hex()
            self._console_print(f'[{accent_color}]Session {self.session_id} found but has no messages. Starting fresh.[/]')
            return False
        try:
            self._session_db._conn.execute('UPDATE sessions SET ended_at = NULL, end_reason = NULL WHERE id = ?', (self.session_id,))
            self._session_db._conn.commit()
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == 'event':
                ...
                lines.append('  ◆ Duck Agent: ', style=f'bold {_assistant_label_c}')
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + '\n', style='')
                for ml in msg_lines[1:]:
                    lines.append(f'            {ml}\n', style='')
            else:
                lines.append('  ◆ Duck Agent: ', style=f'dim bold {_assistant_label_c}')
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + '\n', style='dim')
                for ml in msg_lines[1:]:
                    lines.append(f'            {ml}\n', style='dim')
            if i < len(entries) - 1:
                lines.append('')
        panel = Panel(lines, title=f'[dim {_session_label_c}]Previous Conversation[/]', border_style=f'dim {_session_border_c}', padding=(0, 1), style=_history_text_c)
