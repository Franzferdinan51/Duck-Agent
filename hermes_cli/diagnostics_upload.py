"""Client for uploading ``duck-agent debug share`` bundles to Nous-internal S3.

This is the opt-in (``--nous``) destination for ``duck-agent debug share``.
...
_REQUEST_TIMEOUT = 30
_UPLOAD_TIMEOUT = 120
_USER_AGENT = 'duck-agent/debug-share'
...
def request_upload_url(content_type: str='application/gzip', size_bytes: int | None=None) -> dict:
    ...
    req = urllib.request.Request(f'{NAS_BASE}/api/diagnostics/upload-url', data=data, method='POST', headers={'Content-Type': 'application/json', 'Accept': 'application/json', 'User-Agent': _USER_AGENT})
    with urllib.request.urlopen(req, timeout=_REQUEST_TIMEOUT) as resp:
        status = getattr(resp, 'status', None)
        if status is None:
            status = resp.getcode()
        if not 200 <= status < 300:
            raise RuntimeError(f'diagnostics upload-url request failed: HTTP {status}')
...
def put_bundle(upload_url: str, data: bytes, content_type: str='application/gzip') -> None:
    ...
    """
    req = urllib.request.Request(upload_url, data=data, method='PUT', headers={'Content-Type': content_type, 'User-Agent': _USER_AGENT})
...
def share_to_nous(report_bundle: bytes) -> dict:
    ...
    info = request_upload_url(content_type='application/gzip', size_bytes=size_bytes)
