...
from agent.credential_pool import CredentialPool, PooledCredential, credential_pool_matches_provider, get_custom_provider_pool_key, load_pool
from agent.secret_scope import get_secret as _get_secret
from hermes_cli.auth import ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, AuthError, DEFAULT_CODEX_BASE_URL, DEFAULT_QWEN_BASE_URL, DEFAULT_XAI_OAUTH_BASE_URL, PROVIDER_REGISTRY, _agent_key_is_usable, _nous_inference_env_override, format_auth_error, resolve_provider, resolve_nous_runtime_credentials, resolve_codex_runtime_credentials, resolve_xai_oauth_runtime_credentials, resolve_qwen_runtime_credentials, resolve_api_key_provider_credentials, resolve_external_process_provider_credentials, has_usable_secret, is_actual_local_base_url, normalize_actual_base_url
...
def _resolve_runtime_from_pool_entry(*, provider: str, entry: PooledCredential, requested_provider: str, model_cfg: Optional[Dict[str, Any]]=None, pool: Optional[CredentialPool]=None, target_model: Optional[str]=None) -> Dict[str, Any]:
    ...
    return {'provider': provider, 'api_mode': api_mode, 'base_url': base_url, 'api_key': api_key, 'source': getattr(entry, 'source', 'pool'), 'credential_pool': pool, 'requested_provider': requested_provider}
...
def _try_resolve_from_custom_pool(base_url: str, provider_label: str, api_mode_override: Optional[str]=None, provider_name: Optional[str]=None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not pool_api_key:
            return None
        return {'provider': provider_label, 'api_mode': api_mode_override or _detect_api_mode_for_url(base_url) or 'chat_completions', 'base_url': base_url, 'api_key': pool_api_key, 'source': f'pool:{pool_key}', 'credential_pool': pool}
    except Exception:
        return None
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(providers, dict):
        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in custom_provider_aliases(str(display_name or ep_name), str(ep_name)):
                base_url = entry.get('api') or entry.get('url') or entry.get('base_url') or ''
                if base_url:
                    result = {'name': entry.get('name', ep_name), 'base_url': base_url.strip(), 'api_key': resolved_api_key, 'model': entry.get('default_model', '')}
    ...
    if isinstance(custom_providers, dict):
        logger.warning("custom_providers in config.yaml is a dict, not a list. Each entry must be prefixed with '-' in YAML. Run 'duck-agent doctor' for details.")
        return None
    ...
    for entry in custom_providers:
        ...
        if requested_norm not in custom_provider_aliases(name, provider_key):
            continue
        result = {'name': name.strip(), 'base_url': base_url.strip(), 'api_key': str(entry.get('api_key', '') or '').strip()}
        key_env = str(entry.get('key_env', '') or '').strip()
        if key_env:
...
def canonical_custom_identity(*, base_url: Optional[str]=None, config_provider: Optional[str]=None, model: Optional[str]=None) -> Optional[str]:
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str]=None, explicit_base_url: Optional[str]=None) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm == 'custom' and explicit_base_url:
        ...
        api_key_candidates = [(explicit_api_key or '').strip(), _getenv('OPENAI_API_KEY', '').strip() if _da_is_openai_url else '', _getenv('OPENROUTER_API_KEY', '').strip() if _da_is_openrouter else '', _host_derived_api_key(base_url)]
        api_key = next((c for c in api_key_candidates if has_usable_secret(c)), '') or 'no-key-required'
        return {'provider': 'custom', 'api_mode': _detect_api_mode_for_url(base_url) or 'chat_completions', 'base_url': base_url, 'api_key': api_key, 'source': 'direct-alias', 'requested_provider': requested_provider}
    ...
    if pool_result:
        ...
        if request_overrides:
            pool_result['request_overrides'] = {**dict(pool_result.get('request_overrides') or {}), **request_overrides}
    ...
    api_key_candidates = [(explicit_api_key or '').strip(), str(custom_provider.get('api_key', '') or '').strip(), _getenv(str(custom_provider.get('key_env', '') or '').strip(), '').strip(), _getenv('OPENAI_API_KEY', '').strip() if _cp_is_openai_url else '', _getenv('OPENROUTER_API_KEY', '').strip() if _cp_is_openrouter else '', _host_derived_api_key(base_url)]
    api_key = next((candidate for candidate in api_key_candidates if has_usable_secret(candidate)), '')
    result = {'provider': 'custom', 'api_mode': custom_provider.get('api_mode') or _detect_api_mode_for_url(base_url) or 'chat_completions', 'base_url': base_url, 'api_key': api_key or 'no-key-required', 'source': f"custom_provider:{custom_provider.get('name', requested_provider)}"}
...
def _resolve_openrouter_runtime(*, requested_provider: str, explicit_api_key: Optional[str]=None, explicit_base_url: Optional[str]=None) -> Dict[str, Any]:
    model_cfg = _get_model_config()
    ...
    if cfg_base_url.strip() and (not explicit_base_url):
        if requested_norm == 'auto':
            ...
            use_config_base_url = True
    base_url = ((explicit_base_url or '').strip() or env_custom_base_url or (cfg_base_url.strip() if use_config_base_url else '') or env_openrouter_base_url or OPENROUTER_BASE_URL).rstrip('/')
    _is_openrouter_url = base_url_host_matches(base_url, 'openrouter.ai')
    _is_openrouter_context = _is_openrouter_url or (requested_norm == 'openrouter' and (env_openrouter_base_url or base_url == env_openrouter_base_url) and (base_url == (env_openrouter_base_url or '').rstrip('/')))
    if _is_openrouter_context:
        api_key_candidates = [explicit_api_key, _getenv('OPENROUTER_API_KEY'), _getenv('OPENAI_API_KEY')]
    else:
        ...
        api_key_candidates = [explicit_api_key, cfg_api_key if use_config_base_url else '', _getenv('OLLAMA_API_KEY') if _is_ollama_url else '', _getenv('OPENAI_API_KEY') if _is_openai_url or _is_openai_azure else '', _getenv('OPENROUTER_API_KEY') if _is_openrouter_url else '', _host_derived_api_key(base_url)]
    ...
    if effective_provider == 'custom' and base_url:
        pool_result = _try_resolve_from_custom_pool(base_url, effective_provider, _parse_api_mode(model_cfg.get('api_mode')), provider_name=requested_provider if requested_norm != 'custom' else None)
        if pool_result:
            return pool_result
    if effective_provider == 'custom' and (not api_key) and (not _is_openrouter_url):
        api_key = 'no-key-required'
    return {'provider': effective_provider, 'api_mode': _resolve_plain_custom_api_mode(model_cfg, base_url) if effective_provider == 'custom' else _parse_api_mode(model_cfg.get('api_mode')) or _detect_api_mode_for_url(base_url) or 'chat_completions', 'base_url': base_url, 'api_key': api_key, 'source': source}

def _resolve_azure_foundry_runtime(*, requested_provider: str, model_cfg: Dict[str, Any], explicit_api_key: Optional[str]=None, explicit_base_url: Optional[str]=None, target_model: Optional[str]=None) -> Dict[str, Any]:
    ...
    if not base_url:
        raise AuthError("Azure Foundry requires a base URL. Set it via 'duck-agent model' or the AZURE_FOUNDRY_BASE_URL environment variable.")
    ...
    if cfg_auth_mode == 'entra_id':
        if explicit_api_key:
            ...
            try:
                from agent.azure_identity_adapter import EntraIdentityConfig, SCOPE_AI_AZURE_DEFAULT, build_token_provider
            except Exception as exc:
                raise AuthError(f"Azure Foundry Entra ID auth requires the 'azure-identity' package. Install it with: pip install azure-identity (import failed: {exc})") from exc
            scope = str(cfg_entra.get('scope') or '').strip() or SCOPE_AI_AZURE_DEFAULT
            try:
                entra_config = EntraIdentityConfig(scope=scope)
        ...
        return {'provider': 'azure-foundry', 'api_mode': cfg_api_mode, 'base_url': base_url, 'api_key': api_key, 'auth_mode': auth_mode, 'entra': clean_entra, 'source': source, 'requested_provider': requested_provider}
    ...
    if not api_key:
        raise AuthError("Azure Foundry requires an API key. Set AZURE_FOUNDRY_API_KEY in ~/.duck-agent/.env or run 'duck-agent model' to configure. To use keyless Microsoft Entra ID auth instead, set model.auth_mode: entra_id in config.yaml (or pick 'Microsoft Entra ID' in 'duck-agent model').")
    source = 'explicit' if explicit_api_key or explicit_base_url else 'config'
    return {'provider': 'azure-foundry', 'api_mode': cfg_api_mode, 'base_url': base_url, 'api_key': api_key, 'auth_mode': 'api_key', 'source': source, 'requested_provider': requested_provider}

def _resolve_explicit_runtime(*, provider: str, requested_provider: str, model_cfg: Dict[str, Any], explicit_api_key: Optional[str]=None, explicit_base_url: Optional[str]=None, target_model: Optional[str]=None) -> Optional[Dict[str, Any]]:
    explicit_api_key = str(explicit_api_key or '').strip()
    ...
    if provider == 'anthropic':
        ...
        if not api_key:
            ...
            if not api_key:
                raise AuthError("No Anthropic credentials found. Set ANTHROPIC_TOKEN or ANTHROPIC_API_KEY, run 'claude setup-token', or authenticate with 'claude /login'.")
        return {'provider': 'anthropic', 'api_mode': 'anthropic_messages', 'base_url': base_url, 'api_key': api_key, 'source': 'explicit', 'requested_provider': requested_provider}
    if provider == 'openai-codex':
        ...
        return {'provider': 'openai-codex', 'api_mode': 'codex_responses', 'base_url': base_url, 'api_key': api_key, 'source': 'explicit', 'last_refresh': last_refresh, 'requested_provider': requested_provider}
    if provider == 'nous':
        from hermes_cli.providers import nous_api_mode
        state = auth_mod.get_provider_auth_state('nous') or {}
        base_url = explicit_base_url or _nous_inference_base_url_override() or str(state.get('inference_base_url') or auth_mod.DEFAULT_NOUS_INFERENCE_URL).strip().rstrip('/')
        api_key = explicit_api_key or (str(state.get('agent_key') or '').strip() if _agent_key_is_usable(state, max(60, env_int('HERMES_NOUS_MIN_KEY_TTL_SECONDS', 1800))) else '')
        expires_at = state.get('agent_key_expires_at') or state.get('expires_at')
        if not api_key:
            creds = resolve_nous_runtime_credentials(timeout_seconds=float(_getenv('HERMES_NOUS_TIMEOUT_SECONDS', '15')))
            api_key = creds.get('api_key', '')
            expires_at = creds.get('expires_at')
            if not explicit_base_url:
                base_url = creds.get('base_url', '').rstrip('/') or base_url
        return {'provider': 'nous', 'api_mode': nous_api_mode(target_model or model_cfg.get('default') or ''), 'base_url': base_url, 'api_key': api_key, 'source': 'explicit', 'expires_at': expires_at, 'requested_provider': requested_provider}
    if provider == 'azure-foundry':
        return _resolve_azure_foundry_runtime(requested_provider=requested_provider, model_cfg=model_cfg, explicit_api_key=explicit_api_key, explicit_base_url=explicit_base_url)
    pconfig = PROVIDER_REGISTRY.get(provider)
    if pconfig and pconfig.auth_type == 'api_key':
        ...
        return {'provider': provider, 'api_mode': api_mode, 'base_url': base_url.rstrip('/'), 'api_key': api_key, 'source': 'explicit', 'requested_provider': requested_provider}
    return None
...
def resolve_runtime_provider(*, requested: Optional[str]=None, explicit_api_key: Optional[str]=None, explicit_base_url: Optional[str]=None, target_model: Optional[str]=None) -> Dict[str, Any]:
    ...
    if isinstance(_provs_cfg, dict):
        ...
        if isinstance(_block, dict) and (not is_provider_enabled(_block)):
            raise ValueError(f'provider {requested_provider!r} is disabled in config (providers.{requested_provider}.enabled: false)')
    if requested_provider == 'moa':
        return {'provider': 'moa', 'api_mode': 'chat_completions', 'base_url': 'moa://local', 'api_key': 'moa-virtual-provider', 'source': 'moa-virtual-provider', 'requested_provider': requested_provider}
    _eff_base = (explicit_base_url or '').strip()
    if requested_provider == 'anthropic' and 'azure.com' in _eff_base:
        _azure_key = (explicit_api_key or '').strip() or _getenv('AZURE_ANTHROPIC_KEY', '').strip() or _getenv('ANTHROPIC_API_KEY', '').strip()
        return {'provider': 'anthropic', 'api_mode': 'anthropic_messages', 'base_url': _eff_base.rstrip('/'), 'api_key': _azure_key, 'source': 'azure-explicit', 'requested_provider': requested_provider}
    if requested_provider == 'azure-foundry':
        azure_runtime = _resolve_azure_foundry_runtime(requested_provider=requested_provider, model_cfg=_get_model_config(), explicit_api_key=explicit_api_key, explicit_base_url=explicit_base_url, target_model=target_model)
        return azure_runtime
    if requested_provider in ('vertex', 'google-vertex', 'vertex-ai', 'gcp-vertex', 'vertexai'):
        from agent.vertex_adapter import get_vertex_config
        token, base_url = get_vertex_config()
        if not token or not base_url:
            raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.duck-agent/.env, or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in config.yaml if they aren't embedded in the credentials. Run `duck-agent setup` to install Vertex support.")
        return {'provider': 'vertex', 'api_mode': 'chat_completions', 'base_url': base_url.rstrip('/'), 'api_key': token, 'source': 'vertex-oauth', 'requested_provider': requested_provider}
    custom_runtime = _resolve_named_custom_runtime(requested_provider=requested_provider, explicit_api_key=explicit_api_key, explicit_base_url=explicit_base_url)
    if custom_runtime:
    ...
    if not explicit_base_url and (not explicit_api_key):
        ...
        if cfg_base_url and cfg_provider in ('auto', ''):
            _known_cloud_hosts = ('openrouter.ai', 'anthropic.com', 'openai.com')
            if not any((base_url_host_matches(cfg_base_url, host) for host in _known_cloud_hosts)):
                runtime = _resolve_openrouter_runtime(requested_provider=requested_provider, explicit_api_key=explicit_api_key, explicit_base_url=explicit_base_url)
    ...
    model_cfg = _get_model_config()
    explicit_runtime = _resolve_explicit_runtime(provider=provider, requested_provider=requested_provider, model_cfg=model_cfg, explicit_api_key=explicit_api_key, explicit_base_url=explicit_base_url, target_model=target_model)
    if explicit_runtime:
    ...
    if provider == 'openrouter':
        ...
        env_openrouter_base_url = _getenv('OPENROUTER_BASE_URL', '').strip()
        has_custom_endpoint = bool(explicit_base_url or env_openai_base_url or env_openrouter_base_url)
        ...
        should_use_pool = requested_provider in {'openrouter', 'auto'} and (not has_custom_endpoint) and (not has_runtime_override)
    try:
    ...
    if pool and pool.has_credentials():
        ...
        if provider == 'nous' and entry is not None:
            ...
            nous_state = {'agent_key': getattr(entry, 'agent_key', None), 'agent_key_expires_at': getattr(entry, 'agent_key_expires_at', None), 'scope': getattr(entry, 'scope', None)}
            if not _agent_key_is_usable(nous_state, min_ttl):
                ...
                if not pool_api_key or not _agent_key_is_usable(nous_state, min_ttl):
                    ...
                    pool_api_key = ''
        if entry is not None and pool_api_key and credential_pool_matches_provider(pool, provider, base_url=getattr(entry, 'runtime_base_url', None) or getattr(entry, 'base_url', None) or ''):
            return _resolve_runtime_from_pool_entry(provider=provider, entry=entry, requested_provider=requested_provider, model_cfg=model_cfg, pool=pool, target_model=target_model)
    if provider == 'nous':
        try:
            from hermes_cli.providers import nous_api_mode
            creds = resolve_nous_runtime_credentials(timeout_seconds=float(_getenv('HERMES_NOUS_TIMEOUT_SECONDS', '15')))
            return {'provider': 'nous', 'api_mode': nous_api_mode(target_model or model_cfg.get('default') or ''), 'base_url': creds.get('base_url', '').rstrip('/'), 'api_key': creds.get('api_key', ''), 'source': creds.get('source', 'portal'), 'expires_at': creds.get('expires_at'), 'requested_provider': requested_provider}
        except AuthError:
            if requested_provider != 'auto':
                raise
            logger.info('Auto-detected Nous provider but credentials failed; falling through to next provider.')
    if provider == 'openai-codex':
        try:
            creds = resolve_codex_runtime_credentials()
            return {'provider': 'openai-codex', 'api_mode': 'codex_responses', 'base_url': creds.get('base_url', '').rstrip('/'), 'api_key': creds.get('api_key', ''), 'source': creds.get('source', 'duck-agent-auth-store'), 'last_refresh': creds.get('last_refresh'), 'requested_provider': requested_provider}
        except AuthError:
            if requested_provider != 'auto':
                raise
            logger.info('Auto-detected Codex provider but credentials failed; falling through to next provider.')
    if provider == 'xai-oauth':
        try:
            creds = resolve_xai_oauth_runtime_credentials()
            return {'provider': 'xai-oauth', 'api_mode': 'codex_responses', 'base_url': (creds.get('base_url') or '').rstrip('/') or DEFAULT_XAI_OAUTH_BASE_URL, 'api_key': creds.get('api_key', ''), 'source': creds.get('source', 'duck-agent-auth-store'), 'last_refresh': creds.get('last_refresh'), 'requested_provider': requested_provider}
        except AuthError:
            if requested_provider != 'auto':
                raise
            logger.info('Auto-detected xAI OAuth provider but credentials failed; falling through to next provider.')
    if provider == 'qwen-oauth':
        try:
            creds = resolve_qwen_runtime_credentials()
            return {'provider': 'qwen-oauth', 'api_mode': 'chat_completions', 'base_url': creds.get('base_url', '').rstrip('/'), 'api_key': creds.get('api_key', ''), 'source': creds.get('source', 'qwen-cli'), 'expires_at_ms': creds.get('expires_at_ms'), 'requested_provider': requested_provider}
        except AuthError:
            if requested_provider != 'auto':
                raise
    ...
    if provider == 'minimax-oauth':
        ...
        if pconfig and pconfig.auth_type == 'oauth_minimax':
            ...
            return {'provider': provider, 'api_mode': 'anthropic_messages', 'base_url': creds['base_url'], 'api_key': creds['api_key'], 'source': creds.get('source', 'oauth'), 'requested_provider': requested_provider}
    if provider == 'copilot-acp':
        creds = resolve_external_process_provider_credentials(provider)
        return {'provider': 'copilot-acp', 'api_mode': 'chat_completions', 'base_url': creds.get('base_url', '').rstrip('/'), 'api_key': creds.get('api_key', ''), 'command': creds.get('command', ''), 'args': list(creds.get('args') or []), 'source': creds.get('source', 'process'), 'requested_provider': requested_provider}
    if provider == 'anthropic':
        cfg_provider = str(model_cfg.get('provider') or '').strip().lower()
        cfg_base_url = ''
        if cfg_provider == 'anthropic':
        ...
        _is_azure_endpoint = 'azure.com' in base_url.lower() or (cfg_base_url and 'azure.com' in cfg_base_url.lower())
        if _is_azure_endpoint:
            ...
            if not token:
                token = _getenv('AZURE_ANTHROPIC_KEY', '').strip() or _getenv('ANTHROPIC_API_KEY', '').strip()
            if not token:
                raise AuthError('No Azure Anthropic API key found. Set AZURE_ANTHROPIC_KEY or ANTHROPIC_API_KEY, or point key_env/api_key_env in your config.yaml model section at a custom env var.')
        else:
            from agent.anthropic_adapter import resolve_anthropic_token
            token = resolve_anthropic_token()
            if not token:
                raise AuthError("No Anthropic credentials found. Set ANTHROPIC_TOKEN or ANTHROPIC_API_KEY, run 'claude setup-token', or authenticate with 'claude /login'.")
        return {'provider': 'anthropic', 'api_mode': 'anthropic_messages', 'base_url': base_url, 'api_key': token, 'source': 'env', 'requested_provider': requested_provider}
    if provider == 'bedrock':
        from agent.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region, is_anthropic_bedrock_model
        ...
        if not is_explicit and (not has_aws_credentials()):
            raise AuthError("No AWS credentials found for Bedrock. Configure one of:\n  - AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY\n  - AWS_PROFILE (for SSO / named profiles)\n  - IAM instance role (EC2, ECS, Lambda)\nOr run 'aws configure' to set up credentials.", code='no_aws_credentials')
        ...
        if _gr.get('guardrail_identifier') and _gr.get('guardrail_version'):
            guardrail_config = {'guardrailIdentifier': _gr['guardrail_identifier'], 'guardrailVersion': _gr['guardrail_version']}
        ...
        if is_anthropic_bedrock_model(_current_model) and (not _has_bearer_token):
            runtime = {'provider': 'bedrock', 'api_mode': 'anthropic_messages', 'base_url': f'https://bedrock-runtime.{region}.amazonaws.com', 'api_key': 'aws-sdk', 'source': auth_source, 'region': region, 'bedrock_anthropic': True, 'requested_provider': requested_provider}
        else:
            runtime = {'provider': 'bedrock', 'api_mode': 'bedrock_converse', 'base_url': f'https://bedrock-runtime.{region}.amazonaws.com', 'api_key': 'aws-sdk', 'source': auth_source, 'region': region, 'requested_provider': requested_provider}
        if guardrail_config:
    ...
    if pconfig and pconfig.auth_type == 'api_key':
        ...
        if provider == 'actual' and (not has_usable_secret(creds.get('api_key'))):
            ...
            _effective_url = normalize_actual_base_url(_cfg_url or creds.get('base_url', '').rstrip('/'))
        ...
        if provider == 'actual' and (not api_key) and is_actual_local_base_url(base_url):
            api_key = ACTUAL_LOCAL_NOAUTH_PLACEHOLDER
        return {'provider': provider, 'api_mode': api_mode, 'base_url': base_url, 'api_key': api_key, 'source': creds.get('source', 'env'), 'requested_provider': requested_provider}
    runtime = _resolve_openrouter_runtime(requested_provider=requested_provider, explicit_api_key=explicit_api_key, explicit_base_url=explicit_base_url)
    runtime['requested_provider'] = requested_provider
    return runtime
