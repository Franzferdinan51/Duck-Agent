"""
Dump command for duck-agent CLI.

Outputs a compact, plain-text summary of the user's Duck Agent setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.duck-agent/.env.
    ...
    exports. ``duck-agent debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    """
    try:
        result = subprocess.run(['git', 'rev-parse', '--short=8', 'HEAD'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5, cwd=str(project_root))
...
def _get_git_commit_date(project_root: Path) -> str:
    ...
    """
    try:
        result = subprocess.run(['git', 'log', '-1', '--format=%cd', '--date=short', 'HEAD'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5, cwd=str(project_root))
        if result.returncode == 0:
            value = result.stdout.strip()
            if value:
                return value
...
def _redact(value: str) -> str:
    ...
    ``duck-agent dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _configured_platforms() -> list[str]:
    ...
    checks = {'telegram': 'TELEGRAM_BOT_TOKEN', 'discord': 'DISCORD_BOT_TOKEN', 'slack': 'SLACK_BOT_TOKEN', 'whatsapp': 'WHATSAPP_ENABLED', 'signal': 'SIGNAL_HTTP_URL', 'email': 'EMAIL_ADDRESS', 'sms': 'TWILIO_ACCOUNT_SID', 'matrix': 'MATRIX_HOMESERVER_URL', 'mattermost': 'MATTERMOST_URL', 'homeassistant': 'HASS_TOKEN', 'dingtalk': 'DINGTALK_CLIENT_ID', 'feishu': 'FEISHU_APP_ID', 'wecom': 'WECOM_BOT_ID', 'wecom_callback': 'WECOM_CALLBACK_CORP_ID', 'weixin': 'WEIXIN_ACCOUNT_ID', 'qqbot': 'QQ_APP_ID'}
...
def _config_overrides(config: dict) -> dict[str, str]:
    ...
    overrides = {}
    interesting_paths = [('agent', 'max_turns'), ('agent', 'gateway_timeout'), ('agent', 'session_stall_timeout'), ('agent', 'tool_use_enforcement'), ('terminal', 'backend'), ('terminal', 'docker_image'), ('terminal', 'persistent_shell'), ('browser', 'allow_private_urls'), ('compression', 'enabled'), ('compression', 'threshold'), ('compression', 'in_place'), ('display', 'streaming'), ('display', 'skin'), ('display', 'show_reasoning'), ('privacy', 'redact_pii'), ('tts', 'provider')]
...
def run_dump(args):
    ...
    env_path = get_env_path()
    load_hermes_dotenv(hermes_home=env_path.parent, project_env=get_project_root() / '.env')
    ...
    if env_backend and env_backend != str(config_backend).strip().lower():
        backend = f'{env_backend}  (TERMINAL_ENV overrides config.yaml terminal.backend={config_backend})'
    else:
    ...
    toolsets = config.get('toolsets', ['duck-agent-cli'])
