...
from hermes_cli.config import cfg_get, DEFAULT_CONFIG, OPTIONAL_ENV_VARS, clear_model_endpoint_credentials, get_config_path, get_env_path, get_hermes_home, get_process_hermes_home, load_config, load_env, read_raw_config, save_config, save_env_value, remove_env_value, custom_endpoint_key_env, check_config_version, detect_install_method, format_docker_update_message, recommended_update_command_for_method, redact_key, write_platform_config_field, _deep_merge
from plugins.memory.config_schema import ProviderConfigSchema, ProviderField, STORAGE_HONCHO_HOST_BLOCK, get_provider_config_schema
from gateway.status import derive_gateway_busy, derive_gateway_drainable, get_running_pid_cached, get_running_pid, get_runtime_status_running_pid, normalize_updated_at, parse_active_agents, read_runtime_status, resolve_gateway_liveness
...
try:
    ...
    try:
        ...
        raise SystemExit(f"Web UI requires fastapi and uvicorn.\nInstall with: {sys.executable} -m pip install 'fastapi' 'uvicorn[standard]'")
...
def _start_desktop_cron_ticker(stop_event: 'threading.Event', interval: int=60) -> None:
    ...
    The scheduler tick loop normally lives in ``duck-agent gateway run`` — but the
    desktop app spawns a ``duck-agent dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same DUCK_AGENT_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    """
    for mod in ('hermes_cli.gateway', 'hermes_cli.auth', 'hermes_cli.copilot_auth', 'hermes_cli.runtime_provider', 'hermes_cli.skin_engine', 'hermes_cli.inventory', 'hermes_cli.model_switch'):
        try:
            __import__(mod)
        except Exception:
            pass
...
def _get_pty_active_session_files(app: 'FastAPI') -> dict[str, Path]:
    ...
    try:
        ...
        app.state.pty_active_session_files = {}
        return app.state.pty_active_session_files
app = FastAPI(title='Duck Agent', version=__version__, lifespan=_lifespan)
...
app.add_middleware(CORSMiddleware, allow_origin_regex='^https?://(localhost|127\\.0\\.0\\.1)(:\\d+)?$', allow_methods=['*'], allow_headers=['*'])
...
def should_require_auth(host: str, allow_public: bool=False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``duck-agent-0day``
...
async def host_header_middleware(request: Request, call_next):
    ...
    if bound_host:
        ...
        if not _is_accepted_host(host_header, bound_host):
            return JSONResponse(status_code=400, content={'detail': 'Invalid Host header. Dashboard requests must use the hostname the server was bound to.'})
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if path.startswith('/api/plugins/'):
        _authed = getattr(request.state, 'token_authenticated', False) or getattr(request.app.state, 'auth_required', False) or _has_valid_session_token(request) or _has_valid_query_token(request, path)
        if _authed:
            parts = path.split('/')
            if len(parts) >= 4:
                plugin_name = parts[3]
...
class DashboardHealth:
    ...
    def __init__(self, window_seconds: float=_DASHBOARD_HEALTH_WINDOW_SECONDS) -> None:
        ...
        self.selftest_status: str = 'unknown'
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        return {'status': status, 'recent_unhandled_errors': errors, 'last_error_at': self.last_error_at, 'selftest': self.selftest_status}
...
def _build_schema_from_config(config: Dict[str, Any], prefix: str='') -> Dict[str, Dict[str, Any]]:
    ...
    for key, value in config.items():
        ...
        if isinstance(value, dict):
            ...
            entry: Dict[str, Any] = {'type': _infer_type(value), 'description': full_key.replace('.', ' → ').replace('_', ' ').title(), 'category': category}
    ...
    *kind* is ``"tts"`` or ``"stt"``. The result keeps the built-in display
    ...
    for block in candidate_blocks:
        for name, value in block.items():
            if isinstance(name, str) and name.strip().lower() not in _runtime_builtins and _is_command_provider_block(value):
                _add(name)
    try:
        if kind == 'tts':
...
from hermes_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, MemoryProviderConfigUpdate, MemoryProviderSetupRequest, CustomEndpointUpdate, MessagingPlatformUpdate, TelegramOnboardingStart, TelegramOnboardingApply, WhatsAppOnboardingStart, WhatsAppOnboardingApply, AudioTranscriptionRequest, ManagedFileUpload, ChatImageUpload, ManagedDirectoryCreate, ManagedFileDelete, ModelAssignment, MoaModelSlot, _MoaReferenceControls, MoaPresetPayload, MoaConfigPayload, FsWriteText, GitPathBody, GitFileBody, GitCommitBody, GitWorktreeAddBody, GitWorktreeRemoveBody, GitBranchSwitchBody, CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest, TTSSpeakRequest, OAuthSubmitBody, BulkDeleteSessions, SessionImport, SessionRename, SessionPrune, CronJobCreate, CronJobUpdate, AutomationBlueprintInstantiate, MCPServerCreate, MCPServersReplace, MCPEnabledToggle, MCPCatalogInstall, PairingApprove, PairingRevoke, WebhookCreate, WebhookEnabledToggle, CredentialPoolAdd, MemoryProviderSelect, MemoryReset, BackupRequest, ImportRequest, HookCreate, HookDelete, SkillInstallRequest, SkillUninstallRequest, SkillsUpdateRequest, ProfileCreate, ProfileRename, ProfileSoulUpdate, ProfileActiveUpdate, ProfileDescriptionUpdate, ProfileModelUpdate, ProfileDescribeAuto, SkillToggle, SkillCreate, SkillContentUpdate, ToolsetToggle, ToolsetProviderSelect, ToolsetModelSelect, ToolsetEnvUpdate, ToolsetPostSetup, TerminalBackendSelect, RawConfigUpdate, ThemeSetBody, FontSetBody, _AgentPluginInstallBody, _PluginProvidersPutBody, _PluginVisibilityBody
_AUDIO_MIME_EXTENSIONS: Dict[str, str] = {'audio/aac': '.aac', 'audio/flac': '.flac', 'audio/m4a': '.m4a', 'audio/mp3': '.mp3', 'audio/mp4': '.mp4', 'audio/mpeg': '.mp3', 'audio/ogg': '.ogg', 'audio/wav': '.wav', 'audio/wave': '.wav', 'audio/webm': '.webm', 'audio/x-m4a': '.m4a', 'audio/x-wav': '.wav', 'video/webm': '.webm'}
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Duck Agent provider slug — fine.
    ...
    1. Vendor-name → Duck Agent-provider mapping: when the provider string is
       not a known Duck Agent provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    custom_provider = resolve_custom_provider(prov_in, get_compatible_custom_providers(cfg) if isinstance(cfg, dict) else [])
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and (not is_custom_provider_slug) and ('/' in model_in):
        try:
            ...
            cur_provider = str(cur_cfg.get('provider', '') or '').strip().lower() if isinstance(cur_cfg, dict) else ''
        except Exception:
            cur_provider = ''
...
def _apply_main_model_assignment(model_cfg: 'Any', provider: str, model: str, base_url: str='', api_key: str='') -> dict:
...
try:
    ...
    _log.warning('Invalid GATEWAY_HEALTH_TIMEOUT value %r — using default 1.0s', os.getenv('GATEWAY_HEALTH_TIMEOUT'))
    _GATEWAY_HEALTH_TIMEOUT = 1.0
if _GATEWAY_HEALTH_TIMEOUT <= 0:
    _log.warning('Invalid non-positive GATEWAY_HEALTH_TIMEOUT value %.3fs — using default 1.0s', _GATEWAY_HEALTH_TIMEOUT)
    _GATEWAY_HEALTH_TIMEOUT = 1.0
elif _GATEWAY_HEALTH_TIMEOUT > _GATEWAY_HEALTH_TIMEOUT_MAX:
    _log.warning('Capping GATEWAY_HEALTH_TIMEOUT %.3fs to %.3fs for dashboard liveness probes', _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_TIMEOUT_MAX)
...
def _count_status_active_sessions() -> int:
    ...
    Duck Agent process is using it.
    """
    ...
    try:
        ...
        now = time.time()
        return sum((1 for s in sessions if s.get('ended_at') is None and now - s.get('last_active', s.get('started_at', 0)) < 300))
    finally:
        db.close()
...
async def _status_active_sessions() -> int:
    ...
    try:
        return await asyncio.wait_for(loop.run_in_executor(None, _count_status_active_sessions), timeout=_STATUS_ACTIVE_SESSIONS_TIMEOUT)
    except asyncio.TimeoutError:
        _log.debug('/api/status active session count exceeded %.2fs; returning 0', _STATUS_ACTIVE_SESSIONS_TIMEOUT)
    except Exception as exc:
        _log.debug('/api/status active session count unavailable: %s', exc)
    return 0
_MEDIA_CONTENT_TYPES = {'.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp', '.svg': 'image/svg+xml', '.bmp': 'image/bmp', '.ico': 'image/x-icon'}
...
_FS_READDIR_HIDDEN = {'.git', '.hg', '.svn', '.cache', '.next', '.turbo', '.venv', '__pycache__', 'build', 'dist', 'node_modules', 'target', 'venv'}
_SENSITIVE_MANAGED_FILE_BASENAMES = frozenset({'auth.json', 'auth.lock', 'credentials', 'config.yaml', '.anthropic_oauth.json', 'google_token.json', 'google_oauth_pending.json', 'google_oauth.json', 'webhook_subscriptions.json', 'bws_cache.json', 'bws_cache.enc.json', '.git-credentials'})
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Duck Agent credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no DUCK_AGENT_HOME resolution, so it blocks these trees wherever
...
_FS_PREVIEW_LANGUAGE_BY_EXT = {'.c': 'c', '.conf': 'ini', '.cpp': 'cpp', '.css': 'css', '.csv': 'csv', '.go': 'go', '.graphql': 'graphql', '.h': 'c', '.hpp': 'cpp', '.html': 'html', '.java': 'java', '.js': 'javascript', '.json': 'json', '.jsx': 'jsx', '.kt': 'kotlin', '.lua': 'lua', '.md': 'markdown', '.mjs': 'javascript', '.py': 'python', '.rb': 'ruby', '.rs': 'rust', '.sh': 'shell', '.sql': 'sql', '.svg': 'xml', '.toml': 'toml', '.ts': 'typescript', '.tsx': 'tsx', '.txt': 'text', '.xml': 'xml', '.yaml': 'yaml', '.yml': 'yaml', '.zsh': 'shell'}
_FS_MIME_TYPES = {'.avi': 'video/x-msvideo', '.bmp': 'image/bmp', '.flac': 'audio/flac', '.gif': 'image/gif', '.jpeg': 'image/jpeg', '.jpg': 'image/jpeg', '.m4a': 'audio/mp4', '.mkv': 'video/x-matroska', '.mov': 'video/quicktime', '.mp3': 'audio/mpeg', '.mp4': 'video/mp4', '.ogg': 'audio/ogg', '.opus': 'audio/ogg; codecs=opus', '.png': 'image/png', '.svg': 'image/svg+xml', '.wav': 'audio/wav', '.webm': 'video/webm', '.webp': 'image/webp'}
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get('DUCK_AGENT_HOME', '').strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``duck-agent update``.
    ...
    a container — e.g. the duck-agent-webui image sharing the Duck Agent source tree),
    the dashboard's ``duck-agent update`` button is the correct update path and
...
def _resolve_managed_path(raw_path: str | None, request: Request, *, for_write: bool=False) -> tuple[ManagedFilesPolicy, Path, str]:
...
def _managed_file_entry(policy: ManagedFilesPolicy, target: Path) -> Dict[str, Any]:
    ...
    return {'name': target.name or resolved.name or str(resolved), 'path': str(resolved), 'is_directory': is_dir, 'size': None if is_dir else st.st_size, 'mtime': st.st_mtime, 'mime_type': mime_type}
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str]=None):
    ...
    The dashboard /chat page runs Duck Agent inside an xterm.js PTY. Browser
    ...
    ``DUCK_AGENT_HOME/images/`` — the same directory ``clipboard.paste`` /
    ...
    with _profile_scope(profile) as scoped_home:
        ...
        stem = re.sub('[^A-Za-z0-9_.-]+', '_', stem).strip('._-') or 'pasted-image'
    ...
    return {'ok': True, 'path': str(target), 'name': target.name, 'bytes': len(data), 'mime_type': mime_type}
...
async def list_managed_files(request: Request, path: Optional[str]=None):
    ...
    try:
        entries = [_managed_file_entry(policy, child) for child in target.iterdir() if not _is_sensitive_path(child)]
    except PermissionError:
...
async def read_managed_file(request: Request, path: str):
    ...
    return {'name': target.name, 'path': display_path, 'size': size, 'mime_type': mime_type, 'data_url': f'data:{mime_type};base64,{encoded}', **_managed_response_meta(policy)}
...
async def download_managed_file(request: Request, path: str):
    ...
    if size > _MANAGED_FILE_MAX_BYTES:
        raise HTTPException(status_code=413, detail='File is too large')
    mime_type = mimetypes.guess_type(target.name)[0] or 'application/octet-stream'
    return FileResponse(path=str(target), media_type=mime_type, filename=target.name, content_disposition_type='attachment')
...
async def upload_managed_file(payload: ManagedFileUpload, request: Request):
    ...
    return {'ok': True, 'entry': _managed_file_entry(policy, target), 'path': display_path, **_managed_response_meta(policy)}
_UPLOAD_CHUNK_BYTES = 1024 * 1024

@app.post('/api/files/upload-stream')
async def upload_managed_file_stream(request: Request, file: UploadFile=File(...), path: str=Form(...), overwrite: bool=Form(True)):
        ...
        with os.scandir(target) as scan:
            for entry in scan:
                if entry.name in _FS_READDIR_HIDDEN:
                    continue
                entries.append({'name': entry.name, 'path': str(target / entry.name), 'isDirectory': entry.is_dir(follow_symlinks=False)})
...
async def fs_read_text(path: str):
    ...
    return {'binary': _fs_looks_binary(data[:4096]), 'byteSize': st.st_size, 'language': _FS_PREVIEW_LANGUAGE_BY_EXT.get(target.suffix.lower(), 'text'), 'mimeType': _fs_mime_type(target), 'path': str(target), 'text': data.decode('utf-8', errors='replace'), 'truncated': st.st_size > _FS_TEXT_PREVIEW_MAX_BYTES}
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``duck-agent:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f'.{target.name}.duck-agent-tmp-{os.getpid()}')
    try:
...
from hermes_cli.web_routers import git as _git_routes
app.include_router(_git_routes.router)
from hermes_cli.web_routers.git import git_status_route, git_worktrees_route, git_branches_route, git_base_branches_route, git_review_list_route, git_review_diff_route, git_file_diff_route, git_commit_context_route, git_rev_parse_route, git_ship_info_route, git_stage_route, git_unstage_route, git_revert_route, git_commit_route, git_push_route, git_create_pr_route, git_worktree_add_route, git_worktree_remove_route, git_branch_switch_route
_PORT_BINDING_PLATFORM_PORTS: Dict[str, Tuple[str, int]] = {'webhook': ('port', 8644), 'api_server': ('port', 8642), 'msgraph_webhook': ('port', 8646), 'feishu': ('webhook_port', 8765), 'wecom_callback': ('port', 8645), 'bluebubbles': ('webhook_port', 8645), 'sms': ('webhook_port', 8080), 'whatsapp_cloud': ('webhook_port', 8090), 'line': ('port', 8646)}
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    active = [name for name, state in platforms.items() if name in _PORT_BINDING_PLATFORM_PORTS and isinstance(state, dict) and (state.get('state') not in _PLATFORM_DEAD_STATES)]
    if not active:
        return {}
...
def _topology_cache_get(fn: Any) -> Optional[Dict[str, Any]]:
    if _TOPOLOGY_CACHE['data'] is not None and _TOPOLOGY_CACHE['fn'] is fn and (time.monotonic() - _TOPOLOGY_CACHE['ts'] < _TOPOLOGY_CACHE_TTL):
...
async def get_health():
    ...
    return {'ok': True, 'version': __version__, 'auth_required': bool(getattr(app.state, 'auth_required', False))}
...
async def get_status(profile: Optional[str]=None):
    ...
    try:
        ...
        def _bounded_health_probe():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                ...
                try:
                    ...
                    _log.warning('/api/status gateway health probe exceeded %.2fs; using local status', _GATEWAY_HEALTH_ROUTE_TIMEOUT)
                    return (False, None)
                except Exception:
                    return (False, None)
        local_runtime = read_runtime_status(path=profile_dir / 'gateway_state.json') if profile_dir else read_runtime_status()
        liveness = await run_in_threadpool(lambda: resolve_gateway_liveness(profile_dir=profile_dir, runtime=local_runtime, health_probe=_bounded_health_probe if _GATEWAY_HEALTH_URL else None, pid_probe=get_running_pid_cached, runtime_reader=read_runtime_status, runtime_pid_probe=get_runtime_status_running_pid))
        ...
        try:
            configured_gateway_platforms = await run_in_threadpool(_load_configured_gateway_platforms)
        except Exception:
        ...
        gateway_busy = derive_gateway_busy(gateway_running=gateway_running, gateway_state=gateway_state, active_agents=active_agents)
        gateway_drainable = derive_gateway_drainable(gateway_running=gateway_running, gateway_state=gateway_state)
        restart_drain_timeout = await asyncio.get_running_loop().run_in_executor(None, _resolve_restart_drain_timeout)
        auth_required = bool(getattr(app.state, 'auth_required', False))
        auth_providers: list[str] = []
        auth_flows: list[str] = []
        try:
            from hermes_cli.dashboard_auth import list_providers as _list_providers, list_session_providers as _list_session_providers
            auth_providers = [p.name for p in _list_providers()]
            if auth_required:
                auth_flows.append('cookie')
                brokerable = [p for p in _list_session_providers() if not getattr(p, 'supports_password', False)]
                if brokerable:
        ...
        try:
            ...
            nous_session_valid = 'unknown'
        status = {'version': __version__, 'release_date': __release_date__, 'config_version': current_ver, 'latest_config_version': latest_ver, 'can_update_hermes': not _dashboard_local_update_managed_externally(), 'gateway_running': gateway_running, 'gateway_state': gateway_state, 'gateway_platforms': gateway_platforms, 'gateway_exit_reason': gateway_exit_reason, 'gateway_updated_at': gateway_updated_at, 'active_agents': active_agents, 'gateway_busy': gateway_busy, 'gateway_drainable': gateway_drainable, 'restart_drain_timeout': restart_drain_timeout, 'active_sessions': active_sessions, 'auth_required': auth_required, 'auth_providers': auth_providers, 'auth_flows': auth_flows, 'nous_session_valid': nous_session_valid}
        components: Dict[str, Any] = {'gateway': {'status': 'ok' if gateway_running and gateway_state in {'running', 'draining'} else 'degraded', 'state': gateway_state or ('running' if gateway_running else 'stopped')}, 'dashboard': DASHBOARD_HEALTH.snapshot()}
        try:
            from gateway.readiness import _probe_state_db
            storage_check = await asyncio.get_running_loop().run_in_executor(None, functools.partial(_probe_state_db, get_hermes_home()))
            components['storage'] = {'status': storage_check.get('status', 'degraded')}
        except Exception:
            components['storage'] = {'status': 'degraded'}
        platform_states = [str(value.get('state') or value.get('status') or '').lower() for value in gateway_platforms.values() if isinstance(value, dict)]
        platforms_ok = all((state in {'connected', 'running', 'ok'} for state in platform_states))
        components['platforms'] = {'status': 'ok' if platforms_ok else 'degraded', 'configured': len(gateway_platforms), 'connected': sum((1 for state in platform_states if state in {'connected', 'running', 'ok'}))}
        ...
        try:
            ...
            pass
        topology = await asyncio.get_running_loop().run_in_executor(None, _collect_profile_gateway_topology_cached)
        status['profiles'] = topology['profiles']
        status['gateway_mode'] = topology['gateway_mode']
        if not auth_required:
            status.update({'duck_agent_home': str(get_hermes_home()), 'config_path': str(get_config_path()), 'env_path': str(get_env_path()), 'gateway_pid': gateway_pid, 'gateway_health_url': _GATEWAY_HEALTH_URL, 'gateways': topology['gateways']})
        return status
    finally:
...
def _windows_build_number(version: str, platform_label: str) -> Optional[int]:
    ...
    for value in (version or '', platform_label or ''):
        match = re.search('(?:^|[^\\d])10\\.0\\.(\\d{5,})(?:[^\\d]|$)', value)
        if not match:
            continue
        try:
...
def _display_system_platform(*, system: str, release: str, version: str, platform_label: str) -> Dict[str, str]:
    ...
    if system == 'Windows' and release == '10':
        ...
        if build is not None and build >= _WINDOWS_11_MIN_BUILD:
            platform_label = re.sub('^Windows-10(?=-)', 'Windows-11', platform_label, count=1)
            release = '11'
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the duck-agent home root).
    """
    import platform as _platform
    info: Dict[str, Any] = {**_display_system_platform(system=_platform.system(), release=_platform.release(), version=_platform.version(), platform_label=_platform.platform()), 'arch': _platform.machine(), 'hostname': _platform.node(), 'python_version': _platform.python_version(), 'python_impl': _platform.python_implementation(), 'hermes_version': __version__, 'cpu_count': os.cpu_count()}
    try:
        import psutil
        ...
        try:
            proc = psutil.Process()
            info['process'] = {'pid': proc.pid, 'rss': proc.memory_info().rss, 'create_time': int(proc.create_time()), 'num_threads': proc.num_threads()}
        except Exception:
            pass
        info['psutil'] = True
    except Exception:
...
async def get_curator_status():
    ...
    try:
        ...
        state = {}
    return {'enabled': _safe_call(curator, 'is_enabled', True), 'paused': _safe_call(curator, 'is_paused', False), 'interval_hours': _safe_call(curator, 'get_interval_hours', None), 'last_run_at': state.get('last_run_at'), 'min_idle_hours': _safe_call(curator, 'get_min_idle_hours', None), 'stale_after_days': _safe_call(curator, 'get_stale_after_days', None), 'archive_after_days': _safe_call(curator, 'get_archive_after_days', None)}
...
async def get_portal_status():
    ...
    return {'logged_in': bool(auth.get('logged_in')), 'portal_url': auth.get('portal_base_url'), 'inference_url': auth.get('inference_base_url'), 'provider': str((model_cfg or {}).get('provider') or ''), 'subscription_url': 'https://portal.nousresearch.com/manage-subscription', 'features': features}
...
async def run_debug_share_endpoint(body: DebugShareRequest | None=None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    try:
        result = await asyncio.to_thread(build_debug_share, log_lines=max(1, min(int(req.lines), 5000)), redact=bool(req.redact))
    except RuntimeError as exc:
        raise HTTPException(status_code=502, detail=f'Upload failed: {exc}')
    except Exception as exc:
        _log.exception('debug share failed')
        raise HTTPException(status_code=500, detail=f'Failed: {exc}')
    return {'ok': True, 'urls': result.urls, 'failures': result.failures, 'redacted': result.redacted, 'auto_delete_seconds': result.auto_delete_seconds}
...
_ACTION_LOG_FILES: Dict[str, str] = {'gateway-restart': 'gateway-restart.log', 'gateway-start': 'gateway-start.log', 'gateway-stop': 'gateway-stop.log', 'duck-agent-update': 'duck-agent-update.log', 'doctor': 'action-doctor.log', 'security-audit': 'action-security-audit.log', 'backup': 'action-backup.log', 'import': 'action-import.log', 'checkpoints-prune': 'action-checkpoints-prune.log', 'skills-install': 'action-skills-install.log', 'skills-uninstall': 'action-skills-uninstall.log', 'skills-update': 'action-skills-update.log', 'curator-run': 'action-curator-run.log', 'prompt-size': 'action-prompt-size.log', 'dump': 'action-dump.log', 'config-migrate': 'action-config-migrate.log', 'tools-post-setup': 'action-tools-post-setup.log'}
...
def _spawn_hermes_action(subcommand: List[str], name: str, *, env_overrides: Optional[Dict[str, str]]=None) -> subprocess.Popen:
    """Spawn ``duck-agent <subcommand>`` detached and record the Popen handle.
    ...
    popen_kwargs: Dict[str, Any] = {'cwd': str(PROJECT_ROOT), 'stdin': subprocess.DEVNULL, 'stdout': log_file, 'stderr': subprocess.STDOUT, 'env': {**action_env, **(env_overrides or {})}}
    if sys.platform == 'win32':
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return ' '.join(['duck-agent', *_gateway_subcommand(profile, verb)])
_TELEGRAM_BOT_TOKEN_RE = re.compile('\\d+:[A-Za-z0-9_-]{30,}')
_TELEGRAM_USER_ID_RE = re.compile('\\d+')
_SLACK_MEMBER_ID_RE = re.compile('[UW][A-Z0-9]{2,}')
...
def _validate_messaging_env_value(platform_id: str, key: str, value: str) -> None:
    ...
    if platform_id == 'telegram':
        if key == 'TELEGRAM_BOT_TOKEN' and (not _TELEGRAM_BOT_TOKEN_RE.fullmatch(value)):
            raise HTTPException(status_code=400, detail='Telegram bot token must be the complete token from @BotFather, such as 123456789:ABC…')
        if key == 'TELEGRAM_ALLOWED_USERS':
            ...
            if any((not _TELEGRAM_USER_ID_RE.fullmatch(user_id) for user_id in user_ids)):
                raise HTTPException(status_code=400, detail='Telegram allowed users must be comma-separated numeric user IDs.')
        return
    if platform_id != 'slack':
        return
    if key == 'SLACK_BOT_TOKEN' and (not value.startswith('xoxb-')):
        raise HTTPException(status_code=400, detail='Slack Bot Token must start with xoxb-. Paste the bot token from OAuth & Permissions.')
    if key == 'SLACK_APP_TOKEN' and (not value.startswith('xapp-')):
        raise HTTPException(status_code=400, detail='Slack App Token must start with xapp-. Paste the app-level token from Basic Information > App-Level Tokens.')
    if key == 'SLACK_ALLOWED_USERS':
        user_ids = [part.strip() for part in value.split(',') if part.strip()]
        invalid = [user_id for user_id in user_ids if user_id != '*' and (not _SLACK_MEMBER_ID_RE.fullmatch(user_id))]
        if invalid:
            raise HTTPException(status_code=400, detail='Slack allowed user IDs must be comma-separated member IDs like U01ABC2DEF3.')

def _spawn_gateway_restart(profile: Optional[str]=None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``duck-agent gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``duck-agent gateway restart`` children race each other on the
...
def _restart_gateway_after_webhook_enable(profile: Optional[str]=None) -> dict[str, Any]:
    ...
    if reused:
        _log.info('Webhook enable: reusing in-flight gateway restart (pid %s)', proc.pid)
...
async def restart_gateway(profile: Optional[str]=None):
    """Kick off a ``duck-agent gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    payload = write_drain_request(principal=str(principal), suppress_notification=bool((body or {}).get('suppress_notification', False)))
    _log.info('Gateway drain BEGIN requested by %s (suppress_notification=%s)', principal, payload['suppress_notification'])
    return {'ok': True, 'action': 'drain', 'requested_at': payload['requested_at'], 'draining': drain_requested(), 'suppress_notification': payload['suppress_notification']}

@app.post('/api/duck-agent/update')
async def update_hermes():
    """Kick off ``duck-agent update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = 'Duck Agent updates are managed outside this dashboard in containerized environments. The built-in local updater is disabled here.'
        _record_completed_action('duck-agent-update', message, exit_code=1)
        return {'ok': False, 'pid': None, 'name': 'duck-agent-update', 'error': 'dashboard_update_managed_externally', 'message': message, 'update_command': 'managed outside dashboard'}
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == 'docker':
        message = format_docker_update_message()
        _record_completed_action('duck-agent-update', message, exit_code=1)
        return {'ok': False, 'pid': None, 'name': 'duck-agent-update', 'error': 'docker_update_unsupported', 'message': message, 'update_command': recommended_update_command_for_method(install_method)}
    if install_method in {'nix', 'nixos'}:
        message = recommended_update_command_for_method(install_method)
        _record_completed_action('duck-agent-update', message, exit_code=1)
        return {'ok': False, 'pid': None, 'name': 'duck-agent-update', 'error': 'nix_update_unsupported', 'message': message, 'update_command': message}
    existing = _ACTION_PROCS.get('duck-agent-update')
    if existing is not None and existing.poll() is None:
        response = {'ok': True, 'pid': existing.pid, 'name': 'duck-agent-update', 'already_running': True}
        action_id = _ACTION_IDS.get('duck-agent-update')
        if action_id:
            response['action_id'] = action_id
        return response
    action_id = secrets.token_hex(16)
    try:
        proc = _spawn_hermes_action(['update'], 'duck-agent-update', env_overrides={'HERMES_ACTION_ID': action_id})
    except Exception as exc:
        _log.exception('Failed to spawn duck-agent update')
        raise HTTPException(status_code=500, detail=f'Failed to start update: {exc}')
    return {'ok': True, 'pid': proc.pid, 'name': 'duck-agent-update', 'action_id': action_id}
...
def _recent_upstream_commits(n: int=20) -> List[Dict[str, Any]]:
    ...
    """
    try:
        out = subprocess.run(['git', '-C', str(PROJECT_ROOT), 'log', '--format=%H%x1f%s%x1f%an%x1f%ct', 'HEAD..origin/main', f'-n{int(n)}'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
        if out.returncode != 0:
            return []
...
@app.get('/api/duck-agent/update/check')
async def check_hermes_update(force: bool=False):
    """Report whether a Duck Agent update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/duck-agent/update`` actually runs ``duck-agent update``.

    Returns:
        install_method: 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Duck Agent version string
    ...
    """
    if _dashboard_local_update_managed_externally():
        return {'install_method': 'managed-runtime', 'current_version': __version__, 'behind': None, 'update_available': False, 'can_apply': False, 'update_command': 'managed outside dashboard', 'message': 'Duck Agent updates are managed outside this dashboard in containerized environments.'}
    install_method = detect_install_method(PROJECT_ROOT)
    update_command = recommended_update_command_for_method(install_method)
    payload: Dict[str, Any] = {'install_method': install_method, 'current_version': __version__, 'behind': None, 'update_available': False, 'can_apply': install_method == 'git', 'update_command': update_command, 'message': None}
    if install_method == 'docker':
...
async def transcribe_audio_upload(payload: AudioTranscriptionRequest, profile: Optional[str]=None):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(prefix='duck-agent-desktop-voice-', suffix=suffix, delete=False) as tmp:
    ...
    return {'ok': True, 'transcript': str(result.get('transcript') or '').strip(), 'provider': result.get('provider')}
...
async def get_elevenlabs_voices(profile: Optional[str]=None):
    ...
    request = urllib.request.Request('https://api.elevenlabs.io/v1/voices', headers={'Accept': 'application/json', 'xi-api-key': api_key})
    try:
        loop = asyncio.get_running_loop()
        ...
        if _voice_list_error_logged_once(str(exc)):
            _log.warning('ElevenLabs voice list failed: %s', exc)
        raise HTTPException(status_code=502, detail='Could not load ElevenLabs voices')
    _voice_list_error_logged_once(None)
    voices = []
    for voice in payload.get('voices') or []:
        ...
        if not voice_id:
            continue
        voices.append({'voice_id': voice_id, 'name': str(voice.get('name') or voice_id), 'label': _elevenlabs_voice_label(voice)})
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str]=None):
    ...
    configured in ``~/.duck-agent/config.yaml`` under ``tts.``.
    """
    ...
    if not result.get('success'):
        raise HTTPException(status_code=400, detail=result.get('error') or 'Speech synthesis failed')
    file_path = result.get('file_path')
    if not file_path or not os.path.isfile(file_path):
        raise HTTPException(status_code=500, detail='Audio file missing')
    ext = os.path.splitext(file_path)[1].lower()
    mime_type = {'.mp3': 'audio/mpeg', '.ogg': 'audio/ogg', '.opus': 'audio/ogg', '.wav': 'audio/wav', '.flac': 'audio/flac'}.get(ext, 'audio/mpeg')
    try:
    ...
    return {'ok': True, 'data_url': f'data:{mime_type};base64,{encoded}', 'mime_type': mime_type, 'provider': result.get('provider')}
...
async def speak_stream_ws(ws: 'WebSocket') -> None:
    ...
    if streamer is None:
        with contextlib.suppress(Exception):
            ...
            await ws.close()
        return
    await ws.send_json({'type': 'start', 'sample_rate': streamer.sample_rate, 'channels': streamer.channels})
    ...
    def _produce():
        ...
        idle_polls_before_force_flush = 4
...
async def get_action_status(name: str, lines: int=200):
    ...
    if proc is None:
        ...
        if exit_code is not None:
            ...
            _ACTION_COMMANDS.pop(name, None)
            _ACTION_IDS.pop(name, None)
    return {'name': name, 'running': running, 'exit_code': exit_code, 'pid': pid, 'lines': tail}
...
def _strip_session_list_rows(sessions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    for s in sessions:
        for key in _SESSION_LIST_HEAVY_FIELDS:
            s.pop(key, None)
    return sessions
from hermes_cli.web_routers import sessions as _sessions_routes
app.include_router(_sessions_routes.list_router)
from hermes_cli.web_routers.sessions import get_sessions
from hermes_cli.web_routers import profiles as _profiles_routes
app.include_router(_profiles_routes.sessions_router)
from hermes_cli.web_routers.profiles import get_profiles_sessions, get_profiles_sessions_sidebar
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Duck Agent supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _provider_field_entry(field: ProviderField) -> Dict[str, Any]:
    ...
    return {'key': field.key, 'label': field.label, 'kind': field.kind, 'description': field.description, 'info': field.info, 'placeholder': field.placeholder, 'inline': field.inline, 'group': field.group, 'options': [{'value': opt.value, 'label': opt.label, 'description': opt.description} for opt in field.options]}
_UNSET: Any = object()
...
def _write_provider_flat(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    ...
    atomic_json_write(path, existing, mode=384)
...
def _write_provider_honcho(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    ...
    with _config_refresh_lock(path):
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        atomic_json_write(path, cfg, mode=384)
...
def _memory_provider_setup_manifest(name: str) -> Dict[str, Any]:
    ...
    for raw in manifest.get('external_dependencies') or []:
        if not isinstance(raw, dict):
            continue
        dep = {'name': str(raw.get('name') or '').strip(), 'install': str(raw.get('install') or '').strip(), 'check': str(raw.get('check') or '').strip()}
        if dep['name'] or dep['install'] or dep['check']:
            external_dependencies.append(dep)
    return {'pip_dependencies': _string_list(manifest.get('pip_dependencies')), 'external_dependencies': external_dependencies, 'required_env': _string_list(manifest.get('requires_env'))}
...
def _memory_provider_setup_info(name: str) -> Dict[str, Any]:
    ...
    return setup
_MEMORY_PROVIDER_IMPORT_NAMES = {'honcho-ai': 'honcho', 'mem0ai': 'mem0', 'hindsight-client': 'hindsight_client', 'hindsight-all': 'hindsight'}

def _memory_provider_dependency_package(dep: str) -> str:
    return re.split('[\\[<>=!~;]', dep, maxsplit=1)[0].strip()
...
def _command_result(*, kind: str, name: str, status: str, command: str='', completed: Optional[subprocess.CompletedProcess]=None, error: Optional[str]=None) -> Dict[str, Any]:
    return {'kind': kind, 'name': name, 'status': status, 'command': command, 'returncode': None if completed is None else completed.returncode, 'stdout': '' if completed is None else _trim_setup_output(completed.stdout), 'stderr': _trim_setup_output(error or ('' if completed is None else completed.stderr))}

def _run_setup_command(command: Any, *, display: str, shell: bool=False, timeout: int=180) -> subprocess.CompletedProcess:
    return subprocess.run(command, shell=shell, executable='/bin/bash' if shell else None, env=_memory_provider_setup_env(), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, check=False)
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        return [_command_result(kind='pip', name=', '.join(missing), status='failed', error=str(exc))]
    if outcome.blocked:
        return [_command_result(kind='pip', name=', '.join(missing), status='failed', command=outcome.command, error=outcome.reason)]
    return [_command_result(kind='pip', name=', '.join(missing), status='installed' if outcome.ok else 'failed', command=outcome.command, completed=subprocess.CompletedProcess(args=outcome.command, returncode=0 if outcome.ok else 1, stdout=outcome.stdout, stderr=outcome.stderr))]
    ...
    for dep in dependencies:
        ...
        if check_cmd:
            try:
                ...
                results.append(_command_result(kind='external_check', name=name, status='missing' if install_cmd else 'failed', command=check_cmd, error=str(exc)))
            else:
                if check.returncode == 0:
                    results.append(_command_result(kind='external_check', name=name, status='already_installed', command=check_cmd, completed=check))
                    continue
                results.append(_command_result(kind='external_check', name=name, status='missing' if install_cmd else 'failed', command=check_cmd, completed=check))
            if not install_cmd:
                continue
        if install_cmd:
            try:
                ...
                continue
            results.append(_command_result(kind='external_install', name=name, status='installed' if install.returncode == 0 else 'failed', command=install_cmd, completed=install))
            if check_cmd and install.returncode == 0:
                try:
                    post_check = _run_setup_command(shlex.split(check_cmd), display=check_cmd, timeout=20)
                    results.append(_command_result(kind='external_check', name=name, status='verified' if post_check.returncode == 0 else 'failed', command=check_cmd, completed=post_check))
                except Exception as exc:
                    results.append(_command_result(kind='external_check', name=name, status='failed', command=check_cmd, error=str(exc)))
    return results
...
def _normalize_memory_provider_schema(name: str, provider: Any) -> List[Dict[str, Any]]:
    ...
    for raw in raw_schema:
        ...
        if raw.get('secret'):
            ...
            kind = 'boolean'
        elif explicit_kind in {'int', 'integer'} or (isinstance(raw.get('default'), int) and (not isinstance(raw.get('default'), bool))):
            kind = 'integer'
        ...
        fields.append({'key': key, 'label': str(raw.get('label') or key.replace('_', ' ').title()), 'kind': kind, 'description': description, 'placeholder': str(raw.get('placeholder') or ''), 'required': bool(raw.get('required', False)), 'default': raw.get('default', ''), 'options': options, 'url': str(raw.get('url') or ''), 'when': raw.get('when') if isinstance(raw.get('when'), dict) else None, 'minimum': raw.get('minimum'), 'maximum': raw.get('maximum'), 'step': raw.get('step'), '_env_key': str(raw.get('env_var') or '') or None})
    return fields
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    if name == 'holographic' and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get('duck-agent-memory-store')
...
def _field_visible(field: Dict[str, Any], data: Dict[str, Any], fields_by_key: Optional[Dict[str, Dict[str, Any]]]=None) -> bool:
    when = field.get('when')
    if not isinstance(when, dict) or not when:
        return True
    for dep_key, expected in when.items():
        dep_field = (fields_by_key or {}).get(str(dep_key)) or {'key': str(dep_key), 'kind': 'text', 'default': '', '_env_key': None}
...
def _public_memory_provider_field(field: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
    entry = {'key': field['key'], 'label': field['label'], 'kind': field['kind'], 'description': field['description'], 'placeholder': field['placeholder'], 'required': field['required'], 'value': '' if field['kind'] == 'secret' else _field_value(field, data), 'is_set': _field_is_set(field, data), 'options': field.get('options', []), 'url': field.get('url', ''), 'when': field.get('when'), 'minimum': field.get('minimum'), 'maximum': field.get('maximum'), 'step': field.get('step')}
    return entry

def _memory_provider_payload(name: str, provider: Any) -> Dict[str, Any]:
    data = _read_memory_provider_existing_values(name)
    fields = [_public_memory_provider_field(field, data) for field in _normalize_memory_provider_schema(name, provider)]
    return {'name': name, 'label': _memory_provider_label(name), 'fields': fields, 'setup': _memory_provider_setup_info(name)}
...
def _memory_provider_is_configured(name: str, provider: Any) -> bool:
    ...
    visible_fields = [field for field in fields if _field_visible(field, data, fields_by_key)]
...
def _discover_memory_provider_statuses() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for name, description, available in discover_memory_providers():
            discovered[str(name)] = {'name': str(name), 'description': str(description or ''), 'available': bool(available), 'missing': False}
    except Exception:
    ...
    if active and active not in discovered:
        discovered[active] = {'name': active, 'description': 'Configured provider was not found.', 'available': False, 'missing': True}
    ...
    for name in sorted(discovered):
        ...
        if row['missing']:
            ...
            status = 'ready'
        providers.append({'name': name, 'description': row['description'], 'available': row['available'], 'configured': configured, 'status': status, 'setup': setup})
    return providers
...
def _require_memory_provider_ready(name: str) -> None:
    ...
    if row['status'] != 'ready':
        raise HTTPException(status_code=400, detail=f"Memory provider '{name}' is not ready ({row['status'].replace('_', ' ')}). Configure it in the dashboard first.")
...
def _write_memory_provider_config_values(name: str, provider: Any, values: Dict[str, Any]) -> None:
    ...
    for field in fields:
        ...
        if field['kind'] == 'secret':
            ...
            continue
        raw = values[field['key']] if field['key'] in values else existing.get(field['key'], _field_default(field))
...
_MEMORY_PROVIDER_NAME_RE = re.compile('^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$')
...
async def update_memory_provider_config(name: str, body: MemoryProviderConfigUpdate, surface: Optional[str]=None, profile: Optional[str]=None):
    _require_valid_memory_provider_name(name)
...
_EMPTY_MODEL_INFO: dict = {'model': '', 'provider': '', 'auto_context_length': 0, 'config_context_length': 0, 'effective_context_length': 0, 'capabilities': {}}
...
def get_model_info(profile: Optional[str]=None):
    ...
    try:
        ...
        try:
            ...
            auto_ctx = get_model_context_length(model=model_name, base_url=base_url, provider=provider, config_context_length=None)
        except Exception:
            auto_ctx = 0
        config_ctx_int = 0
        ...
        try:
            ...
            if mc is not None:
                caps = {'supports_tools': mc.supports_tools, 'supports_vision': mc.supports_vision, 'supports_reasoning': mc.supports_reasoning, 'context_window': mc.context_window, 'max_output_tokens': mc.max_output_tokens, 'model_family': mc.model_family}
        except Exception:
            pass
        return {'model': model_name, 'provider': provider, 'auto_context_length': auto_ctx, 'config_context_length': config_ctx_int, 'effective_context_length': effective_ctx, 'capabilities': caps}
    except HTTPException:
        raise
    except Exception:
        _log.exception('GET /api/model/info failed')
        return dict(_EMPTY_MODEL_INFO)
_AUX_TASK_SLOTS: Tuple[str, ...] = ('vision', 'web_extract', 'compression', 'skills_hub', 'approval', 'mcp', 'title_generation', 'triage_specifier', 'kanban_decomposer', 'profile_describer', 'curator')

@app.get('/api/model/options')
async def get_model_options(profile: Optional[str]=None, refresh: bool=False, include_unconfigured: bool=False, explicit_only: bool=False):
    ...
    try:
        ...
        def _build_payload_scoped() -> dict:
            with _profile_scope(profile):
                return build_model_options_payload(load_picker_context(), explicit_only=bool(explicit_only), include_unconfigured=bool(include_unconfigured), refresh=bool(refresh))
...
def get_recommended_default_model(provider: str=''):
    ...
    Mirrors the model-curation `duck-agent model` does so GUI onboarding lands on a
    ...
    if slug == 'nous':
        try:
            from hermes_cli.models import get_curated_nous_model_ids, get_pricing_for_provider, check_nous_free_tier, partition_nous_models_by_tier, pick_silent_default_model, union_with_portal_free_recommendations, union_with_portal_paid_recommendations
...
def get_auxiliary_models(profile: Optional[str]=None):
    ...
    try:
        ...
        for slot in _AUX_TASK_SLOTS:
            ...
            tasks.append({'task': slot, 'provider': str(slot_cfg.get('provider', 'auto') or 'auto'), 'model': str(slot_cfg.get('model', '') or ''), 'base_url': str(slot_cfg.get('base_url', '') or '')})
        model_cfg = cfg.get('model', {})
        if isinstance(model_cfg, dict):
            main = {'provider': str(model_cfg.get('provider', '') or ''), 'model': str(model_cfg.get('default', model_cfg.get('name', '')) or '')}
        else:
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str]=None):
    ...
    try:
        ...
        def _preset_dict(preset: MoaPresetPayload) -> dict:
            return {'reference_models': [_slot_dict(slot) for slot in preset.reference_models], 'aggregator': _slot_dict(preset.aggregator), 'reference_temperature': preset.reference_temperature, 'aggregator_temperature': preset.aggregator_temperature, 'reference_timeout': preset.reference_timeout, 'degraded_reference_policy': preset.degraded_reference_policy, 'max_tokens': preset.max_tokens, 'reference_max_tokens': preset.reference_max_tokens, 'fanout': preset.fanout, 'enabled': preset.enabled}
        with _profile_scope(body.profile or profile):
            cfg = load_config()
            if body.presets:
                raw = {'default_preset': body.default_preset, 'active_preset': body.active_preset, 'presets': {name: _preset_dict(preset) for name, preset in body.presets.items()}}
            else:
                raw = _preset_dict(MoaPresetPayload(reference_models=body.reference_models, aggregator=body.aggregator, reference_temperature=body.reference_temperature, aggregator_temperature=body.aggregator_temperature, reference_timeout=body.reference_timeout, degraded_reference_policy=body.degraded_reference_policy, max_tokens=body.max_tokens, reference_max_tokens=body.reference_max_tokens, fanout=body.fanout, enabled=body.enabled))
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str]=None):
    ...
    Writes to ``~/.duck-agent/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        if model and (not body.confirm_expensive_model):
            try:
                ...
                warning = await asyncio.to_thread(expensive_model_warning, model, provider=provider, base_url=base_url)
            except Exception:
                warning = None
            if warning is not None:
                return {'ok': False, 'scope': scope, 'provider': provider, 'model': model, 'confirm_required': True, 'confirm_message': warning.message}
...
def _apply_model_assignment_sync(scope: str, provider: str, model: str, task: str, base_url: str, api_key: str=''):
    ...
    if scope == 'main':
        ...
        if not api_key and isinstance(provider_entry, dict) and provider_entry.get('api_key'):
        ...
        if isinstance(aux_cfg, dict):
            for slot in _AUX_TASK_SLOTS:
                ...
                if slot_provider and slot_provider.lower() not in {'auto', ''} and (slot_provider.lower() != new_provider):
                    stale_aux.append({'task': slot, 'provider': slot_provider, 'model': str(slot_cfg.get('model', '') or '')})
        return {'ok': True, 'scope': 'main', 'provider': provider, 'model': model, 'base_url': model_cfg.get('base_url', ''), 'gateway_tools': gateway_tools, 'stale_aux': stale_aux}
    ...
    if isinstance(model_val, str) and model_val:
        try:
            ...
            if isinstance(disk_model, dict):
                ...
                config['model'] = {'default': model_val, 'context_length': ctx_override}
        except Exception:
            pass
    return config
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``duck-agent model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        ...
        _OPT = {}
    _non_provider_keys = {k for k, v in _OPT.items() if (v or {}).get('category') and (v or {}).get('category') != 'provider'}
    meta: dict = {}
    for d in provider_catalog():
        ...
        for env_var in d.api_key_env_vars:
            if env_var in _non_provider_keys:
                continue
            meta.setdefault(env_var, {'provider': d.slug, 'provider_label': d.label, 'description': d.description, 'url': d.signup_url or None, 'is_password': True, 'advanced': False, 'category': 'provider'})
        if d.base_url_env_var:
            meta.setdefault(d.base_url_env_var, {'provider': d.slug, 'provider_label': d.label, 'description': f'{d.label} base URL override', 'url': None, 'is_password': False, 'advanced': True, 'category': 'provider'})
        if d.auth_type == 'aws_sdk':
            for aws_var in ('AWS_REGION', 'AWS_PROFILE'):
                existing = meta.get(aws_var, {})
                meta[aws_var] = {'provider': d.slug, 'provider_label': d.label, 'description': existing.get('description') or f'{d.label} ({aws_var})', 'url': existing.get('url'), 'is_password': False, 'advanced': existing.get('advanced', True), 'category': 'provider'}
        if d.auth_type == 'vertex':
            existing = meta.get('VERTEX_CREDENTIALS_PATH', {})
            meta['VERTEX_CREDENTIALS_PATH'] = {'provider': d.slug, 'provider_label': d.label, 'description': existing.get('description') or f'{d.label} — service account JSON path (or use ADC)', 'url': existing.get('url'), 'is_password': False, 'advanced': existing.get('advanced', True), 'category': 'provider'}
    return meta
...
async def get_env_vars(profile: Optional[str]=None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool=False) -> dict:
        ...
        return {'is_set': bool(value), 'redacted_value': redact_key(value) if value else None, 'description': info.get('description') or cat_meta.get('description', ''), 'url': info.get('url') if info.get('url') is not None else cat_meta.get('url'), 'category': info.get('category') or cat_meta.get('category', ''), 'is_password': info.get('password', cat_meta.get('is_password', False)), 'tools': info.get('tools', []), 'advanced': info.get('advanced', cat_meta.get('advanced', False)), 'channel_managed': var_name in channel_keys, 'provider': cat_meta.get('provider', ''), 'provider_label': cat_meta.get('provider_label', ''), 'custom': custom}
    result = {}
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str]=None):
    try:
        ...
        raise HTTPException(status_code=500, detail='Internal server error')
_CREDENTIAL_PROBES: dict[str, tuple[str, str]] = {'OPENROUTER_API_KEY': ('https://openrouter.ai/api/v1/key', 'bearer'), 'OPENAI_API_KEY': ('https://api.openai.com/v1/models', 'bearer'), 'XAI_API_KEY': ('https://api.x.ai/v1/models', 'bearer'), 'GEMINI_API_KEY': ('https://generativelanguage.googleapis.com/v1beta/models', 'query')}
...
def _custom_endpoint_id(raw: str, fallback: str='custom') -> str:
    slug = re.sub('[^A-Za-z0-9_-]+', '-', (raw or '').strip()).strip('-_').lower()
...
def _config_api_key_is_env_ref(endpoint_id: str) -> bool:
    ...
    return bool(isinstance(raw_key, str) and re.search('\\$\\{[^}]+\\}', raw_key))
...
def _custom_endpoint_response(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if current_provider.lower() == 'custom' and current_base_url and (not any((e['id'] == 'custom' for e in endpoints))):
        ...
        endpoints.insert(0, {'id': 'custom', 'name': 'Custom', 'base_url': current_base_url, 'model': current_model, 'models': [current_model] if current_model else [], 'context_length': model_cfg.get('context_length'), 'discover_models': True, 'has_api_key': has_api_key, 'api_key_preview': api_key_preview, 'is_current': True, 'source': 'direct-config'})
    return {'endpoints': endpoints, 'current': {'provider': current_provider, 'model': current_model, 'base_url': current_base_url}}
...
def _write_custom_endpoint(cfg: Dict[str, Any], body: CustomEndpointUpdate) -> Tuple[str, Dict[str, Any]]:
    ...
    entry.update({'name': name, 'base_url': base_url, 'model': model, 'discover_models': bool(body.discover_models)})
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {'telegram': {'name': 'Telegram', 'description': 'Run Duck Agent from Telegram DMs, groups, and topics.', 'docs_url': 'https://core.telegram.org/bots/features#botfather', 'env_vars': ('TELEGRAM_BOT_TOKEN', 'TELEGRAM_ALLOWED_USERS', 'TELEGRAM_PROXY'), 'required_env': ('TELEGRAM_BOT_TOKEN',)}, 'discord': {'name': 'Discord', 'description': 'Connect Duck Agent to Discord DMs, channels, and threads.', 'docs_url': 'https://discord.com/developers/applications', 'env_vars': ('DISCORD_BOT_TOKEN', 'DISCORD_ALLOWED_USERS'), 'required_env': ('DISCORD_BOT_TOKEN',)}, 'slack': {'name': 'Slack', 'description': 'Use Duck Agent from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.', 'docs_url': 'https://api.slack.com/apps', 'env_vars': ('SLACK_BOT_TOKEN', 'SLACK_APP_TOKEN', 'SLACK_ALLOWED_USERS'), 'required_env': ('SLACK_BOT_TOKEN', 'SLACK_APP_TOKEN')}, 'mattermost': {'name': 'Mattermost', 'description': 'Connect Duck Agent to Mattermost channels and direct messages.', 'docs_url': 'https://mattermost.com/deploy/', 'env_vars': ('MATTERMOST_URL', 'MATTERMOST_TOKEN', 'MATTERMOST_ALLOWED_USERS'), 'required_env': ('MATTERMOST_URL', 'MATTERMOST_TOKEN')}, 'matrix': {'name': 'Matrix', 'description': 'Use Duck Agent in Matrix rooms and direct messages.', 'docs_url': 'https://matrix.org/ecosystem/servers/', 'env_vars': ('MATRIX_HOMESERVER', 'MATRIX_ACCESS_TOKEN', 'MATRIX_USER_ID', 'MATRIX_ALLOWED_USERS'), 'required_env': ('MATRIX_HOMESERVER', 'MATRIX_ACCESS_TOKEN', 'MATRIX_USER_ID')}, 'signal': {'name': 'Signal', 'description': 'Connect through a signal-cli REST bridge.', 'docs_url': 'https://github.com/bbernhard/signal-cli-rest-api', 'env_vars': ('SIGNAL_HTTP_URL', 'SIGNAL_ACCOUNT', 'SIGNAL_ALLOWED_USERS'), 'required_env': ('SIGNAL_HTTP_URL', 'SIGNAL_ACCOUNT')}, 'whatsapp': {'name': 'WhatsApp', 'description': 'Use Duck Agent through the bundled WhatsApp bridge with QR-based auth.', 'docs_url': 'https://github.com/tulir/whatsmeow', 'env_vars': ('WHATSAPP_ENABLED', 'WHATSAPP_MODE', 'WHATSAPP_DM_POLICY', 'WHATSAPP_ALLOWED_USERS'), 'required_env': ()}, 'homeassistant': {'name': 'Home Assistant', 'description': 'Control your smart home from Duck Agent via Home Assistant.', 'docs_url': 'https://www.home-assistant.io/docs/authentication/', 'env_vars': ('HASS_URL', 'HASS_TOKEN'), 'required_env': ('HASS_URL', 'HASS_TOKEN')}, 'email': {'name': 'Email', 'description': 'Talk to Duck Agent through an IMAP/SMTP mailbox.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/', 'env_vars': ('EMAIL_ADDRESS', 'EMAIL_PASSWORD', 'EMAIL_IMAP_HOST', 'EMAIL_SMTP_HOST'), 'required_env': ('EMAIL_ADDRESS', 'EMAIL_PASSWORD', 'EMAIL_IMAP_HOST', 'EMAIL_SMTP_HOST')}, 'sms': {'name': 'SMS (Twilio)', 'description': 'Send and receive text messages via Twilio.', 'docs_url': 'https://www.twilio.com/console', 'env_vars': ('TWILIO_ACCOUNT_SID', 'TWILIO_AUTH_TOKEN'), 'required_env': ('TWILIO_ACCOUNT_SID', 'TWILIO_AUTH_TOKEN')}, 'dingtalk': {'name': 'DingTalk', 'description': 'Connect Duck Agent to DingTalk groups (钉钉).', 'docs_url': 'https://open.dingtalk.com/document/orgapp/the-robot-development-process', 'env_vars': ('DINGTALK_CLIENT_ID', 'DINGTALK_CLIENT_SECRET'), 'required_env': ('DINGTALK_CLIENT_ID', 'DINGTALK_CLIENT_SECRET')}, 'feishu': {'name': 'Feishu / Lark', 'description': 'Use Duck Agent inside Feishu / Lark.', 'docs_url': 'https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/intro', 'env_vars': ('FEISHU_APP_ID', 'FEISHU_APP_SECRET', 'FEISHU_ENCRYPT_KEY', 'FEISHU_VERIFICATION_TOKEN'), 'required_env': ('FEISHU_APP_ID', 'FEISHU_APP_SECRET')}, 'google_chat': {'name': 'Google Chat', 'description': 'Connect Duck Agent to Google Chat via Cloud Pub/Sub.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/google_chat'}, 'wecom': {'name': 'WeCom (group bot)', 'description': 'Send-only WeCom group bot via webhook.', 'docs_url': 'https://developer.work.weixin.qq.com/document/path/91770', 'env_vars': ('WECOM_BOT_ID', 'WECOM_SECRET'), 'required_env': ('WECOM_BOT_ID',)}, 'wecom_callback': {'name': 'WeCom (app)', 'description': 'Two-way WeCom integration via callback app.', 'docs_url': 'https://developer.work.weixin.qq.com/document/path/90930', 'env_vars': ('WECOM_CALLBACK_CORP_ID', 'WECOM_CALLBACK_CORP_SECRET', 'WECOM_CALLBACK_AGENT_ID', 'WECOM_CALLBACK_TOKEN', 'WECOM_CALLBACK_ENCODING_AES_KEY'), 'required_env': ('WECOM_CALLBACK_CORP_ID', 'WECOM_CALLBACK_CORP_SECRET', 'WECOM_CALLBACK_AGENT_ID')}, 'weixin': {'name': 'Weixin / WeChat (Personal)', 'description': "Connect a personal WeChat account through Tencent's iLink Bot API.", 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/weixin/', 'env_vars': ('WEIXIN_ACCOUNT_ID', 'WEIXIN_TOKEN', 'WEIXIN_BASE_URL'), 'required_env': ('WEIXIN_ACCOUNT_ID', 'WEIXIN_TOKEN')}, 'bluebubbles': {'name': 'BlueBubbles (iMessage)', 'description': 'Use Duck Agent through iMessage via a BlueBubbles server.', 'docs_url': 'https://bluebubbles.app/', 'env_vars': ('BLUEBUBBLES_SERVER_URL', 'BLUEBUBBLES_PASSWORD', 'BLUEBUBBLES_ALLOWED_USERS'), 'required_env': ('BLUEBUBBLES_SERVER_URL', 'BLUEBUBBLES_PASSWORD')}, 'qqbot': {'name': 'QQ Bot', 'description': 'Connect Duck Agent to a QQ Bot from the QQ Open Platform.', 'docs_url': 'https://q.qq.com', 'env_vars': ('QQ_APP_ID', 'QQ_CLIENT_SECRET', 'QQ_ALLOWED_USERS'), 'required_env': ('QQ_APP_ID', 'QQ_CLIENT_SECRET')}, 'teams': {'description': 'Connect Duck Agent to Microsoft Teams chats via the Bot Framework.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/teams'}, 'irc': {'description': 'Relay messages between an IRC channel (or DMs) and Duck Agent.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/irc'}, 'line': {'description': 'Use Duck Agent from LINE via the LINE Messaging API webhook.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/line'}, 'ntfy': {'description': 'Chat with Duck Agent over ntfy push topics (ntfy.sh or self-hosted).', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/ntfy'}, 'photon': {'description': "Use Duck Agent through iMessage via Photon's managed Spectrum platform.", 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/photon'}, 'raft': {'description': 'Join a Raft workspace as an external agent.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/raft'}, 'simplex': {'description': 'Talk to Duck Agent over SimpleX Chat via a local simplex-chat daemon.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/simplex'}, 'yuanbao': {'name': 'Yuanbao (元宝)', 'description': 'Connect Duck Agent to Tencent Yuanbao.', 'docs_url': '', 'required_env': ()}, 'api_server': {'name': 'API server', 'description': 'Expose Duck Agent as an OpenAI-compatible HTTP API for tools like Open WebUI.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/', 'env_vars': ('API_SERVER_ENABLED', 'API_SERVER_KEY', 'API_SERVER_PORT', 'API_SERVER_HOST', 'API_SERVER_MODEL_NAME'), 'required_env': ()}, 'webhook': {'name': 'Webhooks', 'description': 'Receive events from GitHub, GitLab, and other webhook sources.', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/webhooks/', 'env_vars': ('WEBHOOK_ENABLED', 'WEBHOOK_PORT', 'WEBHOOK_SECRET'), 'required_env': ()}, 'msgraph_webhook': {'name': 'Microsoft Graph Webhook', 'description': 'Receive Microsoft Graph change notifications (Teams meetings, Outlook, …).', 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook', 'required_env': ()}, 'whatsapp_cloud': {'name': 'WhatsApp Cloud API', 'description': "Use Duck Agent via Meta's hosted WhatsApp Cloud API (no local bridge).", 'docs_url': 'https://duck-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud'}, 'relay': {'name': 'Relay (experimental)', 'description': 'Generic relay adapter fronted by the Duck Agent Relay connector.', 'docs_url': '', 'required_env': ()}}
_PLATFORM_ORDER: tuple[str, ...] = ('telegram', 'discord', 'slack', 'mattermost', 'matrix', 'whatsapp', 'signal', 'bluebubbles', 'homeassistant', 'email', 'sms', 'dingtalk', 'feishu', 'google_chat', 'wecom', 'wecom_callback', 'weixin', 'qqbot', 'yuanbao', 'api_server', 'webhook')
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {'SIGNAL_HTTP_URL': {'description': 'signal-cli REST API base URL, e.g. http://127.0.0.1:8080', 'prompt': 'Signal bridge URL', 'url': 'https://github.com/bbernhard/signal-cli-rest-api'}, 'SIGNAL_ACCOUNT': {'description': 'Signal account phone number registered with the bridge', 'prompt': 'Signal account'}, 'SIGNAL_ALLOWED_USERS': {'description': 'Comma-separated Signal users allowed to use the bot', 'prompt': 'Allowed Signal users'}, 'WHATSAPP_ENABLED': {'description': 'Enable the WhatsApp gateway adapter', 'prompt': 'Enable WhatsApp', 'advanced': True}, 'WHATSAPP_MODE': {'description': 'WhatsApp bridge mode', 'prompt': 'WhatsApp mode', 'advanced': True}, 'WHATSAPP_DM_POLICY': {'description': 'How WhatsApp direct messages are authorized', 'prompt': 'WhatsApp DM policy', 'advanced': True}, 'WHATSAPP_ALLOWED_USERS': {'description': 'Comma-separated WhatsApp users allowed to use the bot', 'prompt': 'Allowed WhatsApp users'}, 'HASS_URL': {'description': 'Home Assistant base URL, e.g. https://homeassistant.local:8123', 'prompt': 'Home Assistant URL'}, 'HASS_TOKEN': {'description': 'Long-lived access token from Home Assistant (Profile → Security)', 'prompt': 'Home Assistant access token', 'password': True}, 'EMAIL_ADDRESS': {'description': 'Email address to send and receive from', 'prompt': 'Email address'}, 'EMAIL_PASSWORD': {'description': 'Email account password or app password', 'prompt': 'Email password', 'password': True}, 'EMAIL_IMAP_HOST': {'description': 'IMAP server host (e.g. imap.gmail.com)', 'prompt': 'IMAP host'}, 'EMAIL_SMTP_HOST': {'description': 'SMTP server host (e.g. smtp.gmail.com)', 'prompt': 'SMTP host'}, 'TWILIO_ACCOUNT_SID': {'description': 'Twilio Account SID', 'prompt': 'Twilio Account SID', 'url': 'https://www.twilio.com/console'}, 'TWILIO_AUTH_TOKEN': {'description': 'Twilio Auth Token', 'prompt': 'Twilio Auth Token', 'password': True}, 'WECOM_BOT_ID': {'description': 'WeCom group bot ID', 'prompt': 'WeCom Bot ID'}, 'WECOM_SECRET': {'description': 'WeCom group bot secret', 'prompt': 'WeCom Secret', 'password': True}, 'WECOM_CALLBACK_CORP_ID': {'description': 'WeCom corp ID', 'prompt': 'WeCom Corp ID'}, 'WECOM_CALLBACK_CORP_SECRET': {'description': 'WeCom app corp secret', 'prompt': 'WeCom Corp Secret', 'password': True}, 'WECOM_CALLBACK_AGENT_ID': {'description': 'WeCom app agent ID', 'prompt': 'WeCom Agent ID'}, 'WECOM_CALLBACK_TOKEN': {'description': 'WeCom callback verification token', 'prompt': 'WeCom Token'}, 'WECOM_CALLBACK_ENCODING_AES_KEY': {'description': 'WeCom callback AES encoding key', 'prompt': 'WeCom AES Key', 'password': True}, 'WEIXIN_ACCOUNT_ID': {'description': 'iLink Bot account ID obtained through QR login in duck-agent gateway setup', 'prompt': 'iLink Bot account ID'}, 'WEIXIN_TOKEN': {'description': 'iLink Bot token obtained through QR login in duck-agent gateway setup', 'prompt': 'iLink Bot token', 'password': True}, 'WEIXIN_BASE_URL': {'description': 'iLink API base URL saved by QR login (default: https://ilinkai.weixin.qq.com)', 'prompt': 'iLink API base URL'}, 'FEISHU_APP_ID': {'description': 'Feishu / Lark app ID', 'prompt': 'App ID'}, 'FEISHU_APP_SECRET': {'description': 'Feishu / Lark app secret', 'prompt': 'App secret', 'password': True}, 'FEISHU_ENCRYPT_KEY': {'description': 'Feishu / Lark encrypt key', 'prompt': 'Encrypt key', 'password': True}, 'FEISHU_VERIFICATION_TOKEN': {'description': 'Feishu / Lark verification token', 'prompt': 'Verification token', 'password': True}, 'DINGTALK_CLIENT_ID': {'description': 'DingTalk client ID (App key)', 'prompt': 'Client ID'}, 'DINGTALK_CLIENT_SECRET': {'description': 'DingTalk client secret (App secret)', 'prompt': 'Client secret', 'password': True}}
...
def _messaging_platform_catalog() -> tuple[dict[str, Any], ...]:
    ...
    for member in Platform.__members__.values():
        ...
        entries.append(_build_catalog_entry(member.value, plugin_map.get(member.value)))
...
def _channel_managed_env_keys() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset()
_MESSAGING_KEYS_PAGE_KEYS = frozenset({'GATEWAY_ALLOW_ALL_USERS', 'GATEWAY_PROXY_KEY', 'GATEWAY_PROXY_URL'})

def _platform_env_prefixes(platform_id: str) -> tuple[str, ...]:
    """Env-var prefixes owned by a messaging platform card."""
    aliases: dict[str, tuple[str, ...]] = {'email': ('EMAIL_',), 'homeassistant': ('HASS_',), 'qqbot': ('QQ_', 'QQBOT_'), 'sms': ('TWILIO_',), 'wecom': ('WECOM_BOT_', 'WECOM_SECRET'), 'wecom_callback': ('WECOM_CALLBACK_',)}
...
def _build_catalog_entry(platform_id: str, plugin_entry: Any | None=None) -> dict[str, Any]:
    ...
    return {'id': platform_id, 'name': name, 'description': description or '', 'docs_url': override.get('docs_url', ''), 'env_vars': env_vars, 'required_env': required_env}
...
def _messaging_env_info(key: str) -> dict[str, Any]:
    ...
    return {'description': info.get('description', ''), 'prompt': info.get('prompt', key), 'help': info.get('help', ''), 'url': info.get('url'), 'is_password': info.get('password', False), 'advanced': info.get('advanced', False)}
...
def _messaging_platform_payload(entry: dict[str, Any], env_on_disk: dict[str, str], runtime: dict | None, scoped: bool=False, profile_home: Optional[Path]=None) -> dict[str, Any]:
    ...
    liveness = resolve_gateway_liveness(profile_dir=profile_home, runtime=runtime, health_probe=_probe_gateway_health if _GATEWAY_HEALTH_URL else None, pid_probe=get_running_pid_cached, runtime_reader=read_runtime_status, runtime_pid_probe=get_runtime_status_running_pid)
    gateway_running = liveness.running
    env_vars = []
    for key in entry['env_vars']:
        value = env_on_disk.get(key) or ('' if scoped else os.getenv(key, ''))
        env_vars.append({'key': key, 'required': key in entry['required_env'], 'is_set': bool(value), 'redacted_value': redact_key(value) if value else None, **_messaging_env_info(key)})
    if scoped:
        try:
            cfg = load_config()
    ...
    elif not gateway_running and (not state) and (runtime_gateway_state == 'startup_failed'):
    ...
    if platform_id == 'whatsapp':
        whatsapp_mode = (env_on_disk.get('WHATSAPP_MODE') or ('' if scoped else os.getenv('WHATSAPP_MODE', ''))).strip()
        allowed_users_value = (env_on_disk.get('WHATSAPP_ALLOWED_USERS') or ('' if scoped else os.getenv('WHATSAPP_ALLOWED_USERS', ''))).strip()
        whatsapp_setup = {'mode': whatsapp_mode if whatsapp_mode in {'bot', 'self-chat'} else '', 'allowed_users_set': bool(allowed_users_value), 'home_channel_set': bool(home_channel)}
    payload = {'id': platform_id, 'name': entry['name'], 'description': entry['description'], 'docs_url': entry['docs_url'], 'enabled': enabled, 'configured': configured, 'gateway_running': gateway_running, 'state': state, 'error_code': error_code, 'error_message': error_message, 'updated_at': runtime_platform.get('updated_at') if isinstance(runtime_platform, dict) else None, 'home_channel': home_channel, 'env_vars': env_vars}
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    try:
        result = subprocess.run([npm, 'install', '--silent'], cwd=str(bridge_dir), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, env=with_hermes_node_path(), creationflags=windows_hide_flags())
...
def _spawn_whatsapp_pairing_process(session_path: Path, mode: str) -> subprocess.Popen:
    ...
    return subprocess.Popen([node, str(bridge_script), '--pair-only', '--pair-json', '--session', str(session_path)], cwd=str(bridge_dir), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', start_new_session=True, env=env, creationflags=windows_hide_flags())
...
def _watch_whatsapp_pairing(pairing_id: str, proc: subprocess.Popen) -> None:
    ...
    with _whatsapp_onboarding_lock:
        ...
        if record.status in {'connected', 'cancelled', 'expired'}:
            return
        record.status = 'error'
        record.error = 'WhatsApp pairing process exited before pairing completed.' if returncode == 0 else f'WhatsApp pairing process exited with code {returncode}.'
...
def _prune_whatsapp_onboarding_sessions() -> None:
    ...
    for pairing_id, record in _whatsapp_onboarding_sessions.items():
        if record.proc is not None and record.status not in _WHATSAPP_ONBOARDING_TERMINAL_STATUSES and (record.proc.poll() is not None):
            record.status = 'error'
...
def _whatsapp_onboarding_payload(pairing_id: str, record: _WhatsAppOnboardingSession) -> dict[str, Any]:
    return {'pairing_id': pairing_id, 'status': record.status, 'qr_payload': record.qr_payload, 'expires_at': record.expires_at, 'mode': record.mode, 'allowed_users': record.allowed_users, 'account_id': record.account_id, 'account_name': record.account_name, 'account_phone': record.account_phone, 'error': record.error}
...
def _restart_gateway_after_whatsapp_onboarding(profile: Optional[str]=None) -> dict[str, Any]:
    try:
        ...
        return {'restart_started': False, 'restart_error': str(exc)}
    if reused:
        _log.info('WhatsApp onboarding: reusing in-flight gateway restart (pid %s)', proc.pid)
    return {'restart_started': True, 'restart_action': 'gateway-restart', 'restart_pid': proc.pid}
...
async def start_whatsapp_onboarding(body: WhatsAppOnboardingStart):
    ...
    with _config_profile_scope(effective_profile):
        ...
        if (session_path / 'creds.json').exists():
            ...
            record = _WhatsAppOnboardingSession(proc=None, mode=mode, allowed_users=allowed_users, session_path=str(session_path), expires_at=expires_at, expires_at_ts=expires_at_ts, profile=effective_profile, status='connected', account_id=account_id, account_name=account_name, account_phone=account_phone)
            with _whatsapp_onboarding_lock:
    ...
    pairing_id = secrets.token_urlsafe(16)
    record = _WhatsAppOnboardingSession(proc=None, mode=mode, allowed_users=allowed_users, session_path=str(session_path), expires_at=expires_at, expires_at_ts=expires_at_ts, profile=effective_profile)
    with _whatsapp_onboarding_lock:
        _prune_whatsapp_onboarding_sessions()
        _supersede_whatsapp_onboarding_sessions(session_path)
        _whatsapp_onboarding_sessions[pairing_id] = record
...
async def get_whatsapp_onboarding_status(pairing_id: str):
    with _whatsapp_onboarding_lock:
        ...
        record = _whatsapp_onboarding_sessions.get(pairing_id)
        if not record:
            raise HTTPException(status_code=404, detail='WhatsApp setup session was not found. Start a new setup.')
...
async def apply_whatsapp_onboarding(pairing_id: str, body: WhatsAppOnboardingApply, profile: Optional[str]=None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail='Failed to save WhatsApp setup.') from exc
    with _whatsapp_onboarding_lock:
        _whatsapp_onboarding_sessions.pop(pairing_id, None)
...
async def cancel_whatsapp_onboarding(pairing_id: str):
    ...
    if record:
        ...
        _terminate_whatsapp_pairing(record.proc)
    return {'ok': True}
_TELEGRAM_ONBOARDING_DEFAULT_URL = 'https://setup.duck-agent.nousresearch.com'
...
def _prune_telegram_onboarding_pairings() -> None:
    now = time.time()
    expired = [pairing_id for pairing_id, record in _telegram_onboarding_pairings.items() if record.expires_at_ts <= now]
...
def _telegram_onboarding_error_message(error: str, fallback: str) -> str:
    return {'not_found': 'Telegram pairing was not found. Start a new setup.', 'expired': 'Telegram setup expired. Start a new setup.', 'claimed': 'Telegram setup was already claimed. Start a new setup.', 'unauthorized': 'Telegram setup service rejected this request.', 'telegram_manager_bot_token_not_configured': 'Telegram setup service is not configured.', 'telegram_token_fetch_failed': 'Telegram could not finish bot setup. Try again.'}.get(error, fallback)

def _telegram_onboarding_request_sync(method: str, path: str, *, body: dict[str, Any] | None=None, bearer_token: str | None=None) -> dict[str, Any]:
    import httpx
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail='Telegram setup service is unavailable. Try again shortly.') from exc
    except Exception as exc:
        raise HTTPException(status_code=502, detail='Telegram setup service is unavailable. Try again shortly.') from exc
    try:
        parsed = response.json()
    except Exception as exc:
        raise HTTPException(status_code=502, detail='Telegram setup service returned an invalid response.') from exc
    if not isinstance(parsed, dict):
        raise HTTPException(status_code=502, detail='Telegram setup service returned an invalid response.')
    return parsed

async def _telegram_onboarding_request(method: str, path: str, *, body: dict[str, Any] | None=None, bearer_token: str | None=None) -> dict[str, Any]:
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or 'Duck Agent').strip() or 'Duck Agent'
    ...
    if not pairing_id or not poll_token or (not expires_at) or (not deep_link):
        raise HTTPException(status_code=502, detail='Telegram setup service returned an incomplete response.')
    with _telegram_onboarding_lock:
        _prune_telegram_onboarding_pairings()
        _telegram_onboarding_pairings[pairing_id] = _TelegramOnboardingPairing(poll_token=poll_token, expires_at=expires_at, expires_at_ts=_parse_expiry_ts(expires_at))
    return {'pairing_id': pairing_id, 'suggested_username': suggested_username, 'deep_link': deep_link, 'qr_payload': qr_payload, 'expires_at': expires_at}
...
async def get_telegram_onboarding_status(pairing_id: str):
    with _telegram_onboarding_lock:
        ...
        if not record:
            raise HTTPException(status_code=404, detail='Telegram setup session was not found. Start a new setup.')
        if record.bot_token:
            return {'status': 'ready', 'bot_username': record.bot_username, 'owner_user_id': record.owner_user_id, 'expires_at': record.expires_at}
        poll_token = record.poll_token
    payload = await _telegram_onboarding_request('GET', f"/v1/telegram/pairings/{urllib.parse.quote(pairing_id, safe='')}", bearer_token=poll_token)
    ...
    if status in {'expired', 'claimed'}:
        with _telegram_onboarding_lock:
            _telegram_onboarding_pairings.pop(pairing_id, None)
        raise HTTPException(status_code=410, detail=_telegram_onboarding_error_message(status, 'Telegram setup is no longer available. Start a new setup.'))
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str]=None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Duck Agent appears
    ...
    try:
        ...
        return {'restart_started': False, 'restart_error': str(exc)}
    if reused:
        _log.info('Telegram onboarding: reusing in-flight gateway restart (pid %s)', proc.pid)
    return {'restart_started': True, 'restart_action': 'gateway-restart', 'restart_pid': proc.pid}
...
async def apply_telegram_onboarding(pairing_id: str, body: TelegramOnboardingApply, profile: Optional[str]=None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail='Failed to save Telegram setup.') from exc
    with _telegram_onboarding_lock:
        _telegram_onboarding_pairings.pop(pairing_id, None)
    restart_result = _restart_gateway_after_telegram_onboarding(effective_profile)
    return {'ok': True, 'platform': 'telegram', 'bot_username': bot_username, 'needs_restart': not restart_result['restart_started'], **restart_result}
...
async def get_messaging_platforms(profile: Optional[str]=None):
    with _profile_scope(profile) as scoped_dir:
        ...
        return {'env_path': str(get_env_path()), 'gateway_start_command': _gateway_display_command(profile, 'start'), 'platforms': [_messaging_platform_payload(entry, env_on_disk, runtime, scoped=scoped_dir is not None, profile_home=scoped_dir) for entry in _messaging_platform_catalog()]}
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    with _config_profile_scope('default'):
        if not load_gateway_config().multiplex_profiles:
            return None
    return f"Cannot enable '{platform_id}' on profile '{target}': it binds its own listener port, and gateway.multiplex_profiles is on, so the default profile owns the single shared HTTP listener for every profile. Configure this channel on the default profile instead (disabling or clearing it here is still allowed)."
...
async def update_messaging_platform(platform_id: str, body: MessagingPlatformUpdate, profile: Optional[str]=None):
    ...
    if body.enabled:
        ...
        if conflict:
            _log.info('Rejected messaging platform update: platform=%s profile=%s (multiplex port-binding conflict)', platform_id, target_profile or 'current')
    ...
    try:
        ...
        _log.info('Messaging platform updated: platform=%s profile=%s enabled=%s env_keys=%s cleared_keys=%s', platform_id, target_profile or 'current', body.enabled, sorted(body.env), sorted(body.clear_env))
...
async def test_messaging_platform(platform_id: str, profile: Optional[str]=None):
    ...
    with _profile_scope(profile) as scoped_dir:
        env_on_disk = load_env()
        runtime = read_runtime_status(path=scoped_dir / 'gateway_state.json') if scoped_dir is not None else read_runtime_status()
        payload = _messaging_platform_payload(entry, env_on_disk, runtime, scoped=scoped_dir is not None, profile_home=scoped_dir)
    ...
    if not payload['gateway_running']:
        return {'ok': False, 'state': payload['state'], 'message': 'Gateway is not running. Restart the gateway to connect this platform.'}
    ...
    return {'ok': False, 'state': payload['state'], 'message': 'Setup looks complete, but the gateway has not reported a connection yet. Restart the gateway.'}
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.duck-agent/.anthropic_oauth.json`` — Duck Agent-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get('accessToken'):
        return {'logged_in': True, 'source': 'hermes_pkce', 'source_label': f'Duck Agent PKCE ({(_get_hermes_oauth_file() if _get_hermes_oauth_file else None)})', 'token_preview': _truncate_token(hermes_creds.get('accessToken')), 'expires_at': hermes_creds.get('expiresAt'), 'has_refresh_token': bool(hermes_creds.get('refreshToken'))}
    ...
    try:
        ...
        format_secret_source_suffix = None
    for var in env_var_order:
        ...
        return {'logged_in': True, 'source': 'env_var', 'source_label': f'{var}{suffix}', 'token_preview': _truncate_token(value), 'expires_at': None, 'has_refresh_token': False}
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Duck Agent even
    when they also have a separate Duck Agent-managed PKCE login.
    """
    try:
    ...
    if creds and creds.get('accessToken'):
        return {'logged_in': True, 'source': 'claude_code_cli', 'source_label': '~/.claude/.credentials.json', 'token_preview': _truncate_token(creds.get('accessToken')), 'expires_at': creds.get('expiresAt'), 'has_refresh_token': bool(creds.get('refreshToken'))}
    return {'logged_in': False, 'source': None}
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Duck Agent never claims a login state it can't verify.
    """
    return {'logged_in': False, 'source': 'copilot_cli', 'source_label': 'Managed by the GitHub Copilot CLI', 'token_preview': None, 'expires_at': None, 'has_refresh_token': False}
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = ({'id': 'nous', 'name': 'Nous Portal', 'flow': 'device_code', 'cli_command': 'duck-agent auth add nous', 'docs_url': 'https://portal.nousresearch.com', 'status_fn': None}, {'id': 'openai-codex', 'name': 'OpenAI OAuth (ChatGPT)', 'flow': 'device_code', 'cli_command': 'duck-agent auth add openai-codex', 'docs_url': 'https://platform.openai.com/docs', 'status_fn': None}, {'id': 'qwen-oauth', 'name': 'Qwen (via Qwen CLI)', 'flow': 'external', 'cli_command': 'duck-agent auth add qwen-oauth', 'docs_url': 'https://github.com/QwenLM/qwen-code', 'status_fn': None}, {'id': 'minimax-oauth', 'name': 'MiniMax (OAuth)', 'flow': 'device_code', 'cli_command': 'duck-agent auth add minimax-oauth', 'docs_url': 'https://www.minimax.io', 'status_fn': None}, {'id': 'xai-oauth', 'name': 'xAI Grok OAuth (SuperGrok / Premium+)', 'flow': 'device_code', 'cli_command': 'duck-agent auth add xai-oauth', 'docs_url': 'https://duck-agent.nousresearch.com/docs/guides/xai-grok-oauth', 'status_fn': None}, {'id': 'copilot-acp', 'name': 'GitHub Copilot (ACP)', 'flow': 'external', 'cli_command': 'copilot /login', 'docs_url': 'https://docs.github.com/en/copilot', 'status_fn': _copilot_acp_status}, {'id': 'anthropic', 'name': 'Anthropic API Key', 'flow': 'pkce', 'cli_command': 'duck-agent auth add anthropic', 'docs_url': 'https://docs.claude.com/en/api/getting-started', 'status_fn': _anthropic_oauth_status}, {'id': 'claude-code', 'name': 'Anthropic OAuth: Required Extra Usage Credits to Use Subscription', 'flow': 'external', 'cli_command': 'claude setup-token', 'docs_url': 'https://docs.claude.com/en/docs/claude-code', 'status_fn': _claude_code_only_status})
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        ...
        if provider_id == 'nous':
            ...
            return {'logged_in': bool(raw.get('logged_in')), 'source': 'nous_portal', 'source_label': raw.get('portal_base_url') or 'Nous Portal', 'token_preview': _truncate_token(raw.get('access_token')), 'expires_at': raw.get('access_expires_at'), 'has_refresh_token': bool(raw.get('has_refresh_token'))}
        if provider_id == 'openai-codex':
            raw = hauth.get_codex_auth_status()
            return {'logged_in': bool(raw.get('logged_in')), 'source': raw.get('source') or 'openai_codex', 'source_label': raw.get('auth_mode') or 'OpenAI Codex', 'token_preview': _truncate_token(raw.get('api_key')), 'expires_at': None, 'has_refresh_token': False, 'last_refresh': raw.get('last_refresh')}
        if provider_id == 'qwen-oauth':
            raw = hauth.get_qwen_auth_status()
            return {'logged_in': bool(raw.get('logged_in')), 'source': 'qwen_cli', 'source_label': raw.get('auth_store_path') or 'Qwen CLI', 'token_preview': _truncate_token(raw.get('access_token')), 'expires_at': raw.get('expires_at'), 'has_refresh_token': bool(raw.get('has_refresh_token'))}
        if provider_id == 'minimax-oauth':
            raw = hauth.get_minimax_oauth_auth_status()
            return {'logged_in': bool(raw.get('logged_in')), 'source': 'minimax_oauth', 'source_label': f"MiniMax ({raw.get('region', 'global')})", 'token_preview': None, 'expires_at': raw.get('expires_at'), 'has_refresh_token': True}
        if provider_id == 'xai-oauth':
            raw = hauth.get_xai_oauth_auth_status()
            return {'logged_in': bool(raw.get('logged_in')), 'source': raw.get('source') or 'xai_oauth', 'source_label': raw.get('auth_store') or raw.get('source') or 'xAI Grok OAuth', 'token_preview': _truncate_token(raw.get('api_key')), 'expires_at': None, 'has_refresh_token': True, 'last_refresh': raw.get('last_refresh')}
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Duck Agent, so the disconnect
    ...
    user sees exactly what executes, and Duck Agent then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get('flow') == 'external':
        if _oauth_provider_disconnect_command(provider):
            return 'Managed outside Duck Agent — run the disconnect command to remove it.'
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``duck-agent model`` universe) — so any OAuth/external provider added as a
    ...
    try:
        ...
        for d in provider_catalog():
            if d.tab != 'accounts' or d.slug in seen:
                continue
            seen.add(d.slug)
            rows.append({'id': d.slug, 'name': d.label, 'flow': 'external', 'cli_command': f'duck-agent auth add {d.slug}', 'docs_url': d.signup_url or '', 'status_fn': None})
    except Exception:
        pass
    return rows
...
async def list_oauth_providers(profile: Optional[str]=None):
    ...
    sync with the `duck-agent model` picker; _OAUTH_OVERRIDES supplies per-provider
    ...
    with _profile_scope(profile):
        ...
        for p in _build_oauth_catalog():
            ...
            providers.append({'id': p['id'], 'name': p['name'], 'flow': p['flow'], 'cli_command': p['cli_command'], 'docs_url': p['docs_url'], 'disconnect_hint': disconnect_hint, 'disconnect_command': _oauth_provider_disconnect_command(p), 'disconnectable': disconnect_hint is None, 'status': status})
...
async def disconnect_oauth_provider(provider_id: str, request: Request, profile: Optional[str]=None):
    ...
    with _profile_scope(profile):
        ...
        if provider is None:
            raise HTTPException(status_code=400, detail=f"Unknown provider: {provider_id}. Available: {', '.join(sorted(catalog_by_id))}")
        disconnect_hint = _oauth_provider_disconnect_hint(provider, {})
        if disconnect_hint:
            raise HTTPException(status_code=400, detail=f"{provider['name']} cannot be disconnected automatically. {disconnect_hint}")
...
try:
    from agent.anthropic_adapter import _OAUTH_CLIENT_ID as _ANTHROPIC_OAUTH_CLIENT_ID, _OAUTH_TOKEN_URL as _ANTHROPIC_OAUTH_TOKEN_URL, _OAUTH_TOKEN_URLS as _ANTHROPIC_OAUTH_TOKEN_URLS, _OAUTH_REDIRECT_URI as _ANTHROPIC_OAUTH_REDIRECT_URI, _OAUTH_SCOPES as _ANTHROPIC_OAUTH_SCOPES, _generate_pkce as _generate_pkce_pair
...
def _new_oauth_session(provider_id: str, flow: str, profile: Optional[str]=None) -> tuple[str, Dict[str, Any]]:
    ...
    profile_name = _oauth_profile_name(profile)
    sess = {'session_id': sid, 'provider': provider_id, 'flow': flow, 'profile': profile_name, 'created_at': time.time(), 'status': 'pending', 'error_message': None}
    with _oauth_sessions_lock:
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Duck Agent file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``duck-agent auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_hermes_oauth_file
    oauth_file = _get_hermes_oauth_file()
    payload = {'accessToken': access_token, 'refreshToken': refresh_token, 'expiresAt': expires_at_ms}
    from utils import atomic_json_write
    atomic_json_write(oauth_file, payload, indent=2, mode=384)
    try:
        from agent.credential_pool import PooledCredential, load_pool, AUTH_TYPE_OAUTH, SOURCE_MANUAL
        import uuid
        ...
        for e in existing:
            try:
                ...
                pass
        entry = PooledCredential(provider='anthropic', id=uuid.uuid4().hex[:6], label='dashboard PKCE', auth_type=AUTH_TYPE_OAUTH, priority=0, source=f'{SOURCE_MANUAL}:dashboard_pkce', access_token=access_token, refresh_token=refresh_token, expires_at_ms=expires_at_ms)
...
def _start_anthropic_pkce(profile: Optional[str]=None) -> Dict[str, Any]:
    ...
    sess['state'] = verifier
    params = {'code': 'true', 'client_id': _ANTHROPIC_OAUTH_CLIENT_ID, 'response_type': 'code', 'redirect_uri': _ANTHROPIC_OAUTH_REDIRECT_URI, 'scope': _ANTHROPIC_OAUTH_SCOPES, 'code_challenge': challenge, 'code_challenge_method': 'S256', 'state': verifier}
    auth_url = f'{_ANTHROPIC_OAUTH_AUTHORIZE_URL}?{urllib.parse.urlencode(params)}'
    return {'session_id': sid, 'flow': 'pkce', 'auth_url': auth_url, 'expires_in': _OAUTH_SESSION_TTL_SECONDS}

def _submit_anthropic_pkce(session_id: str, code_input: str, profile: Optional[str]=None) -> Dict[str, Any]:
    ...
    exchange_data = json.dumps({'grant_type': 'authorization_code', 'client_id': _ANTHROPIC_OAUTH_CLIENT_ID, 'code': code, 'state': state_from_callback or sess['state'], 'redirect_uri': _ANTHROPIC_OAUTH_REDIRECT_URI, 'code_verifier': sess['verifier']}).encode()
    result = None
    last_exc = None
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(_endpoint, data=exchange_data, headers={'Content-Type': 'application/json', 'User-Agent': 'duck-agent-dashboard/1.0'}, method='POST')
        try:
...
async def _start_device_code_flow(provider_id: str, profile: Optional[str]=None) -> Dict[str, Any]:
    ...
    if provider_id == 'nous':
        ...
        portal_base_url = (os.getenv('HERMES_PORTAL_BASE_URL') or os.getenv('NOUS_PORTAL_BASE_URL') or pconfig.portal_base_url).rstrip('/')
        client_id = pconfig.client_id
        scope = pconfig.scope

        def _do_nous_device_request():
            with httpx.Client(timeout=httpx.Timeout(15.0), headers={'Accept': 'application/json'}) as client:
                return (_request_device_code(client=client, portal_base_url=portal_base_url, client_id=client_id, scope=scope), scope)
        ...
        threading.Thread(target=_nous_poller, args=(sid,), daemon=True, name=f'oauth-poll-{sid[:6]}').start()
        return {'session_id': sid, 'flow': 'device_code', 'user_code': str(device_data['user_code']), 'verification_url': str(device_data['verification_uri_complete']), 'expires_in': int(device_data['expires_in']), 'poll_interval': int(device_data['interval'])}
    if provider_id == 'openai-codex':
        ...
        return {'session_id': sid, 'flow': 'device_code', 'user_code': s['user_code'], 'verification_url': s['verification_url'], 'expires_in': int(s.get('expires_in') or 900), 'poll_interval': int(s.get('interval') or 5)}
    if provider_id == 'minimax-oauth':
        from hermes_cli.auth import _minimax_pkce_pair, _minimax_request_user_code, MINIMAX_OAUTH_CLIENT_ID, MINIMAX_OAUTH_GLOBAL_BASE
        import httpx
        verifier, challenge, state = _minimax_pkce_pair()
        portal_base_url = (os.getenv('MINIMAX_PORTAL_BASE_URL') or MINIMAX_OAUTH_GLOBAL_BASE).rstrip('/')

        def _do_minimax_request():
            with httpx.Client(timeout=httpx.Timeout(15.0), headers={'Accept': 'application/json'}, follow_redirects=True) as client:
                return _minimax_request_user_code(client=client, portal_base_url=portal_base_url, client_id=MINIMAX_OAUTH_CLIENT_ID, code_challenge=challenge, state=state)
        ...
        if expired_in_raw > 1000000000000:
        ...
        threading.Thread(target=_minimax_poller, args=(sid,), daemon=True, name=f'oauth-poll-{sid[:6]}').start()
        return {'session_id': sid, 'flow': 'device_code', 'user_code': str(device_data['user_code']), 'verification_url': str(device_data['verification_uri']), 'expires_in': expires_in_seconds, 'poll_interval': max(2, (sess['interval_ms'] or 2000) // 1000)}
    if provider_id == 'xai-oauth':
        ...
        sess['interval'] = int(device_data['interval'])
        sess['expires_at'] = time.time() + int(device_data['expires_in'])
        threading.Thread(target=_xai_device_poller, args=(sid,), daemon=True, name=f'oauth-poll-{sid[:6]}').start()
        return {'session_id': sid, 'flow': 'device_code', 'user_code': str(device_data['user_code']), 'verification_url': str(device_data.get('verification_uri_complete') or device_data['verification_uri']), 'expires_in': int(device_data['expires_in']), 'poll_interval': int(device_data['interval'])}
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        with httpx.Client(timeout=httpx.Timeout(15.0), headers={'Accept': 'application/json'}) as client:
            token_data = _poll_for_token(client=client, portal_base_url=portal_base_url, client_id=client_id, device_code=device_code, expires_in=expires_in, poll_interval=interval)
        now = datetime.now(timezone.utc)
        token_ttl = int(token_data.get('expires_in') or 0)
        auth_state = {'portal_base_url': portal_base_url, 'inference_base_url': token_data.get('inference_base_url'), 'client_id': client_id, 'scope': token_data.get('scope') or scope, 'token_type': token_data.get('token_type', 'Bearer'), 'access_token': token_data['access_token'], 'refresh_token': token_data.get('refresh_token'), 'obtained_at': now.isoformat(), 'expires_at': datetime.fromtimestamp(now.timestamp() + token_ttl, tz=timezone.utc).isoformat() if token_ttl else None, 'expires_in': token_ttl}
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``duck-agent auth add minimax-oauth``.
    """
    from hermes_cli.auth import _minimax_poll_token, _minimax_resolve_token_expiry_unix, _minimax_save_auth_state, MINIMAX_OAUTH_GLOBAL_INFERENCE, MINIMAX_OAUTH_SCOPE
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
        sess = _oauth_sessions.get(session_id)
    ...
    try:
        with httpx.Client(timeout=httpx.Timeout(15.0), headers={'Accept': 'application/json'}, follow_redirects=True) as client:
            token_data = _minimax_poll_token(client=client, portal_base_url=portal_base_url, client_id=client_id, user_code=user_code, code_verifier=code_verifier, expired_in=expired_in_raw, interval_ms=interval_ms)
        now = datetime.now(timezone.utc)
...
def _xai_device_poller(session_id: str) -> None:
    ...
    import httpx
    from hermes_cli.auth import _save_xai_oauth_tokens, _xai_oauth_discovery, _xai_oauth_poll_device_token, mark_provider_active_if_unset, unsuppress_credential_source
    with _oauth_sessions_lock:
        sess = _oauth_sessions.get(session_id)
    if not sess:
        return
    ...
    try:
        ...
        with httpx.Client(timeout=httpx.Timeout(20.0), headers={'Accept': 'application/json'}) as client:
            token_data = _xai_oauth_poll_device_token(client, token_endpoint=discovery['token_endpoint'], device_code=device_code, expires_in=expires_in, poll_interval=interval)
        tokens = {'access_token': str(token_data.get('access_token', '') or '').strip(), 'refresh_token': str(token_data.get('refresh_token', '') or '').strip(), 'id_token': str(token_data.get('id_token', '') or '').strip(), 'expires_in': token_data.get('expires_in'), 'token_type': str(token_data.get('token_type') or 'Bearer').strip() or 'Bearer'}
        with _profile_scope(_oauth_session_profile(session_id)):
            _save_xai_oauth_tokens(tokens, discovery=discovery, last_refresh=datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'), auth_mode='oauth_device_code', set_active=False)
...
def _http_response_error_detail(resp: Any) -> str:
    ...
    if isinstance(payload, dict):
        ...
        if isinstance(error, dict):
            parts = [str(error.get(key, '')).strip() for key in ('message', 'error_description', 'code', 'type') if str(error.get(key, '')).strip()]
            if parts:
                return ': '.join(parts)
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if 'device' in lower and ('authori' in lower or 'enable' in lower):
        message = 'OpenAI rejected the device-code login request. Your OpenAI account may need device-code authorization enabled before Duck Agent can start this dashboard login. Enable device-code authorization in OpenAI, then return here and click Login again.'
    else:
        message = 'OpenAI rejected the device-code login request. Please try Login again from the dashboard after checking your OpenAI account settings.'
    if detail:
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            resp = client.post(f'{issuer}/api/accounts/deviceauth/usercode', json={'client_id': CODEX_OAUTH_CLIENT_ID}, headers={'Content-Type': 'application/json'})
        ...
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            while time.monotonic() < deadline:
                ...
                if sess.get('cancelled'):
                    _log.info('oauth/device: openai-codex login cancelled (session=%s)', session_id)
                    return
                poll = client.post(f'{issuer}/api/accounts/deviceauth/token', json={'device_auth_id': device_auth_id, 'user_code': user_code}, headers={'Content-Type': 'application/json'})
        ...
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            token_resp = client.post(CODEX_OAUTH_TOKEN_URL, data={'grant_type': 'authorization_code', 'code': authorization_code, 'redirect_uri': f'{issuer}/deviceauth/callback', 'client_id': CODEX_OAUTH_CLIENT_ID, 'code_verifier': code_verifier}, headers={'Content-Type': 'application/x-www-form-urlencoded'})
...
async def start_oauth_login(provider_id: str, request: Request, profile: Optional[str]=None):
    ...
    if catalog_entry['flow'] == 'external':
        raise HTTPException(status_code=400, detail=f"{provider_id} uses an external CLI; run `{catalog_entry['cli_command']}` manually")
    try:
...
async def submit_oauth_code(provider_id: str, body: OAuthSubmitBody, request: Request, profile: Optional[str]=None):
    ...
    _require_token(request)
    if provider_id == 'anthropic':
        return await asyncio.get_running_loop().run_in_executor(None, _submit_anthropic_pkce, body.session_id, body.code, profile)
...
async def poll_oauth_session(provider_id: str, session_id: str, profile: Optional[str]=None):
    ...
    return {'session_id': session_id, 'status': sess['status'], 'error_message': sess.get('error_message'), 'expires_at': sess.get('expires_at')}
...
def _session_latest_descendant(session_id: str, db):
    ...
    if not sid or not db.get_session(sid):
        return (None, [])
    conn = getattr(db, 'conn', None) or getattr(db, '_conn', None) or getattr(db, 'connection', None) or getattr(db, '_connection', None)
    rows = []
    if conn is not None:
        ...
        for row in raw_rows:
            rows.append({'id': row_get(row, 'id', 0), 'parent_session_id': row_get(row, 'parent_session_id', 1), 'started_at': row_get(row, 'started_at', 2)})
    else:
...
from hermes_cli.web_routers.sessions import bulk_delete_sessions_endpoint, import_sessions_endpoint, count_empty_sessions_endpoint, delete_empty_sessions_endpoint, get_session_stats, get_session_detail, get_session_latest_descendant, get_session_messages, delete_session_endpoint, rename_session_endpoint, export_session_endpoint, prune_sessions_endpoint
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `duck-agent update` until the first message forced a writable open.
    """
    ...
    try:
        ...
        try:
            ...
            if str(db_path) not in _session_db_heal_warned:
                ...
                _log.warning('state.db at %s is missing schema that a writable reconcile could not add (%s); read paths may partially fail until the store is repaired', db_path, still_stale)
            return _open_probed()
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``duck-agent serve`` — it runs neither the
    ...
    try:
        ...
        db = _open_session_db_for_profile(profile, read_only=False)
        try:
            db.maybe_auto_archive(idle_days=float(cfg.get('auto_archive_days', 3)), min_interval_hours=int(cfg.get('min_interval_hours', 24)))
        finally:
            db.close()
    except Exception as exc:
        _log.debug('opportunistic auto-archive skipped: %s', exc)

async def _auto_archive_ticker_loop(interval_s: float=3600.0, initial_delay_s: float=90.0) -> None:
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `duck-agent sessions prune`)."""
    ...
    _attr_filters_set = any((getattr(body, f) is not None for f in ('source', 'title_like', 'end_reason', 'cwd_prefix', 'min_messages', 'max_messages', 'model_like', 'provider', 'user_id', 'chat_id', 'chat_type', 'branch_like', 'min_tokens', 'max_tokens', 'min_cost', 'max_cost', 'min_tool_calls', 'max_tool_calls')))
...
async def get_logs(file: str='agent', lines: int=100, level: Optional[str]=None, component: Optional[str]=None, search: Optional[str]=None):
    ...
    if component and component.lower() != 'all':
        ...
        if comp_prefixes is None:
            raise HTTPException(status_code=400, detail=f"Unknown component: {component}. Available: {', '.join(sorted(COMPONENT_PREFIXES))}")
    else:
        comp_prefixes = None
    has_filters = bool(min_level or comp_prefixes or search)
    result = _read_tail(log_path, min(lines, 500) if not search else 2000, has_filters=has_filters, min_level=min_level, component_prefixes=comp_prefixes)
    if search:
        needle = search.lower()
...
def _validate_dashboard_cron_context_from(refs: Optional[List[str]], profile_name: str) -> None:
    ...
    for ref in refs:
        if not _call_cron_for_profile(profile_name, 'get_job', ref):
            raise HTTPException(status_code=400, detail=f"context_from job '{ref}' not found in profile '{profile_name}'")
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (DUCK_AGENT_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.duck-agent``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    DUCK_AGENT_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, DUCK_AGENT_HOME)."""
    from hermes_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated['duck_agent_home'] = str(home)
...
from hermes_cli.web_routers import cron as _cron_routes
app.include_router(_cron_routes.router)
from hermes_cli.web_routers.cron import list_cron_jobs, get_cron_job, list_cron_job_runs, create_cron_job, get_cron_delivery_targets, update_cron_job, pause_cron_job, resume_cron_job, trigger_cron_job, delete_cron_job, cron_fire_webhook, list_cron_blueprints, instantiate_blueprint
...
def _create_cron_job_sync(body: CronJobCreate, profile: Optional[str]=None):
    try:
        ...
        _validate_dashboard_cron_effective_job({'prompt': body.prompt, 'skills': skills, 'script': script, 'no_agent': no_agent})
        return _call_cron_for_profile(profile_name, 'create_job', prompt=body.prompt or '', schedule=body.schedule, name=body.name, deliver=_cron_optional_text(body.deliver) or 'local', skills=skills, model=_cron_optional_text(body.model), provider=_cron_optional_text(body.provider), base_url=_cron_optional_text(body.base_url, strip_trailing_slash=True), script=script, context_from=context_from, enabled_toolsets=_cron_string_list(body.enabled_toolsets), workdir=_cron_optional_text(body.workdir), no_agent=no_agent)
    except HTTPException:
        raise
    except Exception as e:
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Duck Agent home so the job's store,
...
def _mcp_server_summary(name: str, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not auth and isinstance(headers, dict) and any((str(key).lower() == 'authorization' for key in headers)):
        auth = 'header'
    return {'name': name, 'transport': transport, 'url': cfg.get('url'), 'command': cfg.get('command'), 'args': list(cfg.get('args') or []), 'env': _redact_mcp_env(cfg.get('env') or {}), 'auth': auth, 'enabled': cfg.get('enabled', True) is not False, 'tools': cfg.get('tools')}
from hermes_cli.web_routers import mcp as _mcp_routes
app.include_router(_mcp_routes.router)
from hermes_cli.web_routers.mcp import list_mcp_servers, add_mcp_server, replace_mcp_servers, remove_mcp_server, test_mcp_server, auth_mcp_server, mcp_oauth_flow_status, mcp_oauth_callback, set_mcp_server_enabled, list_mcp_catalog, install_mcp_catalog_entry
...
def _gc_mcp_oauth_flows() -> None:
    ...
    with _mcp_oauth_flows_lock:
        stale = [flow_id for flow_id, flow in _mcp_oauth_flows.items() if getattr(flow, 'created_at', 0) < cutoff]
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    ...
                    tools = _probe_single_server(flow.server_name, cfg, connect_timeout=max(float(cfg.get('connect_timeout', 0) or 0), 315))
                    if not _oauth_tokens_present(flow.server_name):
                        raise RuntimeError('The server responded, but no OAuth token was obtained — this provider may require a manually-registered OAuth client.')
        ...
        try:
            ...
            humanized = humanize_oauth_registration_error(flow.server_name, exc, server_url=cfg.get('url') if isinstance(cfg, dict) else None)
            if humanized:
                msg = humanized
        except Exception:
            pass
...
def _mcp_install_action_name(name: str) -> str:
    ...
    slug = re.sub('[^a-z0-9]+', '-', name.lower()).strip('-')[:48] or 'server'
...
async def approve_pairing(body: PairingApprove):
    ...
    if not by_request_id and store._is_locked_out(platform):
        raise HTTPException(status_code=429, detail=f"Platform '{platform}' is locked out after too many failed approvals.")
    raise HTTPException(status_code=404, detail=f"Pairing request or code not found or expired for platform '{platform}'.")
...
def _webhook_route_summary(name: str, route: Dict[str, Any], base_url: str) -> Dict[str, Any]:
    return {'name': name, 'description': route.get('description', ''), 'events': list(route.get('events') or []), 'deliver': route.get('deliver', 'log'), 'deliver_only': bool(route.get('deliver_only')), 'prompt': route.get('prompt', ''), 'script': route.get('script', ''), 'skills': list(route.get('skills') or []), 'created_at': route.get('created_at'), 'url': f'{base_url}/webhooks/{name}', 'secret_set': bool(route.get('secret')), 'enabled': route.get('enabled', True) is not False}
...
async def list_webhooks():
    ...
    return {'enabled': wh._is_webhook_enabled(), 'base_url': base_url, 'subscriptions': [_webhook_route_summary(name, route, base_url) for name, route in subs.items()]}
...
async def enable_webhooks():
    try:
        ...
        raise HTTPException(status_code=500, detail='Failed to enable webhook platform.') from exc
    restart_result = _restart_gateway_after_webhook_enable()
    return {'ok': True, 'platform': 'webhook', 'enabled': True, 'needs_restart': not restart_result['restart_started'], **restart_result}
...
async def create_webhook(body: WebhookCreate):
    ...
    if not wh._is_webhook_enabled():
        raise HTTPException(status_code=400, detail='Webhook platform is not enabled. Enable it from the Webhooks page first.')
    name = (body.name or '').strip().lower().replace(' ', '-')
    if not _re.match('^[a-z0-9][a-z0-9_-]*$', name):
        raise HTTPException(status_code=400, detail='Invalid name. Use lowercase alphanumeric with hyphens/underscores.')
    if body.deliver_only and body.deliver == 'log':
        raise HTTPException(status_code=400, detail="Direct delivery requires a real target (telegram, discord, …), not 'log'.")
    secret = body.secret or _secrets.token_urlsafe(32)
    route: Dict[str, Any] = {'description': body.description or f'Dashboard-created subscription: {name}', 'events': [e.strip() for e in body.events if e.strip()], 'secret': secret, 'prompt': body.prompt or '', 'skills': [s.strip() for s in body.skills if s.strip()], 'deliver': body.deliver or 'log', 'created_at': _time.strftime('%Y-%m-%dT%H:%M:%SZ', _time.gmtime())}
...
async def list_credential_pool():
    ...
    for provider_id in sorted(raw_pool.keys()):
        ...
        if not entries:
            continue
        providers.append({'provider': provider_id, 'entries': [_pool_entry_summary(e, i) for i, e in enumerate(entries, start=1)]})
    return {'providers': providers}

@app.post('/api/credentials/pool')
async def add_credential_pool_entry(body: CredentialPoolAdd):
    import uuid as _uuid
    from agent.credential_pool import load_pool, PooledCredential, AUTH_TYPE_API_KEY, CUSTOM_POOL_PREFIX, SOURCE_MANUAL
    ...
    try:
        ...
        entry = PooledCredential(provider=provider, id=_uuid.uuid4().hex[:6], label=label, auth_type=AUTH_TYPE_API_KEY, priority=0, source=SOURCE_MANUAL, access_token=api_key)
        pool.add_entry(entry)
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``duck-agent auth remove`` uses: each source cleans up its
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f'duck-agent-backup-{stamp}-{secrets.token_hex(4)}.zip'
...
async def download_dashboard_backup(archive: str):
    ...
    if not target.is_file():
        raise HTTPException(status_code=404, detail='Backup not found')
    return FileResponse(path=str(target), media_type='application/zip', filename=target.name, content_disposition_type='attachment')
...
async def run_import_upload(file: UploadFile=File(...), force: bool=Form(False)):
    ...
    tmp_fd, tmp_name = tempfile.mkstemp(prefix=f'.{target.name}.', suffix='.upload', dir=str(staging_dir))
    tmp_path = Path(tmp_name)
    total = 0
    renamed = False
    try:
        ...
        raise HTTPException(status_code=403, detail='Import staging directory is not writable')
    except OSError as exc:
        raise HTTPException(status_code=500, detail=f'Could not write uploaded archive: {exc}')
    finally:
        if not renamed:
            tmp_path.unlink(missing_ok=True)
        await file.close()
    ...
    try:
        ...
        _log.exception('Failed to spawn import')
        raise HTTPException(status_code=500, detail=f'Failed to run import: {exc}')
    return {'ok': True, 'pid': proc.pid, 'name': 'import', 'archive': str(target), 'uploaded_bytes': total}
...
async def list_hooks():
    ...
    for spec in specs:
        ...
        try:
            ...
            pass
        out.append({'event': spec.event, 'matcher': spec.matcher, 'command': spec.command, 'timeout': spec.timeout, 'allowed': entry is not None, 'approved_at': (entry or {}).get('approved_at'), 'executable': executable})
...
async def delete_hook(body: HookDelete):
    ...
    if isinstance(hooks_cfg, dict) and isinstance(hooks_cfg.get(event), list):
        ...
        hooks_cfg[event] = [e for e in hooks_cfg[event] if not (isinstance(e, dict) and e.get('command') == command)]
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``duck-agent`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    """
    slug = re.sub('[^a-z0-9]+', '-', key.lower()).strip('-')[:48] or 'skill'
    digest = hashlib.sha1(key.encode()).hexdigest()[:8]
    name = f'skills-{verb}-{slug}-{digest}'
    _ACTION_LOG_FILES.setdefault(name, f'action-{name}.log')
    return name
from hermes_cli.web_routers import skills as _skills_routes
app.include_router(_skills_routes.hub_router)
from hermes_cli.web_routers.skills import install_skill_hub, uninstall_skill_hub, update_skills_hub, list_skills_hub_sources, search_skills_hub, preview_skill_hub, scan_skill_hub
_SKILL_HUB_SOURCE_LABELS = {'official': 'Official (Nous)', 'duck-agent-index': 'Duck Agent Index', 'skills-sh': 'skills.sh', 'well-known': 'Well-Known', 'url': 'Direct URL', 'github': 'GitHub', 'clawhub': 'ClawHub', 'lobehub': 'LobeHub', 'browse-sh': 'browse.sh'}

def _skill_meta_to_payload(m) -> dict:
    return {'name': m.name, 'description': m.description, 'source': m.source, 'identifier': m.identifier, 'trust_level': m.trust_level, 'repo': m.repo, 'tags': list(m.tags or [])}
...
def _installed_hub_identifiers(profile: Optional[str]=None) -> dict:
    ...
    try:
        ...
        for entry in lock.list_installed():
            ...
            if ident:
                out[ident] = {'name': entry.get('name'), 'trust_level': entry.get('trust_level'), 'scan_verdict': entry.get('scan_verdict')}
        return out
    except Exception:
        return {}
...
def _profile_setup_command(name: str) -> str:
    ...
    return 'duck-agent setup' if name == 'default' else f'{name} setup'
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local DUCK_AGENT_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List['MCPServerCreate']) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local DUCK_AGENT_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the DUCK_AGENT_HOME override. Returns the
...
from hermes_cli.web_routers.profiles import list_profiles_endpoint, create_profile_endpoint, get_active_profile_endpoint, set_active_profile_endpoint, get_profile_setup_command, open_profile_terminal_endpoint, rename_profile_endpoint, delete_profile_endpoint, get_profile_soul, update_profile_soul, update_profile_description_endpoint, update_profile_model_endpoint, describe_profile_auto_endpoint
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a DUCK_AGENT_HOME override, or under test
    isolation).
    """
    requested = (profile or '').strip()
...
from hermes_cli.web_routers.skills import get_skills, toggle_skill, get_skill_content, create_skill, update_skill_content
...
def _clear_skills_prompt_cache() -> None:
    ...
    try:
        ...
        pass
from hermes_cli.web_routers import tools as _tools_routes
app.include_router(_tools_routes.router)
from hermes_cli.web_routers.tools import get_toolsets, toggle_toolset, get_toolset_config, get_toolset_models, select_toolset_model, select_toolset_provider, save_toolset_env, run_toolset_post_setup, get_terminal_backends, select_terminal_backend, get_computer_use_status, grant_computer_use_permissions
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    """
    if ts_key == 'image_gen':
        return provider_row.get('image_gen_plugin_name') or ('fal' if provider_row.get('imagegen_backend') else None)
    if ts_key == 'video_gen':
...
def _find_toolset_provider_row(ts_key: str, config: dict, provider: Optional[str]) -> Optional[dict]:
    ...
    from hermes_cli.tools_config import TOOL_CATEGORIES, _is_provider_active, _visible_providers
...
def _probe_docker_backend() -> tuple:
    ...
    try:
        proc = subprocess.run(['docker', 'info', '--format', '{{.ServerVersion}}'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=2)
...
def _probe_ssh_backend(terminal_cfg: dict) -> tuple:
    ...
    if missing:
        return ('needs_setup', f"Set {' and '.join(missing)} in config.yaml (or the matching TERMINAL_SSH_* env vars).")
...
def _probe_modal_backend() -> tuple:
    ...
    try:
        ...
        pass
    return ('needs_setup', 'Modal credentials not found — set MODAL_TOKEN_ID and MODAL_TOKEN_SECRET (or run `modal setup`).')
...
def _merge_aux_into_by_model(by_model: List[Dict[str, Any]], aux_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for aux in aux_rows:
        ...
        if target is None:
            target = {'model': model, 'input_tokens': 0, 'output_tokens': 0, 'estimated_cost': 0, 'sessions': 0, 'api_calls': 0}
            merged[model] = target
        ...
        tasks.append({'task': aux.get('task') or '', 'input_tokens': aux.get('input_tokens') or 0, 'output_tokens': aux.get('output_tokens') or 0, 'estimated_cost': aux.get('estimated_cost') or 0, 'api_calls': aux.get('api_calls') or 0})
    result = list(merged.values())
    result.sort(key=lambda r: (r.get('input_tokens') or 0) + (r.get('output_tokens') or 0), reverse=True)
    return result
...
def _aux_task_summary(aux_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for aux in aux_rows:
        ...
        d = by_task.setdefault(task, {'task': task, 'input_tokens': 0, 'output_tokens': 0, 'estimated_cost': 0, 'api_calls': 0, 'models': []})
...
def _get_usage_analytics(days: int=30, profile: Optional[str]=None):
    ...
    try:
        ...
        return {'daily': daily, 'by_model': by_model, 'by_task': _aux_task_summary(aux_rows), 'totals': totals, 'period_days': days, 'skills': usage['skills'], 'tools': usage['tools']}
    finally:
        db.close()

@app.get('/api/analytics/usage')
async def get_usage_analytics(days: int=Query(30, ge=1, le=365), profile: Optional[str]=None):
            ...
            if len(provider_rows) == 1:
                ...
                for row in model_rows:
                    if row is target or row.get('billing_provider'):
                        continue
                    has_usage = any(((row.get(key) or 0) != 0 for key in ('input_tokens', 'output_tokens', 'cache_read_tokens', 'reasoning_tokens', 'estimated_cost', 'actual_cost', 'api_calls', 'tool_calls')))
                    if has_usage:
                        continue
                ...
                rows.append(target)
                rows.extend((r for r in model_rows if r is not target and (r.get('billing_provider') or any(((r.get(key) or 0) != 0 for key in ('input_tokens', 'output_tokens', 'cache_read_tokens', 'reasoning_tokens', 'estimated_cost', 'actual_cost', 'api_calls', 'tool_calls'))))))
            else:
                rows.extend(model_rows)
        rows.sort(key=lambda r: (r.get('input_tokens') or 0) + (r.get('output_tokens') or 0), reverse=True)
        models = []
        for row in rows:
            ...
            try:
                ...
                pass
            models.append({'model': model_name, 'provider': provider, 'input_tokens': row['input_tokens'], 'output_tokens': row['output_tokens'], 'cache_read_tokens': row['cache_read_tokens'], 'reasoning_tokens': row['reasoning_tokens'], 'estimated_cost': row['estimated_cost'], 'actual_cost': row['actual_cost'], 'sessions': row['sessions'], 'api_calls': row['api_calls'], 'tool_calls': row['tool_calls'], 'last_used_at': row['last_used_at'], 'avg_tokens_per_session': row['avg_tokens_per_session'], 'capabilities': caps})
...
async def get_models_analytics(days: int=Query(30, ge=1, le=365), profile: Optional[str]=None):
...
if sys.platform.startswith('win'):
    ...
    try:
        ...
        class PtyUnavailableError(RuntimeError):
            ...
            pass
_RESIZE_RE = re.compile(b'\\x1b\\[RESIZE:(\\d+);(\\d+)\\]')
_PTY_READ_CHUNK_TIMEOUT = 0.2
from hermes_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper
PTY_REGISTRY = PtySessionRegistry(ttl=30 * 60, max_sessions=16, buffer_cap=1 * 1024 * 1024, read_timeout=_PTY_READ_CHUNK_TIMEOUT)
...
async def _legacy_pump(ws: 'WebSocket', bridge) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        await asyncio.to_thread(bridge.close)
_VALID_CHANNEL_RE = re.compile('^[A-Za-z0-9._-]{1,128}$')
...
def _ws_auth_reason(ws: 'WebSocket') -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from hermes_cli.dashboard_auth.ws_tickets import TicketInvalid, consume_internal_credential, consume_ticket
        ...
        if internal:
            try:
                ...
                audit_log(AuditEvent.WS_TICKET_REJECTED, reason=f'internal: {exc}', ip=ws.client.host if ws.client else '', path=ws.url.path)
        ...
        try:
            ...
            audit_log(AuditEvent.WS_TICKET_REJECTED, reason=str(exc), ip=ws.client.host if ws.client else '', path=ws.url.path)
...
def _resolve_chat_argv(resume: Optional[str]=None, sidecar_url: Optional[str]=None, profile: Optional[str]=None, active_session_file: Optional[str]=None) -> tuple[list[str], Optional[str], Optional[dict]]:
    """Resolve the argv + cwd + env for the chat PTY.

    Default: whatever ``duck-agent --tui`` would run.  Tests monkeypatch this
    ...
    ``DUCK_AGENT_HOME`` at the profile dir in the child env. Every spawned
    ...
    — the same propagation ``duck-agent -p <name>`` performs. The in-process
    ...
    if profile_dir is not None:
        env['DUCK_AGENT_HOME'] = str(profile_dir)
    if resume:
        _resume_db = _open_session_db_for_profile(requested if profile_dir is not None else None, read_only=True)
        try:
...
async def _resolve_chat_argv_async(resume: Optional[str]=None, sidecar_url: Optional[str]=None, profile: Optional[str]=None, active_session_file: Optional[str]=None) -> tuple[list[str], Optional[str], Optional[dict]]:
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, 'auth_required', False):
        from hermes_cli.dashboard_auth.ws_tickets import internal_ws_credential
        qs = urllib.parse.urlencode({'internal': internal_ws_credential(), 'channel': channel})
    else:
...
def _active_session_file_for_channel(app: 'FastAPI', channel: str) -> Path:
    ...
    if existing is not None:
        return existing
    fd, raw_path = tempfile.mkstemp(prefix='duck-agent-pty-active-', suffix='.json')
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
def _get_console_executor() -> concurrent.futures.ThreadPoolExecutor:
    ...
    if _console_executor is None:
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(max_workers=_CONSOLE_EXECUTOR_MAX_WORKERS, thread_name_prefix='duck-agent-console')
                atexit.register(lambda: _console_executor and _console_executor.shutdown(wait=False, cancel_futures=True))
...
async def _console_send(ws: WebSocket, send_lock: asyncio.Lock, payload: Dict[str, Any]) -> None:
    async with send_lock:
        await ws.send_json(payload)

async def _console_send_result(ws: WebSocket, send_lock: asyncio.Lock, result: Any, *, command_id: int) -> None:
    command = result.command or ''
    status = result.status
    if status == 'ok':
        if result.output:
            await _console_send(ws, send_lock, {'type': 'output', 'id': command_id, 'stream': 'stdout', 'data': result.output, 'command': command})
        await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'ok', 'command': command, 'prompt': _CONSOLE_PROMPT})
        return
    if status == 'error':
        await _console_send(ws, send_lock, {'type': 'error', 'id': command_id, 'message': result.output or 'Command failed.', 'command': command})
        await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'error', 'command': command, 'prompt': _CONSOLE_PROMPT})
        return
    if status == 'confirm_required':
        await _console_send(ws, send_lock, {'type': 'confirm_required', 'id': command_id, 'command': command, 'message': result.confirmation_message or f'Run `{command}`?', 'prompt': _CONSOLE_PROMPT})
        await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'confirm_required', 'command': command, 'prompt': _CONSOLE_PROMPT})
        return
    if status == 'clear':
        await _console_send(ws, send_lock, {'type': 'clear', 'id': command_id})
        await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'clear', 'command': command, 'prompt': _CONSOLE_PROMPT})
        return
    if status == 'exit':
        await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'exit', 'command': command, 'prompt': ''})
        return
    await _console_send(ws, send_lock, {'type': 'error', 'id': command_id, 'message': f'Unknown console result status: {status}', 'command': command})
...
async def console_ws(ws: WebSocket) -> None:
    ...
    if auth_reason is not None:
        _log.warning('console auth rejected reason=%s mode=%s cred=%s peer=%s', auth_reason, mode, cred, peer)
    ...
    try:
        ...
        await _console_send(ws, send_lock, {'type': 'error', 'message': str(exc.detail), 'prompt': ''})
        await ws.close(code=4400, reason=_ws_close_reason(str(exc.detail)))
        return
    except Exception as exc:
        _log.exception('console failed to initialize')
        await _console_send(ws, send_lock, {'type': 'error', 'message': f'Console unavailable: {exc}', 'prompt': ''})
        await ws.close(code=1011)
        return
    _log.info('console accepted peer=%s mode=%s cred=%s profile=%s', peer, mode, cred, profile or 'current')
    await _console_send(ws, send_lock, {'type': 'ready', 'profile': profile or 'current', 'prompt': _CONSOLE_PROMPT})
    ...
    async def run_command(line: str, *, confirmed: bool, command_id: int) -> None:
        ...
        try:
            loop = asyncio.get_running_loop()
            result = await asyncio.wait_for(loop.run_in_executor(_get_console_executor(), functools.partial(_execute_console_line, engine, line, confirmed=confirmed, profile=profile)), timeout=_CONSOLE_COMMAND_TIMEOUT_SECONDS)
        except asyncio.CancelledError:
            raise
        except asyncio.TimeoutError:
            if command_id == command_generation:
                pending_confirmation = None
                await _console_send(ws, send_lock, {'type': 'error', 'id': command_id, 'message': 'Command timed out. Duck Agent Console returned to the prompt.', 'command': line})
                await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'timeout', 'command': line, 'prompt': _CONSOLE_PROMPT})
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
                _log.exception('console command failed')
                await _console_send(ws, send_lock, {'type': 'error', 'id': command_id, 'message': str(exc) or exc.__class__.__name__, 'command': line})
                await _console_send(ws, send_lock, {'type': 'complete', 'id': command_id, 'status': 'error', 'command': line, 'prompt': _CONSOLE_PROMPT})
        else:
            if command_id != command_generation:
                return
            pending_confirmation = result.command if result.status == 'confirm_required' else None
            await _console_send_result(ws, send_lock, result, command_id=command_id)
    ...
    async def start_command(line: str, *, confirmed: bool=False) -> None:
        ...
        active_task = asyncio.create_task(run_command(line, confirmed=confirmed, command_id=command_id))
    try:
        while True:
            try:
                msg = await ws.receive()
            ...
            if error:
                await _console_send(ws, send_lock, {'type': 'error', 'message': error, 'prompt': _CONSOLE_PROMPT})
                continue
            if payload is None:
                continue
            frame_type = str(payload.get('type') or '').strip().lower()
            if frame_type == 'ping':
                await _console_send(ws, send_lock, {'type': 'pong', 'prompt': _CONSOLE_PROMPT})
                continue
            if frame_type == 'cancel':
                if active_task and (not active_task.done()):
                    ...
                    active_task = None
                    pending_confirmation = None
                    await _console_send(ws, send_lock, {'type': 'complete', 'status': 'cancelled', 'prompt': _CONSOLE_PROMPT})
                elif pending_confirmation:
                    pending_confirmation = None
                    await _console_send(ws, send_lock, {'type': 'complete', 'status': 'cancelled', 'prompt': _CONSOLE_PROMPT})
                else:
                    await _console_send(ws, send_lock, {'type': 'complete', 'status': 'idle', 'prompt': _CONSOLE_PROMPT})
                continue
            if active_task and (not active_task.done()):
                await _console_send(ws, send_lock, {'type': 'error', 'message': 'A console command is already running.', 'prompt': _CONSOLE_PROMPT})
                continue
            if frame_type == 'confirm':
                command = str(payload.get('command') or pending_confirmation or '').strip()
                if not pending_confirmation:
                    await _console_send(ws, send_lock, {'type': 'error', 'message': 'No command is waiting for confirmation.', 'prompt': _CONSOLE_PROMPT})
                    continue
                if command != pending_confirmation:
                    await _console_send(ws, send_lock, {'type': 'error', 'message': 'Confirmation does not match the pending command.', 'prompt': _CONSOLE_PROMPT})
                    continue
                pending_confirmation = None
            ...
            if frame_type in {'input', 'command'}:
                ...
                if not line:
                    await _console_send(ws, send_lock, {'type': 'complete', 'status': 'ok', 'prompt': _CONSOLE_PROMPT})
                    continue
                if pending_confirmation:
                    await _console_send(ws, send_lock, {'type': 'error', 'message': 'Confirm or cancel the pending command before running another one.', 'prompt': _CONSOLE_PROMPT})
                    continue
                await start_command(line)
                continue
            await _console_send(ws, send_lock, {'type': 'error', 'message': f"Unsupported console frame: {frame_type or '?'}", 'prompt': _CONSOLE_PROMPT})
    except WebSocketDisconnect:
        pass
    finally:
        if active_task and (not active_task.done()):
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    mode = _ws_auth_mode()
    if auth_reason is not None:
        _log.warning('pty auth rejected reason=%s mode=%s cred=%s peer=%s', auth_reason, mode, cred, peer)
        await ws.close(code=4401, reason=_ws_close_reason(f'auth: {auth_reason}'))
        return
    host_origin_reason = _ws_host_origin_reason(ws)
    if host_origin_reason is not None:
    ...
    if attach_token is not None and (registry_resume or profile):
        attach_token = f"{attach_token}\x00{profile or ''}\x00{registry_resume or ''}"
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Duck Agent Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return f'<style id="duck-agent-theme-bootstrap">:root{{--background-base:{_esc(bg_hex)};--midground-base:{_esc(mg_hex)};--theme-font-sans:{_esc(font_sans)};--theme-base-size:{_esc(base_size)};}}html,body{{background-color:var(--background-base);color:var(--midground-base);font-family:var(--theme-font-sans);font-size:var(--theme-base-size);}}</style>'
        return ''
    except Exception:
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/duck-agent/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /duck-agent`` on every request. We
    ...
    if _headless or not WEB_DIST.exists():
        _msg = 'Headless backend (duck-agent serve): web UI disabled — use `duck-agent dashboard` for the browser UI.' if _headless else 'Frontend not built. Run: cd web && npm run build'
    ...
    def _serve_index(prefix: str=''):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/duck-agent``)
        ...
        if gated:
            bootstrap_script = f'<script>window.__HERMES_DASHBOARD_EMBEDDED_CHAT__={chat_js};window.__HERMES_BASE_PATH__="{prefix}";window.__HERMES_AUTH_REQUIRED__={gated_js};</script>'
        else:
            bootstrap_script = f'<script>window.__HERMES_SESSION_TOKEN__="{_SESSION_TOKEN}";window.__HERMES_DASHBOARD_EMBEDDED_CHAT__={chat_js};window.__HERMES_BASE_PATH__="{prefix}";window.__HERMES_AUTH_REQUIRED__={gated_js};</script>'
        if prefix:
    ...
    async def serve_css(filename: str, request: Request):
        ...
        if prefix:
            for asset_dir in ('/fonts/', '/fonts-terminal/', '/ds-assets/', '/assets/'):
                ...
                css = css.replace(f'url("{asset_dir}', f'url("{prefix}{asset_dir}')
                css = css.replace(f"url('{asset_dir}", f"url('{prefix}{asset_dir}")
        return Response(content=css, media_type='text/css', headers={'Cache-Control': _IMMUTABLE_ASSET_CACHE_CONTROL})
    ...
    class _ImmutableAssetFiles(StaticFiles):
        ...
        async def get_response(self, path: str, scope):
            ...
            return response
    application.mount('/assets', _ImmutableAssetFiles(directory=WEB_DIST / 'assets'), name='assets')
    ...
    async def serve_spa(full_path: str, request: Request):
        ...
        if full_path and file_path.resolve().is_relative_to(WEB_DIST.resolve()) and file_path.exists() and file_path.is_file():
            return FileResponse(file_path)
        return _serve_index(prefix)
_BUILTIN_DASHBOARD_THEMES = [{'name': 'default', 'label': 'Duck Agent Teal', 'description': 'Classic dark teal — the canonical Duck Agent look'}, {'name': 'default-large', 'label': 'Duck Agent Teal (Large)', 'description': 'Duck Agent Teal with bigger fonts and roomier spacing'}, {'name': 'nous-blue', 'label': 'Nous Blue', 'description': 'Light mode — vivid Nous-blue accents on cream canvas'}, {'name': 'midnight', 'label': 'Midnight', 'description': 'Deep blue-violet with cool accents'}, {'name': 'ember', 'label': 'Ember', 'description': 'Warm crimson and bronze — forge vibes'}, {'name': 'mono', 'label': 'Mono', 'description': 'Clean grayscale — minimal and focused'}, {'name': 'cyberpunk', 'label': 'Cyberpunk', 'description': 'Neon green on black — matrix terminal'}, {'name': 'rose', 'label': 'Rosé', 'description': 'Soft pink and warm ivory — easy on the eyes'}]
...
def _parse_theme_layer(value: Any, default_hex: str, default_alpha: float=1.0) -> Optional[Dict[str, Any]]:
    ...
    return None
_THEME_DEFAULT_TYPOGRAPHY: Dict[str, str] = {'fontSans': 'system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif', 'fontMono': 'ui-monospace, "SF Mono", "Cascadia Mono", Menlo, Consolas, monospace', 'baseSize': '15px', 'lineHeight': '1.55', 'letterSpacing': '0'}
_THEME_DEFAULT_LAYOUT: Dict[str, str] = {'radius': '0.5rem', 'density': 'comfortable'}
_THEME_OVERRIDE_KEYS = {'card', 'cardForeground', 'popover', 'popoverForeground', 'primary', 'primaryForeground', 'secondary', 'secondaryForeground', 'muted', 'mutedForeground', 'accent', 'accentForeground', 'destructive', 'destructiveForeground', 'success', 'warning', 'border', 'input', 'ring'}
_THEME_NAMED_ASSET_KEYS = {'bg', 'hero', 'logo', 'crest', 'sidebar', 'header'}
_THEME_COMPONENT_BUCKETS = {'card', 'header', 'footer', 'sidebar', 'tab', 'progress', 'badge', 'backdrop', 'page'}
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    palette = {'background': _layer('background', '#041c1c', 1.0), 'midground': _layer('midground', '#ffe6cb', 1.0), 'foreground': _layer('foreground', '#ffffff', 0.0), 'warmGlow': palette_src.get('warmGlow') or data.get('warmGlow') or 'rgba(255, 189, 56, 0.35)', 'noiseOpacity': 1.0}
    ...
    if isinstance(custom_assets_src, dict):
        ...
        for key, val in custom_assets_src.items():
            if isinstance(key, str) and key.replace('-', '').replace('_', '').isalnum() and isinstance(val, str) and val.strip():
    ...
    if isinstance(component_styles_src, dict):
        for bucket, props in component_styles_src.items():
            ...
            for prop, value in props.items():
                if isinstance(prop, str) and prop.replace('-', '').replace('_', '').isalnum() and isinstance(value, (str, int, float)) and str(value).strip():
                    clean[prop] = str(value)
            if clean:
                component_styles[bucket] = clean
    layout_variant_src = data.get('layoutVariant')
    layout_variant = layout_variant_src if isinstance(layout_variant_src, str) and layout_variant_src in _THEME_LAYOUT_VARIANTS else 'standard'
    result: Dict[str, Any] = {'name': name, 'label': data.get('label') or name, 'description': data.get('description', ''), 'palette': palette, 'typography': typography, 'layout': layout, 'layoutVariant': layout_variant}
    if color_overrides:
...
def _discover_user_themes() -> list:
    """Scan ~/.duck-agent/dashboard-themes/*.yaml for user-created themes.
    ...
    live under the server's own ``DUCK_AGENT_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.duck-agent/dashboard-themes/*.yaml` ship with their full
...
_FONT_CHOICES = frozenset({'system-sans', 'system-serif', 'system-mono', 'inter', 'ibm-plex-sans', 'work-sans', 'atkinson-hyperlegible', 'dm-sans', 'spectral', 'fraunces', 'source-serif', 'jetbrains-mono', 'ibm-plex-mono', 'space-mono'})
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.duck-agent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.duck-agent/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    from hermes_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [(get_process_hermes_home() / 'plugins', 'user'), (bundled_root / 'memory', 'bundled'), (bundled_root, 'bundled')]
    if env_var_enabled('HERMES_ENABLE_PROJECT_PLUGINS'):
        search_dirs.append((Path.cwd() / '.duck-agent' / 'plugins', 'project'))
        ...
        for child in sorted(plugins_root.iterdir()):
            ...
            try:
                ...
                if raw_api and safe_api is None:
                    _log.warning("Plugin %s: refusing unsafe api path %r (must be a relative file inside the plugin's dashboard/ directory); backend routes from this plugin will not be mounted", name, raw_api)
                plugins.append({'name': name, 'label': data.get('label', name), 'description': data.get('description', ''), 'icon': data.get('icon', 'Puzzle'), 'version': data.get('version', '0.0.0'), 'tab': tab_info, 'slots': slots, 'entry': data.get('entry', 'dist/index.js'), 'css': data.get('css'), 'has_api': bool(safe_api), 'source': source, '_dir': str(dashboard_dir), '_api_file': safe_api})
            except Exception as exc:
    ...
    def _is_active(p: dict) -> bool:
        ...
        if p.get('source') == 'user':
            ...
            if name in disabled_set:
                return False
        return True
    return [{k: v for k, v in p.items() if not k.startswith('_')} for p in plugins if _is_active(p)]
...
def _merged_plugins_hub(force_refresh: bool=False) -> Dict[str, Any]:
    ...
    from hermes_cli.plugins_cmd import _discover_all_plugins, _get_current_context_engine, _get_current_memory_provider, _discover_context_engines, _get_disabled_set, _get_enabled_set, _read_manifest as _read_plugin_manifest_at
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        ...
        try:
            ...
            pass
        can_remove_update = source in {'user', 'git'} and under_user_tree and Path(dir_str).is_dir()
        auth_required = False
        auth_command = ''
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f'duck-agent auth {name}'
                        break
            except Exception:
                pass
    ...
    try:
        ...
        context_engines = []
    payload = {'plugins': rows, 'orphan_dashboard_plugins': orphan_dashboard, 'providers': {'memory_provider': _normalize_memory_provider_name(_get_current_memory_provider()), 'memory_options': memory_providers, 'context_engine': _get_current_context_engine(), 'context_options': context_engines}}
    duration = time.monotonic() - started_at
    if duration >= 0.25:
        _log.info('plugins/hub rebuilt in %.3fs (plugins=%d memory_options=%d)', duration, len(rows), len(memory_providers))
    with _plugins_hub_cache_lock:
...
async def serve_plugin_asset(plugin_name: str, file_path: str):
    ...
    content_types = {'.js': 'application/javascript', '.mjs': 'application/javascript', '.css': 'text/css', '.json': 'application/json', '.html': 'text/html', '.svg': 'image/svg+xml', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp', '.ico': 'image/x-icon', '.woff2': 'font/woff2', '.woff': 'font/woff', '.ttf': 'font/ttf', '.otf': 'font/otf', '.map': 'application/json'}
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.duck-agent/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get('source') == 'user':
            if plugin_name in disabled_set:
                _log.debug('Plugin %s: skipping API mount (explicitly disabled)', plugin_name)
                continue
            if plugin_name not in enabled_set:
                _log.debug('Plugin %s: skipping API mount (not in plugins.enabled)', plugin_name)
                continue
        elif plugin.get('source') == 'bundled':
            if plugin_name in disabled_set:
                _log.debug('Plugin %s: skipping API mount (explicitly disabled)', plugin_name)
        ...
        if plugin.get('source') == 'project':
            _log.warning('Plugin %s: ignoring backend api=%s (project plugins may not auto-import Python code; move the plugin to ~/.duck-agent/plugins/ if you trust it)', plugin['name'], api_file_name)
            continue
        ...
        try:
            ...
            _log.warning('Plugin %s: refusing to import api file outside its dashboard directory (%s)', plugin['name'], api_path)
            continue
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile('w', encoding='utf-8', dir=str(path.parent), prefix=f'{path.name}.', suffix='.tmp', delete=False) as fh:
            fh.write(payload)
            fh.flush()
            os.fsync(fh.fileno())
            tmp_name = fh.name
...
def _maybe_open_browser(host: str, actual_port: int, open_browser: bool, initial_profile: str) -> None:
    ...
    if not open_browser:
        return
    import webbrowser
    _has_display = sys.platform != 'linux' or bool(os.environ.get('DISPLAY')) or bool(os.environ.get('WAYLAND_DISPLAY'))
    if not _has_display:
        _log.debug('Skipping browser-open: no DISPLAY or WAYLAND_DISPLAY detected (headless Linux). Pass --no-open to suppress this detection.')
        return
...
def start_server(host: str='127.0.0.1', port: int=9119, open_browser: bool=True, allow_public: bool=False, initial_profile: str='', headless: bool=False, ssh_session_token: Optional[str]=None, ssh_owner_nonce: Optional[str]=None):
    ...
    if allow_public and host not in _LOOPBACK_HOST_VALUES:
        _log.warning('--insecure no longer bypasses dashboard authentication. A non-loopback bind (%s) now ALWAYS requires an auth provider (OAuth or the bundled password provider). Configure one — see below — or bind to 127.0.0.1 and reach it over an SSH tunnel / Tailscale.', host)
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                ...
                _has_creds = bool(_ba.get('username')) and bool(_ba.get('password_hash') or _ba.get('password'))
                if _has_creds and set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS:
                    _fix_hint = "The 'basic' dashboard-auth plugin is in plugins.disabled but dashboard.basic_auth is configured.\nRemove 'basic' from plugins.disabled (or run `duck-agent plugins enable basic`), then restart the dashboard.\n\n" + _fix_hint
            except Exception:
                pass
            if skip_reasons:
                raise SystemExit(f'Refusing to bind dashboard to {host} — the auth gate engages on non-loopback binds, but no auth providers are registered.\n\nBundled providers reported these issues:\n' + '\n'.join(skip_reasons) + '\n\n' + _fix_hint)
            raise SystemExit(f'Refusing to bind dashboard to {host} — the auth gate engages on non-loopback binds, but no auth providers are registered.\n\n' + _fix_hint)
        _log.info('Dashboard binding to %s with auth gate enabled. Providers: %s', host, ', '.join((p.name for p in list_providers())))
    app.state.bound_host = host
    _is_loopback = host in ('127.0.0.1', 'localhost', '::1')
    config = uvicorn.Config(app, host=host, port=port, log_level='warning', proxy_headers=bool(app.state.auth_required), ws_ping_interval=None if _is_loopback else 20.0, ws_ping_timeout=None if _is_loopback else 20.0, ws_max_size=_DESKTOP_ATTACHMENT_WS_MAX_BYTES)
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                print(f'  Duck Agent backend listening on {host}:{actual_port}')
            else:
                print(f'  Duck Agent Web UI → http://{host}:{actual_port}')
