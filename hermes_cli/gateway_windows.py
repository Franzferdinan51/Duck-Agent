...
from hermes_cli._subprocess_compat import windows_detach_flags, windows_detach_flags_without_breakaway, windows_hide_flags
_SCHTASKS_TIMEOUT_S = 15
_SCHTASKS_NO_OUTPUT_TIMEOUT_S = 30
_FALLBACK_PATTERNS = re.compile('(access is denied|acceso denegado|přístup byl odepřen|schtasks timed out|schtasks produced no output)', re.IGNORECASE)
_ACCESS_DENIED_PATTERN = re.compile('(access is denied|acceso denegado)', re.IGNORECASE)
_TASK_NAME_DEFAULT = 'Hermes_Gateway'
_TASK_DESCRIPTION = 'Duck Agent Gateway - Messaging Platform Integration'
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Duck Agent-owned paths under the configured DUCK_AGENT_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\duck-agent`` as a symlink/junction to
    ...
    path lives under DUCK_AGENT_HOME.
    """
    candidate = Path(path)
    try:
...
def _exec_schtasks(args: list[str]) -> tuple[int, str, str]:
    ...
    try:
        proc = subprocess.run([schtasks, *args], capture_output=True, text=True, encoding=_schtasks_encoding(), errors='replace', timeout=_SCHTASKS_TIMEOUT_S, creationflags=windows_hide_flags())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Duck Agent profile."""
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None=None) -> bool:
    ...
    try:
        result = ctypes.windll.shell32.ShellExecuteW(None, 'runas', elevated_python, params, cwd, 0)
...
def _launch_elevated_install(force: bool=False, *, start_now: bool | None=None, start_on_login: bool | None=None) -> bool:
    ...
    try:
        ...
        for key, old in (('HERMES_GATEWAY_INSTALL_START_NOW', old_start_now), ('HERMES_GATEWAY_INSTALL_START_ON_LOGIN', old_start_on_login), ('HERMES_GATEWAY_ELEVATED_HANDOFF', old_handoff)):
            if old is None:
...
def _sanitize_filename(value: str) -> str:
    ...
    return re.sub('[<>:"/\\\\|?*\\x00-\\x1f]', '_', value)

def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper kept beside the VBS launcher.

    Lives under ``%LOCALAPPDATA%\\duck-agent\\gateway-service\\<task_name>.cmd``
    (or ``<DUCK_AGENT_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Duck Agent installs stay self-contained).
    """
    _assert_windows()
    from hermes_cli.config import get_hermes_home
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``DUCK_AGENT_HOME`` whenever it
    ...
    source checkout only if ``DUCK_AGENT_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(python_path: str, working_dir: str, hermes_home: str, profile_arg: str) -> str:
    ...
    The script:
      ...
      - exports DUCK_AGENT_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    to that terminal like a normal foreground ``duck-agent gateway run``.
    ...
    lines.append(f'set "DUCK_AGENT_HOME={hermes_home}"')
    ...
    pythonpath_entries = [_preserve_hermes_home_path(Path(__file__).resolve().parent.parent), *[_preserve_hermes_home_path(entry) for entry in extra_pythonpath]]
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, hermes_home: str, profile_arg: str) -> str:
    ...
    static_pythonpath = os.pathsep.join([repo_root, *[_preserve_hermes_home_path(entry) for entry in extra_pythonpath]])
    lines = [f"' {_TASK_DESCRIPTION}", 'Option Explicit', 'Dim sh, env, existing_pp', 'Set sh = CreateObject("WScript.Shell")', 'Set env = sh.Environment("PROCESS")', f"env.Item({_quote_vbs_string('DUCK_AGENT_HOME')}) = {_quote_vbs_string(hermes_home)}", f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}", f"env.Item({_quote_vbs_string('HERMES_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}", f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_hermes_home_path(venv_dir))}", f"existing_pp = env.Item({_quote_vbs_string('PYTHONPATH')})", 'If Len(existing_pp) > 0 Then', f"  env.Item({_quote_vbs_string('PYTHONPATH')}) = {_quote_vbs_string(static_pythonpath + os.pathsep)} & existing_pp", 'Else', f"  env.Item({_quote_vbs_string('PYTHONPATH')}) = {_quote_vbs_string(static_pythonpath)}", 'End If', f'sh.CurrentDirectory = {_quote_vbs_string(working_dir)}', f'sh.Run {_quote_vbs_string(command_line)}, 0, False']
    return '\r\n'.join(lines) + '\r\n'
...
def _build_startup_launcher(script_path: Path) -> str:
    ...
    lines = [f"' {_TASK_DESCRIPTION}", 'Option Explicit', 'Dim fso, sh, target', f'target = {_quote_vbs_string(target)}', 'Set fso = CreateObject("Scripting.FileSystemObject")', 'If Not fso.FileExists(target) Then WScript.Quit 0', 'Set sh = CreateObject("WScript.Shell")', f'sh.Run {_quote_vbs_string(command)}, 0, False']
    return '\r\n'.join(lines) + '\r\n'
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Duck Agent builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    from hermes_cli.config import get_hermes_home
    from hermes_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(_preserve_hermes_home_path(get_python_path()))
    ...
    env_overlay = {'DUCK_AGENT_HOME': hermes_home, 'PYTHONIOENCODING': 'utf-8', 'HERMES_GATEWAY_DETACHED': '1', 'VIRTUAL_ENV': _preserve_hermes_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [project_root, *[_preserve_hermes_home_path(entry) for entry in extra_pythonpath]] if extra_pythonpath else [project_root])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    supplies the stable cwd + env overlay (DUCK_AGENT_HOME, VIRTUAL_ENV,
    ...
    rest = run_argv[1:]
    try:
        hidden_console_python, venv_dir, extra_pythonpath = _resolve_detached_python(python_exe)
    except Exception:
        return (run_argv, '', {})
    ...
    if hermes_home:
        env_overlay['DUCK_AGENT_HOME'] = hermes_home
...
def _spawn_detached(script_path: Path | None=None) -> int:
    ...
    try:
        with open(stray_log, 'ab', buffering=0) as log_fh:
            proc = subprocess.Popen(argv, cwd=working_dir, env=env, creationflags=flags, close_fds=True, stdin=subprocess.DEVNULL, stdout=log_fh, stderr=log_fh)
    except OSError:
        flags_no_breakaway = windows_detach_flags_without_breakaway()
        with open(stray_log, 'ab', buffering=0) as log_fh:
            proc = subprocess.Popen(argv, cwd=working_dir, env=env, creationflags=flags_no_breakaway, close_fds=True, stdin=subprocess.DEVNULL, stdout=log_fh, stderr=log_fh)
    return proc.pid
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f'duck-agent {profile_arg} gateway start' if profile_arg else 'duck-agent gateway start'
...
def install(force: bool=False, *, start_now: bool | None=None, start_on_login: bool | None=None, elevated_handoff: bool=False) -> None:
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print('  Run later with: duck-agent gateway start')
        return
    ...
    if not _is_running_as_admin() and (not elevated_handoff):
        ...
        if prompt_yes_no('  Open the UAC prompt now?', False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print('✓ Launched elevated Duck Agent gateway install prompt.')
                if start_now:
                    print('  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.')
                else:
                    print('  Approve the Windows UAC prompt, then run: duck-agent gateway status')
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print('  Start manually with: duck-agent gateway start')
        _print_next_steps()
        return
...
def uninstall() -> None:
        ...
        if code == 0:
            ...
            if prompt_yes_no('  Open the UAC prompt now?', False):
                if _launch_elevated_uninstall():
                    print('✓ Launched elevated Duck Agent gateway uninstall prompt.')
                    print('  Approve the Windows UAC prompt, then run: duck-agent gateway status')
                    return
    ...
    for path, label in [(startup_entry, 'Windows login item'), (legacy_startup_entry, 'legacy Windows login item'), (script_path, 'Task script'), (vbs_script_path, 'Task launcher')]:
        try:
            path.unlink()
...
def start() -> None:
    ...
    if not task_installed and (not startup_installed):
        ...
        if not prompt_yes_no('  Install it now so the gateway starts on login?', True):
            print('  Run: duck-agent gateway install')
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and (not startup_installed):
            ...
            print('  If a UAC prompt opened, approve it, then run: duck-agent gateway start')
            return
    pid = _spawn_detached()
    _report_gateway_start(f'direct spawn (PID {pid})')
...
def restart() -> None:
    ...
    if not _wait_for_gateway_absent(timeout_s=30.0):
        ...
        if not _wait_for_gateway_absent(timeout_s=10.0):
            raise RuntimeError('Gateway process still detected after force kill; refusing to start a duplicate. Investigate stray PIDs before retrying.')
    time.sleep(1.0)
    start()
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError('Gateway restart did not produce a running gateway process. Check logs/gateway.log and run `duck-agent gateway status`.')