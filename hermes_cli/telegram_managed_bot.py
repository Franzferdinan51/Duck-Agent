...
manual BotFather token copy-paste. Duck Agent talks only to the Nous onboarding
...
def generate_deep_link(manager_bot: str=DEFAULT_MANAGER_BOT, suggested_username: Optional[str]=None, suggested_name: Optional[str]=None) -> str:
    ...
    base_url = f'https://t.me/newbot/{urllib.parse.quote(manager)}/{urllib.parse.quote(username)}'
    if suggested_name:
...
def create_pairing(api_url: str | None=None, bot_name: str=DEFAULT_BOT_NAME, timeout: float=10.0) -> TelegramPairing | None:
    ...
    return TelegramPairing(pairing_id=data['pairing_id'], poll_token=data['poll_token'], suggested_username=data['suggested_username'], deep_link=data['deep_link'], qr_payload=qr_payload, expires_at=expires_at if isinstance(expires_at, str) else None)
...
def poll_pairing_result_once(api_url: str | None, pairing: TelegramPairing, timeout: float=10.0) -> TelegramBotSetupResult | None:
    ...
    resp = httpx.get(f'{_api_url(api_url)}/v1/telegram/pairings/{pairing.pairing_id}', headers={'Authorization': f'Bearer {pairing.poll_token}'}, timeout=timeout)
    ...
    return TelegramBotSetupResult(token=token, bot_username=bot_username if isinstance(bot_username, str) and bot_username else None, owner_user_id=_parse_owner_user_id(data.get('owner_user_id')))
...
def auto_setup_telegram_bot_result(api_url: str | None=None, manager_bot: str=DEFAULT_MANAGER_BOT, profile_name: Optional[str]=None, poll_timeout: float=DEFAULT_POLL_TIMEOUT) -> Optional[TelegramBotSetupResult]:
    ...
    print()
    print(f'  Contacting Duck Agent Telegram onboarding service: {resolved_api_url}')
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print('  ✗ Could not reach the Duck Agent Telegram onboarding service.')
...
def auto_setup_telegram_bot(api_url: str | None=None, manager_bot: str=DEFAULT_MANAGER_BOT, profile_name: Optional[str]=None, poll_timeout: float=DEFAULT_POLL_TIMEOUT) -> Optional[str]:
    ...
    result = auto_setup_telegram_bot_result(api_url=api_url, manager_bot=manager_bot, profile_name=profile_name, poll_timeout=poll_timeout)
    return result.token if result else None