...
_COMMIT_CONTEXT_DIFF_MAX_CHARS = 120000
...
def _git(cwd: str, args: list[str], *, timeout: int=_GIT_TIMEOUT) -> tuple[int, str, str]:
    ...
    try:
        proc = subprocess.run(['git', *args], cwd=cwd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, stdin=subprocess.DEVNULL, env=noninteractive_git_env())
...
def _classify(tag: str, xy: str, path: str) -> dict:
    ...
    return {'path': path, 'staged': _entry_staged(tag, xy), 'unstaged': tag == '?' or (tag in ('1', '2') and y not in ('.', '?')), 'untracked': tag == '?', 'conflicted': tag == 'u'}
...
def review_list(cwd: str, scope: str, base_ref: str | None) -> dict:
    ...
    if scope in ('branch', 'lastTurn'):
        ...
        files = [{'path': path, 'added': a, 'removed': r, 'status': 'M', 'staged': False} for path, (a, r) in _numstat(cwd, [rng]).items()]
        if scope == 'lastTurn':
    ...
    for tag, xy, path in _walk_entries(raw):
        ...
        files.append({'path': path, 'added': sa + ua, 'removed': sr + ur, 'status': _status_letter(tag, xy), 'staged': _entry_staged(tag, xy)})
    files.sort(key=lambda f: f['path'])
    _fill_untracked_counts(cwd, files)
...
def _gh(cwd: str, args: list[str]) -> tuple[bool, str]:
    ...
    try:
        proc = subprocess.run(['gh', *args], cwd=cwd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=_GH_TIMEOUT, stdin=subprocess.DEVNULL, env=env)
    except (OSError, subprocess.SubprocessError):
        return (False, '')
...
def worktree_list(cwd: str) -> list[dict]:
    ...
    if not out:
        return []
    return [{'path': tree['path'], 'branch': tree['branch'], 'isMain': index == 0, 'detached': tree['detached'], 'locked': tree['locked']} for index, tree in enumerate(_parse_worktrees(out))]
...
def _slugify(name: str) -> str:
    slug = re.sub('[^a-z0-9]+', '-', str(name or '').strip().lower())
    slug = re.sub('^-+|-+$', '', slug)[:40].rstrip('-')
    return slug or 'work'

def _default_branch(cwd: str) -> str:
    remote = _git_out(cwd, ['symbolic-ref', '--quiet', '--short', 'refs/remotes/origin/HEAD']).strip().replace('origin/', '', 1)
    if remote:
        return remote
...
def _ensure_repo(cwd: str) -> None:
    ...
    if inside != 'true':
        ...
        needs_root = code != 0
    if needs_root:
        _git_ok(cwd, ['-c', 'user.email=duck-agent@localhost', '-c', 'user.name=Duck Agent', 'commit', '--allow-empty', '-m', 'Initial commit'])
...
def worktree_add(cwd: str, options: dict) -> dict:
    ...
    branch = _sanitize_branch(options.get('branch') or '') or f'duck-agent/{slug}'
...
def branch_list(cwd: str) -> list[dict]:
    out = _git_out(cwd, ['for-each-ref', '--format=%(refname:short)', '--sort=-committerdate', 'refs/heads'])
    if not out:
        return []
    trees = worktree_list(cwd)
    path_by_branch = {t['branch']: t['path'] for t in trees if t['branch']}
    trunk = _default_branch(cwd)
    return [{'name': name, 'checkedOut': name in path_by_branch, 'isDefault': bool(trunk and name == trunk), 'worktreePath': path_by_branch.get(name)} for name in (line.strip() for line in out.split('\n')) if name]
...
def base_branch_list(cwd: str) -> list[dict]:
    ...
    """
    out = _git_out(cwd, ['for-each-ref', '--format=%(refname:short)\t%(committerdate:iso)', '--sort=-committerdate', 'refs/heads', 'refs/remotes'])
    if not out:
        return []
    remote_default = _git_out(cwd, ['symbolic-ref', '--quiet', '--short', 'refs/remotes/origin/HEAD']).strip()
    ...
    for line in out.split('\n'):
        ...
        if not line:
            continue
        name = line.split('\t')[0]
        result.append({'name': name, 'isRemote': name.startswith('origin/'), 'isDefault': bool(remote_default and name == remote_default or (not remote_default and local_default and (name == local_default)))})
    return result