"""Profile distributions — shareable, packaged Duck Agent profiles via git.

A distribution is a Duck Agent profile published as a git repository (or
...
* ``duck-agent profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``duck-agent skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``duck-agent profile``, not a parallel tree):

    duck-agent profile install <source> [--name N] [--alias] [--force] [--yes]
    duck-agent profile update  <name>  [--force-config] [--yes]
    duck-agent profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({'auth.json', '.env', 'state.db', 'state.db-shm', 'state.db-wal', 'hermes_state.db', 'response_store.db', 'response_store.db-shm', 'response_store.db-wal', 'gateway.pid', 'gateway_state.json', 'processes.json', 'auth.lock', 'active_profile', '.update_check', 'errors.log', '.hermes_history', 'memories', 'sessions', 'logs', 'plans', 'workspace', 'home', 'image_cache', 'audio_cache', 'document_cache', 'browser_screenshots', 'checkpoints', 'sandboxes', 'backups', 'cache', 'duck-agent', '.worktrees', 'profiles', 'bin', 'node_modules', 'local'})
...
class EnvRequirement:
    ...
    def from_dict(cls, data: Any) -> 'EnvRequirement':
        ...
        return cls(name=name, description=str(data.get('description') or ''), required=bool(data.get('required', True)), default=data.get('default'))
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> 'DistributionManifest':
        ...
        return cls(name=name, version=str(data.get('version') or '0.1.0'), description=str(data.get('description') or ''), hermes_requires=str(data.get('hermes_requires') or ''), author=str(data.get('author') or ''), license=str(data.get('license') or ''), env_requires=env_requires, distribution_owned=distribution_owned, source=str(data.get('source') or ''), installed_at=str(data.get('installed_at') or ''))
...
def write_manifest(profile_dir: Path, manifest: DistributionManifest) -> Path:
    ...
    atomic_yaml_write(mf_path, manifest.to_dict(), sort_keys=False, default_flow_style=False, create_mode=420)
    return mf_path
_VERSION_OP_RE = re.compile('^\\s*(>=|<=|==|!=|>|<)\\s*(.+?)\\s*$')
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    ok = {'>=': cur >= tgt, '<=': cur <= tgt, '==': cur == tgt, '!=': cur != tgt, '>': cur > tgt, '<': cur < tgt}[op]
    if not ok:
        raise DistributionError(f'This distribution requires Duck Agent {op}{target}, but you have {current_version}.')

def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    """Generate a ``.env.template`` body from env_requires."""
    lines = ['# Environment variables required by this Duck Agent distribution.', '# Copy to `.env` and fill in your own values before running.', '']
...
def _looks_like_git_url(s: str) -> bool:
    ...
    if s.startswith(('http://', 'https://')):
        return True
    if re.match('^github\\.com/[\\w.-]+/[\\w.-]+/?$', s):
        return True
    return False

def _git_clone(url: str, dest: Path) -> None:
    if re.match('^github\\.com/[\\w.-]+/[\\w.-]+/?$', url):
        url = f"https://{url.rstrip('/')}"
    try:
        subprocess.run(['git', 'clone', '--depth', '1', url, str(dest)], check=True, capture_output=True, stdin=subprocess.DEVNULL, env=noninteractive_git_env())
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``duck-agent profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(f'No {MANIFEST_FILENAME} at the root of {src_str!r}. This repository is not a Duck Agent profile distribution.')
        return (cloned, src_str)
    path_guess = Path(src_str).expanduser()
    if path_guess.is_dir():
        if not (path_guess / MANIFEST_FILENAME).is_file():
            raise DistributionError(f'No {MANIFEST_FILENAME} in {path_guess}. A local-directory source must contain a distribution.yaml at its root.')
        return (path_guess.resolve(), str(path_guess.resolve()))
    raise DistributionError(f'Cannot resolve distribution source: {source!r}. Expected a git URL (e.g. github.com/user/repo) or a local directory.')
...
def plan_install(source: str, workdir: Path, override_name: Optional[str]=None) -> InstallPlan:
    ...
    if manifest is None:
        raise DistributionError(f'No {MANIFEST_FILENAME} found at the distribution root — this source is not a Duck Agent distribution.')
    ...
    if canon == 'default':
        raise DistributionError("Cannot install a distribution as 'default' — that is the built-in root profile (~/.duck-agent).  Pass --name <name> to install under a new profile.")
    manifest.name = canon
    ...
    return InstallPlan(manifest=manifest, staged_dir=staged, provenance=provenance, target_dir=target_dir, existing=existing, preserves_config=existing, has_cron=has_cron, has_skills=skill_count > 0)
...
def _copy_dist_payload(staged: Path, target: Path, manifest: DistributionManifest, preserve_config: bool) -> None:
    ...
    def _copy_entry(entry: Path, dest: Path) -> None:
        if entry.is_dir():
            ...
            shutil.copytree(entry, dest, ignore=lambda d, names: [n for n in names if n in USER_OWNED_EXCLUDE] if Path(d).resolve() == staged_resolved else [])
        else:
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    for d in ('memories', 'sessions', 'skills', 'skins', 'logs', 'plans', 'workspace', 'cron', 'home'):
...
def install_distribution(source: str, name: Optional[str]=None, force: bool=False, create_alias: bool=False) -> InstallPlan:
    ...
    with tempfile.TemporaryDirectory(prefix='hermes_dist_install_') as tmp:
        ...
        if plan.existing and (not force):
            raise DistributionError(f"Profile '{plan.manifest.name}' already exists at {plan.target_dir}. Use `duck-agent profile update` to upgrade in place, or pass --force to overwrite.")
...
def update_distribution(profile_name: str, force_config: bool=False) -> InstallPlan:
    ...
    if existing_manifest is None:
        raise DistributionError(f"Profile '{canon}' is not a distribution (no {MANIFEST_FILENAME}). Only profiles installed via `duck-agent profile install` can be updated.")
    if not existing_manifest.source:
        raise DistributionError(f"Profile '{canon}' has no recorded source.  Re-install with `duck-agent profile install <source> --name {{canon}} --force`.")
