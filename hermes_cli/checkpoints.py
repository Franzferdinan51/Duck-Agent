"""`duck-agent checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.duck-agent/checkpoints/``.  Actions:

    duck-agent checkpoints               # same as `status`
    duck-agent checkpoints status        # total size, project count, breakdown
    duck-agent checkpoints list          # per-project checkpoint counts + workdir
    duck-agent checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    duck-agent checkpoints clear [-f]    # nuke the entire base (asks first)
    duck-agent checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    duck-agent checkpoints
    duck-agent checkpoints prune --retention-days 3 --max-size-mb 200
    duck-agent checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print('Clear with: duck-agent checkpoints clear-legacy')
    return 0
...
def cmd_prune(args: argparse.Namespace) -> int:
    ...
    if delete_orphans and (not args.force):
        ...
        if orphans or pre_v2_orphans:
            print(f'This will permanently delete {len(orphans) + len(pre_v2_orphans)} orphan checkpoint project(s) whose workdir is not currently reachable:')
            print()
            for p in orphans:
                wd = p.get('workdir') or '(unknown)'
    ...
    print()
    result = prune_checkpoints(retention_days=retention_days, delete_orphans=delete_orphans, max_total_size_mb=max_size_mb, orphan_allowlist=orphan_allowlist)
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``duck-agent checkpoints`` parser."""
    ...
    p_prune.add_argument('--retention-days', type=int, default=7, help='Drop projects whose last_touch is older than N days (default 7)')
    p_prune.add_argument('--max-size-mb', type=int, default=500, help='After orphan/stale prune, drop oldest commits per project until total size <= this (default 500)')
    p_prune.add_argument('--keep-orphans', action='store_true', help='Skip deleting projects whose workdir no longer exists')
    p_prune.add_argument('-f', '--force', action='store_true', help='Skip the orphan-deletion confirmation prompt')
    p_prune.set_defaults(func=cmd_prune)
    p_clear = subs.add_parser('clear', help='Delete the entire checkpoint base (all /rollback history)')
    p_clear.add_argument('-f', '--force', action='store_true', help='Skip confirmation prompt')
    p_clear.set_defaults(func=cmd_clear)
    p_legacy = subs.add_parser('clear-legacy', help='Delete only the legacy-<ts>/ archives from v1 migration')
    p_legacy.add_argument('-f', '--force', action='store_true', help='Skip confirmation prompt')
