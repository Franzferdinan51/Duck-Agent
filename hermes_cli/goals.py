"""Persistent session goals — the Ralph loop for Duck Agent.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Duck Agent feeds a
...
class GoalGate:
    ...
    def from_dict(cls, data: Optional[Dict[str, Any]]) -> 'GoalGate':
        if not isinstance(data, dict):
            return cls(command='')
        return cls(command=str(data.get('command') or ''), timeout_seconds=int(data.get('timeout_seconds', DEFAULT_GATE_TIMEOUT_SECONDS) or DEFAULT_GATE_TIMEOUT_SECONDS), max_retries=int(data.get('max_retries', DEFAULT_GATE_MAX_RETRIES) or DEFAULT_GATE_MAX_RETRIES), attempts=int(data.get('attempts', 0) or 0), last_exit_code=int(data['last_exit_code']) if data.get('last_exit_code') is not None else None, last_output_tail=str(data.get('last_output_tail') or ''), last_failed_fingerprint=str(data.get('last_failed_fingerprint') or ''))
...
def workspace_fingerprint(cwd: Optional[str]=None) -> str:
    ...
    try:
        head = subprocess.run(['git', 'rev-parse', 'HEAD'], capture_output=True, text=True, timeout=10, cwd=workdir)
        if head.returncode != 0:
            return ''
        status = subprocess.run(['git', 'status', '--porcelain'], capture_output=True, text=True, timeout=30, cwd=workdir)
...
def run_gate(gate: GoalGate, *, cwd: Optional[str]=None) -> Tuple[bool, int, str]:
    ...
    """
    try:
        proc = subprocess.run(gate.command, shell=True, capture_output=True, text=True, timeout=max(1, int(gate.timeout_seconds)), cwd=cwd or None)
...
class GoalState:
    ...
    def from_json(cls, raw: str) -> 'GoalState':
        ...
        return cls(goal=data.get('goal', ''), status=data.get('status', 'active'), turns_used=int(data.get('turns_used', 0) or 0), max_turns=int(data.get('max_turns', DEFAULT_MAX_TURNS) or DEFAULT_MAX_TURNS), created_at=float(data.get('created_at', 0.0) or 0.0), last_turn_at=float(data.get('last_turn_at', 0.0) or 0.0), last_verdict=data.get('last_verdict'), last_reason=data.get('last_reason'), paused_reason=data.get('paused_reason'), consecutive_parse_failures=int(data.get('consecutive_parse_failures', 0) or 0), consecutive_transport_failures=int(data.get('consecutive_transport_failures', 0) or 0), subgoals=subgoals, waiting_on_pid=int(data['waiting_on_pid']) if data.get('waiting_on_pid') else None, waiting_on_session=str(data['waiting_on_session']) if data.get('waiting_on_session') else None, waiting_until=float(data.get('waiting_until', 0.0) or 0.0), waiting_reason=data.get('waiting_reason'), waiting_since=float(data.get('waiting_since', 0.0) or 0.0), contract=GoalContract.from_dict(data.get('contract')), gates=[GoalGate.from_dict(g) for g in data.get('gates') or [] if isinstance(g, dict) and str(g.get('command') or '').strip()])
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current DUCK_AGENT_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``duck_agent_home`` path so profile switches still pick up the right DB.
...
def _session_waiting(session_id: str) -> bool:
    ...
    try:
        ...
        return False
_JSON_OBJECT_RE = re.compile('\\{.*?\\}', re.DOTALL)
...
def _goal_judge_max_tokens() -> int:
    ...
    try:
        ...
        cfg = load_config()
        value = (cfg.get('auxiliary') or {}).get('goal_judge', {}).get('max_tokens', DEFAULT_JUDGE_MAX_TOKENS)
        value = int(value)
        if value > 0:
            return value
    except Exception:
...
def judge_goal(goal: str, last_response: str, *, timeout: float=DEFAULT_JUDGE_TIMEOUT, subgoals: Optional[List[str]]=None, background_processes: Optional[List[Dict[str, Any]]]=None, contract: Optional[GoalContract]=None) -> Tuple[str, str, bool, Optional[Dict[str, Any]], bool]:
    ...
    if contract is not None and (not contract.is_empty()):
        ...
        if clean_subgoals:
            extra = '\n'.join((f'- Extra criterion {i}: {text}' for i, text in enumerate(clean_subgoals, start=1)))
            contract_block = f'{contract_block}\n{extra}'
        prompt = JUDGE_USER_PROMPT_WITH_CONTRACT_TEMPLATE.format(goal=_truncate(goal, 2000), contract_block=_truncate(contract_block, 2500), response=_truncate(last_response, _JUDGE_RESPONSE_SNIPPET_CHARS), background_block=background_block, current_time=current_time)
    elif clean_subgoals:
        subgoals_block = '\n'.join((f'- {i}. {text}' for i, text in enumerate(clean_subgoals, start=1)))
        prompt = JUDGE_USER_PROMPT_WITH_SUBGOALS_TEMPLATE.format(goal=_truncate(goal, 2000), subgoals_block=_truncate(subgoals_block, 2000), response=_truncate(last_response, _JUDGE_RESPONSE_SNIPPET_CHARS), background_block=background_block, current_time=current_time)
    else:
        prompt = JUDGE_USER_PROMPT_TEMPLATE.format(goal=_truncate(goal, 2000), response=_truncate(last_response, _JUDGE_RESPONSE_SNIPPET_CHARS), background_block=background_block, current_time=current_time)
    try:
        resp = call_llm(task='goal_judge', messages=[{'role': 'system', 'content': JUDGE_SYSTEM_PROMPT}, {'role': 'user', 'content': prompt}], temperature=0, max_tokens=_goal_judge_max_tokens(), timeout=timeout)
    except Exception as exc:
    ...
    logger.info('goal judge: verdict=%s reason=%s%s', verdict, _truncate(reason, 120), f' wait={wait_directive}' if wait_directive else '')
...
def draft_contract(objective: str, *, timeout: float=DEFAULT_JUDGE_TIMEOUT) -> Optional[GoalContract]:
    ...
    try:
        ...
        return None
    try:
        resp = call_llm(task='goal_judge', messages=[{'role': 'system', 'content': DRAFT_CONTRACT_SYSTEM_PROMPT}, {'role': 'user', 'content': f'Objective:\n{_truncate(objective, 4000)}'}], temperature=0, max_tokens=_goal_judge_max_tokens(), timeout=timeout)
    except Exception as exc:
...
class GoalManager:
    ...
    def set(self, goal: str, *, max_turns: Optional[int]=None, contract: Optional[GoalContract]=None) -> GoalState:
        ...
        state = GoalState(goal=goal, status='active', turns_used=0, max_turns=int(max_turns) if max_turns else self.default_max_turns, created_at=time.time(), last_turn_at=0.0, contract=contract if contract is not None else GoalContract())
        self._state = state
    ...
    def add_gate(self, command: str, *, timeout_seconds: Optional[int]=None, max_retries: Optional[int]=None) -> GoalGate:
        ...
        gate = GoalGate(command=command, timeout_seconds=int(timeout_seconds) if timeout_seconds else DEFAULT_GATE_TIMEOUT_SECONDS, max_retries=int(max_retries) if max_retries else DEFAULT_GATE_MAX_RETRIES)
    ...
    def render_gates(self) -> str:
        ...
        for i, g in enumerate(self._state.gates, start=1):
            ...
            if g.last_exit_code is not None:
                status = ' ✓ passing' if g.last_exit_code == 0 else f' ✗ failing (exit {g.last_exit_code}, attempt {g.attempts}/{g.max_retries})'
    ...
    def _check_gates(self) -> Optional[Dict[str, Any]]:
        ...
        for gate in state.gates:
            ...
            if gate.attempts > gate.max_retries:
                ...
                save_goal(self.session_id, state)
                return {'status': 'paused', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'gate_failed', 'reason': f'gate exhausted retries: $ {gate.command}', 'message': f'⏸ Goal paused — quality gate still failing after {gate.max_retries} retries: $ {gate.command} (exit {exit_code}). Fix it manually or /goal gate remove it, then /goal resume.'}
            save_goal(self.session_id, state)
            prompt = CONTINUATION_PROMPT_GATE_FAILED_TEMPLATE.format(goal=state.goal, command=gate.command, exit_code=exit_code, attempt=gate.attempts, max_retries=gate.max_retries, output=tail or '(no output)')
            return {'status': 'active', 'should_continue': True, 'continuation_prompt': prompt, 'verdict': 'gate_failed', 'reason': f'gate failed (exit {exit_code}): $ {gate.command}', 'message': f'✗ Quality gate failed ({state.turns_used}/{state.max_turns} turns, attempt {gate.attempts}/{gate.max_retries}){skipped_note}: $ {gate.command}'}
        save_goal(self.session_id, state)
        return None
    ...
    def stop_waiting(self) -> bool:
        ...
        if self._state is None:
            return False
        if self._state.waiting_on_pid is None and self._state.waiting_on_session is None and (not self._state.waiting_until):
            return False
        self._state.waiting_on_pid = None
        self._state.waiting_on_session = None
        self._state.waiting_until = 0.0
    ...
    def evaluate_after_turn(self, last_response: str, *, user_initiated: bool=True, background_processes: Optional[List[Dict[str, Any]]]=None) -> Dict[str, Any]:
        ...
        if state is None or state.status != 'active':
            return {'status': state.status if state else None, 'should_continue': False, 'continuation_prompt': None, 'verdict': 'inactive', 'reason': 'no active goal', 'message': ''}
        if self.is_waiting():
            ...
            return {'status': 'active', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'waiting', 'reason': reason, 'message': f'⏳ Goal parked — waiting on {tgt}: {reason}'}
        state.turns_used += 1
        ...
        if gate_decision is not None:
            if gate_decision.get('should_continue') and state.turns_used >= state.max_turns:
                ...
                save_goal(self.session_id, state)
                return {'status': 'paused', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'gate_failed', 'reason': gate_decision.get('reason', ''), 'message': f'⏸ Goal paused — {state.turns_used}/{state.max_turns} turns used (a quality gate is still failing). Use /goal resume to keep going, or /goal clear to stop.'}
            return gate_decision
        verdict, reason, parse_failed, wait_directive, transport_failed = judge_goal(state.goal, last_response, subgoals=state.subgoals or None, background_processes=background_processes, contract=state.contract if state.has_contract() else None)
        ...
        if verdict == 'wait' and wait_directive:
            ...
            return {'status': 'active', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'wait', 'reason': reason, 'message': f'⏳ Goal parked (judge) — waiting on {tgt}: {reason}'}
        if verdict == 'done':
            state.status = 'done'
            save_goal(self.session_id, state)
            return {'status': 'done', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'done', 'reason': reason, 'message': f'✓ Goal achieved: {reason}'}
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            state.status = 'paused'
            state.paused_reason = f'judge API unreachable {state.consecutive_transport_failures} turns in a row (check auxiliary.goal_judge provider/key in config.yaml)'
            save_goal(self.session_id, state)
            return {'status': 'paused', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'continue', 'reason': reason, 'message': f'⏸ Goal paused — judge API returned errors ({state.consecutive_transport_failures} turns). Check the goal_judge provider/key in ~/.duck-agent/config.yaml:\n  auxiliary:\n    goal_judge:\n      provider: deepseek\n      model: deepseek-v4-flash\nThen /goal resume to continue.'}
        if state.consecutive_parse_failures >= DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES:
            ...
            save_goal(self.session_id, state)
            return {'status': 'paused', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'continue', 'reason': reason, 'message': f"⏸ Goal paused — the judge model ({state.consecutive_parse_failures} turns) isn't returning the required JSON verdict. Route the judge to a stricter model in ~/.duck-agent/config.yaml:\n  auxiliary:\n    goal_judge:\n      provider: openrouter\n      model: google/gemini-3-flash-preview\nThen /goal resume to continue."}
        if state.turns_used >= state.max_turns:
            state.status = 'paused'
            state.paused_reason = f'turn budget exhausted ({state.turns_used}/{state.max_turns})'
            save_goal(self.session_id, state)
            return {'status': 'paused', 'should_continue': False, 'continuation_prompt': None, 'verdict': 'continue', 'reason': reason, 'message': f'⏸ Goal paused — {state.turns_used}/{state.max_turns} turns used. Use /goal resume to keep going, or /goal clear to stop.'}
        save_goal(self.session_id, state)
        return {'status': 'active', 'should_continue': True, 'continuation_prompt': self.next_continuation_prompt(), 'verdict': 'continue', 'reason': reason, 'message': f'↻ Continuing toward goal ({state.turns_used}/{state.max_turns}): {reason}'}
    ...
    def next_continuation_prompt(self) -> Optional[str]:
        ...
        if self._state.has_contract():
            ...
            if self._state.subgoals:
                extra = '\n'.join((f'- Extra criterion {i}: {text}' for i, text in enumerate(self._state.subgoals, start=1)))
                contract_block = f'{contract_block}\n{extra}'
            return CONTINUATION_PROMPT_WITH_CONTRACT_TEMPLATE.format(goal=self._state.goal, contract_block=contract_block)
        if self._state.subgoals:
            return CONTINUATION_PROMPT_WITH_SUBGOALS_TEMPLATE.format(goal=self._state.goal, subgoals_block=self._state.render_subgoals_block())
...
def run_kanban_goal_loop(*, task_id: str, goal_text: str, run_turn, task_status_fn, block_fn, max_turns: int=DEFAULT_MAX_TURNS, first_response: str='', log=None) -> Dict[str, Any]:
    ...
    (``duck-agent -p <profile> chat -q "work kanban task <id>"``). The worker's
    ...
    while True:
        ...
        if verdict == 'done':
            if nudged_to_finalize:
                ...
                try:
                    block_fn(f"Goal-mode worker's output looked complete but it never called kanban_complete after a finalize nudge ({reason}).")
                except Exception as exc:
        ...
        if turns_used >= max_turns:
            ...
            try:
                block_fn(f'Goal-mode worker exhausted its turn budget ({turns_used}/{max_turns}) without completing the task. Last judge verdict: {_truncate(reason, 300)}')
            except Exception as exc:
                _log(f'kanban goal loop: block_fn failed ({exc})')
        ...
        turns_used += 1
__all__ = ['GoalState', 'GoalContract', 'GoalGate', 'GoalManager', 'parse_contract', 'draft_contract', 'run_gate', 'workspace_fingerprint', 'CONTINUATION_PROMPT_TEMPLATE', 'CONTINUATION_PROMPT_WITH_SUBGOALS_TEMPLATE', 'CONTINUATION_PROMPT_WITH_CONTRACT_TEMPLATE', 'JUDGE_USER_PROMPT_TEMPLATE', 'JUDGE_USER_PROMPT_WITH_SUBGOALS_TEMPLATE', 'JUDGE_USER_PROMPT_WITH_CONTRACT_TEMPLATE', 'DRAFT_CONTRACT_SYSTEM_PROMPT', 'KANBAN_GOAL_CONTINUATION_TEMPLATE', 'KANBAN_GOAL_FINALIZE_TEMPLATE', 'DEFAULT_MAX_TURNS', 'load_goal', 'save_goal', 'clear_goal', 'migrate_goal_to_session', 'judge_goal', 'run_kanban_goal_loop']