"""CLI handlers for ``duck-agent migrate ...``.

Currently exposes only ``duck-agent migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``duck-agent migrate <subtype>``."""
    sub = getattr(args, 'migrate_type', None)
    if sub == 'xai':
        return cmd_migrate_xai(args)
    print('usage: duck-agent migrate xai [--apply] [--no-backup]', file=sys.stderr)
    return 2

def cmd_migrate_xai(args: Any) -> int:
    """Run xAI May-15 model migration in dry-run or apply mode."""
    from hermes_cli.xai_retirement import MIGRATION_GUIDE_URL, RETIREMENT_DATE, apply_migration, find_retired_xai_refs, format_issue
    ...
    if not apply:
        ...
        print(color(f'Re-run with `duck-agent migrate xai --apply` to rewrite {config_path} in-place (backup created automatically).', Colors.DIM))
        return 0
    if not config_path or not config_path.exists():
        print(f"  {color('✗', Colors.RED)} Could not locate config.yaml (looked at: {config_path})", file=sys.stderr)
        return 1
    try:
    ...
    print(f"  {color('✓', Colors.GREEN)} Updated {len(result.issues_resolved)} slot(s) in {result.file_path}")
    print()
    print(color('Run `duck-agent doctor` to confirm no retired xAI models remain.', Colors.DIM))
    return 0
