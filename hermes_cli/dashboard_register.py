"""``duck-agent dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.duck-agent/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.duck-agent/auth.json``), refreshing it if needed. Fails fast with a
     "run `duck-agent setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.duck-agent/.env`` idempotently.
...
def _register_self_hosted_client(*, access_token: str, portal_base_url: str, name: Optional[str], custom_redirect_uri: Optional[str], existing_client_id: Optional[str]=None, timeout: float=15.0) -> dict:
    ...
    makes re-running ``duck-agent dashboard register`` idempotent. The portal
    ...
    req = urllib.request.Request(url, data=data, method='POST', headers={'Authorization': f'Bearer {access_token}', 'Content-Type': 'application/json', 'Accept': 'application/json'})
    try:
        ...
        try:
            ...
            pass
        if exc.code == 401:
            raise RuntimeError('Nous Portal rejected the access token (401). Try `duck-agent auth add nous` to re-authenticate.') from exc
        if exc.code == 403:
            raise RuntimeError(detail or 'Your account is not permitted to register a self-hosted dashboard.') from exc
...
def _print_post_register_hint(*, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str], wrote_portal_url: bool, public_url: str='') -> None:
    ...
    print()
    print('  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n  `duck-agent dashboard` (localhost) leaves the gate off and serves locally\n  without auth, which is fine for your own machine.')
    print()
    if custom_redirect_uri:
        try:
        ...
        print('    duck-agent dashboard --host 0.0.0.0')
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print('✗ `duck-agent dashboard register` is not available in a managed/hosted install.\n  The dashboard OAuth client is provisioned by the hosting platform.')
        sys.exit(1)
    try:
        ...
        if getattr(exc, 'relogin_required', False):
            ...
            print('  Run `duck-agent setup` (or `duck-agent auth add nous`) first, then retry.')
        else:
    ...
    try:
        result = _register_self_hosted_client(access_token=access_token, portal_base_url=portal_base_url, name=name, custom_redirect_uri=custom_redirect_uri, existing_client_id=existing_client_id)
    ...
    if custom_portal_supplied:
        ...
        should_write_portal = not existing_portal and portal_base_url.rstrip('/') != default_portal
    ...
    if public_url:
        ...
        if existing_public_url != public_url:
            try:
                ...
                pass
    _print_post_register_hint(client_id=client_id, portal_base_url=portal_base_url, custom_redirect_uri=custom_redirect_uri, wrote_portal_url=wrote_portal_url, public_url=public_url if wrote_public_url else '')