...
Scope: **per-profile**, stored at ``$DUCK_AGENT_HOME/projects.db`` (resolved via
...
def projects_db_path() -> Path:
    """The per-profile projects DB path (``$DUCK_AGENT_HOME/projects.db``).
...
_SLUG_RE = re.compile('^[a-z0-9][a-z0-9\\-_]{0,63}$')

def _slugify(name: str) -> str:
    """Derive a slug candidate from a human name (best-effort)."""
    s = str(name or '').strip().lower()
    s = re.sub('[^a-z0-9]+', '-', s).strip('-_')
    s = s[:64].strip('-_')
    return s or 'project'
...
def normalize_slug(slug: Optional[str]) -> Optional[str]:
    ...
    if not _SLUG_RE.match(s):
        raise ValueError(f"invalid project slug {slug!r}: must be 1-64 chars, lowercase alphanumerics / hyphens / underscores, not starting with '-' or '_'")
    return s
...
class Project:
    ...
    def to_dict(self) -> dict:
        return {'id': self.id, 'slug': self.slug, 'name': self.name, 'description': self.description, 'icon': self.icon, 'color': self.color, 'board_slug': self.board_slug, 'primary_path': self.primary_path, 'archived': bool(self.archived), 'created_at': self.created_at, 'folders': [f.to_dict() for f in self.folders]}

def _project_from_row(row: sqlite3.Row) -> Project:
    keys = row.keys()
    return Project(id=row['id'], slug=row['slug'], name=row['name'], created_at=row['created_at'], description=row['description'] if 'description' in keys else None, icon=row['icon'] if 'icon' in keys else None, color=row['color'] if 'color' in keys else None, board_slug=row['board_slug'] if 'board_slug' in keys else None, primary_path=row['primary_path'] if 'primary_path' in keys else None, archived=bool(row['archived']) if 'archived' in keys else False)

def _load_folders(conn: sqlite3.Connection, project_id: str) -> List[ProjectFolder]:
    rows = conn.execute('SELECT path, label, is_primary, added_at FROM project_folders WHERE project_id = ? ORDER BY is_primary DESC, added_at ASC', (project_id,)).fetchall()
    return [ProjectFolder(path=r['path'], label=r['label'], is_primary=bool(r['is_primary']), added_at=r['added_at']) for r in rows]
...
def create_project(conn: sqlite3.Connection, *, name: str, slug: Optional[str]=None, folders: Optional[Iterable[str]]=None, primary_path: Optional[str]=None, description: Optional[str]=None, icon: Optional[str]=None, color: Optional[str]=None, board_slug: Optional[str]=None) -> str:
    ...
    with write_txn(conn):
        ...
        conn.execute('INSERT INTO projects (id, slug, name, description, icon, color, board_slug,  primary_path, created_at, archived) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0)', (pid, unique, name, description, icon, color, normalize_slug(board_slug) if board_slug else None, primary, now))
        for path in folder_paths:
            conn.execute('INSERT INTO project_folders (project_id, path, label, is_primary, added_at) VALUES (?, ?, ?, ?, ?)', (pid, path, None, 1 if path == primary else 0, now))
    return pid
...
def add_folder(conn: sqlite3.Connection, project_id: str, path: str, *, label: Optional[str]=None, is_primary: bool=False) -> str:
    ...
    now = _now()
    with write_txn(conn):
        conn.execute('INSERT OR IGNORE INTO project_folders (project_id, path, label, is_primary, added_at) VALUES (?, ?, ?, 0, ?)', (project_id, norm, label, now))
        ...
        if is_primary:
            ...
            existing_primary = conn.execute('SELECT 1 FROM project_folders WHERE project_id = ? AND is_primary = 1', (project_id,)).fetchone()
...
def remove_folder(conn: sqlite3.Connection, project_id: str, path: str) -> bool:
    ...
    norm = _normalize_path(path)
    with write_txn(conn):
        was_primary = conn.execute('SELECT is_primary FROM project_folders WHERE project_id = ? AND path = ?', (project_id, norm)).fetchone()
        ...
        if was_primary is not None and was_primary['is_primary']:
            nxt = conn.execute('SELECT path FROM project_folders WHERE project_id = ? ORDER BY added_at ASC LIMIT 1', (project_id,)).fetchone()
...
def set_primary(conn: sqlite3.Connection, project_id: str, path: str) -> bool:
    norm = _normalize_path(path)
    with write_txn(conn):
        exists = conn.execute('SELECT 1 FROM project_folders WHERE project_id = ? AND path = ?', (project_id, norm)).fetchone()
        if exists is None:
            return False
        _set_primary_locked(conn, project_id, norm)
    return True
...
def restore_project(conn: sqlite3.Connection, project_id: str) -> bool:
    with write_txn(conn):
        cur = conn.execute('UPDATE projects SET archived = 0 WHERE id = ?', (project_id,))
    return cur.rowcount > 0
...
def set_active(conn: sqlite3.Connection, project_id: Optional[str]) -> None:
    ...
    with write_txn(conn):
        if project_id is None:
            ...
            conn.execute('INSERT INTO project_meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', (_ACTIVE_META_KEY, project_id))
...
def get_discovery_policy_key(conn: sqlite3.Connection) -> Optional[str]:
    row = conn.execute('SELECT value FROM project_meta WHERE key = ?', (_DISCOVERY_POLICY_META_KEY,)).fetchone()
    return row['value'] if row else None
...
def reconcile_discovered_repos_policy(conn: sqlite3.Connection, policy_key: str, *, preserve_unversioned: bool=False) -> bool:
    ...
    with write_txn(conn):
        ...
        conn.execute('INSERT INTO project_meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', (_DISCOVERY_POLICY_META_KEY, policy_key))
    return cleared
...
def record_discovered_repos(conn: sqlite3.Connection, repos: Iterable[tuple[str, Optional[str]]], *, replace: bool=False, policy_key: Optional[str]=None) -> int:
    ...
    with write_txn(conn):
        if replace:
            conn.execute('DELETE FROM discovered_repos')
        if rows:
            conn.executemany('INSERT INTO discovered_repos (root, label, last_seen) VALUES (?, ?, ?) ON CONFLICT(root) DO UPDATE SET label = excluded.label, last_seen = excluded.last_seen', rows)
        if policy_key is not None:
            conn.execute('INSERT INTO project_meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', (_DISCOVERY_POLICY_META_KEY, policy_key))
    return len(rows)
...
def list_discovered_repos(conn: sqlite3.Connection) -> List[dict]:
    ...
    return [{'root': r['root'], 'label': r['label'], 'last_seen': r['last_seen']} for r in rows]
...
def project_for_path(conn: sqlite3.Connection, path: str, *, include_archived: bool=False) -> Optional[Project]:
    ...
    for row in conn.execute(sql).fetchall():
        folder = row['folder']
        if target == folder or target.startswith(folder.rstrip('/\\') + os.sep) or target.startswith(folder.rstrip('/\\') + '/'):
...
_BRANCH_SAFE_RE = re.compile('[^a-z0-9._-]+')
