"""duck-agent hooks — inspect and manage shell-script hooks.

Usage::

    duck-agent hooks list
    duck-agent hooks test <event> [--for-tool X] [--payload-file F]
    duck-agent hooks revoke <command>
    duck-agent hooks doctor

Consent records live under ``~/.duck-agent/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.duck-agent/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``duck-agent hooks`` — dispatches to the requested action."""
    sub = getattr(args, 'hooks_action', None)
    if not sub:
        print('Usage: duck-agent hooks {list|test|revoke|doctor}')
        print("Run 'duck-agent hooks --help' for details.")
        return
    if sub in {'list', 'ls'}:
        _cmd_list(args)
    elif sub == 'test':
...
def _cmd_list(_args) -> None:
    ...
    if not specs and (not outbound):
        print('No shell hooks or outbound webhooks configured in ~/.duck-agent/config.yaml.')
        print('See `duck-agent hooks --help` or')
        print('    website/docs/user-guide/features/hooks.md')
        print('for the config schema and worked examples.')
        return
    if not specs:
        print('No shell hooks configured in ~/.duck-agent/config.yaml.')
    else:
        by_event: Dict[str, List] = {}
        for spec in specs:
            by_event.setdefault(spec.event, []).append(spec)
        allowlist = shell_hooks.load_allowlist()
        approved = {(e.get('event'), e.get('command')) for e in allowlist.get('approvals', []) if isinstance(e, dict)}
        ...
        for event in sorted(by_event.keys()):
            ...
            for spec in by_event[event]:
                ...
                print(f'    - {spec.command}{matcher_part} (timeout={spec.timeout}s, {status})')
                if is_approved:
                    ...
                    if entry and entry.get('approved_at'):
                        ...
                        if mtime_now and mtime_at and (mtime_now > mtime_at):
                            print(f'      ⚠ script modified since approval (was {mtime_at}, now {mtime_now}) — run `duck-agent hooks doctor` to re-validate')
            print()
    if outbound:
        ...
        for target in outbound:
            ...
            print(f"      events:  {', '.join(target.events)}{matcher_part} (timeout={target.timeout}s, {signed})")
        print()
_DEFAULT_PAYLOADS = {'pre_tool_call': {'tool_name': 'terminal', 'args': {'command': 'echo hello'}, 'session_id': 'test-session', 'task_id': 'test-task', 'tool_call_id': 'test-call'}, 'post_tool_call': {'tool_name': 'terminal', 'args': {'command': 'echo hello'}, 'session_id': 'test-session', 'task_id': 'test-task', 'tool_call_id': 'test-call', 'result': '{"output": "hello"}', 'duration_ms': 42}, 'pre_llm_call': {'session_id': 'test-session', 'user_message': 'What is the weather?', 'conversation_history': [], 'is_first_turn': True, 'model': 'gpt-4', 'platform': 'cli'}, 'post_llm_call': {'session_id': 'test-session', 'model': 'gpt-4', 'platform': 'cli'}, 'pre_verify': {'session_id': 'test-session', 'platform': 'cli', 'model': 'gpt-4', 'coding': True, 'attempt': 0, 'final_response': 'All done — the change is applied.', 'changed_paths': ['src/app.tsx']}, 'on_session_start': {'session_id': 'test-session'}, 'on_session_end': {'session_id': 'test-session', 'task_id': 'test-task', 'turn_id': 'test-turn', 'completed': True, 'failed': False, 'interrupted': False, 'turn_exit_reason': 'text_response(stop)', 'model': 'gpt-4', 'platform': 'cli'}, 'on_session_finalize': {'session_id': 'test-session'}, 'on_session_reset': {'session_id': 'test-session'}, 'pre_api_request': {'session_id': 'test-session', 'task_id': 'test-task', 'platform': 'cli', 'model': 'claude-sonnet-4-6', 'provider': 'anthropic', 'base_url': 'https://api.anthropic.com', 'api_mode': 'anthropic_messages', 'api_call_count': 1, 'message_count': 4, 'tool_count': 12, 'approx_input_tokens': 2048, 'request_char_count': 8192, 'max_tokens': 4096}, 'post_api_request': {'session_id': 'test-session', 'task_id': 'test-task', 'platform': 'cli', 'model': 'claude-sonnet-4-6', 'provider': 'anthropic', 'base_url': 'https://api.anthropic.com', 'api_mode': 'anthropic_messages', 'api_call_count': 1, 'api_duration': 1.234, 'finish_reason': 'stop', 'message_count': 4, 'response_model': 'claude-sonnet-4-6', 'usage': {'input_tokens': 2048, 'output_tokens': 512}, 'assistant_content_chars': 1200, 'assistant_tool_call_count': 0}, 'subagent_stop': {'parent_session_id': 'parent-sess', 'child_role': None, 'child_summary': 'Synthetic summary for hooks test', 'child_status': 'completed', 'tool_call_history': [{'tool_name': 'write_file', 'tool_input': {'argument_keys': ['content', 'path'], 'targets': {'path': '/tmp/report.txt'}}, 'input_bytes': 128, 'output_bytes': 32, 'status': 'ok'}], 'duration_ms': 1234}}
...
def _cmd_test(args) -> None:
    ...
    if getattr(args, 'for_tool', None):
        specs = [s for s in specs if s.event not in {'pre_tool_call', 'post_tool_call'} or s.matches_tool(args.for_tool)]
    if not specs:
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f'      parsed (Duck Agent wire shape): {json.dumps(parsed)}')
    else:
...
def _cmd_revoke(args) -> None:
    ...
    print('Note: currently running CLI / gateway processes keep their already-registered callbacks until they restart.')
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if shell_hooks.script_is_executable(spec.command):
        ...
        problems += 1
        print('      ✗ script missing or not executable (chmod +x the file, or fix the path)')
    entry = shell_hooks.allowlist_entry_for(spec.event, spec.command)
    if entry:
        print(f"      ✓ allowlisted (approved {entry.get('approved_at', '?')})")
    else:
        problems += 1
        print('      ✗ not allowlisted — hook will NOT fire at runtime (run with --accept-hooks once, or confirm at the TTY prompt)')
    if entry and entry.get('script_mtime_at_approval'):
        mtime_now = shell_hooks.script_mtime_iso(spec.command)
        mtime_at = entry['script_mtime_at_approval']
        if mtime_now and mtime_at and (mtime_now > mtime_at):
            problems += 1
            print(f'      ⚠ script modified since approval (was {mtime_at}, now {mtime_now}) — review changes, then `duck-agent hooks revoke` + re-approve to refresh')
        elif mtime_now and mtime_at and (mtime_now == mtime_at):
            print('      ✓ script unchanged since approval')
    if not entry:
        print('      ℹ skipped JSON smoke test — not allowlisted yet. Approve the hook first (via TTY prompt or --accept-hooks), then re-run `duck-agent hooks doctor`.')
    elif shell_hooks.script_is_executable(spec.command):
        payload = _DEFAULT_PAYLOADS.get(spec.event, {'extra': {}})
        result = shell_hooks.run_once(spec, payload)
        if result.get('timed_out'):
            problems += 1
            print(f"      ✗ timed out after {result['elapsed_seconds']}s on synthetic payload (timeout={spec.timeout}s)")
            ...
            if stdout:
                try:
                    json.loads(stdout)
                    print(f'      ✓ produced valid JSON on synthetic payload (exit={rc}, {elapsed}s)')
                except json.JSONDecodeError:
                    problems += 1
                    print(f'      ✗ stdout was not valid JSON (exit={rc}, {elapsed}s): {_truncate(stdout, 120)}')
            else:
                print(f'      ✓ ran clean with empty stdout (exit={rc}, {elapsed}s) — hook is observer-only')
    return problems