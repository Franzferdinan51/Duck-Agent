...
_DEP_CHECKS = {'node': lambda: find_node_executable('node') is not None, 'browser': lambda: agent_browser_runnable(shutil.which('agent-browser')) or _has_system_browser() or _has_hermes_agent_browser(), 'ripgrep': lambda: shutil.which('rg') is not None, 'ffmpeg': lambda: shutil.which('ffmpeg') is not None}
_DEP_DESCRIPTIONS = {'node': 'Node.js (required for browser tools and TUI)', 'browser': 'Browser engine (Chromium, for web browsing tools)', 'ripgrep': 'ripgrep (fast file search)', 'ffmpeg': 'ffmpeg (TTS voice messages)'}
...
def _has_hermes_agent_browser() -> bool:
    ...
    return (home / 'node' / 'bin' / 'agent-browser').is_file() or (home / 'node_modules' / '.bin' / 'agent-browser').is_file()
...
def ensure_dependency(dep: str, interactive: bool=True) -> bool:
    ...
    if shell == 'powershell':
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, '-ExecutionPolicy', 'Bypass', '-File', str(script), '-Ensure', dep, '-DuckAgentHome', str(get_hermes_home())]
    else:
