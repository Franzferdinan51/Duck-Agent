...
Used by ``duck-agent kanban specify [task_id | --all]``. Takes a task that
...
def _truncate(text: str, limit: int) -> str:
    if len(text) <= limit:
        return text
    return text[:limit - 1] + '…'
_FENCE_RE = re.compile('^\\s*```(?:json)?\\s*|\\s*```\\s*$', re.IGNORECASE)
...
def specify_task(task_id: str, *, author: Optional[str]=None, timeout: Optional[int]=None) -> SpecifyOutcome:
    ...
    if task.status != 'triage':
        return SpecifyOutcome(task_id, False, f'task is not in triage (status={task.status!r})')
    try:
    ...
    user_msg = _USER_TEMPLATE.format(task_id=task.id, title=_truncate(task.title or '', 400), body=_truncate(task.body or '(no body)', 4000))
    try:
        resp = call_llm(task='triage_specifier', messages=[{'role': 'system', 'content': _SYSTEM_PROMPT}, {'role': 'user', 'content': user_msg}], temperature=0.3, max_tokens=HERMES_KANBAN_SPECIFY_MAX_TOKENS, timeout=timeout or 120)
    except Exception as exc:
        logger.info('specify: API call failed for %s (%s) — skipping', task_id, exc)
    ...
    with kb.connect_closing() as conn:
        ok = kb.specify_triage_task(conn, task_id, title=new_title, body=new_body, author=author or _profile_author())
    if not ok:
