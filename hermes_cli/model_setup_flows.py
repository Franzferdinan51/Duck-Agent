"""Per-provider model-selection wizard flows for ``duck-agent setup`` / ``duck-agent model``.

Extracted from ``hermes_cli/main.py`` as part of the god-file decomposition
campaign (``~/.duck-agent/plans/god-file-decomposition.md``, Phase 2 — splitting
...
def bedrock_region_geo_prefix(region_name: str) -> str:
    ...
    for geo, region_prefixes in (('us.', ('us-', 'us_gov')), ('eu.', ('eu-',)), ('ap.', ('ap-',)), ('ca.', ('ca-',)), ('sa.', ('sa-',)), ('me.', ('me-',)), ('af.', ('af-',))):
...
def _prune_replaced_custom_model_config_credentials(base_url: str, *, provider_name: str='') -> None:
    ...
    try:
        ...
        active_pool_key = get_custom_provider_pool_key(base_url, provider_name=provider_name or None)
        ...
        for pool_key, entries in pools.items():
            if not isinstance(pool_key, str) or not pool_key.startswith(CUSTOM_POOL_PREFIX) or pool_key == active_pool_key or (not isinstance(entries, list)):
                continue
            retained = []
            removed_ids = []
            changed = False
...
def _model_flow_openrouter(config, current_model=''):
    ...
    selected = _prompt_model_selection(openrouter_models, current_model=current_model, pricing=pricing, confirm_provider='openrouter', confirm_base_url=OPENROUTER_BASE_URL, confirm_api_key=_resolved or existing_key)
    if selected:
...
def _model_flow_moa(config, current_model=''):
    ...
    if not presets:
        print('No MoA presets configured. Run `duck-agent moa configure <name>` first.')
        return
...
def _model_flow_nous(config, current_model='', args=None):
    ...
    from hermes_cli.auth import get_provider_auth_state, _prompt_model_selection, _save_model_choice, _update_config_for_provider, resolve_nous_runtime_credentials, AuthError, format_auth_error, _login_nous, PROVIDER_REGISTRY
    ...
    try:
        ...
        if relogin:
            ...
            try:
                mock_args = argparse.Namespace(portal_url=None, inference_url=None, client_id=None, scope=None, no_browser=False, timeout=15.0, ca_bundle=None, insecure=False)
                _login_nous(mock_args, PROVIDER_REGISTRY['nous'])
    ...
    if free_tier:
        try:
            ...
            unavailable_message = ''
        model_ids, pricing = union_with_portal_free_recommendations(model_ids, pricing, _nous_portal_url)
        model_ids, unavailable_models = partition_nous_models_by_tier(model_ids, pricing, free_tier=True)
    else:
        model_ids, pricing = union_with_portal_paid_recommendations(model_ids, pricing, _nous_portal_url)
    ...
    selected = _prompt_model_selection(model_ids, current_model=current_model, pricing=pricing, unavailable_models=unavailable_models, portal_url=_nous_portal_url, unavailable_message=unavailable_message, confirm_provider='nous', confirm_base_url=creds.get('base_url', ''), confirm_api_key=creds.get('api_key', ''))
    if selected:
        _save_model_choice(selected)
...
def _model_flow_openai_codex(config, current_model=''):
    ...
    from hermes_cli.auth import get_codex_auth_status, _prompt_model_selection, _save_model_choice, _update_config_for_provider, _login_openai_codex, PROVIDER_REGISTRY, DEFAULT_CODEX_BASE_URL
    ...
    if status.get('logged_in'):
        ...
        if choice == 'reauth':
            ...
            try:
                ...
                _login_openai_codex(mock_args, PROVIDER_REGISTRY['openai-codex'], force_new_login=True)
            except SystemExit:
                print('Login cancelled or failed.')
                return
            except Exception as exc:
    ...
    selected = _prompt_model_selection(codex_models, current_model=current_model, confirm_provider='openai-codex', confirm_base_url=DEFAULT_CODEX_BASE_URL, confirm_api_key=_codex_token or '')
    if selected:
        _save_model_choice(selected)
...
def _model_flow_xai_oauth(_config, current_model='', *, args=None):
    ...
    from hermes_cli.auth import get_xai_oauth_auth_status, _prompt_model_selection, _save_model_choice, _update_config_for_provider, resolve_xai_oauth_runtime_credentials, _login_xai_oauth, DEFAULT_XAI_OAUTH_BASE_URL, PROVIDER_REGISTRY
    from hermes_cli.models import _PROVIDER_MODELS
    status = get_xai_oauth_auth_status()
    if status.get('logged_in'):
        print('  xAI Grok OAuth (SuperGrok / Premium+) credentials: ✓')
        print()
        choice = _prompt_auth_credentials_choice('xAI Grok OAuth (SuperGrok / Premium+) credentials:')
        if choice == 'reauth':
            print('Starting a fresh xAI OAuth login...')
            print()
            try:
                mock_args = argparse.Namespace(no_browser=bool(getattr(args, 'no_browser', False)), timeout=getattr(args, 'timeout', None))
                _login_xai_oauth(mock_args, PROVIDER_REGISTRY['xai-oauth'], force_new_login=True)
            except SystemExit:
                print('Login cancelled or failed.')
                return
            except Exception as exc:
...
def _model_flow_qwen_oauth(_config, current_model=''):
    ...
    from hermes_cli.auth import get_qwen_auth_status, resolve_qwen_runtime_credentials, _prompt_model_selection, _save_model_choice, _update_config_for_provider, DEFAULT_QWEN_BASE_URL
...
def _model_flow_minimax_oauth(config, current_model='', args=None):
    ...
    from hermes_cli.auth import get_provider_auth_state, _prompt_model_selection, _save_model_choice, _update_config_for_provider, resolve_minimax_oauth_runtime_credentials, AuthError, format_auth_error, _login_minimax_oauth, PROVIDER_REGISTRY
    state = get_provider_auth_state('minimax-oauth')
    if not state or not state.get('access_token'):
        print('Not logged into MiniMax. Starting OAuth login...')
        print()
        try:
            mock_args = argparse.Namespace(region=getattr(args, 'region', None) or 'global', no_browser=bool(getattr(args, 'no_browser', False)), timeout=getattr(args, 'timeout', None) or 15.0)
...
def _model_flow_custom(config):
    ...
    from hermes_cli.auth import _save_model_choice, deactivate_provider
    from hermes_cli.config import custom_endpoint_key_env, get_env_value, load_config, save_config, save_env_value
    ...
    if probe.get('used_fallback') and probe.get('resolved_base_url'):
        print(f"Warning: endpoint verification worked at {probe['resolved_base_url']}/models, not the exact URL you entered. Saving the working base URL instead.")
        ...
        print(f"Warning: could not verify this endpoint via {probe.get('probed_url')}. Duck Agent will still save it.")
    ...
    if model_name:
        ...
        print('Endpoint saved. Use `/model` in chat or `duck-agent model` to set a model.')
    _save_custom_provider(effective_url, effective_key, model_name or '', context_length=context_length, name=display_name, api_mode=api_mode, key_env=custom_key_env)
...
def _model_flow_azure_foundry(config, current_model=''):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print('Anthropic-style API endpoints.  Duck Agent will probe your')
    ...
    print()
    try:
        _placeholder = current_base_url or 'e.g. https://<resource>.openai.azure.com/openai/v1 or https://<resource>.services.ai.azure.com/anthropic'
    ...
    try:
        ...
        auth_choice = input(f'Authentication mode [1/2] ({_auth_default}): ').strip() or _auth_default
    except (KeyboardInterrupt, EOFError):
        print('\nCancelled.')
        return
    ...
    if use_entra:
        try:
            from agent.azure_identity_adapter import EntraIdentityConfig, SCOPE_AI_AZURE_DEFAULT, build_token_provider, describe_active_credential, has_azure_identity_installed
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print('  Duck Agent will install it now (the preflight below triggers the lazy-install). To skip lazy installs, run:  pip install azure-identity')
        ...
        if info.get('ok'):
            ...
            hint = info.get('hint') or 'Run `az login`, attach a managed identity to this VM, or set AZURE_TENANT_ID/AZURE_CLIENT_ID/AZURE_CLIENT_SECRET.'
            print(f'⚠ {err}')
            print(f'  Hint: {hint}')
            try:
        ...
        from hermes_cli.secret_prompt import masked_secret_prompt
        try:
            api_key = masked_secret_prompt(f"API key [{(current_api_key[:8] + '...' if current_api_key else 'required')}]: ").strip()
        except (KeyboardInterrupt, EOFError):
            print('\nCancelled.')
            return
    ...
    if discovered_models:
        ...
        print()
        try:
            pick = input(f'Pick by number, or type a deployment name [{current_model or discovered_models[0]}]: ').strip()
        except (KeyboardInterrupt, EOFError):
            print('\nCancelled.')
            return
        if not pick:
        ...
        try:
            model_name = input(f"Model / deployment name [{current_model or 'e.g. gpt-5.4, claude-sonnet-4-6'}]: ").strip()
        except (KeyboardInterrupt, EOFError):
            print('\nCancelled.')
            return
...
def _model_flow_named_custom(config, provider_info):
    ...
    if isinstance(cfg_models, dict):
        ...
        configured_models = [str(m) for m in cfg_models if isinstance(m, str) and m.strip()]
    ...
    if provider_key:
        ...
        if isinstance(providers_cfg, dict):
            ...
            if isinstance(provider_entry, dict):
                ...
                original_api_key_ref = str(provider_info.get('api_key_ref', '') or '').strip()
...
def _model_flow_copilot(config, current_model=''):
    ...
    from hermes_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection, _save_model_choice, deactivate_provider, resolve_api_key_provider_credentials
    from hermes_cli.config import save_env_value, load_config, save_config
    from hermes_cli.models import _PROVIDER_MODELS, fetch_api_models, fetch_github_model_catalog, github_model_reasoning_efforts, copilot_model_api_mode, normalize_copilot_model_id
    provider_id = 'copilot'
    ...
    normalized_current_model = normalize_copilot_model_id(current_model, catalog=catalog, api_key=api_key) or current_model
    if live_models:
    ...
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=normalized_current_model, confirm_provider=provider_id, confirm_base_url=effective_base, confirm_api_key=api_key)
    else:
        try:
            ...
            selected = None
    if selected:
        selected = normalize_copilot_model_id(selected, catalog=catalog, api_key=api_key) or selected
        ...
        model['api_mode'] = copilot_model_api_mode(selected, catalog=catalog, api_key=api_key)
        clear_model_endpoint_credentials(model, clear_api_mode=False)
...
def _model_flow_copilot_acp(config, current_model=''):
    ...
    from hermes_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection, _save_model_choice, deactivate_provider, get_external_process_provider_status, resolve_api_key_provider_credentials, resolve_external_process_provider_credentials
    from hermes_cli.models import _PROVIDER_MODELS, fetch_github_model_catalog, normalize_copilot_model_id
    from hermes_cli.config import load_config, save_config
    del config
    ...
    print('  GitHub Copilot ACP delegates Duck Agent turns to `copilot --acp`.')
    print('  Duck Agent currently starts its own ACP subprocess for each request.')
    print('  Duck Agent uses your selected model as a hint for the Copilot ACP session.')
    ...
    normalized_current_model = normalize_copilot_model_id(current_model, catalog=catalog, api_key=catalog_api_key) or current_model
    if catalog:
        ...
        if model_list:
            ...
            print('    Use "Enter custom model name" if you do not see your model.')
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=normalized_current_model, confirm_provider=provider_id, confirm_base_url=effective_base, confirm_api_key=catalog_api_key)
    else:
        try:
            selected = input('Model name: ').strip()
        except (KeyboardInterrupt, EOFError):
    ...
    if not selected:
        print('No change.')
        return
    selected = normalize_copilot_model_id(selected, catalog=catalog, api_key=catalog_api_key) or selected
    _save_model_choice(selected)
    cfg = load_config()
    model = cfg.get('model')
    if not isinstance(model, dict):
...
def _model_flow_kimi(config, current_model=''):
    ...
    """
    from hermes_cli.main import _prompt_api_key
    from hermes_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL, _prompt_model_selection, _save_model_choice, deactivate_provider
    from hermes_cli.config import get_env_value, save_env_value, load_config, save_config
    from hermes_cli.models import _PROVIDER_MODELS
    ...
    existing_key, abort = _prompt_api_key(pconfig, existing_key, provider_id=provider_id, existing_source=existing_source)
    if abort:
        return
    ...
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=current_model, confirm_provider=provider_id, confirm_base_url=effective_base, confirm_api_key=existing_key)
    else:
        try:
...
def _model_flow_stepfun(config, current_model=''):
    ...
    region_choices = [('international', f"International ({_stepfun_base_url_for_region('international')})"), ('china', f"China ({_stepfun_base_url_for_region('china')})")]
    ordered_regions = []
...
def _model_flow_bedrock_api_key(config, region, current_model=''):
    ...
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=current_model, confirm_provider='custom', confirm_base_url=mantle_base_url, confirm_api_key=existing_key)
    else:
        try:
...
def _model_flow_bedrock(config, current_model=''):
    ...
    if live_models:
        _EXCLUDE_PREFIXES = ('stability.', 'cohere.embed', 'twelvelabs.', 'us.stability.', 'us.cohere.embed', 'us.twelvelabs.', 'global.cohere.embed', 'global.twelvelabs.')
        ...
        for m in filtered:
            mid = m['id']
            if not mid.startswith(_PROFILE_PREFIXES) and mid in profile_base_ids:
                continue
            deduped.append(m)
        _RECOMMENDED_BASES = ['anthropic.claude-sonnet-4-6', 'anthropic.claude-opus-4-6', 'anthropic.claude-haiku-4-5', 'amazon.nova-pro', 'amazon.nova-lite', 'amazon.nova-micro', 'deepseek.v3', 'meta.llama4-maverick', 'meta.llama4-scout']
        ...
        if model_list:
            ...
            return
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=current_model, confirm_provider='bedrock', confirm_base_url=f'https://bedrock-runtime.{region}.amazonaws.com')
    else:
        try:
            selected = input('  Model ID: ').strip()
        except (KeyboardInterrupt, EOFError):
...
def _model_flow_vertex(config, current_model=''):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    if sa_path:
        ...
        print('      • set VERTEX_CREDENTIALS_PATH in ~/.duck-agent/.env to a service account JSON')
    print()
    cfg = load_config()
    ...
    try:
        project_input = input(f"  GCP project ID [{current_project or 'from credentials'}]: ").strip()
    except (KeyboardInterrupt, EOFError):
        print()
        return
    ...
    base_url_preview = f'https://aiplatform.googleapis.com/v1beta1/projects/<project>/locations/{region}/endpoints/openapi' if region == 'global' else f'https://{region}-aiplatform.googleapis.com/v1beta1/projects/<project>/locations/{region}/endpoints/openapi'
...
def _model_flow_api_key_provider(config, provider_id, current_model=''):
    ...
    from hermes_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection, _save_model_choice, deactivate_provider
    from hermes_cli.config import get_env_value, save_env_value, load_config, save_config
    from hermes_cli.models import _PROVIDER_MODELS, fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
    ...
    if provider_id == 'gemini' and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == 'free':
                print()
                print('❌ This Google API key is on the free tier (<= 250 requests/day for gemini-2.5-flash).')
                print('   Duck Agent typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),')
                print('   so the free tier is exhausted after a handful of messages and cannot sustain')
                print('   an agent session.')
                print()
                print('   To use Gemini with Duck Agent, enable billing on your Google Cloud project and regenerate')
                print('   the key in a billing-enabled project: https://aistudio.google.com/apikey')
                print()
                print('   Alternatives with workable free usage: DeepSeek, OpenRouter (free models), Groq, Nous.')
                print()
    ...
    if model_list:
        ...
        try:
            ...
            pricing = {}
        selected = _prompt_model_selection(model_list, current_model=current_model, pricing=pricing, confirm_provider=provider_id, confirm_base_url=effective_base, confirm_api_key=existing_key)
    else:
        try:
            selected = input('Model name: ').strip()
        except (KeyboardInterrupt, EOFError):
...
def _model_flow_anthropic(config, current_model=''):
    ...
    from hermes_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from hermes_cli.config import save_env_value, load_config, save_config, save_anthropic_api_key
    from hermes_cli.models import _PROVIDER_MODELS
    ...
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=current_model, confirm_provider='anthropic')
    else:
        try:
