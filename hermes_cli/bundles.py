"""Implementation of the ``duck-agent bundles`` CLI subcommand.
...
from agent.skill_bundles import _bundles_dir, delete_bundle, get_bundle, list_bundles, reload_bundles, save_bundle, scan_bundles
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(f'[dim]No bundles installed yet. Create one with:\n  duck-agent bundles create <name> --skill skill1 --skill skill2[/]\nBundles directory: [bold]{_bundles_dir()}[/]')
        return
    ...
    for info in bundles:
        ...
        table.add_row(f"/{info['slug']}", info['name'], str(skill_count), info.get('description') or '')
    c.print(table)
...
def _cmd_create(args) -> None:
    ...
    if not skills:
        c.print('[dim]No skills passed via --skill. Enter one skill name per line.\nSubmit an empty line to finish.[/]')
        try:
            while True:
    ...
    if not skills:
        ...
        sys.exit(1)
    try:
        path = save_bundle(name, skills, description=description, instruction=instruction, overwrite=overwrite)
    ...
    if info:
        c.print(f"  Invoke with: [bold cyan]/{info['slug']}[/]  (loads {len(info['skills'])} skills)")
...
def register_cli(subparser) -> None:
    """Build the ``duck-agent bundles`` argparse tree.
    ...
    p_create = subs.add_parser('create', help='Create a new skill bundle', description='Create a new bundle. Skills can be passed via --skill (repeat for multiple) or entered interactively when omitted.')
    p_create.add_argument('name', help='Bundle name (becomes the /slash command)')
    p_create.add_argument('--skill', '-s', action='append', default=[], help='Skill name to include (repeat for multiple)')
    p_create.add_argument('--description', '-d', default='', help='Human-readable description shown in /help and `duck-agent bundles list`')
    p_create.add_argument('--instruction', '-i', default='', help='Extra guidance prepended to the loaded skill content')
    p_create.add_argument('--force', '-f', action='store_true', help='Overwrite an existing bundle with the same name')
...
def bundles_command(args) -> None:
    """Dispatch ``duck-agent bundles <subcommand>`` to the right handler."""
