"""Rate-limited heap release for long-lived Duck Agent gateway processes.
...
def _config_settings() -> tuple[bool, float, int, float]:
    """Return fail-open settings from the normal Duck Agent config path."""
    enabled = True
    ...
    try:
        ...
        if isinstance(settings, dict):
            ...
            info_log_min_delta_mb = settings.get('info_log_min_delta_mb', _DEFAULT_INFO_LOG_MIN_DELTA_MB)
    except Exception:
        pass
    return (enabled, _cooldown_seconds(cooldown), _log_every_n(log_every_n), _nonnegative_float(info_log_min_delta_mb, _DEFAULT_INFO_LOG_MIN_DELTA_MB))
...
def collect_memory_snapshot(history_bytes: int | None=None) -> dict[str, int | None]:
    ...
    """
    snapshot: dict[str, int | None] = {'rss_kib': None, 'rss_anon_kib': None, 'thread_count': threading.active_count()}
...
def _should_log_trim(*, force: bool, log_every_n: int, call_count: int, before: dict[str, int | None], after: dict[str, int | None], info_log_min_delta_mb: float) -> bool:
    if force:
        return True
...
def trim_memory(*, force: bool=False, reason: str='', cooldown_seconds: float | None=None) -> bool:
    ...
    with _trim_lock:
        ...
        try:
            ...
            _trim_call_count += 1
            if released and _should_log_trim(force=force, log_every_n=log_every_n, call_count=_trim_call_count, before=before, after=after, info_log_min_delta_mb=info_log_min_delta_mb):
                logger.info('memory trim: reason=%s malloc_trim=%s rss_kib=%s->%s rss_anon_kib=%s->%s threads=%s duration_ms=%.1f', reason or 'cleanup', trim_result, before.get('rss_kib'), after.get('rss_kib'), before.get('rss_anon_kib'), after.get('rss_anon_kib'), after.get('thread_count'), duration_ms)
            return released
        except Exception as exc:
            logger.warning('memory trim failed after %s: %s: %s', reason or 'cleanup', type(exc).__name__, exc)
            return False