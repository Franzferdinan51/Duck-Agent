...
Usage:
    duck-agent pairing list              # Show all pending + approved users
    duck-agent pairing approve <platform> <request-id|code>  # Approve a pairing request
    duck-agent pairing revoke <platform> <user_id> # Revoke user access
    duck-agent pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle duck-agent pairing subcommands."""
    ...
    if action == 'list':
        ...
        print('Usage: duck-agent pairing {list|approve|revoke|clear-pending}')
        print("Run 'duck-agent pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print(f"  {'--------':<12} {'----------':<18} {'-------':<20} {'----':<20} {'---'}")
        for p in pending:
            print(f"  {p['platform']:<12} {p.get('request_id') or '-':<18} {p['user_id']:<20} {p.get('user_name') or '':<20} {p['age_minutes']}m ago")
        print('\n  Approve with: duck-agent pairing approve <platform> <request-id>')
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print("  To reset sooner, delete the '_lockout:{0}' entry from ~/.duck-agent/platforms/pairing/_rate_limits.json\n".format(platform))
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'duck-agent pairing list' to see pending requests.\n")
