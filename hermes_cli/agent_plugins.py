...
supported components into records consumed by Duck Agent' existing skill and MCP
...
_PLUGIN_FIELDS = {'$schema', 'name', 'version', 'description', 'author', 'homepage', 'repository', 'license', 'keywords', 'extensions'}
_AUTHOR_FIELDS = {'name', 'email', 'url'}
_STDIO_FIELDS = {'type', 'command', 'args', 'env', 'cwd'}
_REMOTE_FIELDS = {'type', 'url', 'headers'}
_PLUGIN_NAME_RE = re.compile('^(?!.*(?:--|\\.\\.))[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?$')
_SKILL_NAME_RE = re.compile('^(?!.*--)[a-z0-9]+(?:-[a-z0-9]+)*$')
_PLACEHOLDER_RE = re.compile('\\$\\{(PLUGIN_ROOT|PLUGIN_DATA)\\}')
_HEADER_NAME_RE = re.compile("^[!#$%&'*+\\-.^_`|~0-9A-Za-z]+$")
...
def _valid_skill_frontmatter(frontmatter: Mapping[str, Any], directory_name: str) -> str | None:
    ...
    if not isinstance(name, str) or name != directory_name or (not 1 <= len(name) <= 64) or (_SKILL_NAME_RE.fullmatch(name) is None):
...
def _discover_skills(root: Path, diagnostics: list[AgentPluginDiagnostic]) -> tuple[AgentPluginSkill, ...]:
    ...
    for child in children:
        ...
        if not _inside(skill_md, root) or not skill_md.is_file():
            diagnostics.append(AgentPluginDiagnostic(scope, 'SKILL.md must be a regular in-root file'))
            continue
        try:
        ...
        if error:
            ...
            continue
        skills.append(AgentPluginSkill(name=child.name, description=frontmatter['description'], root=child.resolve(strict=True), skill_md=skill_md.resolve(strict=True), frontmatter=dict(frontmatter)))
    return tuple(skills)
...
def _resolve_scoped_path(value: str, plugin_root: Path, data_root: Path, *, expand_placeholders: bool=True) -> Path:
...
def _validate_headers(headers: object) -> bool:
    ...
    for name, value in headers.items():
        if not isinstance(name, str) or _HEADER_NAME_RE.fullmatch(name) is None or (not isinstance(value, str)) or ('\r' in value) or ('\n' in value) or (name.lower() in seen):
            return False
        seen.add(name.lower())
    return True
...
def _translate_stdio(config: Mapping[str, Any], plugin_root: Path, data_root: Path) -> Dict[str, Any]:
    ...
    return {'command': command_value, 'args': [_expand(value, plugin_root, data_root) for value in args], 'env': translated_env, 'cwd': str(cwd_value)}

def _discover_mcp(root: Path, data_root: Path, diagnostics: list[AgentPluginDiagnostic], *, create_data: bool=True) -> Dict[str, Dict[str, Any]]:
    mcp_path = root / 'mcp.json'
    if not mcp_path.exists() and (not mcp_path.is_symlink()):
        return {}
    if not _inside(mcp_path, root) or not mcp_path.is_file():
        diagnostics.append(AgentPluginDiagnostic('mcp', 'mcp.json must be a regular in-root file'))
        return {}
    try:
    ...
    if set(config) != {'$schema', 'mcpServers'}:
        diagnostics.append(AgentPluginDiagnostic('mcp', 'mcp.json has an invalid top-level shape'))
        return {}
    if config.get('$schema') != MCP_SCHEMA_V1:
        diagnostics.append(AgentPluginDiagnostic('mcp', 'mcp.json declares an unsupported schema'))
        return {}
    servers = config.get('mcpServers')
    if not isinstance(servers, dict):
        diagnostics.append(AgentPluginDiagnostic('mcp', 'mcpServers must be an object'))
        return {}
    ...
    for name, server in servers.items():
        ...
        if server_type == 'stdio':
            ...
            if set(server) - _REMOTE_FIELDS or not isinstance(server.get('url'), str) or (not server.get('url')) or (not _validate_headers(server.get('headers'))):
                diagnostics.append(AgentPluginDiagnostic(scope, 'invalid remote entry'))
            else:
                diagnostics.append(AgentPluginDiagnostic(scope, f'portable {server_type} transport is not supported'))
        else:
...
def load_agent_plugin(plugin_root: Path, data_root: Path) -> AgentPluginPackage:
    ...
    return AgentPluginPackage(name=manifest['name'], version=manifest.get('version', ''), description=manifest.get('description', ''), root=root, data_root=resolved_data, manifest=dict(manifest), skills=skills, mcp_servers=mcp_servers, diagnostics=tuple(diagnostics))
