...
def _model_options() -> list[dict[str, Any]]:
    payload = build_models_payload(load_picker_context(), include_unconfigured=False, picker_hints=True, canonical_order=True, pricing=True, capabilities=True, max_models=200)
...
def _pick_slot(current: dict[str, str] | None=None) -> dict[str, str]:
    ...
    if not providers:
        raise RuntimeError('No configured model providers found. Run `duck-agent model` first.')
...
def cmd_moa(args) -> None:
    ...
    if sub == 'delete':
        ...
        if not preset_name:
            raise SystemExit('Usage: duck-agent moa delete <name>')
