"""Migrate Duck Agent' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Duck Agent' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Duck Agent mcp_servers.<n>.command/args/env  → codex stdio transport
  Duck Agent mcp_servers.<n>.url/headers       → codex streamable_http transport
  Duck Agent mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Duck Agent mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Duck Agent-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Duck Agent' own
...
MIGRATION_MARKER = '# managed by duck-agent — `duck-agent codex-runtime migrate` regenerates this section'
MIGRATION_END_MARKER = '# end duck-agent managed section'
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append('No MCP servers found in Duck Agent config.')
        ...
        for err in self.errors:
            lines.append(f'⚠ {err}')
        return '\n'.join(lines)
_KNOWN_HERMES_KEYS = {'command', 'args', 'env', 'cwd', 'url', 'headers', 'transport', 'timeout', 'connect_timeout', 'enabled', 'description'}
_KEYS_DROPPED_WITH_WARNING = {'sampling'}

def _translate_one_server(name: str, hermes_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Duck Agent MCP server config to the codex inline-table dict
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f'{key} (unknown Duck Agent key)')
    return (out, skipped)
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        escaped = value.replace('\\', '\\\\').replace('"', '\\"').replace('\x08', '\\b').replace('\t', '\\t').replace('\n', '\\n').replace('\x0c', '\\f').replace('\r', '\\r')
        return f'"{escaped}"'
...
def render_codex_toml_section(servers: dict[str, dict], plugins: Optional[list[dict]]=None, default_permission_profile: Optional[str]=None) -> str:
    ...
    if not servers and (not plugins) and (not default_permission_profile):
        out.append('# (no MCP servers, plugins, or permissions configured by Duck Agent)')
        out.append(MIGRATION_END_MARKER)
        return '\n'.join(out) + '\n'
    if default_permission_profile:
        normalized = default_permission_profile if default_permission_profile.startswith(':') else f':{default_permission_profile}'
        out.append('')
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Duck Agent' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Duck Agent' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Duck Agent
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
    ...
    for line in lines:
        ...
        if in_managed:
            if line_stripped_nl == MIGRATION_END_MARKER:
                ...
                continue
            stripped = line.lstrip()
            if not saw_end_marker and stripped.startswith('[') and (not (stripped.startswith('[mcp_servers') or stripped.startswith('[plugins') or stripped.startswith('[permissions]') or stripped.startswith('[permissions.'))):
                in_managed = False
                out.append(line)
                continue
            continue
...
def _query_codex_plugins(codex_home: Optional[Path]=None, timeout: float=8.0) -> tuple[list[dict], Optional[str]]:
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name='duck-agent-migration')
    ...
    for marketplace in marketplaces:
        ...
        for plugin in plugins:
            ...
            if availability and availability != 'AVAILABLE':
                logger.debug('skipping plugin %s: availability=%s', plugin.get('name'), availability)
                continue
            ...
            if key in seen:
                continue
            seen.add(key)
            out.append({'name': name, 'marketplace': market_name, 'enabled': bool(plugin.get('enabled', True))})
    return (out, None)
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a DUCK_AGENT_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed duck-agent-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.duck-agent`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Duck Agent' own
    ...
    so a duck-agent installed under /opt/, /usr/local/, or a venv all work.
    DUCK_AGENT_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys
    env: dict[str, str] = {}
    hermes_home = os.environ.get('DUCK_AGENT_HOME') or ''
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ''
    if hermes_home:
        env['DUCK_AGENT_HOME'] = hermes_home
    pythonpath = os.environ.get('PYTHONPATH')
    if pythonpath:
        env['PYTHONPATH'] = pythonpath
    env['HERMES_QUIET'] = '1'
    env['HERMES_REDACT_SECRETS'] = env.get('HERMES_REDACT_SECRETS', 'true')
    out: dict[str, Any] = {'command': sys.executable, 'args': ['-m', 'agent.transports.hermes_tools_mcp_server']}
    if env:
        out['env'] = env
...
def migrate(hermes_config: dict, *, codex_home: Optional[Path]=None, dry_run: bool=False, discover_plugins: bool=True, default_permission_profile: Optional[str]=':workspace', expose_hermes_tools: bool=True) -> MigrationReport:
    """Translate Duck Agent mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.duck-agent/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Duck Agent' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Duck Agent for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append('mcp_servers in Duck Agent config is not a dict; cannot migrate.')
        return report
    translated: dict[str, dict] = {}
    for name, cfg in hermes_servers.items():
        out, skipped = _translate_one_server(str(name), cfg or {})
        if out is None:
            report.errors.append(f"server {name!r} skipped: {', '.join(skipped) or 'no transport configured'}")
            continue
    ...
    if expose_hermes_tools:
        translated['duck-agent-tools'] = _build_hermes_tools_mcp_entry()
        if 'duck-agent-tools' not in report.migrated:
            report.migrated.append('duck-agent-tools')
    managed_block = render_codex_toml_section(translated, plugins=plugins, default_permission_profile=default_permission_profile)
    if target.exists():
        try:
    ...
    try:
        ...
        import tempfile
        tmp_fd, tmp_path_str = tempfile.mkstemp(prefix='.config.toml.', dir=str(codex_home))
