...
remains ``duck-agent cron`` / the ``cronjob`` tool, which runs in isolated
...
POLL_SECONDS = 5.0
HEARTBEAT_PROMPT_TEMPLATE = '[Heartbeat — recurring instruction, fires every {interval}]\n{prompt}\n\nIf there is nothing meaningful to do or report for this instruction right now, reply briefly that nothing has changed and stop — do not invent work.'
_INTERVAL_RE = re.compile('^\\s*(?:every\\s+)?(\\d+(?:\\.\\d+)?)\\s*(s|sec|secs|seconds?|m|min|mins|minutes?|h|hr|hrs|hours?|d|days?)\\s*$', re.IGNORECASE)
_UNIT_SECONDS = {'s': 1, 'sec': 1, 'secs': 1, 'second': 1, 'seconds': 1, 'm': 60, 'min': 60, 'mins': 60, 'minute': 60, 'minutes': 60, 'h': 3600, 'hr': 3600, 'hrs': 3600, 'hour': 3600, 'hours': 3600, 'd': 86400, 'day': 86400, 'days': 86400}
...
class HeartbeatState:
    ...
    def from_json(cls, raw: str) -> 'HeartbeatState':
        data = json.loads(raw)
        return cls(prompt=str(data.get('prompt') or ''), interval_seconds=int(data.get('interval_seconds', 0) or 0), status=str(data.get('status') or 'active'), created_at=float(data.get('created_at', 0.0) or 0.0), last_fired_at=float(data.get('last_fired_at', 0.0) or 0.0), fire_count=int(data.get('fire_count', 0) or 0))
...
def migrate_heartbeat_to_session(old_session_id: str, new_session_id: str) -> bool:
    ...
    try:
        ...
        return False
__all__ = ['HeartbeatState', 'HeartbeatManager', 'parse_interval', 'format_interval', 'load_heartbeat', 'save_heartbeat', 'migrate_heartbeat_to_session', 'HEARTBEAT_PROMPT_TEMPLATE', 'MIN_INTERVAL_SECONDS', 'POLL_SECONDS']