"""Duck Agent middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = 'duck-agent.observer.v1'
MIDDLEWARE_SCHEMA_VERSION = 'duck-agent.middleware.v1'
...
VALID_MIDDLEWARE: set[str] = {TOOL_REQUEST_MIDDLEWARE, TOOL_EXECUTION_MIDDLEWARE, LLM_REQUEST_MIDDLEWARE, LLM_EXECUTION_MIDDLEWARE}
...
def apply_llm_request_middleware(request: Dict[str, Any], **context: Any) -> RequestMiddlewareResult:
    ...
    provider kwargs before Duck Agent sends them.
    """
    if not _has_middleware(LLM_REQUEST_MIDDLEWARE):
        return RequestMiddlewareResult(payload=request, original_payload=request, changed=False, trace=[])
    original_request = _safe_copy(request)
    current_request = _safe_copy(original_request)
    trace: List[Dict[str, Any]] = []
    for result in _invoke_middleware(LLM_REQUEST_MIDDLEWARE, request=current_request, original_request=original_request, **context):
    ...
    return RequestMiddlewareResult(payload=current_request, original_payload=original_request, changed=bool(trace), trace=trace)
...
def apply_tool_request_middleware(tool_name: str, args: Dict[str, Any], **context: Any) -> RequestMiddlewareResult:
    ...
    if session_id and (not skip_relay):
        ...
        relay_args = relay_runtime.apply_tool_request_intercepts(session_id=session_id, tool_name=tool_name, args=current_args)
    ...
    if not _has_middleware(TOOL_REQUEST_MIDDLEWARE):
        return RequestMiddlewareResult(payload=args if not trace else current_args, original_payload=args, changed=bool(trace), trace=trace)
    for result in _invoke_middleware(TOOL_REQUEST_MIDDLEWARE, tool_name=tool_name, args=current_args, original_args=original_args, **context):
        if not isinstance(result, dict):
            continue
        ...
        trace.append(_trace_entry(result))
    return RequestMiddlewareResult(payload=current_args, original_payload=original_args, changed=bool(trace), trace=trace)
...
def run_llm_execution_middleware(request: Dict[str, Any], next_call: Callable[[Dict[str, Any]], Any], **context: Any) -> Any:
    ...
    return _run_execution_chain(LLM_EXECUTION_MIDDLEWARE, callbacks, next_call, request=request, original_request=context.pop('original_request', request), **context)
...
def run_tool_execution_middleware(tool_name: str, args: Dict[str, Any], next_call: Callable[[Dict[str, Any]], Any], **context: Any) -> Any:
    ...
    return _run_execution_chain(TOOL_EXECUTION_MIDDLEWARE, callbacks, next_call, tool_name=tool_name, args=args, original_args=context.pop('original_args', args), **context)
...
def _run_execution_chain(kind: str, callbacks: List[Callable], terminal_call: Callable[[Any], Any], **kwargs: Any) -> Any:
    ...
    def call_at(index: int, payload: Any) -> Any:
        ...
        def next_call(next_payload: Any=None) -> Any:
            ...
            if next_called:
                raise RuntimeError(f"Middleware '{kind}' callback {getattr(callback, '__name__', repr(callback))} called next_call() more than once; downstream execution is single-use")
            next_called = True
            try:
        ...
        try:
            ...
            logger.warning("Middleware '%s' callback %s raised: %s", kind, getattr(callback, '__name__', repr(callback)), exc)
            if next_succeeded:
                return next_result
            if next_called:
                raise
