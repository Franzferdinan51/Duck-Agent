...
__all__ = ['CommandContext', 'CommandReply', 'EXECUTORS', 'execute_command', 'resolve_executor', 'run_execute']
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    return CommandReply(f'Profile: {profile_name}\nHome: {home_display}', data={'profile': profile_name, 'home': home_display})
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(f'No skill bundles installed.\nCreate one with: duck-agent bundles create <name> --skill <s1> --skill <s2>\nDirectory: {bundles_dir}', data={'bundles': [], 'dir': bundles_dir})
...
def _exec_commands(ctx: CommandContext) -> CommandReply:
    ...
    lines = [t('gateway.commands.header', total=len(entries), page=page, total_pages=total_pages), '', *page_entries]
    if total_pages > 1:
        nav_parts = []
        if page > 1:
    ...
    return CommandReply('\n'.join(lines), format='markdown')
EXECUTORS: dict[str, Callable[[CommandContext], CommandReply]] = {'version': _exec_version, 'egress': _exec_egress, 'profile': _exec_profile, 'bundles': _exec_bundles, 'gateway_help': _exec_help, 'gateway_commands': _exec_commands}
