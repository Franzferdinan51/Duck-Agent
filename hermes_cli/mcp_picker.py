"""MCP picker — interactive `duck-agent mcp picker` (also the default `duck-agent mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``duck-agent mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `duck-agent plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from hermes_cli.mcp_catalog import CatalogEntry, CatalogError, catalog_diagnostics, install_entry, is_enabled, is_installed, list_catalog, installed_servers, uninstall_entry
...
def _build_rows() -> List[_Row]:
    ...
    for entry in catalog_entries:
        ...
        rows.append(_Row(name=entry.name, description=entry.description, status=status, entry=entry))
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    save_config(cfg)
    print(color(f"  ✓ '{name}' {('enabled' if enable else 'disabled')}. Start a new Duck Agent session for changes to take effect.", Colors.GREEN))
...
def _handle_row(row: _Row) -> None:
    ...
    if row.entry and (not is_enabled(row.name)):
        ...
        return
    if row.is_custom:
        actions = ['Configure tools (probe server + re-pick)', 'Enable' if not is_enabled(row.name) else 'Disable', 'Remove from config']
    ...
    actions = ['Configure tools (probe server + re-pick)', 'Disable (keep config, stop loading on next session)', 'Uninstall (remove config and any cloned files)', 'Reinstall (re-clone, re-prompt for credentials)']
    ...
    if choice == 0:
        ...
        if prompt_yes_no(f"Uninstall '{row.name}'?", default=False):
            if uninstall_entry(row.name):
                print(color(f"  ✓ Uninstalled '{row.name}'. Credentials in .env preserved — delete manually if no longer needed.", Colors.GREEN))
            else:
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `duck-agent mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color('  Install: duck-agent mcp install <name>    Picker: duck-agent mcp', Colors.DIM))
    diags = catalog_diagnostics()
    future = [d for d in diags if d[1] == 'future_manifest']
    if future:
        print()
        for name, _, msg in future:
            print(color(f"  ⚠ '{name}' requires a newer Duck Agent — run `duck-agent update` to install this entry.", Colors.YELLOW))
        print()
    print()

def show_catalog() -> None:
    """`duck-agent mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())

def run_picker() -> None:
    """`duck-agent mcp picker` (and default `duck-agent mcp`) — interactive selector.
    ...
    while True:
        ...
        idx = curses_single_select('MCP Catalog  —  ↑↓ navigate  ENTER act on entry  ESC/q quit', labels)
        if idx is None:
            return
        _handle_row(rows[idx])

def install_by_name(identifier: str) -> int:
    """`duck-agent mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(f"  ✗ '{identifier}' is not in the catalog. Run `duck-agent mcp catalog` to see available entries.", Colors.RED))
        return 1
    try:
