...
def handle_suggestions_command(args: str, *, origin: Optional[Dict[str, Any]]=None, surface: str='cli') -> str:
    ...
    if sub in ('accept', 'add', 'schedule'):
        ...
        manage = 'Manage it with /cron.' if surface == 'cli' else 'Ask me to list, pause, or remove it any time.'
    ...
    if sub in ('dismiss', 'no', 'reject'):
        ...
        return "Dismissed. Won't suggest that again." if ok else f"No pending suggestion matches '{rest}'."
    if sub == 'catalog':
        try:
        ...
        if not created:
            return 'No new catalog automations to add (already offered, dismissed, or your suggestion list is full). Run /suggestions to see pending.'
