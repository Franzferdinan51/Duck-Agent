"""
Security advisory checker for Duck Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``duck-agent doctor --ack <id>``; the ack is persisted to
...
1. ``duck-agent doctor`` (and ``duck-agent doctor --ack <id>``)
...
run in environments where the rest of Duck Agent failed to import.
"""
...
class Advisory:
    ...
    published: str = ''
    severity: str = 'high'
ADVISORIES: tuple[Advisory, ...] = (Advisory(id='shai-hulud-2026-05', title='Mini Shai-Hulud worm — mistralai 2.4.6 compromised on PyPI', summary='PyPI quarantined the mistralai package on 2026-05-12 after a malicious 2.4.6 release. The worm steals credentials from environment variables and credential files (~/.npmrc, ~/.pypirc, ~/.aws/credentials, GitHub PATs, cloud SDK tokens) and exfils them to a hardcoded webhook. If you ran any Python process that imported mistralai 2.4.6 — including duck-agent when configured with provider=mistral for TTS or STT — assume those credentials are exposed. PyPI has since removed 2.4.6 and the project ships clean releases again (2.4.7, 2.4.8); this advisory only fires if the compromised 2.4.6 is still installed.', url='https://socket.dev/blog/mini-shai-hulud-worm-pypi', compromised=(('mistralai', frozenset({'2.4.6'})),), remediation=('Run: pip uninstall -y mistralai  (or: uv pip uninstall mistralai)', 'Rotate API keys in ~/.duck-agent/.env (OpenRouter, Anthropic, OpenAI, Nous, GitHub, AWS, Google, Mistral, etc.).', 'Audit ~/.npmrc, ~/.pypirc, ~/.aws/credentials, ~/.config/gh/hosts.yml, and any other credential files for tokens that may have been read.', 'Check GitHub for unexpected new SSH keys, deploy keys, or webhook additions on repos you have admin on.', 'After cleanup: duck-agent doctor --ack shai-hulud-2026-05  to dismiss this warning.'), published='2026-05-12', severity='critical'),)
    ...
    for advisory in advisories:
        for pkg_name, bad_versions in advisory.compromised:
            ...
            if not bad_versions or installed in bad_versions:
                hits.append(AdvisoryHit(advisory=advisory, package=pkg_name, installed_version=installed))
    return hits
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    if not hits:
        return []
    primary = hits[0]
    lines = [f'SECURITY ADVISORY [{primary.advisory.id}]: {primary.advisory.title}', f'  Detected: {primary.package}=={primary.installed_version}', "  Run 'duck-agent doctor' for remediation steps."]
    if len(hits) > 1:
...
def full_remediation_text(hit: AdvisoryHit) -> list[str]:
    ...
    a = hit.advisory
    lines = [f'=== {a.title} ===', f'ID:        {a.id}    Severity: {a.severity}    Published: {a.published}', f'Detected:  {hit.package}=={hit.installed_version}', f'Reference: {a.url}', '', a.summary, '', 'Remediation:']
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``duck-agent doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    if len(fresh) == 1:
        h = fresh[0]
        return f'Security advisory [{h.advisory.id}] active: {h.package}=={h.installed_version} matches {h.advisory.title}. See {h.advisory.url}'
    return f"{len(fresh)} security advisories active (IDs: {', '.join((h.advisory.id for h in fresh))}). Run `duck-agent doctor` on the gateway host for details."