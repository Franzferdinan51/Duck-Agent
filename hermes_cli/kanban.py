"""CLI for the Duck Agent Kanban board — ``duck-agent kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/duck-agent-kanban-v1-spec.pdf``).  All DB work is delegated to
...
_STATUS_ICONS = {'todo': '◻', 'ready': '▶', 'running': '●', 'scheduled': '⏱', 'blocked': '⊘', 'done': '✓', 'archived': '—'}
...
def _parse_workspace_flag(value: str) -> tuple[str, Optional[str]]:
    ...
    raise argparse.ArgumentTypeError(f'unknown --workspace value {value!r}: use scratch, worktree, worktree:<path>, or dir:<path>')
...
def _check_dispatcher_presence(hermes_home: Optional[Path]=None) -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this DUCK_AGENT_HOME and its
    ...
    Used by ``duck-agent kanban create`` (and callers) to warn when a task
    ...
    ``duck_agent_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different DUCK_AGENT_HOME than the profile the request
    ...
    if pid and (not dispatch_on):
        return (False, "Gateway is running but kanban.dispatch_in_gateway=false in config.yaml — the task will sit in 'ready' until you flip it back on and restart the gateway, OR run the legacy standalone daemon (`duck-agent kanban daemon --force`).")
    return (False, "No gateway is running — the task will sit in 'ready' until you start it. Run:\n    duck-agent gateway start\nThe gateway hosts an embedded dispatcher (tick interval 60s by default); your task will be picked up on the next tick after the gateway comes up.")
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    """
    kanban_parser = parent_subparsers.add_parser('kanban', help='Multi-profile collaboration board (tasks, links, comments)', description='Durable SQLite-backed task board shared across Duck Agent profiles. Tasks are claimed atomically, can depend on other tasks, and are executed by a named profile in an isolated workspace. See https://duck-agent.nousresearch.com/docs/user-guide/features/kanban or docs/duck-agent-kanban-v1-spec.pdf for the full design.')
    kanban_parser.add_argument('--board', default=None, metavar='<slug>', help='Board slug to operate on. Defaults to the current board (set via `duck-agent kanban boards switch <slug>` or the HERMES_KANBAN_BOARD env var). Use `duck-agent kanban boards list` to see all boards.')
    sub = kanban_parser.add_subparsers(dest='kanban_action')
    sub.add_parser('init', help='Create kanban.db if missing (idempotent)')
    p_boards = sub.add_parser('boards', help='Manage kanban boards (one board per project / workstream)', description="Boards let you separate unrelated streams of work (projects, repos, domains) into isolated queues. Each board has its own DB, workspaces directory, and dispatcher loop — tasks on one board cannot collide with tasks on another. The first board is 'default' and always exists.")
    ...
    b_create.add_argument('--name', default=None, help='Human-readable display name (defaults to Title Case of slug)')
    b_create.add_argument('--description', default=None, help='Optional description')
    b_create.add_argument('--icon', default=None, help='Optional emoji or single-character icon for the dashboard')
    b_create.add_argument('--color', default=None, help="Optional hex color (e.g. '#8b5cf6') for the dashboard")
    b_create.add_argument('--switch', action='store_true', help='Switch to the new board after creating it')
    b_create.add_argument('--default-workdir', default=None, help='Default workspace path for tasks created on this board')
    b_rm = boards_sub.add_parser('rm', aliases=['remove', 'delete'], help='Archive (default) or delete a board')
    b_rm.add_argument('slug')
    b_rm.add_argument('--delete', action='store_true', help="Hard-delete the board directory instead of archiving it. Default is to move it to boards/_archived/ so it's recoverable.")
    b_switch = boards_sub.add_parser('switch', aliases=['use'], help='Set the active board for subsequent CLI calls')
    b_switch.add_argument('slug')
    boards_sub.add_parser('show', aliases=['current'], help='Print the currently-active board slug')
    b_rename = boards_sub.add_parser('rename', help="Change a board's human-readable display name (slug is immutable)")
    b_rename.add_argument('slug')
    b_rename.add_argument('name', help='New display name')
    b_set_wd = boards_sub.add_parser('set-default-workdir', help='Set the default workspace path for tasks on a board')
    b_set_wd.add_argument('slug')
    b_set_wd.add_argument('path', nargs='?', default=None, help='Absolute path to use as default workdir. Omit to clear.')
    p_create = sub.add_parser('create', help='Create a new task')
    p_create.add_argument('title', help='Task title')
    p_create.add_argument('--body', default=None, help='Optional opening post')
    p_create.add_argument('--assignee', default=None, help='Profile name to assign')
    p_create.add_argument('--parent', action='append', default=[], help='Parent task id (repeatable)')
    p_create.add_argument('--workspace', default='scratch', help='scratch | worktree | worktree:<path> | dir:<path> (default: scratch)')
    p_create.add_argument('--branch', default=None, help='Branch name for worktree tasks, e.g. wt/t6-wire')
    p_create.add_argument('--project', default=None, help="Link to a project (id or slug). Anchors the task's worktree under the project's primary repo with a deterministic branch. See `duck-agent project list`.")
    p_create.add_argument('--tenant', default=None, help='Tenant namespace')
    p_create.add_argument('--priority', type=int, default=0, help='Priority tiebreaker')
    p_create.add_argument('--triage', action='store_true', help='Park in triage — a specifier will flesh out the spec and promote to todo')
    p_create.add_argument('--idempotency-key', default=None, help='Dedup key. If a non-archived task with this key exists, its id is returned instead of creating a duplicate.')
    p_create.add_argument('--max-runtime', default=None, help='Per-task runtime cap. Accepts seconds (300) or durations (90s, 30m, 2h, 1d). When exceeded, the dispatcher SIGTERMs (then SIGKILLs) the worker and re-queues the task.')
    p_create.add_argument('--created-by', default='user', help='Author name recorded on the task (default: user)')
    p_create.add_argument('--skill', action='append', default=[], dest='skills', help='Skill to force-load into the worker (repeatable). The kanban lifecycle is already injected automatically. Example: --skill translation --skill github-code-review')
    p_create.add_argument('--max-retries', type=int, default=None, metavar='N', help=f"Per-task override for the consecutive-failure circuit breaker. Trip on the Nth failure — e.g. --max-retries 1 blocks on the first failure (no retries), --max-retries 3 allows two retries. Omit to use the dispatcher's kanban.failure_limit config (default {kb.DEFAULT_FAILURE_LIMIT}).")
    p_create.add_argument('--model', default=None, dest='model_override', help="Pin the worker to this model (passed as -m <model>) without changing the profile's configured model. Combine with --provider when the model belongs to a different backend than the profile's default.")
    p_create.add_argument('--provider', default=None, dest='provider_override', help='Provider the --model belongs to (passed as --provider <name> to the worker). Requires --model.')
    p_create.add_argument('--goal', action='store_true', dest='goal_mode', help="Run the worker in a goal loop: after each turn a judge checks the response against the card title/body and, if not done, the worker keeps going in the same session until the judge agrees it's complete (or the turn budget runs out, which blocks the card for review). Best for open-ended cards one shot rarely finishes.")
    p_create.add_argument('--goal-max-turns', type=int, default=None, metavar='N', dest='goal_max_turns', help='Turn budget for --goal workers (default 20). Ignored without --goal.')
    p_create.add_argument('--initial-status', choices=sorted(kb.VALID_INITIAL_STATUSES), default='running', help="Initial card status. Use 'blocked' for cards that require immediate human ops (R3 gate) to skip the brief running-to-blocked transition.")
    p_create.add_argument('--json', action='store_true', help='Emit JSON output')
    p_swarm = sub.add_parser('swarm', help='Create a Kanban Swarm v1 graph (parallel workers → verifier → synthesizer)')
    p_swarm.add_argument('goal', help='Swarm goal / final outcome')
    p_swarm.add_argument('--worker', action='append', default=[], metavar='PROFILE:TITLE[:SKILL,SKILL]', help='Parallel worker card (repeatable)')
    ...
    p_list.add_argument('--session', default=None, help='Filter by originating chat/agent session id (set on tasks created from inside an ACP loop)')
    p_list.add_argument('--archived', action='store_true', help='Include archived tasks')
    p_list.add_argument('--json', action='store_true')
    p_list.add_argument('--sort', default=None, choices=sorted(kb.VALID_SORT_ORDERS.keys()), help='Sort order for listed tasks (default: priority)')
    p_list.add_argument('--workflow-template-id', default=None, metavar='ID', help='Restrict to tasks with this workflow_template_id')
    p_list.add_argument('--step-key', default=None, dest='current_step_key', metavar='KEY', help='Restrict to tasks with this current_step_key')
    p_show = sub.add_parser('show', help='Show a task with comments + events')
    p_show.add_argument('task_id')
    p_show.add_argument('--json', action='store_true')
    p_show.add_argument('--state-type', choices=('status', 'outcome'), default=None, help='With --state-name: filter listed runs by task_runs column')
    p_show.add_argument('--state-name', default=None, metavar='VALUE', help='With --state-type: keep runs whose column equals this value')
    ...
    p_set_model.add_argument('model', nargs='?', default=None, help="Model to pin the worker to (or 'none' to clear the override)")
    p_set_model.add_argument('--provider', default=None, help='Provider the model belongs to (worker is spawned with --provider <name>). Cleared together with the model.')
    p_reclaim = sub.add_parser('reclaim', help='Release an active worker claim on a running task')
    p_reclaim.add_argument('task_id')
    p_reclaim.add_argument('--reason', default=None, help='Human-readable reason (recorded on the reclaimed event)')
    p_reassign = sub.add_parser('reassign', help='Reassign a task to a different profile, optionally reclaiming first')
    p_reassign.add_argument('task_id')
    p_reassign.add_argument('profile', help="New profile name (or 'none' to unassign)")
    p_reassign.add_argument('--reclaim', action='store_true', help='Release any active claim before reassigning (required if task is running)')
    p_reassign.add_argument('--reason', default=None, help='Human-readable reason (recorded on the reclaimed event)')
    p_diag = sub.add_parser('diagnostics', aliases=['diag'], help='List active diagnostics on the current board')
    p_diag.add_argument('--severity', choices=['warning', 'error', 'critical'], default=None, help='Only show diagnostics at or above this severity')
    p_diag.add_argument('--task', default=None, help='Only show diagnostics for one task id')
    p_diag.add_argument('--json', action='store_true', help='Emit JSON (structured) instead of the default human table')
    ...
    p_claim.add_argument('--ttl', type=int, default=kb.DEFAULT_CLAIM_TTL_SECONDS, help='Claim TTL in seconds (default: 900)')
    ...
    p_comment.add_argument('--author', default=None, help="Author name (default: $HERMES_PROFILE or 'user')")
    p_comment.add_argument('--max-len', type=int, default=None, help='Trim the stored comment body to this many characters')
    ...
    p_attach.add_argument('--content-type', default=None, help='MIME type (default: guessed from the file extension)')
    p_attach.add_argument('--name', default=None, help="Stored filename (default: the source file's basename)")
    p_attach.add_argument('--author', default=None, help="uploaded_by label (default: $HERMES_PROFILE or 'user')")
    ...
    p_complete.add_argument('task_ids', nargs='+', help='One or more task ids (only --result applies to all of them)')
    p_complete.add_argument('--result', default=None, help='Result summary')
    p_complete.add_argument('--summary', default=None, help='Structured handoff summary for downstream tasks. Falls back to --result if omitted.')
    p_complete.add_argument('--metadata', default=None, help='JSON dict of structured facts (e.g. \'{"changed_files": [...], "tests_run": 12}\'). Stored on the closing run.')
    p_edit = sub.add_parser('edit', help='Edit recovery fields on an already-completed task')
    p_edit.add_argument('task_id')
    p_edit.add_argument('--result', required=True, help='Backfilled task result text for a done task')
    p_edit.add_argument('--summary', default=None, help='Structured handoff summary. Falls back to --result if omitted.')
    p_edit.add_argument('--metadata', default=None, help='JSON dict of structured facts to store on the latest completed run.')
    p_block = sub.add_parser('block', help='Mark one or more tasks blocked')
    p_block.add_argument('task_id')
    p_block.add_argument('reason', nargs='*', help='Reason (also appended as a comment)')
    p_block.add_argument('--ids', nargs='+', default=None, help='Additional task ids to block with the same reason (bulk mode)')
    p_block.add_argument('--kind', default=None, choices=sorted(kb.VALID_BLOCK_KINDS), help="Typed block reason. 'dependency' waits in todo (auto-promoted when parents finish, no human); 'needs_input'/'capability' go to blocked for a human; 'transient' marks a maybe-flaky failure. Repeated same-kind re-blocks after unblock route the task to triage to break unblock loops. Omit for a generic block.")
    p_schedule = sub.add_parser('schedule', help='Park one or more tasks in Scheduled (waiting on time, not human input)')
    p_schedule.add_argument('task_id')
    p_schedule.add_argument('reason', nargs='*', help='Reason/timing note (also appended as a comment)')
    p_schedule.add_argument('--ids', nargs='+', default=None, help='Additional task ids to schedule with the same reason (bulk mode)')
    p_unblock = sub.add_parser('unblock', help='Return blocked/scheduled tasks to ready, or todo while parents remain open')
    p_unblock.add_argument('--reason', default=None, help='Optional reason/note — recorded as a comment before unblocking. Quote multi-word reasons.')
    p_unblock.add_argument('task_ids', nargs='+')
    p_promote = sub.add_parser('promote', help='Manually move one or more todo/blocked tasks to ready (recovery path)')
    ...
    p_promote.add_argument('--force', action='store_true', help='Promote even if parent dependencies are not yet done/archived')
    p_promote.add_argument('--dry-run', action='store_true', help='Validate the promotion without mutating state')
    p_promote.add_argument('--json', dest='json', action='store_true', help='Emit machine-readable JSON result')
    p_archive = sub.add_parser('archive', help='Archive one or more tasks')
    p_archive.add_argument('task_ids', nargs='*', help='Task ids to archive (default mode)')
    p_archive.add_argument('--rm', dest='purge_ids', nargs='+', default=None, help='Permanently delete already-archived task ids from the board')
    p_tail = sub.add_parser('tail', help="Follow a task's event stream")
    p_tail.add_argument('task_id')
    p_tail.add_argument('--interval', type=float, default=1.0)
    p_disp = sub.add_parser('dispatch', help='One dispatcher pass: reclaim stale, promote ready, spawn workers')
    p_disp.add_argument('--dry-run', action='store_true', help="Don't actually spawn processes; just print what would happen")
    p_disp.add_argument('--max', type=int, default=None, help='Cap number of spawns this pass')
    p_disp.add_argument('--failure-limit', type=int, default=kb.DEFAULT_SPAWN_FAILURE_LIMIT, help=f'Auto-block a task after this many consecutive non-success attempts (spawn_failed, timed_out, or crashed; default: {kb.DEFAULT_SPAWN_FAILURE_LIMIT})')
    p_disp.add_argument('--json', action='store_true')
    p_daemon = sub.add_parser('daemon', help='DEPRECATED — dispatcher now runs in the gateway. Use `duck-agent gateway start`.')
    p_daemon.add_argument('--interval', type=float, default=60.0, help='Seconds between dispatch ticks (default: 60)')
    ...
    p_daemon.add_argument('--pidfile', default=None, help="Write the daemon's PID to this file on start")
    p_daemon.add_argument('--verbose', '-v', action='store_true', help="Log each tick's outcome to stdout")
    p_daemon.add_argument('--force', action='store_true', help=argparse.SUPPRESS)
    p_watch = sub.add_parser('watch', help='Live-stream task_events to the terminal (Ctrl+C to exit)')
    p_watch.add_argument('--assignee', default=None, help='Only show events for tasks assigned to this profile')
    p_watch.add_argument('--tenant', default=None, help='Only show events from tasks in this tenant')
    p_watch.add_argument('--kinds', default=None, help="Comma-separated event kinds to include (e.g. 'completed,blocked,gave_up,crashed,timed_out')")
    p_watch.add_argument('--interval', type=float, default=0.5, help='Poll interval in seconds (default: 0.5)')
    p_stats = sub.add_parser('stats', help='Per-status + per-assignee counts + oldest-ready age')
    p_stats.add_argument('--json', action='store_true')
    p_nsub = sub.add_parser('notify-subscribe', help="Subscribe a gateway source to a task's terminal events (used by /kanban subscribe in the gateway adapter)")
    ...
    p_nsub.add_argument('--notifier-profile', default=None, help='Profile gateway that owns/delivers this subscription (default: active profile)')
    p_nlist = sub.add_parser('notify-list', help='List notification subscriptions (optionally for a single task)')
    ...
    p_log = sub.add_parser('log', help='Print the worker log for a task (from <kanban-root>/kanban/logs/)')
    p_log.add_argument('task_id')
    p_log.add_argument('--tail', type=int, default=None, help='Only print the last N bytes')
    p_runs = sub.add_parser('runs', help='Show attempt history for a task (one row per run: profile, outcome, elapsed, summary)')
    p_runs.add_argument('task_id')
    p_runs.add_argument('--json', action='store_true')
    p_runs.add_argument('--state-type', choices=('status', 'outcome'), default=None, help='With --state-name: filter runs by task_runs column')
    p_runs.add_argument('--state-name', default=None, metavar='VALUE', help='With --state-type: keep runs whose column equals this value')
    p_hb = sub.add_parser('heartbeat', help='Emit a heartbeat event for a running task (worker liveness signal)')
    p_hb.add_argument('task_id')
    p_hb.add_argument('--note', default=None, help='Optional short note attached to the heartbeat event')
    p_asg = sub.add_parser('assignees', help='List known profiles + per-profile task counts (union of ~/.duck-agent/profiles/ and current assignees on the board)')
    p_asg.add_argument('--json', action='store_true')
    p_ctx = sub.add_parser('context', help='Print the full context a worker sees for a task (title + body + parent results + comments).')
    p_ctx.add_argument('task_id')
    p_specify = sub.add_parser('specify', help='Flesh out a triage-column task into a concrete spec (title + body) and promote it to todo. Uses the auxiliary LLM configured under auxiliary.triage_specifier.')
    p_specify.add_argument('task_id', nargs='?', default=None, help='Task id to specify (required unless --all is given)')
    p_specify.add_argument('--all', dest='all_triage', action='store_true', help='Specify every task currently in the triage column')
    p_specify.add_argument('--tenant', default=None, help='When used with --all, restrict the sweep to this tenant')
    p_specify.add_argument('--author', default=None, help="Author name recorded on the audit comment (default: $HERMES_PROFILE or 'specifier')")
    p_specify.add_argument('--json', action='store_true', help='Emit one JSON object per task on stdout')
    p_decompose = sub.add_parser('decompose', help="Decompose a triage-column task into a graph of child tasks routed to specialist profiles by description. Falls back to specify-style single-task promotion when the task doesn't benefit from fan-out. Uses auxiliary.kanban_decomposer.")
    p_decompose.add_argument('task_id', nargs='?', default=None, help='Task id to decompose (required unless --all is given)')
    p_decompose.add_argument('--all', dest='all_triage', action='store_true', help='Decompose every task currently in the triage column')
    p_decompose.add_argument('--tenant', default=None, help='When used with --all, restrict the sweep to this tenant')
    p_decompose.add_argument('--author', default=None, help="Author name recorded on the audit comment (default: $HERMES_PROFILE or 'decomposer')")
    p_decompose.add_argument('--json', action='store_true', help='Emit one JSON object per task on stdout')
    p_gc = sub.add_parser('gc', help='Garbage-collect archived-task workspaces, old events, and old logs')
    p_gc.add_argument('--event-retention-days', type=int, default=30, help='Delete task_events older than N days for terminal tasks (default: 30)')
    p_gc.add_argument('--log-retention-days', type=int, default=30, help='Delete worker log files older than N days (default: 30)')
    p_repair = sub.add_parser('repair', help='Check kanban.db integrity and auto-repair index-only corruption', description="Runs PRAGMA integrity_check on the board's DB and reports the result. When the failure consists only of index-scoped errors ('wrong # of entries in index <name>' / 'row N missing from index <name>'), the corrupt file is quarantined to a .corrupt.<hash>.bak sibling first and the damaged indexes are rebuilt with REINDEX — the same narrow auto-repair the connect-time guard applies. Any other corruption class is reported and left untouched (fail-closed). Exits 0 when the DB is healthy or was repaired, non-zero when it is still corrupt.")
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``duck-agent kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: duck-agent kanban <action> [options]\nRun 'duck-agent kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    if _is_delegated_child_cli_mutation(args):
        print('kanban: delegate_task child contexts cannot mutate Kanban tasks via the CLI', file=sys.stderr)
        return 1
    if action == 'boards':
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and (not kb.board_exists(normed)):
            print(f'kanban: board {normed!r} does not exist. Create it with `duck-agent kanban boards create {normed}`.', file=sys.stderr)
            return 1
    ...
    with board_scope:
        ...
        try:
            ...
            return 1
        handlers = {'init': _cmd_init, 'create': _cmd_create, 'swarm': _cmd_swarm, 'list': _cmd_list, 'ls': _cmd_list, 'show': _cmd_show, 'assign': _cmd_assign, 'set-model': _cmd_set_model, 'reclaim': _cmd_reclaim, 'reassign': _cmd_reassign, 'diagnostics': _cmd_diagnostics, 'diag': _cmd_diagnostics, 'link': _cmd_link, 'unlink': _cmd_unlink, 'claim': _cmd_claim, 'comment': _cmd_comment, 'attach': _cmd_attach, 'attachments': _cmd_attachments, 'attach-rm': _cmd_attach_rm, 'complete': _cmd_complete, 'edit': _cmd_edit, 'block': _cmd_block, 'schedule': _cmd_schedule, 'unblock': _cmd_unblock, 'promote': _cmd_promote, 'archive': _cmd_archive, 'tail': _cmd_tail, 'dispatch': _cmd_dispatch, 'daemon': _cmd_daemon, 'watch': _cmd_watch, 'stats': _cmd_stats, 'log': _cmd_log, 'runs': _cmd_runs, 'heartbeat': _cmd_heartbeat, 'assignees': _cmd_assignees, 'notify-subscribe': _cmd_notify_subscribe, 'notify-list': _cmd_notify_list, 'notify-unsubscribe': _cmd_notify_unsubscribe, 'context': _cmd_context, 'specify': _cmd_specify, 'decompose': _cmd_decompose, 'gc': _cmd_gc}
...
def _profile_author() -> str:
    ...
    try:
        ...
        return 'user'
_DELEGATED_CHILD_DENIED_ACTIONS: frozenset[str] = frozenset({'init', 'create', 'swarm', 'assign', 'reclaim', 'reassign', 'link', 'unlink', 'claim', 'comment', 'attach', 'attach-rm', 'complete', 'edit', 'block', 'schedule', 'unblock', 'promote', 'archive', 'dispatch', 'daemon', 'repair', 'heartbeat', 'notify-subscribe', 'notify-unsubscribe', 'specify', 'decompose', 'gc'})
_DELEGATED_CHILD_DENIED_BOARD_ACTIONS: frozenset[str] = frozenset({'create', 'new', 'rm', 'remove', 'delete', 'switch', 'use', 'rename', 'set-default-workdir'})
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``duck-agent kanban boards <action>``.
    ...
    DB, so a fresh DUCK_AGENT_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if getattr(args, 'json', False):
        ...
        return 0
    if not boards:
        print('(no boards — create one with `duck-agent kanban boards create <slug>`)')
        return 0
    ...
    if len(boards) > 1:
        print('Switch boards with `duck-agent kanban boards switch <slug>`.')
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    meta = kb.create_board(normed, name=args.name, description=args.description, icon=args.icon, color=args.color, default_workdir=args.default_workdir)
    ...
    if getattr(args, 'switch', False):
        ...
        print(f"  Use `duck-agent kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_boards_switch(args: argparse.Namespace) -> int:
    ...
    if not kb.board_exists(normed):
        print(f'kanban boards switch: board {normed!r} does not exist. Create it with `duck-agent kanban boards create {normed}`.', file=sys.stderr)
        return 1
...
def _cmd_boards_show(args: argparse.Namespace) -> int:
    ...
    print(f"  DB path:      {meta['db_path']}")
    print(f'  Tasks:        {total} total' + (f" ({', '.join((f'{k}={v}' for k, v in sorted(counts.items())))})" if counts else ''))
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print('No profiles found under ~/.duck-agent/profiles/.')
        print('Create one with `duck-agent -p <name> setup` before assigning tasks.')
    print()
    print('Next step: start the gateway so ready tasks actually get picked up.')
    print('  duck-agent gateway start')
    print()
    print("The gateway hosts an embedded dispatcher that ticks every 60 seconds\nby default (config: kanban.dispatch_interval_seconds). Without a\nrunning gateway, tasks stay in 'ready' forever.")
    return 0

def _cmd_heartbeat(args: argparse.Namespace) -> int:
    with kb.connect_closing() as conn:
        ok = kb.heartbeat_worker(conn, args.task_id, note=getattr(args, 'note', None), expected_run_id=_worker_run_id_for(args.task_id))
    if not ok:
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, 'json', False):
        ...
        return 0
    if not data:
        print('(no assignees — create a profile with `duck-agent -p <name> setup`)')
        return 0
...
def _cmd_swarm(args: argparse.Namespace) -> int:
    ...
    if not workers:
        ...
        return 2
    with kb.connect_closing() as conn:
        created = ks.create_swarm(conn, goal=args.goal, workers=workers, verifier_assignee=args.verifier, synthesizer_assignee=args.synthesizer, tenant=args.tenant, created_by=args.created_by or _profile_author(), priority=args.priority, idempotency_key=getattr(args, 'idempotency_key', None))
    if getattr(args, 'json', False):
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ...
        tasks = kb.list_tasks(conn, assignee=assignee, status=args.status, tenant=args.tenant, session_id=args.session, include_archived=args.archived, order_by=getattr(args, 'sort', None), workflow_template_id=args.workflow_template_id, current_step_key=args.current_step_key)
    if getattr(args, 'json', False):
    ...
    if len(all_boards) > 1:
        ...
        print(f"Board: {current} ({other_count} other board{('s' if other_count != 1 else '')} — `duck-agent kanban boards list`)\n")
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if rsk is None:
        print('kanban show: pass both --state-type and --state-name, or omit both', file=sys.stderr)
        return 2
    with kb.connect_closing() as conn:
    ...
    if getattr(args, 'json', False):
        payload = {'task': _task_to_dict(task), 'latest_summary': latest_summary, 'parents': parents, 'children': children, 'comments': [{'author': c.author, 'body': c.body, 'created_at': c.created_at} for c in comments], 'events': [{'kind': e.kind, 'payload': e.payload, 'created_at': e.created_at, 'run_id': e.run_id} for e in events], 'runs': [{'id': r.id, 'profile': r.profile, 'step_key': r.step_key, 'status': r.status, 'outcome': r.outcome, 'summary': r.summary, 'error': r.error, 'metadata': r.metadata, 'worker_pid': r.worker_pid, 'started_at': r.started_at, 'ended_at': r.ended_at} for r in runs]}
    ...
    print(f'  workspace: {task.workspace_kind}' + (f' @ {task.workspace_path}' if task.workspace_path else ''))
    if task.branch_name:
...
def _cmd_reclaim(args: argparse.Namespace) -> int:
    ...
    if not ok:
        print(f'cannot reclaim {args.task_id} (not running or unknown id)', file=sys.stderr)
        return 1
...
def _cmd_reassign(args: argparse.Namespace) -> int:
    profile = None if args.profile.lower() in {'none', '-', 'null'} else args.profile
    with kb.connect_closing() as conn:
        ok = kb.reassign_task(conn, args.task_id, profile, reclaim_first=bool(getattr(args, 'reclaim', False)), reason=getattr(args, 'reason', None))
    if not ok:
        print(f'cannot reassign {args.task_id} (unknown id, or still running — pass --reclaim to release first)', file=sys.stderr)
        return 1
    print(f"Reassigned {args.task_id} to {profile or '(unassigned)'}" + (' (claim reclaimed)' if getattr(args, 'reclaim', False) else ''))
    return 0
    ...
    with kb.connect_closing() as conn:
        if getattr(args, 'task', None):
            ...
            if task is None:
                ...
                return 1
            diags_by_task = {args.task: kd.compute_task_diagnostics(task, kb.list_events(conn, args.task), kb.list_runs(conn, args.task), config=diag_config)}
        else:
            ...
            if not ids:
                ...
                for row in conn.execute(f'SELECT * FROM task_events WHERE task_id IN ({placeholders}) ORDER BY id', tuple(ids)):
                    ev_by.setdefault(row['task_id'], []).append(row)
                run_by = {i: [] for i in ids}
                for row in conn.execute(f'SELECT * FROM task_runs WHERE task_id IN ({placeholders}) ORDER BY id', tuple(ids)):
                    run_by.setdefault(row['task_id'], []).append(row)
                diags_by_task = {}
                for r in rows:
                    tid = r['id']
                    dl = kd.compute_task_diagnostics(r, ev_by.get(tid, []), run_by.get(tid, []), config=diag_config)
                    if dl:
        ...
        if diags_by_task:
            ...
            for r in conn.execute(f'SELECT id, title, status, assignee FROM tasks WHERE id IN ({placeholders})', tuple(diags_by_task.keys())):
                meta[r['id']] = {'title': r['title'], 'status': r['status'], 'assignee': r['assignee']}
    if getattr(args, 'json', False):
        out_json = [{'task_id': tid, **meta.get(tid, {}), 'diagnostics': [d.to_dict() for d in dl]} for tid, dl in diags_by_task.items()]
    ...
    with kb.connect_closing() as conn:
        ...
        if task is None:
            ...
            if existing is None:
                print(f'no such task: {args.task_id}', file=sys.stderr)
                return 1
            print(f"cannot claim {args.task_id}: status={existing.status} lock={existing.claim_lock or '(none)'}", file=sys.stderr)
            return 1
...
def _cmd_attach(args: argparse.Namespace) -> int:
    ...
    try:
        with kb.connect_closing() as conn:
            att_id = kb.store_attachment_bytes(conn, args.task_id, name, data, content_type=content_type, uploaded_by=uploaded_by)
...
def _cmd_attachments(args: argparse.Namespace) -> int:
    ...
    if getattr(args, 'json', False):
        print(json.dumps([{'id': a.id, 'filename': a.filename, 'content_type': a.content_type, 'size': a.size, 'uploaded_by': a.uploaded_by, 'stored_path': a.stored_path, 'created_at': a.created_at} for a in atts], indent=2))
        return 0
    if not atts:
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    if len(ids) > 1 and (summary or raw_meta):
        print("kanban: --summary / --metadata are per-task and can't be used with multiple ids (would apply the same handoff to every task). Complete tasks one at a time, or drop the flags for the bulk close.", file=sys.stderr)
        return 2
    metadata = None
    if raw_meta:
        try:
        ...
        for tid in ids:
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    reason = ''
                    try:
                        verdict, reason, _, _, _ = judge_goal(goal=f"{task.title}\n\n{task.body or ''}".strip(), last_response=(summary or args.result or '').strip())
                    except Exception as judge_exc:
                        import logging as _logging
                        _logging.getLogger(__name__).warning('goal judge check failed, allowing completion: %s', judge_exc, exc_info=True)
                    if verdict != 'done':
                        print(f"kanban: goal completion of {tid} rejected by judge: {reason}. Provide evidence matching the task's acceptance criteria.", file=sys.stderr)
                        failed.append(tid)
                        continue
            if not kb.complete_task(conn, tid, result=args.result, summary=summary, metadata=metadata, expected_run_id=_worker_run_id_for(tid)):
                failed.append(tid)
...
def _cmd_edit(args: argparse.Namespace) -> int:
    ...
    if raw_meta:
        try:
            ...
            return 2
    with kb.connect_closing() as conn:
        if not kb.edit_completed_task_result(conn, args.task_id, result=args.result, summary=getattr(args, 'summary', None), metadata=metadata):
            print(f'cannot edit {args.task_id} (unknown id or task is not done)', file=sys.stderr)
            return 1
...
def _cmd_block(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            if not kb.block_task(conn, tid, reason=reason, kind=kind, expected_run_id=_worker_run_id_for(tid)):
                failed.append(tid)
                ...
                if where == 'todo':
                    ...
                    print(f'{tid} → triage (unblock loop detected — needs a human decision){suffix}')
                else:
...
def _cmd_schedule(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            if not kb.schedule_task(conn, tid, reason=reason, expected_run_id=_worker_run_id_for(tid)):
                failed.append(tid)
...
def _cmd_promote(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ok, err = kb.promote_task(conn, tid, actor=author, reason=reason, force=bool(args.force), dry_run=bool(args.dry_run))
            results.append({'task_id': tid, 'promoted': ok, 'dry_run': bool(args.dry_run), 'forced': bool(args.force), 'reason': reason, 'error': err})
...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    try:
        ...
        max_in_progress_per_profile = _coerce_positive_int(_kanban_cfg.get('max_in_progress_per_profile'))
        max_in_progress = _coerce_positive_int(_kanban_cfg.get('max_in_progress'))
        cli_max = getattr(args, 'max', None)
        max_spawn = cli_max if cli_max is not None else _coerce_positive_int(_kanban_cfg.get('max_spawn'))
    except Exception:
    ...
    with kb.connect_closing() as conn:
        res = kb.dispatch_once(conn, dry_run=args.dry_run, max_spawn=max_spawn, max_in_progress=max_in_progress, failure_limit=getattr(args, 'failure_limit', kb.DEFAULT_SPAWN_FAILURE_LIMIT), default_assignee=default_assignee, max_in_progress_per_profile=max_in_progress_per_profile)
    if getattr(args, 'json', False):
        print(json.dumps({'reclaimed': res.reclaimed, 'crashed': res.crashed, 'timed_out': res.timed_out, 'stale': res.stale, 'auto_blocked': res.auto_blocked, 'promoted': res.promoted, 'spawned': [{'task_id': tid, 'assignee': who, 'workspace': ws} for tid, who, ws in res.spawned], 'skipped_unassigned': res.skipped_unassigned, 'skipped_nonspawnable': res.skipped_nonspawnable, 'skipped_per_profile_capped': [{'task_id': tid, 'assignee': who, 'current': current} for tid, who, current in res.skipped_per_profile_capped], 'auto_assigned_default': res.auto_assigned_default}, indent=2))
        return 0
    ...
    if res.auto_assigned_default:
        print(f"Auto-assigned to kanban.default_assignee={default_assignee!r}: {', '.join(res.auto_assigned_default)}")
    ...
    if res.skipped_nonspawnable:
        print(f"Skipped (non-spawnable assignee — terminal lane, OK): {', '.join(res.skipped_nonspawnable)}")
    return 0
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    print(f'Kanban dispatcher running STANDALONE via --force (interval={args.interval}s, pid={os.getpid()}). Ctrl-C to stop. NOTE: if a gateway is also running with dispatch_in_gateway=true (default), you have two dispatchers racing for claims.', file=sys.stderr)
    HEALTH_WINDOW = 6
    ...
    def _on_tick(res):
        ...
        if health_state['bad_ticks'] >= HEALTH_WINDOW:
            ...
            if now - health_state['last_warn_at'] >= 300:
                print(f"[{_fmt_ts(now)}] WARN dispatcher stuck: ready queue non-empty for {health_state['bad_ticks']} consecutive ticks but 0 workers spawned successfully. Check profile health (venv, PATH, credentials) and `duck-agent kanban list --status ready` / `duck-agent kanban list --status blocked` for recent spawn_failed tasks.", file=sys.stderr, flush=True)
                health_state['last_warn_at'] = now
        if not verbose:
            return
        did_work = res.reclaimed or res.crashed or res.timed_out or res.promoted or res.spawned or res.auto_blocked or res.stale
        if did_work:
            print(f'[{_fmt_ts(int(time.time()))}] reclaimed={res.reclaimed} crashed={len(res.crashed)} timed_out={len(res.timed_out)} stale={len(res.stale)} promoted={res.promoted} spawned={len(res.spawned)} auto_blocked={len(res.auto_blocked)}', flush=True)

    def _ready_queue_nonempty() -> bool:
        """Cheap probe — is there at least one ready+assigned+unclaimed
        task whose assignee maps to a real Duck Agent profile (i.e. one the
        ...
        try:
            ...
            return False
    try:
        kb.run_daemon(interval=args.interval, max_spawn=args.max, failure_limit=getattr(args, 'failure_limit', kb.DEFAULT_SPAWN_FAILURE_LIMIT), on_tick=_on_tick)
    finally:
        if pidfile:
            try:
                Path(pidfile).unlink()
...
def _cmd_watch(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            with kb.connect_closing() as conn:
                rows = conn.execute('SELECT e.id, e.task_id, e.kind, e.payload, e.created_at,        t.assignee, t.tenant FROM task_events e LEFT JOIN tasks t ON t.id = e.task_id WHERE e.id > ? ORDER BY e.id ASC LIMIT 200', (cursor,)).fetchall()
            for r in rows:
                ...
                print(f"[{_fmt_ts(r['created_at'])}] {r['task_id']:10s} {r['kind']:18s} (@{r['assignee'] or '-'}){pl}", flush=True)
            time.sleep(max(0.1, args.interval))
    except KeyboardInterrupt:
        print('\n(stopped)')
        return 0
...
def _cmd_notify_subscribe(args: argparse.Namespace) -> int:
    with kb.connect_closing() as conn:
        if kb.get_task(conn, args.task_id) is None:
            print(f'no such task: {args.task_id}', file=sys.stderr)
            return 1
        kb.add_notify_sub(conn, task_id=args.task_id, platform=args.platform, chat_id=args.chat_id, chat_type=args.chat_type, thread_id=args.thread_id, user_id=args.user_id, notifier_profile=args.notifier_profile or _profile_author())
    print(f'Subscribed {args.platform}:{args.chat_id}' + (f':{args.thread_id}' if args.thread_id else '') + f' to {args.task_id}')
    return 0
...
def _cmd_notify_list(args: argparse.Namespace) -> int:
    ...
    for s in subs:
        ...
        print(f"  {s['task_id']:10s}  {s['platform']}:{s['chat_id']}{thr}  (since event {s['last_event_id']}){owner}")
    return 0

def _cmd_notify_unsubscribe(args: argparse.Namespace) -> int:
    with kb.connect_closing() as conn:
        ok = kb.remove_notify_sub(conn, task_id=args.task_id, platform=args.platform, chat_id=args.chat_id, thread_id=args.thread_id)
    if not ok:
...
def _cmd_log(args: argparse.Namespace) -> int:
    ...
    if content is None:
        print(f'(no log for {args.task_id} — task may not have spawned yet)', file=sys.stderr)
        return 1
...
def _cmd_runs(args: argparse.Namespace) -> int:
    ...
    rsk = _run_state_kwargs(args)
    if rsk is None:
        print('kanban runs: pass both --state-type and --state-name, or omit both', file=sys.stderr)
        return 2
    with kb.connect_closing() as conn:
        runs = kb.list_runs(conn, args.task_id, **rsk)
    if getattr(args, 'json', False):
        print(json.dumps([{'id': r.id, 'profile': r.profile, 'status': r.status, 'outcome': r.outcome, 'started_at': r.started_at, 'ended_at': r.ended_at, 'summary': r.summary, 'error': r.error, 'metadata': r.metadata, 'worker_pid': r.worker_pid, 'step_key': r.step_key} for r in runs], indent=2, ensure_ascii=False))
        return 0
    if not runs:
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    if args.task_id and all_flag:
        print('kanban: pass either a task id OR --all, not both', file=sys.stderr)
        return 2
    if all_flag:
        ids = spec.list_triage_ids(tenant=tenant)
        if not ids:
            msg = 'No triage tasks' + (f' for tenant {tenant!r}' if tenant else '') + '.'
            if want_json:
        ...
        print('kanban: specify requires a task id or --all', file=sys.stderr)
        return 2
    ok_count = 0
    fail_count = 0
    for tid in ids:
        ...
        if outcome.ok:
            ...
            fail_count += 1
        if want_json:
            print(json.dumps({'task_id': outcome.task_id, 'ok': outcome.ok, 'reason': outcome.reason, 'new_title': outcome.new_title}))
        elif outcome.ok:
            title_suffix = f' — retitled: {outcome.new_title!r}' if outcome.new_title else ''
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    if all_flag:
        ...
        print('kanban: decompose requires a task id or --all', file=sys.stderr)
        return 2
    ok_count = 0
    for tid in ids:
        ...
        if outcome.ok:
            ok_count += 1
        if want_json:
            print(json.dumps({'task_id': outcome.task_id, 'ok': outcome.ok, 'reason': outcome.reason, 'fanout': outcome.fanout, 'child_ids': outcome.child_ids, 'new_title': outcome.new_title}))
        elif outcome.ok:
            if outcome.fanout and outcome.child_ids:
                child_summary = ', '.join(outcome.child_ids)
                print(f'Decomposed {outcome.task_id} → {len(outcome.child_ids)} children ({child_summary}); root promoted to todo')
            else:
                title_suffix = f' — retitled: {outcome.new_title!r}' if outcome.new_title else ''
                print(f'Specified {outcome.task_id} → todo (no fanout){title_suffix}')
        else:
            print(f'kanban: decompose {outcome.task_id}: {outcome.reason}', file=sys.stderr)
    if not all_flag:
        return 0 if ok_count == 1 else 1
    return 0 if ok_count > 0 or not ids else 1
...
def _cmd_gc(args: argparse.Namespace) -> int:
    ...
    removed_ws = 0
    with kb.connect_closing() as conn:
        rows = conn.execute("SELECT id, workspace_kind, workspace_path FROM tasks WHERE status = 'archived'").fetchall()
    for row in rows:
    ...
    print(f'GC complete: {removed_ws} workspace(s), {removed_events} event row(s), {removed_logs} log file(s) removed')
    return 0
...
def _cmd_repair(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    if getattr(args, 'json', False):
        print(json.dumps({'status': report.status, 'db_path': str(report.db_path), 'messages': report.messages, 'post_repair_messages': report.post_repair_messages, 'backup_path': str(report.backup_path) if report.backup_path else None, 'reindexed': report.reindexed}, indent=2))
...
def run_slash(rest: str) -> str:
    ...
    kanban_parser.exit_on_error = False
