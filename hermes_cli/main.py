"""
Duck Agent CLI - Main entry point.

Usage:
    duck-agent                     # Interactive chat (default)
    duck-agent chat                # Interactive chat
    duck-agent gateway             # Run gateway in foreground
    duck-agent gateway start       # Start gateway as service
    duck-agent gateway stop        # Stop gateway service
    duck-agent gateway status      # Show gateway status
    duck-agent gateway install     # Install gateway service
    duck-agent gateway uninstall   # Uninstall gateway service
    ...
    duck-agent cron list           # List cron jobs
    duck-agent cron status         # Check if cron scheduler is running
    duck-agent doctor              # Check configuration and dependencies
    duck-agent honcho setup                    # Configure Honcho AI memory integration
    duck-agent honcho status                   # Show Honcho config and connection status
    duck-agent honcho sessions                 # List directory → session name mappings
    duck-agent honcho map <name>               # Map current directory to a session name
    duck-agent honcho peer                     # Show peer names and dialectic settings
    duck-agent honcho peer --user NAME         # Set user peer name
    duck-agent honcho peer --ai NAME           # Set AI peer name
    duck-agent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    duck-agent honcho mode                     # Show current memory mode
    duck-agent honcho mode [hybrid|honcho|local]  # Set memory mode
    duck-agent honcho tokens                   # Show token budget settings
    duck-agent honcho tokens --context N       # Set session.context() token cap
    duck-agent honcho tokens --dialectic N     # Set dialectic result char cap
    duck-agent honcho identity                 # Show AI peer identity representation
    duck-agent honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    duck-agent honcho migrate                  # Step-by-step migration guide: OpenClaw native → Duck Agent + Honcho
    duck-agent version             Show version
    duck-agent update              Update to latest version
    duck-agent uninstall           Uninstall Duck Agent
    duck-agent acp                 Run as an ACP server for editor integration
    duck-agent sessions browse     Interactive session picker with search

    duck-agent claw migrate --dry-run  # Preview migration without changes
"""
try:
...
def _run_and_exit_oneshot(prompt: str, *, model: object=None, provider: object=None, toolsets: object=None, usage_file: object=None) -> None:
    try:
        from hermes_cli.oneshot import run_oneshot
        rc = run_oneshot(prompt, model=model, provider=provider, toolsets=toolsets, usage_file=usage_file)
...
def _set_process_title() -> None:
    """Set the process title to 'duck-agent' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``duck-agent tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``duck-agent.exe``).
    """
    try:
        import setproctitle
        setproctitle.setproctitle('duck-agent')
        return
    except ImportError:
        pass
    import ctypes
...
def _config_default_interface_early() -> str:
    ...
    value = 'cli'
    try:
        home = os.environ.get('DUCK_AGENT_HOME')
        if home:
            cfg_path = os.path.join(home, 'config.yaml')
        else:
            cfg_path = os.path.join(os.path.expanduser('~'), '.duck-agent', 'config.yaml')
        if os.path.exists(cfg_path):
            import yaml as _yaml_iface
            with open(cfg_path, encoding='utf-8') as _f:
                raw = _yaml_iface.load(_f, Loader=getattr(_yaml_iface, 'CSafeLoader', None) or _yaml_iface.SafeLoader) or {}
...
def _wants_tui_early(argv: 'list[str] | None'=None) -> bool:
    ...
    cron jobs, pipes run ``duck-agent … chat -q`` with stdio on a pipe. This
...
def _suppress_mouse_residue_early() -> None:
    ...
    try:
        if not os.isatty(1):
            return
        os.write(1, b'\x1b[?1003l\x1b[?1002l\x1b[?1001l\x1b[?1000l\x1b[?9l\x1b[?1006l\x1b[?1005l\x1b[?1015l\x1b[?1016l\x1b[?2029l')
    except OSError:
        pass
...
def _try_ultrafast_version() -> bool:
    """Handle ``duck-agent --version`` before config/logging imports."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (duck-agent tools, duck-agent setup, duck-agent model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(f"Error: 'duck-agent {command_name}' requires an interactive terminal.\nIt cannot be run through a pipe or non-interactive subprocess.\nRun it directly in your terminal instead.", file=sys.stderr)
        sys.exit(1)
PROJECT_ROOT = Path(_project_root_str_fast())
_ensure_project_root_on_path_fast()

def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set DUCK_AGENT_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `duck-agent mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Duck Agent' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo duck-agent -p <name>` against the invoking user's home.
        ...
        try:
            ...
            return None
        candidate = home / '.duck-agent' / 'profiles' / name
        try:
            if candidate.is_dir():
            ...
            return None
        return None
    value_flags = {'-z', '--oneshot', '-m', '--model', '--provider', '-t', '--toolsets', '-r', '--resume', '-s', '--skills', '--usage-file'}
    ...
    while i < len(argv):
        ...
        if '=' not in arg and arg in value_flags and (i + 1 < len(argv)):
            i += 2
        elif '=' not in arg and arg in optional_value_flags and (i + 1 < len(argv)) and (not argv[i + 1].startswith('-')):
            i += 2
        else:
            i += 1
    if profile_name is not None and consume == 2:
        import re as _re
        if not _re.match('^[a-z0-9][a-z0-9_-]{0,63}$', profile_name):
            profile_name = None
            consume = 0
            profile_index = None
    hermes_home_env = os.environ.get('DUCK_AGENT_HOME', '')
    ...
    if profile_name is not None:
        try:
            ...
            return
        os.environ['DUCK_AGENT_HOME'] = hermes_home
...
try:
    ...
    _setup_logging(mode='gui' if next((arg for arg in sys.argv[1:] if not arg.startswith('-')), '') in {'dashboard', 'serve', 'gui', 'desktop'} else 'cli')
except Exception:
    pass
...
def _is_termux_startup_environment(env: dict[str, str] | None=None) -> bool:
    ...
    return bool(check.get('TERMUX_VERSION') or 'com.termux/files/usr' in prefix or prefix.startswith('/data/data/com.termux/'))
...
def _session_browse_picker(sessions: list) -> Optional[str]:
    ...
    try:
        ...
        def _match(s, query):
            ...
            q = query.lower()
            return q in (s.get('title') or '').lower() or q in (s.get('preview') or '').lower() or q in s.get('id', '').lower() or (q in (s.get('source') or '').lower())
...
def _resolve_workspace_key() -> Optional[str]:
    ...
    try:
        import subprocess
        result = subprocess.run(['git', 'rev-parse', '--show-toplevel'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
def _resolve_last_session(source: str='cli') -> Optional[str]:
    ...
    ``duck-agent -c`` from repo A continues repo A's last session rather than the
...
def _probe_container(cmd: list, backend: str, via_sudo: bool=False):
    ...
    try:
        ...
        print(f'Error: timed out waiting for {label} to respond.\nThe {backend} daemon may be unresponsive or starting up.', file=sys.stderr)
        sys.exit(1)
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'duck-agent')
    """
    ...
    if not runtime:
        print(f'Error: {backend} not found on PATH. Cannot route to container.', file=sys.stderr)
        sys.exit(1)
    sudo_path = None
    probe = _probe_container([runtime, 'inspect', '--format', 'ok', container_name], backend)
    if probe.returncode != 0:
        sudo_path = shutil.which('sudo')
        if sudo_path:
            probe2 = _probe_container([sudo_path, '-n', runtime, 'inspect', '--format', 'ok', container_name], backend, via_sudo=True)
            if probe2.returncode != 0:
                print(f'''Error: container '{container_name}' not found via {backend}.\n\nThe container is likely running as root. Your user cannot see it\nbecause {backend} uses per-user namespaces. Grant passwordless\nsudo for {backend} — the -n (non-interactive) flag is required\nbecause a password prompt would hang or break piped commands.\n\nOn NixOS:\n\n  security.sudo.extraRules = [{{\n    users = [ "{os.getenv('USER', 'your-user')}" ];\n    commands = [{{ command = "{runtime}"; options = [ "NOPASSWD" ]; }}];\n  }}];\n\nOr run: sudo duck-agent {' '.join(cli_args)}''', file=sys.stderr)
                sys.exit(1)
        else:
            print(f"Error: container '{container_name}' not found via {backend}.\nThe container may be running under root. Try: sudo duck-agent {' '.join(cli_args)}", file=sys.stderr)
            sys.exit(1)
    ...
    exec_cmd = cmd_prefix + ['exec'] + tty_flags + ['-u', exec_user] + env_flags + [container_name, hermes_bin] + cli_args
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str]=None) -> None:
    ...
    print(f'  duck-agent --tui --resume {target}')
    if title:
        print(f'  duck-agent --tui -c "{title}"')
    print()
    print(f'Session:        {target}')
    if title:
        print(f'Title:          {title}')
    print(f'Messages:       {message_count}')
    print(f'Tokens:         {total_tokens} (in {input_tokens}, out {output_tokens}, cache {cache_read_tokens + cache_write_tokens}, reasoning {reasoning_tokens})')
...
def _workspace_root(dir: Path) -> Path:
    ...
    directory).
    """
    if (dir / 'package.json').is_file() and (not (dir / 'package-lock.json').is_file()) and (dir.parent / 'package-lock.json').is_file():
        return dir.parent
    return dir
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @duck-agent/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    if entry.is_file() and (not lock.is_file()):
        return False
    ink = ws_root / 'node_modules' / '@duck-agent' / 'ink' / 'package.json'
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
    ...
    for name, pkg in wanted.items():
        ...
        if isinstance(installed[name], dict) and comparable(pkg) != comparable(installed[name]):
            return True
    return False
_TUI_BUILD_INPUT_DIRS = ('src', 'packages/duck-agent-ink/src')
_TUI_BUILD_INPUT_FILES = ('package.json', 'package-lock.json', 'tsconfig.json', 'tsconfig.build.json', 'babel.compiler.config.cjs', 'scripts/build.mjs', 'packages/duck-agent-ink/package.json', 'packages/duck-agent-ink/index.js', 'packages/duck-agent-ink/text-input.js')
...
def _ensure_tui_node() -> None:
    ...
    try:
        result = subprocess.run(['bash', '-c', f'source "{helper}" >&2 && ensure_node >&2 && command -v node'], env={**os.environ, 'DUCK_AGENT_HOME': hermes_home}, capture_output=True, text=True, encoding='utf-8', errors='replace', check=False)
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``duck-agent update`` (HEAD stays
    ...
    if not git or not (tui_dir.parent / '.git').exists():
        return False
    try:
        subprocess.run([git, 'restore', '--', tui_dir.name], cwd=str(tui_dir.parent), capture_output=True, text=True, encoding='utf-8', errors='replace', check=False)
    except OSError:
        return False
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        ...
        return
    print(f'Error: the TUI workspace is missing from this Duck Agent checkout.\nExpected directory: {tui_dir}\nThis usually means `duck-agent update` left tracked ui-tui files deleted.\nRecovery:\n  1. From the Duck Agent checkout, run `git restore -- ui-tui`\n  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n  3. Retry `duck-agent --tui`\nIf the checkout is still inconsistent, run `duck-agent update --force`.', file=sys.stderr)
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev and ext_dir:
        print(f'Error: --dev is incompatible with HERMES_TUI_DIR={ext_dir}\nThe prebuilt TUI has no source code to hot-reload.\nUnset HERMES_TUI_DIR (e.g. `unset HERMES_TUI_DIR`) to use --dev from a checkout.', file=sys.stderr)
        sys.exit(1)
    if not tui_dev:
        if ext_dir:
            p = Path(ext_dir)
    ...
    if not skip_install_for_fresh_termux_bundle and _tui_need_npm_install(tui_dir):
        ...
        npm_install_cmd = [npm, 'install', *npm_workspace_args, '--include=dev', '--silent', '--no-fund', '--no-audit', '--progress=false']

        def _run_tui_install() -> subprocess.CompletedProcess:
            from hermes_constants import with_hermes_node_path
            return subprocess.run(npm_install_cmd, cwd=str(npm_cwd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='replace', env={**with_hermes_node_path(), 'CI': '1'})
    ...
    if tui_dev:
        npm = _node_bin('npm')
        ink_dir = tui_dir / 'packages' / 'duck-agent-ink'
        result = subprocess.run([npm, 'run', 'build'], cwd=str(ink_dir), capture_output=True, text=True, encoding='utf-8', errors='replace')
        if result.returncode != 0:
    ...
    if should_build:
        npm = _node_bin('npm')
        result = subprocess.run([npm, 'run', 'build'], cwd=str(tui_dir), capture_output=True, text=True, encoding='utf-8', errors='replace')
        if result.returncode != 0:
            combined = f"{result.stdout or ''}{result.stderr or ''}".strip()
            preview = '\n'.join(combined.splitlines()[-30:])
...
def _launch_tui(resume_session_id: Optional[str]=None, tui_dev: bool=False, model: Optional[str]=None, provider: Optional[str]=None, toolsets: object=None, skills: object=None, verbose: Optional[bool]=None, quiet: bool=False, query: Optional[str]=None, image: Optional[str]=None, worktree: bool=False, checkpoints: bool=False, pass_session_id: bool=False, max_turns: Optional[int]=None, accept_hooks: bool=False):
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(prefix='duck-agent-tui-active-session-', suffix='.json')
    ...
    if worktree:
        try:
            from cli import _cleanup_worktree, _git_repo_root, _prune_stale_worktrees, _setup_worktree
            repo = _git_repo_root()
            if repo:
...
def _pin_kanban_board_env() -> None:
    ...
    (``duck-agent kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``duck-agent kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.duck-agent/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Duck Agent — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Duck Agent still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``duck-agent … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if continue_val and (not getattr(args, 'resume', None)):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'duck-agent sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if resume_val:
        ...
        if resolved:
            args.resume = resolved
    if getattr(args, 'resume', None) and (not getattr(args, 'no_restore_cwd', False)) and (not getattr(args, 'worktree', False)):
        try:
            from hermes_state import SessionDB
            ...
            pass
    try:
        from hermes_cli.xai_retirement import MIGRATION_GUIDE_URL, RETIREMENT_DATE, find_retired_xai_refs, format_issue
        ...
        if _retired_xai_refs:
            sys.stderr.write(f'\x1b[33m⚠ xAI retires {len(_retired_xai_refs)} model(s) in your config on {RETIREMENT_DATE}:\x1b[0m\n')
            for _ref in _retired_xai_refs:
                sys.stderr.write(f'  \x1b[33m⚠\x1b[0m {format_issue(_ref)}\n')
            sys.stderr.write(f'  \x1b[2mMigration guide: {MIGRATION_GUIDE_URL}\x1b[0m\n')
            sys.stderr.write("  \x1b[2mRun 'duck-agent doctor' for details.\x1b[0m\n\n")
    except Exception:
        pass
    if not _has_any_provider_configured():
        print()
        print("It looks like Duck Agent isn't configured yet -- no API keys or providers found.")
        print()
        print('  Run:  duck-agent setup')
        print()
        ...
        if reply in {'', 'y', 'yes'}:
            ...
            return
        print()
        print("You can run 'duck-agent setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print('How will you use WhatsApp with Duck Agent?')
        print()
    ...
    if not (bridge_dir / 'node_modules').exists():
        ...
        if not npm:
            ...
            return
        try:
            result = subprocess.run([npm, 'install', '--no-fund', '--no-audit', '--progress=false'], cwd=str(bridge_dir), stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='replace', env=with_hermes_node_path())
        except KeyboardInterrupt:
            print('\n  ✗ Install cancelled')
            return
        if result.returncode != 0:
    ...
    if (session_dir / 'creds.json').exists():
        ...
        if response.lower() in {'y', 'yes'}:
            ...
            print('  Start the gateway with: duck-agent gateway')
            return
    print()
    print('─' * 50)
    if wa_mode == 'bot':
    ...
    print()
    try:
        subprocess.run([find_node_executable('node') or 'node', str(bridge_script), '--pair-only', '--session', str(session_dir)], cwd=str(bridge_dir), env=with_hermes_node_path())
    except KeyboardInterrupt:
        pass
    print()
    if (session_dir / 'creds.json').exists():
        ...
        if wa_mode == 'bot':
            print('  Next steps:')
            print('    1. Start the gateway:  duck-agent gateway')
            print("    2. Send a message to the bot's WhatsApp number")
            print('    3. The agent will reply automatically')
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Duck Agent'")
        else:
            print('  Next steps:')
            print('    1. Start the gateway:  duck-agent gateway')
        ...
        print()
        print('  Or install as a service: duck-agent gateway install')
    else:
        print("⚠ Pairing may not have completed. Run 'duck-agent whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``duck-agent whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``duck-agent model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from hermes_cli.auth import resolve_provider, AuthError, format_auth_error
    from hermes_cli.config import get_compatible_custom_providers, load_config, get_env_value
    from hermes_cli.providers import custom_provider_aliases, custom_provider_slug, resolve_provider_full
    config = load_config()
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        ...
        def _record_raw(name: str, provider_key: str, model: str, api_key: str, base_url: str) -> None:
        ...
        if isinstance(raw_list, list):
            for raw_entry in raw_list:
                if not isinstance(raw_entry, dict):
                    continue
                _record_raw(raw_entry.get('name', ''), '', raw_entry.get('model', '') or raw_entry.get('default_model', ''), raw_entry.get('api_key', ''), raw_entry.get('base_url', '') or raw_entry.get('url', '') or raw_entry.get('api', ''))
        raw_providers = raw_cfg.get('providers')
        if isinstance(raw_providers, dict):
            for raw_key, raw_entry in raw_providers.items():
                if not isinstance(raw_entry, dict):
                    continue
                _record_raw(raw_entry.get('name', '') or raw_key, raw_key, raw_entry.get('model', '') or raw_entry.get('default_model', ''), raw_entry.get('api_key', ''), raw_entry.get('base_url', '') or raw_entry.get('url', '') or raw_entry.get('api', ''))
    ...
    def _canonical_named_custom_key(provider_id: str) -> str:
        ...
        for key, provider_info in _custom_provider_map.items():
            if requested in custom_provider_aliases(provider_info.get('name', ''), provider_info.get('provider_key', '')):
                return key
        return provider_id
    ...
    if not active and effective_provider != 'auto':
        active_def = resolve_provider_full(effective_provider, config.get('providers'), compatible_custom_providers)
        if active_def is not None:
            active = active_def.id
            if active_def.source == 'user-config':
                active = _canonical_named_custom_key(active)
        else:
            warning = f"Unknown provider '{effective_provider}'. Check 'duck-agent model' for available providers, or run 'duck-agent doctor' to diagnose config issues."
    ...
    if active == 'openrouter' and get_env_value('OPENAI_BASE_URL'):
        active = 'custom'
    from hermes_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS, _PROVIDER_ALIASES, group_providers, provider_group_for_slug
    ...
    _cli_excluded = {str(p).strip().lower() for p in (config.get('model_catalog', {}) or {}).get('excluded_providers') or [] if p}
    if _cli_excluded:
    ...
    provider_idx = _prompt_provider_choice([label for _, label, _ in ordered], default=default_idx)
    ...
    if selected_members:
        ...
        member_idx = _prompt_provider_choice(member_labels, default=member_default, title=f'Select {group_label} provider:')
    ...
    if selected_provider == 'openrouter':
        ...
        if provider_info is None:
            print('Warning: the selected saved custom provider is no longer available. It may have been removed from config.yaml. No change.')
            return
    ...
    elif selected_provider in {'openai-api', 'gemini', 'deepseek', 'xai', 'zai', 'kimi-coding-cn', 'minimax', 'minimax-cn', 'kilocode', 'opencode-zen', 'opencode-go', 'alibaba', 'huggingface', 'xiaomi', 'arcee', 'gmi', 'nvidia', 'ollama-cloud', 'tencent-tokenhub', 'lmstudio'} or _is_profile_api_key_provider(selected_provider):
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.duck-agent/.env if the active provider is not 'custom'.
...
def _save_aux_choice(task: str, *, provider: str, model: str='', base_url: str='', api_key: str='') -> None:
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print('  Duck Agent only falls back to a lightweight backend (OpenRouter,')
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``duck-agent model`` flow, then route aux tasks
    to them here.
    """
    from hermes_cli.config import load_config
    ...
    try:
        providers = build_aux_picker_rows(current_provider=current_provider, current_model=current_model, current_base_url=current_base_url)
    except Exception as exc:
    ...
    entries.extend(format_aux_picker_entries(providers, current_provider=current_provider, current_base_url=current_base_url))
...
def _aux_flow_provider_model(task: str, provider_slug: str, curated_models: list, current_model: str='') -> None:
    ...
    if not model_list:
        ...
        selected = _prompt_model_selection(model_list, current_model=current_model, pricing=pricing, confirm_provider=provider_slug)
        if selected is None:
            print('No change.')
            return
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    try:
        api_key = masked_secret_prompt('API key (optional, blank = use OPENAI_API_KEY): ').strip()
    except (KeyboardInterrupt, EOFError):
        print()
        return
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str='') -> Optional[str]:
    ...
    mode_options = [('', 'Auto-detect', 'Use Duck Agent URL heuristics; best for standard OpenAI-compatible endpoints.'), ('chat_completions', 'Chat Completions', 'Use /chat/completions for standard OpenAI-compatible servers.'), ('codex_responses', 'Responses / Codex', 'Use /responses for Codex-compatible tool-calling backends.'), ('anthropic_messages', 'Anthropic Messages', 'Use /v1/messages for Anthropic-compatible endpoints.')]
    print()
...
def _save_custom_provider(base_url, api_key='', model='', context_length=None, name=None, api_mode=None, key_env=''):
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str='', existing_source: str='') -> tuple:
    """Shared API-key entry point for ``duck-agent setup`` / ``duck-agent model``.
    ...
    recover from a malformed paste without editing ``~/.duck-agent/.env`` by hand.
    ...
    if choice.startswith('c') and (not pool_backed):
        ...
        print(f'  API key cleared.  Re-run `duck-agent setup` to configure {pconfig.name} again.')
        return ('', True)
    print()
    return (existing_key, False)
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get('refreshToken'))):
            ...
            print(f"    Duck Agent will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
    try:
        print()
        ...
        from hermes_cli.secret_prompt import masked_secret_prompt
        try:
            manual_token = masked_secret_prompt('  Paste setup-token (or Enter to cancel): ').strip()
        except (KeyboardInterrupt, EOFError):
            print()
            return False
        if manual_token:
        ...
        print('    4. Re-run:               duck-agent model')
        print()
...
def cmd_login(args):
    """Authenticate Duck Agent CLI with a provider."""
...
def cmd_sync(args):
    ...
    if sub == 'device':
        ...
        if name is not None:
            ...
            print('New commits from this device will use this label; existing commits keep their previous one.', file=sys.stderr)
            return 0
    ...
    if sub == 'propose':
        ...
        if result.get('proposal_pending'):
            print(f"Shared '{name}' with your organisation — an admin needs to approve it (proposal #{result.get('proposal_id')}). It is not live for the team until then.")
        else:
    ...
    if sub in {'enable', 'disable'}:
        ...
        if not is_curation_eligible(skill):
            print(f"'{skill}' is not sync-eligible (bundled, hub-installed, external, or not found). Only agent-created / user-authored skills under ~/.duck-agent/skills/ can sync.", file=sys.stderr)
            return 1
    ...
    if sub == 'status':
        ...
        if status.get('org_available'):
            ...
            print(f"\nOrg skills: {n} shared skill(s) from your organisation (your role: {status.get('org_role')}). They load alongside your own, labeled by origin, and you can edit them.", file=sys.stderr)
            if modified:
                print(f"  {len(modified)} with local edits not yet shared: {', '.join(modified)}\n  Share them back with `duck-agent sync propose <skill>`. Org updates will not overwrite them.", file=sys.stderr)
        elif status.get('logged_in'):
            print("\nOrg skills: not applicable — this account isn't a member of a shared organisation.", file=sys.stderr)
        if not status.get('logged_in'):
            ...
            print('\nSync feature is off for this instance (set HERMES_SYNC_ENABLED=1 or config.yaml sync.enabled: true). Sync is inert.', file=sys.stderr)
        elif not status.get('base_url'):
            print('\nNo sync base URL configured (config.yaml sync.base_url or HERMES_SYNC_BASE_URL). Sync is inert.', file=sys.stderr)
        return 0
    try:
    ...
    if not identity.get('nous_admin'):
        print('sync unavailable: not enabled for your account yet.', file=sys.stderr)
        return 1
    if not ssc.resolve_sync_base_url():
        print('sync inert: no sync base URL configured (config.yaml sync.base_url or HERMES_SYNC_BASE_URL).', file=sys.stderr)
        return 1
    try:
        if sub == 'pull':
            ...
            if org_result:
                ...
                print(f'org: refreshed {n} shared skill(s) from your organisation.', file=sys.stderr)
                clashes = org_result.get('conflicted') or []
                if clashes:
                    print(f"org: {len(clashes)} skill(s) have BOTH local edits and org updates, so they were left as-is: {', '.join(clashes)}\n     Your local version is intact. Review it, then either propose it or delete the local copy and pull again to take the org version.", file=sys.stderr)
        elif sub == 'push':
            result = ssc.push_skills(identity=identity, message='duck-agent sync push')
        elif sub == 'now':
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message='duck-agent sync now')
...
def cmd_slack(args):
    ...
    Dispatches ``duck-agent slack <subcommand>``. Currently supports:
      manifest — print or write a Slack app manifest with every gateway
                 command registered as a first-class slash.
    """
    sub = getattr(args, 'slack_command', None)
    if sub in {None, ''}:
        print('usage: duck-agent slack <subcommand>\n\nsubcommands:\n  manifest   Generate a Slack app manifest with every gateway\n             command registered as a native slash\n\nRun `duck-agent slack manifest -h` for details.', file=sys.stderr)
        return 1
    if sub == 'manifest':
...
def cmd_security(args):
    """Dispatch `duck-agent security <subcmd>`."""
...
def cmd_approvals(args):
    """Dispatch `duck-agent approvals <subcmd>`."""
...
def cmd_backup(args):
    """Back up Duck Agent home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Duck Agent backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Duck Agent (or just the Chat GUI with --gui)."""
...
def _clear_bytecode_cache(root: Path) -> int:
    ...
    for dirpath, dirnames, _ in os.walk(root):
        dirnames[:] = [d for d in dirnames if d not in {'venv', '.venv', 'node_modules', '.git', '.worktrees'}]
        if os.path.basename(dirpath) == '__pycache__':
            ...
            dirnames.clear()
    return removed
from hermes_cli.update_cmd import _add_upstream_remote, _atomic_replace_dir, _capture_head_sha, _cmd_update_check, _cmd_update_impl, _cold_start_windows_gateway_after_update, _count_commits_between, _detect_venv_python_processes, _discard_lockfile_churn, _discard_stashed_changes, _ensure_acp_launcher, _ensure_fhs_path_guard, _ensure_uv_for_termux, _finish_dashboard_update_cleanup, _for_each_systemd_gateway_unit, _format_concurrent_instances_message, _format_time_ago, _format_venv_python_holders_message, _gateway_prompt, _get_origin_url, _has_upstream_remote, _install_psutil_android_compat, _invalidate_update_cache, _is_android_python, _is_fork, _leftover_pausable_gateway_pids, _log_only_write, _mark_skip_upstream_prompt, _npm_bin_exists, _npm_lockfile_changed, _npm_manifest_paths, _npm_manifests_digest, _pause_windows_gateways_for_update, _print_curator_first_run_notice, _print_curator_recent_run_notice, _print_fts_optimize_available_notice, _print_stash_cleanup_guidance, _print_update_completion, _record_npm_lockfile_hash, _refresh_active_lazy_features, _refresh_active_memory_provider_dependencies, _refresh_windows_gateway_launchers, _reload_updated_runtime_modules, _resolve_pre_update_backup_mode, _resolve_stash_selector, _restore_stashed_changes, _resume_windows_gateways_after_update, _run_logged_subprocess, _run_pre_update_backup, _should_skip_upstream_prompt, _stash_apply_failed_only_on_existing_untracked, _stash_local_changes_if_needed, _sync_fork_with_upstream, _sync_with_upstream_if_needed, _update_node_dependencies, _update_via_zip, _upgrade_pip_before_lazy_refresh, _validate_critical_files_syntax, _validate_critical_modules_import, _venv_core_imports_healthy, _venv_launcher_ancestors, _wait_for_windows_update_gateway_exit, _warn_incomplete_gateway_fleet_restart, _web_build_toolchain_ready, _web_toolchain_roots, _write_lazy_refresh_incomplete_marker, _write_marker_file, _write_update_incomplete_marker, _write_update_planned_stop_marker, _UPDATE_RUNTIME_RELOAD_MODULES, _UPDATE_CRITICAL_FILES, _UPDATE_CRITICAL_MODULES, OFFICIAL_REPO_URLS, OFFICIAL_REPO_URL, SKIP_UPSTREAM_PROMPT_FILE, _PRE_UPDATE_SNAPSHOT_KEEP, _PRE_UPDATE_SNAPSHOT_MAX_FILE_SIZE
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``duck-agent update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``duck-agent update``
    ...
    ``duck-agent`` entry point compares the checkout fingerprint (cheap file
    ...
    try:
        ...
        if removed:
            logger.info('Checkout changed since last launch (%s -> %s): cleared %d stale __pycache__ director%s', recorded or 'unknown', fingerprint, removed, 'y' if removed == 1 else 'ies')
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``duck-agent update``
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $DUCK_AGENT_HOME."""
    from hermes_constants import get_hermes_home
...
def _write_web_ui_build_stamp(project_root: Path, web_dir: Path) -> None:
    ...
    try:
        ...
        stamp_data = {'contentHash': _compute_web_ui_content_hash(project_root, web_dir), 'builtAt': datetime.now(timezone.utc).isoformat()}
        stamp_file.write_text(json.dumps(stamp_data, indent=2) + '\n', encoding='utf-8')
    except Exception as exc:
        logger.debug('Failed to write web UI build stamp: %s', exc)

def _run_with_idle_timeout(cmd: list[str], cwd: Path, *, idle_timeout_seconds: int=180, indent: str='    ', env: dict[str, str] | None=None) -> subprocess.CompletedProcess:
    ...
    ``duck-agent`` launcher present but ``hermes_cli`` not importable.
    ...
    lock = threading.Lock()
    try:
        proc = subprocess.Popen(cmd, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', bufsize=1, env=env)
    except OSError as exc:
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the duck-agent venv's python3 (present in managed installs)
    ...
    try:
        ...
        return None
    if not re.search('^ID=nixos$', os_release, re.M):
        return None
    ...
    try:
        result = subprocess.run(['nix-shell', '-p', 'python3', '--run', 'which python3'], capture_output=True, text=True, encoding='utf-8', errors='replace', check=False, timeout=15)
        if result.returncode == 0:
...
def _run_npm_install_deterministic(npm: str, cwd: Path, *, extra_args: tuple[str, ...]=(), capture_output: bool=True, env: dict[str, str] | None=None) -> subprocess.CompletedProcess:
    ...
    the working tree dirty and causes the next ``duck-agent update`` to stash the
    lockfile — repeatedly.
    ...
    def _run(cmd: list[str]) -> subprocess.CompletedProcess:
        return _run_npm_watching_for_engine_failure(cmd, cwd=cwd, env=run_env, capture_output=capture_output)
...
def _run_npm_watching_for_engine_failure(cmd: list[str], *, cwd: Path, env: dict[str, str], capture_output: bool) -> subprocess.CompletedProcess:
    ...
    """
    if capture_output:
        return subprocess.run(cmd, cwd=cwd, env=env, capture_output=True, text=True, encoding='utf-8', errors='replace', check=False)
    captured: list[str] = []
    with subprocess.Popen(cmd, cwd=cwd, env=env, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='replace') as proc:
...
def _missing_web_build_tool(output: str) -> str | None:
    ...
    for tool in ('tsc', 'vite'):
        if any((phrase in lowered for phrase in (f'{tool}: not found', f'{tool}: command not found', f"'{tool}' is not recognized"))):
            return tool
    return None
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool=False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``duck-agent web``).
    ...
    def _install_web_deps(*, silent: bool) -> 'subprocess.CompletedProcess':
        return _run_npm_install_deterministic(npm, npm_cwd, extra_args=(*npm_workspace_args, '--silent', '--prefer-offline') if silent else (*npm_workspace_args, '--prefer-offline'), env=build_env)
    r1 = _install_web_deps(silent=True)
    if r1.returncode != 0:
        _say(f"  {('✗' if fatal else '⚠')} Web UI npm install failed" + ('' if fatal else ' (duck-agent web will not be available)'))
        _relay(r1)
        if fatal:
    ...
    if r2.returncode != 0:
        ...
        if dist_index.exists():
            ...
            if stderr_tail:
                _say(f'  Build error:\n  {stderr_tail}')
            return True
        _say(f"  {('✗' if fatal else '⚠')} Web UI build failed" + ('' if fatal else ' (duck-agent web will not be available)'))
        _relay(r2)
        if fatal:
            _say('  Run manually:  npm install --workspace web && npm run build -w web')
        return False
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $DUCK_AGENT_HOME."""
    from hermes_constants import get_hermes_home
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``duck-agent update`` that pulled new source but hasn't built yet).
    """
    if source_mode:
...
def _write_desktop_build_stamp(project_root: Path, *, source_mode: bool) -> None:
    ...
    try:
        ...
        from datetime import datetime, timezone
        stamp_data = {'contentHash': content_hash, 'sourceMode': source_mode, 'builtAt': datetime.now(timezone.utc).isoformat()}
        stamp_file.write_text(json.dumps(stamp_data, indent=2) + '\n', encoding='utf-8')
    except Exception as exc:
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == 'darwin':
        candidates = list(release_dir.glob('mac*/Duck Agent.app/Contents/MacOS/Duck Agent'))
    elif sys.platform == 'win32':
        candidates = [release_dir / 'win-unpacked' / 'Duck Agent.exe', release_dir / 'win-ia32-unpacked' / 'Duck Agent.exe', release_dir / 'win-arm64-unpacked' / 'Duck Agent.exe']
    else:
        candidates = [release_dir / 'linux-unpacked' / 'duck-agent', release_dir / 'linux-unpacked' / 'Duck Agent', release_dir / 'linux-arm64-unpacked' / 'duck-agent', release_dir / 'linux-arm64-unpacked' / 'Duck Agent']
...
_PE_MACHINE_I386 = 332
_PE_MACHINE_AMD64 = 34404
_PE_MACHINE_ARM64 = 43620
_PE_MACHINE_NAMES = {_PE_MACHINE_I386: 'x86 (32-bit)', _PE_MACHINE_AMD64: 'x64 (AMD64)', _PE_MACHINE_ARM64: 'ARM64'}
_PE_MACHINE_TO_NAME = {_PE_MACHINE_ARM64: 'ARM64', _PE_MACHINE_AMD64: 'AMD64', _PE_MACHINE_I386: 'X86'}
_MACHINE_ATTRIBUTE_USER_ENABLED = 1
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Duck Agent.exe``.
    ...
    kernel32.IsWow64Process2.argtypes = [wintypes.HANDLE, ctypes.POINTER(wintypes.USHORT), ctypes.POINTER(wintypes.USHORT)]
    kernel32.IsWow64Process2.restype = wintypes.BOOL
    process_machine = wintypes.USHORT(0)
    native_machine = wintypes.USHORT(0)
    if not kernel32.IsWow64Process2(kernel32.GetCurrentProcess(), ctypes.byref(process_machine), ctypes.byref(native_machine)):
        return None
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 duck-agent-setup.exe (and thus
...
def _parse_pe_machine(path: Path) -> int:
    ...
    with path.open('rb') as fh:
        ...
        if len(head) < 64 or head[:2] != b'MZ':
            raise ValueError('missing MZ header — not a Windows executable (a truncated or non-binary file saved as .exe?)')
        ...
        if file_size < max_section_end:
            raise ValueError(f'truncated executable: file is {file_size} bytes but its PE sections extend to {max_section_end} bytes')
    return machine
...
def _desktop_exe_integrity_error(path: Path) -> Optional[str]:
    ...
    if machine not in expected:
        ...
        return f'architecture mismatch: built a {got} executable but this is a {_windows_native_machine()} Windows host'
    return None
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    if error is None:
        return (packaged_executable, False)
    print(f'✗ The built Duck Agent.exe failed its integrity check: {error}')
    ...
    if restored is not None:
        print('  ↩ Update aborted — restored the previous working Duck Agent.exe from backup.')
        print('    Your existing version was kept and still works. Run `duck-agent desktop`')
        print('    (or the in-app update) again to retry with a fresh Electron download.')
        return (restored, True)
    print('  ✗ No usable backup was found to restore.')
    print('    Run `duck-agent desktop --force-build` to rebuild, or re-run the Duck Agent')
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Duck Agent'`` desktop build failure: a corrupt zip in the
...
def _electron_pkg_staged_missing_dist(project_root: Path) -> bool:
    ...
    return (electron_dir / 'package.json').is_file() and (electron_dir / 'install.js').is_file() and (not _electron_dist_ok(project_root))
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Duck Agent.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Duck Agent.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Duck Agent.exe: Access is
    ...
    an unrelated "Duck Agent" process is never touched. Best-effort: never raises.
...
def _desktop_macos_bundle_id(bundle: Path) -> Optional[str]:
    ...
    if not info.exists() and bundle.suffix == '.framework':
        candidates = list(bundle.glob('Versions/*/Resources/Info.plist')) + list(bundle.glob('Resources/Info.plist'))
        if candidates:
            info = candidates[0]
    if not info.exists():
        return None
...
def _desktop_macos_local_signing_identity() -> Optional[str]:
    ...
    try:
        ...
        print(f'  (warning: could not load desktop.macos_signing_identity: {exc}; falling back to ad-hoc signing)')
        return None
...
def _desktop_macos_has_valid_real_signature(app: Path) -> bool:
    ...
    if not codesign:
        return False
    try:
        info = subprocess.run([codesign, '-dv', str(app)], check=False, capture_output=True, text=True)
        output = f'{info.stdout}\n{info.stderr}'
        if info.returncode != 0 or 'TeamIdentifier=' not in output or 'TeamIdentifier=not set' in output:
            return False
        verify = subprocess.run([codesign, '--verify', '--deep', '--strict', str(app)], check=False, capture_output=True)
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str='-') -> bool:
    ...
    def sign_path(path: Path, *, entitlements: Optional[Path]=None, identifier: Optional[str]=None, runtime: bool=True) -> None:
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path, *, publisher_signing_configured: Optional[bool]=None) -> bool:
    ...
    "Duck Agent is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if publisher_signing_configured is None:
        publisher_signing_configured = bool(os.environ.get('CSC_LINK') or os.environ.get('APPLE_SIGNING_IDENTITY'))
    ...
    try:
        ...
        if identity != '-':
            print(f'  (warning: configured macOS signing identity failed: {identity!r}; falling back to ad-hoc — TCC grants may need to be re-granted)')
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``duck-agent desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Duck Agent Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    try:
        sandbox_lstat = sandbox.lstat()
    except OSError:
    ...
    if not stat.S_ISREG(sandbox_lstat.st_mode):
        ...
        return False
    if sandbox_lstat.st_uid == 0 and stat.S_IMODE(sandbox_lstat.st_mode) == 2541:
        return True
    sudo = shutil.which('sudo')
    if not sudo:
        print("✗ Duck Agent Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Duck Agent Desktop (Linux only, best-effort).
    ...
    shell. ``duck-agent uninstall --gui`` removes it.
    """
    try:
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print('Install Node.js, then run:  duck-agent gui')
            sys.exit(1)
    else:
        npm = None
    if skip_build:
        ...
        if not build_needed:
            ...
            if install_result.returncode != 0:
                ...
                if repaired:
                    print('  ⚠ Dependency install failed with a missing Electron dist; repopulated it and continuing.')
                else:
                    print('  ⚠ Dependency install failed with a missing Electron dist; continuing to the build so electron-builder can attempt the Electron fetch itself.')
            build_label = 'source build' if source_mode else 'packaged app'
            print(f'→ Building desktop {build_label}...')
            build_script = 'build' if source_mode else 'pack'
            if _force_adhoc_macos_signing(env, source_mode=source_mode):
                print('  → No Developer ID configured; ad-hoc signing this local rebuild (CSC_IDENTITY_AUTO_DISCOVERY=false)')
            if not source_mode:
                stopped = _stop_desktop_processes_locking_build(desktop_dir)
                if stopped:
                    print(f"  ⚠ Stopped running desktop app to free the build output (pid {', '.join(map(str, stopped))})")
            build_result = subprocess.run([npm, 'run', build_script], cwd=desktop_dir, env=env, check=False)
            if build_result.returncode != 0 and (not source_mode) and (_desktop_packaged_executable(desktop_dir) is None):
                purged: list[Path] = []
                restored = False
                ...
                if restored:
                    ...
                    build_result = subprocess.run([npm, 'run', build_script], cwd=desktop_dir, env=env, check=False)
            if build_result.returncode != 0 and (not source_mode) and (not env.get('ELECTRON_MIRROR')) and (_desktop_packaged_executable(desktop_dir) is None):
                print('  ⚠ Desktop build still failing; the Electron download from GitHub looks blocked. Re-downloading via a public mirror (npmmirror.com)... (set ELECTRON_MIRROR to use another mirror)')
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == 'win32':
                    print('  If this says "Access is denied" on Duck Agent.exe, close any')
                    print('  running Duck Agent desktop window and retry.')
                print('  If the log shows Electron download retries, rebuild via a mirror:')
                print('    ELECTRON_MIRROR=<mirror-base-url> duck-agent desktop --force-build')
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                _desktop_macos_relaunchable_fixup(desktop_dir)
                verified_executable, rolled_back = _ensure_desktop_exe_launchable(desktop_dir, packaged_executable)
                if packaged_executable is not None and (rolled_back or verified_executable is None):
                    sys.exit(1)
    ...
    if getattr(args, 'build_only', False):
        ...
        return
    if source_mode:
        print('→ Launching Duck Agent Desktop from source build...')
    ...
    print(f"→ Launching packaged Duck Agent Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)
from hermes_cli.dashboard_procs import _detect_concurrent_hermes_instances, _kill_stale_dashboard_processes, _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    mode = None
    if any((pattern in command for pattern in ('duck-agent dashboard', 'hermes_cli.main dashboard', 'hermes_cli/main.py dashboard'))):
        mode = 'dashboard'
    elif any((pattern in command for pattern in ('duck-agent serve', 'hermes_cli.main serve', 'hermes_cli/main.py serve'))):
        mode = 'serve'
    if mode is None:
        return None
    port = 9119
    host = '127.0.0.1'
    port_match = re.search('(?:^|\\s)--port(?:=|\\s+)(\\d+)', command)
    if port_match:
        try:
            port = int(port_match.group(1))
        except ValueError:
            return None
    host_match = re.search('(?:^|\\s)--host(?:=|\\s+)(\\"[^\\"]+\\"|\'[^\']+\'|\\S+)', command)
    if host_match:
...
def _dashboard_probe_host(host: str | None) -> str:
    ...
    if normalized in {'', '0.0.0.0', '::'}:
        return '127.0.0.1'
    return normalized
_DASHBOARD_SYSTEMD_UNIT = 'duck-agent-dashboard.service'
...
def _restart_managed_dashboard_service(reason: str, unit: str=_DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    def _systemctl(*args: str, timeout: int=10) -> subprocess.CompletedProcess:
        return subprocess.run(['systemctl', *args], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout)
    ...
    if active_state != 'active' and enabled_state not in {'enabled', 'enabled-runtime', 'linked', 'linked-runtime', 'static', 'generated'}:
        return False
    print()
    ...
    for command in commands:
        try:
            result = subprocess.run(list(command), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=60)
    ...
    for err in errors:
        if err.strip():
            print(f'      {err}')
    print('  Dashboard is managed by systemd; not raw-killing its PID because systemd would treat that as a clean stop.')
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``duck-agent-serve.service``).  Returns ``None`` when the PID is not
...
def _try_restart_systemd_service(svc_name: str, cgroup_path: str | None=None) -> bool:
    ...
    if scope == 'user':
        ...
        cmd = None
        for candidate in (['systemctl', 'restart', svc_name], ['systemctl', '--user', 'restart', svc_name]):
            try:
                r = subprocess.run(candidate, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=15)
                if r.returncode == 0:
                    return True
            except (FileNotFoundError, subprocess.TimeoutExpired, OSError):
                continue
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but duck-agent launch commands don't embed exotic args).
    ...
    try:
        ...
        if os.path.exists(cmdline_path):
            ...
            return argv or None
        result = subprocess.run(['ps', '-p', str(pid), '-o', 'command='], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10)
        if result.returncode != 0:
            return None
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``duck-agent update``.
    ...
    for command in commands:
        try:
            ...
            with open(log_path, 'ab') as log_f:
                subprocess.Popen(command, stdin=subprocess.DEVNULL, stdout=log_f, stderr=subprocess.STDOUT, start_new_session=True, close_fds=True)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``duck-agent update``.
    ...
    protocol stream (``duck-agent acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_lazy_refresh_marker_locked() -> None:
    ...
    print('⚠ A previous lazy-backend refresh may have left the venv unhealthy — running import-based package repair...')
    ...
    if status == 'indeterminate':
        print('  ⚠ Import probes unavailable — cannot confirm venv health. Leaving `.lazy-refresh-incomplete` for the next launch.')
    else:
        print('  ⚠ Lazy-refresh package repair incomplete. Leaving `.lazy-refresh-incomplete` for the next launch.')
        print('  Recover manually with:')
        all_specs = _lazy_refresh_repair_specs(sorted(set(_LAZY_REFRESH_REPAIR_PACKAGES.values())))
        print(f"    {' '.join(install_prefix)} install --force-reinstall " + ' '.join((shlex.quote(s) for s in all_specs)))
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print('⚠ A previous `duck-agent update` was interrupted mid-install — finishing dependency installation now...')
    self_locked = _windows_running_hermes_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print('  → Running from duck-agent.exe; applying package-only first aid, then quarantined full reinstall (core marker stays until that succeeds)...')
        _repair_venv_via_import_probes(install_prefix, env=install_env)
    try:
        from hermes_cli.managed_uv import ensure_uv
        try:
            subprocess.run([sys.executable, '-m', 'ensurepip', '--upgrade', '--default-pip'], cwd=PROJECT_ROOT, capture_output=True)
        except Exception as exc:
        ...
        if uv_bin:
            ...
            _install_python_dependencies_with_optional_fallback([uv_bin, 'pip'], env=uv_env, group='termux-all' if _is_termux_env(uv_env) else 'all')
        else:
            _install_python_dependencies_with_optional_fallback([sys.executable, '-m', 'pip'], group='termux-all' if _is_termux_env() else 'all')
        _clear_update_incomplete_marker()
        ...
        if self_locked:
            print('  Duck Agent is still running from the launcher that needs replacing. Close other Duck Agent windows, restart from a different terminal, then run:')
...
def _windows_running_hermes_launcher_locked() -> bool:
    """True when a venv ``duck-agent*.exe`` shim is this process or an ancestor.
...
def _run_install_with_heartbeat(cmd: list[str], *, env: dict[str, str] | None=None, heartbeat_interval_seconds: int=30) -> None:
    ...
    know ``duck-agent update`` is still progressing even if pip/uv itself is silent.
    """
    ...
    def _heartbeat() -> None:
        while not done.wait(heartbeat_interval_seconds):
            ...
            print(f'  … still installing dependencies ({elapsed}s elapsed) — compiling Rust/C extensions can take several minutes', flush=True)
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    names = set(_load_console_script_names()) or {'duck-agent', 'duck-agent', 'duck-agent-acp'}
    names.add('duck-agent-gateway')
    return [scripts_dir / f'{name}.exe' for name in sorted(names)]

def _quarantine_running_hermes_exe(scripts_dir: Path, *, max_attempts: int=4) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``duck-agent.exe``.
    ...
    when ``duck-agent update`` runs, ``duck-agent.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``duck-agent.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next duck-agent invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Duck Agent Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Duck Agent Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        if scheduled:
            print(f'  ⚠ {shim.name} is locked by another process; scheduled replacement on next reboot.')
            print('    The new shim was written at the same path, but a reboot is needed to fully unload the old one.')
            continue
        print(f'  ⚠ Could not quarantine {shim.name} ({last_exc.__class__.__name__}: another process is holding it open).')
        print('    Close Duck Agent Desktop, exit other `duck-agent` REPLs, stop the gateway, or pause AV scanning, then re-run `duck-agent update`.')
    return moved
...
def _schedule_replace_on_reboot(shim: Path, quarantine_target: Path) -> bool:
    ...
    try:
        ...
        ok = MoveFileExW(str(shim), str(quarantine_target), MOVEFILE_REPLACE_EXISTING | MOVEFILE_DELAY_UNTIL_REBOOT)
        return bool(ok)
    except Exception:
        return False
...
def _run_quarantined_install(cmd: list[str], *, env: dict[str, str] | None=None, scripts_dir: Path | None=None) -> None:
    """Run an editable install, quarantining the running ``duck-agent.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``duck-agent.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``duck-agent`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None=None) -> None:
    """Sweep ``duck-agent.exe.old.*`` left by prior updates.

    Called early on every duck-agent invocation. The .old files are unlocked once
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None=None) -> None:
    ...
    rewrite ``duck-agent.exe``. The editable-install quarantine path would rename
...
def _detect_broken_lazy_refresh_imports(install_cmd_prefix: list[str], *, env: dict[str, str] | None=None) -> list[str] | None:
...
def _repair_broken_lazy_refresh_imports(install_cmd_prefix: list[str], packages: list[str], *, env: dict[str, str] | None=None) -> bool:
    """Force-reinstall ``packages`` and re-probe imports. Never raises."""
    if not packages:
        return True
    specs = _lazy_refresh_repair_specs(packages)
    try:
        _run_package_only_install(install_cmd_prefix + ['install', '--force-reinstall', *specs], env=env)
...
def _repair_venv_via_import_probes(install_cmd_prefix: list[str], *, env: dict[str, str] | None=None) -> str:
    ...
    detected (#57828). Package-only reinstall — never rewrites ``duck-agent.exe``.
    ...
    if not broken:
        return 'healthy'
    print(f"  → Detected corrupted venv packages via import probes: {', '.join(broken)}; repairing...")
    ...
    print('  ⚠ Venv repair incomplete. Run manually, then `duck-agent update`:')
    print(f"    {' '.join(install_cmd_prefix)} install --force-reinstall {manual}")
    return 'failed'

def _install_python_dependencies_with_optional_fallback(install_cmd_prefix: list[str], *, env: dict[str, str] | None=None, group: str='all') -> None:
    ...
    On Windows, pre-renames live ``duck-agent.exe`` / ``duck-agent-gateway.exe`` shims
...
def _verify_console_scripts_installed(install_cmd_prefix: list[str], *, env: dict[str, str] | None=None) -> None:
    """Ensure every declared console_script shim exists on disk after install.

    On Windows, ``uv pip install -e .`` can register ``duck-agent.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``duck-agent.exe`` shim is locked during ``duck-agent update``, or when uv/distlib
    skips a launcher write. The symptom is ``duck-agent.exe`` and
    ``duck-agent-acp.exe`` present but ``duck-agent.exe`` missing, so ``duck-agent`` drops
    ...
    if not missing:
        return
    print(f"  ⚠ Verification: {len(missing)} console script(s) missing on disk: {', '.join(missing)}")
    print('  → Reinstalling entry points with --reinstall...')
    try:
        _run_quarantined_install(install_cmd_prefix + ['install', '--reinstall', '-e', '.'], env=env, scripts_dir=scripts_dir)
    except subprocess.CalledProcessError as e:
        logger.warning('console script verification: repair install failed: %s', e)
        print('  ⚠ Entry point repair failed; try `duck-agent update --force` after closing other duck-agent processes.')
        return
    still_missing = _missing()
    if still_missing:
        print(f"  ⚠ Still missing after repair: {', '.join(still_missing)}. Workaround: python -m hermes_cli.main <command>")
    else:
        print('  ✓ All console entry points restored')

def _verify_core_dependencies_installed(install_cmd_prefix: list[str], *, env: dict[str, str] | None=None, group: str='all') -> None:
    ...
    def _missing_deps() -> list[str]:
        check_script = "import importlib.metadata as md, sys\nmissing=[]\nfor name in sys.argv[1:]:\n    try: md.version(name)\n    except md.PackageNotFoundError: missing.append(name)\nprint('\\n'.join(missing))\n"
        try:
            result = subprocess.run([str(venv_python), '-c', check_script, *applicable], capture_output=True, text=True, encoding='utf-8', errors='replace', check=False, env=env)
        except Exception as e:
    ...
    if not missing:
        return
    print(f"  ⚠ Verification: {len(missing)} declared dep(s) missing after install: {', '.join(missing[:8])}{('...' if len(missing) > 8 else '')}")
    print('  → Reinstalling base group with --reinstall to repair...')
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    repair_args = ['install', '--reinstall', '-e', '.']
    try:
        _run_quarantined_install(install_cmd_prefix + repair_args, env=env, scripts_dir=scripts_dir)
    except subprocess.CalledProcessError as e:
        logger.warning('dep verification: repair install failed: %s', e)
        print('  ⚠ Repair install failed; check `duck-agent update` output above.')
        return
    ...
    try:
        _run_install_with_heartbeat(install_cmd_prefix + ['install', '--reinstall', *specs], env=env)
    except subprocess.CalledProcessError as e:
        logger.warning('dep verification: per-package repair failed: %s', e)
        print(f"  ⚠ Could not install: {', '.join(still_missing)}. Run `duck-agent update --force` after closing other duck-agent processes.")
        return
    final_missing = _missing_deps()
    if final_missing:
        print(f"  ⚠ Still missing after repair: {', '.join(final_missing)}. Run `duck-agent update --force` after closing other duck-agent processes.")
    else:
        print('  ✓ All declared core dependencies now installed')

def _resolve_install_target_python(install_cmd_prefix: list[str], env: dict[str, str] | None) -> Path | None:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``duck-agent update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.duck-agent/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``duck-agent update`` safe to
...
def _install_hangup_protection(gateway_mode: bool=False):
    ...
    Users commonly run ``duck-agent update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.duck-agent/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``duck-agent update --gateway``) the update is already
    ...
    """
    state = {'prev_stdout': sys.stdout, 'prev_stderr': sys.stderr, 'log_file': None, 'installed': False}
    if gateway_mode:
        return state
    ...
    try:
        ...
        log_file.write(f"\n=== duck-agent update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def cmd_update(args):
    """Update Duck Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from hermes_cli.config import detect_install_method, format_docker_update_message, is_managed, managed_error, recommended_update_command_for_method
    if is_managed():
        managed_error('update Duck Agent')
        return
    ...
    if getattr(args, 'check', False):
        ...
        _cmd_update_check(branch=branch, branch_explicit=bool(getattr(args, 'branch', None)))
        return
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``duck-agent -c Pokemon Agent Dev`` without quoting the
    ...
    """
    _SUBCOMMANDS = {'chat', 'model', 'gateway', 'setup', 'whatsapp', 'whatsapp-cloud', 'login', 'logout', 'auth', 'status', 'cron', 'doctor', 'config', 'pairing', 'skills', 'tools', 'mcp', 'sessions', 'insights', 'version', 'update', 'uninstall', 'profile', 'dashboard', 'serve', 'desktop', 'gui', 'honcho', 'claw', 'plugins', 'security', 'acp', 'webhook', 'memory', 'dump', 'debug', 'backup', 'import', 'completion', 'logs'}
...
def cmd_profile(args):
    ...
    from hermes_cli.profiles import list_profiles, create_profile, delete_profile, seed_profile_skills, set_active_profile, get_active_profile_name, check_alias_collision, create_wrapper_script, remove_wrapper_script, _is_wrapper_dir_in_path, _get_wrapper_dir
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == 'default' and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f'Alias:          {alias_display} → duck-agent -p {p.name}')
                break
        print()
        return
    if action == 'list':
        ...
        try:
            ...
            if name == 'default':
                print('Switched to: default (~/.duck-agent)')
            else:
        ...
        try:
            ...
            profile_dir = create_profile(name=name, clone_from=clone_from, clone_all=clone_all, clone_config=clone_config, no_alias=no_alias, no_skills=no_skills, description=getattr(args, 'description', None))
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f'  Choose a custom alias:  duck-agent profile alias {name} --name <custom>')
                    print(f'  Or access via flag:     duck-agent -p {name} chat')
                else:
        ...
        if all_flag and (text_value or name):
            print('profile describe: --all is mutually exclusive with a profile name / --text', file=sys.stderr)
            sys.exit(2)
        ...
        if text_value and auto_flag:
            print('profile describe: --text is mutually exclusive with --auto', file=sys.stderr)
            sys.exit(2)
        ...
        if text_value:
            try:
                if _profiles_mod.normalize_profile_name(name) == 'default':
                    ...
                    profile_dir = _profiles_mod.get_profile_dir(name)
                _profiles_mod.write_profile_meta(profile_dir, description=text_value, description_auto=False)
        ...
        name = args.profile_name
        from hermes_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills, _read_distribution_meta, _get_wrapper_dir, find_alias_for_profile
        ...
        if dist_name:
            ...
            print(f'  (run `duck-agent profile info {name}` for full manifest)')
        if alias_name:
            is_windows = sys.platform == 'win32'
            wrapper = _get_wrapper_dir() / (f'{alias_name}.bat' if is_windows else alias_name)
            print(f'Alias:   {alias_name} → duck-agent -p {name}  ({wrapper})')
        print()
    elif action == 'alias':
        name = args.profile_name
        ...
        import tempfile
        from hermes_cli.profile_distribution import plan_install, install_distribution, DistributionError
        try:
            with tempfile.TemporaryDirectory(prefix='hermes_dist_preview_') as tmp:
                ...
                if not getattr(args, 'yes', False):
                    ...
                    if answer not in {'y', 'yes'}:
                        ...
                        return
            plan = install_distribution(args.source, name=getattr(args, 'install_name', None), force=getattr(args, 'force', False), create_alias=getattr(args, 'alias', False))
            print(f"\n✓ Installed '{plan.manifest.name}' v{plan.manifest.version}")
            print(f'  Profile path: {plan.target_dir}')
            if plan.manifest.env_requires:
                print(f'  Next: copy .env.EXAMPLE to .env and fill in required keys:\n    {plan.target_dir}/.env.EXAMPLE')
            if plan.has_cron:
                print(f'  Cron jobs were included but are NOT scheduled automatically.\n  Review them with:  duck-agent -p {plan.manifest.name} cron list')
            print(f'\n  Use with:      duck-agent -p {plan.manifest.name} chat')
        except (DistributionError, ValueError) as e:
            print(f'Error: {e}')
            sys.exit(1)
    elif action == 'update':
        from hermes_cli.profile_distribution import update_distribution, read_manifest, DistributionError
        ...
        try:
            ...
            if current is None:
                print(f"Error: Profile '{canon}' is not a distribution (no distribution.yaml). Only profiles installed via `duck-agent profile install` can be updated.")
                sys.exit(1)
            ...
            if plan.has_cron:
                print(f'  Cron files were refreshed.  Review with:  duck-agent -p {plan.manifest.name} cron list')
        except (DistributionError, ValueError) as e:
            print(f'Error: {e}')
            sys.exit(1)
    elif action == 'info':
        ...
        if data.get('hermes_requires'):
            print(f"Requires:     Duck Agent {data['hermes_requires']}")
        if data.get('source'):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f'  Requires: Duck Agent {mf.hermes_requires}')
    ...
    if mf.env_requires:
        ...
        for er in mf.env_requires:
            ...
            print(line)
    if plan.has_cron:
        print('\n  ⚠ This distribution ships cron jobs.  They will NOT run automatically — review and enable manually.')
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print('No duck-agent dashboard processes running.')
        return 0
    print(f'{len(live)} duck-agent dashboard process(es) running:')
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``duck-agent dashboard register`` for OAuth.
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        return
    print()
    print(f'⚠ The dashboard is binding to a non-loopback address ({host}) and needs an auth provider.')
    print('  Non-loopback binds always require authentication (--insecure no longer bypasses this).')
    print()
    print('  How do you want to authenticate the dashboard?')
    print('    [1] Username & password (quickest; for a trusted LAN / VPN)')
    print('    [2] OAuth via Nous Portal (run `duck-agent dashboard register`)')
    print('    [3] Cancel')
    print()
    try:
    ...
    if choice == '2':
        print()
        print('  Run this on the host where the dashboard lives, then start the dashboard again:\n    duck-agent dashboard register\n  It provisions a Nous Portal OAuth client and writes HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.duck-agent/.env for you.\n  Docs: https://duck-agent.nousresearch.com/docs/user-guide/features/web-dashboard#authentication-gated-mode')
        sys.exit(0)
    if choice not in ('1',):
        print('  Cancelled.')
        sys.exit(1)
    ...
    try:
        ...
        print(f'  ⚠ Plugin re-discovery failed ({exc}); the gate may still fail closed. Set the password again or restart the dashboard.')
    print()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    token_root = _Path.home() / '.duck-agent' / 'desktop-ssh'
    try:
    ...
    if len(relative.parts) != 2 or not re.fullmatch('[0-9a-f]{32}', relative.parts[0]):
        raise SystemExit('--ssh-session-token-file has an invalid runtime path')
    if not re.fullmatch('[0-9a-f]{16}\\.token', relative.parts[1]):
    ...
    try:
        try:
            ...
            if directory_stat.st_mode & 511 != 448:
        ...
        if hasattr(os, 'getuid') and file_stat.st_mode & 511 & ~384:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    ``.../app.asar.unpacked/dist``. A standalone ``duck-agent dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and (getattr(args, 'status', False) or getattr(args, 'stop', False)):
    ...
    if _ssh_owner_nonce and (not re.fullmatch('[0-9a-f]{16}', _ssh_owner_nonce)):
    ...
    if _token_file and (not _headless_backend):
        raise SystemExit('--ssh-session-token-file is only valid with duck-agent serve')
    ...
    if _launch_profile not in ('default', 'custom') and (not getattr(args, 'isolated', False)) and (not getattr(args, 'open_profile', '')) and (os.environ.get('HERMES_DESKTOP') != '1'):
        ...
        if _dashboard_listening(args.host, args.port):
            ...
            if not args.no_open:
                try:
                    ...
                    pass
            sys.exit(0)
        print(f"Routing to the machine dashboard (profile '{_launch_profile}' preselected). Use --isolated for a dedicated per-profile server.")
        reexec_argv = [sys.executable, '-m', 'hermes_cli.main', '-p', 'default', 'serve' if _headless_backend else 'dashboard', '--port', str(args.port), '--host', args.host, '--open-profile', _launch_profile]
        if _ssh_owner_nonce:
        ...
        try:
            from hermes_constants import get_default_hermes_root
            env['DUCK_AGENT_HOME'] = str(get_default_hermes_root())
        except Exception:
            env.pop('DUCK_AGENT_HOME', None)
        if sys.platform == 'win32':
    ...
    try:
        ...
        print(f'Re-install the package into this interpreter so metadata updates apply:\n  cd {PROJECT_ROOT}\n  {sys.executable} -m pip install -e .\nIf `pip` is missing in this venv, use:  uv pip install -e .')
    ...
    try:
        ...
        logger.debug('terminal config → env bridge failed for dashboard/serve', exc_info=True)
    if _headless_backend:
        ...
        _dist_root = Path(os.environ['HERMES_WEB_DIST']) if 'HERMES_WEB_DIST' in os.environ else PROJECT_ROOT / 'hermes_cli' / 'web_dist'
    ...
    try:
        ...
        logger.debug('Background MCP tool discovery failed at dashboard startup', exc_info=True)
    from hermes_cli.web_server import start_server
    _maybe_setup_dashboard_auth_interactively(args)
    start_server(host=args.host, port=args.port, open_browser=not args.no_open, allow_public=getattr(args, 'insecure', False), initial_profile=getattr(args, 'open_profile', '') or '', headless=_headless_backend, ssh_session_token=_ssh_session_token, ssh_owner_nonce=_ssh_owner_nonce)
...
def cmd_logs(args):
    """View and filter Duck Agent log files."""
    from hermes_cli.logs import tail_log, list_logs
    log_name = getattr(args, 'log_name', 'agent') or 'agent'
    if log_name == 'list':
        list_logs()
        return
    tail_log(log_name, num_lines=getattr(args, 'lines', 50), follow=getattr(args, 'follow', False), level=getattr(args, 'level', None), session=getattr(args, 'session', None), since=getattr(args, 'since', None), component=getattr(args, 'component', None))

def cmd_console(args):
    """Open the safe Duck Agent command console."""
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        ...
        return ['auto', 'openrouter', 'nous', 'openai-codex', 'xai-oauth', 'copilot-acp', 'copilot', 'anthropic', 'gemini', 'vertex', 'xai', 'bedrock', 'azure-foundry', 'ollama-cloud', 'huggingface', 'zai', 'kimi-coding', 'kimi-coding-cn', 'stepfun', 'minimax', 'minimax-cn', 'kilocode', 'novita', 'xiaomi', 'arcee', 'nvidia', 'deepseek', 'alibaba', 'qwen-oauth', 'opencode-zen', 'opencode-go']
_BUILTIN_SUBCOMMANDS = frozenset({'acp', 'approvals', 'auth', 'backup', 'bundles', 'checkpoints', 'claw', 'completion', 'computer-use', 'config', 'console', 'cron', 'curator', 'dashboard', 'serve', 'debug', 'doctor', 'dump', 'egress', 'fallback', 'gateway', 'hooks', 'import', 'import-agent', 'insights', 'gui', 'desktop', 'kanban', 'login', 'logout', 'logs', 'lsp', 'mcp', 'memory', 'migrate', 'moa', 'journey', 'memory-graph', 'learning', 'model', 'monitoring', 'pairing', 'pause', 'pets', 'plugins', 'portal', 'profile', 'project', 'proxy', 'prompt-size', 'resume', 'send', 'sessions', 'setup', 'skin', 'skills', 'slack', 'status', 'sync', 'tools', 'uninstall', 'update', 'version', 'webhook', 'whatsapp', 'whatsapp-cloud', 'chat', 'secrets', 'security', 'verify', 'help'})
_TOP_LEVEL_VALUE_FLAGS = frozenset({'-z', '--oneshot', '-m', '--model', '--provider', '-t', '--toolsets', '-r', '--resume', '-s', '--skills', '--usage-file', '-c', '--continue'})
...
def _first_positional_argv() -> str | None:
    ...
    ``duck-agent -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``duck-agent <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``duck-agent photon``
    ...
    try:
        ...
        logging.getLogger(__name__).debug('Deferred platform CLI resolution failed for %s: %s', command_name, exc)
_AGENT_COMMANDS = {None, 'chat', 'acp', 'rl'}
_AGENT_SUBCOMMANDS = {'cron': ('cron_command', {'run', 'tick'}), 'gateway': ('gateway_command', {'run'}), 'mcp': ('mcp_action', {'serve'})}
...
def _prepare_agent_startup(args) -> None:
    ...
    if not (args.command in _AGENT_COMMANDS or (_sub_attr and getattr(args, _sub_attr, None) in _sub_set)):
        return
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            ...
            logger.debug('Background MCP tool discovery failed at CLI startup', exc_info=True)
        _run_inline_mcp_discovery = False
    ...
    try:
        ...
        logger.debug('shell-hook registration failed at CLI startup', exc_info=True)
...
def _set_chat_arg_defaults(args) -> None:
    for attr, default in [('query', None), ('model', None), ('provider', None), ('toolsets', None), ('verbose', False), ('resume', None), ('continue_last', None), ('worktree', False)]:
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if _is_termux_fast_version_argv(argv):
        ...
        return True
    first = _first_positional_argv()
    has_oneshot = any((arg == '-z' or arg == '--oneshot' or arg.startswith('--oneshot=') for arg in argv))
    ...
    if getattr(args, 'oneshot', None):
        ...
        _run_and_exit_oneshot(args.oneshot, model=getattr(args, 'model', None), provider=getattr(args, 'provider', None), toolsets=getattr(args, 'toolsets', None), usage_file=getattr(args, 'usage_file', None))
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `duck-agent --tui` is the hot path on phones. The full parser setup imports
...
def cmd_acp(args):
    """Launch Duck Agent as an ACP server."""
    try:
...
def cmd_monitoring(args):
    ...
    if action == 'status':
        ...
        print(f"  Health export:  {('enabled' if gh.get('enabled') else 'disabled')} (monitoring.gateway_health_export.enabled)")
        if gh.get('enabled'):
            print(f"    Metrics:            {('on' if gh.get('metrics_enabled', True) else 'off')} (interval {gh.get('export_interval_seconds', 60)}s)")
            print(f"    Diagnostic events:  {('on' if gh.get('diagnostic_events_enabled', True) else 'off')}")
            print(f"    Warning/error logs: {('on' if gh.get('warning_error_events_enabled', True) else 'off')} (interval {gh.get('logs_export_interval_seconds', 5)}s)")
            print('    Content safety:     always on (rendered messages are never exported; not configurable)')
        ...
        print(f"  OTel SDK:       {('installed' if otlp_exporter.is_available() else 'not installed')} (optional extra: duck-agent[otlp])")
...
def main():
    """Main entry point for duck-agent CLI."""
    _set_process_title()
    try:
    ...
    moa_parser = subparsers.add_parser('moa', help='Configure Mixture of Agents provider/model slots', description='Configure the provider/model set used by /moa <prompt>.')
    ...
    fallback_parser = subparsers.add_parser('fallback', help='Manage fallback providers (tried when the primary model fails)', description='Manage the fallback provider chain.  Fallback providers are tried in order when the primary model fails with rate-limit, overload, or connection errors.  See: https://duck-agent.nousresearch.com/docs/user-guide/features/fallback-providers')
    fallback_subparsers = fallback_parser.add_subparsers(dest='fallback_command')
    fallback_subparsers.add_parser('list', aliases=['ls'], help='Show the current fallback chain (default when no subcommand)')
    fallback_subparsers.add_parser('add', help='Pick a provider + model (same picker as `duck-agent model`) and append to the chain')
    fallback_subparsers.add_parser('remove', aliases=['rm'], help='Pick an entry to delete from the chain')
    fallback_subparsers.add_parser('clear', help='Remove all fallback entries')
    fallback_parser.set_defaults(func=cmd_fallback)
    secrets_parser = subparsers.add_parser('secrets', help='Manage external secret sources (Bitwarden, 1Password)', description='Pull API keys from an external secret manager at process startup instead of storing them in ~/.duck-agent/.env.  Supports Bitwarden Secrets Manager and 1Password.  See: https://duck-agent.nousresearch.com/docs/user-guide/secrets/')
    secrets_subparsers = secrets_parser.add_subparsers(dest='secrets_command')
    secrets_bw = secrets_subparsers.add_parser('bitwarden', aliases=['bw'], help='Bitwarden Secrets Manager integration')
    secrets_op = secrets_subparsers.add_parser('onepassword', aliases=['op', '1password'], help='1Password (op:// references) integration')
    ...
    egress_parser = subparsers.add_parser('egress', help='Manage the iron-proxy egress credential-injection firewall', description='Manage iron-proxy, the optional TLS-intercepting egress firewall that swaps proxy tokens for real API credentials before outbound requests leave a sandbox.  Disabled by default.  See: https://duck-agent.nousresearch.com/docs/user-guide/egress/iron-proxy')
    ...
    migrate_parser = subparsers.add_parser('migrate', help='Migrate configuration for retired models or deprecated settings', description='Diagnose and (optionally) rewrite the active config.yaml to replace references to retired models or deprecated settings.')
    ...
    migrate_xai.add_argument('--apply', action='store_true', help='Rewrite config.yaml in-place (default: dry-run, no writes)')
    migrate_xai.add_argument('--no-backup', action='store_true', help='Skip the timestamped backup of config.yaml when applying')
    migrate_xai.set_defaults(func=cmd_migrate_xai)
    migrate_parser.set_defaults(func=cmd_migrate)
    build_gateway_parser(subparsers, cmd_gateway=cmd_gateway, cmd_proxy=cmd_proxy, cmd_gateway_enroll=cmd_gateway_enroll)
    try:
    ...
    whatsapp_cloud_parser = subparsers.add_parser('whatsapp-cloud', help='Set up WhatsApp Business Cloud API integration', description='Configure the official Meta WhatsApp Business Cloud API adapter (Business account required, public webhook URL required). Distinct from `duck-agent whatsapp` which sets up the Baileys bridge for personal accounts.')
    ...
    checkpoints_parser = subparsers.add_parser('checkpoints', help='Inspect / prune / clear ~/.duck-agent/checkpoints/', description='Manage the filesystem checkpoint store — the shadow git repo duck-agent uses to snapshot working directories before write_file/patch/terminal calls. Lets you see how much space checkpoints occupy, force a prune, or wipe the base.')
    ...
    bundles_parser = subparsers.add_parser('bundles', help='Create, list, and manage skill bundles (aliases for multiple skills)', description='Skill bundles let you load several skills under one slash command. `/<bundle>` from the CLI or gateway loads every referenced skill at once.')
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            for cmd_info in discover_plugin_cli_commands():
                plugin_parser = subparsers.add_parser(cmd_info['name'], help=cmd_info['help'], description=cmd_info.get('description', ''), formatter_class=__import__('argparse').RawDescriptionHelpFormatter)
    ...
    pets_parser = subparsers.add_parser('pets', help='Browse, install, and select petdex animated pets', description='Petdex (https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Duck Agent shows it reacting to agent activity across the CLI, TUI, and desktop app.')
    try:
    ...
    computer_use_parser = subparsers.add_parser('computer-use', help='Manage the Computer Use (cua-driver) backend (macOS/Windows/Linux)', description="Install or check the cua-driver binary used by the\n`computer_use` toolset. Supported on macOS, Windows, and\nLinux.\n\nUse `duck-agent computer-use install` to fetch and run the\nupstream cua-driver installer. This is equivalent to the\npost-setup hook that `duck-agent tools` runs when you first\nenable the Computer Use toolset, and is a stable target\nfor re-running the install if it didn't fire (e.g. when\ntoggling the toolset on a returning-user setup).\n\nUse `duck-agent computer-use doctor` to run cua-driver's\n`health_report` MCP tool and surface its check matrix\n(TCC, bundle identity, version, platform support, ...)\nin human-readable form.")
    computer_use_sub = computer_use_parser.add_subparsers(dest='computer_use_action')
    computer_use_install = computer_use_sub.add_parser('install', help='Install or repair the cua-driver binary (macOS/Windows/Linux)')
    computer_use_install.add_argument('--upgrade', action='store_true', help='Re-run the upstream installer even if cua-driver is already on PATH. The upstream install.sh always pulls the latest release, so this performs an in-place upgrade.')
    computer_use_sub.add_parser('status', help='Print whether cua-driver is installed and on PATH')
    computer_use_doctor = computer_use_sub.add_parser('doctor', help='Run cua-driver `health_report` and surface the check matrix', description="Drive cua-driver's stable `health_report` MCP tool and render\nits check matrix (TCC permissions, bundle identity, version,\nplatform support, screenshot probe, …) as human-readable\noutput. cua-driver owns the health model; this command stays\nthin so new checks added upstream surface here without code\nchanges. Exits 0 when overall=ok, 1 when degraded/failed, 2\nwhen the binary is missing or unreachable.")
    computer_use_doctor.add_argument('--include', action='append', default=[], metavar='CHECK', help='Run only the listed checks. Repeat for multiple (e.g. --include tcc_accessibility --include bundle_identity). Unknown names are reported by cua-driver.')
    computer_use_doctor.add_argument('--skip', action='append', default=[], metavar='CHECK', help='Skip the listed checks. Repeat for multiple. Wins over --include.')
    computer_use_doctor.add_argument('--json', action='store_true', help='Emit the raw structured payload as JSON (same shape as `tools/call`).')
    computer_use_perms = computer_use_sub.add_parser('permissions', help='Check or grant macOS Accessibility + Screen Recording (macOS)', description="Computer Use drives the Mac through cua-driver, whose TCC grants\nattach to cua-driver's own identity (com.trycua.driver) — not the\nterminal or the Duck Agent app. `status` reports the driver's grant\nstate; `grant` launches CuaDriver via LaunchServices so the macOS\npermission dialog is attributed to the process that does the work.")
    computer_use_perms_sub = computer_use_perms.add_subparsers(dest='computer_use_perms_action')
    computer_use_perms_status = computer_use_perms_sub.add_parser('status', help='Report Accessibility + Screen Recording grant state (read-only)')
    computer_use_perms_status.add_argument('--json', action='store_true', help='Emit the normalized permission payload as JSON.')
    computer_use_perms_sub.add_parser('grant', help='Request the grants (opens the dialog attributed to CuaDriver)')
    ...
    def cmd_computer_use(args):
        ...
        if action == 'status':
            ...
            if path:
                ...
                try:
                    ...
                    version = subprocess.run([path, '--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5, env=_cua_driver_env()).stdout.strip()
                except Exception:
                    pass
                if version:
                ...
                try:
                    ...
                    if st and st.get('update_available'):
                        ...
                        print('    Run: duck-agent computer-use install --upgrade')
                    elif st:
                        print('  ✓ Up to date.')
                    else:
                        print('  Refresh to latest: duck-agent computer-use install --upgrade')
                except Exception:
                    print('  Refresh to latest: duck-agent computer-use install --upgrade')
                return
        ...
        if action == 'doctor':
            ...
            code = run_doctor(include=list(getattr(args, 'include', []) or []), skip=list(getattr(args, 'skip', []) or []), json_output=bool(getattr(args, 'json', False)))
            sys.exit(code)
        if action == 'permissions':
            ...
            if perms_action == 'status':
                ...
                if not st['platform_supported']:
                    ...
                    sys.exit(1)
                if not st['installed']:
                    print('cua-driver: not installed. Run: duck-agent computer-use install')
                    sys.exit(1)
                ...
                if st['can_grant']:
                    ...
                    if not st['ready']:
                        print('  Grant: duck-agent computer-use permissions grant')
                else:
    ...
    sessions_parser = subparsers.add_parser('sessions', help='Manage session history (list, rename, export, prune, delete)', description='View and manage the SQLite session store')
    ...
    sessions_list.add_argument('--workspace', metavar='NEEDLE', help='Only sessions in one workspace: a git repo root or project dir (matched by path substring or basename).')

    def _add_session_filter_args(p, default_older_help):
        p.add_argument('--older-than', metavar='AGE', help=default_older_help)
        p.add_argument('--newer-than', metavar='AGE', help="Only match sessions active within the last AGE (e.g. '5h', '2d') or after an ISO timestamp")
        p.add_argument('--before', metavar='TIME', help="Only match sessions started before TIME (duration ago like '5h', or ISO timestamp like '2026-07-05 14:30')")
        p.add_argument('--after', metavar='TIME', help="Only match sessions started at/after TIME (duration ago like '5h', or ISO timestamp)")
        ...
        p.add_argument('--cwd', help='Only match sessions whose working directory is under this path')
        ...
        p.add_argument('--model', help="Only match sessions whose model name contains this substring (e.g. 'sonnet', 'gpt-5', 'duck-agent')")
        p.add_argument('--provider', help='Only match sessions billed through this provider (e.g. openrouter, anthropic, nous)')
        ...
        p.add_argument('--chat-type', help='Only match sessions with this chat type (e.g. dm, group)')
        p.add_argument('--branch', help='Only match sessions whose git branch contains this substring')
        p.add_argument('--min-tokens', type=int, help='Only match sessions with >= N total tokens (input+output)')
        p.add_argument('--max-tokens', type=int, help='Only match sessions with <= N total tokens (input+output)')
        p.add_argument('--min-cost', type=float, help='Only match sessions costing >= N USD (actual or estimated)')
        p.add_argument('--max-cost', type=float, help='Only match sessions costing <= N USD (actual or estimated)')
        p.add_argument('--min-tool-calls', type=int, help='Only match sessions with >= N tool calls')
        p.add_argument('--max-tool-calls', type=int, help='Only match sessions with <= N tool calls')
        p.add_argument('--dry-run', action='store_true', help='List matching sessions without changing anything')
        p.add_argument('--yes', '-y', action='store_true', help='Skip confirmation')
    sessions_export = sessions_subparsers.add_parser('export', help='Export sessions to JSONL, Markdown, or QMD')
    sessions_export.add_argument('output', nargs='?', help='Output path. JSONL: file path (use - for stdout, required). md/qmd: output directory (default: <duck-agent home>/session-exports)')
    sessions_export.add_argument('--format', choices=['jsonl', 'md', 'qmd', 'html', 'trace'], default='jsonl', help="Export format (default: jsonl). 'trace' emits Claude Code JSONL for the Hugging Face Agent Trace Viewer")
    sessions_export.add_argument('--upload', action='store_true', help='trace only: upload to your Hugging Face traces dataset instead of writing a local file (needs HF_TOKEN)')
    sessions_export.add_argument('--public', action='store_true', help='trace --upload only: create/update a public dataset instead of private')
    sessions_export.add_argument('--no-redact', action='store_true', help='trace only: skip the forced secret redaction; only use after manual review')
    sessions_export.add_argument('--only', choices=['user-prompts'], help='Export only a filtered view (user-prompts: one prompt record per line for jsonl, headed sections for md)')
    sessions_export.add_argument('--session-id', help='Session ID or unique prefix to export')
    _add_session_filter_args(sessions_export, "Only export sessions older than AGE (duration like '5h'/'2d', bare number of days, or an ISO timestamp)")
    sessions_export.add_argument('--redact', action='store_true', help='Redact secrets (API keys, tokens, credentials) from exported content')
    sessions_export.add_argument('--lineage', choices=['single', 'logical'], default='single', help='md/qmd only: export one row or its compression lineage')
    sessions_export.add_argument('--delete-after-verified', action='store_true', help='md/qmd only: after verified single-session export, delete that session (needs --yes)')
    sessions_export.add_argument('--force', action='store_true', help='md/qmd only: overwrite an existing export file')
    ...
    sessions_prune = sessions_subparsers.add_parser('prune', help='Delete old sessions (filterable by time window, source, title, ...)')
    _add_session_filter_args(sessions_prune, "Delete sessions older than AGE — days if bare number, or a duration like '5h'/'2d'/'1w', or an ISO timestamp (bare prune with no filters defaults to 90 days; any filter matches all ages)")
    sessions_prune.add_argument('--include-archived', action='store_true', help='Also delete archived sessions (excluded by default)')
    sessions_archive = sessions_subparsers.add_parser('archive', help='Bulk-archive (soft-hide) sessions matching filters — no deletion')
    _add_session_filter_args(sessions_archive, "Only archive sessions older than AGE (duration like '5h'/'2d', bare number of days, or ISO timestamp)")
    sessions_subparsers.add_parser('optimize', help='Reclaim disk space: merge FTS5 segments + VACUUM (no data change)')
    sessions_clean_markers = sessions_subparsers.add_parser('clean-markers', help='Permanently clear stale tool-call marker content left by sessions from before #78148', description='Before the #78148 fix, a local tool-call template could persist a bare bracketed marker (e.g. "[memory]") as an assistant turn\'s content instead of real text. This is already repaired in memory on every session load, so running this is optional — it rewrites the affected rows once, in place, so long-lived sessions stop re-scanning/re-repairing the same rows on every resume. Only the content column is touched; tool_calls and every other column on the row are left untouched.')
    sessions_clean_markers.add_argument('--dry-run', action='store_true', default=False, help='Report the affected row count without writing')
    sessions_clean_markers.add_argument('--no-backup', action='store_true', default=False, help='Skip the timestamped state.db backup taken before writing (not recommended)')
    sessions_optimize_storage = sessions_subparsers.add_parser('optimize-storage', help='Migrate the search index to the compact v23 layout (reclaims disk on large DBs)', description='Rebuild the full-text search index in the compact v23 external-content layout. On large databases this reclaims a large fraction of state.db (the old layout stored duplicate copies of every message and indexed tool output). Runs foreground with a progress bar, throttles so a running gateway stays responsive, and VACUUMs at the end. Safe to interrupt and re-run — it resumes where it left off. No conversation data is changed; only the search index is rebuilt.')
    sessions_optimize_storage.add_argument('--no-vacuum', action='store_true', default=False, help="Skip the final VACUUM (index is rebuilt but freed pages aren't returned to the OS until a later VACUUM)")
    sessions_optimize_storage.add_argument('--yes', '-y', action='store_true', default=False, help='Skip the disk-space confirmation prompt')
    sessions_repair = sessions_subparsers.add_parser('repair', help='Repair a malformed state.db schema so hidden sessions reappear', description="Recover a state.db whose schema is malformed (e.g. 'table messages_fts already exists'), which makes Desktop/Dashboard show no sessions. A backup is made first; sessions and messages are preserved and the FTS search index is rebuilt if needed.")
    sessions_repair.add_argument('--check-only', action='store_true', help='Only report whether the database opens cleanly; do not modify it')
    sessions_repair.add_argument('--no-backup', action='store_true', help='Skip the timestamped backup copy (not recommended)')
    sessions_recover = sessions_subparsers.add_parser('recover', help='Rebuild canonical session data into a separate clean database', description='Offline, non-destructive recovery for a damaged state.db. The source database and its WAL/SHM/rollback-journal sidecars are copied before SQLite opens anything. Canonical rows are rebuilt into a new output database; derived search indexes are recreated and the active database is never replaced automatically.')
    sessions_recover.add_argument('--source', type=Path, required=True, help='Source state.db or preserved backup to inspect/recover')
    sessions_recover.add_argument('--output', type=Path, help='New recovery database path (required unless --inspect-only)')
    sessions_recover.add_argument('--inspect-only', action='store_true', help='Only report canonical table readability; do not create an output database')
    sessions_recover.add_argument('--work-dir', type=Path, help='Existing directory for the disposable source copy (defaults beside the output)')
    sessions_recover.add_argument('--chunk-size', type=int, default=1000, help='Rows committed per recovery batch (default: 1000)')
    sessions_recover.add_argument('--allow-partial', action='store_true', help='Best-effort salvage across damaged row ranges; the output remains separate and every skipped range is recorded')
    sessions_recover.add_argument('--report', type=Path, help='JSON report path (defaults to <output>.recovery.json)')
    ...
    sessions_retitle = sessions_subparsers.add_parser('retitle-skills', help="Re-title sessions whose auto-title came from a /skill's own text", description='Sessions opened with a /skill were auto-titled from the expanded message, which embeds the whole skill body — so the title describes the SKILL, not the request. This regenerates those titles from what the user actually typed. Lists what it would change unless --apply is passed.')
    sessions_retitle.add_argument('--apply', action='store_true', help='Write the new titles (default: dry run)')
    sessions_retitle.add_argument('--limit', type=int, default=200, help='Maximum sessions to examine (default: 200)')
    sessions_browse = sessions_subparsers.add_parser('browse', help='Interactive session picker — browse, search, and resume sessions')
    sessions_browse.add_argument('--source', help='Filter by source (cli, telegram, discord, etc.)')
    sessions_browse.add_argument('--limit', type=int, default=500, help='Max sessions to load (default: 500)')
    sessions_parser.set_defaults(func=_functools.partial(cmd_sessions, sessions_parser=sessions_parser))
    ...
    completion_parser = subparsers.add_parser('completion', help='Print shell completion script (bash, zsh, or fish)')
    completion_parser.add_argument('shell', nargs='?', default='bash', choices=['bash', 'zsh', 'fish'], help='Shell type (default: bash)')
    completion_parser.set_defaults(func=lambda args: cmd_completion(args, parser))
    build_dashboard_parser(subparsers, cmd_dashboard=cmd_dashboard, cmd_dashboard_register=cmd_dashboard_register)
    ...
    if (args.resume or args.continue_last) and args.command is None:
        args.command = 'chat'
        for attr, default in [('query', None), ('model', None), ('provider', None), ('toolsets', None), ('verbose', None), ('worktree', False)]:
            if not hasattr(args, attr):
                setattr(args, attr, default)
        cmd_chat(args)
        return
    if args.command is None:
        for attr, default in [('query', None), ('model', None), ('provider', None), ('toolsets', None), ('verbose', None), ('resume', None), ('continue_last', None), ('worktree', False)]:
            if not hasattr(args, attr):
                setattr(args, attr, default)
        cmd_chat(args)
        return
