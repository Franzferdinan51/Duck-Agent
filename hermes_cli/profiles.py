"""
Profile management for multiple isolated Duck Agent instances.

Each profile is a fully independent DUCK_AGENT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.duck-agent/profiles/<name>/`` by default.

The "default" profile is ``~/.duck-agent`` itself — backward compatible,
zero migration needed.

Usage::

    duck-agent profile create coder          # fresh profile + bundled skills
    duck-agent profile create coder --clone  # also copy config, .env, SOUL.md, skills
    duck-agent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    duck-agent -p coder chat                 # or via flag
    duck-agent profile use coder             # set as sticky default
    duck-agent profile delete coder          # remove profile + alias + service
"""
import json
import os
import re
...
_PROFILE_ID_RE = re.compile('^[a-z0-9][a-z0-9_-]{0,63}$')
...
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({'duck-agent', '.worktrees', 'profiles', 'bin', 'node_modules'})
_CLONE_ALL_HISTORY_EXCLUDE_ROOT: frozenset[str] = frozenset({'state.db', 'state.db-wal', 'state.db-shm', 'sessions', 'backups', 'state-snapshots', 'checkpoints'})
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Duck Agent infrastructure directories that only the default profile
         (``~/.duck-agent``) ever contains.  Gated on ``source_dir`` actually
    ...
    def _ignore(directory: str, names: List[str]) -> List[str]:
        ...
        return ignored
    return _ignore
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({'duck-agent', '.worktrees', 'profiles', 'bin', 'node_modules', 'state.db', 'state.db-shm', 'state.db-wal', 'hermes_state.db', 'response_store.db', 'response_store.db-shm', 'response_store.db-wal', 'gateway.pid', 'gateway_state.json', 'processes.json', 'auth.json', '.env', 'auth.lock', 'active_profile', '.update_check', 'errors.log', '.hermes_history', 'image_cache', 'audio_cache', 'document_cache', 'browser_screenshots', 'checkpoints', 'sandboxes', 'logs'})
_DEFAULT_EXPORT_INCLUDE_ROOT = frozenset({'config.yaml', 'SOUL.md', 'MEMORY.md', 'USER.md', 'todo.json', 'system_prompt.md', 'AGENTS.md', 'CLAUDE.md', '.cursorrules', 'desktop.json', 'skills', 'cron', 'scripts', 'sessions', 'plugins', 'memories', 'knowledge', 'preferences'})
_RESERVED_NAMES = frozenset({'duck-agent', 'default', 'test', 'tmp', 'root', 'sudo'})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the duck-agent root, NOT to the current DUCK_AGENT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where DUCK_AGENT_HOME points outside
    ``~/.duck-agent``, profiles live under ``DUCK_AGENT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_hermes_home() / 'profiles'

def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) DUCK_AGENT_HOME path.

    In standard deployments this is ``~/.duck-agent``.
    In Docker/custom deployments where DUCK_AGENT_HOME is outside ``~/.duck-agent``
    (e.g. ``/opt/data``), returns DUCK_AGENT_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``duck-agent``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``duck-agent`` profile inside ``~/.duck-agent/``) or get refused
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(f'Profile name {name!r} is reserved — it collides with either the Duck Agent installation itself or a common system binary.  Pick a different name.')
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DUCK_AGENT_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, duck-agent subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a duck-agent subcommand"
    wrapper_dir = _get_wrapper_dir()
    is_windows = sys.platform == 'win32'
    try:
        result = subprocess.run(['where' if is_windows else 'which', canon], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
...
def create_wrapper_script(name: str, target: Optional[str]=None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hermes_exe = shutil.which('duck-agent') or 'duck-agent'
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Duck Agent. If we
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``duck-agent -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``duck-agent profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``duck-agent profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``DUCK_AGENT_HOME``.
    """
    try:
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    if not isinstance(data, dict):
        return {'description': '', 'description_auto': False}
    return {'description': str(data.get('description') or '').strip(), 'description_auto': bool(data.get('description_auto', False))}
...
def list_profiles() -> List[ProfileInfo]:
    ...
    if default_home.is_dir():
        ...
        profiles.append(ProfileInfo(name='default', path=default_home, is_default=True, gateway_running=_check_gateway_running(default_home), model=model, provider=provider, has_env=(default_home / '.env').exists(), skill_count=_count_skills(default_home), distribution_name=dist_name, distribution_version=dist_version, distribution_source=dist_source, description=meta.get('description', ''), description_auto=meta.get('description_auto', False)))
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own DUCK_AGENT_HOME.
    ...
    The returned ``duck_agent_home`` is the path to pass to
...
def create_profile(name: str, clone_from: Optional[str]=None, clone_all: bool=False, clone_config: bool=False, no_alias: bool=False, no_skills: bool=False, description: Optional[str]=None) -> Path:
    ...
    no_skills:
        ...
        a marker file so ``duck-agent update`` skips re-seeding this profile's
    ...
    if no_skills and (clone_from is not None or clone_config or clone_all):
        raise ValueError('--no-skills is mutually exclusive with --clone / --clone-from / --clone-all (cloning explicitly copies skills from the source profile).')
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon == 'default':
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.duck-agent).")
    ...
    if not env_path.exists():
        try:
            env_path.write_text('# Per-profile secrets for this Duck Agent profile.\n# API keys and tokens set here override the shell environment.\n# Behavioral settings belong in config.yaml, not here.\n', encoding='utf-8')
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text('This profile opted out of bundled-skill seeding (`duck-agent profile create --no-skills`).\nDelete this file to re-enable sync on the next `duck-agent update`.\n', encoding='utf-8')
        except OSError:
            pass
    if not clone_all:
...
def seed_profile_skills(profile_dir: Path, quiet: bool=False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DUCK_AGENT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``duck-agent profile create
    ...
    if has_bundled_skills_opt_out(profile_dir):
        return {'copied': [], 'updated': [], 'user_modified': [], 'skipped_opt_out': True}
    project_root = Path(__file__).parent.parent.resolve()
    try:
        result = subprocess.run([sys.executable, '-c', 'import json; from tools.skills_sync import sync_skills; r = sync_skills(quiet=True); print(json.dumps(r))'], env={**os.environ, 'DUCK_AGENT_HOME': str(profile_dir)}, cwd=str(project_root), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=60)
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Duck Agent *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Duck Agent backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``DUCK_AGENT_HOME``
    ...
    hermes_markers = ('hermes_cli.main', 'duck-agent-gateway', 'tui_gateway')
    pids: list[int] = []
    for proc in psutil.process_iter(['pid', 'name', 'username', 'cmdline']):
        try:
            ...
            is_hermes = any((marker in joined for marker in hermes_markers)) or exe_name == 'duck-agent' or exe_name.startswith('duck-agent')
            if not is_hermes:
                continue
            ...
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get('DUCK_AGENT_HOME', '')
...
def delete_profile(name: str, yes: bool=False) -> Path:
    ...
    validate_profile_name(canon)
    if canon == 'default':
        raise ValueError('Cannot delete the default profile (~/.duck-agent).\nTo remove everything, use: duck-agent uninstall')
    profile_dir = get_profile_dir(canon)
    if not profile_dir.is_dir():
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``DUCK_AGENT_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    old_home = os.environ.get('DUCK_AGENT_HOME')
    try:
        os.environ['DUCK_AGENT_HOME'] = str(profile_dir)
        ...
        if _platform.system() == 'Linux':
            ...
            if svc_file.exists():
                subprocess.run(['systemctl', '--user', 'disable', svc_name], capture_output=True, check=False, timeout=10)
                subprocess.run(['systemctl', '--user', 'stop', svc_name], capture_output=True, check=False, timeout=10)
                svc_file.unlink(missing_ok=True)
                subprocess.run(['systemctl', '--user', 'daemon-reload'], capture_output=True, check=False, timeout=10)
                print(f'✓ Service {svc_name} removed')
        elif _platform.system() == 'Darwin':
            plist_path = get_launchd_plist_path()
            if plist_path.exists():
                subprocess.run(['launchctl', 'unload', str(plist_path)], capture_output=True, check=False, timeout=10)
        ...
        if old_home is not None:
            os.environ['DUCK_AGENT_HOME'] = old_home
        elif 'DUCK_AGENT_HOME' in os.environ:
            del os.environ['DUCK_AGENT_HOME']
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.duck-agent/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != 'default' and (not profile_exists(canon)):
        raise FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: duck-agent profile create {canon}")
    path = _get_active_profile_path()
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DUCK_AGENT_HOME.

    Returns ``"default"`` if DUCK_AGENT_HOME is not set or points to ``~/.duck-agent``.
    Returns the profile name if DUCK_AGENT_HOME points into ``~/.duck-agent/profiles/<name>``.
    Returns ``"custom"`` if DUCK_AGENT_HOME is set to an unrecognized path.
    """
    from hermes_constants import get_hermes_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      DUCK_AGENT_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Duck Agent file the
      user may have lying alongside DUCK_AGENT_HOME (#58394).
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]]=None) -> Path:
    ...
    if canon == 'default':
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            shutil.copytree(profile_dir, staged, symlinks=True, ignore=_default_export_ignore(profile_dir))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        shutil.copytree(profile_dir, staged, symlinks=True, ignore=lambda d, contents: _CREDENTIAL_FILES & set(contents))
        _stage_extras(staged)
...
def _safe_extract_profile_archive(archive: Path, destination: Path) -> None:
    ...
    with tarfile.open(archive, 'r:gz') as tf:
        for member in tf.getmembers():
            ...
            try:
                os.chmod(target, member.mode & 511)
            except OSError:
                pass
...
def _inspect_profile_archive_roots(archive: Path) -> set[str]:
    ...
    import tarfile
    with tarfile.open(archive, 'r:gz') as tf:
        top_dirs = {parts[0] for member in tf.getmembers() for parts in [_normalize_profile_archive_parts(member.name)] if len(parts) > 1 or member.isdir()}
        if not top_dirs:
            top_dirs = {_normalize_profile_archive_parts(member.name)[0] for member in tf.getmembers() if member.isdir()}
    return top_dirs
...
def import_profile(archive_path: str, name: Optional[str]=None) -> Path:
    ...
    if not inferred_name:
        raise ValueError('Cannot determine profile name from archive. Specify it explicitly: duck-agent profile import <archive> --name <name>')
    ...
    validate_profile_name(canon)
    if canon == 'default':
        raise ValueError("Cannot import as 'default' — that is the built-in root profile (~/.duck-agent). Specify a different name: duck-agent profile import <archive> --name <name>")
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise FileExistsError(f"Profile '{canon}' already exists at {profile_dir}")
    profiles_root = _get_profiles_root()
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f'duck-agent.{old_name}'
    new_host = f'hermes_{new_name}'
    candidates = [new_dir / 'honcho.json', _get_default_hermes_home() / 'honcho.json', Path.home() / '.honcho' / 'config.json']
    seen: set[Path] = set()
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DUCK_AGENT_HOME path string.

    Called early in the CLI entry point, before any duck-agent modules
    are imported, to set the DUCK_AGENT_HOME environment variable.
    """
