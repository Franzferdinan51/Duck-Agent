"""CLI subcommand: `duck-agent curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print('  never auto-staled or archived — `duck-agent curator adopt <name>` hands one over')
...
def _cmd_status(args) -> int:
    ...
    print(f"  consolidate:    {('on' if curator.get_consolidate() else 'off')}{('' if curator.get_consolidate() else ' (prune-only; LLM merge pass opt-in)')}")
    ...
    for r in rows:
        ...
        if prov == 'agent':
            ...
            bundled_count += 1
    print(f'\ncurator-managed skills: {len(rows)} total  (agent-created={agent_count}  bundled={bundled_count})')
    ...
    if active:
        ...
        for r in active:
            ...
            print(f"  {r['name']:40s}  activity={r.get('activity_count', 0):3d}  use={r.get('use_count', 0):3d}  view={r.get('view_count', 0):3d}  patches={r.get('patch_count', 0):3d}  last_activity={last}")
    active_all = by_state.get('active', [])
    if active_all:
        most_active = sorted(active_all, key=lambda r: (r.get('activity_count') or 0, r.get('last_activity_at') or ''), reverse=True)[:5]
        if most_active and (most_active[0].get('activity_count') or 0) > 0:
            print('\nmost active (top 5):')
            for r in most_active:
                last = _fmt_ts(r.get('last_activity_at'))
                print(f"  {r['name']:40s}  activity={r.get('activity_count', 0):3d}  use={r.get('use_count', 0):3d}  view={r.get('view_count', 0):3d}  patches={r.get('patch_count', 0):3d}  last_activity={last}")
        least_active = sorted(active_all, key=lambda r: (r.get('activity_count') or 0, r.get('last_activity_at') or ''))[:5]
        if least_active:
            print('\nleast active (top 5):')
...
def _cmd_run(args) -> int:
    ...
    if consolidate is None and (not curator.get_consolidate()):
        print('curator: consolidation is off — running prune-only (deterministic stale/archive). Pass --consolidate or set `curator.consolidate: true` to enable the LLM merge pass.')

    def _on_summary(msg: str) -> None:
        print(msg)
    result = curator.run_curator_review(on_summary=_on_summary, synchronous=synchronous, dry_run=dry, consolidate=consolidate)
    ...
    if not synchronous:
        print('llm pass running in background — check `duck-agent curator status` later')
    if dry:
        if synchronous:
            print('dry-run: no changes applied. Read the report with `duck-agent curator status` and run `duck-agent curator run` (no flag) to apply.')
        else:
            print('dry-run: no changes applied. When the report lands, read it with `duck-agent curator status` and run `duck-agent curator run` (no flag) to apply.')
    return 0
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.is_agent_created(args.skill):
        print(f"curator: '{args.skill}' is bundled or hub-installed — cannot pin (only agent-created skills participate in curation)")
        return 1
...
def _cmd_unpin(args) -> int:
    from tools import skill_usage
    if not skill_usage.is_agent_created(args.skill):
        print(f"curator: '{args.skill}' is bundled or hub-installed — there's nothing to unpin (curator only tracks agent-created skills)")
        return 1
...
def _cmd_list_unmanaged(args) -> int:
    ...
    for r in sorted(rows, key=lambda x: x['name']):
        ...
        last = _fmt_ts(r.get('last_activity_at'))
        print(f"  {r['name']:44s} activity={r.get('activity_count', 0):4d}  last_activity={last:14s}  ({why})")
    print('\nadopt one with `duck-agent curator adopt <name>`, or all with `duck-agent curator adopt --all-unmanaged`')
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get('pinned'):
        print(f"curator: '{args.skill}' is pinned — unpin first with `duck-agent curator unpin {args.skill}`")
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f'curator: snapshot created at ~/.duck-agent/skills/.curator_backups/{snap.name}')
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print('curator: no snapshots exist yet. Take one with `duck-agent curator backup` or wait for the next curator run.')
        else:
    ...
    print('\nThis will replace the current ~/.duck-agent/skills/ tree (a safety snapshot of the current state is taken first so this is undoable). Cron jobs that still exist will have their skills/skill fields restored from the snapshot; all other cron fields are left alone.')
...
def _cmd_usage(args) -> int:
    ...
    for r in rows:
        last = _fmt_ts(r.get('last_activity_at'))
        print(f"  {r['name'][:40]:40s}  {r.get('provenance', 'agent'):8s}  {r.get('use_count', 0):>4d}  {r.get('view_count', 0):>4d}  {r.get('patch_count', 0):>5d}  {r.get('activity_count', 0):>4d}  {last}")
    return 0
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_usage = subs.add_parser('usage', help='Show usage telemetry for ALL skills (built-in, hub, agent) with provenance')
    p_usage.add_argument('--sort', choices=('activity', 'recent', 'name'), default='activity', help='Sort order: activity (most-used first, default), recent (most-recently-active first), or name (alphabetical)')
    p_usage.add_argument('--provenance', choices=('agent', 'bundled', 'hub'), default=None, help='Only show skills of this origin')
    p_usage.add_argument('--json', action='store_true', help='Emit the full report as JSON instead of a table')
    p_usage.set_defaults(func=_cmd_usage)
    p_run = subs.add_parser('run', help='Trigger a curator review now')
    p_run.add_argument('--sync', '--synchronous', dest='synchronous', action='store_true', help='Wait for the LLM review pass to finish (default for manual runs)')
    p_run.add_argument('--background', dest='background', action='store_true', help='Start the LLM review pass in a background thread and return immediately')
    p_run.add_argument('--dry-run', dest='dry_run', action='store_true', help='Report only — no state changes, no archives, no consolidation (use this to preview what curator would do)')
    p_run.add_argument('--consolidate', dest='consolidate', action='store_true', help='Force the LLM umbrella-building consolidation pass on for this run, overriding the config default (off). Without this flag the run is prune-only unless `curator.consolidate: true` is set.')
    ...
    subs.add_parser('list-unmanaged', help='List curation-eligible skills with no provenance marker').set_defaults(func=_cmd_list_unmanaged)
    p_adopt = subs.add_parser('adopt', help='Hand unmanaged skills to the curator (provenance is a user declaration)')
    p_adopt.add_argument('skill', nargs='*', help='Skill name(s) to adopt. Omit when using --all-unmanaged.')
    p_adopt.add_argument('--all-unmanaged', action='store_true', help='Adopt every curation-eligible skill that has no provenance marker')
    p_adopt.add_argument('--dry-run', action='store_true', help='List what would be adopted without writing anything')
    p_adopt.add_argument('--yes', action='store_true', help='Skip the confirmation prompt for --all-unmanaged')
    p_adopt.set_defaults(func=_cmd_adopt)
    p_restore = subs.add_parser('restore', help='Restore an archived skill')
    p_restore.add_argument('skill', help='Skill name')
    p_restore.set_defaults(func=_cmd_restore)
    subs.add_parser('list-archived', help='List archived skills').set_defaults(func=_cmd_list_archived)
    p_archive = subs.add_parser('archive', help='Manually archive a skill (move to .archive/, excluded from prompt)')
    p_archive.add_argument('skill', help='Skill name')
    p_archive.set_defaults(func=_cmd_archive)
    p_prune = subs.add_parser('prune', help='Bulk-archive curator-managed skills idle for >= N days (default 90)')
    p_prune.add_argument('--days', type=int, default=90, help='Archive skills idle for at least N days (default: 90)')
    p_prune.add_argument('-y', '--yes', action='store_true', help='Skip the confirmation prompt')
    p_prune.add_argument('--dry-run', dest='dry_run', action='store_true', help='Show what would be archived without doing it')
    p_prune.set_defaults(func=_cmd_prune)
    p_backup = subs.add_parser('backup', help='Take a manual tar.gz snapshot of ~/.duck-agent/skills/ (curator also does this automatically before every real run)')
    p_backup.add_argument('--reason', default=None, help="Free-text label stored in manifest.json (default: 'manual')")
    p_backup.set_defaults(func=_cmd_backup)
    p_rollback = subs.add_parser('rollback', help='Restore ~/.duck-agent/skills/ from a curator snapshot (defaults to the newest)')
    p_rollback.add_argument('--list', action='store_true', help='List available snapshots and exit without restoring')
    p_rollback.add_argument('--id', dest='backup_id', default=None, help='Snapshot id to restore (see `--list`); default: newest')
    p_rollback.add_argument('-y', '--yes', action='store_true', help='Skip confirmation prompt')
    p_rollback.set_defaults(func=_cmd_rollback)

def cli_main(argv=None) -> int:
    """Standalone entry (also usable by hermes_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog='duck-agent curator')
    register_cli(parser)
