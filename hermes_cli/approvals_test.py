"""``duck-agent approvals test`` — dry-run approval verdict for a command.
...
EXIT_ASK = 2
EXIT_DENY = 3
_VERDICT_EXIT = {'allow': EXIT_ALLOW, 'ask-approval': EXIT_ASK, 'hardline-deny': EXIT_DENY, 'user-deny': EXIT_DENY}
...
def evaluate_command(command: str, env_type: str='local') -> dict:
    ...
    def result(verdict: str, rule=None, detail: str='') -> dict:
        return {'command': command, 'env_type': env_type, 'verdict': verdict, 'exit_code': _VERDICT_EXIT[verdict], 'rule': rule, 'detail': detail, 'normalized_variants': variants}
    if approval._should_skip_container_guards(env_type):
        return result('allow', detail=f"env_type '{env_type}' is an isolated container backend; the runtime skips all command guards for it")
    is_hardline, hardline_desc = approval.detect_hardline_command(command)
    if is_hardline:
        return result('hardline-deny', rule=hardline_desc, detail='matches the hardline blocklist (never bypassable, blocked even under --yolo / approvals.mode=off)')
    ...
    if deny_pattern is not None:
        return result('user-deny', rule=deny_pattern, detail='matches a user-defined approvals.deny rule in config.yaml (blocked even under --yolo / mode=off)')
    if approval._YOLO_MODE_FROZEN or approval.is_current_session_yolo_enabled() or approval._get_approval_mode() == 'off':
        return result('allow', detail='approval bypass active (--yolo or approvals.mode: off); only hardline/deny rules would block')
    ...
    if is_dangerous:
        return result('ask-approval', rule=description, detail=f'matches a dangerous-command pattern; the runtime would raise an interactive approval prompt (pattern key: {pattern_key!r})')
...
def approvals_test_command(args) -> int:
    """Handle ``duck-agent approvals test <command...>``. Returns the exit code."""
    words = list(getattr(args, 'command_words', None) or [])
    if words and words[0] == '--':
        words = words[1:]
    if not words:
        print('usage: duck-agent approvals test [--env-type TYPE] [--json] -- <command...>')
        return EXIT_USAGE
    command = ' '.join(words)
