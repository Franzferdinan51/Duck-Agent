...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``duck-agent -p <profile> auth``). No HTTP side effect.
    ...
    * ``comment`` — nudge the operator to add a comment (for
    ...
    def to_dict(self) -> dict:
        return {'kind': self.kind, 'label': self.label, 'payload': self.payload, 'suggested': self.suggested}
...
class Diagnostic:
    ...
    def to_dict(self) -> dict:
        return {'kind': self.kind, 'severity': self.severity, 'title': self.title, 'detail': self.detail, 'actions': [a.to_dict() for a in self.actions], 'first_seen_at': self.first_seen_at, 'last_seen_at': self.last_seen_at, 'count': self.count, 'run_id': self.run_id, 'data': self.data}
...
def _generic_recovery_actions(task: Any, *, running: bool) -> list[DiagnosticAction]:
    ...
    out.append(DiagnosticAction(kind='reassign', label='Reassign to different profile', payload={'reclaim_first': running}))
    return out
...
def triage_aux_status(config: Optional[dict]) -> Optional[dict]:
    ...
    return {'auto_decompose': auto_decompose, 'decomposer_explicit': decomposer_explicit, 'specifier_explicit': specifier_explicit, 'main_model_visible': _main_model_visible(config)}
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``duck-agent kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = 'Auto-decompose is on, so the dispatcher needs auxiliary.kanban_decomposer (with auxiliary.triage_specifier as a fallback for non-fan-out tasks).'
    else:
        ...
        detail_path = 'Auto-decompose is off, so triage tasks need `duck-agent kanban specify`, which uses auxiliary.triage_specifier.'
    if primary_explicit or main_visible:
        return []
    task_id = _task_field(task, 'id') or '<task_id>'
    actions = [DiagnosticAction(kind='cli_hint', label=f'Configure {primary_slot}', payload={'command': f'duck-agent config set {primary_slot}.provider auto'}, suggested=True)]
    if not fallback_explicit and (not main_visible):
        actions.append(DiagnosticAction(kind='cli_hint', label=f'Or configure fallback {fallback_slot}', payload={'command': f'duck-agent config set {fallback_slot}.provider auto'}))
    if not auto_decompose:
        actions.append(DiagnosticAction(kind='cli_hint', label=f'Specify manually: duck-agent kanban specify {task_id}', payload={'command': f'duck-agent kanban specify {task_id}'}))
    return [Diagnostic(kind='triage_aux_unavailable', severity='warning', title=f'Triage {primary_desc} has no usable model', detail=f'This task is still in triage and no working auxiliary model is visible to the dispatcher. {detail_path} The default slot uses `provider: auto` which falls back to the main model, but no main model is configured either. Configure the slot directly or set a main model so the auto fallback can take over.', actions=actions, first_seen_at=now, last_seen_at=now, count=1, data={'task_id': task_id, 'auto_decompose': auto_decompose, 'primary_slot': primary_slot, 'main_model_visible': main_visible})]
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if _task_field(task, 'status') in ('done', 'archived', 'running'):
        return []
    threshold = _positive_int(cfg.get('failure_threshold', cfg.get('spawn_failure_threshold', 3)), 3)
    failure_limit = _positive_int(cfg.get('failure_limit'), threshold)
    failures = _task_field(task, 'consecutive_failures', None) if _task_field(task, 'consecutive_failures', None) is not None else _task_field(task, 'spawn_failures', 0)
    if failures is None or failures < threshold:
        return []
    last_err = _task_field(task, 'last_failure_error', None) if _task_field(task, 'last_failure_error', None) is not None else _task_field(task, 'last_spawn_error', None)
    ...
    if most_recent_outcome == 'spawn_failed' and assignee and (assignee != 'default'):
        actions.append(DiagnosticAction(kind='cli_hint', label=f'Verify profile: duck-agent -p {assignee} doctor', payload={'command': f'duck-agent -p {assignee} doctor'}, suggested=True))
        actions.append(DiagnosticAction(kind='cli_hint', label=f'Fix profile auth: duck-agent -p {assignee} auth', payload={'command': f'duck-agent -p {assignee} auth'}))
    elif most_recent_outcome in {'timed_out', 'crashed'}:
        task_id = _task_field(task, 'id')
        if task_id:
            actions.append(DiagnosticAction(kind='cli_hint', label=f'Check logs: duck-agent kanban log {task_id}', payload={'command': f'duck-agent kanban log {task_id}'}, suggested=True))
    actions.extend(_generic_recovery_actions(task, running=_task_field(task, 'status') == 'running'))
    ...
    outcome_label = {'spawn_failed': 'spawn', 'timed_out': 'timeout', 'crashed': 'crash'}.get(most_recent_outcome or '', 'failure')
    if err_snippet:
        title = f'Agent {outcome_label} x{failures}: {err_snippet.splitlines()[0][:160]}'
        detail = f'This task has failed {failures} times in a row (most recent: {outcome_label}). Full last error:\n\n{err_snippet}\n\nThe dispatcher circuit breaker is configured for {failure_limit} consecutive non-success attempts. Fix the root cause and reclaim or unblock the task to retry.'
    else:
        title = f'Agent {outcome_label} x{failures} (no error recorded)'
        detail = f'This task has failed {failures} times in a row (most recent: {outcome_label}) but no error text was captured. Check the suggested command or the worker log.'
    return [Diagnostic(kind='repeated_failures', severity=severity, title=title, detail=detail, actions=actions, first_seen_at=now, last_seen_at=now, count=failures, data={'consecutive_failures': failures, 'most_recent_outcome': most_recent_outcome, 'last_error': last_err, 'failure_threshold': threshold, 'failure_limit': failure_limit})]
...
def _rule_repeated_crashes(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if _task_field(task, 'status') in ('done', 'archived', 'running'):
        return []
    failure_threshold = int(cfg.get('failure_threshold', cfg.get('spawn_failure_threshold', 3)))
    ...
    if err_snippet:
        ...
        detail = f'The last {consecutive} runs ended with outcome=crashed. Full last error:\n\n{err_snippet}'
    else:
        title = f'Agent crashed {consecutive}x (no error recorded)'
        detail = f'The last {consecutive} runs ended with outcome=crashed but no error text was captured. Check the worker log for more.'
    return [Diagnostic(kind='repeated_crashes', severity=severity, title=title, detail=detail, actions=actions, first_seen_at=now, last_seen_at=now, count=consecutive, data={'consecutive_crashes': consecutive, 'last_error': last_err})]
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions: list[DiagnosticAction] = []
    if task_id:
        actions.append(DiagnosticAction(kind='cli_hint', label=f'Check block reasons: duck-agent kanban events {task_id}', payload={'command': f'duck-agent kanban events {task_id}'}, suggested=True))
    return [Diagnostic(kind='block_unblock_cycling', severity='warning', title=f'Task block→unblock cycled {cycles}x in {int(window_seconds / 3600)}h', detail=f'This task has been blocked {cycles} times after being unblocked, suggesting the unblock is not addressing the root cause and the worker keeps hitting the same wall. Review the block reasons in the event history; a different intervention (reassign, change scope, archive) may be needed.', actions=actions, first_seen_at=int(initial_blocked_ts) if initial_blocked_ts else int(now), last_seen_at=int(last_cycle_blocked_ts) if last_cycle_blocked_ts else int(now), count=cycles, data={'cycles': cycles, 'window_seconds': int(window_seconds)})]
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong DUCK_AGENT_HOME).
    ...
    works for Duck Agent profiles, registered lanes, external workers, and
    ...
    if age_seconds >= threshold_seconds * 6:
        ...
        severity = 'warning'
    actions = [DiagnosticAction(kind='reassign', label='Reassign to a different worker', payload={'current_assignee': assignee}), DiagnosticAction(kind='cli_hint', label='Check dispatcher status', payload={'command': 'duck-agent kanban diagnostics'})]
    return [Diagnostic(kind='stranded_in_ready', severity=severity, title=f'Ready for {age_str} with no worker', detail=f'This task has been ready for {age_str} but nothing has claimed it. Common causes: assignee {assignee!r} is misspelled, the profile was deleted, or the external worker pool for this lane is down. Confirm the assignee is correct and that a worker is actually polling for it.', actions=actions, first_seen_at=last_ready_ts, last_seen_at=last_ready_ts, count=1, data={'ready_since': last_ready_ts, 'age_seconds': int(age_seconds), 'assignee': assignee, 'threshold_seconds': int(threshold_seconds)})]
_RULES: list[RuleFn] = [_rule_hallucinated_cards, _rule_triage_aux_unavailable, _rule_prose_phantom_refs, _rule_repeated_failures, _rule_repeated_crashes, _rule_stuck_in_blocked, _rule_block_unblock_cycling, _rule_stranded_in_ready]
DIAGNOSTIC_KINDS = ('hallucinated_cards', 'triage_aux_unavailable', 'prose_phantom_refs', 'repeated_failures', 'repeated_crashes', 'stuck_in_blocked', 'block_unblock_cycling', 'stranded_in_ready')
DEFAULT_CONFIG = {'failure_threshold': 2, 'spawn_failure_threshold': 2, 'crash_threshold': 2, 'blocked_stale_hours': 24, 'stranded_threshold_seconds': 30 * 60}
...
def config_from_kanban_config(kanban_cfg: Optional[dict]) -> dict:
    ...
    if 'failure_threshold' not in diag_cfg and 'spawn_failure_threshold' not in diag_cfg:
        diag_cfg['failure_threshold'] = diag_cfg['failure_limit']
    return diag_cfg

def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Duck Agent runtime config.
...
def compute_task_diagnostics(task, events: list, runs: list, *, now: Optional[int]=None, config: Optional[dict]=None) -> list[Diagnostic]:
    ...
    if 'failure_threshold' not in config and 'spawn_failure_threshold' not in config and ('failure_limit' in config):
