"""``duck-agent verify`` — detect a project's run recipe and smoke-test it.
...
``.duck-agent/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``duck-agent verify`` satisfies
...
def run_verify_command(args) -> int:
    ...
    if recipe is None:
        message = f'No recognizable project found at {root}.\nCreate {manifest_path(root)} to define a recipe manually.'
        if args.json:
    ...
    result = run_verify(root, recipe, phases=phases, phase_timeout=args.timeout, ready_timeout=args.ready_timeout, skip_start=args.skip_start, port_override=args.port)
...
def _record_evidence(root: Path, recipe, result, *, partial: bool) -> None:
    ...
    try:
        ...
        if result.readiness is not None:
            r = result.readiness
            readiness_line = f'[start] {recipe.start} -> ' + (f'ready (HTTP {r.status_code})' if r.ready else f"not ready ({r.error or 'timeout'})")
            tails.append(readiness_line)
        record_verify_run(root=root, session_id=os.environ.get('HERMES_SESSION_ID'), ok=result.ok, command='duck-agent verify', scope='targeted' if partial else 'full', output='\n'.join(tails))
    except Exception:
        pass
...
def _print_human_report(recipe, source, result) -> None:
    ...
    if result.readiness is not None:
        ...
        print(f"  {'start'.ljust(max(5, max((len(p.phase) for p in result.phases), default=5)))}  {('PASS' if r.ready else 'FAIL'):<7}  {r.duration:6.1f}s  {recipe.start}")
        print()
