...
def nous_portal_topup_url(account_info: Optional[NousPortalAccountInfo]=None) -> str:
    ...
    """
    base_billing = nous_portal_billing_url(account_info)
    base = base_billing[:-len('/billing')]
...
def format_nous_portal_entitlement_message(account_info: Optional[NousPortalAccountInfo], *, capability: str='this feature', include_refresh_hint: bool=True, coverage_category: Optional[str]=None) -> Optional[str]:
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                return f"{capability} isn't included with your current Nous Portal access. Add credits or a subscription to enable it at {billing_url}."
        elif account_info.tool_gateway_entitled:
            return None
    if account_info is None:
        return f'Duck Agent could not verify your Nous Portal entitlement, so {capability} is unavailable. Run `duck-agent model` to refresh your login, or check billing at {billing_url}.'
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return f'Nous inference credentials are configured, but Duck Agent cannot verify your Nous Portal paid access for {capability}. Log in with `duck-agent model` to enable Portal-managed features. Billing and credits are managed at {billing_url}.'
        return f'Log in to Nous Portal to use {capability}: run `duck-agent model`. Billing and credits are managed at {billing_url}.'
    if account_info.paid_service_access is None:
        detail = f'Duck Agent could not verify your Nous Portal paid access, so {capability} is unavailable.'
        if account_info.error:
            detail += f' Account lookup failed: {account_info.error}.'
        if include_refresh_hint:
            detail += ' Run `duck-agent model` to refresh your session.'
    ...
    if reason == 'account_missing':
        return f'Duck Agent could not find a Nous Portal account or organisation for this login, so {capability} is unavailable. Run `duck-agent model` to authenticate again; if the problem persists, contact Nous support.'
    if reason == 'no_usable_credits' or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and (not account_info.fresh):
            message += ' If you recently bought credits, run `duck-agent model` to refresh Duck Agent.'
        return message
    return f'Your Nous Portal account does not currently have paid service access, so {capability} is unavailable. Add credits or update billing at {billing_url}.'
...
def _no_paid_access_message(account_info: NousPortalAccountInfo, capability: str, billing_url: str) -> str:
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return f"Your Nous Portal access is paused because you've exceeded the per-member spend cap set by your organisation.{cap_detail}{credit_detail} Ask your organisation admin to raise the member spend cap at {billing_url}, then run `duck-agent model` to refresh."
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return f'Your Nous Portal credits are exhausted{credit_detail}, so {capability} is unavailable. Top up or renew credits at {billing_url}.'
    if has_active_subscription and active_subscription_is_paid is False:
        return f'Your current Nous Portal plan does not include paid service access, so {capability} is unavailable. Upgrade or add credits at {billing_url}.'
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return f'Your Nous Portal account has no active subscription or usable credits{credit_detail}, so {capability} is unavailable. Subscribe or add credits at {billing_url}.'
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return f'Your Nous Portal account has no usable paid credits{credit_detail}, so {capability} is unavailable. Add credits or update billing at {billing_url}.'

def _credit_detail(total_usable: Optional[float], subscription_credits: Optional[float], purchased_credits: Optional[float]) -> str:
    parts: list[str] = []
...
def get_nous_portal_account_info(*, force_fresh: bool=False, min_jwt_ttl_seconds: int=60) -> NousPortalAccountInfo:
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        pool_oauth_info = _info_from_oauth_pool(force_fresh=force_fresh, min_jwt_ttl_seconds=min_jwt_ttl_seconds, portal_base_url=portal_base_url)
        if pool_oauth_info is not None:
            return pool_oauth_info
        pool_info = _info_from_inference_key_pool(portal_base_url)
        if pool_info is not None:
            return pool_info
        return NousPortalAccountInfo(logged_in=False, source='none', fresh=False, portal_base_url=portal_base_url)
    if not force_fresh:
        jwt_info = _info_from_valid_jwt(access_token, state=state, portal_base_url=portal_base_url, min_jwt_ttl_seconds=min_jwt_ttl_seconds)
...
def _fresh_account_info(*, state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> NousPortalAccountInfo:
    ...
    try:
        ...
        if isinstance(payload.get('error'), str):
            return _error_info(error=payload.get('error') or 'account_response_error', logged_in=True, portal_base_url=portal_base_url, raw_account=payload)
        info = _info_from_account_payload(payload, state=refreshed_state, portal_base_url=portal_base_url)
        with _ACCOUNT_INFO_CACHE_LOCK:
...
def _info_from_oauth_pool(*, force_fresh: bool, min_jwt_ttl_seconds: int, portal_base_url: Optional[str]) -> Optional[NousPortalAccountInfo]:
    ...
    state = {'access_token': access_token, 'client_id': getattr(entry, 'client_id', None), 'inference_base_url': getattr(entry, 'inference_base_url', None) or getattr(entry, 'runtime_base_url', None) or getattr(entry, 'base_url', None), 'agent_key': getattr(entry, 'agent_key', None), 'credential_source': f"pool:{getattr(entry, 'label', 'unknown')}"}
    if not force_fresh:
        jwt_info = _info_from_valid_jwt(access_token, state=state, portal_base_url=entry_portal_url, min_jwt_ttl_seconds=min_jwt_ttl_seconds)
        if jwt_info is not None:
            return jwt_info
    try:
    ...
    if not payload:
        return _error_info(error='empty_account_response', logged_in=True, portal_base_url=entry_portal_url)
    if isinstance(payload.get('error'), str):
        return _error_info(error=payload.get('error') or 'account_response_error', logged_in=True, portal_base_url=entry_portal_url, raw_account=payload)
    return _info_from_account_payload(payload, state=state, portal_base_url=entry_portal_url)
...
def _info_from_valid_jwt(token: str, *, state: dict[str, Any], portal_base_url: Optional[str], min_jwt_ttl_seconds: int) -> Optional[NousPortalAccountInfo]:
    try:
...
def _subscription_from_payload(value: Any) -> Optional[NousPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return NousPortalSubscriptionInfo(plan=_coerce_str(value.get('plan')), tier=_coerce_int(value.get('tier')), monthly_charge=_coerce_float(value.get('monthly_charge')), monthly_credits=_coerce_float(value.get('monthly_credits')), current_period_end=_coerce_str(value.get('current_period_end')), credits_remaining=_coerce_float(value.get('credits_remaining')), rollover_credits=_coerce_float(value.get('rollover_credits')))
...
def _paid_service_access_from_payload(value: Any) -> Optional[NousPaidServiceAccessInfo]:
    ...
    return NousPaidServiceAccessInfo(allowed=allowed, paid_access=paid_access, reason=_coerce_str(value.get('reason')), organisation_id=_coerce_str(value.get('organisation_id')), effective_at_ms=_coerce_int(value.get('effective_at_ms')), has_active_subscription=_coerce_bool(value.get('has_active_subscription')), active_subscription_is_paid=_coerce_bool(value.get('active_subscription_is_paid')), subscription_tier=_coerce_int(value.get('subscription_tier')), subscription_monthly_charge=_coerce_float(value.get('subscription_monthly_charge')), subscription_credits_remaining=_coerce_float(value.get('subscription_credits_remaining')), purchased_credits_remaining=_coerce_float(value.get('purchased_credits_remaining')), total_usable_credits=_coerce_float(value.get('total_usable_credits')), member_spend_cap_exceeded=_coerce_bool(value.get('member_spend_cap_exceeded')), member_spend_cap_usd=_coerce_float(value.get('member_spend_cap_usd')), member_spend_usd=_coerce_float(value.get('member_spend_usd')), member_spend_cap_remaining_usd=_coerce_float(value.get('member_spend_cap_remaining_usd')))

def _error_info(*, error: object, logged_in: bool, portal_base_url: Optional[str]=None, raw_account: Optional[dict[str, Any]]=None) -> NousPortalAccountInfo:
    return NousPortalAccountInfo(logged_in=logged_in, source='error', fresh=False, portal_base_url=portal_base_url, raw_account=raw_account, error=str(error))
