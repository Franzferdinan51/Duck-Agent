"""``duck-agent approvals suggest`` — mine approval history into allowlist proposals.

Duck Agent has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.duck-agent/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, duck-agent self-management, …) are eligible.
...
_UNSAFE_CLASS_PATTERNS = ['delete', '\\brm\\b', 'destro', 'wipe', 'format', '\\bdisk\\b', 'block device', 'fork bomb', 'kill (?:all )?process', 'kill all', 'self-termination', '\\bsudo\\b', 'privilege', 'credential', '\\bssh\\b', 'shell.rc', 'system config', 'system file', '\\bsql\\b', '\\bchown\\b', '\\bchmod\\b', 'writable', 'overwrite', 'in-place edit', 'pipe', 'obfuscation', 'remote content', 'remote script', 'heredoc', 'encoded', 'command substitution', 'process substitution', '\\bdd\\b', 'shutdown', 'reboot', 'hardline']
_UNSAFE_CLASS_RE = re.compile('|'.join(_UNSAFE_CLASS_PATTERNS), re.IGNORECASE)
_UNSAFE_ROOT_BINARIES = {'rm', 'rmdir', 'unlink', 'shred', 'dd', 'fdisk', 'parted', 'wipefs', 'sudo', 'doas', 'su', 'chmod', 'chown', 'chgrp', 'kill', 'killall', 'pkill', 'halt', 'shutdown', 'reboot', 'poweroff', 'init', 'del', 'format', 'truncate', 'mkswap'}
_UNSAFE_ROOT_PREFIXES = ('mkfs',)
_BLOCK_MARKERS = ('BLOCKED (hardline)', 'BLOCKED: User denied', 'BLOCKED: Action ', 'BLOCKED: Command flagged as dangerous', 'BLOCKED: approval required', 'BLOCKED: Failed to send approval request', 'The user has NOT consented', 'Asking the user for approval', 'approval_required', 'BLOCKED by user deny rule')
...
def _iter_terminal_calls(con: sqlite3.Connection, since_ts: float) -> Iterator[tuple[str, str]]:
    ...
    cur = con.execute("SELECT tool_calls FROM messages WHERE role='assistant' AND tool_calls IS NOT NULL AND tool_calls LIKE '%terminal%' AND timestamp >= ?", (since_ts,))
    while True:
...
def _blocked_tool_call_ids(con: sqlite3.Connection, since_ts: float) -> set:
    ...
    blocked: set = set()
    cur = con.execute("SELECT tool_call_id, content FROM messages WHERE role='tool' AND tool_call_id IS NOT NULL AND timestamp >= ? AND (content LIKE '%BLOCKED%' OR content LIKE '%approval%')", (since_ts,))
    while True:
        rows = cur.fetchmany(2000)
        if not rows:
            break
...
def normalize_command(command: str) -> str:
    """Fold user/duck-agent home prefixes and collapse whitespace.
...
def build_proposals(records: Iterable[tuple[str, str]], existing: Optional[set]=None, min_count: int=2, limit: int=20) -> list[Proposal]:
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    if not proposals:
        print(f'No allowlist candidates found in approval history ({window}).\nEither nothing dangerous was approved often enough (see --min-count/--days), or the approved classes are excluded for safety.')
        return
    ...
    for i, p in enumerate(proposals, 1):
        ...
        for ex in p.examples:
            print(f'       e.g. {ex}')
    print('\nNothing has been changed. Apply selected entries with:\n  duck-agent approvals suggest --apply 1,3\nEntries are merged into command_allowlist in ~/.duck-agent/config.yaml.')

def suggest_command(args) -> int:
    """Entry point for ``duck-agent approvals suggest``."""
    ...
    proposals = build_proposals(records, existing=existing, min_count=getattr(args, 'min_count', 2), limit=getattr(args, 'limit', 20))
    ...
    if apply_spec:
        ...
        if getattr(args, 'json', False):
            ...
            for pattern in applied:
                print(f'  + {pattern}')
            print(f'\ncommand_allowlist now has {len(merged)} entries (~/.duck-agent/config.yaml).')
        return 0
    if getattr(args, 'json', False):
        payload = {'db': str(db_path), 'days': days, 'proposals': [{'n': i, 'pattern': p.pattern, 'kind': p.kind, 'count': p.count, 'classes': sorted(p.classes), 'examples': p.examples} for i, p in enumerate(proposals, 1)]}
...
def approvals_command(args) -> int:
    """Dispatch ``duck-agent approvals <subcommand>``."""
