"""Shared curses-based UI components for Duck Agent CLI.

Used by `duck-agent tools` and `duck-agent skills` for interactive checklists.
...
def _run_curses_menu(*, initial_cursor, item_count, draw_header, draw_row, on_action, reserve_bottom=1, draw_footer=None, extra_color_pairs=False, fallback, cancel_value, searchable=False, search_labels=None):
    ...
    try:
        ...
        def _draw(stdscr):
            ...
            scroll_offset = 0
            search = _SearchState()
            labels: List[str] = search_labels if use_search and search_labels is not None else []
            while True:
                stdscr.clear()
                max_y, max_x = stdscr.getmaxyx()
                filtered = _filter_indices(labels, search.query) if use_search else list(range(item_count))
                ...
                if use_search:
                    ...
                    if search.active:
                        ...
                        if changed:
                            scroll_offset = 0
                            cursor, cursor_pos = _reconcile_cursor(_filter_indices(search_labels, search.query), cursor)
                        if confirm:
                            if filtered:
...
def curses_checklist(title: str, items: List[str], selected: Set[int], *, cancel_returns: Set[int] | None=None, status_fn: Optional[Callable[[Set[int]], str]]=None) -> Set[int]:
    ...
    def _draw_header(stdscr, max_y, max_x):
        ...
        try:
            ...
            stdscr.addnstr(1, 0, '  ↑↓ navigate  SPACE toggle  ENTER confirm  ESC cancel', max_x - 1, curses.A_DIM)
        except curses.error:
            pass
        return 3
...
def curses_radiolist(title: str, items: List[RadioItem], selected: int=0, *, cancel_returns: int | None=None, description: str | None=None, searchable: bool=False, search_labels: List[str] | None=None) -> int:
    ...
    def _on_action(action, cursor):
        if action in (NAV_SELECT, NAV_TOGGLE):
            return cursor
        return cancel_returns
    return _run_curses_menu(initial_cursor=selected, item_count=len(items), draw_header=_draw_header, draw_row=_draw_row, on_action=_on_action, reserve_bottom=1, extra_color_pairs=True, fallback=lambda: _radio_numbered_fallback(title, items, selected, cancel_returns), cancel_value=cancel_returns, searchable=searchable, search_labels=list(search_labels) if searchable and search_labels is not None else plain_labels if searchable else None)
...
def _radio_numbered_fallback(title: str, items: List[RadioItem], selected: int, cancel_returns: int) -> int:
...
def curses_single_select(title: str, items: List[str], default_index: int=0, *, cancel_label: str='Cancel', searchable: bool=False) -> int | None:
    ...
    def _on_action(action, cursor):
        ...
        if action == NAV_CANCEL:
            return None
        return _KEEP
    return _run_curses_menu(initial_cursor=min(default_index, len(all_items) - 1), item_count=len(all_items), draw_header=_draw_header, draw_row=_draw_row, on_action=_on_action, reserve_bottom=1, fallback=lambda: _numbered_single_fallback(title, all_items, cancel_idx), cancel_value=None, searchable=searchable, search_labels=list(all_items) if searchable else None)
...
def _numbered_fallback(title: str, items: List[str], selected: Set[int], cancel_returns: Set[int], status_fn: Optional[Callable[[Set[int]], str]]=None) -> Set[int]:
