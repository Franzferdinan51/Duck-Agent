...
def query_session_listing(session_db: Any, *, source: str | None, session_key: str | None=None, current_session_id: str | None=None, include_all_sources: bool=False, include_unnamed: bool=False, search_query: str | None=None, limit: int=10, exclude_sources: list[str] | None=None) -> list[dict[str, Any]]:
    ...
    rows = session_db.list_sessions_rich(source=query_source, session_key=session_key, exclude_sources=exclude_sources, limit=fetch_limit, search_query=search or None, order_by_last_active=bool(search))
...
def format_gateway_session_listing(rows: list[dict[str, Any]], *, include_source: bool=False, title: str='Sessions') -> str:
    ...
    if not rows:
        return 'No sessions found.\nUse `/title My Session` to name this chat, or `/sessions full` to include unnamed sessions.'
    lines = [f'📋 **{title}**', '']
