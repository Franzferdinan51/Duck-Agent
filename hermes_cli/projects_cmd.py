"""``duck-agent project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$DUCK_AGENT_HOME/projects.db`` store
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_bind.add_argument('board', nargs='?', default='', help='Board slug (omit to unbind)')
    parser.set_defaults(_project_parser=parser)
    return parser

def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``duck-agent project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: duck-agent project <action> [options]\nRun 'duck-agent project --help' for the full list.", file=sys.stderr)
        return 0
    handlers = {'create': _cmd_create, 'list': _cmd_list, 'ls': _cmd_list, 'show': _cmd_show, 'add-folder': _cmd_add_folder, 'remove-folder': _cmd_remove_folder, 'rename': _cmd_rename, 'set-primary': _cmd_set_primary, 'use': _cmd_use, 'archive': _cmd_archive, 'restore': _cmd_restore, 'bind-board': _cmd_bind_board}
...
def _cmd_create(args: argparse.Namespace) -> int:
    try:
        with pdb.connect_closing() as conn:
            pid = pdb.create_project(conn, name=args.name, slug=args.slug, folders=args.folders, primary_path=args.primary, description=args.description, icon=args.icon, color=args.color, board_slug=args.board)
            if args.use:
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print('No projects yet. Create one with `duck-agent project create <name>`.')
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(f"project: '{args.path}' is not a folder of {proj.slug}; add it first with `duck-agent project add-folder`.", file=sys.stderr)
        return 1
