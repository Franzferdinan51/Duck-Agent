"""PTY bridge for `duck-agent dashboard` chat tab.
...
* **Zero Node dependency on the server side.**  We use :mod:`ptyprocess`,
  ...
  to the same ``duck-agent --tui`` binary it would launch from the CLI, so
...
class PtyBridge:
    ...
    @classmethod
    def spawn(cls, argv: Sequence[str], *, cwd: Optional[str]=None, env: Optional[dict]=None, cols: int=80, rows: int=24) -> 'PtyBridge':
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith('win'):
                raise PtyUnavailableError('Pseudo-terminals are unavailable on this platform. Duck Agent supports Windows only via WSL.')
            if ptyprocess is None:
                raise PtyUnavailableError("The `ptyprocess` package is missing. Install with: pip install ptyprocess (or pip install -e '.[pty]').")
