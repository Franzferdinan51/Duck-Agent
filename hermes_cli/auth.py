"""
Multi-provider authentication system for Duck Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.duck-agent/auth.json with cross-process file locking.
...
import httpx
from hermes_cli.config import get_hermes_home, get_config_path, read_raw_config, require_readable_config_before_write
...
DEFAULT_NOUS_CLIENT_ID = 'duck-agent-cli'
...
CODEX_OAUTH_USER_AGENT = f'duck-agent-cli/{_HERMES_CLI_VERSION}'
...
SPOTIFY_DOCS_URL = 'https://duck-agent.nousresearch.com/docs/user-guide/features/spotify'
SPOTIFY_DASHBOARD_URL = 'https://developer.spotify.com/dashboard'
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
OAUTH_OVER_SSH_DOCS_URL = 'https://duck-agent.nousresearch.com/docs/guides/oauth-over-ssh'
DEFAULT_SPOTIFY_SCOPE = ' '.join(('user-modify-playback-state', 'user-read-playback-state', 'user-read-currently-playing', 'user-read-recently-played', 'playlist-read-private', 'playlist-read-collaborative', 'playlist-modify-public', 'playlist-modify-private', 'user-library-read', 'user-library-modify'))
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Duck Agent' Responses transport.
    """
...
try:
    ...
    for _pp in _list_providers_for_registry():
        ...
        PROVIDER_REGISTRY[_pp.name] = ProviderConfig(id=_pp.name, name=_pp.display_name or _pp.name, auth_type='api_key', inference_base_url=_pp.base_url, api_key_env_vars=_api_key_vars or _pp.env_vars, base_url_env_var=_base_url_var or '')
...
def get_anthropic_key() -> str:
    ...
    ``~/.duck-agent/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_kimi_base_url(api_key: str, default_url: str, env_override: str) -> str:
    ...
    return default_url
_PLACEHOLDER_SECRET_VALUES = {'*', '**', '***', 'changeme', 'your_api_key', 'your_api_key_here', 'your-api-key', 'placeholder', 'example', 'dummy', 'null', 'none'}
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    try:
        ...
        pass
    return ('', '')
ZAI_ENDPOINTS = [('global', 'https://api.z.ai/api/paas/v4', ['glm-5'], 'Global'), ('cn', 'https://open.bigmodel.cn/api/paas/v4', ['glm-5'], 'China'), ('coding-global', 'https://api.z.ai/api/coding/paas/v4', ['glm-5.2', 'glm-5.1', 'glm-5v-turbo', 'glm-4.7'], 'Global (Coding Plan)'), ('coding-cn', 'https://open.bigmodel.cn/api/coding/paas/v4', ['glm-5.2', 'glm-5.1', 'glm-5v-turbo', 'glm-4.7'], 'China (Coding Plan)')]
...
def _probe_single_zai_endpoint(api_key: str, endpoint: tuple, timeout: float) -> Optional[Dict[str, str]]:
    ...
    for model in probe_models:
        try:
            resp = httpx.post(f'{base_url}/chat/completions', headers={'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}, json={'model': model, 'stream': False, 'max_tokens': 1, 'messages': [{'role': 'user', 'content': 'ping'}]}, timeout=timeout)
...
def _resolve_zai_base_url(api_key: str, default_url: str, env_override: str) -> str:
    ...
    if detected and detected.get('base_url'):
        ...
        detected_endpoint = {'base_url': detected['base_url'], 'endpoint_id': detected.get('id', ''), 'model': detected.get('model', ''), 'label': detected.get('label', ''), 'key_hash': key_hash}
        try:
...
class AuthError(RuntimeError):
    ...
    def __init__(self, message: str, *, provider: str='', code: Optional[str]=None, relogin_required: bool=False) -> None:
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``duck-agent auth``.
    """
    return isinstance(error, AuthError) and (not error.relogin_required) and (error.code == CODEX_RATE_LIMITED_CODE)
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f'{error} Run `duck-agent model` to re-authenticate.'
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        message = format_nous_portal_entitlement_message(account_info, capability='Nous model access')
        if message:
            return message
    except Exception:
        pass
...
def _auth_file_path() -> Path:
    ...
    if os.environ.get('PYTEST_CURRENT_TEST'):
        real_home_auth = (Path.home() / '.duck-agent' / 'auth.json').resolve(strict=False)
        try:
            resolved = path.resolve(strict=False)
        except Exception:
            resolved = path
        if resolved == real_home_auth:
            raise RuntimeError(f'Refusing to touch real user auth store during test run: {path}. Set DUCK_AGENT_HOME to a tmp_path in your test fixture, or run via scripts/run_tests.sh for hermetic CI-parity env.')
    return path
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom DUCK_AGENT_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.duck-agent/auth.json`` even when DUCK_AGENT_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped DUCK_AGENT_HOME can
    ...
    if os.environ.get('PYTEST_CURRENT_TEST'):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / '.duck-agent' / 'auth.json'
            try:
...
@contextmanager
def _file_lock(lock_path: Path, holder: threading.local, timeout_seconds: float, timeout_message: str):
...
@contextmanager
def _auth_store_lock(timeout_seconds: float=AUTH_LOCK_TIMEOUT_SECONDS, *, target_path: Optional[Path]=None):
    ...
    with _file_lock(lock_path, _auth_lock_holder_for(auth_path), timeout_seconds, 'Timed out waiting for auth store lock'):
        yield
...
def _load_auth_store(auth_file: Optional[Path]=None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning('auth: could not read %s, leaving the store on disk untouched rather than degrading to an empty one', auth_file, exc_info=True)
        raise
    except Exception as exc:
        ...
        try:
            ...
            logger.debug('auth: could not preserve a copy of the corrupt store at %s', corrupt_path, exc_info=True)
        if preserved:
            logger.warning('auth: failed to parse %s (%s), starting with empty store. Corrupt file preserved at %s', auth_file, exc, corrupt_path)
        else:
            logger.warning('auth: failed to parse %s (%s), starting with empty store. A copy could NOT be preserved at %s', auth_file, exc, corrupt_path)
        return {'version': AUTH_STORE_VERSION, 'providers': {}}
...
def _load_provider_state_with_source(auth_store: Dict[str, Any], provider_id: str) -> tuple[Optional[Dict[str, Any]], Optional[Path]]:
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``duck-agent auth login <provider>`` inside
...
def _save_provider_state_to_source(auth_store: Dict[str, Any], provider_id: str, state: Dict[str, Any], source_path: Optional[Path]) -> None:
...
def _store_provider_state(auth_store: Dict[str, Any], provider_id: str, state: Dict[str, Any], *, set_active: bool=True) -> None:
    providers = auth_store.setdefault('providers', {})
    if not isinstance(providers, dict):
        auth_store['providers'] = {}
        providers = auth_store['providers']
...
def _persist_provider_state_to_store(provider_id: str, state: Dict[str, Any], target_path: Path, *, set_active: bool=False) -> Path:
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``duck-agent auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str]=None) -> Dict[str, Any]:
    ...
    ``duck-agent auth add <provider>`` inside the profile, profile entries
...
_POOL_STATUS_FIELDS = ('last_status', 'last_status_at', 'last_error_code', 'last_error_reason', 'last_error_message', 'last_error_reset_at')

def _merge_disk_cooldown_state(entry: Dict[str, Any], disk_entry: Optional[Dict[str, Any]], provider_id: str) -> Dict[str, Any]:
    ...
    if not isinstance(disk_entry, dict):
        return entry
    try:
        from agent.credential_pool import PooledCredential, STATUS_DEAD, STATUS_EXHAUSTED, _exhausted_until, _parse_absolute_timestamp
...
def write_credential_pool(provider_id: str, entries: List[Dict[str, Any]], *, removed_ids: Optional[Iterable[str]]=None) -> Path:
    ...
    with _auth_store_lock():
        ...
        sanitized_entries = [sanitize_borrowed_credential_payload(entry, provider_id) if isinstance(entry, dict) else entry for entry in entries]
        existing = pool.get(provider_id)
        existing_list = existing if isinstance(existing, list) else []
        existing_by_id = {entry.get('id'): entry for entry in existing_list if isinstance(entry, dict) and entry.get('id')}
        new_ids = {entry.get('id') for entry in sanitized_entries if isinstance(entry, dict) and entry.get('id')}
        merged: List[Dict[str, Any]] = [_merge_disk_cooldown_state(entry, existing_by_id.get(entry.get('id')), provider_id) if isinstance(entry, dict) else entry for entry in sanitized_entries]
        ...
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith('env:'):
                ...
                if env_var and has_usable_secret(os.getenv(env_var, '')):
                    return True
                continue
            if source in {'device_code', 'loopback_pkce', 'hermes_pkce', 'manual'} or source.startswith('manual:'):
                return True
    except Exception:
        pass
    return False
...
def clear_provider_auth(provider_id: Optional[str]=None) -> bool:
    """
    Clear auth state for a provider. Used by `duck-agent logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        if not issues:
            return ''
        lines = ["Config issue detected — run 'duck-agent doctor' for full diagnostics:"]
        for ci in issues:
...
def resolve_provider(requested: Optional[str]=None, *, explicit_api_key: Optional[str]=None, explicit_base_url: Optional[str]=None) -> str:
    ...
    if normalized != 'auto':
        ...
        if _config_hint:
            ...
            msg += " Check 'duck-agent model' for available providers, or run 'duck-agent doctor' to diagnose config issues."
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, '')):
                if _oauth_active and _oauth_active != pid:
                    logger.warning('Provider resolved to %r via %s, preempting your logged-in OAuth provider %r. If you meant to use the OAuth login, unset %s or set `model.provider` explicitly.', pid, env_var, _oauth_active, env_var)
                return pid
    if _oauth_active:
        if isinstance(_model_cfg, dict) and _model_cfg and (not _model_cfg.get('provider')):
            logger.warning('Provider resolved to logged-in OAuth provider %r because config.yaml `model` has no `provider` key. If you meant a different provider, set `model.provider` explicitly.', _oauth_active)
    ...
    try:
        ...
        pass
    raise AuthError("No inference provider configured. Run 'duck-agent model' to choose a provider and model, or set an API key (OPENROUTER_API_KEY, OPENAI_API_KEY, etc.) in ~/.duck-agent/.env.", code='no_provider_configured')
...
_NOUS_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({'portal.nousresearch.com', 'localhost', '127.0.0.1'})
...
def _migrate_stale_nous_portal_url(providers: Dict[str, Any]) -> None:
    ...
    if stored:
        ...
        if parsed.hostname in _NOUS_STALE_PORTAL_HOSTS:
            logger.warning('auth: migrating stale nous portal_base_url %s -> %s', stored, DEFAULT_NOUS_PORTAL_URL)
...
def _validate_nous_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != 'https':
        logger.warning('nous: refusing non-https inference URL scheme %r from Portal response', parsed.scheme)
        return None
    if parsed.hostname not in _ALLOWED_NOUS_INFERENCE_HOSTS:
        logger.warning('nous: refusing inference URL host %r from Portal response (not in allowlist); falling back to default', parsed.hostname)
        return None
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    pointing Duck Agent at a non-production Nous Portal (e.g. a hosted agent
    ...
    """
    return _optional_base_url(os.getenv('HERMES_PORTAL_BASE_URL') or os.getenv('NOUS_PORTAL_BASE_URL'))
...
def _nous_invoke_jwt_status(token: Any, *, scope: Any=None, expires_at: Any=None, min_ttl_seconds: int=NOUS_INVOKE_JWT_MIN_TTL_SECONDS) -> Optional[str]:
    """Return None when the token can be used for inference, else a reason."""
    claims = _decode_jwt_claims(token)
    if not claims:
        return 'access_token_not_jwt'
    scopes = _scope_values(scope) | _scope_values(claims.get('scope')) | _scope_values(claims.get('scp'))
...
def _nous_invoke_jwt_is_usable(token: Any, *, scope: Any=None, expires_at: Any=None, min_ttl_seconds: int=NOUS_INVOKE_JWT_MIN_TTL_SECONDS) -> bool:
    return _nous_invoke_jwt_status(token, scope=scope, expires_at=expires_at, min_ttl_seconds=min_ttl_seconds) is None
...
def _assert_nous_inference_jwt_usable(state: Dict[str, Any], *, access_token: Any=None) -> None:
    ...
    reason = _nous_invoke_jwt_status(token, scope=state.get('scope'), expires_at=state.get('expires_at'))
    if reason is None:
        return
    raise AuthError(f'Nous Portal access token is not a usable inference JWT ({reason}). Re-authenticate with: duck-agent auth add nous', provider='nous', code=reason, relogin_required=True)
...
def _log_nous_invoke_jwt_selected(*, access_token: Any, sequence_id: Optional[str]=None) -> None:
    ...
    _oauth_trace('nous_invoke_jwt_selected', sequence_id=sequence_id, access_token_fp=_token_fingerprint(access_token))
...
def _set_nous_agent_key_from_invoke_jwt(state: Dict[str, Any], *, obtained_at: Optional[str]=None) -> None:
    ...
    elif state.get('agent_key') == access_token and isinstance(existing_obtained_at, str) and existing_obtained_at.strip():
    ...
    expires_in = max(0, int(expires_epoch - time.time())) if expires_epoch is not None else _coerce_ttl_seconds(state.get('expires_in'))
    if expires_at:
...
def _select_nous_invoke_jwt(state: Dict[str, Any], *, access_token: Any=None, sequence_id: Optional[str]=None) -> None:
...
def _nous_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {key: value for key, value in state.items() if key not in _NOUS_EFFECTIVE_STATE_IGNORED_KEYS}
...
def _read_qwen_cli_tokens() -> Dict[str, Any]:
    ...
    if not auth_path.exists():
        raise AuthError("Qwen CLI credentials not found. Run 'qwen auth qwen-oauth' first.", provider='qwen-oauth', code='qwen_auth_missing')
    try:
        data = json.loads(auth_path.read_text(encoding='utf-8'))
    except Exception as exc:
        raise AuthError(f'Failed to read Qwen CLI credentials from {auth_path}: {exc}', provider='qwen-oauth', code='qwen_auth_read_failed') from exc
    if not isinstance(data, dict):
        raise AuthError(f'Invalid Qwen CLI credentials in {auth_path}.', provider='qwen-oauth', code='qwen_auth_invalid')
    return data
...
def _refresh_qwen_cli_tokens(tokens: Dict[str, Any], timeout_seconds: float=20.0) -> Dict[str, Any]:
    ...
    if not refresh_token:
        raise AuthError("Qwen OAuth refresh token missing. Re-run 'qwen auth qwen-oauth'.", provider='qwen-oauth', code='qwen_refresh_token_missing')
    try:
        response = httpx.post(QWEN_OAUTH_TOKEN_URL, headers={'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json'}, data={'grant_type': 'refresh_token', 'refresh_token': refresh_token, 'client_id': QWEN_OAUTH_CLIENT_ID}, timeout=timeout_seconds)
    except Exception as exc:
        raise AuthError(f'Qwen OAuth refresh failed: {exc}', provider='qwen-oauth', code='qwen_refresh_failed') from exc
    if response.status_code >= 400:
        body = response.text.strip()
        raise AuthError("Qwen OAuth refresh failed. Re-run 'qwen auth qwen-oauth'." + (f' Response: {body}' if body else ''), provider='qwen-oauth', code='qwen_refresh_failed')
    try:
        payload = response.json()
    except Exception as exc:
        raise AuthError(f'Qwen OAuth refresh returned invalid JSON: {exc}', provider='qwen-oauth', code='qwen_refresh_invalid_json') from exc
    if not isinstance(payload, dict) or not str(payload.get('access_token', '') or '').strip():
        raise AuthError('Qwen OAuth refresh response missing access_token.', provider='qwen-oauth', code='qwen_refresh_invalid_response')
    expires_in = payload.get('expires_in')
    try:
        expires_in_seconds = int(expires_in)
    except Exception:
        expires_in_seconds = 6 * 60 * 60
    refreshed = {'access_token': str(payload.get('access_token', '') or '').strip(), 'refresh_token': str(payload.get('refresh_token', refresh_token) or refresh_token).strip(), 'token_type': str(payload.get('token_type', tokens.get('token_type', 'Bearer')) or 'Bearer').strip() or 'Bearer', 'resource_url': str(payload.get('resource_url', tokens.get('resource_url', 'portal.qwen.ai')) or 'portal.qwen.ai').strip(), 'expiry_date': int(time.time() * 1000) + max(1, expires_in_seconds) * 1000}
...
def resolve_qwen_runtime_credentials(*, force_refresh: bool=False, refresh_if_expiring: bool=True, refresh_skew_seconds: int=QWEN_ACCESS_TOKEN_REFRESH_SKEW_SECONDS) -> Dict[str, Any]:
    ...
    if should_refresh:
        ...
        access_token = str(tokens.get('access_token', '') or '').strip()
    if not access_token:
        raise AuthError("Qwen OAuth access token missing. Re-run 'qwen auth qwen-oauth'.", provider='qwen-oauth', code='qwen_access_token_missing')
    base_url = os.getenv('HERMES_QWEN_BASE_URL', '').strip().rstrip('/') or DEFAULT_QWEN_BASE_URL
    return {'provider': 'qwen-oauth', 'base_url': base_url, 'api_key': access_token, 'source': 'qwen-cli', 'expires_at_ms': tokens.get('expiry_date'), 'auth_file': str(_qwen_cli_auth_path())}

def get_qwen_auth_status() -> Dict[str, Any]:
    auth_path = _qwen_cli_auth_path()
    try:
        creds = resolve_qwen_runtime_credentials(refresh_if_expiring=True)
        return {'logged_in': True, 'auth_file': str(auth_path), 'source': creds.get('source'), 'api_key': creds.get('api_key'), 'expires_at_ms': creds.get('expires_at_ms')}
...
def _spotify_client_id(explicit: Optional[str]=None, state: Optional[Dict[str, Any]]=None) -> str:
    ...
    candidates = (explicit, get_env_value('HERMES_SPOTIFY_CLIENT_ID'), get_env_value('SPOTIFY_CLIENT_ID'), state.get('client_id') if isinstance(state, dict) else None)
    for candidate in candidates:
        cleaned = str(candidate or '').strip()
        if cleaned:
            return cleaned
    raise AuthError('Spotify client_id is required. Set HERMES_SPOTIFY_CLIENT_ID or pass --client-id.', provider='spotify', code='spotify_client_id_missing')

def _spotify_redirect_uri(explicit: Optional[str]=None, state: Optional[Dict[str, Any]]=None) -> str:
    from hermes_cli.config import get_env_value
    candidates = (explicit, get_env_value('HERMES_SPOTIFY_REDIRECT_URI'), get_env_value('SPOTIFY_REDIRECT_URI'), state.get('redirect_uri') if isinstance(state, dict) else None, DEFAULT_SPOTIFY_REDIRECT_URI)
    for candidate in candidates:
        cleaned = str(candidate or '').strip()
        if cleaned:
            return cleaned
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]]=None) -> str:
    from hermes_cli.config import get_env_value
    candidates = (get_env_value('HERMES_SPOTIFY_API_BASE_URL'), state.get('api_base_url') if isinstance(state, dict) else None, DEFAULT_SPOTIFY_API_BASE_URL)
    for candidate in candidates:
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]]=None) -> str:
    from hermes_cli.config import get_env_value
    candidates = (get_env_value('HERMES_SPOTIFY_ACCOUNTS_BASE_URL'), state.get('accounts_base_url') if isinstance(state, dict) else None, DEFAULT_SPOTIFY_ACCOUNTS_BASE_URL)
    for candidate in candidates:
        cleaned = str(candidate or '').strip().rstrip('/')
        if cleaned:
            return cleaned
...
def _spotify_build_authorize_url(*, client_id: str, redirect_uri: str, scope: str, state: str, code_challenge: str, accounts_base_url: str) -> str:
    query = urlencode({'client_id': client_id, 'response_type': 'code', 'redirect_uri': redirect_uri, 'scope': scope, 'state': state, 'code_challenge_method': 'S256', 'code_challenge': code_challenge})
    return f'{accounts_base_url}/authorize?{query}'

def _spotify_validate_redirect_uri(redirect_uri: str) -> tuple[str, int, str]:
    parsed = urlparse(redirect_uri)
    if parsed.scheme != 'http':
        raise AuthError('Spotify PKCE redirect_uri must use http://localhost or http://127.0.0.1.', provider='spotify', code='spotify_redirect_invalid')
    host = parsed.hostname or ''
    if host not in {'127.0.0.1', 'localhost'}:
        raise AuthError('Spotify PKCE redirect_uri must point to localhost or 127.0.0.1.', provider='spotify', code='spotify_redirect_invalid')
    if not parsed.port:
        raise AuthError('Spotify PKCE redirect_uri must include an explicit localhost port.', provider='spotify', code='spotify_redirect_invalid')
...
def _spotify_wait_for_callback(redirect_uri: str, *, timeout_seconds: float=180.0) -> dict[str, Any]:
    ...
    try:
        ...
        raise AuthError(f'Could not bind Spotify callback server on {host}:{port}: {exc}', provider='spotify', code='spotify_callback_bind_failed') from exc
    ...
    raise AuthError('Spotify authorization timed out waiting for the local callback.', provider='spotify', code='spotify_callback_timeout')

def _spotify_token_payload_to_state(token_payload: Dict[str, Any], *, client_id: str, redirect_uri: str, requested_scope: str, accounts_base_url: str, api_base_url: str, previous_state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
    now = datetime.now(timezone.utc)
    ...
    state.update({'client_id': client_id, 'redirect_uri': redirect_uri, 'accounts_base_url': accounts_base_url, 'api_base_url': api_base_url, 'scope': requested_scope, 'granted_scope': str(token_payload.get('scope') or requested_scope).strip(), 'token_type': str(token_payload.get('token_type', 'Bearer') or 'Bearer').strip() or 'Bearer', 'access_token': str(token_payload.get('access_token', '') or '').strip(), 'refresh_token': str(token_payload.get('refresh_token') or state.get('refresh_token') or '').strip(), 'obtained_at': now.isoformat(), 'expires_at': expires_at.isoformat(), 'expires_in': expires_in, 'auth_type': 'oauth_pkce'})
    return state

def _spotify_exchange_code_for_tokens(*, client_id: str, code: str, redirect_uri: str, code_verifier: str, accounts_base_url: str, timeout_seconds: float=20.0) -> Dict[str, Any]:
    try:
        response = httpx.post(f'{accounts_base_url}/api/token', headers={'Content-Type': 'application/x-www-form-urlencoded'}, data={'client_id': client_id, 'grant_type': 'authorization_code', 'code': code, 'redirect_uri': redirect_uri, 'code_verifier': code_verifier}, timeout=timeout_seconds)
    except Exception as exc:
        raise AuthError(f'Spotify token exchange failed: {exc}', provider='spotify', code='spotify_token_exchange_failed') from exc
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError('Spotify token exchange failed.' + (f' Response: {detail}' if detail else ''), provider='spotify', code='spotify_token_exchange_failed')
    payload = response.json()
    if not isinstance(payload, dict) or not str(payload.get('access_token', '') or '').strip():
        raise AuthError('Spotify token response did not include an access_token.', provider='spotify', code='spotify_token_exchange_invalid')
    return payload

def _refresh_spotify_oauth_state(state: Dict[str, Any], *, timeout_seconds: float=20.0) -> Dict[str, Any]:
    refresh_token = str(state.get('refresh_token', '') or '').strip()
    if not refresh_token:
        raise AuthError('Spotify refresh token missing. Run `duck-agent auth spotify` again.', provider='spotify', code='spotify_refresh_token_missing', relogin_required=True)
    client_id = _spotify_client_id(state=state)
    accounts_base_url = _spotify_accounts_base_url(state)
    try:
        response = httpx.post(f'{accounts_base_url}/api/token', headers={'Content-Type': 'application/x-www-form-urlencoded'}, data={'grant_type': 'refresh_token', 'refresh_token': refresh_token, 'client_id': client_id}, timeout=timeout_seconds)
    except Exception as exc:
        raise AuthError(f'Spotify token refresh failed: {exc}', provider='spotify', code='spotify_refresh_failed') from exc
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError('Spotify token refresh failed. Run `duck-agent auth spotify` again.' + (f' Response: {detail}' if detail else ''), provider='spotify', code='spotify_refresh_failed', relogin_required=True)
    payload = response.json()
    if not isinstance(payload, dict) or not str(payload.get('access_token', '') or '').strip():
        raise AuthError('Spotify refresh response did not include an access_token.', provider='spotify', code='spotify_refresh_invalid', relogin_required=True)
    return _spotify_token_payload_to_state(payload, client_id=client_id, redirect_uri=_spotify_redirect_uri(state=state), requested_scope=str(state.get('scope') or DEFAULT_SPOTIFY_SCOPE), accounts_base_url=accounts_base_url, api_base_url=_spotify_api_base_url(state), previous_state=state)
...
def resolve_spotify_runtime_credentials(*, force_refresh: bool=False, refresh_if_expiring: bool=True, refresh_skew_seconds: int=SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS) -> Dict[str, Any]:
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError('Spotify is not authenticated. Run `duck-agent auth spotify` first.', provider='spotify', code='spotify_auth_missing', relogin_required=True)
        should_refresh = bool(force_refresh)
        if not should_refresh and refresh_if_expiring:
            ...
            try:
                ...
                if exc.relogin_required and state.get('refresh_token'):
                    for _k in ('access_token', 'refresh_token', 'expires_at', 'expires_in', 'obtained_at'):
                        state.pop(_k, None)
                    state['last_auth_error'] = {'provider': 'spotify', 'code': exc.code or 'refresh_failed', 'message': str(exc), 'reason': 'runtime_refresh_failure', 'relogin_required': True, 'at': datetime.now(timezone.utc).isoformat()}
                    try:
                        _store_provider_state(auth_store, 'spotify', state, set_active=False)
                        _save_auth_store(auth_store)
    ...
    if not access_token:
        raise AuthError('Spotify access token missing. Run `duck-agent auth spotify` again.', provider='spotify', code='spotify_access_token_missing', relogin_required=True)
    return {'provider': 'spotify', 'access_token': access_token, 'api_key': access_token, 'token_type': str(state.get('token_type', 'Bearer') or 'Bearer'), 'base_url': _spotify_api_base_url(state), 'scope': str(state.get('granted_scope') or state.get('scope') or '').strip(), 'client_id': _spotify_client_id(state=state), 'redirect_uri': _spotify_redirect_uri(state=state), 'expires_at': state.get('expires_at'), 'refresh_token': str(state.get('refresh_token', '') or '').strip()}
...
def get_spotify_auth_status() -> Dict[str, Any]:
    ...
    refresh_token = str(state.get('refresh_token', '') or '').strip()
    return {'logged_in': bool(refresh_token or not _is_expiring(expires_at, 0)), 'auth_type': state.get('auth_type', 'oauth_pkce'), 'client_id': state.get('client_id'), 'redirect_uri': state.get('redirect_uri'), 'scope': state.get('granted_scope') or state.get('scope'), 'expires_at': expires_at, 'api_base_url': state.get('api_base_url'), 'has_refresh_token': bool(refresh_token)}
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.duck-agent/.env, and return it.
    ...
    print('       App name:     anything (e.g. duck-agent)')
    ...
    print()
    print('Saved HERMES_SPOTIFY_CLIENT_ID to ~/.duck-agent/.env')
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    authorize_url = _spotify_build_authorize_url(client_id=client_id, redirect_uri=redirect_uri, scope=scope, state=state_nonce, code_challenge=code_challenge, accounts_base_url=accounts_base_url)
    ...
    print()
    print('Open this URL to authorize Duck Agent:')
    print(authorize_url)
    print()
    ...
    token_payload = _spotify_exchange_code_for_tokens(client_id=client_id, code=str(callback.get('code') or ''), redirect_uri=redirect_uri, code_verifier=code_verifier, accounts_base_url=accounts_base_url, timeout_seconds=float(getattr(args, 'timeout', None) or 20.0))
    spotify_state = _spotify_token_payload_to_state(token_payload, client_id=client_id, redirect_uri=redirect_uri, requested_scope=scope, accounts_base_url=accounts_base_url, api_base_url=api_base_url)
    with _auth_store_lock():
        auth_store = _load_auth_store()
    ...
    **browser-only remote consoles** (GCP Cloud Shell, GitHub
    ...
    for var in ('CLOUD_SHELL', 'CODESPACES', 'CODESPACE_NAME', 'GITPOD_WORKSPACE_ID', 'REPL_ID', 'STACKBLITZ'):
        if os.getenv(var):
            return True
    return False
_CONSOLE_BROWSER_NAMES: FrozenSet[str] = frozenset({'w3m', 'lynx', 'links', 'links2', 'elinks', 'www-browser', 'browsh'})
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None=None) -> None:
    ...
    print(divider)
    print(f'Duck Agent is waiting for the OAuth callback on {redirect_uri}')
...
def _read_codex_tokens(*, _lock: bool=True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Duck Agent auth store (~/.duck-agent/auth.json).
    ...
    if not state:
        raise AuthError('No Codex credentials stored. Run `duck-agent auth` to authenticate.', provider='openai-codex', code='codex_auth_missing', relogin_required=True)
    tokens = state.get('tokens')
    if not isinstance(tokens, dict):
        raise AuthError('Codex auth state is missing tokens. Run `duck-agent auth` to re-authenticate.', provider='openai-codex', code='codex_auth_invalid_shape', relogin_required=True)
    access_token = tokens.get('access_token')
    refresh_token = tokens.get('refresh_token')
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError('Codex auth is missing access_token. Run `duck-agent auth` to re-authenticate.', provider='openai-codex', code='codex_auth_missing_access_token', relogin_required=True)
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError('Codex auth is missing refresh_token. Run `duck-agent auth` to re-authenticate.', provider='openai-codex', code='codex_auth_missing_refresh_token', relogin_required=True)
    return {'tokens': tokens, 'last_refresh': state.get('last_refresh')}

def _sync_codex_pool_entries(auth_store: Dict[str, Any], tokens: Dict[str, str], last_refresh: Optional[str], previous_singleton_tokens: Optional[Dict[str, str]]=None) -> None:
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``duck-agent setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``duck-agent auth add openai-codex``
      ...
      ``duck-agent auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str=None, label: str=None) -> None:
    """Save Codex OAuth tokens to Duck Agent auth store (~/.duck-agent/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        _sync_codex_pool_entries(auth_store, tokens, last_refresh, previous_singleton_tokens=previous_singleton_tokens)
        _save_auth_store(auth_store)

def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Duck Agent auth, if available."""
    imported = _import_codex_cli_tokens()
    if not (imported and str(imported.get('access_token', '') or '').strip() and str(imported.get('refresh_token', '') or '').strip()):
        return None
...
def refresh_codex_oauth_pure(access_token: str, refresh_token: str, *, timeout_seconds: float=20.0) -> Dict[str, Any]:
    """Refresh Codex OAuth tokens without mutating Duck Agent auth state."""
    del access_token
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError('Codex auth is missing refresh_token. Run `duck-agent auth` to re-authenticate.', provider='openai-codex', code='codex_auth_missing_refresh_token', relogin_required=True)
    timeout = httpx.Timeout(max(5.0, float(timeout_seconds)))
    with httpx.Client(timeout=timeout, headers={'Accept': 'application/json', 'User-Agent': CODEX_OAUTH_USER_AGENT}) as client:
        response = client.post(CODEX_OAUTH_TOKEN_URL, headers={'Content-Type': 'application/x-www-form-urlencoded'}, data={'grant_type': 'refresh_token', 'refresh_token': refresh_token, 'client_id': CODEX_OAUTH_CLIENT_ID})
    if response.status_code == 429:
        ...
        if retry_after is not None:
            ...
            message = 'Codex provider quota exhausted (429). Credentials are still valid; retry after the usage limit resets.'
        raise AuthError(message, provider='openai-codex', code=CODEX_RATE_LIMITED_CODE, relogin_required=False)
    if response.status_code != 200:
        ...
        if code == 'refresh_token_reused':
            message = 'Codex refresh token was already consumed by another client (e.g. Codex CLI or VS Code extension). Run `codex` in your terminal to generate fresh tokens, then run `duck-agent auth` to re-authenticate.'
            relogin_required = True
        if response.status_code in {401, 403} and (not relogin_required):
            relogin_required = True
        raise AuthError(message, provider='openai-codex', code=code, relogin_required=relogin_required)
    try:
        refresh_payload = response.json()
    except Exception as exc:
        raise AuthError('Codex token refresh returned invalid JSON.', provider='openai-codex', code='codex_refresh_invalid_json', relogin_required=True) from exc
    refreshed_access = refresh_payload.get('access_token')
    if not isinstance(refreshed_access, str) or not refreshed_access.strip():
        raise AuthError('Codex token refresh response was missing access_token.', provider='openai-codex', code='codex_refresh_missing_access_token', relogin_required=True)
    updated = {'access_token': refreshed_access.strip(), 'refresh_token': refresh_token.strip(), 'last_refresh': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')}
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    Saves the new tokens to Duck Agent auth store automatically.
    """
    try:
        refreshed = refresh_codex_oauth_pure(str(tokens.get('access_token', '') or ''), str(tokens.get('refresh_token', '') or ''), timeout_seconds=timeout_seconds)
    except AuthError as exc:
        if not getattr(exc, 'relogin_required', False):
            raise
        imported = _recover_codex_tokens_from_cli(f"refresh_token rejected: {getattr(exc, 'code', None) or 'auth_error'}")
        if not imported:
            raise
        return imported
...
def resolve_codex_runtime_credentials(*, force_refresh: bool=False, refresh_if_expiring: bool=True, refresh_skew_seconds: int=CODEX_ACCESS_TOKEN_REFRESH_SKEW_SECONDS) -> Dict[str, Any]:
    """Resolve runtime credentials from Duck Agent'ss own Codex token store.
    ...
    try:
        ...
        read_error = exc
        if getattr(exc, 'relogin_required', False) and getattr(exc, 'code', None) in {'codex_auth_missing_access_token', 'codex_auth_missing_refresh_token', 'codex_auth_invalid_shape'}:
    ...
    if data is None:
        ...
        if pool_token:
            ...
            return {'provider': 'openai-codex', 'base_url': base_url, 'api_key': pool_token, 'source': 'credential_pool', 'last_refresh': None, 'auth_mode': 'chatgpt'}
        pool_rate_limit = _codex_pool_rate_limit_status()
        if pool_rate_limit:
            stale_token = str(pool_rate_limit.get('access_token') or '').strip()
            if stale_token and _probe_codex_quota_restored(stale_token, base_url=pool_rate_limit.get('base_url')):
            ...
            if isinstance(reset_at, (int, float)) and reset_at > time.time():
                ...
                message = f'Codex provider quota exhausted (429); retry after {remaining}s. Credentials are still valid.'
            else:
                message = 'Codex provider quota exhausted (429). Credentials are still valid; retry after the usage limit resets.'
            raise AuthError(message, provider='openai-codex', code=CODEX_RATE_LIMITED_CODE, relogin_required=False)
    ...
    if should_refresh:
        with _auth_store_lock(timeout_seconds=max(float(AUTH_LOCK_TIMEOUT_SECONDS), refresh_timeout_seconds + 5.0)):
            ...
            if should_refresh:
                ...
                access_token = str(tokens.get('access_token', '') or '').strip()
    base_url = os.getenv('HERMES_CODEX_BASE_URL', '').strip().rstrip('/') or DEFAULT_CODEX_BASE_URL
    return {'provider': 'openai-codex', 'base_url': base_url, 'api_key': access_token, 'source': 'duck-agent-auth-store', 'last_refresh': data.get('last_refresh'), 'auth_mode': 'chatgpt'}
...
def _is_codex_rate_limit_shaped(code: Any, reason: Any, message: Any) -> bool:
    ...
    return code == 429 or 'rate_limit' in reason_l or 'usage_limit' in reason_l or ('quota' in reason_l) or ('rate limit' in message_l) or ('usage limit' in message_l) or ('quota' in message_l)
...
def _codex_usage_probe_url(base_url: Optional[str]) -> str:
    ...
    if not normalized:
        normalized = os.getenv('HERMES_CODEX_BASE_URL', '').strip().rstrip('/') or DEFAULT_CODEX_BASE_URL
    if normalized.endswith('/codex'):
        normalized = normalized[:-len('/codex')]
    prefix = normalized + ('/wham' if '/backend-api' in normalized else '/api/codex')
    return prefix + '/usage'

def _probe_codex_quota_restored(access_token: Any, *, base_url: Optional[str]=None, min_interval_seconds: float=CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS) -> Optional[bool]:
    """Ask the Codex usage endpoint whether this account's quota is usable again.

    Duck Agent persists a Codex 429's ``reset_at`` locally and freezes the
    ...
    try:
        ...
        claims = _decode_jwt_claims(token)
        account_id = claims.get('https://api.openai.com/auth', {}).get('chatgpt_account_id') if isinstance(claims.get('https://api.openai.com/auth'), dict) else None
        ...
        with _auth_store_lock():
            ...
            for entry in entries:
                ...
                if access_token and str(entry.get('access_token') or '') != access_token:
                    continue
                if not _is_codex_rate_limit_shaped(entry.get('last_error_code'), entry.get('last_error_reason'), entry.get('last_error_message')):
                    continue
                entry['last_status'] = None
                entry['last_status_at'] = None
                entry['last_error_code'] = None
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        for entry in entries:
            ...
            is_rate_limited = code == 429 or 'rate_limit' in reason or 'usage_limit' in reason or ('quota' in reason) or ('rate limit' in message) or ('usage limit' in message) or ('quota' in message)
            if not is_rate_limited:
                continue
            reset_at = _parse_reset_at(entry.get('last_error_reset_at'))
            if reset_at is not None and reset_at <= now:
                continue
            return {'label': entry.get('label'), 'last_refresh': entry.get('last_refresh'), 'reset_at': reset_at, 'reason': entry.get('last_error_reason'), 'message': entry.get('last_error_message'), 'access_token': token.strip(), 'base_url': entry.get('base_url')}
    except Exception:
...
def _xai_oauth_state_from_store(auth_store: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(entries, list):
        for entry in entries:
            ...
            merged['tokens'] = {'access_token': access_token, 'refresh_token': refresh_token, 'token_type': str(entry.get('token_type') or 'Bearer')}
...
def _xai_oauth_state_has_usable_tokens(state: Optional[Dict[str, Any]]) -> bool:
    tokens = state.get('tokens') if isinstance(state, dict) else None
    return isinstance(tokens, dict) and bool(str(tokens.get('access_token', '') or '').strip()) and bool(str(tokens.get('refresh_token', '') or '').strip())
...
def _read_xai_oauth_tokens(*, _lock: bool=True) -> Dict[str, Any]:
    ...
    if not _xai_oauth_state_has_usable_tokens(state):
        ...
        if _xai_oauth_state_has_usable_tokens(global_state):
            state = global_state
    if not state:
        raise AuthError('No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `duck-agent model`.', provider='xai-oauth', code='xai_auth_missing', relogin_required=True)
    tokens = state.get('tokens')
    if not isinstance(tokens, dict):
        raise AuthError('xAI OAuth state is missing tokens. Re-authenticate with `duck-agent model`.', provider='xai-oauth', code='xai_auth_invalid_shape', relogin_required=True)
    access_token = str(tokens.get('access_token', '') or '').strip()
    refresh_token = str(tokens.get('refresh_token', '') or '').strip()
    if not access_token:
        raise AuthError('xAI OAuth state is missing access_token. Re-authenticate with `duck-agent model`.', provider='xai-oauth', code='xai_auth_missing_access_token', relogin_required=True)
    if not refresh_token:
        raise AuthError('xAI OAuth state is missing refresh_token. Re-authenticate with `duck-agent model`.', provider='xai-oauth', code='xai_auth_missing_refresh_token', relogin_required=True)
    return {'tokens': tokens, 'last_refresh': state.get('last_refresh'), 'discovery': state.get('discovery') or {}, 'redirect_uri': state.get('redirect_uri')}
...
def _save_xai_oauth_tokens(tokens: Dict[str, Any], *, discovery: Optional[Dict[str, Any]]=None, redirect_uri: str='', last_refresh: Optional[str]=None, auth_mode: str='oauth_device_code', set_active: bool=True) -> None:
    ...
    with _auth_store_lock():
        ...
        is_from_root = bool(source_path is not None and global_root is not None and _same_path(source_path, global_root))
        if is_from_root:
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``duck-agent auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.duck-agent/auth.json``. A single MITM during initial
    ...
    parsed = urlparse(url)
    if parsed.scheme != 'https':
        raise AuthError(f'xAI OIDC discovery returned a non-HTTPS {field}: {url!r}.', provider='xai-oauth', code='xai_discovery_invalid')
    host = (parsed.hostname or '').lower()
    if not host:
        raise AuthError(f'xAI OIDC discovery {field} is missing a hostname: {url!r}.', provider='xai-oauth', code='xai_discovery_invalid')
    if host != 'x.ai' and (not host.endswith('.x.ai')):
        raise AuthError(f'xAI OIDC discovery {field} host {host!r} is not on the xAI origin (expected x.ai or a *.x.ai subdomain). Refusing to use a cached endpoint that may have been substituted by a MITM during initial discovery; re-authenticate with `duck-agent model` to re-fetch.', provider='xai-oauth', code='xai_discovery_invalid')
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    try:
        ...
        return fallback
    if parsed.scheme != 'https':
        logger.warning('Refusing non-HTTPS xAI base_url override %r (xai-oauth bearer would be sent in cleartext); falling back to %s.', candidate, fallback)
        return fallback
    host = (parsed.hostname or '').lower()
    if not host:
        ...
        return fallback
    if host != 'x.ai' and (not host.endswith('.x.ai')):
        logger.warning("Refusing xAI base_url override %r — host %r is not on the xAI origin (expected x.ai or a *.x.ai subdomain). The xai-oauth bearer is only valid against xAI's inference API; sending it elsewhere would leak the credential. Falling back to %s.", candidate, host, fallback)
        return fallback
    return candidate
...
def _xai_oauth_discovery(timeout_seconds: float=15.0) -> Dict[str, str]:
    try:
        ...
        raise AuthError(f'xAI OIDC discovery failed: {exc}', provider='xai-oauth', code='xai_discovery_failed') from exc
    if response.status_code != 200:
        raise AuthError(f'xAI OIDC discovery returned status {response.status_code}.', provider='xai-oauth', code='xai_discovery_failed')
    try:
        payload = response.json()
    except Exception as exc:
        raise AuthError(f'xAI OIDC discovery returned invalid JSON: {exc}', provider='xai-oauth', code='xai_discovery_invalid_json') from exc
    if not isinstance(payload, dict):
        raise AuthError('xAI OIDC discovery response was not a JSON object.', provider='xai-oauth', code='xai_discovery_incomplete')
    authorization_endpoint = str(payload.get('authorization_endpoint', '') or '').strip()
    token_endpoint = str(payload.get('token_endpoint', '') or '').strip()
    if not authorization_endpoint or not token_endpoint:
        raise AuthError('xAI OIDC discovery response was missing required endpoints.', provider='xai-oauth', code='xai_discovery_incomplete')
...
def refresh_xai_oauth_pure(access_token: str, refresh_token: str, *, token_endpoint: str='', timeout_seconds: float=20.0) -> Dict[str, Any]:
    del access_token
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError('xAI OAuth is missing refresh_token. Re-authenticate with `duck-agent model`.', provider='xai-oauth', code='xai_auth_missing_refresh_token', relogin_required=True)
    ...
    with httpx.Client(timeout=timeout, headers={'Accept': 'application/json'}) as client:
        response = client.post(endpoint, headers={'Content-Type': 'application/x-www-form-urlencoded'}, data={'grant_type': 'refresh_token', 'client_id': XAI_OAUTH_CLIENT_ID, 'refresh_token': refresh_token})
    if response.status_code != 200:
        detail = response.text.strip()
        if response.status_code == 403:
            raise AuthError('xAI token refresh failed with HTTP 403.' + (f' Response: {detail}' if detail else '') + " This OAuth account is not authorized for xAI API access — xAI may be restricting API/OAuth use to specific SuperGrok tiers despite the in-app subscription being active. Re-logging in won't change that; set ``XAI_API_KEY`` and switch to ``provider: xai`` (API-key path) if available, or upgrade your subscription at https://x.ai/grok.", provider='xai-oauth', code='xai_oauth_tier_denied', relogin_required=False)
        raise AuthError('xAI token refresh failed.' + (f' Response: {detail}' if detail else ''), provider='xai-oauth', code='xai_refresh_failed', relogin_required=response.status_code in {400, 401})
    try:
        payload = response.json()
    except Exception as exc:
        raise AuthError(f'xAI token refresh returned invalid JSON: {exc}', provider='xai-oauth', code='xai_refresh_invalid_json') from exc
    if not isinstance(payload, dict):
        raise AuthError('xAI token refresh response was not a JSON object.', provider='xai-oauth', code='xai_refresh_invalid_response', relogin_required=True)
    refreshed_access = str(payload.get('access_token', '') or '').strip()
    if not refreshed_access:
        raise AuthError('xAI token refresh response was missing access_token.', provider='xai-oauth', code='xai_refresh_missing_access_token', relogin_required=True)
    updated = {'access_token': refreshed_access, 'refresh_token': str(payload.get('refresh_token') or refresh_token).strip(), 'id_token': str(payload.get('id_token') or '').strip(), 'expires_in': payload.get('expires_in'), 'token_type': str(payload.get('token_type') or 'Bearer').strip() or 'Bearer', 'last_refresh': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')}
    return updated

def _refresh_xai_oauth_tokens(tokens: Dict[str, Any], *, token_endpoint: str, redirect_uri: str='', timeout_seconds: float) -> Dict[str, Any]:
    try:
        state = _load_provider_state(_load_auth_store(), 'xai-oauth') or {}
        auth_mode = str(state.get('auth_mode') or 'oauth_device_code')
    except Exception:
        auth_mode = 'oauth_device_code'
    refreshed = refresh_xai_oauth_pure(str(tokens.get('access_token', '') or ''), str(tokens.get('refresh_token', '') or ''), token_endpoint=token_endpoint, timeout_seconds=timeout_seconds)
    updated_tokens = dict(tokens)
    updated_tokens['access_token'] = refreshed['access_token']
    updated_tokens['refresh_token'] = refreshed['refresh_token']
    ...
    _save_xai_oauth_tokens(updated_tokens, discovery={'token_endpoint': token_endpoint}, redirect_uri=redirect_uri, last_refresh=refreshed['last_refresh'], auth_mode=auth_mode, set_active=False)
    return updated_tokens
...
def resolve_xai_oauth_runtime_credentials(*, force_refresh: bool=False, refresh_if_expiring: bool=True, refresh_skew_seconds: Optional[int]=None) -> Dict[str, Any]:
    ...
    effective_skew = int(refresh_skew_seconds) if refresh_skew_seconds is not None else _xai_proactive_refresh_skew_seconds(access_token)
    should_refresh = bool(force_refresh)
    if not should_refresh and refresh_if_expiring:
    ...
    if should_refresh:
        with _auth_store_lock(timeout_seconds=max(float(AUTH_LOCK_TIMEOUT_SECONDS), refresh_timeout_seconds + 5.0)):
            ...
            if should_refresh:
                ...
                try:
                    tokens = _refresh_xai_oauth_tokens(tokens, token_endpoint=token_endpoint, redirect_uri=redirect_uri, timeout_seconds=refresh_timeout_seconds)
                    access_token = str(tokens.get('access_token', '') or '').strip()
                except AuthError as exc:
                    if _is_terminal_xai_oauth_refresh_error(exc):
                        try:
                            ...
                            _q_state['last_auth_error'] = {'provider': 'xai-oauth', 'code': exc.code or 'xai_refresh_failed', 'message': str(exc), 'reason': 'runtime_refresh_failure', 'relogin_required': True, 'at': datetime.now(timezone.utc).isoformat()}
                            _store_provider_state(_q_store, 'xai-oauth', _q_state, set_active=False)
                            _save_auth_store(_q_store)
                        except Exception as _save_exc:
                            logger.debug('xAI OAuth: failed to persist quarantined state: %s', _save_exc)
                    raise
    base_url = _xai_validate_inference_base_url(os.getenv('HERMES_XAI_BASE_URL', '').strip().rstrip('/') or os.getenv('XAI_BASE_URL', '').strip().rstrip('/'), fallback=DEFAULT_XAI_OAUTH_BASE_URL)
    return {'provider': 'xai-oauth', 'base_url': base_url, 'api_key': access_token, 'source': 'duck-agent-auth-store', 'last_refresh': data.get('last_refresh'), 'auth_mode': 'oauth_device_code'}
...
def _resolve_verify(*, insecure: Optional[bool]=None, ca_bundle: Optional[str]=None, auth_state: Optional[Dict[str, Any]]=None) -> bool | ssl.SSLContext:
    tls_state = auth_state.get('tls') if isinstance(auth_state, dict) else {}
    tls_state = tls_state if isinstance(tls_state, dict) else {}
    effective_insecure = is_truthy_value(insecure, default=False) if insecure is not None else is_truthy_value(tls_state.get('insecure', False), default=False)
    effective_ca = ca_bundle or tls_state.get('ca_bundle') or os.getenv('HERMES_CA_BUNDLE') or os.getenv('SSL_CERT_FILE') or os.getenv('REQUESTS_CA_BUNDLE')
    if effective_insecure:
        return False
    if effective_ca:
        ca_path = str(effective_ca)
        if not os.path.isfile(ca_path):
            logger.warning('CA bundle path does not exist: %s — falling back to default certificates', ca_path)
...
def _request_device_code(client: httpx.Client, portal_base_url: str, client_id: str, scope: Optional[str]) -> Dict[str, Any]:
    ...
    response = client.post(f'{portal_base_url}/api/oauth/device/code', data={'client_id': client_id, **({'scope': scope} if scope else {})})
    response.raise_for_status()
    data = response.json()
    required_fields = ['device_code', 'user_code', 'verification_uri', 'verification_uri_complete', 'expires_in', 'interval']
...
def _poll_for_token(client: httpx.Client, portal_base_url: str, client_id: str, device_code: str, expires_in: int, poll_interval: int) -> Dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        response = client.post(f'{portal_base_url}/api/oauth/token', data={'grant_type': 'urn:ietf:params:oauth:grant-type:device_code', 'client_id': client_id, 'device_code': device_code})
        if response.status_code == 200:
            payload = response.json()
...
def _nous_shared_auth_dir() -> Path:
    ...
    ``<duck-agent-root>/shared/``, where ``<duck-agent-root>`` is what
    :func:`hermes_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.duck-agent/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\duck-agent\\shared\\``, and
    Docker / custom ``DUCK_AGENT_HOME`` deployments at
    ``<DUCK_AGENT_HOME>/shared/``. Sits outside any named profile so all
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get('PYTEST_CURRENT_TEST'):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(f'Refusing to touch real user shared Nous auth store during test run: {path}. Set HERMES_SHARED_AUTH_DIR to a tmp_path in your test fixture.')
    return path
...
def _nous_shared_store_lock(timeout_seconds: float=AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        yield
        return
    with _file_lock(lock_path, _nous_shared_lock_holder, timeout_seconds, 'Timed out waiting for shared Nous auth lock'):
        yield
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    if not refresh_changed and (not fresher_access):
        return False
    for key in ('access_token', 'refresh_token', 'token_type', 'scope', 'client_id', 'portal_base_url', 'inference_base_url', 'obtained_at', 'expires_at'):
        ...
        with _nous_shared_store_lock():
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace('nous_shared_store_written', path=str(path), refresh_token_fp=_token_fingerprint(refresh_token))
    except Exception as exc:
...
def _is_terminal_nous_refresh_error(exc: Exception) -> bool:
    ...
    return isinstance(exc, AuthError) and exc.provider == 'nous' and (exc.code in {'invalid_grant', 'invalid_token', 'refresh_token_reused'}) and bool(exc.relogin_required)
...
def _is_terminal_xai_oauth_refresh_error(exc: Exception) -> bool:
    ...
    """
    return isinstance(exc, AuthError) and exc.provider == 'xai-oauth' and (exc.code in {'xai_refresh_failed', 'xai_auth_missing_refresh_token'}) and bool(exc.relogin_required)
...
def _is_terminal_codex_oauth_refresh_error(exc: Exception) -> bool:
    ...
    Both carry ``relogin_required=True``; transient failures (429, 5xx) do not.
    """
    return isinstance(exc, AuthError) and exc.provider == 'openai-codex' and (exc.code in {'codex_refresh_failed', 'codex_auth_missing_refresh_token', 'invalid_grant', 'invalid_token', 'refresh_token_reused'}) and bool(exc.relogin_required)
...
def _quarantine_nous_oauth_state(state: Dict[str, Any], error: AuthError, *, reason: str) -> None:
    ...
    forensic: Dict[str, Any] = {'reason': reason, 'error_code': error.code, 'client_id': state.get('client_id'), 'agent_key_id': state.get('agent_key_id'), 'refresh_token_fp': _token_fingerprint(state.get('refresh_token'))}
    try:
    ...
    logger.warning('Nous OAuth state quarantined (terminal auth death): %s', json.dumps(forensic, sort_keys=True, ensure_ascii=False))
    for key in ('access_token', 'refresh_token', 'expires_at', 'expires_in', 'obtained_at', 'agent_key', 'agent_key_id', 'agent_key_expires_at', 'agent_key_expires_in', 'agent_key_reused', 'agent_key_obtained_at'):
        state.pop(key, None)
    state['last_auth_error'] = {'provider': 'nous', 'code': error.code, 'message': str(error), 'reason': reason, 'relogin_required': True, 'at': datetime.now(timezone.utc).isoformat()}
    _clear_shared_nous_state(reason)
    invalidate_nous_auth_status_cache()

def _quarantine_nous_pool_entries(auth_store: Dict[str, Any], error: AuthError, *, reason: str) -> bool:
...
def _try_import_shared_nous_state(*, timeout_seconds: float=15.0) -> Optional[Dict[str, Any]]:
    ...
    try:
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not shared:
                return None
            state: Dict[str, Any] = {'access_token': shared.get('access_token'), 'refresh_token': shared.get('refresh_token'), 'client_id': shared.get('client_id') or DEFAULT_NOUS_CLIENT_ID, 'portal_base_url': shared.get('portal_base_url') or DEFAULT_NOUS_PORTAL_URL, 'inference_base_url': shared.get('inference_base_url') or DEFAULT_NOUS_INFERENCE_URL, 'token_type': shared.get('token_type') or 'Bearer', 'scope': shared.get('scope') or DEFAULT_NOUS_SCOPE, 'obtained_at': shared.get('obtained_at'), 'expires_at': shared.get('expires_at'), 'agent_key': None, 'agent_key_expires_at': None, 'tls': {'insecure': False, 'ca_bundle': None}}

            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_nous_state(updated_state)
            refreshed = refresh_nous_oauth_from_state(state, timeout_seconds=timeout_seconds, force_refresh=True, on_state_update=_persist_shared_refresh)
            _write_shared_nous_state(refreshed)
    except AuthError as exc:
        _oauth_trace('nous_shared_import_failed', error_type=type(exc).__name__, error_code=getattr(exc, 'code', None))
...
def _refresh_access_token(*, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str) -> Dict[str, Any]:
    response = client.post(f'{portal_base_url}/api/oauth/token', headers={'x-nous-refresh-token': refresh_token}, data={'grant_type': 'refresh_token', 'client_id': client_id})
    if response.status_code == 200:
        payload = response.json()
        if 'access_token' not in payload:
            raise AuthError('Refresh response missing access_token', provider='nous', code='invalid_token', relogin_required=True)
        return payload
    try:
        error_payload = response.json()
    except Exception as exc:
    ...
    if code == 'refresh_token_reused' or 'reuse' in lowered or 'reuse detected' in lowered:
        description = "Nous Portal detected refresh-token reuse and revoked this session.\nThis usually means an external process (monitoring script, custom self-heal hook, or another Duck Agent install sharing ~/.duck-agent/auth.json) called POST /api/oauth/token with Duck Agent'ss refresh token without persisting the rotated token back.\nNous refresh tokens are single-use — only Duck Agent may call the refresh endpoint. For health checks, use `duck-agent auth status` instead.\nRe-authenticate with: duck-agent auth add nous"
        relogin = True
...
def fetch_nous_models(*, inference_base_url: str, api_key: str, timeout_seconds: float=15.0, verify: bool | str=True) -> List[str]:
    ...
    with httpx.Client(timeout=timeout, headers={'Accept': 'application/json'}, verify=verify) as client:
        response = client.get(f"{inference_base_url.rstrip('/')}/models", headers={'Authorization': f'Bearer {api_key}'})
    if response.status_code != 200:
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _nous_invoke_jwt_is_usable(key, scope=state.get('scope'), expires_at=state.get('agent_key_expires_at'), min_ttl_seconds=max(0, int(min_ttl_seconds)))
...
def resolve_nous_access_token(*, timeout_seconds: float=15.0, insecure: Optional[bool]=None, ca_bundle: Optional[str]=None, refresh_skew_seconds: int=ACCESS_TOKEN_REFRESH_SKEW_SECONDS) -> str:
    ...
    with _provider_state_transaction('nous') as (auth_store, state, state_source_path):
        if not state:
            raise AuthError('Duck Agent is not logged into Nous Portal.', provider='nous', relogin_required=True)
        env_portal_override = _nous_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip('/')
        else:
            portal_base_url = (_optional_base_url(state.get('portal_base_url')) or DEFAULT_NOUS_PORTAL_URL).rstrip('/')
            ...
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _NOUS_PORTAL_ALLOWED_HOSTS:
                logger.warning('auth: ignoring invalid portal_base_url %r (host %r not in allowlist), using default', portal_base_url, parsed_portal_url.hostname)
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(refresh_token, str) or not refresh_token:
                raise AuthError('Session expired and no refresh token is available.', provider='nous', relogin_required=True)
            timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
            with httpx.Client(timeout=timeout, headers={'Accept': 'application/json'}, verify=verify) as client:
                try:
                    refreshed = _refresh_access_token(client=client, portal_base_url=portal_base_url, client_id=client_id, refresh_token=refresh_token)
                except AuthError as exc:
                    if _is_terminal_nous_refresh_error(exc):
                        _quarantine_nous_oauth_state(state, exc, reason='managed_access_token_refresh_failure')
                        _quarantine_nous_pool_entries(auth_store, exc, reason='managed_access_token_refresh_failure')
                        _save_provider_state_to_source(auth_store, 'nous', state, state_source_path)
                    raise
            now = datetime.now(timezone.utc)
            ...
            state['tls'] = {'insecure': verify is False, 'ca_bundle': verify if isinstance(verify, str) else None}
            _save_provider_state_to_source(auth_store, 'nous', state, state_source_path)
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                if current_invoke_jwt_status is not None:
                    raise AuthError(f'Nous Portal access token is not a usable inference JWT ({current_invoke_jwt_status}) and no refresh token is available. Re-authenticate with: duck-agent auth add nous', provider='nous', code=current_invoke_jwt_status, relogin_required=True)
                raise AuthError('No refresh token is available for Nous Portal.', provider='nous', relogin_required=True)
            refreshed = _refresh_access_token(client=client, portal_base_url=state['portal_base_url'], client_id=state['client_id'], refresh_token=refresh_token_value)
            now = datetime.now(timezone.utc)
            access_ttl = _coerce_ttl_seconds(refreshed.get('expires_in'))
            state['access_token'] = refreshed['access_token']
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str]=None):
    ...
    Historically ``duck-agent auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``duck-agent auth add nous --label <name>``).  It gets embedded in the
...
def resolve_nous_runtime_credentials(*, timeout_seconds: float=15.0, insecure: Optional[bool]=None, ca_bundle: Optional[str]=None, force_refresh: bool=False) -> Dict[str, Any]:
    ...
    with _provider_state_transaction('nous') as (auth_store, state, state_source_path):
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (_optional_base_url(state.get('portal_base_url')) or os.getenv('HERMES_PORTAL_BASE_URL') or os.getenv('NOUS_PORTAL_BASE_URL') or DEFAULT_NOUS_PORTAL_URL).rstrip('/')
            env_portal_override = _nous_portal_env_override()
            if env_portal_override:
                ...
                loopback_http = parsed_portal_url.scheme == 'http' and portal_host in {'localhost', '127.0.0.1'}
                ...
                if not portal_host or portal_host not in _NOUS_PORTAL_ALLOWED_HOSTS or (not trusted_scheme):
                    logger.warning('auth: ignoring invalid portal_base_url %r (host %r or scheme not allowed), using default', portal_url, portal_host)
                    portal_url = DEFAULT_NOUS_PORTAL_URL
            stored_inference_url = _validate_nous_inference_url_from_network(_optional_base_url(state.get('inference_base_url'))) or DEFAULT_NOUS_INFERENCE_URL
            ...
            return (portal_url, stored_inference_url, effective_inference_url, effective_client_id)
        portal_base_url, stored_inference_base_url, inference_base_url, client_id = _resolve_effective_routing_metadata()
        ...
        def _persist_state(reason: str) -> None:
            ...
            if _nous_effective_provider_state(state) == _nous_effective_provider_state(persisted_state):
                _oauth_trace('nous_state_persist_skipped', sequence_id=sequence_id, reason=reason)
                return
            try:
                _save_provider_state_to_source(auth_store, 'nous', state, state_source_path)
            except Exception as exc:
                _oauth_trace('nous_state_persist_failed', sequence_id=sequence_id, reason=reason, error_type=type(exc).__name__)
                raise
            _oauth_trace('nous_state_persisted', sequence_id=sequence_id, reason=reason, refresh_token_fp=_token_fingerprint(state.get('refresh_token')), access_token_fp=_token_fingerprint(state.get('access_token')))
            persisted_state = dict(state)
            state_persisted = True
            _write_shared_nous_state(state)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace('nous_runtime_credentials_start', sequence_id=sequence_id, refresh_token_fp=_token_fingerprint(state.get('refresh_token')))
        with httpx.Client(timeout=timeout, headers={'Accept': 'application/json'}, verify=verify) as client:
            access_token = state.get('access_token')
            refresh_token = state.get('refresh_token')
            if not isinstance(access_token, str) or not access_token:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError('No access token found for Nous Portal login.', provider='nous', relogin_required=True)
            invoke_jwt_status = _nous_invoke_jwt_status(access_token, scope=state.get('scope'), expires_at=state.get('expires_at'))
                ...
                with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        if not isinstance(refresh_token, str) or not refresh_token:
                            ...
                            raise AuthError(f'Nous Portal access token is not a usable inference JWT ({reason}) and no refresh token is available. Re-authenticate with: duck-agent auth add nous', provider='nous', code=reason, relogin_required=True)
                        refresh_reason = 'force_refresh' if force_refresh else invoke_jwt_status or 'access_unusable'
                        _oauth_trace('refresh_start', sequence_id=sequence_id, reason=refresh_reason, refresh_token_fp=_token_fingerprint(refresh_token))
                        try:
                            refreshed = _refresh_access_token(client=client, portal_base_url=portal_base_url, client_id=client_id, refresh_token=refresh_token)
                        except AuthError as exc:
                            if _is_terminal_nous_refresh_error(exc):
                                _quarantine_nous_oauth_state(state, exc, reason='runtime_access_refresh_failure')
                                _quarantine_nous_pool_entries(auth_store, exc, reason='runtime_access_refresh_failure')
                        ...
                        _oauth_trace('refresh_success', sequence_id=sequence_id, reason=refresh_reason, previous_refresh_token_fp=_token_fingerprint(previous_refresh_token), new_refresh_token_fp=_token_fingerprint(refresh_token))
                        _persist_state('post_refresh_access_token')
            _assert_nous_inference_jwt_usable(state, access_token=access_token)
            _select_nous_invoke_jwt(state, access_token=access_token, sequence_id=sequence_id)
            state['portal_base_url'] = portal_base_url
            state['inference_base_url'] = stored_inference_base_url
            state['client_id'] = client_id
            state['tls'] = {'insecure': verify is False, 'ca_bundle': verify if isinstance(verify, str) else None}
    ...
    expires_epoch = _parse_iso_timestamp(expires_at)
    expires_in = max(0, int(expires_epoch - time.time())) if expires_epoch is not None else _coerce_ttl_seconds(state.get('agent_key_expires_in'))
    return {'provider': 'nous', 'base_url': inference_base_url, 'api_key': api_key, 'key_id': state.get('agent_key_id'), 'expires_at': expires_at, 'expires_in': expires_in, 'source': NOUS_AUTH_PATH_INVOKE_JWT, 'auth_path': NOUS_AUTH_PATH_INVOKE_JWT, 'state_path': str(state_source_path or _auth_file_path())}

def _empty_nous_auth_status() -> Dict[str, Any]:
    return {'logged_in': False, 'portal_base_url': None, 'inference_base_url': None, 'access_expires_at': None, 'agent_key_expires_at': None, 'has_refresh_token': False, 'inference_credential_present': False, 'credential_source': None}
...
def get_nous_auth_status() -> Dict[str, Any]:
    ...
    if cached is not None:
        ...
        if cached_auth_file_key == auth_file_key and cached_mtime == mtime and (now - cached_at < _NOUS_AUTH_STATUS_CACHE_TTL):
...
def get_nous_session_validity() -> str:
    ...
    if _nous_invoke_jwt_status(state.get('access_token'), scope=state.get('scope'), expires_at=state.get('expires_at')) is None:
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `duck-agent auth` stores credentials),
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                if api_key and (not _codex_access_token_is_expiring(api_key, 0)):
                    return {'logged_in': True, 'auth_store': str(_auth_file_path()), 'last_refresh': getattr(entry, 'last_refresh', None), 'auth_mode': 'chatgpt', 'source': f"pool:{getattr(entry, 'label', 'unknown')}", 'api_key': api_key}
            rate_limit = _codex_pool_rate_limit_status()
            if rate_limit:
                return {'logged_in': True, 'auth_store': str(_auth_file_path()), 'last_refresh': rate_limit.get('last_refresh'), 'auth_mode': 'chatgpt', 'source': f"pool:{rate_limit.get('label') or 'unknown'}", 'rate_limited': True, 'error_code': CODEX_RATE_LIMITED_CODE, 'error': rate_limit.get('message') or 'Codex provider quota exhausted; retry after the usage limit resets.', 'reset_at': rate_limit.get('reset_at')}
    except Exception:
        pass
    try:
        creds = resolve_codex_runtime_credentials()
        return {'logged_in': True, 'auth_store': str(_auth_file_path()), 'last_refresh': creds.get('last_refresh'), 'auth_mode': creds.get('auth_mode'), 'source': creds.get('source'), 'api_key': creds.get('api_key')}
    except AuthError as exc:
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                if api_key and (not _xai_access_token_is_expiring(api_key, 0)):
                    return {'logged_in': True, 'auth_store': str(_auth_file_path()), 'last_refresh': getattr(entry, 'last_refresh', None), 'auth_mode': 'oauth_device_code', 'source': f"pool:{getattr(entry, 'label', 'unknown')}", 'api_key': api_key}
    except Exception:
        pass
    try:
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    return {'configured': bool(api_key) or actual_local_noauth, 'provider': provider_id, 'name': pconfig.name, 'key_source': key_source or ('local-offline' if actual_local_noauth else ''), 'base_url': base_url, 'logged_in': bool(api_key) or actual_local_noauth}
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    if not pconfig or pconfig.auth_type != 'external_process':
        return {'configured': False}
    command = os.getenv('HERMES_COPILOT_ACP_COMMAND', '').strip() or os.getenv('COPILOT_CLI_PATH', '').strip() or 'copilot'
    ...
    return {'configured': bool(resolved_command or base_url.startswith('acp+tcp://')), 'provider': provider_id, 'name': pconfig.name, 'command': command, 'args': args, 'resolved_command': resolved_command, 'base_url': base_url, 'logged_in': bool(resolved_command or base_url.startswith('acp+tcp://'))}
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``duck-agent doctor`` runs the live
    ...
    if auth_mode == 'entra_id':
        try:
            from agent.azure_identity_adapter import EntraIdentityConfig, SCOPE_AI_AZURE_DEFAULT, has_azure_identity_installed
            ...
            if not installed:
                info['hint'] = "azure-identity not installed. Install with: pip install azure-identity  (or rely on Duck Agent' lazy-install at first use)."
            else:
                info['hint'] = 'azure-identity is installed; live credential validation is skipped here. Run `duck-agent doctor` to verify token acquisition.'
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    pconfig = PROVIDER_REGISTRY.get(provider_id)
    if not pconfig or pconfig.auth_type != 'api_key':
        raise AuthError(f"Provider '{provider_id}' is not an API-key provider.", provider=provider_id, code='invalid_provider')
    api_key = ''
    key_source = ''
    api_key, key_source = _resolve_api_key_provider_secret(provider_id, pconfig)
    ...
    return {'provider': provider_id, 'api_key': api_key, 'base_url': base_url.rstrip('/'), 'source': key_source or 'default'}

def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    """Resolve runtime details for local subprocess-backed providers."""
    pconfig = PROVIDER_REGISTRY.get(provider_id)
    if not pconfig or pconfig.auth_type != 'external_process':
        raise AuthError(f"Provider '{provider_id}' is not an external-process provider.", provider=provider_id, code='invalid_provider')
    base_url = os.getenv(pconfig.base_url_env_var, '').strip() if pconfig.base_url_env_var else ''
    if not base_url:
        base_url = pconfig.inference_base_url
    command = os.getenv('HERMES_COPILOT_ACP_COMMAND', '').strip() or os.getenv('COPILOT_CLI_PATH', '').strip() or 'copilot'
    ...
    if not resolved_command and (not base_url.startswith('acp+tcp://')):
        raise AuthError(f"Could not find the Copilot CLI command '{command}'. Install GitHub Copilot CLI or set HERMES_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.", provider=provider_id, code='missing_copilot_cli')
    return {'provider': provider_id, 'api_key': 'copilot-acp', 'base_url': base_url.rstrip('/'), 'command': resolved_command or command, 'args': args, 'source': 'process'}
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `duck-agent logout` historically keyed off auth.json.active_provider only.
...
def _confirm_expensive_model_selection(model_id: str, *, provider: str='', base_url: str='', api_key: str='') -> bool:
    """Prompt before saving a model whose known pricing exceeds guardrails."""
    try:
        from hermes_cli.model_cost_guard import expensive_model_warning
        warning = expensive_model_warning(model_id, provider=provider, base_url=base_url, api_key=api_key)
    except Exception:
        warning = None
    if warning is None:
        return True
...
def _prompt_model_selection(model_ids: List[str], current_model: str='', pricing: Optional[Dict[str, Dict[str, str]]]=None, unavailable_models: Optional[List[str]]=None, portal_url: str='', unavailable_message: str='', confirm_provider: str='', confirm_base_url: str='', confirm_api_key: str='') -> Optional[str]:
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        if confirm_provider and (not _confirm_expensive_model_selection(mid, provider=confirm_provider, base_url=confirm_base_url, api_key=confirm_api_key)):
            return None
        return mid
    ordered = []
    ...
    name_col = max((len(m) for m in all_models), default=0) + name_pad if has_pricing else 0
        ...
        for mid in all_models:
            ...
            if p:
                ...
                if cache:
                    has_cache = True
                if sale_chrome:
                    sale = compute_sale_discount(p.get('prompt', ''), p.get('completion', ''), p.get('original'))
                    if sale is not None:
                        any_on_sale = True
                        pct, was_prompt_raw, was_out_raw = sale
                        was_inp = _format_price_per_mtok(was_prompt_raw) if was_prompt_raw != '' else '?'
                        was_out = _format_price_per_mtok(was_out_raw) if was_out_raw != '' else '?'
            else:
                inp, out, cache = ('', '', '')
    ...
    def _label_segments(mid):
        ...
        if not has_pricing:
            ...
            return segs
        inp, out, cache, pct, was_inp, was_out = _price_cache.get(mid, ('', '', '', None, '', ''))
        on_sale = pct is not None
        star_w = 2
        if on_sale:
            name_segs: list[tuple[str, str | None]] = [('★ ', 'yellow'), (f'{mid:<{name_col - star_w}}', None)]
        else:
    ...
    try:
        ...
        idx = curses_radiolist('Select default model:', choices, selected=default_idx, cancel_returns=-1, description=description, searchable=True, search_labels=model_search_labels)
        if idx < 0:
            return None
        print()
        if idx < len(ordered):
    ...
    if _unavailable:
        _upgrade_url = (portal_url or DEFAULT_NOUS_PORTAL_URL).rstrip('/')
        unavailable_footer = unavailable_message.strip() or f'Unavailable models (requires paid tier — upgrade at {_upgrade_url})'
        print()
...
def login_command(args) -> None:
    """Deprecated: use 'duck-agent model' or 'duck-agent setup' instead."""
    print("The 'duck-agent login' command has been removed.")
    print("Use 'duck-agent auth' to manage credentials,")
    print("'duck-agent model' to select a provider, or 'duck-agent setup' for full setup.")
    raise SystemExit(0)

def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool=False) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.duck-agent/auth.json."""
    del args, pconfig
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and (not _codex_access_token_is_expiring(_resolved_key, 60)):
                print('Existing Codex credentials found in Duck Agent auth store.')
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print('Duck Agent will create its own session to avoid conflicts with Codex CLI / VS Code.')
            try:
            ...
            if do_import in {'y', 'yes'}:
                ...
                print('Duck Agent will keep working independently with its own session.')
                print(f'  Config updated: {config_path} (model.provider=openai-codex)')
                return
    print()
    print('Signing in to OpenAI Codex...')
    print("(Duck Agent creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(args, pconfig: ProviderConfig, *, force_new_login: bool=False) -> None:
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and (not _xai_access_token_is_expiring(api_key, 60)):
                print('Existing xAI OAuth credentials found in Duck Agent auth store.')
                try:
                    reuse = input('Use existing credentials? [Y/n]: ').strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = 'y'
                if reuse in {'', 'y', 'yes'}:
                    config_path = _update_config_for_provider('xai-oauth', existing.get('base_url', DEFAULT_XAI_OAUTH_BASE_URL))
                    print()
                    print('Login successful!')
    ...
    print('(Duck Agent creates its own local OAuth session)')
    print()
    ...
    _save_xai_oauth_tokens(creds['tokens'], discovery=creds.get('discovery'), redirect_uri=creds.get('redirect_uri', ''), last_refresh=creds.get('last_refresh'), auth_mode='oauth_device_code')
...
def _xai_oauth_request_device_code(client: httpx.Client, *, scope: str=XAI_OAUTH_SCOPE) -> Dict[str, Any]:
    response = client.post(XAI_OAUTH_DEVICE_CODE_URL, headers={'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json'}, data={'client_id': XAI_OAUTH_CLIENT_ID, 'scope': scope})
    if response.status_code != 200:
        raise AuthError(f'xAI device-code request failed (HTTP {response.status_code}).' + (f' Response: {response.text.strip()}' if response.text else ''), provider='xai-oauth', code='device_code_request_failed')
    payload = response.json()
    required = ('device_code', 'user_code', 'verification_uri', 'verification_uri_complete', 'expires_in', 'interval')
    missing = [key for key in required if key not in payload]
    if missing:
        raise AuthError(f"xAI device-code response missing fields: {', '.join(missing)}", provider='xai-oauth', code='device_code_invalid')
    return payload

def _xai_oauth_poll_device_token(client: httpx.Client, *, token_endpoint: str, device_code: str, expires_in: int, poll_interval: int) -> Dict[str, Any]:
    deadline = time.monotonic() + max(1, int(expires_in))
    current_interval = max(1, int(poll_interval))
    while time.monotonic() < deadline:
        response = client.post(token_endpoint, headers={'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json'}, data={'grant_type': 'urn:ietf:params:oauth:grant-type:device_code', 'client_id': XAI_OAUTH_CLIENT_ID, 'device_code': device_code})
        if response.status_code == 200:
            payload = response.json()
            if not payload.get('access_token'):
                raise AuthError('xAI device-code token response did not include an access_token.', provider='xai-oauth', code='xai_device_token_invalid')
            if not payload.get('refresh_token'):
                raise AuthError('xAI device-code token response did not include a refresh_token.', provider='xai-oauth', code='xai_device_token_invalid')
            return payload
        try:
            error_payload = response.json()
        except Exception:
            response.raise_for_status()
            raise AuthError('xAI device-code token polling returned a non-JSON error response.', provider='xai-oauth', code='xai_device_token_failed')
        ...
        raise AuthError(f'xAI device-code token polling failed: {description}', provider='xai-oauth', code='xai_device_token_failed')
    raise AuthError('Timed out waiting for xAI device authorization.', provider='xai-oauth', code='device_code_timeout')
...
def _xai_oauth_device_code_login(*, timeout_seconds: float=20.0, open_browser: bool=True) -> Dict[str, Any]:
    ...
    with httpx.Client(timeout=timeout, headers={'Accept': 'application/json'}) as client:
        ...
        payload = _xai_oauth_poll_device_token(client, token_endpoint=token_endpoint, device_code=str(device_data['device_code']), expires_in=expires_in, poll_interval=interval)
    access_token = str(payload.get('access_token', '') or '').strip()
    refresh_token = str(payload.get('refresh_token', '') or '').strip()
    if not access_token or not refresh_token:
        raise AuthError('xAI device-code token response was missing required tokens.', provider='xai-oauth', code='xai_device_token_invalid')
    base_url = _xai_validate_inference_base_url(os.getenv('HERMES_XAI_BASE_URL', '').strip().rstrip('/') or os.getenv('XAI_BASE_URL', '').strip().rstrip('/'), fallback=DEFAULT_XAI_OAUTH_BASE_URL)
    return {'tokens': {'access_token': access_token, 'refresh_token': refresh_token, 'id_token': str(payload.get('id_token', '') or '').strip(), 'expires_in': payload.get('expires_in'), 'token_type': str(payload.get('token_type') or 'Bearer').strip() or 'Bearer'}, 'discovery': discovery, 'redirect_uri': '', 'base_url': base_url, 'last_refresh': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'), 'source': 'oauth-device-code'}
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
                resp = client.post(f'{issuer}/api/accounts/deviceauth/usercode', json={'client_id': client_id}, headers={'Content-Type': 'application/json'})
        except Exception as exc:
    ...
    if resp is not None and resp.status_code == 429:
        retry_after = _parse_retry_after_seconds(getattr(resp, 'headers', None))
        wait_hint = f' Try again in about {retry_after}s.' if retry_after is not None else ' Wait a minute and run the login again.'
        raise AuthError(f"OpenAI is rate-limiting Codex login requests (HTTP 429). This is a temporary throttle on OpenAI's side, not a credential problem.{wait_hint}", provider='openai-codex', code=CODEX_RATE_LIMITED_CODE)
    if resp is None or resp.status_code != 200:
        status = resp.status_code if resp is not None else 'unknown'
        raise AuthError(f'Device code request returned status {status}.', provider='openai-codex', code='device_code_request_error')
    ...
    print(f'     \x1b[94m{issuer}/codex/device\x1b[0m\n')
    print('  2. Enter this code:')
    print(f'     \x1b[94m{user_code}\x1b[0m\n')
    ...
    try:
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            while _time.monotonic() - start < max_wait:
                ...
                poll_resp = client.post(f'{issuer}/api/accounts/deviceauth/token', json={'device_auth_id': device_auth_id, 'user_code': user_code}, headers={'Content-Type': 'application/json'})
                if poll_resp.status_code == 200:
                    ...
                    raise AuthError(f'Device auth polling returned status {poll_resp.status_code}.', provider='openai-codex', code='device_code_poll_error')
    ...
    if not authorization_code or not code_verifier:
        raise AuthError('Device auth response missing authorization_code or code_verifier.', provider='openai-codex', code='device_code_incomplete_exchange')
    try:
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            token_resp = client.post(CODEX_OAUTH_TOKEN_URL, data={'grant_type': 'authorization_code', 'code': authorization_code, 'redirect_uri': redirect_uri, 'client_id': client_id, 'code_verifier': code_verifier}, headers={'Content-Type': 'application/x-www-form-urlencoded'})
    except Exception as exc:
    ...
    if token_resp.status_code == 429:
        ...
        wait_hint = f' Try again in about {retry_after}s.' if retry_after is not None else ' Wait a minute and run the login again.'
        raise AuthError(f"OpenAI is rate-limiting Codex login requests (HTTP 429) during token exchange. This is a temporary throttle on OpenAI's side, not a credential problem.{wait_hint}", provider='openai-codex', code=CODEX_RATE_LIMITED_CODE)
    if token_resp.status_code != 200:
        raise AuthError(f'Token exchange returned status {token_resp.status_code}.', provider='openai-codex', code='token_exchange_error')
    tokens = token_resp.json()
    access_token = tokens.get('access_token', '')
    refresh_token = tokens.get('refresh_token', '')
    if not access_token:
        raise AuthError('Token exchange did not return an access_token.', provider='openai-codex', code='token_exchange_no_access_token')
    base_url = os.getenv('HERMES_CODEX_BASE_URL', '').strip().rstrip('/') or DEFAULT_CODEX_BASE_URL
    return {'tokens': {'access_token': access_token, 'refresh_token': refresh_token}, 'base_url': base_url, 'last_refresh': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'), 'auth_mode': 'chatgpt', 'source': 'device-code'}
...
def _minimax_request_user_code(client: httpx.Client, *, portal_base_url: str, client_id: str, code_challenge: str, state: str) -> Dict[str, Any]:
    response = _minimax_post_form(client, f'{portal_base_url}/oauth/code', data={'response_type': 'code', 'client_id': client_id, 'scope': MINIMAX_OAUTH_SCOPE, 'code_challenge': code_challenge, 'code_challenge_method': 'S256', 'state': state}, headers={'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json', 'x-request-id': str(uuid.uuid4())})
    if response.status_code != 200:
        body = _minimax_response_error_text(response)
        raise AuthError(f'MiniMax OAuth authorization failed: {body or response.reason_phrase}', provider='minimax-oauth', code='authorization_failed')
    payload = response.json()
    for field in ('user_code', 'verification_uri', 'expired_in'):
        if field not in payload:
            raise AuthError(f'MiniMax OAuth response missing field: {field}', provider='minimax-oauth', code='authorization_incomplete')
    if payload.get('state') != state:
        raise AuthError('MiniMax OAuth state mismatch (possible CSRF).', provider='minimax-oauth', code='state_mismatch')
    return payload
...
def _minimax_poll_token(client: httpx.Client, *, portal_base_url: str, client_id: str, user_code: str, code_verifier: str, expired_in: int, interval_ms: Optional[int]) -> Dict[str, Any]:
    import time as _time
    ...
    while _time.time() < deadline:
        response = _minimax_post_form(client, f'{portal_base_url}/oauth/token', data={'grant_type': MINIMAX_OAUTH_GRANT_TYPE, 'client_id': client_id, 'user_code': user_code, 'code_verifier': code_verifier}, headers={'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json'})
        error_text = ''
        if response.status_code != 200:
            ...
            raise AuthError(f"MiniMax OAuth error: {msg or 'unknown'}", provider='minimax-oauth', code='token_exchange_failed')
        try:
        ...
        if status == 'error':
            raise AuthError('MiniMax OAuth reported an error. Please try again later.', provider='minimax-oauth', code='authorization_denied')
        if status == 'success':
            if not all((payload.get(k) for k in ('access_token', 'refresh_token', 'expired_in'))):
                raise AuthError('MiniMax OAuth success payload missing required token fields.', provider='minimax-oauth', code='token_incomplete')
            return payload
        _time.sleep(interval)
    raise AuthError('MiniMax OAuth timed out before authorization completed.', provider='minimax-oauth', code='timeout')

def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Duck Agent auth store (~/.duck-agent/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(*, region: str='global', open_browser: bool=True, timeout_seconds: float=15.0) -> Dict[str, Any]:
    ...
    if _is_remote_session():
        open_browser = False
    print(f'Starting Duck Agent login via MiniMax ({region}) OAuth...')
    print(f'Portal: {portal_base_url}')
    with httpx.Client(timeout=httpx.Timeout(timeout_seconds), headers={'Accept': 'application/json'}, follow_redirects=True) as client:
        code_data = _minimax_request_user_code(client, portal_base_url=portal_base_url, client_id=pconfig.client_id, code_challenge=challenge, state=state)
        ...
        token_data = _minimax_poll_token(client, portal_base_url=portal_base_url, client_id=pconfig.client_id, user_code=user_code, code_verifier=verifier, expired_in=int(code_data['expired_in']), interval_ms=interval_ms)
    now = datetime.now(timezone.utc)
    ...
    auth_state = {'provider': 'minimax-oauth', 'region': region, 'portal_base_url': portal_base_url, 'inference_base_url': inference_base_url, 'client_id': pconfig.client_id, 'scope': MINIMAX_OAUTH_SCOPE, 'token_type': token_data.get('token_type', 'Bearer'), 'access_token': token_data['access_token'], 'refresh_token': token_data['refresh_token'], 'resource_url': token_data.get('resource_url'), 'obtained_at': now.isoformat(), 'expires_at': datetime.fromtimestamp(expires_at_unix, tz=timezone.utc).isoformat(), 'expires_in': expires_in_s}
...
def _refresh_minimax_oauth_state(state: Dict[str, Any], *, timeout_seconds: float=15.0, force: bool=False) -> Dict[str, Any]:
    ...
    if not state.get('refresh_token'):
        raise AuthError('MiniMax OAuth state has no refresh_token; please re-login.', provider='minimax-oauth', code='no_refresh_token', relogin_required=True)
    try:
    ...
    with httpx.Client(timeout=httpx.Timeout(timeout_seconds), follow_redirects=True) as client:
        response = _minimax_post_form(client, f'{portal_base_url}/oauth/token', data={'grant_type': 'refresh_token', 'client_id': state['client_id'], 'refresh_token': state['refresh_token']}, headers={'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json'})
        if response.status_code != 200:
            body = _minimax_response_error_text(response)
            body_lower = body.lower()
            relogin = any((m in body_lower for m in ('invalid_grant', 'refresh_token_reused', 'invalid_refresh_token')))
            raise AuthError(f'MiniMax OAuth refresh failed: {body or response.reason_phrase}', provider='minimax-oauth', code='refresh_failed', relogin_required=relogin)
    payload = response.json()
    if payload.get('status') != 'success':
        raise AuthError('MiniMax OAuth refresh did not return success.', provider='minimax-oauth', code='refresh_failed', relogin_required=True)
    now_dt = datetime.now(timezone.utc)
    expires_at_unix = _minimax_resolve_token_expiry_unix(int(payload['expired_in']), now=now_dt)
    expires_in_s = max(0, int(expires_at_unix - now_dt.timestamp()))
    new_state = dict(state)
    new_state.update({'access_token': payload['access_token'], 'refresh_token': payload.get('refresh_token', state['refresh_token']), 'obtained_at': now_dt.isoformat(), 'expires_at': datetime.fromtimestamp(expires_at_unix, tz=timezone.utc).isoformat(), 'expires_in': expires_in_s})
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    for _k in ('access_token', 'refresh_token', 'expires_at', 'expires_in', 'obtained_at'):
        state.pop(_k, None)
    state['last_auth_error'] = {'provider': 'minimax-oauth', 'code': exc.code or 'refresh_failed', 'message': str(exc), 'reason': 'runtime_refresh_failure', 'relogin_required': True, 'at': datetime.now(timezone.utc).isoformat()}
    try:
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get('access_token'):
            raise AuthError('Not logged into MiniMax OAuth. Run `duck-agent model` and select MiniMax (OAuth).', provider='minimax-oauth', code='not_logged_in', relogin_required=True)
        try:
        ...
        if not token:
            raise AuthError('MiniMax OAuth state has no access_token after refresh.', provider='minimax-oauth', code='no_access_token', relogin_required=True)
        return token
    return _provide
...
def resolve_minimax_oauth_runtime_credentials(*, min_token_ttl_seconds: int=MINIMAX_OAUTH_REFRESH_SKEW_SECONDS, as_token_provider: bool=False) -> Dict[str, Any]:
    ...
    diagnostic call sites like ``duck-agent status`` that just want to know
    ...
    return {'provider': 'minimax-oauth', 'api_key': api_key, 'base_url': state['inference_base_url'].rstrip('/'), 'source': 'oauth'}
...
def get_minimax_oauth_auth_status() -> Dict[str, Any]:
    ...
    return {'logged_in': token_valid, 'provider': 'minimax-oauth', 'region': state.get('region', 'global'), 'expires_at': state.get('expires_at')}
...
def _login_minimax_oauth(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        _minimax_oauth_login(region=region, open_browser=open_browser, timeout_seconds=timeout)
    except AuthError as exc:
        print(format_auth_error(exc))
        raise SystemExit(1)

def _nous_device_code_login(*, portal_base_url: Optional[str]=None, inference_base_url: Optional[str]=None, client_id: Optional[str]=None, scope: Optional[str]=None, open_browser: bool=True, timeout_seconds: float=15.0, insecure: bool=False, ca_bundle: Optional[str]=None, on_verification: Optional[Callable[[str, str], None]]=None) -> Dict[str, Any]:
    """Run the Nous device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY['nous']
    portal_base_url = (portal_base_url or os.getenv('HERMES_PORTAL_BASE_URL') or os.getenv('NOUS_PORTAL_BASE_URL') or pconfig.portal_base_url).rstrip('/')
    requested_inference_url = (inference_base_url or os.getenv('NOUS_INFERENCE_BASE_URL') or pconfig.inference_base_url).rstrip('/')
    ...
    if _is_remote_session():
        open_browser = False
    print(f'Starting Duck Agent login via {pconfig.name}...')
    print(f'Portal: {portal_base_url}')
    if insecure:
    ...
    with httpx.Client(timeout=timeout, headers={'Accept': 'application/json'}, verify=verify) as client:
        device_data = _request_device_code(client=client, portal_base_url=portal_base_url, client_id=client_id, scope=scope)
        ...
        token_data = _poll_for_token(client=client, portal_base_url=portal_base_url, client_id=client_id, device_code=str(device_data['device_code']), expires_in=expires_in, poll_interval=interval)
    now = datetime.now(timezone.utc)
    token_expires_in = _coerce_ttl_seconds(token_data.get('expires_in', 0))
    expires_at = now.timestamp() + token_expires_in
    resolved_inference_url = _optional_base_url(token_data.get('inference_base_url')) or requested_inference_url
    if resolved_inference_url != requested_inference_url:
        print(f'Using portal-provided inference URL: {resolved_inference_url}')
    auth_state = {'portal_base_url': portal_base_url, 'inference_base_url': resolved_inference_url, 'client_id': client_id, 'scope': token_data.get('scope') or scope, 'token_type': token_data.get('token_type', 'Bearer'), 'access_token': token_data['access_token'], 'refresh_token': token_data.get('refresh_token'), 'obtained_at': now.isoformat(), 'expires_at': datetime.fromtimestamp(expires_at, tz=timezone.utc).isoformat(), 'expires_in': token_expires_in, 'tls': {'insecure': verify is False, 'ca_bundle': verify if isinstance(verify, str) else None}, 'agent_key': None, 'agent_key_id': None, 'agent_key_expires_at': None, 'agent_key_expires_in': None, 'agent_key_reused': None, 'agent_key_obtained_at': None}
    try:
        return refresh_nous_oauth_from_state(auth_state, timeout_seconds=timeout_seconds, force_refresh=False)
    except AuthError as exc:
        if exc.code == 'subscription_required':
            ...
            print()
            print('After subscribing, run `duck-agent model` again to finish setup.')
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(*, open_browser: bool=True, timeout_seconds: float=15.0, on_verification: Optional[Callable[[str, str], None]]=None) -> bool:
    ...
    auth_state = _nous_device_code_login(portal_base_url=prior.get('portal_base_url') or None, inference_base_url=prior.get('inference_base_url') or None, client_id=prior.get('client_id') or pconfig.client_id, scope=scope, open_browser=open_browser, timeout_seconds=timeout_seconds, on_verification=on_verification)
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if auth_state is None:
            auth_state = _nous_device_code_login(portal_base_url=getattr(args, 'portal_url', None), inference_base_url=getattr(args, 'inference_url', None), client_id=getattr(args, 'client_id', None) or pconfig.client_id, scope=getattr(args, 'scope', None), open_browser=not getattr(args, 'no_browser', False), timeout_seconds=timeout_seconds, insecure=insecure, ca_bundle=ca_bundle)
        ...
        try:
            ...
            from hermes_cli.models import get_curated_nous_model_ids, get_pricing_for_provider, check_nous_free_tier, partition_nous_models_by_tier, union_with_portal_free_recommendations, union_with_portal_paid_recommendations
            ...
            if model_ids:
                ...
                if free_tier:
                    try:
                        ...
                        unavailable_message = ''
                    model_ids, pricing = union_with_portal_free_recommendations(model_ids, pricing, _portal_for_recs)
                    model_ids, unavailable_models = partition_nous_models_by_tier(model_ids, pricing, free_tier=True)
                else:
                    model_ids, pricing = union_with_portal_paid_recommendations(model_ids, pricing, _portal_for_recs)
            _portal = auth_state.get('portal_base_url', '')
            if model_ids:
                print(f'Showing {len(model_ids)} curated models — use "Enter custom model name" for others.')
                selected_model = _prompt_model_selection(model_ids, pricing=pricing, unavailable_models=unavailable_models, portal_url=_portal, unavailable_message=unavailable_message, confirm_provider='nous', confirm_base_url=inference_base_url, confirm_api_key=runtime_key)
        ...
        if not selected_model:
            ...
            print('  Run `duck-agent model` again to switch to Nous Portal.')
            return
        config_path = _update_config_for_provider('nous', inference_base_url, default_model=selected_model)
        if selected_model:
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv('OPENROUTER_API_KEY'):
            print('Duck Agent will use OpenRouter for inference.')
        elif should_reset_config:
            print('Run `duck-agent model` or configure an API key to use Duck Agent.')
        else:
