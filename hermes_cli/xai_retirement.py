...
Pure logic: walks a Duck Agent config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `duck-agent doctor` and a future `duck-agent migrate xai`.
"""
...
_RETIRED_MODELS: Dict[str, Dict[str, Optional[str]]] = {'grok-4-0709': {'replacement': 'grok-4.3', 'reasoning_effort': None, 'note': None}, 'grok-4-fast-reasoning': {'replacement': 'grok-4.3', 'reasoning_effort': None, 'note': None}, 'grok-4-fast-non-reasoning': {'replacement': 'grok-4.3', 'reasoning_effort': 'none', 'note': None}, 'grok-4-1-fast-reasoning': {'replacement': 'grok-4.3', 'reasoning_effort': None, 'note': None}, 'grok-4-1-fast-non-reasoning': {'replacement': 'grok-4.3', 'reasoning_effort': 'none', 'note': None}, 'grok-code-fast-1': {'replacement': 'grok-4.3', 'reasoning_effort': None, 'note': None}, 'grok-3': {'replacement': 'grok-4.3', 'reasoning_effort': None, 'note': None}, 'grok-imagine-image-pro': {'replacement': 'grok-imagine-image-quality', 'reasoning_effort': None, 'note': None}}

@dataclass(frozen=True)
class RetirementIssue:
    """A reference to a retired xAI model found in a Duck Agent config."""
    config_path: str
    current_model: str
    replacement: str
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Duck Agent config and return retirement issues.
    ...
    def _check(path: str, model: Any) -> None:
        ...
        if entry is None:
            return
        issues.append(RetirementIssue(config_path=path, current_model=model, replacement=entry['replacement'], reasoning_effort=entry.get('reasoning_effort'), note=entry.get('note')))
...
def apply_migration(config_path: Path, issues: List[RetirementIssue], backup: bool=True) -> ApplyResult:
    ...
    if not issues:
        return ApplyResult(file_path=config_path, backup_path=None, issues_resolved=[], config_changed=False)
    yaml = YAML(typ='rt')
