    ...
    for session in sessions:
        ...
        for message in _messages(session):
            if message.get('role') != 'user':
                continue
            index += 1
            record: Dict[str, Any] = {'session_id': session_id, 'index': index, 'created_at': _format_timestamp(message.get('timestamp')), 'role': 'user', 'text': _message_text(message.get('content'))}
...
def _render_markdown(sessions: List[Dict[str, Any]], *, only: Optional[ExportOnly]) -> str:
    if only == 'user-prompts':
...
def _render_full_markdown(sessions: List[Dict[str, Any]]) -> str:
    ...
    if len(sessions) == 1:
        ...
        lines.append('# Duck Agent sessions export')
        lines.append('')
        for session in sessions:
            lines.append(f'## Session: {_heading_text(_session_title_or_id(session))}')
            lines.extend(_session_metadata_lines(session))
