...
class SwarmCreated:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'root_id': self.root_id, 'worker_ids': list(self.worker_ids), 'verifier_id': self.verifier_id, 'synthesizer_id': self.synthesizer_id}
...
def create_swarm(conn: sqlite3.Connection, *, goal: str, workers: Iterable[SwarmWorkerSpec], verifier_assignee: str, synthesizer_assignee: str, root_title: Optional[str]=None, verifier_title: str='Verify swarm outputs', synthesizer_title: str='Synthesize swarm outputs', tenant: Optional[str]=None, created_by: str='swarm-orchestrator', workspace_kind: str='scratch', workspace_path: Optional[str]=None, priority: int=0, idempotency_key: Optional[str]=None) -> SwarmCreated:
    ...
    root = kb.create_task(conn, title=root_title or f'Swarm: {goal.splitlines()[0][:80]}', body=f'Kanban Swarm v1 planning/root card. This card is completed immediately so parallel workers can start while it remains the shared blackboard and audit anchor.\n\nGoal:\n{goal}', assignee=created_by, created_by=created_by, tenant=tenant, priority=priority, idempotency_key=idempotency_key, workspace_kind=workspace_kind, workspace_path=workspace_path)
    ...
    if isinstance(existing, dict):
        ...
        if worker_ids and verifier_id and synthesizer_id:
            return SwarmCreated(root_id=root, worker_ids=worker_ids, verifier_id=str(verifier_id), synthesizer_id=str(synthesizer_id))
    kb.complete_task(conn, root, summary='Swarm topology planned; root remains the shared blackboard.', metadata={'kind': 'kanban_swarm_v1', 'goal': goal, 'worker_count': len(worker_specs)})
    context_suffix = _swarm_context(root, goal)
    worker_ids: list[str] = []
    for spec in worker_specs:
        worker_id = kb.create_task(conn, title=spec.title, body=(spec.body or '') + context_suffix, assignee=spec.profile, created_by=created_by, parents=[root], tenant=tenant, priority=spec.priority or priority, workspace_kind=workspace_kind, workspace_path=workspace_path, skills=spec.skills or None, max_runtime_seconds=spec.max_runtime_seconds)
        worker_ids.append(worker_id)
    verifier_body = 'Review every worker handoff and blackboard update. Gate the swarm: complete only with metadata {"gate": "pass"} when evidence is sufficient; otherwise block with exact missing work.' + context_suffix
    verifier = kb.create_task(conn, title=verifier_title, body=verifier_body, assignee=verifier_assignee, created_by=created_by, parents=worker_ids, tenant=tenant, priority=priority, workspace_kind=workspace_kind, workspace_path=workspace_path, skills=['requesting-code-review'])
    synthesizer_body = 'Synthesize the verified worker outputs into the final deliverable. Do not start until the verifier has passed the gate.' + context_suffix
    synthesizer = kb.create_task(conn, title=synthesizer_title, body=synthesizer_body, assignee=synthesizer_assignee, created_by=created_by, parents=[verifier], tenant=tenant, priority=priority, workspace_kind=workspace_kind, workspace_path=workspace_path, skills=['humanizer'])
