"""
duck-agent fallback — manage the fallback provider chain.

Fallback providers are tried in order when the primary model fails with
rate-limit, overload, or connection errors. See:
https://duck-agent.nousresearch.com/docs/user-guide/features/fallback-providers

Subcommands:
  duck-agent fallback [list]   Show the current fallback chain (default when no subcommand)
  duck-agent fallback add      Pick provider + model via the same picker as `duck-agent model`,
                           then append the selection to the chain
  duck-agent fallback remove   Pick an entry to delete from the chain
  duck-agent fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.duck-agent/config.yaml`` (top-level, list of
...
def cmd_fallback_list(args) -> None:
    ...
    if not chain:
        ...
        print()
        print('  Add one with:  duck-agent fallback add')
        print()
        return
    ...
    print('  Docs: https://duck-agent.nousresearch.com/docs/user-guide/features/fallback-providers')
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `duck-agent model`, then append the selection to the chain."""
    ...
    print('  `duck-agent model` — select the provider + model you want as a fallback.')
    print()
    try:
    ...
    new_ident = BackendIdentity.build(provider=new_entry.get('provider'), model=new_entry.get('model'), base_url=new_entry.get('base_url'))
    primary_entry = _extract_fallback_from_model_cfg(model_before)
    if primary_entry and same_deployment(BackendIdentity.build(provider=primary_entry.get('provider'), model=primary_entry.get('model'), base_url=primary_entry.get('base_url')), new_ident):
        _restore_model_cfg(model_before)
        _restore_auth_active_provider(active_provider_before)
        print()
    ...
    for existing in chain:
        if same_deployment(BackendIdentity.build(provider=existing.get('provider'), model=existing.get('model'), base_url=existing.get('base_url')), new_ident):
            print()
    ...
    print()
    print('  Run `duck-agent fallback list` to view, or `duck-agent fallback remove` to delete.')
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``duck-agent fallback [subcommand]``."""
