"""
Duck Agent Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``duck-agent desktop`` / ``duck-agent gui``)
     Built inside the agent checkout under ``$DUCK_AGENT_HOME/duck-agent/``:
       ...
       - ``$DUCK_AGENT_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Duck Agent.app`` or ``~/Applications/Duck Agent.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Duck Agent`` (NSIS per-user)
...
the app name ("Duck Agent"), separate from ``$DUCK_AGENT_HOME``:
  - macOS:   ``~/Library/Application Support/Duck Agent``
  - Windows: ``%APPDATA%\\Duck Agent``
  - Linux:   ``$XDG_CONFIG_HOME/Duck Agent`` (default ``~/.config/Duck Agent``)
...
``duck-agent uninstall --gui`` (and friends) without paying for the full CLI.
"""
import os
import shutil
import sys
...
def _agent_root(hermes_home: Path) -> Path:
    ...
    return hermes_home / 'duck-agent'

def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Duck Agent"
    ...
    if sys.platform == 'darwin':
        return home / 'Library' / 'Application Support' / 'Duck Agent'
...
def source_built_gui_artifacts(hermes_home: Path) -> 'list[Path]':
    """GUI build artifacts produced by ``duck-agent desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``duck-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(hermes_home)
    desktop_dir = agent_root / 'apps' / 'desktop'
    return [desktop_dir / 'dist', desktop_dir / 'release', desktop_dir / 'node_modules', agent_root / 'node_modules', hermes_home / 'desktop-build-stamp.json']
...
def packaged_gui_app_paths() -> 'list[Path]':
    ...
    electron-builder output locations for the "Duck Agent" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == 'darwin':
        paths += [Path('/Applications/Duck Agent.app'), home / 'Applications' / 'Duck Agent.app']
    elif sys.platform == 'win32':
        local = os.environ.get('LOCALAPPDATA')
        local_base = Path(local) if local else home / 'AppData' / 'Local'
        paths += [local_base / 'Programs' / 'Duck Agent', local_base / 'duck-agent-desktop']
        program_files = os.environ.get('ProgramFiles')
        if program_files:
            paths.append(Path(program_files) / 'Duck Agent')
    else:
        from hermes_cli.linux_desktop_entry import desktop_entry_path
        data = os.environ.get('XDG_DATA_HOME')
        data_base = Path(data) if data else home / '.local' / 'share'
        paths += [desktop_entry_path(), data_base / 'applications' / 'Duck Agent.desktop']
    return paths

def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under DUCK_AGENT_HOME.
...
def gui_install_summary(hermes_home: 'Path | None'=None) -> dict:
    ...
    return {'duck_agent_home': str(home), 'agent_installed': agent_is_installed(home), 'gui_installed': gui_is_installed(home), 'source_built_artifacts': [str(p) for p in source_artifacts], 'packaged_app_paths': [str(p) for p in packaged], 'userdata_dir': str(userdata), 'userdata_exists': userdata.exists(), 'platform': sys.platform}
...
def uninstall_gui(hermes_home: 'Path | None'=None, *, remove_userdata: bool=True) -> 'list[Path]':
    ...
    Never touches ``duck-agent/hermes_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$DUCK_AGENT_HOME``.
    ...
    if sys.platform.startswith('linux'):
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager (e.g. 'sudo apt remove duck-agent' or 'sudo dnf remove duck-agent'). AppImage builds are a single file you can delete from wherever you saved it.")
    return removed