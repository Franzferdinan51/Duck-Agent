...
class CLIBillingMixin:
    ...
    def _show_subscription(self):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                print('  Run `duck-agent portal` to log in, then /subscription.')
            return
    ...
    def _subscription_open_portal(self, state, manage_url, *, verb='Manage your subscription'):
        ...
        if not manage_url:
            ...
            return
        print()
        choices = [('open', verb, 'open the subscription page in your browser'), ('copy', 'Copy link', 'copy the manage-subscription URL to your clipboard'), ('cancel', 'Cancel', 'do nothing')]
    ...
    def _subscription_change_menu(self, state, manage_url):
        ...
        if has_pending:
            choices = [('keep', f'Keep {keep_name} (undo the scheduled change)', 'cancel the pending change'), ('change', 'Change plan', 'upgrade or downgrade in the terminal')]
        else:
            choices = [('change', 'Change plan', 'upgrade or downgrade in the terminal'), ('cancel_sub', 'Cancel subscription', 'schedule cancellation at period end')]
    ...
    def _subscription_pick_tier(self, state):
        ...
        raw = self._prompt_text_input_modal(title='Change plan', detail=f"Current: {(c.tier_name if c else 'Free')}. Pick a plan to preview the effect.", choices=choices)
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if not choice or choice == 'cancel':
    ...
    def _subscription_confirm_cancel(self, state):
        ...
        confirm_choices = [('yes', 'Cancel subscription', 'schedule cancellation at period end'), ('cancel', 'Go back', 'keep your plan')]
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from hermes_cli.nous_billing import BillingError, BillingTransient, BillingRemoteSpendingRevoked, BillingScopeRequired, BillingSessionRevoked, delete_subscription_pending_change, post_subscription_upgrade, put_subscription_pending_change
        kind, arg = action
        key = None
        if kind == 'upgrade':
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, '_app', None):
            print('  Run `duck-agent portal` and allow Remote Spending, then re-run /subscription.')
            return
        confirm_choices = [('yes', 'Allow Remote Spending', 'open your browser to authorize'), ('no', 'Not now', 'cancel')]
        raw = self._prompt_text_input_modal(title='Allow Remote Spending', detail='Opens your browser to authorize this terminal.', choices=confirm_choices)
        if self._normalize_slash_confirm_choice(raw, confirm_choices) != 'yes':
    ...
    def _show_billing(self, command: str='/topup'):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print('  Run `duck-agent portal` to log in, then /topup.')
            return
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(state, reason="A billing admin can turn it on from the portal's Duck Agent page to add funds here.")
            return
        if not getattr(self, '_app', None):
            self._billing_portal_hint(state)
            return
        _cprint(f"  {_d('Add funds now — a single charge, added to your balance today.')}")
        if ar is not None and ar.enabled and (ar.reload_to_usd is not None) and ar.reload_to_usd.is_finite() and (ar.threshold_usd is not None) and ar.threshold_usd.is_finite():
        ...
        print(f"  {'─' * 41}")
        choices = [('buy', 'Add funds', 'a single charge, added to your balance today'), ('auto', 'Auto-reload', 'refill automatically when your balance runs low'), ('limit', 'Monthly limit', 'show the monthly spend cap (read-only)'), ('portal', 'Manage on portal', 'open the billing page in your browser'), ('cancel', 'Cancel', 'do nothing')]
        raw = self._prompt_text_input_modal(title='Top up your balance', detail='', choices=choices)
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice == 'buy':
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            print()
            _cprint(f"  💳 {_d('Remote spending is off for this org.')}")
            self._billing_portal_hint(state, reason="A billing admin can turn it on from the portal's Duck Agent page before adding funds.")
            return False
        return True
    ...
    def _billing_add_card_flow(self, state):
        ...
        choices = [('portal', 'Add a card on the portal', 'opens the billing page in your browser'), ('recheck', "I've added it — check again", 're-check for the card and continue'), ('cancel', 'Back', 'do nothing')]
        for _ in range(8):
    ...
    def _billing_buy_flow(self, state):
        ...
        raw = self._prompt_text_input_modal(title='Add funds', detail=detail, choices=preset_choices)
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        confirm_choices = [('pay', f'Pay {format_money(amount)} now', 'submit the charge'), ('portal', 'Manage on portal', 'manage your card / billing in the browser'), ('cancel', 'Go back', 'do not charge')]
        if not getattr(self, '_app', None):
            print('  Run in the interactive CLI to confirm a purchase.')
            return
        raw = self._prompt_text_input_modal(title=f'Pay {format_money(amount)}?', detail=card.display if card else 'no saved card', choices=confirm_choices)
    ...
    def _billing_render_charge_failed(self, state, reason):
        ...
        if reason == 'authentication_required':
            print('  🔴 Your bank requires verification (3DS). Complete it on the portal to finish this purchase.')
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == 'remote_spending_revoked':
            who = "An admin stopped this terminal's spending." if actor == 'admin' else "You stopped this terminal's spending."
            print(f'  🔴 {who} Reconnect to restore — run `duck-agent portal` to re-authorize.')
        elif isinstance(exc, BillingSessionRevoked) or code == 'session_revoked':
            print('  🔴 Your session was logged out. Run `duck-agent portal` to log in again.')
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Duck Agent page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, '_app', None):
            print('  Run `duck-agent portal` and allow Remote Spending, then retry.')
            return
        confirm_choices = [('yes', 'Allow Remote Spending', 'open your browser to authorize'), ('no', 'Not now', 'cancel')]
        raw = self._prompt_text_input_modal(title='Allow Remote Spending', detail='Opens your browser to authorize this terminal.', choices=confirm_choices)
        choice = self._normalize_slash_confirm_choice(raw, confirm_choices)
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Duck Agent page, then run /topup again.")
        ...
        resume_choices = [('resume', f'Resume {format_money(amount)} top-up', 'finish the held purchase'), ('cancel', 'Cancel', 'do not charge')]
        raw = self._prompt_text_input_modal(title='Resume your top-up', detail=f'{format_money(amount)} is ready to finish — press Enter to resume.', choices=resume_choices)
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        if not getattr(self, '_app', None):
            ...
            return
        if currently_on:
            top_choices = [('edit', 'Edit thresholds', 'change when / how much to reload'), ('off', 'Turn off', 'disable auto-reload'), ('cancel', 'Cancel', 'do nothing')]
            raw = self._prompt_text_input_modal(title='Auto-reload', detail=f'On — below {format_money(ar.threshold_usd)} → reload to {format_money(ar.reload_to_usd)}', choices=top_choices)
        ...
        if not (threshold_raw or '').strip() and currently_on:
            ...
            tv = validate_charge_amount(threshold_raw or '', min_usd=state.min_usd, max_usd=state.max_usd)
        ...
        if not (reload_raw or '').strip() and currently_on:
            ...
            rv = validate_charge_amount(reload_raw or '', min_usd=state.min_usd, max_usd=state.max_usd)
        ...
        if reload_amt is None or threshold_amt is None or reload_amt <= threshold_amt:
            ...
            return
        print()
        _ar_consent = f'By confirming, you authorize Nous Research to charge {card.masked} whenever your balance reaches {format_money(threshold_amt)}. Turn off any time here or on the portal.'
        _cprint(f'  {_d(_ar_consent)}')
        confirm_choices = [('agree', 'Agree and turn on', 'enable auto-reload'), ('cancel', 'Cancel', 'do nothing')]
        raw = self._prompt_text_input_modal(title='Turn on auto-reload?', detail=f'Below {format_money(threshold_amt)} → reload to {format_money(reload_amt)}', choices=confirm_choices)
        choice = self._normalize_slash_confirm_choice(raw, confirm_choices)
        if choice != 'agree':
            print('  🟡 Cancelled.')
            return
        ...
        try:
            patch_auto_top_up(enabled=True, threshold=float(threshold_amt), top_up_amount=float(reload_amt))
        except BillingScopeRequired:
    ...
    def _billing_limit_screen(self, state):
        ...
        _limit_note = 'The monthly limit is set on the portal — the terminal shows it read-only.'
