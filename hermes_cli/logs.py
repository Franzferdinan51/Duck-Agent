"""``duck-agent logs`` — view and filter Duck Agent log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.duck-agent/logs/``.

Usage examples::

    duck-agent logs                    # last 50 lines of agent.log
    duck-agent logs -f                 # follow agent.log in real time
    duck-agent logs errors             # last 50 lines of errors.log
    duck-agent logs gateway -n 100    # last 100 lines of gateway.log
    duck-agent logs gui -f            # follow gui.log (dashboard/pty/ws)
    duck-agent logs desktop -f        # follow desktop.log (Electron app boot/backend)
    duck-agent logs --level WARNING    # only WARNING+ lines
    duck-agent logs --session abc123   # filter by session ID substring
    duck-agent logs --component tools  # only tool-related lines
    duck-agent logs --since 1h         # lines from the last hour
    duck-agent logs --since 30m -f     # follow, starting 30 min ago
"""
import re
import sys
import time
...
LOG_FILES = {'agent': 'agent.log', 'errors': 'errors.log', 'gateway': 'gateway.log', 'gui': 'gui.log', 'desktop': 'desktop.log', 'mcp': 'mcp-stderr.log'}
_TS_RE = re.compile('^(\\d{4}-\\d{2}-\\d{2}\\s+\\d{2}:\\d{2}:\\d{2})')
_LEVEL_RE = re.compile('\\s(DEBUG|INFO|WARNING|ERROR|CRITICAL)\\s')
_LOGGER_NAME_RE = re.compile('\\s(?:DEBUG|INFO|WARNING|ERROR|CRITICAL)(?:\\s+\\[.*?\\])?\\s+(\\S+):')
...
def _parse_since(since_str: str) -> Optional[datetime]:
    ...
    match = re.match('^(\\d+)\\s*([smhd])$', since_str)
    if not match:
        return None
    value = int(match.group(1))
    unit = match.group(2)
    delta = {'s': timedelta(seconds=value), 'm': timedelta(minutes=value), 'h': timedelta(hours=value), 'd': timedelta(days=value)}[unit]
...
def _matches_filters(line: str, *, min_level: Optional[str]=None, session_filter: Optional[str]=None, since: Optional[datetime]=None, component_prefixes: Optional[Sequence[str]]=None) -> bool:
...
def tail_log(log_name: str='agent', *, num_lines: int=50, follow: bool=False, level: Optional[str]=None, session: Optional[str]=None, since: Optional[str]=None, component: Optional[str]=None) -> None:
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Duck Agent runs — try 'duck-agent chat' first)")
        sys.exit(1)
    since_dt = None
    if since:
    ...
    has_filters = min_level is not None or session is not None or since_dt is not None or (component_prefixes is not None)
    try:
        lines = _read_tail(log_path, num_lines, has_filters=has_filters, min_level=min_level, session_filter=session, since=since_dt, component_prefixes=component_prefixes)
    ...
    if not follow:
        return
    try:
        _follow_log(log_path, min_level=min_level, session_filter=session, since=since_dt, component_prefixes=component_prefixes)
    except KeyboardInterrupt:
        print('\n--- stopped ---')

def _read_tail(path: Path, num_lines: int, *, has_filters: bool=False, min_level: Optional[str]=None, session_filter: Optional[str]=None, since: Optional[datetime]=None, component_prefixes: Optional[Sequence[str]]=None) -> list:
    ...
    if has_filters:
        ...
        filtered = [l for l in raw_lines if _matches_filters(l, min_level=min_level, session_filter=session_filter, since=since, component_prefixes=component_prefixes)]
...
def _follow_log(path: Path, *, min_level: Optional[str]=None, session_filter: Optional[str]=None, since: Optional[datetime]=None, component_prefixes: Optional[Sequence[str]]=None) -> None:
    ...
    with open(path, 'r', encoding='utf-8', errors='replace') as f:
        ...
        while True:
            line = f.readline()
            if line:
                if _matches_filters(line, min_level=min_level, session_filter=session_filter, since=since, component_prefixes=component_prefixes):
                    print(line, end='')
                    sys.stdout.flush()
            else:
                time.sleep(0.3)
...
def list_logs() -> None:
    ...
    for entry in sorted(log_dir.iterdir()):
        if entry.is_file() and entry.suffix == '.log':
            ...
            found = True
    if not found:
        print("  (no log files yet — run 'duck-agent chat' to generate logs)")