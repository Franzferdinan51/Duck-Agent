...
def _estimate_tokens(agent: Any, messages: Optional[List[dict]]) -> Optional[int]:
    ...
    if messages is not None:
        protect = int(getattr(cc, 'protect_first_n', 3)) + int(getattr(cc, 'protect_last_n', 20)) + 1
        ...
        try:
            ...
            return int(estimate_request_tokens_rough(messages, system_prompt=system_prompt, tools=tools or None))
        except Exception:
            pass
...
def merge_preflight_compression_warning(result: ModelSwitchResult, *, agent: Any=None, messages: Optional[List[dict]]=None, custom_providers: list | None=None, config_context_length: int | None=None, configured_model: str | None=None, configured_provider: str | None=None, configured_base_url: str | None=None) -> None:
    ...
    new_ctx = resolve_display_context_length(result.new_model, result.target_provider, base_url=result.base_url or getattr(agent, 'base_url', '') or '', api_key=result.api_key or getattr(agent, 'api_key', '') or '', model_info=result.model_info, custom_providers=custom_providers, config_context_length=config_context_length, configured_model=configured_model if configured_model is not None else getattr(agent, 'model', None), configured_provider=configured_provider if configured_provider is not None else getattr(agent, 'provider', None), configured_base_url=configured_base_url if configured_base_url is not None else getattr(agent, 'base_url', None))
    if not new_ctx:
        return
    ...
    parts.append(f'Session is ~{estimate:,} tokens; {result.new_model} allows {new_ctx:,} (auto-compress at ~{new_threshold:,}). Your next message will run preflight compression before the model replies.')
...
def enrich_model_switch_warnings_for_gateway(result: ModelSwitchResult, runner: Any, *, session_key: str, source: Any, custom_providers: list | None=None, load_gateway_config: Callable[[], dict] | None=None) -> None:
    ...
    if db is not None and store is not None:
        try:
            ...
            pass
    merge_preflight_compression_warning(result, agent=agent, messages=messages, custom_providers=custom_providers, config_context_length=cfg_ctx, configured_model=configured_model, configured_provider=configured_provider, configured_base_url=configured_base_url)