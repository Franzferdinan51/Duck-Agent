...
def run_approval_mode_command(requested_mode: Optional[str]) -> ApprovalModeResult:
    ...
    if effective != requested:
        return ApprovalModeResult(False, effective, False, f'Approval mode remains {effective}; the requested value did not become effective.')
    return ApprovalModeResult(True, effective, effective != current, f'Approval mode: {effective} (persistent profile setting).')