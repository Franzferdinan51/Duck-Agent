"""Shared time/filter parsing for `duck-agent sessions prune` / `archive`.
...
_DURATION_RE = re.compile('^(\\d+(?:\\.\\d+)?)\\s*(s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days|w|wk|wks|week|weeks)$')
...
def parse_point_in_time(value: str, flag: str) -> float:
    ...
    try:
        ...
        raise ValueError(f"Invalid value for {flag}: '{value}'. Use a duration like '5h', '30m', '2d', '1w', a bare number of days, or an ISO timestamp like '2026-07-05' or '2026-07-05 14:30'.") from None
...
def build_prune_filters(args: Any) -> Dict[str, Any]:
    ...
    if older_than is not None:
        last_active_before = _tighter(last_active_before, parse_point_in_time(older_than, '--older-than'), True)
    newer_than = getattr(args, 'newer_than', None)
    if newer_than is not None:
        last_active_after = _tighter(last_active_after, parse_point_in_time(newer_than, '--newer-than'), False)
    before = getattr(args, 'before', None)
    if before is not None:
        started_before = _tighter(started_before, parse_point_in_time(before, '--before'), True)
    after = getattr(args, 'after', None)
    if after is not None:
        started_after = _tighter(started_after, parse_point_in_time(after, '--after'), False)
    if started_before is not None and started_after is not None and (started_after >= started_before):
        raise ValueError(f'Empty start-time window: the --after bound ({format_epoch(started_after)}) is not earlier than the --before bound ({format_epoch(started_before)}).')
    if last_active_before is not None and last_active_after is not None and (last_active_after >= last_active_before):
        raise ValueError(f'Empty activity window: the --newer-than bound ({format_epoch(last_active_after)}) is not earlier than the --older-than bound ({format_epoch(last_active_before)}).')
    filters: Dict[str, Any] = {'older_than_days': None, 'last_active_before': last_active_before, 'last_active_after': last_active_after, 'started_before': started_before, 'started_after': started_after, 'source': getattr(args, 'source', None), 'title_like': getattr(args, 'title', None), 'end_reason': getattr(args, 'end_reason', None), 'cwd_prefix': getattr(args, 'cwd', None), 'min_messages': getattr(args, 'min_messages', None), 'max_messages': getattr(args, 'max_messages', None), 'model_like': getattr(args, 'model', None), 'provider': getattr(args, 'provider', None), 'user_id': getattr(args, 'user', None), 'chat_id': getattr(args, 'chat_id', None), 'chat_type': getattr(args, 'chat_type', None), 'branch_like': getattr(args, 'branch', None), 'min_tokens': getattr(args, 'min_tokens', None), 'max_tokens': getattr(args, 'max_tokens', None), 'min_cost': getattr(args, 'min_cost', None), 'max_cost': getattr(args, 'max_cost', None), 'min_tool_calls': getattr(args, 'min_tool_calls', None), 'max_tool_calls': getattr(args, 'max_tool_calls', None)}
    return filters
...
def describe_filters(filters: Dict[str, Any]) -> str:
    ...
    if filters.get('last_active_before') is not None:
        parts.append(f"last active before {format_epoch(filters['last_active_before'])}")
    if filters.get('last_active_after') is not None:
        parts.append(f"last active after {format_epoch(filters['last_active_after'])}")
