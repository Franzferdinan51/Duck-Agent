"""Duck Agent-managed uv and Python runtime repair.

Duck Agent owns its own uv binary at ``$DUCK_AGENT_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$DUCK_AGENT_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Duck Agent
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.duck-agent-runtime/python``. A
...
_RUNTIME_DIR_NAME = '.duck-agent-runtime'
_VENV_NAME = 'venv'
_ALT_VENV_NAME = '.venv'
_REPAIR_LOCK_NAME = 'runtime-repair.lock'

def managed_uv_path() -> Path:
    """Return the path where Duck Agent keeps *its* uv binary.

    ``$DUCK_AGENT_HOME/bin/uv`` on POSIX, ``$DUCK_AGENT_HOME\\bin\\uv.exe`` on
...
def managed_python_env(project_root: Path | None=None, *, install_dir: Path | None=None, base_env: dict[str, str] | None=None) -> dict[str, str]:
    """Return a sanitized environment for Duck Agent-private uv Python commands."""
    target = Path(install_dir) if install_dir is not None else managed_python_install_dir(project_root)
    env = dict(os.environ if base_env is None else base_env)
    for key in ('CONDA_DEFAULT_ENV', 'CONDA_PREFIX', 'UV_PROJECT_ENVIRONMENT', 'UV_NO_MANAGED_PYTHON', 'UV_PYTHON', 'UV_PYTHON_DOWNLOADS', 'UV_SYSTEM_PYTHON', 'VIRTUAL_ENV', 'PYTHONHOME', 'PYTHONPATH'):
        env.pop(key, None)
    env.update({'UV_MANAGED_PYTHON': '1', 'UV_NO_CONFIG': '1', 'UV_PYTHON_INSTALL_BIN': '0', 'UV_PYTHON_INSTALL_DIR': str(target), 'UV_PYTHON_INSTALL_REGISTRY': '0'})
    return env
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        print(f'  ℹ Managed Python runtime was not replaced; the existing venv is unchanged ({repair.detail}).')
        print('    Sessions stay protected meanwhile: Duck Agent keeps databases out of WAL mode on this SQLite build. The next `duck-agent update` will retry.')
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``duck-agent update`` runs
...
def _ensure_uv_path(*, repair_observer: Callable[[RuntimeRepairResult], None] | None=None) -> Optional[str]:
    ...
    result = resolve_uv()
    if result:
        version = subprocess.run([result, '--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', check=False).stdout.strip()
...
def ensure_uv(*, repair_observer: Callable[[RuntimeRepairResult], None] | None=None):
...
def _uv_self_update_is_fresh(now: float | None=None) -> bool:
    ...
    uv releases roughly weekly while many users run ``duck-agent update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under DUCK_AGENT_HOME
...
def update_managed_uv(*, repair_observer: Callable[[RuntimeRepairResult], None] | None=None, force: bool=False) -> Optional[str]:
    """Run ``uv self update`` on the managed uv binary.

    Call this during ``duck-agent update`` so the managed copy stays current.
    ...
    if force or not _uv_self_update_is_fresh():
        try:
            result = subprocess.run([existing, 'self', 'update'], capture_output=True, text=True, encoding='utf-8', errors='replace', check=False, timeout=UV_SELF_UPDATE_TIMEOUT_SECONDS)
        except subprocess.TimeoutExpired:
            logger.debug('uv self update timed out after %ss', UV_SELF_UPDATE_TIMEOUT_SECONDS)
            result = None
        if result is not None and result.returncode == 0:
            _touch_uv_self_update_stamp()
            version = subprocess.run([existing, '--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', check=False).stdout.strip()
...
def _reload_hermes_constants():
    ...
    ``duck-agent update`` imports ``hermes_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        (~/.duck-agent/duck-agent/hermes_constants.py)
...
def _make_world_traversable(path: Path) -> None:
    ...
    try:
        path.chmod(path.stat().st_mode | 493)
    except OSError:
        pass
...
def _list_available_patches(uv_bin: str, minor: str, *, cwd: Path, env: dict) -> list[tuple[int, int, int]]:
    ...
    """
    try:
        result = subprocess.run([uv_bin, 'python', 'list', minor, '--all-versions', '--only-downloads', '--output-format', 'json', '--no-config'], cwd=cwd, env=env, capture_output=True, text=True, check=False, timeout=15)
        ...
        for entry in entries:
            ...
            try:
                versions.append((int(parts['major']), int(parts['minor']), int(parts['patch'])))
...
def _attempt_install_generation(uv_bin: str, request: str, *, project_root: Path, python_root: Path, current: SQLiteRuntimeInfo) -> tuple[Path, Path, SQLiteRuntimeInfo] | None:
    ...
    install = subprocess.run([uv_bin, 'python', 'install', request, '--reinstall', '--no-bin', '--no-registry', '--no-config'], cwd=project_root, env=env, capture_output=True, text=True, check=False)
    if install.returncode != 0:
        logger.warning('private Python install failed for %s (rc=%d): %s', request, install.returncode, (install.stderr or install.stdout or '').strip())
        _remove_tree(generation, boundary=python_root)
        return None
    found = subprocess.run([uv_bin, 'python', 'find', request, '--managed-python', '--no-config'], cwd=project_root, env=env, capture_output=True, text=True, check=False)
    if found.returncode != 0 or not found.stdout.strip():
        logger.warning('private Python lookup failed for %s (rc=%d): %s', request, found.returncode, (found.stderr or '').strip())
        _remove_tree(generation, boundary=python_root)
        return None
    ...
    try:
        ...
        logger.warning('uv resolved Python outside the Duck Agent generation: %s', python)
        _remove_tree(generation, boundary=python_root)
        return None
    ...
    if candidate is None:
        ...
        _remove_tree(generation, boundary=python_root)
        return None
    if candidate.python_version[:2] != current.python_version[:2] or candidate.python_version < current.python_version:
        logger.warning('candidate Python drifted off the %s minor line or downgraded: %s', '.'.join((str(p) for p in current.python_version[:2])), candidate.python_version)
        _remove_tree(generation, boundary=python_root)
        return None
    if candidate.wal_reset_vulnerable:
        logger.warning('candidate Python still links vulnerable SQLite %s (%s)', candidate.sqlite_version_string, candidate.sqlite_source_id)
        _remove_tree(generation, boundary=python_root)
        return None
    return (generation, python, candidate)

def _install_safe_python_generation(uv_bin: str, *, project_root: Path, current: SQLiteRuntimeInfo) -> tuple[Path, Path, SQLiteRuntimeInfo] | None:
    ...
    result = _attempt_install_generation(uv_bin, request, project_root=project_root, python_root=python_root, current=current)
    ...
    for version_tuple in patches:
        ...
        attempts += 1
        result = _attempt_install_generation(uv_bin, explicit_request, project_root=project_root, python_root=python_root, current=current)
        if result is not None:
            return result
    return None
...
def _smoke_candidate_venv(venv_dir: Path) -> tuple[bool, str, SQLiteRuntimeInfo | None]:
    ...
    check = 'import dotenv, fastapi, openai, prompt_toolkit, pydantic, rich, uvicorn, yaml\nimport hermes_state\n'
    env = dict(os.environ)
    for key in ('CONDA_DEFAULT_ENV', 'CONDA_PREFIX', 'PYTHONHOME', 'PYTHONPATH', 'UV_PROJECT_ENVIRONMENT', 'UV_PYTHON', 'VIRTUAL_ENV'):
        env.pop(key, None)
    try:
        result = subprocess.run([str(python), '-I', '-c', check], cwd=venv_dir.parent, env=env, capture_output=True, text=True, timeout=90, check=False)
...
def _stage_candidate_venv(uv_bin: str, *, project_root: Path, generation: Path, python: Path) -> Path | None:
    runtime_root = project_root / _RUNTIME_DIR_NAME
    token = f'{int(time.time())}-{os.getpid()}-{uuid.uuid4().hex[:8]}'
    candidate = runtime_root / f'venv-candidate-{token}'
    env = managed_python_env(project_root, install_dir=generation)
    env.update({'UV_PROJECT_ENVIRONMENT': str(candidate), 'UV_PYTHON': str(python), 'UV_PYTHON_DOWNLOADS': 'never', 'VIRTUAL_ENV': str(candidate)})
    print('  → Building a relocatable replacement environment...')
    created = subprocess.run([uv_bin, 'venv', str(candidate), '--python', str(python), '--managed-python', '--no-python-downloads', '--relocatable', '--no-config'], cwd=project_root, env=env, capture_output=True, text=True, check=False)
    if created.returncode != 0:
        logger.warning('candidate venv creation failed (rc=%d): %s', created.returncode, (created.stderr or created.stdout or '').strip())
    ...
    synced = subprocess.run([uv_bin, 'sync', '--extra', 'all', '--locked', '--python', str(_venv_python(candidate))], cwd=project_root, env=sync_env, check=False)
...
def _cut_over_candidate(candidate: Path, *, project_root: Path, live: Path | None=None) -> tuple[bool, Path | None, SQLiteRuntimeInfo | None, str]:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return (False, backup, None, f'could not promote the replacement venv ({promote_error}); rollback failed ({rollback_error})')
        ...
        try:
            ...
            return (False, backup, info, f'post-cutover smoke failed ({detail}); rollback failed ({exc}); rejected venv: {rejected}')
        ...
        if not live.exists() and backup.exists():
            try:
                ...
                logger.error('interrupted runtime cutover could not restore %s from %s: %s', live, backup, exc)
        raise
...
def _acquire_repair_lock(runtime_root: Path) -> _RepairLock | None:
    ...
    try:
        fd = os.open(path, os.O_CREAT | os.O_RDWR, 384)
    except OSError:
        return None
    try:
        if os.name == 'nt':
...
def _windows_runtime_holders() -> tuple[bool, str]:
    ...
    if holders:
        ...
        return (True, f'other Duck Agent processes still hold the venv (PID {pids})')
    return (False, '')

def _uv_version_string(uv_bin: str) -> str:
    """Return ``uv --version`` output, or ``""`` when it cannot be read."""
    try:
        result = subprocess.run([uv_bin, '--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', check=False, timeout=15)
    except Exception:
        return ''
    if result.returncode != 0:
        return ''
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Duck Agent-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``duck-agent update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``duck-agent update`` repairs the runtime (issue class:
...
def _sweep_stale_runtime_backups(live: Path, *, root: Path, keep: Path | None=None, min_age_seconds: float=3600.0) -> None:
...
def repair_vulnerable_runtime(uv_bin: str, *, project_root: Path | None=None, venv_dir: Path | None=None) -> RuntimeRepairResult:
    ...
    if not current.wal_reset_vulnerable:
        ...
        return RuntimeRepairResult('safe', sqlite_before=current.sqlite_version_string, sqlite_after=current.sqlite_version_string)
    blocked, detail = _windows_runtime_holders()
    if blocked:
        print(f'  ⚠ SQLite runtime repair deferred: {detail}')
        return RuntimeRepairResult('skipped', detail, sqlite_before=current.sqlite_version_string)
    ...
    try:
        ...
        if not current.wal_reset_vulnerable:
            return RuntimeRepairResult('safe', sqlite_before=current.sqlite_version_string, sqlite_after=current.sqlite_version_string)
        print(f'  ⚠ Duck Agent venv links SQLite {current.sqlite_version_string}, which has the WAL-reset bug.')
        ...
        if provisioned is None:
            if _refresh_managed_uv_catalog(uv_bin):
                ...
                provisioned = _install_safe_python_generation(uv_bin, project_root=root, current=current)
        if provisioned is None:
            return RuntimeRepairResult('failed', 'could not provision a fixed private Python runtime', sqlite_before=current.sqlite_version_string)
        generation, python, candidate_info = provisioned
        candidate = _stage_candidate_venv(uv_bin, project_root=root, generation=generation, python=python)
        if candidate is None:
            _remove_tree(generation, boundary=managed_python_install_dir(root))
            return RuntimeRepairResult('failed', 'replacement environment did not pass dependency and import smoke tests', sqlite_before=current.sqlite_version_string, sqlite_after=candidate_info.sqlite_version_string)
        cut_over, backup, final_info, cutover_detail = _cut_over_candidate(candidate, project_root=root, live=live)
        if not cut_over:
            if backup is None:
                _remove_tree(candidate, boundary=runtime_root)
                _remove_tree(generation, boundary=managed_python_install_dir(root))
            return RuntimeRepairResult('failed', cutover_detail, sqlite_before=current.sqlite_version_string, sqlite_after=final_info.sqlite_version_string if final_info is not None else '', backup_venv=backup)
        final_version = final_info.sqlite_version_string if final_info is not None else candidate_info.sqlite_version_string
        print(f'  ✓ Managed Python runtime repaired (SQLite {current.sqlite_version_string} → {final_version})')
        if backup is not None and backup.exists():
            _remove_tree(backup, boundary=root)
        return RuntimeRepairResult('repaired', sqlite_before=current.sqlite_version_string, sqlite_after=final_version, backup_venv=backup)
    finally:
...
def _install_uv(target: Path) -> None:
    ...
    ``$DUCK_AGENT_HOME/bin/`` instead of ``~/.local/bin/``.
    """
    system = platform.system()
    env = {**os.environ, 'UV_UNMANAGED_INSTALL': str(target.parent), 'UV_INSTALL_DIR': str(target.parent)}
    if system == 'Windows':
...
def _install_uv_posix(env: dict[str, str]) -> None:
    ...
    try:
        subprocess.run(['curl', '-LsSf', 'https://astral.sh/uv/install.sh', '-o', installer_path], check=True, capture_output=True)
...
def _install_uv_windows(env: dict[str, str]) -> None:
    ...
    subprocess.run(['powershell', '-ExecutionPolicy', 'Bypass', '-c', cmd], env=env, check=True, capture_output=True)
