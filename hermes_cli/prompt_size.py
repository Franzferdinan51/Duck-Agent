"""Prompt-size diagnostic: ``duck-agent prompt-size``.
...
_SKILLS_BLOCK_RE = re.compile('<available_skills>.*?</available_skills>', re.DOTALL)
_SKILL_LINE_PREFIX = '    - '
_NAMES_ONLY_LINE_RE = re.compile('^  .+ \\[names only\\]: (?P<names>.+)$')
...
def _build_inspection_agent(platform: str) -> Any:
    ...
    return AIAgent(model=model, api_key='inspect-only', base_url='https://openrouter.ai/api/v1', quiet_mode=True, save_trajectories=False, platform=platform, enabled_toolsets=enabled_toolsets, disabled_toolsets=disabled_toolsets)
    ...
    def append_entry(name: str, *, attributed_bytes: int, total_bytes: int, shared_bytes: int, skill_count: int) -> None:
        ...
        if path is not None:
            try:
                ...
                md_bytes = None
        entries.append({'name': name, 'index_line_bytes': attributed_bytes, 'index_line_total_bytes': total_bytes, 'index_line_shared_bytes': shared_bytes, 'index_line_skill_count': skill_count, 'skill_md_bytes': md_bytes, 'path': str(path) if path is not None else ''})
    for line in skills_block.splitlines():
        compact_match = _NAMES_ONLY_LINE_RE.match(line)
        if compact_match is not None:
            names = [name.strip() for name in compact_match.group('names').split(',') if name.strip()]
            if not names:
                continue
            ...
            for index, name in enumerate(names):
                ...
                append_entry(name, attributed_bytes=name_bytes[index] + shared_bytes, total_bytes=total_bytes, shared_bytes=shared_bytes, skill_count=len(names))
            continue
...
def compute_prompt_breakdown(platform: str='cli') -> Dict[str, Any]:
    ...
    sections: List[Tuple[str, int, int]] = [('stable (identity/guidance/skills)', len(stable), _bytes(stable)), ('context (AGENTS.md/cwd files)', len(context), _bytes(context)), ('volatile (memory/profile/timestamp)', len(volatile), _bytes(volatile))]
    return {'platform': platform, 'model': getattr(agent, 'model', '') or '', 'system_prompt': {'chars': len(full), 'bytes': _bytes(full)}, 'skills_index': {'chars': len(skills_index), 'bytes': _bytes(skills_index)}, 'memory': {'chars': len(memory_block), 'bytes': _bytes(memory_block)}, 'user_profile': {'chars': len(user_block), 'bytes': _bytes(user_block)}, 'tools': {'count': len(tools), 'json_bytes': _bytes(tools_json)}, 'sections': sections, 'skills_breakdown': _compute_skills_breakdown(skills_index), 'toolsets_breakdown': _compute_toolsets_breakdown(tools)}
...
def render_breakdown(data: Dict[str, Any]) -> str:
    ...
    if toolsets:
        ...
        for ts in toolsets:
            lines.append(f"    {ts['toolset']:<22} {ts['tool_count']:>5}  {ts['json_bytes']:>8,} B  ({_fmt_kb(ts['json_bytes'])})")
    skills = data.get('skills_breakdown') or []
    if skills:
        lines.append('')
        lines.append('  Skills by size (SKILL.md on-disk = read cost; index cost = attributed always-on bytes, largest first):')
...
def cmd_prompt_size(args: Any) -> None:
    """Entry point for ``duck-agent prompt-size``."""
