...
Add, remove, or reorder entries here — both `duck-agent setup` and
`duck-agent` provider-selection will pick up the change automatically.
"""
...
_HERMES_USER_AGENT = f'duck-agent-cli/{_HERMES_VERSION}'
...
OPENROUTER_MODELS: list[tuple[str, str]] = [('anthropic/claude-fable-5', ''), ('anthropic/claude-opus-5', ''), ('anthropic/claude-opus-5-fast', '2x price, higher output speed'), ('anthropic/claude-opus-4.8', ''), ('anthropic/claude-opus-4.8-fast', '2x price, higher output speed'), ('anthropic/claude-sonnet-5', ''), ('anthropic/claude-haiku-4.5', ''), ('openai/gpt-5.6-sol', ''), ('openai/gpt-5.6-sol-pro', ''), ('openai/gpt-5.6-terra', ''), ('openai/gpt-5.6-terra-pro', ''), ('openai/gpt-5.6-luna', ''), ('openai/gpt-5.6-luna-pro', ''), ('openai/gpt-5.5', ''), ('openai/gpt-5.5-pro', ''), ('openai/gpt-5.4-mini', ''), ('google/gemini-3.1-pro-preview', ''), ('google/gemini-3.6-flash', ''), ('x-ai/grok-4.5', ''), ('deepseek/deepseek-v4-pro', ''), ('deepseek/deepseek-v4-flash', ''), ('deepseek/deepseek-v4-flash-0731', 'dated snapshot of v4-flash'), ('qwen/qwen3.8-max', ''), ('moonshotai/kimi-k3', 'recommended'), ('minimax/minimax-m3', ''), ('z-ai/glm-5.2', 'default'), ('z-ai/glm-5.1', ''), ('xiaomi/mimo-v2.5-pro', ''), ('tencent/hy3', ''), ('stepfun/step-3.7-flash', ''), ('nvidia/nemotron-3-super-120b-a12b', ''), ('sakana/fugu-ultra', ''), ('openrouter/pareto-code', 'auto-routes to cheapest coder meeting openrouter.min_coding_score'), ('openrouter/elephant-alpha', 'free'), ('poolside/laguna-m.1:free', 'free'), ('tencent/hy3:free', 'free'), ('nvidia/nemotron-3-super-120b-a12b:free', 'free'), ('nvidia/nemotron-3-ultra-550b-a55b:free', 'free'), ('inclusionai/ring-2.6-1t:free', 'free')]
_openrouter_catalog_cache: list[tuple[str, str]] | None = None
VERCEL_AI_GATEWAY_MODELS: list[tuple[str, str]] = [('moonshotai/kimi-k2.6', 'recommended'), ('alibaba/qwen3.6-plus', ''), ('zai/glm-5.1', ''), ('minimax/minimax-m2.7', ''), ('anthropic/claude-sonnet-4.6', ''), ('anthropic/claude-opus-4.7', ''), ('anthropic/claude-opus-4.6', ''), ('anthropic/claude-haiku-4.5', ''), ('openai/gpt-5.4', ''), ('openai/gpt-5.4-mini', ''), ('openai/gpt-5.3-codex', ''), ('google/gemini-3.1-pro-preview', ''), ('google/gemini-3-flash', ''), ('google/gemini-3.1-flash-lite-preview', ''), ('xai/grok-4.20-reasoning', '')]
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `duck-agent model`
    flow without maintaining a separate static list.
    """
    from hermes_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
    return _add_forward_compat_models(list(DEFAULT_CODEX_MODELS))
_XAI_STATIC_FALLBACK: list[str] = ['grok-build-0.1', 'grok-4.5', 'grok-4.3', 'grok-4.20-0309-reasoning', 'grok-4.20-0309-non-reasoning', 'grok-4.20-multi-agent-0309']
_XAI_CURATED_EXTRAS: list[str] = ['grok-4.5', 'grok-composer-2.5-fast']
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Duck Agent-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $DUCK_AGENT_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Duck Agent refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {'moa': ['default'], 'nous': ['anthropic/claude-fable-5', 'anthropic/claude-opus-5', 'anthropic/claude-opus-4.8', 'anthropic/claude-sonnet-5', 'anthropic/claude-haiku-4.5', 'openai/gpt-5.6-sol', 'openai/gpt-5.6-sol-pro', 'openai/gpt-5.6-terra', 'openai/gpt-5.6-terra-pro', 'openai/gpt-5.6-luna', 'openai/gpt-5.6-luna-pro', 'openai/gpt-5.5', 'openai/gpt-5.5-pro', 'openai/gpt-5.4-mini', 'google/gemini-3.1-pro-preview', 'google/gemini-3.6-flash', 'x-ai/grok-4.5', 'deepseek/deepseek-v4-pro', 'deepseek/deepseek-v4-flash', 'deepseek/deepseek-v4-flash-0731', 'qwen/qwen3.8-max', 'moonshotai/kimi-k3', 'minimax/minimax-m3', 'z-ai/glm-5.2', 'z-ai/glm-5.1', 'xiaomi/mimo-v2.5-pro', 'tencent/hy3', 'stepfun/step-3.7-flash', 'nvidia/nemotron-3-super-120b-a12b', 'sakana/fugu-ultra'], 'openai': ['gpt-5.4', 'gpt-5.4-mini', 'gpt-5-mini', 'gpt-5.3-codex', 'gpt-5.2-codex', 'gpt-4.1', 'gpt-4o', 'gpt-4o-mini'], 'openai-api': ['gpt-5.6-sol', 'gpt-5.6-sol-pro', 'gpt-5.6-terra', 'gpt-5.6-terra-pro', 'gpt-5.6-luna', 'gpt-5.6-luna-pro', 'gpt-5.5', 'gpt-5.5-pro', 'gpt-5.4', 'gpt-5.4-mini', 'gpt-5.4-nano', 'gpt-5-mini', 'gpt-5.3-codex', 'gpt-4.1', 'gpt-4o', 'gpt-4o-mini'], 'openai-codex': _codex_curated_models(), 'xai-oauth': _xai_curated_models(), 'copilot-acp': ['copilot-acp'], 'copilot': ['gpt-5.4', 'gpt-5.4-mini', 'gpt-5-mini', 'gpt-5.3-codex', 'gpt-5.2-codex', 'gpt-4.1', 'gpt-4o', 'gpt-4o-mini', 'claude-sonnet-4.6', 'claude-sonnet-5', 'claude-sonnet-4', 'claude-sonnet-4.5', 'claude-haiku-4.5', 'gemini-3.1-pro-preview', 'gemini-3-pro-preview', 'gemini-3-flash-preview', 'gemini-2.5-pro'], 'gemini': ['gemini-3.1-pro-preview', 'gemini-3-pro-preview', 'gemini-3.6-flash', 'gemini-3.1-flash-lite-preview'], 'zai': ['glm-5.2', 'glm-5.1', 'glm-5', 'glm-5v-turbo', 'glm-5-turbo', 'glm-4.7', 'glm-4.5', 'glm-4.5-flash'], 'xai': _xai_curated_models(), 'nvidia': ['nvidia/nemotron-3-ultra-550b-a55b', 'nvidia/nemotron-3-super-120b-a12b', 'nvidia/nemotron-3-nano-omni-30b-a3b-reasoning', 'z-ai/glm-5.2', 'moonshotai/kimi-k2.6', 'minimaxai/minimax-m3'], 'kimi-coding': ['kimi-k3', 'kimi-k2.7-code', 'kimi-k2.6', 'kimi-k2.5', 'kimi-for-coding', 'kimi-for-coding-highspeed', 'kimi-k2-thinking', 'kimi-k2-thinking-turbo', 'kimi-k2-turbo-preview', 'kimi-k2-0905-preview'], 'kimi-coding-cn': ['kimi-k3', 'kimi-k2.7-code', 'kimi-k2.7-code-highspeed', 'kimi-k2.6', 'kimi-k2.5', 'kimi-k2-thinking', 'kimi-k2-turbo-preview', 'kimi-k2-0905-preview'], 'stepfun': ['step-3.5-flash', 'step-3.5-flash-2603'], 'moonshot': ['kimi-k3', 'kimi-k2.6', 'kimi-k2.5', 'kimi-k2-thinking', 'kimi-k2-turbo-preview', 'kimi-k2-0905-preview'], 'minimax': ['MiniMax-M3', 'MiniMax-M2.7', 'MiniMax-M2.5', 'MiniMax-M2.1', 'MiniMax-M2'], 'minimax-oauth': ['MiniMax-M3', 'MiniMax-M2.7', 'MiniMax-M2.7-highspeed'], 'minimax-cn': ['MiniMax-M3', 'MiniMax-M2.7', 'MiniMax-M2.5', 'MiniMax-M2.1', 'MiniMax-M2'], 'anthropic': ['claude-fable-5', 'claude-sonnet-5', 'claude-opus-4-8', 'claude-opus-4-7', 'claude-opus-4-6', 'claude-sonnet-4-6', 'claude-opus-4-5-20251101', 'claude-sonnet-4-5-20250929', 'claude-opus-4-20250514', 'claude-sonnet-4-20250514', 'claude-haiku-4-5-20251001'], 'deepseek': ['deepseek-v4-pro', 'deepseek-v4-flash'], 'xiaomi': ['mimo-v2.5-pro', 'mimo-v2.5', 'mimo-v2-pro', 'mimo-v2-omni', 'mimo-v2-flash'], 'tencent-tokenhub': ['hy3-preview'], 'arcee': ['trinity-large-thinking', 'trinity-large-preview', 'trinity-mini'], 'gmi': ['zai-org/GLM-5.1-FP8', 'deepseek-ai/DeepSeek-V3.2', 'moonshotai/Kimi-K2.5', 'google/gemini-3.1-flash-lite-preview', 'anthropic/claude-sonnet-5', 'anthropic/claude-sonnet-4.6', 'openai/gpt-5.4'], 'opencode-zen': ['kimi-k2.5', 'kimi-k2.6', 'gpt-5.5', 'gpt-5.5-pro', 'gpt-5.4-pro', 'gpt-5.4', 'gpt-5.4-mini', 'gpt-5.4-nano', 'gpt-5.3-codex', 'gpt-5.3-codex-spark', 'gpt-5.2', 'gpt-5.2-codex', 'gpt-5.1', 'gpt-5.1-codex', 'gpt-5.1-codex-max', 'gpt-5.1-codex-mini', 'gpt-5', 'gpt-5-codex', 'gpt-5-nano', 'claude-fable-5', 'claude-sonnet-5', 'claude-opus-4-8', 'claude-opus-4-7', 'claude-opus-4-6', 'claude-opus-4-5', 'claude-opus-4-1', 'claude-sonnet-4-6', 'claude-sonnet-4-5', 'claude-sonnet-4', 'claude-haiku-4-5', 'gemini-3.5-flash', 'gemini-3.1-pro', 'gemini-3-flash', 'minimax-m3', 'minimax-m2.7', 'minimax-m2.5', 'glm-5.2', 'glm-5.1', 'glm-5', 'kimi-k2.7-code', 'deepseek-v4-pro', 'deepseek-v4-flash', 'deepseek-v4-flash-free', 'qwen3.7-plus', 'qwen3.6-plus', 'qwen3.5-plus', 'grok-build-0.1', 'big-pickle', 'mimo-v2.5-free', 'north-mini-code-free', 'nemotron-3-ultra-free'], 'opencode-go': ['kimi-k3', 'kimi-k2.7-code', 'kimi-k2.6', 'kimi-k2.5', 'glm-5.2', 'glm-5.1', 'glm-5', 'mimo-v2.5-pro', 'mimo-v2.5', 'mimo-v2-pro', 'mimo-v2-omni', 'minimax-m3', 'minimax-m2.7', 'minimax-m2.5', 'deepseek-v4-pro', 'deepseek-v4-flash', 'qwen3.7-max', 'qwen3.7-plus', 'qwen3.6-plus', 'qwen3.5-plus'], 'kilocode': ['anthropic/claude-opus-4.6', 'anthropic/claude-sonnet-4.6', 'openai/gpt-5.4', 'google/gemini-3-pro-preview', 'google/gemini-3-flash-preview'], 'alibaba': ['qwen3.7-max', 'qwen3.7-plus', 'qwen3.6-plus', 'kimi-k2.5', 'qwen3.5-plus', 'qwen3-coder-plus', 'qwen3-coder-next', 'glm-5', 'glm-4.7', 'MiniMax-M2.5'], 'alibaba-coding-plan': ['qwen3.7-plus', 'qwen3.6-plus', 'qwen3.5-plus', 'qwen3-max-2026-01-23', 'qwen3-coder-plus', 'qwen3-coder-next', 'kimi-k2.5', 'glm-5', 'glm-4.7', 'MiniMax-M2.5'], 'huggingface': ['moonshotai/Kimi-K2.5', 'Qwen/Qwen3.5-397B-A17B', 'Qwen/Qwen3.5-35B-A3B', 'deepseek-ai/DeepSeek-V3.2', 'MiniMaxAI/MiniMax-M2.5', 'zai-org/GLM-5', 'XiaomiMiMo/MiMo-V2-Flash', 'moonshotai/Kimi-K2-Thinking', 'moonshotai/Kimi-K2.6'], 'bedrock': ['us.anthropic.claude-sonnet-5', 'us.anthropic.claude-sonnet-4-6', 'us.anthropic.claude-opus-4-6-v1', 'us.anthropic.claude-haiku-4-5-20251001-v1:0', 'us.anthropic.claude-sonnet-4-5-20250929-v1:0', 'us.amazon.nova-pro-v1:0', 'us.amazon.nova-lite-v1:0', 'us.amazon.nova-micro-v1:0', 'deepseek.v3.2', 'us.meta.llama4-maverick-17b-instruct-v1:0', 'us.meta.llama4-scout-17b-instruct-v1:0'], 'azure-foundry': [], 'vertex': ['google/gemini-3.1-pro-preview', 'google/gemini-3-pro-preview', 'google/gemini-3.6-flash', 'google/gemini-3.5-flash', 'google/gemini-3.5-flash-lite', 'google/gemini-3-flash-preview', 'google/gemini-3.1-flash-lite-preview', 'google/gemini-3.1-flash-lite'], 'novita': ['moonshotai/kimi-k2.5', 'minimax/minimax-m2.7', 'zai-org/glm-5', 'deepseek/deepseek-v3-0324', 'deepseek/deepseek-r1-0528', 'qwen/qwen3-235b-a22b-fp8']}
...
def union_with_portal_free_recommendations(curated_ids: list[str], pricing: dict[str, dict[str, str]], portal_base_url: str='', *, force_refresh: bool=False) -> tuple[list[str], dict[str, dict[str, str]]]:
    ...
    older Duck Agent that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(curated_ids: list[str], pricing: dict[str, dict[str, str]], portal_base_url: str='', *, force_refresh: bool=False) -> tuple[list[str], dict[str, dict[str, str]]]:
    ...
    picker even if the user is running an older Duck Agent that doesn't ship
...
def _write_nous_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).debug('nous recommended-models disk cache write failed: %s', exc)
...
def fetch_nous_recommended_models(portal_base_url: str='', timeout: float=5.0, *, force_refresh: bool=False) -> dict[str, Any]:
    ...
    (``$DUCK_AGENT_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
PROVIDER_GROUPS: dict[str, tuple[str, str, list[str]]] = {'kimi': ('Kimi / Moonshot', 'Coding Plan, Moonshot global & China endpoints', ['kimi-coding', 'kimi-coding-cn']), 'minimax': ('MiniMax', 'Global, OAuth Coding Plan & China endpoints', ['minimax', 'minimax-oauth', 'minimax-cn']), 'xai': ('xAI Grok', 'Direct API or SuperGrok / Premium+ OAuth', ['xai', 'xai-oauth']), 'google': ('Google Gemini', 'Google AI Studio (API key)', ['gemini']), 'openai': ('OpenAI', 'Codex CLI or direct OpenAI API', ['openai-codex', 'openai-api']), 'qwen': ('Qwen', 'Qwen Cloud / DashScope, Coding Plan & Qwen CLI OAuth', ['alibaba', 'alibaba-coding-plan', 'qwen-oauth']), 'opencode': ('OpenCode', 'Zen pay-as-you-go or Go subscription', ['opencode-zen', 'opencode-go']), 'copilot': ('GitHub Copilot', 'GitHub token API or copilot --acp process', ['copilot', 'copilot-acp'])}
_SLUG_TO_GROUP: dict[str, str] = {slug: gid for gid, (_label, _desc, members) in PROVIDER_GROUPS.items() for slug in members}
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``duck-agent model``, the
    ...
    for slug in slugs:
        ...
        if len(members) <= 1:
            ...
            rows.append({'kind': 'group', 'group_id': gid, 'label': label, 'description': desc, 'members': list(members)})
    return rows
_PROVIDER_ALIASES = {'glm': 'zai', 'z-ai': 'zai', 'z.ai': 'zai', 'zhipu': 'zai', 'github': 'copilot', 'github-copilot': 'copilot', 'github-models': 'copilot', 'github-model': 'copilot', 'github-copilot-acp': 'copilot-acp', 'copilot-acp-agent': 'copilot-acp', 'google': 'gemini', 'google-gemini': 'gemini', 'google-ai-studio': 'gemini', 'google-vertex': 'vertex', 'vertex-ai': 'vertex', 'gcp-vertex': 'vertex', 'vertexai': 'vertex', 'kimi': 'kimi-coding', 'moonshot': 'kimi-coding', 'kimi-cn': 'kimi-coding-cn', 'moonshot-cn': 'kimi-coding-cn', 'step': 'stepfun', 'stepfun-coding-plan': 'stepfun', 'arcee-ai': 'arcee', 'arceeai': 'arcee', 'gmi-cloud': 'gmi', 'gmicloud': 'gmi', 'fireworks-ai': 'fireworks', 'fw': 'fireworks', 'actual-computer': 'actual', 'actualcomputer': 'actual', 'aci': 'actual', 'minimax-china': 'minimax-cn', 'minimax_cn': 'minimax-cn', 'minimax-portal': 'minimax-oauth', 'minimax-global': 'minimax-oauth', 'minimax_oauth': 'minimax-oauth', 'claude': 'anthropic', 'claude-code': 'anthropic', 'deep-seek': 'deepseek', 'opencode': 'opencode-zen', 'zen': 'opencode-zen', 'go': 'opencode-go', 'opencode-go-sub': 'opencode-go', 'aigateway': 'ai-gateway', 'vercel': 'ai-gateway', 'vercel-ai-gateway': 'ai-gateway', 'kilo': 'kilocode', 'kilo-code': 'kilocode', 'kilo-gateway': 'kilocode', 'dashscope': 'alibaba', 'aliyun': 'alibaba', 'qwen': 'alibaba', 'alibaba-cloud': 'alibaba', 'qwen-portal': 'qwen-oauth', 'hf': 'huggingface', 'hugging-face': 'huggingface', 'huggingface-hub': 'huggingface', 'novita-ai': 'novita', 'novitaai': 'novita', 'mimo': 'xiaomi', 'xiaomi-mimo': 'xiaomi', 'tencent': 'tencent-tokenhub', 'tokenhub': 'tencent-tokenhub', 'tencent-cloud': 'tencent-tokenhub', 'tencentmaas': 'tencent-tokenhub', 'aws': 'bedrock', 'aws-bedrock': 'bedrock', 'amazon-bedrock': 'bedrock', 'amazon': 'bedrock', 'grok': 'xai', 'grok-oauth': 'xai-oauth', 'xai-oauth': 'xai-oauth', 'x-ai-oauth': 'xai-oauth', 'xai-grok-oauth': 'xai-oauth', 'x-ai': 'xai', 'x.ai': 'xai', 'nim': 'nvidia', 'nvidia-nim': 'nvidia', 'build-nvidia': 'nvidia', 'nemotron': 'nvidia', 'lmstudio': 'lmstudio', 'lm-studio': 'lmstudio', 'lm_studio': 'lmstudio', 'ollama': 'custom', 'ollama_cloud': 'ollama-cloud'}
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``duck-agent auth add openai-codex`` without
    ``duck-agent model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``duck-agent model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    duck-agent is tool-calling-first — every provider path assumes the model
...
def fetch_ai_gateway_models(timeout: float=8.0, *, force_refresh: bool=False) -> list[tuple[str, str]]:
    ...
    preferred_ids = [mid for mid, _ in fallback]
    try:
        req = urllib.request.Request(f"{AI_GATEWAY_BASE_URL.rstrip('/')}/models", headers={'Accept': 'application/json'})
    ...
    if not curated:
        return list(_ai_gateway_catalog_cache or fallback)
    free_moonshot = next((mid for mid, item in live_by_id.items() if mid.startswith('moonshotai/') and _ai_gateway_model_is_free(item.get('pricing'))), None)
    if free_moonshot:
...
def compute_sale_discount(prompt: str, completion: str, original: Any) -> tuple[int, str, str] | None:
    ...
    if cur_prompt is not None and orig_prompt is not None and (cur_prompt < orig_prompt):
        ...
        if pct < 1:
            return None
        return (pct, str(was_prompt), str(was_completion) if was_completion not in (None, '') else '')
...
def fetch_models_with_pricing(api_key: str | None=None, base_url: str='https://openrouter.ai/api', timeout: float=8.0, *, force_refresh: bool=False, include_sale_original: bool=False) -> dict[str, dict[str, Any]]:
    ...
    for item in payload.get('data', []):
        ...
        if mid and isinstance(pricing, dict):
            entry: dict[str, Any] = {'prompt': str(pricing.get('prompt', '')), 'completion': str(pricing.get('completion', ''))}
...
def fetch_ai_gateway_pricing(timeout: float=8.0, *, force_refresh: bool=False) -> dict[str, dict[str, str]]:
    """Fetch Vercel AI Gateway /v1/models and return duck-agent-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; duck-agent's picker expects
    ...
    if normalized == 'nous':
        ...
        if base_url:
            ...
            if stripped.endswith('/v1'):
                stripped = stripped[:-3]
            return fetch_models_with_pricing(api_key=api_key, base_url=stripped, force_refresh=force_refresh, include_sale_original=True)
    return {}
    ...
    try:
        ...
        for mid, entry in models.items():
            ...
            if inp is None and out is None:
                continue
            row: dict[str, str] = {'prompt': str(float(inp or 0) / 1000000), 'completion': str(float(out or 0) / 1000000)}
            cache_read = cost.get('cache_read')
            if cache_read:
                row['input_cache_read'] = str(float(cache_read) / 1000000)
            result[str(mid)] = row
    except Exception:
        result = {}
    _pricing_cache[cache_key] = result
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``duck-agent model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        try:
            ...
            pass
        result.append({'id': pid, 'label': label, 'aliases': alias_list, 'authenticated': has_creds})
    return result
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:duck-agent-3                           →  ("nous", "duck-agent-3")
...
_PROVIDER_RETIRED_ALIASES: dict[str, tuple[str, ...]] = {'deepseek': ('deepseek-chat', 'deepseek-reasoner')}
...
def _model_in_provider_catalog(name_lower: str, providers: set[str]) -> bool:
    return any((name_lower == model.lower() for provider in providers for model in _provider_catalog_names(provider)))
...
def _resolve_static_model_alias(name_lower: str, current_keys: set[str]) -> Optional[tuple[str, str]]:
    ...
    def _match(provider: str) -> Optional[str]:
        ...
        if not models:
            return None
        prefix = (f'{vendor}/{family}' if provider in _AGGREGATOR_PROVIDERS else family).lower()
        for model in models:
    ...
    for provider in _PROVIDER_MODELS:
        if provider in current_keys or provider in _AGGREGATOR_PROVIDERS or provider in _BORROWED_MODEL_PROVIDERS:
            continue
        if (matched := _match(provider)):
            return (provider, matched)
...
def detect_static_provider_for_model(model_name: str, current_provider: str) -> Optional[tuple[str, str]]:
    ...
    if _model_in_provider_catalog(name_lower, current_keys):
        return None
    _is_custom_current = current_provider == 'custom' or current_provider.startswith('custom:')
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Duck Agent' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Duck Agent should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``duck-agent auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.duck-agent/.env``.
    ...
    try:
        ...
        pass
    return ''
_MODELS_DEV_PREFERRED: frozenset[str] = frozenset({'opencode-go', 'opencode-zen', 'deepseek', 'kilocode', 'fireworks', 'mistral', 'togetherai', 'cohere', 'perplexity', 'groq', 'nvidia', 'huggingface', 'zai', 'gemini', 'google'})
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool=False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Duck Agent release.
    """
    normalized = normalize_provider(provider)
    if normalized == 'openrouter':
    ...
    if normalized == 'custom':
        ...
        if base_url:
            model_cfg = _get_model_config_dict()
            api_key = str(model_cfg.get('api_key', '') or '').strip() or os.getenv('CUSTOM_API_KEY', '') or os.getenv('OPENAI_API_KEY', '') or os.getenv('OPENROUTER_API_KEY', '')
    ...
    try:
        ...
        if _p and _p.auth_type == 'api_key' and _p.base_url:
            ...
            if api_key:
                ...
                if live:
                    curated = list(_PROVIDER_MODELS.get(normalized, [])) or list(_p.fallback_models or ())
                    if curated:
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    def _default_refresh():
        ...
        if not live:
            return None
        return {'fp': _credential_fingerprint(cache_key), 'at': time.time(), 'models': list(live)}
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$DUCK_AGENT_HOME/auth.json`` and external
    ...
    try:
        ...
        pass
    for path in (_os.path.expanduser('~/.codex/auth.json'), _os.path.expanduser('~/.claude/.credentials.json'), _os.path.expanduser('~/.config/github-copilot/hosts.json'), _os.path.expanduser('~/.minimax/credentials.json')):
        try:
...
def _fetch_anthropic_models(timeout: float=5.0, *, base_url: Optional[str]=None, api_key: Optional[str]=None) -> Optional[list[str]]:
    ...
    def _do_request(h: dict[str, str]):
        req = urllib.request.Request(_anthropic_models_url(base_url), headers=h)
        with _urlopen_model_catalog_request(req, timeout=timeout) as resp:
...
def copilot_default_headers(*, is_agent_turn: bool=True) -> dict[str, str]:
    ...
    try:
        ...
        return {'Editor-Version': COPILOT_EDITOR_VERSION, 'User-Agent': 'HermesAgent/1.0', 'Openai-Intent': 'conversation-edits', 'x-initiator': 'agent' if is_agent_turn else 'user'}
...
def fetch_github_model_catalog(api_key: Optional[str]=None, timeout: float=5.0) -> Optional[list[dict[str, Any]]]:
    ...
    if _github_model_catalog_cache is not None and _github_model_catalog_cache_key == api_key and (time.monotonic() - _github_model_catalog_cache_time < _GITHUB_MODEL_CATALOG_CACHE_TTL):
...
_COPILOT_CONTEXT_CACHE_TTL = 3600
...
def _is_github_models_base_url(base_url: Optional[str]) -> bool:
    ...
    return normalized.startswith(COPILOT_BASE_URL) or normalized.startswith('https://models.github.ai/inference') or normalized.startswith('https://models.inference.ai.azure.com')
...
def _lmstudio_fetch_raw_models(api_key: Optional[str]=None, base_url: Optional[str]=None, timeout: float=5.0) -> Optional[list[dict]]:
    ...
    try:
        ...
        if exc.code in {401, 403}:
            ...
            raise AuthError(f'LM Studio rejected the request with HTTP {exc.code}.', provider='lmstudio', code='auth_rejected') from exc
        import logging
        logging.getLogger(__name__).debug('LM Studio probe at %s failed with HTTP %s', server_root, exc.code)
        return None
    except Exception as exc:
        import logging
        logging.getLogger(__name__).debug('LM Studio probe at %s failed: %s', server_root, exc)
        return None
    raw_models = payload.get('models') if isinstance(payload, dict) else None
    if not isinstance(raw_models, list):
        import logging
        logging.getLogger(__name__).debug('LM Studio probe at %s returned malformed payload (no `models` list)', server_root)
        return None
    return raw_models
...
def ensure_lmstudio_model_loaded(model: str, base_url: Optional[str], api_key: Optional[str], target_context_length: Optional[int], timeout: float=120.0, *, return_load_result: bool=False) -> Optional[int] | LMStudioLoadResult:
    ...
    def _result(context_length: Optional[int], *, load_attempted: bool=False, rejected: bool=False) -> Optional[int] | LMStudioLoadResult:
...
_COPILOT_MODEL_ALIASES = {'openai/gpt-5': 'gpt-5-mini', 'openai/gpt-5-chat': 'gpt-5-mini', 'openai/gpt-5-mini': 'gpt-5-mini', 'openai/gpt-5-nano': 'gpt-5-mini', 'openai/gpt-4.1': 'gpt-4.1', 'openai/gpt-4.1-mini': 'gpt-4.1', 'openai/gpt-4.1-nano': 'gpt-4.1', 'openai/gpt-4o': 'gpt-4o', 'openai/gpt-4o-mini': 'gpt-4o-mini', 'openai/o1': 'gpt-5.2', 'openai/o1-mini': 'gpt-5-mini', 'openai/o1-preview': 'gpt-5.2', 'openai/o3': 'gpt-5.3-codex', 'openai/o3-mini': 'gpt-5-mini', 'openai/o4-mini': 'gpt-5-mini', 'anthropic/claude-opus-4.6': 'claude-opus-4.6', 'anthropic/claude-sonnet-5': 'claude-sonnet-5', 'anthropic/claude-sonnet-4.6': 'claude-sonnet-4.6', 'anthropic/claude-sonnet-4': 'claude-sonnet-4', 'anthropic/claude-sonnet-4.5': 'claude-sonnet-4.5', 'anthropic/claude-haiku-4.5': 'claude-haiku-4.5', 'claude-sonnet-5': 'claude-sonnet-5', 'claude-opus-4-6': 'claude-opus-4.6', 'claude-sonnet-4-6': 'claude-sonnet-4.6', 'claude-sonnet-4-0': 'claude-sonnet-4', 'claude-sonnet-4-5': 'claude-sonnet-4.5', 'claude-haiku-4-5': 'claude-haiku-4.5', 'anthropic/claude-opus-4-6': 'claude-opus-4.6', 'anthropic/claude-sonnet-5': 'claude-sonnet-5', 'anthropic/claude-sonnet-4-6': 'claude-sonnet-4.6', 'anthropic/claude-sonnet-4-0': 'claude-sonnet-4', 'anthropic/claude-sonnet-4-5': 'claude-sonnet-4.5', 'anthropic/claude-haiku-4-5': 'claude-haiku-4.5'}
...
def probe_api_models(api_key: Optional[str], base_url: Optional[str], timeout: float=5.0, api_mode: Optional[str]=None, request_headers: Optional[dict[str, str]]=None) -> dict[str, Any]:
    ...
    if _is_github_models_base_url(normalized):
        ...
        return {'models': models, 'probed_url': COPILOT_MODELS_URL, 'resolved_base_url': COPILOT_BASE_URL, 'suggested_base_url': None, 'used_fallback': False}
    ...
    if urllib.parse.urlparse(normalized).hostname == 'generativelanguage.googleapis.com':
        headers['X-Goog-Api-Client'] = f'duck-agent/{_HERMES_VERSION}'
    ...
    for candidate_base, is_fallback in candidates:
        ...
        try:
            with _urlopen_model_catalog_request(req, timeout=timeout) as resp:
                data = json.loads(resp.read().decode())
                return {'models': [m.get('id', '') for m in data.get('data', [])], 'probed_url': url, 'resolved_base_url': candidate_base.rstrip('/'), 'suggested_base_url': alternate_base if alternate_base != candidate_base else normalized, 'used_fallback': is_fallback}
        except Exception:
            continue
    return {'models': None, 'probed_url': tried[0] if tried else normalized.rstrip('/') + '/models', 'resolved_base_url': normalized, 'suggested_base_url': alternate_base if alternate_base != normalized else None, 'used_fallback': False}
_DEEPINFRA_EXCLUDE_RE = re.compile('(?i)(embed|rerank|whisper|stable-diffusion|flux|sdxl|tts|bark|speech|image-gen|clip|vit-|dpt-)')
_DEEPINFRA_SURFACE_TAGS: frozenset[str] = frozenset({'chat', 'embed', 'image-gen', 'tts', 'stt', 'video-gen'})
_DEEPINFRA_DEFAULT_BASE_URL = 'https://api.deepinfra.com/v1/openai'
_DEEPINFRA_MODELS_QUERY = 'filter=true&sort_by=duck-agent'
_deepinfra_catalog_cache: dict[str, list[dict]] = {}
_deepinfra_catalog_neg_cache: dict[str, float] = {}
_DEEPINFRA_CATALOG_NEG_TTL = 60.0
...
def _fetch_deepinfra_catalog(*, timeout: float=5.0, force_refresh: bool=False) -> Optional[list[dict]]:
...
def _fetch_deepinfra_models_by_tag(tag: str, *, timeout: float=5.0, force_refresh: bool=False) -> Optional[list[dict]]:
...
def _fetch_deepinfra_models(timeout: float=5.0, *, force_refresh: bool=False) -> Optional[list[str]]:
...
def _fetch_deepinfra_pricing(timeout: float=5.0, *, force_refresh: bool=False) -> dict[str, dict[str, str]]:
    ...
    for item in items:
        ...
        if inp is not None:
            entry['prompt'] = str(float(inp) / 1000000)
        if out is not None:
            entry['completion'] = str(float(out) / 1000000)
        if cache_read is not None:
            entry['input_cache_read'] = str(float(cache_read) / 1000000)
        if entry:
...
def _fetch_ai_gateway_models(timeout: float=5.0) -> Optional[list[str]]:
    ...
    headers: dict[str, str] = {'Authorization': f'Bearer {api_key}', 'User-Agent': _HERMES_USER_AGENT}
    req = urllib.request.Request(url, headers=headers)
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            data = json.loads(resp.read().decode())
            return [m['id'] for m in data.get('data', []) if m.get('id') and m.get('type') == 'language' and ('tool-use' in (m.get('tags') or []))]
    except Exception:
        return None
...
def _custom_endpoint_fingerprint(api_key: Optional[str], api_mode: Optional[str], headers: Optional[dict[str, str]]) -> str:
...
def _cache_entry_valid(entry: Any, fp: str) -> 'TypeGuard[dict[str, Any]]':
    ...
    """
    return isinstance(entry, dict) and entry.get('fp') == fp and isinstance(entry.get('models'), list) and bool(entry['models']) and isinstance(entry.get('at'), (int, float)) and (not isinstance(entry.get('at'), bool))

def cached_fetch_api_models(api_key: Optional[str], base_url: Optional[str], *, timeout: float=5.0, api_mode: Optional[str]=None, headers: Optional[dict[str, str]]=None, force_refresh: bool=False, ttl_seconds: int=_PROVIDER_MODELS_CACHE_TTL) -> Optional[list[str]]:
...
def validate_requested_model(model_name: str, provider: Optional[str], *, api_key: Optional[str]=None, base_url: Optional[str]=None, api_mode: Optional[str]=None) -> dict[str, Any]:
    ...
    if normalized == 'copilot':
        requested_for_lookup = normalize_copilot_model_id(requested, api_key=api_key) or requested
    if not requested:
    ...
    if normalized == 'moa':
        try:
            ...
            return {'accepted': False, 'persist': False, 'recognized': False, 'message': f'MoA preset `{requested}` was not found. Run `duck-agent moa list`.'}
        except Exception as exc:
            return {'accepted': False, 'persist': False, 'recognized': False, 'message': f'Could not read MoA presets: {exc}'}
    ...
    if normalized == 'lmstudio':
        ...
        try:
            ...
            return {'accepted': False, 'persist': False, 'recognized': False, 'message': f"{exc} Set `LM_API_KEY` (or update it) to match the server's bearer token."}
        if models is None:
            return {'accepted': False, 'persist': False, 'recognized': False, 'message': f"Could not reach LM Studio's `/api/v1/models` to validate `{requested}`."}
        if not models:
            return {'accepted': False, 'persist': False, 'recognized': False, 'message': f'LM Studio is reachable but no chat-capable models are loaded. Load `{requested}` in LM Studio (Developer tab → Load Model) and try again.'}
        if requested_for_lookup in set(models):
            return {'accepted': True, 'persist': True, 'recognized': True, 'message': None}
        return {'accepted': False, 'persist': False, 'recognized': False, 'message': f"Model `{requested}` was not found in LM Studio's model listing."}
    if normalized == 'custom' or normalized.startswith('custom:'):
        ...
        if api_models is not None:
            ...
            message = f"Note: `{requested}` was not found in this custom endpoint's model listing ({probe.get('probed_url')}). It may still work if the server supports hidden or aliased models.{suggestion_text}"
            if probe.get('used_fallback'):
                message += f"\n  Endpoint verification succeeded after trying `{probe.get('resolved_base_url')}`. Consider saving that as your base URL."
            return {'accepted': True, 'persist': True, 'recognized': False, 'message': message}
        message = f"Note: could not reach this custom endpoint's model listing at `{probe.get('probed_url')}`. Duck Agent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        if api_mode == 'anthropic_messages':
        ...
        return {'accepted': api_mode == 'anthropic_messages', 'persist': True, 'recognized': False, 'message': message}
    if normalized in {'openai-codex', 'xai-oauth'}:
        ...
        if catalog_models:
            ...
            _family_prefixes = {'openai-codex': ('gpt-', 'codex-', 'o1', 'o3', 'o4'), 'xai-oauth': ('grok-',)}.get(normalized, ())
            _lower = requested_for_lookup.strip().lower()
            _plausible = not _family_prefixes or any((_lower.startswith(p) for p in _family_prefixes))
            if not _plausible:
                return {'accepted': False, 'persist': False, 'recognized': False, 'message': f"`{requested}` doesn't look like a {provider_label} model and isn't in its listing, so it was not accepted. If it belongs to another configured provider, switch with `--provider <slug>` (or select it from the `/model` picker).{suggestion_text}"}
            return {'accepted': True, 'persist': True, 'recognized': False, 'message': f'Note: `{requested}` was not found in the {provider_label} model listing. It may still work if your account has access to a newer or hidden model ID.{suggestion_text}'}
    if normalized in {'minimax', 'minimax-cn'}:
        ...
        if catalog_models:
            ...
            return {'accepted': True, 'persist': True, 'recognized': False, 'message': f'Note: `{requested}` was not found in the MiniMax catalog.{suggestion_text}\n  MiniMax does not expose a /models endpoint, so Duck Agent cannot verify the model name.\n  The model may still work if it exists on the server.'}
    if normalized == 'anthropic':
        anthropic_models = _fetch_anthropic_models(base_url=base_url or None, api_key=api_key or None)
        if anthropic_models is not None:
            ...
            if suggestions:
                suggestion_text = '\n  Similar models: ' + ', '.join((f'`{s}`' for s in suggestions))
            return {'accepted': True, 'persist': True, 'recognized': False, 'message': f"Note: `{requested}` was not found in Anthropic's /v1/models listing. It may still work if you have early-access or snapshot IDs.{suggestion_text}"}
    if api_mode == 'anthropic_messages':
        ...
        if api_models is not None:
            ...
            if auto:
                return {'accepted': True, 'persist': True, 'recognized': True, 'corrected_model': auto[0], 'message': f'Auto-corrected `{requested}` → `{auto[0]}`'}
        return {'accepted': True, 'persist': True, 'recognized': False, 'message': f"Note: could not verify `{requested}` against this endpoint's model listing.  Many Anthropic-compatible proxies do not implement GET /v1/models.  The model name has been accepted without verification."}
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            if not _openai_listing_is_authoritative and _model_in_provider_catalog(requested_for_lookup.lower(), _provider_keys(normalized)):
                return {'accepted': True, 'persist': True, 'recognized': True, 'message': f'Note: `{requested}` was not found in the live /v1/models listing but exists in the curated catalog — accepted.'}
        return {'accepted': False, 'persist': False, 'recognized': False, 'message': f"Model `{requested}` was not found in this provider's model listing.{suggestion_text}"}
    if normalized == 'bedrock':
        try:
            ...
            if suggestions:
                suggestion_text = '\n  Similar models: ' + ', '.join((f'`{s}`' for s in suggestions))
            return {'accepted': True, 'persist': True, 'recognized': False, 'message': f'Note: `{requested}` was not found in Bedrock model discovery for {region}. It may still work with custom inference profiles or cross-account access.{suggestion_text}'}
        except Exception:
            pass
    ...
    if catalog_models:
        ...
        if suggestions:
            suggestion_text = '\n  Similar models: ' + ', '.join((f'`{catalog_lower[s]}`' for s in suggestions))
        return {'accepted': True, 'persist': True, 'recognized': False, 'message': f'Note: `{requested}` was not found in the {provider_label} curated catalog and the /models endpoint was unreachable.{suggestion_text}\n  The model may still work if it exists on the provider.'}
    return {'accepted': True, 'persist': True, 'recognized': False, 'message': f"Note: could not reach the {provider_label} API to validate `{requested}`. If the service isn't down, this model may not be valid."}