"""CLI handlers for ``duck-agent egress ...``.
...
The top-level command is ``duck-agent egress``.  Note that the inbound OAuth
reverse-proxy command (``duck-agent proxy``) lives elsewhere in
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    ``duck-agent egress`` parser.
    """
    sub = parent_parser.add_subparsers(dest='egress_command')
    install = sub.add_parser('install', help=f'Download iron-proxy binary (v{ip._IRON_PROXY_VERSION})')
    install.add_argument('--force', action='store_true', help='Re-download even if a managed copy already exists')
    install.set_defaults(func=cmd_install)
    setup = sub.add_parser('setup', help='Interactive wizard: install + CA + mint tokens + write config')
    setup.add_argument('--tunnel-port', type=int, default=None, help=f'Override the tunnel port (default {ip._DEFAULT_TUNNEL_PORT})')
    setup.add_argument('--from-bitwarden', action='store_true', help='Treat secrets as managed by Bitwarden — discover provider keys from secrets.bitwarden config instead of the current env.  Fails loudly if BW is unreachable rather than silently falling back.')
    setup.add_argument('--no-bitwarden', action='store_true', help='Explicitly switch credential_source back to env on re-setup (only meaningful when the previous setup used --from-bitwarden).')
    setup.add_argument('--rotate-tokens', action='store_true', help='Mint fresh proxy tokens for every provider (default is to preserve tokens for providers that already had one — avoids 401-ing already-running sandboxes on re-setup).')
    setup.add_argument('--restart', dest='restart', action='store_true', default=None, help='If a daemon is already running, restart it automatically after writing the new config/tokens (non-interactive default on a tty is to ask).')
    setup.add_argument('--no-restart', dest='restart', action='store_false', help="Do not restart a running daemon after setup; you'll need to run `duck-agent egress restart` yourself for changes to take effect.")
    ...
    restart = sub.add_parser('restart', help='Restart the managed iron-proxy (stop if running, then start)')
    restart.set_defaults(func=cmd_restart)
    reload_p = sub.add_parser('reload', help="Hot-reload the running daemon's ruleset from proxy.yaml (management API — no restart, no dropped connections)")
    reload_p.set_defaults(func=cmd_reload)
    status = sub.add_parser('status', help='Show proxy state and mappings')
    status.add_argument('--show-tokens', action='store_true', help='Print the proxy tokens (default: redacted prefix only). Beware: tokens may persist in your shell history.')
...
def cmd_setup(args: argparse.Namespace) -> int:
    console = Console()
    console.print(Panel.fit('[bold]iron-proxy setup[/bold]\n\nRoutes outbound sandbox traffic through a local TLS-intercepting\nproxy so prompt-injected agents never see real provider API keys.\n\n[dim]Project: https://github.com/ironsh/iron-proxy  (Apache-2.0)[/dim]', border_style='cyan'))
    console.print()
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get('enabled'):
            ...
            console.print('  Run `duck-agent secrets bitwarden setup` first, or omit --from-bitwarden.')
            return 1
        try:
            from agent.secret_sources import bitwarden as bw
            access_token = os.environ.get(bw_cfg.get('access_token_env', 'BWS_ACCESS_TOKEN'), '').strip()
            if not access_token:
                console.print(f"  [red]✗ --from-bitwarden requested but {bw_cfg.get('access_token_env', 'BWS_ACCESS_TOKEN')} is not set in the environment.[/red]")
                return 1
            secrets, _ = bw.fetch_bitwarden_secrets(access_token=access_token, project_id=bw_cfg.get('project_id', ''), cache_ttl_seconds=0, use_cache=False)
            available_env_names = list(secrets.keys())
            if not available_env_names:
                console.print("  [red]✗ Bitwarden returned an empty secrets list.[/red]\n  Check the project_id in secrets.bitwarden and the BWS access-token's project scope.")
                return 1
            console.print(f'  Pulled {len(available_env_names)} env names from Bitwarden.')
        except Exception as exc:
            console.print(f'  [red]✗ Could not enumerate Bitwarden secrets: {exc}[/red]')
            console.print('  Either fix the Bitwarden config and retry, or rerun setup without --from-bitwarden (the proxy will read secrets from the host process env at start time).')
            return 1
    else:
        loaded = _load_env_file_into_environ()
        if loaded:
            console.print(f'  [dim]Loaded {loaded} provider key name(s) from ~/.duck-agent/.env for discovery.[/dim]')
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print('[yellow]⚠[/yellow]  --rotate-tokens will invalidate proxy tokens in every running Duck Agent sandbox.  They will start 401-ing against upstreams until restarted.')
            try:
        ...
        console.print('[dim]Note: --rotate-tokens is a no-op on first-time setup (no existing tokens to rotate).[/dim]')
    ...
    if uncovered:
        ...
        console.print('  [dim]These providers use request signing or SDK-minted OAuth (SigV4, service-account files) and will hold real credentials inside the sandbox.  Egress isolation is INCOMPLETE for these.[/dim]')
    ...
    if args.tunnel_port is not None:
        if args.tunnel_port < 1 or args.tunnel_port > 65534:
            console.print('  [red]✗ --tunnel-port must be between 1 and 65534 (the plain-HTTP listener uses port+1).[/red]')
            return 1
    ...
    iron_cfg = ip.build_proxy_config(mappings=mappings, ca_cert=ca_crt, ca_key=ca_key, tunnel_port=tunnel_port, audit_log=audit_log_path, allowed_hosts=allowed, upstream_deny_cidrs=deny_cidrs)
    ...
    if audit_log_ok:
        console.print(f'  [green]✓[/green] audit log: {audit_log_path} [dim](reserved — not written by iron-proxy v0.39; per-request records land in iron-proxy.log)[/dim]')
    ...
    if args.from_bitwarden:
        ...
        console.print('[dim]Keeping credential_source=bitwarden from existing config. Pass --no-bitwarden to switch to env-based credentials.[/dim]')
    else:
        proxy_cfg['credential_source'] = 'env'
    save_config(cfg)
    ...
    if restart_pref is True:
        ...
        if _sys.stdin.isatty():
            try:
                ans = input('  Restart the running proxy now with the new config? [Y/n] ').strip().lower()
            except EOFError:
                ans = ''
    ...
    if do_restart:
        try:
            ...
            console.print('  Run [cyan]duck-agent egress start[/cyan] manually before launching new Docker sandboxes.')
        else:
            listening = 'listening' if new_status.listening else 'not yet listening'
            verb = 'restarted' if was_running else 'started'
            console.print(f'  [green]✓[/green] {verb} iron-proxy with the new config (pid={new_status.pid}, port={new_status.tunnel_port}, {listening})')
    elif was_running:
        console.print('  [yellow]⚠ stopped the running iron-proxy; config or tokens changed.  Run [cyan]duck-agent egress restart[/cyan] (or [cyan]start[/cyan]) before launching new Docker sandboxes.[/yellow]')
    console.print()
    console.print('[green]✓ iron-proxy is configured.[/green]  Sandboxes will route outbound traffic through it.')
    console.print('  Start:   [cyan]duck-agent egress start[/cyan]\n  Restart: [cyan]duck-agent egress restart[/cyan]  (after any re-setup)\n  Reload:  [cyan]duck-agent egress reload[/cyan]   (apply ruleset edits in-place, no restart)\n  Status:  [cyan]duck-agent egress status[/cyan]\n  Stop:    [cyan]duck-agent egress stop[/cyan]\n  Disable: [cyan]duck-agent egress disable[/cyan]')
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get('enabled'):
        console.print('[yellow]proxy.enabled is false — run `duck-agent egress setup` first.[/yellow]')
        return 1
    ...
    if credential_source == 'bitwarden' and (not refresh_bw):
        if bool(proxy_cfg.get('allow_env_fallback', False)):
            console.print('[yellow]⚠ credential_source=bitwarden but secrets.bitwarden is disabled or missing — falling back to host-env secrets (allow_env_fallback=true).  Rotated Bitwarden keys will NOT propagate.[/yellow]')
        else:
            console.print("[red]✗ Refusing to start: proxy.credential_source is 'bitwarden' but secrets.bitwarden is disabled or missing.[/red]")
            console.print('  Re-enable it (`secrets.bitwarden.enabled: true`), switch back to env credentials with `duck-agent egress setup --no-bitwarden`, or set `proxy.allow_env_fallback: true` to opt into the host-env fallback.')
            return 1
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, '').strip():
            console.print(f'[red]✗ Refusing to start: credential_source=bitwarden but {bw_access_env} is not set in the environment.[/red]')
            console.print('  Either export the access token, or run `duck-agent egress setup --no-bitwarden` to switch back to env-based credentials.')
            return 1
        if not (bw_cfg or {}).get('project_id'):
            console.print('[red]✗ Refusing to start: credential_source=bitwarden but secrets.bitwarden.project_id is empty.[/red]')
            console.print('  Run `duck-agent secrets bitwarden setup` to configure the project, or switch back via `duck-agent egress setup --no-bitwarden`.')
            return 1
    try:
        status = ip.start_proxy(install_if_missing=bool(proxy_cfg.get('auto_install', True)), refresh_secrets_from_bitwarden=refresh_bw, bitwarden_config=bw_cfg)
    except Exception as exc:
    ...
    if status.pid:
        ...
        console.print(f'[green]✓[/green] iron-proxy running  pid={status.pid}  port={status.tunnel_port}  {listening}')
    else:
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``duck-agent egress restart`` instead: the daemon reads real credentials
    ...
    console.print('[dim]Note: new upstream secrets (rotated keys, new providers) still need `duck-agent egress restart` — the daemon reads real credentials from its environment at spawn time.[/dim]')
    return 0
...
def format_status_text(*, show_tokens: bool=False) -> str:
    ...
    if bool(proxy_cfg.get('enabled')) and (not status.configured):
        lines.extend(['', 'Next: run `duck-agent egress setup` to mint tokens and write proxy.yaml.'])
    elif bool(proxy_cfg.get('enabled')) and (not (status.pid and status.listening)):
        lines.extend(['', 'Next: run `duck-agent egress start` before launching Docker sandboxes.'])
    return '\n'.join(lines)
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if mappings:
        ...
        if args.show_tokens:
            console.print('[yellow]⚠[/yellow]  proxy tokens just printed in full — they may persist in your shell history.  Consider clearing it after this command.')
    uncovered = ip.discover_uncovered_providers()
    if uncovered:
        console.print()
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print('  iron-proxy is still running — stop it with [cyan]duck-agent egress stop[/cyan] if you want it down too.')
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print('[yellow](no config generated — run `duck-agent egress setup`)[/yellow]')
        return 1
    console.print(str(status.config_path))
    return 0

def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.duck-agent/.env`` into ``os.environ``.

    ``duck-agent egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.duck-agent/.env`` (which the agent
