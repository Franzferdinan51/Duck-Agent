"""CLI handlers for ``duck-agent secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    ``duck-agent secrets`` parser.
    """
    sub = parent_parser.add_subparsers(dest='secrets_bw_command')
    setup = sub.add_parser('setup', help='Interactive wizard: install bws, store access token, pick project')
    setup.add_argument('--project-id', help='Pre-select a project UUID instead of prompting')
    setup.add_argument('--access-token', help='Provide the access token non-interactively (will be stored in .env)')
    setup.add_argument('--server-url', help='Bitwarden region / self-hosted endpoint. Examples: https://vault.bitwarden.com (US, default), https://vault.bitwarden.eu (EU), or your self-hosted URL. Skips the interactive region prompt.')
    setup.set_defaults(func=cmd_setup)
    status = sub.add_parser('status', help='Show config + binary + token validation status')
    status.set_defaults(func=cmd_status)
    token = sub.add_parser('token', help='Rotate the access token: validate a new one and store it in .env')
    token.add_argument('--access-token', help='Provide the new token non-interactively (default: masked prompt)')
    token.add_argument('--no-verify', action='store_true', help='Store without probing Bitwarden first (not recommended)')
    token.set_defaults(func=cmd_token)
    sync = sub.add_parser('sync', help='Fetch secrets now and report what changed')
    sync.add_argument('--apply', action='store_true', help="Actually export the secrets into the current shell's env (default: dry-run)")
    sync.set_defaults(func=cmd_sync)
    disable = sub.add_parser('disable', help='Turn off the Bitwarden integration')
    disable.set_defaults(func=cmd_disable)
    install = sub.add_parser('install', help=f'Download and verify the pinned bws binary (v{bw._BWS_VERSION})')
    install.add_argument('--force', action='store_true', help='Re-download even if a managed copy already exists')
    install.set_defaults(func=cmd_install)

def cmd_setup(args: argparse.Namespace) -> int:
    console = Console()
    console.print(Panel.fit('[bold]Bitwarden Secrets Manager setup[/bold]\n\nNeed an access token? In the Bitwarden web app:\n  Secrets Manager → Machine accounts → [your account] →\n  Access tokens → Create access token\n\nCopy the token (starts with [cyan]0.[/cyan]…) — it cannot be retrieved later.', border_style='cyan'))
    console.print()
    ...
    if not token.startswith('0.'):
        console.print("  [yellow]Warning: token doesn't start with '0.' — usually that means you pasted something other than a BSM access token.  Continuing anyway.[/yellow]")
    ...
    if server_url:
        ...
        console.print('  [green]✓[/green] using bws default (US Cloud, https://vault.bitwarden.com)')
    if args.project_id and args.project_id.strip():
        ...
        if not projects:
            ...
            console.print('  In the Bitwarden web app, open the machine account → Projects tab and grant it access to at least one project.')
            return 1
    ...
    try:
        secrets, warnings = bw.fetch_bitwarden_secrets(access_token=token, project_id=project_id, binary=binary, use_cache=False, server_url=server_url)
    except Exception as exc:
    ...
    save_config(cfg)
    console.print()
    console.print('[green]✓ Bitwarden Secrets Manager is enabled.[/green]  Secrets will be pulled at the start of every Duck Agent process.')
    console.print('  Status:  [cyan]duck-agent secrets bitwarden status[/cyan]\n  Refresh: [cyan]duck-agent secrets bitwarden sync[/cyan]\n  Disable: [cyan]duck-agent secrets bitwarden disable[/cyan]')
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    binary = bw.find_bws(install_if_missing=False)
    token_validation, validation_messages = _token_validation_status(enabled=enabled, binary=binary, token=token, server_url=server_url)
    ...
    table.add_row('Server URL', server_url or '[dim]default (US Cloud, https://vault.bitwarden.com)[/dim]')
    ...
    if not enabled:
        console.print('\n  Run [cyan]duck-agent secrets bitwarden setup[/cyan] to enable.')
        return 0
    if not token_set:
        console.print(f'\n  [yellow]Enabled but {token_env} is not set — Duck Agent will skip BSM and warn on next startup.[/yellow]')
    if not project_id:
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not token:
        if not sys.stdin.isatty():
            console.print('[red]No TTY — pass the token with --access-token.[/red]')
            return 1
        console.print('Create a new token in the Bitwarden web app:\n  Secrets Manager → Machine accounts → [your account] → Access tokens → Create access token\n')
        token = masked_secret_prompt(f'Paste new access token ({token_env}): ').strip()
    if not token:
        console.print('[red]Empty token, aborting.[/red]')
        return 1
    if not token.startswith('0.'):
        console.print("[yellow]Warning: token doesn't start with '0.' — usually that means you pasted something other than a BSM access token.[/yellow]")
    if not args.no_verify:
        binary = bw.find_bws(install_if_missing=True)
        if binary is None:
            console.print('[red]bws binary not available — cannot verify.  Re-run with --no-verify to store anyway.[/red]')
            return 1
        ...
        if projects is None:
            ...
            return 1
        console.print(f"[green]✓ Token accepted[/green] ({len(projects)} project{('s' if len(projects) != 1 else '')} visible).")
        project_id = str(bw_cfg.get('project_id', '') or '')
        if project_id and projects and (project_id not in {p['id'] for p in projects}):
            console.print(f'[yellow]Warning: configured project {project_id} is not visible to this machine account.  Grant it access in the Bitwarden web app or re-run `duck-agent secrets bitwarden setup` to pick a different project.[/yellow]')
    save_env_value(token_env, token)
    os.environ[token_env] = token
    bw.clear_caches()
    console.print(f'[green]✓[/green] stored in {get_env_path()} as {token_env}.  Takes effect on the next Duck Agent invocation.')
    if not bw_cfg.get('enabled'):
        console.print('[yellow]Note: the Bitwarden integration is currently disabled — run `duck-agent secrets bitwarden setup` (or set secrets.bitwarden.enabled: true) to turn it on.[/yellow]')
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    bw_cfg = (cfg.get('secrets') or {}).get('bitwarden') or {}
    if not bw_cfg.get('enabled'):
        console.print('[yellow]Bitwarden integration is disabled.  Run `duck-agent secrets bitwarden setup` first.[/yellow]')
        return 1
    token_env = bw_cfg.get('access_token_env', 'BWS_ACCESS_TOKEN')
    token = os.environ.get(token_env, '').strip()
    if not token:
    ...
    server_url = str(bw_cfg.get('server_url', '') or '').strip()
    try:
        secrets, warnings = bw.fetch_bitwarden_secrets(access_token=token, project_id=project_id, use_cache=False, server_url=server_url)
    except Exception as exc:
        console.print(f'[red]Fetch failed: {exc}[/red]')
        return 1
    if not secrets:
    ...
    for w in warnings:
        console.print(f'[yellow]warning:[/yellow] {w}')
    if not args.apply:
        console.print('\n  This was a dry-run — secrets are picked up automatically on the next [cyan]duck-agent[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export into the current shell instead.')
    else:
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    save_config(cfg)
    console.print('[green]Disabled.[/green]  Bitwarden secrets will NOT be pulled on the next Duck Agent invocation.\n  Your access token is left in .env — remove it manually if you also want to revoke the credential.')
    return 0
...
def _bws_version(binary: Path) -> str:
    try:
        res = subprocess.run([str(binary), '--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5)
        if res.returncode == 0:
...
def _token_validation_status(*, enabled: bool, binary: Optional[Path], token: str, server_url: str='') -> tuple[str, list[str]]:
    if not enabled:
    ...
    messages: list[str] = []
    if not token.startswith('0.'):
        messages.append("  [yellow]Warning: token doesn't start with '0.' — usually that means you pasted something other than a BSM access token.  Continuing anyway.[/yellow]")
    capture = io.StringIO()
...
def _list_projects(binary: Path, token: str, console: Console, *, server_url: str='') -> Optional[List[dict]]:
    ...
    try:
        res = subprocess.run([str(binary), 'project', 'list', '--output', 'json'], env=env, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=15)
    ...
    if res.returncode != 0:
        ...
        if 'invalid_client' in lowered or '400 bad request' in lowered:
            console.print("  [yellow]'invalid_client' from the US identity endpoint usually means the token is for a different Bitwarden region.  Re-run [cyan]duck-agent secrets bitwarden setup[/cyan] and pick EU or self-hosted at the region prompt, or set [cyan]secrets.bitwarden.server_url[/cyan] in config.yaml.[/yellow]")
        elif 'authorization' in lowered or 'invalid' in lowered:
            console.print('  [yellow]This usually means the access token is wrong or revoked. Double-check it in the Bitwarden web app.[/yellow]')
        return None
    try:
...
_REGION_PRESETS = [('US Cloud  (https://vault.bitwarden.com — bws default)', ''), ('EU Cloud  (https://vault.bitwarden.eu)', 'https://vault.bitwarden.eu')]

def _resolve_server_url(args: argparse.Namespace, secrets_cfg: dict, console: Console) -> Optional[str]:
    ...
    if existing:
        console.print(f'  Existing config: [cyan]{existing}[/cyan]. Press Enter to keep, or pick a different option below.')
