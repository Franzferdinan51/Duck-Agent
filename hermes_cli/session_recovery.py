"""Offline, non-destructive recovery for a damaged Duck Agent session database.
...
_CANONICAL_TABLES = ('system_prompts', 'sessions', 'messages', 'session_model_usage', 'compression_locks', 'gateway_routing', 'async_delegations')
_TOPIC_TABLES = ('telegram_dm_topic_mode', 'telegram_dm_topic_bindings')
_GENERATED_META_KEYS = frozenset({'fts_storage_version', 'fts_optimize_available', 'fts_rebuild_high_water', 'fts_rebuild_progress', 'fts_cjk_stale', 'fts_cjk_rebuild_high_water', 'fts_cjk_rebuild_progress', 'telegram_dm_topic_schema_version'})
_SIDECAR_SUFFIXES = ('', '-wal', '-shm', '-journal')
_MINIMUM_SPACE_HEADROOM = 256 * 1024 * 1024
_MAX_SALVAGE_RANGE_QUERIES = 10000
...
def _validate_paths(source_path: Path, output_path: Optional[Path]=None, work_dir: Optional[Path]=None) -> tuple[Path, Optional[Path], Path]:
    ...
    if output_path is not None:
        ...
        if output.resolve(strict=False) in protected:
            raise SessionRecoverySafetyError('The recovery output must not be the source database or one of its journal sidecars.')
    ...
    work_root = work_dir.expanduser().resolve(strict=True) if work_dir is not None else output.parent if output is not None else source.parent
...
def _disk_space_preflight(source: Path, work_root: Path, output_parent: Optional[Path]) -> dict[str, Any]:
    ...
    bundle_bytes = sum((_sidecar_path(source, suffix).stat().st_size for suffix in _SIDECAR_SUFFIXES if _sidecar_path(source, suffix).exists()))
    ...
    report: dict[str, Any] = {'source_bundle_bytes': bundle_bytes, 'estimated_output_bytes': output_allowance, 'headroom_bytes': headroom, 'work_dir': str(work_root), 'work_dir_free_bytes': work_free}
    if output_parent is None or _same_filesystem(work_root, output_parent):
        ...
        if work_free < required:
            raise SessionRecoverySafetyError(f'Not enough free disk space for a safe recovery copy: {_format_bytes(work_free)} available at {work_root}, {_format_bytes(required)} required ({_format_bytes(bundle_bytes)} source bundle + {_format_bytes(output_allowance)} output allowance + {_format_bytes(headroom)} headroom). Use --work-dir or --output on a filesystem with more free space.')
        return report
    ...
    report.update({'shared_filesystem': False, 'work_dir_required_bytes': work_required, 'output_dir': str(output_parent), 'output_dir_free_bytes': output_free, 'output_dir_required_bytes': output_required})
    shortages: list[str] = []
    if work_free < work_required:
        shortages.append(f'{work_root}: {_format_bytes(work_free)} available, {_format_bytes(work_required)} required')
    if output_free < output_required:
        shortages.append(f'{output_parent}: {_format_bytes(output_free)} available, {_format_bytes(output_required)} required')
    if shortages:
        raise SessionRecoverySafetyError('Not enough free disk space for safe recovery: ' + '; '.join(shortages) + '. Choose work/output filesystems with more free space.')
    return report
...
def _snapshot_and_inspect(source: Path, work_root: Path) -> tuple[tempfile.TemporaryDirectory[str], Path, dict[str, Any]]:
    before = _source_fingerprint(source)
    temp_dir = tempfile.TemporaryDirectory(prefix='duck-agent-session-recovery-', dir=str(work_root))
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError('The source database bundle changed while it was being copied. Stop every Duck Agent process using this profile and retry.')
        conn = sqlite3.connect(str(snapshot_source), isolation_level=None, timeout=1.0)
        try:
...
def inspect_session_database(source_path: Path, *, work_dir: Optional[Path]=None) -> dict[str, Any]:
    ...
    try:
        return {'operation': 'inspect', 'source': str(source), 'disk_space': disk_space, **inspection, 'source_unchanged': _source_fingerprint(source) == inspection['source_fingerprint']}
    finally:
        temp_dir.cleanup()

def _copy_table(source: sqlite3.Connection, destination: sqlite3.Connection, table: str, *, chunk_size: int, progress_cb: Optional[ProgressCallback], source_rows: Optional[int]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {'source_rows': source_rows, 'copied_rows': 0, 'columns': columns}
    ...
    try:
        ...
        while True:
            ...
            if progress_cb is not None:
                progress_cb({'table': table, 'copied_rows': result['copied_rows'], 'source_rows': source_rows})
    except sqlite3.DatabaseError as exc:
        result['status'] = 'partial' if result['copied_rows'] else 'failed'
        result['error'] = str(exc)
        return result
    result['status'] = 'complete' if source_rows is None or result['copied_rows'] == source_rows else 'partial'
    if result['status'] == 'partial':
        result['error'] = f"copied {result['copied_rows']} of {source_rows} readable rows"
    return result
...
def _copy_table_salvage(source: sqlite3.Connection, destination: sqlite3.Connection, table: str, *, chunk_size: int, progress_cb: Optional[ProgressCallback], source_rows: Optional[int], insert_prefix: str='INSERT', row_filter: Optional[Callable[[tuple[Any, ...], tuple[str, ...]], bool]]=None) -> dict[str, Any]:
    """Best-effort rowid-range copy that continues past damaged source pages."""
    source_columns = _table_columns(source, table)
    destination_columns = _table_columns(destination, table)
    columns = [column for column in destination_columns if column in source_columns]
    result: dict[str, Any] = {'mode': 'rowid_range_salvage', 'source_rows': source_rows, 'copied_rows': 0, 'excluded_rows': 0, 'columns': columns, 'range_queries': 0, 'skipped_rowid_ranges': []}
    if not source_columns:
        result['status'] = 'missing'
        return result
    if not columns:
    ...
    quoted = ', '.join((f'"{column}"' for column in columns))
    placeholders = ', '.join(('?' for _ in columns))
    select_sql = f'SELECT rowid, {quoted} FROM "{table}" WHERE rowid BETWEEN ? AND ? ORDER BY rowid'
    insert_sql = f'{insert_prefix} INTO "{table}" ({quoted}) VALUES ({placeholders})'
    ...
    def copy_range(low: int, high: int) -> None:
        ...
        if result['range_queries'] >= _MAX_SALVAGE_RANGE_QUERIES:
            ...
            _append_skipped_range(result['skipped_rowid_ranges'], low, high, 'salvage range query limit reached')
            return
        ...
        try:
            ...
            while True:
                ...
                if row_filter is not None:
                    included = [row for row in values if row_filter(row, column_names)]
                ...
                if progress_cb is not None:
                    progress_cb({'table': table, 'copied_rows': result['copied_rows'], 'source_rows': source_rows, 'skipped_ranges': len(result['skipped_rowid_ranges'])})
        except sqlite3.DatabaseError as exc:
    ...
    if skipped_ranges:
        ...
        result['error'] = f"copied {result['copied_rows']} and excluded {result['excluded_rows']} of {source_rows} source rows"
    else:
        result['status'] = 'complete'
    return result

def _copy_state_meta(source: sqlite3.Connection, destination: sqlite3.Connection, *, chunk_size: int, progress_cb: Optional[ProgressCallback], source_rows: Optional[int]) -> dict[str, Any]:
    source_columns = _table_columns(source, 'state_meta')
    destination_columns = _table_columns(destination, 'state_meta')
    result: dict[str, Any] = {'source_meta_rows': source_rows, 'copied_rows': 0, 'columns': ['key', 'value'], 'excluded_keys': sorted(_GENERATED_META_KEYS)}
    ...
    try:
        filtered_source_rows = int(source.execute(f'SELECT COUNT(*) FROM state_meta WHERE key NOT IN ({placeholders})', tuple(_GENERATED_META_KEYS)).fetchone()[0])
    except sqlite3.DatabaseError:
        pass
    try:
        cursor = source.execute(f'SELECT key, value FROM state_meta WHERE key NOT IN ({placeholders})', tuple(_GENERATED_META_KEYS))
        while True:
            rows = cursor.fetchmany(chunk_size)
            if not rows:
                break
            destination.execute('BEGIN IMMEDIATE')
            try:
                destination.executemany('INSERT OR REPLACE INTO state_meta(key, value) VALUES (?, ?)', rows)
                destination.execute('COMMIT')
            except BaseException:
                destination.execute('ROLLBACK')
                raise
            result['copied_rows'] += len(rows)
            if progress_cb is not None:
                progress_cb({'table': 'state_meta', 'copied_rows': result['copied_rows'], 'source_rows': filtered_source_rows})
    except sqlite3.DatabaseError as exc:
        result['status'] = 'partial' if result['copied_rows'] else 'failed'
        result['error'] = str(exc)
        return result
    result['status'] = 'complete' if filtered_source_rows is None or result['copied_rows'] == filtered_source_rows else 'partial'
    if result['status'] == 'partial':
        result['error'] = f"copied {result['copied_rows']} of {filtered_source_rows} readable rows"
    return result

def _copy_state_meta_salvage(source: sqlite3.Connection, destination: sqlite3.Connection, *, chunk_size: int, progress_cb: Optional[ProgressCallback], source_rows: Optional[int]) -> dict[str, Any]:
    ...
    destination_columns = _table_columns(destination, 'state_meta')
    if not source_columns:
        return {'mode': 'rowid_range_salvage', 'source_meta_rows': source_rows, 'copied_rows': 0, 'columns': ['key', 'value'], 'excluded_keys': sorted(_GENERATED_META_KEYS), 'status': 'missing'}
    if not {'key', 'value'}.issubset(source_columns):
        return {'mode': 'rowid_range_salvage', 'source_meta_rows': source_rows, 'copied_rows': 0, 'columns': ['key', 'value'], 'excluded_keys': sorted(_GENERATED_META_KEYS), 'status': 'failed', 'error': f"source state_meta exists but is missing the key/value columns (found: {', '.join(source_columns) or 'none'})"}
    if not {'key', 'value'}.issubset(destination_columns):
        return {'mode': 'rowid_range_salvage', 'source_meta_rows': source_rows, 'copied_rows': 0, 'columns': ['key', 'value'], 'excluded_keys': sorted(_GENERATED_META_KEYS), 'status': 'failed', 'error': 'destination state_meta schema is incomplete'}

    def keep_user_meta(row: tuple[Any, ...], columns: tuple[str, ...]) -> bool:
        return str(row[columns.index('key')]) not in _GENERATED_META_KEYS
    result = _copy_table_salvage(source, destination, 'state_meta', chunk_size=chunk_size, progress_cb=progress_cb, source_rows=source_rows, insert_prefix='INSERT OR REPLACE', row_filter=keep_user_meta)
...
def _reconstruct_missing_sessions(destination: sqlite3.Connection) -> dict[str, Any]:
    ...
    if not _table_columns(destination, 'messages'):
        return result
    orphaned = destination.execute('SELECT m.session_id, MIN(m.timestamp), COUNT(*) FROM messages AS m WHERE m.session_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.id = m.session_id) GROUP BY m.session_id').fetchall()
    if not orphaned:
        return result
    title_sequence = 1
    for session_id, first_timestamp, message_count in orphaned:
        ...
        while True:
            ...
            if destination.execute('SELECT 1 FROM sessions WHERE title = ? LIMIT 1', (title,)).fetchone() is None:
                break
        cursor = destination.execute("INSERT INTO sessions (id, source, started_at, title, message_count) VALUES (?, 'recovered', ?, ?, ?)", (session_id, started_at, title, int(message_count)))
...
def _cleanup_partial_orphans(destination: sqlite3.Connection) -> dict[str, Any]:
    ...
    are removed.
    """
    result: dict[str, Any] = {'session_prompt_refs_cleared': 0, 'system_prompts_removed': 0, 'sessions_parent_cleared': 0, 'sessions_reconstructed': 0, 'messages_retained': 0, 'messages_removed': 0, 'session_model_usage_removed': 0, 'compression_locks_removed': 0, 'telegram_dm_topic_bindings_removed': 0}
    destination.execute('BEGIN IMMEDIATE')
    try:
        ...
        parent_count = int(destination.execute('SELECT COUNT(*) FROM sessions AS child WHERE child.parent_session_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM sessions AS parent WHERE parent.id = child.parent_session_id)').fetchone()[0])
        if parent_count:
            destination.execute('UPDATE sessions SET parent_session_id = NULL WHERE parent_session_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM sessions AS parent WHERE parent.id = sessions.parent_session_id)')
        result['sessions_parent_cleared'] = parent_count
        prompt_ref_count = int(destination.execute('SELECT COUNT(*) FROM sessions WHERE system_prompt_hash IS NOT NULL AND NOT EXISTS (SELECT 1 FROM system_prompts WHERE system_prompts.hash = sessions.system_prompt_hash)').fetchone()[0])
        if prompt_ref_count:
            destination.execute('UPDATE sessions SET system_prompt_hash = NULL WHERE system_prompt_hash IS NOT NULL AND NOT EXISTS (SELECT 1 FROM system_prompts WHERE system_prompts.hash = sessions.system_prompt_hash)')
        result['session_prompt_refs_cleared'] = prompt_ref_count
        unreferenced_prompt_count = int(destination.execute('SELECT COUNT(*) FROM system_prompts WHERE NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.system_prompt_hash = system_prompts.hash)').fetchone()[0])
        if unreferenced_prompt_count:
            destination.execute('DELETE FROM system_prompts WHERE NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.system_prompt_hash = system_prompts.hash)')
        result['system_prompts_removed'] = unreferenced_prompt_count
        dependent_tables = (('messages', 'messages_removed'), ('session_model_usage', 'session_model_usage_removed'), ('compression_locks', 'compression_locks_removed'), ('telegram_dm_topic_bindings', 'telegram_dm_topic_bindings_removed'))
        for table, report_key in dependent_tables:
            if not _table_columns(destination, table):
                continue
            orphan_count = int(destination.execute(f'SELECT COUNT(*) FROM "{table}" AS dependent WHERE NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.id = dependent.session_id)').fetchone()[0])
            if orphan_count:
                destination.execute(f'DELETE FROM "{table}" WHERE NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.id = "{table}".session_id)')
            result[report_key] = orphan_count
        destination.execute('COMMIT')
    except BaseException:
        destination.execute('ROLLBACK')
        raise
    result['total_removed_or_relinked'] = int(result['session_prompt_refs_cleared']) + int(result['sessions_parent_cleared']) + int(result['messages_removed']) + int(result['session_model_usage_removed']) + int(result['compression_locks_removed']) + int(result['telegram_dm_topic_bindings_removed'])
    return result

def _verify_recovered_database(output: Path, *, expected_counts: dict[str, Optional[int]], copy_report: dict[str, dict[str, Any]], allow_partial: bool=False, orphan_cleanup: Optional[dict[str, Any]]=None) -> dict[str, Any]:
    ...
    try:
        ...
        verification['journal_mode'] = str(journal_row[0]).lower() if journal_row else None
        ...
        if verification['schema_version'] != SCHEMA_VERSION:
            verification['errors'].append(f"schema version is {verification['schema_version']}, expected {SCHEMA_VERSION}")
        meta = {str(row[0]): row[1] for row in conn.execute("SELECT key, value FROM state_meta WHERE key LIKE 'fts_%'").fetchall()}
        ...
        pending_keys = sorted((key for key in ('fts_optimize_available', 'fts_rebuild_high_water', 'fts_rebuild_progress', 'fts_cjk_stale', 'fts_cjk_rebuild_high_water', 'fts_cjk_rebuild_progress') if key in meta))
        ...
        for table in ('sessions', 'messages'):
            ...
            if expected is not None and counts.get(table) != expected:
                message = f'{table} count is {counts.get(table)}, expected {expected}'
                if allow_partial:
        ...
        sessions_fully_reconstructed = bool(rebuilt_sessions > 0 and counts.get('sessions') == rebuilt_sessions and (counts.get('messages') == retained_messages) and (removed_messages == 0))
        for table, table_report in copy_report.items():
            status = table_report.get('status')
            if status not in {'failed', 'partial'}:
                continue
            message = f'{table} copy status is {status}'
            if allow_partial and (status == 'partial' or table not in {'sessions', 'messages'} or (table == 'sessions' and status == 'failed' and sessions_fully_reconstructed)):
                verification['warnings'].append(message)
                verification['loss_detected'] = True
            else:
                verification['errors'].append(message)
        if orphan_cleanup:
            ...
            if orphan_count:
                verification['warnings'].append(f'{orphan_count} orphaned reference(s) were removed or relinked')
                verification['loss_detected'] = True
            rebuilt_sessions = int(orphan_cleanup.get('sessions_reconstructed') or 0)
            if rebuilt_sessions:
                retained = int(orphan_cleanup.get('messages_retained') or 0)
                verification['warnings'].append(f'{rebuilt_sessions} session(s) could not be salvaged and were reconstructed as placeholders to retain {retained} message(s); their metadata (title, model, timestamps, cost) is lost')
                verification['loss_detected'] = True
...
def _finalize_derived_metadata(destination: sqlite3.Connection) -> dict[str, Any]:
    ...
    fts_tables = {str(row[0]) for row in destination.execute("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('messages_fts', 'messages_fts_trigram')").fetchall()}
    ...
    try:
        ...
        destination.execute('INSERT INTO state_meta(key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', ('fts_storage_version', str(FTS_STORAGE_VERSION)))
        destination.execute('COMMIT')
    except BaseException:
        destination.execute('ROLLBACK')
        raise
...
def recover_session_database(source_path: Path, output_path: Path, *, work_dir: Optional[Path]=None, chunk_size: int=1000, progress_cb: Optional[ProgressCallback]=None, allow_partial: bool=False) -> dict[str, Any]:
    ...
    try:
        if not inspection.get('recoverable') and (not allow_partial):
            ...
            raise SessionRecoverySourceError(f'Required canonical tables are not readable: {reasons}')
        if allow_partial:
            missing_required = [table for table in ('sessions', 'messages') if not inspection['tables'][table].get('available')]
            if missing_required:
                raise SessionRecoverySourceError('Partial recovery still requires readable table schemas for: ' + ', '.join(missing_required))
        source_conn = sqlite3.connect(str(snapshot_source), isolation_level=None, timeout=1.0)
        ...
        try:
            has_topic_tables = any((inspection['tables'][table].get('available') for table in _TOPIC_TABLES))
            destination_db = SessionDB(db_path=output)
            if has_topic_tables:
                destination_db.apply_telegram_topic_migration()
            destination_db.close()
            destination_db = None
            destination_conn = sqlite3.connect(str(output), isolation_level=None, timeout=1.0)
            ...
            for table in _CANONICAL_TABLES:
                ...
                copy_function = _copy_table_salvage if allow_partial else _copy_table
                copy_report[table] = copy_function(source_conn, destination_conn, table, chunk_size=chunk_size, progress_cb=progress_cb, source_rows=table_inspection.get('rows'))
            state_meta_inspection = inspection['tables']['state_meta']
            if state_meta_inspection.get('available'):
                state_meta_copy_function = _copy_state_meta_salvage if allow_partial else _copy_state_meta
                copy_report['state_meta'] = state_meta_copy_function(source_conn, destination_conn, chunk_size=chunk_size, progress_cb=progress_cb, source_rows=state_meta_inspection.get('rows'))
            else:
            ...
            if destination_db is not None:
                destination_db.close()
        verification = _verify_recovered_database(output, expected_counts={table: inspection['tables'][table].get('rows') for table in _CANONICAL_TABLES}, copy_report=copy_report, allow_partial=allow_partial, orphan_cleanup=orphan_cleanup)
        ...
        return {'operation': 'recover', 'allow_partial': allow_partial, 'source': str(source), 'output': str(output), 'source_bundle': inspection['source_bundle'], 'source_fingerprint': inspection['source_fingerprint'], 'source_unchanged': source_unchanged, 'disk_space': disk_space, 'inspection': {'journal_mode': inspection.get('journal_mode'), 'tables': inspection['tables'], 'errors': inspection['errors'], 'warnings': inspection['warnings']}, 'copy': copy_report, 'orphan_cleanup': orphan_cleanup, 'derived_metadata': derived_metadata, 'verification': verification, 'complete': bool(verification.get('complete') and source_unchanged), 'partial': bool(verification.get('loss_detected')), 'verified': bool(verification.get('healthy') and source_unchanged), 'installed': False}
    finally:
        temp_dir.cleanup()
