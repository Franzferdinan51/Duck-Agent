...
def _get_rss_mb() -> Optional[int]:
    ...
    falls back to ``psutil`` which is an optional duck-agent dep.
    """
    try:
        import resource
...
def log_memory_usage(prefix: str='') -> None:
    ...
    if rss is None:
        logger.info('[MEMORY] %srss=unavailable gc=%s threads=%d uptime=%ds', tag, gc_counts, thread_count, uptime)
    else:
        logger.info('[MEMORY] %srss=%dMB gc=%s threads=%d uptime=%ds', tag, rss, gc_counts, thread_count, uptime)
...
def start_memory_monitoring(interval_seconds: float=300.0) -> bool:
    ...
    with _lock:
        ...
        if _get_rss_mb() is None:
            logger.warning('[MEMORY] Memory monitoring unavailable: neither resource.getrusage nor psutil could read process RSS — skipping periodic logging.')
            return False
        _start_time = time.monotonic()
        _interval_seconds = float(interval_seconds)
        _stop_event = threading.Event()
        log_memory_usage(prefix='baseline')
        _monitor_thread = threading.Thread(target=_monitor_loop, args=(_stop_event, _interval_seconds), name='gateway-memory-monitor', daemon=True)
