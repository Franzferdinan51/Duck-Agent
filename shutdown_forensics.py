...
def snapshot_shutdown_context(received_signal: Any=None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    pid = os.getpid()
    ppid = os.getppid()
    ctx: Dict[str, Any] = {'ts': now, 'ts_monotonic': monotonic, 'signal': _signal_name(received_signal), 'signal_num': int(received_signal) if received_signal is not None else None, 'pid': pid, 'ppid': ppid, 'parent': _proc_summary(ppid), 'self': _proc_summary(pid)}
    ...
    try:
        ...
        pass
    try:
        hermes_home_str = os.environ.get('DUCK_AGENT_HOME')
        if hermes_home_str:
...
def spawn_async_diagnostic(log_path: Path, signal_name: str, *, timeout_seconds: float=5.0) -> Optional[int]:
    ...
    if sys.platform == 'win32':
        return None
    script = f"echo '=== shutdown diagnostic @ {signal_name} ==='; echo '--- date ---'; date -u +%Y-%m-%dT%H:%M:%SZ; echo '--- ps auxf (top 60 by cpu) ---'; ps auxf --sort=-pcpu 2>/dev/null | head -60; echo '--- pstree of self ---'; pstree -plau {os.getpid()} 2>/dev/null | head -40 || true; echo '--- /proc/loadavg ---'; cat /proc/loadavg 2>/dev/null || true; echo '--- recent dmesg (oom/killed) ---'; dmesg -T 2>/dev/null | tail -20 || journalctl --user -n 20 --no-pager 2>/dev/null | tail -20 || true; echo '=== end ==='"
    try:
        fd = os.open(str(log_path), os.O_WRONLY | os.O_CREAT | os.O_APPEND, 420)
    except OSError:
        return None
    try:
        proc = subprocess.Popen(['timeout', f'{timeout_seconds:.0f}', 'bash', '-c', script], stdout=fd, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL, start_new_session=True, close_fds=True)
...
def format_context_for_log(ctx: Dict[str, Any]) -> str:
    ...
    return f'signal={sig} under_systemd={under_systemd} parent_pid={parent_pid} parent_name={parent_name} loadavg_1m={load_str}{extras_str} parent_cmdline={parent_cmd!r}'
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded duck-agent but never re-ran ``duck-agent setup`` to regenerate
    ...
    for flag in (['--user'], []):
        try:
            result = subprocess.run(['systemctl', *flag, 'show', unit_name, '--property=TimeoutStopUSec'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=2.0)
    ...
    if timeout_us is None:
        return None
    timeout_stop_sec = timeout_us / 1000000.0
    headroom = 30.0
    expected = drain_timeout + headroom
    return {'unit': unit_name, 'timeout_stop_sec': timeout_stop_sec, 'drain_timeout': drain_timeout, 'expected_min': expected, 'mismatch': timeout_stop_sec < expected}
...
def _parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    if not raw:
        return None
    units = {'us': 1, 'ms': 1000, 's': 1000000, 'sec': 1000000, 'min': 60000000, 'h': 3600000000, 'hr': 3600000000}
    total_us = 0
    token = ''
    digits = ''
    for ch in raw + ' ':
        if ch.isdigit() or ch == '.':
            ...
            try:
                total_us += int(float(digits) * 1000000)
            except ValueError:
                return None
            digits = ''
