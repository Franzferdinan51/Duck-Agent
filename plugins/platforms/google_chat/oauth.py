...
- Per-user tokens (keyed by sender email):
    ``${DUCK_AGENT_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${DUCK_AGENT_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${DUCK_AGENT_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${DUCK_AGENT_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${DUCK_AGENT_HOME}/google_chat_user_client_secret.json``
"""
...
def _hermes_home() -> Path:
    """Resolve DUCK_AGENT_HOME at call time (NOT module import).

    Tests and ``DUCK_AGENT_HOME=...`` env overrides need this to be late-
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into DUCK_AGENT_HOME."""
