...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a duck-agent MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a duck-agent MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Duck Agent is thinking…' marker message.
        ...
        "Duck Agent is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Duck Agent is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Duck Agent is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Duck Agent is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``duck-agent setup``.
    ...
    prompt for env vars, persist them to ``~/.duck-agent/.env`` so the next
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``duck-agent cron`` running as a
    separate process from ``duck-agent gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Duck Agent plugin system at startup.
