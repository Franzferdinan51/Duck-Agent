...
under an immutable ``/opt/duck-agent`` (read-only for the duck-agent user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$DUCK_AGENT_HOME/photon/sidecar`` (the durable data volume,
...
``cli.py`` (``duck-agent photon ...``) use it.
"""
