...
Credential storage mirrors every other Duck Agent channel:

    * runtime SDK creds  -> ``~/.duck-agent/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.duck-agent/auth.json`` under
...
def _auth_json_path() -> Path:
    """Resolve ``~/.duck-agent/auth.json`` honouring the active Duck Agent profile."""
    try:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.duck-agent/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.duck-agent/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.duck-agent/.env`` (canonical runtime store).
