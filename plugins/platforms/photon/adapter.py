"""
Photon Spectrum (iMessage) platform adapter for Duck Agent.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `duck-agent photon status` so all three agree on
...
def _sidecar_deps_stale() -> bool:
    ...
    `duck-agent update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``duck-agent photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Duck Agent does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Duck Agent defaults).
...
def register(ctx) -> None:
    """Called by the Duck Agent plugin loader at startup."""
