"""
``duck-agent photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Duck Agent
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `duck-agent photon ...` subcommands."""
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``duck-agent photon login`` command; Photon onboards
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.duck-agent/.env.
...
def gateway_setup() -> None:
    """Run Photon first-time setup from the `duck-agent gateway setup` wizard."""
