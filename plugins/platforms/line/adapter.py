"""
LINE Messaging API platform adapter for Duck Agent.
...
to Duck Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``DUCK_AGENT_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``duck-agent status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``duck-agent status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``duck-agent setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.duck-agent/.env`` via ``hermes_cli.config``.
    """
    print()
