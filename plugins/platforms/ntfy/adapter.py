"""ntfy platform adapter (Duck Agent plugin).
...
a Duck Agent dependency.

This adapter ships as a Duck Agent platform plugin under
``plugins/platforms/ntfy/``. The Duck Agent plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "duck-agent-in"              # subscribe topic (incoming)
          publish_topic: "duck-agent-out"     # optional — defaults to topic
...
async def _standalone_send(
    ...
    *,
    ...
    ``duck-agent cron`` running standalone). Without this hook,
