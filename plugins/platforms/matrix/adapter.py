...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Duck Agent command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Duck Agent accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Duck Agent ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Duck Agent" become "Agent".
        """
        if not body:
            return ""
