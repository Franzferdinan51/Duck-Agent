"""SimpleX Chat platform adapter (Duck Agent plugin).
...
This adapter ships as a Duck Agent platform plugin under
``plugins/platforms/simplex/``. The Duck Agent plugin loader scans the
...
discoverable and ``duck-agent setup`` can describe it even when websockets is
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``duck-agent setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.duck-agent/.env`` via
    ``hermes_cli.config``.
    """
    print()
