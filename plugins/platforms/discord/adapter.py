...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.duck-agent/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Duck Agent gateway
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Duck Agent response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Duck Agent work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Duck Agent/gateway outage notices."""
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Duck Agent manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Duck Agent pairing grant."""
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Duck Agent: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Duck Agent has an LLM-generated session title and can safely rename
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Duck Agent: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Duck Agent handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``duck-agent update --gateway``
...
def _component_check_auth(
    ...
    the pairing store (``duck-agent pairing approve``).
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``duck-agent update`` prompts.
...
def register(ctx) -> None:
    """Plugin entry point — called by the Duck Agent plugin system."""
