"""
Microsoft Teams platform adapter for Duck Agent.
...
Requires:
    ...
    manually: ``<duck-agent-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Duck Agent owns dotenv loading; third-party import side effects must not.
    """
    try:
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Duck Agent
