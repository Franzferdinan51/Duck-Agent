...
``_check_spotify_available()`` — when the user has not run ``duck-agent auth
spotify``, the tools remain registered (so they appear in ``duck-agent
...
The Spotify auth flow (``duck-agent auth spotify``), CLI plumbing, and docs
