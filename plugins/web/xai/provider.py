...
``{title, url, description, position}`` rows every other Duck Agent web
provider produces.
...
prefers Duck Agent-managed xAI Grok OAuth (via ``duck-agent auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.duck-agent/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``duck-agent tools`` repaint and at tool-registration time.
