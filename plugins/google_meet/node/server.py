...
Launched by ``duck-agent meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$DUCK_AGENT_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``duck-agent meet node approve <name> <url> <token>``.
