"""`duck-agent meet node ...` subcommand tree.

Wired into the existing ``duck-agent meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``duck-agent meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``duck-agent meet node ...``.
