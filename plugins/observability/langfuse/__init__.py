"""langfuse — Duck Agent plugin for Langfuse observability.

Traces Duck Agent conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Duck Agent plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``duck-agent plugins enable
observability/langfuse`` or ``duck-agent tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``duck-agent tools`` or ~/.duck-agent/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Duck Agent plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Duck Agent paths only expose ``task_id``/``session_id``. Newer paths
