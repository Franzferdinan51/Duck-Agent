"""disk_cleanup — ephemeral file cleanup for Duck Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under DUCK_AGENT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly DUCK_AGENT_HOME and /tmp/duck-agent-*
Never touches: ~/.duck-agent/logs/ or any system directory.
"""
...
def get_state_dir() -> Path:
    """State dir — separate from ``$DUCK_AGENT_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$DUCK_AGENT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under DUCK_AGENT_HOME or ``/tmp/duck-agent-*``.
