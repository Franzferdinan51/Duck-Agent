"""disk-cleanup plugin — auto-cleanup of ephemeral Duck Agent session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``DUCK_AGENT_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$DUCK_AGENT_HOME/disk-cleanup/cleanup.log``.
...
All operations are scoped to DUCK_AGENT_HOME and /tmp/duck-agent-*.
