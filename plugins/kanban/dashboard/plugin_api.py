...
This means ``duck-agent dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def list_diagnostics(
    ...
    AND for ``duck-agent kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``duck-agent kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``duck-agent kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``duck-agent kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.duck-agent/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Duck Agent profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.duck-agent/profiles/*`` on disk and every
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of Duck Agent
...
def list_kanban_projects():
    """List first-class Duck Agent projects for board scoping.
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``duck-agent profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``duck-agent kanban decompose <task_id>``.
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.duck-agent/config.yaml.
