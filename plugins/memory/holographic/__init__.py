"""duck-agent-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $DUCK_AGENT_HOME/config.yaml (profile-scoped):
  plugins:
    duck-agent-memory-store:
      db_path: $DUCK_AGENT_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to config.yaml under plugins.duck-agent-memory-store."""
