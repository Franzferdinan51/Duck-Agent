...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: duck-agent)
...
Or via $DUCK_AGENT_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Duck Agent can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``duck-agent memory status`` would stay green while the daemon
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $DUCK_AGENT_HOME/hindsight/config.json  (profile-scoped)
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Duck Agent config."""
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Duck Agent profile name (from agent_identity)
      {workspace} — Duck Agent workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``duck-agent-{user}`` with no user becomes ``duck-agent``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $DUCK_AGENT_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
