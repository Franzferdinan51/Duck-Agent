...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Duck Agent peer ID in OpenViking (default: duck-agent)
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Duck Agent slash-skill scaffolding before sending content to OpenViking.
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.duck-agent/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts duck-agent (#21130).
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Duck Agent' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Duck Agent canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See duck-agent#28296.
