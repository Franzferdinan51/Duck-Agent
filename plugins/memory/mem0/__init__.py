...
Configuration
-------------
Secret (lives in $DUCK_AGENT_HOME/.env or the environment):
...
Behavioral settings (live in $DUCK_AGENT_HOME/mem0.json, set via `duck-agent memory
setup`):
  ...
  agent_id           — Agent identifier (default: duck-agent)
...
def _load_config() -> dict:
    """Load config from env vars, with $DUCK_AGENT_HOME/mem0.json overrides.
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $DUCK_AGENT_HOME/mem0.json."""
        import json
