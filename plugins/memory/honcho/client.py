...
Resolution order for config file:
  1. $DUCK_AGENT_HOME/honcho.json  (instance-local, enables isolated Duck Agent instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Duck Agent profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Duck Agent profile.
    ...
    Resolution order:
      ...
      4. Fallback: ``"duck-agent"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $DUCK_AGENT_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.duck-agent/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $DUCK_AGENT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Duck Agent profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Duck Agent session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Duck Agent session title (from /title command; non-per-session)
