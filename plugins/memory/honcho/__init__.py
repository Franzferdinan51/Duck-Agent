...
Config: Uses the existing Honcho config chain:
  1. $DUCK_AGENT_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $DUCK_AGENT_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Duck Agent CLI/gateway startup responsive when Honcho is down,
