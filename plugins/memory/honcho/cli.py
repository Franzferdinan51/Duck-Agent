...
Handles: duck-agent honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all Duck Agent profiles and creates host blocks for any that don't
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `duck-agent update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Duck Agent profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $DUCK_AGENT_HOME/honcho.json so each profile/instance gets
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'duck-agent honcho setup' to try again.\n")
                return
...
def _active_profile_name() -> str:
    """Return the active Duck Agent profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: duck-agent honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: duck-agent honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("\n  Set with: duck-agent honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Duck Agent + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Duck Agent\n" + "─" * 50)
    print()
...
def register_cli(subparser) -> None:
    """Build the ``duck-agent honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``duck-agent honcho``.
    """
