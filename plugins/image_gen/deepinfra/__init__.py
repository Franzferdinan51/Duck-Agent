...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=duck-agent``
...
models show up in ``duck-agent tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from duck-agent the next time the catalog is fetched, no patch required.
