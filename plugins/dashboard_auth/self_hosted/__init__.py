...
A standards-compliant OpenID Connect Relying Party for the ``duck-agent dashboard``
...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/duck-agent/   # required
          client_id: duck-agent-dashboard                              # required
          scopes: "openid profile email"                           # optional
          # client_secret: set ONLY for a confidential client. It is a
          # credential — prefer the env var / ~/.duck-agent/.env over config.yaml.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Duck Agent does not call
