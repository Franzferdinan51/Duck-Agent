...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<duck_agent_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active DUCK_AGENT_HOME."""
    from hermes_constants import get_hermes_home
    flush_dir = get_hermes_home() / 'pending_messages'
    flush_dir.mkdir(parents=True, exist_ok=True, mode=448)
    if os.name == 'posix':
        os.chmod(flush_dir, 448)
    return flush_dir
...
def _write_payload(flush_dir: Path, payload: Dict[str, Any]) -> None:
    ...
    atomic_json_write(final_path, payload, mode=384, default=str)
    try:
    ...
    for session_key, value in list(pending.items()):
        ...
        try:
            ...
            if serialised is None:
                continue
            _write_payload(flush_dir, {'session_key': session_key, 'reason': reason, 'ts': ts, 'data': serialised})
            flushed += 1
        except Exception as exc:
            logger.debug('Failed to flush pending message for %s: %s', session_key, exc)
    if flushed:
    ...
    for path in flush_files:
        try:
            ...
            if not text or not session_key:
                logger.warning('Cannot recover structurally invalid pending message from %s; the flush file has been preserved', path)
                continue
            session_id = data.get('session_id', '')
            if not session_id:
                logger.warning('Cannot recover pending message for %s: no session_id in flush file and session_key-to-id resolution is not available at this recovery stage. The message text is preserved in %s', session_key, path)
                continue
            session_db.append_message(session_id=session_id, role='user', content=text, timestamp=payload.get('ts', int(time.time())))
            recovered += 1
    ...
    try:
        ...
        for _m in history:
            try:
                ...
                continue
        _write_payload(flush_dir, {'reason': 'shutdown-with-unpersisted-agent-history', 'issue': '#72680', 'session_id': session_id, 'count': len(snapshot), 'messages': snapshot})
        logger.warning('Preserved %d in-memory message(s) for session %s (possible FTS corruption — recover after repairing state.db)', len(snapshot), session_id)
