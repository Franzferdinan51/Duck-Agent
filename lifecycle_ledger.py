...
``<DUCK_AGENT_HOME>/state/gateway.lifecycle.json``:
...
_LOW_MEM_AVAILABLE_KIB = 64 * 1024
_LOW_MEM_AVAILABLE_FRACTION = 0.05

def _process_hermes_home() -> Path:
    """DUCK_AGENT_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path]=None) -> Path:
    """Return ``<DUCK_AGENT_HOME>/state/gateway.lifecycle.json``."""
...
def detect_unclean_exit(home: Optional[Path]=None) -> Optional[Dict[str, Any]]:
    ...
    if _pid_alive_with_start_time(sentinel.get('pid'), sentinel.get('start_time')):
        return None
    evidence: Dict[str, Any] = {'prior_pid': sentinel.get('pid'), 'prior_started_at': sentinel.get('started_at'), 'prior_start_time': sentinel.get('start_time')}
    try:
    ...
    if hb:
        ...
        if isinstance(mem, dict):
            ...
            if isinstance(avail, int) and (avail < _LOW_MEM_AVAILABLE_KIB or (isinstance(total, int) and total > 0 and (avail / total < _LOW_MEM_AVAILABLE_FRACTION))):
...
def record_startup(home: Optional[Path]=None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if evidence is not None:
            record = {'ts': datetime.now(timezone.utc).isoformat(), 'tag': 'gateway.previous_unclean_exit', 'pid': os.getpid(), **evidence}
            _append_exit_diag(record, home)
            logger.warning('Previous gateway life (pid=%s, started_at=%s) exited UNCLEANLY (no exit path ran — SIGKILL / OOM / VM death). last_heartbeat_at=%s last_mem=%s suspected_oom=%s', evidence.get('prior_pid'), evidence.get('prior_started_at'), evidence.get('last_heartbeat_at'), evidence.get('last_heartbeat_mem'), evidence.get('suspected_oom', False))
    except Exception:
        logger.debug('Unclean-exit detection failed', exc_info=True)
    try:
        _write_sentinel({'phase': 'running', 'pid': os.getpid(), 'start_time': time.time(), 'started_at': datetime.now(timezone.utc).isoformat()}, home)
    except Exception:
...
def mark_exited(exit_code: Optional[int]=None, reason: str='graceful_shutdown', home: Optional[Path]=None) -> None:
    ...
    try:
        ...
        if sentinel is not None and sentinel.get('pid') != os.getpid():
            return
        _write_sentinel({'phase': 'exited', 'pid': os.getpid(), 'exit_code': exit_code, 'exit_reason': reason, 'exited_at': datetime.now(timezone.utc).isoformat()}, home)
    except Exception:
