...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        db = _get_db()
        return _ok(rid, {'repos': _discover_repos_payload(db, include_cached=policy['enabled']) if db is not None else [], 'accepted': accepted, 'discovery_policy': policy})
    except Exception as e:
        return _err(rid, 5061, str(e))
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        tree, active_id = _build_project_tree(db, preview_limit=int(params.get('preview_limit') or 3), hydrate=False, session_limit=int(params.get('session_limit') or 2000), include_discovered=True)
        return _ok(rid, {'projects': tree['projects'], 'active_id': active_id, 'scoped_session_ids': tree['scoped_session_ids']})
    except Exception as e:
        return _err(rid, 5061, str(e))
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        tree, _active = _build_project_tree(db, preview_limit=0, hydrate=True, session_limit=int(params.get('session_limit') or 5000), include_discovered=False)
...
def _(rid, params: dict) -> dict:
    ...
    if key == 'provider':
        try:
            ...
            parts = model.split('/', 1)
            return _ok(rid, {'model': model, 'provider': normalize_provider(parts[0]) if len(parts) > 1 else 'unknown', 'providers': list_available_providers()})
        except Exception as e:
    ...
    if key == 'indicator':
        ...
        return _ok(rid, {'value': norm if norm in INDICATOR_STYLES else DEFAULT_INDICATOR_STYLE})
    if key == 'personality':
        return _ok(rid, {'value': (_load_cfg().get('display') or {}).get('personality') or 'none'})
    if key == 'reasoning':
        cfg = _load_cfg()
    ...
    if key == 'details_mode':
        ...
        raw = str((_load_cfg().get('display') or {}).get('details_mode', 'collapsed') or 'collapsed').strip().lower()
    ...
    if key == 'focus':
        ...
        return _ok(rid, {'value': 'on' if on else 'off', 'tool_progress': _load_tool_progress_mode()})
    if key == 'mouse':
        display = _load_cfg().get('display')
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if not provider_configured and provider == 'bedrock' and (source in {'iam-role', 'aws-sdk-default-chain'}):
            return _ok(rid, {'ok': False, 'provider': provider, 'model': runtime.get('model'), 'source': source, 'error': 'No Duck Agent provider is configured.'})
        ...
        if not credential_ok:
            return _ok(rid, {'ok': False, 'provider': provider, 'model': runtime.get('model'), 'source': runtime.get('source'), 'error': f'No usable credentials found for {provider}.'})
        return _ok(rid, {'ok': True, 'provider': runtime.get('provider'), 'model': runtime.get('model'), 'source': runtime.get('source')})
    except Exception as e:
