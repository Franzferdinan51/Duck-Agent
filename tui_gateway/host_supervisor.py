...
MUTATOR_ROUTE_TABLE: dict[str, str] = {'prompt.submit': 'turn-path', 'session.interrupt': 'turn-path', 'reload.mcp': 'run-concurrent', 'session.save': 'run-concurrent', 'session.compress': 'idle-gated', 'prompt.submit.truncate': 'idle-gated', 'slash.model': 'idle-gated', 'slash.personality': 'idle-gated', 'slash.prompt': 'idle-gated', 'slash.compress': 'idle-gated', 'session.reset': 'idle-gated', 'session.history.reload': 'idle-gated', 'slash.retry': 'idle-gated'}
...
def append_log_record(path: str | Path, record: str) -> None:
    ...
    fd = os.open(str(p), os.O_WRONLY | os.O_CREAT | os.O_APPEND, 384)
    try:
        os.write(fd, data)
    finally:
        os.close(fd)
...
def _pid_command(pid: int) -> str:
    ...
    try:
        ...
        pass
    try:
        return subprocess.check_output(['ps', '-p', str(pid), '-o', 'command='], text=True, encoding='utf-8', errors='replace', stderr=subprocess.DEVNULL, timeout=2).strip()
    except Exception:
        return ''
...
class HostSupervisor:
    ...
    def __init__(self, *, registry_path: str | Path | None=None, argv: list[str] | None=None, cwd: str | Path | None=None, env: dict[str, str] | None=None, rpc_sink: Callable[[dict], None] | None=None, respawn_max: int=3, heartbeat_secs: int=15, expected_build_sha: str | None=None, expected_hermes_home: str | None=None, autostart: bool=True) -> None:
    ...
    def submit_turn(self, frame: dict[str, Any], *, on_complete: Callable[[dict], None] | None=None) -> str:
        ...
        try:
            ...
            err = {'type': 'turn.error', 'sid': sid, 'request_id': request_id, 'reason': 'send_failed', 'message': str(exc)}
    ...
    def reload_mcp(self, sid: str, *, request_id: str | None=None) -> dict:
        return self.control(sid, route_name='reload.mcp', payload={'type': 'reload_mcp', 'sid': sid, 'request_id': request_id or uuid.uuid4().hex}, wait=True)

    def control(self, sid: str, *, route_name: str, payload: dict[str, Any] | None=None, wait: bool=True, timeout: float=30.0) -> dict:
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        proc = subprocess.Popen(self.argv, cwd=str(self.cwd), env=env, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='replace', bufsize=1, start_new_session=True)
        self._proc = proc
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get('duck_agent_home') or '')
        if got_home and got_home != self.expected_hermes_home:
            raise RuntimeError(f'compute host DUCK_AGENT_HOME mismatch: {got_home} != {self.expected_hermes_home}')
    ...
    def _persist_registry(self) -> None:
        ...
        payload = {'host_pid': self.pid, 'boot_id': self._hello.get('boot_id') or '', 'build_sha': self._hello.get('build_sha') or '', 'started_at': time.time(), 'argv': self.argv}
    ...
    def _fail_pending_turns(self, *, reason: str, message: str) -> None:
        ...
        for request_id, (sid, cb) in pending.items():
            frame = {'type': 'turn.error', 'sid': sid, 'request_id': request_id, 'reason': reason, 'message': message}
            self.rpc_sink({'jsonrpc': '2.0', 'method': 'event', 'params': {'type': 'error', 'session_id': sid, 'payload': {'message': message, 'reason': reason}}})
            if cb is not None:
                try:
                    cb(frame)
                except Exception:
