...
from hermes_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES, get_hermes_home, get_hermes_home_override, reset_hermes_home_override, set_hermes_home_override
...
def _thread_panic_hook(args):
    ...
    try:
        os.makedirs(os.path.dirname(_CRASH_LOG), exist_ok=True)
        with open(_CRASH_LOG, 'a', encoding='utf-8') as f:
            f.write(f"\n=== thread exception · {time.strftime('%Y-%m-%d %H:%M:%S')} · thread={args.thread.name} ===\n")
            f.write(trace)
    except Exception:
        pass
    first_line = str(args.exc_value).strip().splitlines()[0] if str(args.exc_value).strip() else args.exc_type.__name__
    print(f'[gateway-crash] thread {args.thread.name} raised {args.exc_type.__name__}: {first_line}', file=sys.stderr, flush=True)
...
try:
    _ws_orphan_reap_grace = float(os.environ.get('HERMES_TUI_WS_ORPHAN_REAP_GRACE_S') or '20')
except (ValueError, TypeError):
    _ws_orphan_reap_grace = 20.0
_WS_ORPHAN_REAP_GRACE_S = max(0.0, _ws_orphan_reap_grace)
_DETAIL_SECTION_NAMES = ('thinking', 'tools', 'subagents', 'activity')
_DETAIL_MODES = frozenset({'hidden', 'collapsed', 'expanded'})
_LONG_HANDLERS = frozenset({'billing.state', 'subscription.state', 'subscription.preview', 'subscription.change', 'subscription.resume', 'subscription.upgrade', 'usage.bars', 'session.usage', 'billing.step_up', 'browser.manage', 'cli.exec', 'complete.path', 'complete.slash', 'llm.oneshot', 'model.options', 'pet.cells', 'pet.gallery', 'pet.generate', 'pet.hatch', 'pet.info', 'pet.select', 'pet.thumb', 'learning.frames', 'plugins.manage', 'reload.mcp', 'process.list', 'projects.discover_repos', 'projects.record_repos', 'projects.for_cwd', 'projects.tree', 'projects.project_sessions', 'setup.runtime_check', 'setup.status', 'session.active_list', 'session.branch', 'session.compress', 'session.list', 'session.resume', 'session.workspace.move', 'shell.exec', 'skills.manage', 'slash.exec'})
try:
    _rpc_pool_workers = max(2, int(os.environ.get('HERMES_TUI_RPC_POOL_WORKERS') or '8'))
except (ValueError, TypeError):
    _rpc_pool_workers = 8
_pool = concurrent.futures.ThreadPoolExecutor(max_workers=_rpc_pool_workers, thread_name_prefix='tui-rpc')
atexit.register(lambda: _pool.shutdown(wait=False, cancel_futures=True))
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar('hermes_gateway_runtime_session_record', default=None)
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None=None):
        ...
        env = build_subprocess_env(hermes_subprocess_env(inherit_credentials=True), scrub_secrets=False, inherit_profile_home=False, extra={'DUCK_AGENT_HOME': str(profile_home)} if profile_home else None)
        self.proc = subprocess.Popen(argv, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='replace', bufsize=1, cwd=os.getcwd(), env=env, creationflags=windows_hide_flags(), start_new_session=True)
...
def _claim_active_session_slot(session_key: str, *, live_session_id: str, surface: str='tui') -> tuple[Any, str | None]:
    try:
        from hermes_cli.active_sessions import try_acquire_active_session
        return try_acquire_active_session(session_id=session_key, surface=surface, config=_load_cfg(), metadata={'live_session_id': live_session_id})
...
def _ensure_active_session_slot(sid: str, session: dict) -> str | None:
    ...
    if session.get('active_session_lease') is not None:
        return None
    lease, limit_message = _claim_active_session_slot(str(session.get('session_key') or ''), live_session_id=sid, surface=_session_source(session))
...
def _transfer_active_session_slot(sid: str, session: dict, *, new_session_id: str) -> bool:
    ...
    new_lease, limit_message = _claim_active_session_slot(new_session_id, live_session_id=sid, surface=_session_source(session))
    if new_lease is not None:
        ...
        return True
    if limit_message:
        logger.warning('Compression session lease re-anchor failed (kept old lease): sid=%s new_session_id=%s reason=%s', sid, new_session_id, limit_message)
    return False
_NON_GATEWAY_SOURCES = frozenset({'', 'tui', 'cli', 'webui', 'desktop', 'cron', 'kanban', 'subagent', 'test', 'local', 'acp', 'webhook', 'api_server', 'msgraph_webhook'})
...
def _finalize_session(session: dict | None, end_reason: str='tui_close') -> None:
    ...
    if agent is not None:
        try:
            ...
            invoke_hook('on_session_end', session_id=getattr(agent, 'session_id', None) or session.get('session_key', ''), completed=False, interrupted=True, model=getattr(agent, 'model', 'unknown'), platform=getattr(agent, 'platform', None) or 'tui')
        except Exception:
            pass
    ...
    try:
        ...
        if not _own_sid:
            try:
                ...
                _own_sid = ''
        interrupt_for_session(session_key=str(session_key or '') if _tui_owns_lifecycle else '', origin_ui_session_id=_own_sid, reason=end_reason)
    except Exception:
        pass
    try:
...
def _announce_session_reclaimed(session: dict, end_reason: str) -> None:
    ...
    if end_reason not in _RECLAIM_END_REASONS:
        return
    try:
        _broadcast_global_event('session.reclaimed', {'session_id': str(session.get('_sid') or ''), 'stored_session_id': str(session.get('session_key') or ''), 'reason': end_reason})
    except Exception:
...
def _close_session_by_id(sid: str, *, end_reason: str='tui_close', predicate: Callable[[dict], bool] | None=None) -> bool:
...
def _reap_idle_sessions() -> None:
    ...
    for sid in victims:
        _close_session_by_id(sid, end_reason='idle_timeout', predicate=lambda session, victim_sid=sid: _session_is_evictable(victim_sid, session, time.time()))
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        ...
        with _sessions_lock:
            live = {lease.lease_id for session in _sessions.values() if (lease := session.get('active_session_lease')) is not None}
    ...
    for sid, _s in evictable:
        with _sessions_lock:
            if len(_sessions) <= cap:
                break
        _close_session_by_id(sid, end_reason='lru_evict', predicate=lambda session, victim_sid=sid: _session_is_lru_evictable(victim_sid, session))
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s DUCK_AGENT_HOME around a pet RPC handler.
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``duck-agent dashboard`` was launched) instead of the configured
...
def _session_uses_compute_host(session: dict, cfg: dict | None=None) -> bool:
    if not _turn_isolation_enabled(cfg):
        return False
    return bool(session.get('_compute_host_active')) or (session.get('agent') is None and session.get('agent_ready') is not None)
...
def _get_compute_host_supervisor(cfg: dict | None=None):
    ...
    with _compute_host_supervisor_lock:
        if _compute_host_supervisor is None:
            ...
            _compute_host_supervisor = HostSupervisor(rpc_sink=write_json, heartbeat_secs=int(isolation_cfg.get('compute_host_heartbeat_secs') or 15), respawn_max=int(isolation_cfg.get('compute_host_respawn_max') or 3))
        return _compute_host_supervisor

def _compute_host_turn_frame(rid: str, sid: str, session: dict, text: Any, image_paths: list[str] | None=None, queued_prompt_generation: int | None=None) -> dict:
    with session['history_lock']:
        history = list(session.get('history', []))
        history_version = int(session.get('history_version', 0))
        attached_images = list(image_paths) if image_paths is not None else list(session.get('attached_images', []))
    return {'type': 'turn.start', 'sid': sid, 'request_id': rid, 'session_key': session.get('session_key') or sid, 'text': text, 'history': history, 'history_version': history_version, 'cols': int(session.get('cols', 80) or 80), 'cwd': _session_cwd(session), 'profile_home': session.get('profile_home') or '', 'model_override': session.get('model_override'), 'reasoning_config_override': session.get('create_reasoning_override'), 'service_tier_override': session.get('create_service_tier_override'), 'source': _session_source(session), 'attached_images': attached_images, 'queued_prompt_generation': queued_prompt_generation}
...
def _apply_compute_host_metadata_mirror(session: dict, frame: dict | None) -> None:
    ...
    with session.get('history_lock', threading.Lock()):
        ...
        if frame.get('history_version') is not None:
            try:
                session['history_version'] = max(int(session.get('history_version', 0)), int(frame.get('history_version') or 0))
            except Exception:
                pass
...
def _submit_prompt_to_compute_host(rid: str, sid: str, session: dict, text: Any, image_paths: list[str] | None=None, queued_prompt_generation: int | None=None) -> dict:
    cfg = _load_dashboard_process_isolation_config()
    frame = _compute_host_turn_frame(rid, sid, session, text, image_paths=image_paths, queued_prompt_generation=queued_prompt_generation)
...
def _send_compute_host_control(sid: str, *, route_name: str, command: str='', payload: dict | None=None, wait: bool=True, timeout: float=30.0) -> dict:
    frame = dict(payload or {})
    frame.setdefault('type', 'control')
    frame.setdefault('command', command)
    return _get_compute_host_supervisor().control(sid, route_name=route_name, payload=frame, wait=wait, timeout=timeout)
...
def _current_session_steer_authority(session_id: str) -> tuple[Transport | None, dict | None]:
    ...
    with _sessions_lock:
        ...
        if session is None or (expected_session is not None and session is not expected_session) or session.get('transport') is not transport:
            return (None, None)
...
def _wait_agent_for_prompt(session: dict, rid: str, sid: str) -> dict | None:
    ...
    while not ready.wait(timeout=_AGENT_BUILD_WAIT_SLICE):
        ...
        if waited >= cap:
            return _err(rid, 5032, f'agent initialization timed out after {int(waited)}s — your message was not sent; retry once the session is ready')
        ...
        if not notified_slow and waited >= _AGENT_BUILD_SLOW_NOTICE_AFTER:
            notified_slow = True
            _emit('notification.show', sid, {'text': "Still starting the agent (tool discovery / model setup) — your message will be sent as soon as it's ready.", 'level': 'info', 'kind': 'agent', 'ttl_ms': None, 'key': _AGENT_BUILD_SLOW_NOTICE_KEY, 'id': _AGENT_BUILD_SLOW_NOTICE_KEY})
    if notified_slow:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `duck-agent` shows the prompt before constructing AIAgent; the TUI used
...
def _completion_cwd(params: dict | None=None) -> str:
    params = params or {}
    raw = params.get('cwd') or _sessions.get(params.get('session_id') or '', {}).get('cwd') or _profile_configured_cwd(_profile_home(params.get('profile'))) or _launch_configured_cwd() or os.environ.get('TERMINAL_CWD') or os.getcwd()
    try:
...
def _register_session_cwd(session: dict | None) -> None:
    ...
    try:
        ...
        register_task_env_overrides(session['session_key'], {'cwd': _terminal_task_cwd(session)})
    except Exception:
        pass
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``duck-agent`` / ``duck-agent --tui`` / CLI) is started from a
    ...
    model_config: dict = {}
    for src_key, cfg_key in (('model', 'model'), ('provider', 'provider'), ('base_url', 'base_url'), ('api_mode', 'api_mode')):
    ...
    if str(model_config.get('provider') or '').strip().lower() == 'custom':
        try:
            ...
            healed = canonical_custom_identity(base_url=model_config.get('base_url') or None, model=model_config.get('model') or row_model or None)
            if healed:
    ...
    try:
        db.create_session(key, source=_session_source(session), model=row_model, model_config=model_config or None, parent_session_id=parent_session_id, cwd=_persisted_session_cwd(session), profile_name=Path(profile_home).name if profile_home else None)
    except Exception as exc:
...
def _persist_branch_seed(session: dict) -> None:
    ...
    with _session_db(session) as db:
        if db is None:
            return
        try:
            db.append_messages_batch(key, [{'role': msg.get('role', 'user'), 'content': msg.get('content'), 'timestamp': msg.get('timestamp')} for msg in seed], chunk_rows=500)
        ...
        with _sessions_lock:
            for sess in list(_sessions.values()):
                if sess.get('session_key') == session_key:
                    ...
                    break
        return set_session_vars(session_key=session_key, session_id=session_id, source=source, cwd=resolved, ui_session_id=ui_session_id, cron_session='')
    except Exception:
        return []
...
def _block(event: str, sid: str, payload: dict, timeout: float | None=300) -> str:
    ...
    if not answered and (not answer_present) and (event in {'secret.request', 'sudo.request', 'clarify.request', 'terminal.read.request', 'preview.read.request'}):
...
def resolve_skin() -> dict:
    try:
        ...
        return {'name': skin.name, 'colors': skin.colors, 'light_colors': skin.light_colors, 'dark_colors': skin.dark_colors, 'branding': skin.branding, 'banner_logo': skin.banner_logo, 'banner_hero': skin.banner_hero, 'tool_prefix': skin.tool_prefix, 'help_header': (skin.branding or {}).get('help_header', '')}
    except Exception:
        return {}
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``duck-agent config set display.skin``) OR edited the active skin's colors in
...
def _pet_changed_payload() -> dict:
    ...
    try:
        ...
        return {'enabled': True, 'slug': pet.slug, 'displayName': pet.display_name, 'scale': scale, 'spritesheetRevision': _pet_sheet_revision(pet.spritesheet)}
    except Exception:
        return {'enabled': False}
...
def _pairing_sig():
    ...
    for root in roots:
        ...
        for entry in entries:
            ...
            sig = mtime if sig is None else max(sig, mtime)
    return sig
_CHANGE_WATCHES: dict[str, tuple[float, Any, Any]] = {'pet.changed': (2.0, _pet_sig, _pet_changed_payload), 'cron.changed': (1.0, _cron_sig, lambda: {}), 'sessions.changed': (0.5, _sessions_sig, lambda: {}), 'platforms.changed': (2.0, _platforms_sig, lambda: {}), 'pairing.changed': (2.0, _pairing_sig, lambda: {})}
...
def _ensure_skin_watcher() -> None:
    ...
    Duck Agent activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name='duck-agent-change-watcher', daemon=True).start()

def _resolve_model() -> str:
    env = (os.environ.get('HERMES_MODEL', '') or os.environ.get('HERMES_INFERENCE_MODEL', '')).strip()
    if env:
        return env
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      ...
      * ``HERMES_DESKTOP_TERMINAL=1`` → "tui"
        (``duck-agent --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``duck-agent --tui``.)
    """
    if is_truthy_value(os.environ.get('HERMES_DESKTOP')) and (not is_truthy_value(os.environ.get('HERMES_DESKTOP_TERMINAL'))):
        return 'desktop'
    return 'tui'
...
def _config_model_target() -> tuple[str, str]:
    ...
    (`duck-agent --tui -m <model>`, hosted-instance provisioning); if they
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_model = (os.environ.get('HERMES_MODEL', '') or os.environ.get('HERMES_INFERENCE_MODEL', '')).strip()
    if not explicit_model:
        return (model, None)
    try:
        from hermes_cli.models import detect_static_provider_for_model
        cfg = _load_cfg().get('model') or {}
        current_provider = (str(cfg.get('provider') or '').strip().lower() if isinstance(cfg, dict) else '') or os.environ.get('HERMES_INFERENCE_PROVIDER', '').strip().lower() or 'auto'
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if model:
        overrides['model_override'] = {'model': model, 'provider': provider or None, 'base_url': base_url or None, 'api_mode': api_mode or None}
    if provider:
...
def _append_model_switch_marker(session: dict | None, *, model: str, provider: str) -> None:
    ...
    marker = f'{_MODEL_SWITCH_MARKER_PREFIX}{model}{provider_part}. From this point forward, use this runtime metadata when answering questions about what model/provider is active.]'
...
def _coerce_statusbar(raw) -> str:
    ...
    if isinstance(raw, str) and (s := raw.strip().lower()) in _STATUSBAR_MODES:
        return s
    return 'top'
_MOUSE_TRACKING_ALIASES = {'0': 'off', '1': 'all', 'all': 'all', 'any': 'all', 'button': 'buttons', 'buttons': 'buttons', 'click': 'buttons', 'false': 'off', 'full': 'all', 'no': 'off', 'off': 'off', 'on': 'all', 'scroll': 'wheel', 'true': 'all', 'wheel': 'wheel', 'yes': 'all'}
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    the same backend told the model it was "chatting inside the Duck Agent desktop
...
def _load_enabled_toolsets(platform: str | None=None) -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        if any((name in {'all', '*'} for name in built_in)):
            ...
            if ignored:
                print(f"[tui] HERMES_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}", file=sys.stderr, flush=True)
            return None
        if not unresolved:
            return built_in
        ...
        if unknown:
            print(f"[tui] ignoring unknown HERMES_TUI_TOOLSETS entries: {', '.join(unknown)}", file=sys.stderr, flush=True)
        if disabled:
            print(f"[tui] ignoring disabled MCP servers in HERMES_TUI_TOOLSETS (set enabled: true in config.yaml to use): {', '.join(disabled)}", file=sys.stderr, flush=True)
        if valid:
            return valid
    ...
    try:
        ...
        if fallback_notice is not None:
            print('[tui] no valid HERMES_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets', file=sys.stderr, flush=True)
        return None
...
def _restart_slash_worker(sid: str, session: dict):
    ...
    try:
        ...
        pass
    try:
        new_worker = _SlashWorker(session['session_key'], getattr(session.get('agent'), 'model', _resolve_model()), profile_home=session.get('profile_home'))
    except Exception:
...
def _snapshot_agent_model_runtime(agent) -> dict:
    ...
    return {'model': getattr(agent, 'model', ''), 'provider': getattr(agent, 'provider', ''), 'api_key': getattr(agent, 'api_key', ''), 'base_url': getattr(agent, 'base_url', ''), 'api_mode': getattr(agent, 'api_mode', ''), 'primary_runtime': copy.deepcopy(getattr(agent, '_primary_runtime', None))}
...
def _restore_agent_model_runtime(agent, snapshot: dict | None) -> None:
    ...
    if hasattr(agent, 'switch_model'):
        agent.switch_model(new_model=snapshot.get('model', ''), new_provider=snapshot.get('provider', ''), api_key=snapshot.get('api_key', ''), base_url=snapshot.get('base_url', ''), api_mode=snapshot.get('api_mode', ''))

def _apply_model_switch(sid: str, session: dict, raw_input: str, *, confirm_expensive_model: bool=False, pin_session_override: bool=True, parsed_flags: Any | None=None, persist_override: bool | None=None) -> dict:
    from hermes_cli.model_switch import parse_model_switch_args, resolve_persist_behavior, switch_model, MODEL_SWITCH_ERR_ONCE_WITH_GLOBAL, MODEL_SWITCH_ERROR_TEXT
    ...
    persist_global = persist_override if persist_override is not None else resolve_persist_behavior(is_global_flag, is_session, is_once=one_turn, explicit_provider=explicit_provider)
    if not model_input:
    ...
    try:
        ...
        pass
    result = switch_model(raw_input=model_input, current_provider=current_provider, current_model=current_model, current_base_url=current_base_url, current_api_key=current_api_key, is_global=persist_global, explicit_provider=explicit_provider, user_providers=user_provs, custom_providers=custom_provs)
    ...
    if agent:
        try:
            ...
            merge_preflight_compression_warning(result, agent=agent, messages=list(session.get('history', [])), custom_providers=custom_provs, config_context_length=_cfg_ctx)
        except Exception as exc:
            logger.debug('preflight-compression switch warning failed: %s', exc)
    if not confirm_expensive_model:
        try:
            from hermes_cli.model_cost_guard import expensive_model_warning
            warning = expensive_model_warning(result.new_model, provider=result.target_provider, base_url=result.base_url or current_base_url, api_key=result.api_key or current_api_key, model_info=result.model_info)
        except Exception:
            warning = None
    ...
    if agent:
        try:
            agent.switch_model(new_model=result.new_model, new_provider=result.target_provider, api_key=result.api_key, base_url=result.base_url, api_mode=result.api_mode)
        except Exception as exc:
            logger.warning('In-place model switch failed for TUI agent: %s', exc)
            raise ValueError(f"Model switch to {result.new_model} failed ({exc}); staying on {getattr(agent, 'model', current_model)}.") from exc
    ...
    if pin_session_override and isinstance(session, dict) and (not one_turn):
        session['model_override'] = {'model': result.new_model, 'provider': result.target_provider, 'base_url': result.base_url, 'api_key': result.api_key, 'api_mode': result.api_mode}
    if persist_global:
        _persist_model_switch(result)
    return {'value': result.new_model, 'warning': result.warning_message or '', 'confirm_required': False, 'scope': 'once' if one_turn else 'global' if persist_global else 'session'}
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(sid, session, raw, confirm_expensive_model=True, pin_session_override=False, persist_override=False)
    except Exception as e:
...
def _compress_session_history(session: dict, focus_topic: str | None=None, approx_tokens: int | None=None, before_messages: list | None=None, history_version: int | None=None) -> tuple[int, dict]:
    ...
    from hermes_cli.partial_compress import parse_partial_compress_args, rejoin_compressed_head_and_tail, split_history_for_partial_compress
    agent = session['agent']
    ...
    try:
        compressed, _ = agent._compress_context(head, None, approx_tokens=approx_tokens, focus_topic=focus_topic or None, force=True, defer_context_engine_notification=True)
    except Exception:
...
def _sync_session_key_after_compress(sid: str, session: dict, *, clear_pending_title: bool=True, restart_slash_worker: bool=True) -> None:
    ...
    if not lease_reanchored:
        logger.warning('Compression session lease did not re-anchor: sid=%s old_session_id=%s new_session_id=%s', sid, old_key, new_session_id)
    try:
        from tools.approval import disable_session_yolo, enable_session_yolo, is_session_yolo_enabled, register_gateway_notify, unregister_gateway_notify
        try:
...
def _get_usage(agent) -> dict:
    ...
    usage = {'model': getattr(agent, 'model', '') or '', 'input': g('session_input_tokens', 'session_prompt_tokens'), 'output': g('session_output_tokens', 'session_completion_tokens'), 'reasoning': g('session_reasoning_tokens'), 'prompt': g('session_prompt_tokens'), 'completion': g('session_completion_tokens'), 'total': g('session_total_tokens'), 'calls': g('session_api_calls')}
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if not null_keys:
        ...
        warnings.append(f'config.yaml has empty section(s): {keys}. Remove the line(s) or set them to `{{}}` — empty sections silently drop nested settings.')
    display_cfg = cfg.get('display')
    agent_cfg = cfg.get('agent')
    if isinstance(display_cfg, dict):
        personality = str(display_cfg.get('personality', '') or '').strip().lower()
        if personality and personality not in {'default', 'none', 'neutral'} and isinstance(agent_cfg, dict) and (agent_cfg.get('personalities') is None):
            warnings.append('`display.personality` is set but `agent.personalities` is empty/null; personality overlay will be skipped.')
...
def _session_info(agent, session: dict | None=None) -> dict:
    ...
    info: dict = {'model': pending_model or mirror.get('model', getattr(agent, 'model', '')), 'provider': pending_provider or mirror.get('provider', getattr(agent, 'provider', '')), 'reasoning_effort': reasoning_effort, 'service_tier': service_tier, 'fast': service_tier == 'priority', 'yolo': yolo, 'approval_mode': approval_mode, 'tools': dict(mirror.get('tools') or {}) if isinstance(mirror.get('tools'), dict) else {}, 'skills': dict(mirror.get('skills') or {}) if isinstance(mirror.get('skills'), dict) else {}, 'cwd': cwd, 'branch': _git_branch_for_cwd(cwd), 'project': _project_info_for_cwd(cwd), 'personality': str(personality or ''), 'running': bool((session or {}).get('running')), 'title': _session_live_title(session or {}, session_key) if session_key else '', 'stored_session_id': session_key or '', 'desktop_contract': DESKTOP_BACKEND_CONTRACT, 'version': '', 'release_date': '', 'update_behind': None, 'update_command': '', 'usage': _session_usage_snapshot(session), 'profile_name': _response_profile_name(Path(session['profile_home']).name if isinstance(session, dict) and session.get('profile_home') else None) if isinstance(session, dict) and session.get('profile_home') else _current_profile_name()}
    try:
    ...
    try:
        ...
        info['mcp_servers'] = []
    try:
        info['system_prompt'] = mirror.get('system_prompt') if 'system_prompt' in mirror else getattr(agent, '_cached_system_prompt', '') or ''
    except Exception:
        pass
    try:
...
def _emit_session_info_for_session(sid: str, session: dict) -> None:
    ...
    try:
        ...
        pass
_TUI_VERBOSE_TEXT_MAX_CHARS = 1000
...
def _cap_tui_verbose_text(text: str) -> str:
    ...
    if omitted_lines:
        label = f'[showing verbose tail; omitted {omitted_lines} lines / {omitted_chars} chars]\n'
    else:
...
def _on_tool_progress(sid: str, event_type: str, name: str | None=None, preview: str | None=None, _args: dict | None=None, **_kwargs):
    ...
    if event_type == 'tool.output_risk' and name:
        ...
        if not isinstance(metadata, dict):
            return
        payload: dict[str, object] = {'tool_id': str(_kwargs.get('tool_call_id') or ''), 'name': str(name), 'risk': str(metadata.get('risk') or 'low'), 'findings': [str(item) for item in metadata.get('findings', [])], 'redacted': bool(metadata.get('redacted', False))}
    ...
    if event_type.startswith('subagent.'):
        payload = {'goal': str(_kwargs.get('goal') or ''), 'task_count': int(_kwargs.get('task_count') or 1), 'task_index': int(_kwargs.get('task_index') or 0)}
        ...
        if _kwargs.get('output_tail'):
            payload['output_tail'] = list(_kwargs['output_tail'])
        if name:
...
def _mirror_subagent_to_child(event_type: str, payload: dict) -> None:
    ...
    with _child_mirrors_lock:
        ...
        if event_type == 'subagent.thinking':
            ...
            st['seq'] += 1
            tool = {'name': str(payload.get('tool_name') or 'tool'), 'tool_id': f"submirror:{child_key}:{st['seq']}", 'args': {}}
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {'tool_start_callback': lambda tc_id, name, args: _on_tool_start(sid, tc_id, name, args), 'tool_complete_callback': lambda tc_id, name, args, result: _on_tool_complete(sid, tc_id, name, args, result), 'tool_progress_callback': lambda event_type, name=None, preview=None, args=None, **kwargs: _on_tool_progress(sid, event_type, name, preview, args, **kwargs), 'tool_gen_callback': lambda name: _tool_progress_enabled(sid) and _emit('tool.generating', sid, {'name': name}), 'thinking_callback': lambda text: _emit('thinking.delta', sid, {'text': text}), 'reaction_callback': lambda kind: _emit('reaction', sid, {'kind': kind}), 'reasoning_callback': lambda text: _emit('reasoning.delta', sid, {'text': text, **({'verbose': True} if _session_verbose(sid) else {})}), 'status_callback': lambda kind, text=None: _status_update(sid, str(kind), None if text is None else str(text)), 'notice_callback': lambda n: _emit('notification.show', sid, {'text': n.text, 'level': n.level, 'kind': n.kind, 'ttl_ms': n.ttl_ms, 'key': n.key, 'id': n.id}), 'notice_clear_callback': lambda key: _emit('notification.clear', sid, {'key': key}), 'clarify_callback': lambda q, c, multi_select=False: _block('clarify.request', sid, {'question': q, 'choices': c, 'multi_select': True} if multi_select else {'question': q, 'choices': c}, timeout=_clarify_timeout_seconds()), 'read_terminal_callback': lambda start=None, count=None: _block('terminal.read.request', sid, {k: v for k, v in (('start', start), ('count', count)) if v is not None}, timeout=30), 'read_preview_callback': lambda start=None, count=None: _block('preview.read.request', sid, {k: v for k, v in (('start', start), ('count', count)) if v is not None}, timeout=45)}
    if _load_interim_assistant_messages():
        callbacks['interim_assistant_callback'] = lambda text, *, already_streamed=False: _emit('message.interim', sid, {'text': str(text), 'already_streamed': bool(already_streamed)})
    return callbacks
...
def _apply_project_workspace(task_id: str, path: str, _name: str='') -> None:
    ...
    try:
        agent = session.get('agent')
        info = _session_info(agent, session) if agent is not None else {'cwd': resolved, 'branch': _git_branch_for_cwd(resolved), 'project': _project_info_for_cwd(resolved), 'lazy': True}
        _emit('session.info', sid, info)
    except Exception:
...
def _apply_personality_to_session(sid: str, session: dict, new_prompt: str, personality: str='') -> tuple[bool, dict | None]:
    ...
    if agent:
        ...
        if new_prompt:
            marker = f"[System: The user has changed the assistant's personality. From this point forward, adopt the following persona and respond accordingly: {new_prompt}]"
        else:
            marker = '[System: The user has cleared the personality overlay. From this point forward, respond in your normal default style.]'
        with session['history_lock']:
...
def _preview_restart_callbacks(parent: str, task_id: str) -> dict:
    ...
    return {'tool_start_callback': tool_start, 'tool_complete_callback': tool_complete, 'tool_progress_callback': tool_progress, 'tool_gen_callback': lambda name: progress(f'Preparing {name}'), 'status_callback': lambda kind, text=None: progress(text if text is not None else kind)}
...
def _reset_session_agent(sid: str, session: dict) -> dict:
    ...
    try:
        ...
        session.pop('one_turn_model_restore', None)
        new_agent = _make_agent(sid, session['session_key'], session_id=session['session_key'], platform_override=_session_source(session))
    finally:
        _clear_session_context(tokens)
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            try:
                ...
                import logging
                logging.getLogger(__name__).warning('Primary auth failed (%s), falling back to %s model %s', primary_exc, fb_provider, fb_model)
                return _RuntimeFallbackResolution(runtime, fb_model, True)
            except Exception:
                continue
        raise

def _make_agent(sid: str, key: str, session_id: str | None=None, session_db=None, model_override: dict | str | None=None, provider_override: str | None=None, reasoning_config_override: dict | None=None, service_tier_override: str | None=None, platform_override: str | None=None):
    ...
    if startup_skills:
        ...
        skills_prompt, loaded_skills, missing_skills = build_preloaded_skills_prompt(startup_skills, task_id=session_id or key)
        if missing_skills:
            missing_display = ', '.join(missing_skills)
            if loaded_skills:
                logger.warning('Unknown skill(s) requested, skipping: %s. Continuing with: %s. List available skills with `duck-agent skills list`.', missing_display, ', '.join(loaded_skills))
            else:
    ...
    if isinstance(model_override, dict) and model_override.get('model'):
        ...
        resolution = _resolve_runtime_with_fallback({'requested': requested_provider, 'target_model': model or None})
        runtime = resolution.runtime
        if resolution.used_fallback:
            if not resolution.selected_model:
                raise RuntimeError('Auth fallback resolved without a model')
    ...
    return AIAgent(model=model, max_iterations=_cfg_max_turns(cfg, 500), provider=runtime.get('provider'), base_url=runtime.get('base_url'), api_key=runtime.get('api_key'), api_mode=runtime.get('api_mode'), acp_command=runtime.get('command'), acp_args=runtime.get('args'), credential_pool=runtime.get('credential_pool'), quiet_mode=True, verbose_logging=False, reasoning_config=reasoning_config_override if reasoning_config_override is not None else _load_reasoning_config(str(model or '')), service_tier=service_tier_override if service_tier_override is not None else _load_service_tier(), enabled_toolsets=_load_enabled_toolsets(_resolve_agent_platform(platform_override)), providers_allowed=_pr.get('only'), providers_ignored=_pr.get('ignore'), providers_order=_pr.get('order'), provider_sort=_pr.get('sort'), provider_require_parameters=_pr.get('require_parameters', False), provider_data_collection=_pr.get('data_collection'), platform=_resolve_agent_platform(platform_override), session_id=session_id or key, session_db=session_db if session_db is not None else _get_db(), ephemeral_system_prompt=system_prompt or None, checkpoints_enabled=is_truthy_value(os.environ.get('HERMES_TUI_CHECKPOINTS')), pass_session_id=is_truthy_value(os.environ.get('HERMES_TUI_PASS_SESSION_ID')), skip_context_files=is_truthy_value(os.environ.get('HERMES_IGNORE_RULES')), skip_memory=is_truthy_value(os.environ.get('HERMES_IGNORE_RULES')), fallback_model=_load_fallback_model(), **_agent_cbs(sid))

def _init_session(sid: str, key: str, agent, history: list, cols: int=80, cwd: str | None=None, session_db=None, source: str | None=None, profile_home: str | None=None):
    now = time.time()
    with _sessions_lock:
        _sessions[sid] = {'agent': agent, 'session_key': key, 'history': history, 'history_lock': threading.Lock(), 'history_version': 0, 'inflight_turn': None, 'created_at': now, 'last_active': now, 'running': False, 'attached_images': [], 'image_counter': 0, 'cwd': cwd or _completion_cwd(), 'cols': cols, 'slash_worker': None, 'show_reasoning': _load_show_reasoning(), 'source': _resolve_session_source(source), 'tool_progress_mode': _load_tool_progress_mode(), 'edit_snapshots': {}, 'tool_started_at': {}, 'profile_home': profile_home, 'model_override': None, 'transport': current_transport() or _stdio_transport}
    _init_owns_db = False
...
def _enrich_with_attached_images(user_text: str, image_paths: list[str]) -> str:
    ...
    prompt = 'Describe everything visible in this image in thorough detail. Include any text, code, data, objects, people, layout, colors, and any other notable visual information.'
    parts: list[str] = []
...
def _history_to_messages(history: list[dict]) -> list[dict]:
    ...
    for m in history:
        ...
        if role == 'tool':
            ...
            continue
        reasoning_keys = ('reasoning', 'reasoning_content', 'reasoning_details', 'codex_reasoning_items')
        has_reasoning = role == 'assistant' and any((m.get(key) for key in reasoning_keys))
...
def _auto_continue_config() -> tuple[bool, float, int]:
    ...
    return (is_truthy_value(cfg.get('enabled'), default=_AUTO_CONTINUE_ENABLED_DEFAULT), max(0.0, minutes) * 60.0, _coerce_int_config_value(cfg.get('max_attempts'), _AUTO_CONTINUE_MAX_ATTEMPTS_DEFAULT, min_value=0))

def _session_home(session: dict) -> Path:
    """The DUCK_AGENT_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get('profile_home')
...
def _auto_continue_note(prompt: str) -> str:
    return f'{_AUTO_CONTINUE_NOTE_PREFIX} — the app or its backend process stopped before the turn could finish. Some of the work may already be complete; check the current state before redoing anything, then finish the task. The interrupted request was:]\n\n{prompt}'
...
def _enqueue_prompt(session: dict, text: Any, transport: Any, image_paths: list[str] | None=None) -> None:
    ...
    if existing and isinstance(existing.get('text'), str) and isinstance(text, str) and (not existing.get('image_paths')) and (not image_paths) and (not session.get('queued_prompts')):
        prev = existing['text']
...
def _handle_busy_submit(rid, sid: str, session: dict, text: Any, transport: Any, queued: bool=False) -> dict | None:
    ...
    if mode == 'steer' and text_only and plain_text and (agent is not None) and hasattr(agent, 'steer'):
        try:
            ...
            pass
    if mode == 'interrupt' and text_only and plain_text and (agent is not None) and (getattr(agent, '_supports_active_turn_redirect', False) is True) and hasattr(agent, 'redirect'):
        try:
    ...
    try:
        if use_compute_host:
            if queued.get('image_paths'):
                resp = _submit_prompt_to_compute_host(rid, sid, session, queued['text'], image_paths=queued['image_paths'], queued_prompt_generation=queue_generation)
            else:
                resp = _submit_prompt_to_compute_host(rid, sid, session, queued['text'], queued_prompt_generation=queue_generation)
            if resp.get('error'):
        ...
        elif queued.get('image_paths'):
            _run_prompt_submit(rid, sid, session, queued['text'], image_paths=queued['image_paths'], queued_prompt_generation=queue_generation)
        else:
            _run_prompt_submit(rid, sid, session, queued['text'], queued_prompt_generation=queue_generation)
    except Exception as exc:
...
def _emit_terminal_turn_error(sid: str, session: dict, error: Any) -> None:
    ...
    agent = session.get('agent')
    payload = {'text': text, 'usage': _get_usage(agent) if agent is not None else {}, 'status': 'error', 'error': message, 'recoverable': True}
    if partial:
...
def _lazy_resume_info(cwd: str, *, model: str='', provider: str='', profile: str | None=None) -> dict:
    ...
    info = {'cwd': cwd, 'branch': _git_branch_for_cwd(cwd), 'project': _project_info_for_cwd(cwd), 'model': model or _resolve_model(), 'tools': {}, 'skills': {}, 'lazy': True, 'desktop_contract': DESKTOP_BACKEND_CONTRACT, 'profile_name': _response_profile_name(profile)}
    if provider:
        info['provider'] = provider
    return info

def _deferred_session_record(session_key: str, *, cols: int, cwd: str, history: list, lease, source: str='tui', close_on_disconnect: bool=False, display_history_prefix: list | None=None, profile_home: Path | None=None, lazy: bool=False, model_override=None, resume_runtime_overrides: dict | None=None) -> dict:
    """A live-session record whose AIAgent is built later (lazy watch / cold
    resume) — _init_session's shape minus the agent."""
    now = time.time()
    return {'agent': None, 'agent_error': None, 'agent_ready': threading.Event(), 'attached_images': [], 'close_on_disconnect': close_on_disconnect, 'active_session_lease': lease, 'cols': cols, 'created_at': now, 'cwd': cwd, 'display_history_prefix': display_history_prefix or [], 'edit_snapshots': {}, 'explicit_cwd': False, 'history': history, 'history_lock': threading.Lock(), 'history_version': 0, 'image_counter': 0, 'inflight_turn': None, 'last_active': now, 'lazy': lazy, 'model_override': model_override, 'pending_title': None, 'profile_home': str(profile_home) if profile_home is not None else None, 'resume_runtime_overrides': resume_runtime_overrides, 'resume_session_id': session_key, 'running': False, 'session_key': session_key, 'show_reasoning': _load_show_reasoning(), 'slash_worker': None, 'source': source, 'tool_progress_mode': _load_tool_progress_mode(), 'tool_started_at': {}, 'transport': current_transport() or _stdio_transport}
...
def _fallback_session_info(session: dict) -> dict:
    ...
    cwd = _session_cwd(session)
    return {'cwd': cwd, 'branch': _git_branch_for_cwd(cwd), 'project': _project_info_for_cwd(cwd), 'lazy': True, 'model': _resolve_model(), 'skills': {}, 'tools': {}}
...
def _live_session_payload(sid: str, session: dict, *, cols: int | None=None, touch: bool=False, transport: Transport | None=None, omit_messages: bool=False) -> dict:
    with session['history_lock']:
        if cols is not None:
            session['cols'] = cols
    ...
    history = in_memory_history if omit_messages else _live_visible_history(session, _get_db(), in_memory_history)
    payload = {'info': _fallback_session_info(session), 'message_count': len(history), 'messages': [] if omit_messages else _history_to_messages(history), 'messages_omitted': omit_messages, 'running': running, 'session_id': sid, 'session_key': _session_lookup_key(session, fallback=sid), 'started_at': float(session.get('created_at') or time.time()), 'status': _session_live_status(sid, session)}
    if inflight:
...
def _pet_sprite_payload(pet, *, scale: float) -> dict:
    ...
    payload = {'slug': pet.slug, 'displayName': pet.display_name, 'mime': mime, 'spritesheetBase64': base64.standard_b64encode(raw).decode('ascii'), 'spritesheetRevision': _pet_sheet_revision(pet.spritesheet), 'frameW': constants.FRAME_W, 'frameH': constants.FRAME_H, 'framesPerState': constants.FRAMES_PER_STATE, 'framesByState': _pet_frame_counts(pet.spritesheet), 'framesByRow': _pet_row_frame_counts(pet.spritesheet), 'loopMs': constants.LOOP_MS, 'scale': scale, 'stateRows': _pet_state_rows(pet.spritesheet)}
    if cache_key is not None:
        with _pet_payload_cache_lock:
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Duck Agent has to support both the legacy 8-row petdex atlas and the current
...
def _pet_reference_images_from_data_url(ref_raw: str, stage) -> list:
    ...
    import binascii
    import re as _re
    match = _re.match('^data:image/([a-zA-Z0-9.+-]+);base64,(.*)$', ref_raw, _re.DOTALL)
    if not match:
...
def _serialize_billing_error(exc) -> dict:
    ...
    from hermes_cli.nous_billing import BillingRemoteSpendingRevoked, BillingScopeRequired, BillingSessionRevoked, BillingTransient
    kind = 'error'
    if isinstance(exc, BillingRemoteSpendingRevoked):
        ...
        kind = str(exc.error)
    return {'ok': False, 'error': kind, 'message': str(exc), 'portal_url': getattr(exc, 'portal_url', None), 'retry_after': getattr(exc, 'retry_after', None), 'payload': getattr(exc, 'payload', {}) or {}, 'actor': getattr(exc, 'actor', None), 'code': getattr(exc, 'code', None), 'recovery': getattr(exc, 'recovery', None)}
...
def _serialize_billing_state(state) -> dict:
    ...
    if state.card is not None:
        card = {'brand': state.card.brand, 'last4': state.card.last4, 'masked': state.card.masked, 'display': state.card.display, 'resolved_via': state.card.resolved_via}
    payment_method = None
    if state.payment_method is not None:
        pm = state.payment_method
        if pm.kind == 'card':
            payment_method = {'kind': 'card', 'brand': pm.brand, 'last4': pm.last4, 'wallet': pm.wallet, 'resolved_via': pm.resolved_via}
        elif pm.kind == 'link':
            payment_method = {'kind': 'link', 'email': pm.email, 'resolved_via': pm.resolved_via}
        else:
            payment_method = {'kind': 'unknown', 'raw_kind': pm.raw_kind, 'resolved_via': pm.resolved_via}
    monthly_cap = None
    if state.monthly_cap is not None:
        mc = state.monthly_cap
        monthly_cap = {'limit_usd': _s(mc.limit_usd), 'limit_display': format_money(mc.limit_usd), 'spent_this_month_usd': _s(mc.spent_this_month_usd), 'spent_display': format_money(mc.spent_this_month_usd), 'is_default_ceiling': mc.is_default_ceiling}
    auto_reload = None
    if state.auto_reload is not None:
        ...
        if ar.card is not None:
            if ar.card.kind == 'distinct':
                card_out = {'kind': 'distinct', 'payment_method_id': ar.card.payment_method_id, 'brand': ar.card.brand, 'last4': ar.card.last4}
            else:
                card_out = {'kind': ar.card.kind}
        auto_reload = {'enabled': ar.enabled, 'threshold_usd': _s(ar.threshold_usd), 'threshold_display': format_money(ar.threshold_usd), 'reload_to_usd': _s(ar.reload_to_usd), 'reload_to_display': format_money(ar.reload_to_usd), 'card': card_out}
    return {'ok': True, 'logged_in': state.logged_in, 'org_name': state.org_name, 'org_slug': state.org_slug, 'role': state.role, 'is_admin': state.is_admin, 'can_change_plan': state.can_change_plan, 'can_charge': state.can_charge, 'balance_usd': _s(state.balance_usd), 'balance_display': format_money(state.balance_usd), 'cli_billing_enabled': state.cli_billing_enabled, 'charge_presets': [_s(p) for p in state.charge_presets], 'charge_presets_display': [format_money(p) for p in state.charge_presets], 'min_usd': _s(state.min_usd), 'max_usd': _s(state.max_usd), 'card': card, 'payment_method': payment_method, 'monthly_cap': monthly_cap, 'auto_reload': auto_reload, 'portal_url': state.portal_url, 'error': state.error, 'usage': _usage_payload(state)}
...
def _serialize_usage_bar(bar) -> Optional[dict]:
    ...
    return {'kind': bar.kind, 'remaining_display': _fmt_usd(bar.remaining_usd), 'total_display': _fmt_usd(bar.total_usd), 'spent_display': _fmt_usd(bar.spent_usd), 'pct_used': bar.pct_used, 'fill_fraction': bar.fill_fraction}
...
def _serialize_subscription_state(state) -> dict:
    ...
    if state.current is not None:
        c = state.current
        current = {'tier_id': c.tier_id, 'tier_name': c.tier_name, 'monthly_credits': _s(c.monthly_credits), 'credits_remaining': _s(c.credits_remaining), 'cycle_ends_at': c.cycle_ends_at, 'pending_downgrade_tier_name': c.pending_downgrade_tier_name, 'pending_downgrade_at': c.pending_downgrade_at, 'pending_downgrade_display': format_renews(c.pending_downgrade_at), 'cancel_at_period_end': c.cancel_at_period_end, 'cancellation_effective_at': c.cancellation_effective_at, 'cancellation_effective_display': format_renews(c.cancellation_effective_at)}
    tiers = [{'tier_id': t.tier_id, 'name': t.name, 'tier_order': t.tier_order, 'dollars_per_month_display': format_money(t.dollars_per_month), 'monthly_credits': _s(t.monthly_credits), 'is_current': t.is_current, 'is_enabled': t.is_enabled} for t in state.tiers]
    return {'ok': True, 'logged_in': state.logged_in, 'is_admin': state.is_admin, 'can_change_plan': state.can_change_plan, 'org_name': state.org_name, 'org_id': state.org_id, 'role': state.role, 'context': state.context, 'current': current, 'tiers': tiers, 'portal_url': state.portal_url, 'error': state.error, 'usage': _usage_payload(state)}

def _serialize_subscription_preview(p) -> dict:
    """Serialize a SubscriptionChangePreview for the wire (Decimal → string)."""
    return {'ok': True, 'effect': p.effect, 'reason': p.reason, 'current_tier_id': p.current_tier_id, 'current_tier_name': p.current_tier_name, 'target_tier_id': p.target_tier_id, 'target_tier_name': p.target_tier_name, 'monthly_credits_delta': None if p.monthly_credits_delta is None else str(p.monthly_credits_delta), 'amount_due_now_cents': p.amount_due_now_cents, 'effective_at': p.effective_at}
...
def _notification_event_belongs_elsewhere(sid: str, session: dict, evt: dict) -> bool:
    ...
    if not evt_key:
        return False
    current_keys = {str(session.get('session_key') or ''), _session_lookup_key(session, fallback=sid)}
    ...
    if resolved_key != evt_key:
        ...
        try:
            with _sessions_lock:
                continuation_live = any((not s.get('_finalized') and (str(s.get('session_key') or '') == resolved_key or _session_lookup_key(s, fallback='') == resolved_key) for s in _sessions.values()))
        except Exception:
    ...
    try:
        ...
        return False
    return any((s is not session and (not s.get('_finalized')) and (str(s.get('session_key') or '') in {evt_key, resolved_key} or _session_lookup_key(s, fallback='') in {evt_key, resolved_key}) for s in snapshot))
...
def _session_owns_notification_event(sid: str, session: dict, evt: dict) -> bool:
    ...
    try:
        db = _get_db()
        resolved_key = (db.resolve_resume_session_id(evt_key) if db is not None else evt_key) or evt_key
    except Exception:
        resolved_key = evt_key
...
def _notification_event_requires_owner(evt: dict) -> bool:
    ...
    return evt.get('type') == 'async_delegation' or bool(str(evt.get('origin_ui_session_id') or '') or str(evt.get('session_key') or ''))
...
def _notification_event_dedup_key(evt: dict) -> tuple:
    ...
    if evt_type == 'watch_match':
        return (evt_sid, evt_type, evt.get('command', ''), evt.get('pattern', ''), evt.get('output', ''), evt.get('suppressed', 0), evt.get('message_id', ''))
        ...
        try:
            ...
            for sub in subs:
                ...
                if sub.get('chat_id') != session_key:
                    continue
                _old, _new, events = _kb.claim_unseen_events_for_sub(conn, task_id=sub['task_id'], platform=sub['platform'], chat_id=sub['chat_id'], thread_id=sub.get('thread_id') or '', kinds=_KANBAN_NOTIFY_KINDS)
                if not events:
                    continue
                ...
                if task and getattr(task, 'status', '') in {'done', 'archived'}:
                    try:
                        _kb.remove_notify_sub(conn, task_id=sub['task_id'], platform=sub['platform'], chat_id=sub['chat_id'], thread_id=sub.get('thread_id') or '')
                    except Exception:
                        pass
        finally:
            conn.close()
    ...
    while not stop_event.is_set() and (not session.get('_finalized')):
        ...
        if _now - _last_kanban_poll >= _KANBAN_POLL_SECONDS:
            ...
            try:
                ...
                print(f'[tui_gateway] kanban notification poll failed: {type(_kb_exc).__name__}: {_kb_exc}', file=sys.stderr)
                _kanban_texts = []
            if _kanban_texts:
            ...
            if _pending:
                ...
                if _batch:
                    ...
                    try:
                        ...
                        print(f'[tui_gateway] kanban notification dispatch failed: {type(exc).__name__}: {exc}', file=sys.stderr)
                        with session['history_lock']:
                            session['running'] = False
        try:
        ...
        if requires_owner and (not _session_owns_notification_event(sid, session, evt)):
            ...
            log('Dropping unowned %s notification (origin=%r key=%r) instead of delivering to session %s', evt.get('type', 'completion'), str(evt.get('origin_ui_session_id') or ''), str(evt.get('session_key') or ''), sid)
            continue
        ...
        try:
            ...
            if evt.get('type') == 'async_delegation':
                _run_prompt_submit(rid, sid, session, text, display_kind='async_delegation_complete', display_metadata=_async_delegation_display_metadata(evt))
            else:
                _run_prompt_submit(rid, sid, session, text)
            complete_event_delivery(evt, _claim)
        except Exception as exc:
            release_event_delivery(evt, _claim)
            print(f'[tui_gateway] notification poller dispatch failed: {type(exc).__name__}: {exc}', file=sys.stderr)
            with session['history_lock']:
                session['running'] = False
    deferred: list = []
    while not process_registry.completion_queue.empty():
        ...
        if requires_owner and (not _session_owns_notification_event(sid, session, evt)):
            if evt.get('type') == 'async_delegation':
                ...
                logger.debug('Dropping unowned %s notification during shutdown drain (origin=%r key=%r)', evt.get('type', 'completion'), str(evt.get('origin_ui_session_id') or ''), str(evt.get('session_key') or ''))
            continue
        _evt_sid = evt.get('session_id', '')
        if evt.get('type') == 'completion' and process_registry.is_completion_consumed(_evt_sid):
            continue
...
def _async_delegation_display_metadata(evt: dict) -> dict:
    ...
    results: list[dict] = [result for result in raw_results if isinstance(result, dict)] if isinstance(raw_results, list) else []
    task_count = len(results) or 1
    completed_count = sum((1 for result in results if result.get('status') in {'completed', 'success'}))
    failed_count = sum((1 for result in results if result.get('status') in {'failed', 'error'}))
    metadata = {'delegation_id': str(evt.get('delegation_id') or ''), 'task_count': task_count, 'completed_count': completed_count or task_count - failed_count, 'failed_count': failed_count}
...
def _start_notification_poller(sid: str, session: dict) -> threading.Event:
    ...
    t = threading.Thread(target=_notification_poller_loop, args=(stop, sid, session), daemon=True)
    t.start()
    return stop

def _run_prompt_submit(rid, sid: str, session: dict, text: Any, *, display_kind: str | None=None, display_metadata: dict | None=None, image_paths: list[str] | None=None, queued_prompt_generation: int | None=None) -> None:
    with session['history_lock']:
    ...
    def run():
        ...
        try:
            ...
            session_tokens = _set_session_context(session['session_key'], ui_session_id=sid)
            ...
            if isinstance(prompt, str) and '@' in prompt:
                ...
                ctx_len = get_model_context_length(getattr(agent, 'model', '') or _resolve_model(), base_url=getattr(agent, 'base_url', '') or '', api_key=getattr(agent, 'api_key', '') or '', provider=getattr(agent, 'provider', '') or '', config_context_length=getattr(agent, '_config_context_length', None))
                ctx = preprocess_context_references(prompt, cwd=cwd, allowed_root=cwd, context_length=ctx_len)
                if ctx.blocked:
            ...
            if images:
                try:
                    from agent.image_routing import decide_image_input_mode, build_native_content_parts
                    ...
                    _mode = decide_image_input_mode(_provider, _model, _cfg, requested_provider=getattr(agent, 'requested_provider', ''))
                    if getattr(agent, 'api_mode', '') == 'codex_app_server':
                        _mode = 'text'
                except Exception as _img_exc:
                    print(f'[tui_gateway] image_routing decision failed, defaulting to text: {_img_exc}', file=sys.stderr)
                    _mode = 'text'
                if _mode == 'native':
                    try:
                        _parts, _skipped = build_native_content_parts(prompt, images)
                        if _skipped:
                            print(f'[tui_gateway] native image attachment skipped {len(_skipped)} unreadable path(s)', file=sys.stderr)
                        if any((p.get('type') == 'image_url' for p in _parts)):
                            run_message = _parts
                        else:
                            run_message = _enrich_with_attached_images(prompt, images)
                    except Exception as _img_exc:
                        print(f'[tui_gateway] native attach failed, falling back to text: {_img_exc}', file=sys.stderr)
                        run_message = _enrich_with_attached_images(prompt, images)
                else:
                    run_message = _enrich_with_attached_images(prompt, images)
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        ...
                        try:
                            ...
                            return True
                    thinking_started = start_thinking_sound(should_play=_thinking_should_play)
                except Exception:
                    thinking_started = False
            ...
            run_kwargs = {'conversation_history': list(history), 'stream_callback': _stream, 'persist_user_message': _build_persist_user_message(prompt, images, run_message) if images else prompt}
            try:
            ...
            if display_kind and isinstance(text, str):
                ...
                if db is not None:
                    try:
                        db.set_latest_matching_message_display_kind(current_session_id, role='user', content=text, display_kind=display_kind, display_metadata=display_metadata)
                    except Exception:
            ...
            if 'moa_one_shot_restore' in session:
                ...
                if isinstance(_restore, dict):
                    ...
                    if _prev_model:
                        ...
                        try:
                            _apply_model_switch(sid, session, _raw, confirm_expensive_model=False, pin_session_override=bool(_prev_override), persist_override=False)
            ...
            if isinstance(result, dict):
                if isinstance(result.get('messages'), list):
                    with session['history_lock']:
                        ...
                        if current_version == history_version:
                            ...
                            history_no_markers = [e for e in history if not _is_model_switch_marker(e)]
                            current_no_markers = [e for e in current_history if not _is_model_switch_marker(e)]
                            model_switch_only = current_no_markers == history_no_markers and any((_is_model_switch_marker(e) for e in current_history))
                            if model_switch_only:
                                ...
                                print(f'[tui_gateway] prompt.submit: history_version mismatch (expected={history_version} current={current_version}) — agent output NOT written to session history', file=sys.stderr)
                                status_note = 'History changed during this turn — the response above is visible but was not saved to session history.'
                ...
                if status == 'interrupted' and isinstance(raw, str) and raw.strip().startswith(INTERRUPT_WAITING_FOR_MODEL_PREFIX):
                    raw = ''
            ...
            with session['history_lock']:
                if status == 'error':
                    _fail_inflight_turn(session, result.get('error') if isinstance(result, dict) else raw)
                    turn_error_retained = True
                else:
                    _clear_inflight_turn(session)
            if status == 'error':
                payload['error'] = str((result.get('error') if isinstance(result, dict) else '') or raw)
            ...
            if status == 'complete' and isinstance(raw, str) and raw.strip():
                try:
                    ...
                    if sid_key:
                        ...
                        if goal_mgr.is_active():
                            try:
                                ...
                                _bg_procs = None
                            decision = goal_mgr.evaluate_after_turn(raw, user_initiated=True, background_processes=_bg_procs)
                    ...
                    print(f'[tui_gateway] goal continuation hook failed: {type(_goal_exc).__name__}: {_goal_exc}', file=sys.stderr)
            ...
            if _pending and status == 'complete':
                ...
                try:
                    ...
                    pass
            if status == 'complete' and isinstance(raw, str) and raw.strip() and isinstance(text, str) and text.strip():
                try:
                    from agent.title_generator import maybe_auto_title
                    _title_key = session.get('session_key') or sid
                    _title_model = getattr(agent, 'model', None)
                    _title_provider = getattr(agent, 'provider', None)
                    maybe_auto_title(_get_db(), _title_key, text, raw, session.get('history', []), main_runtime={'model': getattr(agent, 'model', None), 'provider': getattr(agent, 'provider', None), 'base_url': getattr(agent, 'base_url', None), 'api_key': getattr(agent, 'api_key', None), 'api_mode': getattr(agent, 'api_mode', None)}, runtime_validator=lambda: getattr(agent, 'model', None) == _title_model and getattr(agent, 'provider', None) == _title_provider, title_callback=lambda t, _k=_title_key: _emit('session.title', sid, {'session_id': _k, 'title': t}))
                except Exception:
                    pass
            if status == 'complete' and tts_queue is None and isinstance(raw, str) and raw.strip() and _voice_tts_enabled():
                try:
                    spoken = raw
            ...
            try:
                ...
                print(f'[gateway-turn] terminal error emit failed: {type(emit_exc).__name__}: {emit_exc}', file=sys.stderr, flush=True)
        ...
        if goal_followup:
            ...
            try:
                ...
                print(f'[tui_gateway] goal continuation dispatch failed: {type(_cont_exc).__name__}: {_cont_exc}', file=sys.stderr)
                with session['history_lock']:
                    session['running'] = False
        try:
            from tools.process_registry import process_registry
            drained = process_registry.drain_notifications(session_key=session.get('session_key', ''), owns_event=lambda e: _session_owns_notification_event(sid, session, e), skip_poll_observed=False)
            for index, (_evt, synth) in enumerate(drained):
                ...
                try:
                    ...
                    print(f'[tui_gateway] completion notification dispatch failed: {type(_n_exc).__name__}: {_n_exc}', file=sys.stderr)
                    with session['history_lock']:
                        session['running'] = False
        except Exception as _drain_exc:
            print(f'[tui_gateway] completion queue drain failed: {type(_drain_exc).__name__}: {_drain_exc}', file=sys.stderr)
...
def _decode_attach_base64(raw: str, *, mime_prefix: str) -> bytes | None:
    ...
    import re as _re
    cleaned = raw.strip()
    m = _re.match(f'^data:{_re.escape(mime_prefix)}[a-zA-Z0-9.+-]*;base64,(.*)$', cleaned, _re.DOTALL)
    if m:
        cleaned = m.group(1)
    cleaned = _re.sub('\\s+', '', cleaned)
    try:
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile DUCK_AGENT_HOME
...
def _format_ref_value(value: str) -> str:
    ...
    if _ATTACHMENT_REF_NEEDS_QUOTING_RE is None:
        _ATTACHMENT_REF_NEEDS_QUOTING_RE = _re.compile('[\\s()\\[\\]{}<>"\'`]')
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / '.duck-agent' / 'desktop-attachments'
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _sanitize_attachment_name(name: str) -> str:
    ...
    candidate = _re.sub('[\\x00-\\x1f]+', '_', candidate)
...
def _decode_attachment_data_url(data_url: str) -> bytes:
    ...
    m = _re.match('^data:[^;,]*(?:;[^;,=]+=[^;,]+)*;base64,(.*)$', cleaned, _re.DOTALL | _re.I)
    if m:
        cleaned = m.group(1)
    cleaned = _re.sub('\\s+', '', cleaned)
    try:
...
def _stage_session_file_attachment(session: dict, *, raw_path: str, data_url: str, name: str) -> tuple[Path, bool]:
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.duck-agent/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.duck-agent/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
        ...
        try:
            ...
            if session:
                ...
                if session.get('running'):
                    ...
                    return _ok(rid, {'key': key, 'value': pending_model, 'warning': '', 'confirm_required': False, 'confirm_message': '', 'scope': 'session', 'deferred': True})
                ...
                result = _apply_model_switch(params.get('session_id', ''), session, value, confirm_expensive_model=bool(params.get('confirm_expensive_model', False)), parsed_flags=parsed_flags)
            else:
                result = _apply_model_switch('', {'agent': None}, value, confirm_expensive_model=bool(params.get('confirm_expensive_model', False)))
            return _ok(rid, {'key': key, 'value': result['value'], 'warning': result['warning'], 'confirm_required': result.get('confirm_required', False), 'confirm_message': result.get('confirm_message', ''), 'scope': result.get('scope', 'session')})
        except Exception as e:
    ...
    if key == 'fast':
        ...
        if nv == 'fast':
            ...
            if agent is not None:
                ...
                target_model = (session_override.get('model') if isinstance(session_override, dict) else None) or _resolve_model()
            if not target_model:
    ...
    if key == 'focus':
        ...
        if target:
            saved = normalize_tool_progress_mode(d_f.get('focus_saved_tool_progress') or _load_tool_progress_mode() if cur_focus else _load_tool_progress_mode())
    ...
    if key == 'reasoning':
        try:
            ...
            if session and session.get('agent') is not None:
                ...
                _persist_live_session_runtime(session)
                _emit('session.info', params.get('session_id', ''), _session_info(session['agent'], session))
    ...
    if key in {'prompt', 'personality', 'skin'}:
        try:
            ...
            if key == 'prompt':
                ...
                history_reset, info = _apply_personality_to_session(sid_key, session, new_prompt, pname)
            else:
...
def _(rid, params, pdb, conn) -> dict:
    pid = pdb.create_project(conn, name=str(params.get('name') or ''), slug=params.get('slug'), folders=params.get('folders') or [], primary_path=params.get('primary_path'), description=params.get('description'), icon=params.get('icon'), color=params.get('color'), board_slug=params.get('board_slug'))
    if params.get('use'):
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under DUCK_AGENT_HOME (~/.duck-agent by default) — config/sessions/skills,
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    DUCK_AGENT_HOME may be an intentional prose/data workspace. The pre-Projects
...
def _repo_discovery_policy_key(policy: dict) -> str:
    ...
    canonical = {'enabled': bool(policy['enabled']), 'roots': _paths(policy['roots']), 'exclude_paths': _paths(policy['exclude_paths'])}
    return json.dumps(canonical, sort_keys=True, separators=(',', ':'))

def _repo_discovery_policy_is_default(policy: dict) -> bool:
    from hermes_cli.config import DEFAULT_CONFIG
    return _repo_discovery_policy_key(policy) == _repo_discovery_policy_key(_repo_discovery_policy(DEFAULT_CONFIG['desktop']))

def _discover_repos_payload(db, *, conn=None, backfill: bool=True, include_cached: bool=True) -> list[dict]:
    ...
    repos even with zero duck-agent sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (duck-agent home subtree + bare
...
def _project_tree_inputs(db, session_limit: int, *, include_discovered: bool) -> tuple[list[dict], list[dict], list[dict], str | None]:
    ...
    with pdb.connect_closing() as conn:
        ...
        discovered = _discover_repos_payload(db, conn=conn, backfill=False, include_cached=policy['enabled']) if include_discovered else []
...
def _build_project_tree(db, *, preview_limit: int, hydrate: bool, session_limit: int, include_discovered: bool) -> tuple[dict, str | None]:
    """Gather inputs and run the one authoritative builder. Returns (tree, active_id)."""
    from tui_gateway import project_tree
    _DIR_EXISTS_CACHE.clear()
    sessions, projects, discovered, active_id = _project_tree_inputs(db, session_limit, include_discovered=include_discovered)
    tree = project_tree.build_tree(projects, sessions, discovered, _resolve_cwd_git, preview_limit=preview_limit, hydrate=hydrate, is_junk_root=_is_repo_junk, is_junk_cwd=_is_session_cwd_junk, exists=_dir_exists_cached)
...
_TUI_EXTRA: list[tuple[str, str, str]] = [('/density', 'Toggle compact display mode', 'TUI'), ('/logs', 'Show recent gateway log lines', 'TUI'), ('/mouse', 'Set mouse tracking preset [on|off|toggle|wheel|buttons|all]', 'TUI'), ('/sessions', 'Switch between live TUI sessions', 'TUI')]
_PENDING_INPUT_COMMANDS: frozenset[str] = frozenset({'retry', 'queue', 'q', 'steer', 'plan', 'goal', 'moa', 'undo', 'learn', 'init', 'compress', 'compact'})
...
def _rank_slash_completions(items: list[dict], usage, origin_of, *, browsing: bool) -> list[dict]:
    ...
    of skills someone invokes daily lead the ones that shipped with Duck Agent
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return 'bare `duck-agent` is interactive — use `/duck-agent chat -q …` or run `duck-agent` in another terminal'
    a0 = argv[0].lower()
    if a0 == 'setup':
        return '`duck-agent setup` needs a full terminal — run it outside the TUI'
    if a0 == 'gateway':
        return '`duck-agent gateway` is long-running — run it in another terminal'
    if a0 == 'sessions' and len(argv) > 1 and (argv[1].lower() == 'browse'):
        return '`duck-agent sessions browse` is interactive — use /resume here, or run browse in another terminal'
    if a0 == 'config' and len(argv) > 1 and (argv[1].lower() == 'edit'):
        return '`duck-agent config edit` needs $EDITOR in a real terminal'
    return None
...
_FUZZY_FALLBACK_EXCLUDES = frozenset({'.git', '.hg', '.svn', '.next', '.cache', '.venv', 'venv', 'node_modules', '__pycache__', 'dist', 'build', 'target', '.mypy_cache', '.pytest_cache', '.ruff_cache'})
...
def _list_repo_files(root: str) -> list[str]:
    ...
    try:
        top_result = subprocess.run(['git', '-C', root, 'rev-parse', '--show-toplevel'], capture_output=True, timeout=2.0, check=False, stdin=subprocess.DEVNULL, creationflags=_creationflags)
        if top_result.returncode == 0:
            top = top_result.stdout.decode('utf-8', 'replace').strip()
            list_result = subprocess.run(['git', '-C', top, 'ls-files', '-z', '--cached', '--others', '--exclude-standard'], capture_output=True, timeout=2.0, check=False, stdin=subprocess.DEVNULL, creationflags=_creationflags)
            if list_result.returncode == 0:
                for p in list_result.stdout.decode('utf-8', 'replace').split('\x00'):
                    if not p:
                        continue
...
def _details_completions(text: str) -> list[dict] | None:
    ...
    if not body or (len(parts) == 0 and has_trailing_space):
        return [*[_details_root_completion_item(mode, 'global mode', not has_trailing_space) for mode in modes], _details_root_completion_item('cycle', 'cycle global mode', not has_trailing_space), *[_details_root_completion_item(section, 'section override', not has_trailing_space) for section in sections]]
    if len(parts) == 1 and (not has_trailing_space):
        prefix = parts[0].lower()
        candidates = [*modes, 'cycle', *sections]
        return [_details_completion_item(candidate, 'section override' if candidate in sections else 'cycle global mode' if candidate == 'cycle' else 'global mode') for candidate in candidates if candidate.startswith(prefix) and candidate != prefix]
    if len(parts) == 1 and has_trailing_space and (parts[0].lower() in sections):
        return [*[_details_completion_item(mode, f'set {parts[0].lower()}') for mode in modes], _details_completion_item('reset', f'clear {parts[0].lower()} override')]
    if len(parts) == 2 and (not has_trailing_space) and (parts[0].lower() in sections):
        prefix = parts[1].lower()
        return [_details_completion_item(candidate, f'clear {parts[0].lower()} override' if candidate == 'reset' else f'set {parts[0].lower()}') for candidate in (*modes, 'reset') if candidate.startswith(prefix) and candidate != prefix]
    return []
...
def _model_picker_context(agent):
    ...
    if str(provider or '').strip().lower() == 'custom':
        try:
            from hermes_cli.runtime_provider import canonical_custom_identity
            provider = canonical_custom_identity(base_url=base_url or None, config_provider=ctx.current_provider, model=(getattr(agent, 'model', '') if agent else '') or None) or provider
        except Exception:
            logger.debug('custom provider identity recovery failed (model picker)', exc_info=True)
    return ctx.with_overrides(current_provider=provider, current_model=(getattr(agent, 'model', '') if agent else '') or _resolve_model(), current_base_url=base_url)
_LIVE_SESSION_DIRECT_COMMANDS = frozenset({'clear', 'compress', 'effort', 'history', 'models', 'prompt', 'rename', 'status', 'usage'})
...
def _format_live_usage_output(session: dict) -> str:
    ...
    lines = ['Session Token Usage', '────────────────────────────────────────', f"Model: {usage.get('model') or _metadata_mirror(session).get('model') or getattr(agent, 'model', '') or '(unknown)'}", f"Input tokens:                 {int(usage.get('input') or 0):,}", f"Output tokens:                {int(usage.get('output') or 0):,}"]
    reasoning = int(usage.get('reasoning') or 0)
    if reasoning:
        lines.append(f'Reasoning tokens:             {reasoning:,}')
    lines.extend([f"Prompt tokens:                {int(usage.get('prompt') or 0):,}", f"Completion tokens:            {int(usage.get('completion') or 0):,}", f"Total tokens:                 {int(usage.get('total') or 0):,}", f"API calls:                    {int(usage.get('calls') or 0):,}"])
    if usage.get('context_max'):
        lines.append(f"Current context:              {int(usage.get('context_used') or 0):,} / {int(usage.get('context_max') or 0):,} ({int(usage.get('context_percent') or 0)}%)")
    lines.extend([f'Messages:                     {message_count:,}', f"Compressions:                 {int(usage.get('compressions') or 0):,}"])
    return '\n'.join(lines)
...
def _format_live_history_output(session: dict) -> str:
    ...
    if db is not None and session.get('session_key'):
        try:
            history = db.get_messages_as_conversation(session['session_key'], include_ancestors=True, include_row_ids=True)
        except Exception:
            pass
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = 'You' if role == 'user' else 'Duck Agent' if role == 'assistant' else role.title()
...
def _format_live_prompt_output(session: dict) -> str:
    ...
    prompt = mirror.get('system_prompt') or getattr(agent, 'ephemeral_system_prompt', None) or getattr(agent, '_cached_system_prompt', None) or ''
    if not prompt:
...
def _format_live_context_output(session: dict) -> str:
    ...
    if db is not None and session.get('session_key'):
        try:
            messages = _history_to_messages(db.get_messages_as_conversation(session['session_key'], include_ancestors=True, include_row_ids=True))
        except Exception:
            messages = []
    if not messages:
        with session['history_lock']:
    ...
    lines.append(f"  user: {roles.get('user', 0)}, assistant: {roles.get('assistant', 0)}, tool: {roles.get('tool', 0)}, system: {roles.get('system', 0)}")
    ...
    if context_used:
        if context_max:
            ...
            lines.append(f'Context usage: ~{context_used:,} / {context_max:,} tokens ({usage_pct:.1f}%)')
        else:
...
def _live_slash_command_output(sid: str, session: Optional[dict], name: str, arg: str) -> Optional[str]:
    ...
    if name not in _LIVE_SESSION_DIRECT_COMMANDS:
        if not (name in _ISOLATED_SESSION_READ_COMMANDS and session is not None and _session_uses_compute_host(session)):
            return None
...
def _mirror_slash_side_effects(sid: str, session: dict, command: str) -> str:
    ...
    if _session_uses_compute_host(session) and name in _MUTATES_WHILE_RUNNING:
        ...
        try:
            ack = _send_compute_host_control(sid, route_name=route_name, command=command, wait=True)
        except Exception as exc:
    ...
    try:
        if name == 'model' and arg and agent:
            ...
            _sys_prompt = getattr(agent, '_cached_system_prompt', '') or ''
            _tools = getattr(agent, 'tools', None) or None
            _before_tokens = estimate_request_tokens_rough(_before_messages, system_prompt=_sys_prompt, tools=_tools) if _before_count else 0
            try:
            ...
            _after_tokens = estimate_request_tokens_rough(_after_messages, system_prompt=_sys_prompt_after, tools=_tools_after) if _after_messages else 0
            _emit('session.info', sid, _session_info(agent, session))
            _fb = summarize_manual_compression(_before_messages, _after_messages, _before_tokens, _after_tokens, compression_state=getattr(agent, 'context_compressor', None))
...
def _tts_stream_stop(user_barge: bool=True) -> None:
    ...
    *user_barge* latches the interruption for the next turn's model note
    ...
    if user_barge and (not state['done'].is_set()):
        ...
        logger.debug('TTS CUT: _tts_stream_stop(user_barge=True) — new turn or interrupt cutting in-flight TTS\n%s', ''.join(_tb.format_stack()))
...
def _full_duplex_listener() -> None:
    ...
    * playback phase: cuts TTS (streaming pipeline + fallback speak paths +
    ...
    try:
        from tools.tts_streaming import mark_speech_interrupted
        from tools.voice_mode import full_duplex_listen, is_audio_output_active, stop_playback, transcribe_recording
        cfg = _voice_cfg_dict()
        try:
        ...
        wav_path = full_duplex_listen(_should_stop, is_playing=is_audio_output_active, on_trigger=_on_trigger, multiplier=_mult or None, grace_ms=max(0, _grace_ms))
...
def _wake_resume_if_owner(owner: 'Transport', *, retry_seconds: float=15.0, retry_interval: float=1.0) -> bool:
        ...
        try:
            while time.monotonic() < deadline:
                ...
                try:
                    ...
                    continue
                return
            logger.warning('wake: could not resume detector after voice turn (microphone still busy?) — toggle the wake word to re-arm')
        finally:
            with _wake_resume_retry_lock:
                _wake_resume_retry_active = False
...
def _(rid, params: dict) -> dict:
    ...
    transport = current_transport() or _stdio_transport
    try:
        from tools.wake_word import WakeWordInUse, check_wake_word_requirements, detector_frame_info, load_wake_word_config, owns_listener, resolve_capture_mode, start_listening, wake_phrase, wake_surface_enabled
    except Exception as e:
    ...
    if not reqs['available']:
        ...
        return _ok(rid, {'started': False, 'reason': 'unavailable', 'hint': reqs.get('hint') or '', 'capture': capture_mode})
    ...
    def _on_detect() -> None:
        ...
        logger.info('wake.detected: emitting to sid=%r (transport=%s, profile=%r)', sid, type(transport).__name__, matched_profile)
        token = bind_transport(transport)
        try:
            _emit('wake.detected', sid, {'phrase': matched_phrase or phrase, 'profile': matched_profile or None, 'start_new_session': new_session})
        finally:
            reset_transport(token)
    try:
        start_listening(_on_detect, owner=transport, config=cfg, external_audio=external_audio)
    ...
    logger.info('wake.start(%s): listening for %r (%s) capture=%s frame=%s', surface, reqs['phrase'], reqs['provider'], capture_mode, frame.get('frame_length'))
    return _ok(rid, {'started': True, 'phrase': reqs['phrase'], 'provider': reqs['provider'], 'owner_surface': surface, 'enabled_persisted': enabled_persisted, 'capture': capture_mode, 'sample_rate': frame.get('sample_rate', 16000), 'frame_length': frame.get('frame_length', 1280)})
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {'stopped': stopped, 'reason': None if stopped else 'not_owner', 'disabled_persisted': disabled_persisted})
...
def _(rid, params: dict) -> dict:
    try:
        from tools.wake_word import audio_is_silent, check_wake_word_requirements, detector_frame_info, get_input_device_status, is_listening, load_wake_word_config, owns_listener, resolve_capture_mode, silent_audio_hint
        cfg = load_wake_word_config()
        prefer_client = bool(params.get('client_capture')) or str(params.get('surface') or '').strip().lower() in ('gui', 'desktop')
        probe_cfg = dict(cfg)
        ...
        if owned_by_caller and frame.get('external_audio'):
            ...
            capture = probe_cfg.get('capture') or reqs.get('capture') or str(cfg.get('capture') or 'auto')
        return _ok(rid, {'listening': listening, 'owned_by_caller': owned_by_caller, 'owner_surface': owner_surface if owner is not None else None, 'phrase': reqs['phrase'], 'provider': reqs['provider'], 'configured_surface': str(cfg.get('surface') or 'auto'), 'input_device': input_device, 'available': reqs['available'], 'hint': hint, 'enabled': bool(cfg.get('enabled')), 'audio_silent': silent, 'capture': capture, 'local_input_available': bool(reqs.get('local_input_available')), 'sample_rate': frame.get('sample_rate', 16000), 'frame_length': frame.get('frame_length', 1280)})
    except Exception as e:
        return _err(rid, 5026, str(e))
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {'on', 'off'}:
        ...
        return _ok(rid, {'enabled': enabled, 'record_key': _voice_record_key(), 'tts': _voice_tts_enabled(), 'stop_hint': stop_hint})
    if action == 'tts':
        if not _voice_mode_enabled():
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == 'start':
            ...
            safe_max_rec = (max_rec if max_rec > 0 else 0.0) if isinstance(max_rec, (int, float)) and (not isinstance(max_rec, bool)) else 120.0
            started = start_continuous(on_transcript=_on_transcript, on_status=_on_status, on_silent_limit=_on_silent, silence_threshold=safe_threshold, silence_duration=safe_duration, auto_restart=False, max_recording_seconds=safe_max_rec, on_stop_phrase=_on_stop_phrase)
            if started is False:
                _resume_voice_wake()
...
def _is_default_local_cdp(parsed) -> bool:
    ...
    return parsed.scheme in {'http', 'ws'} and parsed.hostname in {'127.0.0.1', 'localhost'} and (port == 9222) and discovery_path
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    ...
    hint = ['Start a Chromium-family browser with remote debugging, then retry /browser connect:', command] if command else ['No supported Chromium-family browser executable was found in this environment.', f'Install one or start a Chromium-family browser with --remote-debugging-port={port}, then retry /browser connect.']
    return [f'Browser CDP is not reachable at {url}.', *hint, 'Browser not connected — start a Chromium-family browser with remote debugging and retry /browser connect']
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    try:
        if parsed.scheme in {'ws', 'wss'} and parsed.path.startswith('/devtools/browser/'):
            ...
            from hermes_cli.browser_connect import discover_local_cdp_url, find_free_debug_port, launch_chrome_debug, local_port_in_use
            discovered = discover_local_cdp_url(port, timeout=2.0)
            launch_port = port
            if discovered is None:
                if local_port_in_use(port):
                    launch_port = find_free_debug_port(port)
                    announce(f"Port {port} is occupied by another application that isn't a CDP browser (an IDE debugger or dev server may be using it) — launching a debug browser on port {launch_port} instead...")
                else:
...
from . import methods_complete as _methods_complete, methods_config as _methods_config, methods_prompt as _methods_prompt, methods_session as _methods_session, methods_tools as _methods_tools
for _m in (_methods_session, _methods_prompt, _methods_config, _methods_complete, _methods_tools):
