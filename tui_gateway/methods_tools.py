    ...
    try:
        ...
        if not user_confirm:
            ...
            if _confirm_required:
                return _ok(rid, {'status': 'confirm_required', 'message': '⚠️  /reload-mcp invalidates the prompt cache (next message re-sends full input tokens). Reply `/reload-mcp now` to proceed, or `/reload-mcp always` to proceed and silence this prompt permanently.'})
        if session and _session_uses_compute_host(session):
            try:
                ack = _get_compute_host_supervisor().reload_mcp(str(params.get('session_id') or ''), request_id=f'reload-mcp-{rid}')
...
@method('reload.env')
def _(rid, params: dict) -> dict:
    """Re-read ``~/.duck-agent/.env`` into the gateway process via
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        return _ok(rid, {'blocked': False, 'code': r.returncode, 'output': out[:48000]})
...
def _(rid, params: dict) -> dict:
    ...
    if name in qcmds:
        ...
        if qc.get('type') == 'exec':
            ...
            from hermes_cli._subprocess_compat import windows_hide_flags
            r = subprocess.run(qc.get('command', ''), shell=True, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, stdin=subprocess.DEVNULL, env=sanitized_env, creationflags=windows_hide_flags())
    ...
    if bundle_key is not None:
        try:
            bundle_result = build_bundle_invocation_message(bundle_key, arg, task_id=session.get('session_key', '') if session else '', platform=_resolve_session_platform())
        except Exception as exc:
    ...
    try:
        ...
        if key in cmds:
            ...
            if msg:
                return _ok(rid, {'type': 'skill', 'message': msg, 'name': cmds[key].get('name', name), 'display': _skill_scaffold_projection(msg)})
    except Exception:
        pass
    if name in {'queue', 'q'}:
        if not arg:
    ...
    if name == 'moa':
        try:
            ...
            if agent is not None:
                try:
                    _apply_model_switch(sid, session, f'{preset} --provider moa', confirm_expensive_model=False, pin_session_override=True, persist_override=False)
                except Exception as exc:
            ...
            return _ok(rid, {'type': 'send', 'notice': f'MoA one-shot queued with preset {preset}; previous model will be restored after this turn.', 'message': arg})
        except Exception as exc:
            return _err(rid, 5030, f'moa unavailable: {exc}')
    if name == 'focus':
        ...
        _res = _methods['config.set'](rid, {'key': 'focus', 'value': 'on' if _target else 'off', 'session_id': params.get('session_id', '')})
        if 'error' in _res:
            return _res
        _payload = _res.get('result') or {}
        return _ok(rid, {'type': 'exec', 'output': format_focus_toggle_message(bool(_target), _payload.get('tool_progress') or 'all')})
    if name == 'retry':
        if not session:
    ...
    if name in {'snapshot', 'snap'}:
        ...
        if subcommand in {'restore', 'rewind'}:
            return _ok(rid, {'type': 'exec', 'output': '/snapshot restore is blocked in the TUI because it changes config/state on disk while the live agent has cached settings. Run it in the classic CLI, then restart the TUI.'})
    if name in {'compress', 'compact'}:
        ...
        if _session_uses_compute_host(session):
            ...
            try:
                ack = _send_compute_host_control(sid, route_name='slash.compress', command=command, wait=True)
            except Exception as exc:
...
def _(rid, params: dict) -> dict:
    ...
    if _cmd_base in _PENDING_INPUT_COMMANDS:
        return _methods['command.dispatch'](rid, {'name': _cmd_base, 'arg': _cmd_arg, 'session_id': params.get('session_id', '')})
    ...
    try:
        ...
        if _bundle_key is not None:
            return _methods['command.dispatch'](rid, {'name': _bundle_key.lstrip('/'), 'arg': _cmd_arg, 'session_id': params.get('session_id', '')})
    except Exception:
        pass
    try:
    ...
    if not worker:
        ...
        with spawn_lock:
            ...
            if not worker:
                try:
                    worker = _SlashWorker(session['session_key'], getattr(session.get('agent'), 'model', _resolve_model()), profile_home=session.get('profile_home'))
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        def go(mgr, cwd):
            ...
            return _ok(rid, {'enabled': True, 'checkpoints': [{'hash': c.get('hash', ''), 'timestamp': c.get('timestamp', ''), 'message': c.get('message', '')} for c in mgr.list_checkpoints(cwd)]})
...
def _(rid, params: dict) -> dict:
    try:
        ...
        return _ok(rid, {'plugins': [{'name': n, 'version': getattr(i, 'version', '?'), 'enabled': getattr(i, 'enabled', True)} for n, i in get_plugin_manager()._plugins.items()]})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        sections = [{'title': 'Model', 'rows': [['Model', model], ['Base URL', base_url or '(default)'], ['API Key', masked]]}, {'title': 'Agent', 'rows': [['Max Turns', str(_cfg_max_turns(cfg, 500))], ['Toolsets', ', '.join(cfg.get('enabled_toolsets', [])) or 'all'], ['Verbose', str(cfg.get('verbose', False))]]}, {'title': 'Environment', 'rows': [['Working Dir', os.getcwd()], ['Config File', str(_hermes_home / 'config.yaml')]]}]
...
def _(rid, params: dict) -> dict:
    try:
        ...
        return _ok(rid, {'sections': [{'name': name, 'tools': rows} for name, rows in sorted(sections.items())], 'total': len(tools)})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        return _ok(rid, {'changed': changed, 'enabled_toolsets': enabled, 'info': info, 'missing_servers': sorted(missing_servers), 'reset': bool(session), 'unknown': unknown})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        return _ok(rid, {'processes': [{'session_id': p['session_id'], 'command': p['command'][:80], 'status': p['status'], 'uptime': p['uptime_seconds']} for p in procs]})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == 'add':
            return _ok(rid, json.loads(cronjob(action='create', name=jid, schedule=params.get('schedule', ''), prompt=params.get('prompt', ''))))
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``duck-agent journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``duck-agent plugins`` / the dashboard, so the three surfaces
    ...
    try:
        ...
        if action == 'toggle':
            ...
            return _ok(rid, {'ok': True, 'unchanged': bool(result.get('unchanged')), 'name': name, 'plugin': row})
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hermes_cli._subprocess_compat import windows_hide_flags
        r = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=30, cwd=os.getcwd(), encoding='utf-8', errors='replace', stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
