...
class SpikeAgent:
    ...
    def run_conversation(self, prompt: str, *, conversation_history: list[dict[str, str]] | None=None, stream_callback: Callable[[str], None] | None=None, delta_count: int=24, delay_s: float=0.001) -> dict[str, Any]:
...
def _build_sha() -> str:
    try:
        return subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=str(_repo_root()), text=True, encoding='utf-8', errors='replace', stderr=subprocess.DEVNULL, timeout=2).strip()
    except Exception:
        return 'unknown'
...
class ComputeHost:
    ...
    def __init__(self, *, stdout: Any=None, max_workers: int | None=None, heartbeat_secs: int | float | None=None) -> None:
        self._stdout = stdout or sys.stdout
        self._write_lock = threading.Lock()
        self._sessions: dict[str, HostSession] = {}
        self._executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers or _default_workers(), thread_name_prefix='compute-host-turn')
        ...
        self._heartbeat_secs = float(heartbeat_secs) if heartbeat_secs is not None else float(os.environ.get('HERMES_COMPUTE_HOST_HEARTBEAT_SECS') or '15')
    ...
    def flush_all_sessions(self, *, reason: str='shutdown', skip_sids: Collection[str] | None=None) -> None:
    ...
    def _run_spike_turn(self, session: HostSession, frame: dict[str, Any]) -> None:
        ...
        try:
            result = session.agent.run_conversation(prompt, conversation_history=history, stream_callback=stream, delta_count=delta_count, delay_s=delay_s)
            with session.lock:
                session.history_version += 1
                session.running = False
                history_version = session.history_version
            self._bump_progress()
            self.emit({'type': 'turn.end', 'sid': session.sid, 'request_id': request_id, 'history_version': history_version, 'message_count': len(result.get('messages') or []), 'interrupted': bool(result.get('interrupted')), 'ended_ns': now_ns()})
        except Exception as exc:
            with session.lock:
                session.running = False
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            with session['history_lock']:
                ...
                if queued_prompt_generation is not None and int(session.get('_queued_prompt_generation', 0)) != int(queued_prompt_generation):
            ...
            self._bump_progress()
            self.emit({'type': 'turn.end', 'sid': sid, 'request_id': request_id, 'history_version': history_version, 'session_key': session_key, 'message_count': message_count, 'interrupted': interrupted, 'ended_ns': now_ns(), 'session_info': session_info, 'session_info_emitted': True})
        except Exception as exc:
            try:
                from tui_gateway import server
                session = server._sessions.get(sid)
    ...
    def _ensure_server_session(self, server: Any, frame: dict[str, Any]) -> dict:
        ...
        try:
            if profile_home:
                ...
                owns_db = True
            agent = server._make_agent(sid, key, session_id=key, model_override=frame.get('model_override'), reasoning_config_override=frame.get('reasoning_config_override'), service_tier_override=frame.get('service_tier_override'), platform_override=frame.get('source'), session_db=session_db)
        ...
        try:
            ...
            try:
                server._init_session(sid, key, agent, list(history), cols=int(frame.get('cols') or 80), cwd=str(frame.get('cwd') or '') or None, session_db=session_db, source=frame.get('source'))
            finally:
                reset_transport(token)
        except Exception:
            server._sessions[sid] = {'agent': agent, 'session_key': key, 'history': list(history), 'history_lock': threading.Lock(), 'history_version': int(frame.get('history_version') or 0), 'inflight_turn': None, 'created_at': time.time(), 'last_active': time.time(), 'running': False, 'attached_images': [], 'image_counter': 0, 'cwd': str(frame.get('cwd') or os.getcwd()), 'cols': int(frame.get('cols') or 80), 'slash_worker': None, 'show_reasoning': server._load_show_reasoning(), 'tool_progress_mode': server._load_tool_progress_mode(), 'edit_snapshots': {}, 'tool_started_at': {}, 'model_override': frame.get('model_override'), 'source': server._sanitize_client_source(frame.get('source')), 'transport': self._transport}
    ...
    def _handle_control(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            if route_name == 'session.save':
                response = server._methods['session.save'](request_id, {'session_id': sid})
                if 'error' in response:
                    self.emit({'type': 'control.error', 'sid': sid, 'request_id': request_id, 'message': str(response['error'].get('message') or 'session save failed')})
                    return
                self.emit({'type': 'control.ack', 'sid': sid, 'request_id': request_id, 'route_name': route_name, 'result': response.get('result') or {}})
                return
            if route_name == 'session.compress':
                command = str(frame.get('command') or '')
                focus_topic = command.removeprefix('/compress').strip()
                response = server._methods['session.compress'](request_id, {'session_id': sid, **({'focus_topic': focus_topic} if focus_topic else {})})
                if 'error' in response:
                    self.emit({'type': 'control.error', 'sid': sid, 'request_id': request_id, 'message': str(response['error'].get('message') or 'session compression failed')})
                    return
                with session['history_lock']:
                    session_key = str(session.get('session_key') or '')
                    history_version = int(session.get('history_version', 0))
                    message_count = len(session.get('history') or [])
                self.emit({'type': 'control.ack', 'sid': sid, 'request_id': request_id, 'route_name': route_name, 'result': response.get('result') or {}, 'session_key': session_key, 'history_version': history_version, 'message_count': message_count, 'session_info': server._session_info(session.get('agent'), session)})
                return
            command = str(frame.get('command') or '')
            output = ''
            if command:
            ...
            with session['history_lock']:
                ...
                message_count = len(session.get('history') or [])
                session_key = str(session.get('session_key') or '')
            self.emit({'type': 'control.ack', 'sid': sid, 'request_id': request_id, 'route_name': route_name, 'output': output, 'session_key': session_key, 'history_version': history_version, 'message_count': message_count, 'messages': messages, 'session_info': server._session_info(session.get('agent'), session)})
        except Exception as exc:
...
def run_host(stdin: Any=None, stdout: Any=None) -> None:
    ...
    try:
        ...
        pass
    host.emit({'type': 'hello', 'host_pid': os.getpid(), 'boot_id': host._boot_id, 'build_sha': _build_sha(), 'cwd': os.getcwd(), 'duck_agent_home': os.environ.get('DUCK_AGENT_HOME', '')})
