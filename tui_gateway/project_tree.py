...
_KANBAN_DIR_RE = re.compile('^(.*[/\\\\]\\.worktrees)[/\\\\]t_[0-9a-f]+[/\\\\]?$')
...
def _segments(path: str) -> list[str]:
    return [s for s in re.split('[/\\\\]', (path or '').rstrip('/\\')) if s]

def _is_windows_path(path: str) -> bool:
    value = (path or '').strip()
    return bool(re.match('^[A-Za-z]:[/\\\\]', value)) or value.startswith(('\\', '//'))
...
def _with_base_name(path: str, name: str) -> str:
    stripped = re.sub('[/\\\\]+$', '', path)
    return re.sub('[^/\\\\]+$', name, stripped)

def _placement(repo_root: str, lane_key: str, lane_label: str, lane_path: str, is_main: bool, is_kanban: bool) -> dict:
    return {'repo_key': repo_root, 'repo_label': base_name(repo_root) or repo_root, 'repo_path': repo_root, 'lane_key': lane_key, 'lane_label': lane_label, 'lane_path': lane_path, 'is_main': is_main, 'is_kanban': is_kanban}

def _parent_dir(path: str) -> str:
    """The containing directory of ``path`` (``""`` once the root is passed)."""
    stripped = re.sub('[/\\\\]+$', '', path or '')
    return re.sub('[/\\\\]+$', '', re.sub('[^/\\\\]+$', '', stripped))
...
def _place_by_heuristic(path: str) -> Optional[dict]:
    ...
    if kanban_dir:
        repo_path = re.sub('[/\\\\]+$', '', _with_base_name(kanban_dir, ''))
...
def _build_repos(sessions: list[dict], resolve: Optional[Resolve], hydrate: bool) -> list[dict]:
    ...
    for session in sessions:
        ...
        if entry is None:
            entry = {'group': {'id': placement['lane_key'], 'label': placement['lane_label'], 'path': placement['lane_path'], 'isMain': placement['is_main'], 'isKanban': placement['is_kanban'], 'sessions': []}, 'repo_key': placement['repo_key'], 'repo_label': placement['repo_label'], 'repo_path': placement['repo_path']}
    ...
    for entry in lanes.values():
        ...
        if repo is None:
            repo = {'id': entry['repo_key'], 'label': entry['repo_label'], 'path': entry['repo_path'], 'groups': [], 'sessionCount': 0}
...
def _seed_folder_repos(repos: list[dict], folders: list[dict], resolve: Optional[Resolve]) -> list[dict]:
    ...
    for folder in folders or []:
        ...
        root = (info or {}).get('repo_root') or re.sub('[/\\\\]+$', '', raw)
...
def _project_node(*, pid: str, label: str, path: Optional[str], repos: list[dict], session_count: int, last_active: float, preview_sessions: list[dict], color: Any=None, icon: Any=None, is_auto: bool=False, is_no_project: bool=False) -> dict:
    return {'id': pid, 'label': label, 'path': path, 'color': color, 'icon': icon, 'isAuto': is_auto, 'isNoProject': is_no_project, 'sessionCount': session_count, 'lastActive': last_active, 'repos': repos, 'previewSessions': preview_sessions}

def build_tree(projects: list[dict], sessions: list[dict], discovered_repos: list[dict], resolve: Optional[Resolve]=None, *, preview_limit: int=3, hydrate: bool=False, is_junk_root: Optional[Callable[[str], bool]]=None, is_junk_cwd: Optional[Callable[[str], bool]]=None, exists: Optional[Exists]=None) -> dict:
    ...
    bare home dir, the DUCK_AGENT_HOME subtree). ``is_junk_cwd`` is the narrower
    policy for non-git session folders: selected descendants may be intentional
    workspaces even when their parent tree contains Duck Agent state. User-created
    ...
    for project in active_projects:
        ...
        result.append(_project_node(pid=project['id'], label=project.get('name') or project['id'], path=project.get('primary_path'), color=project.get('color'), icon=project.get('icon'), repos=repos, session_count=len(psessions), last_active=_last_active(psessions), preview_sessions=_previews(psessions)))
    ...
    for bucket in by_auto_root.values():
        ...
        repo_node = next((repo for repo in repos if _path_key(repo.get('id') or repo.get('path') or '') == auto_key), None)
        if repo_node is None:
            homeless.extend(auto_sessions)
            continue
        seen.add(auto_key)
        scoped_ids.extend((s['id'] for s in auto_sessions if s.get('id')))
        result.append(_project_node(pid=auto_root, label=base_name(auto_root) or auto_root, path=auto_root, repos=repos, session_count=repo_node['sessionCount'], last_active=_last_active(auto_sessions), preview_sessions=_previews(auto_sessions), is_auto=True))
    for repo in discovered_repos or []:
        ...
        result.append(_project_node(pid=root, label=label, path=root, repos=[{'id': root, 'label': label, 'path': root, 'groups': [], 'sessionCount': 0}], session_count=int(repo.get('sessions') or 0), last_active=float(repo.get('last_active') or 0), preview_sessions=[], is_auto=True))
    ...
    if homeless:
        ...
        lane = {'id': NO_PROJECT_ID, 'label': NO_PROJECT_LABEL, 'path': None, 'isMain': False, 'isKanban': False, 'sessions': homeless if hydrate else []}
        result.insert(0, _project_node(pid=NO_PROJECT_ID, label=NO_PROJECT_LABEL, path=None, repos=[{'id': NO_PROJECT_ID, 'label': NO_PROJECT_LABEL, 'path': None, 'groups': [lane], 'sessionCount': len(homeless)}], session_count=len(homeless), last_active=_last_active(homeless), preview_sessions=_previews(homeless), is_no_project=True))
