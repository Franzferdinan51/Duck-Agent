...
def _(rid, params: dict) -> dict:
    ...
    while True:
        ...
        with session['history_lock']:
            if session.get('running'):
                ...
                break
        busy_response = _handle_busy_submit(rid, sid, session, text, busy_transport, queued=bool(params.get('queued')))
    ...
    with session['history_lock']:
        ...
        if truncate_user_ordinal is not None:
            ...
            if not is_truthy_value(params.get('confirm_truncate')):
                logger.warning('prompt.submit: REFUSED unconfirmed truncation of session %s (%d messages held; ordinal=%d). The client attached truncate_before_user_ordinal without confirm_truncate — likely a stale ordinal on an ordinary submit.', sid, len(history), ordinal)
                return _err(rid, 4029, 'truncate_before_user_ordinal requires confirm_truncate=true; an ordinary prompt.submit must not drop session history (update your Duck Agent client if a rewind was intended)')
            user_indices = [i for i, m in enumerate(history) if m.get('role') == 'user' and (not m.get('display_kind'))]
            if ordinal < 0 or ordinal >= len(user_indices):
                return _err(rid, 4018, 'target user message is no longer in session history')
            truncated = history[:user_indices[ordinal]]
            if not truncated and history and (not is_truthy_value(params.get('confirm_empty_truncate'))):
                logger.warning('prompt.submit: REFUSED empty truncation of session %s (%d messages would be wiped; ordinal=%d).', sid, len(history), ordinal)
                return _err(rid, 4028, 'truncation would erase the entire session transcript; resubmit with confirm_empty_truncate=true if this is intended')
            log_fn = logger.warning if not truncated else logger.info
            log_fn('prompt.submit: truncating session %s history %d -> %d messages (ordinal=%d)', sid, len(history), len(truncated), ordinal)
            if (db := _get_db()) is not None:
                try:
                    db.replace_messages(session['session_key'], truncated, active_only=True)
                except Exception as exc:
                    logger.error('prompt.submit: replace_messages failed for session %s (ordinal=%d); refusing turn so memory and DB stay aligned: %s', sid, ordinal, exc, exc_info=True)
    ...
    if turn_isolation:
        ...
        logger.warning('compute-host dispatch failed for session %s; falling back inline: %s', sid, isolated_response['error'].get('message', 'unknown error'))
    try:
        ...
        if is_disk_full_error(exc):
            return _err(rid, 5070, 'disk full: session storage could not be written — free some disk space and try again')
    ...
    def run_after_agent_ready() -> None:
        ...
        if err:
            _emit_terminal_turn_error(sid, session, (err.get('error') or {}).get('message', 'agent initialization failed'))
            with session['history_lock']:
                session['running'] = False
                session['last_active'] = time.time()
        ...
        with session['history_lock']:
            if session.get('_turn_cancel_requested') or not session.get('running'):
                session['running'] = False
                _clear_inflight_turn(session)
                _emit('error', sid, {'message': 'Turn cancelled before the agent was ready' if session.get('_turn_cancel_requested') else 'Session no longer running before the agent was ready'})
                return
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {'attached': True, 'path': str(img_path), 'count': len(session['attached_images']), **_image_meta(img_path)})
...
def _(rid, params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix='pdf_attach_') as td:
        ...
        argv = ['pdftoppm', '-png', '-r', '150', '-f', str(first_page), '-l', str(last_page), str(pdf_path), str(out_prefix)]
        from hermes_cli._subprocess_compat import windows_hide_flags
        try:
            res = subprocess.run(argv, capture_output=True, text=True, timeout=120, stdin=subprocess.DEVNULL, encoding='utf-8', errors='replace', creationflags=windows_hide_flags())
        ...
        return _ok(rid, {'attached': True, 'filename': display_name, 'pages_attached': len(attached_pages), 'pages': attached_pages, 'count': len(session['attached_images']), 'text': f'[User attached PDF: {display_name} ({len(attached_pages)} page(s))]'})
...
def _(rid, params: dict) -> dict:
    ...
    try:
        stored_path, uploaded = _stage_session_file_attachment(session, raw_path=raw, data_url=data_url, name=name)
        ref_path = _attachment_ref_path(session, stored_path)
        return _ok(rid, {'attached': True, 'name': stored_path.name, 'path': str(stored_path), 'ref_path': ref_path, 'ref_text': f'@file:{_format_ref_value(ref_path)}', 'uploaded': uploaded})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {'detached': len(session['attached_images']) != before, 'count': len(session['attached_images'])})
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if dropped['is_image']:
            ...
            return _ok(rid, {'matched': True, 'is_image': True, 'path': str(drop_path), 'count': len(session['attached_images']), 'text': text, **_image_meta(drop_path)})
        text = f'[User attached file: {drop_path}]' + (f'\n{remainder}' if remainder else '')
        return _ok(rid, {'matched': True, 'is_image': False, 'path': str(drop_path), 'name': drop_path.name, 'text': text})
    except Exception as e:
        return _err(rid, 5027, str(e))
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            result = AIAgent(**_background_agent_kwargs(session['agent'], task_id)).run_conversation(user_message=text, task_id=task_id)
            _emit('background.complete', parent, {'task_id': task_id, 'text': result.get('final_response', str(result)) if isinstance(result, dict) else str(result)})
        except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            history_note = f' (with {len(parent_history)} parent-session messages of context)' if parent_history else ''
            _emit('preview.restart.progress', parent, {'task_id': task_id, 'text': f'Starting hidden restart agent{history_note}'})
            result = AIAgent(**_ephemeral_preview_agent_kwargs(session['agent'], task_id), **_preview_restart_callbacks(parent, task_id)).run_conversation(user_message=prompt, task_id=task_id, conversation_history=parent_history or None)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        return _ok(rid, {'resolved': resolve_gateway_approval(session['session_key'], params.get('choice', 'deny'), resolve_all=params.get('all', False))})
    except Exception as e:
...
def register(server) -> None:
    ...
    server._pending_reaction_notes = types.FunctionType(_pending_reaction_notes.__code__, vars(server), _pending_reaction_notes.__name__, _pending_reaction_notes.__defaults__, _pending_reaction_notes.__closure__)