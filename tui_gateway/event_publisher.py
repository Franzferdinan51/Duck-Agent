...
The dashboard's `/api/pty` spawns `duck-agent --tui` as a child process, which
...
class WsPublisherTransport:
    ...
    def __init__(self, url: str, *, connect_timeout: float=2.0) -> None:
        ...
        try:
            ...
            self._ws = None
            return
        self._worker = threading.Thread(target=self._drain, name='duck-agent-ws-pub', daemon=True)
        self._worker.start()
