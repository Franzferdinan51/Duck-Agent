...
Markers are stored per ``DUCK_AGENT_HOME`` (callers pass the session's home so
...
def _prune(entries: dict[str, dict], now: float) -> dict[str, dict]:
    fresh = {key: entry for key, entry in entries.items() if now - float(entry.get('started_at') or 0) <= _MAX_AGE_SECS}
