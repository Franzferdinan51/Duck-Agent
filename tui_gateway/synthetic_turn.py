...
class SyntheticHeavyAgent:
    ...
    def __init__(self, session_id: str, *, model: str='synthetic-heavy') -> None:
        ...
        self._config_context_length = 200000
        ...
        if isinstance(message, str):
            ...
            if text.startswith('{'):
                try:
                    ...
                    spec = {}
        return {'duration_s': float(spec.get('duration_s', _env_float('HERMES_ISO_CERTIFY_DURATION_S', 8.0))), 'chunk': int(spec.get('chunk', _env_int('HERMES_ISO_CERTIFY_CHUNK', 20000))), 'delta_interval_s': float(spec.get('delta_interval_s', _env_float('HERMES_ISO_CERTIFY_DELTA_S', 0.05))), 'tokens_per_delta': int(spec.get('tokens_per_delta', _env_int('HERMES_ISO_CERTIFY_TPD', 512))), 'sleep_s': float(spec.get('sleep_s', 0.0))}

    def run_conversation(self, message: Any, *, conversation_history: Optional[list[dict[str, str]]]=None, stream_callback: Optional[Callable[[str], None]]=None, task_id: Optional[str]=None, **_kwargs: Any) -> dict[str, Any]:
        ...
        while True:
            ...
            for _ in range(chunk):
                acc = acc * 1000003 + 12345 & 18446744073709551615
            if sleep_s:
                time.sleep(sleep_s)
        ...
        final = f'[synthetic heavy turn] deltas={deltas} out_tokens={self.session_output_tokens} interrupted={interrupted} checksum={acc & 65535:04x}'
        messages = [*base_history, {'role': 'user', 'content': str(message)[:200]}, {'role': 'assistant', 'content': final}]
        self.history = messages
        return {'final_response': final, 'messages': messages, 'interrupted': interrupted, 'error': None, 'last_reasoning': None}
