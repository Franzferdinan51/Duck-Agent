...
def _purge_allowlist_entries(entries, platform: str, user_id: str):
    ...
    if isinstance(entries, str):
        ...
        remaining = [part for part in parts if part == '*' or not _user_ids_match(platform, part, str(user_id))]
        return ','.join(remaining)
    if isinstance(entries, (set, frozenset)):
        return {entry for entry in entries if str(entry).strip() == '*' or not _user_ids_match(platform, str(entry), str(user_id))}
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if not current:
        return
    ids = _split_allowlist(current)
    remaining = [i for i in ids if i == '*' or not _user_ids_match(platform, i, str(user_id))]
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{DUCK_AGENT_HOME}/pairing`` while newer code/docs may
    write ``{DUCK_AGENT_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own DUCK_AGENT_HOME using the same legacy/consolidated layout rules
    as ``duck-agent -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current DUCK_AGENT_HOME.
    """
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                logger.warning('Pairing file %s exists but is not readable as uid=%s (%s; %s). If you ran `docker exec <container> duck-agent pairing approve ...` as root, re-run with `docker exec -u duck-agent <container> ...` and chown the existing file to the duck-agent user, or restart the container so the entrypoint can fix ownership.', path, euid, owner_info, e)
                return {}
            except (json.JSONDecodeError, OSError):
                return {}
        return {}
    ...
    def _approve_user(self, platform: str, user_id: str, user_name: str='') -> None:
        ...
        duplicate_ids = [approved_user_id for approved_user_id in approved if self._user_ids_match(platform, approved_user_id, normalized_user_id)]
    ...
    def revoke(self, platform: str, user_id: str) -> bool:
        ...
        with self._lock:
            ...
            matching_ids = [approved_user_id for approved_user_id in approved if self._user_ids_match(platform, approved_user_id, user_id)]
            if matching_ids:
    ...
    def _finish_approval(self, platform: str, pending: dict, matched_key: str, matched_entry: dict) -> dict:
        ...
        return {'user_id': matched_entry['user_id'], 'user_name': matched_entry.get('user_name', '')}
    ...
    def generate_code(self, platform: str, user_id: str, user_name: str='') -> Optional[str]:
        ...
        with self._lock:
            ...
            pending[entry_id] = {'hash': code_hash, 'salt': salt.hex(), 'user_id': normalized_user_id, 'user_name': user_name, 'created_at': time.time()}
            self._save_json(self._pending_path(platform), pending)
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``duck-agent
        ...
        with self._lock:
            ...
            for p in platforms:
                ...
                for entry_id, info in pending.items():
                    ...
                    results.append({'platform': p, 'request_id': str(entry_id) if is_modern else '', 'user_id': info.get('user_id', ''), 'user_name': info.get('user_name', ''), 'age_minutes': age_min})
        return results
    ...
    def _record_failed_attempt(self, platform: str) -> None:
        ...
        if fails >= MAX_FAILED_ATTEMPTS:
            ...
            limits[fail_key] = 0
            print(f'[pairing] Platform {platform} locked out for {LOCKOUT_SECONDS}s after {MAX_FAILED_ATTEMPTS} failed attempts', flush=True)
        self._save_json(self._rate_limit_path(), limits)
