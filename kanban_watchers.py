...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`duck-agent pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                ...
                def _collect():
                    ...
                    notifier_profiles.update((str(profile).strip() for profile in getattr(self, '_profile_adapters', {}) if str(profile).strip()))
                    active_platforms = {getattr(platform, 'value', str(platform)).lower() for platform in self.adapters.keys()}
                    for _profile_adapter_map in getattr(self, '_profile_adapters', {}).values():
                        active_platforms.update((getattr(platform, 'value', str(platform)).lower() for platform in _profile_adapter_map.keys()))
                    ...
                    for board_meta in boards:
                        ...
                        try:
                            if _kb.count_notify_subs(board=slug, notifier_profiles=notifier_profiles, include_unowned=include_unowned) == 0:
                                logger.debug('kanban notifier: board %s has no subscriptions owned by %s; skipping open', slug, sorted(notifier_profiles))
                                continue
                        except Exception as exc:
                            logger.debug('kanban notifier: read-only subscription probe failed for board %s (%s); falling back to writable open', slug, exc)
                        try:
                            ...
                            continue
                        try:
                            subs = _kb.list_notify_subs(conn, notifier_profiles=notifier_profiles, include_unowned=include_unowned)
                            if not subs:
                                ...
                                try:
                                    ...
                                    if owner_profile and owner_profile != notifier_profile:
                                        ...
                                        if not _owner_adapters:
                                            logger.debug('kanban notifier: subscription for %s owned by profile %s; current profile %s has no adapter for it, skipping', sub.get('task_id'), owner_profile, notifier_profile)
                                            continue
                                    platform = (sub.get('platform') or '').lower()
                                    if platform not in active_platforms:
                                        logger.debug('kanban notifier: subscription for %s on %s skipped; adapter not connected', sub.get('task_id'), platform or '<missing>')
                                        continue
                                    old_cursor, cursor, events = _kb.claim_unseen_events_for_sub(conn, task_id=sub['task_id'], platform=sub['platform'], chat_id=sub['chat_id'], thread_id=sub.get('thread_id') or '', kinds=TERMINAL_KINDS)
                                    if not events:
                                        continue
                                    task = _kb.get_task(conn, sub['task_id'])
                                    logger.debug('kanban notifier: claimed %d event(s) for %s on board %s cursor %s→%s', len(events), sub['task_id'], slug, old_cursor, cursor)
                                    deliveries.append({'sub': sub, 'old_cursor': old_cursor, 'cursor': cursor, 'events': events, 'task': task, 'board': slug})
                                except Exception as sub_exc:
                                    logger.warning('kanban notifier: subscription for %s on board %s failed: %s', sub.get('task_id'), slug, sub_exc)
                        finally:
                            conn.close()
                    return deliveries
                ...
                for d in deliveries:
                    ...
                    if adapter is None:
                        logger.debug('kanban notifier: adapter %s disconnected before delivery for %s; rewinding claim', platform_str, sub['task_id'])
                        await asyncio.to_thread(self._kanban_rewind, sub, d['cursor'], d.get('old_cursor', 0), board_slug)
                        continue
                    ...
                    for ev in d['events']:
                        ...
                        if kind == 'completed':
                            ...
                            msg = f"✖ {board_tag}{tag}Kanban {sub['task_id']} gave up after repeated spawn failures{err}"
                        elif kind == 'crashed':
                            msg = f"✖ {board_tag}{tag}Kanban {sub['task_id']} worker crashed (pid gone); dispatcher will retry"
                        elif kind == 'timed_out':
                            limit = 0
                            if ev.payload and ev.payload.get('limit_seconds'):
                                limit = int(ev.payload['limit_seconds'])
                            msg = f"⏱ {board_tag}{tag}Kanban {sub['task_id']} timed out (max_runtime={limit}s); will retry"
                        elif kind == 'status':
                            new_status = ''
                            ...
                            msg = f"🛑 {board_tag}{tag}Kanban {sub['task_id']} routed to TRIAGE — needs a human decision{rc}{reason}"
                        else:
                            continue
                        ...
                        if not adapter_supports_push(adapter):
                            logger.debug('kanban notifier: adapter %s has no push channel; skipping text ping for %s, relying on wake self-post instead', platform_str, sub['task_id'])
                            continue
                        try:
                            ...
                            logger.debug('kanban notifier: delivered %s event for %s to %s/%s on board %s', kind, sub['task_id'], platform_str, sub['chat_id'], board_slug)
                            if kind == 'completed':
                                try:
                                    await self._deliver_kanban_artifacts(adapter=adapter, chat_id=sub['chat_id'], metadata=metadata, event_payload=getattr(ev, 'payload', None), task=task)
                                except Exception as art_exc:
                                    logger.debug('kanban notifier: artifact delivery for %s failed: %s', sub['task_id'], art_exc)
                            sub_fail_counts.pop(sub_key, None)
                        except Exception as exc:
                            fails = sub_fail_counts.get(sub_key, 0) + 1
                            sub_fail_counts[sub_key] = fails
                            logger.warning('kanban notifier: send failed for %s on %s (attempt %d/%d): %s', sub['task_id'], platform_str, fails, MAX_SEND_FAILURES, exc)
                            if fails >= MAX_SEND_FAILURES:
                                logger.warning('kanban notifier: dropping subscription %s on %s after %d consecutive send failures', sub['task_id'], platform_str, fails)
                        ...
                        if _wake_kinds and _session_key:
                            ...
                            _synth = t('gateway.kanban.wake.message', task_id=sub['task_id'], status=_status, title=_title, assignee=_assignee, board=board_slug)
                        if not _is_push_adapter and _wake_kinds and _session_key:
                            from gateway.wake import deliver_wake
                            try:
                                await deliver_wake(adapter, text=_synth, session_id=_session_key)
                                logger.info('kanban notifier: woke agent for %s on %s/%s profile=%s events=%s', sub['task_id'], platform_str, sub['chat_id'], sub_profile or 'default', _wake_kinds)
                                sub_fail_counts.pop(sub_key, None)
                            except Exception as _wk_err:
                                fails = sub_fail_counts.get(sub_key, 0) + 1
                                sub_fail_counts[sub_key] = fails
                                logger.warning('kanban notifier: wake self-post failed for %s (attempt %d/%d): %s', sub['task_id'], fails, MAX_SEND_FAILURES, _wk_err, exc_info=True)
                                if fails >= MAX_SEND_FAILURES:
                                    logger.warning('kanban notifier: dropping subscription %s on %s after %d consecutive wake failures', sub['task_id'], platform_str, fails)
                                    await asyncio.to_thread(self._kanban_unsub, sub, board_slug)
                                    sub_fail_counts.pop(sub_key, None)
                                else:
                                    await asyncio.to_thread(self._kanban_rewind, sub, d['cursor'], d.get('old_cursor', 0), board_slug)
                        ...
                        if _is_push_adapter and _wake_kinds and _session_key:
                            try:
                                ...
                                _source = SessionSource(platform=plat, chat_id=sub['chat_id'], chat_type=_chat_type, thread_id=sub.get('thread_id') or None, user_id=sub.get('user_id'), profile=sub_profile or None)
                                await deliver_wake(adapter, text=_synth, session_id=_session_key, source=_source)
                                logger.info('kanban notifier: woke agent for %s on %s/%s profile=%s events=%s', sub['task_id'], platform_str, sub['chat_id'], sub_profile or 'default', _wake_kinds)
                            except Exception as _wk_err:
                                logger.warning('kanban notifier: wakeup injection failed for %s: %s', sub['task_id'], _wk_err, exc_info=True)
                        if task_terminal:
                            await asyncio.to_thread(self._kanban_unsub, sub, board_slug)
            except Exception as exc:
    ...
    def _kanban_advance(self, sub: dict, cursor: int, board: Optional[str]=None) -> None:
        ...
        try:
            _kb.advance_notify_cursor(conn, task_id=sub['task_id'], platform=sub['platform'], chat_id=sub['chat_id'], thread_id=sub.get('thread_id') or '', new_cursor=cursor)
        finally:
            conn.close()
    ...
    def _kanban_rewind(self, sub: dict, claimed_cursor: int, old_cursor: int, board: Optional[str]=None) -> None:
        ...
        conn = _kb.connect(board=board)
        try:
            _kb.rewind_notify_cursor(conn, task_id=sub['task_id'], platform=sub['platform'], chat_id=sub['chat_id'], thread_id=sub.get('thread_id') or '', claimed_cursor=claimed_cursor, old_cursor=old_cursor)
        finally:
            conn.close()
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `duck-agent kanban daemon` process needed. When false, the
        ...
        if _lock_state == 'contended':
            logger.info('kanban dispatcher: another gateway already holds the dispatcher lock (%s); this gateway will NOT dispatch.', _lock_path)
            return
        if _lock_state == 'held':
            self._kanban_dispatcher_lock_handle = _lock_handle
            logger.info('kanban dispatcher: holding singleton dispatcher lock (%s)', _lock_path)
        else:
            logger.warning('kanban dispatcher: advisory lock unavailable at %s; proceeding on config control alone.', _lock_path)
        try:
            interval = float(kanban_cfg.get('dispatch_interval_seconds', 60) or 60)
        except (ValueError, TypeError):
            logger.warning('kanban dispatcher: invalid dispatch_interval_seconds=%r, using default 60', kanban_cfg.get('dispatch_interval_seconds'))
            interval = 60.0
        ...
        if raw_max_in_progress is not None:
            try:
                ...
                logger.warning('kanban dispatcher: invalid kanban.max_in_progress=%r; ignoring', raw_max_in_progress)
                max_in_progress = None
            else:
                if max_in_progress < 1:
                    logger.warning('kanban dispatcher: kanban.max_in_progress=%r is below 1; ignoring', raw_max_in_progress)
                    max_in_progress = None
                else:
        ...
        try:
            ...
            logger.warning('kanban dispatcher: invalid kanban.failure_limit=%r; using default %d', raw_failure_limit, _kb.DEFAULT_FAILURE_LIMIT)
            failure_limit = _kb.DEFAULT_FAILURE_LIMIT
        if failure_limit < 1:
            logger.warning('kanban dispatcher: kanban.failure_limit=%r is below 1; using default %d', raw_failure_limit, _kb.DEFAULT_FAILURE_LIMIT)
            failure_limit = _kb.DEFAULT_FAILURE_LIMIT
        raw_stale = kanban_cfg.get('dispatch_stale_timeout_seconds', 0)
        try:
            stale_timeout_seconds = int(raw_stale or 0)
        except (TypeError, ValueError):
            logger.warning('kanban dispatcher: invalid kanban.dispatch_stale_timeout_seconds=%r; disabling stale detection', raw_stale)
            stale_timeout_seconds = 0
        reconcile_orphans = bool(kanban_cfg.get('reconcile_orphans', True))
        default_assignee = (kanban_cfg.get('default_assignee') or '').strip() or None
        if default_assignee:
            logger.info('kanban dispatcher: default_assignee=%r (unassigned ready tasks will route to this profile)', default_assignee)
        ...
        if raw_per_profile is not None:
            try:
                ...
                logger.warning('kanban dispatcher: invalid kanban.max_in_progress_per_profile=%r; ignoring', raw_per_profile)
                max_in_progress_per_profile = None
            else:
                if max_in_progress_per_profile < 1:
                    logger.warning('kanban dispatcher: kanban.max_in_progress_per_profile=%r is below 1; ignoring', raw_per_profile)
                    max_in_progress_per_profile = None
                else:
                    logger.info('kanban dispatcher: max_in_progress_per_profile=%d', max_in_progress_per_profile)
        await asyncio.sleep(5)
        HEALTH_WINDOW = 6
        bad_ticks = 0
        last_warn_at = 0
        ...
        def _tick_once_for_board(slug: str) -> 'Optional[object]':
            ...
            if disabled_entry is not None:
                ...
                if disabled_fingerprint == fingerprint and age < CORRUPT_BOARD_RETRY_AFTER_SECONDS:
                    return None
                if disabled_fingerprint == fingerprint:
                    logger.info('kanban dispatcher: board %s database fingerprint unchanged after %.0fs quarantine; retrying dispatch', slug, age)
                else:
                    logger.info('kanban dispatcher: board %s database changed; retrying dispatch', slug)
                disabled_corrupt_boards.pop(slug, None)
            try:
                conn = _kb.connect(board=slug)
                return _kb.dispatch_once(conn, board=slug, max_spawn=max_spawn, max_in_progress=max_in_progress, failure_limit=failure_limit, stale_timeout_seconds=stale_timeout_seconds, default_assignee=default_assignee, max_in_progress_per_profile=max_in_progress_per_profile, reconcile_orphans=reconcile_orphans)
            except sqlite3.DatabaseError as exc:
                if _is_corrupt_board_db_error(exc):
                    disabled_corrupt_boards[slug] = (fingerprint, time.monotonic())
                    logger.error('kanban dispatcher: board %s database %s is not a valid SQLite database; pausing dispatch for this board until the file changes, the gateway restarts, or the quarantine timer expires. Move or restore the file, then run `duck-agent kanban init` if you need a fresh board.', slug, fingerprint[0])
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Duck Agent profile
            ...
            PATH, missing venv, credential loss for a real Duck Agent profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
            ...
            try:
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning('kanban dispatcher stuck: ready queue non-empty for %d consecutive ticks but 0 workers spawned. Check profile health (venv, PATH, credentials) and `duck-agent kanban list --status ready`.', bad_ticks)
                        last_warn_at = now
