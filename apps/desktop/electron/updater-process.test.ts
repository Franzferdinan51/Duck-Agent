...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\Hermes\\duck-agent-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    ...
    path.join('C:\\Hermes', 'duck-agent-setup.exe')
  )
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  ...
  const staged = path.join(home, 'duck-agent-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when duck-agent-setup is staged (#74836)', () => {
  const home = '/Users/hermes/.hermes'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages duck-agent-setup on macOS/Linux too, so "it exists" is
