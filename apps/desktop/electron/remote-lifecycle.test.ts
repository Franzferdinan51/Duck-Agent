...
test('locateHermes throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  ...
  await assert.rejects(
    ...
    (err: any) => {
      assert.equal(err.kind, 'duck-agent-not-found')
...
test('locateHermes canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v hermes/, '/home/u/.local/bin/hermes\n'],
    [/\[ -x .*\.local\/bin\/hermes/, 'OK'],
    [/python3 -c/, '/home/u/.hermes/duck-agent-agent/venv/bin/hermes\n']
  ])

  assert.equal(await locateHermes(ssh, ''), '/home/u/.hermes/duck-agent-agent/venv/bin/hermes')
})
...
test('locateHermes tries the conventional venv path last', async () => {
  ...
  assert.equal(await locateHermes(ssh, ''), '~/.hermes/duck-agent-agent/venv/bin/hermes')
})

test('locateHermes throws a duck-agent-not-found error with an install hint', async () => {
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  ...
  assert.doesNotThrow(() => validateRemotePath('/home/user/.hermes/duck-agent-agent/venv/bin/hermes'))
})
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('duck-agent'), /absolute|relative/i)
