...
test('redactSecrets scrubs Authorization and X-Duck-Agent-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Duck-Agent-Session-Token: hdr_888'), /X-Duck-Agent-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Duck-Agent-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('duck-agent', 'remote-build-server', 22) // no baseDir → default
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `duck-agent-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `duck-agent-ssh-test-${process.pid}-${Date.now()}`)
