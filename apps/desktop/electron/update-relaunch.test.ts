...
const ROOT = '/home/u/.hermes/duck-agent-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'duck-agent')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.hermes/duck-agent-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'duck-agent'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'duck-agent')
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.hermes/duck-agent-agent/apps/desktop/release/linux-unpacked/Hermes',
  ...
  const tmp = path.join(os.tmpdir(), `duck-agent-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  const tmp = path.join(os.tmpdir(), `duck-agent-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
