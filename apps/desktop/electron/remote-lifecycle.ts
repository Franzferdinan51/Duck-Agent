...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  if (remoteHermesPath) {
    ...
    const err: any = new Error(
      `The Hermes path you set is not an executable on the remote host: "${remoteHermesPath}". ` +
        ...
        '~/duck-agent-agent/.venv/bin/hermes), or clear it to auto-detect.'
    )

    err.kind = 'duck-agent-not-found'
    throw err
  }
  ...
  candidates.push('~/.hermes/duck-agent-agent/venv/bin/hermes')
  ...
  const err: any = new Error(
    'Hermes is not installed on the remote host (could not find a `hermes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://duck-agent-agent.nousresearch.com/install.sh | sh  ' +
...
// e.g. "Duck Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Duck Agent Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
