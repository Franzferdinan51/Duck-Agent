...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'duck-agent-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/duck-agent-runner-test',
    sourceRepoRoot: null,
    hermesHome: '/tmp/duck-agent-runner-test',
    logRoot: '/tmp/duck-agent-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'duck-agent-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'duck-agent-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/duck-agent-agent',
      hermesHome: '/tmp/hermes'
    }),
    ['--dir', '/tmp/duck-agent-agent', '--duck-agent-home', '/tmp/hermes', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/duck-agent-agent', '--duck-agent-home', '/tmp/hermes', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/hermes', '--duck-agent-home', '/tmp/home', '--branch', 'main']
  )
})
