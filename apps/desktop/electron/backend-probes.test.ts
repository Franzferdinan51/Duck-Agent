...
test('canImportHermesCli returns false when binary does not exist', () => {
  const ghost = path.join(os.tmpdir(), 'duck-agent-probes-ghost-' + Date.now() + '.exe')
...
test('verifyHermesCli returns true when --version exits 0', () => {
  ...
  const scriptPath = path.join(os.tmpdir(), `duck-agent-probes-ok-${Date.now()}-${process.pid}.cjs`)
