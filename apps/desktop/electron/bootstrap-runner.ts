 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Duck Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.duck-agent-bootstrap-complete'), 'utf8')
...
// checkout under ~/.hermes/duck-agent-agent. Used as a last-resort fallback when
