...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    'if(-not $hermesHome){$hermesHome=Join-Path $env:LOCALAPPDATA "duck-agent"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command hermes.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $hermesHome "duck-agent-agent\\venv\\Scripts\\hermes.exe")',
    '$candidates+=(Join-Path $HOME "duck-agent-agent\\.venv\\Scripts\\hermes.exe")',
