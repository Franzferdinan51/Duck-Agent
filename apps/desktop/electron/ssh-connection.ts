...
const _REDACTIONS: Array<[RegExp, string]> = [
  ...
  [/(X-Duck-Agent-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'duck-agent-desktop-ssh')
  }

  // Use Duck-Agent's own home so SSH control-mux sockets never land inside
  // Hermes's ~/.hermes directory.
  return path.join(os.homedir(), '.duck-agent', 'desktop-ssh')
}
