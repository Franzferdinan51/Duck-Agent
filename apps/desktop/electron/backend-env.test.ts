...
test('desktop backend PATH adds Duck-Agent-managed bins and missing POSIX sane entries', () => {
  const result = buildDesktopBackendPath({
    hermesHome: '/Users/test/.hermes',
    venvRoot: '/Users/test/.hermes/duck-agent-agent/venv',
  ...
  assert.deepEqual(entries.slice(0, 3), [
    ...
    '/Users/test/.hermes/duck-agent-agent/venv/bin'
  ])
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  const env = buildDesktopBackendEnv({
    hermesHome: '/Users/test/.hermes',
    pythonPathEntries: ['/repo/duck-agent-agent'],
    venvRoot: '/Users/test/.hermes/duck-agent-agent/venv',
    currentEnv: {
      PATH: '/usr/bin:/bin',
  ...
  assert.equal(env.PYTHONPATH, '/repo/duck-agent-agent:/existing/pythonpath')
  assert.ok(
    env.PATH.startsWith(
      '/Users/test/.hermes/node/bin:/Users/test/.hermes/node:/Users/test/.hermes/duck-agent-agent/venv/bin:'
    )
  )
...
test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  const env = buildDesktopBackendEnv({
    ...
    pythonPathEntries: ['C:\\repo\\duck-agent-agent'],
    venvRoot: 'C:\\Users\\test\\AppData\\Local\\hermes\\duck-agent-agent\\venv',
    currentEnv: {
