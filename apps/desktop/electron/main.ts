...
//
// Duck-Agent is fully standalone. It uses its OWN home directory, never
// Hermes's. Hermes defaults to ~/.hermes (macOS/Linux) and
// %LOCALAPPDATA%\hermes (Windows) for its install, config, sessions, and logs.
// Sharing that directory would let Duck-Agent and Hermes overwrite each other's
// state, so every default lives under a duck-agent-namespaced root and the two
// agents can never collide. A live Hermes install (e.g. the user's real
// ~/.hermes with a large state.db) is NEVER opened or written by Duck-Agent.
//
// Defaults:
...
// touches the user's real ~/.duck-agent.
...
// ACTIVE_HERMES_ROOT — the canonical mutable runtime checkout. Because
// Duck-Agent is fully standalone, ACTIVE_HERMES_ROOT is Duck-Agent's OWN
// runtime directory under its own home (~/.duck-agent/runtime on macOS/Linux,
// %LOCALAPPDATA%\duck-agent\runtime on Windows). It is never Hermes's home.
// Duck-Agent's updater git-pulls and its `duck-agent serve`/`hermes serve` is
// launched from this path, which lives entirely inside ~/.duck-agent and never
// touches a real Hermes install.
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HERMES_ROOT, '.duck-agent-bootstrap-complete')
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Duck Agent'
...
const MEDIA_PROTOCOL = 'duck-agent-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(duck-agent-media://…)`.
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-duck-agent-wsl-windows-fonts.conf')
    let existing = ''
...
// The Tauri updater writes HERMES_HOME/.duck-agent-update-in-progress for the whole
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the duck-agent-import
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'duck-agent', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'duck-agent', 'git', 'bin', 'git.exe'))
  }
...
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ...
    : path.join(updateRoot, 'venv', 'bin', 'duck-agent')
}
...
// `hermes update`). Instead we spawn the staged Duck-Agent-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged duck-agent-setup or not — the resolver
    ...
    if (!updater) {
      ...
      // duck-agent-setup.exe into HERMES_HOME). They DO have a working `hermes`
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `duck-agent update --branch ${branch}`
          }
        }
      } catch {
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'hermes.exe' : 'duck-agent')
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvHermes) || fileExists(path.join(updateRoot, '.duck-agent-bootstrap-complete'))
...
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'duck-agent')

  if (fileExists(venvHermes)) {
    return venvHermes
  }

  return findOnPath('duck-agent') || null
}
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  if (!hermes) {
    emitUpdateProgress({ stage: 'manual', message: 'duck-agent update', percent: null })

    return { ok: true, manual: true, command: 'duck-agent update', hermesRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(HERMES_HOME, rememberLog)

  // Put the Duck-Agent-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'duck-agent update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'duck-agent update failed' }
  }
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Duck Agent…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `duck-agent-desktop-update-${Date.now()}.sh`)
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.duck-agent-update-new"; then
    rm -rf "$DST.duck-agent-update-old" 2>/dev/null || true
    mv "$DST" "$DST.duck-agent-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.duck-agent-update-new" "$DST"
    rm -rf "$DST.duck-agent-update-old" 2>/dev/null || true
  fi
fi
...
function resolveHermesBackend(backendArgs) {
  ...
  //    %LOCALAPPDATA%\\hermes\\duck-agent-agent (Windows) or ~/.hermes/duck-agent-agent.
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      hermesCommand = findOnPath('duck-agent')
    }
  ...
  if (python) {
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\hermes\duck-agent-agent\venv.
  ...
  return {
    ...
    label: 'Duck Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
function multipartBody(upload) {
  const boundary = `----duck-agent-${crypto.randomBytes(12).toString('hex')}`
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Duck-Agent-Session-Token': token,
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Duck-Agent-Session-Token`` header — used by the auth-mode probe before
...
const OAUTH_SESSION_PARTITION = 'persist:duck-agent-remote-oauth'
...
// portal session cookie living in `persist:duck-agent-remote-oauth`:
...
function showAboutPanelFresh() {
  app.setAboutPanelOptions({
    applicationName: APP_NAME,
    applicationVersion: resolveHermesVersion(),
    copyright: 'Copyright © 2026 Duck-Agent contributors'
  })
  app.showAboutPanel()
}
...
async function runDesktopUninstall(mode) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `duck-agent-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `duck-agent-uninstall-${Date.now()}.sh`)
...
// ---------------------------------------------------------------------------
const HERMES_PROTOCOL = 'duck-agent'
