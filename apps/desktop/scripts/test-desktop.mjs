...
// no longer bundles the Duck Agent Python payload (that's fetched at first
...
const VENV_ROOT = path.join(DEFAULT_HERMES_HOME, 'duck-agent-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'duck-agent-desktop-fresh-install')
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Duck-Agent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Duck-Agent-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Duck-Agent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Duck-Agent-${version}-${os}-${arch}.${ext}'
...
function launchFresh() {
  ...
  const hermesHome = path.join(sandbox, 'duck-agent-home')
  ...
  return { runtimeRoot: path.join(hermesHome, 'duck-agent-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Duck Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'duck-agent-agent', 'hermes_cli', 'main.py')
