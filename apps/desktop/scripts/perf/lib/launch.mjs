...
export async function startIsolatedInstance({
  ...
  const home = hermesHome ?? mkTemp('duck-agent-perf-home-')
  const userData = userDataDir ?? mkTemp('duck-agent-perf-ud-')
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'duck-agent-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'duck-agent-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.hermes'), home)
