...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Duck Agent Desktop tool calls' },
    ...
    expect(view.searchQuery).toBe('Duck Agent Desktop tool calls')
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://duck-agent-agent.nousresearch.com/docs' },
    ...
    expect(view.title).toBe('Failed to open duck-agent-agent.nousresearch.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://duck-agent-agent.nousresearch.com/docs' },
        result: { success: true, url: 'https://duck-agent-agent.nousresearch.com/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened duck-agent-agent.nousresearch.com/docs')
  })
})
