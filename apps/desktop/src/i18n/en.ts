...
export const en: Translations = {
  ...
  boot: {
    ready: 'Duck Agent Desktop is ready',
    desktopBootFailedWithMessage: message => `Desktop boot failed: ${message}`,
    steps: {
      connectingGateway: 'Connecting live desktop gateway',
      loadingSettings: 'Loading Duck Agent settings',
      loadingSessions: 'Loading recent sessions',
      startingDesktopConnection: 'Starting desktop connection',
      startingHermesDesktop: 'Starting Duck Agent Desktop…'
    },
    errors: {
      backgroundExited: 'Duck Agent background process exited.',
      backgroundExitedDuringStartup: 'Duck Agent background process exited during startup.',
    ...
    failure: {
      title: "Duck Agent couldn't start",
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Your Duck Agent backend is older than this desktop build and may not work correctly. Update to align them.',
    installMethodUnsupportedTitle: 'Unsupported install method',
    updateHermes: 'Update Duck Agent',
    ...
    errors: {
      ...
      methodNotAllowed:
        'The desktop backend rejected that request (405 Method Not Allowed). Try restarting Duck Agent Desktop.',
  ...
  settings: {
    ...
    searchPlaceholder: {
      about: 'About Duck Agent Desktop',
    ...
    gateway: {
      ...
      remoteDesc: 'Connect this desktop shell to a remote Duck Agent backend.',
      ...
      cloudDesc: 'Sign in once to Duck Agent Cloud and pick from the agents on your account — no URL to paste.',
      cloudSignInTitle: 'Duck Agent Cloud',
      cloudSignIn: 'Sign in to Duck Agent Cloud',
      cloudSignedIn: 'Signed in to Duck Agent Cloud',
      cloudNeedsSignIn: 'Sign in to Duck Agent Cloud to discover the agents on your account.',
      ...
      cloudDiscoverFailed: 'Could not load your Duck Agent Cloud agents',
      cloudConnectFailed: 'Could not connect to that agent',
      cloudSignInFailed: 'Duck Agent Cloud sign-in failed',
      cloudSignedOutTitle: 'Signed out of Duck Agent Cloud',
      cloudSignedOutMessage: 'Cleared the Duck Agent Cloud session.',
      ...
      remoteUrlDesc: 'Base URL for the remote dashboard backend. Path prefixes are supported, for example /duck-agent.',
      ...
      restartingMessage: 'Duck Agent Desktop will reconnect using the saved settings — the shell stays open.',
      ...
      sshDesc:
        'Duck Agent is launched on the remote over SSH and tunneled to this app — nothing to start or expose yourself. Requires working key-based SSH access to the host.',
      ...
      sshHermesPathTitle: 'Duck Agent path (optional)',
      sshHermesPathDesc: 'Full path to the remote duck-agent binary. Blank = auto-detect.',
      ...
      sshReachable: (host, platform) => `Reachable: ${host} (${platform}) — Duck Agent found`,
      ...
      sshErrAuth:
        'SSH authentication failed. Load your key into the ssh-agent (ssh-add) or set an IdentityFile in ~/.ssh/config — Duck Agent runs ssh non-interactively.',
      sshErrHostKey:
        'The host key has CHANGED since you last connected. Verify this is expected, then run ssh-keygen -R <host> and reconnect.',
      sshErrNotInstalled:
        'Duck Agent is not installed on the remote host. Install it there (curl -fsSL https://duck-agent-agent.nousresearch.com/install.sh | sh) or set the Duck Agent path.',
      sshErrPlatform:
        'Unsupported remote platform. Duck Agent Desktop SSH mode supports Linux, macOS, and Windows remote hosts.',
      sshErrTimeout: 'SSH connection timed out. The host may be unreachable or asleep.',
      sshErrUpdateRequired: 'Update Duck Agent on the remote host before connecting with Desktop SSH.',
    ...
    providers: {
      ...
      intro:
        'Sign in with a subscription — no API key to copy. Duck Agent runs the browser sign-in for you, right here in the app.',
      connected: 'Connected',
      collapse: 'Collapse',
      ...
      localEndpoint: {
        ...
        description: 'Point Duck Agent at any OpenAI-compatible endpoint (Zyphra, vLLM, llama.cpp, Ollama, etc).'
      },
  ...
  skills: {
    ...
    skillArchivedMessage: 'Restorable via duck-agent curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'As Duck Agent builds skills and memories for your work, they appear here.',
    share: 'Share map',
    shareHint:
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Restart Duck Agent to use pets — the backend predates this feature.',
    ...
    generatePet: {
      ...
      staleBackend: 'Update Duck Agent to generate pets.',
      backgroundHint: 'You can close this — Duck Agent will notify you when it’s done.',
    ...
    nav: {
      ...
      settings: { title: 'Settings', detail: 'Configure Duck Agent desktop' },
    ...
    hermesActiveSessions: (version, count) => `Duck Agent ${version} · Active sessions ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot user ID', placeholder: '@duck-agent:example.org' },
  ...
  profiles: {
    ...
    deleting: 'Deleting...',
    createDesc: 'Profiles are independent Duck Agent environments: separate config, skills, and SOUL.md.',
    nameLabel: 'Name',
  ...
  cron: {
    ...
    newCron: 'New cron',
    emptyDescNew:
      'Schedule a prompt to run on a cron expression. Duck Agent will run it and deliver results to the destination you pick.',
  ...
  install: {
    ...
    setupChoiceTitle: 'Set up Duck Agent Desktop',
    setupChoiceDesc:
      'Connect this app to a Duck-Agent gateway you already run, or initialize Duck-Agent locally on this computer.',
    connectExistingTitle: 'Connect to existing Duck-Agent',
    connectExistingShort: 'Connect existing',
    connectExistingDesc: 'Use a remote Duck-Agent backend with a session token or browser sign-in. No local install will start.',
    installLocalTitle: 'Initialize Duck-Agent locally',
    installLocalDesc: 'Create Duck-Agent’s local runtime and run the backend on this computer.',
    localStartUnavailable: 'Local Duck-Agent initialization could not start. Restart Duck Agent Desktop and try again.',
    remoteSetupTitle: 'Connect to existing backend',
    remoteSetupDesc: 'Enter your gateway URL. Duck Agent Desktop will detect whether it needs a token or browser sign-in.',
    ...
    settingUpTitle: 'Setting up Duck Agent',
  ...
  onboarding: {
    headerTitle: "Let's get you setup with Duck Agent",
    headerDesc: 'Connect a model provider to start chatting. Most options take one click.',
    preparingInstall: 'Duck Agent is finishing install. This usually takes under a minute on first run.',
    starting: 'Starting Duck Agent…',
    ...
    autoBrowser: provider =>
      `We opened ${provider} in your browser. Authorize Duck Agent there and you'll be connected automatically — nothing to copy or paste.`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} is ${size}. Duck Agent will only show the first 512 KB.`,
    ...
    web: {
      ...
      lookingRestart: taskId => `Duck Agent is looking for a preview server to restart (${taskId})`,
      ...
      finishedRestarting: message => `Duck Agent finished restarting the preview server${message ? `: ${message}` : ''}`,
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `This adds the “${pattern}” pattern to your permanent allowlist (~/.duck-agent/config.yaml). Duck Agent won’t ask again for commands like this — in this session or any future one.`,
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Restart Duck Agent Desktop to use Save Image.',
    restartToSaveImages: 'Restart Duck Agent Desktop to save images',
    ...
    handoff: {
      ...
      timedOut: 'Timed out waiting for the gateway. Is `duck-agent gateway` running?'
    }
  },
