 ...
 *   (`@duck-agent/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
...
/** Rewrite ONLY mapped import specifiers (@duck-agent/plugin-sdk, react*) to their
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @duck-agent/plugin-sdk and react`
      )
    }
