...
const CONFIGURED_CACHE_KEY = 'duck-agent-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'duck-agent-onboarding-skipped-v1'
const POLL_MS = 2000
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Duck Agent Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
