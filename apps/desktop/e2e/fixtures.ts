...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `duck-agent-e2e-${prefix}-${Math.random()}`))
  const hermesHome = path.join(root, 'duck-agent-home')
 ...
 *
 * Key env vars:
 *  - HERMES_HOME → sandbox duck-agent-home (isolated config/sessions)
...
function resolvePackagedBinaryPath(): string {
  ...
  return path.join(RELEASE_ROOT, 'linux-unpacked', 'duck-agent')
}
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  // Clear dev-server + duck-agent-root overrides — the packaged binary
