...
Defenses 1 and 2 (the ``_HERMES_GATEWAY`` guard on ``duck-agent gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.duck-agent.gateway")``,
...
State lives in ``<DUCK_AGENT_HOME>/gateway/restart_loop.json`` so it is
...
def check_and_record(max_restarts: int=DEFAULT_MAX_RESTARTS, window_seconds: int=DEFAULT_WINDOW_SECONDS, *, now: Optional[float]=None) -> bool:
    ...
    if tripped:
        logger.warning('Restart-loop breaker TRIPPED: %d restart-interrupted gateway boots within %ds (threshold %d). Skipping auto-resume to break a suspected SIGTERM-respawn loop (#30719). Restart-interrupted sessions stay resume-pending and will continue on the next real user message. If this is a false positive, delete %s.', len(boots), window_seconds, max_restarts, _state_path())
    return tripped