...
These are the built-in automations Duck Agent can offer a new user out of the box —
...
def seed_catalog_suggestions(*, add_fn: Optional[Callable[..., Optional[Dict[str, Any]]]]=None, keys: Optional[List[str]]=None) -> List[Dict[str, Any]]:
    ...
    for entry in CATALOG:
        if wanted is not None and entry.key not in wanted:
            continue
        rec = add_fn(title=entry.title, description=entry.description, source='catalog', job_spec=dict(entry.job_spec), dedup_key=entry.key)
        if rec is not None:
            created.append(rec)
    return created