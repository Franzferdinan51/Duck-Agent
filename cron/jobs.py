...
Jobs are stored in ~/.duck-agent/cron/jobs.json
Output is saved to ~/.duck-agent/cron/output/{job_id}/{timestamp}.md
"""
import contextlib
import copy
...
class _CronStorePaths:
    ...
    jobs_file: Path
    output_dir: Path
_cron_store_override: ContextVar[Optional[_CronStorePaths]] = ContextVar('cron_store_override', default=None)
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_hermes_home()
       (context-local override, then the DUCK_AGENT_HOME env var) — so a test
       or embedder that re-points DUCK_AGENT_HOME after this module was
...
def use_cron_store(home: Union[str, Path]):
    ...
    token = _cron_store_override.set(_CronStorePaths(cron_dir=cron_dir, jobs_file=cron_dir / 'jobs.json', output_dir=cron_dir / 'output'))
    try:
        yield
    finally:
...
def _job_running_in_this_process(job_id: str) -> bool:
    ...
    try:
        ...
        logger.warning('Cron running-set liveness check failed for job %r; keeping the entry to avoid deleting a possibly live one-shot run', job_id, exc_info=True)
        return True
...
def _jobs_lock():
    ...
    and standalone ``duck-agent`` CLI invocations, which previously shared no lock
    ...
    with _jobs_file_lock:
        ...
        try:
            try:
                    ...
                    while True:
                        try:
                            ...
                            if time.monotonic() >= _deadline:
                                logger.error('Timed out after %.0fs waiting for the cron jobs lock (%s) — another process is holding it. Proceeding with in-process locking only so the scheduler stays alive (#60703).', _JOBS_LOCK_TIMEOUT_SECONDS, _jobs_lock_file())
                                try:
                                    lock_fd.close()
                                except OSError:
                                    pass
                ...
                logger.warning('jobs.json cross-process lock unavailable (%s); proceeding with in-process lock only', e)
            try:
                yield
            finally:
...
def _normalize_job_record(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not name:
        ...
        label_source = prompt or (normalized['skills'][0] if normalized.get('skills') else '') or script or job_id or 'cron job'
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec duck-agent duck-agent cron create ...`` — ``docker exec``
    ...
    try:
        ...
        logger.warning('Could not restore ownership of %s to uid=%s gid=%s after rewrite: %s — if the gateway runs as a different user, its cron ticker may now be locked out (see issue #68483).', path, before.st_uid, before.st_gid, e)
...
def parse_duration(s: str) -> int:
    ...
    """
    s = s.strip().lower()
    match = re.match('^(\\d+)\\s*(m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days)$', s)
    if not match:
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if len(parts) >= 5 and all((re.match('^[\\d\\*\\-,/]+$', p) for p in parts[:5])):
    ...
    if 'T' in schedule or re.match('^\\d{4}-\\d{2}-\\d{2}', schedule):
        try:
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Duck Agent configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Duck Agent timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Duck Agent/system local time
...
def compute_next_run(schedule: Dict[str, Any], last_run_at: Optional[str]=None) -> Optional[str]:
    ...
    if kind == 'once':
        ...
        if not expr:
            return None
        if not _ensure_croniter():
            logger.warning("Cannot compute next run for cron schedule %r: 'croniter' is not installed. croniter is a core dependency as of v0.9.x; reinstall duck-agent or run 'pip install croniter' in your runtime env.", expr)
            return None
        base_time = now
        if last_run_at:
            try:
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``duck-agent cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool=False) -> None:
    ...
    `duck-agent cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``duck-agent cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``duck-agent cron
    ...
    try:
        ensure_dirs()
        fd, tmp_path = tempfile.mkstemp(dir=str(path.parent), suffix='.tmp', prefix='.terr_')
        try:
            with os.fdopen(fd, 'w', encoding='utf-8') as f:
                f.write(f'{time.time()}\n{message.strip()}\n')
                f.flush()
...
def _merge_unexpected_disk_jobs(jobs: List[Dict[str, Any]], *, removed_ids: Optional[Collection[str]]=None) -> List[Dict[str, Any]]:
    ...
    if not recovered:
        return jobs
    logger.warning('Preserved %d cron job(s) present on disk but missing from the in-memory save payload (concurrent create under degraded lock or stale writer) (#80624): %s', len(recovered), [j.get('id') for j in recovered])
    return jobs + recovered

def _save_jobs_unlocked(jobs: List[Dict[str, Any]], *, removed_ids: Optional[Collection[str]]=None, replace: bool=False):
        ...
        for _attempt in range(5):
            ...
            if not replace:
                ...
                if disk_jobs is not None:
                    payload_ids = {str(j['id']) for j in jobs if isinstance(j, dict) and j.get('id')}
                    intended = {str(i) for i in removed_ids or () if i}
                    if any((isinstance(dj, dict) and dj.get('id') and (str(dj['id']) not in payload_ids) and (str(dj['id']) not in intended) for dj in disk_jobs)):
                        try:
                            os.unlink(tmp_path)
                        except OSError:
                            pass
...
def save_jobs(jobs: List[Dict[str, Any]], *, removed_ids: Optional[Collection[str]]=None, replace: bool=False):
...
def _normalize_workdir(workdir: Optional[str]) -> Optional[str]:
    ...
    if not expanded.is_absolute():
        raise ValueError(f'Cron workdir must be an absolute path (got {raw!r}). Cron jobs run detached from any shell cwd, so relative paths are ambiguous.')
...
def _compute_provider_model_snapshots(*, provider: Any, model: Any, base_url: Any, no_agent: Any) -> Tuple[Optional[str], Optional[str]]:
...
def _normalized_inference_axes(job: Dict[str, Any]) -> Tuple[Optional[str], Optional[str], Optional[str], bool]:
    ...
    return (_normalize_job_optional_text(job.get('provider')), _normalize_job_optional_text(job.get('model')), _normalize_job_optional_text(job.get('base_url'), strip_trailing_slash=True), bool(job.get('no_agent')))

def create_job(prompt: Optional[str], schedule: str, name: Optional[str]=None, repeat: Optional[int]=None, deliver: Optional[str]=None, origin: Optional[Dict[str, Any]]=None, skill: Optional[str]=None, skills: Optional[List[str]]=None, model: Optional[str]=None, provider: Optional[str]=None, base_url: Optional[str]=None, script: Optional[str]=None, context_from: Optional[Union[str, List[str]]]=None, enabled_toolsets: Optional[List[str]]=None, workdir: Optional[str]=None, no_agent: bool=False, attach_to_session: Optional[bool]=None, monitor_script: Optional[str]=None, monitor_url: Optional[str]=None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.duck-agent/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.duck-agent/scripts/, .sh/.bash via bash, else Python). Each
    ...
    if normalized_monitor_script and normalized_monitor_url:
        raise ValueError('monitor_script and monitor_url are mutually exclusive — a job can only have one monitor source.')
    if (normalized_monitor_script or normalized_monitor_url) and normalized_no_agent:
        raise ValueError('monitor_script/monitor_url cannot be combined with no_agent=True — the whole point of a monitor job is to suppress or wake the AGENT based on source changes. Use a plain no_agent script job instead.')
    ...
    provider_snapshot, model_snapshot = _compute_provider_model_snapshots(provider=normalized_provider, model=normalized_model, base_url=normalized_base_url, no_agent=normalized_no_agent)
    next_run_at = compute_next_run(parsed_schedule)
    if parsed_schedule.get('kind') == 'once' and next_run_at is None:
        run_at = parsed_schedule.get('run_at') or schedule
        logger.warning("Rejecting one-shot cron job '%s': run_at %s is outside the %ss grace window", name or label_source[:50].strip(), run_at, ONESHOT_GRACE_SECONDS)
        raise ValueError(f'Requested one-shot time {run_at} is more than {ONESHOT_GRACE_SECONDS}s in the past and cannot be scheduled.')
    job = {'id': job_id, 'name': name or label_source[:50].strip(), 'prompt': prompt_text, 'skills': normalized_skills, 'skill': normalized_skills[0] if normalized_skills else None, 'model': normalized_model, 'provider': normalized_provider, 'provider_snapshot': provider_snapshot, 'model_snapshot': model_snapshot, 'base_url': normalized_base_url, 'script': normalized_script, 'no_agent': normalized_no_agent, 'monitor_script': normalized_monitor_script, 'monitor_url': normalized_monitor_url, 'monitor_state': None, 'context_from': context_from, 'schedule': parsed_schedule, 'schedule_display': parsed_schedule.get('display', schedule), 'repeat': {'times': repeat, 'completed': 0}, 'enabled': True, 'state': 'scheduled', 'paused_at': None, 'paused_reason': None, 'created_at': now, 'next_run_at': next_run_at, 'last_run_at': None, 'last_status': None, 'last_error': None, 'last_delivery_error': None, 'deliver': deliver, 'origin': origin, 'enabled_toolsets': normalized_toolsets, 'workdir': normalized_workdir}
...
class AmbiguousJobReference(LookupError):
    ...
    def __init__(self, ref: str, matches: List[Dict[str, Any]]):
        ...
        super().__init__(f"Job name '{ref}' is ambiguous — matches {len(matches)} jobs: {ids}. Use the job ID instead.")
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    with _jobs_lock():
        ...
        for i, job in enumerate(jobs):
            ...
            inference_fields_changed = bool({'provider', 'model', 'base_url', 'no_agent'}.intersection(updates)) and _normalized_inference_axes(updated) != previous_inference_axes
            ...
            if schedule_changed:
                ...
                updated['schedule_display'] = updates.get('schedule_display', updated_schedule.get('display', updated.get('schedule_display')))
                if updated.get('state') != 'paused':
                    updated_next_run = compute_next_run(updated_schedule)
                    if updated_next_run is None and updated_schedule.get('kind') == 'once':
                        run_at = updated_schedule.get('run_at') or updated_schedule
                        logger.warning("Rejecting one-shot cron job update '%s': run_at %s is outside the %ss grace window", updated.get('name', job_id), run_at, ONESHOT_GRACE_SECONDS)
                        raise ValueError(f'Requested one-shot time {run_at} is more than {ONESHOT_GRACE_SECONDS}s in the past and cannot be scheduled.')
                    updated['next_run_at'] = updated_next_run
            if inference_fields_changed:
                provider_snapshot, model_snapshot = _compute_provider_model_snapshots(provider=updated.get('provider'), model=updated.get('model'), base_url=updated.get('base_url'), no_agent=updated.get('no_agent'))
            ...
            if updated.get('enabled', True) and updated.get('state') != 'paused' and (not updated.get('next_run_at')):
                ...
                if next_run is None and updated['schedule'].get('kind') == 'once':
                    ...
                    raise ValueError(f'Requested one-shot time {run_at} is in the past (grace window: {ONESHOT_GRACE_SECONDS}s) and cannot be scheduled.')
...
def pause_job(job_id: str, reason: Optional[str]=None) -> Optional[Dict[str, Any]]:
    ...
    if not job:
        return None
    return update_job(job['id'], {'enabled': False, 'state': 'paused', 'paused_at': _hermes_now().isoformat(), 'paused_reason': reason})
...
def resume_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    if next_run_at is None and job['schedule'].get('kind') == 'once':
        ...
        raise ValueError(f'Cannot resume: one-shot time {run_at} is in the past (grace window: {ONESHOT_GRACE_SECONDS}s) and will never fire.')
    return update_job(job['id'], {'enabled': True, 'state': 'scheduled', 'paused_at': None, 'paused_reason': None, 'next_run_at': next_run_at})
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not job:
        return None
    return update_job(job['id'], {'enabled': True, 'state': 'scheduled', 'paused_at': None, 'paused_reason': None, 'next_run_at': _hermes_now().isoformat()})
...
def mark_job_run(job_id: str, success: bool, error: Optional[str]=None, delivery_error: Optional[str]=None, status: Optional[str]=None):
    """
        ...
        for i, job in enumerate(jobs):
            if job['id'] == job_id:
                ...
                if job.get('repeat'):
                    ...
                    preclaimed_oneshot = kind == 'once' and times is not None and (times > 0) and (completed > 0)
                ...
                if job['next_run_at'] is None:
                    ...
                    if kind in {'cron', 'interval'}:
                        ...
                        if not job.get('last_error'):
                            job['last_error'] = "Failed to compute next run for recurring schedule (is the 'croniter' package installed in the gateway's Python env?)"
                        logger.error("Job '%s' (%s) could not compute next_run_at; leaving enabled and marking state=error so the job is not silently disabled.", job.get('name', job.get('id', '?')), kind)
                    else:
                        job['enabled'] = False
                        job['state'] = 'completed'
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = f"# Cron job removed without producing output\n\n- job id: {job.get('id')}\n- name: {job.get('name')}\n- dispatch claimed: {repeat.get('completed', '?')}/{repeat.get('times', '?')}\n- run claimed at: {claim.get('at', 'unknown')} by {claim.get('by', 'unknown')}\n- removed at: {_hermes_now().isoformat()}\n\nThis one-shot job's dispatch was claimed, but the run never completed (`last_run_at` was never written) — the scheduler process was most likely killed or restarted mid-execution. The job has been removed to stop it re-firing; recreate it to run again.\n"
        save_job_output(job.get('id', ''), text)
        logger.warning("Job '%s': removed without a completed run — diagnostic written to its output directory", job.get('name', job.get('id', '?')))
    except Exception as e:
        logger.debug('Failed to write wedged-oneshot diagnostic for job %r: %s', job.get('id'), e)
        ...
        for i, job in enumerate(jobs):
            ...
            if completed >= times:
                if job.get('last_run_at') is not None:
                    ...
                    job['next_run_at'] = None
                    save_jobs(jobs)
                    logger.info("Job '%s': dispatch limit reached (%d/%d) — marking completed", job.get('name', job.get('id', '?')), completed, times)
                    return False
                jobs.pop(i)
                save_jobs(jobs, removed_ids={job_id})
                _write_wedged_oneshot_diagnostic(job)
                logger.info("Job '%s': dispatch limit reached (%d/%d) — removing", job.get('name', job.get('id', '?')), completed, times)
                return False
            repeat['completed'] = completed + 1
            save_jobs(jobs)
            logger.debug("Job '%s': claimed dispatch %d/%d", job.get('name', job.get('id', '?')), repeat['completed'], times)
            return True
        logger.debug('claim_dispatch: job_id %s not in store — proceeding without claim (handed-in job dict; nothing to persist a claim against)', job_id)
        return True
...
def _completed_oneshot_retention_days() -> float:
    ...
    try:
        ...
        return float(cron_cfg.get('completed_retention_days', COMPLETED_ONESHOT_RETENTION_DAYS))
    except Exception:
        return float(COMPLETED_ONESHOT_RETENTION_DAYS)

def _sweep_completed_oneshots(raw_jobs: List[Dict[str, Any]], now: datetime, *, removed_ids: Optional[Set[str]]=None) -> bool:
    ...
    for rj in list(raw_jobs):
        try:
            ...
            logger.info("Job '%s': pruning completed one-shot record (finished %s, retention %.1f days)", rj.get('name', rj.get('id', '?')), last_run, retention_days)
        except Exception:
            logger.debug('Retention sweep skipped malformed job record %r', rj.get('id', '?'), exc_info=True)
    return removed
    ...
    for job in jobs:
        try:
            ...
            if existing_claim and job.get('schedule', {}).get('kind') == 'once':
                try:
                    claimed_at = _ensure_aware(datetime.fromisoformat(existing_claim['at']))
                    _age = (now - claimed_at).total_seconds()
            ...
            if not next_run:
                schedule = job.get('schedule', {})
                kind = schedule.get('kind')
                recovered_next = _recoverable_oneshot_run_at(schedule, now, last_run_at=job.get('last_run_at'))
                ...
                logger.info("Job '%s' had no next_run_at; recovering %s run at %s", job.get('name', job.get('id', '?')), recovery_kind, recovered_next)
                for rj in raw_jobs:
                    if rj['id'] == job['id']:
            ...
            if kind == 'cron' and next_run_dt <= now and _timezone_offset_mismatch(raw_next_run_dt, now) and _stored_wall_clock_is_future(raw_next_run_dt, now):
                new_next = compute_next_run(schedule, now.isoformat())
                if new_next:
                    logger.info("Job '%s' next_run_at offset changed (%s -> %s). Recomputing cron run to preserve local wall-clock intent: %s", job.get('name', job.get('id', '?')), raw_next_run_dt.utcoffset(), now.utcoffset(), new_next)
                    for rj in raw_jobs:
                        if rj['id'] == job['id']:
            ...
            if next_run_dt <= now:
                ...
                if kind in {'cron', 'interval'} and (now - next_run_dt).total_seconds() > grace:
                    new_next = compute_next_run(schedule, now.isoformat())
                    if new_next:
                        logger.info("Job '%s' missed its scheduled time (%s, grace=%ds). Running now; next run provisionally set to: %s (re-anchored on completion)", job.get('name', job.get('id', '?')), next_run, grace, new_next)
                        for rj in raw_jobs:
                            if rj['id'] == job['id']:
                                rj['next_run_at'] = new_next
                                needs_save = True
                    ...
                    if repeat:
                        ...
                        if times is not None and times > 0 and (completed >= times):
                            if _job_running_in_this_process(job.get('id', '')):
                                logger.info("Job '%s': dispatch limit reached (%d/%d) but its run is still in flight in this process — keeping entry", job.get('name', job.get('id', '?')), completed, times)
                                continue
                            logger.info("Job '%s': one-shot dispatch limit reached (%d/%d) — removing stale due entry", job.get('name', job.get('id', '?')), completed, times)
                            for rj in raw_jobs:
                                if rj['id'] == job['id']:
                                    raw_jobs.remove(rj)
            ...
            logger.exception('Skipping malformed cron job %r during due scan', job.get('name') or job.get('id') or '?')
            continue
    if needs_save:
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    if keep <= 0:
        return 0
    try:
        files = sorted((f for f in job_output_dir.glob('*.md') if f.is_file()), key=lambda f: f.name, reverse=True)
    except OSError:
        return 0
    deleted = 0
...
def rewrite_skill_refs(consolidated: Optional[Dict[str, str]]=None, pruned: Optional[List[str]]=None) -> Dict[str, Any]:
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            changed = True
            rewrites.append({'job_id': job.get('id'), 'job_name': job.get('name') or job.get('id'), 'before': list(skills_before), 'after': list(new_skills), 'mapped': mapped, 'dropped': dropped})
        if changed:
            save_jobs(jobs)
