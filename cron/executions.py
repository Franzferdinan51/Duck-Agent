...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute('CREATE INDEX IF NOT EXISTS idx_executions_job_claimed ON executions(job_id, claimed_at DESC, id DESC)')
...
def _emit_execution_state(record: Optional[Dict[str, Any]], *, delivery_outcome: Optional[str]=None) -> None:
...
def create_execution(job_id: str, *, source: str) -> Dict[str, Any]:
    ...
    with _transaction() as conn:
        conn.execute("INSERT INTO executions\n               (id, job_id, source, process_id, pid, process_started_at,\n                status, claimed_at)\n               VALUES (?, ?, ?, ?, ?, ?, 'claimed', ?)", (execution_id, str(job_id), str(source), _PROCESS_ID, pid, _process_start_time(pid), now))
...
def mark_execution_running(execution_id: str) -> Optional[Dict[str, Any]]:
    ...
    now = _hermes_now().isoformat()
    with _transaction() as conn:
        cur = conn.execute("UPDATE executions SET status='running', started_at=?\n               WHERE id=? AND status='claimed'", (now, execution_id))
        if cur.rowcount != 1:
            return None
        record = _record(conn.execute('SELECT * FROM executions WHERE id=?', (execution_id,)).fetchone())
    _emit_execution_state(record)
    return record

def finish_execution(execution_id: str, *, success: bool, error: Optional[str]=None, delivery_outcome: Optional[str]=None) -> Optional[Dict[str, Any]]:
    """Write a terminal result once; terminal attempts cannot be rewritten."""
    now = _hermes_now().isoformat()
    status = 'completed' if success else 'failed'
    detail = None if success else str(error) if error else 'unknown failure'
    with _transaction() as conn:
        cur = conn.execute("UPDATE executions SET status=?, finished_at=?, error=?\n               WHERE id=? AND status IN ('claimed','running')", (status, now, detail, execution_id))
        if cur.rowcount != 1:
            return None
...
def recover_interrupted_executions() -> int:
    ...
    with _transaction() as conn:
        rows = conn.execute("SELECT id, process_id, pid, process_started_at FROM executions\n               WHERE status IN ('claimed','running')").fetchall()
        for row in rows:
            if row['process_id'] == _PROCESS_ID:
                continue
            if _owner_is_live(int(row['pid']), row['process_started_at']):
                continue
            cur = conn.execute("UPDATE executions SET status='unknown', finished_at=?, error=?\n                   WHERE id=? AND status IN ('claimed','running')", (now, "Scheduler restarted after this execution's owner exited before a durable terminal state; whether side effects ran is unknown.", row['id']))
            changed += cur.rowcount
            if cur.rowcount:
                record = _record(conn.execute('SELECT * FROM executions WHERE id=?', (row['id'],)).fetchone())
...
def list_executions(*, job_id: Optional[str]=None, limit: int=50, before_claimed_at: Optional[str]=None) -> List[Dict[str, Any]]:
    ...
    with _transaction() as conn:
        rows = conn.execute('SELECT * FROM executions' + where + ' ORDER BY claimed_at DESC, id DESC LIMIT ?', params).fetchall()
...
def latest_executions(job_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    with _transaction() as conn:
        rows = conn.execute(f'SELECT e.* FROM executions e\n                WHERE e.job_id IN ({placeholders})\n                  AND e.id=(SELECT e2.id FROM executions e2\n                            WHERE e2.job_id=e.job_id\n                            ORDER BY e2.claimed_at DESC, e2.id DESC LIMIT 1)', clean).fetchall()
