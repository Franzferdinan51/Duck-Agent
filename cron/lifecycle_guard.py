...
``duck-agent gateway restart`` (or ``launchctl kickstart ai.duck-agent.gateway``
or ``systemctl restart duck-agent-gateway``).  When the cron fires, the
...
``duck-agent cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``duck-agent gateway``, ``launchctl ... duck-agent-gateway``,
``systemctl ... duck-agent-gateway``, ``pkill`` against the gateway) so it
...
primitive as a persistent KeepAlive job, not a one-shot task. ``duck-agent gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile('(?i)(?:duck-agent\\s+gateway\\s+(?:restart|stop))|(?:launchctl\\s+(?:kickstart|unload|load|stop|restart|submit|bootstrap)\\b[^\\n]*\\bhermes[.\\-]?gateway)|(?:systemctl\\s+(?:-\\S+\\s+)*(?:restart|stop|start)\\b[^\\n]*\\bhermes[.\\-]?gateway)|(?:p?kill\\b[^\\n]*\\bhermes\\b[^\\n]*\\bgateway)|(?:p?kill\\b[^\\n]*\\bgateway\\b[^\\n]*\\bhermes)')
...
_DATA_SINK_EXECUTABLES = frozenset({'grep', 'egrep', 'fgrep', 'rg', 'ag', 'ack', 'journalctl', 'sqlite3', 'psql'})
_UNSAFE_DATA_ARG_MARKERS = ('`', '$(', '<(', '>(', '\\!')
_PIPE_TO_INTERPRETER = re.compile('\\|\\s*&?\\s*(?:sudo\\s+)?(?:sh|bash|dash|ksh|zsh|xargs|eval|source)\\b')
_BINARY_MAGIC_PREFIXES = (b'\x7fELF', b'MZ', b'\xca\xfe\xba\xbe', b'\xcf\xfa\xed\xfe', b'\xce\xfa\xed\xfe', b'\xfe\xed\xfa\xce', b'\xfe\xed\xfa\xcf')
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.duck-agent.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart duck-agent-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        grep -c 'systemctl restart duck-agent-gateway' /var/log/syslog
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart duck-agent-gateway%'"
    ...
    for line in text.splitlines() or [text]:
        ...
        for segment in segments:
            ...
            if index is not None and Path(segment[index]).name in _DATA_SINK_EXECUTABLES:
                arguments = segment[index + 1:]
                if not any((argument.startswith('.') or any((marker in argument for marker in _UNSAFE_DATA_ARG_MARKERS)) for argument in arguments)):
                    changed = True
...
def _contains_unsafe_gateway_action(command: str, *, cwd: Optional[str], depth: int, visited: set[Path], read_remote_script: Optional[_ReadRemoteScriptFn]=None) -> bool:
    if _direct_lifecycle_scan(command):
        return True
    if depth >= _MAX_REFERENCED_SCRIPT_DEPTH:
        return True
    for payload in _iter_shell_command_payloads(command):
        if _contains_unsafe_gateway_action(payload, cwd=cwd, depth=depth + 1, visited=visited, read_remote_script=read_remote_script):
            return True
    for script_path in _iter_referenced_shell_scripts(command, cwd=cwd):
        ...
        if _contains_unsafe_gateway_action(script_text, cwd=script_dir, depth=depth + 1, visited=visited, read_remote_script=read_remote_script):
            return True
    return False

def contains_gateway_lifecycle_command_or_referenced_script(command: str, *, cwd: Optional[str]=None, read_remote_script: Optional[_ReadRemoteScriptFn]=None) -> bool:
    ...
    """
    try:
        return _contains_unsafe_gateway_action(command, cwd=cwd, depth=0, visited=set(), read_remote_script=read_remote_script)
    except Exception:
        logger.warning('lifecycle guard referenced-script walk failed; falling back to direct-scan verdict', exc_info=True)
        try:
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<DUCK_AGENT_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.duck-agent/scripts/restart.sh``) but is passed as the bare name
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return 'duck-agent gateway restart'
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str]=None) -> None:
    ...
    if python_script:
        ...
        unsafe = contains_gateway_lifecycle_command_or_referenced_script(combined, cwd=script_dir)
    if unsafe:
        raise GatewayLifecycleBlocked('Blocked: cron job contains a gateway lifecycle command or persistent launchctl submit operation. This is blocked to prevent agent-driven SIGTERM-respawn loops under launchd/systemd supervision (#30719). Run `duck-agent gateway restart` from a shell outside the running gateway instead.')