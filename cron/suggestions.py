...
A *suggestion* is a ready-to-run cron job spec that Duck Agent surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.duck-agent/cron/suggestions.json``, atomic
...
def add_suggestion(*, title: str, description: str, source: str, job_spec: Dict[str, Any], dedup_key: str) -> Optional[Dict[str, Any]]:
    ...
    with _suggestions_lock:
        ...
        if pending_count >= MAX_PENDING:
            ...
            return None
        record = {'id': uuid.uuid4().hex[:12], 'title': title.strip(), 'description': description.strip(), 'source': source, 'job_spec': job_spec, 'dedup_key': dedup_key.strip(), 'status': _STATUS_PENDING, 'created_at': _hermes_now().isoformat()}
