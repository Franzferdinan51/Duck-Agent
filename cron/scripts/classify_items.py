...
Design choices:
  * Uses Duck Agent' auxiliary client with task="monitor", so the classifier model
...
def main() -> int:
    ...
    try:
        resp = call_llm(task='monitor', messages=[{'role': 'user', 'content': prompt}], max_tokens=1024, temperature=0)
    ...
    if not surfaced:
        return 0
    if args.format == 'json':
        out = [{'id': _item_id(item, i), 'score': s.get('score'), 'reason': s.get('reason', ''), 'item': item} for i, item, s in surfaced]
