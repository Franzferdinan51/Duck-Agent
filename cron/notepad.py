...
Write path is the CLI (``duck-agent cron notepad <job_id> set <key> <value>``),
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS cron_notepad (\n             job_id TEXT NOT NULL,\n             key TEXT NOT NULL,\n             value TEXT NOT NULL,\n             updated_at TEXT NOT NULL,\n             PRIMARY KEY (job_id, key)\n           )')
...
def set_note(job_id: str, key: str, value: str) -> Dict[str, Any]:
    ...
    with _transaction() as conn:
        row = conn.execute('SELECT COALESCE(SUM(LENGTH(CAST(key AS BLOB))\n                 + LENGTH(CAST(value AS BLOB))), 0)\n               FROM cron_notepad WHERE job_id=? AND key<>?', (job_id, key)).fetchone()
        ...
        conn.execute('INSERT INTO cron_notepad (job_id, key, value, updated_at)\n               VALUES (?, ?, ?, ?)\n               ON CONFLICT(job_id, key)\n               DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at', (job_id, key, value, now))
    return {'job_id': job_id, 'key': key, 'value': value, 'updated_at': now}

def get_note(job_id: str, key: str) -> Optional[str]:
    with _transaction() as conn:
        row = conn.execute('SELECT value FROM cron_notepad WHERE job_id=? AND key=?', (str(job_id), str(key))).fetchone()
    return None if row is None else row['value']

def delete_note(job_id: str, key: str) -> bool:
    with _transaction() as conn:
        cur = conn.execute('DELETE FROM cron_notepad WHERE job_id=? AND key=?', (str(job_id), str(key)))
    return cur.rowcount > 0

def list_notes(job_id: str) -> List[Dict[str, Any]]:
    """All entries for one job, sorted by key."""
    with _transaction() as conn:
        rows = conn.execute('SELECT job_id, key, value, updated_at FROM cron_notepad WHERE job_id=? ORDER BY key', (str(job_id),)).fetchall()
    return [dict(row) for row in rows]

def clear_notepad(job_id: str) -> int:
    """Delete every key for one job (e.g. on job removal). Returns row count."""
    with _transaction() as conn:
        cur = conn.execute('DELETE FROM cron_notepad WHERE job_id=?', (str(job_id),))
    return cur.rowcount
...
def render_notepad_section(job_id: str) -> str:
    ...
    return f'## Job notepad (persistent across runs)\nThis durable scratchpad survives between scheduled runs of this job. Update it via the CLI, e.g.:\n`duck-agent cron notepad {job_id} set <key> <value>` (also: get/delete/list; `duck-agent cron notepad {job_id} delete <key>` removes an entry).\n\n' + '\n'.join(lines) + '\n\n'