...
Hooks are discovered from ~/.duck-agent/hooks/ directories, each containing:
...
Context dict passed to ``agent:start`` / ``agent:end`` handlers:
  ...
  session_id   -- Duck Agent session id
    ...
    def discover_and_load(self) -> None:
        ...
        for hook_dir in sorted(HOOKS_DIR.iterdir()):
            ...
            try:
                ...
                self._loaded_hooks.append({'name': hook_name, 'description': manifest.get('description', ''), 'events': events, 'path': str(hook_dir)})
