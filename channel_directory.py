...
~/.duck-agent/channel_directory.json.  The send_message tool reads this file for
        ...
        for chat_id, friendly in id_map.items():
            ...
            for e in entries:
                if isinstance(e, dict) and e.get('id') == chat_id:
                    ...
                    matched = True
            if not matched:
                entries.append({'id': chat_id, 'name': friendly, 'type': 'group' if str(chat_id).endswith('@g.us') else 'dm', 'thread_id': None})
...
async def build_channel_directory(adapters: Dict[Any, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for entry in platform_registry.plugin_entries():
            if entry.name not in _SKIP_SESSION_DISCOVERY and entry.name not in platforms and (entry.name in adapter_platform_names):
...
def _build_discord(adapter) -> List[Dict[str, str]]:
    ...
    for guild in client.guilds:
        for ch in guild.text_channels:
            channels.append({'id': str(ch.id), 'name': ch.name, 'guild': guild.name, 'type': 'channel'})
        forums = getattr(guild, 'forum_channels', None) or []
        for ch in forums:
            channels.append({'id': str(ch.id), 'name': ch.name, 'guild': guild.name, 'type': 'forum'})
    ...
    for team_id, client in team_clients.items():
        try:
            ...
            for _page in range(20):
                response = await client.users_conversations(types='public_channel,private_channel', exclude_archived=True, limit=200, cursor=cursor)
...
def _build_from_sessions_db(platform_name: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        for row in rows:
            ...
            entries.append({'id': entry_id, 'name': _session_entry_name(origin), 'type': row.get('chat_type') or 'dm', 'thread_id': origin.get('thread_id')})
    except Exception as e:
    ...
    try:
        ...
        for _key, session in data.items():
            ...
            if not entry_id or entry_id in seen_ids:
                continue
            seen_ids.add(entry_id)
            entries.append({'id': entry_id, 'name': _session_entry_name(origin), 'type': session.get('chat_type', 'dm'), 'thread_id': origin.get('thread_id')})
    except Exception as e:
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]]=None) -> str:
    ...
    ``duck-agent send --list`` to merge in configured-but-undiscovered
    ...
    for plat_name, channels in sorted(platforms.items()):
        if not channels:
            ...
            lines.append(f"  (no channels discovered yet — send directly with {plat_name}:<chat_id>, or bare '{plat_name}' for the home channel)")
            lines.append('')
            continue
