...
if [[ "$USE_DIST" == "true" ]]; then
  BUNDLE="$SCRIPT_DIR/prime-agent-packages/coding-agent/dist/bundle/cli.js"
  if [[ ! -f "$BUNDLE" ]]; then
...
TSX_BIN="$SCRIPT_DIR/prime-agent-packages/coding-agent/node_modules/.bin/tsx"
if [[ ! -x "$TSX_BIN" ]]; then
  echo "tsx not found at $TSX_BIN. Run npm install from the repo root first." >&2
  exit 1
fi

"$TSX_BIN" "$SCRIPT_DIR/prime-agent-packages/coding-agent/src/cli.ts" ${ARGS[@]+"${ARGS[@]}"}
