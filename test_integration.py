...
class TestEndToEndLauncher(unittest.TestCase):
    ...
    def test_backends_list_all_three(self):
        ...
        result = subprocess.run([str(LAUNCHER), '--backends'], capture_output=True, text=True, timeout=10)
        self.assertEqual(result.returncode, 0)
        self.assertIn('grok-build', result.stdout)
        self.assertIn('duck-agent-compatible', result.stdout)
    ...
    def test_status_shows_current_backend(self):
        ...
        result = subprocess.run([str(LAUNCHER), '--status'], capture_output=True, text=True, timeout=10, env=env)
        self.assertEqual(result.returncode, 0)
        self.assertIn('grok-build', result.stdout)

    def test_status_with_hermes_backend(self):
        """--status should show the duck-agent-compatible backend when set."""
        env = os.environ.copy()
        env['DUCK_AGENT_BACKEND'] = 'duck-agent-compatible'
        result = subprocess.run([str(LAUNCHER), '--status'], capture_output=True, text=True, timeout=10, env=env)
        self.assertEqual(result.returncode, 0)
        self.assertIn('duck-agent-compatible', result.stdout)
...
class TestNoHermesConflicts(unittest.TestCase):
    """Verify Duck Agent does not conflict with Duck Agent."""

    def test_app_id_is_duck_agent(self):
        """The app ID should be com.duckagent.desktop, not com.nousresearch.duck-agent."""
        package_json = DUCK_AGENT_DIR / 'apps/desktop/package.json'
        content = package_json.read_text()
        self.assertIn('"com.duckagent.desktop"', content)
        self.assertNotIn('"com.nousresearch.duck-agent"', content)
    ...
    def test_product_name_is_duck_agent(self):
        ...
        self.assertNotIn('"Duck Agent"', content)

    def test_protocol_scheme_is_duck_agent(self):
        """The protocol scheme should be duck-agent, not duck-agent."""
        package_json = DUCK_AGENT_DIR / 'apps/desktop/package.json'
        content = package_json.read_text()
        self.assertIn('"duck-agent"', content)
        self.assertNotIn('"duck-agent"', content)
    ...
    def test_executable_name_is_duck(self):
        """The executable name should be DuckAgent, not Duck Agent."""
        package_json = DUCK_AGENT_DIR / 'apps/desktop/package.json'
        content = package_json.read_text()
...
class TestMultiBackendIntegration(unittest.TestCase):
    ...
    def test_hermes_compatible_full_init(self):
        """Test full initialization of Duck Agent-compatible backend."""
        env = os.environ.copy()
        env['DUCK_AGENT_BACKEND'] = 'duck-agent-compatible'
        result = subprocess.run([str(LAUNCHER), '--status'], capture_output=True, text=True, timeout=10, env=env)
        self.assertEqual(result.returncode, 0)
