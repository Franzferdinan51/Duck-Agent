...
_HUMAN_TIMESTAMP_RE = re.compile('^\\[(?P<dow>[A-Z][a-z]{2}) (?P<date>\\d{4}-\\d{2}-\\d{2}) (?P<time>\\d{2}:\\d{2}:\\d{2})(?: (?P<tz>[A-Za-z0-9_+\\-/:]+))?\\]\\s*')
_ISO_TIMESTAMP_RE = re.compile('^\\[(?P<iso>\\d{4}-\\d{2}-\\d{2}T[^\\]]+)\\]\\s*')
