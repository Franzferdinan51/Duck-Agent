...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `duck-agent backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - duck_agent_home (str): The active DUCK_AGENT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.duck-agent``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "duck-agent").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'duck-agent memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], hermes_home: str) -> None:
        ...
        Called by 'duck-agent memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``duck_agent_home`` is the active DUCK_AGENT_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE DUCK_AGENT_HOME.

        ``duck-agent backup`` only walks DUCK_AGENT_HOME, so any provider state kept
        ...
        of the archive, and ``duck-agent import`` restores them to their original
