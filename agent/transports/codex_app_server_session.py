...
Owns one Codex thread per Duck Agent session. Drives `turn/start`, consumes
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Duck Agent/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Duck Agent session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        into Duck Agent' messages shape.
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Duck Agent' approval
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Duck Agent
        approval-mode/timeout logic. The Duck Agent-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Duck Agent approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Duck Agent approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Duck Agent returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Duck Agent approval mode/timeout resolution
...
def _get_hermes_version() -> str:
    """Best-effort Duck Agent version string for codex's userAgent line."""
    try:
