...
``kind: backend``) or ``~/.duck-agent/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    ``os.environ`` first, then ``~/.duck-agent/.env``) so credentials set
    through Duck Agent' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``duck-agent tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``duck-agent tools`` paint.
        """
