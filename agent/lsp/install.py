...
appropriate.  All installs go to a Duck Agent-owned bin staging dir,
``<DUCK_AGENT_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``duck-agent lsp
  status``.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the Duck Agent-owned bin staging dir for LSP servers."""
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a duck-agent-owned target dir.
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``duck-agent lsp status`` CLI to give users a quick
