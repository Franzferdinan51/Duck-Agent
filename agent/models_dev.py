...
Data resolution order:
  ...
  2. Disk cache (~/.duck-agent/models_dev_cache.json — any age; stale data is
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``duck-agent config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Duck Agent provider ID to its models dict from models.dev.
...
def get_provider_info(
    ...
    Accepts either a Duck Agent provider ID (e.g. "kilocode") or a models.dev
...
def get_model_info(
    ...
    Accepts Duck Agent or models.dev provider ID.  Tries exact match then
