...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Duck Agent does
...
class EntraIdentityConfig:
    ...
    Captures the Duck Agent-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Duck Agent-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.duck-agent/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Duck Agent processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``duck-agent doctor`` /
    ``duck-agent auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``duck-agent doctor`` and the wizard preflight — never
