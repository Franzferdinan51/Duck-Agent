...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Duck Agent with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.duck-agent/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Duck Agent
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``duck-agent tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``duck-agent setup`` for
