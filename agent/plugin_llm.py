...
Plugins built on Duck Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Duck Agent
...
embedded slugs, no shorthands. This mirrors Duck Agent' main config
...
handles every provider, fallback chain, and per-task override Duck Agent
supports.
"""
