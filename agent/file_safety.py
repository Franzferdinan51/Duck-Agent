...
def _hermes_home_path() -> Path:
    """Resolve the active DUCK_AGENT_HOME (profile-aware) without circular imports."""
    try:
...
def _hermes_root_path() -> Path:
    """Resolve the Duck Agent root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Duck Agent path.

    Three categories are blocked:

      * Internal Duck Agent cache files under ``DUCK_AGENT_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under DUCK_AGENT_HOME and the global Duck Agent
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.duck-agent/.env`` and exfiltrate the file. The read-deny exists
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Duck Agent read (see
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from DUCK_AGENT_HOME.

    ``~/.duck-agent``              -> ``"default"``
    ``~/.duck-agent/profiles/X``  -> ``"X"``
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Duck Agent scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Duck Agent entirely. Caller is expected to surface the warning to the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.duck-agent`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.duck-agent/…`` and returns the
    index where the inner Duck Agent-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Duck Agent state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.duck-agent``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.duck-agent`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Duck Agent resolver to
    succeed, so it works correctly even when called from contexts where
    DUCK_AGENT_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host DUCK_AGENT_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Duck Agent state.
