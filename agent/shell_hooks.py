...
* First-use consent is gated by the allowlist under
  ``~/.duck-agent/shell-hooks-allowlist.json``.  Non-TTY callers must pass
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Duck Agent-canonical
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``duck-agent hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Duck Agent wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Duck Agent
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Duck Agent is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    Used by ``duck-agent hooks test`` and ``duck-agent hooks doctor``.
    ...
    produce — otherwise scripts tested via ``duck-agent hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Duck Agent-wire-shape response."""
