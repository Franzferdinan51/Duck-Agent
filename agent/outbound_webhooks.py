...
(``gateway/platforms/webhook.py``): inbound wakes Duck Agent when the world
changes; outbound tells the world when Duck Agent does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Duck Agent-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
Config schema (``~/.duck-agent/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/duck-agent-events
          ...
          name: ci-notify   # optional label for logs / `duck-agent hooks list`
...
Headers::
    ...
    User-Agent:              Duck Agent-Agent-Outbound-Webhook
    X-Duck Agent-Event:          <hook event name>
    X-Duck Agent-Delivery:       <delivery_id>
    X-Duck Agent-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Duck Agent-Delivery`` header so
