...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Duck Agent' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Duck Agent-created Telegram private-chat
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Duck Agent root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* DUCK_AGENT_HOME's cache, so a gateway running at
    the root (e.g. ``DUCK_AGENT_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when DUCK_AGENT_HOME is symlinked under a
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Duck Agent
    secrets (``~/.duck-agent/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Duck Agent-managed cache, under an operator-allowlisted root
...
class BasePlatformAdapter(ABC):
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Duck Agent sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-DUCK_AGENT_HOME holder may be replaced only when the runner
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Duck Agent home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Duck Agent home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.duck-agent/media/generated/stale.png"}
    ...
    def build_source(
        ...
        per-profile DUCK_AGENT_HOME isolation.
        """
