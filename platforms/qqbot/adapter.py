...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.duck-agent/.update_response`` for the
          detached ``duck-agent update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``duck-agent update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``duck-agent update --gateway`` watcher. Button
        ...
        ``~/.duck-agent/.update_response`` so the detached update process
        can read it.
        """
