...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to DUCK_AGENT_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/duck-agent), this function
    mirrors the bridge source to a writable DUCK_AGENT_HOME location and returns that
