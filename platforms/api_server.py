...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Duck Agent-Session-Id header; opt-in long-term memory scoping via X-Duck Agent-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Duck Agent-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists duck-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Duck Agent sessions
- POST /api/sessions               — create an empty Duck Agent session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to duck-agent
...
def _hermes_version() -> str:
    """Return the canonical Duck Agent version string.
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``duck-agent``)
    ...
    ``direct_model_requests`` config value here, while Duck Agent-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Duck Agent-aware and is
    always honored.
    """
...
def _derive_chat_session_id(
    ...
    the same Duck Agent session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through duck-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Duck Agent-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Duck Agent-Session-Id``: callers may send
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``duck-agent sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the duck-agent-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Duck Agent-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list duck-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Duck Agent provider/model inventory.
        ...
        Duck Agent provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Duck Agent version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Duck Agent sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Duck Agent session row.
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``duck-agent.tool.progress`` with ``status: running``.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: duck-agent.tool.progress`` SSE event so
