...
_SILENCE_NARRATION = re.compile('^[\\s*_~`]*\\(?\\s*(silent|silence|no\\s+response|no\\s+reply)\\s*\\.?\\)?[\\s*_~`]*$|^[\\s*_~`]*[\\U0001F507\\.\\u2026]+[\\s*_~`]*$', re.IGNORECASE)
...
class DeliveryTransport:
    ...
    async def send(self, logical_platform: Platform, chat_id: str, content: str, metadata: Optional[Dict[str, Any]]) -> Any:
...
def resolve_delivery_transport(platform: Platform, config: GatewayConfig, adapters: Optional[Dict[Platform, Any]]) -> Optional[DeliveryTransport]:
    ...
    if native is not None and (native_config is None or native_config.enabled):
        return DeliveryTransport(adapter=native, config=native_config, transport_platform=platform)
    ...
    if relay is not None and (relay_config is None or relay_config.enabled) and callable(fronts_platform) and fronts_platform(platform):
        return DeliveryTransport(adapter=relay, config=relay_config, transport_platform=Platform.RELAY)
    return None
...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: Dict[Platform, Any]=None, dead_targets: Optional[DeadTargetRegistry]=None):
        """
    ...
    async def deliver(self, content: str, targets: List[DeliveryTarget], job_id: Optional[str]=None, job_name: Optional[str]=None, metadata: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        ...
        for target in targets:
            if target.platform != Platform.LOCAL and target.chat_id and self.dead_targets.is_dead(target.platform.value, target.chat_id):
                logger.info('Skipping delivery to known-dead target %s:%s (send to it again to clear)', target.platform.value, target.chat_id)
                results[target.to_string()] = {'success': False, 'skipped': 'dead_target', 'error': 'target previously confirmed unreachable'}
                continue
            try:
                ...
                if target.platform != Platform.LOCAL and target.chat_id:
                    ...
                    if dead_kind:
                        self.dead_targets.mark_dead(target.platform.value, target.chat_id, reason=f'{dead_kind}: {str(e)[:120]}')
                results[target.to_string()] = {'success': False, 'error': str(e)}
        return results

    def _deliver_local(self, content: str, job_id: Optional[str], job_name: Optional[str], metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _deliver_to_platform(self, target: DeliveryTarget, content: str, metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(content) > MAX_PLATFORM_OUTPUT:
            try:
                ...
                logger.warning('Audit save failed for cron output (%d chars, job=%s): %s — delivery proceeds without audit copy', len(content), job_id, exc)
            if getattr(adapter, 'splits_long_messages', False):
                if saved_path:
                    logger.info('Cron output preserved for chunking adapter (%d chars) — full output saved to %s', len(content), saved_path)
            else:
        ...
        if self._filter_silence_narration_enabled() and _is_silence_narration(content):
            logger.warning('Dropped silence-narration outbound to %s (chat=%s): %r', target.platform.value, target.chat_id, content[:40])
        ...
        if target.thread_id:
            has_explicit_direct_topic = 'direct_messages_topic_id' in send_metadata or 'telegram_direct_messages_topic_id' in send_metadata
            target_thread_id = target.thread_id
            is_named_telegram_private_topic = target.platform == Platform.TELEGRAM and looks_like_telegram_private_chat_id(target.chat_id) and (not _looks_like_int(target_thread_id)) and ('thread_id' not in send_metadata) and ('message_thread_id' not in send_metadata) and (not has_explicit_direct_topic)
            if is_named_telegram_private_topic:
                ...
                if not created_thread_id:
                    raise RuntimeError(f"Failed to create Telegram private DM topic '{target_thread_id}'")
                target_thread_id = str(created_thread_id)
                send_metadata['thread_id'] = target_thread_id
                send_metadata['telegram_dm_topic_created_for_send'] = True
            elif target.platform == Platform.TELEGRAM and looks_like_telegram_private_chat_id(target.chat_id) and ('thread_id' not in send_metadata) and ('message_thread_id' not in send_metadata) and (not has_explicit_direct_topic):
                reply_anchor = send_metadata.get('telegram_reply_to_message_id')
                if reply_anchor is None:
                    raise RuntimeError('Telegram private DM topic delivery requires telegram_reply_to_message_id; send to the bare chat or provide a reply anchor')
                send_metadata['thread_id'] = target_thread_id
        ...
        if _send_result_failed(result):
            if is_named_telegram_private_topic and named_telegram_private_topic_name and _is_thread_not_found_delivery_error(result):
                ensure_dm_topic = getattr(adapter, 'ensure_dm_topic', None)
                if ensure_dm_topic is None:
                    raise RuntimeError('Telegram adapter cannot refresh named private DM topics')
                refreshed_thread_id = await ensure_dm_topic(target.chat_id, named_telegram_private_topic_name, force_create=True)
                if not refreshed_thread_id:
                    raise RuntimeError(f"Failed to refresh Telegram private DM topic '{named_telegram_private_topic_name}'")
