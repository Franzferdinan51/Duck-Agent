...
2. An event-loop heartbeat file at ``<DUCK_AGENT_HOME>/state/gateway.heartbeat`` so
...
def _arm_loop_floor_timer(loop: asyncio.AbstractEventLoop, interval: float=DEFAULT_LOOP_FLOOR_TIMER_INTERVAL_S) -> _LoopFloorTimerHandle:
...
def start_loop_liveness_watchdog(loop: asyncio.AbstractEventLoop, *, probe_interval: float=DEFAULT_LOOP_WATCHDOG_INTERVAL_S, probe_timeout: float=DEFAULT_LOOP_WATCHDOG_TIMEOUT_S, max_strikes: int=DEFAULT_LOOP_WATCHDOG_MAX_STRIKES, exit_code: int=GATEWAY_SERVICE_RESTART_EXIT_CODE) -> Optional[_LoopLivenessWatchdogHandle]:
    ...
    def _watchdog() -> None:
        ...
        while not stop_event.wait(timeout=interval):
            ...
            if stop_event.is_set():
                return
            try:
                logger.critical('Gateway event loop missed %d consecutive liveness probes; dumping all thread stacks and exiting with code %d so the service supervisor can restart it.', strikes, exit_code)
            except Exception:
                pass
            try:
...
def _process_hermes_home() -> Path:
    """DUCK_AGENT_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path]=None) -> Path:
    """Return ``<DUCK_AGENT_HOME>/state/gateway.heartbeat``."""
...
def write_loop_heartbeat(*, pid: Optional[int]=None, start_time: Optional[float]=None, home: Optional[Path]=None, extra: Optional[Dict[str, Any]]=None) -> Path:
    ...
    payload: Dict[str, Any] = {'pid': int(pid if pid is not None else os.getpid()), 'updated_at': datetime.now(timezone.utc).isoformat(), 'monotonic': time.monotonic()}
...
def _write_watchdog_dump(dump_path: Path, *, delay_s: float, snapshot: Optional[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        return
    header = {'event': 'shutdown_watchdog_fired', 'pid': os.getpid(), 'delay_s': delay_s, 'fired_at': datetime.now(timezone.utc).isoformat(), 'snapshot': snapshot or {}}
    try:
        ...
        pass
    try:
        sys.stderr.write(f'Gateway shutdown watchdog fired after {delay_s:.0f}s (pid={os.getpid()}); dumping all thread stacks.\n')
        sys.stderr.flush()
        faulthandler.dump_traceback(all_threads=True)
    except Exception:
        pass
...
def arm_shutdown_watchdog(delay_s: float, *, done_event: Optional[threading.Event]=None, snapshot_fn: Optional[Callable[[], Dict[str, Any]]]=None, exit_code: int=1, dump_path: Optional[Path]=None, name: str='gateway-shutdown-watchdog') -> threading.Event:
    ...
    def _watchdog() -> None:
        ...
        try:
            logger.critical('Shutdown watchdog fired after %.0fs — forcing process exit (asyncio drain path appears wedged; see %s)', delay, target)
        except Exception:
            pass
...
async def loop_heartbeat_forever(*, interval_s: float=DEFAULT_HEARTBEAT_INTERVAL_S, start_time: Optional[float]=None, home: Optional[Path]=None, should_continue: Optional[Callable[[], bool]]=None) -> None:
