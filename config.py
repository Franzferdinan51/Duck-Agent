...
def _env_multiplex_profiles_override() -> 'bool | None':
    ...
    if token in _MULTIPLEX_FALSY_STRINGS:
        return False
    logger.warning('Ignoring unrecognized GATEWAY_MULTIPLEX_PROFILES=%r (expected one of %s or %s); falling back to config.yaml.', raw, sorted(_MULTIPLEX_TRUTHY_STRINGS), sorted(_MULTIPLEX_FALSY_STRINGS))
    return None
...
def _coerce_optional_positive_int(value: Any, key: str) -> Optional[int]:
    ...
    if parsed <= 0:
        return None
    return parsed
_SYSTEMD_WATCHDOG_MAX_SECONDS = 2147483647
        ...
        try:
            ...
            if platforms_dir.is_dir():
                for child in platforms_dir.iterdir():
                    if child.is_dir() and (child / '__init__.py').exists() and ((child / 'plugin.yaml').exists() or (child / 'plugin.yml').exists()):
                        names.add(child.name.lower())
        except Exception:
            pass
        return names
_BUILTIN_PLATFORM_VALUES = frozenset((m.value for m in Platform.__members__.values()))
PORT_BINDING_PLATFORM_VALUES = frozenset({'webhook', 'api_server', 'msgraph_webhook', 'feishu', 'wecom_callback', 'bluebubbles', 'sms', 'whatsapp_cloud', 'line'})
...
class HomeChannel:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'HomeChannel':
        return cls(platform=Platform(data['platform']), chat_id=str(data['chat_id']), name=data.get('name', 'Home'), thread_id=str(data['thread_id']) if data.get('thread_id') else None, user_id=str(data['user_id']) if data.get('user_id') else None, scope_id=str(data['scope_id']) if data.get('scope_id') else None)
...
class SessionResetPolicy:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'mode': self.mode, 'at_hour': self.at_hour, 'idle_minutes': self.idle_minutes, 'notify': self.notify, 'notify_exclude_platforms': list(self.notify_exclude_platforms), 'bg_process_max_age_hours': self.bg_process_max_age_hours}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SessionResetPolicy':
        ...
        return cls(mode=mode if mode is not None else 'none', at_hour=at_hour if at_hour is not None else 4, idle_minutes=idle_minutes if idle_minutes is not None else 1440, notify=_coerce_bool(notify, True), notify_exclude_platforms=tuple(exclude) if exclude is not None else ('api_server', 'webhook'), bg_process_max_age_hours=bg_max_age if bg_max_age is not None else 24)
...
PLATFORM_TOKEN_ENV_NAMES: dict['Platform', str] = {Platform.TELEGRAM: 'TELEGRAM_BOT_TOKEN', Platform.DISCORD: 'DISCORD_BOT_TOKEN', Platform.SLACK: 'SLACK_BOT_TOKEN', Platform.MATTERMOST: 'MATTERMOST_TOKEN', Platform.MATRIX: 'MATRIX_ACCESS_TOKEN', Platform.WEIXIN: 'WEIXIN_TOKEN'}
...
class PlatformConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {'enabled': self.enabled, 'extra': self.extra, 'reply_to_mode': self.reply_to_mode, 'gateway_restart_notification': self.gateway_restart_notification, 'typing_indicator': self.typing_indicator}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'PlatformConfig':
        ...
        return cls(enabled=_coerce_bool(data.get('enabled'), False), token=data.get('token'), api_key=data.get('api_key'), home_channel=home_channel, reply_to_mode=data.get('reply_to_mode', 'first'), gateway_restart_notification=_coerce_bool(_grn, True), typing_indicator=_coerce_bool(_typing, True), typing_status_text=_typing_text, channel_overrides=channel_overrides, extra=extra)
...
class StreamingConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'enabled': self.enabled, 'transport': self.transport, 'edit_interval': self.edit_interval, 'buffer_threshold': self.buffer_threshold, 'cursor': self.cursor, 'fresh_final_after_seconds': self.fresh_final_after_seconds}
...
class GatewayConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'platforms': {p.value: c.to_dict() for p, c in self.platforms.items()}, 'default_reset_policy': self.default_reset_policy.to_dict(), 'reset_by_type': {k: v.to_dict() for k, v in self.reset_by_type.items()}, 'reset_by_platform': {p.value: v.to_dict() for p, v in self.reset_by_platform.items()}, 'reset_triggers': self.reset_triggers, 'quick_commands': self.quick_commands, 'sessions_dir': str(self.sessions_dir), 'write_sessions_json': self.write_sessions_json, 'always_log_local': self.always_log_local, 'filter_silence_narration': self.filter_silence_narration, 'stt_enabled': self.stt_enabled, 'stt_echo_transcripts': self.stt_echo_transcripts, 'group_sessions_per_user': self.group_sessions_per_user, 'thread_sessions_per_user': self.thread_sessions_per_user, 'max_concurrent_sessions': self.max_concurrent_sessions, 'multiplex_profiles': self.multiplex_profiles, 'systemd_watchdog_seconds': self.systemd_watchdog_seconds, 'loop_watchdog': self.loop_watchdog, 'unauthorized_dm_behavior': self.unauthorized_dm_behavior, 'streaming': self.streaming.to_dict(), 'session_store_max_age_days': self.session_store_max_age_days, 'profile_routes': [asdict(r) if is_dataclass(r) and (not isinstance(r, type)) else r for r in self.profile_routes]}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'GatewayConfig':
        ...
        systemd_watchdog_seconds = coerce_systemd_watchdog_seconds(systemd_watchdog_raw, systemd_watchdog_key)
        ...
        max_concurrent_sessions = _coerce_optional_positive_int(max_concurrent_raw, max_concurrent_key)
        unauthorized_dm_behavior = _normalize_unauthorized_dm_behavior(data.get('unauthorized_dm_behavior'), 'pair')
        try:
    ...
    def get_unauthorized_dm_behavior(self, platform: Optional[Platform]=None) -> str:
        ...
        if platform:
            ...
            if platform_cfg and 'unauthorized_dm_behavior' in platform_cfg.extra:
                return _normalize_unauthorized_dm_behavior(platform_cfg.extra.get('unauthorized_dm_behavior'), self.unauthorized_dm_behavior)
    ...
    def get_notice_delivery(self, platform: Optional[Platform]=None) -> str:
        ...
        if platform:
            ...
            if platform_cfg and 'notice_delivery' in platform_cfg.extra:
                return _normalize_notice_delivery(platform_cfg.extra.get('notice_delivery'), 'public')
        return 'public'
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.duck-agent/config.yaml (primary user-facing config)
    3. ~/.duck-agent/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    if gateway_json_path.exists():
        try:
            ...
            logger.info('Loaded legacy %s — consider moving settings to config.yaml', gateway_json_path)
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if isinstance(gateway_section, dict):
                ...
                if 'systemd_watchdog_seconds' in gateway_section:
                    gw_data['systemd_watchdog_seconds'] = gateway_section['systemd_watchdog_seconds']
            ...
            if 'filter_silence_narration' in yaml_cfg:
                gw_data['filter_silence_narration'] = yaml_cfg['filter_silence_narration']
            elif isinstance(gateway_section, dict) and 'filter_silence_narration' in gateway_section:
                gw_data['filter_silence_narration'] = gateway_section['filter_silence_narration']
            if 'unauthorized_dm_behavior' in yaml_cfg:
                ...
                gw_data['unauthorized_dm_behavior'] = _normalize_unauthorized_dm_behavior(gateway_section.get('unauthorized_dm_behavior'), 'pair')
            ...
            for plat in _shared_loop_targets:
                ...
                if 'unauthorized_dm_behavior' in platform_cfg:
                    bridged['unauthorized_dm_behavior'] = _normalize_unauthorized_dm_behavior(platform_cfg.get('unauthorized_dm_behavior'), gw_data.get('unauthorized_dm_behavior', 'pair'))
                if 'notice_delivery' in platform_cfg:
                    bridged['notice_delivery'] = _normalize_notice_delivery(platform_cfg.get('notice_delivery'), 'public')
                ...
                if has_channel_overrides:
                    ...
                    if isinstance(raw_overrides, dict):
                        plat_data, _extra = _ensure_platform_extra_dict(platforms_data, plat.value)
                        plat_data['channel_overrides'] = {str(cid): ov_data for cid, ov_data in raw_overrides.items() if isinstance(ov_data, dict)}
        ...
        logger.warning('Failed to process config.yaml — falling back to .env / gateway.json values. Check %s for syntax errors. Error: %s', _home / 'config.yaml', e)
    ...
    if has_usable_secret is not None:
        for platform, pconfig in config.platforms.items():
            ...
            if token and token.strip() and (not has_usable_secret(token, min_length=4)):
                logger.error("%s is enabled but %s is set to a placeholder value ('%s'). Set a real bot token before starting the gateway. The adapter will NOT be started.", platform.value, env_name, token.strip()[:6] + '...')
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if telegram_fallback_ips:
        if Platform.TELEGRAM not in config.platforms:
            config.platforms[Platform.TELEGRAM] = PlatformConfig()
        config.platforms[Platform.TELEGRAM].extra['fallback_ips'] = [ip.strip() for ip in telegram_fallback_ips.split(',') if ip.strip()]
    telegram_home = getenv('TELEGRAM_HOME_CHANNEL')
    if telegram_home and Platform.TELEGRAM in config.platforms:
        config.platforms[Platform.TELEGRAM].home_channel = HomeChannel(platform=Platform.TELEGRAM, chat_id=telegram_home, name=getenv('TELEGRAM_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('TELEGRAM_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if discord_home and Platform.DISCORD in config.platforms:
        config.platforms[Platform.DISCORD].home_channel = HomeChannel(platform=Platform.DISCORD, chat_id=discord_home, name=getenv('DISCORD_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('DISCORD_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if whatsapp_home and Platform.WHATSAPP in config.platforms:
        config.platforms[Platform.WHATSAPP].home_channel = HomeChannel(platform=Platform.WHATSAPP, chat_id=whatsapp_home, name=getenv('WHATSAPP_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('WHATSAPP_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if whatsapp_cloud_phone_id and whatsapp_cloud_token:
        ...
        config.platforms[Platform.WHATSAPP_CLOUD].extra.update({'phone_number_id': whatsapp_cloud_phone_id, 'access_token': whatsapp_cloud_token})
    ...
    if whatsapp_cloud_home and Platform.WHATSAPP_CLOUD in config.platforms:
        config.platforms[Platform.WHATSAPP_CLOUD].home_channel = HomeChannel(platform=Platform.WHATSAPP_CLOUD, chat_id=whatsapp_cloud_home, name=getenv('WHATSAPP_CLOUD_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('WHATSAPP_CLOUD_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if slack_home:
        ...
        slack_config.home_channel = HomeChannel(platform=Platform.SLACK, chat_id=slack_home, name=getenv('SLACK_HOME_CHANNEL_NAME', ''), thread_id=getenv('SLACK_HOME_CHANNEL_THREAD_ID') or None, user_id=existing_home.user_id if existing_home and same_home else None, scope_id=existing_home.scope_id if existing_home and same_home else None)
    ...
    if signal_url and signal_account:
        ...
        signal_config.extra.update({'http_url': signal_url, 'account': signal_account, 'ignore_stories': is_truthy_value(getenv('SIGNAL_IGNORE_STORIES', 'true'))})
    signal_home = getenv('SIGNAL_HOME_CHANNEL')
    if signal_home and Platform.SIGNAL in config.platforms:
        config.platforms[Platform.SIGNAL].home_channel = HomeChannel(platform=Platform.SIGNAL, chat_id=signal_home, name=getenv('SIGNAL_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('SIGNAL_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if mattermost_home and Platform.MATTERMOST in config.platforms:
        config.platforms[Platform.MATTERMOST].home_channel = HomeChannel(platform=Platform.MATTERMOST, chat_id=mattermost_home, name=getenv('MATTERMOST_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('MATTERMOST_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if matrix_token or getenv('MATRIX_PASSWORD'):
        ...
        matrix_e2ee = matrix_e2ee_mode in ('required', 'require', 'optional', 'prefer', 'preferred') or is_truthy_value(getenv('MATRIX_ENCRYPTION', ''))
    ...
    if matrix_home and Platform.MATRIX in config.platforms:
        config.platforms[Platform.MATRIX].home_channel = HomeChannel(platform=Platform.MATRIX, chat_id=matrix_home, name=getenv('MATRIX_HOME_ROOM_NAME', 'Home'), thread_id=getenv('MATRIX_HOME_ROOM_THREAD_ID') or None)
    ...
    if all([email_addr, email_pwd, email_imap, email_smtp]):
        ...
        config.platforms[Platform.EMAIL].extra.update({'address': email_addr, 'imap_host': email_imap, 'smtp_host': email_smtp})
    email_home = getenv('EMAIL_HOME_ADDRESS')
    if email_home and Platform.EMAIL in config.platforms:
        config.platforms[Platform.EMAIL].home_channel = HomeChannel(platform=Platform.EMAIL, chat_id=email_home, name=getenv('EMAIL_HOME_ADDRESS_NAME', 'Home'), thread_id=getenv('EMAIL_HOME_ADDRESS_THREAD_ID') or None)
    ...
    if sms_home and Platform.SMS in config.platforms:
        config.platforms[Platform.SMS].home_channel = HomeChannel(platform=Platform.SMS, chat_id=sms_home, name=getenv('SMS_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('SMS_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if msgraph_webhook_enabled or Platform.MSGRAPH_WEBHOOK in config.platforms or msgraph_webhook_port or msgraph_webhook_client_state or msgraph_webhook_resources or msgraph_webhook_allowed_cidrs:
        ...
        if msgraph_webhook_resources:
            resources = [resource.strip() for resource in msgraph_webhook_resources.split(',') if resource.strip()]
            if resources:
                config.platforms[Platform.MSGRAPH_WEBHOOK].extra['accepted_resources'] = resources
        if msgraph_webhook_allowed_cidrs:
            cidrs = [cidr.strip() for cidr in msgraph_webhook_allowed_cidrs.split(',') if cidr.strip()]
            if cidrs:
                config.platforms[Platform.MSGRAPH_WEBHOOK].extra['allowed_source_cidrs'] = cidrs
    ...
    if dingtalk_client_id and dingtalk_client_secret:
        ...
        config.platforms[Platform.DINGTALK].extra.update({'client_id': dingtalk_client_id, 'client_secret': dingtalk_client_secret})
        dingtalk_home = getenv('DINGTALK_HOME_CHANNEL')
        if dingtalk_home:
            config.platforms[Platform.DINGTALK].home_channel = HomeChannel(platform=Platform.DINGTALK, chat_id=dingtalk_home, name=getenv('DINGTALK_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('DINGTALK_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if feishu_app_id and feishu_app_secret:
        ...
        config.platforms[Platform.FEISHU].extra.update({'app_id': feishu_app_id, 'app_secret': feishu_app_secret, 'domain': getenv('FEISHU_DOMAIN', 'feishu'), 'connection_mode': getenv('FEISHU_CONNECTION_MODE', 'websocket')})
        ...
        if feishu_home:
            config.platforms[Platform.FEISHU].home_channel = HomeChannel(platform=Platform.FEISHU, chat_id=feishu_home, name=getenv('FEISHU_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('FEISHU_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if wecom_bot_id and wecom_secret:
        ...
        config.platforms[Platform.WECOM].extra.update({'bot_id': wecom_bot_id, 'secret': wecom_secret})
        ...
        if wecom_home:
            config.platforms[Platform.WECOM].home_channel = HomeChannel(platform=Platform.WECOM, chat_id=wecom_home, name=getenv('WECOM_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('WECOM_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if wecom_callback_corp_id and wecom_callback_corp_secret:
        ...
        config.platforms[Platform.WECOM_CALLBACK].extra.update({'corp_id': wecom_callback_corp_id, 'corp_secret': wecom_callback_corp_secret, 'agent_id': getenv('WECOM_CALLBACK_AGENT_ID', ''), 'token': getenv('WECOM_CALLBACK_TOKEN', ''), 'encoding_aes_key': getenv('WECOM_CALLBACK_ENCODING_AES_KEY', ''), 'host': getenv('WECOM_CALLBACK_HOST', ''), 'port': getenv_int('WECOM_CALLBACK_PORT', 8645)})
    ...
    if weixin_token or weixin_account_id:
        ...
        if weixin_home:
            config.platforms[Platform.WEIXIN].home_channel = HomeChannel(platform=Platform.WEIXIN, chat_id=weixin_home, name=getenv('WEIXIN_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('WEIXIN_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if bluebubbles_server_url and bluebubbles_password:
        ...
        config.platforms[Platform.BLUEBUBBLES].extra.update({'server_url': bluebubbles_server_url.rstrip('/'), 'password': bluebubbles_password, 'webhook_host': getenv('BLUEBUBBLES_WEBHOOK_HOST', '127.0.0.1'), 'webhook_port': getenv_int('BLUEBUBBLES_WEBHOOK_PORT', 8645), 'webhook_path': getenv('BLUEBUBBLES_WEBHOOK_PATH', '/bluebubbles-webhook'), 'send_read_receipts': is_truthy_value(getenv('BLUEBUBBLES_SEND_READ_RECEIPTS', 'true'))})
        bluebubbles_require_mention = getenv('BLUEBUBBLES_REQUIRE_MENTION')
        if bluebubbles_require_mention is not None:
            config.platforms[Platform.BLUEBUBBLES].extra['require_mention'] = bluebubbles_require_mention.lower() in {'true', '1', 'yes', 'on'}
        ...
        if bluebubbles_mention_patterns:
            try:
                ...
                parsed_patterns = [part.strip() for part in bluebubbles_mention_patterns.replace('\n', ',').split(',') if part.strip()]
    ...
    if bluebubbles_home and Platform.BLUEBUBBLES in config.platforms:
        config.platforms[Platform.BLUEBUBBLES].home_channel = HomeChannel(platform=Platform.BLUEBUBBLES, chat_id=bluebubbles_home, name=getenv('BLUEBUBBLES_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('BLUEBUBBLES_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if qq_app_id or qq_client_secret:
        ...
        if not qq_home:
            ...
            if legacy_home:
                ...
                logging.getLogger(__name__).warning('QQ_HOME_CHANNEL is deprecated; rename to QQBOT_HOME_CHANNEL in your .env for consistency with the platform key.')
        if qq_home:
            config.platforms[Platform.QQBOT].home_channel = HomeChannel(platform=Platform.QQBOT, chat_id=qq_home, name=getenv('QQBOT_HOME_CHANNEL_NAME') or getenv(qq_home_name_env, 'Home'), thread_id=getenv('QQBOT_HOME_CHANNEL_THREAD_ID') or getenv('QQ_HOME_CHANNEL_THREAD_ID') or None)
    ...
    if yuanbao_app_id and yuanbao_app_secret:
        ...
        if yuanbao_home:
            config.platforms[Platform.YUANBAO].home_channel = HomeChannel(platform=Platform.YUANBAO, chat_id=yuanbao_home, name=getenv('YUANBAO_HOME_CHANNEL_NAME', 'Home'), thread_id=getenv('YUANBAO_HOME_CHANNEL_THREAD_ID') or None)
    ...
    try:
        ...
        for entry in platform_registry.plugin_entries():
            ...
            if existing_cfg is not None and (not existing_cfg.enabled) and bool((existing_cfg.extra or {}).get('_enabled_explicit', False)):
                continue
            seed_for_probe = None
                    ...
                    try:
                        if existing_cfg is not None:
                            ...
                            if not probe_cfg.enabled:
                                probe_cfg = PlatformConfig(enabled=True, extra=dict(probe_cfg.extra or {}))
                        else:
                        ...
                        configured = False
                    if not configured:
                        logger.debug("Plugin platform '%s' available but not configured (is_connected returned False) — skipping enable", entry.name)
                        continue
            try:
            ...
            if isinstance(seed_for_probe, dict) and seed_for_probe:
                ...
                if isinstance(home, dict) and home.get('chat_id'):
                    config.platforms[platform].home_channel = HomeChannel(platform=platform, chat_id=str(home['chat_id']), name=str(home.get('name') or 'Home'), thread_id=str(home['thread_id']) if home.get('thread_id') else None)
    except Exception as e:
