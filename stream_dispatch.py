...
from gateway.stream_events import Commentary, GatewayNotice, LongToolHint, MessageChunk, MessageStop, StreamEvent, ToolCallChunk, ToolCallFinished
...
class GatewayEventDispatcher:
    ...
    def __init__(self, adapter: Any, sink: Any=None, *, enqueue_tool_line: Optional[Callable[[Any], None]]=None, tool_mode: str='all', preview_max_len: int=40, on_long_tool: Optional[Callable[[LongToolHint], None]]=None, on_notice: Optional[Callable[[GatewayNotice], None]]=None) -> None:
    ...
    def _dispatch(self, event: StreamEvent) -> None:
        ...
        if isinstance(event, ToolCallChunk):
            ...
            line = self.adapter.format_tool_event(event, mode=self.tool_mode, preview_max_len=self.preview_max_len)
            if line:
