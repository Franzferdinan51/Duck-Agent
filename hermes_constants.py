"""Shared constants for Duck Agent.
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Duck Agent home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Duck Agent home override."""
    _HERMES_HOME_OVERRIDE.reset(token)

def get_hermes_home_override() -> str | None:
    """Return the active context-local Duck Agent home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Duck Agent home path."""
    if sys.platform == 'win32':
        local_appdata = os.environ.get('LOCALAPPDATA', '').strip()
        base = Path(local_appdata) if local_appdata else Path.home() / 'AppData' / 'Local'
        return base / 'duck-agent'
    return Path.home() / '.duck-agent'

def _hermes_home_from_env() -> Path:
    """Resolve DUCK_AGENT_HOME from the process environment only.

    Reads the ``DUCK_AGENT_HOME`` env var, falling back to the platform-native
    ...
    """
    val = os.environ.get('DUCK_AGENT_HOME', '').strip()
    if val:
        return Path(val)
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``DUCK_AGENT_HOME`` is
    ...
    if active and active != 'default':
        ...
        msg = f'[DUCK_AGENT_HOME fallback] DUCK_AGENT_HOME is unset but active profile is {active!r}. Falling back to {fallback_home}, which is the DEFAULT profile — not {active!r}. Any data this process writes will land in the wrong profile. The subprocess spawner should pass DUCK_AGENT_HOME explicitly (see issue #18594).'
        try:
...
def get_hermes_home() -> Path:
    """Return the Duck Agent home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_hermes_home_override`) → ``DUCK_AGENT_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``DUCK_AGENT_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``DUCK_AGENT_HOME`` explicitly (see the systemd
    template in ``hermes_cli/gateway.py`` and the kanban dispatcher in
    ``hermes_cli/kanban_db.py``).  See https://github.com/NousResearch/duck-agent/issues/18594.
    """
    override = get_hermes_home_override()
    if override:
        return Path(override)
    if not os.environ.get('DUCK_AGENT_HOME', '').strip():
        _warn_profile_fallback_once()
    return _hermes_home_from_env()

def get_process_hermes_home() -> Path:
    """Return the Duck Agent home for the running process, ignoring task overrides.
    ...
    process ``DUCK_AGENT_HOME`` env var (falling back to the platform default),
...
def get_default_hermes_root() -> Path:
    """Return the root Duck Agent directory for profile-level operations.

    In standard deployments this is the platform-native Duck Agent home
    (``~/.duck-agent`` on POSIX, ``%LOCALAPPDATA%\\duck-agent`` on native Windows).

    In Docker or custom deployments where ``DUCK_AGENT_HOME`` points outside
    ``~/.duck-agent`` (e.g. ``/opt/data``), returns ``DUCK_AGENT_HOME`` directly
    — that IS the root.

    In profile mode where ``DUCK_AGENT_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.duck-agent/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_hermes_home()
    env_home = os.environ.get('DUCK_AGENT_HOME', '')
    if not env_home:
        return native_home
...
def get_bundled_skills_dir(default: Path | None=None) -> Path:
    ...
    Resolution order:
        ...
        3. ``<DUCK_AGENT_HOME>/skills`` last-resort
    """
...
def get_hermes_dir(new_subpath: str, old_name: str, *, home: Path | None=None) -> Path:
    """Resolve a Duck Agent subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to DUCK_AGENT_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to DUCK_AGENT_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Duck Agent home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local DUCK_AGENT_HOME.
...
def iter_hermes_node_dirs(home: Path | None=None) -> list[Path]:
    """Return Duck Agent-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\duck-agent\\node``. POSIX installs use
    ``$DUCK_AGENT_HOME/node/bin``. Include both shapes on every platform so mixed
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Duck Agent-managed Node trees live under ``$DUCK_AGENT_HOME/node`` (or a profile's
    ``DUCK_AGENT_HOME``). A partial upgrade or interrupted install can leave
    ...
    used to trust file presence alone, so ``duck-agent update`` would pick that
    ...
    try:
        ...
        result = subprocess.run([path, '--version'], capture_output=True, timeout=10, env=with_hermes_node_path(), creationflags=windows_hide_flags())
    except (OSError, subprocess.TimeoutExpired, ValueError):
        return False
    return result.returncode == 0

def hermes_managed_node_tree_present(home: Path | None=None) -> bool:
    """Return True when any Duck Agent-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%DUCK_AGENT_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    try:
        ...
        return False
    match = re.search(f'node-v{_HERMES_NODE_TARGET_MAJOR}\\.\\d+\\.\\d+-win-{node_arch}\\.zip', index_html)
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$DUCK_AGENT_HOME/node`` on POSIX.
    ...
    import subprocess
    try:
        result = subprocess.run(['bash', '-c', f'source "{_NODE_BOOTSTRAP_SCRIPT}" && _nb_install_bundled_node'], env={**os.environ, 'DUCK_AGENT_HOME': str(get_hermes_home()), 'HERMES_NODE_SKIP_LINKS': '1'}, capture_output=True, timeout=600, check=False)
    except (OSError, subprocess.SubprocessError):
        return False
    return result.returncode == 0

def bootstrap_hermes_managed_node() -> str | None:
    """Install a Duck Agent-managed Node tree and return its npm path.
    ...
    Duck Agent never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$DUCK_AGENT_HOME/node`` (the same tree a fresh install
...
def heal_hermes_managed_node() -> bool:
    """Redownload Duck Agent-managed Node when the tree exists but is broken.
    ...
    import subprocess
    try:
        result = subprocess.run(['bash', '-c', f'source "{_NODE_BOOTSTRAP_SCRIPT}" && heal_managed_node'], env={**os.environ, 'DUCK_AGENT_HOME': str(get_hermes_home())}, capture_output=True, timeout=300, check=False)
    except (OSError, subprocess.SubprocessError):
        return False
    return result.returncode == 0
...
def _managed_node_tree_outdated(home: Path | None=None) -> bool:
    ...
    for directory in iter_hermes_node_dirs(home):
        for name in _candidate_node_command_names('node'):
            ...
            try:
                from hermes_cli._subprocess_compat import windows_hide_flags
                result = subprocess.run([str(candidate), '--version'], capture_output=True, timeout=10, creationflags=windows_hide_flags())
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Duck Agent-managed Node/npm executable path, healing broken trees.
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Duck Agent-owned
    subprocesses.
    """
    if sys.platform != 'win32':
        return shutil.which(command)
    command_str = str(command)
    has_path_separator = any((sep and sep in command_str for sep in (os.sep, os.altsep, '/', '\\')))
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Duck Agent-managed installs.

    This is for Duck Agent-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None=None) -> dict[str, str]:
    """Return *env* with Duck Agent-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``duck-agent update`` — leaving a **dangling symlink** that ``which`` still
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current DUCK_AGENT_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.duck-agent``
        profile:  ``~/.duck-agent/profiles/coder``
        custom:   ``/opt/duck-agent-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.duck-agent``.  For code that needs a real ``Path``, use
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``DUCK_AGENT_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/duck-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None=None) -> str | None:
    """Return ``{DUCK_AGENT_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get('DUCK_AGENT_HOME') or os.getenv('DUCK_AGENT_HOME')
    if not hermes_home:
        return None
...
def get_real_home(env: dict[str, str] | None=None) -> str:
    """Return the OS user's real home directory, avoiding Duck Agent profile HOME.

    ``DUCK_AGENT_HOME`` scopes Duck Agent state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={DUCK_AGENT_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None=None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{DUCK_AGENT_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{DUCK_AGENT_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Duck Agent' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def _canonical_model_variants(model: str) -> list[str]:
    ...
    """
    import re
    _dash_to_dot = lambda s: re.sub('(\\d)-(\\d)', '\\1.\\2', s)
    _dot_to_dash = lambda s: re.sub('(\\d)\\.(\\d)', '\\1-\\2', s)
    seen = set()
    variants = []
    ...
    known_providers = ('anthropic', 'openai', 'google', 'openrouter', 'groq', 'mistral', 'xai', 'cohere', 'perplexity', 'together', 'fireworks', 'deepseek')
...
def windows_path_to_wsl(path: str) -> str | None:
    ...
    import re
    match = re.match('^([A-Za-z]):[\\\\/](.*)$', str(path or '').strip())
    if not match:
        return None
...
def wsl_unc_path_to_posix(path: str) -> str | None:
    ...
    match = re.match('^\\\\\\\\wsl(?:\\.localhost|\\$)\\\\[^\\\\]+\\\\(.*)$', normalized, re.IGNORECASE)
    if not match:
        return None
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Duck Agent itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/duck-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under DUCK_AGENT_HOME.
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under DUCK_AGENT_HOME."""
    return get_hermes_home() / 'skills'

def get_env_path() -> Path:
    """Return the path to the ``.env`` file under DUCK_AGENT_HOME."""
...
FIRST_PARTY_MODULE_ROOTS = frozenset({'agent', 'acp_adapter', 'cli', 'cron', 'gateway', 'model_tools', 'plugins', 'providers', 'tools', 'toolsets', 'run_agent', 'tui_gateway', 'utils'})

def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Duck Agent.
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `duck-agent update` is exactly
    ...
    if not is_first_party_module(name):
        return []
    return ['', 'This looks like a partially-updated install: one module was refreshed and a related one was not.', 'Re-run the update to bring the whole tree to the same version:', '    duck-agent update', 'If that also fails, reinstall: https://duck-agent.nousresearch.com']