...
`duck-agent config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.duck-agent/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `DUCK_AGENT_HOME`, secret scope, and
