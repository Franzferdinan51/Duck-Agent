...
This document is the formal interface between the **Duck Agent gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `duck-agent gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`duck-agent gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `duck-agent` CLI.
