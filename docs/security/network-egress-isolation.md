...
When running Duck Agent inside Docker, the default `network_mode: host` gives the
...
The Duck Agent [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ duck-agent │   │ duck-agent-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=duck-agent,duck-agent-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://duck-agent-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Duck Agent trust model and vulnerability reporting
