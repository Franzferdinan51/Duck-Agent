...
Counts consecutive restarts via a JSON file (`{DUCK_AGENT_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `duck-agent update`, `duck-agent gateway restart`,
or `/restart`.
