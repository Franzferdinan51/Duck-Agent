...
Duck Agent includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Duck Agent and does not require `duck-agent plugins enable
observability/nemo_relay`. Duck Agent remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Duck Agent execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `duck-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Duck Agent requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Duck Agent installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Duck Agent process so configured interceptors can
...
Collection remains off unless Duck Agent policy enables it:
...
Duck Agent core owns one Relay host and one isolated Relay session scope per Duck Agent
...
marks do not require Duck Agent plugin registration. Shared-metrics marks must
...
```text
Duck Agent turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Duck Agent shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Duck Agent sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Duck Agent used for the logical call, such as
...
New calls use `duck-agent.model_route.count`. The previous
`duck-agent.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `duck-agent.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Duck Agent
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Duck Agent processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `duck-agent.task_run`, parented to
the owning Duck Agent session. The start counter contains only bounded execution
...
provider attempts for the same Duck Agent API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Duck Agent tool-call
...
cancellations. Active task ownership follows the task ID if Duck Agent rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`duck-agent.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Duck Agent
...
tool or plugin names. Duck Agent does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `duck-agent.tool_approval` marks and
...
Successful skill mutations emit `duck-agent.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `duck-agent.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Duck Agent derives reuse and patch-generation
...
```text
$DUCK_AGENT_HOME/telemetry/shared_metrics/metrics.sqlite3
$DUCK_AGENT_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Duck Agent version,
...
schema field name, its current scope is one `DUCK_AGENT_HOME`, so it is more
...
Deleting `$DUCK_AGENT_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `DUCK_AGENT_HOME`. To reset it, stop Duck Agent
processes and remove `$DUCK_AGENT_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Duck Agent CLI turn against the deterministic local model server:
