# Duck Agent Observer Hooks

Duck Agent observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Duck Agent' planner, model providers, memory, tool registry,
...
Duck Agent also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "duck-agent.observer.v1"
```

Hook callbacks are fail-open. Duck Agent catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Duck Agent only builds
