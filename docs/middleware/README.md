# Duck Agent Middleware

Duck Agent middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Duck Agent'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Duck Agent calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Duck Agent retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Duck Agent guardrails,
...
- `telemetry_schema_version`: currently `duck-agent.observer.v1`
- `middleware_schema_version`: currently `duck-agent.middleware.v1`
...
Duck Agent stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Duck Agent runs
them as a nested chain in registration order. Middleware failures are fail-open:
Duck Agent logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Duck Agent applies middleware in this order:
...
For each tool call, Duck Agent applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Duck Agent pre-execution path against the effective arguments:
...
```bash
duck-agent plugins enable <plugin-name>
```

For isolated local testing, use one `DUCK_AGENT_HOME` for plugin enablement and the
agent run:

```bash
export DUCK_AGENT_HOME=/tmp/duck-agent-middleware-test
mkdir -p "$DUCK_AGENT_HOME"
duck-agent plugins enable <plugin-name>
duck-agent chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run duck-agent plugins enable <plugin-name>
uv run duck-agent chat --query 'Reply exactly ok'
```
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/duck-agent-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Duck Agent expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Duck Agent treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Duck Agent preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Duck Agent does not convert downstream
