...
has to be thrown away or summarized. Duck Agent has always done this in one batch:
...
Duck Agent folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Duck Agent'ss normal session flush is
...
With `micro_compact` unset or `false` Duck Agent behaves exactly as it always has:
...
Defaults to `$DUCK_AGENT_HOME/logs/agent.log`. It reports passes, outcome mix, net
