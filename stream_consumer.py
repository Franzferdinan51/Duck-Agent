...
from gateway.config import DEFAULT_STREAMING_EDIT_INTERVAL as _DEFAULT_STREAMING_EDIT_INTERVAL, DEFAULT_STREAMING_BUFFER_THRESHOLD as _DEFAULT_STREAMING_BUFFER_THRESHOLD, DEFAULT_STREAMING_CURSOR as _DEFAULT_STREAMING_CURSOR
from gateway.response_filters import is_intentional_silence_response as _is_intentional_silence_response, is_partial_silence_marker as _is_partial_silence_marker
...
def ensure_closed_code_fences(text: str) -> str:
    ...
    if text.count('```') % 2 == 1:
        text = text.rstrip('\n') + '\n```'
    import re
    without_fences = re.sub('```.*?```', '', text, flags=re.DOTALL)
    without_fences = re.sub('```[^`]*$', '', without_fences)
...
class GatewayStreamConsumer:
    ...
    _OPEN_THINK_TAGS = ('<REASONING_SCRATCHPAD>', '<think>', '<reasoning>', '<THINKING>', '<thinking>', '<thought>')
    _CLOSE_THINK_TAGS = ('</REASONING_SCRATCHPAD>', '</think>', '</reasoning>', '</THINKING>', '</thinking>', '</thought>')
    _draft_id_counter: int = 0

    def __init__(self, adapter: Any, chat_id: str, config: Optional[StreamConsumerConfig]=None, metadata: Optional[dict]=None, on_new_message: Optional[callable]=None, on_before_finalize: Optional[Callable[[], Any]]=None, initial_reply_to_id: Optional[str]=None, run_still_current: Optional[Callable[[], bool]]=None):
        ...
        self._adapter_requires_finalize: bool = getattr(adapter, 'REQUIRES_EDIT_FINALIZE', False) is True
    ...
    async def _edit_message(self, *, message_id: str, content: str, finalize: bool=False):
        ...
        kwargs = {'chat_id': self.chat_id, 'message_id': message_id, 'content': content}
        kwargs['finalize'] = finalize
        if self.metadata:
            try:
                params = inspect.signature(self.adapter.edit_message).parameters
                if 'metadata' in params or any((param.kind is inspect.Parameter.VAR_KEYWORD for param in params.values())):
    ...
    def _record_turn_final_payload(self, text: str) -> None:
        ...
        self._delivered_final_text = ensure_closed_code_fences(self._clean_for_display(source)).strip()
    ...
    def delivered_final_matches(self, final_text: str) -> Optional[bool]:
        ...
        """
        target = ensure_closed_code_fences(self._clean_for_display(final_text or '')).strip()
        if not target:
            return None
    ...
    def has_delivered_text(self, text: str) -> bool:
        ...
        if visible_prefix == target:
            return True
        return any((sent.strip() == target for sent in (*self._delivered_commentary_texts, *self._delivered_segment_texts)))
    ...
    async def run(self) -> None:
        ...
        _len_fn: 'Callable[[str], int]' = self.adapter.message_len_fn_for_chat(self.chat_id) if isinstance(self.adapter, _BasePlatformAdapter) else len
        ...
        if self._use_draft_streaming:
            type(self)._draft_id_counter += 1
            self._draft_id = type(self)._draft_id_counter
            logger.debug('Stream consumer using native-draft transport (chat=%s draft_id=%s)', self.chat_id, self._draft_id)
        try:
            while True:
                ...
                should_edit = got_done or got_segment_break or commentary_text is not None
                if not self.cfg.buffer_only:
                    should_edit = should_edit or (elapsed >= self._current_edit_interval and self._accumulated or len(self._accumulated) >= self.cfg.buffer_threshold)
                ...
                if should_edit and self._accumulated:
                    if _len_fn(self._accumulated) > _safe_limit and self._message_id is None:
                        chunks = self._truncate_for_stream(self._accumulated, _safe_limit, _len_fn)
                        if len(chunks) <= 1:
                            chunks = self._split_text_chunks(self._accumulated, _safe_limit, _len_fn)
                        chunks_delivered = False
                        reply_to = self._initial_reply_to_id
                        all_heads_delivered = len(chunks) > 1
                        for chunk in chunks[:-1]:
                            new_id = await self._send_new_chunk(chunk, reply_to, final=got_done)
                    ...
                    while _len_fn(self._accumulated) > _safe_limit and self._message_id is not None and self._edit_supported:
                        _cp_budget = _custom_unit_to_cp(self._accumulated, _safe_limit, _len_fn)
                    ...
                    current_update_visible = await self._send_or_edit(display_text, finalize=got_done or got_segment_break, is_turn_final=got_done)
                    self._last_edit_time = time.monotonic()
                if got_done:
                    ...
                    if self._accumulated:
                        if self._fallback_final_send:
                            ...
                            self._final_content_delivered = True
                            self._record_turn_final_payload(self._accumulated)
                        elif current_update_visible and (not self._adapter_requires_finalize or self._last_edit_overflowed):
                ...
                if commentary_text is not None:
                    ...
                    self._reset_segment_state()
                if got_segment_break:
                    if self._accumulated and (not current_update_visible) and self._message_id and (self._message_id != '__no_edit__'):
            ...
            if self._accumulated and self._message_id:
                try:
                    _best_effort_ok = bool(await self._send_or_edit(self._accumulated, finalize=True, is_turn_final=False))
                except Exception:
                    pass
    ...
    async def _send_new_chunk(self, text: str, reply_to_id: Optional[str], *, final: bool=False) -> Optional[str]:
        ...
        if not text.strip():
            return reply_to_id
        try:
            result = await self.adapter.send(chat_id=self.chat_id, content=text, reply_to=reply_to_id, metadata=self._metadata_for_send(final=final, expect_edits=True))
    ...
    @staticmethod
    def _split_text_chunks(text: str, limit: int, len_fn: 'Callable[[str], int]'=len) -> list[str]:
        ...
        return split_text_fence_aware(text, limit, len_fn, prefer_paragraphs=False, balance_fences=True)

    def _truncate_for_stream(self, text: str, limit: int, len_fn: 'Callable[[str], int]') -> list[str]:
        ...
        if not isinstance(chunks, (list, tuple)) or not all((isinstance(chunk, str) for chunk in chunks)):
            return self._split_text_chunks(text, limit, len_fn)
        return list(chunks)
    ...
    async def _send_fallback_final(self, text: str) -> None:
        ...
        if not continuation.strip():
            if final_text.strip() and final_text == self._visible_prefix() and (getattr(self.adapter, 'RESEND_FINAL_ON_EMPTY_STREAM_FALLBACK', False) is True):
            ...
            if final_text.strip() and final_text != self._visible_prefix():
                ...
                if self._message_id and self._last_sent_text and self.cfg.cursor and self._last_sent_text.endswith(self.cfg.cursor):
        ...
        _len_fn: 'Callable[[str], int]' = self.adapter.message_len_fn if isinstance(self.adapter, _BasePlatformAdapter) else len
        if isinstance(self.adapter, _BasePlatformAdapter):
            try:
        ...
        stale_message_id = self._message_id
        ...
        for chunk in chunks:
            ...
            for attempt in range(2):
                result = await self.adapter.send(chat_id=self.chat_id, content=chunk, metadata=self._metadata_for_send(final=True))
                if result.success:
                    break
                retry_delay = self._fallback_flood_retry_delay(result)
                if attempt == 0 and retry_delay is not None:
                    logger.debug('Flood control on fallback send, retrying in %.1fs', retry_delay)
            ...
            self._notify_new_message()
        if stale_message_id and stale_message_id != last_message_id and (not self._fallback_preserve_partial_messages) and (continuation == final_text):
            delete_fn = getattr(self.adapter, 'delete_message', None)
            if delete_fn is not None:
                try:
                    await delete_fn(self.chat_id, stale_message_id)
                except Exception as e:
                    logger.debug('Fallback partial cleanup failed (%s): %s', stale_message_id, e)
        self._message_id = last_message_id
        self._already_sent = True
        self._final_response_sent = True
        self._final_content_delivered = True
    ...
    async def _send_empty_fallback_final(self, final_text: str) -> str:
        ...
        for attempt in range(2):
            try:
                result = await self.adapter.send(chat_id=self.chat_id, content=final_text, metadata=self._metadata_for_send(final=True))
            ...
            retry_delay = self._fallback_flood_retry_delay(result)
            if attempt == 0 and retry_delay is not None:
                logger.debug('Flood control on empty fallback final send; retrying in %.1fs', retry_delay)
                await asyncio.sleep(retry_delay)
                continue
        ...
        if delete_fn is not None:
            for stale_id in stale_ids:
                ...
                try:
                    ...
                    logger.debug('Empty fallback preview cleanup failed (%s): %s', stale_id, exc)
        self._segment_preview_message_ids = set()
        self._message_id = new_message_id or '__no_edit__'
        self._already_sent = True
        self._final_response_sent = True
        self._final_content_delivered = True
        self._delivered_final_text = ensure_closed_code_fences(self._clean_for_display(final_text or '')).strip()
    ...
    def _fallback_flood_retry_delay(self, result: Any) -> float | None:
        ...
        if delay > self._max_fallback_flood_retry_seconds:
            logger.debug('Flood control requests %.1fs; leaving final delivery to the gateway', delay)
            return None
    ...
    def _resolve_draft_streaming(self) -> bool:
        ...
        if not isinstance(self.adapter, _BasePlatformAdapter):
            return False
        try:
            supported = self.adapter.supports_draft_streaming(chat_type=self.cfg.chat_type or None, metadata=self.metadata)
        except Exception:
            logger.debug('supports_draft_streaming probe raised', exc_info=True)
            supported = False
        if not supported:
            if transport == 'draft':
                logger.debug('Draft streaming requested but unsupported (chat=%s, type=%r) — falling back to edit', self.chat_id, self.cfg.chat_type)
            return False
        return True
    ...
    async def _send_draft_frame(self, text: str) -> bool:
        ...
        if self._draft_id is None:
            ...
            return False
        try:
            result = await self.adapter.send_draft(chat_id=self.chat_id, draft_id=self._draft_id, content=text, metadata=self.metadata)
        except Exception as e:
        ...
        if not getattr(result, 'success', False):
            logger.debug('send_draft returned success=False, disabling draft transport: %s', getattr(result, 'error', 'unknown'))
            self._draft_failures += 1
            self._use_draft_streaming = False
            return False
        self._last_sent_text = text
    ...
    async def _flush_segment_tail_on_edit_failure(self) -> None:
        ...
        if not tail.strip():
            return
        try:
            result = await self.adapter.send(chat_id=self.chat_id, content=tail, metadata=self.metadata)
            if result.success:
                self._already_sent = True
        except Exception as e:
    ...
    async def _send_commentary(self, text: str) -> bool:
        ...
        if not text.strip():
            return False
        try:
            result = await self.adapter.send(chat_id=self.chat_id, content=text, metadata=self.metadata)
            if result.success:
                self._notify_new_message()
    ...
    def _adapter_prefers_fresh_final(self, text: str) -> bool:
        ...
        currently renders richer markdown than Duck Agent' MarkdownV2 edit path.
    ...
    async def _try_fresh_final(self, text: str, *, is_turn_final: bool=True) -> bool:
        ...
        try:
            result = await self.adapter.send(chat_id=self.chat_id, content=text, metadata=self._metadata_for_send(final=True))
        except Exception as e:
        ...
        if delete_fn is not None:
            for stale_id in stale_ids:
                ...
                try:
                    ...
                    logger.debug('Fresh-final preview cleanup failed (%s): %s', stale_id, e)
        ...
        if delete_fn is not None:
            for stale_id in stale_ids:
                ...
                try:
                    ...
                    logger.debug('Silence-marker preview cleanup failed (%s): %s', stale_id, e)
        self._preview_message_ids = set()
        self._message_id = None
        self._accumulated = ''
        self._stream_ledger = ''
    ...
    async def _send_or_edit(self, text: str, *, finalize: bool=False, is_turn_final: bool=True) -> bool:
        ...
        if self._message_id is None and self.cfg.cursor and (self.cfg.cursor in text) and (len(_visible_stripped) < _MIN_NEW_MSG_CHARS):
            return True
            ...
            if self._message_id is not None:
                if self._edit_supported:
                    if text == self._last_sent_text and (not (finalize and self._adapter_requires_finalize)):
                        return True
                    _has_prefers_hook = hasattr(type(self.adapter), 'prefers_fresh_final_streaming') or 'prefers_fresh_final_streaming' in getattr(self.adapter, '__dict__', {})
                    _prefers_fresh = self._adapter_prefers_fresh_final(text)
                    if finalize and (_prefers_fresh or (not _has_prefers_hook and self._should_send_fresh_final())) and await self._try_fresh_final(text, is_turn_final=is_turn_final):
                        return True
                    result = await self._edit_message(message_id=self._message_id, content=text, finalize=finalize)
                    if result.success:
                        self._already_sent = True
                        self._track_preview_ids_from_result(result)
                        _continuation_ids = getattr(result, 'continuation_message_ids', ()) or ()
                        if _continuation_ids and result.message_id and (result.message_id != self._message_id):
                        ...
                        if finalize and is_turn_final and self.cfg.cursor and self._last_sent_text.endswith(self.cfg.cursor) and (self._visible_prefix() == text):
                            self._final_content_delivered = True
                            self._record_turn_final_payload(text)
                        raw_response = getattr(result, 'raw_response', None)
                        if isinstance(raw_response, dict) and raw_response.get('partial_overflow'):
                            self._message_id = str(raw_response.get('last_message_id') or result.message_id or self._message_id)
                        ...
                        if self._is_flood_error(result):
                            ...
                            logger.debug('Flood control on edit (strike %d/%d), backoff interval → %.1fs', self._flood_strikes, self._MAX_FLOOD_STRIKES, self._current_edit_interval)
                            immediate_final_fallback = finalize and is_turn_final and (getattr(self.adapter, 'FALLBACK_ON_FINAL_EDIT_FLOOD', False) is True)
                ...
                result = await self.adapter.send(chat_id=self.chat_id, content=text, reply_to=self._initial_reply_to_id, metadata=self._metadata_for_send(final=finalize, expect_edits=True))
                if result.success:
