...
class StreamingTTSConsumer:
    ...
    def __init__(self, adapter: Any, chat_id: str, tts_config: Dict[str, Any], loop: asyncio.AbstractEventLoop, *, metadata: Optional[Dict[str, Any]]=None, audio_format: Optional[AudioFormat]=None) -> None:
        ...
        if self._streamer is not None:
            self._audio_format = AudioFormat(sample_rate=int(getattr(self._streamer, 'sample_rate', AudioFormat.sample_rate)), channels=int(getattr(self._streamer, 'channels', AudioFormat.channels)), sample_width=int(getattr(self._streamer, 'sample_width', AudioFormat.sample_width)))
        else:
    ...
    async def _run(self) -> None:
        ...
        if not self._adapter.supports_streaming_tts(self._chat_id, self._audio_format):
            ...
            return
        try:
            self._handle = await self._adapter.begin_streaming_tts(self._chat_id, self._audio_format, metadata=self._metadata)
