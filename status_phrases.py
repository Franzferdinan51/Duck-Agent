...
Duck Agent' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``DUCK_AGENT_HOME`` either by using
conventional paths::

    ~/.duck-agent/status_phrases.yaml
    ~/.duck-agent/status_phrases/*.yaml
...
or by pointing config at a relative file/directory::
    ...
    display:
      status_phrases:
        path: status_phrases/whatsapp.yaml  # relative to DUCK_AGENT_HOME
...
_FALLBACK_PHRASES: dict[str, list[str]] = {'status': ['still on it', 'still working through it', 'waiting for the result'], 'generic': ['on it', 'one sec', 'checking that now']}
...
def classify_status_context(kind: str, *, tool_name: str | None=None, preview: str | None=None, args: Any=None) -> str:
    """Classify an internal gateway event into a Duck Agent UI-surface bucket."""
