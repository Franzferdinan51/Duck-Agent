...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.duck-agent/config.yaml``. Duck Agent
...
Configuration is loaded from ~/.duck-agent/config.yaml under the 'tts:' key.
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``duck-agent auth add <provider_id>``).
...
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.duck-agent/config.yaml.
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``duck-agent tools``); otherwise the historical Edge backend remains active.
    """
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Duck Agent secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Duck Agent secrets (salvage of #56332) while still
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Duck Agent caches Piper voice models.

    Resolves to ``~/.duck-agent/cache/piper-voices/`` under the active
    DUCK_AGENT_HOME so voice downloads follow profile boundaries.
    """
    from hermes_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.duck-agent/config.yaml (tts: section).
