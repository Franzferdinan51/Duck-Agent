...
409), and is driven by:
  ...
  * the ``duck-agent sync status|pull|push|now`` CLI.
...
is toggled via ``duck-agent sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.duck-agent/skills/`` are eligible; bundled and
...
def sync_feature_enabled() -> bool:
    ...
    switch a Duck Agent Cloud deployment sets to opt its instances into sync by
...
def sync_org_auto_propose() -> bool:
    ...
    runs ``duck-agent sync propose <skill>``. The skill keeps working with the
...
def sync_default_opt_in() -> bool:
    ...
    ``duck-agent sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Duck Agent Cloud deployment wants. Per the design notes, this default is
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.duck-agent/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Duck Agent Cloud "on by default"):** every *eligible* skill syncs
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.duck-agent/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.duck-agent/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``duck-agent sync device --name`` to set an explicit
    label."""
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.duck-agent/skills/.sync_device_id, overwriting
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.duck-agent/skills/ (posix), or None."""
    try:
...
def pull_skills(
    ...
    *,
    ...
    ~/.duck-agent/skills/. Only paths the user has opted into (``sync: true``) are
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``duck-agent sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.duck-agent/skills/_org/<org_id>/``.
