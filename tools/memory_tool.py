...
def _read_failed_error(path: 'Path') -> Dict[str, Any]:
    ...
    """
    return {'success': False, 'error': f'Refusing to write {path.name}: the file exists on disk but could not be read right now (temporarily locked by another program, a permission change, invalid/corrupt text encoding, or a filesystem error). Treating an unreadable file as empty and saving would wipe existing memory, so the write is refused. Nothing was changed — retry in a moment.'}
...
class MemoryStore:
    ...
    def _consolidation_failure(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._consolidation_failures <= self._MAX_CONSOLIDATION_FAILURES_PER_TURN:
            return response
        return {'success': False, 'done': True, 'error': f'Memory consolidation failed {self._consolidation_failures} times this turn. Stop retrying memory calls — leave memory unchanged for now and continue with your reply to the user. The fact can be saved in a later turn.'}
    ...
    def load_from_disk(self):
        ...
        self._system_prompt_snapshot = {'memory': self._render_block('memory', sanitized_memory), 'user': self._render_block('user', sanitized_user)}
    ...
    def _sanitize_entries_for_snapshot(entries: List[str], filename: str) -> List[str]:
        ...
        for entry in entries:
            ...
            if findings:
                ...
                sanitized.append(f"[BLOCKED: {filename} entry contained threat pattern(s): {', '.join(findings)}. Removed from system prompt; use memory(action=remove) to delete the original.]")
            else:
                sanitized.append(entry)
        return sanitized
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            if new_total > limit:
                ...
                return self._consolidation_failure({'success': False, 'error': f"Memory at {current:,}/{limit:,} chars. Adding this entry ({len(content)} chars) would exceed the limit. Consolidate now: use 'replace' to merge overlapping entries into shorter ones or 'remove' stale or less important entries (see current_entries below), then retry this add — all in this turn.", 'current_entries': entries, 'usage': f'{current:,}/{limit:,}'})
    ...
    def replace(self, target: str, old_text: str, new_content: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            if not matches:
                return self._consolidation_failure({'success': False, 'error': f"No entry matched '{old_text}'. Check current_entries below and retry with the exact text of the entry you want to replace.", 'current_entries': entries})
            if len(matches) > 1:
            ...
            if new_total > limit:
                current = self._char_count(target)
                return self._consolidation_failure({'success': False, 'error': f"Replacement would put memory at {new_total:,}/{limit:,} chars. Shorten the new content, or 'remove' other stale or less important entries to make room (see current_entries below), then retry — all in this turn.", 'current_entries': entries, 'usage': f'{current:,}/{limit:,}'})
    ...
    def remove(self, target: str, old_text: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            matches = [(i, e) for i, e in enumerate(entries) if old_text in e]
            if not matches:
                return self._consolidation_failure({'success': False, 'error': f"No entry matched '{old_text}'. Check current_entries below and retry with the exact text of the entry you want to remove.", 'current_entries': entries})
            if len(matches) > 1:
                unique_texts = {e for _, e in matches}
                if len(unique_texts) > 1:
                    previews = self._previews([e for _, e in matches])
    ...
    def apply_batch(self, target: str, operations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            if new_total > limit:
                current = self._char_count(target)
                return self._consolidation_failure({'success': False, 'error': f'After applying all {len(operations)} operations, memory would be at {new_total:,}/{limit:,} chars -- over the limit. Remove or shorten more entries in the same batch (see current_entries below), then retry.', 'current_entries': self._entries_for(target), 'usage': f'{current:,}/{limit:,}'})
    ...
    def _batch_error(self, target: str, message: str) -> Dict[str, Any]:
        ...
        current = self._char_count(target)
        limit = self._char_limit(target)
        return self._consolidation_failure({'success': False, 'error': message + ' No operations were applied (batch is all-or-nothing).', 'current_entries': self._entries_for(target), 'usage': f'{current:,}/{limit:,}'})
    ...
    def _success_response(self, target: str, message: str=None) -> Dict[str, Any]:
        ...
        resp = {'success': True, 'done': True, 'target': target, 'usage': f'{pct}% — {current:,}/{limit:,} chars', 'entry_count': len(entries)}
        if message:
...
def _apply_write_gate(action: str, target: str, content: Optional[str], old_text: Optional[str]) -> Optional[str]:
    ...
    return json.dumps({'success': True, 'staged': True, 'pending_id': record['id'], 'message': decision.message}, ensure_ascii=False)
...
def _missing_old_text_error(store: 'MemoryStore', target: str, action: str) -> str:
    ...
    return json.dumps({'success': False, 'error': f"'{action}' needs old_text -- a short unique substring of the entry to {action}. None was provided. Reissue the {action} with old_text set to part of one of the current_entries below.", 'current_entries': entries, 'usage': f'{current:,}/{limit:,}'}, ensure_ascii=False)
