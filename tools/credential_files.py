...
def register_credential_file(relative_path: str, container_base: str='/root/.duck-agent') -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``DUCK_AGENT_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside DUCK_AGENT_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because DUCK_AGENT_HOME is exactly where
    ...
    if os.path.isabs(relative_path):
        logger.warning('credential_files: rejected absolute path %r (must be relative to DUCK_AGENT_HOME)', relative_path)
        return False
    ...
    if get_read_block_error is None:
        logger.error('credential_files: refusing %r — agent.file_safety could not be imported, so the master-store deny-list cannot be consulted', relative_path)
        return False
    try:
        ...
        return False
    if denied:
        logger.warning('credential_files: refused %r — it is a credential store the agent is denied from reading; a skill may mount its own service token, not the master key files', relative_path)
        return False
...
def register_credential_files(entries: list, container_base: str='/root/.duck-agent') -> List[str]:
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f'/root/.duck-agent/{rel}'
                        result.append({'host_path': str(resolved_path), 'container_path': container_path})
...
def get_skills_directory_mount(container_base: str='/root/.duck-agent') -> list[Dict[str, str]]:
    ...
    **Security:** Bind mounts follow symlinks, so a malicious symlink inside
    ...
    if skills_dir.is_dir():
        ...
        mounts.append({'host_path': host_path, 'container_path': f"{container_base.rstrip('/')}/skills"})
    try:
        from agent.skill_utils import get_external_skills_dirs
        for idx, ext_dir in enumerate(get_external_skills_dirs()):
            if ext_dir.is_dir():
                host_path = _safe_skills_path(ext_dir)
                mounts.append({'host_path': host_path, 'container_path': f"{container_base.rstrip('/')}/external_skills/{idx}"})
    except ImportError:
        pass
    return mounts
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix='duck-agent-skills-safe-'))
...
def iter_skills_files(container_base: str='/root/.duck-agent') -> List[Dict[str, str]]:
    ...
    if skills_dir.is_dir():
        ...
        for item in skills_dir.rglob('*'):
            if item.is_symlink() or not item.is_file():
                continue
            rel = item.relative_to(skills_dir)
            result.append({'host_path': str(item), 'container_path': f'{container_root}/{rel}'})
    try:
        from agent.skill_utils import get_external_skills_dirs
        for idx, ext_dir in enumerate(get_external_skills_dirs()):
        ...
        pass
    return result
_CACHE_DIRS: list[tuple[str, str]] = [('cache/documents', 'document_cache'), ('cache/images', 'image_cache'), ('cache/audio', 'audio_cache'), ('cache/videos', 'video_cache'), ('cache/screenshots', 'browser_screenshots'), ('cache/web', 'web_cache'), ('cache/delegation', 'delegation_cache'), ('images', 'images')]

def get_cache_directory_mounts(container_base: str='/root/.duck-agent') -> List[Dict[str, str]]:
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if host_dir.is_dir():
            ...
            mounts.append({'host_path': str(host_dir), 'container_path': container_path})
    return mounts

def map_cache_path_to_container(host_path: str, container_base: str='/root/.duck-agent') -> Optional[str]:
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.duck-agent``,
    SSH ``<remote_home>/.duck-agent``, etc.) and whether translation is wanted.
...
def from_agent_visible_cache_path(container_path: str, container_base: str='/root/.duck-agent') -> str:
...
def to_agent_visible_cache_path(host_path: str, container_base: str='/root/.duck-agent') -> str:
...
def iter_cache_files(container_base: str='/root/.duck-agent') -> List[Dict[str, str]]:
