...
machine as Duck Agent itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Duck Agent ships under one Python (e.g. 3.11 in a bundled venv) while the
...
_REMOTE_BACKENDS = frozenset({'docker', 'singularity', 'modal', 'daytona', 'ssh', 'managed_modal', 'vercel_sandbox'})
...
def _run(cmd: list[str], timeout: float=3.0) -> tuple[int, str, str]:
    ...
    try:
        with tempfile.TemporaryFile() as out_f, tempfile.TemporaryFile() as err_f:
            try:
                result = subprocess.run(cmd, stdout=out_f, stderr=err_f, timeout=timeout, check=False, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
...
def _detect_pep668(binary: str) -> bool:
    ...
    if not shutil.which(binary):
        return False
    code = "import sys, os;stdlib = os.path.dirname(os.__file__);marker = os.path.join(stdlib, 'EXTERNALLY-MANAGED');print('yes' if os.path.exists(marker) else 'no')"
...
def get_environment_probe_line(*, force_refresh: bool=False) -> str:
    ...
    if not _PROBE_DONE.wait(timeout=wait_timeout):
        if not _WAIT_ALREADY_TIMED_OUT:
            ...
            logger.warning('env_probe did not finish within %.0fs; building the system prompt without the Python toolchain line', _PROBE_WAIT_TIMEOUT)
        return ''
    return _CACHED_LINE or ''
...
def _ensure_probe_started() -> None:
    ...
    with _CACHE_LOCK:
        ...
        if _PROBE_THREAD is not None and _PROBE_THREAD.is_alive():
            return
        _PROBE_THREAD = threading.Thread(target=_probe_worker, args=(_PROBE_GEN,), name='env-probe', daemon=True)
        _PROBE_THREAD.start()
