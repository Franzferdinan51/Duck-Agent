"""
Cron job management tools for Duck Agent.
...
from cron.jobs import AmbiguousJobReference, claim_job_for_fire, get_job, list_jobs, mark_job_run, parse_schedule, pause_job, remove_job, resolve_job_ref, resume_job, update_job
...
def _notify_provider_jobs_changed_safe() -> None:
    ...
    try:
        ...
        pass
_CRON_THREAT_PATTERNS = [('ignore\\s+(?:\\w+\\s+)*(?:previous|all|above|prior)\\s+(?:\\w+\\s+)*instructions', 'prompt_injection'), ('do\\s+not\\s+tell\\s+the\\s+user', 'deception_hide'), ('system\\s+prompt\\s+override', 'sys_prompt_override'), ('disregard\\s+(your|all|any)\\s+(instructions|rules|guidelines)', 'disregard_rules'), ('cat\\s+[^\\n]*(\\.env|credentials|\\.netrc|\\.pgpass|id_rsa|id_ed25519|id_ecdsa)', 'read_secrets'), ('authorized_keys', 'ssh_backdoor'), ('/etc/sudoers|visudo', 'sudoers_mod'), ('rm\\s+-rf\\s+/', 'destructive_root_rm')]
_CRON_SKILL_ASSEMBLED_PATTERNS = [('ignore\\s+(?:\\w+\\s+)*(?:previous|all|above|prior)\\s+(?:\\w+\\s+)*instructions', 'prompt_injection'), ('do\\s+not\\s+tell\\s+the\\s+user', 'deception_hide'), ('system\\s+prompt\\s+override', 'sys_prompt_override'), ('disregard\\s+(your|all|any)\\s+(instructions|rules|guidelines)', 'disregard_rules')]
_CRON_SECRET_VAR_RE = '\\$\\{?\\w*(?:KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|API)\\w*\\}?'
_CRON_EXFIL_COMMAND_PATTERNS = [(f"""curl\\s+[^\\n]*https?://[^\\s"\\'`]*{_CRON_SECRET_VAR_RE}""", 'exfil_curl_url'), (f"""wget\\s+[^\\n]*https?://[^\\s"\\'`]*{_CRON_SECRET_VAR_RE}""", 'exfil_wget_url'), (f'curl\\s+[^\\n]*(?:--data(?:-raw|-binary|-urlencode)?|-d|--form|-F)\\s+[^\\n]*{_CRON_SECRET_VAR_RE}', 'exfil_curl_data'), (f'wget\\s+[^\\n]*--post-(?:data|file)=[^\\n]*{_CRON_SECRET_VAR_RE}', 'exfil_wget_post'), (f"""curl\\s+[^\\n]*(?:-H|--header)\\s+["\\']Authorization:\\s*(?:Bearer|token)\\s+{_CRON_SECRET_VAR_RE}["\\']""", 'exfil_curl_auth_header')]
from tools.threat_patterns import INVISIBLE_CHARS as _CRON_INVISIBLE_CHARS
_EMOJI_NEIGHBOUR_CP_RANGES = ((126976, 131071), (9728, 10175), (8960, 9215), (127462, 127487), (8419, 8419))
_VARIATION_SELECTOR_CP = 65039
...
def _zwj_has_emoji_neighbour(text: str, idx: int) -> bool:
    ...
    while right < len(text) and ord(text[right]) == _VARIATION_SELECTOR_CP:
        right += 1
    return left >= 0 and right < len(text) and _is_emoji_cp(ord(text[left])) and _is_emoji_cp(ord(text[right]))
...
def _strip_cron_safe_constructs(prompt: str) -> str:
    ...
    """
    return re.sub(f"""curl\\s+[^\\n;&|$`]*(?:-H|--header)\\s+["\\']Authorization:\\s*token\\s+{_CRON_SECRET_VAR_RE}["\\']\\s+["\\']?https://api\\.github\\.com(?::\\d+)?(?:/|\\s|$|["\\'])[^\\s;&|$`]*""", 'curl https://api.github.com/user', prompt, flags=re.IGNORECASE)
...
def _scan_cron_skill_assembled(assembled: str) -> tuple[str, str]:
    ...
    if removed:
        logger.warning('Cron skill-assembled prompt: stripped %d invisible-unicode char(s) (%s) from vetted skill content', len(removed), ', '.join(removed))
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    if origin_platform and origin_chat_id:
        ...
        if thread_id:
            logger.debug('Cron origin captured thread_id=%s for %s:%s', thread_id, origin_platform, origin_chat_id)
        return {'platform': origin_platform, 'chat_id': origin_chat_id, 'chat_name': get_session_env('HERMES_SESSION_CHAT_NAME') or None, 'thread_id': thread_id, 'user_id': get_session_env('HERMES_SESSION_USER_ID') or None}
    return None
...
def _validate_cron_base_url(provider: Optional[Any], base_url: Optional[Any]) -> Optional[str]:
    ...
    if not prov:
        return 'base_url override requires an explicit provider. Set provider to a configured custom provider to use a custom endpoint.'
    try:
        from hermes_cli.runtime_provider import has_named_custom_provider, resolve_requested_provider, _get_named_custom_provider
    ...
    if has_named_custom_provider(prov):
        ...
        if cfg_host and base_url_host_matches(bu, cfg_host):
            return None
        return f"base_url {bu!r} is not allowed for provider {prov!r}. A named custom provider's stored credential may only be sent to its own configured endpoint ({cfg_host or 'unknown'})."
    try:
    ...
    if known_host and base_url_host_matches(bu, known_host):
        return None
    return f"""base_url {bu!r} is not allowed for provider {prov!r}. A named provider's stored credential may only be sent to its own endpoint; use a configured custom provider (provider="custom") for a custom base_url."""

def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    """Validate a cron job script path at the API boundary.

    Scripts must be relative paths that resolve within DUCK_AGENT_HOME/scripts/.
    ...
    if raw.startswith(('/', '~')) or (len(raw) >= 2 and raw[1] == ':'):
        return f'Script path must be relative to ~/.duck-agent/scripts/. Got absolute or home-relative path: {raw!r}. Place scripts in ~/.duck-agent/scripts/ and use just the filename.'
...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {'job_id': job_id, 'name': name, 'skill': skills[0] if skills else None, 'skills': skills, 'prompt_preview': prompt[:100] + '...' if len(prompt) > 100 else prompt, 'model': job.get('model'), 'provider': job.get('provider'), 'base_url': job.get('base_url'), 'schedule': job.get('schedule_display') or '?', 'repeat': _repeat_display(job), 'deliver': job.get('deliver', 'local'), 'next_run_at': job.get('next_run_at'), 'last_run_at': job.get('last_run_at'), 'last_status': job.get('last_status'), 'last_delivery_error': job.get('last_delivery_error'), 'enabled': job.get('enabled', True), 'state': job.get('state', 'scheduled' if job.get('enabled', True) else 'paused'), 'paused_at': job.get('paused_at'), 'paused_reason': job.get('paused_reason')}
    if job.get('script'):
...
def _run_claimed_job(job: Dict[str, Any], extra_prompt: Optional[str]=None) -> Dict[str, Any]:
    ...
    try:
        ...
        if not try_register_running_job(job_id):
            return {'claimed': True, 'success': False, 'error': 'Job is already running (a scheduler tick or another manual run is executing it); not started again.'}
        _registered = True
        try:
            ...
            def _heartbeat_loop() -> None:
                ...
                while not _heartbeat_stop.wait(_CRON_RUN_HEARTBEAT_INTERVAL):
                    ...
                    if elapsed > _CRON_RUN_HEARTBEAT_CEILING:
                        logger.warning("cronjob run heartbeat ceiling reached for job '%s' (%.0fs) — stopping heartbeat; gateway watchdog regains authority", job_name, elapsed)
                        return
                    try:
                        activity_cb(f"cronjob: running job '{job_name}' ({int(elapsed)}s elapsed)")
                    except Exception:
                        continue
            _heartbeat_thread = threading.Thread(target=_heartbeat_loop, daemon=True, name='cronjob-run-heartbeat')
        ...
        return {'claimed': True, 'success': bool(processed and ok), 'error': refreshed.get('last_error')}
    except Exception as e:
...
def _latest_job_output_excerpt(job_id: str, max_chars: int=2000) -> Optional[str]:
    ...
    ``~/.duck-agent/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(job: Dict[str, Any], session_id: Optional[str]=None, extra_prompt: Optional[str]=None) -> Optional[Dict[str, Any]]:
    ...
    None
        ...
        ``duck-agent -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    try:
        try:
            ...
            if job_id in get_running_job_ids():
                return {'claimed': False, 'success': False, 'error': 'Job is already running (a scheduler tick or another manual run is executing it); not started again.'}
        except Exception:
            pass
        if not claim_job_for_fire(job_id):
            refreshed = get_job(job_id)
    ...
    try:
        ...
        logger.warning("cronjob run: async delegation registry unavailable (%s); running job '%s' inline.", e, job_name)
    ...
    def _runner() -> Dict[str, Any]:
        ...
        refreshed = get_job(job_id) or {}
        lines = [f"Cron job '{job_name}' ({job_id}) finished its manual run.", f"Result: {('ok' if res.get('success') else 'FAILED')}" + (f" — {res.get('error')}" if res.get('error') else ''), f'Delivery target: {deliver}' + (' (output was delivered there by the job itself)' if deliver != 'local' else ' (output saved locally only)')]
...
def cronjob(action: str, job_id: Optional[str]=None, prompt: Optional[str]=None, schedule: Optional[str]=None, name: Optional[str]=None, repeat: Optional[int]=None, deliver: Optional[str]=None, include_disabled: bool=False, skill: Optional[str]=None, skills: Optional[List[str]]=None, model: Optional[str]=None, provider: Optional[str]=None, base_url: Optional[str]=None, reason: Optional[str]=None, script: Optional[str]=None, context_from: Optional[Union[str, List[str]]]=None, enabled_toolsets: Optional[List[str]]=None, workdir: Optional[str]=None, no_agent: Optional[bool]=None, attach_to_session: Optional[bool]=None, monitor_script: Optional[str]=None, monitor_url: Optional[str]=None, task_id: str=None, session_id: Optional[str]=None) -> str:
    ...
    try:
        ...
        if normalized == 'create':
            ...
            if _no_agent:
                if not script:
                    return tool_error('create with no_agent=True requires a script — the script is the job.', success=False)
            ...
            if context_from:
                ...
                for ref_id in refs:
                    if not _get_job(ref_id):
                        return tool_error(f"context_from job '{ref_id}' not found. Use cronjob(action='list') to see available jobs.", success=False)
            from cron.scheduler import CronSchedulerRegistrationError, create_job_with_scheduler_registration
            try:
                job = create_job_with_scheduler_registration(prompt=prompt or '', schedule=schedule, name=name, repeat=repeat, deliver=_normalize_deliver_param(deliver), origin=_origin_from_env(), skills=canonical_skills, model=_normalize_optional_job_value(model), provider=_normalize_optional_job_value(provider), base_url=_normalize_optional_job_value(base_url, strip_trailing_slash=True), script=_normalize_optional_job_value(script), context_from=context_from, enabled_toolsets=enabled_toolsets or None, workdir=_normalize_optional_job_value(workdir), no_agent=_no_agent, attach_to_session=attach_to_session, monitor_script=_normalize_optional_job_value(monitor_script), monitor_url=_normalize_optional_job_value(monitor_url))
            ...
            return json.dumps({'success': True, 'job_id': job['id'], 'name': job['name'], 'skill': job.get('skill'), 'skills': job.get('skills', []), 'schedule': job['schedule_display'], 'repeat': _repeat_display(job), 'deliver': job.get('deliver', 'local'), 'next_run_at': job['next_run_at'], 'job': _format_job(job), 'message': _create_message}, indent=2)
        if normalized == 'list':
        ...
        try:
            ...
            return json.dumps({'success': False, 'error': str(exc), 'matches': [{'id': m['id'], 'name': m.get('name'), 'schedule': m.get('schedule_display'), 'next_run_at': m.get('next_run_at')} for m in exc.matches]}, indent=2)
        if not job:
            return json.dumps({'success': False, 'error': f"Job with ID or name '{job_id}' not found. Use cronjob(action='list') to inspect jobs."}, indent=2)
        job_id = job['id']
        if normalized == 'remove':
            ...
            return json.dumps({'success': True, 'message': f"Cron job '{job['name']}' removed.", 'removed_job': {'id': job_id, 'name': job['name'], 'schedule': job.get('schedule_display')}}, indent=2)
        if normalized == 'pause':
        ...
        if normalized in {'run', 'run_now', 'trigger'}:
            ...
            if extra_prompt:
                ...
                if scan_error:
                    return tool_error(scan_error, success=False)
            bg = _try_dispatch_background_run(job, session_id=session_id, extra_prompt=extra_prompt)
            if bg is not None and bg.get('dispatched'):
                ...
                return json.dumps({'success': True, 'job': result, 'note': 'The job is running in the background. You and the user can keep working; its outcome re-enters the conversation as a new message when it finishes. Do not wait or poll — just continue.'}, indent=2)
            exec_result = bg if bg is not None else _execute_job_now(job, extra_prompt=extra_prompt)
            ...
            if not exec_result.get('claimed', False):
                result['execution_skipped'] = exec_result.get('error') or 'Already being fired by the scheduler; not run again.'
        ...
        if normalized == 'update':
            ...
            eff_provider = updates['provider'] if 'provider' in updates else job.get('provider')
            eff_base_url = updates['base_url'] if 'base_url' in updates else job.get('base_url')
            ...
            if monitor_script is not None:
                if monitor_script:
                    ...
                    if monitor_error:
                        return tool_error(monitor_error, success=False)
                updates['monitor_script'] = _normalize_optional_job_value(monitor_script) if monitor_script else None
            ...
            if monitor_script is not None or monitor_url is not None:
                ...
                if eff_mon_script and eff_mon_url:
                    return tool_error('monitor_script and monitor_url are mutually exclusive — clear one before setting the other.', success=False)
            ...
            if no_agent is not None:
                ...
                if target_no_agent:
                    ...
                    if not effective_script:
                        return tool_error('Cannot set no_agent=True on a job without a script. Set `script` in the same update, or on the job first.', success=False)
...
def check_cronjob_requirements() -> bool:
    ...
    return env_var_enabled('HERMES_INTERACTIVE') or env_var_enabled('HERMES_GATEWAY_SESSION') or env_var_enabled('HERMES_EXEC_ASK')
from tools.registry import registry, tool_error
registry.register(name='cronjob', toolset='cronjob', schema=CRONJOB_SCHEMA, handler=lambda args, **kw: cronjob(action=args.get('action', ''), job_id=args.get('job_id'), prompt=args.get('prompt'), schedule=args.get('schedule'), name=args.get('name'), repeat=args.get('repeat'), deliver=args.get('deliver'), include_disabled=args.get('include_disabled', True), skill=args.get('skill'), skills=args.get('skills'), reason=args.get('reason'), script=args.get('script'), context_from=args.get('context_from'), enabled_toolsets=args.get('enabled_toolsets'), workdir=args.get('workdir'), no_agent=args.get('no_agent'), monitor_script=args.get('monitor_script'), monitor_url=args.get('monitor_url'), task_id=kw.get('task_id'), session_id=kw.get('session_id')), check_fn=check_cronjob_requirements, emoji='⏰')