...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <DUCK_AGENT_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  DUCK_AGENT_HOME, etc.) falls back to a byte-length truncation with an
...
def get_spill_config() -> Dict[str, Any]:
    ...
    if directory is not None and (not isinstance(directory, str)):
        directory = None
    return {'enabled': enabled, 'max_chars': _coerce_positive_int(section.get('max_chars'), DEFAULT_MAX_CHARS), 'preview_head': _coerce_non_negative_int(section.get('preview_head'), DEFAULT_PREVIEW_HEAD), 'preview_tail': _coerce_non_negative_int(section.get('preview_tail'), DEFAULT_PREVIEW_TAIL), 'directory': directory}
...
def _build_preview(text: str, head: int, tail: int, saved_path: Optional[str], *, source: str) -> str:
    ...
    parts = [f'[{source} output truncated — {total:,} chars; full content ' + (f'saved to {saved_path}]' if saved_path else 'unavailable — spill write failed]')]
    if head_chunk:
        parts.append('--- head ---')
...
__all__ = ['DEFAULT_MAX_CHARS', 'DEFAULT_PREVIEW_HEAD', 'DEFAULT_PREVIEW_TAIL', 'DEFAULT_ENABLED', 'get_spill_config', 'spill_if_oversized']