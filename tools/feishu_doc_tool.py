...
def _handle_feishu_doc_read(args: dict, **kwargs) -> str:
    ...
    request = BaseRequest.builder().http_method(HttpMethod.GET).uri(_RAW_CONTENT_URI).token_types({AccessTokenType.TENANT}).paths({'document_id': doc_token}).build()
...
registry.register(name='feishu_doc_read', toolset='feishu_doc', schema=FEISHU_DOC_READ_SCHEMA, handler=_handle_feishu_doc_read, check_fn=_check_feishu, requires_env=[], is_async=False, description='Read Feishu document content', emoji='📄')