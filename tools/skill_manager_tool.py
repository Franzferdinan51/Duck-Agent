...
~/.duck-agent/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from agent.skill_utils import extract_skill_description, is_skill_description_truncated_for_prompt, parse_frontmatter as _parse_frontmatter, SKILL_PROMPT_DESC_LIMIT
logger = logging.getLogger(__name__)
_background_review_read_paths: '_ctxvars.ContextVar[frozenset[str]]' = _ctxvars.ContextVar('background_review_read_paths', default=frozenset())
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `duck-agent config set skills.guard_agent_created true`.
    """
    try:
        from hermes_cli.config import load_config
        cfg = load_config()
        return is_truthy_value(cfg_get(cfg, 'skills', 'guard_agent_created'), default=False)
    except Exception:
        return False
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch DUCK_AGENT_HOME and
    ...
    live profile-scoped DUCK_AGENT_HOME on every call.
    """
...
def _validate_delete_target(skill_dir: Path) -> Optional[str]:
    ...
    if _is_path_redirect(skill_dir):
        return f"Refusing to delete '{skill_dir}': the skill directory is a symlink/junction. Remove the link target manually if intended."
    try:
    ...
    for root in roots:
        if resolved == root:
            return f"Refusing to delete '{skill_dir}': resolves to the skills root itself, which would remove every installed skill."
        try:
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    try:
        ...
        if rec.get('pinned'):
            return f"Skill '{name}' is pinned and cannot be deleted by skill_manage. Ask the user to run `duck-agent curator unpin {name}` if they want to delete it. Patches and edits are allowed on pinned skills; only deletion is blocked."
    except Exception:
...
def _background_review_write_guard(name: str, skill_dir: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if skill_usage.get_record(name).get('pinned'):
            return {'success': False, 'error': f"Refusing background curator {action} for pinned skill '{name}': pinned skills are off-limits to autonomous maintenance. Ask the user to run `duck-agent curator unpin {name}` if they want it changed."}
    except Exception:
        logger.debug('pinned skill guard lookup failed for %s', name, exc_info=True)
    try:
        from agent.skill_utils import is_external_skill_path
        if is_external_skill_path(skill_dir):
            return {'success': False, 'error': f"Refusing background curator {action} for skill '{name}': the skill lives in skills.external_dirs, which are externally owned and read-only to autonomous curation."}
    except Exception:
    ...
    try:
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {'success': False, 'error': f"Refusing background curator {action} for skill '{name}': the skill is not curator-managed ({_detail}). User-owned skills are off-limits to autonomous curation. Run `duck-agent curator adopt {name}` to opt it in."}
    except Exception:
        logger.warning('owned skill guard lookup failed for %s', name, exc_info=True)
        return {'success': False, 'error': f"Refusing background curator {action} for skill '{name}': agent ownership could not be verified because the provenance record is unavailable or unreadable."}
    return None
...
def _background_review_read_before_write_guard(name: str, target: Path, action: str, file_label: str) -> Optional[Dict[str, Any]]:
    ...
    if _background_review_has_read(target):
        return None
    return {'success': False, 'error': f"Refusing background curator {action} for skill '{name}': the current {file_label} content has not been loaded in this review turn. Call skill_view(name) for SKILL.md, or skill_view(name, file_path=...) for a supporting file, then retry the write using the content just returned.", '_read_before_write_required': True}
...
def _validate_frontmatter(content: str, *, new_skill: bool=False) -> Optional[str]:
    ...
    if new_skill and len(desc.strip().strip('\'"')) > SKILL_PROMPT_DESC_LIMIT:
        return f"Description is {len(desc.strip())} chars — new skills must fit the {SKILL_PROMPT_DESC_LIMIT}-char system-prompt budget (one sentence, trigger first, ends with a period). The skill index truncates longer descriptions to {SKILL_PROMPT_DESC_LIMIT - 3} chars + '...', destroying the routing signal. Move detail into the skill body."
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.duck-agent/skills/) first, then any
...
def _maybe_auto_propose_org_edit(name: str, skill_path: Path) -> Optional[str]:
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return f'This skill is shared by your organisation. Your edit is saved locally and will not be overwritten by org updates. Run `duck-agent sync propose {name}` to share it back.'
        result = ssc.propose_skill(name)
        if result.get('proposal_pending'):
            return f"Auto-proposed to your organisation as proposal #{result.get('proposal_id')} (pending admin review)."
        return 'Auto-proposed to your organisation (merged into the shared set).'
    except Exception as e:
        logger.debug('auto-propose skipped for %s: %s', name, e)
        return f'Edit saved locally. Could not submit it to your organisation right now — run `duck-agent sync propose {name}` to retry.'
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `duck-agent sync propose`, or
    ...
    try:
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {'success': False, 'error': f"Cannot {action} '{name}' locally: it is shared by your organisation, so a local delete would just come back on the next sync. Ask an org admin to remove it for everyone. (Editing it IS allowed — your changes are kept and can be proposed back with `duck-agent sync propose {name}`.)"}
    except Exception:
        logger.debug('org mirror guard lookup failed for %s', name, exc_info=True)
    return None

def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Duck Agent profiles.
...
def _skill_not_found_error(name: str, suffix: str='') -> str:
    ...
    if others:
        if len(others) == 1:
            ...
            base += f" A skill by that name exists in profile '{other_profile}' ({other_path}). To edit a skill in another profile, switch profiles (`duck-agent -p {other_profile}`) or operate via explicit file tools with ``cross_profile=True``."
        else:
            names = ', '.join((f"'{p}'" for p, _ in others))
            base += f' Skills by that name exist in other profiles: {names}. Switch profiles (`duck-agent -p <name>`) to edit there, or operate via explicit file tools with ``cross_profile=True``.'
    else:
...
def _add_description_prompt_preview(result: Dict[str, Any], content: str) -> None:
    ...
    if is_skill_description_truncated_for_prompt(fm):
        result['system_prompt_preview'] = f'System prompt will show: "{extract_skill_description(fm)}" — keep the trigger self-contained in the first {SKILL_PROMPT_DESC_LIMIT - 3} chars.'
...
def _create_skill(name: str, content: str, category: str=None) -> Dict[str, Any]:
    ...
    try:
        ...
        pass
    result = {'success': True, 'message': f"Skill '{name}' created.", 'path': str(skill_dir.relative_to(_skills_dir())), 'skill_md': str(skill_md), '_change': {'description': _desc}}
    if category:
        result['category'] = category
    result['hint'] = "To add reference files, templates, or scripts, use skill_manage(action='write_file', name='{}', file_path='references/example.md', file_content='...')".format(name)
...
def _edit_skill(name: str, content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        pass
    result = {'success': True, 'message': f"Skill '{name}' updated (full rewrite).", 'path': str(existing['path']), '_change': {'description': _desc}}
...
def _patch_skill(name: str, old_string: str, new_string: str, file_path: str=None, replace_all: bool=False) -> Dict[str, Any]:
    ...
    read_guard = _background_review_read_before_write_guard(name, target, 'patch', 'SKILL.md' if not file_path else file_path)
    if read_guard:
        return read_guard
    ...
    if scan_error:
        ...
        return {'success': False, 'error': scan_error}
    result = {'success': True, 'message': f"Patched {('SKILL.md' if not file_path else file_path)} in skill '{name}' ({match_count} replacement{('s' if match_count > 1 else '')})."}
    result['_change'] = {'old': old_string[:200] + ('…' if len(old_string) > 200 else ''), 'new': new_string[:200] + ('…' if len(new_string) > 200 else '')}
...
def _delete_skill(name: str, absorbed_into: Optional[str]=None) -> Dict[str, Any]:
    ...
    absorbed_target = absorbed_into.strip() if absorbed_into is not None and isinstance(absorbed_into, str) else ''
    ...
    if is_consolidation:
        ...
        if not target:
            return {'success': False, 'error': f"absorbed_into='{target_name}' does not exist. Create or patch the umbrella skill first, then retry the delete."}
    skill_dir = existing['path']
...
def _write_file(name: str, file_path: str, file_content: str) -> Dict[str, Any]:
    ...
    if content_bytes > MAX_SKILL_FILE_BYTES:
        return {'success': False, 'error': f'File content is {content_bytes:,} bytes (limit: {MAX_SKILL_FILE_BYTES:,} bytes / 1 MiB). Consider splitting into smaller files.'}
...
def _remove_file(name: str, file_path: str) -> Dict[str, Any]:
    ...
    if not target.exists():
        ...
        return {'success': False, 'error': f"File '{file_path}' not found in skill '{name}'.", 'available_files': available if available else None}
...
import contextvars as _ctxvars
_skill_gate_bypass: '_ctxvars.ContextVar[bool]' = _ctxvars.ContextVar('skill_gate_bypass', default=False)
...
def _apply_skill_write_gate(action, name, **payload_kwargs):
    ...
    gist = wa.skill_gist(action, name, content=payload_kwargs.get('content') or '', file_path=payload_kwargs.get('file_path') or '', old_string=payload_kwargs.get('old_string') or '', new_string=payload_kwargs.get('new_string') or '')
    record = wa.stage_write(wa.SKILLS, payload, summary=gist, origin=wa.current_origin())
    return json.dumps({'success': True, 'staged': True, 'pending_id': record['id'], 'gist': gist, 'message': decision.message}, ensure_ascii=False)
...
def skill_manage(action: str, name: str, content: str=None, category: str=None, file_path: str=None, file_content: str=None, old_string: str=None, new_string: str=None, replace_all: bool=False, absorbed_into: str=None, task_id: str=None, session_id: str=None) -> str:
    """
    ...
    gate_result = _apply_skill_write_gate(action, name, content=content, category=category, file_path=file_path, file_content=file_content, old_string=old_string, new_string=new_string, replace_all=replace_all, absorbed_into=absorbed_into)
