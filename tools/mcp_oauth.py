    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Duck Agent"
"""
import asyncio
import contextvars
import json
...
def _ensure_sdk_loaded() -> bool:
    ...
    if not _SDK_CLASSES:
        try:
            ...
            from mcp.shared.auth import OAuthClientInformationFull as _InfoFull, OAuthClientMetadata as _ClientMeta, OAuthMetadata as _Meta, OAuthToken as _Token
        except ImportError:
            _SDK_LOAD_FAILED = True
            _OAUTH_AVAILABLE = False
            logger.debug('MCP OAuth types not available -- OAuth MCP auth disabled')
            return False
        _SDK_CLASSES.update(OAuthClientProvider=_Provider, OAuthClientInformationFull=_InfoFull, OAuthClientMetadata=_ClientMeta, OAuthMetadata=_Meta, OAuthToken=_Token)
    g = globals()
...
_oauth_interactive_enabled: 'contextvars.ContextVar[bool]' = contextvars.ContextVar('_oauth_interactive_enabled', default=True)
_oauth_interactive_forced: 'contextvars.ContextVar[bool]' = contextvars.ContextVar('_oauth_interactive_forced', default=False)
...
def _get_token_dir(hermes_home: str | Path | None=None) -> Path:
    ...
    Uses DUCK_AGENT_HOME so each profile gets its own OAuth tokens.
    Layout: ``DUCK_AGENT_HOME/mcp-tokens/``
    """
...
def _safe_filename(name: str) -> str:
    ...
    return re.sub('[^\\w\\-]', '_', name).strip('_')[:128] or 'default'
...
def _cached_redirect_port(storage: 'HermesTokenStorage | None') -> int | None:
    ...
    redirect URI that was registered with it. If Duck Agent restarts and chooses a
    ...
    for uri in data.get('redirect_uris') or []:
        try:
            ...
            continue
        if parsed.scheme == 'http' and parsed.hostname in {'127.0.0.1', 'localhost'} and (parsed.path == '/callback') and (parsed.port is not None):
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``duck-agent mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(f'{lead} Run `duck-agent mcp login <server>` interactively to (re)authorize, then restart or reload the gateway.')
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        DUCK_AGENT_HOME/mcp-tokens/<server_name>.json         -- tokens
        DUCK_AGENT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        DUCK_AGENT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool=False) -> None:
        ...
        if only_if_absent and any((path.exists() for path in (self._tokens_path(), self._client_info_path(), self._meta_path()))):
    ...
    def poison_client_registration(self) -> bool:
        ...
        logger.warning("MCP OAuth '%s': cached client registration rejected as invalid_client; removed client.json + meta.json (backup at %s) to force re-registration", self._server_name, backup.name)
        return True
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            result['state'] = state
            result['error'] = error
            body = '<html><body><h2>Authorization Successful</h2><p>You can close this tab and return to Duck Agent.</p></body></html>' if code else f"<html><body><h2>Authorization Failed</h2><p>Error: {error or 'unknown'}</p></body></html>"
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    if _oauth_port is None:
        raise RuntimeError('OAuth callback port not set — build_oauth_auth must be called before _wait_for_oauth_callback')
...
def _make_callback_waiter(port: int):
    ...
    async def _wait() -> tuple[str, str | None]:
        ...
        _raise_if_non_interactive('OAuth callback requires an interactive session but none is available (non-interactive/background context); skipping browser authorization without binding a callback listener.')
        ...
        try:
            ...
            raise OAuthNonInteractiveError(f'OAuth callback port {port} is already in use ({exc}). Close any other in-progress login, or set a free `oauth.redirect_port` in the server config, then retry.') from exc
        server_thread = threading.Thread(target=server.handle_request, daemon=True)
        server_thread.start()
        paste_thread: threading.Thread | None = None
        if _is_interactive():
            print('\n  Or paste the redirect URL here (or the ``?code=...&state=...`` portion) and press Enter. Type ``skip`` + Enter to continue without this server:', file=sys.stderr, flush=True)
            paste_thread = threading.Thread(target=_paste_callback_reader, args=(result,), daemon=True)
            paste_thread.start()
        timeout = 300.0
        poll_interval = 0.5
        elapsed = 0.0
        ...
        if result['auth_code'] is None:
            raise OAuthNonInteractiveError('OAuth callback timed out — no authorization code received. Ensure you completed the browser authorization flow.')
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print('  OAuth skipped. Run `duck-agent mcp login <server>` later to authenticate, or set ``enabled: false`` on that server in config.yaml to disable persistently.', file=sys.stderr)
        return
    query = line
    if '?' in line:
...
def apply_oauth_provider_defaults(cfg: dict, *, server_name: str='', server_url: str | None=None) -> dict:
    ...
    if _is_figma_remote_mcp(server_name, server_url):
        if not cfg.get('client_name'):
            ...
            logger.info("MCP OAuth '%s': Figma DCR allowlist — registering as client_name=%r (override via oauth.client_name)", server_name or server_url, _FIGMA_DCR_CLIENT_NAME)
...
def _build_client_metadata(cfg: dict) -> 'OAuthClientMetadata':
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get('client_name', 'Duck Agent')
    scope = cfg.get('scope')
    ...
    metadata_kwargs: dict[str, Any] = {'client_name': client_name, 'redirect_uris': [AnyUrl(redirect_uri)], 'grant_types': ['authorization_code', 'refresh_token'], 'response_types': ['code'], 'token_endpoint_auth_method': auth_method}
    if scope:
...
def _maybe_preregister_client(storage: 'HermesTokenStorage', cfg: dict, client_metadata: 'OAuthClientMetadata') -> None:
    ...
    redirect_uri = _resolve_redirect_uri(cfg, port)
    info_dict: dict[str, Any] = {'client_id': client_id, 'redirect_uris': [redirect_uri], 'grant_types': client_metadata.grant_types, 'response_types': client_metadata.response_types, 'token_endpoint_auth_method': client_metadata.token_endpoint_auth_method}
...
def humanize_oauth_registration_error(server_name: str, exc: BaseException | str, *, server_url: str | None=None) -> str | None:
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Duck Agent auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Duck Agent is running.
    """
    msg = str(exc)
    lowered = msg.lower()
    if '403' not in msg and 'forbidden' not in lowered:
        return None
    looks_like_registration = 'regist' in lowered or 'client registration' in lowered or 'dcr' in lowered or ('dynamic client' in lowered) or (lowered.strip() in {'forbidden', '403 forbidden', 'http 403: forbidden'}) or ('403' in msg and 'forbidden' in lowered)
    if not looks_like_registration:
        return None
    if _is_figma_remote_mcp(server_name, server_url):
        return f''''{server_name}' is Figma's remote MCP — DCR is allowlisted by exact client_name ("{_FIGMA_DCR_CLIENT_NAME}" and "Codex" work; most other names 403). Duck Agent defaults to client_name: {_FIGMA_DCR_CLIENT_NAME!r} automatically. If you set oauth.client_name yourself, change it to one of those, or clear it and re-run:\n  duck-agent mcp login {server_name}'''
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None=None) -> 'OAuthClientProvider | None':
    ...
    if not _is_interactive() and (not storage.has_cached_tokens()):
        raise OAuthNonInteractiveError(f"MCP OAuth for '{server_name}': non-interactive environment and no cached tokens found. The OAuth flow requires browser authorization. Run `duck-agent mcp login {server_name}` interactively first to complete initial authorization, then cached tokens will be reused.")
    _configure_callback_port(cfg, storage)
    client_metadata = _build_client_metadata(cfg)
    _maybe_preregister_client(storage, cfg, client_metadata)
    resolved_port = cfg.get('_resolved_port', _oauth_port)
    redirect_handler = _make_redirect_handler(resolved_port, redirect_uri=cfg.get('redirect_uri') or None)
    callback_handler = _make_callback_waiter(resolved_port)
    return OAuthClientProvider(server_url=server_url, client_metadata=client_metadata, storage=storage, redirect_handler=redirect_handler, callback_handler=callback_handler, timeout=float(cfg.get('timeout', 300)))