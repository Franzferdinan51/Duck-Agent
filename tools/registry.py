"""Central registry for all duck-agent tools.
...
def _is_registry_register_call(node: ast.AST) -> bool:
    ...
    return isinstance(func, ast.Attribute) and func.attr == 'register' and isinstance(func.value, ast.Name) and (func.value.id == 'registry')
...
class ToolEntry:
    ...
    __slots__ = ('name', 'toolset', 'schema', 'handler', 'check_fn', 'requires_env', 'is_async', 'description', 'emoji', 'max_result_size_chars', 'dynamic_schema_overrides')

    def __init__(self, name, toolset, schema, handler, check_fn, requires_env, is_async, description, emoji, max_result_size_chars=None, dynamic_schema_overrides=None):
        self.name = name
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Duck Agent-home override for every
...
def _check_fn_cached(fn: Callable) -> bool:
    ...
    if scope == CHECK_FN_CACHE_BYPASS:
        try:
            ...
            logger.warning('check_fn %s raised while profile cache scope was unresolved; dependent tools will be unavailable this turn', getattr(fn, '__qualname__', fn), exc_info=True)
            return False
    cache_key = (fn, scope)
    ...
    with _check_fn_cache_lock:
        ...
        if last_good is not None and now - last_good < _CHECK_FN_FAILURE_GRACE_SECONDS:
            logger.warning('check_fn %s failed (%s) within %.0fs of last success; treating as transient and keeping tool(s) available', getattr(fn, '__qualname__', fn), 'raised' if raised else 'returned False', _CHECK_FN_FAILURE_GRACE_SECONDS)
            return True
        logger.warning('check_fn %s %s; dependent tools will be unavailable this turn', getattr(fn, '__qualname__', fn), 'raised' if raised else 'returned False')
        _check_fn_cache[cache_key] = (now, False)
        return False

def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results. Call after config changes that
    affect tool availability (e.g. ``duck-agent tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def register_toolset_alias(self, alias: str, toolset: str) -> None:
        ...
        with self._lock:
            ...
            if existing and existing != toolset:
                logger.warning("Toolset alias collision: '%s' (%s) overwritten by %s", alias, existing, toolset)
            ...
            if existing and existing.toolset != toolset:
                if override:
                    ...
                    if _owner is not None and (not self._plugin_override_policy.get(_owner, False)):
                        logger.error('Tool registration REJECTED: plugin %r attempted to override built-in tool %r (existing toolset %r) without operator opt-in. Set plugins.entries.<plugin_id>.allow_tool_override: true in config.yaml to allow it.', _owner, name, existing.toolset)
                        raise PermissionError(f'Plugin module {_owner!r} cannot override built-in tool {name!r} without operator opt-in (allow_tool_override).')
                    logger.info("Tool '%s': toolset '%s' overriding existing toolset '%s' (override=True opt-in)", name, toolset, existing.toolset)
                else:
                    logger.error("Tool registration REJECTED: '%s' (toolset '%s') would shadow existing tool from toolset '%s'. Pass override=True to register() if the replacement is intentional, or deregister the existing tool first.", name, toolset, existing.toolset)
                    return
            self._tools[name] = ToolEntry(name=name, toolset=toolset, schema=schema, handler=handler, check_fn=check_fn, requires_env=requires_env or [], is_async=is_async, description=description or schema.get('description', ''), emoji=emoji, max_result_size_chars=max_result_size_chars, dynamic_schema_overrides=dynamic_schema_overrides)
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith('mcp-'):
                ...
                if caller_mod.startswith('hermes_plugins.') and (not same_plugin) and (not self._plugin_override_policy.get(caller_root, False)):
                    logger.error('Tool deregistration REJECTED: plugin %r attempted to remove tool %r (toolset %r) it does not own, without operator opt-in. Set plugins.entries.%s.allow_tool_override: true in config.yaml to allow it.', caller_mod, name, entry.toolset, caller_mod)
                    raise PermissionError(f'Plugin module {caller_mod!r} cannot deregister tool {name!r} (toolset {entry.toolset!r}) without operator opt-in (allow_tool_override).')
            del self._tools[name]
            toolset_still_exists = any((e.toolset == entry.toolset for e in self._tools.values()))
            if not toolset_still_exists:
                self._toolset_checks.pop(entry.toolset, None)
                self._toolset_aliases = {alias: target for alias, target in self._toolset_aliases.items() if target != entry.toolset}
            self._generation += 1
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool=False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``duck-agent tools enable foo``)
        ...
        for name in sorted(tool_names):
            ...
            if entry.dynamic_schema_overrides is not None:
                try:
                    ...
                    logger.warning('dynamic_schema_overrides for tool %s raised %s; using static schema', name, exc)
    ...
    def _normalize_handler_result(name: str, result):
        ...
        return tool_error(f'Tool handler returned unsupported result type: {result_type}', error_type='tool_result_contract', tool=name, result_type=result_type)
    ...
    def check_toolset_requirements(self) -> Dict[str, bool]:
        ...
        return {toolset: self._toolset_has_exposable_tools(toolset, entries) for toolset in toolsets}
    ...
    def get_available_toolsets(self) -> Dict[str, dict]:
        ...
        for entry in entries:
            ...
            if ts not in toolsets:
                toolsets[ts] = {'available': self._toolset_has_exposable_tools(ts, entries), 'tools': [], 'description': '', 'requirements': []}
    ...
    def get_toolset_requirements(self) -> Dict[str, dict]:
        ...
        for entry in entries:
            ts = entry.toolset
            if ts not in result:
                result[ts] = {'name': ts, 'env_vars': [], 'check_fn': toolset_checks.get(ts), 'setup_url': None, 'tools': []}
    ...
    def check_tool_availability(self, quiet: bool=False):
        ...
        for ts in sorted({entry.toolset for entry in entries}):
            ...
            if self._toolset_has_exposable_tools(ts, entries):
                ...
                unavailable.append({'name': ts, 'env_vars': ts_entries[0].requires_env if ts_entries else [], 'tools': [entry.name for entry in ts_entries]})
