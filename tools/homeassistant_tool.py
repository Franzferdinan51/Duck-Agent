...
def _get_config():
    ...
    return ((_HASS_URL or get_secret('HASS_URL', 'http://homeassistant.local:8123') or '').rstrip('/'), _HASS_TOKEN or get_secret('HASS_TOKEN', '') or '')
_ENTITY_ID_RE = re.compile('^[a-z_][a-z0-9_]*\\.[a-z0-9_]+$')
_SERVICE_NAME_RE = re.compile('^[a-z][a-z0-9_]*$')
...
def _filter_and_summarize(states: list, domain: Optional[str]=None, area: Optional[str]=None) -> Dict[str, Any]:
    ...
    if area:
        ...
        states = [s for s in states if area_lower in (s.get('attributes', {}).get('friendly_name', '') or '').lower() or area_lower in (s.get('attributes', {}).get('area', '') or '').lower()]
    entities = []
    for s in states:
        entities.append({'entity_id': s['entity_id'], 'state': s['state'], 'friendly_name': s.get('attributes', {}).get('friendly_name', '')})
...
async def _async_get_state(entity_id: str) -> Dict[str, Any]:
    ...
    return {'entity_id': data['entity_id'], 'state': data['state'], 'attributes': data.get('attributes', {}), 'last_changed': data.get('last_changed'), 'last_updated': data.get('last_updated')}
...
async def _async_call_service(domain: str, service: str, entity_id: Optional[str]=None, data: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=_get_headers(hass_token), json=payload, timeout=aiohttp.ClientTimeout(total=15)) as resp:
            resp.raise_for_status()
...
def _handle_call_service(args: dict, **kw) -> str:
    ...
    if domain in _BLOCKED_DOMAINS:
        return tool_error(f"Service domain '{domain}' is blocked for security. Blocked domains: {', '.join(sorted(_BLOCKED_DOMAINS))}")
    entity_id = args.get('entity_id')
...
registry.register(name='ha_list_entities', toolset='homeassistant', schema=HA_LIST_ENTITIES_SCHEMA, handler=_handle_list_entities, check_fn=_check_ha_available, emoji='🏠')
registry.register(name='ha_get_state', toolset='homeassistant', schema=HA_GET_STATE_SCHEMA, handler=_handle_get_state, check_fn=_check_ha_available, emoji='🏠')
registry.register(name='ha_list_services', toolset='homeassistant', schema=HA_LIST_SERVICES_SCHEMA, handler=_handle_list_services, check_fn=_check_ha_available, emoji='🏠')
registry.register(name='ha_call_service', toolset='homeassistant', schema=HA_CALL_SERVICE_SCHEMA, handler=_handle_call_service, check_fn=_check_ha_available, emoji='🏠')