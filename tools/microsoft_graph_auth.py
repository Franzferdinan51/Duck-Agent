...
class GraphCredentials:
    ...
    def from_env(cls, environ: dict[str, str] | None=None, *, required: bool=True) -> 'GraphCredentials | None':
        ...
        missing = [name for name, value in (('MSGRAPH_TENANT_ID', tenant_id), ('MSGRAPH_CLIENT_ID', client_id), ('MSGRAPH_CLIENT_SECRET', client_secret)) if not value]
        if missing:
            if not required:
                return None
            raise MicrosoftGraphConfigError(f"Missing Microsoft Graph configuration: {', '.join(missing)}")
        return cls(tenant_id=tenant_id, client_id=client_id, client_secret=client_secret, scope=scope, authority_url=authority_url)
...
class MicrosoftGraphTokenProvider:
    ...
    def __init__(self, credentials: GraphCredentials, *, timeout: float=20.0, skew_seconds: int=DEFAULT_TOKEN_SKEW_SECONDS, transport: httpx.AsyncBaseTransport | None=None) -> None:
    ...
    async def _fetch_access_token(self) -> CachedAccessToken:
        data = {'grant_type': 'client_credentials', 'client_id': self.credentials.client_id, 'client_secret': self.credentials.client_secret, 'scope': self.credentials.scope}
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        async with httpx.AsyncClient(timeout=httpx.Timeout(self.timeout), transport=self._transport) as client:
            response = await client.post(self.credentials.token_url, data=data, headers=headers)
        if response.status_code >= 400:
            detail = _extract_error_detail(response)
            raise MicrosoftGraphTokenError(f'Microsoft Graph token request failed with HTTP {response.status_code}: {detail}')
        try:
        ...
        try:
            ...
            raise MicrosoftGraphTokenError('Microsoft Graph token response did not include a valid expires_in.') from exc
        return CachedAccessToken(access_token=access_token, token_type=token_type, expires_at=time.time() + max(0, expires_in_seconds))
