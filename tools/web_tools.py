...
Backend is selected during ``duck-agent tools`` setup (web.backend in config.yaml).
When available, Duck Agent can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Duck Agent config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Duck Agent' config/.env layer (``duck-agent config set``, ``duck-agent tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.duck-agent/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``duck-agent tools``).
