...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.duck-agent/.env``.
...
def _managed_persistence_enabled() -> bool:
    """Return whether Duck Agent-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so Duck Agent operates in the same browser profile instead of creating a
    separate private session.
    """
    user_id = (get_secret('CAMOFOX_USER_ID', '') or '').strip() or str(camofox_cfg.get('user_id') or '').strip()
    if not user_id:
        return None
    session_key = (get_secret('CAMOFOX_SESSION_KEY', '') or '').strip() or str(camofox_cfg.get('session_key') or '').strip() or f"task_{(task_id or 'default')[:16]}"
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether Duck Agent should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for Duck Agent talking to the
...
def _loopback_rewrite_host(camofox_cfg: Dict[str, Any]) -> str:
    ...
    return os.getenv('CAMOFOX_LOOPBACK_HOST_ALIAS', '').strip() or str(camofox_cfg.get('loopback_host_alias') or '').strip() or 'host.docker.internal'
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside Duck Agent. Gateway
    ...
    matching_tabs = [tab for tab in tabs if isinstance(tab, dict) and tab.get('listItemId') == session_key]
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Duck Agent profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if identity_override:
            session = {'user_id': identity_override['user_id'], 'tab_id': None, 'session_key': identity_override['session_key'], 'managed': True, 'adopt_existing_tab': _adopt_existing_tab_enabled(camofox_cfg)}
        elif bool(camofox_cfg.get('managed_persistence')):
            identity = get_camofox_identity(task_id)
            session = {'user_id': identity['user_id'], 'tab_id': None, 'session_key': identity['session_key'], 'managed': True, 'adopt_existing_tab': _adopt_existing_tab_enabled(camofox_cfg)}
        else:
            session = {'user_id': f'hermes_{uuid.uuid4().hex[:10]}', 'tab_id': None, 'session_key': f'task_{task_id[:16]}', 'managed': False, 'adopt_existing_tab': False}
...
def _ensure_tab(task_id: Optional[str], url: str='about:blank') -> Dict[str, Any]:
    ...
    resp = requests.post(f'{base}/tabs', json={'userId': session['user_id'], 'listItemId': session['session_key'], 'url': url}, timeout=_get_command_timeout(), headers=_auth_headers())
...
def camofox_navigate(url: str, task_id: Optional[str]=None) -> str:
    ...
    try:
        ...
        if not session['tab_id']:
            ...
            try:
                ...
                if e.response is not None and e.response.status_code == 404:
                    logger.warning('Camofox tab %s returned 404 — tab was garbage collected. Creating a fresh tab.', session['tab_id'])
                    session['tab_id'] = None
                    session = _ensure_tab(task_id, browser_url)
                    data = {'ok': True, 'url': browser_url}
                else:
        ...
        if rewrite_info:
            ...
            result['warning'] = f"Rewrote loopback URL for Docker-hosted Camofox: {rewrite_info['from']} -> {rewrite_info['to']}"
        vnc = get_vnc_url()
        if vnc:
            result['vnc_url'] = vnc
            result['vnc_hint'] = 'Browser is visible via VNC. Share this link with the user so they can watch the browser live.'
        try:
        ...
        return json.dumps({'success': False, 'error': f'Cannot connect to Camofox at {get_camofox_url()}. Is the server running? Start with: npm start (in camofox-browser dir) or: docker run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser'})
...
def camofox_snapshot(full: bool=False, task_id: Optional[str]=None, user_task: Optional[str]=None) -> str:
    ...
    try:
        ...
        from tools.browser_tool import SNAPSHOT_SUMMARIZE_THRESHOLD, _extract_relevant_content, _truncate_snapshot
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith(('- img ', 'img ')):
                alt_match = re.search('img\\s+"([^"]*)"', stripped)
                alt = alt_match.group(1) if alt_match else ''
                src = ''
                if i + 1 < len(lines):
                    url_match = re.search('/url:\\s*(\\S+)', lines[i + 1].strip())
                    if url_match:
...
def camofox_vision(question: str, annotate: bool=False, task_id: Optional[str]=None) -> str:
    ...
    try:
        ...
        response = call_llm(messages=[{'role': 'user', 'content': [{'type': 'text', 'text': vision_prompt}, {'type': 'image_url', 'image_url': {'url': f'data:image/png;base64,{img_b64}'}}]}], task='vision', temperature=_vision_temperature, timeout=_vision_timeout)
...
def camofox_console(clear: bool=False, task_id: Optional[str]=None) -> str:
    ...
    """
    return json.dumps({'success': True, 'console_messages': [], 'js_errors': [], 'total_messages': 0, 'total_errors': 0, 'note': 'Console log capture is not available with the Camofox backend. Use browser_snapshot or browser_vision to inspect page state.'})