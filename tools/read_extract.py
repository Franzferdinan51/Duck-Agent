...
ANYDOC_EXTENSIONS = frozenset({'.doc', '.docm', '.ppt', '.pps', '.pot', '.pptx', '.pptm', '.ppsx', '.ppsm', '.xls', '.xlsm', '.xlsb', '.odt', '.ods', '.odp', '.rtf', '.epub', '.pdf'})
...
def _anydoc() -> Optional[Any]:
    ...
    with _anydoc_lock:
        if _anydoc_module is not _ANYDOC_UNSET:
            return _anydoc_module
        if _anydoc_failed_at is not None and time.monotonic() - _anydoc_failed_at < ANYDOC_RETRY_SECONDS:
            return None
        try:
...
def _extract_anydoc(path: str) -> str:
    ...
    if size > MAX_ANYDOC_BYTES:
        raise ExtractionError(f'Document too large to convert ({size:,} bytes, limit is {MAX_ANYDOC_BYTES:,})')
    try:
...
def _workbook_sheets(zf: zipfile.ZipFile) -> list[tuple[str, str, str]]:
    ...
    return [(sheet.get('name', 'Sheet'), sheet.get('state', 'visible'), sheet.get(f'{r}id', '')) for sheet in root.iter(f'{s}sheet')]
