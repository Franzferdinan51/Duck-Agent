...
Stores per-server tool manifests on disk so Duck Agent can register MCP tools
...
def config_fingerprint(config: dict) -> str:
    ...
    payload = {'command': config.get('command'), 'args': config.get('args') or [], 'url': config.get('url'), 'transport': config.get('transport'), 'tools_include': sorted(tools_filter.get('include') or []), 'tools_exclude': sorted(tools_filter.get('exclude') or [])}
...
def _save_all(data: Dict[str, Any]) -> None:
    ...
    atomic_json_write(_cache_path(), data, mode=384)
...
def write_cache_entry(server_name: str, fingerprint: str, *, tools: List[dict], utility_tools: Optional[List[dict]]=None) -> None:
