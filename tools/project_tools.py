...
def project_list(task_id: Optional[str]=None) -> str:
    ...
    return json.dumps({'active_id': active, 'projects': [{'id': p.id, 'slug': p.slug, 'name': p.name, 'primary_path': _primary_path(p), 'active': p.id == active} for p in projects]})
