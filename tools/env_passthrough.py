...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Duck Agent-managed provider credential (API key,
    ...
    Non-Duck Agent API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Duck Agent credential into the execute_code child.
    """
    try:
        from tools.environments.local import _HERMES_PROVIDER_ENV_BLOCKLIST, _is_hermes_internal_secret
    except Exception as e:
        logger.warning('env passthrough: provider credential blocklist import failed; failing closed and refusing passthrough registration for %r: %s', name, e)
        return True
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Duck Agent-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Duck Agent-managed
    ...
    Non-Duck Agent third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning("env passthrough: refusing to register Duck Agent provider credential %r (blocked by _HERMES_PROVIDER_ENV_BLOCKLIST). Skills must not override the execute_code sandbox's credential scrubbing; see GHSA-rhgp-j443-p4rf.", name)
            continue
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                name = item.strip()
                if _is_hermes_provider_credential(name):
                    logger.warning("env passthrough: refusing to register Duck Agent provider credential %r from config.yaml (blocked by _HERMES_PROVIDER_ENV_BLOCKLIST). Operator configuration must not override the execute_code sandbox's credential scrubbing; see GHSA-rhgp-j443-p4rf.", name)
                    continue
                result.add(name)
    except Exception as e:
...
def resolve_passthrough_value(name: str, fallback: str | None=None) -> str | None:
    ...
    """
    from agent.secret_scope import _is_global_env, current_secret_scope, get_secret, is_multiplex_active
