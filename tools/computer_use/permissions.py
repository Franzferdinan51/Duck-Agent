...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT Duck Agent — so no Duck Agent entitlement is
...
``duck-agent computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def _run(binary: str, *args: str, timeout: float) -> subprocess.CompletedProcess:
    return subprocess.run([binary, *args], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, env=_child_env(), stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
...
def _doctor(binary: str) -> Optional[Dict[str, Any]]:
    ...
    if not isinstance(data, dict):
        return None
    checks: List[Dict[str, str]] = [{'label': str(p.get('label', '')), 'status': str(p.get('status', '')), 'message': str(p.get('message', ''))} for p in data.get('probes', []) if isinstance(p, dict)]
...
def computer_use_status(driver_cmd: Optional[str]=None) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {'platform': plat, 'platform_supported': plat in _RUNTIME_PLATFORMS, 'installed': bool(binary), 'version': None, 'ready': None, 'can_grant': plat == 'darwin', 'checks': [], 'source': None, 'error': None, **{k: None for k in _BOOLS}}
    if not binary:
        return out
    try:
