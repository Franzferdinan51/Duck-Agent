...
* the driver session id is injected by the adapter, never accepted from the
  model;
* refs are usable only from the latest snapshot in this Duck Agent session;
...
ordinary Duck Agent tool approval never substitutes for protected consent.
"""
...
def _ref_map(payload: Dict[str, Any]) -> Dict[str, Set[str]]:
    ...
    for key, value in entries:
        ...
        if not isinstance(ref, str) or not ref:
            continue
        normalized[ref] = {action for action in actions or [] if isinstance(action, str)}
    return normalized
...
def _refusal(code: str, message: str, *, native_fallback: bool=False, **extra: Any) -> Dict[str, Any]:
    payload: Dict[str, Any] = {'ok': False, 'status': 'refused', 'code': code, 'message': message}
    if native_fallback:
...
class CuaTypedBrowserRoute:
    ...
    def __init__(self, *, session_id: str, call_tool: ToolCaller, has_tool: ToolProbe) -> None:
    ...
    def _require_tool(self, name: str) -> Optional[Dict[str, Any]]:
        if self._has_tool(name):
            return None
        return _refusal('typed_browser_unavailable', f'The connected cua-driver does not advertise {name}; use the native AX/PX/foreground ladder.', native_fallback=True)

    def observe(self, *, pid: Any=None, window_id: Any=None, tab_id: Optional[str]=None, snapshot_format: str='semantic_v2', query: Optional[str]=None, scope_ref: Optional[str]=None, continuation: Optional[str]=None) -> Dict[str, Any]:
        ...
        if binding_request:
            ...
            if exact_pid is None or exact_window is None:
                return _refusal('browser_exact_target_required', 'Typed browser binding requires an exact positive pid and window_id pair.', native_fallback=True)
            payload = self._call('get_browser_state', {'pid': exact_pid, 'window_id': exact_window})
            ...
            if not isinstance(target_id, str) or not target_id:
                return _refusal('browser_binding_unproven', 'Browser bind returned no opaque target capability; use native control.', native_fallback=True)
        ...
        if not target_id or self.state.binding_quality != 'exact':
            return _refusal('browser_exact_binding_required', 'Bind the exact native pid/window_id before reading a browser tab.', native_fallback=True)
        selected_tab = tab_id or self.state.tab_id
        if not isinstance(selected_tab, str) or not selected_tab:
            return _refusal('browser_tab_required', 'Choose an opaque tab_id returned by the exact bind.')
        if selected_tab not in self.state.tab_ids:
            return _refusal('browser_tab_unbound', "The requested tab_id was not minted by this session's exact bind.")
        if continuation is not None and continuation != self.state.continuation:
            return _refusal('browser_continuation_stale', 'The continuation is not current for this session/tab; take a fresh snapshot.')
        if scope_ref is not None and scope_ref not in self.state.refs:
            return _refusal('browser_ref_stale', "scope_ref must come from this session's latest browser snapshot.")
        args: Dict[str, Any] = {'target_id': target_id, 'tab_id': selected_tab, 'snapshot_format': snapshot_format}
        if query:
            args['query'] = query
        if scope_ref:
    ...
    def prepare(self, *, pid: Any, window_id: Any=None, profile_mode: str, profile_name: Optional[str]=None, allow_launch: bool=False) -> Dict[str, Any]:
        ...
        if exact_pid is None:
            return _refusal('browser_pid_required', 'browser_prepare requires a positive pid.')
        if profile_mode == 'existing_profile':
            exact_window = _positive_int(window_id)
            if exact_window is None:
                return _refusal('browser_exact_target_required', 'Existing-profile attachment requires an exact positive pid and window_id pair.')
            self.state.clear()
            return self._call('browser_prepare', {'pid': exact_pid, 'window_id': exact_window, 'strategy': {'kind': 'existing_profile'}})
        if profile_mode not in {'isolated_new', 'isolated_named'}:
            return _refusal('browser_profile_mode_invalid', 'Use isolated_new, isolated_named, or existing_profile.')
        if not allow_launch:
            return _refusal('browser_launch_not_approved', 'Driver-owned isolated setup requires explicit allow_launch=true.')
        profile: Dict[str, Any] = {'mode': profile_mode}
        if profile_mode == 'isolated_named':
            if not isinstance(profile_name, str) or not profile_name:
                return _refusal('browser_profile_name_required', 'isolated_named requires a non-empty profile name.')
            profile['name'] = profile_name
        args: Dict[str, Any] = {'pid': exact_pid, 'allow_launch': True, 'profile': profile}
        exact_window = _positive_int(window_id)
    ...
    def _require_mutation(self, *, tool: str, tab_id: Optional[str], allow_without_snapshot: bool=False) -> tuple[Optional[str], Optional[Dict[str, Any]]]:
        missing = self._require_tool(tool)
        if missing is not None:
            return (None, missing)
        if not self.state.target_id or self.state.binding_quality != 'exact' or (not self.state.mutation_allowed):
            return (None, _refusal('browser_mutation_unproven', 'Typed browser mutation requires status=ok, binding_quality=exact, and mutation_allowed=true; use native control otherwise.', native_fallback=True))
        selected_tab = tab_id or self.state.tab_id
        if not isinstance(selected_tab, str) or not selected_tab:
            return (None, _refusal('browser_tab_required', 'Choose a bound tab_id first.'))
        if selected_tab not in self.state.tab_ids:
            return (None, _refusal('browser_tab_unbound', "The requested tab_id was not minted by this session's exact bind."))
        if self.state.verification_required and (not allow_without_snapshot):
            return (None, _refusal('browser_verification_required', 'Take a fresh cua_browser_state snapshot before another browser mutation.'))
        return (selected_tab, None)

    def _require_ref(self, ref: Any, *, actions: Set[str]) -> Optional[Dict[str, Any]]:
        if not isinstance(ref, str) or ref not in self.state.refs:
            return _refusal('browser_ref_stale', 'Use a current ref from the latest cua_browser_state snapshot.')
        declared = self.state.refs[ref]
        if actions and (not declared.intersection(actions)):
            return _refusal('browser_action_unavailable', 'The current ref does not declare the requested browser action.')
        return None

    def mutate(self, tool: str, *, tab_id: Optional[str]=None, args: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        """Invoke one typed browser tool against current capabilities."""
        call_args = dict(args or {})
        dialog_inspect = tool == 'browser_dialog' and call_args.get('action') == 'inspect'
        selected_tab, refusal = self._require_mutation(tool=tool, tab_id=tab_id, allow_without_snapshot=dialog_inspect)
        ...
        if requested_route is not None and (not supports_trust_choice):
            return _refusal('browser_input_route_unsupported', f'{tool} does not expose a trust-route choice in the live 0.9 schema.')
        route = requested_route or 'trusted'
        if route not in {'trusted', 'dom_event'}:
            return _refusal('browser_input_route_invalid', 'Use input_route=trusted or explicitly request dom_event.')
        if route == 'dom_event' and (not ref):
            return _refusal('browser_dom_event_ref_required', 'The dom_event trust class requires a current semantic ref.')
        ...
        if tool == 'browser_click' and ref:
            ...
            required_actions = {'scroll', 'pointer'} if pointer_action == 'scroll' else {'pointer'}
        ...
        if destination_ref is not None:
            invalid_destination = self._require_ref(destination_ref, actions={'pointer', 'drag', 'drop'})
        ...
        code = _refusal_code(payload)
        refused = payload.get('isError') is True or payload.get('status') not in (None, 'ok') or code is not None
        ...
        if refused:
            payload['native_fallback_available'] = True
            if dialog_inspect and code in {'browser_ref_stale', 'browser_binding_ambiguous'}:
                self.state.clear_refs()
                self.state.verification_required = True
