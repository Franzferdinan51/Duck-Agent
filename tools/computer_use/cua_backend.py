...
etc.) surface as specific blocked checks via `duck-agent computer-use doctor`
...
def _action_result_from(name: str, ok: bool, message: str, meta: Dict[str, Any], structured: Dict[str, Any], *, requested_delivery: Optional[str]=None) -> ActionResult:
    ...
    cua-driver's mcp-tool-notes and NousResearch/duck-agent#67052.
    """
    ...
    return ActionResult(ok=ok, action=name, message=message, meta=meta, verified=verified, effect=effect, escalation=escalation, path=path, degraded=degraded, delivery_mode=delivery_mode, code=code)
...
_NON_APP_WINDOW_TITLE_PREFIXES = ('@!', 'Desktop', 'gnome-shell', 'GNOME Shell')
...
def _cua_no_overlay() -> bool:
    """True when Duck Agent should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Duck Agent should disable cua-driver telemetry for this user.
...
def _parse_xprop_net_active_window(stdout: str) -> Optional[int]:
    ...
    """
    text = stdout or ''
    match = re.search('window id # (0x[0-9a-fA-F]+)', text)
    if not match:
        match = re.search('(0x[0-9a-fA-F]+)', text)
    if not match:
        return None
    try:
...
def _linux_x11_active_window_id() -> Optional[int]:
    ...
    if sys.platform != 'linux' or not os.environ.get('DISPLAY'):
        return None
    try:
        proc = subprocess.run(['xprop', '-root', '_NET_ACTIVE_WINDOW'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=2, check=False)
    except Exception:
        return None
...
def _select_capture_target(windows: List[Dict[str, Any]], *, app_requested: bool, exact_target: bool=False) -> Dict[str, Any]:
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Duck Agent runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Duck Agent process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Duck Agent session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == 'win32':
            self.socket_path = f'\\\\.\\pipe\\duck-agent-cua-{token}'
        else:
    ...
    def start(self) -> None:
        ...
        command = [self._command, 'serve', '--embedded', '--socket', self.socket_path, '--no-permissions-gate', '--permission-mode', 'unrestricted', '--dangerously-bypass-approvals']
        self._process = subprocess.Popen(command, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, env=env)
        self._stderr_thread = threading.Thread(target=self._drain_stderr, args=(self._process,), name='duck-agent-cua-daemon-stderr', daemon=True)
        ...
        while time.monotonic() < deadline:
            ...
            try:
                probe = subprocess.run([self._command, 'status', '--socket', self.socket_path], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=2.0, env=env)
    ...
    def stop(self) -> None:
        ...
        if process is not None and process.poll() is None:
            from tools.environments.local import _sanitize_subprocess_env
            try:
                subprocess.run([self._command, 'stop', '--socket', self.socket_path], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3.0, env=_sanitize_subprocess_env(self.child_env()))
            except (OSError, subprocess.SubprocessError):
                pass
            try:
...
def _resolve_mcp_invocation(driver_cmd: str, *, timeout: float=6.0) -> Tuple[str, List[str]]:
    ...
    Surface 8 of NousResearch/duck-agent#47072: instead of hardcoding
    ...
    try:
        from tools.environments.local import _sanitize_subprocess_env
        proc = subprocess.run([driver_cmd, 'manifest'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags(), env=_sanitize_subprocess_env(cua_driver_child_env()))
    except Exception:
...
def _cua_driver_supports_no_overlay(driver_cmd: str) -> bool:
    ...
    try:
        from tools.environments.local import _sanitize_subprocess_env
        proc = subprocess.run([driver_cmd, '--help'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=3.0, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags(), env=_sanitize_subprocess_env(cua_driver_child_env()))
        help_text = (proc.stdout or '') + (proc.stderr or '')
        return '--no-overlay' in help_text
    except Exception:
        return False
_ELEMENT_LINE_RE = re.compile('^\\s*(?:-\\s+)?\\[(\\d+)\\]\\s+(\\w+)(?:\\s*=\\s*"([^"]*)"|\\s+"([^"]*)"|\\s+\\((?!\\d+\\))([^)]*)\\))?(?:\\s+(?:\\(\\d+\\)\\s+)?id=([^\\s\\[\\]]+))?', re.MULTILINE)
...
def _candidate_cua_driver_commands(override: Optional[str]=None) -> List[str]:
    ...
    Duck Agent Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `duck-agent computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == 'win32':
        candidates.extend([os.path.join(home, '.local', 'bin', 'cua-driver.exe'), os.path.join(home, '.local', 'bin', 'cua-driver')])
    else:
        candidates.extend([os.path.join(home, '.local', 'bin', 'cua-driver'), os.path.join(home, '.cargo', 'bin', 'cua-driver'), '/opt/homebrew/bin/cua-driver', '/usr/local/bin/cua-driver'])
    return candidates
...
def cua_driver_update_check(*, timeout: Optional[float]=None) -> Optional[Dict[str, Any]]:
    ...
    try:
        from tools.environments.local import _sanitize_subprocess_env
        proc = subprocess.run([driver_cmd, 'check-update', '--json'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags(), env=_sanitize_subprocess_env(cua_driver_child_env()))
    except Exception:
        return None
    out = (proc.stdout or '').strip()
    if not out:
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return f'cua-driver {latest} is available (you have {current}); update with `duck-agent computer-use install --upgrade`.'
...
def cua_driver_install_hint() -> str:
    if sys.platform == 'win32':
        installer = '  irm https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.ps1 | iex'
    else:
        installer = '  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh)"'
    return f'cua-driver is not installed. Install with one of:\n  duck-agent computer-use install\nOr run the upstream installer directly:\n{installer}\nOr run `duck-agent tools` and enable the Computer Use toolset to install it automatically.'
...
def _parse_elements_from_tree(markdown: str) -> List[UIElement]:
    ...
    for m in _ELEMENT_LINE_RE.finditer(markdown):
        ...
        elements.append(UIElement(index=int(m.group(1)), role=m.group(2), label=label, bounds=(0, 0, 0, 0)))
    return elements

def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/duck-agent#47072: read the canonical
...
def _image_dimensions_from_bytes(raw: bytes) -> Tuple[int, int]:
    ...
    if raw.startswith(b'\xff\xd8'):
        ...
        while i + 9 < n:
            ...
            marker = raw[i + 1]
            i += 2
            if marker in {216, 217} or 208 <= marker <= 215:
                continue
            if i + 2 > n:
                break
...
def _parse_key_combo(keys: str) -> Tuple[Optional[str], List[str]]:
    ...
    parts = [p.strip().lower() for p in re.split('[+\\-]', keys) if p.strip()]
    modifiers = []
    key = None
    for part in parts:
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge, embedded_daemon: Optional[_EmbeddedCuaDaemon]=None) -> None:
        self._bridge = bridge
        ...
        try:
            ...
            async with stdio_client(params) as (read, write):
                ...
                async with ClientSession(read, write) as session:
                    ...
                    logger.info('cua-driver session ready in %.1fs (manifest=%.1fs, mcp_init=%.1fs)', _time.monotonic() - _t0, _t_manifest - _t0, _t_init - _t_manifest)
        ...
        try:
            ...
            for tool in getattr(tools_list, 'tools', []) or []:
                ...
                if isinstance(caps, list):
                    self._capabilities[tool_name] = {c for c in caps if isinstance(c, str)}
                else:
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(f'cua-driver session never reached ready (timeout 30s; stuck in phase: {phase}). Run `duck-agent computer-use doctor` and check {display_hermes_home()}/logs/agent.log for the phase timings.')
    ...
    def _is_ended_session_result(cls, result: Any) -> bool:
        ...
        return 'session' in message and ('has ended' in message or 'session ended' in message) and ('start_session' in message)

    def _revive_declared_session_once(self, name: str, args: Dict[str, Any], first_result: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        revive_result = self._bridge.run(self._call_tool_async('start_session', {'session': session_id}), timeout=timeout)
        if revive_result.get('isError') is True:
            logger.warning('cua-driver session %s could not be revived: %s', session_id, self._logical_error_text(revive_result))
            return first_result
    ...
    def _is_closed_session_error(exc: Exception) -> bool:
        ...
        return name in {'ClosedResourceError', 'BrokenResourceError', 'EndOfStream'} or (module.startswith('anyio') and 'Resource' in name) or isinstance(exc, (BrokenPipeError, EOFError))
    ...
    def _is_transient_daemon_error(exc: Exception) -> bool:
        ...
        """
        msg = str(exc)
        return 'Resource temporarily unavailable' in msg or 'os error 35' in msg or 'daemon transport error' in msg or ('daemon proxy' in msg)
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                try:
                    proc = _subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=max(15.0, timeout), creationflags=windows_hide_flags(), env=_sanitize_subprocess_env(child_env))
                except Exception as e:
                ...
                if attempt < attempts - 1:
                    logger.warning('cua-driver CLI fallback for %s got no JSON (attempt %d/%d); retrying in %.1fs', name, attempt + 1, attempts, backoff)
                    _time.sleep(backoff)
                    backoff *= 2
            if parsed is None:
                raise RuntimeError(f'cua-driver CLI fallback for {name} returned no JSON after {attempts} attempts: {last_err}')
            images: List[str] = []
            data: Any = None
    ...
    def call_tool(self, name: str, args: Dict[str, Any], timeout: float=30.0) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._is_transient_daemon_error(e):
                logger.warning('cua-driver MCP transport failed on %s (%s); falling back to CLI transport', name, e)
        ...
        if name == 'end_session' and result.get('isError') is not True and (args.get('session') == self._declared_session_id):
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/duck-agent#47072). Each entry corresponds index-for-index
    ...
    if text_chunks:
        ...
        try:
            ...
            data = joined
    return {'data': data, 'images': images, 'image_mime_types': image_mime_types, 'structuredContent': structured, 'isError': is_error}
...
def _image_from_tool_result(out: Dict[str, Any]) -> tuple[Optional[str], Optional[str]]:
    ...
    if b64:
        mime = structured.get('screenshot_mime_type') or structured.get('mime_type') or None
        return (b64, mime)
    return (None, None)
...
def _ingest_windows(raw_windows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for w in raw_windows:
        ...
        title = w.get('title', '')
        windows.append({'app_name': app_name if isinstance(app_name, str) else '', 'pid': pid_int, 'window_id': window_id_int, 'off_screen': w.get('is_on_screen') is False, 'title': title if isinstance(title, str) else '', 'z_index': z_index})
    return windows
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str='standard') -> None:
        ...
        self.permission_mode = permission_mode
        self._embedded_daemon = _EmbeddedCuaDaemon(resolve_cua_driver_cmd() or '', permission_mode) if permission_mode == 'unrestricted' else None
        ...
        self._session_id: str = f'duck-agent-{uuid.uuid4().hex[:12]}'
        self._typed_browser = CuaTypedBrowserRoute(session_id=self._session_id, call_tool=self._session.call_tool, has_tool=self._session._has_tool)
    ...
    def _browser_route(self) -> CuaTypedBrowserRoute:
        ...
        if route is None:
            route = CuaTypedBrowserRoute(session_id=self._session_id, call_tool=self._session.call_tool, has_tool=self._session._has_tool)
    ...
    def _failed_capture(self, mode: str, message: str='') -> CaptureResult:
        ...
        return CaptureResult(mode=mode, width=0, height=0, png_b64=None, elements=[], app='', window_title=message, png_bytes_len=0)
    ...
    def _call_capture_tool(self, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if out.get('isError') is True:
            ...
            self._clear_active_target()
            raise RuntimeError(f'cua-driver {name} failed' + (f': {message}' if isinstance(message, str) and message else ''))
        return out
    ...
    def _load_windows(self) -> List[Dict[str, Any]]:
        ...
        """
        out = self._call_capture_tool('list_windows', {'on_screen_only': True, 'session': self._session_id})
        ...
        try:
            cli_out = self._session._call_tool_via_cli('list_windows', {'on_screen_only': True, 'session': self._session_id}, 20.0)
    ...
    def _match_windows_for_app(self, windows: List[Dict[str, Any]], app: str) -> List[Dict[str, Any]]:
        ...
        if not app_lower:
            return []
        direct_exact = [w for w in windows if app_lower == str(w.get('app_name', '')).strip().lower()]
        if direct_exact:
            return direct_exact
        try:
        ...
        for raw_app in running_apps:
            ...
            if pid <= 0:
                continue
            aliases = {value.strip().lower() for key in ('bundle_id', 'bundleId', 'name', 'app_name', 'display_name') if isinstance((value := raw_app.get(key)), str) and value.strip()}
        ...
        direct_partial = [w for w in windows if app_lower in str(w.get('app_name', '')).lower()]
        if direct_partial:
            return direct_partial
        metadata_partial = [w for w in windows if w.get('pid') in partial_pids]
        if metadata_partial:
            return metadata_partial
        return [w for w in windows if not str(w.get('app_name', '')).strip() and app_lower in str(w.get('title', '')).lower()]
    ...
    def capture(self, mode: str='som', app: Optional[str]=None, pid: Optional[int]=None, window_id: Optional[int]=None) -> CaptureResult:
        ...
        Maps duck-agent `capture(mode, app)` → cua-driver `list_windows` +
        ...
        if pid is not None or window_id is not None:
            ...
            windows = [{'app_name': app or '', 'pid': target_pid, 'window_id': target_window_id, 'off_screen': False, 'title': '', 'z_index': 0}]
        else:
            try:
        ...
        if pid is None and window_id is None and app and (app.strip().lower() in _SCREEN_CAPTURE_SENTINELS):
            ...
            if not desktop:
                return self._failed_capture(mode, f"<no desktop/shell window found for app={app!r}; cua-driver captures one window at a time and exposes no whole-virtual-desktop or per-monitor capture. Call list_apps / capture(app='<AppName>') to target a specific window instead. On Windows the taskbar is 'Shell_TrayWnd' and the desktop is 'Progman'.>")
            windows = sorted(desktop, key=lambda w: 0 if any((n in f"{w.get('app_name', '')} {w.get('title', '')}".lower() for n in ('progman', 'workerw', 'program manager', 'finder', 'desktop'))) else 1)
        elif pid is None and window_id is None and app:
            filtered = self._match_windows_for_app(windows, app)
            if not filtered:
                return self._failed_capture(mode, f"<no on-screen window matched app={app!r}; call list_apps to see available app names or bundle IDs (macOS reports localized names, e.g. '計算機' instead of 'Calculator'; some Linux/Qt apps only resolve via list_apps metadata)>")
            windows = filtered
        target = _select_capture_target(windows, app_requested=bool(app), exact_target=pid is not None or window_id is not None)
        ...
        window_title = ''
        if mode == 'vision':
            use_screenshot = self._session._has_tool('screenshot') or not self._session.capabilities_discovered
            sc_out: Optional[Dict[str, Any]] = None
            if use_screenshot:
                sc_out = self._call_capture_tool('screenshot', {'window_id': self._active_window_id, 'format': 'jpeg', 'quality': 85, 'session': self._session_id})
                png_b64, image_mime_type = _image_from_tool_result(sc_out)
                if not png_b64:
                    sc_out = None
            if sc_out is None:
                gws_out = self._call_capture_tool('get_window_state', {'pid': self._active_pid, 'window_id': self._active_window_id, 'session': self._session_id})
                png_b64, image_mime_type = _image_from_tool_result(gws_out)
                text = gws_out['data'] if isinstance(gws_out['data'], str) else ''
                _, tree = _split_tree_text(text)
                wt = re.search('AXWindow\\s+"([^"]+)"', tree)
                if wt:
                    window_title = wt.group(1)
            if not png_b64:
                logger.warning('cua-driver vision capture returned no image over MCP (window_id=%s); re-fetching via CLI transport', self._active_window_id)
                try:
                    cli_out = self._session._call_tool_via_cli('get_window_state', {'pid': self._active_pid, 'window_id': self._active_window_id, 'session': self._session_id}, 30.0)
                    if cli_out.get('isError') is True:
                        self._clear_active_target()
                    ...
                    logger.error('cua-driver CLI re-fetch for vision screenshot failed: %s', cli_exc)
        else:
            gws_out = self._call_capture_tool('get_window_state', {'pid': self._active_pid, 'window_id': self._active_window_id, 'session': self._session_id})
            ...
            if _gws_is_empty(gws_out):
                logger.warning('cua-driver get_window_state returned an empty result over MCP (pid=%s window_id=%s); re-fetching via CLI transport', self._active_pid, self._active_window_id)
                try:
                    cli_out = self._session._call_tool_via_cli('get_window_state', {'pid': self._active_pid, 'window_id': self._active_window_id, 'session': self._session_id}, 30.0)
                    if cli_out.get('isError') is True:
                        self._clear_active_target()
                    ...
                    logger.error('cua-driver CLI re-fetch for get_window_state failed: %s', cli_exc)
            text = gws_out['data'] if isinstance(gws_out['data'], str) else ''
        ...
        return CaptureResult(mode=mode, width=width, height=height, png_b64=png_b64, elements=elements, app=app_name, window_title=window_title, png_bytes_len=png_bytes_len, image_mime_type=image_mime_type)
    ...
    def _run_input_action(self, action: str, args: Dict[str, Any], delivery_mode: Optional[str], bring_to_front: bool) -> ActionResult:
        ...
        if bring_to_front:
            if delivery_mode != 'foreground':
                return ActionResult(ok=False, action=action, code='bring_to_front_requires_foreground', message="bring_to_front requires delivery_mode='foreground'.")
            if not self._session._has_tool('bring_to_front'):
                return ActionResult(ok=False, action=action, code='bring_to_front_unsupported', delivery_mode='foreground', message='The connected cua-driver does not advertise the standalone bring_to_front tool.')
            if self._active_pid is None or self._active_window_id is None:
                return ActionResult(ok=False, action=action, code='bring_to_front_target_required', delivery_mode='foreground', message='Capture an exact target before requesting persistent foreground focus.')
    ...
    def click(self, *, element: Optional[int]=None, x: Optional[int]=None, y: Optional[int]=None, button: str='left', click_count: int=1, modifiers: Optional[List[str]]=None, delivery_mode: Optional[str]=None, bring_to_front: bool=False) -> ActionResult:
        ...
        if button_norm not in {'left', 'right', 'middle'}:
            return ActionResult(ok=False, action='click', message=f'unknown button {button!r} — expected left, right, middle.')
        ...
        if element is not None:
            ...
            if self._active_window_id is None:
                return ActionResult(ok=False, action=tool, message='No active window_id for coordinate click.')
            args['x'] = x
            args['y'] = y
            args['window_id'] = self._active_window_id
        else:
    ...
    def drag(self, *, from_element: Optional[int]=None, to_element: Optional[int]=None, from_xy: Optional[Tuple[int, int]]=None, to_xy: Optional[Tuple[int, int]]=None, button: str='left', modifiers: Optional[List[str]]=None, delivery_mode: Optional[str]=None, bring_to_front: bool=False) -> ActionResult:
        ...
        if from_element is not None and to_element is not None:
            ...
            if self._active_window_id is None:
                return ActionResult(ok=False, action='drag', message='No active window_id for coordinate drag.')
    ...
    def scroll(self, *, direction: str, amount: int=3, element: Optional[int]=None, x: Optional[int]=None, y: Optional[int]=None, modifiers: Optional[List[str]]=None, delivery_mode: Optional[str]=None, bring_to_front: bool=False) -> ActionResult:
        ...
        args: Dict[str, Any] = {'pid': pid, 'direction': direction, 'amount': max(1, min(50, amount))}
        if element is not None and self._active_window_id is not None:
            args['element_index'] = element
            args['window_id'] = self._active_window_id
        elif x is not None and y is not None:
            if self._active_window_id is None:
                return ActionResult(ok=False, action='scroll', message='No active window_id for coordinate scroll.')
    ...
    def key(self, keys: str, *, delivery_mode: Optional[str]=None, bring_to_front: bool=False) -> ActionResult:
        ...
        if modifiers:
            args: Dict[str, Any] = {'pid': pid, 'window_id': window_id, 'keys': modifiers + [key_name]}
    ...
    def set_value(self, value: str, element: Optional[int]=None) -> ActionResult:
        ...
        args: Dict[str, Any] = {'pid': pid, 'window_id': window_id, 'element_index': element, 'value': value}
    ...
    def focus_app(self, app: str, raise_window: bool=False) -> ActionResult:
        ...
        separately approved by the Duck Agent adapter, and uses cua-driver's
        ...
        if target:
            ...
            self._active_window_id = target['window_id']
            self._snapshot_tokens = {}
            self._last_app = target['app_name'] or app
            self._last_target = {'pid': self._active_pid, 'window_id': self._active_window_id}
            if raise_window:
                if not self._session._has_tool('bring_to_front'):
                    return ActionResult(ok=False, action='focus_app', code='bring_to_front_unsupported', message='The connected cua-driver does not advertise the standalone bring_to_front tool.')
                focused = self.bring_to_front(pid=self._active_pid, window_id=self._active_window_id)
                if not focused.ok:
                    return focused
                ...
                return focused
            return ActionResult(ok=True, action='focus_app', message=f"Targeted {target['app_name']} (pid {self._active_pid}, window {self._active_window_id}) without raising window.")
        self._clear_active_target()
    ...
    def get_cursor_position(self) -> Tuple[int, int]:
        ...
        (origin top-left)."""
        out = self._session.call_tool('get_cursor_position', {'session': self._session_id})
    ...
    def zoom(self, *, window_id: int, x: float, y: float, w: float, h: float, factor: float=1.0, format: str='jpeg', quality: int=85) -> Dict[str, Any]:
        ...
        return self._session.call_tool('zoom', {'window_id': int(window_id), 'x': float(x), 'y': float(y), 'w': float(w), 'h': float(h), 'factor': float(factor), 'format': format, 'quality': int(quality), 'session': self._session_id})
    ...
    def start_recording(self, *, output_dir: str, record_video: bool=False) -> Dict[str, Any]:
        ...
        out = self._session.call_tool('start_recording', {'output_dir': output_dir, 'record_video': bool(record_video), 'session': self._session_id})
        return out.get('structuredContent') or {}
    ...
    def stop_recording(self) -> Dict[str, Any]:
        ...
        out = self._session.call_tool('stop_recording', {'session': self._session_id})
        return out.get('structuredContent') or {}
    ...
    def get_recording_state(self) -> Dict[str, Any]:
        ...
        out = self._session.call_tool('get_recording_state', {'session': self._session_id})
        return out.get('structuredContent') or {}
    ...
    def replay_trajectory(self, *, trajectory_dir: str, dry_run: bool=False, speed_factor: float=1.0) -> Dict[str, Any]:
        ...
        return self._session.call_tool('replay_trajectory', {'trajectory_dir': trajectory_dir, 'dry_run': bool(dry_run), 'speed_factor': float(speed_factor), 'session': self._session_id})
    ...
    def get_accessibility_tree(self) -> Dict[str, Any]:
        ...
        out = self._session.call_tool('get_accessibility_tree', {'session': self._session_id})
    ...
    def page(self, *, pid: int, action: str, **page_args: Any) -> Dict[str, Any]:
        ...
        args: Dict[str, Any] = {'pid': int(pid), 'action': action, 'session': self._session_id}
    ...
    def _action(self, name: str, args: Dict[str, Any], *, inject_session: bool=True) -> ActionResult:
        ...
        return _action_result_from(name, ok, message, meta, structured, requested_delivery=args.get('delivery_mode'))