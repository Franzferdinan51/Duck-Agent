...
from tools.managed_tool_gateway import build_managed_media_uploader, managed_gateway_auth_headers, managed_vendor_endpoints, peek_nous_access_token, read_nous_access_token
logger = logging.getLogger(__name__)
_TOOLSET = 'bfl'
_VENDOR = 'bfl'
_TRANSPORT_READ_TIMEOUT_SECONDS = 180.0
_TRANSPORT_CONNECT_TIMEOUT_SECONDS = 10.0
_SIGN_IN_MESSAGE = 'BFL video generation needs a Nous Portal sign-in. Ask the user to run `duck-agent model` and sign in to Nous, then retry.'
_WINDOWS_DRIVE_PATH = re.compile('^[A-Za-z]:[\\\\/]')
_BASE64_PAYLOAD = re.compile('^[A-Za-z0-9+/\\r\\n]+={0,2}[\\r\\n]*$')
...
async def _call_gateway(method: str, url: str, json_body: Optional[dict]=None, read_timeout: Optional[float]=None) -> str:
    ...
    timeout = httpx.Timeout(_TRANSPORT_CONNECT_TIMEOUT_SECONDS, read=_TRANSPORT_READ_TIMEOUT_SECONDS if read_timeout is None else read_timeout)
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            response = await client.request(method, url, headers=headers, json=json_body)
    except Exception as exc:
        return json.dumps({'error': f'Could not reach the video-generation gateway: {type(exc).__name__}: {exc}', 'transport_error': True})
    ...
    if not isinstance(payload, dict):
        return json.dumps({'error': f'The video-generation gateway answered HTTP {response.status_code} with an unreadable body.', 'transport_error': True})
    ...
    return json.dumps({'result': guidance or 'Request accepted.', 'details': payload}, ensure_ascii=False)
_MEDIA_FIELDS = {'input_image': ('image',), 'input_images': ('image',), 'input_video': ('video',)}
_MAX_IMAGES = 10
...
_TERMINAL_POLL_STATUSES = frozenset({'Ready', 'Error', 'Request Moderated', 'Content Moderated', 'Task not found'})
...
async def _prepare_media(args: dict, task_id: Optional[str]) -> dict:
    ...
    for field, permitted in _MEDIA_FIELDS.items():
        ...
        if isinstance(value, list):
            ...
            prepared[field] = list(await asyncio.gather(*(_deliver_media(entry, permitted, task_id) for entry in value)))
        else:
...
async def _save_if_ready(raw: str, save_to, started: float) -> str:
    ...
    try:
        ...
        payload['result'] = f'The clip finished but saving it failed: {type(exc).__name__}: {exc}. Poll this job again to retry the download; the job itself is unaffected.'
...
def _delivery_lead_in(target) -> str:
    ...
    if _delivers_as_an_attachment():
        return f'Saved to {target}. To deliver it, copy the next line into your reply exactly as written, alone on its own line, with nothing added around it:\nMEDIA:{target}\n'
...
def _filename_from_url(url: str) -> str:
    ...
    name = re.sub('[^A-Za-z0-9._-]', '_', name).lstrip('.')[:120]
...
def _still_generating(job_id: str) -> str:
    ...
    return json.dumps({'result': f'Still generating. This call reached its own time limit, which the job is unaffected by — call bfl_flux3_get_result again with id={job_id} to keep waiting.', 'details': {'id': job_id, 'status': 'Generating'}}, ensure_ascii=False)
...
async def _handle_get_result(args: dict, **kwargs) -> str:
    ...
    try:
        return await asyncio.wait_for(_poll_until_done(url, save_to, started), timeout=_CALL_BACKSTOP_SECONDS)
...
_MEDIA_SENTENCE = 'Media fields accept a local file path (uploaded automatically to Nous-managed temporary storage and deleted when the generation finishes) or a URL. '
_OVERRIDE_SENTENCE = 'All guidance is defaults: explicit user instructions override it.'

def _shared_submit_properties() -> dict:
    return {'prompt': {'type': 'string', 'minLength': 1, 'description': 'Generation brief in plain prose (it is interpreted and expanded by a reasoning harness). Order it subject, distinguishing visual specifics, action, camera, lighting, environment, audio, style. Audio is generated by default — say "no music" if unwanted.'}, 'aspect_ratio': {'type': 'string', 'enum': list(_ASPECT_RATIOS), 'default': 'auto', 'description': 'Output aspect ratio. "auto" lets the model choose.'}, 'duration': {'oneOf': [{'type': 'integer', 'minimum': 5, 'maximum': 20}, {'type': 'string', 'const': 'auto'}], 'default': 'auto', 'description': 'Clip duration in whole seconds (5-20), or "auto".'}, 'resolution': {'type': 'string', 'enum': list(_RESOLUTIONS), 'default': '720p', 'description': 'Output resolution bin.'}, 'generate_audio': {'type': 'boolean', 'default': True, 'description': 'Generate synchronized audio.'}, 'grounding': {'type': 'boolean', 'default': True, 'description': 'Allow a short research pass before generation.'}, 'seed': {'type': 'integer', 'minimum': 0, 'maximum': 4294967295, 'description': 'Optional reproducibility seed.'}, 'version': {'type': 'string', 'description': 'Model version pin. Defaults to "latest".'}}
TEXT_TO_VIDEO_SCHEMA = {'name': 'bfl_flux3_text_to_video', 'description': _GUIDE_POINTER + 'FLUX 3 text-to-video: generates a clip (with audio) from the prompt alone. Nothing but the prompt anchors the subject here, so research anything with a real, checkable appearance before writing it — whatever you leave unspecified is filled in for you. Generation takes several minutes: this returns a job id immediately; poll bfl_flux3_get_result. ' + _OVERRIDE_SENTENCE, 'parameters': {'type': 'object', 'properties': _shared_submit_properties(), 'required': ['prompt'], 'additionalProperties': False}}
IMAGE_TO_VIDEO_SCHEMA = {'name': 'bfl_flux3_image_to_video', 'description': _GUIDE_POINTER + 'FLUX 3 image-to-video: animates one image as the literal opening frame — those pixels are frame 0 and the clip moves from there. ' + _MEDIA_SENTENCE + 'Returns a job id; poll bfl_flux3_get_result. ' + _OVERRIDE_SENTENCE, 'parameters': {'type': 'object', 'properties': {**_shared_submit_properties(), 'input_image': {'type': 'string', 'minLength': 1, 'description': 'Exactly one opening-frame image: a local file path or a URL (PNG/JPEG/WebP, up to 10MB).'}}, 'required': ['prompt', 'input_image'], 'additionalProperties': False}}
KEYFRAMES_TO_VIDEO_SCHEMA = {'name': 'bfl_flux3_keyframes_to_video', 'description': _GUIDE_POINTER + 'FLUX 3 keyframe video: a storyboard of 1-10 images pinned at chosen frame positions (24fps). Name the subject and describe the motion that carries it between the pins, and keep the subject consistent across every pinned image. ' + _MEDIA_SENTENCE + 'Returns a job id; poll bfl_flux3_get_result. ' + _OVERRIDE_SENTENCE, 'parameters': {'type': 'object', 'properties': {**_shared_submit_properties(), 'input_images': {'type': 'array', 'items': {'type': 'string', 'minLength': 1}, 'minItems': 1, 'maxItems': 10, 'description': '1-10 keyframe images: local file paths or URLs (PNG/JPEG/WebP, up to 10MB each).'}, 'keyframe_indices': {'type': 'array', 'items': {'type': 'integer', 'minimum': 0, 'maximum': 480}, 'minItems': 1, 'maxItems': 10, 'description': 'One unique non-negative frame index per image (24fps). Each must be at most duration×24, so set an explicit duration rather than "auto" whenever you pin indices — "auto" resolves to 5, 10, 15 or 20 seconds and an index past the length it picks is rejected.'}}, 'required': ['prompt', 'input_images', 'keyframe_indices'], 'additionalProperties': False}}
VIDEO_CONTINUATION_SCHEMA = {'name': 'bfl_flux3_video_continuation', 'description': _GUIDE_POINTER + 'FLUX 3 video continuation: the new generation picks up from the input clip\'s final frames. Open the prompt with "Continue this video from its final frames:", re-establish the subject and the moment it ended on, then describe what happens next. input_video must be an mp4 of at most 50MB and 15 seconds, and the generated segment tops out at 15s too — chain a second continuation for a longer sequence. duration is the new segment only. ' + _MEDIA_SENTENCE + 'Returns a job id; poll bfl_flux3_get_result. ' + _OVERRIDE_SENTENCE, 'parameters': {'type': 'object', 'properties': {**_shared_submit_properties(), 'input_video': {'type': 'string', 'minLength': 1, 'description': 'The clip to continue: a local file path or a URL. mp4 only, at most 50MB and 15 seconds.'}}, 'required': ['prompt', 'input_video'], 'additionalProperties': False}}
GET_RESULT_SCHEMA = {'name': 'bfl_flux3_get_result', 'description': 'Poll a FLUX 3 video job by the job id a generate tool returned. Generation takes minutes and a long Generating phase is normal. This call waits for you while the job runs, so it may run for several minutes; if it returns still generating, just call it again. Do not sleep between calls. On Ready the clip is downloaded for you and the response gives its local path; your only remaining step is to deliver that file as the response describes.', 'parameters': {'type': 'object', 'properties': {'id': {'type': 'string', 'minLength': 1, 'description': 'Job id from a previous bfl_flux3_* generate call.'}, 'save_to': {'type': 'string', 'description': 'Where to save the finished clip: a directory or a full file path. Set this only when the user asked for a particular location; the default is ~/Downloads. An existing file is never overwritten.'}}, 'required': ['id'], 'additionalProperties': False}}
PROMPTING_GUIDE_SCHEMA = {'name': 'bfl_flux3_prompting_guide', 'description': 'Read this before your first FLUX 3 generation. The prompting and grounding guide: how to research a subject so it renders as itself, how to assemble a prompt, which generate tool fits, and how to save and deliver the finished clip. Takes no arguments and spends no generation budget.', 'parameters': {'type': 'object', 'properties': {}, 'additionalProperties': False}}
...
registry.register(name='bfl_flux3_text_to_video', toolset=_TOOLSET, schema=TEXT_TO_VIDEO_SCHEMA, handler=_handle_text_to_video, check_fn=check_bfl_requirements, requires_env=[], is_async=True, emoji='🎬')
registry.register(name='bfl_flux3_image_to_video', toolset=_TOOLSET, schema=IMAGE_TO_VIDEO_SCHEMA, handler=_handle_image_to_video, check_fn=check_bfl_requirements, requires_env=[], is_async=True, emoji='🎬')
registry.register(name='bfl_flux3_keyframes_to_video', toolset=_TOOLSET, schema=KEYFRAMES_TO_VIDEO_SCHEMA, handler=_handle_keyframes_to_video, check_fn=check_bfl_requirements, requires_env=[], is_async=True, emoji='🎬')
registry.register(name='bfl_flux3_video_continuation', toolset=_TOOLSET, schema=VIDEO_CONTINUATION_SCHEMA, handler=_handle_video_continuation, check_fn=check_bfl_requirements, requires_env=[], is_async=True, emoji='🎬')
registry.register(name='bfl_flux3_get_result', toolset=_TOOLSET, schema=GET_RESULT_SCHEMA, handler=_handle_get_result, check_fn=check_bfl_requirements, requires_env=[], is_async=True, emoji='🎬')
registry.register(name='bfl_flux3_prompting_guide', toolset=_TOOLSET, schema=PROMPTING_GUIDE_SCHEMA, handler=_handle_prompting_guide, check_fn=check_bfl_requirements, requires_env=[], is_async=True, emoji='📖')