...
per child under::
    ...
    <duck_agent_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.duck-agent)."""
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str, context: Optional[str]=None, root: Optional[Path]=None):
        ...
        try:
            ...
            header = ['=== Duck Agent subagent live transcript ===', f'delegation: {delegation_id}   task: {task_index}', f'goal: {_redact(_one_line(goal, _KICKOFF_MAX))}', f"started: {time.strftime('%Y-%m-%d %H:%M:%S')}", '(append-only; streams while the subagent runs — tail -f me)', '=' * 40]
            self.path.write_text('\n'.join(header) + '\n', encoding='utf-8')
            self.event('user', 'kickoff: ' + _one_line(goal, _KICKOFF_MAX) + (f' | context: {_one_line(context, _KICKOFF_MAX)}' if context else ''))
    ...
    def observe(self, event_type: Any, tool_name: Any=None, preview: Any=None, args: Any=None, **kwargs: Any) -> None:
        ...
        if et == 'tool.started':
            ...
            self.tool_result(str(tool_name or ''), result=kwargs.get('result'), duration=kwargs.get('duration'), is_error=bool(kwargs.get('is_error')))
        elif et == '_thinking':
...
def create_live_transcripts(task_list: List[Dict[str, Any]], context: Optional[str]=None, delegation_id: Optional[str]=None) -> tuple[Optional[str], List[Optional[LiveTranscriptWriter]], List[str]]:
    ...
    try:
        ...
        for i, t in enumerate(task_list):
            w = LiveTranscriptWriter(deleg_id, i, str(t.get('goal', '')), context=t.get('context') or context)
...
def _write_manifest(delegation_id: str, task_list: List[Dict[str, Any]], paths: List[str]) -> None:
    try:
        manifest = {'delegation_id': delegation_id, 'started': time.strftime('%Y-%m-%d %H:%M:%S'), 'task_count': len(task_list), 'tasks': [{'index': i, 'goal': _redact(str(t.get('goal', ''))[:500]), 'log': paths[i] if i < len(paths) else None, 'status': 'running'} for i, t in enumerate(task_list)]}
        _manifest_path(delegation_id).write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding='utf-8')
    except Exception as exc:
