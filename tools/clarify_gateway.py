...
class _ClarifyEntry:
    ...
    def signature(self) -> Dict[str, object]:
        return {'clarify_id': self.clarify_id, 'session_key': self.session_key, 'question': self.question, 'choices': list(self.choices) if self.choices else None, 'multi_select': bool(self.multi_select)}
_lock = threading.RLock()
_entries: Dict[str, _ClarifyEntry] = {}
_session_index: Dict[str, List[str]] = {}

def register(clarify_id: str, session_key: str, question: str, choices: Optional[List[str]], multi_select: bool=False) -> _ClarifyEntry:
    ...
    """
    entry = _ClarifyEntry(clarify_id=clarify_id, session_key=session_key, question=question, choices=list(choices) if choices else None, multi_select=bool(multi_select) and bool(choices), awaiting_text=not bool(choices))
    with _lock:
