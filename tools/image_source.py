...
async def resolve_image_source(src: str, ctx: ResolveContext, *, permitted: tuple=('image',)) -> ResolvedImage:
    ...
    if _SCHEME_RE.match(s) and (not s.lower().startswith('file://')):
        raise UnsupportedScheme('Unrecognized image source scheme. Use an http(s) URL, a local file path, a file:// URI, or a data: URL.', src=s)
...
def _media_cache_roots() -> list:
    """Agent-managed media cache directories under DUCK_AGENT_HOME (host side).
    ...
    return [home / 'cache', home / 'images', home / 'image_cache', home / 'audio_cache', home / 'video_cache', home / 'temp_vision_images', home / 'temp_video_files']
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Optional[Path]:
    ...
    - Non-local backend: permitted only if the path resolves inside a media
      cache root. A container-visible cache path (e.g. ``/root/.duck-agent/cache/
...
async def _resolve_container_fallback(p: Path, ctx: ResolveContext, src: str, permitted: tuple=('image',)) -> ResolvedImage:
    ...
    if env is None:
        raise SourceNotFound(f"'{p}' is not reachable inside the sandbox and no active sandbox session is available to read it", src=src, origin='container')
    qp = shlex.quote(str(p))
    res = await asyncio.to_thread(env.execute, f"head -c {_MAX_INGEST_BYTES + 1} < {qp} | base64 | tr -d '\\n'")
...
def _finalize(data: bytes, declared_mime: str, origin: str, src: str, permitted: tuple=('image',)) -> ResolvedImage:
