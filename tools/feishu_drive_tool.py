...
FEISHU_DRIVE_LIST_COMMENTS_SCHEMA = {'name': 'feishu_drive_list_comments', 'description': 'List comments on a Feishu document. Use is_whole=true to list whole-document comments only.', 'parameters': {'type': 'object', 'properties': {'file_token': {'type': 'string', 'description': 'The document file token.'}, 'file_type': {'type': 'string', 'description': 'File type (default: docx).', 'default': 'docx'}, 'is_whole': {'type': 'boolean', 'description': 'If true, only return whole-document comments.', 'default': False}, 'page_size': {'type': 'integer', 'description': 'Number of comments per page (max 100).', 'default': 100}, 'page_token': {'type': 'string', 'description': 'Pagination token for next page.'}}, 'required': ['file_token']}}
...
def _handle_list_comments(args: dict, **kwargs) -> str:
    ...
    queries = [('file_type', file_type), ('user_id_type', 'open_id'), ('page_size', str(page_size))]
    if is_whole:
        queries.append(('is_whole', 'true'))
    if page_token:
        queries.append(('page_token', page_token))
    code, msg, data = _do_request(client, 'GET', _LIST_COMMENTS_URI, paths={'file_token': file_token}, queries=queries)
    if code != 0:
...
def _handle_list_replies(args: dict, **kwargs) -> str:
    ...
    if page_token:
        queries.append(('page_token', page_token))
    code, msg, data = _do_request(client, 'GET', _LIST_REPLIES_URI, paths={'file_token': file_token, 'comment_id': comment_id}, queries=queries)
    if code != 0:
...
def _handle_reply_comment(args: dict, **kwargs) -> str:
    ...
    code, msg, data = _do_request(client, 'POST', _REPLY_COMMENT_URI, paths={'file_token': file_token, 'comment_id': comment_id}, queries=[('file_type', file_type)], body=body)
    if code != 0:
...
def _handle_add_comment(args: dict, **kwargs) -> str:
    ...
    code, msg, data = _do_request(client, 'POST', _ADD_COMMENT_URI, paths={'file_token': file_token}, body=body)
    if code != 0:
        return tool_error(f'Add comment failed: code={code} msg={msg}')
    return tool_result(success=True, data=data)
registry.register(name='feishu_drive_list_comments', toolset='feishu_drive', schema=FEISHU_DRIVE_LIST_COMMENTS_SCHEMA, handler=_handle_list_comments, check_fn=_check_feishu, requires_env=[], is_async=False, description='List document comments', emoji='💬')
registry.register(name='feishu_drive_list_comment_replies', toolset='feishu_drive', schema=FEISHU_DRIVE_LIST_REPLIES_SCHEMA, handler=_handle_list_replies, check_fn=_check_feishu, requires_env=[], is_async=False, description='List comment replies', emoji='💬')
registry.register(name='feishu_drive_reply_comment', toolset='feishu_drive', schema=FEISHU_DRIVE_REPLY_SCHEMA, handler=_handle_reply_comment, check_fn=_check_feishu, requires_env=[], is_async=False, description='Reply to a document comment', emoji='✉️')
registry.register(name='feishu_drive_add_comment', toolset='feishu_drive', schema=FEISHU_DRIVE_ADD_COMMENT_SCHEMA, handler=_handle_add_comment, check_fn=_check_feishu, requires_env=[], is_async=False, description='Add a whole-document comment', emoji='✉️')