...
def _get_message_storage_state(db, message_id) -> Optional[Dict[str, Any]]:
    ...
    try:
        with db._lock:
            cursor = db._conn.execute('SELECT session_id, active, compacted FROM messages WHERE id = ?', (message_id,))
            row = cursor.fetchone()
    except Exception:
        logging.debug('message storage-state lookup failed for %s', message_id, exc_info=True)
        return None
...
def _annotate_rebuild_status(db, payload: Dict[str, Any]) -> None:
    ...
    if status is None:
        return
    payload['index_rebuild'] = {'percent': status['percent'], 'note': f"The search index is rebuilding in the background ({status['percent']}% done, {status['indexed']:,} of {status['total']:,} messages). Results from older messages may be incomplete until it finishes."}
...
def _shape_message(m: Dict[str, Any], anchor_id: Optional[int]=None, max_content_len: Optional[int]=None) -> Dict[str, Any]:
...
def _read_session(db, session_id: str, head: int=20, tail: int=10, link_profile: str=None) -> str:
    ...
    response = {'success': True, 'mode': 'read', 'session_id': session_id, 'link': _session_link(session_id, link_profile), 'session_meta': {'when': _format_timestamp(meta.get('started_at')), 'source': meta.get('source'), 'model': meta.get('model'), 'title': meta.get('title')}, 'message_count': total, 'truncated': truncated, 'messages': window}
    if truncated:
        response['message'] = f'Session has {total} messages; showing first {head} + last {tail}. Pass around_message_id (any id above) to scroll the middle.'
    return json.dumps(response, ensure_ascii=False)

def _list_recent_sessions(db, limit: int, current_session_id: str=None, link_profile: str=None) -> str:
    """Return metadata for the most recent sessions (no LLM calls, no FTS5)."""
    try:
        sessions = db.list_sessions_rich(limit=limit + 5, exclude_sources=list(_HIDDEN_SESSION_SOURCES), order_by_last_active=True)
        ...
        for s in sessions:
            ...
            if s.get('parent_session_id'):
                continue
            results.append({'session_id': sid, 'link': _session_link(sid, link_profile), 'title': s.get('title') or None, 'source': s.get('source', ''), 'started_at': s.get('started_at', ''), 'last_active': s.get('last_active', ''), 'message_count': s.get('message_count', 0), 'preview': s.get('preview', '')})
            if len(results) >= limit:
                break
        return json.dumps({'success': True, 'mode': 'browse', 'results': results, 'count': len(results), 'message': f'Showing {len(results)} most recent sessions. Pass a query= to search, or session_id+around_message_id to scroll.'}, ensure_ascii=False)
    except Exception as e:
        logging.error('Error listing recent sessions: %s', e, exc_info=True)
        return tool_error(f'Failed to list recent sessions: {e}', success=False)

def _scroll(db, session_id: str, around_message_id: int, window: int=5, current_session_id: str=None) -> str:
    ...
    if current_session_id:
        ...
        if a_root and c_root and (a_root == c_root):
            is_compacted_anchor = anchor_state is not None and anchor_state['active'] == 0 and (anchor_state['compacted'] == 1)
            is_inactive_non_compacted_anchor = anchor_state is not None and anchor_state['active'] == 0 and (anchor_state['compacted'] != 1)
            is_compression_history = not is_inactive_non_compacted_anchor and _is_compression_ended(db, anchor_session_id)
        ...
        if owning and owning != session_id:
            ...
            if a_root and o_root and (a_root == o_root):
                try:
                    ...
                    if messages:
                        view = rebind_view
                        rebind_warning = f'around_message_id {around_message_id} lives in {owning} (child of {session_id}); rebound transparently'
                        try:
    ...
    response = {'success': True, 'mode': 'scroll', 'session_id': session_id, 'around_message_id': around_message_id, 'session_meta': {'when': _format_timestamp(session_meta.get('started_at')), 'source': session_meta.get('source'), 'model': session_meta.get('model'), 'title': session_meta.get('title')}, 'window': window, 'messages': [_shape_message(m, anchor_id=around_message_id) for m in messages], 'messages_before': view.get('messages_before', 0), 'messages_after': view.get('messages_after', 0)}
    if rebind_warning:
...
def _title_match_result(db, query: str, current_lineage_root: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    if anchor_id is not None:
        ...
        view = {}
    entry = {'session_id': session_id, 'when': _format_timestamp(session_meta.get('started_at')), 'source': session_meta.get('source', 'unknown'), 'model': session_meta.get('model') or 'unknown', 'title': session_meta.get('title') or title_query, 'matched_role': 'session_title', 'match_message_id': anchor_id, 'snippet': f"Session title matched: {session_meta.get('title') or title_query}", 'bookend_start': [_shape_message(m) for m in view.get('bookend_start') or messages[:3]], 'messages': [_shape_message(m, anchor_id=anchor_id) for m in view.get('window') or messages[:5]], 'bookend_end': [_shape_message(m) for m in view.get('bookend_end') or messages[-3:]], 'messages_before': view.get('messages_before', 0), 'messages_after': view.get('messages_after', max(len(messages) - 5, 0)), '_lineage_root': lineage_root}
    if lineage_root and lineage_root != session_id:
        entry['parent_session_id'] = lineage_root
    return entry

def _discover(db, query: str, role_filter: Optional[List[str]], limit: int, sort: Optional[str], current_session_id: str=None, link_profile: str=None) -> str:
    """Discovery shape: FTS5 + anchored window + bookends per hit. Single call."""
    role_list = role_filter if role_filter else ['user', 'assistant']
    current_lineage_root = _resolve_lineage(db, current_session_id) if current_session_id else None
    title_result = _title_match_result(db, query, current_lineage_root)
    try:
        raw_results = db.search_messages(query=query, role_filter=role_list, exclude_sources=list(_HIDDEN_SESSION_SOURCES), limit=_DISCOVER_SCAN_LIMIT, offset=0, sort=sort, fields=_DISCOVER_SEARCH_FIELDS)
    except Exception as e:
        logging.error('FTS5 search failed: %s', e, exc_info=True)
        return tool_error(f'Search failed: {e}', success=False)
    raw_results = _order_for_recall(raw_results)
    if not raw_results and (not title_result):
        _empty_payload = {'success': True, 'mode': 'discover', 'query': query, 'results': [], 'count': 0, 'message': 'No matching sessions found.'}
...
def session_search(query: str='', role_filter: str=None, limit: int=3, db=None, current_session_id: str=None, session_id: str=None, around_message_id: int=None, window: int=5, sort: str=None, profile: str=None) -> str:
    ...
    if (isinstance(session_id, str) and session_id.strip()) and around_message_id is not None:
        return _scroll(db=db, session_id=session_id, around_message_id=around_message_id, window=window, current_session_id=current_session_id)
    ...
    if isinstance(sort, str):
        ...
        if candidate in ('newest', 'oldest'):
            sort_norm = candidate
    return _discover(db=db, query=query.strip(), role_filter=role_list, limit=limit, sort=sort_norm, current_session_id=current_session_id, link_profile=profile)
