...
transport, discovers their tools, and registers them into the duck-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.duck-agent/config.yaml under the ``mcp_servers`` key.
      ...
      remote_api:
        ...
        identity_header:       # optional per-user identity header attached
          ...
          value: "alice"       # required for static; profile mode uses the
                               # active Duck Agent profile name
        timeout: 180
...
try:
    ...
    try:
        from mcp.types import CreateMessageResult, CreateMessageResultWithTools, ErrorData, SamplingCapability, SamplingToolsCapability, TextContent, ToolUseContent
    ...
    try:
        from mcp.types import ServerNotification, ToolListChangedNotification, PromptListChangedNotification, ResourceListChangedNotification
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Duck Agent' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
_MCP_LOG_LEVEL_MAP = {'debug': logging.DEBUG, 'info': logging.INFO, 'notice': logging.INFO, 'warning': logging.WARNING, 'error': logging.ERROR, 'critical': logging.ERROR, 'alert': logging.ERROR, 'emergency': logging.ERROR}
...
_SAFE_ENV_KEYS_CASE_INSENSITIVE = frozenset({'ALLUSERSPROFILE', 'APPDATA', 'COMMONPROGRAMFILES', 'COMMONPROGRAMFILES(X86)', 'COMMONPROGRAMW6432', 'COMPUTERNAME', 'COMSPEC', 'HOMEDRIVE', 'HOMEPATH', 'LOCALAPPDATA', 'NUMBER_OF_PROCESSORS', 'OS', 'PATHEXT', 'PROCESSOR_ARCHITECTURE', 'PROGRAMDATA', 'PROGRAMFILES', 'PROGRAMFILES(X86)', 'PROGRAMW6432', 'PUBLIC', 'SYSTEMDRIVE', 'SYSTEMROOT', 'TEMP', 'TMP', 'USERDOMAIN', 'USERNAME', 'USERPROFILE', 'WINDIR'})
_CREDENTIAL_PATTERN = re.compile('(?:ghp_[A-Za-z0-9_]{1,255}|sk-[A-Za-z0-9_]{1,255}|Bearer\\s+\\S+|token=[^\\s&,;\\"\']{1,255}|key=[^\\s&,;\\"\']{1,255}|API_KEY=[^\\s&,;\\"\']{1,255}|password=[^\\s&,;\\"\']{1,255}|secret=[^\\s&,;\\"\']{1,255})', re.IGNORECASE)
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Duck Agent explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Duck Agent and
    ...
    for key, value in os.environ.items():
        if key in _SAFE_ENV_KEYS or key.upper() in _SAFE_ENV_KEYS_CASE_INSENSITIVE or key.startswith('XDG_') or (get_secret_source is not None and get_secret_source(key)):
            env[key] = value
    if user_env:
        env.update(user_env)
    return env
...
def _is_method_not_found_error(exc: BaseException) -> bool:
    ...
    if not msg:
        return False
    return str(_JSONRPC_METHOD_NOT_FOUND) in msg or 'method not found' in msg or 'unknown method' in msg or ('not found: ping' in msg)
_MCP_INJECTION_PATTERNS = [(re.compile('ignore\\s+(all\\s+)?previous\\s+instructions', re.I), "prompt override attempt ('ignore previous instructions')"), (re.compile('you\\s+are\\s+now\\s+a', re.I), "identity override attempt ('you are now a...')"), (re.compile('your\\s+new\\s+(task|role|instructions?)\\s+(is|are)', re.I), 'task override attempt'), (re.compile('system\\s*:\\s*', re.I), 'system prompt injection attempt'), (re.compile('<\\s*(system|human|assistant)\\s*>', re.I), 'role tag injection attempt'), (re.compile('do\\s+not\\s+(tell|inform|mention|reveal)', re.I), 'concealment instruction'), (re.compile('(curl|wget|fetch)\\s+https?://', re.I), 'network command in description'), (re.compile('base64\\.(b64decode|decodebytes)', re.I), 'base64 decode reference'), (re.compile('exec\\s*\\(|eval\\s*\\(', re.I), 'code execution reference'), (re.compile('import\\s+(subprocess|os|shutil|socket)', re.I), 'dangerous import reference')]
...
async def _paginate_full_list(list_method, items_attr: str, server_name: str):
    ...
    for _ in range(_MCP_LIST_MAX_PAGES):
        ...
        logger.warning("MCP server '%s': %s pagination exceeded %d pages; truncating at %d items", server_name, items_attr, _MCP_LIST_MAX_PAGES, len(items))
    return items
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(os.getenv('DUCK_AGENT_HOME', os.path.join(os.path.expanduser('~'), '.duck-agent')))
            candidates = [os.path.join(hermes_home, 'node', 'bin', resolved_command), os.path.join(os.path.expanduser('~'), '.local', 'bin', resolved_command), os.path.join(os.sep, 'usr', 'local', 'bin', resolved_command)]
...
def _wrap_command_with_watchdog(command: str, args: list) -> tuple[str, list]:
    ...
    try:
        ...
        return (command, args)
    watchdog_args = [os.path.join(os.path.dirname(os.path.abspath(__file__)), 'mcp_stdio_watchdog.py'), '--ppid', str(my_pid), '--', command, *args]
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Duck Agent gateways know how to render.
    ...
    *name hint* — `cache_document_from_bytes` re-sanitizes and prefixes it,
    ...
    if uri:
        try:
            ...
            name = ''
    name = _re.sub('[\\x00-\\x1f\\x7f]', '', name).strip()
    if len(name) > 150:
...
def _cache_mcp_audio_block(block) -> str:
    ...
    try:
        ...
        import mimetypes
        ext = {'audio/wav': '.wav', 'audio/x-wav': '.wav', 'audio/wave': '.wav'}.get(mime_type) or mimetypes.guess_extension(mime_type) or '.ogg'
...
def _render_mcp_resource_block(block, server_name: str='') -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Duck Agent document cache; returns a marker with
...
def _validate_remote_mcp_url(server_name: str, url: Any) -> str:
    ...
    if not isinstance(url, str):
        raise InvalidMcpUrlError(f"Invalid MCP URL for '{server_name}': expected a string, got {type(url).__name__}")
    stripped = url.strip()
    if not stripped:
...
def _resolve_client_cert(server_name: str, config: dict):
    ...
    def _expand(path: Any, label: str) -> str:
        if not isinstance(path, str) or not path.strip():
            raise ValueError(f"MCP server '{server_name}': {label} must be a non-empty string path (got {type(path).__name__})")
    ...
    if isinstance(raw_cert, (list, tuple)):
        if raw_key is not None:
            raise ValueError(f"MCP server '{server_name}': specify either client_cert as a list [cert, key] OR client_cert + client_key, not both")
        if len(raw_cert) == 2:
        ...
        if len(raw_cert) == 3:
            ...
            if not isinstance(password, str):
                raise ValueError(f"MCP server '{server_name}': client_cert[2] (key passphrase) must be a string")
            return (cert_path, key_path, password)
        raise ValueError(f"MCP server '{server_name}': client_cert list form must have 2 or 3 elements (got {len(raw_cert)})")
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Duck Agent profile name once at
    ...
    if not isinstance(raw, dict):
        logger.warning("MCP server '%s': identity_header must be a mapping with 'name' and 'value'/'value_from' keys (got %s) — ignoring", server_name, type(raw).__name__)
        return None
    name = raw.get('name')
    if not isinstance(name, str) or not name.strip():
        logger.warning("MCP server '%s': identity_header requires a non-empty 'name' — ignoring", server_name)
        return None
    value_from = (raw.get('value_from') or 'static').strip().lower()
    if value_from == 'static':
        value = raw.get('value')
        if not isinstance(value, str) or not value.strip():
            logger.warning("MCP server '%s': identity_header with value_from: static requires a non-empty string 'value' — ignoring", server_name)
            return None
        return (name.strip(), value)
    if value_from == 'profile':
        from hermes_cli.profiles import get_active_profile_name
        return (name.strip(), get_active_profile_name())
    logger.warning("MCP server '%s': identity_header value_from must be 'static' or 'profile' (got %r) — ignoring", server_name, value_from)
    return None
...
def _apply_identity_header(server_name: str, config: dict, headers: dict) -> dict:
    ...
    if any((key.lower() == name.lower() for key in headers)):
        logger.debug("MCP server '%s': identity_header '%s' already set via explicit headers config — keeping the explicit value", server_name, name)
        return headers
    headers[name] = value
    return headers
...
def _format_connect_error(exc: BaseException) -> str:
    ...
    def _find_missing(current: BaseException) -> Optional[str]:
        ...
        if isinstance(current, FileNotFoundError):
            ...
            match = re.search("No such file or directory: '([^']+)'", str(current))
            if match:
    ...
    if missing:
        ...
        if os.path.basename(missing) in {'npx', 'npm', 'node'}:
            message += ' (ensure Node.js is installed and PATH includes its bin directory, or set mcp_servers.<name>.command to an absolute path and include that directory in mcp_servers.<name>.env.PATH)'
...
class SamplingHandler:
    ...
    def _convert_messages(self, params) -> List[dict]:
        ...
        for msg in params.messages:
            ...
            if tool_uses:
                tc_list = []
                for tu in tool_uses:
                    tc_list.append({'id': getattr(tu, 'id', f'call_{len(tc_list)}'), 'type': 'function', 'function': {'name': tu.name, 'arguments': json.dumps(tu.input, ensure_ascii=False) if isinstance(tu.input, dict) else str(tu.input)}})
    ...
    def _build_tool_use_result(self, choice, response):
        ...
        logger.log(self.audit_level, "MCP server '%s' sampling response: model=%s, tokens=%s, tool_calls=%d", self.server_name, response.model, getattr(getattr(response, 'usage', None), 'total_tokens', '?'), len(content_blocks))
    ...
    def _build_text_result(self, choice, response):
        ...
        logger.log(self.audit_level, "MCP server '%s' sampling response: model=%s, tokens=%s", self.server_name, response.model, getattr(getattr(response, 'usage', None), 'total_tokens', '?'))
        return CreateMessageResult(role='assistant', content=TextContent(type='text', text=_sanitize_error(response_text)), model=response.model, stopReason=self._STOP_REASON_MAP.get(choice.finish_reason, 'endTurn'))
    ...
    async def __call__(self, context, params):
        ...
        if server_tools:
            call_tools = [{'type': 'function', 'function': {'name': getattr(t, 'name', ''), 'description': getattr(t, 'description', '') or '', 'parameters': _normalize_mcp_input_schema(getattr(t, 'inputSchema', None))}} for t in server_tools]
        ...
        def _sync_call():
            return call_llm(task='mcp', model=resolved_model or None, messages=messages, temperature=call_temperature, max_tokens=max_tokens, tools=call_tools, timeout=self.timeout)
        try:
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Duck Agent' existing approval
    ...
    async def __call__(self, context, params):
        ...
        if mode == 'url':
            logger.info("MCP server '%s' requested URL-mode elicitation; declining (URL-mode elicitation not implemented)", self.server_name)
        ...
        def _invoke_consent() -> str:
            if captured is None:
                return request_elicitation_consent(message, description, timeout_seconds=int(self.timeout), surface=f'mcp-elicitation/{self.server_name}')
            return captured.copy().run(request_elicitation_consent, message, description, timeout_seconds=int(self.timeout), surface=f'mcp-elicitation/{self.server_name}')
        try:
            answer = await asyncio.wait_for(asyncio.to_thread(_invoke_consent), timeout=self.timeout + self._OUTER_TIMEOUT_GRACE_SECONDS)
        except asyncio.TimeoutError:
...
class MCPServerTask:
    ...
    """
    __slots__ = ('name', 'session', 'tool_timeout', '_task', '_ready', '_shutdown_event', '_reconnect_event', '_tools', '_error', '_config', '_sampling', '_elicitation', '_registered_tool_names', '_auth_type', '_refresh_lock', '_rpc_lock', '_pending_refresh_tasks', '_pending_call_context', '_lifecycle_started_at', '_last_tool_call_at', '_idle_timeout_seconds', '_max_lifetime_seconds', '_recycled_reason', 'initialize_result', '_ping_unsupported', '_reconnect_retries', '_session_proven', '_was_parked')
    ...
    def _stdio_recycle_reason(self, now: Optional[float]=None) -> Optional[str]:
        ...
        if self._max_lifetime_seconds is not None and now - self._lifecycle_started_at >= self._max_lifetime_seconds:
            return 'max_lifetime_seconds'
        if self._idle_timeout_seconds is not None and now - self._last_tool_call_at >= self._idle_timeout_seconds:
    ...
    def _make_logging_callback(self):
        ...
        server into Duck Agent' logging (agent.log via hermes_logging), tagged
        ...
        async def _on_log(params):
            try:
                level = _MCP_LOG_LEVEL_MAP.get(str(getattr(params, 'level', 'info')).lower(), logging.INFO)
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            async with self._rpc_lock:
                new_mcp_tools = await _paginate_full_list(self.session.list_tools, 'tools', self.name)
            toolset_name = f'mcp-{self.name}'
            stale_tool_names = old_tool_names - {mcp_prefixed_tool_name(self.name, tool.name) for tool in new_mcp_tools}
    ...
    async def _keepalive_probe(self) -> None:
        ...
        if not self._ping_unsupported:
            try:
                ...
                logger.info("MCP server '%s': does not implement the optional 'ping' utility (-32601); using 'list_tools' for keepalive on this connection.", self.name)
    ...
    def _mark_session_proven(self) -> None:
        ...
        if not self._session_proven:
            ...
            if self._was_parked:
                ...
                logger.warning("MCP server '%s': revived — session healthy again after parking (state: parked → connected)", self.name)
    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        """
        keepalive_interval = max(_MIN_KEEPALIVE_INTERVAL, float(self._config.get('keepalive_interval', _DEFAULT_KEEPALIVE_INTERVAL)))
        ...
        try:
            while True:
                ...
                done, _pending = await asyncio.wait({shutdown_task, reconnect_task}, timeout=timeout, return_when=asyncio.FIRST_COMPLETED)
                if done:
                    break
                recycle_reason = self._stdio_recycle_reason()
                if recycle_reason is not None:
                ...
                if self.session:
                    try:
                        ...
                        root = _unwrap_exception_group(exc)
                        logger.warning("MCP server '%s' keepalive failed, triggering reconnect (state: connected → degraded): %s: %s", self.name, type(root).__name__, root)
    ...
    async def _run_stdio(self, config: dict):
        ...
        if config.get('identity_header') is not None:
            logger.warning("MCP server '%s': identity_header is only supported on HTTP/SSE transports — ignored for stdio servers", self.name)
        if not _MCP_AVAILABLE:
            raise ImportError(f"MCP server '{self.name}' requires the 'mcp' Python SDK, but it is not installed. Run `duck-agent setup` to install MCP support, then retry.")
        ...
        try:
            malware_error = await asyncio.wait_for(asyncio.to_thread(check_package_for_malware, command, args), timeout=_OSV_MALWARE_CHECK_TIMEOUT_S)
        except asyncio.TimeoutError:
            logger.warning("MCP server '%s': OSV malware preflight timed out after %.0fs (network slow/unreachable) — proceeding without the check.", self.name, _OSV_MALWARE_CHECK_TIMEOUT_S)
            malware_error = None
        if malware_error:
            raise ValueError(f"MCP server '{self.name}': {malware_error}")
        command, args = _wrap_command_with_watchdog(command, args)
        server_params = StdioServerParameters(command=command, args=args, env=safe_env if safe_env else None, cwd=config.get('cwd'), encoding_error_handler='replace')
    ...
    async def _preflight_content_type(self, url: str, *, headers: Optional[dict]=None, ssl_verify: bool=True, client_cert=None, timeout: float=5.0) -> None:
        ...
        try:
            async with _httpx.AsyncClient(**client_kwargs) as client:
                ...
                if ct and ct not in self._MCP_CONTENT_TYPES and (200 <= resp.status_code < 300):
                    post_resp = await client.post(url, headers={**probe_headers, 'Content-Type': 'application/json', 'Accept': 'application/json, text/event-stream'}, content='{"jsonrpc":"2.0","id":"_probe","method":"initialize","params":{"protocolVersion":"2025-03-26","capabilities":{},"clientInfo":{"name":"duck-agent-probe","version":"0.1"}}}')
    ...
    def _reconnect_or_reraise_group(self, eg: BaseExceptionGroup) -> str:
        ...
        if not self._ready.is_set():
            raise eg
        logger.debug("MCP server '%s': transport TaskGroup exited after a live session (%r) — reconnecting immediately instead of backing off", self.name, eg)
        return 'reconnect'
        ...
        if config.get('transport') == 'sse':
            ...
            try:
                async with sse_client(**_sse_kwargs) as (read_stream, write_stream):
                    async with ClientSession(read_stream, write_stream, **sampling_kwargs) as session:
                        self.initialize_result = await asyncio.wait_for(session.initialize(), timeout=float(connect_timeout))
                        self.session = session
                        await self._discover_tools()
                        self._ready.set()
                        _reset_server_error(self.name)
            ...
            async def _strip_auth_on_cross_origin_redirect(response):
                ...
                if response.is_redirect and response.next_request:
                    ...
                    if (target.scheme, target.host, target.port) != (_original_url.scheme, _original_url.host, _original_url.port):
                        ...
                        response.next_request.headers.pop('Authorization', None)
            client_kwargs: dict = {'follow_redirects': True, 'timeout': httpx.Timeout(float(connect_timeout), read=300.0), 'verify': ssl_verify, 'event_hooks': {'response': [_strip_auth_on_cross_origin_redirect]}}
            if headers:
    ...
    async def _discover_tools(self):
        ...
        if self.session is None:
            return
        if not self._advertises_tools():
            logger.info("MCP server '%s': does not advertise 'tools' capability — skipping tools/list (prompts/resources remain available)", self.name)
            self._tools = []
            self._register_discovered_tools_if_needed()
            return
        async with self._rpc_lock:
            self._tools = await _paginate_full_list(self.session.list_tools, 'tools', self.name)
        self._register_discovered_tools_if_needed()
        ...
        if self._is_http():
            ...
            if config.get('transport') != 'sse' and (not config.get('skip_preflight')) and (not self._ready.is_set()) and (self._auth_type != 'oauth'):
                try:
                    ...
                    await self._preflight_content_type(config['url'], headers=_probe_headers, ssl_verify=config.get('ssl_verify', True), client_cert=_resolve_client_cert(self.name, config))
        ...
        while True:
            try:
                ...
                if lifecycle_reason == 'recycle':
                    logger.info("MCP server '%s': stdio session recycled after %s; waiting for lazy reconnect", self.name, self._recycled_reason)
                    self.session = None
                ...
                if self._session_proven:
                    ...
                    if self._reconnect_retries > _MAX_RECONNECT_RETRIES:
                        logger.warning("MCP server '%s': %d consecutive reconnects without a healthy session (rapid-drop budget exhausted), parking; will self-probe every %ds until it recovers (state: degraded → parked)", self.name, _MAX_RECONNECT_RETRIES, _PARKED_RETRY_INTERVAL)
                        self._was_parked = True
                        self._deregister_tools()
                        self._reconnect_event.clear()
                        parked = await self._wait_for_reconnect_or_shutdown(timeout=_PARKED_RETRY_INTERVAL)
                        if parked == 'shutdown':
                            break
                        logger.debug("MCP server '%s': attempting revival from parked state (self-probe or explicit reconnect request); rebuilding transport.", self.name)
                ...
                if self._is_recycled_stdio():
                    logger.warning("MCP server '%s': lazy reconnect after stdio recycle failed, marking unavailable while retrying: %s: %s", self.name, type(root).__name__, root)
                    self._recycled_reason = None
                if not self._ready.is_set():
                    if _is_auth_error(root):
                        logger.warning("MCP server '%s' failed initial OAuth authentication, not retrying automatically: %s: %s", self.name, type(root).__name__, root)
                        self._error = exc
                        self._ready.set()
                        return
                    if failure_class == 'permanent':
                        logger.warning("MCP server '%s' failed initial connection with a permanent error, parking without retries (state: connecting → parked): %s: %s", self.name, type(root).__name__, root)
                        self._error = exc
                        self._ready.set()
                        self._was_parked = True
                        self._deregister_tools()
                        ...
                        if parked == 'shutdown':
                            return
                        logger.debug("MCP server '%s': attempting revival after permanent initial failure (self-probe or explicit reconnect request); rebuilding transport.", self.name)
                        initial_retries = 0
                        self._reconnect_retries = 0
                        backoff = 1.0
                        self._error = None
                    ...
                    if initial_retries > _MAX_INITIAL_CONNECT_RETRIES:
                        logger.warning("MCP server '%s' failed initial connection after %d attempts, parking until a reconnect is requested (state: connecting → parked): %s: %s", self.name, _MAX_INITIAL_CONNECT_RETRIES, type(root).__name__, root)
                        self._error = exc
                        self._ready.set()
                        self._was_parked = True
                        self._deregister_tools()
                        ...
                        if parked == 'shutdown':
                            return
                        logger.debug("MCP server '%s': attempting revival after initial connection failures (self-probe or explicit reconnect request); rebuilding transport.", self.name)
                        initial_retries = 0
                        self._reconnect_retries = 0
                        backoff = 1.0
                        self._error = None
                        self._ready.clear()
                        continue
                    logger.debug("MCP server '%s' initial connection failed (attempt %d/%d), retrying in %.0fs: %s: %s", self.name, initial_retries, _MAX_INITIAL_CONNECT_RETRIES, backoff, type(root).__name__, root)
                    ...
                    continue
                if self._shutdown_event.is_set():
                    logger.debug("MCP server '%s' disconnected during shutdown: %s: %s", self.name, type(root).__name__, root)
                    return
                if failure_class == 'permanent':
                    logger.warning("MCP server '%s' hit a permanent error, parking without retries; will self-probe every %ds (state: connected → parked): %s: %s", self.name, _PARKED_RETRY_INTERVAL, type(root).__name__, root)
                    self._was_parked = True
                    self._deregister_tools()
                    self._reconnect_event.clear()
                    parked = await self._wait_for_reconnect_or_shutdown(timeout=_PARKED_RETRY_INTERVAL)
                    if parked == 'shutdown':
                        return
                    logger.debug("MCP server '%s': attempting revival from parked state (permanent error; self-probe or explicit reconnect request); rebuilding transport.", self.name)
                    self._reconnect_retries = _MAX_RECONNECT_RETRIES
                    backoff = 1.0
                    continue
                self._reconnect_retries += 1
                if self._reconnect_retries > _MAX_RECONNECT_RETRIES:
                    logger.warning("MCP server '%s' failed after %d reconnection attempts, parking; will self-probe every %ds until it recovers (state: degraded → parked): %s: %s", self.name, _MAX_RECONNECT_RETRIES, _PARKED_RETRY_INTERVAL, type(root).__name__, root)
                    self._was_parked = True
                    self._deregister_tools()
                    self._reconnect_event.clear()
                    parked = await self._wait_for_reconnect_or_shutdown(timeout=_PARKED_RETRY_INTERVAL)
                    ...
                    backoff = 1.0
                    continue
                logger.debug("MCP server '%s' connection lost (attempt %d/%d), reconnecting in %.0fs: %s: %s", self.name, self._reconnect_retries, _MAX_RECONNECT_RETRIES, backoff, type(root).__name__, root)
                await asyncio.sleep(_jittered(backoff))
                backoff = min(backoff * 2, _MAX_BACKOFF_SECONDS)
                if self._shutdown_event.is_set():
                    return
...
_connect_server_claim: contextvars.ContextVar[Optional[Callable[[MCPServerTask], None]]] = contextvars.ContextVar('mcp_connect_server_claim', default=None)
...
def _record_connect_failure(server_name: str) -> None:
    ...
    backoff = min(_CONNECT_RETRY_BASE_BACKOFF_SEC * 2 ** (n - 1), _CONNECT_RETRY_MAX_BACKOFF_SEC)
...
def _record_tool_trust_metadata(server_name: str, config: dict, tools: List[Any]) -> None:
    ...
    with _lock:
        _server_trust_levels[server_name] = _normalize_server_trust((config or {}).get('trust'))
...
def _trust_gate_check(server_name: str, tool_name: str) -> Optional[str]:
    ...
    try:
        from tools.approval import request_elicitation_consent
        answer = request_elicitation_consent(f"MCP tool '{tool_name}' on UNTRUSTED server '{server_name}' wants to run. This tool is write-capable (no readOnlyHint=true annotation) and may modify external state.", f"Server '{server_name}' is configured 'trust: untrusted'. Approve to run '{tool_name}' once, or deny to block it.", surface=f'mcp-trust/{server_name}')
    except Exception as exc:
        logger.error('MCP trust gate: approval check failed for %s.%s: %s', server_name, tool_name, exc, exc_info=True)
        return tool_error(f"MCP tool '{tool_name}' on untrusted server '{server_name}' was blocked: the approval system was unavailable (fail-closed).")
    if answer == 'accept':
        return None
    logger.info("MCP trust gate: user %s '%s' on untrusted server '%s'", 'cancelled' if answer == 'cancel' else 'denied', tool_name, server_name)
    return tool_error(f"The user did not approve running write-capable MCP tool '{tool_name}' on untrusted server '{server_name}'. The command was NOT run. Do not retry without explicit user direction.")
...
def _wait_for_server_session_ready(srv: 'MCPServerTask', *, old_session: Any=None, timeout: float=15.0) -> bool:
...
def _signal_reconnect_and_wait(server_name: str, srv: 'MCPServerTask', *, op_description: str, timeout: float=15.0) -> bool:
...
def _handle_auth_error_and_retry(server_name: str, exc: BaseException, retry_call, op_description: str):
    ...
    if recovered:
        ...
        if srv is not None and hasattr(srv, '_reconnect_event'):
            reconnected = _signal_reconnect_and_wait(server_name, srv, op_description=f'{op_description} after OAuth recovery', timeout=15)
        if reconnected:
    ...
    return tool_error(f"MCP server '{server_name}' requires re-authentication. Run `duck-agent mcp login {server_name}` (or delete the tokens file under ~/.duck-agent/mcp-tokens/ and restart). Do NOT retry this tool — ask the user to re-authenticate.", needs_reauth=True, server=server_name)
_SESSION_EXPIRED_MARKERS: tuple = ('invalid or expired session', 'expired session', 'session expired', 'session not found', 'unknown session', 'session terminated', 'closedresourceerror', 'closed resource', 'transport is closed', 'connection closed', 'broken pipe', 'end of file')
_EXC_TRAVERSAL_MAX_NODES = 10000
...
def _is_session_expired_error(exc: BaseException) -> bool:
    ...
    try:
        ...
        transport_error_types = (BrokenResourceError, ClosedResourceError, EndOfStream)
    except ImportError:
...
def _handle_session_expired_and_retry(server_name: str, exc: BaseException, retry_call, op_description: str):
    ...
    if not _signal_reconnect_and_wait(server_name, srv, op_description=op_description, timeout=15):
...
_MCP_DISCOVERY_LOCK_PATH: Optional[str] = None
...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(get_hermes_home() / '.mcp-discovery.lock')
...
def _snapshot_child_pids() -> set:
    ...
    try:
        ...
        pass
    return set()
_NON_MCP_CHILD_CMDLINE_MARKERS: tuple[str, ...] = ('tui_gateway.slash_worker', 'tui_gateway.entry', '-dorg.eclipse.equinox.launcher', 'eclipse.jdt.ls', 'org.eclipse.equinox.launcher_')
...
def _wrap_with_home_override(coro: 'Coroutine') -> 'Coroutine':
    """Carry the caller's context-local DUCK_AGENT_HOME override into ``coro``.
...
def _warn_hidden_whitespace(server_name: str, config: dict) -> List[str]:
    ...
    for key_path in flagged:
        ...
        logger.warning("MCP server '%s': config value '%s' has hidden leading or trailing whitespace — this often causes authentication or connection failures. Check for stray spaces/newlines in config.yaml (or the referenced env var).", server_name, key_path)
    return flagged
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Duck Agent config file.
    ...
    ``os.environ`` (which includes ``~/.duck-agent/.env`` loaded at startup).
    """
    try:
...
def _ensure_lazy_server_connected(server_name: str) -> bool:
    ...
    if phantom_names:
        ...
        for tool_name in phantom_names:
            registry.deregister(tool_name)
            _forget_mcp_tool_server(tool_name)
        logger.info("MCP server '%s': deregistered %d phantom cached tool(s) not served live (stale schema-cache fingerprint %s): %s", server_name, len(phantom_names), stale_fingerprint, ', '.join(phantom_names))
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        if _server_error_counts.get(server_name, 0) >= _CIRCUIT_BREAKER_THRESHOLD:
            ...
            if age < _CIRCUIT_BREAKER_COOLDOWN_SEC:
                ...
                return tool_error(f"MCP server '{server_name}' is unreachable after {_server_error_counts[server_name]} consecutive failures. Auto-retry available in ~{remaining}s. Do NOT retry this tool yet — use alternative approaches or ask the user to check the MCP server.")
        server = _get_connected_server_for_call(server_name)
        if not server:
            _bump_server_error(server_name)
            return tool_error(f"MCP server '{server_name}' is not connected")
        if not server.session:
            if _wait_for_server_session_ready(server, timeout=min(5.0, float(tool_timeout or 5.0))):
                pass
            else:
                _bump_server_error(server_name)
                if _signal_reconnect(server):
                    return tool_error(f"MCP server '{server_name}' transport is down; reconnect requested. Do NOT retry this tool immediately — give it a few seconds to come back.")
                return tool_error(f"MCP server '{server_name}' is not connected")
...
def _make_list_resources_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            _mark_server_call_started(server)
            async with server._rpc_lock:
                all_resources = await _paginate_full_list(server.session.list_resources, 'resources', server_name)
            resources = []
...
def _make_list_prompts_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            _mark_server_call_started(server)
            async with server._rpc_lock:
                all_prompts = await _paginate_full_list(server.session.list_prompts, 'prompts', server_name)
            prompts = []
            for p in all_prompts:
                ...
                if hasattr(p, 'arguments') and p.arguments:
                    entry['arguments'] = [{'name': a.name, **({'description': a.description} if hasattr(a, 'description') and a.description else {}), **({'required': a.required} if hasattr(a, 'required') else {})} for a in p.arguments]
        ...
        if isinstance(node, dict):
            ...
            for key, value in node.items():
                if key in ('properties', 'patternProperties') and isinstance(value, dict):
                    normalized[key] = {prop_name: _rewrite_local_refs(prop_schema) for prop_name, prop_schema in value.items()}
                else:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Duck Agent'ss historical behavior of converting hyphens to
    ...
    """
    return re.sub('[^A-Za-z0-9_]', '_', str(value or ''))
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Duck Agent registry schema format.
...
def _existing_tool_names() -> List[str]:
    ...
    with _lock:
        lazy_names = [n for sname, tool_names in _lazy_server_tool_names.items() if sname not in _servers for n in tool_names]
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    for mcp_tool in server._tools:
        ...
        candidates.append({'registry_name': schema['name'], 'origin': f'tool {mcp_tool.name!r}', 'schema': schema, 'handler': _make_tool_handler(name, mcp_tool.name, server.tool_timeout), 'check_fn': check_fn})
    handler_factories = {'list_resources': _make_list_resources_handler, 'read_resource': _make_read_resource_handler, 'list_prompts': _make_list_prompts_handler, 'get_prompt': _make_get_prompt_handler}
    for entry in _select_utility_schemas(name, server, config):
        schema = entry['schema']
        handler_key = entry['handler_key']
        candidates.append({'registry_name': schema['name'], 'origin': f'generated utility {handler_key!r}', 'schema': schema, 'handler': handler_factories[handler_key](name, server.tool_timeout), 'check_fn': check_fn})
    ...
    for candidate in candidates:
        ...
        if key in seen_candidates:
            logger.debug("MCP server '%s': duplicate registration candidate %s for '%s'; keeping one", name, candidate['origin'], candidate['registry_name'])
            continue
        seen_candidates.add(key)
        unique_candidates.append(candidate)
        origins_by_name.setdefault(candidate['registry_name'], set()).add(candidate['origin'])
    ambiguous_names = {registry_name: sorted(origins) for registry_name, origins in origins_by_name.items() if len(origins) > 1}
    for registry_name, origins in sorted(ambiguous_names.items()):
        logger.error("MCP server '%s': name normalization collision for '%s' from %s; skipping every colliding entry instead of choosing an arbitrary handler", name, registry_name, ', '.join(origins))
    for candidate in unique_candidates:
        ...
        if existing_toolset and existing_toolset != toolset_name:
            if existing_toolset.startswith('mcp-'):
                logger.error("MCP server '%s': %s normalizes to '%s', already owned by MCP toolset '%s' — skipping to preserve the existing owner", name, candidate['origin'], registry_name, existing_toolset)
            else:
                logger.warning("MCP server '%s': %s (→ '%s') collides with built-in tool in toolset '%s' — skipping to preserve built-in", name, candidate['origin'], registry_name, existing_toolset)
            continue
        registry.register(name=registry_name, toolset=toolset_name, schema=candidate['schema'], handler=candidate['handler'], check_fn=candidate['check_fn'], is_async=False, description=candidate['schema']['description'])
        if registry.get_toolset_for_tool(registry_name) != toolset_name:
            logger.error("MCP server '%s': registration of %s as '%s' was rejected by the registry; skipping provenance/count updates", name, candidate['origin'], registry_name)
            continue
    ...
    if registered_names:
        ...
        try:
            ...
            for mcp_tool in server._tools:
                ...
                tools_payload.append({'name': mcp_tool.name, 'description': mcp_tool.description or '', 'inputSchema': schema_obj if isinstance(schema_obj, dict) else {}, 'annotations': {'readOnlyHint': _annotation_read_only_hint(mcp_tool)}})
            utility_payload = [{'schema': entry['schema'], 'handler_key': entry['handler_key']} for entry in _select_utility_schemas(name, server, config)]
...
def _register_from_cache_sync(name: str, config: dict, entry: dict) -> List[str]:
    ...
    def _should_register(tool_name: str) -> bool:
        ...
        return True
    check_fn = _make_check_fn(name)
    cached_tool_objs = [SimpleNamespace(name=raw.get('name'), annotations=raw.get('annotations') if isinstance(raw.get('annotations'), dict) else None) for raw in tools_from_cache_entry(entry) if isinstance(raw, dict) and raw.get('name')]
    ...
    for raw in tools_from_cache_entry(entry):
        ...
        mcp_tool = _CachedMCPTool(raw_name, raw.get('description') or '', raw_schema if isinstance(raw_schema, dict) else {})
        _scan_mcp_description(name, mcp_tool.name, mcp_tool.description or '')
        schema = _convert_mcp_schema(name, mcp_tool)
        registry_name = schema['name']
        existing_toolset = registry.get_toolset_for_tool(registry_name)
        if existing_toolset and existing_toolset != toolset_name:
            logger.warning("MCP server '%s' (lazy): cached tool '%s' collides with toolset '%s' — skipping", name, registry_name, existing_toolset)
            continue
        registry.register(name=registry_name, toolset=toolset_name, schema=schema, handler=_make_tool_handler(name, raw_name, tool_timeout), check_fn=check_fn, is_async=False, description=schema['description'])
        if registry.get_toolset_for_tool(registry_name) != toolset_name:
            continue
        _track_mcp_tool_server(registry_name, name)
        registered_names.append(registry_name)
    ...
    for raw in utility_tools_from_cache_entry(entry):
        ...
        if existing_toolset and existing_toolset != toolset_name:
            continue
        registry.register(name=util_name, toolset=toolset_name, schema=schema, handler=handler_factories[handler_key](name, tool_timeout), check_fn=check_fn, is_async=False, description=schema.get('description') or '')
    ...
    if registered_names:
        ...
        logger.info("MCP server '%s' (lazy): registered %d tool(s) from schema cache", name, len(registered_names))
    return registered_names
...
async def _discover_and_register_server(name: str, config: dict) -> List[str]:
    ...
    try:
        ...
        if server is not None and server._error is not None and (task is not None) and (not task.done()) and (not task_cancelling):
            with _lock:
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    with _lock:
        ...
        new_servers = {k: v for k, v in servers.items() if k not in _servers and k not in connecting and (k not in _lazy_server_configs) and _parse_boolish(v.get('enabled', True), default=True) and (not _connect_cooldown_active(k))}
        stale_cached = [_servers[k] for k in servers if k in _servers and getattr(_servers[k], 'session', None) is None]
    ...
    if not new_servers:
        if lazy_registered:
            logger.info('MCP: registered %d lazy tool(s) from schema cache (no processes spawned)', lazy_registered)
        return _existing_tool_names()
    _ensure_mcp_loop()
    ...
    async def _discover_all():
        ...
        for name, result in zip(server_names, results):
            if isinstance(result, BaseException):
                ...
                logger.warning("Failed to connect to MCP server '%s'%s: %s", name, f' (command={command})' if command else '', message)
            else:
                with _lock:
                    _server_connecting.discard(name)
                    _server_connect_errors.pop(name, None)
    ...
    try:
        ...
        with _lock:
            ...
            if stale:
                logger.warning('MCP discovery %s while %d server(s) were still connecting; clearing stale connecting set: %s', 'timed out' if isinstance(_e, TimeoutError) else 'interrupted', len(stale), ', '.join(stale))
                _server_connecting.difference_update(stale)
                for _sn in stale:
                    _server_connect_errors.setdefault(_sn, f"Connection attempt {('timed out' if isinstance(_e, TimeoutError) else 'interrupted')} during discovery")
        raise
    finally:
        if _was_interrupted:
            _set_interrupt(True)
    with _lock:
        connected = [n for n in new_servers if n in _servers and n not in _server_connect_errors]
    ...
    if cookie is None:
        ...
        for _ in range(_MCP_DISCOVERY_LOCK_MAX_RETRIES):
            ...
            if cookie is not None:
                break
        if cookie is None:
            logger.warning('MCP discovery lock still held after %d retries -- running discovery unguarded', _MCP_DISCOVERY_LOCK_MAX_RETRIES)
        elif cookie is not _LOCK_UNAVAILABLE:
            logger.debug('Retry succeeded -- acquired MCP discovery lock')
    try:
        with _lock:
            connecting = set(_server_connecting)
            new_server_names = [name for name, cfg in servers.items() if name not in _servers and name not in connecting and _parse_boolish(cfg.get('enabled', True), default=True)]
        tool_names = register_mcp_servers(servers)
        if not new_server_names:
            return tool_names
        with _lock:
            connected_server_names = [name for name in new_server_names if name in _servers and name not in _server_connect_errors]
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        if server and server.session is not None:
            entry = {'name': name, 'transport': transport, 'tools': len(server._registered_tool_names) if hasattr(server, '_registered_tool_names') else len(server._tools), 'connected': True, 'disabled': False, 'status': 'connected'}
            if server._sampling:
                entry['sampling'] = dict(server._sampling.metrics)
            result.append(entry)
        elif not enabled:
            result.append({'name': name, 'transport': transport, 'tools': 0, 'connected': False, 'disabled': True, 'status': 'disabled'})
        elif name in connecting:
            result.append({'name': name, 'transport': transport, 'tools': 0, 'connected': False, 'disabled': False, 'status': 'connecting'})
        elif name in connect_errors:
            result.append({'name': name, 'transport': transport, 'tools': 0, 'connected': False, 'disabled': False, 'status': 'failed', 'error': connect_errors[name]})
        else:
            result.append({'name': name, 'transport': transport, 'tools': 0, 'connected': False, 'disabled': False, 'status': 'configured'})
    return result

def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    """Temporarily connect to configured MCP servers and list their tools.

    Designed for ``duck-agent tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Duck Agent registry.
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    try:
        ...
        if callable(get_mem_schemas):
            ...
            if memory_provider_tools_enabled(getattr(agent, 'enabled_toolsets', None), getattr(agent, 'disabled_toolsets', None), memory_tool_present='memory' in name_set):
...
def _kill_orphaned_mcp_children(include_active: bool=False, server_name: Optional[str]=None) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple duck-agent
    ...
    with _lock:
        ...
        if include_active:
            ...
            if server_name is not None:
                active = {pid: owner for pid, owner in active.items() if owner == server_name}
            pids.update(active)
            for pid in active:
    ...
    def _send_signal(pid: int, sig: int, server_name: str) -> None:
        ...
        if pgid is not None and killpg is not None:
            if _my_pgid is not None and pgid == _my_pgid:
                logger.warning("MCP server '%s' pgid %d matches gateway pgid; skipping killpg to avoid self-kill and using per-pid kill — any grandchildren in this group may not be reaped", server_name, pgid)
            else:
                try:
                    killpg(pgid, sig)
                    return
...
async def _drain_mcp_loop_tasks(*, timeout: float=_MCP_LOOP_DRAIN_TIMEOUT) -> None:
...
def _stop_mcp_loop(*, only_if_idle: bool=False) -> bool:
    ...
    if loop is not None:
        ...
        if loop.is_running():
            from agent.async_utils import safe_schedule_threadsafe
            future = safe_schedule_threadsafe(_drain_and_stop_mcp_loop(), loop, logger=logger, log_message='MCP loop drain: failed to schedule', log_level=logging.WARNING)
            if future is not None:
                stop_owned_by_loop = True
                try:
                    future.result(timeout=_MCP_LOOP_DRAIN_TIMEOUT + 1)
                except TimeoutError:
                    logger.warning('Timed out waiting for MCP loop drain after %.1fs', _MCP_LOOP_DRAIN_TIMEOUT + 1)
                except BaseException as exc:
                    logger.warning('Error draining MCP loop tasks: %s', exc)
        elif not loop.is_closed():
            try:
                loop.run_until_complete(_drain_mcp_loop_tasks(timeout=_MCP_LOOP_DRAIN_TIMEOUT))
            except BaseException as exc:
