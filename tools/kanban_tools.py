...
orchestrator work. A normal ``duck-agent chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``duck-agent kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``duck-agent kanban complete …``
   inside the container, where ``duck-agent`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.duck-agent/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``duck-agent kanban …``), the dashboard
(``duck-agent dashboard``), and the slash command (``/kanban …``) — all
...
def _reject_delegated_child_mutation(tool_name: str) -> Optional[str]:
    ...
    if not _is_delegated_child_context():
        return None
    return tool_error(f'{tool_name} refused: delegate_task child agents are not Kanban run owners. Return findings to the parent agent; the dispatcher worker or an explicitly configured Kanban orchestrator must perform board mutations.')
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``duck-agent chat`` without the kanban toolset see zero
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    if not env_tid:
        return None
    if tid != env_tid:
        return tool_error(f'worker is scoped to task {env_tid}; refusing to mutate {tid}. Use kanban_comment to hand off information to other tasks, or kanban_create to spawn follow-up work.')
    return None
...
def _connect(board: Optional[str]=None):
    ...
    the env-pinned active board without restarting Duck Agent.
    """
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    note = 'New note' + ('s' if len(fresh) > 1 else '') + ' on your kanban task from the operator (delivered mid-run). ' + "Take it into account for the work you're doing right now:\n" + '\n'.join(lines)
    try:
...
def _require_orchestrator_tool(tool_name: str) -> Optional[str]:
    ...
    if os.environ.get('HERMES_KANBAN_TASK'):
        return tool_error(f'{tool_name} is orchestrator-only; dispatcher-spawned workers must use kanban_complete, kanban_block, kanban_heartbeat, or kanban_comment for their assigned task.')
    return None
...
def _task_summary_dict(kb, conn, task) -> dict[str, Any]:
    ...
    return {'id': task.id, 'title': task.title, 'assignee': task.assignee, 'status': task.status, 'priority': task.priority, 'tenant': task.tenant, 'workspace_kind': task.workspace_kind, 'workspace_path': task.workspace_path, 'project_id': task.project_id, 'created_by': task.created_by, 'created_at': task.created_at, 'started_at': task.started_at, 'completed_at': task.completed_at, 'current_run_id': task.current_run_id, 'model_override': task.model_override, 'provider_override': task.provider_override, 'parents': parents, 'children': children, 'parent_count': len(parents), 'child_count': len(children)}
...
def _handle_show(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            def _task_dict(t):
                return {'id': t.id, 'title': t.title, 'body': t.body, 'assignee': t.assignee, 'status': t.status, 'tenant': t.tenant, 'priority': t.priority, 'workspace_kind': t.workspace_kind, 'workspace_path': t.workspace_path, 'created_by': t.created_by, 'created_at': t.created_at, 'started_at': t.started_at, 'completed_at': t.completed_at, 'result': t.result, 'current_run_id': t.current_run_id, 'model_override': t.model_override, 'provider_override': t.provider_override}

            def _run_dict(r):
                return {'id': r.id, 'profile': r.profile, 'status': r.status, 'outcome': r.outcome, 'summary': r.summary, 'error': r.error, 'metadata': r.metadata, 'started_at': r.started_at, 'ended_at': r.ended_at}
            return json.dumps({'task': _task_dict(task), 'parents': parents, 'children': children, 'comments': [{'author': c.author, 'body': c.body, 'created_at': c.created_at} for c in comments], 'events': [{'kind': e.kind, 'payload': e.payload, 'created_at': e.created_at, 'run_id': e.run_id} for e in events[-50:]], 'runs': [_run_dict(r) for r in runs], 'worker_context': kb.build_worker_context(conn, tid)})
        finally:
            conn.close()
...
def _handle_list(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            rows = kb.list_tasks(conn, assignee=assignee, status=status, tenant=tenant, include_archived=include_archived, limit=limit + 1)
            truncated = len(rows) > limit
            tasks = rows[:limit]
            return json.dumps({'tasks': [_task_summary_dict(kb, conn, t) for t in tasks], 'count': len(tasks), 'limit': limit, 'truncated': truncated, 'next_limit': min(limit * 2, KANBAN_LIST_MAX_LIMIT) if truncated and limit < KANBAN_LIST_MAX_LIMIT else None, 'promoted': promoted})
        finally:
            conn.close()
    except ValueError as e:
...
def _handle_complete(args: dict, **kw) -> str:
    ...
    if created_cards is not None:
        ...
        if not isinstance(created_cards, (list, tuple)):
            return tool_error(f'created_cards must be a list of task ids, got {type(created_cards).__name__}')
        created_cards = [str(c).strip() for c in created_cards if str(c).strip()]
    if artifacts is not None:
        if isinstance(artifacts, str):
            artifacts = [artifacts]
        if not isinstance(artifacts, (list, tuple)):
            return tool_error(f'artifacts must be a list of file paths, got {type(artifacts).__name__}')
        ...
        try:
            ...
            if task and task.goal_mode and _goal_judge_available():
                ...
                try:
                    ...
                    logger.warning('goal judge check failed, allowing completion: %s', judge_exc, exc_info=True)
                if verdict != 'done':
                    return tool_error(f"Goal completion rejected by judge: {reason}. To proceed, either: (1) provide explicit acceptance evidence in your summary matching the task's criteria, or (2) create continuation tasks with parents=[{tid}] and keep this task alive.")
            try:
                ok = kb.complete_task(conn, tid, result=result, summary=summary, metadata=metadata, created_cards=created_cards, expected_run_id=_worker_run_id(tid))
            except kb.ArtifactPreservationError as artifact_err:
                return tool_error(f'kanban_complete could not preserve the declared artifacts: {artifact_err}. Your task is still in-flight and its scratch workspace was kept. Fix the artifact path or storage error, then retry kanban_complete with the same handoff.')
            except kb.HallucinatedCardsError as hall_err:
                return tool_error(f"kanban_complete blocked: the following created_cards do not exist or were not created by this worker: {', '.join(hall_err.phantom)}. Your task is still in-flight (no state change). Retry kanban_complete with the same summary/metadata and either drop these ids from created_cards, or pass created_cards=[] to skip the card-claim check entirely.")
            if not ok:
...
def _handle_block(args: dict, **kw) -> str:
    ...
    try:
        ...
        if task and task.goal_mode and (kind not in _GOAL_MODE_BLOCK_ALLOWED_KINDS):
            conn.close()
            return tool_error(f'goal_mode tasks can only block with kind in {sorted(_GOAL_MODE_BLOCK_ALLOWED_KINDS)} (got {kind!r}). If the task is actually finished or cannot proceed for another reason, call kanban_complete instead — the completion judge will evaluate it.')
        try:
            ...
            return _ok(task_id=tid, run_id=run.id if run else None, status=landed.status if landed else 'blocked', block_kind=kind)
        finally:
            conn.close()
    except ValueError as e:
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            claim_lock = os.environ.get('HERMES_KANBAN_CLAIM_LOCK')
            kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            ok = kb.heartbeat_worker(conn, tid, note=note, expected_run_id=_worker_run_id(tid))
            if not ok:
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error("task_id is required (use the current task id if that's what you mean — pulls from env but kept explicit here)")
    body = args.get('body')
...
def _handle_attach(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            att_id = kb.store_attachment_bytes(conn, tid, str(filename), data, content_type=content_type, uploaded_by='agent', board=board)
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        chunks: list[bytes] = []
        total = 0
        with httpx.stream('GET', current_url, headers={'User-Agent': 'duck-agent-kanban/attach'}, timeout=30, follow_redirects=False) as resp:
            if resp.is_redirect:
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; Duck Agent downloads it (with the shared size cap)
    ...
    try:
        _, conn = _connect(board=board)
        try:
            att_id = kb.store_attachment_bytes(conn, tid, str(filename), data, content_type=content_type or fetched_ct, uploaded_by='agent', board=board)
            return _ok(task_id=tid, attachment_id=att_id, size=len(data))
        finally:
            conn.close()
    except kb.AttachmentTooLarge as e:
...
def _handle_attachments(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            return json.dumps({'ok': True, 'task_id': tid, 'attachments': [{'id': a.id, 'filename': a.filename, 'content_type': a.content_type, 'size': a.size, 'uploaded_by': a.uploaded_by, 'stored_path': a.stored_path, 'created_at': a.created_at} for a in atts]})
        finally:
            conn.close()
    except ValueError as e:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    assignee = args.get('assignee')
    if not assignee:
        return tool_error('assignee is required — name the profile that should execute this task (the dispatcher will only spawn tasks with an assignee)')
    body = args.get('body')
    parents = args.get('parents') or []
    tenant = args.get('tenant') or os.environ.get('HERMES_TENANT')
    from tools.async_delegation import _current_origin_session_id
    session_id = args.get('session_id') or _current_origin_session_id() or os.environ.get('HERMES_SESSION_ID')
    ...
    try:
        ...
        try:
            ...
            new_tid = kb.create_task(conn, title=str(title).strip(), body=body, assignee=str(assignee), parents=tuple(parents), tenant=tenant, priority=int(priority) if priority is not None else 0, workspace_kind=str(workspace_kind), workspace_path=workspace_path, project_id=project_id, project_source_task_id=project_source_task_id, triage=triage, idempotency_key=idempotency_key, max_runtime_seconds=int(max_runtime_seconds) if max_runtime_seconds is not None else None, skills=skills, model_override=model_override, provider_override=provider_override, goal_mode=goal_mode, goal_max_turns=int(goal_max_turns) if goal_max_turns is not None else None, initial_status=str(initial_status), created_by=os.environ.get('HERMES_PROFILE') or 'worker', session_id=session_id)
            new_task = kb.get_task(conn, new_tid)
            subscribed = _maybe_auto_subscribe(conn, new_tid)
            return _ok(task_id=new_tid, status=new_task.status if new_task else None, workspace_kind=new_task.workspace_kind if new_task else None, workspace_path=new_task.workspace_path if new_task else None, project_id=new_task.project_id if new_task else None, subscribed=subscribed)
        finally:
            conn.close()
    except ValueError as e:
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``duck-agent dashboard``).
    ...
    try:
        ...
        if platform.lower() == 'telegram' and thread_id and ((chat_type or '').lower() in {'dm', 'direct', 'private'}):
        ...
        _kb.add_notify_sub(conn, task_id=task_id, platform=platform, chat_id=chat_id, chat_type=chat_type, thread_id=thread_id, user_id=user_id, notifier_profile=notifier_profile, delivery_metadata=delivery_metadata or None)
        return True
...
def _handle_link(args: dict, **kw) -> str:
    ...
    try:
        ...
        return tool_error(f'kanban_link: {e}')
_DESC_TASK_ID_DEFAULT = 'Task id. If omitted, defaults to HERMES_KANBAN_TASK from the env (the task the dispatcher spawned you to work on).'
_DESC_BOARD = "Kanban board slug to target. When omitted, the call resolves the active board the usual way: HERMES_KANBAN_DB env → HERMES_KANBAN_BOARD env → the 'current' symlink under the kanban home → 'default'. Pass an explicit slug only when the caller (e.g. a Telegram routing layer) needs to override the env-pinned active board for this one call."
...
KANBAN_SHOW_SCHEMA = {'name': 'kanban_show', 'description': "Read a task's full state — title, body, assignee, parent task handoffs, your prior attempts on this task if any, comments, and recent events. Use this to (re)orient yourself before starting work, especially on retries. The response includes a pre-formatted ``worker_context`` string suitable for inclusion verbatim in your reasoning.", 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'board': _board_schema_prop()}, 'required': []}}
KANBAN_LIST_SCHEMA = {'name': 'kanban_list', 'description': 'List Kanban task summaries so an orchestrator profile can discover work to route. Supports the same core filters as the CLI: assignee, status, tenant, include_archived, and limit. Returns compact rows with ids, title, status, assignee, priority, parent/child ids, and counts. Bounded to 50 rows by default, 200 max, with truncation metadata. Also recomputes ready tasks before listing, matching the CLI. Orchestrator-only — dispatcher-spawned task workers never see this tool.', 'parameters': {'type': 'object', 'properties': {'assignee': {'type': 'string', 'description': 'Optional assignee/profile filter.'}, 'status': {'type': 'string', 'enum': ['triage', 'todo', 'ready', 'running', 'blocked', 'done', 'archived'], 'description': 'Optional task status filter.'}, 'tenant': {'type': 'string', 'description': 'Optional tenant/project namespace filter.'}, 'include_archived': {'type': 'boolean', 'description': 'Include archived tasks. Defaults to false.'}, 'limit': {'type': 'integer', 'description': 'Optional maximum rows to return (default 50, max 200).'}, 'board': _board_schema_prop()}, 'required': []}}
KANBAN_COMPLETE_SCHEMA = {'name': 'kanban_complete', 'description': 'Mark your current task done with a structured handoff for downstream workers and humans. Prefer ``summary`` for a human-readable 1-3 sentence description of what you did; put machine-readable facts in ``metadata`` (changed_files, tests_run, decisions, findings, etc). At least one of ``summary`` or ``result`` is required. If you created new tasks via ``kanban_create`` during this run, list their ids in ``created_cards`` — the kernel verifies them so phantom references are caught before they leak into downstream automation. If you produced deliverable files (charts, PDFs, spreadsheets, generated images), list their absolute paths in ``artifacts`` — the gateway notifier will upload them as native attachments to the human who subscribed to the task, so the deliverable lands in their chat alongside the summary instead of being a path they have to fetch by hand.', 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'summary': {'type': 'string', 'description': "Human-readable handoff, 1-3 sentences. Appears in Run History on the dashboard and in downstream workers' context."}, 'metadata': {'type': 'object', 'description': 'Free-form dict of structured facts about this attempt — {"changed_files": [...], "tests_run": 12, "findings": [...]}. Surfaced to downstream workers alongside ``summary``.'}, 'result': {'type': 'string', 'description': 'Short result log line (legacy field, maps to task.result). Use ``summary`` instead when possible; this exists for compatibility with callers that still set --result on the CLI.'}, 'created_cards': {'type': 'array', 'items': {'type': 'string'}, 'description': "Optional structured manifest of task ids you created via ``kanban_create`` during this run. The kernel verifies each id exists and was created by this worker's profile; any phantom id blocks the completion with an error listing what went wrong (auditable in the task's events). Only list ids you got back from a successful ``kanban_create`` call — do not invent or remember ids from prose. Omit the field if you did not create any cards."}, 'artifacts': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Optional list of absolute paths to deliverable files you produced during this run — generated charts, PDFs, spreadsheets, images, archives. Examples: ["/tmp/q3-revenue.png", "/tmp/report.pdf"]. The gateway notifier uploads each path as a native attachment to the subscribed chat (images embed inline, everything else uploads as a file) so the deliverable lands with the completion notification. Skip intermediate scratch files and references that are not the deliverable. The path must exist on disk at completion. Files inside a managed scratch workspace are copied to durable task attachments before cleanup; a missing declared scratch artifact keeps the task in-flight so you can fix the path and retry.'}, 'board': _board_schema_prop()}, 'required': []}}
KANBAN_BLOCK_SCHEMA = {'name': 'kanban_block', 'description': "Stop work on this task and route it according to WHY you're stuck. Set ``kind`` to say which: 'dependency' (waiting on another task — goes to todo and auto-resumes when that task finishes, no human needed), 'needs_input' (you need a human decision/answer), 'capability' (a hard wall: no access, missing credentials, an action no agent can do), or 'transient' (a flaky failure that may clear). ``reason`` is shown to the human on the board. If a task keeps getting unblocked and re-blocked for the same reason, it is auto-escalated to triage. Use for genuine blockers only — don't block on things you can resolve yourself.", 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'reason': {'type': 'string', 'description': "What you need answered or what stopped you, in one or two sentences. Don't paste the whole conversation; the human has the board and can ask follow-ups via comments."}, 'kind': {'type': 'string', 'enum': ['dependency', 'needs_input', 'capability', 'transient'], 'description': "Why you're blocked. 'dependency' waits in todo and resumes automatically; the others surface to a human. Omit only if none apply."}, 'board': _board_schema_prop()}, 'required': ['reason']}}
KANBAN_HEARTBEAT_SCHEMA = {'name': 'kanban_heartbeat', 'description': "Signal that you're still alive during a long operation (training, encoding, large crawls). Call every few minutes so humans see liveness separately from PID checks. Pure side effect — no work changes.", 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'note': {'type': 'string', 'description': 'Optional short note describing current progress. Shown in the event log.'}, 'board': _board_schema_prop()}, 'required': []}}
KANBAN_COMMENT_SCHEMA = {'name': 'kanban_comment', 'description': "Append a comment to a task's thread. Use for durable notes that should outlive this run (questions for the next worker, partial findings, rationale). Ephemeral reasoning doesn't belong here — use your normal response instead.", 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': "Task id. Required (may be your own task or another's — comment threads are per-task)."}, 'body': {'type': 'string', 'description': 'Markdown-supported comment body.'}, 'board': _board_schema_prop()}, 'required': ['task_id', 'body']}}
KANBAN_ATTACH_SCHEMA = {'name': 'kanban_attach', 'description': "Attach a file to a task by passing its bytes inline (base64). Use for genuine file artifacts the next worker or a human should be able to download — generated reports, images, exports. The file is stored as a real attachment (not a comment link) under the task's attachments dir, capped at 25 MB. Prefer kanban_attach_url when you only have a URL.", 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'filename': {'type': 'string', 'description': "File name to store it under (e.g. 'report.pdf'). Directory components are stripped; only the leaf is kept."}, 'content_base64': {'type': 'string', 'description': 'The file contents, base64-encoded. Max 25 MB decoded.'}, 'content_type': {'type': 'string', 'description': "Optional MIME type (e.g. 'application/pdf')."}, 'board': _board_schema_prop()}, 'required': ['filename', 'content_base64']}}
KANBAN_ATTACH_URL_SCHEMA = {'name': 'kanban_attach_url', 'description': 'Attach a file to a task by URL — Duck Agent downloads it server-side and stores it as a real attachment (capped at 25 MB). Use when you have a link rather than the bytes. Only http/https URLs are accepted.', 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'url': {'type': 'string', 'description': 'http(s) URL to fetch and store.'}, 'filename': {'type': 'string', 'description': "Optional name to store it under. Defaults to the URL path's leaf component."}, 'content_type': {'type': 'string', 'description': 'Optional MIME type override. Defaults to the Content-Type the server returns.'}, 'board': _board_schema_prop()}, 'required': ['url']}}
KANBAN_ATTACHMENTS_SCHEMA = {'name': 'kanban_attachments', 'description': 'List the files attached to a task: id, filename, content_type, size, who uploaded it, and the absolute on-disk path you can read.', 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': _DESC_TASK_ID_DEFAULT}, 'board': _board_schema_prop()}, 'required': []}}
KANBAN_CREATE_SCHEMA = {'name': 'kanban_create', 'description': 'Create a new kanban task, optionally as a child of the current one (pass the current task id in ``parents``). Used by orchestrator workers to fan out — decompose work into child tasks with specific assignees, link them into a pipeline, then complete your own task. The dispatcher picks up the new tasks on its next tick and spawns the assigned profiles.', 'parameters': {'type': 'object', 'properties': {'title': {'type': 'string', 'description': 'Short task title (required).'}, 'assignee': {'type': 'string', 'description': "Profile name that should execute this task (e.g. 'researcher-a', 'reviewer', 'writer'). Required — tasks without an assignee are never dispatched."}, 'body': {'type': 'string', 'description': 'Opening post: full spec, acceptance criteria, links. The assigned worker reads this as part of its context.'}, 'parents': {'type': 'array', 'items': {'type': 'string'}, 'description': "Parent task ids. The new task stays in 'todo' until every parent reaches 'done'; then it auto-promotes to 'ready'. Typical fan-in: list all the researcher task ids when creating a synthesizer task."}, 'tenant': {'type': 'string', 'description': 'Optional namespace for multi-project isolation. Defaults to HERMES_TENANT env if set.'}, 'priority': {'type': 'integer', 'description': 'Dispatcher tiebreaker. Higher = picked sooner when multiple ready tasks share an assignee.'}, 'workspace_kind': {'type': 'string', 'enum': ['scratch', 'dir', 'worktree'], 'description': "Workspace flavor: 'scratch' (fresh tmp dir, default), 'dir' (shared directory, requires absolute workspace_path), 'worktree' (git worktree)."}, 'workspace_path': {'type': 'string', 'description': "Absolute path for 'dir' or 'worktree' workspace. Relative paths are rejected at dispatch."}, 'project': {'type': 'string', 'description': "Optional project id or slug to link the task to. When set, the task becomes a git worktree under the project's primary repo with a deterministic branch (project slug + task id), instead of a random branch."}, 'triage': {'type': 'boolean', 'description': "If true, task lands in 'triage' instead of 'todo' — a specifier profile is expected to flesh out the body before work starts."}, 'idempotency_key': {'type': 'string', 'description': "If a non-archived task with this key already exists, return that task's id instead of creating a duplicate. Useful for retry-safe automation."}, 'max_runtime_seconds': {'type': 'integer', 'description': "Per-task runtime cap. When exceeded, the dispatcher SIGTERMs the worker and re-queues the task with outcome='timed_out'."}, 'initial_status': {'type': 'string', 'enum': ['running', 'blocked'], 'description': "Initial card status. Use 'blocked' for tasks that require immediate human ops (R3 gate) to skip the brief running-to-blocked transition. Defaults to 'running', which preserves the usual dispatch path."}, 'skills': {'type': 'array', 'items': {'type': 'string'}, 'description': "Skill names to force-load into the dispatched worker. The kanban lifecycle is already injected automatically; use this to pin a task to a specialist context — e.g. ['translation'] for a translation task, ['github-code-review'] for a reviewer task. The names must match skills installed on the assignee's profile."}, 'goal_mode': {'type': 'boolean', 'description': "Run the dispatched worker in a goal loop. When true, after each turn an auxiliary judge checks the worker's response against this card's title/body; if the work isn't done and budget remains, the worker keeps going in the same session until the judge agrees it's complete (or the goal-turn budget is exhausted, which blocks the task for human review). Use this for open-ended cards where one shot rarely finishes the work. Defaults to false (classic single-shot worker)."}, 'goal_max_turns': {'type': 'integer', 'description': 'Turn budget for goal_mode workers. Caps how many continuation turns the worker may take before the task is blocked for review. Ignored unless goal_mode is true. Defaults to the goal-engine default (20).'}, 'model': {'type': 'string', 'description': "Pin the dispatched worker to this model instead of the assignee profile's configured model. Use the exact model name the target provider expects. Omit to use the profile default."}, 'provider': {'type': 'string', 'description': "Provider the 'model' belongs to (e.g. 'openrouter', 'anthropic', 'nous'). Set this whenever the model is not from the assignee profile's configured provider — a model name alone is resolved against the profile's provider and will fail if it belongs to a different one. Requires 'model'."}, 'board': _board_schema_prop()}, 'required': ['title', 'assignee']}}
KANBAN_UNBLOCK_SCHEMA = {'name': 'kanban_unblock', 'description': 'Unblock a Kanban task. It moves to ready when all parents are done, or todo while any parent remains open. Orchestrator-only — only profiles with the kanban toolset can unblock routed work; dispatcher-spawned task workers never see this tool.', 'parameters': {'type': 'object', 'properties': {'task_id': {'type': 'string', 'description': 'Blocked task id to move to ready or parent-gated todo.'}, 'board': _board_schema_prop()}, 'required': ['task_id']}}
KANBAN_LINK_SCHEMA = {'name': 'kanban_link', 'description': "Add a parent→child dependency edge after both tasks already exist. The child won't promote to 'ready' until all parents are 'done'. Cycles and self-links are rejected.", 'parameters': {'type': 'object', 'properties': {'parent_id': {'type': 'string', 'description': 'Parent task id.'}, 'child_id': {'type': 'string', 'description': 'Child task id.'}, 'board': _board_schema_prop()}, 'required': ['parent_id', 'child_id']}}
registry.register(name='kanban_show', toolset='kanban', schema=KANBAN_SHOW_SCHEMA, handler=_handle_show, check_fn=_check_kanban_mode, emoji='📋')
registry.register(name='kanban_list', toolset='kanban', schema=KANBAN_LIST_SCHEMA, handler=_handle_list, check_fn=_check_kanban_orchestrator_mode, emoji='📋')
registry.register(name='kanban_complete', toolset='kanban', schema=KANBAN_COMPLETE_SCHEMA, handler=_handle_complete, check_fn=_check_kanban_mode, emoji='✔')
registry.register(name='kanban_block', toolset='kanban', schema=KANBAN_BLOCK_SCHEMA, handler=_handle_block, check_fn=_check_kanban_mode, emoji='⏸')
registry.register(name='kanban_heartbeat', toolset='kanban', schema=KANBAN_HEARTBEAT_SCHEMA, handler=_handle_heartbeat, check_fn=_check_kanban_mode, emoji='💓')
registry.register(name='kanban_comment', toolset='kanban', schema=KANBAN_COMMENT_SCHEMA, handler=_handle_comment, check_fn=_check_kanban_mode, emoji='💬')
registry.register(name='kanban_attach', toolset='kanban', schema=KANBAN_ATTACH_SCHEMA, handler=_handle_attach, check_fn=_check_kanban_mode, emoji='📎')
registry.register(name='kanban_attach_url', toolset='kanban', schema=KANBAN_ATTACH_URL_SCHEMA, handler=_handle_attach_url, check_fn=_check_kanban_mode, emoji='📎')
registry.register(name='kanban_attachments', toolset='kanban', schema=KANBAN_ATTACHMENTS_SCHEMA, handler=_handle_attachments, check_fn=_check_kanban_mode, emoji='📎')
registry.register(name='kanban_create', toolset='kanban', schema=KANBAN_CREATE_SCHEMA, handler=_handle_create, check_fn=_check_kanban_mode, emoji='➕')
registry.register(name='kanban_unblock', toolset='kanban', schema=KANBAN_UNBLOCK_SCHEMA, handler=_handle_unblock, check_fn=_check_kanban_orchestrator_mode, emoji='▶')
registry.register(name='kanban_link', toolset='kanban', schema=KANBAN_LINK_SCHEMA, handler=_handle_link, check_fn=_check_kanban_mode, emoji='🔗')