"""Shared debug session infrastructure for Duck Agent tools.
...
class DebugSession:
    ...
    def save(self) -> None:
        ...
        try:
            ...
            payload = {'session_id': self.session_id, 'start_time': self._start_time, 'end_time': datetime.datetime.now().isoformat(), 'debug_enabled': True, 'total_calls': len(self._calls), 'tool_calls': self._calls}
    ...
    def get_session_info(self) -> Dict[str, Any]:
        ...
        return {'enabled': True, 'session_id': self.session_id, 'log_path': str(self.log_dir / f'{self.tool_name}_debug_{self.session_id}.json'), 'total_calls': len(self._calls)}