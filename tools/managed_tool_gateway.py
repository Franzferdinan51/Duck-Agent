...
def auth_json_path():
    """Return the Duck Agent auth store path, respecting DUCK_AGENT_HOME overrides."""
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`duck-agent tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        ...
        refreshed_token = resolve_nous_access_token(refresh_skew_seconds=_NOUS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS)
...
def managed_vendor_endpoints(vendor: str, gateway_builder: Optional[Callable[[str], str]]=None) -> Optional[dict]:
    ...
    if not origin:
        return None
    return {'origin': origin, 'base_url': f'{origin}{managed_vendor_base_path(vendor)}', 'upload_path': managed_vendor_upload_path(vendor)}

def is_managed_nous_gateway_url(url: object, gateway_builder: Optional[Callable[[str], str]]=None) -> bool:
...
def managed_gateway_auth_headers(url: object, gateway_builder: Optional[Callable[[str], str]]=None, token_reader: Optional[Callable[[], Optional[str]]]=None) -> dict:
...
def build_managed_media_uploader(server_url: object, upload_path: object, gateway_builder: Optional[Callable[[str], str]]=None, token_reader: Optional[Callable[[], Optional[str]]]=None) -> Optional[Callable]:
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=presign_timeout) as client:
            presign = await client.post(presign_url, headers=headers, json={'contentType': mime, 'contentLength': len(data)})
        ...
        put_timeout = httpx.Timeout(_MEDIA_UPLOAD_PRESIGN_TIMEOUT_SECONDS, read=_MEDIA_UPLOAD_PUT_READ_TIMEOUT_SECONDS, write=_MEDIA_UPLOAD_PUT_WRITE_TIMEOUT_SECONDS)
