...
def _parse_pypi_package(token: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    match = re.match('^([a-zA-Z0-9._-]+)(?:\\[[^\\]]*\\])?(?:==(.+))?$', token)
    if match:
        return (match.group(1), match.group(2))
    return (token, None)
...
def _query_osv(package: str, ecosystem: str, version: Optional[str]=None) -> list:
    ...
    req = urllib.request.Request(_OSV_ENDPOINT, data=data, headers={'Content-Type': 'application/json', 'User-Agent': 'duck-agent-osv-check/1.0'}, method='POST')
