...
Copies bundled skills from the repo's skills/ directory into ~/.duck-agent/skills/
...
The manifest lives at ~/.duck-agent/skills/.bundled_manifest.
"""
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.duck-agent/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.duck-agent/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``duck-agent skills uninstall``),
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.duck-agent/skills/ using the manifest.
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``duck-agent update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``duck-agent update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``duck-agent skills reset`` back to upstream.
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes DUCK_AGENT_HOME/.no-bundled-skills so the
    installer, ``duck-agent update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``duck-agent skills opt-out`` /
