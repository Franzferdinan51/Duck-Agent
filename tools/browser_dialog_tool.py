...
def browser_dialog(action: str, prompt_text: Optional[str]=None, dialog_id: Optional[str]=None, task_id: Optional[str]=None) -> str:
    ...
    if supervisor is None:
        return json.dumps({'success': False, 'error': "No CDP supervisor is attached to this task. Either the browser backend doesn't expose CDP (Camofox, default Playwright) or no browser session has been started yet. Call browser_navigate or /browser connect first."})
...
registry.register(name='browser_dialog', toolset='browser-cdp', schema=BROWSER_DIALOG_SCHEMA, handler=lambda args, **kw: browser_dialog(action=args.get('action', ''), prompt_text=args.get('prompt_text'), dialog_id=args.get('dialog_id'), task_id=kw.get('task_id')), check_fn=_browser_dialog_check, emoji='💬')