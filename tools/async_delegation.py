...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    for name, sql_type in (('owner_pid', 'INTEGER'), ('owner_started_at', 'INTEGER'), ('task_json', 'TEXT'), ('delivery_claim', 'TEXT'), ('delivery_claimed_at', 'REAL'), ('origin_session_id', 'TEXT')):
...
def _persist_dispatch(record: Dict[str, Any]) -> None:
    ...
    task_payload = {key: record.get(key) for key in ('goal', 'goals', 'context', 'toolsets', 'role', 'model', 'is_batch') if key in record}
    with _DB_LOCK, _transaction() as conn:
        conn.execute("INSERT OR REPLACE INTO async_delegations\n               (delegation_id, origin_session, origin_ui_session_id,\n                parent_session_id, state, dispatched_at, updated_at,\n                delivery_state, delivery_attempts, owner_pid,\n                owner_started_at, task_json, origin_session_id)\n               VALUES (?, ?, ?, ?, 'running', ?, ?, 'pending', 0, ?, ?, ?, ?)", (record['delegation_id'], record.get('session_key', ''), record.get('origin_ui_session_id', ''), record.get('parent_session_id'), record['dispatched_at'], now, __import__('os').getpid(), owner_started_at, json.dumps(task_payload), record.get('origin_session_id', '')))
...
def _prune_durable_records() -> None:
    ...
    with _DB_LOCK, _transaction() as conn:
        conn.execute("DELETE FROM async_delegations WHERE delivery_state='delivered' AND updated_at < ?", (cutoff,))
        terminal_count = conn.execute("SELECT COUNT(*) FROM async_delegations WHERE state NOT IN ('running','finalizing')").fetchone()[0]
...
def _persist_completion(event: Dict[str, Any], result: Dict[str, Any]) -> None:
    now = time.time()
    with _DB_LOCK, _transaction() as conn:
        conn.execute("UPDATE async_delegations SET state=?, completed_at=?, updated_at=?,\n               event_json=?, result_json=?, delivery_state='pending'\n               WHERE delegation_id=?", (event.get('status', 'completed'), event.get('completed_at', now), now, json.dumps(event), json.dumps(result), event['delegation_id']))

def _note_delivery_attempt(delegation_id: str) -> None:
    with _DB_LOCK, _transaction() as conn:
        conn.execute('UPDATE async_delegations SET delivery_attempts=delivery_attempts+1, updated_at=? WHERE delegation_id=?', (time.time(), delegation_id))
...
def recover_abandoned_delegations() -> int:
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        for row in rows:
            delegation_id, session_key, origin_ui, parent_id, dispatched_at, pid, started, task_json, origin_session_id = row
            live = False
            if pid:
            ...
            event = {'type': 'async_delegation', 'delegation_id': delegation_id, 'session_key': session_key, 'origin_ui_session_id': origin_ui, 'origin_session_id': origin_session_id or '', 'parent_session_id': parent_id, 'goal': task.get('goal', ''), 'goals': task.get('goals'), 'context': task.get('context'), 'toolsets': task.get('toolsets'), 'role': task.get('role'), 'model': task.get('model'), 'is_batch': bool(task.get('is_batch')), 'status': 'unknown', 'summary': None, 'error': 'Delegation owner exited before recording a terminal result; outcome unknown.', 'dispatched_at': dispatched_at, 'completed_at': now}
...
def mark_completion_delivered(delegation_id: str) -> bool:
    ...
    now = time.time()
    with _DB_LOCK, _transaction() as conn:
        cur = conn.execute("UPDATE async_delegations SET delivery_state='delivered', delivered_at=?, updated_at=?\n               WHERE delegation_id=? AND delivery_state!='delivered'", (now, now, delegation_id))
...
def claim_completion_delivery(delegation_id: str, claim_id: str) -> bool:
    ...
    now = time.time()
    with _DB_LOCK, _transaction() as conn:
        row = conn.execute('SELECT delivery_state FROM async_delegations WHERE delegation_id=?', (delegation_id,)).fetchone()
        if row is None:
            return True
        cur = conn.execute("UPDATE async_delegations SET delivery_claim=?, delivery_claimed_at=?,\n                      delivery_attempts=delivery_attempts+1, updated_at=?\n               WHERE delegation_id=? AND delivery_state='pending'\n                 AND (delivery_claim IS NULL OR delivery_claimed_at < ?)", (claim_id, now, now, delegation_id, now - 300))
        return cur.rowcount == 1
...
def release_completion_delivery(delegation_id: str, claim_id: str) -> bool:
    ...
    now = time.time()
    with _DB_LOCK, _transaction() as conn:
        capped = conn.execute("UPDATE async_delegations SET delivery_state='dropped',\n                      delivery_claim=NULL, delivery_claimed_at=NULL, updated_at=?\n               WHERE delegation_id=? AND delivery_state='pending'\n                 AND delivery_claim=? AND delivery_attempts>=?", (now, delegation_id, claim_id, _MAX_DELIVERY_ATTEMPTS))
...
def drop_completion_delivery(delegation_id: str, claim_id: str) -> bool:
    ...
    now = time.time()
    with _DB_LOCK, _transaction() as conn:
        cur = conn.execute("UPDATE async_delegations SET delivery_state='dropped',\n                      updated_at=?, delivery_claim=NULL,\n                      delivery_claimed_at=NULL\n               WHERE delegation_id=? AND delivery_state='pending'\n                 AND delivery_claim=?", (now, delegation_id, claim_id))
        return cur.rowcount == 1
...
def get_durable_delegation(delegation_id: str) -> Optional[Dict[str, Any]]:
    ...
    if row is None:
        return None
    return {'delegation_id': delegation_id, 'origin_session': row[0], 'state': row[1], 'dispatched_at': row[2], 'completed_at': row[3], 'result': json.loads(row[4]) if row[4] else None, 'delivery_state': row[5], 'delivery_attempts': row[6], 'origin_session_id': row[7] or ''}
...
def _get_executor(max_workers: int) -> ThreadPoolExecutor:
    ...
    with _executor_lock:
        if _executor is None or max_workers > _executor_max_workers:
            _executor = _DaemonThreadPoolExecutor(max_workers=max_workers, thread_name_prefix='async-delegate')
...
def active_count() -> int:
    ...
    """
    with _records_lock:
        return sum((1 for r in _records.values() if r.get('status') in {'running', 'stalling', 'finalizing'}))
...
def active_for_session(origin_ui_session_id: str) -> int:
    ...
    if not origin_ui_session_id:
        return 0
    with _records_lock:
        return sum((1 for r in _records.values() if r.get('status') in {'running', 'stalling', 'finalizing'} and str(r.get('origin_ui_session_id') or '') == origin_ui_session_id))
...
def _matches_session_selectors(record: Dict[str, Any], *, session_key: str='', origin_ui_session_id: str='', parent_session_id: str='') -> bool:
    return origin_ui_session_id and str(record.get('origin_ui_session_id') or '') == origin_ui_session_id or (session_key and str(record.get('session_key') or '') == session_key) or (parent_session_id and str(record.get('parent_session_id') or '') == parent_session_id)
...
def has_live_for_session(session_key: str='', origin_ui_session_id: str='', parent_session_id: str='') -> bool:
    ...
    if not session_key and (not origin_ui_session_id) and (not parent_session_id):
        return False
    with _records_lock:
        return any((r.get('status') in {'running', 'stalling', 'finalizing'} and _matches_session_selectors(r, session_key=session_key, origin_ui_session_id=origin_ui_session_id, parent_session_id=parent_session_id) for r in _records.values()))
...
def _current_origin_session_id() -> str:
    ...
    ``X-Duck Agent-Session-Id``, and its only writer is ``set_session_vars`` —
...
def dispatch_async_delegation(*, goal: str, context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str], session_key: str, parent_session_id: Optional[str]=None, runner: Callable[[], Dict[str, Any]], origin_ui_session_id: str='', origin_session_id: str='', interrupt_fn: Optional[Callable[[], None]]=None, max_async_children: int=_DEFAULT_MAX_ASYNC_CHILDREN, progress_fn: Optional[Callable[[], tuple]]=None) -> Dict[str, Any]:
    ...
    record: Dict[str, Any] = {'delegation_id': delegation_id, 'goal': goal, 'context': context, 'toolsets': list(toolsets) if toolsets else None, 'role': role, 'model': model, 'session_key': session_key, 'origin_ui_session_id': origin_ui_session_id, 'origin_session_id': origin_session_id, 'parent_session_id': parent_session_id, 'status': 'running', 'dispatched_at': dispatched_at, 'completed_at': None, 'interrupt_fn': interrupt_fn, 'progress_fn': progress_fn, '_progress_token': None, '_progress_ts': dispatched_at, '_interrupted_at': None}
    with _records_lock:
        running = sum((1 for r in _records.values() if r.get('status') in ('running', 'stalling')))
        if running >= max_async_children:
            return {'status': 'rejected', 'error': f'Async delegation capacity reached ({max_async_children} running). Wait for one to finish (its result will re-enter the chat), or run this task synchronously (background=false). Raise delegation.max_concurrent_children in config.yaml to allow more concurrent background subagents.'}
    ...
    def _worker() -> None:
        ...
        try:
            ...
            result = {'status': 'error', 'summary': None, 'error': f'{type(exc).__name__}: {exc}', 'api_calls': 0, 'duration_seconds': round(time.time() - dispatched_at, 2)}
            status = 'error'
        finally:
    ...
    logger.info('Dispatched async delegation %s (session_key=%s): %s', delegation_id, session_key or '<cli>', (goal or '')[:80])
...
def _begin_finalization(delegation_id: str) -> Optional[tuple[Dict[str, Any], Optional[Callable[[], None]]]]:
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    try:
        ...
        logger.error('Async delegation %s finished but process_registry import failed; result lost: %s', record.get('delegation_id'), exc)
        return
    summary = result.get('summary')
    error = result.get('error')
    dispatched_at = record.get('dispatched_at') or time.time()
    completed_at = record.get('completed_at') or time.time()
    evt = {'type': 'async_delegation', 'delegation_id': record.get('delegation_id'), 'session_key': record.get('session_key', ''), 'origin_ui_session_id': record.get('origin_ui_session_id', ''), 'origin_session_id': record.get('origin_session_id', ''), 'parent_session_id': record.get('parent_session_id'), 'goal': record.get('goal', ''), 'context': record.get('context'), 'toolsets': record.get('toolsets'), 'role': record.get('role'), 'model': result.get('model') or record.get('model'), 'status': status, 'summary': summary, 'error': error, 'api_calls': result.get('api_calls', 0), 'duration_seconds': result.get('duration_seconds', round(completed_at - dispatched_at, 2)), 'dispatched_at': dispatched_at, 'completed_at': completed_at, 'exit_reason': result.get('exit_reason')}
    for _k in ('stalled_after_quiet_seconds', 'stall_threshold_seconds', 'stall_phase', 'stall_grace_seconds'):
        if _k in result:
            evt[_k] = result[_k]
    ...
    try:
        ...
        logger.error('Async delegation %s: failed to enqueue completion event; result lost: %s', record.get('delegation_id'), exc)

def dispatch_async_delegation_batch(*, goals: List[str], context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str], session_key: str, parent_session_id: Optional[str]=None, runner: Callable[[], Dict[str, Any]], origin_ui_session_id: str='', origin_session_id: str='', interrupt_fn: Optional[Callable[[], None]]=None, max_async_children: int=_DEFAULT_MAX_ASYNC_CHILDREN, delegation_id: Optional[str]=None, progress_fn: Optional[Callable[[], tuple]]=None) -> Dict[str, Any]:
    ...
    record: Dict[str, Any] = {'delegation_id': delegation_id, 'goal': combined_goal, 'goals': list(goals), 'context': context, 'toolsets': list(toolsets) if toolsets else None, 'role': role, 'model': model, 'session_key': session_key, 'origin_ui_session_id': origin_ui_session_id, 'origin_session_id': origin_session_id, 'parent_session_id': parent_session_id, 'status': 'running', 'dispatched_at': dispatched_at, 'completed_at': None, 'interrupt_fn': interrupt_fn, 'is_batch': True, 'progress_fn': progress_fn, '_progress_token': None, '_progress_ts': dispatched_at, '_interrupted_at': None}
    with _records_lock:
        running = sum((1 for r in _records.values() if r.get('status') in ('running', 'stalling')))
        if running >= max_async_children:
            return {'status': 'rejected', 'error': f'Async delegation capacity reached ({max_async_children} running). Wait for one to finish (its result will re-enter the chat), or raise delegation.max_concurrent_children in config.yaml to allow more concurrent background units.'}
        _records[delegation_id] = record
    _persist_dispatch(record)
    executor = _get_executor(max_async_children)
    ...
    def _worker() -> None:
        ...
        try:
            ...
            if child_results and all((r.get('status') not in ('completed', 'success') for r in child_results)):
                status = 'error'
            else:
                status = 'completed'
        except Exception as exc:
    ...
    try:
        ...
        with _records_lock:
            _records.pop(delegation_id, None)
        _delete_durable_delegation(delegation_id)
        return {'status': 'rejected', 'error': f'Failed to schedule async delegation batch: {exc}'}
    if progress_fn is not None:
        _ensure_stale_monitor()
    logger.info('Dispatched async delegation batch %s (%d task(s), session_key=%s)', delegation_id, n, session_key or '<cli>')
    return {'status': 'dispatched', 'delegation_id': delegation_id}
...
def _push_batch_completion_event(event_record: Dict[str, Any], combined: Dict[str, Any], status: str) -> None:
    ...
    try:
        ...
        logger.error('Async delegation batch %s: failed to enqueue completion event; result lost: %s', event_record.get('delegation_id'), exc)
...
def _ensure_stale_monitor() -> None:
    ...
    with _monitor_lock:
        if _monitor_thread is not None and _monitor_thread.is_alive():
            return
        _monitor_stop.clear()
        _monitor_thread = threading.Thread(target=_stale_monitor_loop, name='async-delegate-stale-monitor', daemon=True)
        ...
        with _records_lock:
            for record in _records.values():
                ...
                if quiet_for >= limit:
                    ...
                    stalled.append((record['delegation_id'], bool(record.get('is_batch')), quiet_for, in_tool))
        for delegation_id, _is_batch, quiet_for, in_tool in stalled:
            logger.warning('Async delegation %s made no progress for %.0fs (in_tool=%s) — interrupting; grace window %.0fs', delegation_id, quiet_for, in_tool, _STALL_GRACE_SECONDS)
            with _records_lock:
                record = _records.get(delegation_id)
            ...
            if callable(fn):
                try:
                    ...
                    logger.debug('Async delegation %s stall interrupt failed: %s', delegation_id, exc)
...
def _finalize_stalled(delegation_id: str) -> None:
    ...
    error = f'Async delegation {delegation_id} stalled: the detached subagent stopped making progress (no new API calls, tool activity, or streamed tokens), did not respond to interruption, and never produced a completion event. The worker may be wedged inside a model API call — this is a known failure mode of long-lived gateway processes (#60203). Re-dispatch the task if it is still needed.'
    logger.error('Async delegation %s force-finalized as stalled after %.0fs', delegation_id, duration)
    stall_meta = {'stalled_after_quiet_seconds': quiet_seconds, 'stall_threshold_seconds': threshold_seconds, 'stall_phase': 'in_tool' if stall_in_tool else 'idle' if stall_in_tool is not None else None, 'stall_grace_seconds': _STALL_GRACE_SECONDS}
    if event_record.get('is_batch'):
        _push_batch_completion_event(event_record, {'results': [], 'error': error, 'total_duration_seconds': duration, **stall_meta}, 'stalled')
    else:
        _push_completion_event(event_record, {'status': 'stalled', 'summary': None, 'error': error, 'api_calls': 0, 'duration_seconds': duration, 'exit_reason': 'stalled', **stall_meta}, 'stalled')
    ...
    with _records_lock:
        ...
        for r in _records.values():
            ...
            if status in ('stalling', 'stalled'):
                for src, dst in (('_stall_quiet_seconds', 'stalled_after_quiet_seconds'), ('_stall_threshold_seconds', 'stall_threshold_seconds'), ('_stall_in_tool', 'stall_in_tool')):
...
def interrupt_all(reason: str='shutdown') -> int:
    ...
    count = 0
    with _records_lock:
        targets = [r for r in _records.values() if r.get('status') in ('running', 'stalling')]
    for r in targets:
        ...
        if callable(fn):
            try:
                ...
                logger.debug('interrupt_all: %s interrupt failed: %s', r.get('delegation_id'), exc)
    if count:
...
def interrupt_for_session(session_key: str='', origin_ui_session_id: str='', parent_session_id: str='', reason: str='session_end') -> int:
    ...
    count = 0
    with _records_lock:
        targets = [r for r in _records.values() if r.get('status') in ('running', 'stalling') and _matches_session_selectors(r, session_key=session_key, origin_ui_session_id=origin_ui_session_id, parent_session_id=parent_session_id)]
    for r in targets:
        fn = r.get('interrupt_fn')
        if callable(fn):
            try:
                ...
                logger.debug('interrupt_for_session: %s interrupt failed: %s', r.get('delegation_id'), exc)
    if count:
        logger.info('Interrupted %d async delegation(s) for ending session (%s)', count, reason)
    return count
