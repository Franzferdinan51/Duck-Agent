...
def _worker_memory_max_bytes() -> int:
    ...
    if override:
        ...
        if not override_valid:
            logger.warning('Ignoring invalid TERMINAL_LOCAL_MEMORY_MAX_MB=%r; expected an integer representing at least %d MiB', override, _MIN_WORKER_MEMORY_MAX_BYTES // (1024 * 1024))
    candidates: List[int] = []
    try:
    ...
    try:
        ...
        physical_bound = min(_WORKER_MEMORY_MAX_CAP_BYTES, max(_MIN_WORKER_MEMORY_MAX_BYTES, physical_bytes // 2))
...
def _systemd_run_user_scope_available() -> bool:
    ...
    if cached is True:
        return True
    if cached is False and now - _SYSTEMD_SCOPE_PROBED_AT < _SYSTEMD_SCOPE_FAILURE_TTL_SECONDS:
        return False
    with _SYSTEMD_SCOPE_PROBE_LOCK:
        ...
        if not _IS_WINDOWS:
            try:
                ...
                if binary:
                    probe_unit = f'duck-agent-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}'
                    result = subprocess.run([binary, '--user', '--scope', '--quiet', '--unit', probe_unit, '--collect', '--property', 'MemoryAccounting=yes', '--property', f'MemoryMax={_worker_memory_max_bytes()}', '--property', 'OOMPolicy=kill', '--', '/bin/true'], capture_output=True, timeout=3)
                    available = result.returncode == 0
                    if not available:
                        logger.debug('systemd-run --user --scope probe failed (rc=%s): %s', result.returncode, (result.stderr or b'').decode('utf-8', 'replace').strip())
...
def _build_systemd_scope_argv(shell_argv: List[str], unit_suffix: str) -> List[str]:
    ...
    if binary is None:
        return shell_argv
    unit_name = f'duck-agent-worker-{unit_suffix}'
    memory_max = _worker_memory_max_bytes()
    return [binary, '--user', '--scope', '--quiet', '--unit', unit_name, '--collect', '--property', 'MemoryAccounting=yes', '--property', f'MemoryMax={memory_max}', '--property', 'OOMPolicy=kill', '--', *shell_argv]
...
def _stop_systemd_unit(unit_name: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if any((marker in stderr_lower for marker in ('not loaded', 'not found', 'does not exist'))):
                return True
...
class ProcessRegistry:
    ...
    """
    _SHELL_NOISE_SUBSTRINGS = ('bash: cannot set terminal process group', 'bash: no job control in this shell', 'no job control in this shell', 'cannot set terminal process group', 'tcsetattr: Inappropriate ioctl for device')
    ...
    def _check_watch_patterns(self, session: ProcessSession, new_text: str) -> None:
        ...
        if return_early:
            if should_disable:
                self.completion_queue.put({'session_id': session.id, 'session_key': session.session_key, 'command': session.command, 'type': 'watch_disabled', 'suppressed': session._watch_suppressed, 'platform': session.watcher_platform, 'chat_id': session.watcher_chat_id, 'user_id': session.watcher_user_id, 'user_name': session.watcher_user_name, 'thread_id': session.watcher_thread_id, 'message_id': session.watcher_message_id, 'message': f"Watch patterns disabled for process {session.id} — {WATCH_STRIKE_LIMIT} consecutive rate-limit windows triggered (min spacing {WATCH_MIN_INTERVAL_SECONDS}s). Falling back to notify_on_complete semantics; you'll get exactly one notification when the process exits."})
            return
        ...
        if not self._global_watch_admit(now):
            return
        self.completion_queue.put({'session_id': session.id, 'session_key': session.session_key, 'command': session.command, 'type': 'watch_match', 'pattern': matched_pattern, 'output': output, 'suppressed': suppressed, 'platform': session.watcher_platform, 'chat_id': session.watcher_chat_id, 'user_id': session.watcher_user_id, 'user_name': session.watcher_user_name, 'thread_id': session.watcher_thread_id, 'message_id': session.watcher_message_id})
    ...
    def _global_watch_admit(self, now: float) -> bool:
        ...
        with self._global_watch_lock:
            if self._global_watch_tripped_until and now >= self._global_watch_tripped_until:
                ...
                if suppressed > 0:
                    release_msg = {'session_id': '', 'session_key': '', 'command': '', 'type': 'watch_overflow_released', 'suppressed': suppressed, 'message': f'Watch-pattern notifications resumed. {suppressed} match event(s) were suppressed during the flood.', 'platform': '', 'chat_id': '', 'user_id': '', 'user_name': '', 'thread_id': ''}
                else:
                    release_msg = None
            else:
                release_msg = None
        ...
        if trip_now is not None:
            self.completion_queue.put({'session_id': '', 'session_key': '', 'command': '', 'type': 'watch_overflow_tripped', 'message': f'Watch-pattern overflow: >{WATCH_GLOBAL_MAX_PER_WINDOW} notifications in {WATCH_GLOBAL_WINDOW_SECONDS}s across all processes. Suppressing further watch_match events for {WATCH_GLOBAL_COOLDOWN_SECONDS}s.', 'platform': '', 'chat_id': '', 'user_id': '', 'user_name': '', 'thread_id': ''})
        return admit
    ...
    def _terminate_host_pid(cls, pid: int, expected_start: Optional[int]=None) -> None:
        ...
        if expected_start is not None and (not cls._host_pid_is_ours(pid, expected_start)):
            logger.warning('Refusing to terminate host pid %d: start-time mismatch — PID was recycled onto an unrelated process.', pid)
            return
        if _IS_WINDOWS:
            try:
                subprocess.run(['taskkill', '/PID', str(pid), '/T', '/F'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, creationflags=windows_hide_flags(), stdin=subprocess.DEVNULL)
    ...
    def spawn_local(self, command: str, cwd: str=None, task_id: str='', session_key: str='', env_vars: dict=None, use_pty: bool=False) -> ProcessSession:
        ...
        session = ProcessSession(id=f'proc_{uuid.uuid4().hex[:12]}', command=command, task_id=task_id, session_key=session_key, cwd=_resolve_safe_cwd(cwd or os.getcwd()), started_at=time.time())
        ...
        if use_pty:
            try:
                ...
                try:
                    ...
                    pty_use_systemd_scope = not _IS_WINDOWS and pty_under_supervisor and _systemd_run_user_scope_available()
                except Exception:
                    pty_use_systemd_scope = False
                if pty_use_systemd_scope:
                    pty_argv = _build_systemd_scope_argv(pty_argv, unit_suffix=session.id)
                    session.systemd_unit = f'duck-agent-worker-{session.id}.scope'
                    pty_scope_attempted = True
                elif not _IS_WINDOWS:
                    try:
                        from gateway.restart import is_gateway_supervisor_process as _sup
                        if _sup():
                            logger.debug('PTY background executor not isolated in a systemd scope (systemd-run --user unavailable); worker shares the gateway cgroup.')
                    except Exception:
                        pass
                ...
                if pty_scope_attempted and session.systemd_unit:
                    if not _stop_systemd_unit(session.systemd_unit):
                        raise RuntimeError('PTY scope could not be reaped; refusing pipe fallback to avoid duplicate command execution') from e
        ...
        try:
            ...
            use_systemd_scope = not _IS_WINDOWS and under_supervisor and _systemd_run_user_scope_available()
        except Exception:
            use_systemd_scope = False
        if use_systemd_scope:
            unit_suffix = f'{session.id}-pipe-fallback' if pty_scope_attempted else session.id
            spawn_argv = _build_systemd_scope_argv(shell_argv, unit_suffix=unit_suffix)
            session.systemd_unit = f'duck-agent-worker-{unit_suffix}.scope'
            popen_start_new_session = False
        else:
            spawn_argv = shell_argv
            popen_start_new_session = True
            if not _IS_WINDOWS and under_supervisor:
                logger.debug('Local background executor not isolated in a systemd scope (supervisor=%s, systemd-run --user available=%s); worker shares the gateway cgroup.', under_supervisor, _systemd_run_user_scope_available())
        proc = subprocess.Popen(spawn_argv, text=True, cwd=session.cwd, env=bg_env, encoding='utf-8', errors='replace', stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL, start_new_session=popen_start_new_session, **_popen_kwargs)
    ...
    def spawn_via_env(self, env: Any, command: str, cwd: str=None, task_id: str='', session_key: str='', timeout: int=10) -> ProcessSession:
        ...
        """
        session = ProcessSession(id=f'proc_{uuid.uuid4().hex[:12]}', command=command, task_id=task_id, session_key=session_key, cwd=cwd, started_at=time.time(), env_ref=env, pid_scope='sandbox')
        ...
        bg_command = f"""mkdir -p {quoted_temp_dir} && ( nohup bash -lc {quoted_command} > {quoted_log_path} 2>&1; rc=$?; printf '%s\\n' "$rc" > {quoted_exit_path} ) & echo $! > {quoted_pid_path} && cat {quoted_pid_path}"""
        try:
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        if was_running and session.notify_on_complete:
            ...
            self.completion_queue.put({'type': 'completion', 'session_id': session.id, 'session_key': session.session_key, 'command': session.command, 'exit_code': session.exit_code, 'completion_reason': session.completion_reason, 'termination_source': session.termination_source, 'output': output_tail, 'started_at': session.started_at})
    ...
    def _reconcile_local_exit(self, session: 'ProcessSession') -> None:
        ...
        a daemon spawned by `duck-agent update` restarting the gateway) is still
    ...
    def poll(self, session_id: str) -> dict:
        ...
        result = {'session_id': session.id, 'command': session.command, 'status': 'exited' if session.exited else 'running', 'pid': session.pid, 'uptime_seconds': int(time.time() - session.started_at), 'output_preview': output_preview}
        if session.exited:
    ...
    def read_log(self, session_id: str, offset: int=0, limit: int=200) -> dict:
        ...
        result = {'session_id': session.id, 'command': session.command, 'status': 'exited' if session.exited else 'running', 'output': '\n'.join(selected), 'total_lines': total_lines, 'showing': f'{len(selected)} lines'}
    ...
    def wait(self, session_id: str, timeout: int=None) -> dict:
        ...
        while time.monotonic() < deadline:
            ...
            if session.exited:
                self._completion_consumed.add(session_id)
                result = {'status': 'exited', 'command': session.command, 'exit_code': session.exit_code, 'completion_reason': session.completion_reason, 'termination_source': session.termination_source, 'output': strip_ansi(session.output_buffer[-2000:])}
                if timeout_note:
                    result['timeout_note'] = timeout_note
                return result
            if _is_interrupted():
                result = {'status': 'interrupted', 'command': session.command, 'output': strip_ansi(session.output_buffer[-1000:]), 'note': 'User sent a new message -- wait interrupted'}
                if timeout_note:
                    result['timeout_note'] = timeout_note
                return result
        ...
        result = {'status': 'timeout', 'command': session.command, 'output': strip_ansi(session.output_buffer[-1000:]), 'process_running': True}
        ...
        if session.notify_on_complete:
            ...
            base_note += ' Poll again later or use terminal(background=true, notify_on_complete=true) next time for automatic notification.'
        if timeout_note:
    ...
    def kill_process(self, session_id: str, *, source: str='process.kill', consume_output: bool=True) -> dict:
        ...
        if session.exited:
            if session.systemd_unit:
                _stop_systemd_unit(session.systemd_unit)
            with session._lock:
                result = {'status': 'already_exited', 'command': session.command, 'exit_code': session.exit_code, 'completion_reason': session.completion_reason, 'termination_source': session.termination_source, 'output': strip_ansi(session.output_buffer[-2000:])}
            if consume_output:
                self._completion_consumed.add(session_id)
            return result
        try:
            ...
            self._move_to_finished(session)
            self._write_checkpoint()
            return {'status': 'killed', 'session_id': session.id, 'completion_reason': session.completion_reason, 'termination_source': session.termination_source, 'output': output}
        except Exception as e:
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        sink = self.on_close
        if sink is None:
            return {'status': 'error', 'error': 'close_terminal is only available in the Duck Agent desktop app.'}
        session = self.get(session_id)
        try:
            sink(session, session_id)
        except Exception as e:
            return {'status': 'error', 'error': str(e)}
        return {'status': 'ok', 'closed': session_id, 'note': 'Closed the read-only terminal tab. The process was not killed; its output remains available and the user can reopen the tab from the status stack.'}
    ...
    def list_sessions(self, task_id: str=None, session_key: str=None) -> list:
        ...
        if task_id or session_key:
            all_sessions = [s for s in all_sessions if task_id and s.task_id == task_id or (session_key and s.session_key == session_key)]
        result = []
        for s in all_sessions:
            entry = {'session_id': s.id, 'command': s.command[:200], 'cwd': s.cwd, 'pid': s.pid, 'started_at': time.strftime('%Y-%m-%dT%H:%M:%S', time.localtime(s.started_at)), 'uptime_seconds': int(time.time() - s.started_at), 'status': 'exited' if s.exited else 'running', 'output_preview': s.output_buffer[-200:] if s.output_buffer else ''}
    ...
    def has_active_for_session(self, session_key: str, max_active_age: Optional[float]=None) -> bool:
        ...
        now = time.time()
        with self._lock:
            return any((s.session_key == session_key and (not s.exited) and (max_active_age is None or now - s.started_at < max_active_age) for s in self._running.values()))
    ...
    def kill_all(self, task_id: Optional[str]=None, *, exclude_ids: frozenset=frozenset(), source: str='kill_all', consume_output: bool=False) -> int:
        """Kill all running processes, optionally filtered by task_id. Returns count killed."""
        with self._lock:
            targets = [s for s in self._running.values() if (task_id is None or s.task_id == task_id) and s.id not in exclude_ids and (not s.exited)]
        killed = 0
            ...
            with self._lock:
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({'session_id': s.id, 'command': redact_sensitive_text(s.command, code_file=True), 'pid': s.pid, 'pid_scope': s.pid_scope, 'host_start_time': s.host_start_time, 'systemd_unit': s.systemd_unit, 'cwd': s.cwd, 'started_at': s.started_at, 'task_id': s.task_id, 'session_key': s.session_key, 'watcher_platform': s.watcher_platform, 'watcher_chat_id': s.watcher_chat_id, 'watcher_user_id': s.watcher_user_id, 'watcher_user_name': s.watcher_user_name, 'watcher_thread_id': s.watcher_thread_id, 'watcher_message_id': s.watcher_message_id, 'watcher_interval': s.watcher_interval, 'notify_on_complete': s.notify_on_complete, 'watch_patterns': s.watch_patterns})
                if extra_entries:
        ...
        for entry in entries:
            ...
            if not self._host_pid_is_ours(pid, recorded_start):
                if self._is_host_pid_alive(pid):
                    logger.info('Not recovering session %s: pid %d is alive but its start time no longer matches — PID was recycled onto an unrelated process; refusing to adopt it.', entry.get('session_id', '?'), pid)
                systemd_unit = entry.get('systemd_unit', '')
                if systemd_unit and (not _stop_systemd_unit(systemd_unit)):
                    logger.warning('Could not reap persisted scope %s for dead wrapper pid %s; retaining checkpoint entry for the next startup', systemd_unit, pid)
                    unresolved_scope_entries.append(entry)
                continue
            session = ProcessSession(id=entry['session_id'], command=entry.get('command', 'unknown'), task_id=entry.get('task_id', ''), session_key=entry.get('session_key', ''), pid=pid, host_start_time=recorded_start, pid_scope=pid_scope, systemd_unit=entry.get('systemd_unit', ''), cwd=entry.get('cwd'), started_at=entry.get('started_at', time.time()), detached=True, watcher_platform=entry.get('watcher_platform', ''), watcher_chat_id=entry.get('watcher_chat_id', ''), watcher_user_id=entry.get('watcher_user_id', ''), watcher_user_name=entry.get('watcher_user_name', ''), watcher_thread_id=entry.get('watcher_thread_id', ''), watcher_message_id=entry.get('watcher_message_id', ''), watcher_interval=entry.get('watcher_interval', 0), notify_on_complete=entry.get('notify_on_complete', False), watch_patterns=entry.get('watch_patterns', []))
            with self._lock:
                self._running[session.id] = session
            recovered += 1
            logger.info('Recovered detached process: %s (pid=%d)', session.command[:60], pid)
            if session.watcher_interval > 0:
                self.pending_watchers.append({'session_id': session.id, 'check_interval': session.watcher_interval, 'session_key': session.session_key, 'platform': session.watcher_platform, 'chat_id': session.watcher_chat_id, 'user_id': session.watcher_user_id, 'user_name': session.watcher_user_name, 'thread_id': session.watcher_thread_id, 'message_id': session.watcher_message_id, 'notify_on_complete': session.notify_on_complete})
...
def _format_async_delegation(evt: dict) -> str:
    ...
    if evt.get('is_batch') or isinstance(batch_results, list):
        ...
        lines = [f'[ASYNC DELEGATION BATCH COMPLETE — {deleg_id}]', f'A background fan-out of {n} subagent(s) you dispatched earlier has finished. All ran in parallel and waited on each other; their consolidated results are below. You may have moved on since dispatching — act on these or re-dispatch if things have changed.', '']
    ...
    if isinstance(dispatched_at, (int, float)):
        age = f' ({_format_age(completed_at - dispatched_at)} ago)'
    lines = [f'[ASYNC DELEGATION COMPLETE — {deleg_id}]', 'A background subagent you dispatched earlier has finished. You may have moved on since dispatching it; the full task source is below so you can act on the result or re-dispatch if things have changed.', '']
    if isinstance(dispatched_at, (int, float)):
        ts = _time.strftime('%Y-%m-%d %H:%M:%S', _time.localtime(dispatched_at))
        lines.append(f'Dispatched: {ts}{age}')
    ...
    if status in ('completed', 'success') and summary:
        ...
        lines.append(f'The subagent did not complete successfully (status={status}).' + (f'\n{error}' if error else ''))
        if summary:
            lines.append('Partial output:')
            lines.append(summary)
    return '\n'.join(lines)
...
def format_process_notification(evt: dict) -> 'str | None':
    ...
    if _exit in {-15, 143, '-15', '143'}:
        _signal = ', SIGTERM'
    if _reason == 'killed':
        _status = f"terminated by {_source or 'Duck Agent'}"
    elif _reason == 'lost':
        ...
        _status = 'exited'
    return f'[IMPORTANT: Background process {_sid} {_status} (exit code {_exit}{_signal}).\nCommand: {_cmd}\nOutput:\n{_out}]'
...
def _handle_process(args, **kw):
    ...
    if action == 'list':
        try:
            ...
            session_key = ''
        return json.dumps({'processes': [_redact_process_result(p) for p in process_registry.list_sessions(task_id=task_id, session_key=session_key or None)]}, ensure_ascii=False)
        ...
        if action == 'poll':
            ...
            return json.dumps(_redact_process_result(process_registry.read_log(session_id, offset=args.get('offset', 0), limit=args.get('limit', 200))), ensure_ascii=False)
        elif action == 'wait':
...
registry.register(name='process', toolset='terminal', schema=PROCESS_SCHEMA, handler=_handle_process, emoji='⚙️')