"""Duck Agent-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Duck Agent
...
def get_camofox_identity(task_id: Optional[str]=None) -> Dict[str, str]:
    """Return the stable Duck Agent-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Duck Agent profile = same userId).
    ...
    session_digest = uuid.uuid5(uuid.NAMESPACE_URL, f'camofox-session:{scope_root}:{logical_scope}').hex[:16]
