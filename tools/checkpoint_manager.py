...
_REFS_PREFIX = 'refs/duck-agent'
_INDEXES_DIRNAME = 'indexes'
_PROJECTS_DIRNAME = 'projects'
_LEGACY_PREFIX = 'legacy-'
DEFAULT_EXCLUDES = ['node_modules/', 'dist/', 'build/', 'target/', 'out/', '.next/', '.nuxt/', '__pycache__/', '*.pyc', '*.pyo', '.cache/', '.pytest_cache/', '.mypy_cache/', '.ruff_cache/', 'coverage/', '.coverage', '.venv/', 'venv/', 'env/', '.git/', '.hg/', '.svn/', '.worktrees/', '*.so', '*.dylib', '*.dll', '*.o', '*.a', '*.jar', '*.class', '*.exe', '*.obj', '*.mp4', '*.mov', '*.mkv', '*.webm', '*.zip', '*.tar', '*.tar.gz', '*.tgz', '*.7z', '*.rar', '*.iso', '.env', '.env.*', '.env.local', '.env.*.local', '.DS_Store', 'Thumbs.db', '*.log']
_GIT_TIMEOUT: int = max(10, min(60, env_int('HERMES_CHECKPOINT_TIMEOUT', 30)))
_MAX_FILES = 50000
_COMMIT_HASH_RE = re.compile('^[0-9a-fA-F]{4,64}$')
...
def _git_env(store: Path, working_dir: str, index_file: Optional[Path]=None) -> dict:
    ...
    The shared store is internal Duck Agent infrastructure — it must NOT inherit
...
def _run_git(args: List[str], store: Path, working_dir: str, timeout: int=_GIT_TIMEOUT, allowed_returncodes: Optional[Set[int]]=None, index_file: Optional[Path]=None) -> Tuple[bool, str, str]:
    ...
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, env=env, cwd=str(normalized_working_dir), stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
        ok = result.returncode == 0
        stdout = result.stdout.strip()
        stderr = result.stderr.strip()
        if not ok and result.returncode not in allowed_returncodes:
            logger.error('Git command failed: %s (rc=%d) stderr=%s', ' '.join(cmd), result.returncode, stderr)
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``duck-agent checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info('Migrated pre-v2 checkpoint repos to %s. Clear with `duck-agent checkpoints clear-legacy` when safe.', legacy_root)
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    for k in ('GIT_DIR', 'GIT_WORK_TREE', 'GIT_INDEX_FILE', 'GIT_NAMESPACE', 'GIT_ALTERNATE_OBJECT_DIRECTORIES'):
        init_env.pop(k, None)
    try:
        result = subprocess.run(['git', 'init', '--bare', str(store)], capture_output=True, text=True, encoding='utf-8', errors='replace', env=init_env, timeout=_GIT_TIMEOUT, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
        if result.returncode != 0:
            return f'Shadow store init failed: {result.stderr.strip()}'
    except (subprocess.TimeoutExpired, FileNotFoundError) as exc:
        return f'Shadow store init failed: {exc}'
    cfg_wd = str(base)
    _run_git(['config', 'user.email', 'duck-agent@local'], store, cfg_wd)
    _run_git(['config', 'user.name', 'Duck Agent Checkpoint'], store, cfg_wd)
...
def _register_project(store: Path, working_dir: str) -> None:
    ...
    now = time.time()
    meta: Dict = {'workdir': str(_normalize_path(working_dir)), 'created_at': now, 'last_touch': now}
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        out.append({'path': child, 'workdir': workdir, 'exists': bool(workdir) and Path(workdir).exists(), 'marker_unreadable': marker_unreadable})
    return out
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    try:
        (shadow_repo / 'HERMES_WORKDIR').write_text(str(_normalize_path(working_dir)) + '\n', encoding='utf-8')
    except OSError:
        pass
    return None
...
class CheckpointManager:
    ...
    def __init__(self, enabled: bool=False, max_snapshots: int=20, max_total_size_mb: int=500, max_file_size_mb: int=10):
    ...
    def list_checkpoints(self, working_dir: str) -> List[Dict]:
        ...
        ok, stdout, _ = _run_git(['log', ref, '--format=%H|%h|%aI|%s', '-n', str(self.max_snapshots)], store, abs_dir, allowed_returncodes={128, 129})
        ...
        for line in stdout.splitlines():
            parts = line.split('|', 3)
            if len(parts) == 4:
                entry = {'hash': parts[0], 'short_hash': parts[1], 'timestamp': parts[2], 'reason': parts[3], 'files_changed': 0, 'insertions': 0, 'deletions': 0}
                stat_ok, stat_out, _ = _run_git(['diff', '--shortstat', f'{parts[0]}~1', parts[0]], store, abs_dir, allowed_returncodes={128, 129})
    ...
    def _parse_shortstat(stat_line: str, entry: Dict) -> None:
        ...
        m = re.search('(\\d+) file', stat_line)
        if m:
            entry['files_changed'] = int(m.group(1))
        m = re.search('(\\d+) insertion', stat_line)
        if m:
            entry['insertions'] = int(m.group(1))
        m = re.search('(\\d+) deletion', stat_line)
        if m:
    ...
    def diff(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        ok_stat, stat_out, _ = _run_git(['diff', '--stat', commit_hash, '--cached'], store, abs_dir, index_file=index_file)
        ok_diff, diff_out, _ = _run_git(['diff', commit_hash, '--cached', '--no-color'], store, abs_dir, index_file=index_file)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Duck Agent changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Duck Agent changed", not an exact per-session ledger.
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str=None) -> Dict:
        ...
        ok, stdout, err = _run_git(['checkout', commit_hash, '--', restore_target], store, abs_dir, timeout=_GIT_TIMEOUT * 2, index_file=index_file)
        if not ok:
            return {'success': False, 'error': f'Restore failed: {err}', 'debug': err or None}
        ok2, reason_out, _ = _run_git(['log', '--format=%s', '-1', commit_hash], store, abs_dir)
        reason = reason_out if ok2 else 'unknown'
        result = {'success': True, 'restored_to': commit_hash[:8], 'reason': reason, 'directory': abs_dir}
        if file_path:
    ...
    def get_working_dir_for_path(self, file_path: str) -> str:
        ...
        markers = {'.git', 'pyproject.toml', 'package.json', 'Cargo.toml', 'go.mod', 'Makefile', 'pom.xml', '.hg', 'Gemfile'}
        check = candidate
    ...
    def _take(self, working_dir: str, reason: str) -> bool:
        ...
        if index_file.exists():
            ok_ref, ref_commit, _ = _run_git(['rev-parse', '--verify', ref + '^{commit}'], store, working_dir, allowed_returncodes={128})
        ...
        ok, _, err = _run_git(['add', '-A'], store, working_dir, timeout=_GIT_TIMEOUT * 2, index_file=index_file)
        if not ok:
        ...
        if has_ref:
            ok_diff, _, _ = _run_git(['diff-index', '--cached', '--quiet', ref_commit], store, working_dir, allowed_returncodes={1}, index_file=index_file)
            if ok_diff:
                logger.debug('Checkpoint skipped: no changes in %s', working_dir)
                return False
        else:
            ok_ls, ls_out, _ = _run_git(['ls-files', '--cached'], store, working_dir, index_file=index_file)
    ...
    def _drop_oversize_from_index(self, store: Path, working_dir: str, index_file: Path) -> None:
        ...
        if not oversize:
            return
        logger.debug('Checkpoint: dropping %d oversize file(s) (>%d MB) from index', len(oversize), self.max_file_size_mb)
        BATCH = 200
        for i in range(0, len(oversize), BATCH):
            chunk = oversize[i:i + BATCH]
            _run_git(['rm', '--cached', '--quiet', '--'] + chunk, store, working_dir, index_file=index_file, allowed_returncodes={128})
    ...
    def _prune(self, store: Path, working_dir: str, ref: str) -> None:
        ...
        """
        ok, stdout, _ = _run_git(['rev-list', '--count', ref], store, working_dir, allowed_returncodes={128})
        if not ok:
            return
        try:
            count = int(stdout)
        ...
        if count <= self.max_snapshots:
            return
        ok_list, list_out, _ = _run_git(['rev-list', '--reverse', ref], store, working_dir)
    ...
    def _enforce_size_cap(self, store: Path) -> None:
        ...
        if size <= cap_bytes:
            return
        logger.info('Checkpoint store exceeded %d MB (actual %d MB) — pruning oldest', self.max_total_size_mb, size // (1024 * 1024))
        ok, stdout, _ = _run_git(['for-each-ref', '--format=%(refname)', _REFS_PREFIX], store, str(store.parent), allowed_returncodes={128})
        if not ok or not stdout:
            return
        ...
        for _ in range(20):
            ...
            if size <= cap_bytes:
                break
            for ref in refs:
                ok_count, count_out, _ = _run_git(['rev-list', '--count', ref], store, str(store.parent), allowed_returncodes={128})
                try:
                ...
                if count <= 1:
                    continue
                ok_list, list_out, _ = _run_git(['rev-list', '--reverse', ref], store, str(store.parent))
                if not ok_list or not list_out:
                    continue
                commits = list_out.splitlines()
                keep = commits[1:]
                ...
                fail = False
                for sha in keep:
                    ok_tree, tree_sha, _ = _run_git(['rev-parse', f'{sha}^{{tree}}'], store, str(store.parent))
                    if not ok_tree or not tree_sha:
                        fail = True
                        break
                    ok_msg, msg, _ = _run_git(['log', '--format=%s', '-1', sha], store, str(store.parent))
                    commit_msg = msg if ok_msg and msg else 'checkpoint'
                    args = ['commit-tree', tree_sha, '-m', commit_msg, '--no-gpg-sign']
                    if new_parent is not None:
                        args = ['commit-tree', tree_sha, '-p', new_parent, '-m', commit_msg, '--no-gpg-sign']
        ...
        _run_git(['gc', '--prune=now', '--quiet'], store, str(store.parent), timeout=_GIT_TIMEOUT * 3)
        _repair_bare_repo_dirs(store)
...
def _workdir_is_observably_gone(workdir: str, parent_dev: Optional[int]=None, parent_ino: Optional[int]=None, require_parent_identity: bool=True) -> bool:
...
def prune_checkpoints(retention_days: int=7, delete_orphans: bool=True, checkpoint_base: Optional[Path]=None, max_total_size_mb: int=0, orphan_allowlist: Optional[set]=None) -> Dict[str, int]:
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for repo in _pre_v2_shadow_repos(base):
        ...
        if delete_orphans and (not repo['marker_unreadable']) and (repo['workdir'] is None or _workdir_is_observably_gone(repo['workdir'], require_parent_identity=False)) and (orphan_allowlist is None or str(child) in orphan_allowlist):
            reason = 'orphan'
    ...
    if (store / 'HEAD').exists():
        for meta in _list_projects(store):
            ...
            if not isinstance(parent_ino, int) or isinstance(parent_ino, bool):
                parent_ino = None
            if delete_orphans and (not workdir or _workdir_is_observably_gone(workdir, parent_dev=parent_dev, parent_ino=parent_ino)) and (orphan_allowlist is None or dir_hash in orphan_allowlist):
                reason = 'orphan'
        ...
        if max_total_size_mb > 0:
            ...
            for _i in range(20):
                ...
                if size <= cap_bytes:
                    break
                ok, stdout, _ = _run_git(['for-each-ref', '--format=%(refname)', _REFS_PREFIX], store, str(base), allowed_returncodes={128})
                refs = [r for r in stdout.splitlines() if r.strip()] if ok else []
                if not refs:
                    break
                any_drop = False
                for ref in refs:
                    ok_c, count_out, _ = _run_git(['rev-list', '--count', ref], store, str(base), allowed_returncodes={128})
                    try:
                    ...
                    if count <= 1:
                        continue
                    ok_l, lo, _ = _run_git(['rev-list', '--reverse', ref], store, str(base))
                    ...
                    fail = False
                    for sha in keep:
                        ok_t, tsha, _ = _run_git(['rev-parse', f'{sha}^{{tree}}'], store, str(base))
                        if not ok_t or not tsha:
                            fail = True
                            break
                        ok_m, m, _ = _run_git(['log', '--format=%s', '-1', sha], store, str(base))
                        msg = m if ok_m and m else 'checkpoint'
                        args = ['commit-tree', tsha, '-m', msg, '--no-gpg-sign']
                        if new_parent is not None:
                            args = ['commit-tree', tsha, '-p', new_parent, '-m', msg, '--no-gpg-sign']
...
def maybe_auto_prune_checkpoints(retention_days: int=7, min_interval_hours: int=24, delete_orphans: bool=True, checkpoint_base: Optional[Path]=None, max_total_size_mb: int=0) -> Dict[str, object]:
    ...
    try:
        if not base.exists():
            out['result'] = {'scanned': 0, 'deleted_orphan': 0, 'deleted_stale': 0, 'errors': 0, 'bytes_freed': 0}
            return out
        ...
        if marker.exists():
            try:
                ...
                pass
        result = prune_checkpoints(retention_days=retention_days, delete_orphans=delete_orphans, checkpoint_base=base, max_total_size_mb=max_total_size_mb)
        out['result'] = result
        try:
        ...
        if total > 0:
            logger.info('checkpoint auto-maintenance: pruned %d entry(ies) (%d orphan, %d stale), reclaimed %.1f MB', total, result['deleted_orphan'], result['deleted_stale'], result['bytes_freed'] / (1024 * 1024))
    except Exception as exc:
...
def store_status(checkpoint_base: Optional[Path]=None) -> Dict:
    ...
    """
    base = checkpoint_base or CHECKPOINT_BASE
    out: Dict = {'base': str(base), 'store_size_bytes': 0, 'legacy_size_bytes': 0, 'total_size_bytes': 0, 'project_count': 0, 'projects': [], 'pre_v2_projects': [], 'legacy_archives': []}
    if not base.exists():
        return out
    store = _store_path(base)
    if store.exists():
        ...
        if (store / 'HEAD').exists():
            for meta in _list_projects(store):
                ...
                workdir = meta.get('workdir') or ''
                ref = _ref_name(dir_hash)
                ok, count_out, _ = _run_git(['rev-list', '--count', ref], store, str(base), allowed_returncodes={128})
                try:
                    commits = int(count_out) if ok else 0
                except ValueError:
                    commits = 0
                out['projects'].append({'hash': dir_hash, 'workdir': workdir, 'exists': bool(workdir) and Path(workdir).exists(), 'created_at': meta.get('created_at'), 'last_touch': meta.get('last_touch'), 'commits': commits})
    out['project_count'] = len(out['projects'])
    out['pre_v2_projects'] = [{'path': str(r['path']), 'workdir': r['workdir'], 'exists': r['exists']} for r in _pre_v2_shadow_repos(base)]
    for child in base.iterdir():
