...
class _ManagedFalSyncClient:
    ...
    def submit(self, application: str, arguments: Dict[str, Any], *, path: str='', hint: Optional[str]=None, webhook_url: Optional[str]=None, priority: Any=None, headers: Optional[Dict[str, str]]=None, start_timeout: Optional[Union[int, float]]=None):
        ...
        response = self._maybe_retry_request(self._http_client, 'POST', url, json=arguments, timeout=getattr(self._sync_client, 'default_timeout', 120.0), headers=request_headers)
        self._raise_for_status(response)
        data = response.json()
        return self._request_handle_class(request_id=data['request_id'], response_url=data['response_url'], status_url=data['status_url'], cancel_url=data['cancel_url'], client=self._http_client)