...
class MicrosoftGraphAPIError(MicrosoftGraphClientError):
    ...
    def __init__(self, status_code: int, method: str, url: str, message: str, *, retry_after_seconds: float | None=None, payload: Any=None) -> None:
...
class MicrosoftGraphClient:
    ...
    def __init__(self, token_provider: MicrosoftGraphTokenProvider, *, base_url: str=DEFAULT_GRAPH_BASE_URL, timeout: float=60.0, max_retries: int=3, transport: httpx.AsyncBaseTransport | None=None, sleep: Callable[[float], Awaitable[None]] | None=None, user_agent: str='Duck Agent-Agent/graph-client') -> None:
    ...
    async def iterate_pages(self, path: str, *, params: dict[str, Any] | None=None, headers: dict[str, str] | None=None) -> AsyncIterator[dict[str, Any]]:
        ...
        while next_url:
            ...
            if not isinstance(payload, dict):
                raise MicrosoftGraphClientError(f'Expected paginated Graph response dict, got {type(payload).__name__}.')
            yield payload
    ...
    async def download_to_file(self, path: str, destination: str | Path, *, headers: dict[str, str] | None=None, chunk_size: int=65536) -> dict[str, Any]:
        ...
        while attempt <= self.max_retries:
            token = await self.token_provider.get_access_token(force_refresh=attempt > 0 and self._should_refresh_token(last_error))
            request_headers = {'Authorization': f'Bearer {token}', 'Accept': 'application/json', 'User-Agent': self.user_agent}
            if headers:
            ...
            try:
                ...
                if attempt >= self.max_retries:
                    raise MicrosoftGraphClientError(f'Microsoft Graph download failed for GET {url}: {exc}') from exc
        ...
        tmp_target.unlink(missing_ok=True)
        raise MicrosoftGraphClientError(f'Microsoft Graph download exhausted retries for GET {url}.')

    async def _request(self, method: str, path_or_url: str, *, params: dict[str, Any] | None=None, json_body: Any | None=None, headers: dict[str, str] | None=None) -> httpx.Response:
        ...
        while attempt <= self.max_retries:
            ...
            try:
                ...
                last_error = exc
                if attempt >= self.max_retries:
                    raise MicrosoftGraphClientError(f'Microsoft Graph request failed for {method} {url}: {exc}') from exc
                await self._sleep(self._retry_delay(None, attempt))
                attempt += 1
                continue
            if response.status_code < 400:
            ...
            if self._should_retry(response) and attempt < self.max_retries:
                ...
                continue
            raise api_error
        raise MicrosoftGraphClientError(f'Microsoft Graph request exhausted retries for {method} {url}.')
    ...
    def _decode_json(response: httpx.Response) -> Any:
        try:
            ...
            raise MicrosoftGraphClientError(f'Microsoft Graph response was not valid JSON for {response.request.method} {response.request.url}') from exc
    ...
    def _build_api_error(method: str, url: str, response: httpx.Response) -> MicrosoftGraphAPIError:
        ...
        return MicrosoftGraphAPIError(response.status_code, method, url, message, retry_after_seconds=retry_after, payload=payload)