...
<url>``) is spawned as a direct child of the Duck Agent process. Duck Agent'ss own
...
Duck Agent process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Duck Agent startup's opt-in
...
def main(argv: list[str] | None=None) -> int:
    ...
    watchdog = threading.Thread(target=_watchdog_loop, args=(proc, args.ppid), daemon=True)
    watchdog.start()
    try:
        return proc.wait()
