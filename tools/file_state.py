    ...
    def check_stale(self, task_id: str, resolved: str) -> Optional[str]:
        ...
        if last_writer is not None:
            ...
            if writer_tid != task_id:
                if stamp is None:
                    return f"{resolved} was modified by sibling subagent {writer_tid!r} but this agent never read it. Read the file before writing to avoid overwriting the sibling's changes."
                read_ts = stamp[1]
                if writer_ts > read_ts:
                    return f"{resolved} was modified by sibling subagent {writer_tid!r} at {_fmt_ts(writer_ts)} — after this agent's last read at {_fmt_ts(read_ts)}. Re-read the file before writing."
        if stamp is not None:
            read_mtime, _read_ts, partial = stamp
            if current_mtime != read_mtime:
                return f'{resolved} was modified since you last read it on disk (external edit or unrecorded writer). Re-read the file before writing.'
            if partial:
                return f'{resolved} was last read with offset/limit pagination (partial view). Re-read the whole file before overwriting it.'
        if stamp is None:
            return f'{resolved} was not read by this agent. Read the file first so you can write an informed edit.'
        return None
...
__all__ = ['FileStateRegistry', 'get_registry', 'record_read', 'note_write', 'check_stale', 'lock_path', 'writes_since', 'known_reads']