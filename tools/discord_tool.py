...
Only included in the duck-agent-discord toolset, so it has zero cost
...
def _discord_request(method: str, path: str, token: str, params: Optional[Dict[str, str]]=None, body: Optional[Dict[str, Any]]=None, timeout: int=15) -> Any:
    ...
    req = urllib.request.Request(url, data=data, method=method, headers={'Authorization': f'Bot {token}', 'Content-Type': 'application/json', 'User-Agent': 'Duck Agent-Agent (https://github.com/NousResearch/duck-agent)'})
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            if resp.status == 204:
                return None
            response_body = _read_limited_response_body(resp, _DISCORD_RESPONSE_BODY_MAX_BYTES, label='response body')
            return json.loads(response_body.decode('utf-8'))
    except urllib.error.HTTPError as e:
        error_body = ''
        try:
            raw_error_body = _read_limited_response_body(e, _DISCORD_ERROR_BODY_MAX_BYTES, label='error body')
...
_CHANNEL_TYPE_NAMES = {0: 'text', 2: 'voice', 4: 'category', 5: 'announcement', 10: 'announcement_thread', 11: 'public_thread', 12: 'private_thread', 13: 'stage', 15: 'forum', 16: 'media'}
...
def _detect_capabilities_nonblocking(token: str) -> Dict[str, Any]:
    ...
    if disk is not None:
        ...
        return disk
    caps_default = {'has_members_intent': True, 'has_message_content': True, 'detected': False}
...
def _fetch_capabilities(token: str) -> Dict[str, Any]:
    ...
    caps: Dict[str, Any] = {'has_members_intent': True, 'has_message_content': True, 'detected': False}
    try:
        app = _discord_request('GET', '/applications/@me', token, timeout=5)
        flags = int(app.get('flags', 0) or 0)
        caps['has_members_intent'] = bool(flags & (_FLAG_GATEWAY_GUILD_MEMBERS | _FLAG_GATEWAY_GUILD_MEMBERS_LIMITED))
        caps['has_message_content'] = bool(flags & (_FLAG_GATEWAY_MESSAGE_CONTENT | _FLAG_GATEWAY_MESSAGE_CONTENT_LIMITED))
        caps['detected'] = True
...
def _list_guilds(token: str, **_kwargs: Any) -> str:
    ...
    result = []
    for g in guilds:
        result.append({'id': g['id'], 'name': g['name'], 'icon': g.get('icon'), 'owner': g.get('owner', False), 'permissions': g.get('permissions')})
...
def _server_info(token: str, guild_id: str, **_kwargs: Any) -> str:
    ...
    return json.dumps({'id': g['id'], 'name': g['name'], 'description': g.get('description'), 'icon': g.get('icon'), 'owner_id': g.get('owner_id'), 'member_count': g.get('approximate_member_count'), 'online_count': g.get('approximate_presence_count'), 'features': g.get('features', []), 'premium_tier': g.get('premium_tier'), 'premium_subscription_count': g.get('premium_subscription_count'), 'verification_level': g.get('verification_level')})
...
def _list_channels(token: str, guild_id: str, **_kwargs: Any) -> str:
    ...
    for ch in channels:
        if ch['type'] == 4:
            categories[ch['id']] = {'id': ch['id'], 'name': ch['name'], 'position': ch.get('position', 0), 'channels': []}
    for ch in channels:
        if ch['type'] == 4:
            continue
        entry = {'id': ch['id'], 'name': ch.get('name', ''), 'type': _channel_type_name(ch['type']), 'position': ch.get('position', 0), 'topic': ch.get('topic'), 'nsfw': ch.get('nsfw', False)}
...
def _channel_info(token: str, channel_id: str, **_kwargs: Any) -> str:
    ...
    return json.dumps({'id': ch['id'], 'name': ch.get('name'), 'type': _channel_type_name(ch['type']), 'guild_id': ch.get('guild_id'), 'topic': ch.get('topic'), 'nsfw': ch.get('nsfw', False), 'position': ch.get('position'), 'parent_id': ch.get('parent_id'), 'rate_limit_per_user': ch.get('rate_limit_per_user', 0), 'last_message_id': ch.get('last_message_id')})
...
def _list_roles(token: str, guild_id: str, **_kwargs: Any) -> str:
    ...
    for r in sorted(roles, key=lambda r: r.get('position', 0), reverse=True):
        result.append({'id': r['id'], 'name': r['name'], 'color': f"#{r.get('color', 0):06x}" if r.get('color') else None, 'position': r.get('position', 0), 'mentionable': r.get('mentionable', False), 'managed': r.get('managed', False), 'member_count': r.get('member_count'), 'hoist': r.get('hoist', False)})
...
def _member_info(token: str, guild_id: str, user_id: str, **_kwargs: Any) -> str:
    ...
    user = m.get('user', {})
    return json.dumps({'user_id': user.get('id'), 'username': user.get('username'), 'display_name': user.get('global_name'), 'nickname': m.get('nick'), 'avatar': user.get('avatar'), 'bot': user.get('bot', False), 'roles': m.get('roles', []), 'joined_at': m.get('joined_at'), 'premium_since': m.get('premium_since')})
...
def _search_members(token: str, guild_id: str, query: str, limit: int=20, **_kwargs: Any) -> str:
    ...
    for m in members:
        user = m.get('user', {})
        result.append({'user_id': user.get('id'), 'username': user.get('username'), 'display_name': user.get('global_name'), 'nickname': m.get('nick'), 'bot': user.get('bot', False), 'roles': m.get('roles', [])})
    return json.dumps({'members': result, 'count': len(result)})

def _fetch_messages(token: str, channel_id: str, limit: int=50, before: Optional[str]=None, after: Optional[str]=None, **_kwargs: Any) -> str:
    ...
    for msg in messages:
        ...
        result.append({'id': msg['id'], 'content': msg.get('content', ''), 'author': {'id': author.get('id'), 'username': author.get('username'), 'display_name': author.get('global_name'), 'bot': author.get('bot', False)}, 'timestamp': msg.get('timestamp'), 'edited_timestamp': msg.get('edited_timestamp'), 'attachments': [{'filename': a.get('filename'), 'url': a.get('url'), 'size': a.get('size')} for a in msg.get('attachments', [])], 'reactions': [{'emoji': r.get('emoji', {}).get('name'), 'count': r.get('count', 0)} for r in msg.get('reactions', [])] if msg.get('reactions') else [], 'pinned': msg.get('pinned', False)})
...
def _list_pins(token: str, channel_id: str, **_kwargs: Any) -> str:
    ...
    for msg in messages:
        author = msg.get('author', {})
        result.append({'id': msg['id'], 'content': msg.get('content', '')[:200], 'author': author.get('username'), 'timestamp': msg.get('timestamp')})
...
def _create_thread(token: str, channel_id: str, name: str, message_id: Optional[str]=None, auto_archive_duration: int=1440, **_kwargs: Any) -> str:
...
def _load_allowed_actions_config() -> Optional[List[str]]:
    ...
    if invalid:
        logger.warning('discord.server_actions: unknown action(s) ignored: %s. Known: %s', ', '.join(invalid), ', '.join(_ACTIONS.keys()))
    return valid
...
def _build_schema(actions: List[str], caps: Optional[Dict[str, Any]]=None, tool_name: str='discord') -> Optional[Dict[str, Any]]:
    ...
    if not actions:
        return None
    manifest_lines = [f'  {name}{sig}  — {desc}' for name, sig, desc in _ACTION_MANIFEST if name in actions]
    manifest_block = '\n'.join(manifest_lines)
    content_note = ''
...
def _run_discord_action(action: str, valid_actions: Dict[str, Any], tool_label: str, guild_id: str='', channel_id: str='', user_id: str='', role_id: str='', message_id: str='', query: str='', name: str='', limit: int=50, before: str='', after: str='', auto_archive_duration: int=1440) -> str:
    ...
    if allowlist is not None and action not in allowlist:
        return tool_error(f"Action '{action}' is disabled by config (discord.server_actions). Allowed: {(', '.join(allowlist) if allowlist else '<none>')}")
    local_vars = {'guild_id': guild_id, 'channel_id': channel_id, 'user_id': user_id, 'role_id': role_id, 'message_id': message_id, 'query': query, 'name': name}
    missing = [p for p in _REQUIRED_PARAMS.get(action, []) if not local_vars.get(p)]
    if missing:
        return tool_error(f"Missing required parameters for '{action}': {', '.join(missing)}")
    try:
        return action_fn(token=token, guild_id=guild_id, channel_id=channel_id, user_id=user_id, role_id=role_id, message_id=message_id, query=query, name=name, limit=limit, before=before, after=after, auto_archive_duration=auto_archive_duration)
...
_HANDLER_DEFAULTS = {'action': '', 'guild_id': '', 'channel_id': '', 'user_id': '', 'role_id': '', 'message_id': '', 'query': '', 'name': '', 'limit': 50, 'before': '', 'after': '', 'auto_archive_duration': 1440}

def _make_handler(handler_fn):
    """Create a registry-compatible handler lambda for a discord handler."""
    return lambda args, **kw: handler_fn(**{k: args.get(k, v) for k, v in _HANDLER_DEFAULTS.items()})
_STATIC_CORE_SCHEMA = _build_schema(list(_CORE_ACTIONS.keys()), caps={'detected': False}, tool_name='discord')
_STATIC_ADMIN_SCHEMA = _build_schema(list(_ADMIN_ACTIONS.keys()), caps={'detected': False}, tool_name='discord_admin')
registry.register(name='discord', toolset='discord', schema=_STATIC_CORE_SCHEMA, handler=_make_handler(discord_core), check_fn=check_discord_tool_requirements, requires_env=['DISCORD_BOT_TOKEN'])
registry.register(name='discord_admin', toolset='discord_admin', schema=_STATIC_ADMIN_SCHEMA, handler=_make_handler(discord_admin_handler), check_fn=check_discord_tool_requirements, requires_env=['DISCORD_BOT_TOKEN'])