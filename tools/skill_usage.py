...
Tracks per-skill usage metadata in a sidecar JSON file (~/.duck-agent/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.duck-agent/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.duck-agent/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.duck-agent/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``duck-agent update`` would
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.duck-agent/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``duck-agent curator
    list-archived`` to help users pass a name to ``duck-agent curator restore``.
    """
...
def is_agent_created(skill_name: str) -> bool:
    ...
    if skill_name in off_limits:
        return False
    return not (_find_skill_dir(skill_name) is None and _find_external_skill_dir(skill_name) is not None)
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``duck-agent curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``duck-agent curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``duck-agent curator
...
def _empty_record() -> Dict[str, Any]:
    return {'created_by': None, 'use_count': 0, 'view_count': 0, 'last_used_at': None, 'last_viewed_at': None, 'patch_count': 0, 'patch_generation': 0, 'last_reused_patch_generation': 0, 'last_patched_at': None, 'created_at': _now_iso(), 'state': STATE_ACTIVE, 'pinned': False, 'archived_at': None}
...
def _emit_skill_lifecycle(skill_name: str, action: str, *, record: Optional[Dict[str, Any]]=None, task_id: Optional[str]=None, session_id: Optional[str]=None, use_count: Optional[int]=None, reused: Optional[bool]=None, reuse_after_patch: Optional[bool]=None) -> None:
    """Emit one best-effort lifecycle fact after authoritative state changes."""
    try:
        from hermes_cli.lifecycle import has_hook, invoke_hook
        if not has_hook('on_skill_lifecycle'):
            return
        invoke_hook('on_skill_lifecycle', action=action, skill_name=skill_name, provenance=telemetry_provenance(skill_name, record), task_id=task_id or '', session_id=session_id or '', use_count=use_count, reused=reused, reuse_after_patch=reuse_after_patch)
    except Exception:
...
def bump_use(skill_name: str, *, task_id: Optional[str]=None, session_id: Optional[str]=None) -> None:
    ...
    def _apply(rec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {'created_by': rec.get('created_by'), 'use_count': rec['use_count'], 'reused': reused, 'reuse_after_patch': reuse_after_patch}
    facts = _mutate(skill_name, _apply)
    if isinstance(facts, dict):
        _emit_skill_lifecycle(skill_name, 'loaded', record=facts, task_id=task_id, session_id=session_id, use_count=facts['use_count'], reused=facts['reused'], reuse_after_patch=facts['reuse_after_patch'])

def bump_patch(skill_name: str, *, action: str='patch', task_id: Optional[str]=None, session_id: Optional[str]=None) -> None:
    ...
    facts = _mutate(skill_name, _apply)
    if isinstance(facts, dict):
        _emit_skill_lifecycle(skill_name, lifecycle_action, record=facts, task_id=task_id, session_id=session_id)

def record_created(skill_name: str, *, agent_created: bool, task_id: Optional[str]=None, session_id: Optional[str]=None) -> None:
    ...
    facts = _mutate(skill_name, _apply)
    if isinstance(facts, dict):
        _emit_skill_lifecycle(skill_name, 'created', record=facts, task_id=task_id, session_id=session_id)
...
def set_state(skill_name: str, state: str) -> None:
    ...
    def _apply(rec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if state == STATE_ARCHIVED:
            ...
            rec['archived_at'] = None
        return {'changed': True, 'created_by': rec.get('created_by'), 'previous_state': previous_state}
    facts = _mutate(skill_name, _apply, require_curation_eligible=True)
    if not isinstance(facts, dict) or not facts.get('changed'):
        return
    action = {STATE_ARCHIVED: 'archived', STATE_STALE: 'stale'}.get(state)
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.duck-agent/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.duck-agent/skills/. Restores to the flat
    ...
    if not candidates:
        prefix = f'{skill_name}-'
        candidates = sorted([p for p in archive_root.rglob('*') if p.is_dir() and p.name.startswith(prefix) and (len(p.name) - len(prefix) == 14) and p.name[len(prefix):].isdigit()], reverse=True)
    if not candidates:
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.duck-agent/skills/<skill>/SKILL.md) and category-nested
    (~/.duck-agent/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
...
def usage_report() -> List[Dict[str, Any]]:
    ...
    for skill_md in base.rglob('SKILL.md'):
        ...
        for k, v in base_rec.items():
            rec.setdefault(k, v)
        row = {'name': name, **rec, 'provenance': provenance(name), '_persisted': persisted}
        row['last_activity_at'] = latest_activity_at(row)
        row['activity_count'] = activity_count(row)
        rows.append(row)
