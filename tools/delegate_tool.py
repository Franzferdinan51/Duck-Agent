...
DELEGATE_BLOCKED_TOOLS = frozenset(['delegate_task', 'clarify', 'memory', 'send_message', 'cronjob'])
...
def _subagent_auto_deny(command: str, description: str, **kwargs) -> str:
    ...
    """
    logger.warning('Subagent auto-denied dangerous command: %s (%s). Set delegation.subagent_auto_approve: true to allow.', command, description)
    return 'deny'
...
def steer_subagent(subagent_id: str, text: str, *, owner_session_id: Optional[str]=None, owner_transport: Any=None, owner_session_record: Any=None) -> bool:
    ...
    with _active_subagents_lock:
        ...
        if owner_session_id is not None:
            if record.get('owner_session_id') != owner_session_id or owner_transport is None or record.get('owner_transport') is not owner_transport or (owner_session_record is None) or (record.get('owner_session_record') is not owner_session_record):
                return False
        agent = record.get('agent')
        if agent is None:
            return False
...
def list_active_subagents() -> List[Dict[str, Any]]:
    ...
    """
    with _active_subagents_lock:
        return [{k: v for k, v in r.items() if k not in {'agent', 'owner_session_id', 'owner_transport', 'owner_session_record', 'accepting_steer'}} for r in _active_subagents.values()]
...
def _stringify_tool_content(content: Any) -> str:
    ...
    return str(content)
_TOOL_INPUT_TARGET_KEYS = frozenset({'cwd', 'destination_path', 'directory', 'dst', 'endpoint', 'file_path', 'new_path', 'old_path', 'path', 'source_path', 'src', 'target_path', 'url', 'urls'})
...
def _subagent_stop_tool_call_history(tool_trace: Any) -> List[Dict[str, Any]]:
    ...
    for item in tool_trace:
        ...
        history.append({'tool_name': tool_name, 'tool_input': _sanitize_tool_input_summary(item.get('input_summary')), 'input_bytes': _byte_count('args_bytes'), 'output_bytes': _byte_count('result_bytes'), 'status': status})
    return history
...
def _looks_like_error_output(content: Any) -> bool:
    ...
    return first.startswith('error:') or first.startswith('failed:') or first.startswith('traceback ') or first.startswith('exception:')
        ...
        try:
            ...
            if result > 10:
                ...
                if not _HIGH_CONCURRENCY_WARNED:
                    ...
                    logger.warning('delegation.max_concurrent_children=%d: each child consumes API tokens independently. High values multiply cost linearly.', result)
            return result
        except (TypeError, ValueError):
            logger.warning('delegation.max_concurrent_children=%r is not a valid integer; using default %d', val, _DEFAULT_MAX_CONCURRENT_CHILDREN)
...
def _get_max_async_children() -> int:
    ...
    if cfg.get('max_async_children') is not None and (not _LEGACY_MAX_ASYNC_WARNED):
        ...
        logger.warning('delegation.max_async_children is deprecated and ignored; delegation.max_concurrent_children now caps background delegations too. Remove the stale key from config.yaml.')
...
def _get_child_timeout() -> Optional[float]:
    ...
    if val is not None:
        try:
            ...
            logger.warning('delegation.child_timeout_seconds=%r is not a valid number; using default (no timeout)', val)
        else:
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``duck-agent-cli`` (which bundles
...
_LEGACY_EVENT_MAP: Dict[str, DelegateEvent] = {'_thinking': DelegateEvent.TASK_THINKING, 'reasoning.available': DelegateEvent.TASK_THINKING, 'tool.started': DelegateEvent.TASK_TOOL_STARTED, 'tool.completed': DelegateEvent.TASK_TOOL_COMPLETED, 'subagent_progress': DelegateEvent.TASK_PROGRESS}

def check_delegate_requirements() -> bool:
    """Delegation has no external requirements -- always available."""
    return True

def _build_child_system_prompt(goal: str, context: Optional[str]=None, *, workspace_path: Optional[str]=None, role: str='leaf', max_spawn_depth: int=2, child_depth: int=1) -> str:
...
def _resolve_workspace_hint(parent_agent) -> Optional[str]:
    ...
    """
    candidates = [os.getenv('TERMINAL_CWD'), getattr(getattr(parent_agent, '_subdirectory_hints', None), 'working_dir', None), getattr(parent_agent, 'terminal_cwd', None), getattr(parent_agent, 'cwd', None)]
...
def _strip_blocked_tools(toolsets: List[str]) -> List[str]:
    ...
    blocked_toolset_names = {name for name, defn in TOOLSETS.items() if name in _COMPOSITE_BLOCKED_TOOLSETS or all((t in DELEGATE_BLOCKED_TOOLS for t in defn.get('tools', [])))}
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``duck-agent-cli`` because those also contain
    ...
    return sorted((name for name, defn in TOOLSETS.items() if defn.get('tools') and set(defn.get('tools', ())).issubset(blocked_names)))
...
def _build_child_progress_callback(task_index: int, goal: str, parent_agent, task_count: int=1, *, subagent_id: Optional[str]=None, parent_id: Optional[str]=None, depth: Optional[int]=None, model: Optional[str]=None, toolsets: Optional[List[str]]=None, session_ref: Optional[Dict[str, Any]]=None) -> Optional[callable]:
    ...
    def _identity_kwargs() -> Dict[str, Any]:
        kw: Dict[str, Any] = {'task_index': task_index, 'task_count': task_count, 'goal': goal_label}
...
def _build_child_agent(task_index: int, goal: str, context: Optional[str], toolsets: Optional[List[str]], model: Optional[str], max_iterations: int, task_count: int, parent_agent, override_provider: Optional[str]=None, override_base_url: Optional[str]=None, override_api_key: Optional[str]=None, override_api_mode: Optional[str]=None, override_request_overrides: Optional[Dict[str, Any]]=None, override_max_tokens: Optional[int]=None, override_acp_command: Optional[str]=None, override_acp_args: Optional[List[str]]=None, role: str='leaf'):
    """
    ...
    if parent_enabled is not None:
        ...
        import model_tools
        parent_toolsets = {ts for name in parent_agent.valid_tool_names if (ts := model_tools.get_toolset_for_tool(name)) is not None}
    else:
    ...
    child_prompt = _build_child_system_prompt(goal, context, workspace_path=workspace_hint, role=effective_role, max_spawn_depth=max_spawn, child_depth=child_depth)
    parent_api_key = getattr(parent_agent, 'api_key', None)
    if not parent_api_key and hasattr(parent_agent, '_client_kwargs'):
        parent_api_key = parent_agent._client_kwargs.get('api_key')
    effective_model_for_cb = model or getattr(parent_agent, 'model', None)
    child_session_ref: Dict[str, Any] = {}
    child_progress_cb = _build_child_progress_callback(task_index, goal, parent_agent, task_count, subagent_id=subagent_id, parent_id=parent_subagent_id, depth=tui_depth, model=effective_model_for_cb, toolsets=child_toolsets, session_ref=child_session_ref)
    ...
    if override_acp_command:
        ...
        if not _shutil.which(override_acp_command):
            logger.warning('Ignoring acp_command=%r: binary not found on PATH; falling back to default transport.', override_acp_command)
    ...
    child_provider_require_parameters = getattr(parent_agent, 'provider_require_parameters', False)
    child_provider_data_collection = getattr(parent_agent, 'provider_data_collection', None) or ''
    ...
    child_pool = _resolve_child_credential_pool(effective_provider, parent_agent, effective_base_url)
    ...
    try:
        ...
        _invoke_hook('subagent_start', parent_session_id=getattr(parent_agent, 'session_id', None), parent_turn_id=getattr(parent_agent, '_current_turn_id', '') or '', parent_subagent_id=parent_subagent_id, child_session_id=getattr(child, 'session_id', None), child_subagent_id=subagent_id, child_role=effective_role, child_goal=goal)
    except Exception:
        logger.debug('subagent_start hook invocation failed', exc_info=True)
    return child

def _dump_subagent_timeout_diagnostic(*, child: Any, task_index: int, timeout_seconds: float, duration_seconds: float, worker_thread: Optional[threading.Thread], goal: str) -> Optional[str]:
    ...
    writes a dedicated log under ``~/.duck-agent/logs/subagent-<sid>-<ts>.log``
    ...
    try:
        ...
        _w('')
        _w('## Child config')
        for attr in ('model', 'provider', 'api_mode', 'base_url', 'max_iterations', 'quiet_mode', 'skip_memory', 'skip_context_files', 'platform', '_delegate_role', '_delegate_depth'):
            try:
...
def _trim_summary_with_footer(summary: str, cap: int, task_index: int) -> tuple[str, Optional[str]]:
    ...
    footer_lines = ['', '─' * 8 + ' [SUMMARY TRUNCATED] ' + '─' * 8, f"Showing {len(head):,} chars (head) + {len(tail):,} chars (tail) of {original_len:,} total — trimmed to protect the parent's context window."]
    if spill_path:
        middle_start_line = head.count('\n') + 2
        footer_lines.append(f'Full subagent output saved to: {spill_path}')
        footer_lines.append(f'To read the omitted middle: read_file path="{spill_path}" offset={middle_start_line} limit=200  (the file is the complete summary; raise/lower offset to page through it).')
    else:
...
def _apply_summary_budget(results: List[Dict[str, Any]], parent_agent) -> None:
    ...
    for entry in summaries:
        ...
        logger.debug('[subagent-%s] summary trimmed %d → ~%d chars (spill=%s)', entry.get('task_index', '?'), original_len, cap, spill_path or 'none')

def _run_single_child(task_index: int, goal: str, child=None, parent_agent=None, *, owner_session_id: Optional[str]=None, owner_transport: Any=None, owner_session_record: Any=None, **_kwargs) -> Dict[str, Any]:
    """
    ...
    import model_tools
    _saved_tool_names = getattr(child, '_delegate_saved_tool_names', list(model_tools._last_resolved_tool_names))
    ...
    def _heartbeat_loop():
        while not _heartbeat_stop.wait(_HEARTBEAT_INTERVAL):
            ...
            try:
                ...
                activity_advanced = child_activity_ts is not None and (_last_seen_activity_ts[0] is None or child_activity_ts > _last_seen_activity_ts[0])
                ...
                if _stale_count[0] >= stale_limit:
                    logger.warning('Subagent %d appears stale (no progress for %d heartbeat cycles, tool=%s) — stopping heartbeat', task_index, _stale_count[0], child_tool or '<none>')
                    break
                if child_tool:
                    ...
                    if child_desc:
                        desc = f'delegate_task: subagent {child_desc} (iteration {child_iter}/{child_max})'
            except Exception:
                pass
            try:
                touch(desc)
    ...
    if _subagent_id:
        ...
        _register_subagent({'subagent_id': _subagent_id, 'parent_id': _parent_sid if isinstance(_parent_sid, str) else None, 'depth': _tui_depth, 'goal': goal, 'model': getattr(child, 'model', None) if isinstance(getattr(child, 'model', None), str) else None, 'started_at': time.time(), 'status': 'running', 'tool_count': 0, 'agent': child, 'owner_session_id': owner_session_id, 'owner_transport': owner_transport, 'owner_session_record': owner_session_record})
    try:
        ...
        _timeout_executor = DaemonThreadPoolExecutor(max_workers=1, initializer=_set_subagent_approval_cb, initargs=(_get_subagent_approval_callback(),))
        ...
        try:
            ...
            logger.warning('Subagent %d %s after %.1fs', task_index, 'timed out' if is_timeout else f'raised {type(_timeout_exc).__name__}', duration)
            ...
            if is_timeout and child_api_calls == 0:
                diagnostic_path = _dump_subagent_timeout_diagnostic(child=child, task_index=task_index, timeout_seconds=float(child_timeout or 0.0), duration_seconds=float(duration), worker_thread=_worker_thread_holder.get('t'), goal=goal)
                if diagnostic_path:
                    logger.warning('Subagent %d 0-API-call timeout — diagnostic written to %s', task_index, diagnostic_path)
            if child_progress_cb:
                try:
                    child_progress_cb('subagent.complete', preview=f'Timed out after {duration}s' if is_timeout else str(_timeout_exc), status='timeout' if is_timeout else 'error', duration_seconds=duration, summary='')
                except Exception:
                    pass
            if is_timeout:
                if child_api_calls == 0:
                    _err = f'Subagent timed out after {child_timeout}s without making any API call — the child never reached its first LLM request (prompt construction, credential resolution, or transport may be stuck).'
                    if diagnostic_path:
                        _err += f' Diagnostic: {diagnostic_path}'
                else:
                    _err = f'Subagent timed out after {child_timeout}s with {child_api_calls} API call(s) completed — likely stuck on a slow API call, tool call, or unresponsive network request.'
                    if diagnostic_path:
                        _err += f' Diagnostic: {diagnostic_path}'
            else:
                _err = str(_timeout_exc)
            _error_entry = {'task_index': task_index, 'status': 'timeout' if is_timeout else 'error', 'summary': None, 'error': _err, 'exit_reason': 'timeout' if is_timeout else 'error', 'api_calls': child_api_calls, 'duration_seconds': duration, 'timeout_seconds': child_timeout if is_timeout else None, 'timed_out_after_seconds': duration if is_timeout else None, 'timeout_phase': 'before_first_llm_call' if is_timeout and child_api_calls == 0 else 'after_llm_calls' if is_timeout else None, '_child_role': getattr(child, '_delegate_role', None), 'diagnostic_path': diagnostic_path}
            if _late_pending_steer:
                _error_entry['missed_steer'] = _late_pending_steer
                _error_entry['error'] += f' [steer did not land before the subagent stopped: {_late_pending_steer}]'
            return _error_entry
        finally:
        ...
        if isinstance(_output_schema, dict):
            ...
            if not _schema_valid and _first_text.strip() and (not result.get('interrupted', False)):
                ...
                _retry_result = None
                try:
                    _retry_result = child.run_conversation(user_message=build_retry_message(_schema_errors), task_id=child_task_id, stream_callback=_relay_child_text)
        ...
        if _late_pending_steer:
            ...
            result['pending_steer'] = f'{_existing_pending}\n{_late_pending_steer}' if isinstance(_existing_pending, str) and _existing_pending else _late_pending_steer
            ...
            for msg in messages:
                ...
                if msg.get('role') == 'assistant':
                    for tc in msg.get('tool_calls') or []:
                        ...
                        entry_t = {'tool': fn.get('name', 'unknown'), 'args_bytes': len(arguments), 'input_summary': _summarize_tool_arguments(arguments)}
                    ...
                    is_error = _looks_like_error_output(content)
                    result_meta = {'result_bytes': len(content), 'status': 'error' if is_error else 'ok'}
        ...
        entry: Dict[str, Any] = {'task_index': task_index, 'status': status, 'summary': summary, 'api_calls': api_calls, 'duration_seconds': duration, 'model': _model if isinstance(_model, str) else None, 'exit_reason': exit_reason, 'tokens': {'input': _input_tokens if isinstance(_input_tokens, (int, float)) else 0, 'output': _output_tokens if isinstance(_output_tokens, (int, float)) else 0}, 'tool_trace': tool_trace, '_child_role': getattr(child, '_delegate_role', None), '_child_cost_usd': float(getattr(child, 'session_estimated_cost_usd', 0.0) or 0.0) if isinstance(getattr(child, 'session_estimated_cost_usd', 0.0), (int, float)) else 0.0}
        ...
        if isinstance(_missed_steer, str) and _missed_steer.strip():
            ...
            _miss_note = f'[steer did not land — the subagent finished before it could be delivered: {_missed_steer}]'
            entry['summary'] = f'{summary}\n\n{_miss_note}' if summary else _miss_note
        try:
            if parent_task_id and parent_reads_snapshot:
                sibling_writes = file_state.writes_since(parent_task_id, wall_start, parent_reads_snapshot)
                if sibling_writes:
                    mod_paths = sorted({p for paths in sibling_writes.values() for p in paths})
                    if mod_paths:
                        reminder = '\n\n[NOTE: subagent modified files the parent previously read — re-read before editing: ' + ', '.join(mod_paths[:8]) + (f' (+{len(mod_paths) - 8} more)' if len(mod_paths) > 8 else '') + ']'
        ...
        try:
            ...
            _files_written_map = {}
        _files_written = sorted({p for tid, paths in _files_written_map.items() if tid == child_task_id for p in paths})[:40]
        _output_tail = _extract_output_tail(result, max_entries=8, max_chars=600)
        complete_kwargs: Dict[str, Any] = {'preview': summary[:160] if summary else entry.get('error', ''), 'status': status, 'duration_seconds': duration, 'summary': summary[:500] if summary else entry.get('error', ''), 'input_tokens': int(_input_tokens) if isinstance(_input_tokens, (int, float)) else 0, 'output_tokens': int(_output_tokens) if isinstance(_output_tokens, (int, float)) else 0, 'reasoning_tokens': int(_reasoning_tokens) if isinstance(_reasoning_tokens, (int, float)) else 0, 'api_calls': int(api_calls) if isinstance(api_calls, (int, float)) else 0, 'files_read': _files_read, 'files_written': _files_written, 'output_tail': _output_tail}
        ...
        _error_entry = {'task_index': task_index, 'status': 'error', 'summary': None, 'error': str(exc), 'api_calls': 0, 'duration_seconds': duration, '_child_role': getattr(child, '_delegate_role', None)}
        if _late_pending_steer:
            _error_entry['missed_steer'] = _late_pending_steer
            _error_entry['error'] += f' [steer did not land before the subagent stopped: {_late_pending_steer}]'
        return _error_entry
        ...
        try:
            ...
            child_turn_is_active = relay_runtime.SESSION_COORDINATOR.has_active_turn(profile_key=relay_runtime.current_profile_key(), session_id=child_session_id)
...
def _finalize_child_results(results: List[Dict[str, Any]], task_list: List[Dict[str, Any]], children: List[tuple[int, Dict[str, Any], Any]], parent_agent) -> None:
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        if parent_agent and getattr(parent_agent, '_memory_manager', None):
            for entry in results:
                try:
                    ...
                    task_goal = task_list[task_index]['goal'] if isinstance(task_index, int) and 0 <= task_index < len(task_list) else ''
                    child = child_by_index.get(task_index)
                    parent_agent._memory_manager.on_delegation(task=task_goal, result=entry.get('summary', '') or '', child_session_id=getattr(child, 'session_id', ''))
                except Exception:
                    pass
        ...
        for entry in results:
            ...
            try:
                ...
                invoke_hook('subagent_stop', parent_session_id=parent_session_id, parent_turn_id=getattr(parent_agent, '_current_turn_id', '') or '', child_session_id=getattr(child, 'session_id', None), child_role=child_role, child_summary=entry.get('summary'), child_status=entry.get('status'), tool_call_history=_subagent_stop_tool_call_history(entry.get('tool_trace')), duration_ms=int((entry.get('duration_seconds') or 0) * 1000))
            except Exception:
...
def _recover_tasks_from_json_string(tasks: Any) -> tuple[Optional[List[Dict[str, Any]]], Optional[str]]:
    ...
    return (parsed, None)
_PLACEHOLDER_GOAL_RE = re.compile('^(todo|task\\s*\\d+)$', re.IGNORECASE)
...
def _validate_batch_tasks(task_list: List[Dict[str, Any]]) -> Optional[str]:
    ...
    """
    if len(task_list) < 2:
        return 'Batch mode requires at least 2 tasks. For a single task, use the `goal` parameter instead of `tasks`: delegate_task(goal="...", context="...").'
    seen: Dict[str, int] = {}
    for i, task in enumerate(task_list):
        ...
        if prev is not None:
            return f"Task {i} duplicates task {prev}: both have the goal {goal!r}. Each task in a batch must do distinct work — rewrite the goals so they don't overlap, or drop the duplicate."
        seen[normalized] = i
        if _PLACEHOLDER_GOAL_RE.match(normalized):
            return f'Task {i} has a placeholder goal ({goal!r}). Replace it with a specific, self-contained description of what the subagent should accomplish.'
        marker = _TEMPLATE_MARKER_RE.search(goal)
        if marker:
            return f'Task {i} goal contains an unexpanded template marker ({marker.group(0)!r}). Substitute the real value before calling delegate_task — subagents cannot resolve placeholders.'
        if len(goal) < _MIN_BATCH_GOAL_LEN:
            return f'Task {i} goal is too short ({goal!r}). Write a specific, self-contained goal of at least {_MIN_BATCH_GOAL_LEN} characters so the subagent knows exactly what to do.'
    return None

def delegate_task(goal: Optional[str]=None, context: Optional[str]=None, tasks: Optional[List[Dict[str, Any]]]=None, max_iterations: Optional[int]=None, role: Optional[str]=None, background: Optional[bool]=None, output_schema: Optional[Dict[str, Any]]=None, parent_agent=None) -> str:
    """
    ...
    if is_spawn_paused():
        return tool_error('Delegation spawning is paused. Clear the pause via the TUI (`p` in /agents) or the `delegation.pause` RPC before retrying.')
    top_role = _normalize_role(role)
    background = is_truthy_value(background, default=False) if background is not None else False
    depth = getattr(parent_agent, '_delegate_depth', 0)
    max_spawn = _get_max_spawn_depth()
    if depth >= max_spawn:
        return tool_error(f'Delegation depth limit reached (depth={depth}, max_spawn_depth={max_spawn}). Raise delegation.max_spawn_depth in config.yaml if deeper nesting is required (no hard ceiling, but each level multiplies API cost).')
    cfg = _load_config()
    default_max_iter = cfg.get('max_iterations', DEFAULT_MAX_ITERATIONS)
    if max_iterations is not None and max_iterations != default_max_iter:
        logger.debug('delegate_task: ignoring caller-supplied max_iterations=%s; using delegation.max_iterations=%s from config', max_iterations, default_max_iter)
    ...
    if tasks and isinstance(tasks, list):
        if len(tasks) > max_children:
            return tool_error(f'Too many tasks: {len(tasks)} provided, but max_concurrent_children is {max_children}. Either reduce the task count, split into multiple delegate_task calls, or increase delegation.max_concurrent_children in config.yaml.')
        task_list = tasks
    ...
    from tools.delegation_live_log import create_live_transcripts, update_manifest_statuses, wrap_progress_callback
    ...
    _origin_owner_transport, _origin_owner_session_record = _capture_gateway_steer_authority(_origin_ui_session_id)
    children = []
    ...
    def _execute_and_aggregate(*, honor_parent_interrupt: bool=True) -> dict:
        ...
        if n_tasks == 1:
            ...
            result = _run_single_child(_i, _t['goal'], child, parent_agent, owner_session_id=_origin_ui_session_id or None, owner_transport=_origin_owner_transport, owner_session_record=_origin_owner_session_record)
            ...
            with DaemonThreadPoolExecutor(max_workers=max_children) as executor:
                ...
                for i, t, child in children:
                    ...
                    future = executor.submit(child_context.run, _run_single_child, task_index=i, goal=t['goal'], child=child, parent_agent=parent_agent, owner_session_id=_origin_ui_session_id or None, owner_transport=_origin_owner_transport, owner_session_record=_origin_owner_session_record)
                    futures[future] = i
                        ...
                        for f in pending:
                            ...
                            if f.done():
                                try:
                                    ...
                                    entry = {'task_index': idx, 'status': 'error', 'summary': None, 'error': str(exc), 'api_calls': 0, 'duration_seconds': 0, '_child_role': getattr(_child_by_index.get(idx), '_delegate_role', None)}
                            else:
                                entry = {'task_index': idx, 'status': 'interrupted', 'summary': None, 'error': 'Parent agent interrupted — child did not finish in time', 'api_calls': 0, 'duration_seconds': 0, '_child_role': getattr(_child_by_index.get(idx), '_delegate_role', None)}
    ...
    if background:
        ...
        if not _async_ok:
            ...
            if _wake_sid:
                logger.info('delegate_task: async delivery unsupported on this session, but a session id is bound (%s) — dispatching in the background and waking the session via self-post when it completes instead of forcing synchronous execution.', _wake_sid)
                _async_ok = True
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result['note'] = 'background=true is not available in this session — it cannot receive a detached subagent result after the turn ends (a one-shot runner such as `duck-agent -z`, a cron job, a Kanban worker, or a stateless HTTP endpoint). The subagent(s) ran SYNCHRONOUSLY and the result is included above.'
            return json.dumps(_sync_result, ensure_ascii=False)
        _session_key = get_current_session_key(default='')
        try:
            from gateway.session_context import get_session_env
            _source = get_session_env('HERMES_SESSION_SOURCE', '')
            _origin_ui_session_id = get_session_env('HERMES_UI_SESSION_ID', '') or _origin_ui_session_id
            if _source == 'tui':
        ...
        def _batch_progress():
            ...
            for _c in _child_agents:
                try:
                    ...
                    parts.append((_summary.get('api_call_count', 0), _tool, _summary.get('last_activity_ts')))
                    in_tool = in_tool or bool(_tool)
                except Exception:
                    parts.append(None)
            return (tuple(parts), in_tool)
        _goals = [t['goal'] for t in task_list]
        dispatch = dispatch_async_delegation_batch(goals=_goals, context=context, toolsets=None, role=top_role, model=creds['model'], session_key=_session_key, origin_ui_session_id=_origin_ui_session_id, origin_session_id=_wake_sid, parent_session_id=_parent_session_id, runner=_batch_runner, interrupt_fn=_batch_interrupt, max_async_children=_get_max_async_children(), delegation_id=live_deleg_id, progress_fn=_batch_progress)
        if dispatch.get('status') == 'dispatched':
            n = len(_goals)
            note = 'Subagent is running in the background. You and the user can keep working; its full result re-enters the conversation as a new message when it finishes. Do not wait or poll — just continue.' if n == 1 else f'{n} subagents are running in parallel in the background. You and the user can keep working; they wait on each other and their consolidated results re-enter the conversation as a single message once ALL of them finish. Do not wait or poll — just continue.'
            payload = {'status': 'dispatched', 'mode': 'background', 'count': n, 'delegation_id': dispatch['delegation_id'], 'goals': _goals, 'note': note}
            if live_paths:
                payload['live_transcripts'] = list(live_paths)
                payload['live_transcripts_hint'] = 'Each subagent streams a human-readable transcript of its operations to the file listed above (append-only, one per task). Read or `tail -f` these paths at any time to watch a child work while it runs.'
            return json.dumps(payload, ensure_ascii=False)
        logger.info('delegate_task: async pool at capacity (%s); running the whole batch synchronously instead.', dispatch.get('error', 'rejected'))
        _cap_result = _execute_and_aggregate()
        if isinstance(_cap_result, dict):
            _cap_result['note'] = 'The background delegation pool was at capacity (delegation.max_concurrent_children), so the subagent(s) ran SYNCHRONOUSLY and the result is included above. Raise delegation.max_concurrent_children in config.yaml to allow more concurrent background delegations.'
        return json.dumps(_cap_result, ensure_ascii=False)
    return json.dumps(_execute_and_aggregate(), ensure_ascii=False)

def _resolve_child_credential_pool(effective_provider: Optional[str], parent_agent, effective_base_url: Optional[str]=None):
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url and (not _is_native_sdk_provider):
        ...
        if base_url_hostname(configured_base_url) == 'chatgpt.com' and '/backend-api/codex' in base_lower:
        ...
        return {'model': configured_model, 'provider': provider, 'base_url': configured_base_url, 'api_key': api_key, 'api_mode': api_mode}
    if not configured_provider:
        return {'model': configured_model, 'provider': None, 'base_url': None, 'api_key': None, 'api_mode': None, 'request_overrides': None, 'max_output_tokens': None}
    try:
        from hermes_cli.runtime_provider import resolve_runtime_provider
        runtime = resolve_runtime_provider(requested=configured_provider, target_model=configured_model)
    except Exception as exc:
        raise ValueError(f"Cannot resolve delegation provider '{configured_provider}': {exc}. Check that the provider is configured (API key set, valid provider name), or set delegation.base_url/delegation.api_key for a direct endpoint. Available providers: openrouter, nous, zai, kimi-coding, minimax.") from exc
    api_key = runtime.get('api_key', '')
    if not api_key:
        raise ValueError(f"Delegation provider '{configured_provider}' resolved but has no API key. Set the appropriate environment variable or run 'duck-agent auth'.")
    return {'model': configured_model or runtime.get('model') or None, 'provider': configured_provider if runtime.get('provider') == _RUNTIME_PROVIDER_CUSTOM else runtime.get('provider'), 'base_url': runtime.get('base_url'), 'api_key': api_key, 'api_mode': runtime.get('api_mode'), 'request_overrides': dict(runtime.get('request_overrides') or {}), 'max_output_tokens': runtime.get('max_output_tokens'), 'command': runtime.get('command'), 'args': list(runtime.get('args') or [])}

def _load_config() -> dict:
    """Load delegation config from the active Duck Agent config.

    Prefer the shared persistent loader because it follows the active
    DUCK_AGENT_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``HERMES_IGNORE_USER_CONFIG=1`` (``duck-agent chat --ignore-user-config``) is
...
def _build_tasks_param_description() -> str:
    ...
    return f'Batch mode: tasks to run in parallel (up to {max_children} for this user, set via delegation.max_concurrent_children). Each gets its own subagent with isolated context and terminal session. When provided, top-level goal/context/role are ignored.'
...
def _build_role_param_description() -> str:
    ...
    if max_depth >= 2 and orchestrator_on:
        nesting_note = f'Nesting IS enabled for this user (max_spawn_depth={max_depth}): orchestrator children can themselves delegate up to {max_depth - 1} more level(s) deep.'
    elif max_depth >= 2 and (not orchestrator_on):
        nesting_note = "Nesting is currently disabled (delegation.orchestrator_enabled=false); 'orchestrator' is silently forced to 'leaf'."
    else:
        nesting_note = f"Nesting is OFF for this user (max_spawn_depth={max_depth}); 'orchestrator' is silently forced to 'leaf'. Raise delegation.max_spawn_depth in config.yaml to enable."
    return f"Role of the child agent. 'leaf' (default) = focused worker, cannot delegate further. 'orchestrator' = can use delegate_task to spawn its own workers. {nesting_note}"
...
DELEGATE_TASK_SCHEMA = {'name': 'delegate_task', 'description': "Spawn one or more subagents in isolated contexts. Description is rebuilt at every get_definitions() call to reflect the user's current delegation limits.", 'parameters': {'type': 'object', 'properties': {'goal': {'type': 'string', 'description': 'What the subagent should accomplish. Be specific and self-contained -- the subagent knows nothing about your conversation history.'}, 'context': {'type': 'string', 'description': 'Background information the subagent needs: file paths, error messages, project structure, constraints. The more specific you are, the better the subagent performs.'}, 'tasks': {'type': 'array', 'items': {'type': 'object', 'properties': {'goal': {'type': 'string', 'description': 'Task goal'}, 'context': {'type': 'string', 'description': 'Task-specific context'}, 'role': {'type': 'string', 'enum': ['leaf', 'orchestrator'], 'description': "Per-task role override. See top-level 'role' for semantics."}, 'output_schema': {'type': 'object', 'description': "Optional JSON Schema the subagent's final answer must validate against. The child is told the contract up front; the parent validates the final answer and allows one bounded correction retry. The result entry gains schema_valid (and schema_errors on final failure). Keep schemas forgiving: require only fields you will actually read."}}, 'required': ['goal']}, 'description': '(rebuilt at get_definitions() time)'}, 'role': {'type': 'string', 'enum': ['leaf', 'orchestrator'], 'description': '(rebuilt at get_definitions() time)'}, 'output_schema': {'type': 'object', 'description': "Optional JSON Schema for the single-goal form — the subagent's final answer must validate against it (same semantics as tasks[].output_schema)."}, 'background': {'type': 'boolean', 'description': 'DEPRECATED / IGNORED. Top-level single and batch delegations run in the background automatically — you do not need to (and cannot) opt in or out. A single result or consolidated batch result re-enters the conversation when the work finishes; just continue working in the meantime. Setting this has no effect; the parameter remains only for backward compatibility.'}}, 'required': []}}
...
registry.register(name='delegate_task', toolset='delegation', schema=DELEGATE_TASK_SCHEMA, handler=lambda args, **kw: delegate_task(goal=args.get('goal'), context=args.get('context'), tasks=_strip_model_hidden_task_fields(args.get('tasks')), max_iterations=args.get('max_iterations'), role=args.get('role'), background=_model_background_value(args, kw.get('parent_agent')), output_schema=args.get('output_schema'), parent_agent=kw.get('parent_agent')), check_fn=check_delegate_requirements, emoji='🔀', dynamic_schema_overrides=_build_dynamic_schema_overrides)