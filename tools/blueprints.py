...
`duck-agent skills publish` for sharing. No new source type, no new store, no new
...
__all__ = ['BlueprintSpec', 'parse_blueprint', 'blueprint_spec_for_installed', 'blueprint_to_job_spec', 'create_blueprint_job', 'register_blueprint_suggestion', 'export_blueprint', 'BlueprintError']
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.duck-agent.blueprint`` automation spec for a skill."""
    skill_name: str
    schedule: str
    deliver: str = 'origin'
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.duck-agent.blueprint`` is a mapping containing
    ...
    hermes = meta.get('duck-agent') if isinstance(meta, dict) else None
    blueprint = hermes.get('blueprint') if isinstance(hermes, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError('metadata.duck-agent.blueprint must be a mapping')
    ...
    return BlueprintSpec(skill_name=name, schedule=schedule, deliver=deliver, prompt=prompt, no_agent=no_agent, model=str(model).strip() if model else None, provider=str(provider).strip() if provider else None, enabled_toolsets=[str(t) for t in toolsets] if toolsets else None, raw=blueprint)
...
def blueprint_to_job_spec(spec: BlueprintSpec, *, name: Optional[str]=None) -> Dict[str, Any]:
    ...
    """
    return {'prompt': spec.prompt, 'schedule': spec.schedule, 'name': name or f'blueprint:{spec.skill_name}', 'deliver': spec.deliver, 'skills': [spec.skill_name] if spec.skill_name else None, 'model': spec.model, 'provider': spec.provider, 'enabled_toolsets': spec.enabled_toolsets, 'no_agent': spec.no_agent}
...
def register_blueprint_suggestion(spec: BlueprintSpec) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return None
    return add_suggestion(title=f"Schedule '{spec.skill_name}'", description=f"The '{spec.skill_name}' blueprint runs on schedule {spec.schedule}" + (f', delivering to {spec.deliver}' if spec.deliver and spec.deliver != 'origin' else '') + '.', source='blueprint', job_spec=blueprint_to_job_spec(spec), dedup_key=f'blueprint:{spec.skill_name}:{spec.schedule}')
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str]=None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.duck-agent.blueprint`` block) they can hand
    to ``duck-agent skills publish`` to share. ``body`` is the plain-language
    ...
    description = (body.strip().splitlines() or ['Shared automation blueprint.'])[0][:200] if body.strip() else 'Shared automation blueprint.'
    frontmatter = {'name': name, 'description': description, 'version': '1.0.0', 'license': 'MIT', 'metadata': {'duck-agent': {'tags': ['blueprint', 'automation'], 'blueprint': blueprint_block}}}
