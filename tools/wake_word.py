"""Wake-word ("Hey Duck Agent") detection — hands-free session trigger.
...
wake word, Duck Agent opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey duck-agent" model (``tools/wakewords/``) so the wake word
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey duck-agent", "hey coder", or any other phrase works
