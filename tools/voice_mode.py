...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.duck-agent/config.yaml during a
...
def is_tts_echo(
    ...
    Duck Agent just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match Duck Agent' own words, so a high ratio is
