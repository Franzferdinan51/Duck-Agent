...
One ``CDPSupervisor`` runs per Duck Agent ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Duck Agent logs or tracebacks. Falls back to a
...
class PendingDialog:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'id': self.id, 'type': self.type, 'message': _redact_supervisor_text(self.message), 'default_prompt': _redact_supervisor_text(self.default_prompt), 'opened_at': self.opened_at, 'frame_id': self.frame_id}
...
class DialogRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'id': self.id, 'type': self.type, 'message': _redact_supervisor_text(self.message), 'opened_at': self.opened_at, 'closed_at': self.closed_at, 'closed_by': self.closed_by, 'frame_id': self.frame_id}
...
class CDPSupervisor:
    ...
    def start(self, timeout: float=15.0) -> None:
        ...
        if not self._ready_event.wait(timeout=timeout):
            ...
            raise TimeoutError(f'CDP supervisor did not attach within {timeout}s (cdp_url={_safe_url[:80]}...)')
    ...
    def snapshot(self) -> SupervisorSnapshot:
        ...
        with self._state_lock:
            ...
            active = self._active
        return SupervisorSnapshot(pending_dialogs=dialogs, recent_dialogs=recent, frame_tree=frames_tree, console_errors=console, active=active, cdp_url=self.cdp_url, task_id=self.task_id)
    ...
    def evaluate_runtime(self, expression: str, *, return_by_value: bool=True, await_promise: bool=True, timeout: float=10.0) -> Dict[str, Any]:
        ...
        async def _do_eval(by_value: bool) -> Dict[str, Any]:
            return await self._cdp('Runtime.evaluate', {'expression': expression, 'returnByValue': by_value, 'awaitPromise': await_promise, 'userGesture': True}, session_id=session_id, timeout=timeout)
        from agent.async_utils import safe_schedule_threadsafe
        ...
        while not self._stop_requested:
            try:
                ...
                if not self._ready_event.is_set():
                    ...
                    self._ready_event.set()
                    return
                logger.warning('CDP supervisor %s: connect failed (attempt %s): %s', self.task_id, attempt, _redact_cdp_error_text(e))
            ...
            try:
                ...
                if not self._ready_event.is_set():
                    ...
                    self._ready_event.set()
                    raise
                logger.warning('CDP supervisor %s: session dropped after %.1fs: %s', self.task_id, time.time() - last_success_at, _redact_cdp_error_text(e))
            finally:
                with self._state_lock:
                    self._active = False
    ...
    async def _install_dialog_bridge(self, session_id: str) -> None:
        ...
        try:
            await self._cdp('Fetch.enable', {'patterns': [{'urlPattern': DIALOG_BRIDGE_URL_PATTERN, 'requestStage': 'Request'}], 'handleAuthRequests': False}, session_id=session_id, timeout=5.0)
        except Exception as e:
    ...
    async def _cdp(self, method: str, params: Optional[Dict[str, Any]]=None, *, session_id: Optional[str]=None, timeout: float=10.0) -> Dict[str, Any]:
    ...
    async def _on_event(self, method: str, params: Dict[str, Any], session_id: Optional[str]) -> None:
    ...
    async def _on_dialog_opening(self, params: Dict[str, Any], session_id: Optional[str]) -> None:
        self._dialog_seq += 1
        dialog = PendingDialog(id=f'd-{self._dialog_seq}', type=str(params.get('type') or ''), message=str(params.get('message') or ''), default_prompt=str(params.get('defaultPrompt') or ''), opened_at=time.time(), cdp_session_id=session_id or self._page_session_id or '', frame_id=params.get('frameId'))
        if self.dialog_policy == DIALOG_POLICY_AUTO_DISMISS:
            ...
            handle = loop.call_later(self.dialog_timeout_s, lambda: asyncio.create_task(self._dialog_timeout_expired(dialog.id)))
    ...
    def _archive_dialog_locked(self, dialog: PendingDialog, closed_by: str) -> None:
        ...
        record = DialogRecord(id=dialog.id, type=dialog.type, message=dialog.message, opened_at=dialog.opened_at, closed_at=time.time(), closed_by=closed_by, frame_id=dialog.frame_id)
    ...
    async def _on_dialog_closed(self, params: Dict[str, Any], session_id: Optional[str]) -> None:
        with self._state_lock:
            candidate_ids = [d.id for d in self._pending_dialogs.values() if d.cdp_session_id == session_id and d.bridge_request_id is None]
            if candidate_ids:
                did = candidate_ids[0]
    ...
    async def _on_fetch_paused(self, params: Dict[str, Any], session_id: Optional[str]) -> None:
        ...
        self._dialog_seq += 1
        dialog = PendingDialog(id=f'd-{self._dialog_seq}', type=kind, message=message, default_prompt=default_prompt, opened_at=time.time(), cdp_session_id=session_id or self._page_session_id or '', frame_id=params.get('frameId'), bridge_request_id=str(request_id))
        if self.dialog_policy == DIALOG_POLICY_AUTO_DISMISS:
            with self._state_lock:
                self._archive_dialog_locked(dialog, 'auto_policy')
    ...
    async def _fulfill_bridge_request(self, dialog: PendingDialog, *, accept: bool, prompt_text: str) -> None:
        ...
        try:
            ...
            await self._cdp('Fetch.fulfillRequest', {'requestId': dialog.bridge_request_id, 'responseCode': 200, 'responseHeaders': [{'name': 'Content-Type', 'value': 'application/json'}, {'name': 'Access-Control-Allow-Origin', 'value': '*'}], 'body': _b64.b64encode(body).decode()}, session_id=dialog.cdp_session_id or None, timeout=5.0)
        except Exception as e:
    ...
    async def _on_target_attached(self, params: Dict[str, Any]) -> None:
        ...
        if target_type == 'iframe':
            ...
            with self._state_lock:
                ...
                self._frames[target_id] = FrameInfo(frame_id=target_id, url=str(info.get('url') or ''), origin='', parent_frame_id=existing.parent_frame_id if existing else None, is_oopif=True, cdp_session_id=sid, name=str(info.get('title') or (existing.name if existing else '')))
        ...
        with self._state_lock:
            for fid, frame in list(self._frames.items()):
                if frame.cdp_session_id == sid:
                    self._frames[fid] = FrameInfo(frame_id=frame.frame_id, url=frame.url, origin=frame.origin, parent_frame_id=frame.parent_frame_id, is_oopif=frame.is_oopif, cdp_session_id=None, name=frame.name)
...
class _SupervisorRegistry:
    ...
    def get_or_start(self, task_id: str, cdp_url: str, *, dialog_policy: str=DEFAULT_DIALOG_POLICY, dialog_timeout_s: float=DEFAULT_DIALOG_TIMEOUT_S, start_timeout: float=15.0) -> CDPSupervisor:
...
__all__ = ['CDPSupervisor', 'ConsoleEvent', 'DEFAULT_DIALOG_POLICY', 'DEFAULT_DIALOG_TIMEOUT_S', 'DIALOG_POLICY_AUTO_ACCEPT', 'DIALOG_POLICY_AUTO_DISMISS', 'DIALOG_POLICY_MUST_RESPOND', 'DialogRecord', 'FrameInfo', 'PendingDialog', 'SUPERVISOR_REGISTRY', 'SupervisorSnapshot', '_SupervisorRegistry']