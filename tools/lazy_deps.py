"""
Lazy dependency installer for opt-in Duck Agent backends.

Many Duck Agent features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``duck-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``duck-agent tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  and makes ``/opt/duck-agent`` read-only), setting
  ...
  installed package cannot brick Duck Agent, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {'provider.anthropic': ('anthropic==0.87.0',), 'provider.bedrock': ('boto3==1.42.89',), 'provider.vertex': ('google-auth==2.55.1', 'pyasn1==0.6.4'), 'provider.azure_identity': ('azure-identity==1.25.3',), 'search.exa': ('exa-py==2.10.2',), 'search.firecrawl': ('firecrawl-py==4.17.0',), 'search.parallel': ('parallel-web==0.4.2',), 'export.otlp': ('opentelemetry-sdk==1.39.1', 'opentelemetry-exporter-otlp-proto-http==1.39.1'), 'tts.mistral': ('mistralai==2.4.8',), 'tts.edge': ('edge-tts==7.2.7',), 'tts.elevenlabs': ('elevenlabs==1.59.0',), 'stt.mistral': ('mistralai==2.4.8',), 'stt.faster_whisper': ('faster-whisper==1.2.1', 'sounddevice==0.5.5', 'numpy==2.4.3'), 'stt.silk': ('pilk==0.2.4',), 'wake.openwakeword.tflite': ('ai-edge-litert==2.1.6',), 'wake.openwakeword': ('openwakeword==0.6.0', 'onnxruntime==1.27.0', 'sounddevice==0.5.5', 'numpy==2.4.3'), 'wake.sherpa': ('sherpa-onnx==1.13.4', 'sentencepiece==0.2.2', 'sounddevice==0.5.5', 'numpy==2.4.3'), 'wake.porcupine': ('pvporcupine==4.0.3', 'sounddevice==0.5.5', 'numpy==2.4.3'), 'image.fal': ('fal-client==0.13.1',), 'memory.honcho': ('honcho-ai==2.2.0',), 'memory.hindsight': ('hindsight-client==0.6.1',), 'memory.supermemory': ('supermemory==3.50.0',), 'memory.mem0': ('mem0ai==2.0.10',), 'platform.telegram': ('python-telegram-bot[webhooks]==22.6',), 'platform.discord': ('discord.py[voice]==2.7.1', 'brotlicffi==1.2.0.1', 'aiohttp==3.14.1'), 'platform.slack': ('slack-bolt==1.29.0', 'slack-sdk==3.43.0', 'aiohttp==3.14.1'), 'platform.matrix': ('mautrix[encryption]==0.21.0', 'aiosqlite==0.22.1', 'asyncpg==0.31.0', 'aiohttp-socks==0.11.0', 'aiohttp==3.14.1'), 'platform.dingtalk': ('dingtalk-stream==0.24.3', 'alibabacloud-dingtalk==2.2.42', 'qrcode==7.4.2'), 'platform.feishu': ('lark-oapi==1.6.8', 'qrcode==7.4.2'), 'platform.wecom_callback': ('defusedxml==0.7.1',), 'platform.teams': ('microsoft-teams-apps==2.0.13.4', 'aiohttp==3.14.1'), 'terminal.modal': ('modal==1.3.4',), 'terminal.daytona': ('daytona==0.155.0',), 'terminal.vercel': ('vercel==0.7.2',), 'skill.google_workspace': ('google-api-python-client==2.194.0', 'google-auth==2.55.1', 'google-auth-oauthlib==1.3.1', 'google-auth-httplib2==0.3.1', 'httplib2==0.32.0', 'pyasn1==0.6.4'), 'skill.youtube': ('youtube-transcript-api==1.2.4',), 'tool.acp': ('agent-client-protocol==0.9.0',), 'tool.dashboard': ('fastapi==0.133.1', 'uvicorn[standard]==0.41.0', 'starlette==1.3.1', 'python-multipart==0.0.32'), 'tool.vision': ('Pillow==12.3.0',), 'tool.doc_extract': ('firecrawl-anydoc==0.1.6',), 'tool.computer_use': ('mcp==1.28.1', 'starlette==1.3.1'), 'tool.trace_upload': ('huggingface-hub==1.24.0',)}
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``duck-agent update`` lazy-refresh pass.
    """
    if sys.platform == 'win32' and feature == 'platform.matrix':
        return 'unsupported on Windows: Matrix E2EE depends on python-olm, which has no Windows wheel and requires make + libolm to build from sdist. Run Duck Agent under WSL to use Matrix on Windows.'
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``duck-agent update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix='duck-agent-core-constraints-', suffix='.txt')
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int=300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        if uv_bin:
            try:
                r = subprocess.run([uv_bin, 'pip', 'install', *target_args, *constraint_args, *specs], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, env=uv_env, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
                if r.returncode == 0:
                    if target is not None:
                        _activate_target_on_syspath(target)
        ...
        try:
            probe = subprocess.run(pip_cmd + ['--version'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=15, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
            if probe.returncode != 0:
                raise FileNotFoundError('pip not in venv')
        except (subprocess.TimeoutExpired, FileNotFoundError):
            try:
                subprocess.run([sys.executable, '-m', 'ensurepip', '--upgrade', '--default-pip'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=120, check=True, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
            except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
                return _InstallResult(False, '', f'pip not available and ensurepip failed: {e}')
        try:
            r = subprocess.run(pip_cmd + ['install', *target_args, *constraint_args, *specs], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=timeout, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
...
def ensure(feature: str, *, prompt: bool=True) -> None:
    ...
    if _lazy_install_target() is None:
        try:
            ...
            managed_by = ''
        if managed_by:
            raise FeatureUnavailable(feature, missing, f"unsupported on {managed_by}-managed installs: this build's packages come from {managed_by}, so Duck Agent cannot install them at runtime. Add the dependencies for {feature!r} via {managed_by} (or run a pip/uv install of Duck Agent instead).")
    for spec in missing:
...
def feature_install_command(feature: str, *, venv_pip: bool=False) -> Optional[str]:
    ...
    (default install, ``DUCK_AGENT_HOME`` overrides, profile installs) and
    ...
    * **Gated** — honors ``security.allow_lazy_installs`` and refuses to run
    ...
    if not _allow_lazy_installs():
        ...
        if os.environ.get('HERMES_DISABLE_LAZY_INSTALLS') == '1' and target is None:
            reason = 'runtime installs are disabled on this deployment: the agent environment is immutable and no writable install target is configured (HERMES_LAZY_INSTALL_TARGET)'
        else:
...
def active_features() -> list[str]:
    ...
    Used by ``duck-agent update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool=False) -> dict[str, str]:
    ...
    Intended for ``duck-agent update``. Never raises; lazy-install failures
