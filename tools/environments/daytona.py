...
class DaytonaEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str='/home/daytona', timeout: int=60, cpu: int=1, memory: int=5120, disk: int=10240, persistent_filesystem: bool=True, task_id: str='default'):
        requested_cwd = cwd
        ...
        if disk_gib > 10:
            logger.warning('Daytona: requested disk (%dGB) exceeds platform limit (10GB). Capping to 10GB.', disk_gib)
            disk_gib = 10
        resources = Resources(cpu=cpu, memory=memory_gib, disk=disk_gib)
        labels = {'hermes_task_id': task_id}
        sandbox_name = f'duck-agent-{task_id}'
        if self._persistent:
            try:
            ...
            if self._sandbox is None:
                try:
                    ...
                    self._sandbox = None
        if self._sandbox is None:
            self._sandbox = self._daytona.create(CreateSandboxFromImageParams(image=image, name=sandbox_name, labels=labels, auto_stop_interval=0, resources=resources))
        ...
        self._sync_manager = FileSyncManager(get_files_fn=lambda: iter_sync_files(f'{self._remote_home}/.duck-agent'), upload_fn=self._daytona_upload, delete_fn=self._daytona_delete, bulk_upload_fn=self._daytona_bulk_upload, bulk_download_fn=self._daytona_bulk_download)
    ...
    def _daytona_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        uploads = [FileUpload(source=host_path, destination=remote_path) for host_path, remote_path in files]
