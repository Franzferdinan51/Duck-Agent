...
def _msys_to_windows_path(cwd: str) -> str:
    ...
    if not _IS_WINDOWS or not cwd:
        return cwd
    m = re.match('^/(?:(?:cygdrive|mnt)/)?([a-zA-Z])(/.*)?$', cwd)
    if not m:
        return cwd
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Duck Agent was already launched from (for example ``duck-agent``
    while the process cwd is ``~/.duck-agent/duck-agent``), passing it through
    unchanged makes the wrapper run ``cd duck-agent`` *inside* the project
...
def _resolve_safe_cwd(cwd: str) -> str:
    ...
    if cwd and os.path.isdir(cwd):
        logger.warning('Configured terminal cwd %r exists but is not accessible to this user (uid=%s) — falling back to the nearest usable directory. If this is a gateway/cron process, check for root-owned paths leaking into terminal.cwd / TERMINAL_CWD (#65583).', cwd, getattr(os, 'getuid', lambda: '?')())
...
def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for Duck Agent-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Duck Agent-internal dynamic secret"
    ...
    """
    upper = key.upper()
    if upper.startswith('AUXILIARY_') and (upper.endswith('_API_KEY') or upper.endswith('_BASE_URL')):
        return True
    if upper.startswith('GATEWAY_RELAY_') and (upper.endswith('_SECRET') or upper.endswith('_KEY') or upper.endswith('_TOKEN')):
        return True
    return False

def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Duck Agent home override into subprocess env."""
    try:
        from hermes_constants import get_hermes_home_override
        value = get_hermes_home_override()
        if value:
            env['DUCK_AGENT_HOME'] = value
    except Exception:
        pass
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None=None) -> dict:
    """Filter Duck Agent-managed secrets from a subprocess environment."""
    try:
        from tools.env_passthrough import is_env_passthrough as _is_passthrough, resolve_passthrough_value as _resolve_passthrough_value
    except Exception:
...
def _scrub_delegated_child_kanban_env(env: dict[str, str]) -> dict[str, str]:
    ...
    try:
        ...
        pass
    return env
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({'GH_TOKEN', 'GITHUB_TOKEN', 'GITHUB_APP_ID', 'GITHUB_APP_PRIVATE_KEY_PATH', 'GITHUB_APP_INSTALLATION_ID', 'TELEGRAM_BOT_TOKEN', 'DISCORD_BOT_TOKEN', 'SLACK_BOT_TOKEN', 'SLACK_APP_TOKEN', 'SLACK_SIGNING_SECRET', 'GATEWAY_ALLOWED_USERS', 'GATEWAY_ALLOW_ALL_USERS', 'GATEWAY_RELAY_ID', 'GATEWAY_RELAY_SECRET', 'GATEWAY_RELAY_DELIVERY_KEY', 'HASS_TOKEN', 'EMAIL_PASSWORD', 'HERMES_DASHBOARD_SESSION_TOKEN', 'MODAL_TOKEN_ID', 'MODAL_TOKEN_SECRET', 'DAYTONA_API_KEY'})
...
def hermes_subprocess_env(*, inherit_credentials: bool=False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Duck Agent spawns legitimately needs them.
...
def build_subprocess_env(base: 'Mapping[str, str] | None'=None, *, inherit_profile_home: bool=True, scrub_secrets: bool=True, extra: 'Mapping[str, str] | None'=None) -> dict[str, str]:
    ...
    propagation (``DUCK_AGENT_HOME`` / subprocess ``HOME`` contract) and the
    Duck Agent secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      ``DUCK_AGENT_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Duck Agent home override into ``DUCK_AGENT_HOME`` and apply the
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``DUCK_AGENT_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        return _sanitize_subprocess_env(dict(base) if base is not None else os.environ.copy(), dict(extra) if extra else None)
...
def _find_bash() -> str:
    ...
    if not _IS_WINDOWS:
        return shutil.which('bash') or ('/usr/bin/bash' if os.path.isfile('/usr/bin/bash') else None) or ('/bin/bash' if os.path.isfile('/bin/bash') else None) or os.environ.get('SHELL') or '/bin/sh'
    candidates: list[str] = []
    custom = os.environ.get('HERMES_GIT_BASH_PATH')
    if custom and os.path.isfile(custom):
        candidates.append(custom)
    _local_appdata = os.environ.get('LOCALAPPDATA', '')
    _hermes_portable_git = os.path.join(_local_appdata, 'duck-agent', 'git') if _local_appdata else ''
    if _hermes_portable_git:
        for candidate in (os.path.join(_hermes_portable_git, 'bin', 'bash.exe'), os.path.join(_hermes_portable_git, 'usr', 'bin', 'bash.exe')):
            if os.path.isfile(candidate) and candidate not in candidates:
                candidates.append(candidate)
    for candidate in (os.path.join(os.environ.get('ProgramFiles', 'C:\\Program Files'), 'Git', 'bin', 'bash.exe'), os.path.join(os.environ.get('ProgramFiles(x86)', 'C:\\Program Files (x86)'), 'Git', 'bin', 'bash.exe'), os.path.join(_local_appdata, 'Programs', 'Git', 'bin', 'bash.exe') if _local_appdata else ''):
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning('HERMES_GIT_BASH_PATH=%s fails to start; using %s instead', custom, candidate)
            return candidate
    if candidates:
        probe_details = '\n'.join((detail for candidate in candidates if (detail := _bash_probe_details_cache.get(candidate))))
        ...
        return candidates[0]
    raise RuntimeError('Git Bash not found. Duck Agent requires Git for Windows on Windows.\nInstall it from: https://git-scm.com/download/win\nOr set HERMES_GIT_BASH_PATH to your bash.exe location.')
...
def _looks_like_msys_spawn_failure(details: str) -> bool:
    ...
    return any((marker in lowered for marker in ('dofork:', 'child_copy:', '0xc0000142', '0xc0000005')))
...
def _mandatory_aslr_enabled() -> 'bool | None':
    ...
    try:
        ...
        result = subprocess.run([powershell, '-NoProfile', '-NonInteractive', '-Command', '(Get-ProcessMitigation -System).Aslr.ForceRelocateImages.ToString()'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, creationflags=windows_hide_flags())
...
def _git_bash_aslr_help(bash: str, details: str='') -> str:
    ...
    return f"""Git Bash at {bash} cannot launch required MSYS child processes while Windows Mandatory ASLR (ForceRelocateImages) is enabled, or its output matches that Git-for-Windows failure class.{detail_line}\nReinstalling Git will not change the Windows mitigation policy. Open PowerShell as Administrator and run:\n$gitRoot = '{escaped_root}'\nGet-Item "$gitRoot\\bin\\bash.exe", "$gitRoot\\usr\\bin\\*.exe" -ErrorAction SilentlyContinue | ForEach-Object {{ Set-ProcessMitigation -Name $_.FullName -Disable ForceRelocateImages }}\nThen restart Duck Agent. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."""
...
def _bash_starts(bash: str) -> bool:
    ...
    if cached is not None:
        return cached
    try:
        result = subprocess.run([bash, '--noprofile', '--norc', '-c', _BASH_EXTERNAL_PROGRAM_PROBE], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=15, creationflags=windows_hide_flags() if _IS_WINDOWS else 0)
...
def _git_bash_bin_dirs() -> list[str]:
    ...
    for candidate in (os.path.join(root, 'mingw64', 'bin'), os.path.join(root, 'mingw32', 'bin'), os.path.join(root, 'usr', 'local', 'bin'), os.path.join(root, 'usr', 'bin'), os.path.join(root, 'bin')):
...
def _find_shell() -> str:
    ...
    if not _IS_WINDOWS:
        ...
        if user_shell and os.path.isfile(user_shell) and os.access(user_shell, os.X_OK) and (Path(user_shell).name in _SPAWN_COMPATIBLE_SHELLS):
            return user_shell
    return _find_bash()
...
def _resolve_hermes_bin_dir() -> str | None:
    """Return the directory holding the ``duck-agent`` console-script, or None.
    ...
    duck-agent install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``duck-agent``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``duck-agent`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``duck-agent`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("duck-agent")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``duck-agent`` executable (covers nix-store / venv wrappers).
    ...
    which = shutil.which('duck-agent')
    if which:
        candidate = os.path.dirname(which)
    if candidate is None:
        argv0 = sys.argv[0] if sys.argv else ''
        base = os.path.basename(argv0).lower()
        if os.path.isabs(argv0) and (base == 'duck-agent' or base.startswith('duck-agent.')) and os.path.isfile(argv0):
            candidate = os.path.dirname(argv0)
    if candidate is None:
        exe_dir = os.path.dirname(sys.executable) if sys.executable else ''
        if exe_dir:
            shim = 'duck-agent.exe' if _IS_WINDOWS else 'duck-agent'
...
def _prepend_hermes_bin_dir(existing_path: str) -> str:
    """Prepend the duck-agent install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Duck Agent-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Duck Agent installs for
    itself, so on a machine where Duck Agent provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$DUCK_AGENT_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$DUCK_AGENT_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Duck Agent-managed runtime dirs are appended alongside the sane entries, not
    ...
    sane_entries.extend((entry for entry in _managed_runtime_path_entries() if entry not in sane_entries))
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Duck Agent
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Duck Agent trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``DUCK_AGENT_HOME`` instead — single-word path, guaranteed to exist, same
    ...
    def _run_bash(self, cmd_string: str, *, login: bool=False, timeout: int=120, stdin_data: str | None=None) -> subprocess.Popen:
        ...
        if safe_cwd != self.cwd:
            ...
            if safe_cwd != normalized:
                logger.warning('LocalEnvironment cwd %r is missing on disk; falling back to %r so terminal commands keep working.', self.cwd, safe_cwd)
            self.cwd = safe_cwd
        _popen_cwd = self.cwd
        _popen_kwargs = {'creationflags': windows_hide_flags()} if _IS_WINDOWS else {}
        proc = subprocess.Popen(args, text=True, env=run_env, encoding='utf-8', errors='replace', stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE if stdin_data is not None else subprocess.DEVNULL, start_new_session=True, cwd=_popen_cwd, **_popen_kwargs)
        if not _IS_WINDOWS:
            try:
