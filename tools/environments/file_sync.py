...
def iter_sync_files(container_base: str='/root/.duck-agent') -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.duck-agent to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry['container_path'].replace('/root/.duck-agent', container_base, 1)
...
class FileSyncManager:
    ...
    def __init__(self, get_files_fn: GetFilesFn, upload_fn: UploadFn, delete_fn: DeleteFn, sync_interval: float=_SYNC_INTERVAL_SECONDS, bulk_upload_fn: BulkUploadFn | None=None, bulk_download_fn: BulkDownloadFn | None=None):
    ...
    def sync_back(self, hermes_home: Path | None=None) -> None:
        ...
        Downloads the remote ``.duck-agent/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix='.tar') as tf:
            ...
            if tar_size > _SYNC_BACK_MAX_BYTES:
                logger.warning('sync_back: remote tar is %d bytes (cap %d) — skipping extraction', tar_size, _SYNC_BACK_MAX_BYTES)
                return
            with tempfile.TemporaryDirectory(prefix='duck-agent-sync-back-') as staging:
                    ...
                    for fname in filenames:
                        ...
                        if os.path.exists(host_path) and pushed_hash is not None:
                            ...
                            if host_hash != pushed_hash:
                                logger.warning('sync_back: conflict on %s — host modified since push, remote also changed. Applying remote version (last-write-wins).', remote_path)
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None=None, *, upload_only_host_paths: set[str] | None=None) -> str | None:
        ...
        For example, if the mapping has ``/root/.duck-agent/skills/a.md`` →
        ``~/.duck-agent/skills/a.md``, a new remote file at
        ``/root/.duck-agent/skills/b.md`` maps to ``~/.duck-agent/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
