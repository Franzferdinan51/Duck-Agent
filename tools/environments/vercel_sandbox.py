...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Duck Agent'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``DUCK_AGENT_HOME`` and restores
...
def _is_transient_vercel_error(exc: BaseException) -> bool:
    for error in _exception_chain(exc):
        ...
        if status_code in _TRANSIENT_STATUS_CODES:
            return True
        if isinstance(error, (httpx.NetworkError, httpx.ProtocolError, httpx.ReadError)):
            return True
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def __init__(self, runtime: str | None=None, cwd: str=DEFAULT_VERCEL_CWD, timeout: int=60, cpu: float=1, memory: int=5120, disk: int=_DEFAULT_CONTAINER_DISK_MB, persistent_filesystem: bool=True, task_id: str='default'):
        requested_cwd = cwd
    ...
    def _build_create_params(self, *, cpu: float, memory: int, disk: int) -> _SandboxCreateParams:
        if disk not in {0, _DEFAULT_CONTAINER_DISK_MB}:
            raise ValueError('Vercel Sandbox does not support configurable container_disk. Use the default shared setting.')
        _ensure_vercel_sdk()
    ...
    def _create_sandbox(self) -> Sandbox:
        ...
        if snapshot_id:
            try:
                return _retry_vercel_call('sandbox restore', lambda: Sandbox.create(timeout=self._create_params.timeout, runtime=self._create_params.runtime, resources=self._create_params.resources, source={'type': 'snapshot', 'snapshot_id': snapshot_id}), attempts=_CREATE_RETRY_ATTEMPTS)
            except Exception as exc:
                logger.warning('Vercel: failed to restore snapshot %s for task %s; falling back to a fresh sandbox: %s', snapshot_id, self._task_id, exc)
                _delete_snapshot(self._task_id, snapshot_id)
        params = self._create_params
        return _retry_vercel_call('sandbox create', lambda: Sandbox.create(timeout=params.timeout, runtime=params.runtime, resources=params.resources), attempts=_CREATE_RETRY_ATTEMPTS)
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == '/':
            container_base = '/.duck-agent'
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.duck-agent"
        self._sync_manager = FileSyncManager(get_files_fn=lambda: iter_sync_files(container_base), upload_fn=self._vercel_upload, delete_fn=self._vercel_delete, bulk_upload_fn=self._vercel_bulk_upload, bulk_download_fn=self._vercel_bulk_download)
        if requested_cwd == '~':
    ...
    def _wait_for_running(self, timeout: timedelta=_RUNNING_WAIT_TIMEOUT) -> None:
        ...
        try:
            sandbox.wait_for_status(SandboxStatus.RUNNING, timeout=max(timeout, _MIN_RUNNING_WAIT), poll_interval=_RUNNING_WAIT_POLL_INTERVAL)
    ...
    def _vercel_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        if not files:
            return
        payload: list[WriteFile] = [{'path': remote_path, 'content': Path(host_path).read_bytes()} for host_path, remote_path in files]
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError('Vercel sandbox is not attached')
    ...
    def _vercel_delete(self, remote_paths: list[str]) -> None:
        ...
        if sandbox is None:
            raise RuntimeError('Vercel sandbox is not attached')
        result = sandbox.run_command('bash', ['-lc', quoted_rm_command(remote_paths)], cwd=self._workspace_root)
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = '/.duck-agent' if self._remote_home == '/' else f"{self._remote_home.rstrip('/')}/.duck-agent"
        ...
        if sandbox is None:
            raise RuntimeError('Vercel sandbox is not attached')
        try:
            result = sandbox.run_command('bash', ['-lc', f'tar cf {shlex.quote(remote_tar)} -C / {shlex.quote(archive_member)}'], cwd=self._workspace_root)
            if _extract_result_returncode(result) != 0:
