...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str='~', timeout: int=60, port: int=22, key_path: str=''):
        ...
        self.control_dir = Path(tempfile.gettempdir()) / 'duck-agent-ssh'
        ...
        self._sync_manager = FileSyncManager(get_files_fn=lambda: iter_sync_files(f'{self._remote_home}/.duck-agent'), upload_fn=self._scp_upload, delete_fn=self._ssh_delete, bulk_upload_fn=self._ssh_bulk_upload, bulk_download_fn=self._ssh_bulk_download)
    ...
    def _establish_connection(self):
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=15, stdin=subprocess.DEVNULL)
            if result.returncode != 0:
                error_msg = result.stderr.strip() or result.stdout.strip()
                raise EnvironmentConnectionError(f'SSH connection failed: {error_msg}', retry_hint=f'Verify {self.user}@{self.host}:{self.port} is reachable (host up, sshd running, key/agent auth working), then retry — the connection is re-established automatically.')
        except subprocess.TimeoutExpired:
            raise EnvironmentConnectionError(f'SSH connection to {self.user}@{self.host} timed out', retry_hint=f'Check network connectivity to {self.host}:{self.port} and that sshd is accepting connections, then retry.')
    ...
    def _detect_remote_home(self) -> str:
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, stdin=subprocess.DEVNULL)
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.duck-agent directory tree on remote in one SSH call."""
        base = f'{self._remote_home}/.duck-agent'
        dirs = [base, f'{base}/skills', f'{base}/credentials', f'{base}/cache']
        cmd = self._build_ssh_command()
        cmd.append(quoted_mkdir_command(dirs))
        subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, stdin=subprocess.DEVNULL)
    ...
    def _scp_upload(self, host_path: str, remote_path: str) -> None:
        ...
        subprocess.run(mkdir_cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, stdin=subprocess.DEVNULL)
        ...
        result = subprocess.run(scp_cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, stdin=subprocess.DEVNULL)
        if result.returncode != 0:
            raise EnvironmentConnectionError(f'scp failed: {result.stderr.strip()}', retry_hint=f'File sync to {self.user}@{self.host} failed — verify the SSH connection is healthy, then retry.')
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        if parents:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, stdin=subprocess.DEVNULL)
            if result.returncode != 0:
                raise EnvironmentConnectionError(f'remote mkdir failed: {result.stderr.strip()}', retry_hint=f'Remote directory setup on {self.host} failed — verify the SSH connection is healthy, then retry.')
        with tempfile.TemporaryDirectory(prefix='duck-agent-ssh-bulk-') as staging:
            ...
            try:
                ...
                tar_proc.wait()
                ssh_proc.wait()
                raise EnvironmentConnectionError('SSH bulk upload timed out', retry_hint=f'Bulk file sync to {self.host} timed out — check the connection and retry.')
            if tar_proc.returncode != 0:
                raise RuntimeError(f"tar create failed (rc={tar_proc.returncode}): {tar_stderr_raw.decode(errors='replace').strip()}")
            if ssh_proc.returncode != 0:
                raise EnvironmentConnectionError(f"tar extract over SSH failed (rc={ssh_proc.returncode}): {ssh_stderr.decode(errors='replace').strip()}", retry_hint=f'File sync over SSH to {self.host} failed — verify the connection is healthy, then retry.')
        logger.debug('SSH: bulk-uploaded %d file(s) via tar pipe', len(files))

    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .duck-agent/ as a tar archive."""
        rel_base = f'{self._remote_home}/.duck-agent'.lstrip('/')
        ssh_cmd = self._build_ssh_command()
        ...
        if result.returncode != 0:
            raise EnvironmentConnectionError(f"SSH bulk download failed: {result.stderr.decode(errors='replace').strip()}", retry_hint=f'File sync from {self.host} failed — verify the SSH connection is healthy, then retry.')
    ...
    def _ssh_delete(self, remote_paths: list[str]) -> None:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, stdin=subprocess.DEVNULL)
        if result.returncode != 0:
            raise EnvironmentConnectionError(f'remote rm failed: {result.stderr.strip()}', retry_hint=f'Remote file cleanup on {self.host} failed — verify the SSH connection is healthy, then retry.')
