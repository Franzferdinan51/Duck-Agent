...
def _find_singularity_executable() -> str:
    ...
    if shutil.which('singularity'):
        return 'singularity'
    raise RuntimeError("Neither 'apptainer' nor 'singularity' was found in PATH. Install Apptainer (https://apptainer.org/docs/admin/main/installation.html) or Singularity and ensure the CLI is available.")
...
def _ensure_singularity_available() -> str:
    ...
    try:
        result = subprocess.run([exe, 'version'], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10, stdin=subprocess.DEVNULL)
...
def _get_scratch_dir() -> Path:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv('USER', 'duck-agent') / 'duck-agent'
...
def _get_or_build_sif(image: str, executable: str='apptainer') -> str:
    ...
    with _sif_build_lock:
        ...
        try:
            result = subprocess.run([executable, 'build', str(sif_path), image], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=600, env=env, stdin=subprocess.DEVNULL)
            if result.returncode != 0:
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str='~', timeout: int=60, cpu: float=0, memory: int=0, disk: int=0, persistent_filesystem: bool=False, task_id: str='default'):
        ...
        self._memory = memory
        if self._persistent:
            overlay_base = _get_scratch_dir() / 'duck-agent-overlays'
    ...
    def cleanup(self):
        ...
        if self._instance_started:
            try:
                subprocess.run([self.executable, 'instance', 'stop', self.instance_id], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, stdin=subprocess.DEVNULL)
