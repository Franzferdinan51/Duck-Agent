...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    """Gateway-owned Modal sandbox with Duck Agent-compatible execute/cleanup."""
    ...
    def __init__(self, image: str, cwd: str='/root', timeout: int=60, modal_sandbox_kwargs: Optional[Dict[str, Any]]=None, persistent_filesystem: bool=True, task_id: str='default'):
    ...
    def _start_modal_exec(self, prepared: PreparedModalExec) -> ModalExecStart:
        ...
        payload: Dict[str, Any] = {'execId': exec_id, 'command': prepared.command, 'cwd': prepared.cwd, 'timeoutMs': int(prepared.timeout * 1000)}
        ...
        if response.status_code >= 400:
            return ModalExecStart(immediate_result=self._error_result(self._format_error('Managed Modal exec failed', response)))
        body = response.json()
        status = body.get('status')
        if status in {'completed', 'failed', 'cancelled', 'timeout'}:
            return ModalExecStart(immediate_result=self._result(body.get('output', ''), body.get('returncode', 1)))
        if body.get('execId') != exec_id:
            return ModalExecStart(immediate_result=self._error_result('Managed Modal exec start did not return the expected exec id'))
    ...
    def _poll_modal_exec(self, handle: _ManagedModalExecHandle) -> dict | None:
        try:
            status_response = self._request('GET', f'/v1/sandboxes/{self._sandbox_id}/execs/{handle.exec_id}', timeout=(self._CONNECT_TIMEOUT_SECONDS, self._POLL_READ_TIMEOUT_SECONDS))
        except Exception as exc:
        ...
        if status in {'completed', 'failed', 'cancelled', 'timeout'}:
            return self._result(status_body.get('output', ''), status_body.get('returncode', 1))
        return None
    ...
    def cleanup(self):
        if not getattr(self, '_sandbox_id', None):
            return
        try:
            self._request('POST', f'/v1/sandboxes/{self._sandbox_id}/terminate', json={'snapshotBeforeTerminate': self._persistent}, timeout=60)
        except Exception as exc:
    ...
    def _create_sandbox(self) -> str:
        ...
        create_payload = {'image': self._image, 'cwd': self.cwd, 'cpu': cpu, 'memoryMiB': memory, 'timeoutMs': 3600000, 'idleTimeoutMs': max(300000, int(self.timeout * 1000)), 'persistentFilesystem': self._persistent, 'logicalKey': self._task_id}
        if disk is not None:
            create_payload['diskMiB'] = disk
        response = self._request('POST', '/v1/sandboxes', json=create_payload, timeout=60, extra_headers={'x-idempotency-key': self._create_idempotency_key})
        if response.status_code >= 400:
    ...
    def _guard_unsupported_credential_passthrough(self) -> None:
        ...
        if mounts:
            raise ValueError('Managed Modal does not support host credential-file passthrough. Use TERMINAL_MODAL_MODE=direct when skills or config require credential files inside the sandbox.')

    def _request(self, method: str, path: str, *, json: Dict[str, Any] | None=None, timeout: int=30, extra_headers: Dict[str, str] | None=None) -> requests.Response:
    ...
    def _cancel_exec(self, exec_id: str) -> None:
        try:
            self._request('POST', f'/v1/sandboxes/{self._sandbox_id}/execs/{exec_id}/cancel', timeout=(self._CONNECT_TIMEOUT_SECONDS, self._CANCEL_READ_TIMEOUT_SECONDS))
        except Exception as exc:
