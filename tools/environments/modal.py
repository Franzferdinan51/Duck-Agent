...
wrapper, while preserving Duck Agent' persistent snapshot behavior across sessions.
"""
import asyncio
import base64
import io
...
def _resolve_modal_image(image_spec: Any) -> Any:
    ...
    setup_commands = ['RUN rm -rf /usr/local/lib/python*/site-packages/pip* 2>/dev/null; python -m ensurepip --upgrade --default-pip 2>/dev/null || true']
    if add_python:
        setup_commands.insert(0, 'RUN apt-get update -qq && apt-get install -y -qq python3 python3-venv > /dev/null 2>&1 || true')
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str='/root', timeout: int=60, modal_sandbox_kwargs: Optional[dict[str, Any]]=None, persistent_filesystem: bool=True, task_id: str='default'):
        ...
        try:
            ...
            for mount_entry in get_credential_file_mounts():
                cred_mounts.append(_modal.Mount.from_local_file(mount_entry['host_path'], remote_path=mount_entry['container_path']))
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio('duck-agent', create_if_missing=True)
            ...
            sandbox = await _modal.Sandbox.create.aio('sleep', 'infinity', image=image_spec, app=app, timeout=int(create_kwargs.pop('timeout', 3600)), **create_kwargs)
            return (app, sandbox)
        try:
            target_image_spec = restored_snapshot_id or image
            try:
                effective_image = _resolve_modal_image(target_image_spec)
                self._app, self._sandbox = self._worker.run_coroutine(_create_sandbox(effective_image), timeout=300)
            except Exception as exc:
                if not restored_snapshot_id:
                    raise
                logger.warning('Modal: failed to restore snapshot %s, retrying with base image: %s', restored_snapshot_id[:20], exc)
                _delete_direct_snapshot(self._task_id, restored_snapshot_id)
                base_image = _resolve_modal_image(image)
                self._app, self._sandbox = self._worker.run_coroutine(_create_sandbox(base_image), timeout=300)
            else:
        ...
        self._sync_manager = FileSyncManager(get_files_fn=lambda: iter_sync_files('/root/.duck-agent'), upload_fn=self._modal_upload, delete_fn=self._modal_delete, bulk_upload_fn=self._modal_bulk_upload, bulk_download_fn=self._modal_bulk_download)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.duck-agent is hardcoded
        (consistent with iter_sync_files call on line 269).
        """

        async def _download():
            proc = await self._sandbox.exec.aio('bash', '-c', 'tar cf - -C / root/.duck-agent')
