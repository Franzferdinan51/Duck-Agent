...
_CDP_PRIVATE_PAGE_ALLOWED_METHODS = {'Browser.getVersion', 'Target.getTargets', 'Target.attachToTarget', 'Target.detachFromTarget', 'Page.navigate', 'Page.reload', 'Page.stopLoading'}
...
def _private_page_guard_error(blocked_url: str, method: str) -> str:
    return tool_error(f'Blocked: page URL targets a private or internal address ({blocked_url}). Raw CDP method {method!r} could expose private page content or state.', method=method, cdp_docs=CDP_DOCS_URL)
...
def _browser_cdp_private_guard(*, task_id: str, method: str, params: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        if not bt._eval_ssrf_guard_active(task_id):
            return None
        if method == 'Page.navigate':
            ...
            if target_url and (bt._is_always_blocked_url(target_url) or not bt._is_safe_url(target_url)):
                return tool_error(f'Blocked: CDP Page.navigate target is a private or internal address ({target_url}).', method=method, cdp_docs=CDP_DOCS_URL)
        if method == 'Runtime.evaluate':
            expression = str((params or {}).get('expression') or '')
            blocked_literal = bt._expression_targets_private_url(expression)
            if blocked_literal:
                return tool_error(f'Blocked: CDP Runtime.evaluate expression targets a private or internal address ({blocked_literal}).', method=method, cdp_docs=CDP_DOCS_URL)
        if method not in _CDP_PRIVATE_PAGE_ALLOWED_METHODS:
            blocked_url = bt._current_page_private_url(task_id)
            if blocked_url:
...
async def _cdp_call(ws_url: str, method: str, params: Dict[str, Any], target_id: Optional[str], timeout: float) -> Dict[str, Any]:
...
def _browser_cdp_via_supervisor(task_id: str, frame_id: str, method: str, params: Optional[Dict[str, Any]], timeout: float) -> str:
    ...
    try:
        ...
        return tool_error(f'CDP supervisor is not available: {exc}. frame_id routing requires a running supervisor attached via /browser connect or an active Browserbase session.')
    supervisor = SUPERVISOR_REGISTRY.get(task_id)
    if supervisor is None:
        return tool_error(f'No CDP supervisor is attached for task={task_id!r}. Call browser_navigate or /browser connect first so the supervisor can attach. Once attached, browser_snapshot will populate frame_tree with frame_ids you can pass here.')
    ...
    if frame_info is None:
        return tool_error(f'frame_id {frame_id!r} not found in supervisor state. Call browser_snapshot to see current frame_tree.')
    child_sid = frame_info.get('session_id')
    if not child_sid:
        return tool_error(f"""frame_id {frame_id!r} is not an out-of-process iframe (no dedicated CDP session). For same-origin iframes, use `browser_cdp(method='Runtime.evaluate', params={{'expression': "document.querySelector('iframe').contentDocument.title"}})` at the top-level page instead.""")
    loop = supervisor._loop
    if loop is None or not loop.is_running():
        return tool_error('CDP supervisor loop is not running. Try reconnecting with /browser connect.')

    async def _do_cdp():
        return await supervisor._cdp(method, params or {}, session_id=child_sid, timeout=timeout)
    try:
    ...
    payload: Dict[str, Any] = {'success': True, 'method': method, 'frame_id': frame_id, 'session_id': child_sid, 'result': result_msg.get('result', {})}
...
def browser_cdp(method: str, params: Optional[Dict[str, Any]]=None, target_id: Optional[str]=None, frame_id: Optional[str]=None, timeout: float=30.0, task_id: Optional[str]=None) -> str:
    ...
    if frame_id:
        ...
        if blocked:
            return blocked
        return _browser_cdp_via_supervisor(task_id=effective_task_id, frame_id=frame_id, method=method, params=params, timeout=timeout)
    if not method or not isinstance(method, str):
        return tool_error("'method' is required (e.g. 'Target.getTargets')", cdp_docs=CDP_DOCS_URL)
    if not _WS_AVAILABLE:
        return tool_error("The 'websockets' Python package is required but not installed. Install it with: pip install websockets")
    endpoint = _resolve_cdp_endpoint()
    if not endpoint:
        return tool_error("No CDP endpoint is available. Run '/browser connect' to attach to a running Chrome, Brave, Chromium, or Edge browser, or set 'browser.cdp_url' in config.yaml. The Camofox backend is REST-only and does not expose CDP.", cdp_docs=CDP_DOCS_URL)
    if not endpoint.startswith(('ws://', 'wss://')):
        return tool_error(f'CDP endpoint is not a WebSocket URL: {endpoint!r}. Expected ws://... or wss://... — the /browser connect resolver should have rewritten this. Check that a Chromium-family browser is actually listening on the debug port.')
...
def _browser_cdp_check() -> bool:
    ...
    """
    try:
        from tools.browser_tool import _get_cdp_override_raw, check_browser_requirements
...
registry.register(name='browser_cdp', toolset='browser-cdp', schema=BROWSER_CDP_SCHEMA, handler=lambda args, **kw: browser_cdp(method=args.get('method', ''), params=args.get('params'), target_id=args.get('target_id'), frame_id=args.get('frame_id'), timeout=args.get('timeout', 30.0), task_id=kw.get('task_id')), check_fn=_browser_cdp_check, emoji='🧪')