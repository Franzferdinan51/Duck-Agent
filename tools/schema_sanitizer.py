...
_PROP_KEY_RE = re.compile('^[a-zA-Z0-9_.-]{1,64}$')
_PROP_KEY_BAD_CHARS = re.compile('[^a-zA-Z0-9_.-]')
...
def _rename_property_keys(props: dict, path: str) -> dict[str, str]:
    ...
    for key in props:
        ...
        logger.debug('schema_sanitizer[%s]: renamed property key %r -> %r (provider key-pattern compat)', path, key, candidate)
    return renames
    ...
    for key, value in args.items():
        ...
        if isinstance(subschema, dict):
            if isinstance(value, dict):
                ...
                value = [unrename_tool_args(subschema['items'], item) if isinstance(item, dict) else item for item in value]
        out[orig] = value
    return out
...
def _strip_top_level_combinators(params: dict, *, path: str='<tool>') -> dict:
    ...
    for key in _TOP_LEVEL_FORBIDDEN_KEYS:
        if key in out:
            logger.debug('schema_sanitizer[%s]: stripped top-level %r combinator from tool parameters (strict-backend compat)', path, key)
            out.pop(key, None)
    return out
...
def collapse_const_unions(schema: Any) -> Any:
    ...
    for key in ('anyOf', 'oneOf'):
        ...
        if not isinstance(variants, list) or not variants:
            continue
        null_branches = [item for item in variants if isinstance(item, dict) and item.get('type') == 'null' and ('const' not in item)]
