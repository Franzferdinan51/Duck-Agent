...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``duck-agent auth add
    ...
    2. The environment / ``~/.duck-agent/.env``. Under a multiplexed gateway turn
    ...
    3. The credential pool / auth store for ``provider_id`` (``duck-agent auth
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``duck-agent auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.duck-agent/.env`` (via
