...
def _probe_gateway(runtime_status: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(platforms, dict):
        ...
        connected = sum((1 for value in platforms.values() if isinstance(value, dict) and str(value.get('state') or value.get('status') or '').lower() in {'connected', 'running', 'ok'}))
...
def collect_runtime_readiness(*, configured_model: str, runtime_status: dict[str, Any] | None, active_api_runs: int=0, process_completion_queue_depth: int=0, active_delegations: int=0) -> dict[str, Any]:
    ...
    checks = {'state_db': _probe_state_db(home), 'config': _probe_config(home), 'model': _check('ok' if str(configured_model or '').strip() else 'degraded'), 'disk': _probe_disk(home), 'gateway': _probe_gateway(runtime), 'background_queues': _check('ok', active_api_runs=max(0, int(active_api_runs)), process_completions=max(0, int(process_completion_queue_depth)), active_delegations=max(0, int(active_delegations)))}
